...
# Reuse the challenge's configuration and the axiom-footprint parser used for
# functional correctness proofs.
: "${submission_checker_script:?submission_checker_script must name a checker}"
: "${challenge_slug:?challenge_slug must name the challenge for temp files}"
: "${marker_id:?marker_id must name the generated README section}"
: "${report_script_path:?report_script_path must name the public report command}"
: "${vector_count:?vector_count must name the number of scorer vectors}"
: "${expected_rows:?expected_rows must name twice the vector count}"
: "${precompile_base:?precompile_base must name the fixed precompile gas}"
: "${precompile_per_word:?precompile_per_word must name the per-word gas}"
: "${precompile_name:?precompile_name must name the precompile schedule}"
source "$submission_checker_script"
  ...
  while IFS=, read -r vector bytes state status gas; do
    ...
    case "$state" in
      clean)
        ...
        precompile_total=$((precompile_total + precompile_base +
          precompile_per_word * ((bytes + 31) / 32)))
        ;;
      dirty)
        dirty["$vector"]="$gas"
...
main() {
  ...
  printf '%s\n' \
    ...
    "dirty\` is shown otherwise. The total is the sum across all $vector_count vectors," \
    'all of which the script checks. Ratios compare those suite totals against' \
    "the $precompile_name precompile schedule, \`$precompile_base + $precompile_per_word × ceil(bytes / 32)\`, and" \
