...
source scripts/check-ripemd160-submissions.sh
...
declare -a artifact_paths=("Challenge/Ripemd160/Reference/reference.hex")
declare -a gas_files=("Challenge/Ripemd160/Reference/Proofs/Gas.lean")
declare -a gas_modules=("Challenge.Ripemd160.Reference.Proofs.Gas")
declare -a bytecode_names=("Challenge.Ripemd160.referenceBytecode")
declare -a formula_names=("Challenge.Ripemd160.Reference.Proofs.Gas.gasFormula")
declare -a schedule_names=("Challenge.Ripemd160.Reference.Proofs.Gas.gasSchedule")
declare -a schedule_theorems=("Challenge.Ripemd160.Reference.Proofs.Gas.gasSchedule_correct")
...
report_measured_row() {
  ...
  if ! csv="$(lake exe ripemd160challenge --hex="$path" --csv)"; then
  ...
  while IFS=, read -r vector bytes state status gas; do
    ...
    case "$state" in
      clean)
        ...
        precompile_total=$((precompile_total + 600 + 120 * ((bytes + 31) / 32)))
        ;;
      dirty)
        dirty["$vector"]="$gas"
  ...
  if [[ "$rows" -ne 34 ]]; then
    printf 'error: expected 34 Tier-1 rows for %s, got %s\n' "$name" "$rows" >&2
    return 1
  fi
...
report_proved_row() {
  ...
  proof_check="$(mktemp /tmp/ripemd160-gas-proof.XXXXXX.lean)"
  eval_check="$(mktemp /tmp/ripemd160-gas-eval.XXXXXX.lean)"
  value_check="$(mktemp /tmp/ripemd160-gas-values.XXXXXX.lean)"
  trap 'cleanup_files "$proof_check" "$eval_check" "$value_check"; trap - RETURN' RETURN

  {
    printf 'import %s\n' "$gas_module"
    printf 'import Challenge.Ripemd160.AdditionalGoals.GasSchedule\n\n'
    printf 'example : Challenge.Ripemd160.CorrectWithSchedule %s %s := %s\n\n' \
...
run_axiom_self_test() {
  ...
  mkdir -p -- "$fixture_dir"
  {
    printf 'import Challenge.Ripemd160.Reference.Bytecode\n\n'
    printf 'namespace %s.%s\n\n' "$module_prefix" "$name"
    printf 'def bytecode : ByteArray := Challenge.Ripemd160.referenceBytecode\n\n'
    printf 'end %s.%s\n' "$module_prefix" "$name"
  } > "$fixture_dir/Bytecode.lean"
  {
    ...
    printf 'import Challenge.Ripemd160.AdditionalGoals.GasSchedule\n\n'
    printf 'namespace %s.%s\n\n' "$module_prefix" "$name"
    printf 'def gasFormula : Challenge.Ripemd160.GasFormula := 0\n'
    printf 'def gasSchedule : Nat → Nat := gasFormula.eval\n\n'
    printf 'axiom gasSchedule_correct :\n'
    printf '  Challenge.Ripemd160.CorrectWithSchedule bytecode gasSchedule\n\n'
    printf 'end %s.%s\n' "$module_prefix" "$name"
...
main() {
  ...
  printf '<!-- BEGIN GENERATED RIPEMD160 GAS REPORT -->\n'
  printf '## 6. Gas report\n\n'
  printf 'Generated by `scripts/report-ripemd160-gas.sh`; CI checks that this section is current.\n\n'
  ...
  printf '\n'
  printf '<!-- END GENERATED RIPEMD160 GAS REPORT -->\n'
}
