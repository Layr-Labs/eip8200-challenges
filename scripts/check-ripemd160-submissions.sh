...
readonly submission_root="Challenge/Ripemd160/Submissions"
readonly module_prefix="Challenge.Ripemd160.Submissions"
...
check_submission() {
...

  {
    ...
    printf 'example : Challenge.Ripemd160.Correct %s.bytecode := %s.correct\n\n' "$module" "$module"
  ...
  if ! lake exe ripemd160challenge --hex="$submission_dir/bytecode.hex"; then
    printf 'error: executable RIPEMD-160 checks failed for %s\n' "$name" >&2
    return 1
  fi
...
run_reference_control() {
  ...
  cp Challenge/Ripemd160/Reference/reference.hex "$self_test_dir/bytecode.hex"
  ...
  } > "$self_test_dir/Bytecode.lean"
  {
    ...
    printf 'import Challenge.Ripemd160.Reference.Proofs.Bytecode.ReferenceCorrect\n\n'
    printf 'namespace %s.%s\n\n' "$module_prefix" "$name"
    printf 'theorem correct : Challenge.Ripemd160.Correct bytecode := by\n'
    printf '  simpa [bytecode] using\n'
    printf '    Challenge.Ripemd160.Reference.Proofs.Bytecode.ReferenceCorrect.reference_correct\n\n'
    printf 'end %s.%s\n' "$module_prefix" "$name"
...
run_axiom_control() {
  ...
  {
    printf 'import %s.%s.Bytecode\n' "$module_prefix" "$name"
    printf 'import Challenge.Ripemd160.Spec\n\n'
    printf 'namespace %s.%s\n\n' "$module_prefix" "$name"
    printf 'axiom correct : Challenge.Ripemd160.Correct bytecode\n\n'
    printf 'end %s.%s\n' "$module_prefix" "$name"
  } > "$self_test_dir/Proof.lean"
...
main() {
  ...
  if [[ "$found" == false ]]; then
    printf 'no RIPEMD-160 candidate directories found\n'
  fi
}
