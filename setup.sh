...
case "${track}" in
  ...
  ripemd160)
    ...
    ;;
  blake2f)
    readonly target_module="Challenge.Blake2f.Benchmark.Target"
    readonly reference_module="Challenge.Blake2f.Reference.Proofs.Bytecode.ReferenceCorrect"
    readonly scorer_module="Blake2fMain"
    readonly scorer_exe="blake2fchallenge"
    ;;
  *)
    echo "usage: $0 {sha256|modexp|ripemd160|blake2f}" >&2
    exit 64
    ;;
esac
