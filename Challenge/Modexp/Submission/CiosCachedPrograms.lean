...
/-- `macFusedPostProgram` with the incoming-carry `DUP4; ADD` of zero replaced by two `JUMPDEST`s:
same byte length, three gas cheaper per head, equivalent only when incoming carry is already zero. -/
