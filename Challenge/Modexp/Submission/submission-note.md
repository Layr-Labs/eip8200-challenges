...
## Inline-MCOPY select (v4)

The one-mask select branch of `addMaskedMod` called the `copyLimbs`
subroutine (`JUMP` to pc 58, loop, return to 1455). It is replaced by ten
inline instructions at pc 1442-1454:

```
JUMPDEST DUP5 PUSH2 0x1400 SWAP1 DUP11 PUSH1 0x20 MUL SWAP2 SWAP1 MCOPY
```

which compute `32*count` on the stack and invoke the Osaka `MCOPY` opcode
(`0x5e`) to move the candidate array from `0x1400` to `dst`, landing at the
existing epilogue `JUMPDEST` at pc 1455. The artifact stays **1,567 bytes**
and grows to **1,158 instructions**. Per-call cost on the C branch drops from
~370 gas (subroutine call + per-limb loop) to the `MCOPY` base + expansion +
`3*count` words.

Proof: `Proofs/Mcopy.lean` (transferred from the public `75ec39b` result,
which independently proved the same opcode) supplies `represents_mcopy` /
`represents_mcopy_disjoint_region`. `maskChoice`'s one-branch and
`maskCopied` are stated natively in `MCOPY` terms (`mcopyMemory`,
`mcopyWords`); the `run` proof closes definitionally over the frozen bytes
with only the existing `count < 2^256` hypothesis, and the `Represents`
proofs discharge the word-level size reduction with the ambient fits. The
`copyLimbs` subroutine itself is untouched (still used by multiplication and
exponent paths).

Local insecure run: Lean default kernel accepts the solution,
13/13 correctness vectors, verified aggregate **133,817,639**
(RSA-2048: 128,278,919). Ranked Linux CI remains authoritative.
