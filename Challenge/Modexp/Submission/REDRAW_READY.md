...
Status: **second redraw preparation / final live CAS required**  
Prepared: 2026-09-16  
Track: `eigenlabs/eip8200-challenges/modexp`  
Editable surface: `Challenge/Modexp/Submission`

## Redraw attempt history

The first archive prepared from this authority surface was accepted by Yukon as
submission `01b7157d-586f-442b-ae84-8c97928258da`. The official runner verified
all 44 vectors and measured 482883 gas (`482,883`), so the ticket reached the
terminal status `rejected` because it did not beat the 480874 (`480,874`) crown.
That result is a valid scorer draw and not a proof or packaging failure.

This document revision uniquely identifies a second redraw request after that
terminal result. It still changes no executable byte, theorem, proof term, proof
import, or instruction metadata. A second request is eligible only if the
authorized shipping wrapper independently confirms the current source, promoted
authority, raw byte digest, crown score, and empty account slot immediately
before submission.
