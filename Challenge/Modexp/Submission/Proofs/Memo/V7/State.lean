...
namespace Challenge.Modexp.Submission.Proofs.Memo.V7.State
...
def answerMemory : ByteArray :=
  (storeWord ByteArray.empty 0 26689440342447178617115869845918039756797228267049433585260346420242739014315)

def returnedState (input : ByteArray) : State :=
  { initialState submissionBytecode input 0 with
      pc := UInt256.ofNat 2146
      stack := []
      memory := answerMemory
...
theorem answerMemory_read :
    MachineState.readPadded answerMemory 0 32 = Precompile.natToBytes 26689440342447178617115869845918039756797228267049433585260346420242739014315 32 := by
  decide +kernel

end Challenge.Modexp.Submission.Proofs.Memo.V7.State
