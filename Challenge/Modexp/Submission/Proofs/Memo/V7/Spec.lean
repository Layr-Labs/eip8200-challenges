import Challenge.Modexp.Submission.Proofs.Memo.V7.Data
...
namespace Challenge.Modexp.Submission.Proofs.Memo.V7.Spec
...
theorem sizes {input : ByteArray} (hvalid : ValidInput input) (hm : WordsMatch checks input) :
    baseSize input = 1 ∧ exponentSize input = 2 ∧ modulusSize input = 32 := by
  refine ⟨?_, ?_, ?_⟩
  · show Precompile.bytesToNatPadded input 0 32 = 1
    ...
    decide +kernel
  · show Precompile.bytesToNatPadded input 32 32 = 2
...
theorem exp_eq {input : ByteArray} (hm : WordsMatch checks input) :
    Precompile.bytesToNatPadded input 97 2 = 65535 := by
  rw [Cover.bytesToNatPadded_eq_of_cover input target checks 97 2 hm checks_ok (by decide +kernel)]
  decide +kernel

theorem mod_eq {input : ByteArray} (hm : WordsMatch checks input) :
    Precompile.bytesToNatPadded input 99 32 = 57896044618658097711785492504343953926634992332820282019728792003956564819968 := by
  rw [Cover.bytesToNatPadded_eq_of_cover input target checks 99 32 hm checks_ok (by decide +kernel)]
  decide +kernel

theorem cert : Precompile.modPow 3 65535 57896044618658097711785492504343953926634992332820282019728792003956564819968 = 26689440342447178617115869845918039756797228267049433585260346420242739014315 := by
  rw [Challenge.Modexp.Submission.Proofs.Algorithm.modPow_eq, if_neg (by norm_num)]
  have h : ((3 : ℕ) : ZMod 57896044618658097711785492504343953926634992332820282019728792003956564819968) ^ 65535 = ((26689440342447178617115869845918039756797228267049433585260346420242739014315 : ℕ) : ZMod 57896044618658097711785492504343953926634992332820282019728792003956564819968) := by
    reduce_mod_char
...
theorem spec_eq {input : ByteArray} (hvalid : ValidInput input) (hm : WordsMatch checks input) :
    spec input = Precompile.natToBytes 26689440342447178617115869845918039756797228267049433585260346420242739014315 32 := by
  obtain ⟨hb, he, hmm⟩ := sizes hvalid hm
  simp only [spec, hb, he, hmm, base_eq hm, exp_eq hm, mod_eq hm, cert]
  rw [if_neg (by decide)]

end Challenge.Modexp.Submission.Proofs.Memo.V7.Spec
