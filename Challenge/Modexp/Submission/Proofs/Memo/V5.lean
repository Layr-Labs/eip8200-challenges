import Challenge.Modexp.Submission.Proofs.Memo.V5.Trace
import Challenge.Modexp.Submission.Proofs.Memo.V5.Spec
...
namespace Challenge.Modexp.Submission.Proofs.Memo.V5
...
def gasSteps_match (input : ByteArray) (h : guardDiff Data.checks input = 0) :
    Challenge.EvmProof.GasSteps (Main.trampolineState input 1728) (returnedState input) :=
...
def gasSteps_fallback (input : ByteArray) (h : guardDiff Data.checks input ≠ 0) :
    Challenge.EvmProof.GasSteps (Main.trampolineState input 1728) (Main.trampolineState input 1196) :=
  ((((sound preludePath rfl (run_prelude input) rfl rfl deployAddress_not_precompile).trans
    (sound chunk0Path rfl (run_chunk0 input) rfl rfl deployAddress_not_precompile)).trans
...
end Challenge.Modexp.Submission.Proofs.Memo.V5
