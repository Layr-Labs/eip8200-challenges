import Challenge.Modexp.Submission.Proofs.Memo.V10.State
import Challenge.Modexp.Submission.Proofs.Memo.V10.Paths
...
namespace Challenge.Modexp.Submission.Proofs.Memo.V10.Trace
...
theorem run_prelude (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock preludePath (Main.trampolineState input 2656) =
      some (accState input 2662 (acc0 input)) := by
  ...
  simp [preludePath, accState, acc0, Main.opAt, Main.pushAt, Main.wfOp,
    ...
    initialState, Main.trampolineState, PCs.pc15, PCs.pc16,
...
theorem run_chunk0 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock chunk0Path (accState input 2662 (acc0 input)) =
      some (accState input 2752 (acc1 input)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
theorem run_chunk1 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock chunk1Path (accState input 2752 (acc1 input)) =
      some (accState input 2790 (acc2 input)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
def branchJumpState (input : ByteArray) : State :=
  { initialState submissionBytecode input 0 with
      pc := UInt256.ofNat 2794
      stack := [UInt256.ofNat 2799, UInt256.ofNat 1] }
...
theorem run_branch_jump (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocated branch_jumpLocated (branchJumpState input) =
      some (Main.trampolineState input 2799) := by
  have hjump : Decode.isValidJumpDest submissionBytecode 2799 = true :=
    Artifact.isValidJumpDest_index 1560 (by rfl)
  have hpc : (branchJumpState input).pc.toNat = Artifact.submissionArtifact.instructionPC 1557 := by
    simp [branchJumpState, initialState, PCs.pc16, Challenge.EvmProof.Word.word_toNat_ofNat]
  exact (Step.runLocated_jumpi_taken branch_jumpLocated rfl (branchJumpState input) 2799 (UInt256.ofNat 1) [] hpc rfl (by simp) Logic.isTrue_one rfl (by norm_num) hjump).trans rfl

theorem run_branch_match_prefix (input : ByteArray)
    (hw : MachineState.readWord input 96 =
      UInt256.ofNat 73247641362558725300106169323372519318985509881989093824173738694050148637181)
    (h : guardDiff Data.checks input = 0) :
    Challenge.EvmProof.Stepper.runLocatedBlock branchPrefixPath (accState input 2790 (acc2 input)) =
      some (branchJumpState input) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
  have hz : UInt256.isZero (acc2 input) = UInt256.ofNat 1 := by
    rw [acc2_eq_guardDiff input hw]; exact Logic.isZero_of_eq _ h
...
theorem run_branch_mismatch (input : ByteArray)
    (hw : MachineState.readWord input 96 =
      UInt256.ofNat 73247641362558725300106169323372519318985509881989093824173738694050148637181)
    (h : guardDiff Data.checks input ≠ 0) :
    Challenge.EvmProof.Stepper.runLocatedBlock branchPath (accState input 2790 (acc2 input)) =
      some (Main.trampolineState input 2795) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
  have hz : UInt256.isZero (acc2 input) = UInt256.ofNat 0 := by
    rw [acc2_eq_guardDiff input hw]; exact Logic.isZero_of_ne _ h
...
theorem run_fallback_jump (input : ByteArray) :
  ...
  have hpc : (fallbackJumpState input).pc.toNat = Artifact.submissionArtifact.instructionPC 1559 := by
    simp [fallbackJumpState, initialState, PCs.pc16, Challenge.EvmProof.Word.word_toNat_ofNat]
...
theorem run_fallback_prefix (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock fallbackPrefixPath (Main.trampolineState input 2795) =
...
theorem run_return (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock returnPath (Main.trampolineState input 2799) =
...
end Challenge.Modexp.Submission.Proofs.Memo.V10.Trace
