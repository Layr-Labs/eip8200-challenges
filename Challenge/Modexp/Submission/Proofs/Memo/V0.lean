...
def prefixPath :
  ...
  [
   Main.opAt 1002 (EvmSemantics.Operation.StackMemFlow (EvmSemantics.Operation.StackMemFlowOps.JUMPDEST)),
   Main.opAt 1003 (EvmSemantics.Operation.Env (EvmSemantics.Operation.EnvOps.CALLDATASIZE)),
   Main.pushAt 1004 2 1196
  ]
...
def notTakenPath :
  ...
  [
   ...
   Main.opAt 1003 (EvmSemantics.Operation.Env (EvmSemantics.Operation.EnvOps.CALLDATASIZE)),
   Main.pushAt 1004 2 1196,
   Main.opAt 1005 (EvmSemantics.Operation.StackMemFlow (EvmSemantics.Operation.StackMemFlowOps.JUMPI))
  ]

def returnPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [
   Main.pushAt 1006 0 0,
   Main.pushAt 1007 0 0,
   Main.opAt 1008 (EvmSemantics.Operation.System (EvmSemantics.Operation.SystemOps.RETURN))
  ]

def jumpState (input : ByteArray) : State :=
  { initialState submissionBytecode input 0 with
      pc := UInt256.ofNat 1382
...
theorem run_prefix (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock prefixPath (Main.trampolineState input 1377) =
...
theorem run_jump_taken (input : ByteArray) (hsize : input.size < 2 ^ 256) (h : input.size ≠ 0) :
  ...
  have hpc : (jumpState input).pc.toNat = Artifact.submissionArtifact.instructionPC 1005 := by
...
theorem run_notTaken (input : ByteArray) (h : input.size = 0) :
    Challenge.EvmProof.Stepper.runLocatedBlock notTakenPath (Main.trampolineState input 1377) =
      some (Main.trampolineState input 1383) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
theorem run_return (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock returnPath (Main.trampolineState input 1383) =
...
def gasSteps_match (input : ByteArray) (h : input.size = 0) :
    Challenge.EvmProof.GasSteps (Main.trampolineState input 1377) (returnedState input) :=
...
def gasSteps_fallback (input : ByteArray) (hsize : input.size < 2 ^ 256) (h : input.size ≠ 0) :
    Challenge.EvmProof.GasSteps (Main.trampolineState input 1377) (Main.trampolineState input 1196) :=
