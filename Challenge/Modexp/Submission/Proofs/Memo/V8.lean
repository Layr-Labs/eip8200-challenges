import Challenge.Modexp.Submission.Proofs.Memo.V8.Trace
import Challenge.Modexp.Submission.Proofs.Memo.V8.Spec
...
namespace Challenge.Modexp.Submission.Proofs.Memo.V8
...
def gasSteps_match (input : ByteArray) (h : guardDiff Data.checks input = 0) :
    Challenge.EvmProof.GasSteps (Main.trampolineState input 2176) (returnedState input) :=
...
def gasSteps_fallback (input : ByteArray) (h : guardDiff Data.checks input ≠ 0) :
    Challenge.EvmProof.GasSteps (Main.trampolineState input 2176) (Main.trampolineState input 1196) :=
  ((((sound preludePath rfl (run_prelude input) rfl rfl deployAddress_not_precompile).trans
    (sound chunk0Path rfl (run_chunk0 input) rfl rfl deployAddress_not_precompile)).trans
...
theorem returnedState_result (input : ByteArray) (hvalid : ValidInput input) (h : guardDiff Data.checks input = 0) :
  ...
  change ExecutionResult.returned (MachineState.readPadded answerMemory 31 33) = _
  rw [answerMemory_read, Spec.spec_eq hvalid hm]

end Challenge.Modexp.Submission.Proofs.Memo.V8
