...
def pretestJumpState (input : ByteArray) : State :=
  { initialState submissionBytecode input 0 with
      ...
      stack := [UInt256.ofNat 4327, UInt256.eq (UInt256.ofNat 0) (MachineState.readWord input 64)] }
...
theorem run_pretest_taken (input : ByteArray)
    ...
    Challenge.EvmProof.Stepper.runLocated pretestJumpLocated (pretestJumpState input) =
      some (Main.trampolineState input 4327) := by
  ...
  have hjump : Decode.isValidJumpDest submissionBytecode 4327 = true :=
  ...
  exact (Step.runLocated_jumpi_taken pretestJumpLocated rfl (pretestJumpState input) 4327
