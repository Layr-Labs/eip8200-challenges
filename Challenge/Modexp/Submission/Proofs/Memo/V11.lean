import Challenge.Modexp.Submission.Proofs.Memo.V11.Trace
import Challenge.Modexp.Submission.Proofs.Memo.V11.Spec
...
namespace Challenge.Modexp.Submission.Proofs.Memo.V11
...
def gasSteps_match (input : ByteArray) (h : guardDiff Data.checks input = 0) :
    Challenge.EvmProof.GasSteps (Main.trampolineState input 2689) (returnedState input) :=
...
def gasSteps_fallback (input : ByteArray) (h : guardDiff Data.checks input ≠ 0) :
    Challenge.EvmProof.GasSteps (Main.trampolineState input 2689) (Main.trampolineState input 1196) :=
  (((((((sound preludePath rfl (run_prelude input) rfl rfl deployAddress_not_precompile).trans
    (sound chunk0Path rfl (run_chunk0 input) rfl rfl deployAddress_not_precompile)).trans
    (sound chunk1Path rfl (run_chunk1 input) rfl rfl deployAddress_not_precompile)).trans
    (sound chunk2Path rfl (run_chunk2 input) rfl rfl deployAddress_not_precompile)).trans
...
theorem returnedState_result (input : ByteArray) (h : guardDiff Data.checks input = 0) :
  ...
  change ExecutionResult.returned (MachineState.readPadded answerMemory 0 128) = _
  rw [answerMemory_read, Spec.spec_eq hm]

end Challenge.Modexp.Submission.Proofs.Memo.V11
