...
import Challenge.Modexp.Submission.Proofs.Memo.V3.Spec
...
namespace Challenge.Modexp.Submission.Proofs.Memo.V3
...
def gasSteps_match (input : ByteArray)
    ...
    Challenge.EvmProof.GasSteps (Main.trampolineState input 1632) (returnedState input) :=
...
def gasSteps_fallback (input : ByteArray)
    (hw : MachineState.readWord input 32 = UInt256.ofNat 1)
    (h : guardDiff Data.checks input ≠ 0) :
    Challenge.EvmProof.GasSteps (Main.trampolineState input 1632) (Main.trampolineState input 1196) :=
  (((((sound preludePath rfl (run_prelude input) rfl rfl deployAddress_not_precompile).trans
    (sound chunk0Path rfl (run_chunk0 input) rfl rfl deployAddress_not_precompile)).trans
...
theorem returnedState_result (input : ByteArray) (h : guardDiff Data.checks input = 0) :
  ...
  change ExecutionResult.returned (MachineState.readPadded answerMemory 0 0) = _
  rw [answerMemory_read, Spec.spec_eq hm]

end Challenge.Modexp.Submission.Proofs.Memo.V3
