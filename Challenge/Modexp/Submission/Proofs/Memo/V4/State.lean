...
namespace Challenge.Modexp.Submission.Proofs.Memo.V4.State
...
def chunk0 : List (Nat × UInt256) := (Data.checks.drop 1).take 2
...
def returnedState (input : ByteArray) : State :=
  { initialState submissionBytecode input 0 with
      ...
      halt := .Returned
      hReturn := MachineState.readPadded answerMemory 20 12 }

theorem answerMemory_read :
    MachineState.readPadded answerMemory 20 12 = Precompile.natToBytes 0 12 := by
  decide +kernel

end Challenge.Modexp.Submission.Proofs.Memo.V4.State
