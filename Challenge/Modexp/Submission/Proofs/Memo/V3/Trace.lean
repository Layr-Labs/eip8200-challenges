...
theorem run_prelude (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock preludePath (Main.trampolineState input 1600) =
      some (accState input 1636 (acc0 input)) := by
  ...
  simp [preludePath, accState, acc0, Main.opAt, Main.pushAt, Main.wfOp,
    ...
    initialState, Main.trampolineState, PCs.pc4,
...
theorem run_branch_hit (input : ByteArray) (h : guardDiff Data.checks input = 0) :
    Challenge.EvmProof.Stepper.runLocatedBlock branchPath (accState input 1636 (acc0 input)) =
      some (Main.trampolineState input 1640) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
  have hz : acc0 input = UInt256.ofNat 0 := by
    rw [acc0_eq_guardDiff, h]; rfl
...
def branchJumpState (input : ByteArray) : State :=
  { initialState submissionBytecode input 0 with
      pc := UInt256.ofNat 1639
      stack := [UInt256.ofNat 1196, acc0 input] }
...
theorem run_branch_prefix (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock branchPrefixPath (accState input 1636 (acc0 input)) =
...
theorem run_branch_miss (input : ByteArray) (h : guardDiff Data.checks input ≠ 0) :
  ...
  have hcond : UInt256.isTrue (acc0 input) :=
    Cover.isTrue_of_ne_zero _ (by rw [acc0_eq_guardDiff]; exact h)
  have hjump : Decode.isValidJumpDest submissionBytecode 1196 = true :=
    Artifact.isValidJumpDest_index 899 (by rfl)
  have hpc : (branchJumpState input).pc.toNat = Artifact.submissionArtifact.instructionPC 1148 := by
  ...
  exact (Step.runLocated_jumpi_taken branchJumpLocated rfl (branchJumpState input) 1196
    (acc0 input) [] hpc rfl (by simp) hcond rfl (by norm_num) hjump).trans rfl

theorem run_return (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock returnPath (Main.trampolineState input 1640) =
