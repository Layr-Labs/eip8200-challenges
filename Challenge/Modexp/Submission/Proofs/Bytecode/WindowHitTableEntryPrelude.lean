...
@[simp] private theorem preludePCs (index : Nat)
    (hlo : 1852 ≤ index) (hhi : index ≤ 1866) :
    Artifact.submissionArtifact.instructionPC index =
      ([2819,2821,2822,2824,2825,2826,2827,2829,2830,2831,2832,2833,2834,2835,2837] : List Nat)[index - 1852]! := by
...
theorem run_tablePrelude (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock tablePreludePath
      (nonzeroState input) = some (tableState input 2 2838) := by
  ...
  have hend : endState (Dispatch.wordEntryState input)
      ...
      (modulusWord input) (routeStack input) = tableState input 2 2838 := by rfl
  rw [hstart, hend] at h
  exact h
