...
@[simp] private theorem preludePCs (index : Nat)
    (hlo : 1850 ≤ index) (hhi : index ≤ 1864) :
    Artifact.submissionArtifact.instructionPC index =
      [3030, 3032, 3033, 3035, 3036, 3037, 3038, 3040,
       3041, 3042, 3043, 3044, 3045, 3046, 3048][index - 1850]! := by
...
theorem run_tablePrelude (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock tablePreludePath
      (nonzeroState input) = some (tableState input 2 3049) := by
  ...
  have hend : endState (Dispatch.wordEntryState input)
      ...
      (modulusWord input) (routeStack input) = tableState input 2 3049 := by rfl
  rw [hstart, hend] at h
  exact h
