...
@[simp] private theorem preludePCs (index : Nat)
    (hlo : 1880 ≤ index) (hhi : index ≤ 1894) :
    Artifact.submissionArtifact.instructionPC index =
      [3063, 3065, 3066, 3068, 3069, 3070, 3071, 3073,
       3074, 3075, 3076, 3077, 3078, 3079, 3081][index - 1880]! := by
...
theorem run_tablePrelude (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock tablePreludePath
      (nonzeroState input) = some (tableState input 2 3082) := by
  ...
  have hend : endState (Dispatch.wordEntryState input)
      ...
      (modulusWord input) (routeStack input) = tableState input 2 3082 := by rfl
  rw [hstart, hend] at h
  exact h
