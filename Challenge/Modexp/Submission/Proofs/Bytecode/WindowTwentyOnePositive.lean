...
def normalized (template : State) (input : ByteArray) : State :=
  WindowTwentyOneTablePrelude.initial (context template input) (UInt256.ofNat 2691)
...
theorem run_prepare (template : State) (input : ByteArray)
    ...
    (hzeroBase : Decode.isValidJumpDest template.executionEnv.code 3305 = true)
    (hzeroModulus : Decode.isValidJumpDest template.executionEnv.code 3297 = true) :
    runInstructions prepareProgram (state template input (UInt256.ofNat 2666)) =
  ...
  have hb' : runInstructions WindowTwentyOneEntry.baseProgram
      ...
      some (state template input (UInt256.ofNat 2673)) := by
    simpa only [state, if_neg hb] using hbr
  have hm' : runInstructions WindowTwentyOneEntry.modulusProgram
      (state template input (UInt256.ofNat 2673)) =
      some (WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 2681)
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 2681)
...
theorem run_positive (template : State) (input : ByteArray)
    ...
    (hzeroBase : Decode.isValidJumpDest template.executionEnv.code 3305 = true)
    (hzeroModulus : Decode.isValidJumpDest template.executionEnv.code 3297 = true)
    (hloop : Decode.isValidJumpDest template.executionEnv.code 2828 = true) :
    runInstructions program (state template input (UInt256.ofNat 2666)) =
