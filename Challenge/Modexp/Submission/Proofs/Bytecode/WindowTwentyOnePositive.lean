...
def normalized (template : State) (input : ByteArray) : State :=
  WindowTwentyOneTablePrelude.initial (context template input) (UInt256.ofNat 2465)
...
theorem run_prepare (template : State) (input : ByteArray)
    ...
    (hzeroBase : Decode.isValidJumpDest template.executionEnv.code 3095 = true)
    (hzeroModulus : Decode.isValidJumpDest template.executionEnv.code 3087 = true) :
    runInstructions prepareProgram (state template input (UInt256.ofNat 2440)) =
  ...
  have hb' : runInstructions WindowTwentyOneEntry.baseProgram
      ...
      some (state template input (UInt256.ofNat 2447)) := by
    simpa only [state, if_neg hb] using hbr
  have hm' : runInstructions WindowTwentyOneEntry.modulusProgram
      (state template input (UInt256.ofNat 2447)) =
      some (WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 2455)
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 2455)
...
theorem run_positive (template : State) (input : ByteArray)
    ...
    (hzeroBase : Decode.isValidJumpDest template.executionEnv.code 3095 = true)
    (hzeroModulus : Decode.isValidJumpDest template.executionEnv.code 3087 = true)
    (hloop : Decode.isValidJumpDest template.executionEnv.code 2618 = true) :
    runInstructions program (state template input (UInt256.ofNat 2440)) =
