...
def routeStack (input : ByteArray) : List UInt256 :=
  [UInt256.ofNat (baseSize input), UInt256.ofNat (exponentSize input),
   ...
   exponentOffset input, modulusOffset input, UInt256.ofNat 1260,
...
def normalized (template : State) (input : ByteArray) : State :=
  WindowTwentyOneTablePrelude.initial (context template input) (UInt256.ofNat 2681)
...
theorem run_prepare (template : State) (input : ByteArray)
    ...
    (hzeroBase : Decode.isValidJumpDest template.executionEnv.code 3297 = true)
    (hzeroModulus : Decode.isValidJumpDest template.executionEnv.code 3289 = true) :
    runInstructions prepareProgram (state template input (UInt256.ofNat 2656)) =
  ...
  have hb' : runInstructions WindowTwentyOneEntry.baseProgram
      ...
      some (state template input (UInt256.ofNat 2663)) := by
    simpa only [state, if_neg hb] using hbr
  have hm' : runInstructions WindowTwentyOneEntry.modulusProgram
      (state template input (UInt256.ofNat 2663)) =
      some (WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 2671)
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 2671)
...
theorem run_positive (template : State) (input : ByteArray)
    ...
    (hzeroBase : Decode.isValidJumpDest template.executionEnv.code 3297 = true)
    (hzeroModulus : Decode.isValidJumpDest template.executionEnv.code 3289 = true)
    (hloop : Decode.isValidJumpDest template.executionEnv.code 2820 = true) :
    runInstructions program (state template input (UInt256.ofNat 2656)) =
