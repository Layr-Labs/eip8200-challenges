...
namespace Challenge.Modexp.Submission.Proofs.Bytecode.WindowNineTail

open EvmSemantics EvmSemantics.EVM YulEvmCompiler
open WindowNibbleKernel

def updateProgram : List Instr :=
  [.op (.Swap ⟨1, by decide⟩), .push 1 36, .op .SHL,
...
def updated (template : State) (pc base modulus accumulator exponent counter : UInt256)
  ...
  WindowNineLookup.framed template pc base modulus
    ([counter, accumulator, modulus,
      UInt256.shiftLeft exponent (UInt256.ofNat 36), UInt256.ofNat 480,
...
theorem run_head (template : State) (pc base modulus accumulator exponent counter : UInt256)
    ...
    runInstructions [.op .JUMPDEST]
      (WindowNineGroup.state template pc base modulus accumulator exponent counter 0 rest) =
    some (WindowNineGroup.state template pc.succ base modulus accumulator exponent counter 0 rest) := by
  have hcap : rest.length + 5 < 1024 := by omega
  simp [runInstructions, WindowNineGroup.state, WindowNineLookup.framed,
...
theorem run_update (template : State) (pc base modulus accumulator exponent counter : UInt256)
  ...
  simp [runInstructions, updateProgram, updated, WindowNineGroup.state,
    WindowNineLookup.framed, Challenge.EvmProof.Stepper.runInstr,
...
theorem run_jump (template : State)
    ...
    some (WindowNineGroup.state template
      (if UInt256.isTrue counter then target else advancePC 4 pc)
      base modulus accumulator (UInt256.shiftLeft exponent (UInt256.ofNat 36))
  ...
  by_cases hc : UInt256.isTrue counter <;>
    simp [runInstructions, jumpProgram, updated, WindowNineGroup.state,
      WindowNineLookup.framed, Challenge.EvmProof.Stepper.runInstr,
...
theorem shift_nine (exponent : UInt256) (processed : Nat)
    (hprocessed : processed + 9 < 64) :
    UInt256.shiftLeft (UInt256.shiftLeft exponent (UInt256.ofNat (4 * processed)))
        (UInt256.ofNat 36) =
      UInt256.shiftLeft exponent (UInt256.ofNat (4 * (processed + 9))) := by
  apply Challenge.EvmProof.Word.word_ext
  rw [WindowNineBits.shiftLeft_toNat_mod _ 36 (by decide),
    WindowNineBits.shiftLeft_toNat_mod _ (4 * processed) (by omega),
    WindowNineBits.shiftLeft_toNat_mod _ (4 * (processed + 9)) (by omega)]
  simp only [Nat.shiftLeft_eq, Nat.mod_mul_mod, Nat.mul_assoc, ← Nat.pow_add]
  congr 3

end Challenge.Modexp.Submission.Proofs.Bytecode.WindowNineTail
