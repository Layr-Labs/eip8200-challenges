...
theorem run_width (template : State) (baseSize exponentSize modulusSize : UInt256)
  ...
  have hpc : advancePC 3 (UInt256.ofNat 1767) = UInt256.ofNat 1770 := by decide
...
def hitTailProgram : List Instr := [.op .POP, .op .POP, .push 1 115, .op .JUMP]

theorem run_hit_tail (template : State) (x y : UInt256) (rest : List UInt256)
    (hrest : rest.length ≤ 1000)
    (htarget : Decode.isValidJumpDest template.executionEnv.code 115 = true) :
    runInstructions hitTailProgram (framed template (UInt256.ofNat 1770) (x :: y :: rest)) =
    some (framed template (UInt256.ofNat 115) rest) := by
...
theorem run_adapter (template : State) (a b c d e f g : UInt256) (rest : List UInt256)
    ...
    runInstructions adapterProgram
      (framed template (UInt256.ofNat 115) (a :: b :: c :: d :: e :: f :: g :: rest)) =
...
theorem run_base (template : State) (value : UInt256)
    (rest : List UInt256) (hrest : rest.length ≤ 1000) :
    runInstructions baseProgram (framed template (UInt256.ofNat 1775) (value :: rest)) =
    some (framed template (UInt256.ofNat 1776) (value :: rest)) := by
  have hcap1 : rest.length + 1 < 1024 := by omega
...
theorem run_modulus (template : State)
    ...
    runInstructions modulusProgram (framed template (UInt256.ofNat 1776) (value :: rest)) =
  ...
  simp [runInstructions, modulusProgram, framed, Challenge.EvmProof.Stepper.runInstr,
    hcap1, hcap2, hcap3, h96Word, Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod]
...
theorem run_normalize (template : State) (modulus : UInt256)
    ...
    runInstructions normalizeProgram
      (framed template (UInt256.ofNat 1783) (UInt256.ofNat 96 :: modulus :: rest)) =
    some (framed template (UInt256.ofNat 1792)
