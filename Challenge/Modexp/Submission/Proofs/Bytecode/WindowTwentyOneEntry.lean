...
theorem run_width_value (template : State) (baseSize exponentSize modulusSize : UInt256)
    ...
    runInstructions widthValueProgram
      (framed template (UInt256.ofNat 1634) ([baseSize, exponentSize, modulusSize] ++ tail)) =
    some (framed template (UInt256.ofNat 1649)
...
theorem run_width (template : State) (baseSize exponentSize modulusSize : UInt256)
  ...
  have hpc : advancePC 4 (UInt256.ofNat 1649) = UInt256.ofNat 1653 := by decide
  simpa only [widthProgram, framed, hpc] using both

def missProgram : List Instr := [.push 1 133, .op .JUMP]

theorem run_miss (template : State) (rest : List UInt256) (hrest : rest.length ≤ 1000)
    (htarget : Decode.isValidJumpDest template.executionEnv.code 133 = true) :
    runInstructions missProgram (framed template (UInt256.ofNat 1653) rest) =
    some (framed template (UInt256.ofNat 133) rest) := by
...
theorem run_base (template : State) (value : UInt256)
    (rest : List UInt256) (hrest : rest.length ≤ 1000) :
    runInstructions baseProgram (framed template (UInt256.ofNat 1656) (value :: rest)) =
    some (framed template (UInt256.ofNat 1657) (value :: rest)) := by
  have hcap1 : rest.length + 1 < 1024 := by omega
...
theorem run_normalize (template : State) (modulus baseOffset : UInt256)
    ...
    runInstructions normalizeProgram
      (framed template (UInt256.ofNat 1657) (modulus :: rest)) =
    some (framed template (UInt256.ofNat 1667)
