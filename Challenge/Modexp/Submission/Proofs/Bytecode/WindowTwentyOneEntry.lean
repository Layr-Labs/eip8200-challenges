...
theorem run_width_value (template : State) (baseSize exponentSize modulusSize : UInt256)
    ...
    runInstructions widthValueProgram
      (framed template (UInt256.ofNat 2171) ([baseSize, exponentSize, modulusSize] ++ tail)) =
    some (framed template (UInt256.ofNat 2186)
...
theorem run_width (template : State) (baseSize exponentSize modulusSize : UInt256)
  ...
  have hpc : advancePC 5 (UInt256.ofNat 2186) = UInt256.ofNat 2191 := by decide
...
theorem run_miss (template : State) (rest : List UInt256) (hrest : rest.length ≤ 1000)
    ...
    runInstructions missProgram (framed template (UInt256.ofNat 2191) rest) =
...
theorem run_base (template : State) (value : UInt256)
    (rest : List UInt256) (hrest : rest.length ≤ 1000) :
    runInstructions baseProgram (framed template (UInt256.ofNat 2195) (value :: rest)) =
    some (framed template (UInt256.ofNat 2197) rest) := by
  have hcap1 : rest.length + 1 < 1024 := by omega
...
def modulusProgram : List Instr :=
  [.op (.Dup ⟨5, by decide⟩), .op .CALLDATALOAD, .op (.Dup ⟨0, by decide⟩)] ++
    testProgram (UInt256.ofNat 2821)
...
theorem run_modulus (template : State) (modulusOffset : UInt256)
    ...
    (htarget : Decode.isValidJumpDest template.executionEnv.code 2821 = true) :
    let modulus := MachineState.readWord template.executionEnv.calldata modulusOffset.toNat
    runInstructions modulusProgram (framed template (UInt256.ofNat 2197) rest) =
    some (framed template (if modulus.toNat = 0 then UInt256.ofNat 2821 else UInt256.ofNat 2205)
      (modulus :: rest)) := by
  let modulus := MachineState.readWord template.executionEnv.calldata modulusOffset.toNat
  have hcap0 : rest.length < 1024 := by omega
  have hcap1 : rest.length + 1 < 1024 := by omega
  have hh : runInstructions [.op (.Dup ⟨5, by decide⟩), .op .CALLDATALOAD, .op (.Dup ⟨0, by decide⟩)]
      (framed template (UInt256.ofNat 2197) rest) =
      some (framed template (UInt256.ofNat 2200) (modulus :: modulus :: rest)) := by
  ...
  have ht := run_test template (UInt256.ofNat 2200) (UInt256.ofNat 2821) modulus
    (modulus :: rest) (by simp only [List.length_cons]; omega) htarget
  have both := runInstructions_append_some _ _ _ _ _ hh ht
  have hpc : advancePC 5 (UInt256.ofNat 2200) = UInt256.ofNat 2205 := by decide
...
theorem run_normalize (template : State) (modulus baseOffset : UInt256)
    ...
    runInstructions normalizeProgram
      (framed template (UInt256.ofNat 2205) (modulus :: rest)) =
    some (framed template (UInt256.ofNat 2215)
