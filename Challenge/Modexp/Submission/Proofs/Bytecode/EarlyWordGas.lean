...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (framed template (UInt256.ofNat 5218) [])
  ...
  exact (paths.guard.steps
    (env.transfer (t := framed template (UInt256.ofNat 5218) []) rfl rfl) rfl hg).trans
    (paths.hit.steps
      (env.transfer (t := framed template (UInt256.ofNat 5245) (headerStack input)) rfl rfl) rfl hh)
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    ...
    (input : ByteArray) (hdata : template.executionEnv.calldata = input)
    (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (framed template (UInt256.ofNat 5218) []) (framed template (UInt256.ofNat 1314) []) := by
  have hg := run_guard template input hdata (jump_env env paths.missJump)
  ...
  exact (paths.guard.steps
    (env.transfer (t := framed template (UInt256.ofNat 5218) []) rfl rfl) rfl hg).trans
    (paths.miss.steps
      (env.transfer (t := framed template (UInt256.ofNat 5266) (headerStack input)) rfl rfl) rfl hm)
