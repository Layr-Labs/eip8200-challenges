...
theorem exponentPhase_represents (s : State) (accumulatorWord : UInt256)
  ...
  · have hi : i < e := by omega
    ...
    have hzeroBitsWord : ∀ t, t < j →
        (BigExponent.exponentBit byte t).toNat = 0 := by
      intro t ht
      rw [exponentBit_toNat_eq_bitNat byte t (by omega)]
      exact hzeroBits t ht
    have hhit : (BigExponent.exponentBit byte j).toNat ≠ 0 := by
      simpa [byte, j] using coldBitIndex_hit byte hj
    have hfirstValue := natBitAfter_succ_of_zero_prefix modulus byte base j
      hmodulusPos _hbaseReduced hj hzeroBitsWord hhit
    ...
    have hcopiedAcc : Limbs.Represents copied.memory 2048 count base := by
      simpa [copied, copiedState, BigHelpers.copyReturned, copyEntryState,
        bitLoop, h1024, h2048] using
    ...
    have hcopiedMod : Limbs.Represents copied.memory 0 count modulus := by
      simpa [copied, copiedState, BigHelpers.copyReturned, copyEntryState,
        bitLoop, h1024, h2048] using
        BigHelpers.represents_copyMemory_disjoint_region s.memory 2048 1024 0
          count modulus (by omega) (Or.inr (by omega)) hmodulus
    have hbyteCopied : loadedByte copied expOff i = byte := by
      simp [loadedByte, BigExponent.loadedExponentByte, copied, copiedState,
        BigHelpers.copyReturned, copyEntryState, bitLoop, byte]
    have hbitsRep := exponentBitProgressFrom_represents copied accumulatorWord
      count b e m baseOff expOff i (loadedOffset expOff i) byte rest (j + 1)
      (8 - (j + 1)) base base modulus (by omega) hcount hmodulusPos
      _hbaseReduced hcopiedAcc hcopiedBase hcopiedMod
    have hsplit : natBitFrom modulus byte base (j + 1) base (8 - (j + 1)) =
        WordCorrect.natBitAfter modulus byte base 8 (1 % modulus) := by
      calc
        natBitFrom modulus byte base (j + 1) base (8 - (j + 1)) =
            natBitFrom modulus byte base (j + 1)
              (WordCorrect.natBitAfter modulus byte base (j + 1)
                (1 % modulus)) (8 - (j + 1)) := by rw [hfirstValue]
        _ = WordCorrect.natBitAfter modulus byte base
              ((j + 1) + (8 - (j + 1))) (1 % modulus) :=
          (natBitAfter_split modulus byte base (j + 1) (8 - (j + 1))
            (1 % modulus)).symm
        _ = WordCorrect.natBitAfter modulus byte base 8 (1 % modulus) := by
          congr 2
          omega
    have hbyteVal : BigExponentCorrect.exponentValueAfter s modulus base expOff
      ...
      rfl
    let first := firstByteProgress copied accumulatorWord count b e m baseOff
      expOff i (j + 1) rest
    have hbytesRep := exponentByteProgressFrom_represents first accumulatorWord
      ...
      (by rw [hbyteVal, ← hsplit]
          simpa [first, firstByteProgress, hbyteCopied] using hbitsRep.1)
      (by simpa [first, firstByteProgress, hbyteCopied] using hbitsRep.2.1)
      (by simpa [first, firstByteProgress, hbyteCopied] using hbitsRep.2.2)
    have henv : first.executionEnv = s.executionEnv := by
      simp [first, firstByteProgress, copied, copiedState,
        copyEntryState, bitLoop, BigHelpers.copyReturned]
    have htail : BigExponentCorrect.exponentValueAfter s modulus base expOff e
      ...
      omega
    rw [htail]
    simpa [exponentPhaseState, i, hiEq, foundProgress, first, copied, byte, j]
      using hbytesRep.1
