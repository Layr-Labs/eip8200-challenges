...
def copyBodyPath :
  ...
  [opAt 55 (.Dup ⟨0, by decide⟩), pushAt 56 1 5, opAt 57 .SHL,
   ...
   pushAt 67 1 1, opAt 68 .ADD,
   pushAt 69 2 60, opAt 70 .JUMP]
...
theorem run_copyBody (s : State) (dst src : UInt256) (count i : Nat)
  ...
  simp (config := { maxSteps := 250000 })
    [copyBodyPath, opAt, pushAt, wfOp,
      ...
      hincLeft, hsrccomm, hdstcomm, hsixty, hsixtyNat, hjump, jump60,
      State.activeWordsAfterUInt256,
      Challenge.EvmProof.Word.succ_ofNat_mod,
      Challenge.EvmProof.Word.ofNat_add_mod,
      Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt,
...
theorem run_addFinishGuard (s : State) (dst src take modulus : UInt256)
  ...
  simp [addGuardPath, opAt, pushAt, wfOp, addLoop, addPCs,
    ...
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt,
    hdest, hdestNat, hjump, jump170, hpc]
...
def addCopyPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 1064 .JUMPDEST, opAt 1065 (.Dup ⟨4, by decide⟩),
   pushAt 1066 2 5120, opAt 1067 (.Swap ⟨0, by decide⟩),
   opAt 1068 (.Dup ⟨10, by decide⟩), pushAt 1069 1 32, opAt 1070 .MUL,
   opAt 1071 (.Swap ⟨1, by decide⟩), opAt 1072 (.Swap ⟨0, by decide⟩),
   opAt 1073 .MCOPY]

def addCopyReturnPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 1074 .JUMPDEST, pushAt 1075 0 0, pushAt 1076 2 293, opAt 1077 .JUMP]

@[simp] private theorem addTailPCs (i : Nat) (hi : 1046 ≤ i) (hii : i ≤ 1077) :
    Artifact.submissionArtifact.instructionPC i =
      ([1418,1419,1420,1421,1422,1423,1424,1425,1426,1427,1430,1431,1432,
        1435,1436,1437,1438,1441,1442,1443,1444,1447,1448,1449,1451,1452,
        1453,1454,1455,1456,1457,1460])[i - 1046]! := by
  interval_cases i <;> decide
...
@[simp] private theorem jump1455 :
  ...
  Artifact.isValidJumpDest_index 1074 (by rfl)
...
/-- `MCOPY dst, 0x1400, 32*count` memory effect, with the word-level `MUL`
size reduced to `(32 * count) % 2 ^ 256` (the `run` proof discharges the
reduction with `hcount`; the `Represents` proofs remove it with fits). -/
def mcopyMemory (memory : ByteArray) (dst : UInt256) (count : Nat) : ByteArray :=
  MachineState.writeBytes memory
    (MachineState.readPadded memory 5120 ((32 * count) % 2 ^ 256)) dst.toNat

/-- `MCOPY` two-range expansion over the destination and `0x1400` windows. -/
def mcopyWords (activeWords dst : UInt256) (count : Nat) : UInt256 :=
  UInt256.ofNat (MachineState.activeWordsAfter
    (MachineState.activeWordsAfter activeWords.toNat dst.toNat
      ((32 * count) % 2 ^ 256)) 5120 ((32 * count) % 2 ^ 256))

/-- `toNat` of a word multiplication (no `MUL` normaliser in `Word`). -/
private theorem word_toNat_mul (a b : UInt256) :
    (a * b).toNat = (a.toNat * b.toNat) % 2 ^ 256 := by
  change (a.val * b.val).val = _
  rw [Fin.val_mul]
  unfold EvmSemantics.UInt256.toNat
  rfl
...
def maskChoice (memory : ByteArray) (activeWords dst selectMask : UInt256)
  ...
  else ⟨mcopyMemory memory dst count, mcopyWords activeWords dst count⟩
...
/-- Non-zero mask: `maskChoice` is one inline `MCOPY`. -/
theorem maskChoice_of_pos (memory : ByteArray)
    (activeWords dst selectMask : UInt256) (count : Nat)
    (h : ¬ selectMask.toNat = 0) :
    maskChoice memory activeWords dst selectMask count =
      ⟨mcopyMemory memory dst count,
       mcopyWords activeWords dst count⟩ := by
  simp [maskChoice, h]
...
/-- `JUMPI` taken: the mask is all ones, take the inline-`MCOPY` shortcut. -/
def maskCopyEntry (s : State) (dst src take modulus : UInt256) (count : Nat)
    (returnDest : UInt256) (rest : List UInt256) : State :=
  { subtractLoop s dst src take modulus count count returnDest rest with
      pc := UInt256.ofNat 1442
      stack := maskFrame s dst src take modulus count returnDest rest }

/-- State after the inline `MCOPY(dst, 0x1400, 32*count)` lands at 1455. -/
def maskCopied (s : State) (dst src take modulus : UInt256) (count : Nat)
    (returnDest : UInt256) (rest : List UInt256) : State :=
  { maskCopyEntry s dst src take modulus count returnDest rest with
    pc := UInt256.ofNat 1455
    stack := maskFrame s dst src take modulus count returnDest rest
    memory := mcopyMemory
      (maskCopyEntry s dst src take modulus count returnDest rest).memory
      dst count
    activeWords := mcopyWords
      (maskCopyEntry s dst src take modulus count returnDest rest).activeWords
      dst count }
...
/-- All-ones mask: the select loop is replaced by one inline `MCOPY`. -/
...
theorem addReturned_represents_mod (s : State)
  ...
  · -- No conditional subtraction: the select loop was skipped entirely and the
    ...
    simpa [addReturned, added, subtracted, useSub] using hgoal
  · -- Conditional subtraction happens: inline `MCOPY` moves the candidate
    ...
    have hsize : (32 * count) % 2 ^ 256 = 32 * count := by
      rw [Nat.mod_eq_of_lt (by omega : 32 * count < 2 ^ 256)]
    have hcopy : Limbs.Represents
        (mcopyMemory subtracted.memory (UInt256.ofNat dst) count)
        dst count candidate := by
      simp only [mcopyMemory, hdstNat, hsize]
      exact Challenge.Modexp.Submission.Proofs.Mcopy.represents_mcopy
        subtracted.memory dst 5120 count candidate hsubtractedCandidate
    have hgoal : Limbs.Represents
        (maskChoice subtracted.memory subtracted.activeWords
          (UInt256.ofNat dst) (0 - useSub) count).memory dst count
          (total % modulusValue) := by
      rw [maskChoice_of_pos _ _ _ _ _ hmask, ← hchosen']
      exact hcopy
    simpa [addReturned, added, subtracted, useSub] using hgoal
...
theorem addReturned_preserves_region (s : State)
  ...
  · have hdstNat : (UInt256.ofNat dst).toNat = dst := by
    ...
    have hsize : (32 * count) % 2 ^ 256 = 32 * count := by
      rw [Nat.mod_eq_of_lt (by omega : 32 * count < 2 ^ 256)]
    have hdisjoint : ptr + 32 * count ≤ dst ∨ dst + 32 * count ≤ ptr := by
      rcases hptrDst with h | h
      · exact Or.inr h
      · exact Or.inl h
    have hcopy : Limbs.Represents
        (mcopyMemory subtracted.memory (UInt256.ofNat dst) count)
      ...
      simp only [mcopyMemory, hdstNat, hsize]
      exact
        Challenge.Modexp.Submission.Proofs.Mcopy.represents_mcopy_disjoint_region
          subtracted.memory dst 5120 count ptr count value hdisjoint
          hsubtracted
    have hgoal : Limbs.Represents
        (maskChoice subtracted.memory subtracted.activeWords
          (UInt256.ofNat dst) (0 - useSub) count).memory ptr count value := by
...
theorem run_addMaskBranch (s : State) (dst src take modulus : UInt256)
  ...
  have hzero : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzero' : UInt256.ofNat 0 = (0 : UInt256) := by decide
  have h1442 : (1442 : UInt256) = UInt256.ofNat 1442 := by decide
...
set_option linter.unusedSimpArgs false in
/-- The inline-`MCOPY` shortcut lands directly in the copied post-state. -/
theorem run_addCopyCall (s : State) (dst src take modulus : UInt256)
  ...
  have h32lt : 32 < 2 ^ 256 := by norm_num
  have hcountNat : (UInt256.ofNat count).toNat = count := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt hcount]
  have hmul : (UInt256.ofNat 32 * UInt256.ofNat count).toNat =
      (32 * count) % 2 ^ 256 := by
    simp only [word_toNat_mul, Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt h32lt, Nat.mod_eq_of_lt hcount]
  simp [addCopyPath, opAt, pushAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    maskCopyEntry, maskCopied, mcopyMemory, mcopyWords, maskFrame,
    subtractLoop, State.activeWordsAfterUInt256_2,
    addTailPCs, hc10, hc11, hc12, hc13, hc14, hrun,
    h32, h5120, h5120Nat, hcountNat, hmul,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt,
    List.exchange]
...
theorem run_addCopyReturn (s : State) (dst src take modulus : UInt256)
  ...
  simp [addCopyReturnPath, opAt, pushAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    maskCopied, maskCopyExit, maskCopyEntry, maskFrame,
...
theorem run_addExit (s : State) (dst src take modulus : UInt256)
  ...
  simp [selectExitPath, opAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    addExitFrame, addReturned, maskFrame, selectMaskOf, subtractLoop,
...
selection entirely (mask zero) or it replaces it by one inline `MCOPY` (mask all
...
def gasSteps_addMaskSegment (s : State) (dst src take modulus : UInt256)
  ...
  · have hbranch := Challenge.EvmProof.Stepper.runLocatedBlock_sound
    ...
    have hcall := Challenge.EvmProof.Stepper.runLocatedBlock_sound
      Artifact.submissionArtifact .Osaka addCopyPath
        ...
        (run_addCopyCall s dst src take modulus count returnDest rest hcap
          hcount hrun)
    ...
    have hret := Challenge.EvmProof.Stepper.runLocatedBlock_sound
      Artifact.submissionArtifact .Osaka addCopyReturnPath
        (by simpa [maskCopied, maskCopyEntry, subtractLoop,
          Artifact.submissionArtifact] using hcode)
        (by simpa [maskCopied, maskCopyEntry, subtractLoop,
          State.fork] using hfork)
        (run_addCopyReturn s dst src take modulus count returnDest rest hcap
          hcode hrun)
        (by simpa [maskCopied, maskCopyEntry, subtractLoop] using hrun)
        (by simpa [maskCopied, maskCopyEntry, subtractLoop,
          State.fork] using hnp)
    exact Challenge.EvmProof.GasSteps.cast
      (hbranch.trans (hcall.trans hret)) rfl
