...
def bitEntryPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 484 .JUMPDEST, pushAt 485 2 (UInt256.ofNat 3739)]
...
def bitHeadPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2445 .JUMPDEST, pushAt 2446 1 (UInt256.ofNat 1),
   opAt 2447 (.Dup ⟨6, by decide⟩), opAt 2448 .SUB]

/-- Its tail drops `base - 1` and rejoins the byte loop. -/
def bitExitPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2585 .POP, pushAt 2586 2 (UInt256.ofNat 655), opAt 2587 .JUMP]

/-- Byte offset of the copy of the unrolled body that handles exponent bit `j`. -/
def bitPC (j : Nat) : Nat := 3744 + 20 * j
...
def bitPushState (input : ByteArray) (outer : Nat) (byte offset : UInt256)
  ...
  { s with pc := UInt256.ofNat 610, stack := UInt256.ofNat 3739 :: s.stack }
...
theorem jump3695 : Decode.isValidJumpDest submissionBytecode 3739 = true :=
