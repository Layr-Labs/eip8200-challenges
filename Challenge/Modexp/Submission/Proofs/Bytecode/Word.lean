...
def startPath :
  ...
  [opAt 430 .JUMPDEST, opAt 431 (.Dup ⟨5, by decide⟩),
   opAt 432 .CALLDATALOAD, opAt 433 (.Dup ⟨3, by decide⟩),
   pushAt 434 1 32, opAt 435 .SUB, pushAt 436 1 3,
   opAt 437 .SHL, opAt 438 .SHR, opAt 439 (.Dup ⟨0, by decide⟩),
   pushAt 440 2 536, opAt 441 .JUMPI]

def zeroTailPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 442 (.Dup ⟨3, by decide⟩), pushAt 443 0 0,
   opAt 444 .RETURN]
...
def startJumpPath := [opAt 441 .JUMPI]

def baseSetupPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 445 .JUMPDEST, pushAt 446 0 0, pushAt 447 0 0]

def baseGuardPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 448 .JUMPDEST, opAt 449 (.Dup ⟨3, by decide⟩),
   opAt 450 (.Dup ⟨1, by decide⟩), opAt 451 .LT, opAt 452 .ISZERO,
   pushAt 453 2 576, opAt 454 .JUMPI]

def baseCallPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 455 (.Dup ⟨2, by decide⟩), pushAt 456 2 559, pushAt 457 0 0,
   opAt 458 (.Dup ⟨3, by decide⟩), opAt 459 (.Dup ⟨10, by decide⟩),
   opAt 460 .ADD, pushAt 461 1 48, opAt 462 .JUMP]

def baseTailHeadPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 463 .JUMPDEST, opAt 464 (.Dup ⟨4, by decide⟩), pushAt 465 2 256,
   opAt 466 (.Dup ⟨5, by decide⟩), opAt 467 .MULMOD, opAt 468 .ADDMOD,
  ]

def baseTailSwapPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 469 (.Swap ⟨1, by decide⟩)]
...
def baseTailFinishPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 471 1 1,
   opAt 472 .ADD, pushAt 473 2 539, opAt 474 .JUMP]
...
def baseFinishTailPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 475 .JUMPDEST, opAt 476 .POP, opAt 477 (.Dup ⟨1, by decide⟩),
   pushAt 478 1 1, opAt 479 .MOD, pushAt 480 0 0]

def expGuardPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 481 .JUMPDEST, opAt 482 (.Dup ⟨5, by decide⟩),
   opAt 483 (.Dup ⟨1, by decide⟩), opAt 484 .EQ,
   pushAt 485 2 614, opAt 486 .JUMPI]

def expLoadPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 487 (.Dup ⟨0, by decide⟩), opAt 488 (.Dup ⟨9, by decide⟩),
   opAt 489 .ADD, opAt 490 (.Dup ⟨0, by decide⟩), opAt 491 .CALLDATALOAD,
   pushAt 492 0 0, opAt 493 .BYTE, pushAt 494 0 0]
...
def bitHeadPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2170 .JUMPDEST, pushAt 2171 1 1,
   opAt 2172 (.Dup ⟨6, by decide⟩), opAt 2173 .SUB]

/-- Its tail drops `base - 1` and rejoins the byte loop. -/
def bitExitPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2308 .POP, pushAt 2309 2 603, opAt 2310 .JUMP]

/-- Byte offset of the copy of the unrolled body that handles exponent bit `j`. -/
def bitPC (j : Nat) : Nat := if j = 8 then 3106 else 2949 + 20 * j
...
def loadedState (input : ByteArray) : State :=
  { nonzeroState input with
    ...
    stack := (536 : UInt256) :: UInt256.ofNat (modulusValue input) ::
...
def baseGuardState (input : ByteArray) (i : Nat) (base : UInt256) : State :=
  { baseLoopState input i base with pc := UInt256.ofNat 548 }
...
def baseFinishDispatchState (input : ByteArray) (base : UInt256) : State :=
  { baseLoopState input (baseSize input) base with pc := UInt256.ofNat 576 }
...
def expGuardState (input : ByteArray) (i : Nat) (acc base : UInt256) : State :=
  { expLoopState input i acc base with pc := UInt256.ofNat 591 }
...
def bitPushState (input : ByteArray) (outer : Nat) (byte offset : UInt256)
  ...
  { s with pc := UInt256.ofNat 602, stack := UInt256.ofNat 3127 :: s.stack }

def bitHeadState (input : ByteArray) (outer : Nat) (byte offset : UInt256)
    (acc base : UInt256) : State :=
  WordStep.stW (bitLoopState input outer 0 byte offset acc base) 2944
    ([UInt256.ofNat 0, byte, offset, UInt256.ofNat outer, acc, base,
      UInt256.ofNat (modulusValue input)] ++ bitTail input)

theorem jump3695 : Decode.isValidJumpDest submissionBytecode 2811 = true :=
...
@[simp] private theorem startPCs (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC i =
      ([517,518,519,520,521,523,524,526,527,528,529,532,533,534,535] : List Nat)[i - 430]! := by
  interval_cases i <;> decide

@[simp] private theorem jump538 :
    Decode.isValidJumpDest submissionBytecode 536 = true :=
  Artifact.isValidJumpDest_index 445 (by rfl)

@[simp] theorem wordPCs (i : Nat)
    (hi : 445 ≤ i) (hii : i ≤ 474) :
    Artifact.submissionArtifact.instructionPC i =
      ([536,537,538,539,540,541,542,543,544,547,548,549,552,553,554,555,556,558,559,560,561,564,565,566,567,568,569,571,572,575] : List Nat)[i - 445]! := by
  interval_cases i <;> decide

@[simp] theorem jump582 :
    Decode.isValidJumpDest submissionBytecode 576 = true :=
  Artifact.isValidJumpDest_index 475 (by rfl)

@[simp] theorem jump562 :
    Decode.isValidJumpDest submissionBytecode 559 = true :=
...
@[simp] theorem baseFinishPCs (i : Nat)
    (hi : 475 ≤ i) (hii : i ≤ 480) :
    Artifact.submissionArtifact.instructionPC i =
      ([576,577,578,579,581,582] : List Nat)[i - 475]! := by
  interval_cases i <;> decide

@[simp] theorem expPCs (i : Nat)
    (hi : 481 ≤ i) (hii : i ≤ 504) :
    Artifact.submissionArtifact.instructionPC i =
      ([583,584,585,586,587,590,591,592,593,594,595,596,597,598,599,602,603,604,605,606,607,609,610,613] : List Nat)[i - 481]! := by
  interval_cases i <;> decide

@[simp] theorem jump669 :
    Decode.isValidJumpDest submissionBytecode 614 = true :=
...
@[simp] theorem jump589 :
  ...
  Artifact.isValidJumpDest_index 481 (by rfl)
...
theorem run_startJump_nonzero (input : ByteArray)
  ...
  have h538Word : (536 : UInt256) = UInt256.ofNat 536 := by decide
...
theorem run_baseGuard (input : ByteArray) (i : Nat) (base : UInt256)
  ...
  have hcondLiteral :
    ...
    exact hcond
  have h550 : (548 : UInt256).toNat = 548 := by decide
...
theorem run_baseFinishGuard (input : ByteArray) (base : UInt256)
  ...
  have hbmod : baseSize input % 2 ^ 256 = baseSize input :=
    Nat.mod_eq_of_lt hb256
  have h582 : (576 : UInt256).toNat = 576 := by decide
  have h582Word : (576 : UInt256) = UInt256.ofNat 576 := by decide
