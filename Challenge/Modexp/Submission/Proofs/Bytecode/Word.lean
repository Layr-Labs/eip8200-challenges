...
def expGuardPath :
  ...
  [opAt 469 .JUMPDEST, opAt 470 (.Dup ⟨5, by decide⟩),
   ...
   pushAt 474 2 666, opAt 475 .JUMPI]
...
def bitEntryPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 484 .JUMPDEST, pushAt 485 2 3438]
...
def bitHeadPath :
  ...
  [opAt 2519 .JUMPDEST, pushAt 2520 1 1,
   opAt 2521 (.Dup ⟨6, by decide⟩), opAt 2522 .SUB]

/-- Its tail drops `base - 1` and rejoins the byte loop. -/
def bitExitPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2659 .POP, pushAt 2660 2 652, opAt 2661 .JUMP]

/-- Byte offset of the copy of the unrolled body that handles exponent bit `j`. -/
def bitPC (j : Nat) : Nat := 3443 + 20 * j - (if 8 ≤ j then 1 else 0)
...
def bitAdvanceFinishPath :
  ...
  [pushAt 518 1 1, opAt 519 .ADD,
   pushAt 520 2 604, opAt 521 .JUMP]
...
def nonzeroState (input : ByteArray) : State :=
  { Dispatch.wordEntryState input with
    ...
    stack := [UInt256.ofNat (modulusValue input),
      ...
      UInt256.ofNat 1260] ++ callerRest input }
...
def zeroModulusFinalState (input : ByteArray) : State :=
  { Dispatch.wordEntryState input with
    ...
    stack := [UInt256.ofNat 0, UInt256.ofNat (baseSize input),
      ...
      UInt256.ofNat (modulusOffset input), UInt256.ofNat 1260] ++ callerRest input
    halt := .Returned
...
def baseFinishDispatchState (input : ByteArray) (base : UInt256) : State :=
  { baseLoopState input (baseSize input) base with pc := UInt256.ofNat 580 }
...
def expGuardState (input : ByteArray) (i : Nat) (acc base : UInt256) : State :=
  { expLoopState input i acc base with pc := UInt256.ofNat 596 }
...
def bitPushState (input : ByteArray) (outer : Nat) (byte offset : UInt256)
  ...
  { s with pc := UInt256.ofNat 608, stack := UInt256.ofNat 3438 :: s.stack }

def bitHeadState (input : ByteArray) (outer : Nat) (byte offset : UInt256)
    (acc base : UInt256) : State :=
  WordStep.stW (bitLoopState input outer 0 byte offset acc base) 3438
    ([UInt256.ofNat 0, byte, offset, UInt256.ofNat outer, acc, base,
      UInt256.ofNat (modulusValue input)] ++ bitTail input)

theorem jump3695 : Decode.isValidJumpDest submissionBytecode 3438 = true :=
...
def bitGuardState (input : ByteArray) (outer j : Nat) (byte offset : UInt256)
    (acc base : UInt256) : State :=
  { bitLoopState input outer j byte offset acc base with pc := UInt256.ofNat 613 }
...
@[simp] private theorem startPCs (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC i =
      ([517,518,519,520,521,523,524,526,527,528,529,532,533,534,535] : List Nat)[i - 415]! := by
...
@[simp] theorem wordPCs (i : Nat)
    (hi : 430 ≤ i) (hii : i ≤ 462) :
    Artifact.submissionArtifact.instructionPC i =
      ([536,537,538,539,540,541,542,543,544,547,548,549,552,553,554,555,556,559,560,561,562,565,566,567,568,569,570,572,573,576,577,578,579] : List Nat)[i - 430]! := by
  interval_cases i <;> decide
...
@[simp] theorem baseFinishPCs (i : Nat)
    (hi : 463 ≤ i) (hii : i ≤ 468) :
    Artifact.submissionArtifact.instructionPC i =
      ([580,581,582,583,585,586] : List Nat)[i - 463]! := by
  interval_cases i <;> decide

@[simp] theorem expPCs (i : Nat)
    (hi : 469 ≤ i) (hii : i ≤ 535) :
    Artifact.submissionArtifact.instructionPC i =
      ([587,588,589,590,591,592,595,596,597,598,599,600,601,602,603,604,605,608,609,610,611,612,613,615,616,617,619,620,621,622,623,624,625,626,627,628,629,630,631,632,633,634,635,636,637,638,639,640,641,642,644,645,648,649,650,651,652,653,654,655,656,658,659,662,663,664,665] : List Nat)[i - 469]! := by
  interval_cases i <;> decide

@[simp] theorem jump669 :
    Decode.isValidJumpDest submissionBytecode 666 = true :=
  Artifact.isValidJumpDest_index 536 (by rfl)

@[simp] theorem jump655 :
    Decode.isValidJumpDest submissionBytecode 652 = true :=
  Artifact.isValidJumpDest_index 525 (by rfl)

@[simp] theorem jump606 :
    Decode.isValidJumpDest submissionBytecode 604 = true :=
...
theorem run_zeroTail (input : ByteArray) (hvalid : ValidInput input)
  ...
  simp [zeroTailPath, opAt, pushAt,
    ...
    Nat.mod_eq_of_lt hm', h0, hzeroNat, hmodulus]
...
theorem run_baseFinishGuard (input : ByteArray) (base : UInt256)
  ...
  have h582Word : (580 : UInt256) = UInt256.ofNat 580 := by decide
