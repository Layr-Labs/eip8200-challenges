...
def expGuardPath :
  ...
  [opAt 469 .JUMPDEST, opAt 470 (.Dup ⟨5, by decide⟩),
   ...
   pushAt 474 2 665, opAt 475 .JUMPI]
...
def bitEntryPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 484 .JUMPDEST, pushAt 485 2 3657]
...
def bitHeadPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2232 .JUMPDEST, pushAt 2233 1 1,
   opAt 2234 (.Dup ⟨6, by decide⟩), opAt 2235 .SUB]

/-- Its tail drops `base - 1` and rejoins the byte loop. -/
def bitExitPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2372 .POP, pushAt 2373 2 651, opAt 2374 .JUMP]

/-- Byte offset of the copy of the unrolled body that handles exponent bit `j`. -/
def bitPC (j : Nat) : Nat := 3662 + 20 * j

def bitDecodePath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 488 1 1, opAt 489 (.Dup ⟨2, by decide⟩),
   opAt 490 (.Dup ⟨2, by decide⟩), pushAt 491 1 7, opAt 492 .SUB,
   opAt 493 .SHR, opAt 494 .AND]

def bitSquarePath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 495 (.Dup ⟨7, by decide⟩), opAt 496 (.Dup ⟨6, by decide⟩),
   opAt 497 (.Dup ⟨7, by decide⟩), opAt 498 .MULMOD]

def bitMaskPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 499 (.Dup ⟨1, by decide⟩), pushAt 500 0 0, opAt 501 .SUB]

def bitProductPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 502 (.Dup ⟨9, by decide⟩), opAt 503 (.Dup ⟨9, by decide⟩),
   opAt 504 (.Dup ⟨3, by decide⟩), opAt 505 .MULMOD]

def bitChoosePath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 506 (.Dup ⟨2, by decide⟩), opAt 507 .XOR, opAt 508 .AND,
   opAt 509 (.Dup ⟨1, by decide⟩), opAt 510 .XOR]

def bitAdvanceSwapPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 511 (.Swap ⟨6, by decide⟩)]

def bitAdvanceDropPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 512 .POP, opAt 513 .POP, opAt 514 .POP]

def bitAdvanceFinishPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 515 1 1, opAt 516 .ADD,
   pushAt 517 2 606, opAt 518 .JUMP]
...
def nonzeroState (input : ByteArray) : State :=
  { Dispatch.wordEntryState input with
    ...
    stack := [UInt256.ofNat (modulusValue input),
      ...
      UInt256.ofNat 1263] ++ callerRest input }
...
def zeroModulusFinalState (input : ByteArray) : State :=
  { Dispatch.wordEntryState input with
    ...
    stack := [UInt256.ofNat 0, UInt256.ofNat (baseSize input),
      ...
      UInt256.ofNat (modulusOffset input), UInt256.ofNat 1263] ++ callerRest input
    halt := .Returned
...
def bitPushState (input : ByteArray) (outer : Nat) (byte offset : UInt256)
  ...
  { s with pc := UInt256.ofNat 610, stack := UInt256.ofNat 3657 :: s.stack }

def bitHeadState (input : ByteArray) (outer : Nat) (byte offset : UInt256)
    (acc base : UInt256) : State :=
  WordStep.stW (bitLoopState input outer 0 byte offset acc base) 3657
    ([UInt256.ofNat 0, byte, offset, UInt256.ofNat outer, acc, base,
      UInt256.ofNat (modulusValue input)] ++ bitTail input)

theorem jump3695 : Decode.isValidJumpDest submissionBytecode 3657 = true :=
...
def bitGuardState (input : ByteArray) (outer j : Nat) (byte offset : UInt256)
    (acc base : UInt256) : State :=
  { bitLoopState input outer j byte offset acc base with pc := UInt256.ofNat 612 }
...
@[simp] theorem wordPCs (i : Nat)
    (hi : 430 ≤ i) (hii : i ≤ 462) :
    Artifact.submissionArtifact.instructionPC i =
      [538,539,540,541,542,543,544,545,546,549,550,551,554,555,556,557,558,561,562,563,564,567,568,569,570,571,572,574,575,578,579,580,581][i - 430]! := by
  interval_cases i <;> decide
...
@[simp] theorem expPCs (i : Nat)
    (hi : 469 ≤ i) (hii : i ≤ 532) :
    Artifact.submissionArtifact.instructionPC i =
      [589,590,591,592,593,594,597,598,599,600,601,602,603,604,605,606,607,610,611,612,614,615,616,618,619,620,621,622,623,624,625,626,627,628,629,630,631,632,633,634,635,636,637,638,639,640,641,643,644,647,648,649,650,651,652,653,654,655,657,658,661,662,663,664][i - 469]! := by
  interval_cases i <;> decide

@[simp] theorem jump669 :
    Decode.isValidJumpDest submissionBytecode 665 = true :=
  Artifact.isValidJumpDest_index 533 (by rfl)

@[simp] theorem jump655 :
    Decode.isValidJumpDest submissionBytecode 651 = true :=
