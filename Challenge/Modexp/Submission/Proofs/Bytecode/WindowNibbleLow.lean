...
def lowFourSquareProgram : List Instr :=
  lowBeginSquareProgram ++ lowTopSquareProgram ++ lowTopSquareProgram ++ lowTopSquareProgram

def lowFusedSquareLookupProgram : List Instr :=
  [.op (.Dup ⟨5, by decide⟩), .op (.Swap ⟨1, by decide⟩),
   .push 14 5, .op .SHL, .op .MLOAD, .op .MULMOD,
...
bytes, peak depth eight plus `rest`. -/
theorem run_lowFourSquares (template : State) (pc : UInt256)
    ...
    runInstructions lowFourSquareProgram
      (lowNibbleState template pc base modulus nibble byte word pointer
        accumulator rest) =
      some (lowSquareTopState template (advancePC 16 pc) base modulus nibble
        byte word pointer accumulator
        (WindowMath.squareWordAfter modulus 4 accumulator) rest) := by
  rw [lowFourSquareProgram, runInstructions_append, runInstructions_append,
    runInstructions_append,
    run_lowBeginSquare template pc base modulus nibble byte word pointer
      accumulator rest hrest]
  simp only [Option.bind_some]
  rw [run_lowTopSquare (hrest := hrest)]
  simp only [Option.bind_some]
  rw [run_lowTopSquare (hrest := hrest)]
  simp only [Option.bind_some]
  rw [run_lowTopSquare (hrest := hrest)]
...
whole program is twenty-four instructions in thirty-eight bytes. -/
theorem run_lowSquareLookup (template : State) (pc : UInt256)
  ...
  rw [lowSquareLookupProgram, runInstructions_append,
    run_lowFourSquares template pc base modulus nibble byte word pointer
      accumulator rest hrest]
  simp only [Option.bind_some]
  rw [run_lowFusedSquareLookup template (advancePC 16 pc) base modulus nibble
