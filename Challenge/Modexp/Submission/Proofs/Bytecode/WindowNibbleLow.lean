...
theorem run_lowFourSquares (template : State) (pc : UInt256)
  ...
  rw [run_lowStagedPair start (advancePC 5 pc) accumulator modulus
    (modulus :: modulus :: modulus :: start.stack) (by simp [start, lowNibbleState]; omega)]
  simp only [Option.bind_some]
  rw [run_lowStagedPair start (advancePC 2 (advancePC 5 pc))
    (UInt256.mulMod accumulator accumulator modulus) modulus
    (modulus :: modulus :: start.stack) (by simp [start, lowNibbleState]; omega)]
  simp only [Option.bind_some]
  rw [run_lowStagedPair start (advancePC 2 (advancePC 2 (advancePC 5 pc)))
    ...
    (modulus :: start.stack) (by simp [start, lowNibbleState]; omega)]
  simp only [Option.bind_some]
  rw [run_lowStagedPair start (advancePC 2 (advancePC 2 (advancePC 2 (advancePC 5 pc))))
    (UInt256.mulMod
      (UInt256.mulMod (UInt256.mulMod accumulator accumulator modulus)
        (UInt256.mulMod accumulator accumulator modulus) modulus)
      (UInt256.mulMod (UInt256.mulMod accumulator accumulator modulus)
