...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2662 WindowNineTableBuild.program
  init : Block artifact fork 2779 WindowNineInit.program
  iteration : Block artifact fork 2801 WindowNineLoop.iterationProgram
  finish : Block artifact fork 3008 WindowNineReturn.program
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2801 = true) :
    GasSteps (WindowNineTablePrelude.initial template (UInt256.ofNat 2662) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowNineTablePrelude.initial template (UInt256.ofNat 2662) base modulus rest)
  ...
  have hi' : runInstructions WindowNineInit.program
      (WindowNineTable.state template (UInt256.ofNat 2779) base modulus exponent 15 rest) =
  ...
  have hinit := paths.init.steps
    (s := WindowNineTable.state template (UInt256.ofNat 2779) base modulus exponent 15 rest)
