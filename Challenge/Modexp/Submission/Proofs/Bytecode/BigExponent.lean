...
theorem run_innerGuard (s : State) (accumulatorWord : UInt256)
  ...
  have h8mod : 8 % 2 ^ 256 = 8 := Nat.mod_eq_of_lt (by norm_num)
  ...
  have heq : UInt256.eq (UInt256.ofNat j) (UInt256.ofNat 8) =
    ...
    rw [UInt256.eq, Challenge.EvmProof.Word.word_toNat_ofNat,
      Challenge.EvmProof.Word.word_toNat_ofNat, hjmod, h8mod, if_neg hne]
  have honeNat : (1 : UInt256).toNat = 1 := by decide
  have hzeroNat : (UInt256.ofNat 0).toNat = 0 := by decide
  simp [innerGuardPath, opAt, pushAt, wfOp, innerLoop, innerBody,
    exponentPCs, hrun, heq, honeNat, hzeroNat, hc11, hc12, hc13,
