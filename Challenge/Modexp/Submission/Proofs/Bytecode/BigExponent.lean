...
/-- Entry to the multiply-only half of a bit step (`pc = 1297`), taken from an
arbitrary machine state.  The hot loop reaches this block after its squaring
and limb copy; the cold path jumps straight here. -/
def bitMulEntry (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) : State :=
  { t with pc := UInt256.ofNat 1297
           stack := bitTailFrame accumulatorWord count b e m baseOff expOff i j
             offset byte rest }

/-- `ACC * BASE mod M` in the scratch region, from an arbitrary entry state. -/
def bitMulReturned (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) : State :=
  mulResult t 2048 1024 3072 0 count 1316
    (bitTailFrame accumulatorWord count b e m baseOff expOff i j offset byte
      rest)

/-- The product copied back into the accumulator region. -/
def bitMulCopyBack (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) : State :=
  BigHelpers.copyReturned
    (bitMulReturned t accumulatorWord count b e m baseOff expOff i j offset byte
      rest)
    2048 3072 count 1289
    (bitTailFrame accumulatorWord count b e m baseOff expOff i j offset byte
      rest)

@[simp] theorem bitMulEntry_executionEnv (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
    (bitMulEntry t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).executionEnv = t.executionEnv := rfl

@[simp] theorem bitMulEntry_halt (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
    (bitMulEntry t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).halt = t.halt := rfl

@[simp] theorem bitMulReturned_executionEnv (t : State)
    (accumulatorWord : UInt256) (count b e m baseOff expOff i j : Nat)
    (offset byte : UInt256) (rest : List UInt256) :
    (bitMulReturned t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).executionEnv = t.executionEnv := by
  simp [bitMulReturned]

@[simp] theorem bitMulReturned_halt (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
    (bitMulReturned t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).halt = t.halt := by
...
@[simp] theorem bitMulReturned_pc (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
    (bitMulReturned t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).pc = UInt256.ofNat 1316 := by
  have h1316 : (1316 : UInt256) = UInt256.ofNat 1316 := by decide
  simpa [bitMulReturned] using h1316

@[simp] theorem bitMulReturned_stack (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
    (bitMulReturned t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).stack =
...
@[simp] theorem bitMulCopyBack_executionEnv (t : State)
    (accumulatorWord : UInt256) (count b e m baseOff expOff i j : Nat)
    (offset byte : UInt256) (rest : List UInt256) :
    (bitMulCopyBack t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).executionEnv = t.executionEnv := by
  simp [bitMulCopyBack, BigHelpers.copyReturned]

@[simp] theorem bitMulCopyBack_halt (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
    (bitMulCopyBack t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).halt = t.halt := by
...
@[simp] theorem bitMulCopyBack_pc (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
    (bitMulCopyBack t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).pc = UInt256.ofNat 1289 := by
  have h1289 : (1289 : UInt256) = UInt256.ofNat 1289 := by decide
  simpa [bitMulCopyBack, BigHelpers.copyReturned] using h1289

@[simp] theorem bitMulCopyBack_stack (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
    (bitMulCopyBack t accumulatorWord count b e m baseOff expOff i j offset byte
      rest).stack =
...
theorem bitProductEntry_eq_bitMulEntry (s : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
    bitProductEntry s accumulatorWord count b e m baseOff expOff i j offset byte
        rest =
      bitMulEntry
        (copiedSquare s accumulatorWord count b e m baseOff expOff i j offset
          byte rest)
        accumulatorWord count b e m baseOff expOff i j offset byte rest := rfl

theorem bitProductReturned_eq_bitMulReturned (s : State)
    (accumulatorWord : UInt256) (count b e m baseOff expOff i j : Nat)
    (offset byte : UInt256) (rest : List UInt256) :
    bitProductReturned s accumulatorWord count b e m baseOff expOff i j offset
        byte rest =
      bitMulReturned
        (copiedSquare s accumulatorWord count b e m baseOff expOff i j offset
          byte rest)
...
theorem bitCopyBack_eq_bitMulCopyBack (s : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) :
...
set_option linter.unusedSimpArgs false in
/-- `run_productCall`, with the multiply-only block entered from an arbitrary
state.  The hot loop instantiates `t` at `copiedSquare`; the cold path
instantiates it at the untouched exponent-entry state. -/
theorem run_productCallAt (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) (hcap : rest.length < 1005)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock productCallPath
      (bitMulEntry t accumulatorWord count b e m baseOff expOff i j offset byte
        rest) =
      some (BigMul.mulEntry t 2048 1024 3072 0 count 1316
        (bitTailFrame accumulatorWord count b e m baseOff expOff i j offset
          byte rest)) := by
  have hc11 : rest.length + 11 < 1024 := by omega
  have hc12 : rest.length + 12 < 1024 := by omega
  ...
  have h310Word : (310 : UInt256) = UInt256.ofNat 310 := by decide
  have hzero : ({ val := 0 } : UInt256) = 0 := by decide
  simp [productCallPath, opAt, pushAt, wfOp, bitMulEntry,
    BigMul.mulEntry, bitTailFrame,
    bitRoutinePCs, hcode, hrun, jump310, h310, h310Word, hzero,
    hc11, hc12, hc13, hc14, hc15, hc16, hc17, hc18, htail,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
...
set_option linter.unusedSimpArgs false in
/-- `run_copyBack`, stated for any state sitting at the multiply return. -/
theorem run_copyBackAt (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) (hcap : rest.length < 1005)
    (hpc : t.pc = UInt256.ofNat 1316)
    (hstack : t.stack = bitTailFrame accumulatorWord count b e m baseOff expOff
      i j offset byte rest)
    (hcode : t.executionEnv.code = submissionBytecode)
    (hrun : t.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock copyBackPath t =
      some (BigHelpers.copyEntry t 2048 3072 count 1289
        (bitTailFrame accumulatorWord count b e m baseOff expOff i j offset
          byte rest)) := by
  have hc11 : rest.length + 11 < 1024 := by omega
  have hc12 : rest.length + 12 < 1024 := by omega
  ...
  have h58Word : (58 : UInt256) = UInt256.ofNat 58 := by decide
  simp [copyBackPath, opAt, pushAt, wfOp, BigHelpers.copyEntry, bitTailFrame,
    bitRoutinePCs, hpc, hstack, hcode, hrun, jump58, h58, h58Word,
    hc11, hc12, hc13, hc14, hc15, hc16, htail,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    Challenge.EvmProof.Word.word_toNat_ofNat,
...
def coldHitPath : List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 1037 .JUMPDEST, pushAt 1038 2 1297, opAt 1039 .JUMP]

@[simp] private theorem coldPCs (i : Nat) (_hi : 995 ≤ i) (_hii : i ≤ 1039) :
    Artifact.submissionArtifact.instructionPC i =
      ([1343,1344,1345,1346,1347,1348,1349,1350,1353,1354,1355,1356,1357,1358,1359,1360,1361,1362,1363,1366,1367,1368,1369,1371,1372,1373,1375,1376,1377,1378,1381,1382,1384,1385,1388,1389,1390,1391,1392,1394,1395,1398,1399,1400,1403])[i - 995]! := by
  interval_cases i <;> decide
...
theorem run_coldHit (s : State) (accumulatorWord : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock coldHitPath
      (coldHitState s accumulatorWord count b e m baseOff expOff i offset byte
        rest j) =
      some (bitMulEntry s accumulatorWord count b e m baseOff expOff i j offset
        byte rest) := by
  have hc11 : rest.length + 11 < 1024 := by omega
  have hc12 : rest.length + 12 < 1024 := by omega
  have h1297 : (1297 : UInt256).toNat = 1297 := by decide
  have h1297Word : (1297 : UInt256) = UInt256.ofNat 1297 := by decide
  simp [coldHitPath, opAt, pushAt, wfOp, coldHitState, coldBitLoop, bitMulEntry,
    bitTailFrame,
    coldPCs, hcode, hrun, jump1297, h1297, h1297Word, hc11, hc12,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    Challenge.EvmProof.Word.word_toNat_ofNat,
    Challenge.EvmProof.Word.ofNat_add_mod,
