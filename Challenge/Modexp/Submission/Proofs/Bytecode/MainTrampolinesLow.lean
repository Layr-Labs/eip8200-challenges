...
private theorem run_tramp0_code (code input : ByteArray)
    (hjump : Decode.isValidJumpDest code 5277 = true) :
    Challenge.EvmProof.Stepper.runLocatedBlock tramp0Path
      (initialState code input 0) =
      some { initialState code input 0 with pc := UInt256.ofNat 5277 } := by
  have hzero : (0 : UInt256).toNat = 0 := by decide
  have hadd := Challenge.EvmProof.Word.ofNat_add_ofNat
    (a := 0) (b := 3) (by norm_num : 0 + 3 < 2 ^ 256)
  have hdest : (5277 : UInt256).toNat = 5277 := by decide
  have hdestWord : (5277 : UInt256) = UInt256.ofNat 5277 := by decide
...
theorem run_tramp0 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock tramp0Path
      (initialState submissionBytecode input 0) = some (trampolineState input 5277) := by
