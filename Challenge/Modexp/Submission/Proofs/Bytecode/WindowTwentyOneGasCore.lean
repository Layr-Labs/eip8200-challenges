...
def steps_pass {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 2479 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 2479 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2342) base modulus rest)
