...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 1791 WindowTwentyOneTableBuild.program
  init : Block artifact fork 1884 WindowTwentyOneInit.program
  entry : Block artifact fork 1905 WindowTwentyOneLoop.entryProgram
  trampoline : Block artifact fork 1905 WindowTwentyOneLoop.trampolineProgram
  body : Block artifact fork 1924 WindowTwentyOneLoop.bodyProgram
  finish : Block artifact fork 2381 WindowTwentyOneReturn.program
...
def steps_pass {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1905 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1905 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1791) base modulus rest)
  ...
  have ht := Block.stepsX paths.table
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1791) base modulus rest)
    (env.transfer rfl rfl) rfl
  ...
  have hi' : runInstructions WindowTwentyOneInit.program
      (WindowTwentyOneTable.framed template (UInt256.ofNat 1884) base modulus 16 ([base, exponent] ++ rest)) =
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 1884) base modulus 16 ([base, exponent] ++ rest))
