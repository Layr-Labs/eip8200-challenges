...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 1785 WindowTwentyOneTableBuild.program
  init : Block artifact fork 1874 WindowTwentyOneInit.program
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1905 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1785) base modulus rest)
  ...
  have ht := Block.stepsX paths.table
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1785) base modulus rest)
    (env.transfer rfl rfl) rfl
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 1874) base modulus 16 rest)
