...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2231 WindowTwentyOneTableBuild.program
  init : Block artifact fork 2347 WindowTwentyOneInit.program
  iteration : Block artifact fork 2368 WindowTwentyOneLoop.iterationProgram
  finish : Block artifact fork 2831 WindowTwentyOneReturn.program
...
def steps_continue {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2368 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2368 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2231) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2231) base modulus rest)
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 2347) base modulus 16 ([base, exponent] ++ rest))
