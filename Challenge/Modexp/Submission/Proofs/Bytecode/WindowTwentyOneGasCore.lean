...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2264 WindowTwentyOneTableBuild.program
  init : Block artifact fork 2380 WindowTwentyOneInit.program
  iteration : Block artifact fork 2401 WindowTwentyOneLoop.iterationProgram
  finish : Block artifact fork 2864 WindowTwentyOneReturn.program
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2401 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2264) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2264) base modulus rest)
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 2380) base modulus 16 ([base, exponent] ++ rest))
