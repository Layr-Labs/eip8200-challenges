...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2340 WindowTwentyOneTableBuild.program
  init : Block artifact fork 2456 WindowTwentyOneInit.program
  entry : Block artifact fork 2477 WindowTwentyOneLoop.entryProgram
  trampoline : Block artifact fork 2477 WindowTwentyOneLoop.trampolineProgram
  body : Block artifact fork 2499 WindowTwentyOneLoop.bodyProgram
  finish : Block artifact fork 2956 WindowTwentyOneReturn.program
...
def steps_pass {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 2477 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 2477 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2340) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2340) base modulus rest)
    (env.transfer rfl rfl) rfl
  ...
  have hi' : runInstructions WindowTwentyOneInit.program
      (WindowTwentyOneTable.framed template (UInt256.ofNat 2456) base modulus 16 ([base, exponent] ++ rest)) =
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 2456) base modulus 16 ([base, exponent] ++ rest))
