...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 1808 WindowTwentyOneTableBuild.program
  init : Block artifact fork 1901 WindowTwentyOneInit.program
  entry : Block artifact fork 1945 WindowTwentyOneLoop.entryProgram
  trampoline : Block artifact fork 1945 WindowTwentyOneLoop.trampolineProgram
  body : Block artifact fork 1967 WindowTwentyOneLoop.bodyProgram
  finish : Block artifact fork 2424 WindowTwentyOneReturn.program
...
def steps_pass {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1945 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1945 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1808) base modulus rest)
  ...
  have ht := Block.stepsX paths.table
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1808) base modulus rest)
    (env.transfer rfl rfl) rfl
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 1901) base modulus 16 ([base, exponent] ++ rest))
