...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2691 WindowTwentyOneTableBuild.program
  init : Block artifact fork 2807 WindowTwentyOneInit.program
  iteration : Block artifact fork 2828 WindowTwentyOneLoop.iterationProgram
  finish : Block artifact fork 3291 WindowTwentyOneReturn.program
...
def steps_continue {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2828 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2828 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2691) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2691) base modulus rest)
  ...
  have hi' : runInstructions WindowTwentyOneInit.program
      (WindowTwentyOneTable.framed template (UInt256.ofNat 2807) base modulus 16 ([base, exponent] ++ rest)) =
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 2807) base modulus 16 ([base, exponent] ++ rest))
