...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2465 WindowTwentyOneTableBuild.program
  init : Block artifact fork 2581 WindowTwentyOneInit.program
  iteration : Block artifact fork 2618 WindowTwentyOneLoop.iterationProgram
  finish : Block artifact fork 3081 WindowTwentyOneReturn.program
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2618 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2465) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2465) base modulus rest)
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 2581) base modulus 16 ([base, exponent] ++ rest))
