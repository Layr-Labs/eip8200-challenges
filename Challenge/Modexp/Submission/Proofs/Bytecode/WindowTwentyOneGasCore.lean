...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 1795 WindowTwentyOneTableBuild.program
  init : Block artifact fork 1888 WindowTwentyOneInit.program
  entry : Block artifact fork 1909 WindowTwentyOneLoop.entryProgram
  trampoline : Block artifact fork 1909 WindowTwentyOneLoop.trampolineProgram
  body : Block artifact fork 1928 WindowTwentyOneLoop.bodyProgram
  finish : Block artifact fork 2385 WindowTwentyOneReturn.program
...
def steps_pass {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1909 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1909 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1795) base modulus rest)
  ...
  have ht := Block.stepsX paths.table
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1795) base modulus rest)
    (env.transfer rfl rfl) rfl
  ...
  have hi' : runInstructions WindowTwentyOneInit.program
      (WindowTwentyOneTable.framed template (UInt256.ofNat 1888) base modulus 16 ([base, exponent] ++ rest)) =
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 1888) base modulus 16 ([base, exponent] ++ rest))
