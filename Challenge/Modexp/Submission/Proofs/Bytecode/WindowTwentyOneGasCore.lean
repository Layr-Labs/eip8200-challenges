...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2662 WindowTwentyOneTableBuild.program
  init : Block artifact fork 2779 WindowTwentyOneInit.program
  iteration : Block artifact fork 2801 WindowTwentyOneLoop.iterationProgram
  finish : Block artifact fork 3264 WindowTwentyOneReturn.program
...
def steps_continue {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2801 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2801 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2662) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2662) base modulus rest)
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.state template (UInt256.ofNat 2779) base modulus exponent 15 rest)
