...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 1792 WindowTwentyOneTableBuild.program
  ...
  finish : Block artifact fork 2380 WindowTwentyOneReturn.program
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1904 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1792) base modulus rest)
      (WindowTwentyOneCore.returnedState template base modulus exponent rest) := by
  have ht := Block.stepsX paths.table
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1792) base modulus rest)
    (env.transfer rfl rfl) rfl
