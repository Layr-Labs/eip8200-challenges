...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2215 WindowTwentyOneTableBuild.program
  init : Block artifact fork 2331 WindowTwentyOneInit.program
  iteration : Block artifact fork 2352 WindowTwentyOneLoop.iterationProgram
  finish : Block artifact fork 2815 WindowTwentyOneReturn.program
...
def steps_continue {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2352 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2352 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2215) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2215) base modulus rest)
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 2331) base modulus 16 ([base, exponent] ++ rest))
