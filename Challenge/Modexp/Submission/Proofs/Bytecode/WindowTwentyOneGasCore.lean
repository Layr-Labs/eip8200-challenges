...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  ...
  init : Block artifact fork 2778 WindowTwentyOneInit.program
  iteration : Block artifact fork 2799 WindowTwentyOneLoop.iterationProgram
  finish : Block artifact fork 3262 WindowTwentyOneReturn.program
...
def steps_continue {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2799 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 2778) base modulus 16 ([base, exponent] ++ rest))
