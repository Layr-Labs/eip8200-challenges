...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 1804 WindowTwentyOneTableBuild.program
  init : Block artifact fork 1897 WindowTwentyOneInit.program
  entry : Block artifact fork 1941 WindowTwentyOneLoop.entryProgram
  trampoline : Block artifact fork 1941 WindowTwentyOneLoop.trampolineProgram
  body : Block artifact fork 1960 WindowTwentyOneLoop.bodyProgram
  finish : Block artifact fork 2417 WindowTwentyOneReturn.program
...
def steps_pass {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1941 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (htramp : Decode.isValidJumpDest template.executionEnv.code 1941 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1804) base modulus rest)
  ...
  have ht := Block.stepsX paths.table
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1804) base modulus rest)
    (env.transfer rfl rfl) rfl
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 1897) base modulus 16 ([base, exponent] ++ rest))
