...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2086 WindowTwentyOneTableBuild.program
  init : Block artifact fork 2202 WindowTwentyOneInit.program
  iteration : Block artifact fork 2227 WindowTwentyOneLoop.iterationProgram
  finish : Block artifact fork 2690 WindowTwentyOneReturn.program
...
def steps_continue {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2227 = true) :
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2227 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2086) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2086) base modulus rest)
  ...
  have hi' : runInstructions WindowTwentyOneInit.program
      (WindowTwentyOneTable.framed template (UInt256.ofNat 2202) base modulus 16 ([base, exponent] ++ rest)) =
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 2202) base modulus 16 ([base, exponent] ++ rest))
