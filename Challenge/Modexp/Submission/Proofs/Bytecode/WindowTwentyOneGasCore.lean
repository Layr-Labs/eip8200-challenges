...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2821 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2682) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2682) base modulus rest)
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.state template (UInt256.ofNat 2799) base modulus exponent 15 rest)
