...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  table : Block artifact fork 2652 WindowTwentyOneTableBuild.program
  init : Block artifact fork 2768 WindowTwentyOneInit.program
  iteration : Block artifact fork 2789 WindowTwentyOneLoop.iterationProgram
  finish : Block artifact fork 3252 WindowTwentyOneReturn.program
...
def steps_core {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2789 = true) :
    GasSteps (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2652) base modulus rest)
  ...
  have ht := paths.table.steps
    (s := WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2652) base modulus rest)
  ...
  have hinit := paths.init.steps
    (s := WindowTwentyOneTable.framed template (UInt256.ofNat 2768) base modulus 16 ([base, exponent] ++ rest))
