...
@[simp] theorem bitStepProgress_callStack (s : State)
    (accumulatorWord : UInt256) (count b e m baseOff expOff i j : Nat)
    (offset byte : UInt256) (rest : List UInt256) :
    (BigExponent.bitStepProgress s accumulatorWord count b e m baseOff expOff
      i j offset byte rest).callStack = s.callStack := by
...
@[simp] theorem bitMulReturned_callStack (t : State)
    (accumulatorWord : UInt256) (count b e m baseOff expOff i j : Nat)
    (offset byte : UInt256) (rest : List UInt256) :
    (BigExponent.bitMulReturned t accumulatorWord count b e m baseOff expOff i j
      offset byte rest).callStack = t.callStack := by
  simp [BigExponent.bitMulReturned]

@[simp] theorem bitMulCopyBack_callStack (t : State)
    (accumulatorWord : UInt256) (count b e m baseOff expOff i j : Nat)
    (offset byte : UInt256) (rest : List UInt256) :
    (BigExponent.bitMulCopyBack t accumulatorWord count b e m baseOff expOff i j
      offset byte rest).callStack = t.callStack := by
  simp [BigExponent.bitMulCopyBack, BigHelpers.copyReturned]
