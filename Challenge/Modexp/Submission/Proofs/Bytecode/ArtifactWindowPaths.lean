...
private def fermat_prime : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4877 FermatProgram.primeProgram :=
  WindowTwentyOneSlice.block allWellFormed 3728 14 4877 FermatProgram.primeProgram (by decide) (by rfl) (by rfl) (by decide)
private def fermat_exponent : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4930 FermatProgram.exponentProgram :=
  WindowTwentyOneSlice.block allWellFormed 3742 9 4930 FermatProgram.exponentProgram (by decide) (by rfl) (by rfl) (by decide)
private def fermat_result : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4942 FermatProgram.returnProgram :=
  WindowTwentyOneSlice.block allWellFormed 3751 16 4942 FermatProgram.returnProgram (by decide) (by rfl) (by rfl) (by decide)
private def fermat_miss : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4961 FermatProgram.missProgram :=
  WindowTwentyOneSlice.block allWellFormed 3767 4 4961 FermatProgram.missProgram (by decide) (by rfl) (by rfl) (by decide)
def fermatPaths : FermatProgram.Paths submissionArtifact .Osaka where
  prime := fermat_prime
  exponent := fermat_exponent
  result := fermat_result
  miss := fermat_miss
  missJump := by exact isValidJumpDest_index 3767 (by rfl)
...
def twentyOnePaths : WindowTwentyOneGasRoute.Paths submissionArtifact .Osaka where
  ...
  hitJump := by
    have h := isValidJumpDest_index 3728 (by rfl)
    exact h
  emptyJump := by
