...
private def fermat_prime : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4882 FermatProgram.primeProgram :=
  WindowTwentyOneSlice.block allWellFormed 3763 14 4882 FermatProgram.primeProgram (by decide) (by rfl) (by rfl) (by decide)
private def fermat_exponent : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4935 FermatProgram.exponentProgram :=
  WindowTwentyOneSlice.block allWellFormed 3777 9 4935 FermatProgram.exponentProgram (by decide) (by rfl) (by rfl) (by decide)
private def fermat_result : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4947 FermatProgram.returnProgram :=
  WindowTwentyOneSlice.block allWellFormed 3786 16 4947 FermatProgram.returnProgram (by decide) (by rfl) (by rfl) (by decide)
private def fermat_miss : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4966 FermatProgram.missProgram :=
  WindowTwentyOneSlice.block allWellFormed 3802 4 4966 FermatProgram.missProgram (by decide) (by rfl) (by rfl) (by decide)
def fermatPaths : FermatProgram.Paths submissionArtifact .Osaka where
  prime := fermat_prime
  exponent := fermat_exponent
  result := fermat_result
  miss := fermat_miss
  missJump := by exact isValidJumpDest_index 3802 (by rfl)
...
def twentyOnePaths : WindowTwentyOneGasRoute.Paths submissionArtifact .Osaka where
  ...
  hitJump := by
    have h := isValidJumpDest_index 3763 (by rfl)
    exact h
  emptyJump := by
