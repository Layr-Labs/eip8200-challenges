...
private def fermat_prime : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4903 FermatProgram.primeProgram :=
  WindowTwentyOneSlice.block allWellFormed 3673 14 4903 FermatProgram.primeProgram (by decide) (by rfl) (by rfl) (by decide)
private def fermat_exponent : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4956 FermatProgram.exponentProgram :=
  WindowTwentyOneSlice.block allWellFormed 3687 9 4956 FermatProgram.exponentProgram (by decide) (by rfl) (by rfl) (by decide)
private def fermat_result : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4968 FermatProgram.returnProgram :=
  WindowTwentyOneSlice.block allWellFormed 3696 16 4968 FermatProgram.returnProgram (by decide) (by rfl) (by rfl) (by decide)
private def fermat_miss : WindowTwentyOneBinding.Block submissionArtifact .Osaka 4987 FermatProgram.missProgram :=
  WindowTwentyOneSlice.block allWellFormed 3712 4 4987 FermatProgram.missProgram (by decide) (by rfl) (by rfl) (by decide)
def fermatPaths : FermatProgram.Paths submissionArtifact .Osaka where
  prime := fermat_prime
  exponent := fermat_exponent
  result := fermat_result
  miss := fermat_miss
  missJump := by exact isValidJumpDest_index 3712 (by rfl)
...
def twentyOnePaths : WindowTwentyOneGasRoute.Paths submissionArtifact .Osaka where
  ...
  hitJump := by
    have h := isValidJumpDest_index 3673 (by rfl)
    exact h
  emptyJump := by
