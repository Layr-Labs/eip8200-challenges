...
Each of the three passes enters the trampoline at 2482 (from the loop head `PUSH2 2482
JUMP` at 2482 on the first pass, from the tail's `JUMPI` afterwards).  The trampoline
stores the two exponent copies of the pass's shifted exponent above the table, replays
the first five staging instructions and jumps back to the `JUMPDEST` at 2504, where the
...
def entryState (template : State) (base modulus exponent : UInt256)
  ...
  WindowTwentyOneGroup.state template (UInt256.ofNat 2482) (WindowTableMemory.tableMemory base modulus) 16
...
def loopState (template : State) (base modulus exponent : UInt256)
  ...
  WindowTwentyOneGroup.state template (UInt256.ofNat 2482) (loopMem base modulus exponent count)
...
/-- Body entry (2504) of pass `count`, with the pass's copies stored. -/
def headState (template : State) (base modulus exponent : UInt256)
    (count : Nat) (rest : List UInt256) : State :=
  WindowTwentyOneGroup.headState template (UInt256.ofNat 2504) (loopMem base modulus exponent (count + 1))
...
def finishState (template : State) (base modulus exponent : UInt256)
    (rest : List UInt256) : State :=
  WindowTwentyOneGroup.state template (UInt256.ofNat 2961) (loopMem base modulus exponent 3) 19 modulus
...
theorem run_entry (template : State) (base modulus exponent : UInt256)
    ...
    (_hjump : Decode.isValidJumpDest template.executionEnv.code 2482 = true) :
...
theorem run_trampoline (template : State) (base modulus exponent : UInt256)
  ...
  have hs := run_setup template (UInt256.ofNat 2482) mem (loopActive count) hactive
    modulus a e c rest hrest
  let core := WindowTwentyOneLookup.framed template (advancePC 17 (UInt256.ofNat 2482))
    (WindowCopyMemory.copyMem mem e) 19 []
  have hh := WindowTwentyOneStage.run_stageHead core (advancePC 17 (UInt256.ofNat 2482))
    a modulus e (UInt256.ofNat 480) c rest hrest
  have hh' : runInstructions WindowTwentyOneStage.stageHead
      (WindowTwentyOneLookup.framed template (advancePC 17 (UInt256.ofNat 2482))
        (WindowCopyMemory.copyMem mem e) 19 ([a, modulus, e, UInt256.ofNat 480, c] ++ rest)) =
      some (WindowTwentyOneLookup.framed template (advancePC 5 (advancePC 17 (UInt256.ofNat 2482)))
        (WindowCopyMemory.copyMem mem e) 19
  ...
  have hpc : advancePC 5 (advancePC 17 (UInt256.ofNat 2482)) = UInt256.ofNat 2504 := by decide
...
theorem run_body (template : State) (base modulus exponent : UInt256)
    ...
    (rest : List UInt256) (hrest : rest.length ≤ 1000)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2482 = true) :
    runInstructions bodyProgram (headState template base modulus exponent count rest) =
    some (WindowTwentyOneGroup.state template
      (if UInt256.isTrue (UInt256.ofNat (2 - count)) then UInt256.ofNat 2482 else UInt256.ofNat 2961)
  ...
  have hb := WindowTwentyOneBody.run_twentyOne template (UInt256.ofNat 2504) mem
    base modulus a exponent c (loopMem_table base modulus exponent count)
    (1 + 21 * count) (by omega) rest hrest
  have ht := WindowTwentyOneTail.run_tail template (UInt256.ofNat 2947) (UInt256.ofNat 2482)
    (WindowCopyMemory.copyMem mem e) 19 modulus nextA e c rest hrest
    (by rw [show (UInt256.ofNat 2482).toNat = 2482 from rfl]; exact hjump)
  rw [ha, advancePC_ofNat] at hb
  rw [advancePC_ofNat] at ht
  have hall := runInstructions_append_some _ _ _ _ _ hb ht
  simpa only [bodyProgram, headState, loopMem, mem, a, e, c, nextA, eAt] using hall
