...
def bodyProgram : List Instr :=
  WindowTwentyOneBody.program ++ WindowTwentyOneTail.program (UInt256.ofNat 1780)
...
def entryState (template : State) (base modulus exponent : UInt256)
  ...
  WindowTwentyOneGroup.state template (UInt256.ofNat 1780) (WindowTableMemory.tableMemory base modulus) 16
...
def loopState (template : State) (base modulus exponent : UInt256)
  ...
  WindowTwentyOneGroup.state template (UInt256.ofNat 1780) (loopMem base modulus exponent count)
...
def headState (template : State) (base modulus exponent : UInt256)
    (count : Nat) (rest : List UInt256) : State :=
  WindowTwentyOneGroup.headState template (UInt256.ofNat 1799) (loopMem base modulus exponent (count + 1))
...
def finishState (template : State) (base modulus exponent : UInt256)
    (rest : List UInt256) : State :=
  WindowTwentyOneGroup.state template (UInt256.ofNat 2256) (loopMem base modulus exponent 3) 18 modulus
...
theorem run_entry (template : State) (base modulus exponent : UInt256)
    ...
    (_hjump : Decode.isValidJumpDest template.executionEnv.code 1780 = true) :
...
theorem run_trampoline (template : State) (base modulus exponent : UInt256)
  ...
  have hs := run_setup template (UInt256.ofNat 1780) mem (loopActive count) hactive
    modulus a e c rest hrest
  let core := WindowTwentyOneLookup.framed template (advancePC 14 (UInt256.ofNat 1780))
    (WindowCopyMemory.copyMem mem e) 18 []
  have hh := WindowTwentyOneStage.run_stageHead core (advancePC 14 (UInt256.ofNat 1780))
    a modulus e (UInt256.ofNat 480) c rest hrest
  have hh' : runInstructions WindowTwentyOneStage.stageHead
      (WindowTwentyOneLookup.framed template (advancePC 14 (UInt256.ofNat 1780))
        (WindowCopyMemory.copyMem mem e) 18 ([a, modulus, e, UInt256.ofNat 480, c] ++ rest)) =
      some (WindowTwentyOneLookup.framed template (advancePC 5 (advancePC 14 (UInt256.ofNat 1780)))
        (WindowCopyMemory.copyMem mem e) 18
  ...
  have hpc : advancePC 5 (advancePC 14 (UInt256.ofNat 1780)) = UInt256.ofNat 1799 := by decide
...
theorem run_body (template : State) (base modulus exponent : UInt256)
    ...
    (rest : List UInt256) (hrest : rest.length ≤ 1000)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 1780 = true) :
    runInstructions bodyProgram (headState template base modulus exponent count rest) =
    some (WindowTwentyOneGroup.state template
      (if UInt256.isTrue (UInt256.ofNat (2 - count)) then UInt256.ofNat 1780 else UInt256.ofNat 2256)
  ...
  have hb := WindowTwentyOneBody.run_twentyOne template (UInt256.ofNat 1799) mem
    base modulus a exponent c (loopMem_table base modulus exponent count)
    (1 + 21 * count) (by omega) (by omega) rest hrest
  have ht := WindowTwentyOneTail.run_tail template (UInt256.ofNat 2242) (UInt256.ofNat 1780)
    (WindowCopyMemory.copyMem mem e) 18 modulus nextA e c rest hrest
    (by rw [show (UInt256.ofNat 1780).toNat = 1780 from rfl]; exact hjump)
