...
theorem selectSkippedProgress_represents_bitStep (s : State)
    ...
    Limbs.Represents
        (selectSkippedProgress s accumulatorWord count b e m baseOff expOff i
          j offset byte rest count).memory 2048 count
        (bitStepValue modulus byte j acc base) ∧
      Limbs.Represents
        (selectSkippedProgress s accumulatorWord count b e m baseOff expOff i
          j offset byte rest count).memory 1024 count base ∧
      Limbs.Represents
        (selectSkippedProgress s accumulatorWord count b e m baseOff expOff i
          j offset byte rest count).memory 0 count modulus := by
  ...
  have hword : UInt256.ofNat (exponentBit byte j).toNat =
      exponentBit byte j := WordCorrect.ofNat_toNat _
  have hsq : bitStepValue modulus byte j acc base = squareValue := by
    simp [bitStepValue, hbit, squareValue]
  have hselected : Limbs.Represents
      (selectMemory copied.memory (selectMask byte j) count) 2048 count
      squareValue := by
    rw [selectMask, ← hword, hbit,
      selectMemory_zero copied.memory count hcount]
    exact copyMemory_self_represents copied.memory 2048 count squareValue
      (by omega) hcopiedSquare
  have hselectedBase := selectMemory_preserves_region copied.memory byte j
    count 1024 base hcount (Or.inr (by omega)) hcopiedBase
  have hselectedModulus := selectMemory_preserves_region copied.memory byte j
    count 0 modulus hcount (Or.inr (by omega)) hcopiedModulus
  have hmem : (selectSkippedProgress s accumulatorWord count b e m baseOff
      expOff i j offset byte rest count).memory =
      selectMemory copied.memory (selectMask byte j) count := rfl
  rw [hsq]
  exact ⟨by simpa [hmem] using hselected,
    by simpa [hmem] using hselectedBase,
    by simpa [hmem] using hselectedModulus⟩

theorem selectProgress_represents_bitStep (s : State)    (accumulatorWord : UInt256) (count b e m baseOff expOff i j : Nat)
    (offset byte : UInt256) (rest : List UInt256) (acc base modulus : Nat)
    (hcount : count ≤ 32) (hmodulusPos : 0 < modulus)
    (haccReduced : acc < modulus)
    (hacc : Limbs.Represents s.memory 2048 count acc)
  ...
  have h1000 : (1000 : UInt256) = UInt256.ofNat 1000 := by decide
  have h1015 : (1015 : UInt256) = UInt256.ofNat 1015 := by decide
  have h1034 : (1040 : UInt256) = UInt256.ofNat 1040 := by decide
  have hbodyAcc : Limbs.Represents body.memory 2048 count acc := by
    simpa [body, innerBody, innerLoop] using hacc
  have hbodyBase : Limbs.Represents body.memory 1024 count base := by
    simpa [body, innerBody, innerLoop] using hbase
  ...
  have hproduct : Limbs.Represents product.memory 3072 count productValue := by
    simpa [product, productReturned, mulResult, squareValue, productValue,
      ...
      BigMul.mulReturned_represents_product copied 1024 count squareValue base
        modulus (UInt256.ofNat 1040) frame hcount (by omega) hmodulusPos
  ...
  have hproductBase : Limbs.Represents product.memory 1024 count base := by
    simpa [product, productReturned, mulResult, squareValue, frame, h0, h1024,
      ...
      (BigMul.mulOuterProgress_afterCopy_represents_product copied 1024 count
        squareValue base modulus (UInt256.ofNat 1040) frame hcount (by omega)
    ...
    (by
      simpa [product, productReturned, squareValue, frame, h0, h1024, h2048,
        ...
        mulResult_preserves_region copied (UInt256.ofNat 2048)
          (UInt256.ofNat 1024) count 2048 squareValue (UInt256.ofNat 1040)
  ...
  | succ steps ih =>
      ...
      have hbeforeReduced : beforeValue < modulus :=
        WordCorrect.natBitAfter_lt modulus byte base acc steps hmodulusPos
          haccReduced
      by_cases hbit : (exponentBit byte steps).toNat = 0
      · have hstep := selectSkippedProgress_represents_bitStep before
          ...
          hbefore.2.1 hbefore.2.2 hbit
        simpa [BigExponent.exponentBitProgress, before, beforeValue,
          WordCorrect.natBitAfter, BigExponent.bitStepProgress, hbit,
