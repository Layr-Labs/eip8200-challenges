...
/-- `oneMod_pow` at `N = 2`: squaring `1 % m` is the identity, uniformly in
`m`.  This is exactly the work the cold hand-off skips. -/
theorem oneMod_mul_self (modulus : Nat) :
    (1 % modulus) * (1 % modulus) % modulus = 1 % modulus := by
  have h := oneMod_pow modulus 2
  rwa [show (1 % modulus) ^ 2 = (1 % modulus) * (1 % modulus) from by
    simp [Nat.pow_succ]] at h

/-- With a zero bit prefix and a set bit at `j0`, running the first `j0 + 1`
bits over `1 % modulus` is just one modular multiplication by the base: the
squaring at bit `j0` is the identity. -/
theorem natBitAfter_succ_of_zero_prefix (modulus : Nat) (byte : UInt256)
    (base j0 : Nat) (hmodulusPos : 0 < modulus)
    (hz : ∀ t, t < j0 → WordCorrect.exponentBitNat byte t = 0)
    (hhit : ¬ WordCorrect.exponentBitNat byte j0 = 0) :
    WordCorrect.natBitAfter modulus byte base (j0 + 1) (1 % modulus) =
      (1 % modulus) * base % modulus := by
  rw [WordCorrect.natBitAfter,
    natBitAfter_of_zero_prefix modulus byte base j0 hmodulusPos hz]
  unfold WordCorrect.natBitStep
  rw [if_neg hhit, oneMod_mul_self]
...
/-- The multiply-only half of a bit step multiplies the accumulator region by
the base region, and leaves the base and the modulus alone. -/
theorem bitMulCopyBack_represents (t : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (offset byte : UInt256)
    (rest : List UInt256) (acc base modulus : Nat)
    (hcount : count ≤ 32) (hmodulusPos : 0 < modulus)
    (haccReduced : acc < modulus)
    (hacc : Limbs.Represents t.memory 2048 count acc)
    (hbase : Limbs.Represents t.memory 1024 count base)
    (hmodulus : Limbs.Represents t.memory 0 count modulus) :
    Limbs.Represents
        (BigExponent.bitMulCopyBack t accumulatorWord count b e m baseOff expOff
          i j offset byte rest).memory 2048 count ((acc * base) % modulus) ∧
      Limbs.Represents
        (BigExponent.bitMulCopyBack t accumulatorWord count b e m baseOff expOff
          i j offset byte rest).memory 1024 count base ∧
      Limbs.Represents
        (BigExponent.bitMulCopyBack t accumulatorWord count b e m baseOff expOff
          i j offset byte rest).memory 0 count modulus := by
  let tail := bitTailFrame accumulatorWord count b e m baseOff expOff i j
    offset byte rest
  let product := BigExponent.bitMulReturned t accumulatorWord count b e m
  ...
  have h1316 : (1316 : UInt256) = UInt256.ofNat 1316 := by decide
  have hproduct :
      Limbs.Represents product.memory 3072 count ((acc * base) % modulus) := by
    simpa [product, BigExponent.bitMulReturned, mulResult, tail, h0, h1024,
      h2048, h3072, h1316] using
      BigMul.mulReturned_represents_product t 1024 count acc base modulus
        (UInt256.ofNat 1316) tail hcount (by omega) hmodulusPos haccReduced
        hacc hbase hmodulus
  have hproductBase : Limbs.Represents product.memory 1024 count base := by
    simpa [product, BigExponent.bitMulReturned, mulResult, tail, h0, h1024,
      h2048, h3072, h1316, BigMul.mulReturned] using
      (BigMul.mulOuterProgress_afterCopy_represents_product t 1024 count acc
        base modulus (UInt256.ofNat 1316) tail hcount (by omega) hmodulusPos
        haccReduced hacc hbase hmodulus).2.1
  have hproductModulus : Limbs.Represents product.memory 0 count modulus := by
    simpa [product, BigExponent.bitMulReturned, mulResult, tail, h0, h1024,
      h2048, h3072, h1316, BigMul.mulReturned] using
      (BigMul.mulOuterProgress_afterCopy_represents_product t 1024 count acc
        base modulus (UInt256.ofNat 1316) tail hcount (by omega) hmodulusPos
        haccReduced hacc hbase hmodulus).2.2
  refine ⟨?_, ?_, ?_⟩
  · simpa [BigExponent.bitMulCopyBack, BigHelpers.copyReturned, product, tail,
      h2048, h3072, h1289] using
      BigHelpers.copyMemory_represents product.memory 2048 3072 count
        ((acc * base) % modulus) hproduct (by omega) (by omega)
        (Or.inl (by omega))
  · simpa [BigExponent.bitMulCopyBack, BigHelpers.copyReturned, product, tail,
      h2048, h3072, h1289] using
      BigHelpers.represents_copyMemory_disjoint_region product.memory 2048 3072
        1024 count base (by omega) (Or.inr (by omega)) hproductBase
  · simpa [BigExponent.bitMulCopyBack, BigHelpers.copyReturned, product, tail,
      h2048, h3072, h1289] using
      BigHelpers.represents_copyMemory_disjoint_region product.memory 2048 3072
        0 count modulus (by omega) (Or.inr (by omega)) hproductModulus

/-- The cold hand-off.  The search writes no memory, so all three regions reach
the multiply block unchanged; the block then performs the one modular
multiplication the first set bit needs.  The squaring the hot loop would have
run before it is the identity, because the accumulator still holds
`1 % modulus`. -/
theorem coldPhaseHit_represents (s : State) (accumulatorWord : UInt256)
  ...
  have hbyteLt : (BigExponent.coldPhaseByte s expOff e).toNat < 256 := by
    simpa [BigExponent.coldPhaseByte] using
      BigExponent.loadedExponentByte_lt256 s expOff
        (BigExponent.coldByteIndex s expOff e)
  have hbyteNz : ¬ (BigExponent.coldPhaseByte s expOff e).toNat = 0 := by
    simpa [BigExponent.coldPhaseByte] using
      BigExponent.coldByteIndex_hit s expOff e hklt
  have hj0lt : BigExponent.coldPhaseBit s expOff e < 8 :=
    BigExponent.coldBitIndex_lt (BigExponent.coldPhaseByte s expOff e) hbyteLt
      hbyteNz
  have hbitnz : ¬ WordCorrect.exponentBitNat
      (BigExponent.coldPhaseByte s expOff e)
      (BigExponent.coldPhaseBit s expOff e) = 0 := by
    rw [← BigExponent.exponentBit_toNat_eq_bitNat _ _ hj0lt]
    exact BigExponent.coldBitIndex_hit (BigExponent.coldPhaseByte s expOff e)
      hj0lt
  have hzeros : ∀ t, t < BigExponent.coldPhaseBit s expOff e →
      WordCorrect.exponentBitNat (BigExponent.coldPhaseByte s expOff e) t = 0 := by
    intro t ht
  ...
  have hval : WordCorrect.natBitAfter modulus
      (BigExponent.coldPhaseByte s expOff e) base
      (BigExponent.coldPhaseStart s expOff e) (1 % modulus) =
      (1 % modulus) * base % modulus := by
    show WordCorrect.natBitAfter modulus (BigExponent.coldPhaseByte s expOff e)
      base (BigExponent.coldPhaseBit s expOff e + 1) (1 % modulus) = _
    exact natBitAfter_succ_of_zero_prefix modulus
      (BigExponent.coldPhaseByte s expOff e) base
      (BigExponent.coldPhaseBit s expOff e) hmodulusPos hzeros hbitnz
  rw [hval]
  simpa [BigExponent.coldPhaseHit] using
    bitMulCopyBack_represents s accumulatorWord count b e m baseOff expOff
      (BigExponent.coldByteIndex s expOff e)
      (BigExponent.coldPhaseBit s expOff e)
      (BigExponent.coldPhaseOffset s expOff e)
      (BigExponent.coldPhaseByte s expOff e) rest (1 % modulus) base modulus
      hcount hmodulusPos (Nat.mod_lt _ hmodulusPos) hacc hbase hmodulus
