...
def guardPath :
  ...
  [Main.opAt 1820 .JUMPDEST,
   Main.opAt 1821 (.Dup ⟨0, by decide⟩),
   Main.pushAt 1822 1 32,
   Main.opAt 1823 .XOR,
   Main.opAt 1824 (.Dup ⟨2, by decide⟩),
   Main.pushAt 1825 1 32,
   Main.opAt 1826 .XOR,
   Main.opAt 1827 .OR,
   Main.opAt 1828 (.Dup ⟨3, by decide⟩),
   Main.pushAt 1829 1 32,
   Main.opAt 1830 .XOR,
   Main.opAt 1831 .OR,
   Main.opAt 1832 .ISZERO]

def branchPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [Main.pushAt 1833 2 3019, Main.opAt 1834 .JUMPI]

def missPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [Main.pushAt 1835 2 517, Main.opAt 1836 .JUMP]
...
@[simp] theorem routePCs (i : Nat)
    (hlo : 1820 ≤ i) (hhi : i ≤ 1837) :
    Artifact.submissionArtifact.instructionPC i =
      ([2995,2996,2997,2999,3000,3001,3003,3004,3005,3006,3008,3009,3010,3011,3014,3015,3018,3019] : List Nat)[i - 1820]! := by
