...
def expFinishTailPath :
  ...
  [opAt 533 .JUMPDEST, opAt 534 .POP, opAt 535 (.Dup ⟨0, by decide⟩),
   opAt 536 (.Dup ⟨6, by decide⟩), pushAt 537 1 32, opAt 538 .SUB,
   pushAt 539 1 3, opAt 540 .SHL, opAt 541 .SHL,
   pushAt 542 0 0, opAt 543 .MSTORE,
   opAt 544 (.Dup ⟨5, by decide⟩), pushAt 545 4 0, opAt 546 .RETURN]

def expFinishDispatchState (input : ByteArray) (acc base : UInt256) : State :=
  { expLoopState input (exponentSize input) acc base with pc := UInt256.ofNat 665 }
...
@[simp] private theorem exitPCs (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC i =
      [665,666,667,668,669,671,672,674,675,676,677,678,679,684][i - 533]! := by
...
theorem run_expFinishGuard (input : ByteArray) (acc base : UInt256)
  ...
  have heq : UInt256.eq (UInt256.ofNat (exponentSize input))
    ...
    simp [UInt256.eq]
  have h669 : (665 : UInt256).toNat = 665 := by decide
  have h669Word : (665 : UInt256) = UInt256.ofNat 665 := by decide
