...
def expFinishTailPath :
  ...
  [opAt 505 .JUMPDEST, opAt 506 .POP, opAt 507 (.Dup ⟨0, by decide⟩),
   opAt 508 (.Dup ⟨6, by decide⟩), pushAt 509 1 32, opAt 510 .SUB,
   pushAt 511 1 3, opAt 512 .SHL, opAt 513 .SHL,
   pushAt 514 0 0, opAt 515 .MSTORE,
   opAt 516 (.Dup ⟨5, by decide⟩), pushAt 517 0 0, opAt 518 .RETURN]

def expFinishDispatchState (input : ByteArray) (acc base : UInt256) : State :=
  { expLoopState input (exponentSize input) acc base with pc := UInt256.ofNat 614 }
...
@[simp] private theorem exitPCs (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC i =
      ([614,615,616,617,618,620,621,623,624,625,626,627,628,629] : List Nat)[i - 505]! := by
...
theorem run_expFinishGuard (input : ByteArray) (acc base : UInt256)
  ...
  have heq : UInt256.eq (UInt256.ofNat (exponentSize input))
    ...
    simp [UInt256.eq]
  have h669 : (614 : UInt256).toNat = 614 := by decide
  have h669Word : (614 : UInt256) = UInt256.ofNat 614 := by decide
