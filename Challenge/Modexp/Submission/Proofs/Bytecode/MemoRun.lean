...
`Main.trampolineState input 553` exactly: empty stack, untouched memory, only the
...
def legacyState (input : ByteArray) : State := Main.trampolineState input 553
...
private def widthMidState (input : ByteArray) : State :=
  { Main.trampolineState input 809 with
      stack := [UInt256.ofNat 553, MachineState.readWord input 68 - (137506062208 : UInt256)] }
...
private theorem run_width_prefix (input : ByteArray) :
  ...
  have h598 : (553 : UInt256).toNat = 553 := by decide
  have h598Word : (553 : UInt256) = UInt256.ofNat 553 := by decide
