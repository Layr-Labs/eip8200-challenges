...
/-- The multiply-only half of a set-bit step, entered from an arbitrary state:
build the `mulModBig` frame, multiply the accumulator by the base, copy the
product back, and fall into the inner loop at the next bit.  The hot loop
reaches this after its squaring; the cold path jumps straight to it. -/
def gasSteps_bitMulTail (t : State) (accumulatorWord : UInt256)
    ...
    (hfork : t.fork = .Osaka) (hrun : t.halt = .Running)
    ...
    Challenge.EvmProof.GasSteps
      (bitMulEntry t accumulatorWord count b e m baseOff expOff i j offset byte
        rest)
      (innerLoop
        (bitMulCopyBack t accumulatorWord count b e m baseOff expOff i j offset
          byte rest)
  ...
  have hcall := Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka productCallPath
      ...
      (run_productCallAt t accumulatorWord count b e m baseOff expOff i j offset
        byte rest (by omega) hcode hrun)
      (by simpa using hrun)
      (by simpa [State.fork] using hnp)
  have hproductRaw := BigMul.gasSteps_mulModBig t 2048 1024 3072 0 count 1316
    tail htail hcount hcode hfork hrun hnp jump1316
  have hproduct : Challenge.EvmProof.GasSteps
      (BigMul.mulEntry t 2048 1024 3072 0 count 1316 tail)
      (bitMulReturned t accumulatorWord count b e m baseOff expOff i j offset
        byte rest) := by
    simpa [bitMulReturned, mulResult, tail] using hproductRaw
  have hpcMul : (bitMulReturned t accumulatorWord count b e m baseOff expOff i j
      offset byte rest).pc = UInt256.ofNat 1316 := by simp
  have hstackMul : (bitMulReturned t accumulatorWord count b e m baseOff expOff
      i j offset byte rest).stack = tail := by simp [tail]
  have hcopyBack := Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka copyBackPath
      ...
      (run_copyBackAt
        ...
        (by omega) hpcMul hstackMul (by simpa using hcode)
        (by simpa using hrun))
      (by simpa using hrun)
      (by simpa [State.fork] using hnp)
  ...
  have hcopy : Challenge.EvmProof.GasSteps
      (BigHelpers.copyEntry
        (bitMulReturned t accumulatorWord count b e m baseOff expOff i j offset
          byte rest) 2048 3072 count 1289 tail)
      (bitMulCopyBack t accumulatorWord count b e m baseOff expOff i j offset
        byte rest) := by
    simpa [bitMulCopyBack, tail] using hcopyRaw
  have hpc : (bitMulCopyBack t accumulatorWord count b e m baseOff expOff i j
      offset byte rest).pc = UInt256.ofNat 1289 := by simp
  have hstackEq : (bitMulCopyBack t accumulatorWord count b e m baseOff expOff i
      j offset byte rest).stack =
    ...
    Artifact.submissionArtifact .Osaka tailPath
      ...
      (run_tail
        (bitMulCopyBack t accumulatorWord count b e m baseOff expOff i j offset
          byte rest)
        accumulatorWord count b e m baseOff expOff i j offset byte rest
        (by omega) hpc hstackEq (by simpa using hcode) (by simpa using hrun))
      (by simpa using hrun)
      (by simpa [State.fork] using hnp)
  exact hcall.trans (hproduct.trans (hcopyBack.trans (hcopy.trans hstep)))
...
def gasSteps_coldHit (s : State) (accumulatorWord : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (coldBitLoop s accumulatorWord count b e m baseOff expOff i offset byte
        rest j)
      (bitMulEntry s accumulatorWord count b e m baseOff expOff i j offset byte
        rest) :=
  (Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka coldBitComputePath
      (by simpa [coldBitLoop, Artifact.submissionArtifact] using hcode)
...
/-- The cold search writes no memory, so the exponent-entry state reaches the
multiply block unchanged; the state below is what that multiply leaves behind.
The squaring the hot loop would have run first is skipped because the
accumulator is still `1 % modulus`, whose square is itself. -/
def coldPhaseHit (s : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff : Nat) (rest : List UInt256) : State :=
  bitMulCopyBack s accumulatorWord count b e m baseOff expOff
    (coldByteIndex s expOff e) (coldPhaseBit s expOff e)
    (coldPhaseOffset s expOff e) (coldPhaseByte s expOff e) rest

/-- The first set bit is fully processed by the cold hand-off, so the hot inner
loop resumes at the bit after it. -/
def coldPhaseStart (s : State) (expOff e : Nat) : Nat :=
  coldPhaseBit s expOff e + 1
...
theorem coldPhaseStart_le_eight (s : State) (expOff e : Nat)
    (hbyte : (coldPhaseByte s expOff e).toNat < 256)
    (hnz : ¬ (coldPhaseByte s expOff e).toNat = 0) :
    coldPhaseStart s expOff e ≤ 8 := by
  have hj0 : coldPhaseBit s expOff e < 8 :=
    coldBitIndex_lt (coldPhaseByte s expOff e) hbyte hnz
  simp only [coldPhaseStart]
  omega
...
def gasSteps_exponentPhase (s : State) (accumulatorWord : UInt256)
  ...
  · have hklt : coldByteIndex s expOff e < e := by omega
    ...
    have hmul := gasSteps_bitMulTail s accumulatorWord count b e m baseOff
      expOff (coldByteIndex s expOff e) (coldPhaseBit s expOff e)
      (coldPhaseOffset s expOff e) (coldPhaseByte s expOff e) rest hcap hcount
      hcode hfork hrun hnp
    ...
    exact Challenge.EvmProof.GasSteps.cast
      (hstart.trans (hbytes.trans (henter.trans (hcoldbits.trans
        (hhit.trans (hmul.trans (hhotbits.trans (hfinish.trans
