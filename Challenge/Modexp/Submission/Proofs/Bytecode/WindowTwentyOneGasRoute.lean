...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowTwentyOneGasCore.Paths artifact fork where
  width : Block artifact fork 2603 WindowTwentyOneEntry.widthProgram
  miss : Block artifact fork 2623 WindowTwentyOneEntry.missProgram
  base : Block artifact fork 2627 WindowTwentyOneEntry.baseProgram
  modulus : Block artifact fork 2634 WindowTwentyOneEntry.modulusProgram
  normalize : Block artifact fork 2642 WindowTwentyOneEntry.normalizeProgram
  emptyReturn : Block artifact fork 3266 WindowTwentyOneReturn.emptyProgram
  zeroReturn : Block artifact fork 3258 WindowTwentyOneReturn.zeroProgram
  hitJump : Decode.isValidJumpDest artifact.code 5194 = true
  emptyJump : Decode.isValidJumpDest artifact.code 3266 = true
  zeroJump : Decode.isValidJumpDest artifact.code 3258 = true
  loopJump : Decode.isValidJumpDest artifact.code 2789 = true
  missJump : Decode.isValidJumpDest artifact.code 516 = true
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2627)) (returned template input) := by
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed ctx (UInt256.ofNat 2642)
...
def Handled (template : State) (input : ByteArray) : Prop :=
  ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 2627)) final) ∧
...
def empty_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowTwentyOneReturn.returned ctx (UInt256.ofNat 3281)
...
def zero_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowTwentyOneReturn.returned ctx (UInt256.ofNat 3265) (UInt256.ofNat 0)
...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    runInstructions WindowTwentyOneEntry.widthProgram (state template input (UInt256.ofNat 2603)) =
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 5194 else UInt256.ofNat 2623)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2603)) (state template input (UInt256.ofNat 5194)) := by
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 2603)) (state template input (UInt256.ofNat 516)) := by
  have h := width_raw template input (jump_env env paths.hitJump)
