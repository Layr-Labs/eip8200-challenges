...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowTwentyOneGasCore.Paths artifact fork where
  width : Block artifact fork 2632 WindowTwentyOneEntry.widthProgram
  miss : Block artifact fork 2652 WindowTwentyOneEntry.missProgram
  base : Block artifact fork 2656 WindowTwentyOneEntry.baseProgram
  modulus : Block artifact fork 2663 WindowTwentyOneEntry.modulusProgram
  normalize : Block artifact fork 2671 WindowTwentyOneEntry.normalizeProgram
  emptyReturn : Block artifact fork 3297 WindowTwentyOneReturn.emptyProgram
  zeroReturn : Block artifact fork 3289 WindowTwentyOneReturn.zeroProgram
  hitJump : Decode.isValidJumpDest artifact.code 5160 = true
  emptyJump : Decode.isValidJumpDest artifact.code 3297 = true
  zeroJump : Decode.isValidJumpDest artifact.code 3289 = true
  loopJump : Decode.isValidJumpDest artifact.code 2820 = true
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2656)) (returned template input) := by
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed ctx (UInt256.ofNat 2671)
...
def Handled (template : State) (input : ByteArray) : Prop :=
  ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 2656)) final) ∧
...
def empty_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowTwentyOneReturn.returned ctx (UInt256.ofNat 3312)
...
def zero_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowTwentyOneReturn.returned ctx (UInt256.ofNat 3296) (UInt256.ofNat 0)
...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    runInstructions WindowTwentyOneEntry.widthProgram (state template input (UInt256.ofNat 2632)) =
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 5160 else UInt256.ofNat 2652)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2632)) (state template input (UInt256.ofNat 5160)) := by
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 2632)) (state template input (UInt256.ofNat 517)) := by
  have h := width_raw template input (jump_env env paths.hitJump)
