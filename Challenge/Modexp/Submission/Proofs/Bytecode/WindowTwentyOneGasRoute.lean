...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowTwentyOneGasCore.Paths artifact fork where
  width : Block artifact fork 2216 WindowTwentyOneEntry.widthProgram
  miss : Block artifact fork 2236 WindowTwentyOneEntry.missProgram
  base : Block artifact fork 2240 WindowTwentyOneEntry.baseProgram
  modulus : Block artifact fork 2242 WindowTwentyOneEntry.modulusProgram
  normalize : Block artifact fork 2250 WindowTwentyOneEntry.normalizeProgram
  zeroReturn : Block artifact fork 2870 WindowTwentyOneReturn.zeroProgram
  hitJump : Decode.isValidJumpDest artifact.code 4763 = true
  zeroJump : Decode.isValidJumpDest artifact.code 2870 = true
  loopJump : Decode.isValidJumpDest artifact.code 2401 = true
...
def entryState (template : State) (input : ByteArray) : State :=
  WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 2240)
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed ctx (UInt256.ofNat 2250)
        (WindowTwentyOneInput.modulusWord input :: routeStack input)) =
...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    runInstructions WindowTwentyOneEntry.widthProgram (state template input (UInt256.ofNat 2216)) =
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 4763 else UInt256.ofNat 2236)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 2216)) (state template input (UInt256.ofNat 4763)) := by
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 2216)) (state template input (UInt256.ofNat 471)) := by
  have h := width_raw template input (jump_env env paths.hitJump)
