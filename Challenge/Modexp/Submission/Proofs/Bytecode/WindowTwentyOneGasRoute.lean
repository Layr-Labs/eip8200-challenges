...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowTwentyOneGasCore.Paths artifact fork where
  width : Block artifact fork 2223 WindowTwentyOneEntry.widthProgram
  miss : Block artifact fork 2243 WindowTwentyOneEntry.missProgram
  base : Block artifact fork 2247 WindowTwentyOneEntry.baseProgram
  modulus : Block artifact fork 2248 WindowTwentyOneEntry.modulusProgram
  normalize : Block artifact fork 2254 WindowTwentyOneEntry.normalizeProgram
  ...
  missJump : Decode.isValidJumpDest artifact.code 524 = true
...
def entryState (template : State) (input : ByteArray) : State :=
  WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 2247)
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed ctx (UInt256.ofNat 2254)
        (WindowTwentyOneInput.modulusWord input :: routeStack input)) =
...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    runInstructions WindowTwentyOneEntry.widthProgram (state template input (UInt256.ofNat 2223)) =
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 4955 else UInt256.ofNat 2243)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 2223)) (state template input (UInt256.ofNat 4955)) := by
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 2223)) (state template input (UInt256.ofNat 524)) := by
  have h := width_raw template input (jump_env env paths.hitJump)
