...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowTwentyOneGasCore.Paths artifact fork where
  width : Block artifact fork 2642 WindowTwentyOneEntry.widthProgram
  miss : Block artifact fork 2662 WindowTwentyOneEntry.missProgram
  base : Block artifact fork 2666 WindowTwentyOneEntry.baseProgram
  modulus : Block artifact fork 2673 WindowTwentyOneEntry.modulusProgram
  normalize : Block artifact fork 2681 WindowTwentyOneEntry.normalizeProgram
  emptyReturn : Block artifact fork 3305 WindowTwentyOneReturn.emptyProgram
  zeroReturn : Block artifact fork 3297 WindowTwentyOneReturn.zeroProgram
  hitJump : Decode.isValidJumpDest artifact.code 5264 = true
  emptyJump : Decode.isValidJumpDest artifact.code 3305 = true
  zeroJump : Decode.isValidJumpDest artifact.code 3297 = true
  loopJump : Decode.isValidJumpDest artifact.code 2828 = true
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2666)) (returned template input) := by
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed ctx (UInt256.ofNat 2681)
...
def Handled (template : State) (input : ByteArray) : Prop :=
  ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 2666)) final) ∧
...
def empty_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowTwentyOneReturn.returned ctx (UInt256.ofNat 3320)
...
def zero_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowTwentyOneReturn.returned ctx (UInt256.ofNat 3304) (UInt256.ofNat 0)
...
private theorem width_raw (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5264 = true) :
    runInstructions WindowTwentyOneEntry.widthProgram (state template input (UInt256.ofNat 2642)) =
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 5264 else UInt256.ofNat 2662)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2642)) (state template input (UInt256.ofNat 5264)) := by
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 2642)) (state template input (UInt256.ofNat 517)) := by
  have h := width_raw template input (jump_env env paths.hitJump)
