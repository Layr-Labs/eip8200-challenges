...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowTwentyOneGasCore.Paths artifact fork where
  width : Block artifact fork 1756 WindowTwentyOneEntry.widthProgram
  miss : Block artifact fork 1775 WindowTwentyOneEntry.missProgram
  base : Block artifact fork 1778 WindowTwentyOneEntry.baseProgram
  modulus : Block artifact fork 1779 WindowTwentyOneEntry.modulusProgram
  normalize : Block artifact fork 1785 WindowTwentyOneEntry.normalizeProgram
  zeroReturn : Block artifact fork 2391 WindowTwentyOneReturn.zeroProgram
  hitJump : Decode.isValidJumpDest artifact.code 42 = true
  zeroJump : Decode.isValidJumpDest artifact.code 2391 = true
  trampJump : Decode.isValidJumpDest artifact.code 1909 = true
...
def entryState (template : State) (input : ByteArray) : State :=
  WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 1778)
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed ctx (UInt256.ofNat 1785)
        (WindowTwentyOneInput.modulusWord input :: routeStack input)) =
...
def zero_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowTwentyOneReturn.returned ctx (UInt256.ofNat 2398) (UInt256.ofNat 0)
...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    runInstructions WindowTwentyOneEntry.widthProgram (state template input (UInt256.ofNat 1756)) =
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 42 else UInt256.ofNat 1775)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 1756)) (state template input (UInt256.ofNat 42)) := by
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 1756)) (state template input (UInt256.ofNat 143)) := by
  have h := width_raw template input (jump_env env paths.hitJump)
