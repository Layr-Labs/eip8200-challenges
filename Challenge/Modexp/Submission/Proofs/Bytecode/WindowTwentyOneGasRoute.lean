...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowTwentyOneGasCore.Paths artifact fork where
  width : Block artifact fork 1634 WindowTwentyOneEntry.widthProgram
  miss : Block artifact fork 1653 WindowTwentyOneEntry.missProgram
  base : Block artifact fork 1656 WindowTwentyOneEntry.baseProgram
  normalize : Block artifact fork 1657 WindowTwentyOneEntry.normalizeProgram
  zeroReturn : Block artifact fork 2258 WindowTwentyOneReturn.zeroProgram
  hitJump : Decode.isValidJumpDest artifact.code 42 = true
  zeroJump : Decode.isValidJumpDest artifact.code 2258 = true
  trampJump : Decode.isValidJumpDest artifact.code 1776 = true
  missJump : Decode.isValidJumpDest artifact.code 133 = true
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed ctx (UInt256.ofNat 1657)
        (WindowTwentyOneInput.modulusWord input :: routeStack input)) =
...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    runInstructions WindowTwentyOneEntry.widthProgram (state template input (UInt256.ofNat 1634)) =
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 42 else UInt256.ofNat 1653)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 1634)) (state template input (UInt256.ofNat 42)) := by
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 1634)) (state template input (UInt256.ofNat 133)) := by
  have h := width_raw template input (jump_env env paths.hitJump)
