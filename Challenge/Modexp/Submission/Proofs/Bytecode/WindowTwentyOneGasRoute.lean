...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 5185 else UInt256.ofNat 2653)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2633)) (state template input (UInt256.ofNat 5185)) := by
