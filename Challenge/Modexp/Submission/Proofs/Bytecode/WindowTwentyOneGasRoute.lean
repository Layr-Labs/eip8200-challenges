...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowTwentyOneGasCore.Paths artifact fork where
  width : Block artifact fork 2686 WindowTwentyOneEntry.widthProgram
  miss : Block artifact fork 2706 WindowTwentyOneEntry.missProgram
  base : Block artifact fork 2710 WindowTwentyOneEntry.baseProgram
  modulus : Block artifact fork 2717 WindowTwentyOneEntry.modulusProgram
  normalize : Block artifact fork 2725 WindowTwentyOneEntry.normalizeProgram
  emptyReturn : Block artifact fork 3349 WindowTwentyOneReturn.emptyProgram
  zeroReturn : Block artifact fork 3341 WindowTwentyOneReturn.zeroProgram
  hitJump : Decode.isValidJumpDest artifact.code 5229 = true
  emptyJump : Decode.isValidJumpDest artifact.code 3349 = true
  zeroJump : Decode.isValidJumpDest artifact.code 3341 = true
  loopJump : Decode.isValidJumpDest artifact.code 2872 = true
  missJump : Decode.isValidJumpDest artifact.code 500 = true
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2710)) (returned template input) := by
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed ctx (UInt256.ofNat 2725)
...
def Handled (template : State) (input : ByteArray) : Prop :=
  ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 2710)) final) ∧
...
def empty_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowTwentyOneReturn.returned ctx (UInt256.ofNat 3364)
...
def zero_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowTwentyOneReturn.returned ctx (UInt256.ofNat 3348) (UInt256.ofNat 0)
...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    runInstructions WindowTwentyOneEntry.widthProgram (state template input (UInt256.ofNat 2686)) =
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 5229 else UInt256.ofNat 2706)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2686)) (state template input (UInt256.ofNat 5229)) := by
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 2686)) (state template input (UInt256.ofNat 500)) := by
  have h := width_raw template input (jump_env env paths.hitJump)
