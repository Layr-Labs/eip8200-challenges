...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowTwentyOneGasCore.Paths artifact fork where
  width : Block artifact fork 1372 WindowTwentyOneEntry.widthProgram
  hitTail : Block artifact fork 1390 WindowTwentyOneEntry.hitTailProgram
  adapter : Block artifact fork 116 WindowTwentyOneEntry.adapterProgram
  base : Block artifact fork 1395 WindowTwentyOneEntry.baseProgram
  modulus : Block artifact fork 1396 WindowTwentyOneEntry.modulusProgram
  normalize : Block artifact fork 1399 WindowTwentyOneEntry.normalizeProgram
  adapterJump : Decode.isValidJumpDest artifact.code 116 = true
  entryJump : Decode.isValidJumpDest artifact.code 25 = true
  trampJump : Decode.isValidJumpDest artifact.code 1519 = true
...
def entryState (template : State) (input : ByteArray) : State :=
  WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 1395)
...
def core_steps {artifact : ProgramArtifact} {fork : Fork}
  ...
  have hn' : runInstructions WindowTwentyOneEntry.normalizeProgram
      (WindowTwentyOneEntry.framed ctx (UInt256.ofNat 1399)
...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    runInstructions WindowTwentyOneEntry.widthProgram (state template input (UInt256.ofNat 1372)) =
    some (state template input
      (if (WindowTwentyOneInput.guardDiff input).toNat = 0 then UInt256.ofNat 1390 else UInt256.ofNat 135)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 1372))
  ...
  have ht' : runInstructions WindowTwentyOneEntry.hitTailProgram
      (state template input (UInt256.ofNat 1390)) =
  ...
  have ha' : runInstructions WindowTwentyOneEntry.adapterProgram
      (WindowTwentyOneEntry.framed (context template input) (UInt256.ofNat 116)
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowTwentyOneInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 1372)) (state template input (UInt256.ofNat 135)) := by
  have h := width_raw template input (jump_env env paths.missJump)
