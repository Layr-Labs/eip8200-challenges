...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowNineGasCore.Paths artifact fork where
  width : Block artifact fork 2995 WindowNineEntry.widthProgram
  miss : Block artifact fork 3015 WindowNineEntry.missProgram
  base : Block artifact fork 3019 WindowNineEntry.baseProgram
  modulus : Block artifact fork 3026 WindowNineEntry.modulusProgram
  normalize : Block artifact fork 3034 WindowNineEntry.normalizeProgram
  emptyReturn : Block artifact fork 3404 WindowNineReturn.emptyProgram
  zeroReturn : Block artifact fork 3396 WindowNineReturn.zeroProgram
  hitJump : Decode.isValidJumpDest artifact.code 3019 = true
  emptyJump : Decode.isValidJumpDest artifact.code 3404 = true
  zeroJump : Decode.isValidJumpDest artifact.code 3396 = true
  loopJump : Decode.isValidJumpDest artifact.code 3183 = true
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 3019)) (returned template input) := by
  ...
  have hn' : runInstructions WindowNineEntry.normalizeProgram
      (WindowNineEntry.framed ctx (UInt256.ofNat 3034)
...
def Handled (template : State) (input : ByteArray) : Prop :=
  ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 3019)) final) ∧
...
def empty_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowNineReturn.returned ctx (UInt256.ofNat 3419)
...
def zero_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowNineReturn.returned ctx (UInt256.ofNat 3403) (UInt256.ofNat 0)
...
private theorem width_raw (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 3019 = true) :
    runInstructions WindowNineEntry.widthProgram (state template input (UInt256.ofNat 2995)) =
    some (state template input
      (if (WindowNineInput.guardDiff input).toNat = 0 then UInt256.ofNat 3019 else UInt256.ofNat 3015)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2995)) (state template input (UInt256.ofNat 3019)) := by
...
def steps_miss {artifact : ProgramArtifact} {fork : Fork}
    (paths : Paths artifact fork) (template : State) (env : Environment artifact fork template)
    (input : ByteArray) (hmatch : ¬ WindowNineInput.Matches input) :
    GasSteps (state template input (UInt256.ofNat 2995)) (state template input (UInt256.ofNat 517)) := by
  have h := width_raw template input (jump_env env paths.hitJump)
