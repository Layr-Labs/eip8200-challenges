...
structure Paths (artifact : ProgramArtifact) (fork : Fork) extends WindowNineGasCore.Paths artifact fork where
  width : Block artifact fork 2613 WindowNineEntry.widthProgram
  miss : Block artifact fork 2633 WindowNineEntry.missProgram
  base : Block artifact fork 2637 WindowNineEntry.baseProgram
  modulus : Block artifact fork 2644 WindowNineEntry.modulusProgram
  normalize : Block artifact fork 2652 WindowNineEntry.normalizeProgram
  emptyReturn : Block artifact fork 3022 WindowNineReturn.emptyProgram
  zeroReturn : Block artifact fork 3014 WindowNineReturn.zeroProgram
  hitJump : Decode.isValidJumpDest artifact.code 2637 = true
  emptyJump : Decode.isValidJumpDest artifact.code 3022 = true
  zeroJump : Decode.isValidJumpDest artifact.code 3014 = true
  loopJump : Decode.isValidJumpDest artifact.code 2801 = true
...
def positive_steps {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2637)) (returned template input) := by
  ...
  have hn' : runInstructions WindowNineEntry.normalizeProgram
      (WindowNineEntry.framed ctx (UInt256.ofNat 2652)
...
def Handled (template : State) (input : ByteArray) : Prop :=
  ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 2637)) final) ∧
...
def empty_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowNineReturn.returned ctx (UInt256.ofNat 3037)
...
def zero_handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  let final := WindowNineReturn.returned ctx (UInt256.ofNat 3021) (UInt256.ofNat 0)
...
private theorem width_raw (template : State) (input : ByteArray)
    ...
    runInstructions WindowNineEntry.widthProgram (state template input (UInt256.ofNat 2613)) =
    some (state template input
      (if (WindowNineInput.guardDiff input).toNat = 0 then UInt256.ofNat 2637 else UInt256.ofNat 2633)) := by
...
def steps_hit {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps (state template input (UInt256.ofNat 2613)) (state template input (UInt256.ofNat 2637)) := by
