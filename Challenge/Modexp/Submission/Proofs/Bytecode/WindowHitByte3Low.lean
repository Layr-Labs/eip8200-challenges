...
namespace Challenge.Modexp.Submission.Proofs.Bytecode.WindowHitByte3Low
...
theorem run_prep (template : State) (base modulus : UInt256) (high : Nat)
    ...
    runLocatedBlock (lowPrepPath 3)
      (droppedNibbleState { template with halt := .Running } (UInt256.ofNat 3427)
        base modulus high (byteValue 3 word) word pointer accumulator rest) =
    some (lowNibbleState { template with halt := .Running } (UInt256.ofNat 3430)
...
theorem run_squareLookup (template : State) (base modulus : UInt256)
    ...
    runLocatedBlock (lowSquareLookupPath 3)
      (lowNibbleState { template with halt := .Running } (UInt256.ofNat 3430)
        base modulus nibble byte word pointer accumulator rest) =
    some (lowResultState { template with halt := .Running } (UInt256.ofNat 3461)
...
theorem run_finish (template : State) (base modulus : UInt256)
    ...
    runLocatedBlock (finishPath 3)
      (lowResultState { template with halt := .Running } (UInt256.ofNat 3461)
        base modulus nibble byte word pointer accumulator rest) =
