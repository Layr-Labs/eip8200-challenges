...
def baseProgram : List Instr :=
  [.op .JUMPDEST, .op (.Dup ⟨0, by decide⟩)] ++ testProgram (UInt256.ofNat 3022)

theorem run_base (template : State) (baseSize : UInt256)
    (rest : List UInt256) (hrest : rest.length ≤ 1000)
    (hbase : rest[0]? = some baseSize)
    (htarget : Decode.isValidJumpDest template.executionEnv.code 3022 = true) :
    runInstructions baseProgram (framed template (UInt256.ofNat 2637) rest) =
    some (framed template (if baseSize.toNat = 0 then UInt256.ofNat 3022 else UInt256.ofNat 2644) rest) := by
  ...
  have ht := run_test template (UInt256.ofNat 2639) (UInt256.ofNat 3022) baseSize rest hrest htarget
...
def modulusProgram : List Instr :=
  [.op (.Dup ⟨5, by decide⟩), .op .CALLDATALOAD, .op (.Dup ⟨0, by decide⟩)] ++
    testProgram (UInt256.ofNat 3014)
...
theorem run_modulus (template : State) (modulusOffset : UInt256)
    ...
    (htarget : Decode.isValidJumpDest template.executionEnv.code 3014 = true) :
    ...
    some (framed template (if modulus.toNat = 0 then UInt256.ofNat 3014 else UInt256.ofNat 2652)
      (modulus :: rest)) := by
  let modulus := MachineState.readWord template.executionEnv.calldata modulusOffset.toNat
  have hcap0 : rest.length < 1024 := by omega
  have hcap1 : rest.length + 1 < 1024 := by omega
  ...
  have ht := run_test template (UInt256.ofNat 2647) (UInt256.ofNat 3014) modulus
