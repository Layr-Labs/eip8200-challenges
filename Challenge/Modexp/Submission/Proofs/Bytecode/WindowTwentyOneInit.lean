...
/-- Two `JUMPDEST`s (one gas each) fill pc 2399–2400, so the frame falls
through into the loop head at 2401 without a `PUSH2 2401 JUMP`. -/
...
theorem run_enter (template : State) (base modulus exponent modulusOffset : UInt256)
  ...
  have hpc : advancePC 12 (advancePC 11 (UInt256.ofNat 2376)) = UInt256.ofNat 2399 := by decide
  rw [hpc] at both
  have _hj := hjump
  have hbranch : runInstructions padProgram
      (WindowTwentyOneGroup.state template (UInt256.ofNat 2399) base modulus
        (WindowTwentyOneMath.initialAccumulator base modulus exponent.toNat)
