...
def program : List Instr := lookupProgram ++ frameProgram ++ [.push 2 2401, .op .POP]

theorem run_enter (template : State) (base modulus exponent modulusOffset : UInt256)
    (rest : List UInt256) (hrest : rest.length ≤ 1000)
    (hoffset : rest[5]? = some modulusOffset)
    (hmodulus : MachineState.readWord template.executionEnv.calldata modulusOffset.toNat = modulus)
    (_hjump : Decode.isValidJumpDest template.executionEnv.code 2401 = true) :
  ...
  rw [hpc] at both
  have hbranch : runInstructions [.push 2 2401, .op .POP]
    ...
    have hpc2 : (UInt256.ofNat 2397 + UInt256.ofNat 3).succ = UInt256.ofNat 2401 := by decide
