...
/-- Two `JUMPDEST`s (one gas each) fill the two bytes before the loop head, so the
frame falls through instead of executing a `PUSH2` / `JUMP` pair. -/
...
theorem run_enter (template : State) (base modulus exponent modulusOffset : UInt256)
  ...
  have hpc : advancePC 12 (advancePC 11 (UInt256.ofNat 2347)) = UInt256.ofNat 2370 := by decide
  rw [hpc] at both
  have _hj := hjump
  have hbranch : runInstructions padProgram
      (WindowTwentyOneGroup.state template (UInt256.ofNat 2370) base modulus
        (WindowTwentyOneMath.initialAccumulator base modulus exponent.toNat)
