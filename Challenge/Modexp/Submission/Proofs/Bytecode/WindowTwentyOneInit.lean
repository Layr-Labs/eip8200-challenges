...
theorem run_enter (template : State) (base modulus exponent modulusOffset : UInt256)
    ...
    some (WindowTwentyOneGroup.state template (UInt256.ofNat 2820) base modulus
      (WindowTwentyOneMath.initialAccumulator base modulus exponent.toNat)
      (UInt256.shiftLeft exponent (UInt256.ofNat 4)) (UInt256.ofNat 2) 0 rest) := by
  have hl := run_lookup template (UInt256.ofNat 2798) base modulus exponent rest hrest
  have hf := run_frame template (advancePC 12 (UInt256.ofNat 2798)) base modulus exponent modulusOffset
  ...
  have hpc : advancePC 10 (advancePC 12 (UInt256.ofNat 2798)) = UInt256.ofNat 2820 := by decide
