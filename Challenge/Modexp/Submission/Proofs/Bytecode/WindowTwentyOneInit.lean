...
def frameArrangeProgram : List Instr :=
  [.op (.Swap ⟨1, by decide⟩), .push 1 4, .op .SHL, .push 5 2,
   .op (.Swap ⟨2, by decide⟩)]
...
private theorem run_arrange (template : State) (pc base modulus exponent accumulator : UInt256)
  ...
  have hcap4 : rest.length + 4 < 1024 := by omega
  have hp2 : UInt256.ofNat 2 = UInt256.ofNat 1 + UInt256.ofNat 1 := by decide
  have hp6 : UInt256.ofNat 6 = UInt256.ofNat 1 + UInt256.ofNat 1 + UInt256.ofNat 1 +
  ...
  simp [runInstructions, frameArrangeProgram, WindowTwentyOneTable.framed,
    ...
    advancePC, succ_eq_add, hp2, hp6, word_add_assoc]
...
theorem run_frame (template : State) (pc base modulus exponent modulusOffset accumulator : UInt256)
    ...
    runInstructions frameProgram
      (WindowTwentyOneTable.framed template pc base modulus 16
        ([accumulator, UInt256.ofNat 480, exponent] ++ rest)) =
    some (WindowTwentyOneGroup.state template (advancePC 14 pc) base modulus accumulator
  ...
  have hl := run_frameLoad template (advancePC 11 pc) base modulus exponent modulusOffset
  ...
  simpa only [frameProgram, WindowTwentyOneGroup.state, WindowTwentyOneLookup.framed,
    ...
    List.nil_append, List.cons_append, ← advancePC_add, show 11 + 3 = 14 by decide] using both

/-- Five `JUMPDEST`s (one gas each) fill pc 2525–2529, so the frame falls
through into the loop head at 2530 without a `PUSH2 2530 JUMP`. -/
...
theorem run_enter (template : State) (base modulus exponent modulusOffset : UInt256)
    ...
    (hoffset : rest[5]? = some modulusOffset)
    (hmodulus : MachineState.readWord template.executionEnv.calldata modulusOffset.toNat = modulus)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2530 = true) :
    runInstructions program
      (WindowTwentyOneTable.framed template (UInt256.ofNat 2500) base modulus 16 ([base, exponent] ++ rest)) =
    some (WindowTwentyOneGroup.state template (UInt256.ofNat 2530) base modulus
      (WindowTwentyOneMath.initialAccumulator base modulus exponent.toNat)
      (UInt256.shiftLeft exponent (UInt256.ofNat 4)) (UInt256.ofNat 2) 0 rest) := by
  have hl := run_lookup template (UInt256.ofNat 2500) base modulus exponent rest hrest
  have hf := run_frame template (advancePC 11 (UInt256.ofNat 2500)) base modulus exponent modulusOffset
  ...
  have hpc : advancePC 14 (advancePC 11 (UInt256.ofNat 2500)) = UInt256.ofNat 2525 := by decide
  rw [hpc] at both
  have _hj := hjump
  have hbranch : runInstructions padProgram
      (WindowTwentyOneGroup.state template (UInt256.ofNat 2525) base modulus
        (WindowTwentyOneMath.initialAccumulator base modulus exponent.toNat)
