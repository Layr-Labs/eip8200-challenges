...
def program : List Instr := lookupProgram ++ frameProgram ++ [.push 2 2530, .op .JUMP]
...
theorem run_enter (template : State) (base modulus exponent modulusOffset : UInt256)
  ...
  have hpc : advancePC 10 (advancePC 11 (UInt256.ofNat 2500)) = UInt256.ofNat 2521 := by decide
  rw [hpc] at both
  have hbranch : runInstructions [.push 2 2530, .op .JUMP]
      (WindowTwentyOneGroup.state template (UInt256.ofNat 2521) base modulus
        (WindowTwentyOneMath.initialAccumulator base modulus exponent.toNat)
