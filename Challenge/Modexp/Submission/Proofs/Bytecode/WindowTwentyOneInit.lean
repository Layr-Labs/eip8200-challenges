...
-- The dead literal that used to sit here (`PUSH2 2360; POP`, a hop left behind at its own
-- fall-through pc) is deleted in this candidate, so the block ends with `frameProgram` and
-- lands on the loop's `JUMPDEST` directly.  The `JUMPDEST` itself stays in the code, so the
-- loop's back-edge and the `hjump` witness the loop lemmas take are unaffected.
...
theorem run_enter (template : State) (base modulus exponent modulusOffset : UInt256)
    ...
    runInstructions program
      (WindowTwentyOneTable.framed template (UInt256.ofNat 2331) base modulus 16 ([base, exponent] ++ rest)) =
    some (WindowTwentyOneGroup.state template (UInt256.ofNat 2352) base modulus
      (WindowTwentyOneMath.initialAccumulator base modulus exponent.toNat)
      (UInt256.shiftLeft exponent (UInt256.ofNat 4)) (UInt256.ofNat 2) 0 rest) := by
  have hl := run_lookup template (UInt256.ofNat 2331) base modulus exponent rest hrest
  have hf := run_frame template (advancePC 11 (UInt256.ofNat 2331)) base modulus exponent modulusOffset
  ...
  have hpc : advancePC 10 (advancePC 11 (UInt256.ofNat 2331)) = UInt256.ofNat 2352 := by decide
  rw [hpc] at both
  exact both
