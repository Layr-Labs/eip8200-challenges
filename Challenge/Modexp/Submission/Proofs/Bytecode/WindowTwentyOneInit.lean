...
theorem run_enter (template : State) (base modulus exponent modulusOffset : UInt256)
    ...
    some (WindowTwentyOneGroup.state template (UInt256.ofNat 2227) base modulus
      (WindowTwentyOneMath.initialAccumulator base modulus exponent.toNat)
      (UInt256.shiftLeft exponent (UInt256.ofNat 4)) (UInt256.ofNat 2) 0 rest) := by
  have hl := run_lookup template (UInt256.ofNat 2202) base modulus exponent rest hrest
  have hf := run_frame template (advancePC 11 (UInt256.ofNat 2202)) base modulus exponent modulusOffset
  ...
  have hpc : advancePC 12 (advancePC 11 (UInt256.ofNat 2202)) = UInt256.ofNat 2225 := by decide
  rw [hpc] at both
  have hbranch : runInstructions padProgram
      (WindowTwentyOneGroup.state template (UInt256.ofNat 2225) base modulus
        (WindowTwentyOneMath.initialAccumulator base modulus exponent.toNat)
