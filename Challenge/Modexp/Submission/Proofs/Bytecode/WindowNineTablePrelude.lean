...
namespace Challenge.Modexp.Submission.Proofs.Bytecode.WindowNineTablePrelude
...
theorem run_init (template : State) (pc base modulus : UInt256)
    ...
    some (WindowNineTable.framed template (advancePC 8 pc) base modulus 2
  ...
  simp [runInstructions, initProgram, initial, WindowNineTable.framed,
...
theorem run_load (template : State) (pc base modulus exponentOffset : UInt256)
    ...
    runInstructions loadProgram
      (WindowNineTable.framed template pc base modulus 2 ([base, modulus] ++ rest)) =
    some (WindowNineTable.framed template (advancePC 3 pc) base modulus 2
      ([modulus, base, MachineState.readWord template.executionEnv.calldata exponentOffset.toNat] ++ rest)) := by
  have hcap2 : rest.length + 2 < 1024 := by omega
  have hcap3 : rest.length + 3 < 1024 := by omega
  simp [runInstructions, loadProgram, WindowNineTable.framed,
...
private theorem run_square (template : State) (pc base modulus exponent : UInt256)
    ...
    runInstructions squareProgram
      (WindowNineTable.framed template pc base modulus 2
  ...
  simp [runInstructions, squareProgram, WindowNineTable.framed,
...
theorem run_staged (template : State) (pc base modulus exponent : UInt256)
    ...
    runInstructions stagedProgram
      (WindowNineTable.framed template pc base modulus 2 ([modulus, base, exponent] ++ rest)) =
    some (WindowNineTable.framed template (advancePC 16 pc) base modulus 2
      (WindowMath.tableWord base modulus 2 :: List.replicate 13 modulus ++
        [base, exponent] ++ rest)) := by
  let core := WindowNineTable.framed template pc base modulus 2 []
  have hd := WindowNineStage.run_topCopies core pc modulus ([base, exponent] ++ rest) 13
  ...
  have hd' : runInstructions (List.replicate 13 (.op (.Dup ⟨0, by decide⟩)))
      (WindowNineTable.framed template pc base modulus 2 ([modulus, base, exponent] ++ rest)) =
    some (WindowNineTable.framed template (advancePC 13 pc) base modulus 2
      (List.replicate 14 modulus ++ [base, exponent] ++ rest)) := by
    simpa only [core, WindowNineStage.framed, WindowNineTable.framed,
...
def program : List Instr :=
  initProgram ++ loadProgram ++ stagedProgram ++ WindowNineTable.storeProgram 1 2

def endPC (pc : UInt256) : UInt256 :=
  WindowNineTable.storePC 1 (advancePC 16 (advancePC 3 (advancePC 8 pc)))
...
theorem run_prelude (template : State) (pc base modulus exponentOffset : UInt256)
    ...
    some (WindowNineTable.state template (endPC pc) base modulus
  ...
  have ht := WindowNineTable.run_store template (advancePC 16 (advancePC 3 (advancePC 8 pc)))
  ...
  simpa only [program, endPC, WindowNineTable.state, exponent, List.cons_append,
    List.append_assoc] using hall

end Challenge.Modexp.Submission.Proofs.Bytecode.WindowNineTablePrelude
