...
def gasSteps_scanFound (s : State) (accumulatorWord : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (BigExponent.innerLoop
        ...
        accumulatorWord count b e m baseOff expOff i (loadedOffset expOff i)
        (loadedByte s expOff i) rest (j + 1)) := by
  ...
  have hfound := soundBlock
    ...
    (run_bitFound s accumulatorWord count b e m baseOff expOff i j rest
      (by omega) hcode hrun)
  have hcall := soundBlock
    (s := copyEntryState s accumulatorWord count b e m baseOff expOff i j rest)
    copyCallPath
    (by simpa [copyEntryState, BigExponentScan.bitLoop] using hcode)
    (by simpa [copyEntryState, BigExponentScan.bitLoop, State.fork] using hfork)
    (by simpa [copyEntryState, BigExponentScan.bitLoop] using hrun)
    (by simpa [copyEntryState, BigExponentScan.bitLoop, State.fork] using hnp)
    (run_copyCall s accumulatorWord count b e m baseOff expOff i j rest
      (by omega) hcode hrun)
  have hframe : (copyTailFrame accumulatorWord count b e m baseOff expOff i j
      (loadedOffset expOff i) (loadedByte s expOff i) rest).length < 1016 := by
    simp [copyTailFrame]
    omega
  have hvalid : Decode.isValidJumpDest submissionBytecode
      ((1410 : UInt256)).toNat = true := by
    rw [show ((1410 : UInt256)).toNat = 1410 from by decide]
    exact jump1410
  have hcopy := BigHelpers.gasSteps_copy
    ...
    (by simpa [copyEntryState, BigExponentScan.bitLoop, State.fork] using hnp)
    hvalid
  have hret := soundBlock
    (s := copiedState s accumulatorWord count b e m baseOff expOff i j rest)
    copyReturnPath
    (by simpa [copiedState, copyEntryState, BigExponentScan.bitLoop,
    ...
    (run_copyReturn
      (copiedState s accumulatorWord count b e m baseOff expOff i j rest)
      accumulatorWord count b e m baseOff expOff i j (loadedOffset expOff i)
      (loadedByte s expOff i) rest (by omega)
      (by simp [copiedState, BigHelpers.copyReturned,
        show ((1410 : UInt256)) = UInt256.ofNat 1410 from by decide])
      (by simp [copiedState, BigHelpers.copyReturned])
      (by simpa [copiedState, copyEntryState, BigExponentScan.bitLoop,
        BigHelpers.copyReturned] using hcode)
      (by simpa [copiedState, copyEntryState, BigExponentScan.bitLoop,
  ...
  exact hstart.trans <| hbytes.trans <| hguard.trans <| hload.trans <|
    hbitsStart.trans <| hbits.trans <| htest.trans <| hfound.trans <|
      hcall.trans <| hcopy.trans hret
...
def foundProgress (s : State) (accumulatorWord : UInt256)
    (count b e m baseOff expOff i j : Nat) (rest : List UInt256) : State :=
  let copied := copiedState s accumulatorWord count b e m baseOff expOff i j rest
  let first := firstByteProgress copied accumulatorWord count b e m baseOff expOff
    i (j + 1) rest
...
def gasSteps_finishFound (s : State) (accumulatorWord : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (BigExponent.innerLoop
        (copiedState s accumulatorWord count b e m baseOff expOff i j rest)
        accumulatorWord count b e m baseOff expOff i (loadedOffset expOff i)
        (loadedByte s expOff i) rest (j + 1))
      (BigExponent.outerLoop
  ...
  let first := firstByteProgress copied accumulatorWord count b e m baseOff expOff
    i (j + 1) rest
  have hfirst := gasSteps_finishFirstByte copied accumulatorWord count b e m
    baseOff expOff i (j + 1) rest hcap hcount (by omega) hi he
    (by simpa [copied, copiedState, copyEntryState, BigExponentScan.bitLoop,
      BigHelpers.copyReturned] using hcode)
    (by simpa [copied, copiedState, copyEntryState, BigExponentScan.bitLoop,
      BigHelpers.copyReturned, State.fork] using hfork)
    (by simpa [copied, copiedState, copyEntryState, BigExponentScan.bitLoop,
  ...
  have hrest := gasSteps_exponentBytesFrom first accumulatorWord count b e m
    ...
    (by omega) hexpFit
    (by simpa [first, firstByteProgress, copied, copiedState, copyEntryState,
      BigExponentScan.bitLoop, BigHelpers.copyReturned] using hcode)
    (by simpa [first, firstByteProgress, copied, copiedState, copyEntryState,
      BigExponentScan.bitLoop, BigHelpers.copyReturned, State.fork] using hfork)
    (by simpa [first, firstByteProgress, copied, copiedState, copyEntryState,
      BigExponentScan.bitLoop, BigHelpers.copyReturned] using hrun)
    (by simpa [first, firstByteProgress, copied, copiedState, copyEntryState,
      BigExponentScan.bitLoop, BigHelpers.copyReturned, State.fork] using hnp)
  have hsum : i + 1 + (e - (i + 1)) = e := by omega
  exact Challenge.EvmProof.GasSteps.cast (hfirst.trans hrest) rfl
    (by simp [outerLoopFromState, foundProgress, first, copied, hsum])
...
@[simp] theorem exponentPhaseState_executionEnv (s : State)
  ...
  by_cases h : coldByteIndex s expOff e = e <;>
    simp [exponentPhaseState, h, foundProgress, firstByteProgress, copiedState,
      copyEntryState, BigExponentScan.bitLoop, BigHelpers.copyReturned]
...
def gasSteps_exponentPhase (s : State) (accumulatorWord : UInt256)
  ...
  · have hi : i < e := by omega
    ...
    have hscan := gasSteps_scanFound s accumulatorWord count b e m baseOff
      expOff i j rest hcap hcount hi hj hexpFit
