...
@[simp] private theorem tablePCs (index : Nat)
    (hlo : 1881 ≤ index) (hhi : index ≤ 1887) :
    Artifact.submissionArtifact.instructionPC index =
      ([2858,2859,2860,2861,2862,2863,2867] : List Nat)[index - 1881]! := by
...
private theorem run_generic (template : State) (base modulus : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableKernelState template base modulus 4 2858 rest) =
        some (tableKernelState template base modulus 5 2868 rest) := by
...
theorem run_table5 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableState input 4 2858) = some (tableState input 5 2868) := by
...
def gasSteps_table5 (input : ByteArray) : TableUpdateStep input 4 2858 := by
