...
@[simp] private theorem tablePCs (index : Nat)
    (hlo : 1858 ≤ index) (hhi : index ≤ 1864) :
    Artifact.submissionArtifact.instructionPC index =
      [3035,3036,3037,3038,3039,3040,3044][index - 1858]! := by
...
private theorem run_generic (template : State) (base modulus : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableKernelState template base modulus 4 3035 rest) =
        some (tableKernelState template base modulus 5 3045 rest) := by
...
theorem run_table5 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableState input 4 3035) = some (tableState input 5 3045) := by
...
def gasSteps_table5 (input : ByteArray) : TableUpdateStep input 4 3035 := by
