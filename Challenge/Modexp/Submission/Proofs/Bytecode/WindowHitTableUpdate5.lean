...
@[simp] private theorem tablePCs (index : Nat)
    (hlo : 1913 ≤ index) (hhi : index ≤ 1921) :
    Artifact.submissionArtifact.instructionPC index =
      ([3102, 3103, 3104, 3105, 3106, 3107, 3108, 3109, 3111]
        : List Nat)[index - 1913]! := by
...
private theorem run_generic (template : State) (base modulus : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableKernelState template base modulus 4 3102 rest) =
        some (tableKernelState template base modulus 5 3112 rest) := by
...
theorem run_table5 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableState input 4 3102) = some (tableState input 5 3112) := by
...
def gasSteps_table5 (input : ByteArray) : TableUpdateStep input 4 3102 := by
