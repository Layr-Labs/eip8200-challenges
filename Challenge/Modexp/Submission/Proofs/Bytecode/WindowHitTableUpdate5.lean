...
@[simp] private theorem tablePCs (index : Nat)
    (hlo : 1883 ≤ index) (hhi : index ≤ 1891) :
    Artifact.submissionArtifact.instructionPC index =
      ([3066, 3067, 3068, 3069, 3070, 3071, 3072, 3073, 3075]
        : List Nat)[index - 1883]! := by
...
private theorem run_generic (template : State) (base modulus : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableKernelState template base modulus 4 3066 rest) =
        some (tableKernelState template base modulus 5 3076 rest) := by
...
theorem run_table5 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableState input 4 3066) = some (tableState input 5 3076) := by
...
def gasSteps_table5 (input : ByteArray) : TableUpdateStep input 4 3066 := by
