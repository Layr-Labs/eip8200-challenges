...
@[simp] private theorem tablePCs (index : Nat)
    (hlo : 1884 ≤ index) (hhi : index ≤ 1890) :
    Artifact.submissionArtifact.instructionPC index =
      ([2688,2689,2690,2692,2693,2694,2695] : List Nat)[index - 1884]! := by
...
private theorem run_generic (template : State) (base modulus : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableKernelState template base modulus 4 2686 rest) =
        some (tableKernelState template base modulus 5 2696 rest) := by
...
theorem run_table5 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock table5Path
      (tableState input 4 2686) = some (tableState input 5 2696) := by
...
def gasSteps_table5 (input : ByteArray) : TableUpdateStep input 4 2686 := by
