...
def legacy (input : ByteArray) (hmiss : ¬ WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5251)
...
def hit (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5251)
    (state (initialState submissionBytecode input 0) input (UInt256.ofNat 4883)) at entrySteps
