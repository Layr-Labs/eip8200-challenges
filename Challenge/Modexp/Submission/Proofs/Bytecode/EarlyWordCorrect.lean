...
def legacy (input : ByteArray) (hmiss : ¬ WindowTwentyOneInput.Matches input) :
    Challenge.EvmProof.GasSteps (initialState submissionBytecode input 0)
      (Main.trampolineState input 1117) := by
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5169)
    (Main.trampolineState input 1117) at tail
...
def hit (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5169)
    (state (initialState submissionBytecode input 0) input (UInt256.ofNat 4825)) at entrySteps
