...
def legacy (input : ByteArray) (hmiss : ¬ WindowTwentyOneInput.Matches input) :
    Challenge.EvmProof.GasSteps (initialState submissionBytecode input 0)
      (Main.trampolineState input 1275) := by
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5318)
    (Main.trampolineState input 1275) at tail
...
def hit (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5318)
    (state (initialState submissionBytecode input 0) input (UInt256.ofNat 5229)) at entrySteps
