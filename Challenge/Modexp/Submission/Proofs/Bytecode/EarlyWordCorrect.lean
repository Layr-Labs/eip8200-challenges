...
def legacy (input : ByteArray) (hmiss : ¬ WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5224)
...
def hit (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5224)
    (state (initialState submissionBytecode input 0) input (UInt256.ofNat 4885)) at entrySteps
