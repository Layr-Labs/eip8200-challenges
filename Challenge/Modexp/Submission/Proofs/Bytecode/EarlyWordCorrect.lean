...
The initial hop always reaches pc 5279. Matching headers enter the existing
Fermat/window proof at pc 5190. Every other header restores the exact legacy
...
def legacy (input : ByteArray) (hmiss : ¬ WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5279)
...
def hit (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5279)
    (state (initialState submissionBytecode input 0) input (UInt256.ofNat 5190)) at entrySteps
