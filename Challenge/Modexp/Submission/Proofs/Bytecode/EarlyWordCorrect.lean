...
Fermat/window proof at pc 4853. Every other header restores the exact legacy
...
def legacy (input : ByteArray) (hmiss : ¬ WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5146)
...
def hit (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5146)
    (state (initialState submissionBytecode input 0) input (UInt256.ofNat 4798)) at entrySteps
