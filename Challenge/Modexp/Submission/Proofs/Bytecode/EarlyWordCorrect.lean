...
The initial hop always reaches pc 5277. Matching headers enter the existing
Fermat/window proof at pc 5188. Every other header restores the exact legacy
...
def legacy (input : ByteArray) (hmiss : ¬ WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5277)
...
def hit (input : ByteArray) (hmatch : WindowTwentyOneInput.Matches input) :
  ...
  change Challenge.EvmProof.GasSteps (Main.trampolineState input 5277)
    (state (initialState submissionBytecode input 0) input (UInt256.ofNat 5188)) at entrySteps
