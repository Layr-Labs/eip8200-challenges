...
private theorem notSecp : UInt256.lnot (4294968272 : UInt256) = secp := by decide

def primeValueProgram : List Instr :=
  [.op .JUMPDEST, .op (.Dup ⟨5, by decide⟩), .op .CALLDATALOAD,
   .op (.Dup ⟨0, by decide⟩), .push 32 bn, .op .EQ,
   .op (.Dup ⟨1, by decide⟩), .push 5 4294968272, .op .NOT, .op .EQ, .op .OR]

def primeProgram : List Instr := primeValueProgram ++ WindowTwentyOneEntry.testProgram (UInt256.ofNat 4987)
...
def exponentProgram : List Instr := exponentValueProgram ++
  WindowTwentyOneEntry.testProgram (UInt256.ofNat 4987)
...
def missProgram : List Instr :=
  [.op .JUMPDEST, .op .POP, .push 2 2359, .op .JUMP]

theorem run_prime (template : State) (offset : UInt256) (rest : List UInt256)
    (hrest : rest.length ≤ 999) (hoff : rest[5]? = some offset)
    (htarget : Decode.isValidJumpDest template.executionEnv.code 4987 = true) :
    runInstructions primeProgram (framed template (UInt256.ofNat 4903) rest) =
    some (framed template
      (if (primeValue (MachineState.readWord template.executionEnv.calldata offset.toNat)).toNat = 0
        then UInt256.ofNat 4987 else UInt256.ofNat 4956)
  ...
  have hv : runInstructions primeValueProgram (framed template (UInt256.ofNat 4903) rest) =
      some (framed template (UInt256.ofNat 4951)
        (primeValue (MachineState.readWord template.executionEnv.calldata offset.toNat) ::
          MachineState.readWord template.executionEnv.calldata offset.toNat :: rest)) := by
    simp (config := { maxSteps := 500000 }) [primeValueProgram, primeValue, notSecp, runInstructions, framed, hoff,
      Challenge.EvmProof.Stepper.runInstr, hc0, hc1, hc2, hc3, hc4,
      Challenge.EvmProof.Word.succ_ofNat_mod,
      Challenge.EvmProof.Word.ofNat_add_mod]
  have ht := WindowTwentyOneEntry.run_test template (UInt256.ofNat 4951) (UInt256.ofNat 4987)
    (primeValue (MachineState.readWord template.executionEnv.calldata offset.toNat))
  ...
  have hpc : advancePC 5 (UInt256.ofNat 4951) = UInt256.ofNat 4956 := by decide
...
theorem run_exponent (template : State) (modulus offset : UInt256) (rest : List UInt256)
    ...
    (htarget : Decode.isValidJumpDest template.executionEnv.code 4987 = true) :
    runInstructions exponentProgram (framed template (UInt256.ofNat 4956) (modulus :: rest)) =
    some (framed template
      (if (UInt256.eq (modulus - UInt256.ofNat 1)
        (MachineState.readWord template.executionEnv.calldata offset.toNat)).toNat = 0
        then UInt256.ofNat 4987 else UInt256.ofNat 4968)
      (modulus :: rest)) := by
  have hc1 : rest.length + 1 < 1024 := by omega
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hv : runInstructions exponentValueProgram (framed template (UInt256.ofNat 4956)
      (modulus :: rest)) =
      some (framed template (UInt256.ofNat 4963)
    ...
    simp (config := { maxSteps := 500000 }) [exponentValueProgram, runInstructions, framed, hoff,
      ...
      Challenge.EvmProof.Word.succ_ofNat_mod,
      Challenge.EvmProof.Word.ofNat_add_mod]
  have ht := WindowTwentyOneEntry.run_test template (UInt256.ofNat 4963) (UInt256.ofNat 4987)
    (UInt256.eq (modulus - UInt256.ofNat 1)
      (MachineState.readWord template.executionEnv.calldata offset.toNat)) (modulus :: rest)
    (by simp; omega) htarget
  have both := runInstructions_append_some _ _ _ _ _ hv ht
  have hpc : advancePC 5 (UInt256.ofNat 4963) = UInt256.ofNat 4968 := by decide
...
private theorem run_value (template : State) (modulus offset : UInt256) (rest : List UInt256)
    ...
    runInstructions valueProgram (framed template (UInt256.ofNat 4968) (modulus :: rest)) =
    some (framed template (UInt256.ofNat 4981)
...
theorem run_return (template : State) (modulus offset : UInt256) (rest : List UInt256)
    ...
    runInstructions returnProgram (framed template (UInt256.ofNat 4968) (modulus :: rest)) =
    some (WindowTwentyOneReturn.returned template (UInt256.ofNat 4986)
      (UInt256.isZero (UInt256.isZero (UInt256.mod
        (UInt256.shiftRight (MachineState.readWord template.executionEnv.calldata offset.toNat)
  ...
  have hr := WindowTwentyOneReturn.run_return template (UInt256.ofNat 4981) word active
    hsmall hactive rest (by omega)
  have both := runInstructions_append_some _ _ _ _ _ hv hr
  have hpc : advancePC 5 (UInt256.ofNat 4981) = UInt256.ofNat 4986 := by decide
...
theorem run_miss (template : State) (value : UInt256) (rest : List UInt256)
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2359 = true) :
    runInstructions missProgram (framed template (UInt256.ofNat 4987) (value :: rest)) =
    some (framed template (UInt256.ofNat 2359) rest) := by
  have hc0 : rest.length < 1024 := by omega
  have hc1 : rest.length + 1 < 1024 := by omega
...
structure Paths (artifact : Challenge.EvmProof.ProgramArtifact) (fork : Fork) where
  prime : WindowTwentyOneBinding.Block artifact fork 4903 primeProgram
  exponent : WindowTwentyOneBinding.Block artifact fork 4956 exponentProgram
  result : WindowTwentyOneBinding.Block artifact fork 4968 returnProgram
  miss : WindowTwentyOneBinding.Block artifact fork 4987 missProgram
  missJump : Decode.isValidJumpDest artifact.code 4987 = true
