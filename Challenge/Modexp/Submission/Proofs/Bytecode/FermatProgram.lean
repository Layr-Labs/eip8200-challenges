...
def primeProgram : List Instr := primeValueProgram ++ WindowTwentyOneEntry.testProgram (UInt256.ofNat 2211)
...
def exponentProgram : List Instr := exponentValueProgram ++
  WindowTwentyOneEntry.testProgram (UInt256.ofNat 2211)
...
theorem run_prime (template : State) (offset : UInt256) (rest : List UInt256)
    ...
    (htarget : Decode.isValidJumpDest template.executionEnv.code 2211 = true) :
    runInstructions primeProgram (framed template (UInt256.ofNat 4777) rest) =
    some (framed template
      (if (primeValue (MachineState.readWord template.executionEnv.calldata offset.toNat)).toNat = 0
        then UInt256.ofNat 2211 else UInt256.ofNat 4830)
  ...
  have hv : runInstructions primeValueProgram (framed template (UInt256.ofNat 4777) rest) =
  ...
  have ht := WindowTwentyOneEntry.run_test template (UInt256.ofNat 4825) (UInt256.ofNat 2211)
    (primeValue (MachineState.readWord template.executionEnv.calldata offset.toNat))
  ...
  have hpc : advancePC 5 (UInt256.ofNat 4825) = UInt256.ofNat 4830 := by decide
...
theorem run_exponent (template : State) (modulus offset : UInt256) (rest : List UInt256)
    ...
    (htarget : Decode.isValidJumpDest template.executionEnv.code 2211 = true) :
    runInstructions exponentProgram (framed template (UInt256.ofNat 4830) (modulus :: rest)) =
    some (framed template
      (if (UInt256.eq (modulus - UInt256.ofNat 1)
        (MachineState.readWord template.executionEnv.calldata offset.toNat)).toNat = 0
        then UInt256.ofNat 2211 else UInt256.ofNat 4842)
      (modulus :: rest)) := by
  have hc1 : rest.length + 1 < 1024 := by omega
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hv : runInstructions exponentValueProgram (framed template (UInt256.ofNat 4830)
      (modulus :: rest)) =
      some (framed template (UInt256.ofNat 4837)
    ...
    simp (config := { maxSteps := 500000 }) [exponentValueProgram, runInstructions, framed, hoff,
      ...
      Challenge.EvmProof.Word.succ_ofNat_mod,
      Challenge.EvmProof.Word.ofNat_add_mod]
  have ht := WindowTwentyOneEntry.run_test template (UInt256.ofNat 4837) (UInt256.ofNat 2211)
    (UInt256.eq (modulus - UInt256.ofNat 1)
      (MachineState.readWord template.executionEnv.calldata offset.toNat)) (modulus :: rest)
    (by simp; omega) htarget
  have both := runInstructions_append_some _ _ _ _ _ hv ht
  have hpc : advancePC 5 (UInt256.ofNat 4837) = UInt256.ofNat 4842 := by decide
...
private theorem run_value (template : State) (modulus offset : UInt256) (rest : List UInt256)
    ...
    runInstructions valueProgram (framed template (UInt256.ofNat 4842) (modulus :: rest)) =
    some (framed template (UInt256.ofNat 4855)
...
theorem run_return (template : State) (modulus offset : UInt256) (rest : List UInt256)
    ...
    runInstructions returnProgram (framed template (UInt256.ofNat 4842) (modulus :: rest)) =
    some (WindowTwentyOneReturn.returned template (UInt256.ofNat 4860)
      (UInt256.isZero (UInt256.isZero (UInt256.mod
        (UInt256.shiftRight (MachineState.readWord template.executionEnv.calldata offset.toNat)
  ...
  have hr := WindowTwentyOneReturn.run_return template (UInt256.ofNat 4855) word active
    hsmall hactive rest (by omega)
  have both := runInstructions_append_some _ _ _ _ _ hv hr
  have hpc : advancePC 5 (UInt256.ofNat 4855) = UInt256.ofNat 4860 := by decide
  simpa only [returnProgram, hpc, word] using both

structure Paths (artifact : Challenge.EvmProof.ProgramArtifact) (fork : Fork) where
  prime : WindowTwentyOneBinding.Block artifact fork 4777 primeProgram
  exponent : WindowTwentyOneBinding.Block artifact fork 4830 exponentProgram
  result : WindowTwentyOneBinding.Block artifact fork 4842 returnProgram
  legacyJump : Decode.isValidJumpDest artifact.code 2211 = true
