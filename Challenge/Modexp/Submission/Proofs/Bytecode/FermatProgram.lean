...
def primeProgram : List Instr := primeValueProgram ++ WindowTwentyOneEntry.testProgram (UInt256.ofNat 5348)
...
def exponentProgram : List Instr := exponentValueProgram ++
  [.push 2 (UInt256.ofNat 5348), .op .JUMPI]
...
def missProgram : List Instr :=
  [.op .JUMPDEST, .op .POP, .push 2 2666, .op .JUMP]

theorem run_prime (template : State) (offset : UInt256) (rest : List UInt256)
    (hrest : rest.length ≤ 999) (hoff : rest[5]? = some offset)
    (htarget : Decode.isValidJumpDest template.executionEnv.code 5348 = true) :
    runInstructions primeProgram (framed template (UInt256.ofNat 5264) rest) =
    some (framed template
      (if (primeValue (MachineState.readWord template.executionEnv.calldata offset.toNat)).toNat = 0
        then UInt256.ofNat 5348 else UInt256.ofNat 5317)
  ...
  have hv : runInstructions primeValueProgram (framed template (UInt256.ofNat 5264) rest) =
      some (framed template (UInt256.ofNat 5312)
  ...
  have ht := WindowTwentyOneEntry.run_test template (UInt256.ofNat 5312) (UInt256.ofNat 5348)
    (primeValue (MachineState.readWord template.executionEnv.calldata offset.toNat))
  ...
  have hpc : advancePC 5 (UInt256.ofNat 5312) = UInt256.ofNat 5317 := by decide
...
theorem run_exponent (template : State) (modulus offset : UInt256) (rest : List UInt256)
    ...
    (htarget : Decode.isValidJumpDest template.executionEnv.code 5348 = true) :
    runInstructions exponentProgram (framed template (UInt256.ofNat 5317) (modulus :: rest)) =
    some (framed template
      (if (UInt256.eq (modulus - UInt256.ofNat 1)
        (MachineState.readWord template.executionEnv.calldata offset.toNat)).toNat = 0
        then UInt256.ofNat 5348 else UInt256.ofNat 5329)
      (modulus :: rest)) := by
  have hc1 : rest.length + 1 < 1024 := by omega
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
...
private theorem run_value (template : State) (modulus offset : UInt256) (rest : List UInt256)
    ...
    runInstructions valueProgram (framed template (UInt256.ofNat 5329) (modulus :: rest)) =
    some (framed template (UInt256.ofNat 5342)
...
theorem run_return (template : State) (modulus offset : UInt256) (rest : List UInt256)
    ...
    runInstructions returnProgram (framed template (UInt256.ofNat 5329) (modulus :: rest)) =
    some (WindowTwentyOneReturn.returned template (UInt256.ofNat 5347)
      (UInt256.isZero (UInt256.isZero (UInt256.mod
        (UInt256.shiftRight (MachineState.readWord template.executionEnv.calldata offset.toNat)
  ...
  have hr := WindowTwentyOneReturn.run_return template (UInt256.ofNat 5342) word active
    hsmall hactive rest (by omega)
  have both := runInstructions_append_some _ _ _ _ _ hv hr
  have hpc : advancePC 5 (UInt256.ofNat 5342) = UInt256.ofNat 5347 := by decide
...
theorem run_miss (template : State) (value : UInt256) (rest : List UInt256)
    ...
    (hjump : Decode.isValidJumpDest template.executionEnv.code 2666 = true) :
    runInstructions missProgram (framed template (UInt256.ofNat 5348) (value :: rest)) =
    some (framed template (UInt256.ofNat 2666) rest) := by
  have hc0 : rest.length < 1024 := by omega
  have hc1 : rest.length + 1 < 1024 := by omega
...
structure Paths (artifact : Challenge.EvmProof.ProgramArtifact) (fork : Fork) where
  prime : WindowTwentyOneBinding.Block artifact fork 5264 primeProgram
  exponent : WindowTwentyOneBinding.Block artifact fork 5317 exponentProgram
  result : WindowTwentyOneBinding.Block artifact fork 5329 returnProgram
  miss : WindowTwentyOneBinding.Block artifact fork 5348 missProgram
  missJump : Decode.isValidJumpDest artifact.code 5348 = true
  legacyJump : Decode.isValidJumpDest artifact.code 2666 = true
