...
def normalized (template : State) (input : ByteArray) : State :=
  WindowNineTablePrelude.initial (context template input) (UInt256.ofNat 3062)
...
theorem run_prepare (template : State) (input : ByteArray)
    ...
    (hzeroBase : Decode.isValidJumpDest template.executionEnv.code 3422 = true)
    (hzeroModulus : Decode.isValidJumpDest template.executionEnv.code 3414 = true) :
    runInstructions prepareProgram (state template input (UInt256.ofNat 3037)) =
  ...
  have hb' : runInstructions WindowNineEntry.baseProgram
      (state template input (UInt256.ofNat 3037)) =
      some (state template input (UInt256.ofNat 3044)) := by
    simpa only [state, if_neg hb] using hbr
  have hm' : runInstructions WindowNineEntry.modulusProgram
      (state template input (UInt256.ofNat 3044)) =
      some (WindowNineEntry.framed (context template input) (UInt256.ofNat 3052)
  ...
  have hn' : runInstructions WindowNineEntry.normalizeProgram
      (WindowNineEntry.framed (context template input) (UInt256.ofNat 3052)
...
theorem run_positive (template : State) (input : ByteArray)
    ...
    (hzeroBase : Decode.isValidJumpDest template.executionEnv.code 3422 = true)
    (hzeroModulus : Decode.isValidJumpDest template.executionEnv.code 3414 = true)
    (hloop : Decode.isValidJumpDest template.executionEnv.code 3201 = true) :
    runInstructions program (state template input (UInt256.ofNat 3037)) =
