...
@[simp] private theorem table4PCs (index : Nat)
    (hlo : 1874 ≤ index) (hhi : index ≤ 1880) :
    Artifact.submissionArtifact.instructionPC index =
      ([2848,2849,2850,2851,2852,2853,2857] : List Nat)[index - 1874]! := by
...
private theorem run_table4_generic (template : State) (base modulus : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock table4Path
      (tableKernelState template base modulus 3 2848 rest) =
        some (tableKernelState template base modulus 4 2858 rest) := by
...
theorem run_table4 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock table4Path
      (tableState input 3 2848) = some (tableState input 4 2858) := by
...
def gasSteps_table4 (input : ByteArray) : TableUpdateStep input 3 2848 := by
