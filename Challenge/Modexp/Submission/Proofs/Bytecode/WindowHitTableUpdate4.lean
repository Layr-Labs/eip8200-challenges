...
@[simp] private theorem table4PCs (index : Nat)
    (hlo : 1874 ≤ index) (hhi : index ≤ 1882) :
    Artifact.submissionArtifact.instructionPC index =
      ([3056, 3057, 3058, 3059, 3060, 3061, 3062, 3063, 3065]
        : List Nat)[index - 1874]! := by
...
private theorem run_table4_generic (template : State) (base modulus : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock table4Path
      (tableKernelState template base modulus 3 3056 rest) =
        some (tableKernelState template base modulus 4 3066 rest) := by
...
theorem run_table4 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock table4Path
      (tableState input 3 3056) = some (tableState input 4 3066) := by
...
def gasSteps_table4 (input : ByteArray) : TableUpdateStep input 3 3056 := by
