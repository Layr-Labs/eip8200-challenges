...
@[simp] private theorem table4PCs (index : Nat)
    (hlo : 1851 ≤ index) (hhi : index ≤ 1857) :
    Artifact.submissionArtifact.instructionPC index =
      [3025,3026,3027,3028,3029,3030,3034][index - 1851]! := by
...
private theorem run_table4_generic (template : State) (base modulus : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock table4Path
      (tableKernelState template base modulus 3 3025 rest) =
        some (tableKernelState template base modulus 4 3035 rest) := by
...
theorem run_table4 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock table4Path
      (tableState input 3 3025) = some (tableState input 4 3035) := by
...
def gasSteps_table4 (input : ByteArray) : TableUpdateStep input 3 3025 := by
