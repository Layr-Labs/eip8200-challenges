...
theorem run_loopAdvance (input : ByteArray) (pointer : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock loopAdvancePath
      (wordState input pointer 4 3465 accumulator) =
  ...
  let template := wordState input pointer 4 3465 accumulator
  ...
  have hall := Challenge.EvmProof.Stepper.runLocatedBlock_append
    ...
    (WindowHitLoopAdvanceHead.framed template 3469
      (UInt256.ofNat (pointer + 4) :: chunk :: modulusWord input ::
        routeStack input))
    (WindowHitLoopAdvanceHead.framed template 3171
      (UInt256.ofNat (pointer + 4) :: chunk :: modulusWord input ::
        routeStack input))
    hhead rfl hjump
