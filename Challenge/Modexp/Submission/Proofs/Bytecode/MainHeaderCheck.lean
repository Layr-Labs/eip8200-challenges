...
theorem run_headerCheck (input : ByteArray) :
  ...
  -- The block is two `JUMPDEST`s, so both steps are fall-throughs.
  ...
  have hs1065 := Challenge.EvmProof.Word.succ_ofNat
    (n := 1065) (by norm_num : 1065 + 1 < 2 ^ 256)
  have hs1066 := Challenge.EvmProof.Word.succ_ofNat
    (n := 1066) (by norm_num : 1066 + 1 < 2 ^ 256)
  simp [headerCheckPath, opAt, pushAt,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    headerLoadedState, headerState, initialState, hs1065, hs1066,
