...
def branchProgram : List Instr := [.push 2 5289, .op .JUMPI]
...
def frameProgram : List Instr :=
  [.push 2 1186, .op (.Dup ⟨1, by decide⟩), .op (.Dup ⟨3, by decide⟩),
   ...
   .op (.Dup ⟨10, by decide⟩), .push 2 4903, .op .JUMP]
...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  guard : WindowTwentyOneBinding.Block artifact fork 5241 guardProgram
  hit : WindowTwentyOneBinding.Block artifact fork 5268 hitProgram
  miss : WindowTwentyOneBinding.Block artifact fork 5289 missProgram
  helperJump : Decode.isValidJumpDest artifact.code 5241 = true
  missJump : Decode.isValidJumpDest artifact.code 5289 = true
  hitJump : Decode.isValidJumpDest artifact.code 4903 = true
...
theorem run_header (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input) :
    runInstructions headerProgram (framed template (UInt256.ofNat 5241) []) =
      some (framed template (UInt256.ofNat 5250) (headerStack input)) := by
...
theorem run_guard_value (template : State) (b e m : UInt256) :
    runInstructions guardValueProgram (framed template (UInt256.ofNat 5250) [m, e, b]) =
      some (framed template (UInt256.ofNat 5264) (widthDiff b e m :: [m, e, b])) := by
...
theorem run_branch (template : State) (value b e m : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5289 = true) :
    runInstructions branchProgram
      (framed template (UInt256.ofNat 5264) (value :: [m, e, b])) =
      some (framed template
        (if value.toNat = 0 then UInt256.ofNat 5268 else UInt256.ofNat 5289) [m, e, b]) := by
...
theorem run_guard (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5289 = true) :
    runInstructions guardProgram (framed template (UInt256.ofNat 5241) []) =
      some (framed template
        (if (WindowTwentyOneInput.guardDiff input).toNat = 0
          then UInt256.ofNat 5268 else UInt256.ofNat 5289) (headerStack input)) := by
...
theorem run_offsets (template : State) (b e m : UInt256) :
    runInstructions offsetsProgram (framed template (UInt256.ofNat 5268) [m, e, b]) =
      some (framed template (UInt256.ofNat 5275)
...
theorem run_frame (template : State) (b e m x y : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4903 = true) :
    runInstructions frameProgram (framed template (UInt256.ofNat 5275) [y, x, m, e, b]) =
      some (framed template (UInt256.ofNat 4903)
...
theorem run_hit (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4903 = true) :
    runInstructions hitProgram (framed template (UInt256.ofNat 5268) (headerStack input)) =
      some (framed template (UInt256.ofNat 4903) (WindowTwentyOnePositive.routeStack input)) := by
...
theorem run_miss (template : State) (input : ByteArray)
    ...
    runInstructions missProgram (framed template (UInt256.ofNat 5289) (headerStack input)) =
