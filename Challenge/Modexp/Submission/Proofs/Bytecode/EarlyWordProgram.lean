...
/-- Twenty instructions at pc 5256, ending at the conditional branch. -/
...
def frameProgram : List Instr :=
  [.push 2 1267, .op (.Dup ⟨1, by decide⟩), .op (.Dup ⟨3, by decide⟩),
   ...
   .op (.Dup ⟨10, by decide⟩), .push 2 4877, .op .JUMP]

/-- Fifteen instructions at pc 5283, with the canonical return frame. -/
...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  guard : WindowTwentyOneBinding.Block artifact fork 5256 guardProgram
  ...
  hitJump : Decode.isValidJumpDest artifact.code 4877 = true
...
theorem run_header (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input) :
    runInstructions headerProgram (framed template (UInt256.ofNat 5256) []) =
      some (framed template (UInt256.ofNat 5265) (headerStack input)) := by
...
theorem run_guard_value (template : State) (b e m : UInt256) :
    runInstructions guardValueProgram (framed template (UInt256.ofNat 5265) [m, e, b]) =
      some (framed template (UInt256.ofNat 5279) (widthDiff b e m :: [m, e, b])) := by
...
theorem run_offsets (template : State) (b e m : UInt256) :
    runInstructions offsetsProgram (framed template (UInt256.ofNat 5283) [m, e, b]) =
      some (framed template (UInt256.ofNat 5290)
...
theorem run_frame (template : State) (b e m x y : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4877 = true) :
    runInstructions frameProgram (framed template (UInt256.ofNat 5290) [y, x, m, e, b]) =
      some (framed template (UInt256.ofNat 4877)
...
theorem run_hit (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4877 = true) :
    runInstructions hitProgram (framed template (UInt256.ofNat 5283) (headerStack input)) =
      some (framed template (UInt256.ofNat 4877) (WindowTwentyOnePositive.routeStack input)) := by
...
theorem run_miss (template : State) (input : ByteArray)
    ...
    runInstructions missProgram (framed template (UInt256.ofNat 5304) (headerStack input)) =
