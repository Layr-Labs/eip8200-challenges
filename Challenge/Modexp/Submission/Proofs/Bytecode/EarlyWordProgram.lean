...
def branchProgram : List Instr := [.push 2 5266, .op .JUMPI]

/-- Twenty instructions at pc 5218, ending at the conditional branch. -/
...
/-- Fifteen instructions at pc 5245, with the canonical return frame. -/
...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  guard : WindowTwentyOneBinding.Block artifact fork 5218 guardProgram
  hit : WindowTwentyOneBinding.Block artifact fork 5245 hitProgram
  miss : WindowTwentyOneBinding.Block artifact fork 5266 missProgram
  helperJump : Decode.isValidJumpDest artifact.code 5218 = true
  missJump : Decode.isValidJumpDest artifact.code 5266 = true
...
theorem run_header (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input) :
    runInstructions headerProgram (framed template (UInt256.ofNat 5218) []) =
      some (framed template (UInt256.ofNat 5227) (headerStack input)) := by
...
theorem run_guard_value (template : State) (b e m : UInt256) :
    runInstructions guardValueProgram (framed template (UInt256.ofNat 5227) [m, e, b]) =
      some (framed template (UInt256.ofNat 5241) (widthDiff b e m :: [m, e, b])) := by
...
theorem run_branch (template : State) (value b e m : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5266 = true) :
    runInstructions branchProgram
      (framed template (UInt256.ofNat 5241) (value :: [m, e, b])) =
      some (framed template
        (if value.toNat = 0 then UInt256.ofNat 5245 else UInt256.ofNat 5266) [m, e, b]) := by
...
theorem run_guard (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5266 = true) :
    runInstructions guardProgram (framed template (UInt256.ofNat 5218) []) =
      some (framed template
        (if (WindowTwentyOneInput.guardDiff input).toNat = 0
          then UInt256.ofNat 5245 else UInt256.ofNat 5266) (headerStack input)) := by
...
theorem run_offsets (template : State) (b e m : UInt256) :
    runInstructions offsetsProgram (framed template (UInt256.ofNat 5245) [m, e, b]) =
      some (framed template (UInt256.ofNat 5252)
...
theorem run_frame (template : State) (b e m x y : UInt256)
    ...
    runInstructions frameProgram (framed template (UInt256.ofNat 5252) [y, x, m, e, b]) =
...
theorem run_hit (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4879 = true) :
    runInstructions hitProgram (framed template (UInt256.ofNat 5245) (headerStack input)) =
...
theorem run_miss (template : State) (input : ByteArray)
    ...
    runInstructions missProgram (framed template (UInt256.ofNat 5266) (headerStack input)) =
