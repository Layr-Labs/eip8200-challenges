...
def branchProgram : List Instr := [.push 2 4995, .op .JUMPI]

/-- Twenty instructions at pc 5220, ending at the conditional branch. -/
...
def frameProgram : List Instr :=
  [.push 0 0, .op (.Dup ⟨1, by decide⟩), .op (.Dup ⟨3, by decide⟩),
   ...
   .op (.Dup ⟨10, by decide⟩), .push 2 4808, .op .JUMP]

/-- Fifteen instructions at pc 5247, with the canonical return frame. -/
...
def missProgram : List Instr :=
  [.op .JUMPDEST, .op .POP, .op .POP, .op .POP, .push 2 1233, .op .JUMP]
...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  guard : WindowTwentyOneBinding.Block artifact fork 4947 guardProgram
  hit : WindowTwentyOneBinding.Block artifact fork 4974 hitProgram
  miss : WindowTwentyOneBinding.Block artifact fork 4995 missProgram
  helperJump : Decode.isValidJumpDest artifact.code 4947 = true
  missJump : Decode.isValidJumpDest artifact.code 4995 = true
  hitJump : Decode.isValidJumpDest artifact.code 4808 = true
...
theorem run_header (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input) :
    runInstructions headerProgram (framed template (UInt256.ofNat 4947) []) =
      some (framed template (UInt256.ofNat 4956) (headerStack input)) := by
...
theorem run_guard_value (template : State) (b e m : UInt256) :
    runInstructions guardValueProgram (framed template (UInt256.ofNat 4956) [m, e, b]) =
      some (framed template (UInt256.ofNat 4970) (widthDiff b e m :: [m, e, b])) := by
...
theorem run_branch (template : State) (value b e m : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4995 = true) :
    runInstructions branchProgram
      (framed template (UInt256.ofNat 4970) (value :: [m, e, b])) =
      some (framed template
        (if value.toNat = 0 then UInt256.ofNat 4974 else UInt256.ofNat 4995) [m, e, b]) := by
...
theorem run_guard (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4995 = true) :
    runInstructions guardProgram (framed template (UInt256.ofNat 4947) []) =
      some (framed template
        (if (WindowTwentyOneInput.guardDiff input).toNat = 0
          then UInt256.ofNat 4974 else UInt256.ofNat 4995) (headerStack input)) := by
...
theorem run_offsets (template : State) (b e m : UInt256) :
    runInstructions offsetsProgram (framed template (UInt256.ofNat 4974) [m, e, b]) =
...
theorem run_frame (template : State) (b e m x y : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4808 = true) :
    runInstructions frameProgram (framed template (UInt256.ofNat 4981) [y, x, m, e, b]) =
      some (framed template (UInt256.ofNat 4808)
...
theorem run_hit (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4808 = true) :
    runInstructions hitProgram (framed template (UInt256.ofNat 4974) (headerStack input)) =
      some (framed template (UInt256.ofNat 4808) (WindowTwentyOnePositive.routeStack input)) := by
...
theorem run_miss (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 1233 = true) :
    runInstructions missProgram (framed template (UInt256.ofNat 4995) (headerStack input)) =
      some (framed template (UInt256.ofNat 1233) []) := by
