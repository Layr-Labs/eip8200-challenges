...
def branchProgram : List Instr := [.push 2 5194, .op .JUMPI]
...
def frameProgram : List Instr :=
  [.push 2 1186, .op (.Dup ⟨1, by decide⟩), .op (.Dup ⟨3, by decide⟩),
   ...
   .op (.Dup ⟨10, by decide⟩), .push 2 4798, .op .JUMP]
...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  guard : WindowTwentyOneBinding.Block artifact fork 5146 guardProgram
  hit : WindowTwentyOneBinding.Block artifact fork 5173 hitProgram
  miss : WindowTwentyOneBinding.Block artifact fork 5194 missProgram
  helperJump : Decode.isValidJumpDest artifact.code 5146 = true
  missJump : Decode.isValidJumpDest artifact.code 5194 = true
...
theorem run_header (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input) :
    runInstructions headerProgram (framed template (UInt256.ofNat 5146) []) =
      some (framed template (UInt256.ofNat 5155) (headerStack input)) := by
...
theorem run_guard_value (template : State) (b e m : UInt256) :
    runInstructions guardValueProgram (framed template (UInt256.ofNat 5155) [m, e, b]) =
      some (framed template (UInt256.ofNat 5169) (widthDiff b e m :: [m, e, b])) := by
...
theorem run_branch (template : State) (value b e m : UInt256)
    ...
    runInstructions branchProgram
      (framed template (UInt256.ofNat 5169) (value :: [m, e, b])) =
      some (framed template
        (if value.toNat = 0 then UInt256.ofNat 5173 else UInt256.ofNat 5194) [m, e, b]) := by
...
theorem run_guard (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5194 = true) :
    runInstructions guardProgram (framed template (UInt256.ofNat 5146) []) =
      some (framed template
        (if (WindowTwentyOneInput.guardDiff input).toNat = 0
          then UInt256.ofNat 5173 else UInt256.ofNat 5194) (headerStack input)) := by
...
theorem run_offsets (template : State) (b e m : UInt256) :
    runInstructions offsetsProgram (framed template (UInt256.ofNat 5173) [m, e, b]) =
      some (framed template (UInt256.ofNat 5180)
...
theorem run_frame (template : State) (b e m x y : UInt256)
    ...
    runInstructions frameProgram (framed template (UInt256.ofNat 5180) [y, x, m, e, b]) =
      some (framed template (UInt256.ofNat 4798)
...
theorem run_hit (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4798 = true) :
    runInstructions hitProgram (framed template (UInt256.ofNat 5173) (headerStack input)) =
      some (framed template (UInt256.ofNat 4798) (WindowTwentyOnePositive.routeStack input)) := by
...
theorem run_miss (template : State) (input : ByteArray)
    ...
    runInstructions missProgram (framed template (UInt256.ofNat 5194) (headerStack input)) =
