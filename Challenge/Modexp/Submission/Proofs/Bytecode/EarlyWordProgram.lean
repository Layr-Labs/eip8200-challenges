...
def branchProgram : List Instr := [.push 2 5277, .op .JUMPI]

/-- Twenty instructions at pc 5229, ending at the conditional branch. -/
...
def frameProgram : List Instr :=
  [.push 2 1267, .op (.Dup ⟨1, by decide⟩), .op (.Dup ⟨3, by decide⟩),
   ...
   .op (.Dup ⟨10, by decide⟩), .push 2 4890, .op .JUMP]

/-- Fifteen instructions at pc 5256, with the canonical return frame. -/
...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  guard : WindowTwentyOneBinding.Block artifact fork 5229 guardProgram
  hit : WindowTwentyOneBinding.Block artifact fork 5256 hitProgram
  miss : WindowTwentyOneBinding.Block artifact fork 5277 missProgram
  helperJump : Decode.isValidJumpDest artifact.code 5229 = true
  missJump : Decode.isValidJumpDest artifact.code 5277 = true
  hitJump : Decode.isValidJumpDest artifact.code 4890 = true
...
theorem run_header (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input) :
    runInstructions headerProgram (framed template (UInt256.ofNat 5229) []) =
      some (framed template (UInt256.ofNat 5238) (headerStack input)) := by
...
theorem run_guard_value (template : State) (b e m : UInt256) :
    runInstructions guardValueProgram (framed template (UInt256.ofNat 5238) [m, e, b]) =
      some (framed template (UInt256.ofNat 5252) (widthDiff b e m :: [m, e, b])) := by
...
theorem run_branch (template : State) (value b e m : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5277 = true) :
    runInstructions branchProgram
      (framed template (UInt256.ofNat 5252) (value :: [m, e, b])) =
      some (framed template
        (if value.toNat = 0 then UInt256.ofNat 5256 else UInt256.ofNat 5277) [m, e, b]) := by
...
theorem run_guard (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5277 = true) :
    runInstructions guardProgram (framed template (UInt256.ofNat 5229) []) =
      some (framed template
        (if (WindowTwentyOneInput.guardDiff input).toNat = 0
          then UInt256.ofNat 5256 else UInt256.ofNat 5277) (headerStack input)) := by
...
theorem run_offsets (template : State) (b e m : UInt256) :
    runInstructions offsetsProgram (framed template (UInt256.ofNat 5256) [m, e, b]) =
      some (framed template (UInt256.ofNat 5263)
...
theorem run_frame (template : State) (b e m x y : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4890 = true) :
    runInstructions frameProgram (framed template (UInt256.ofNat 5263) [y, x, m, e, b]) =
...
theorem run_hit (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4890 = true) :
    runInstructions hitProgram (framed template (UInt256.ofNat 5256) (headerStack input)) =
      some (framed template (UInt256.ofNat 4890) (WindowTwentyOnePositive.routeStack input)) := by
...
theorem run_miss (template : State) (input : ByteArray)
    ...
    runInstructions missProgram (framed template (UInt256.ofNat 5277) (headerStack input)) =
