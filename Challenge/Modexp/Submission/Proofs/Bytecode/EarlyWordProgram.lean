...
def branchProgram : List Instr := [.push 2 4999, .op .JUMPI]
...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  guard : WindowTwentyOneBinding.Block artifact fork 4951 guardProgram
  hit : WindowTwentyOneBinding.Block artifact fork 4978 hitProgram
  miss : WindowTwentyOneBinding.Block artifact fork 4999 missProgram
  helperJump : Decode.isValidJumpDest artifact.code 4951 = true
  missJump : Decode.isValidJumpDest artifact.code 4999 = true
...
theorem run_header (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input) :
    runInstructions headerProgram (framed template (UInt256.ofNat 4951) []) =
      some (framed template (UInt256.ofNat 4960) (headerStack input)) := by
...
theorem run_guard_value (template : State) (b e m : UInt256) :
    runInstructions guardValueProgram (framed template (UInt256.ofNat 4960) [m, e, b]) =
      some (framed template (UInt256.ofNat 4974) (widthDiff b e m :: [m, e, b])) := by
...
theorem run_branch (template : State) (value b e m : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4999 = true) :
    runInstructions branchProgram
      (framed template (UInt256.ofNat 4974) (value :: [m, e, b])) =
      some (framed template
        (if value.toNat = 0 then UInt256.ofNat 4978 else UInt256.ofNat 4999) [m, e, b]) := by
...
theorem run_guard (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4999 = true) :
    runInstructions guardProgram (framed template (UInt256.ofNat 4951) []) =
      some (framed template
        (if (WindowTwentyOneInput.guardDiff input).toNat = 0
          then UInt256.ofNat 4978 else UInt256.ofNat 4999) (headerStack input)) := by
...
theorem run_offsets (template : State) (b e m : UInt256) :
    runInstructions offsetsProgram (framed template (UInt256.ofNat 4978) [m, e, b]) =
      some (framed template (UInt256.ofNat 4985)
...
theorem run_hit (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4812 = true) :
    runInstructions hitProgram (framed template (UInt256.ofNat 4978) (headerStack input)) =
      some (framed template (UInt256.ofNat 4812) (WindowTwentyOnePositive.routeStack input)) := by
...
theorem run_miss (template : State) (input : ByteArray)
    ...
    runInstructions missProgram (framed template (UInt256.ofNat 4999) (headerStack input)) =
