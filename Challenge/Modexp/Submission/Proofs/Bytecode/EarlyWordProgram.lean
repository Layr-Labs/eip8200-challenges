...
def branchProgram : List Instr := [.push 2 5296, .op .JUMPI]

/-- Twenty instructions at pc 5248, ending at the conditional branch. -/
...
def frameProgram : List Instr :=
  [.push 2 1267, .op (.Dup ⟨1, by decide⟩), .op (.Dup ⟨3, by decide⟩),
   ...
   .op (.Dup ⟨10, by decide⟩), .push 2 4869, .op .JUMP]

/-- Fifteen instructions at pc 5275, with the canonical return frame. -/
...
structure Paths (artifact : ProgramArtifact) (fork : Fork) where
  guard : WindowTwentyOneBinding.Block artifact fork 5248 guardProgram
  hit : WindowTwentyOneBinding.Block artifact fork 5275 hitProgram
  miss : WindowTwentyOneBinding.Block artifact fork 5296 missProgram
  helperJump : Decode.isValidJumpDest artifact.code 5248 = true
  missJump : Decode.isValidJumpDest artifact.code 5296 = true
  hitJump : Decode.isValidJumpDest artifact.code 4869 = true
...
theorem run_header (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input) :
    runInstructions headerProgram (framed template (UInt256.ofNat 5248) []) =
      some (framed template (UInt256.ofNat 5257) (headerStack input)) := by
...
theorem run_guard_value (template : State) (b e m : UInt256) :
    runInstructions guardValueProgram (framed template (UInt256.ofNat 5257) [m, e, b]) =
      some (framed template (UInt256.ofNat 5271) (widthDiff b e m :: [m, e, b])) := by
...
theorem run_branch (template : State) (value b e m : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5296 = true) :
    runInstructions branchProgram
      (framed template (UInt256.ofNat 5271) (value :: [m, e, b])) =
      some (framed template
        (if value.toNat = 0 then UInt256.ofNat 5275 else UInt256.ofNat 5296) [m, e, b]) := by
...
theorem run_guard (template : State) (input : ByteArray)
    (hdata : template.executionEnv.calldata = input)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 5296 = true) :
    runInstructions guardProgram (framed template (UInt256.ofNat 5248) []) =
      some (framed template
        (if (WindowTwentyOneInput.guardDiff input).toNat = 0
          then UInt256.ofNat 5275 else UInt256.ofNat 5296) (headerStack input)) := by
...
theorem run_offsets (template : State) (b e m : UInt256) :
    runInstructions offsetsProgram (framed template (UInt256.ofNat 5275) [m, e, b]) =
      some (framed template (UInt256.ofNat 5282)
...
theorem run_frame (template : State) (b e m x y : UInt256)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4869 = true) :
    runInstructions frameProgram (framed template (UInt256.ofNat 5282) [y, x, m, e, b]) =
      some (framed template (UInt256.ofNat 4869)
...
theorem run_hit (template : State) (input : ByteArray)
    (hjump : Decode.isValidJumpDest template.executionEnv.code 4869 = true) :
    runInstructions hitProgram (framed template (UInt256.ofNat 5275) (headerStack input)) =
      some (framed template (UInt256.ofNat 4869) (WindowTwentyOnePositive.routeStack input)) := by
...
theorem run_miss (template : State) (input : ByteArray)
    ...
    runInstructions missProgram (framed template (UInt256.ofNat 5296) (headerStack input)) =
