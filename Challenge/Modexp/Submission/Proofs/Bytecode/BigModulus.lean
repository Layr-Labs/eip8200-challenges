...
def scanSetupPath :
  ...
  [opAt 595 .JUMPDEST, pushAt 596 0 0, pushAt 597 0 0]

def scanGuardPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 598 .JUMPDEST, opAt 599 (.Dup ⟨2, by decide⟩),
   opAt 600 (.Dup ⟨1, by decide⟩), opAt 601 .LT, opAt 602 .ISZERO,
   pushAt 603 2 795, opAt 604 .JUMPI]

def scanBodyPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 605 (.Dup ⟨0, by decide⟩), pushAt 606 1 5,
   opAt 607 .SHL, opAt 608 .MLOAD, opAt 609 (.Dup ⟨2, by decide⟩),
   opAt 610 .OR, opAt 611 (.Swap ⟨1, by decide⟩), opAt 612 .POP,
   pushAt 613 1 1, opAt 614 (.Dup ⟨1, by decide⟩), opAt 615 .ADD,
   opAt 616 (.Swap ⟨0, by decide⟩), opAt 617 .POP,
   pushAt 618 2 767, opAt 619 .JUMP]

def scanNonzeroPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 620 .JUMPDEST, opAt 621 .POP, opAt 622 (.Dup ⟨0, by decide⟩),
   pushAt 623 2 807, opAt 624 .JUMPI]

def scanZeroPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  scanNonzeroPath ++
    [opAt 625 (.Dup ⟨4, by decide⟩), pushAt 626 2 6144,
     opAt 627 .RETURN]
...
def scanBody (s : State) (count i : Nat) (rest : List UInt256) : State :=
  { scanLoop s count i rest with pc := UInt256.ofNat 776 }

def scanExit (s : State) (count : Nat) (rest : List UInt256) : State :=
  { scanLoop s count count rest with pc := UInt256.ofNat 795 }

def scanNonzero (s : State) (count : Nat) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 807
...
@[simp] private theorem scanPCs (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC i =
      [764,765,766,767,768,769,770,771,772,775,776,777,779,780,781,782,783,784,785,787,788,789,790,791,794,795,796,797,798,801,802,803,806][i - 595]! := by
  interval_cases i <;> decide

private theorem jump771 :
    Decode.isValidJumpDest submissionBytecode 767 = true :=
  Artifact.isValidJumpDest_index 598 (by rfl)

private theorem jump799 :
    Decode.isValidJumpDest submissionBytecode 795 = true :=
  Artifact.isValidJumpDest_index 620 (by rfl)

private theorem jump811 :
    Decode.isValidJumpDest submissionBytecode 807 = true :=
...
theorem run_scanBody (s : State) (count i : Nat) (rest : List UInt256)
  ...
  have h771 : (767 : UInt256).toNat = 767 := by decide
  have h771Word : (767 : UInt256) = UInt256.ofNat 767 := by decide
...
theorem run_scanFinishGuard (s : State) (count : Nat) (rest : List UInt256)
  ...
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have h799 : (795 : UInt256).toNat = 795 := by decide
  have h799Word : (795 : UInt256) = UInt256.ofNat 795 := by decide
...
theorem run_scanNonzero (s : State) (count : Nat) (rest : List UInt256)
  ...
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc2 : rest.length + 2 < 1024 := by omega
  have h811 : (807 : UInt256).toNat = 807 := by decide
  have h811Word : (807 : UInt256) = UInt256.ofNat 807 := by decide
