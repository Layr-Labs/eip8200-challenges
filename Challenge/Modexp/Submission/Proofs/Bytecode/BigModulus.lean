...
def scanSetupPath :
  ...
  [opAt 555 .JUMPDEST, pushAt 556 0 0, pushAt 557 0 0]

def scanGuardPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 558 .JUMPDEST, opAt 559 (.Dup ⟨2, by decide⟩),
   opAt 560 (.Dup ⟨1, by decide⟩), opAt 561 .LT, opAt 562 .ISZERO,
   pushAt 563 2 721, opAt 564 .JUMPI]

def scanBodyPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 565 (.Dup ⟨0, by decide⟩), pushAt 566 1 5,
   opAt 567 .SHL, opAt 568 .MLOAD, opAt 569 (.Dup ⟨2, by decide⟩),
   opAt 570 .OR, opAt 571 (.Swap ⟨1, by decide⟩), opAt 572 .POP,
   pushAt 573 1 1, opAt 574 (.Dup ⟨1, by decide⟩), opAt 575 .ADD,
   opAt 576 (.Swap ⟨0, by decide⟩), opAt 577 .POP,
   pushAt 578 2 693, opAt 579 .JUMP]

def scanNonzeroPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 580 .JUMPDEST, opAt 581 .POP, opAt 582 (.Dup ⟨0, by decide⟩),
   pushAt 583 2 733, opAt 584 .JUMPI]

def scanZeroPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  scanNonzeroPath ++
    [opAt 585 (.Dup ⟨4, by decide⟩), pushAt 586 2 6144,
     opAt 587 .RETURN]
...
def scanBody (s : State) (count i : Nat) (rest : List UInt256) : State :=
  { scanLoop s count i rest with pc := UInt256.ofNat 702 }

def scanExit (s : State) (count : Nat) (rest : List UInt256) : State :=
  { scanLoop s count count rest with pc := UInt256.ofNat 721 }

def scanNonzero (s : State) (count : Nat) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 733
...
@[simp] private theorem scanPCs (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC i =
      ([690,691,692,693,694,695,696,697,698,701,702,703,705,706,707,708,709,710,711,713,714,715,716,717,720,721,722,723,724,727,728,729,732] : List Nat)[i - 555]! := by
  interval_cases i <;> decide

private theorem jump771 :
    Decode.isValidJumpDest submissionBytecode 693 = true :=
...
theorem run_scanBody (s : State) (count i : Nat) (rest : List UInt256)
  ...
  have h771 : (693 : UInt256).toNat = 693 := by decide
  have h771Word : (693 : UInt256) = UInt256.ofNat 693 := by decide
...
theorem run_scanFinishGuard (s : State) (count : Nat) (rest : List UInt256)
  ...
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have h799 : (721 : UInt256).toNat = 721 := by decide
  have h799Word : (721 : UInt256) = UInt256.ofNat 721 := by decide
...
theorem run_scanNonzero (s : State) (count : Nat) (rest : List UInt256)
  ...
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc2 : rest.length + 2 < 1024 := by omega
  have h811 : (733 : UInt256).toNat = 733 := by decide
  have h811Word : (733 : UInt256) = UInt256.ofNat 733 := by decide
