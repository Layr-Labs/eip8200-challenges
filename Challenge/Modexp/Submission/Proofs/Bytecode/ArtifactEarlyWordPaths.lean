...
private def earlyWord_guard : WindowTwentyOneBinding.Block submissionArtifact .Osaka 5115 EarlyWordProgram.guardProgram :=
  WindowTwentyOneSlice.block allWellFormed 3885 20 5115 EarlyWordProgram.guardProgram (by decide) (by rfl) (by rfl) (by decide)
private def earlyWord_hit : WindowTwentyOneBinding.Block submissionArtifact .Osaka 5142 EarlyWordProgram.hitProgram :=
  WindowTwentyOneSlice.block allWellFormed 3905 15 5142 EarlyWordProgram.hitProgram (by decide) (by rfl) (by rfl) (by decide)
private def earlyWord_miss : WindowTwentyOneBinding.Block submissionArtifact .Osaka 5163 EarlyWordProgram.missProgram :=
  WindowTwentyOneSlice.block allWellFormed 3920 6 5163 EarlyWordProgram.missProgram (by decide) (by rfl) (by rfl) (by decide)
def earlyWordPaths : EarlyWordProgram.Paths submissionArtifact .Osaka where
  guard := earlyWord_guard
  hit := earlyWord_hit
  miss := earlyWord_miss
  helperJump := by exact isValidJumpDest_index 3885 (by rfl)
  missJump := by exact isValidJumpDest_index 3920 (by rfl)
  hitJump := by exact isValidJumpDest_index 3656 (by rfl)
