...
def loopAdvancePath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [Main.opAt 2215 .POP,
   Main.pushAt 2216 1 4,
   Main.opAt 2217 .ADD,
   Main.pushAt 2218 2 3197,
   Main.opAt 2219 .JUMP]

def normalReturnPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [Main.opAt 2220 .JUMPDEST,
   Main.opAt 2221 .POP,
   Main.pushAt 2222 0 0,
   Main.opAt 2223 .MSTORE,
   Main.pushAt 2224 1 32,
   Main.pushAt 2225 0 0,
   Main.opAt 2226 .RETURN]

def zeroReturnPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [Main.opAt 2227 .JUMPDEST,
   Main.pushAt 2228 0 0,
   Main.pushAt 2229 0 0,
   Main.opAt 2230 .MSTORE,
   Main.pushAt 2231 1 32,
   Main.pushAt 2232 0 0,
   Main.opAt 2233 .RETURN]
...
@[simp] theorem jump3555 :
  ...
  Artifact.isValidJumpDest_index 2220 (by rfl)
