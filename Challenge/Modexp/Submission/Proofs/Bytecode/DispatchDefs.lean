...
def zeroSizePath :
  ...
  [opAt 858 (.Dup ⟨0, by decide⟩),
   pushAt 859 2 1124, opAt 860 .JUMPI,
   pushAt 861 0 0, pushAt 862 0 0, opAt 863 .RETURN]

def wordEntryPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 858 (.Dup ⟨0, by decide⟩),
   pushAt 859 2 1124, opAt 860 .JUMPI,
   opAt 864 .JUMPDEST, opAt 865 (.Dup ⟨2, by decide⟩),
   pushAt 866 1 96, opAt 867 .ADD,
   opAt 868 (.Dup ⟨2, by decide⟩), opAt 869 (.Dup ⟨1, by decide⟩),
   opAt 870 .ADD, pushAt 871 1 32, opAt 872 (.Dup ⟨3, by decide⟩),
   opAt 873 .GT, pushAt 874 2 1154, opAt 875 .JUMPI,
   pushAt 876 2 1186, opAt 877 (.Dup ⟨1, by decide⟩),
   opAt 878 (.Dup ⟨3, by decide⟩), pushAt 879 1 96,
   opAt 880 (.Dup ⟨6, by decide⟩), opAt 881 (.Dup ⟨8, by decide⟩),
   opAt 882 (.Dup ⟨10, by decide⟩), pushAt 883 2 2223, opAt 884 .JUMP]

def zeroSetupPath := zeroSizePath.take 5
def zeroReturnPath := [opAt 863 .RETURN]
...
@[simp] theorem dispatchPCs (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC i =
      ([1116,1117,1120,1121,1122,1123,1124,1125,1126,1128,1129,1130,1131,1132,1134,1135,1136,1139,1140,1143,1144,1145,1147,1148,1149,1150,1153] : List Nat)[i - 858]! := by
...
@[simp] theorem jump1237 :
  ...
  Artifact.isValidJumpDest_index 864 (by rfl)

@[simp] theorem jump517 :
    Decode.isValidJumpDest submissionBytecode 524 = true :=
  Artifact.isValidJumpDest_index 435 (by rfl)

set_option maxRecDepth 20000 in
@[simp] theorem jump3000 :
    Decode.isValidJumpDest submissionBytecode 2223 = true := by
  have hpc : Artifact.instructionPC 1647 = 2223 := by decide
  simpa only [hpc] using Artifact.isValidJumpDest_index 1647 (by rfl)
...
def wordRouteEntryState (input : ByteArray) : State :=
  { wordEntryState input with pc := UInt256.ofNat 2223 }
