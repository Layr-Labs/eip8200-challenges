...
def loopContinueState (input : ByteArray) (pointer : Nat)
  ...
  { loopState input pointer accumulator with pc := UInt256.ofNat 3250 }
...
def finishState (input : ByteArray) (accumulator : UInt256) : State :=
  { loopState input 160 accumulator with pc := UInt256.ofNat 3599 }
...
abbrev LoopAdvanceStep (input : ByteArray) (pointer : Nat)
    (accumulator : UInt256) : Type :=
    Challenge.EvmProof.GasSteps (wordState input pointer 4 3591 accumulator)
