...
def loopContinueState (input : ByteArray) (pointer : Nat)
  ...
  { loopState input pointer accumulator with pc := UInt256.ofNat 2788 }
...
def finishState (input : ByteArray) (accumulator : UInt256) : State :=
  { loopState input 160 accumulator with pc := UInt256.ofNat 3007 }
