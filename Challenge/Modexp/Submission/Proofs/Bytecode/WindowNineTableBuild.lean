...
theorem run_build (template : State) (base modulus exponent : UInt256)
    ...
    runInstructions (buildProgram count)
      (WindowNineTable.state template (UInt256.ofNat 2693) base modulus exponent 2 rest) =
...
def program : List Instr := WindowNineTablePrelude.program ++ buildProgram 13
...
theorem run_all (template : State) (base modulus exponentOffset : UInt256)
    ...
    runInstructions program
      (WindowNineTablePrelude.initial template (UInt256.ofNat 2662) base modulus rest) =
    some (WindowNineTable.state template (UInt256.ofNat 2779) base modulus
      (MachineState.readWord template.executionEnv.calldata exponentOffset.toNat) 15 rest) := by
  have hp := WindowNineTablePrelude.run_prelude template (UInt256.ofNat 2662)
    base modulus exponentOffset rest hrest hoffset
  have hpc : WindowNineTablePrelude.endPC (UInt256.ofNat 2662) = UInt256.ofNat 2693 := by decide
  rw [hpc] at hp
