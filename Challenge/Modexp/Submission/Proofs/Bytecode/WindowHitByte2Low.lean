...
theorem run_prep (template : State) (base modulus : UInt256) (high : Nat)
    ...
    runLocatedBlock (lowPrepPath 2)
      (droppedNibbleState { template with halt := .Running } (UInt256.ofNat 3356)
        base modulus high (byteValue 2 word) word pointer accumulator rest) =
    some (lowNibbleState { template with halt := .Running } (UInt256.ofNat 3359)
...
theorem run_squareLookup (template : State) (base modulus : UInt256)
    ...
    runLocatedBlock (lowSquareLookupPath 2)
      (lowNibbleState { template with halt := .Running } (UInt256.ofNat 3359)
        base modulus nibble byte word pointer accumulator rest) =
    some (lowResultState { template with halt := .Running } (UInt256.ofNat 3390)
...
theorem run_finish (template : State) (base modulus : UInt256)
    ...
    runLocatedBlock (finishPath 2)
      (lowResultState { template with halt := .Running } (UInt256.ofNat 3390)
        base modulus nibble byte word pointer accumulator rest) =
