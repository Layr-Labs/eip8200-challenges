import Challenge.Modexp.Submission.Proofs.Bytecode.Artifact
set_option warningAsError true
set_option maxRecDepth 40000
set_option maxHeartbeats 4000000
/-! # Program-counter tables for the multi-limb exponentiation path

Kept in a module with minimal imports: `interval_cases … <;> decide` over these
ranges is elaborated far more cheaply without the whole `Big*` simp environment
in scope.
-/

namespace Challenge.Modexp.Submission.Proofs.Bytecode.BigExponent

open EvmSemantics
open EvmSemantics.EVM
open Challenge.Modexp

theorem jump310 :
    Decode.isValidJumpDest submissionBytecode 310 = true :=
  Artifact.isValidJumpDest_index 265 (by rfl)



theorem jump58 :
    Decode.isValidJumpDest submissionBytecode 58 = true :=
  Artifact.isValidJumpDest_index 46 (by rfl)



theorem jump1039 :
    Decode.isValidJumpDest submissionBytecode 1035 = true :=
  Artifact.isValidJumpDest_index 774 (by rfl)


theorem jump1090 :
    Decode.isValidJumpDest submissionBytecode 1086 = true :=
  Artifact.isValidJumpDest_index 813 (by rfl)


theorem jump963 :
    Decode.isValidJumpDest submissionBytecode 959 = true :=
  Artifact.isValidJumpDest_index 731 (by rfl)



theorem jump1104 :
    Decode.isValidJumpDest submissionBytecode 1100 = true :=
  Artifact.isValidJumpDest_index 824 (by rfl)


theorem jump946 :
    Decode.isValidJumpDest submissionBytecode 942 = true :=
  Artifact.isValidJumpDest_index 716 (by rfl)
end Challenge.Modexp.Submission.Proofs.Bytecode.BigExponent
