...
def headerLoadPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 897 0 0, opAt 898 .CALLDATALOAD,
   pushAt 899 1 32, opAt 900 .CALLDATALOAD,
   pushAt 901 1 64, opAt 902 .CALLDATALOAD]
...
def headerCheckPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 903 2 1224, opAt 904 .JUMP]
...
def tramp7Path := [opAt 557 .JUMPDEST, pushAt 558 2 1192, opAt 559 .JUMP,
  opAt 896 .JUMPDEST]
def tramp7JumpPath := [opAt 557 .JUMPDEST, pushAt 558 2 1192, opAt 559 .JUMP]
def tramp7DestPath := [opAt 896 .JUMPDEST]
...
@[simp] theorem headerPCs560 (i : Nat)
    (hi : 557 ≤ i) (hii : i ≤ 559) :
    Artifact.submissionArtifact.instructionPC i =
      [695,696,699][i - 557]! := by
  interval_cases i <;> decide

@[simp] theorem headerPCs899 (i : Nat)
    (hi : 896 ≤ i) (hii : i ≤ 916) :
    Artifact.submissionArtifact.instructionPC i =
      [1192,1193,1194,1195,1197,1198,1200,1201,1204,1205,1206,1209,1210,1211,1214,1215,1216,1217,1218,1219,1222][i - 896]! := by
  interval_cases i <;> decide
...
@[simp] theorem jump699 :
  ...
  Artifact.isValidJumpDest_index 557 (by rfl)
...
@[simp] theorem jump1228 :
  ...
  Artifact.isValidJumpDest_index 918 (by rfl)
