...
def headerLoadPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 847 0 0, opAt 848 .CALLDATALOAD,
   pushAt 849 1 32, opAt 850 .CALLDATALOAD,
   pushAt 851 3 64, opAt 852 .CALLDATALOAD]
...
def headerCheckPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 853 .JUMPDEST, opAt 854 .JUMPDEST]
...
def tramp0Path :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  []
def tramp7DestPath := [opAt 846 .JUMPDEST]
...
def headerEntryState (input : ByteArray) : State :=
  { initialState submissionBytecode input 0 with pc := UInt256.ofNat 1101 }
...
@[simp] theorem headerPCs899 (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC i =
      ([1100,1101,1102,1103,1105,1106,1110,1111,1112] : List Nat)[i - 846]! := by
  interval_cases i <;> decide
