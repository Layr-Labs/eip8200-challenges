...
def primeProgram : List Instr :=
  primeValueProgram ++ WindowTwentyOneEntry.testProgram (UInt256.ofNat 1775)

theorem run_entry (template : State) (b e m : UInt256)
    (htarget : Decode.isValidJumpDest template.executionEnv.code 1775 = true) :
    ...
    some (framed template
      (if (fermatDiff
        ...
        then UInt256.ofNat 45 else UInt256.ofNat 1775)
      [MachineState.readWord template.executionEnv.calldata
          ((UInt256.ofNat 96 + b) + UInt256.ofNat 32).toNat,
...
theorem run_prime (template : State) (modulus : UInt256) (rest : List UInt256)
    ...
    some (framed template
      (if (FermatProgram.primeValue modulus).toNat = 0
        then UInt256.ofNat 1775 else UInt256.ofNat 95)
      (modulus :: rest)) := by
  ...
  have hv : runInstructions primeValueProgram
      ...
      some (framed template (UInt256.ofNat 90)
    ...
    simp (config := { maxSteps := 500000 })
      [primeValueProgram, FermatProgram.primeValue, runInstructions, framed,
        ...
        Challenge.EvmProof.Word.succ_ofNat_mod, Challenge.EvmProof.Word.ofNat_add_mod]
  have ht := WindowTwentyOneEntry.run_test template (UInt256.ofNat 90)
    (UInt256.ofNat 1775) (FermatProgram.primeValue modulus) (modulus :: rest)
    (by simp; omega) htarget
  have both := runInstructions_append_some _ _ _ _ _ hv ht
  have hpc : advancePC 5 (UInt256.ofNat 90) = UInt256.ofNat 95 := by decide
...
structure Paths (artifact : Challenge.EvmProof.ProgramArtifact) (fork : Fork) where
  ...
  result : WindowTwentyOneBinding.Block artifact fork 95 FermatProgram.returnProgram
  legacyJump : Decode.isValidJumpDest artifact.code 1775 = true
