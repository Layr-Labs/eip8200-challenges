...
def loadGuardPath :
  ...
  [opAt 355 .JUMPDEST, opAt 356 (.Dup ⟨2, by decide⟩),
   opAt 357 (.Dup ⟨1, by decide⟩), opAt 358 .EQ, opAt 359 .JUMPDEST,
...
private theorem jump441 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 441 = true :=
...
private theorem jump484 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 484 = true :=
  Artifact.isValidJumpDest_index 387 (by rfl)

private theorem jump506 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 506 = true :=
...
theorem run_loadGuard (s : State) (offset length : Nat) (dst returnDest : UInt256)
  ...
  have hlmod : length % 2 ^ 256 = length := Nat.mod_eq_of_lt hlength
  have hne : i ≠ length := Nat.ne_of_lt hi
  have heq : UInt256.eq (UInt256.ofNat i) (UInt256.ofNat length) =
      UInt256.ofNat 0 := by
    rw [UInt256.eq, Challenge.EvmProof.Word.word_toNat_ofNat,
      Challenge.EvmProof.Word.word_toNat_ofNat, himod, hlmod, if_neg hne]
  ...
  simp (disch := omega) [loadGuardPath, opAt, pushAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    loadLoop, loadSetupPCs, hrun, UInt256.isTrue, hi, hi256, hlength,
    himod, hlmod, hne, heq, hpc,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt,
...
theorem run_loadFinishGuard (s : State) (offset length : Nat)
  ...
  have heq : UInt256.eq (UInt256.ofNat length) (UInt256.ofNat length) =
      UInt256.ofNat 1 := by
    simp [UInt256.eq]
  ...
  simp (disch := omega) [loadGuardPath, opAt, pushAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    loadLoop, loadSetupPCs, hcode, hrun, UInt256.isTrue, UInt256.eq,
    heq, hvalid, jump506, Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt,
...
theorem loadGuard_staticCost :
    Challenge.EvmProof.Meter.runLocatedBlockStaticCost loadGuardPath = 24 := by
  decide
...
theorem gasSteps_loadLoop_cost_potential (s : State) (offset length : Nat)
    ...
    (gasSteps_loadLoop s offset length dst returnDest rest hcap hoffset
        ...
        MachineState.memCost
          (loadLoop s (UInt256.ofNat offset) (UInt256.ofNat length) dst 0
            returnDest rest).activeWords.toNat =
      length * 188 + MachineState.memCost
        (loadLoop s (UInt256.ofNat offset) (UInt256.ofNat length) dst length
          returnDest rest).activeWords.toNat := by
  ...
  have htelescope := Challenge.EvmProof.GasSteps.iterateBounded_cost_potential_eq
    length 188 (fun i => MachineState.memCost
      (loadLoop s (UInt256.ofNat offset) (UInt256.ofNat length) dst i
...
theorem gasSteps_loadBigEndian_cost_potential (s : State)
    ...
    (gasSteps_loadBigEndian s offset length dst returnDest rest hcap
      ...
      (44 + length * 188) + MachineState.memCost
        (loadReturned s (UInt256.ofNat offset) (UInt256.ofNat length) dst
          returnDest rest).activeWords.toNat := by
