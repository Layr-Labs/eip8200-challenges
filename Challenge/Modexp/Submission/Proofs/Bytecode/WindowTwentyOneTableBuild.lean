...
def tablePC (power : Nat) : Nat := 2364 + 5 * power
...
theorem run_build (template : State) (base modulus exponent : UInt256)
    ...
    runInstructionsX (buildProgram count)
      (WindowTwentyOneTable.state template (UInt256.ofNat 2374) base modulus exponent 2 rest) =
...
/-- The complete 94-byte table construction, including the exponent load. -/
theorem run_all (template : State) (base modulus exponentOffset : UInt256)
    ...
    runInstructionsX program
      (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2345) base modulus rest) =
    some (WindowTwentyOneTable.framed template (UInt256.ofNat 2438) base modulus 16
      ([base, MachineState.readWord template.executionEnv.calldata exponentOffset.toNat] ++ rest)) := by
  have hp := WindowTwentyOneTablePrelude.run_prelude template (UInt256.ofNat 2345)
    base modulus exponentOffset rest hrest hoffset
  have hpc : WindowTwentyOneTablePrelude.endPC (UInt256.ofNat 2345) = UInt256.ofNat 2374 := by decide
  rw [hpc] at hp
