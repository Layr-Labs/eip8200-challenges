...
def tablePC (power : Nat) : Nat := 1814 + 5 * power
...
theorem run_build (template : State) (base modulus exponent : UInt256)
    ...
    runInstructionsX (buildProgram count)
      (WindowTwentyOneTable.state template (UInt256.ofNat 1824) base modulus exponent 2 rest) =
...
theorem run_all (template : State) (base modulus exponentOffset : UInt256)
    ...
    runInstructionsX program
      (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 1795) base modulus rest) =
    some (WindowTwentyOneTable.framed template (UInt256.ofNat 1888) base modulus 16
      ([base, MachineState.readWord template.executionEnv.calldata exponentOffset.toNat] ++ rest)) := by
  have hp := WindowTwentyOneTablePrelude.run_prelude template (UInt256.ofNat 1795)
    base modulus exponentOffset rest hrest hoffset
  have hpc : WindowTwentyOneTablePrelude.endPC (UInt256.ofNat 1795) = UInt256.ofNat 1824 := by decide
  rw [hpc] at hp
