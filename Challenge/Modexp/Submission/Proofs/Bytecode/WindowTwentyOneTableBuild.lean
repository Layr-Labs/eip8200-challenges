...
def tablePC (power : Nat) : Nat :=
  if power ≤ 7 then 2361 + 6 * power else 2354 + 7 * power
...
theorem run_build (template : State) (base modulus exponent : UInt256)
    ...
    runInstructions (buildProgram count)
      (WindowTwentyOneTable.state template (UInt256.ofNat 2373) base modulus exponent 2 rest) =
...
theorem run_all (template : State) (base modulus exponentOffset : UInt256)
    ...
    some (WindowTwentyOneTable.framed template (UInt256.ofNat 2458) base modulus 16
  ...
  have hpc : WindowTwentyOneTablePrelude.endPC (UInt256.ofNat 2342) = UInt256.ofNat 2373 := by decide
  rw [hpc] at hp
