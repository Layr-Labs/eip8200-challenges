...
def tablePC (power : Nat) : Nat :=
  if power ≤ 7 then 2484 + 6 * power else 2477 + 7 * power
...
theorem run_build (template : State) (base modulus exponent : UInt256)
    ...
    runInstructions (buildProgram count)
      (WindowTwentyOneTable.state template (UInt256.ofNat 2496) base modulus exponent 2 rest) =
...
theorem run_all (template : State) (base modulus exponentOffset : UInt256)
    ...
    runInstructions program
      (WindowTwentyOneTablePrelude.initial template (UInt256.ofNat 2465) base modulus rest) =
    some (WindowTwentyOneTable.framed template (UInt256.ofNat 2581) base modulus 16
      ([base, MachineState.readWord template.executionEnv.calldata exponentOffset.toNat] ++ rest)) := by
  have hp := WindowTwentyOneTablePrelude.run_prelude template (UInt256.ofNat 2465)
    base modulus exponentOffset rest hrest hoffset
  have hpc : WindowTwentyOneTablePrelude.endPC (UInt256.ofNat 2465) = UInt256.ofNat 2496 := by decide
  rw [hpc] at hp
