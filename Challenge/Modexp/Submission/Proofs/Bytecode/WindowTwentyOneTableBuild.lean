...
def tablePC (power : Nat) : Nat :=
  if power ≤ 7 then 2359 + 6 * power else 2352 + 7 * power
...
theorem run_all (template : State) (base modulus exponentOffset : UInt256)
    ...
    some (WindowTwentyOneTable.framed template (UInt256.ofNat 2456) base modulus 16
  ...
  have hpc : WindowTwentyOneTablePrelude.endPC (UInt256.ofNat 2340) = UInt256.ofNat 2371 := by decide
  rw [hpc] at hp
