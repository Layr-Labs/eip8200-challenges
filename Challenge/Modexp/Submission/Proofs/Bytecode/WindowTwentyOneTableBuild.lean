...
def tablePC (power : Nat) : Nat :=
  if power ≤ 7 then 2364 + 6 * power else 2357 + 7 * power
...
theorem run_build (template : State) (base modulus exponent : UInt256)
    ...
    runInstructions (buildProgram count)
      (WindowTwentyOneTable.state template (UInt256.ofNat 2376) base modulus exponent 2 rest) =
...
theorem run_all (template : State) (base modulus exponentOffset : UInt256)
    ...
    some (WindowTwentyOneTable.framed template (UInt256.ofNat 2461) base modulus 16
  ...
  have hpc : WindowTwentyOneTablePrelude.endPC (UInt256.ofNat 2345) = UInt256.ofNat 2376 := by decide
  rw [hpc] at hp
