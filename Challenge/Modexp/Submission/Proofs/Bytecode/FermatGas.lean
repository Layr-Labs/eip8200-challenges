...
def handled {artifact : ProgramArtifact} {fork : Fork}
    ...
    ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 4798)) final) ∧
  ...
  · rw [if_pos hprime] at hpraw
    have headSteps : GasSteps (state template input (UInt256.ofNat 4798))
  ...
  · rw [if_neg hprime] at hpraw
    have headSteps : GasSteps (state template input (UInt256.ofNat 4798))
        (withModulus template input (UInt256.ofNat 4851)) :=
      paths.prime.steps (ec.transfer rfl rfl) rfl hpraw
    ...
    · rw [if_pos he] at heraw
      have expSteps : GasSteps (withModulus template input (UInt256.ofNat 4851))
          (WindowTwentyOneGasRoute.entryState template input) :=
    ...
    · rw [if_neg he] at heraw
      have expSteps : GasSteps (withModulus template input (UInt256.ofNat 4851))
          (withModulus template input (UInt256.ofNat 4863)) :=
        paths.exponent.steps (ec.transfer rfl rfl) rfl heraw
      ...
      have hexponent : WindowTwentyOneInput.exponentValue input =
        ...
        exact h.symm
      let final := WindowTwentyOneReturn.returned (context template input) (UInt256.ofNat 4881)
      ...
      change runInstructions FermatProgram.returnProgram
        (withModulus template input (UInt256.ofNat 4863)) = some final at hr
      have resultSteps : GasSteps (withModulus template input (UInt256.ofNat 4863)) final :=
      ...
      · change (true && template.callStack.isEmpty) = true
        rw [hcall]
        rfl
      · have h := WindowTwentyOneReturn.returned_result (context template input) (UInt256.ofNat 4881)
          (FermatMath.resultWord input) 0 (routeStack input)
