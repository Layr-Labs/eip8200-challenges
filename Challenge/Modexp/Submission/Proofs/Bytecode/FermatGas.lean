...
def handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  have hpraw := FermatProgram.run_prime (context template input) (modulusOffset input)
    (routeStack input) (by simp [routeStack]) (by simp [routeStack]) hlegacyJump
  ...
  · rw [if_neg hprime] at hpraw
    have headSteps : GasSteps (state template input (UInt256.ofNat 4812))
        (state template input (UInt256.ofNat 4865)) :=
      paths.prime.steps (ec.transfer rfl rfl) rfl hpraw
    have heraw := FermatProgram.run_exponent (context template input) m (exponentOffset input)
      (modulusOffset input) (routeStack input) (by simp [routeStack]) (by simp [routeStack])
      (by simp [routeStack]) (modulus_at template input hmatch) hlegacyJump
    ...
    · rw [if_pos he] at heraw
      have expSteps : GasSteps (state template input (UInt256.ofNat 4865))
          (state template input (UInt256.ofNat 2359)) :=
    ...
    · rw [if_neg he] at heraw
      have expSteps : GasSteps (state template input (UInt256.ofNat 4865))
          (state template input (UInt256.ofNat 4878)) :=
        paths.exponent.steps (ec.transfer rfl rfl) rfl heraw
      ...
      have hexponent : WindowTwentyOneInput.exponentValue input =
        ...
        exact h.symm
      let final := WindowTwentyOneReturn.returned (context template input) (UInt256.ofNat 4898)
        (FermatMath.resultWord input) 0 (routeStack input)
      have hr := FermatProgram.run_result (context template input) m (modulusOffset input)
        (UInt256.ofNat 96) (routeStack input) (baseSize input) hmatch.1 (by simp [routeStack])
        (by simp [routeStack]) (by simp [routeStack]) (by simp [routeStack])
        (modulus_at template input hmatch) 0 (by decide) rfl
      change runInstructions FermatProgram.resultProgram
        (state template input (UInt256.ofNat 4878)) = some final at hr
      have resultSteps : GasSteps (state template input (UInt256.ofNat 4878)) final :=
      ...
      · change (true && template.callStack.isEmpty) = true
        rw [hcall]
        rfl
      · have h := WindowTwentyOneReturn.returned_result (context template input) (UInt256.ofNat 4898)
          (FermatMath.resultWord input) 0 (routeStack input)
