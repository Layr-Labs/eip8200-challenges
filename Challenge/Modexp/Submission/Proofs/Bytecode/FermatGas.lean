...
def handled {artifact : ProgramArtifact} {fork : Fork}
    ...
    ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 5194)) final) ∧
  ...
  have hjump : Decode.isValidJumpDest (context template input).executionEnv.code 5277 = true := by
    rw [ec.code]
    exact paths.missJump
  have hlegacyJump : Decode.isValidJumpDest (context template input).executionEnv.code 2627 = true := by
    rw [ec.code]
  ...
  have loadSteps : GasSteps (state template input (UInt256.ofNat 5194))
      (withModulus template input (UInt256.ofNat 5197)) :=
  ...
  have missSteps : GasSteps (withModulus template input (UInt256.ofNat 5277))
      (state template input (UInt256.ofNat 2627)) :=
  ...
  · rw [if_pos he] at heraw
    have expSteps : GasSteps (withModulus template input (UInt256.ofNat 5197))
        (withModulus template input (UInt256.ofNat 5208)) :=
    ...
    · rw [if_pos hprime] at hpraw
      have primeSteps : GasSteps (withModulus template input (UInt256.ofNat 5208))
          (withModulus template input (UInt256.ofNat 5277)) :=
    ...
    · rw [if_neg hprime] at hpraw
      have primeSteps : GasSteps (withModulus template input (UInt256.ofNat 5208))
          (withModulus template input (UInt256.ofNat 5258)) :=
        paths.prime.steps (ec.transfer rfl rfl) rfl hpraw
      ...
      have hexponent : WindowTwentyOneInput.exponentValue input =
        ...
        exact h.symm
      let final := WindowTwentyOneReturn.returned (context template input) (UInt256.ofNat 5276)
      ...
      change runInstructions FermatProgram.returnProgram
        (withModulus template input (UInt256.ofNat 5258)) = some final at hr
      have resultSteps : GasSteps (withModulus template input (UInt256.ofNat 5258)) final :=
      ...
      · change (true && template.callStack.isEmpty) = true
        rw [hcall]
        rfl
      · have h := WindowTwentyOneReturn.returned_result (context template input) (UInt256.ofNat 5276)
          (FermatMath.resultWord input) 0 (routeStack input)
