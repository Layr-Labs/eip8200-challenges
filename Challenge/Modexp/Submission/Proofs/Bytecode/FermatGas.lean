...
def handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  have hjump : Decode.isValidJumpDest (context template input).executionEnv.code 2334 = true := by
    rw [ec.code]
  ...
  · rw [if_pos hprime] at hpraw
    have headSteps : GasSteps (state template input (UInt256.ofNat 43))
  ...
  · rw [if_neg hprime] at hpraw
    ...
    · rw [if_pos he] at heraw
      have expSteps : GasSteps (withModulus template input (UInt256.ofNat 96))
          (WindowTwentyOneGasRoute.entryState template input) :=
