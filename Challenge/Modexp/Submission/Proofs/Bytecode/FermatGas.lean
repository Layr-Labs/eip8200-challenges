...
def handled {artifact : ProgramArtifact} {fork : Fork}
  ...
  have hjump : Decode.isValidJumpDest (context template input).executionEnv.code 1775 = true := by
    rw [ec.code]
  ...
  have hraw' : runInstructions FermatNext.entryProgram (entryState template input) =
      some (WindowTwentyOneEntry.framed (context template input)
        (if (FermatNext.fermatDiff m e).toNat = 0 then UInt256.ofNat 45 else UInt256.ofNat 1775)
  ...
  · rw [if_pos he] at hraw'
    ...
    · rw [if_neg hprime] at hpraw
      have primeSteps : GasSteps (withModulus template input (UInt256.ofNat 45))
          (withModulus template input (UInt256.ofNat 95)) :=
        paths.prime.steps (ec.transfer rfl rfl) rfl hpraw
      ...
      have hexponent : WindowTwentyOneInput.exponentValue input =
        ...
        exact h.symm
      let final := WindowTwentyOneReturn.returned (context template input) (UInt256.ofNat 114)
      ...
      change runInstructions FermatProgram.returnProgram
        (withModulus template input (UInt256.ofNat 95)) = some final at hr
      have resultSteps : GasSteps (withModulus template input (UInt256.ofNat 95)) final :=
      ...
      · change (true && template.callStack.isEmpty) = true
        rw [hcall]
        rfl
      · have h := WindowTwentyOneReturn.returned_result (context template input) (UInt256.ofNat 114)
          (FermatMath.resultWord input) 0 (coreStack input)
