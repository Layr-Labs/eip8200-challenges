...
def handled {artifact : ProgramArtifact} {fork : Fork}
    ...
    ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 4873)) final) ∧
  ...
  have hjump : Decode.isValidJumpDest (context template input).executionEnv.code 4957 = true := by
    rw [ec.code]
  ...
  have missSteps : GasSteps (withModulus template input (UInt256.ofNat 4957))
  ...
  · rw [if_pos hprime] at hpraw
    have headSteps : GasSteps (state template input (UInt256.ofNat 4873))
        (withModulus template input (UInt256.ofNat 4957)) :=
  ...
  · rw [if_neg hprime] at hpraw
    have headSteps : GasSteps (state template input (UInt256.ofNat 4873))
        (withModulus template input (UInt256.ofNat 4926)) :=
      paths.prime.steps (ec.transfer rfl rfl) rfl hpraw
    ...
    · rw [if_pos he] at heraw
      have expSteps : GasSteps (withModulus template input (UInt256.ofNat 4926))
          (withModulus template input (UInt256.ofNat 4957)) :=
    ...
    · rw [if_neg he] at heraw
      ...
      have hexponent : WindowTwentyOneInput.exponentValue input =
        ...
        exact h.symm
      let final := WindowTwentyOneReturn.returned (context template input) (UInt256.ofNat 4956)
      ...
      change runInstructions FermatProgram.returnProgram
        (withModulus template input (UInt256.ofNat 4938)) = some final at hr
      have resultSteps : GasSteps (withModulus template input (UInt256.ofNat 4938)) final :=
      ...
      · change (true && template.callStack.isEmpty) = true
        rw [hcall]
        rfl
      · have h := WindowTwentyOneReturn.returned_result (context template input) (UInt256.ofNat 4956)
          (FermatMath.resultWord input) 0 (routeStack input)
