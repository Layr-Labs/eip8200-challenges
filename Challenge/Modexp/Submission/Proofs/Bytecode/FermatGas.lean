...
def handled {artifact : ProgramArtifact} {fork : Fork}
    ...
    ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 5185)) final) ∧
  ...
  have hjump : Decode.isValidJumpDest (context template input).executionEnv.code 5268 = true := by
    rw [ec.code]
  ...
  have loadSteps : GasSteps (state template input (UInt256.ofNat 5185))
      (withModulus template input (UInt256.ofNat 5188)) :=
  ...
  have missSteps : GasSteps (withModulus template input (UInt256.ofNat 5268))
  ...
  · rw [if_pos he] at heraw
    have expSteps : GasSteps (withModulus template input (UInt256.ofNat 5188))
        (withModulus template input (UInt256.ofNat 5199)) :=
    ...
    · rw [if_pos hprime] at hpraw
      have primeSteps : GasSteps (withModulus template input (UInt256.ofNat 5199))
          (withModulus template input (UInt256.ofNat 5268)) :=
    ...
    · rw [if_neg hprime] at hpraw
      have primeSteps : GasSteps (withModulus template input (UInt256.ofNat 5199))
          (withModulus template input (UInt256.ofNat 5249)) :=
        paths.prime.steps (ec.transfer rfl rfl) rfl hpraw
      ...
      have hexponent : WindowTwentyOneInput.exponentValue input =
        ...
        exact h.symm
      let final := WindowTwentyOneReturn.returned (context template input) (UInt256.ofNat 5267)
      ...
      change runInstructions FermatProgram.returnProgram
        (withModulus template input (UInt256.ofNat 5249)) = some final at hr
      have resultSteps : GasSteps (withModulus template input (UInt256.ofNat 5249)) final :=
      ...
      · change (true && template.callStack.isEmpty) = true
        rw [hcall]
        rfl
      · have h := WindowTwentyOneReturn.returned_result (context template input) (UInt256.ofNat 5267)
          (FermatMath.resultWord input) 0 (routeStack input)
