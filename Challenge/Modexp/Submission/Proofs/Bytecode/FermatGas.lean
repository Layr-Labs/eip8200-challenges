...
def handled {artifact : ProgramArtifact} {fork : Fork}
    ...
    ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 5188)) final) ∧
  ...
  have hjump : Decode.isValidJumpDest (context template input).executionEnv.code 5271 = true := by
    rw [ec.code]
  ...
  have loadSteps : GasSteps (state template input (UInt256.ofNat 5188))
      (withModulus template input (UInt256.ofNat 5191)) :=
  ...
  have missSteps : GasSteps (withModulus template input (UInt256.ofNat 5271))
  ...
  · rw [if_pos he] at heraw
    have expSteps : GasSteps (withModulus template input (UInt256.ofNat 5191))
        (withModulus template input (UInt256.ofNat 5202)) :=
    ...
    · rw [if_pos hprime] at hpraw
      have primeSteps : GasSteps (withModulus template input (UInt256.ofNat 5202))
          (withModulus template input (UInt256.ofNat 5271)) :=
    ...
    · rw [if_neg hprime] at hpraw
      have primeSteps : GasSteps (withModulus template input (UInt256.ofNat 5202))
          (withModulus template input (UInt256.ofNat 5252)) :=
        paths.prime.steps (ec.transfer rfl rfl) rfl hpraw
      ...
      have hexponent : WindowTwentyOneInput.exponentValue input =
        ...
        exact h.symm
      let final := WindowTwentyOneReturn.returned (context template input) (UInt256.ofNat 5270)
      ...
      change runInstructions FermatProgram.returnProgram
        (withModulus template input (UInt256.ofNat 5252)) = some final at hr
      have resultSteps : GasSteps (withModulus template input (UInt256.ofNat 5252)) final :=
      ...
      · change (true && template.callStack.isEmpty) = true
        rw [hcall]
        rfl
      · have h := WindowTwentyOneReturn.returned_result (context template input) (UInt256.ofNat 5270)
          (FermatMath.resultWord input) 0 (routeStack input)
