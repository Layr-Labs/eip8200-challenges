...
def handled {artifact : ProgramArtifact} {fork : Fork}
    ...
    ∃ final : State, Nonempty (GasSteps (state template input (UInt256.ofNat 5160)) final) ∧
  ...
  have hjump : Decode.isValidJumpDest (context template input).executionEnv.code 5243 = true := by
    rw [ec.code]
    exact paths.missJump
  have hlegacyJump : Decode.isValidJumpDest (context template input).executionEnv.code 2656 = true := by
    rw [ec.code]
  ...
  have loadSteps : GasSteps (state template input (UInt256.ofNat 5160))
      (withModulus template input (UInt256.ofNat 5163)) :=
  ...
  have missSteps : GasSteps (withModulus template input (UInt256.ofNat 5243))
      (state template input (UInt256.ofNat 2656)) :=
  ...
  · rw [if_pos he] at heraw
    have expSteps : GasSteps (withModulus template input (UInt256.ofNat 5163))
        (withModulus template input (UInt256.ofNat 5174)) :=
    ...
    · rw [if_pos hprime] at hpraw
      have primeSteps : GasSteps (withModulus template input (UInt256.ofNat 5174))
          (withModulus template input (UInt256.ofNat 5243)) :=
    ...
    · rw [if_neg hprime] at hpraw
      have primeSteps : GasSteps (withModulus template input (UInt256.ofNat 5174))
          (withModulus template input (UInt256.ofNat 5224)) :=
        paths.prime.steps (ec.transfer rfl rfl) rfl hpraw
      ...
      have hexponent : WindowTwentyOneInput.exponentValue input =
        ...
        exact h.symm
      let final := WindowTwentyOneReturn.returned (context template input) (UInt256.ofNat 5242)
      ...
      change runInstructions FermatProgram.returnProgram
        (withModulus template input (UInt256.ofNat 5224)) = some final at hr
      have resultSteps : GasSteps (withModulus template input (UInt256.ofNat 5224)) final :=
      ...
      · change (true && template.callStack.isEmpty) = true
        rw [hcall]
        rfl
      · have h := WindowTwentyOneReturn.returned_result (context template input) (UInt256.ofNat 5242)
          (FermatMath.resultWord input) 0 (routeStack input)
