import Challenge.Modexp.Submission.Proofs.Bytecode.MonproKNMidWords

set_option warningAsError true

namespace Challenge.Modexp.Submission.Proofs.Bytecode.MonproKNMidProduct
...
theorem program_eq : productProgram =
    (loadMuProgram ++ loadM0Program) ++ MonproKNMidWords.program := rfl

theorem run_loadMu (s : State) (n : Nat) (bi pbi paEnd pbEnd dst ret : UInt256)
    ...
    runInstructions loadMuProgram
      (framed s (UInt256.ofNat 2027) (baseStack bi pbi paEnd pbEnd dst ret rest)) =
    some (framed s (UInt256.ofNat 2037)
      ([rowMu s.memory n] ++ baseStack bi pbi paEnd pbEnd dst ret rest)) := by
...
theorem run_loadM0 (s : State) (n : Nat) (mu bi pbi paEnd pbEnd dst ret : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 1007) (hact : 296 ≤ s.activeWords.toNat)
    (hn32 : n ≤ 32)
    (hml : MachineState.readWord s.memory 9408 = UInt256.ofNat (32*n-32)) :
    runInstructions loadM0Program
      (framed s (UInt256.ofNat 2037) ([mu] ++ baseStack bi pbi paEnd pbEnd dst ret rest)) =
    some (framed s (UInt256.ofNat 2043)
...
theorem run_product (s : State) (n : Nat) (bi pbi paEnd pbEnd dst ret : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 1007) (hact : 296 ≤ s.activeWords.toNat)
    (_hn : 2 ≤ n) (hn32 : n ≤ 32)
    (hml : MachineState.readWord s.memory 9408 = UInt256.ofNat (32*n-32))
    (htl : MachineState.readWord s.memory 9440 = UInt256.ofNat (8224+32*n)) :
    runInstructions productProgram
      (framed s (UInt256.ofNat 2027) (baseStack bi pbi paEnd pbEnd dst ret rest)) =
    some (product s n bi pbi paEnd pbEnd dst ret rest) := by
  rw [program_eq]
  exact runInstructions_append_some _ _ _ _ _
    (runInstructions_append_some _ _ _ _ _
      (run_loadMu s n bi pbi paEnd pbEnd dst ret rest hcap hact hn32 htl)
      (run_loadM0 s n (rowMu s.memory n) bi pbi paEnd pbEnd dst ret rest hcap hact hn32 hml))
    (MonproKNMidWords.run_words s (MachineState.readWord s.memory (32*n-32)) (rowMu s.memory n)
      bi pbi paEnd pbEnd dst ret rest hcap)

end Challenge.Modexp.Submission.Proofs.Bytecode.MonproKNMidProduct
