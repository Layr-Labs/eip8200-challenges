...
def bigCheckJumpPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 934 2 1257, opAt 935 .JUMPI]
...
def bigTailFramePath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 946 .JUMPDEST, pushAt 947 2 1272,
...
def bigTailJumpPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 954 2 693, opAt 955 .JUMP]
...
theorem jump704 : Decode.isValidJumpDest submissionBytecode 693 = true :=
  Artifact.isValidJumpDest_index 559 (by rfl)

theorem jump1268 : Decode.isValidJumpDest submissionBytecode 1257 = true :=
