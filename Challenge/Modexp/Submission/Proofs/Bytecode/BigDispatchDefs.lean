...
def bigCheckComparePath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 871 1 32, opAt 872 (.Dup ⟨3, by decide⟩), opAt 873 .GT]

def bigCheckJumpPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 874 2 1154, opAt 875 .JUMPI]
...
def bigTailFramePath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 885 .JUMPDEST, pushAt 886 2 1202,
   opAt 887 (.Dup ⟨1, by decide⟩), opAt 888 (.Dup ⟨3, by decide⟩)]

def bigTailArgsPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 889 1 96, opAt 890 (.Dup ⟨6, by decide⟩),
   opAt 891 (.Dup ⟨8, by decide⟩), opAt 892 (.Dup ⟨10, by decide⟩)]

def bigTailJumpPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 893 2 637, opAt 894 .JUMP]
...
def bigCheckedState (input : ByteArray) : State :=
  { Dispatch.wordCheckedState input with pc := UInt256.ofNat 1154 }
...
@[simp] theorem bigTailPCs (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC i =
      ([1154,1155,1158,1159,1160,1162,1163,1164,1165,1168] : List Nat)[i - 885]! := by
  interval_cases i <;> decide

theorem jump704 : Decode.isValidJumpDest submissionBytecode 637 = true :=
  Artifact.isValidJumpDest_index 524 (by rfl)

theorem jump1268 : Decode.isValidJumpDest submissionBytecode 1154 = true :=
