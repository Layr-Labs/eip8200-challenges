...
theorem run_prep (template : State) (base modulus : UInt256) (high : Nat)
    ...
    runLocatedBlock (lowPrepPath 1)
      (droppedNibbleState { template with halt := .Running } (UInt256.ofNat 3285)
        base modulus high (byteValue 1 word) word pointer accumulator rest) =
    some (lowNibbleState { template with halt := .Running } (UInt256.ofNat 3288)
  ...
  have hlow := mask_lowNibble 1 word
...
theorem run_squareLookup (template : State) (base modulus : UInt256)
    ...
    runLocatedBlock (lowSquareLookupPath 1)
      (lowNibbleState { template with halt := .Running } (UInt256.ofNat 3288)
        base modulus nibble byte word pointer accumulator rest) =
    some (lowResultState { template with halt := .Running } (UInt256.ofNat 3319)
...
theorem run_finish (template : State) (base modulus : UInt256)
    ...
    runLocatedBlock (finishPath 1)
      (lowResultState { template with halt := .Running } (UInt256.ofNat 3319)
        base modulus nibble byte word pointer accumulator rest) =
