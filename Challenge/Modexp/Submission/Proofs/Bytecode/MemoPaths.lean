...
/-- The width test: seven instructions ending in the first miss branch. -/
def widthPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 572 .JUMPDEST,
   pushAt 573 0 0,
   opAt 574 .CALLDATALOAD,
   pushAt 575 1 1,
   opAt 576 .SUB,
   pushAt 577 2 599,
   opAt 578 .JUMPI]

/-- The tail test: fourteen instructions ending in the second miss branch. -/
def tailPath :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 579 1 32,
   ...
   pushAt 581 1 2,
   opAt 582 .SUB,
   pushAt 583 5 137506062208,
   pushAt 584 1 68,
   opAt 585 .CALLDATALOAD,
   opAt 586 .SUB,
   opAt 587 .OR,
   pushAt 588 1 100,
...
@[simp] theorem widthPCs (i : Nat) (hi : 572 ≤ i) (hii : i ≤ 578) :
    Artifact.submissionArtifact.instructionPC i =
      ([795, 796, 797, 798, 800, 801, 804] : List Nat)[i - 572]! := by
  rw [Challenge.Modexp.Submission.Proofs.Bytecode.PCFast.instructionPC_eq_byteLength]
  interval_cases i <;> rfl

@[simp] theorem tailPCs (i : Nat) (hi : 579 ≤ i) (hii : i ≤ 592) :
    Artifact.submissionArtifact.instructionPC i =
      ([805, 807, 808, 810, 811, 817, 819, 820, 821, 822, 824, 825, 826, 829] : List Nat)[i - 579]! := by
  rw [Challenge.Modexp.Submission.Proofs.Bytecode.PCFast.instructionPC_eq_byteLength]
  interval_cases i <;> rfl
