...
def guardPath :
  ...
  [opAt 4211 .JUMPDEST,
   opAt 4212 (.Dup ⟨2, by decide⟩),
   pushAt 4213 1 1,
   opAt 4214 .SUB,
   pushAt 4215 5 137506062208,
   pushAt 4216 1 68,
   opAt 4217 .CALLDATALOAD,
   opAt 4218 .SUB,
   opAt 4219 .OR,
   pushAt 4220 1 100,
   opAt 4221 .CALLDATALOAD,
   opAt 4222 .OR,
   opAt 4223 (.Dup ⟨2, by decide⟩),
   pushAt 4224 1 2,
   opAt 4225 .SUB,
   opAt 4226 .OR,
   pushAt 4227 2 570,
...
@[simp] theorem guardPCs (i : Nat) (hi : 4211 ≤ i) (hii : i ≤ 4231) :
    Artifact.submissionArtifact.instructionPC i =
      ([5251,5252,5253,5255,5256,5262,5264,5265,5266,5267,5269,5270,5271,5272,
        5274,5275,5276,5279,5280,5281,5284] : List Nat)[i - 4211]! := by
  rw [Challenge.Modexp.Submission.Proofs.Bytecode.PCFast.instructionPC_eq_byteLength]
  interval_cases i <;> rfl

@[simp] theorem answerPCs (i : Nat) (hi : 4351 ≤ i) (hii : i ≤ 4358) :
    Artifact.submissionArtifact.instructionPC i =
      ([5428,5429,5432,5434,5435,5436,5437,5438] : List Nat)[i - 4351]! := by
  rw [Challenge.Modexp.Submission.Proofs.Bytecode.PCFast.instructionPC_eq_byteLength]
  interval_cases i <;> rfl
