import Challenge.Modexp.Submission.Proofs.Bytecode.MonproKNEntryPointerWords

set_option warningAsError true

namespace Challenge.Modexp.Submission.Proofs.Bytecode.MonproKNEntryPointers
...
theorem run_pointers (s : State) (mem : ByteArray) (pa pb n : Nat)
    ...
    runInstructions pointersProgram (cleared s mem pa pb n dst ret rest) =
  ...
  have h := MonproKNEntryPointerWords.run_words { s with memory := mpZeroed s mem n } (UInt256.ofNat (32*n))
    (UInt256.ofNat pa) (UInt256.ofNat pb) dst ret rest hcap
  simpa only [cleared, outer, framed, hp0, Challenge.EvmProof.Word.ofNat_add_mod,
    hsub1, hsub2, hsub3] using h

end Challenge.Modexp.Submission.Proofs.Bytecode.MonproKNEntryPointers
