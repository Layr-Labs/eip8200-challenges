...
def l2BodyFor (k : Nat) : List Instr :=
  ...
  else if k = 2 then partialL2Body (UInt256.ofNat 64) (UInt256.ofNat 8352)
  else CiosCached.l2Program (UInt256.ofNat (32*k))
    (UInt256.ofNat (8256+32*k)) (UInt256.ofNat (8256+32*(k+1)))

def l2BodySize (k : Nat) : Nat := if k < 2 then 30 else if k = 2 then 35 else 38
...
theorem run_l2Step (pc : Nat) (s : State) (c : CachedMemory) (r : ReadOnlyCache) (bi mu c0 : UInt256)
  ...
  · by_cases heq : n-2-k = 2
    · have hx : MachineState.readWord p.cache.virtual (32*(n-2-k)) =
      ...
      have trace := run_partialL2Body st (UInt256.ofNat pc) p.cache r
        (UInt256.ofNat (ptrAt (pb+32*n-32) i)) (UInt256.ofNat (pa+32*n-32))
      ...
      rw [hx, ht, hn1, heq]
      change runInstructions (partialL2Body (UInt256.ofNat 64) (UInt256.ofNat 8352)) _ =
        some (rowState s (pc+35) (p.cache.write 8352
    ...
    · let x := UInt256.ofNat (32*(n-2-k))
      ...
      have trace := run_memoryL2Body st (UInt256.ofNat pc) p.cache r
        (UInt256.ofNat (ptrAt (pb+32*n-32) i)) (UInt256.ofNat (pa+32*n-32))
