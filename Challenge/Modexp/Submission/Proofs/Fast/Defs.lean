...
@[simp] theorem fastPC21 (i : Nat) (hi : 1784 ≤ i) (hii : i ≤ 1809) :
    Artifact.submissionArtifact.instructionPC i =
      [2936,2937,2940,2941,2942,2943,2946,2947,2948,2950,2951,2954,2955,2956,2957,2960,2961,2962,2964,2965,2966,2967,2970,2971,2972,2973][i - 1784]! := by
  interval_cases i <;> decide
...
theorem jumpDest2896 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 2896 = true :=
  Artifact.isValidJumpDest_index 1763 (by rfl)

theorem jumpDest2915 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 2915 = true :=
  Artifact.isValidJumpDest_index 1773 (by rfl)

@[simp] theorem fastPC20 (i : Nat) (hi : 1742 ≤ i) (hii : i ≤ 1783) :
    Artifact.submissionArtifact.instructionPC i =
      [2863,2864,2865,2866,2869,2870,2873,2874,2875,2876,2877,2878,2880,
       2881,2884,2885,2887,2888,2891,2892,2895,2896,2897,2898,2901,2902,
       2905,2908,2909,2911,2914,2915,2916,2919,2920,2923,2926,2927,2928,
       2930,2932,2935][i - 1742]! := by
  interval_cases i <;> decide
...
theorem jumpDest2936 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 2936 = true :=
  Artifact.isValidJumpDest_index 1784 (by rfl)

theorem jumpDest2947 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 2947 = true :=
  Artifact.isValidJumpDest_index 1791 (by rfl)

theorem jumpDest2950 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 2950 = true :=
...
theorem jumpDest2961 :
  ...
  Artifact.isValidJumpDest_index 1800 (by rfl)
