...
The appended Montgomery path occupies instruction indices 977..1721
...
private theorem fastPCAnchor12 :
    Artifact.submissionArtifact.instructionPC 1453 = 2032 := by
  calc
...
private theorem fastPCAnchor13 :
    Artifact.submissionArtifact.instructionPC 1496 = 2091 := by
  calc
...
private theorem fastPCAnchor14 :
    Artifact.submissionArtifact.instructionPC 1537 = 2138 := by
  calc
...
private theorem fastPCAnchor15 :
    Artifact.submissionArtifact.instructionPC 1574 = 2184 := by
  calc
...
private theorem fastPCAnchor16 :
    Artifact.submissionArtifact.instructionPC 1615 = 2240 := by
  calc
...
private theorem fastPCAnchor17 :
    Artifact.submissionArtifact.instructionPC 1658 = 2288 := by
  calc
...
private theorem fastPCAnchor18 :
    Artifact.submissionArtifact.instructionPC 1698 = 2426 := by
  calc
...
private theorem fastPCAnchor19 :
    Artifact.submissionArtifact.instructionPC 1719 = 2491 := by
  calc
    Artifact.submissionArtifact.instructionPC 1719 =
        Artifact.submissionArtifact.instructionPC 1698 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1698).take 21)).length :=
      instructionPC_add Artifact.submissionArtifact 1698 21
    _ = 2491 := by
      rw [fastPCAnchor18]
      rfl

private theorem fastPCAnchor20 :
    Artifact.submissionArtifact.instructionPC 1724 = 2496 := by
  calc
    Artifact.submissionArtifact.instructionPC 1724 =
        Artifact.submissionArtifact.instructionPC 1719 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1719).take 5)).length :=
      instructionPC_add Artifact.submissionArtifact 1719 5
    _ = 2496 := by
      rw [fastPCAnchor19]
      rfl

private theorem fastPCAnchor21 :
    Artifact.submissionArtifact.instructionPC 1750 = 2534 := by
  calc
    Artifact.submissionArtifact.instructionPC 1750 =
        Artifact.submissionArtifact.instructionPC 1724 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1724).take 26)).length :=
      instructionPC_add Artifact.submissionArtifact 1724 26
    _ = 2534 := by
      rw [fastPCAnchor20]
      rfl

private theorem fastPCAnchor22 :
    Artifact.submissionArtifact.instructionPC 1763 = 2555 := by
  calc
    Artifact.submissionArtifact.instructionPC 1763 =
        Artifact.submissionArtifact.instructionPC 1750 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1750).take 13)).length :=
      instructionPC_add Artifact.submissionArtifact 1750 13
    _ = 2555 := by
      rw [fastPCAnchor21]
      rfl

private theorem fastPCAnchor23 :
    Artifact.submissionArtifact.instructionPC 1798 = 2604 := by
  calc
    Artifact.submissionArtifact.instructionPC 1798 =
        Artifact.submissionArtifact.instructionPC 1763 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1763).take 35)).length :=
      instructionPC_add Artifact.submissionArtifact 1763 35
    _ = 2604 := by
      rw [fastPCAnchor22]
      rfl
...
@[simp] theorem fastPC2 (i : Nat) (hi : 1057 ≤ i) (hii : i ≤ 1096) :
    Artifact.submissionArtifact.instructionPC i =
      [1428,1429,1432,1433,1436,1437,1438,1439,1440,1441,1442,1443,1444,1445,1446,1447,1448,1449,1450,1451,1452,1455,1456,1458,1459,1460,1461,1462,1463,1464,1465,1466,1467,1468,1469,1470,1472,1473,1474,1475][i - 1057]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC11 (i : Nat) (hi : 1416 ≤ i) (hii : i ≤ 1452) :
    Artifact.submissionArtifact.instructionPC i =
      [1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025,2026,2027,2028,2029,2030,2031][i - 1416]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC12 (i : Nat) (hi : 1453 ≤ i) (hii : i ≤ 1495) :
    Artifact.submissionArtifact.instructionPC i =
      [2032,2033,2035,2036,2037,2038,2040,2041,2042,2043,2044,2045,2046,2049,2050,2051,2052,2053,2056,2057,2058,2059,2062,2063,2064,2067,2068,2071,2072,2073,2076,2077,2078,2079,2082,2083,2084,2085,2086,2087,2088,2089,2090][i - 1453]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC13 (i : Nat) (hi : 1496 ≤ i) (hii : i ≤ 1536) :
    Artifact.submissionArtifact.instructionPC i =
      [2091,2092,2093,2094,2095,2096,2097,2098,2099,2100,2101,2102,2103,2106,2107,2109,2110,2111,2114,2115,2117,2118,2119,2120,2121,2122,2123,2124,2125,2126,2127,2128,2129,2130,2131,2132,2133,2134,2135,2136,2137][i - 1496]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC14 (i : Nat) (hi : 1537 ≤ i) (hii : i ≤ 1573) :
    Artifact.submissionArtifact.instructionPC i =
      [2138,2139,2140,2141,2142,2143,2144,2145,2146,2147,2148,2149,2150,2151,2153,2154,2156,2157,2158,2159,2160,2161,2163,2164,2165,2168,2169,2170,2173,2174,2175,2176,2177,2178,2179,2180,2181][i - 1537]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC15 (i : Nat) (hi : 1574 ≤ i) (hii : i ≤ 1614) :
    Artifact.submissionArtifact.instructionPC i =
      [2184,2185,2186,2187,2190,2191,2192,2195,2196,2197,2200,2201,2203,2204,2205,2206,2207,2208,2211,2212,2213,2214,2215,2218,2219,2220,2223,2224,2225,2226,2227,2229,2230,2231,2232,2233,2234,2236,2237,2238,2239][i - 1574]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC16 (i : Nat) (hi : 1615 ≤ i) (hii : i ≤ 1657) :
    Artifact.submissionArtifact.instructionPC i =
      [2240,2241,2242,2243,2246,2247,2248,2249,2250,2251,2252,2253,2254,2255,2256,2257,2258,2259,2260,2261,2262,2263,2264,2265,2266,2267,2268,2269,2270,2271,2272,2274,2275,2276,2277,2279,2280,2281,2282,2283,2285,2286,2287][i - 1615]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC17 (i : Nat) (hi : 1658 ≤ i) (hii : i ≤ 1697) :
    Artifact.submissionArtifact.instructionPC i =
      [2288,2291,2292,2293,2296,2297,2298,2299,2300,2303,2304,2305,2334,2335,2336,2337,2338,2339,2340,2373,2374,2375,2376,2377,2378,2379,2380,2381,2382,2383,2384,2385,2386,2387,2388,2389,2390,2391,2392,2425][i - 1658]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC18 (i : Nat) (hi : 1698 ≤ i) (hii : i ≤ 1718) :
    Artifact.submissionArtifact.instructionPC i =
      [2426,2427,2428,2461,2462,2465,2466,2467,2470,2471,2472,2473,2476,2477,2478,2481,2482,2483,2486,2487,2490][i - 1698]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC19 (i : Nat) (hi : 1719 ≤ i) (hii : i ≤ 1723) :
    Artifact.submissionArtifact.instructionPC i =
      [2491,2492,2493,2494,2495][i - 1719]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1719 + (i - 1719)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1719 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1719).take (i - 1719))).length :=
      instructionPC_add Artifact.submissionArtifact 1719 (i - 1719)
    _ = _ := by
      rw [fastPCAnchor19]
      interval_cases i <;> rfl

@[simp] theorem fastPC20 (i : Nat) (hi : 1724 ≤ i) (hii : i ≤ 1749) :
    Artifact.submissionArtifact.instructionPC i =
      [2496,2497,2500,2501,2502,2503,2506,2507,2508,2510,2511,2514,2515,2516,2517,2520,2521,2522,2523,2524,2525,2526,2530,2531,2532,2533][i - 1724]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1724 + (i - 1724)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1724 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1724).take (i - 1724))).length :=
      instructionPC_add Artifact.submissionArtifact 1724 (i - 1724)
    _ = _ := by
      rw [fastPCAnchor20]
      interval_cases i <;> rfl

@[simp] theorem fastPC21 (i : Nat) (hi : 1750 ≤ i) (hii : i ≤ 1762) :
    Artifact.submissionArtifact.instructionPC i =
      [2534,2535,2536,2537,2539,2540,2541,2544,2545,2547,2550,2551,2554][i - 1750]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1750 + (i - 1750)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1750 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1750).take (i - 1750))).length :=
      instructionPC_add Artifact.submissionArtifact 1750 (i - 1750)
    _ = _ := by
      rw [fastPCAnchor21]
      interval_cases i <;> rfl

@[simp] theorem fastPC22 (i : Nat) (hi : 1763 ≤ i) (hii : i ≤ 1797) :
    Artifact.submissionArtifact.instructionPC i =
      [2555,2556,2557,2560,2561,2562,2563,2564,2565,2566,2567,2570,2571,2573,2576,2577,2578,2579,2580,2582,2583,2584,2585,2587,2588,2589,2590,2592,2593,2594,2596,2597,2599,2600,2603][i - 1763]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC23 (i : Nat) (hi : 1798 ≤ i) (hii : i ≤ 1812) :
    Artifact.submissionArtifact.instructionPC i =
      [2604,2605,2606,2609,2610,2613,2614,2617,2620,2621,2624,2627,2628,2629,2632][i - 1798]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1798 + (i - 1798)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1798 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1798).take (i - 1798))).length :=
      instructionPC_add Artifact.submissionArtifact 1798 (i - 1798)
    _ = _ := by
      rw [fastPCAnchor23]
      interval_cases i <;> rfl
...
@[simp] theorem fullBasePC (i : Nat)
    (hi : 2447 ≤ i) (hii : i ≤ 2499) :
    Artifact.submissionArtifact.instructionPC i =
      ([3365,3366,3367,3368,3369,3370,3371,3373,3374,3375,3376,3379,3380,3381,3383,3386,3387,3390,3393,3396,3399,3402,3403,3404,3407,3410,3413,3416,3419,3420,3421,3422,3424,3425,3427,3428,3429,3430,3432,3433,3434,3436,3437,3439,3440,3441,3442,3443,3446,3447,3448,3450,3453] : List Nat)[i - 2447]! := by
...
theorem jumpDest2877 :
  ...
  Artifact.isValidJumpDest_index 1733 (by rfl)
