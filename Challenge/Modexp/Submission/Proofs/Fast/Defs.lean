...
private theorem fastPCAnchor19 :
    Artifact.submissionArtifact.instructionPC 1730 = 2472 := by
  calc
    Artifact.submissionArtifact.instructionPC 1730 =
        Artifact.submissionArtifact.instructionPC 1690 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1690).take 40)).length :=
      instructionPC_add Artifact.submissionArtifact 1690 40
    _ = 2472 := by
      rw [fastPCAnchor18]
      rfl

private theorem fastPCAnchor20 :
    Artifact.submissionArtifact.instructionPC 1735 = 2477 := by
  calc
    Artifact.submissionArtifact.instructionPC 1735 =
        Artifact.submissionArtifact.instructionPC 1730 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1730).take 5)).length :=
      instructionPC_add Artifact.submissionArtifact 1730 5
    _ = 2477 := by
      rw [fastPCAnchor19]
      rfl

private theorem fastPCAnchor21 :
    Artifact.submissionArtifact.instructionPC 1761 = 2514 := by
  calc
    Artifact.submissionArtifact.instructionPC 1761 =
        Artifact.submissionArtifact.instructionPC 1735 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1735).take 26)).length :=
      instructionPC_add Artifact.submissionArtifact 1735 26
    _ = 2514 := by
      rw [fastPCAnchor20]
      rfl

private theorem fastPCAnchor22 :
    Artifact.submissionArtifact.instructionPC 1774 = 2535 := by
  calc
    Artifact.submissionArtifact.instructionPC 1774 =
        Artifact.submissionArtifact.instructionPC 1761 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1761).take 13)).length :=
      instructionPC_add Artifact.submissionArtifact 1761 13
    _ = 2535 := by
      rw [fastPCAnchor21]
      rfl

private theorem fastPCAnchor23 :
    Artifact.submissionArtifact.instructionPC 1809 = 2584 := by
  calc
    Artifact.submissionArtifact.instructionPC 1809 =
        Artifact.submissionArtifact.instructionPC 1774 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1774).take 35)).length :=
      instructionPC_add Artifact.submissionArtifact 1774 35
    _ = 2584 := by
      rw [fastPCAnchor22]
      rfl
...
@[simp] theorem fastPC18 (i : Nat) (hi : 1690 ≤ i) (hii : i ≤ 1729) :
    Artifact.submissionArtifact.instructionPC i =
      [2324,2325,2326,2327,2328,2329,2330,2331,2332,2333,2334,2335,2368,2369,2370,2403,2404,2405,2406,2439,2440,2441,2444,2445,2446,2449,2450,2451,2452,2453,2454,2457,2458,2459,2462,2463,2464,2467,2468,2471][i - 1690]!  := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC19 (i : Nat) (hi : 1730 ≤ i) (hii : i ≤ 1734) :
    Artifact.submissionArtifact.instructionPC i =
      [2472,2473,2474,2475,2476][i - 1730]!  := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1730 + (i - 1730)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1730 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1730).take (i - 1730))).length :=
      instructionPC_add Artifact.submissionArtifact 1730 (i - 1730)
    _ = _ := by
      rw [fastPCAnchor19]
      interval_cases i <;> rfl

@[simp] theorem fastPC20 (i : Nat) (hi : 1735 ≤ i) (hii : i ≤ 1760) :
    Artifact.submissionArtifact.instructionPC i =
      [2477,2478,2481,2482,2483,2484,2487,2488,2489,2491,2492,2495,2496,2497,2498,2501,2502,2503,2504,2505,2506,2507,2510,2511,2512,2513][i - 1735]!  := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC21 (i : Nat) (hi : 1761 ≤ i) (hii : i ≤ 1773) :
    Artifact.submissionArtifact.instructionPC i =
      [2514,2515,2516,2517,2519,2520,2521,2524,2525,2527,2530,2531,2534][i - 1761]!  := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1761 + (i - 1761)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1761 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1761).take (i - 1761))).length :=
      instructionPC_add Artifact.submissionArtifact 1761 (i - 1761)
    _ = _ := by
      rw [fastPCAnchor21]
      interval_cases i <;> rfl

@[simp] theorem fastPC22 (i : Nat) (hi : 1774 ≤ i) (hii : i ≤ 1808) :
    Artifact.submissionArtifact.instructionPC i =
      [2535,2536,2537,2540,2541,2542,2543,2544,2545,2546,2547,2550,2551,2553,2556,2557,2558,2559,2560,2562,2563,2564,2565,2567,2568,2569,2570,2572,2573,2574,2576,2577,2579,2580,2583][i - 1774]!  := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC23 (i : Nat) (hi : 1809 ≤ i) (hii : i ≤ 1823) :
    Artifact.submissionArtifact.instructionPC i =
      [2584,2585,2586,2589,2590,2593,2594,2597,2600,2601,2604,2607,2608,2609,2612][i - 1809]!  := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1809 + (i - 1809)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1809 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1809).take (i - 1809))).length :=
      instructionPC_add Artifact.submissionArtifact 1809 (i - 1809)
    _ = _ := by
      rw [fastPCAnchor23]
      interval_cases i <;> rfl
...
@[simp] theorem fullBasePC (i : Nat)
    (hi : 2456 ≤ i) (hii : i ≤ 2508) :
    Artifact.submissionArtifact.instructionPC i =
      ([3329,3330,3331,3332,3333,3334,3335,3337,3338,3339,3340,3343,3344,3345,3347,3350,3351,3354,3357,3360,3363,3366,3367,3368,3371,3374,3377,3380,3383,3384,3385,3386,3388,3389,3391,3392,3393,3394,3396,3397,3398,3400,3401,3403,3404,3405,3406,3407,3410,3411,3412,3414,3417] : List Nat)[i - 2456]! := by
...
theorem jumpDest2879 :
  ...
  Artifact.isValidJumpDest_index 1761 (by rfl)
...
theorem jumpDest2949 :
  ...
  Artifact.isValidJumpDest_index 1809 (by rfl)
