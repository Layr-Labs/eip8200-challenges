...
private theorem fastPCAnchor18 :
    Artifact.submissionArtifact.instructionPC 1690 = 2340 := by
  calc
...
private theorem fastPCAnchor19 :
    Artifact.submissionArtifact.instructionPC 1731 = 2491 := by
  calc
    Artifact.submissionArtifact.instructionPC 1731 =
        Artifact.submissionArtifact.instructionPC 1690 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1690).take 41)).length :=
      instructionPC_add Artifact.submissionArtifact 1690 41
    _ = 2491 := by
      rw [fastPCAnchor18]
      rfl

private theorem fastPCAnchor20 :
    Artifact.submissionArtifact.instructionPC 1736 = 2496 := by
  calc
    Artifact.submissionArtifact.instructionPC 1736 =
        Artifact.submissionArtifact.instructionPC 1731 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1731).take 5)).length :=
      instructionPC_add Artifact.submissionArtifact 1731 5
    _ = 2496 := by
      rw [fastPCAnchor19]
      rfl

private theorem fastPCAnchor21 :
    Artifact.submissionArtifact.instructionPC 1762 = 2534 := by
  calc
    Artifact.submissionArtifact.instructionPC 1762 =
        Artifact.submissionArtifact.instructionPC 1736 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1736).take 26)).length :=
      instructionPC_add Artifact.submissionArtifact 1736 26
    _ = 2534 := by
      rw [fastPCAnchor20]
      rfl

private theorem fastPCAnchor22 :
    Artifact.submissionArtifact.instructionPC 1775 = 2555 := by
  calc
    Artifact.submissionArtifact.instructionPC 1775 =
        Artifact.submissionArtifact.instructionPC 1762 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1762).take 13)).length :=
      instructionPC_add Artifact.submissionArtifact 1762 13
    _ = 2555 := by
      rw [fastPCAnchor21]
      rfl

private theorem fastPCAnchor23 :
    Artifact.submissionArtifact.instructionPC 1810 = 2604 := by
  calc
    Artifact.submissionArtifact.instructionPC 1810 =
        Artifact.submissionArtifact.instructionPC 1775 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1775).take 35)).length :=
      instructionPC_add Artifact.submissionArtifact 1775 35
    _ = 2604 := by
      rw [fastPCAnchor22]
      rfl
...
@[simp] theorem fastPC17 (i : Nat) (hi : 1650 ≤ i) (hii : i ≤ 1689) :
    Artifact.submissionArtifact.instructionPC i =
      [2288,2291,2292,2293,2296,2297,2298,2299,2300,2303,2304,2305,2308,2309,2312,2313,2314,2317,2318,2319,2320,2321,2322,2323,2324,2325,2326,2327,2328,2329,2330,2331,2332,2333,2334,2335,2336,2337,2338,2339][i - 1650]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC18 (i : Nat) (hi : 1690 ≤ i) (hii : i ≤ 1730) :
    Artifact.submissionArtifact.instructionPC i =
      [2340,2341,2342,2343,2344,2345,2346,2379,2380,2381,2414,2415,2416,2417,2450,2451,2452,2455,2456,2457,2460,2461,2462,2463,2464,2465,2468,2469,2470,2473,2474,2475,2478,2479,2482,2483,2484,2485,2486,2488,2490][i - 1690]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
@[simp] theorem fastPC19 (i : Nat) (hi : 1731 ≤ i) (hii : i ≤ 1735) :
    Artifact.submissionArtifact.instructionPC i =
      [2491,2492,2493,2494,2495][i - 1731]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1731 + (i - 1731)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1731 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1731).take (i - 1731))).length :=
      instructionPC_add Artifact.submissionArtifact 1731 (i - 1731)
    _ = _ := by
      rw [fastPCAnchor19]
      interval_cases i <;> rfl

@[simp] theorem fastPC20 (i : Nat) (hi : 1736 ≤ i) (hii : i ≤ 1761) :
    Artifact.submissionArtifact.instructionPC i =
      [2496,2497,2500,2501,2502,2503,2506,2507,2508,2510,2511,2514,2515,2516,2517,2520,2521,2522,2523,2524,2525,2526,2530,2531,2532,2533][i - 1736]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1736 + (i - 1736)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1736 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1736).take (i - 1736))).length :=
      instructionPC_add Artifact.submissionArtifact 1736 (i - 1736)
    _ = _ := by
      rw [fastPCAnchor20]
      interval_cases i <;> rfl

@[simp] theorem fastPC21 (i : Nat) (hi : 1762 ≤ i) (hii : i ≤ 1774) :
    Artifact.submissionArtifact.instructionPC i =
      [2534,2535,2536,2537,2539,2540,2541,2544,2545,2547,2550,2551,2554][i - 1762]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1762 + (i - 1762)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1762 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1762).take (i - 1762))).length :=
      instructionPC_add Artifact.submissionArtifact 1762 (i - 1762)
    _ = _ := by
      rw [fastPCAnchor21]
      interval_cases i <;> rfl

@[simp] theorem fastPC22 (i : Nat) (hi : 1775 ≤ i) (hii : i ≤ 1809) :
    Artifact.submissionArtifact.instructionPC i =
      [2555,2556,2557,2560,2561,2562,2563,2564,2565,2566,2567,2570,2571,2573,2576,2577,2578,2579,2580,2582,2583,2584,2585,2587,2588,2589,2590,2592,2593,2594,2596,2597,2599,2600,2603][i - 1775]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1775 + (i - 1775)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1775 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1775).take (i - 1775))).length :=
      instructionPC_add Artifact.submissionArtifact 1775 (i - 1775)
    _ = _ := by
      rw [fastPCAnchor22]
      interval_cases i <;> rfl

@[simp] theorem fastPC23 (i : Nat) (hi : 1810 ≤ i) (hii : i ≤ 1824) :
    Artifact.submissionArtifact.instructionPC i =
      [2604,2605,2606,2609,2610,2613,2614,2617,2620,2621,2624,2627,2628,2629,2632][i - 1810]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
        Artifact.submissionArtifact.instructionPC (1810 + (i - 1810)) := by
      rw [Nat.add_sub_of_le hi]
    _ = Artifact.submissionArtifact.instructionPC 1810 +
          (assembleBytes ((Artifact.submissionArtifact.instructions.drop 1810).take (i - 1810))).length :=
      instructionPC_add Artifact.submissionArtifact 1810 (i - 1810)
    _ = _ := by
      rw [fastPCAnchor23]
      interval_cases i <;> rfl
...
@[simp] theorem fullBasePC (i : Nat)
    (hi : 2459 ≤ i) (hii : i ≤ 2511) :
    Artifact.submissionArtifact.instructionPC i =
      ([3365,3366,3367,3368,3369,3370,3371,3373,3374,3375,3376,3379,3380,3381,3383,3386,3387,3390,3393,3396,3399,3402,3403,3404,3407,3410,3413,3416,3419,3420,3421,3422,3424,3425,3427,3428,3429,3430,3432,3433,3434,3436,3437,3439,3440,3441,3442,3443,3446,3447,3448,3450,3453] : List Nat)[i - 2459]! := by
...
theorem jumpDest2666 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 2323 = true :=
...
theorem jumpDest2863 :
  ...
  Artifact.isValidJumpDest_index 1736 (by rfl)
