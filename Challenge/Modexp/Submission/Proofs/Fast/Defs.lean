...
@[simp] theorem fastPC11 (i : Nat) (hi : 1417 ≤ i) (hii : i ≤ 1456) :
    Artifact.submissionArtifact.instructionPC i =
      [1991,1992,1993,1994,1995,1996,1997,1998,1999,2000,2001,2002,2003,2036,2037,2038,2039,2040,2041,2042,2043,2044,2045,2046,2047,2048,2049,2050,2051,2052,2053,2054,2055,2056,2057,2058,2059,2060,2061,2062][i - 1417]! := by
  calc
    Artifact.submissionArtifact.instructionPC i =
...
their proof modules. -/
@[simp] theorem fullBasePC (i : Nat) (hi : 2257 ≤ i) (hii : i ≤ 2309) :
    Artifact.submissionArtifact.instructionPC i =
      [3606,3607,3608,3609,3610,3611,3612,3614,3615,3616,3617,3620,3621,3622,3624,3627,3628,3631,3634,3637,3640,3643,3644,3645,3648,3651,3654,3657,3660,3661,3662,3663,3665,3666,3668,3669,3670,3671,3673,3674,3675,3677,3678,3680,3681,3682,3683,3684,3687,3688,3689,3691,3694][i - 2257]! := by
  interval_cases i <;> decide

/-- PC table for the fused ADDMOD/CSUB entry appended after the CIOS2 helper. -/
@[simp] theorem fusedCsubPC0 (i : Nat) (hi : 3003 ≤ i) (hii : i ≤ 3018) :
    Artifact.submissionArtifact.instructionPC i =
      [4569,4570,4573,4574,4575,4578,4579,4580,4581,4582,4583,4584,4585,4586,4587,4590][i - 3003]! := by
  interval_cases i <;> decide

/-- PC table for the fused limb loop. -/
@[simp] theorem fusedCsubPC1 (i : Nat) (hi : 3019 ≤ i) (hii : i ≤ 3073) :
    Artifact.submissionArtifact.instructionPC i =
      [4591,4592,4593,4594,4595,4596,4597,4598,4599,4600,4601,4602,4603,4604,4605,4606,4607,4608,4609,4610,4611,4612,4613,4614,4617,4618,4619,4620,4621,4622,4623,4624,4625,4626,4627,4628,4629,4630,4631,4632,4633,4634,4637,4638,4639,4640,4641,4642,4644,4645,4646,4649,4650,4651,4654][i - 3019]! := by
  interval_cases i <;> decide

/-- PC table for the fused selection/return tail. -/
@[simp] theorem fusedCsubPC2 (i : Nat) (hi : 3074 ≤ i) (hii : i ≤ 3096) :
    Artifact.submissionArtifact.instructionPC i =
      [4655,4656,4659,4660,4661,4662,4663,4664,4667,4668,4671,4672,4675,4676,4677,4678,4679,4680,4681,4682,4683,4684,4685][i - 3074]! := by
  interval_cases i <;> decide
...
theorem jumpDest3571 :
  ...
  Artifact.isValidJumpDest_index 2234 (by rfl)
...
theorem jumpDest3661 :
  ...
  Artifact.isValidJumpDest_index 2286 (by rfl)

theorem jumpDest5267 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4569 = true :=
  Artifact.isValidJumpDest_index 3003 (by rfl)

theorem jumpDest5289 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4591 = true :=
