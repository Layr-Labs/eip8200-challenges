...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 2112, .op .MLOAD,
   ...
   .push 2 4342, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 4130, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 2112, .push 2 2688, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 3963 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 2990 11 3963 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 3980 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3001 2 3980 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 4342 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3252 7 4342 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 3963 dst ret rest) =
      some (atState s mem (if Skip mem then 4342 else 3980) dst ret rest) := by
  ...
  have heq5165 : (4342 : UInt256) = UInt256.ofNat 4342 := by decide
  have h5165 : (4342 : UInt256).toNat = 4342 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 3980 dst ret rest) =
      some (atState s mem 4130 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (4130 : UInt256) = UInt256.ofNat 4130 := by decide
  have h4925 : (4130 : UInt256).toNat = 4130 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 4342 dst ret rest) =
