...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 8256, .op .MLOAD,
   .op .LT, .push 2 8224, .op .MLOAD, .op .ISZERO, .op .AND,
   .push 2 5216, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 4976, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 8256, .push 2 9344, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4804 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3635 12 4804 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4822 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3647 2 4822 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 5216 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3937 7 5216 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4804 dst ret rest) =
      some (atState s mem (if Skip mem then 5216 else 4822) dst ret rest) := by
  ...
  have heq5165 : (5216 : UInt256) = UInt256.ofNat 5216 := by decide
  have h5165 : (5216 : UInt256).toNat = 5216 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4822 dst ret rest) =
      some (atState s mem 4976 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (4976 : UInt256) = UInt256.ofNat 4976 := by decide
  have h4925 : (4976 : UInt256).toNat = 4976 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 5216 dst ret rest) =
