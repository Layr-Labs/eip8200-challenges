...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 2112, .op .MLOAD,
   ...
   .push 2 4733, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 4497, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 2112, .push 2 2688, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4336 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3291 11 4336 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4353 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3302 2 4353 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 4733 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3577 7 4733 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4336 dst ret rest) =
      some (atState s mem (if Skip mem then 4733 else 4353) dst ret rest) := by
  ...
  have heq5165 : (4733 : UInt256) = UInt256.ofNat 4733 := by decide
  have h5165 : (4733 : UInt256).toNat = 4733 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4353 dst ret rest) =
      some (atState s mem 4497 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (4497 : UInt256) = UInt256.ofNat 4497 := by decide
  have h4925 : (4497 : UInt256).toNat = 4497 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 4733 dst ret rest) =
