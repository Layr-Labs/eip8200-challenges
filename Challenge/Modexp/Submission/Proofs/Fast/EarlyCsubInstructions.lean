...
def jumpProgram : List Instr := [.push 2 5037, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 4160, .push 2 5248, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4874 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3752 12 4874 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4892 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3764 2 4892 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 5277 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 4046 7 5277 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4874 dst ret rest) =
      some (atState s mem (if Skip mem then 5277 else 4892) dst ret rest) := by
  ...
  have heq5165 : (5277 : UInt256) = UInt256.ofNat 5277 := by decide
  have h5165 : (5277 : UInt256).toNat = 5277 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4892 dst ret rest) =
      some (atState s mem 5037 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (5037 : UInt256) = UInt256.ofNat 5037 := by decide
  have h4925 : (5037 : UInt256).toNat = 5037 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 5277 dst ret rest) =
