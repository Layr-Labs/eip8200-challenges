...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 4160, .op .MLOAD,
   ...
   .push 2 5236, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 4996, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 4160, .push 2 5248, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4824 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3662 12 4824 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4842 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3674 2 4842 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 5236 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3964 7 5236 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4824 dst ret rest) =
      some (atState s mem (if Skip mem then 5236 else 4842) dst ret rest) := by
  ...
  have heq5165 : (5236 : UInt256) = UInt256.ofNat 5236 := by decide
  have h5165 : (5236 : UInt256).toNat = 5236 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4842 dst ret rest) =
      some (atState s mem 4996 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (4996 : UInt256) = UInt256.ofNat 4996 := by decide
  have h4925 : (4996 : UInt256).toNat = 4996 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 5236 dst ret rest) =
