...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 2112, .op .MLOAD,
   ...
   .push 2 4544, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 4299, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 2112, .push 2 2688, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4132 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3135 11 4132 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4149 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3146 2 4149 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 4544 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3421 7 4544 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4132 dst ret rest) =
      some (atState s mem (if Skip mem then 4544 else 4149) dst ret rest) := by
  ...
  have heq5165 : (4544 : UInt256) = UInt256.ofNat 4544 := by decide
  have h5165 : (4544 : UInt256).toNat = 4544 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4149 dst ret rest) =
      some (atState s mem 4299 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (4299 : UInt256) = UInt256.ofNat 4299 := by decide
  have h4925 : (4299 : UInt256).toNat = 4299 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 4544 dst ret rest) =
