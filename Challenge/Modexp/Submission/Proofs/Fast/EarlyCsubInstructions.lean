...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 2112, .op .MLOAD,
   ...
   .push 2 5259, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 5023, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 2112, .push 2 2688, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4871 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3722 11 4871 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4888 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3733 2 4888 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 5259 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 4005 7 5259 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4871 dst ret rest) =
      some (atState s mem (if Skip mem then 5259 else 4888) dst ret rest) := by
  ...
  have heq5165 : (5259 : UInt256) = UInt256.ofNat 5259 := by decide
  have h5165 : (5259 : UInt256).toNat = 5259 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4888 dst ret rest) =
      some (atState s mem 5023 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (5023 : UInt256) = UInt256.ofNat 5023 := by decide
  have h4925 : (5023 : UInt256).toNat = 5023 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 5259 dst ret rest) =
