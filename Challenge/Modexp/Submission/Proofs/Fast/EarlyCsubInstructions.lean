...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 8256, .op .MLOAD,
   ...
   .push 2 4822, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 5062, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 8256, .push 2 9344, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4642 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3545 12 4642 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4660 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3557 2 4660 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 5062 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3836 7 5062 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4642 dst ret rest) =
      some (atState s mem (if Skip mem then 4660 else 4822) dst ret rest) := by
  ...
  have heq4978 : (4822 : UInt256) = UInt256.ofNat 4822 := by decide
  have h4978 : (4822 : UInt256).toNat = 4822 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4660 dst ret rest) =
      some (atState s mem 5062 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq5219 : (5062 : UInt256) = UInt256.ofNat 5062 := by decide
  have h5219 : (5062 : UInt256).toNat = 5062 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 5062 dst ret rest) =
