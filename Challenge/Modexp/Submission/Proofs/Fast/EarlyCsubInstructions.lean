...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 8256, .op .MLOAD,
   ...
   .push 2 5279, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 5039, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 8256, .push 2 9344, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4792 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3714 12 4792 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4810 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3726 2 4810 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 5279 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 4047 7 5279 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4792 dst ret rest) =
      some (atState s mem (if Skip mem then 5279 else 4810) dst ret rest) := by
  ...
  have heq5165 : (5279 : UInt256) = UInt256.ofNat 5279 := by decide
  have h5165 : (5279 : UInt256).toNat = 5279 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4810 dst ret rest) =
      some (atState s mem 5039 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (5039 : UInt256) = UInt256.ofNat 5039 := by decide
  have h4925 : (5039 : UInt256).toNat = 5039 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 5279 dst ret rest) =
