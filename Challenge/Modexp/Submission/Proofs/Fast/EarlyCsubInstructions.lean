...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 2112, .op .MLOAD,
   ...
   .push 2 4732, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 4496, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 2112, .push 2 2688, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4335 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3290 11 4335 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4352 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3301 2 4352 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 4732 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3576 7 4732 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4335 dst ret rest) =
      some (atState s mem (if Skip mem then 4732 else 4352) dst ret rest) := by
  ...
  have heq5165 : (4732 : UInt256) = UInt256.ofNat 4732 := by decide
  have h5165 : (4732 : UInt256).toNat = 4732 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4352 dst ret rest) =
      some (atState s mem 4496 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (4496 : UInt256) = UInt256.ofNat 4496 := by decide
  have h4925 : (4496 : UInt256).toNat = 4496 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 4732 dst ret rest) =
