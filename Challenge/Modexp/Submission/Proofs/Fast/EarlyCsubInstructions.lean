...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .push 0 0, .op .MLOAD, .push 2 8256, .op .MLOAD,
   ...
   .push 2 5111, .op .JUMPI]
def jumpProgram : List Instr := [.push 2 4871, .op .JUMP]
def copyProgram : List Instr :=
  [.op .JUMPDEST, .push 2 8256, .push 2 9344, .op .MLOAD,
   .op (.Swap ⟨1, by decide⟩), .op .MCOPY, .op .JUMP]

def checkBlock : Block Artifact.submissionArtifact .Osaka 4624 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3554 12 4624 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def jumpBlock : Block Artifact.submissionArtifact .Osaka 4642 jumpProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3566 2 4642 jumpProgram
    (by decide) (by rfl) (by rfl) (by decide)
def copyBlock : Block Artifact.submissionArtifact .Osaka 5111 copyProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3887 7 5111 copyProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions checkProgram (atState s mem 4624 dst ret rest) =
      some (atState s mem (if Skip mem then 5111 else 4642) dst ret rest) := by
  ...
  have heq5165 : (5111 : UInt256) = UInt256.ofNat 5111 := by decide
  have h5165 : (5111 : UInt256).toNat = 5111 := by decide
...
theorem run_jump (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions jumpProgram (atState s mem 4642 dst ret rest) =
      some (atState s mem 4871 dst ret rest) := by
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have heq4925 : (4871 : UInt256) = UInt256.ofNat 4871 := by decide
  have h4925 : (4871 : UInt256).toNat = 4871 := by decide
...
theorem run_copy (s : State) (mem : ByteArray) (n : Nat) (dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions copyProgram (atState s mem 5111 dst ret rest) =
