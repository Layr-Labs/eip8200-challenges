...
theorem run_pointers (s : State) (mem : ByteArray) (pa pb n : Nat)
    (dst ret : UInt256) (rest : List UInt256) (hcap : rest.length ≤ 999) (hn32 : n ≤ 32)
  ...
  have h := CiosCachedEntryPointerWords.run_words { s with memory := mpZeroed s mem n } (UInt256.ofNat (32*n))
    (UInt256.ofNat pa) (UInt256.ofNat pb) (l1Target n) (l2Target n) (modulusValue mem n) (inverseValue mem :: tailPointerValue mem :: low96Value mem :: low64Value mem :: low32Value mem :: dst :: ret :: rest) (by simp only [List.length_cons]; omega)
  simpa only [List.cons_append, List.nil_append, clearedState, outState, CiosCachedMacCore.framed, hp0, Challenge.EvmProof.Word.ofNat_add_mod,
    hsub1, hsub2, modulusValue_zero s mem n hn32, inverseValue_zero s mem n hn32, tailPointerValue_zero s mem n hn32, low96Value_zero s mem n hn32, low64Value_zero s mem n hn32, low32Value_zero s mem n hn32] using h
