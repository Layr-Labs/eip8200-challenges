...
def cacheProgram : List Instr := entryProgram.take 34
def entryBodyProgram : List Instr := entryProgram.drop 34

theorem entryProgram_split : entryProgram = cacheProgram ++ entryBodyProgram := by
  exact (List.take_append_drop 34 entryProgram).symm

def cachedEntryState (s : State) (mem : ByteArray) (pa pb n : Nat)
    (pdst ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 4212
           stack := [UInt256.ofNat pa, UInt256.ofNat pb,
             l1Target n, negative32, allOnes, l2Target n, modulusValue mem n, inverseValue mem, tailPointerValue mem, low96Value mem, low64Value mem, low32Value mem, pdst, ret] ++ rest
           memory := mem }
...
theorem run_cache (s : State) (mem : ByteArray) (pa pb n : Nat)
  ...
  have hshuffle := CachePrefix.run_shuffle {s with memory := mem} (UInt256.ofNat pa) (UInt256.ofNat pb)
    pdst ret (MachineState.readWord mem (32*n-32)) (inverseValue mem) (tailPointerValue mem) (low96Value mem) (low64Value mem) (low32Value mem) (CachePrefix.displacement mem) rest hcap
  have h := runInstructions_append_some _ _ _ _ _ hload hshuffle
  simpa only [entryState, cachedEntryState, l1Target, l2Target, modulusValue, inverseValue, tailPointerValue, low96Value, low64Value, low32Value,
