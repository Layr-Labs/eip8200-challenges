...
def cacheProgram : List Instr := entryProgram.take 23
def entryBodyProgram : List Instr := entryProgram.drop 23

theorem entryProgram_split : entryProgram = cacheProgram ++ entryBodyProgram := by
  exact (List.take_append_drop 23 entryProgram).symm

def cachedEntryState (s : State) (mem : ByteArray) (pa pb n : Nat)
    (pdst ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 4515
           stack := [UInt256.ofNat pa, UInt256.ofNat pb,
             l1Target n, negative32, allOnes, l2Target n, modulusAddress n, pdst, ret] ++ rest
           memory := mem }
...
theorem run_cache (s : State) (mem : ByteArray) (pa pb n : Nat)
  ...
  simp [hml, h9408, hactML, hExtra9, hExtra10, hExtra11, hExtra12, hExtra13, hExtra14, cacheProgram, entryProgram, runInstructions, Challenge.EvmProof.Stepper.runInstr,
