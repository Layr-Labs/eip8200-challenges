...
def cacheProgram : List Instr := entryProgram.take 28
def entryBodyProgram : List Instr := entryProgram.drop 28

theorem entryProgram_split : entryProgram = cacheProgram ++ entryBodyProgram := by
  exact (List.take_append_drop 28 entryProgram).symm

def cachedEntryState (s : State) (mem : ByteArray) (pa pb n : Nat)
    (pdst ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 4203
           stack := [UInt256.ofNat pa, UInt256.ofNat pb,
             l1Target n, negative32, allOnes, l2Target n, modulusValue mem n, inverseValue mem, tailPointerValue mem, pdst, ret] ++ rest
           memory := mem }
...
theorem run_cache (s : State) (mem : ByteArray) (pa pb n : Nat)
  ...
  have hshuffle := CachePrefix.run_shuffle {s with memory := mem} (UInt256.ofNat pa) (UInt256.ofNat pb)
    pdst ret (MachineState.readWord mem (32*n-32)) (inverseValue mem) (tailPointerValue mem) (CachePrefix.displacement mem) rest hcap
  have h := runInstructions_append_some _ _ _ _ _ hload hshuffle
  simpa only [entryState, cachedEntryState, l1Target, l2Target, modulusValue, inverseValue, tailPointerValue,
