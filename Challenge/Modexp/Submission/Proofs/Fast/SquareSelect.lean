...
def guard (mem : ByteArray) (pa pb : UInt256) : UInt256 :=
  UInt256.land (UInt256.eq 256 (MachineState.readWord mem 2784)) (UInt256.eq pb pa)

theorem guard_def (mem : ByteArray) (pa pb : UInt256) :
    guard mem pa pb =
      UInt256.land (UInt256.eq 256 (MachineState.readWord mem 2784)) (UInt256.eq pb pa) := rfl

def checkProgram : List Instr :=
  [.op .JUMPDEST, .op (.Dup ⟨0, by decide⟩), .op (.Dup ⟨2, by decide⟩), .op .EQ,
   .push 2 2784, .op .MLOAD, .push 2 256, .op .EQ, .op .AND,
   .push 2 5032, .op .JUMPI]
def normalProgram : List Instr :=
  [.push 0 0, .push 2 2368, .op .MSTORE, .push 2 4169, .push 2 5036, .op .JUMP]
def squareProgram : List Instr := [.op .JUMPDEST, .push 2 5045]
def storeProgram : List Instr :=
  [.op .JUMPDEST, .push 2 2720, .op .MSTORE, .push 2 4076, .op .JUMP]

theorem destSquare : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 5032 = true :=
  Artifact.isValidJumpDest_index 3816 (by rfl)
theorem destStore : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 5036 = true :=
...
def checkBlock : Block Artifact.submissionArtifact .Osaka 5003 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3799 11 5003 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def normalBlock : Block Artifact.submissionArtifact .Osaka 5020 normalProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3810 6 5020 normalProgram
    (by decide) (by rfl) (by rfl) (by decide)
def squareBlock : Block Artifact.submissionArtifact .Osaka 5032 squareProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3816 2 5032 squareProgram
    (by decide) (by rfl) (by rfl) (by decide)
def storeBlock : Block Artifact.submissionArtifact .Osaka 5036 storeProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3818 5 5036 storeProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (pa pb dst ret : UInt256)
    ...
    runInstructions checkProgram (stateAt s mem 5003 ([pa,pb,dst,ret] ++ rest)) =
      some (stateAt s mem (if UInt256.isTrue (guard mem pa pb) then 5032 else 5020)
  ...
  have h5036 : (5032 : UInt256).toNat = 5032 := by decide
  have he5036 : (5032 : UInt256) = UInt256.ofNat 5032 := by decide
...
theorem run_normal (s : State) (mem : ByteArray) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions normalProgram (stateAt s mem 5020 rest) =
      some (stateAt s (storeWord mem 2368 (UInt256.ofNat 0)) 5036
        (UInt256.ofNat 4169 :: rest)) := by
  ...
  have hp : (5036 : UInt256).toNat = 5036 := by decide
  have hep : (5036 : UInt256) = UInt256.ofNat 5036 := by decide
  have hv : (4169 : UInt256) = UInt256.ofNat 4169 := by decide
...
theorem run_square (s : State) (mem : ByteArray) (rest : List UInt256)
    ...
    runInstructions squareProgram (stateAt s mem 5032 rest) =
      some (stateAt s mem 5036 (UInt256.ofNat 5045 :: rest)) := by
  have hv : (5045 : UInt256) = UInt256.ofNat 5045 := by decide
...
theorem run_store (s : State) (mem : ByteArray) (route : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1018) (hact : 91 ≤ s.activeWords.toNat)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions storeProgram (stateAt s mem 5036 (route :: rest)) =
...
def selectedMemory (mem : ByteArray) (pa pb : UInt256) : ByteArray :=
  if UInt256.isTrue (guard mem pa pb) then storeWord mem 2720 (UInt256.ofNat 5045)
  else storeWord (storeWord mem 2368 (UInt256.ofNat 0)) 2720 (UInt256.ofNat 4169)
...
def gasSteps_select (s : State) (mem : ByteArray) (pa pb dst ret : UInt256)
    ...
    Challenge.EvmProof.GasSteps (stateAt s mem 5003 ([pa,pb,dst,ret] ++ rest))
  ...
  have check := checkBlock.steps (EarlyCsub.environment (stateAt s mem 5003 args) hcode hfork hrun hnp) rfl
    (run_check s mem pa pb dst ret rest hcap hact hcode)
  by_cases hs : UInt256.isTrue (guard mem pa pb)
  · rw [if_pos hs] at check
    have sq := squareBlock.steps (EarlyCsub.environment (stateAt s mem 5032 args) hcode hfork hrun hnp) rfl
      (run_square s mem args hargs)
    have put := storeBlock.steps (EarlyCsub.environment (stateAt s mem 5036 (UInt256.ofNat 5045 :: args)) hcode hfork hrun hnp) rfl
      (run_store s mem (UInt256.ofNat 5045) args hargs hact hcode)
    simpa only [selectedMemory, if_pos hs] using check.trans (sq.trans put)
  · rw [if_neg hs] at check
    have normal := normalBlock.steps (EarlyCsub.environment (stateAt s mem 5020 args) hcode hfork hrun hnp) rfl
      (run_normal s mem args hargs hact hcode)
    have put := storeBlock.steps (EarlyCsub.environment (stateAt s (storeWord mem 2368 (UInt256.ofNat 0)) 5036 (UInt256.ofNat 4169 :: args)) hcode hfork hrun hnp) rfl
      (run_store s (storeWord mem 2368 (UInt256.ofNat 0)) (UInt256.ofNat 4169)
