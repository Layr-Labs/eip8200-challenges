...
def checkProgram : List Instr :=
  [.op .JUMPDEST, .op (.Dup ⟨0, by decide⟩), .op (.Dup ⟨2, by decide⟩), .op .EQ,
   ...
   .push 2 5039, .op .JUMPI]
def normalProgram : List Instr :=
  [.push 0 0, .push 2 8928, .op .MSTORE, .push 2 4165, .push 2 5043, .op .JUMP]
def squareProgram : List Instr := [.op .JUMPDEST, .push 2 5052]
def storeProgram : List Instr :=
  [.op .JUMPDEST, .push 2 9280, .op .MSTORE, .push 2 4072, .op .JUMP]

theorem destSquare : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 5039 = true :=
  Artifact.isValidJumpDest_index 3826 (by rfl)
theorem destStore : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 5043 = true :=
...
def checkBlock : Block Artifact.submissionArtifact .Osaka 5010 checkProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3809 11 5010 checkProgram
    (by decide) (by rfl) (by rfl) (by decide)
def normalBlock : Block Artifact.submissionArtifact .Osaka 5027 normalProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3820 6 5027 normalProgram
    (by decide) (by rfl) (by rfl) (by decide)
def squareBlock : Block Artifact.submissionArtifact .Osaka 5039 squareProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3826 2 5039 squareProgram
    (by decide) (by rfl) (by rfl) (by decide)
def storeBlock : Block Artifact.submissionArtifact .Osaka 5043 storeProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3828 5 5043 storeProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_check (s : State) (mem : ByteArray) (pa pb dst ret : UInt256)
    ...
    runInstructions checkProgram (stateAt s mem 5010 ([pa,pb,dst,ret] ++ rest)) =
      some (stateAt s mem (if UInt256.isTrue (guard mem pa pb) then 5039 else 5027)
  ...
  have h5039 : (5039 : UInt256).toNat = 5039 := by decide
  have he5039 : (5039 : UInt256) = UInt256.ofNat 5039 := by decide
  by_cases hc : UInt256.isTrue (guard mem pa pb) <;>
    simp [checkProgram, runInstructions, Challenge.EvmProof.Stepper.runInstr, stateAt,
      hc4, hc5, hc6, hc7, ha, h9344, h5039, he5039, hcode, destSquare, ← guard_def, hc,
...
theorem run_normal (s : State) (mem : ByteArray) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions normalProgram (stateAt s mem 5027 rest) =
      some (stateAt s (storeWord mem 8928 (UInt256.ofNat 0)) 5043
  ...
  have hp : (5043 : UInt256).toNat = 5043 := by decide
  have hep : (5043 : UInt256) = UInt256.ofNat 5043 := by decide
...
theorem run_square (s : State) (mem : ByteArray) (rest : List UInt256)
    ...
    runInstructions squareProgram (stateAt s mem 5039 rest) =
      some (stateAt s mem 5043 (UInt256.ofNat 5052 :: rest)) := by
  have hv : (5052 : UInt256) = UInt256.ofNat 5052 := by decide
...
theorem run_store (s : State) (mem : ByteArray) (route : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1018) (hact : 296 ≤ s.activeWords.toNat)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions storeProgram (stateAt s mem 5043 (route :: rest)) =
      some (stateAt s (storeWord mem 9280 route) 4072 rest) := by
  have hc0 : rest.length < 1024 := by omega
  have hc1 : rest.length+1 < 1024 := by omega
  have hc2 : rest.length+2 < 1024 := by omega
  have ha := EarlyCsub.activeWords_fix s 9280 32 (by decide) (by decide) hact
  have hd : (9280 : UInt256).toNat = 9280 := by decide
  have hp : (4072 : UInt256).toNat = 4072 := by decide
  have hep : (4072 : UInt256) = UInt256.ofNat 4072 := by decide
...
def selectedMemory (mem : ByteArray) (pa pb : UInt256) : ByteArray :=
  if UInt256.isTrue (guard mem pa pb) then storeWord mem 9280 (UInt256.ofNat 5052)
...
def gasSteps_select (s : State) (mem : ByteArray) (pa pb dst ret : UInt256)
    ...
    Challenge.EvmProof.GasSteps (stateAt s mem 5010 ([pa,pb,dst,ret] ++ rest))
      (stateAt s (selectedMemory mem pa pb) 4072 ([pa,pb,dst,ret] ++ rest)) := by
  ...
  have check := checkBlock.steps (EarlyCsub.environment (stateAt s mem 5010 args) hcode hfork hrun hnp) rfl
    (run_check s mem pa pb dst ret rest hcap hact hcode)
  by_cases hs : UInt256.isTrue (guard mem pa pb)
  · rw [if_pos hs] at check
    have sq := squareBlock.steps (EarlyCsub.environment (stateAt s mem 5039 args) hcode hfork hrun hnp) rfl
      (run_square s mem args hargs)
    have put := storeBlock.steps (EarlyCsub.environment (stateAt s mem 5043 (UInt256.ofNat 5052 :: args)) hcode hfork hrun hnp) rfl
      (run_store s mem (UInt256.ofNat 5052) args hargs hact hcode)
    simpa only [selectedMemory, if_pos hs] using check.trans (sq.trans put)
  · rw [if_neg hs] at check
    have normal := normalBlock.steps (EarlyCsub.environment (stateAt s mem 5027 args) hcode hfork hrun hnp) rfl
      (run_normal s mem args hargs hact hcode)
    have put := storeBlock.steps (EarlyCsub.environment (stateAt s (storeWord mem 8928 (UInt256.ofNat 0)) 5043 (UInt256.ofNat 4165 :: args)) hcode hfork hrun hnp) rfl
