...
theorem run_l1Last (t : UInt256) (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hpa : 32 ≤ pa) (hpaFit : pa + 32*n ≤ 9472) :
    runInstructions (l1LastProgram t) (l1At 4825 s mem bi pa pb n i (n-1) pdst ret rest) =
      some (midState s (l1Step mem bi pa n n).memory (l1Step mem bi pa n n).carry bi
        pa pb n i pdst ret rest) := by
  have h := CiosCachedL1.run_last s (UInt256.ofNat 4825) mem bi pa n t ht
    (UInt256.ofNat (ptrAt (pb+32*n-32) i))
    (UInt256.ofNat (pb-32)) (isFour n) pdst ret rest hcap hact hn32 hpos hpa hpaFit
  have hpc : UInt256.ofNat 4825 + UInt256.ofNat 35 = UInt256.ofNat 4860 := by decide
...
theorem run_l2Mac (pc : Nat) (w : Fin 33) (x tl ts : UInt256)
  ...
  have h := CiosCachedL2.run_step w s (UInt256.ofNat pc) mid bi mu c0 n k x tl ts hx htl hts
    ...
    (UInt256.ofNat (pb-32)) (isFour n) pdst ret rest hcap hact hn32 hk hpush
