...
theorem run_middle (s : State) (mem : ByteArray) (c bi : UInt256)
    ...
    some (CiosCached.l2At 4579 s (midMem mem c) bi (rowMu mem n) (rowC0 mem n)
  ...
  have trace := runInstructions_append_some _ _ _ _ _
    (run_store { s with memory := mem } c bi
      ...
      (l1Target n) (l2Target n) (modulusValue mem n) (inverseValue mem :: tailPointerValue mem :: low96Value mem :: low64Value mem :: low32Value mem :: dst :: ret :: rest) (by simp only [List.length_cons]; omega) hact)
    (run_product { s with memory := midMem mem c } n bi
      (UInt256.ofNat (ptrAt (pb+32*n-32) i)) (UInt256.ofNat pa) (UInt256.ofNat (pb-32))
      (l1Target n) (l2Target n) (modulusValue mem n) (inverseValue mem :: tailPointerValue mem :: low96Value mem :: low64Value mem :: low32Value mem :: dst :: ret :: rest) (by simp only [List.length_cons]; omega) hact hn hn32 hml' htl' hminv' (modulusValue_mid mem c n hn32).symm
      (by simp only [List.getElem?_cons_zero]; exact congrArg some (inverseValue_mid mem c).symm)
      (by change some (tailPointerValue mem) = some (tailPointerValue (midMem mem c)); exact congrArg some (tailPointerValue_mid mem c).symm))
  ...
  simpa only [List.cons_append, List.nil_append, input, stored, product, baseStack, framed, CiosCached.midState,
    ...
    List.cons_append, List.nil_append, modulusValue_mid mem c n hn32, inverseValue_mid mem c, tailPointerValue_mid mem c, low96Value_mid mem c, low64Value_mid mem c, low32Value_mid mem c] using trace
