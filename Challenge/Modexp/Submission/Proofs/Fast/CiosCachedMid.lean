...
import Challenge.Modexp.Submission.Proofs.Fast.CiosCachedMidPointers
...
open CiosCachedMidStore CiosCachedMidProduct CiosCachedMidPointers CiosCachedMidMemory
...
theorem run_middle (s : State) (mem : ByteArray) (paj ptj c bi : UInt256)
    ...
    runInstructions midProgram
      (CiosCached.midState s mem paj ptj c bi pa pb n i dst ret rest) =
    some (CiosCached.l2At 4948 s (midMem mem c) bi (rowMu mem n) (rowC0 mem n)
  ...
  simpa only [input, stored, product, result, baseStack, framed, CiosCached.midState,
    CiosCached.l2At, l2Step, hp0, hmu, hc0, List.append_assoc,
