...
The R0 artifact keeps the square's row frame at the kernel exit: the dispatch at 4839
sends a square to `sq_exit` (4906), which
...
* `again` (4987) re-stages the operand (`MCOPY 0x800 → 0x2300`), re-zeroes the accumulator
  ...
  previous-limb slots, then falls into `sq_row` (4981) — exactly the row-0 state that
  `Cios2Dispatch.gasSteps_commonSetupInput` produces for a fresh call;
* if the counter reached zero, `last` (4923) overwrites the frame's `ret` slot with
  `after_sq` (3360) and leaves through `nx` (4848), the 14 `POP`s and the CSUB.
-/
...
def pcNx : Nat := 4859
def pcSqExit : Nat := 4917
def pcLast : Nat := 4933
def pcMore : Nat := 4942
def pcAgain : Nat := 4951
...
def frameStack (n : Nat) (pbi ent tl inv m0 m96 m64 m32 aprev pdst ret : UInt256) :
  ...
  [pbi, UInt256.ofNat 4984, UInt256.ofNat (512 - 32), ent, negative32, allOnes, l2Target n,
...
theorem frameAt_eq_sqExitState (s : State) (mem : ByteArray) (n : Nat)
    ...
    frameAt pcSqExit s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest =
      CiosCachedTailDefs.sqExitState s mem pbi 512 n (UInt256.ofNat 4984) ent inv m0
...
theorem frameAt_eq_nxJdState (s : State) (mem : ByteArray) (n : Nat)
    ...
    frameAt pcNx s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest =
      CiosCachedTailDefs.nxJdState s mem pbi 512 n (UInt256.ofNat 4984) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest) := rfl

/-! ## Programs -/

/-- `sq_exit` (4906): load the counter, decrement, store it back, branch to `more`. -/
def sqExitProgram : List Instr :=
  [.op .JUMPDEST, .push 2 5184, .op .MLOAD, .op (.Dup ⟨6, by decide⟩), .op .ADD,
   .op (.Dup ⟨0, by decide⟩), .push 2 5184, .op .MSTORE, .push 2 4942, .op .JUMPI]

/-- `last` (4923): the frame's `ret` slot becomes `after_sq`, then leave through `nx`. -/
def lastProgram : List Instr :=
  [.push 2 3348, .op (.Swap ⟨15, by decide⟩), .op .POP, .push 2 4859, .op .JUMP]

/-- `more` (4896): call the CSUB as a subroutine returning to `again`. -/
def moreProgram : List Instr :=
  [.op .JUMPDEST, .push 2 4951, .op (.Dup ⟨15, by decide⟩), .push 2 4874, .op .JUMP]
...
def sqExitBlock : Block Artifact.submissionArtifact .Osaka 4917 sqExitProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3780 10 4917 sqExitProgram
    (by decide) (by rfl) (by rfl) (by decide)

def lastBlock : Block Artifact.submissionArtifact .Osaka 4933 lastProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3790 5 4933 lastProgram
    (by decide) (by rfl) (by rfl) (by decide)

def moreBlock : Block Artifact.submissionArtifact .Osaka 4942 moreProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3795 5 4942 moreProgram
    (by decide) (by rfl) (by rfl) (by decide)

def againBlock : Block Artifact.submissionArtifact .Osaka 4951 againProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3800 22 4951 againProgram
    (by decide) (by rfl) (by rfl) (by decide)

/-! ## Jump destinations of the loop -/

theorem jumpDest4753 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4942 = true :=
  Artifact.isValidJumpDest_index 3795 (by rfl)
