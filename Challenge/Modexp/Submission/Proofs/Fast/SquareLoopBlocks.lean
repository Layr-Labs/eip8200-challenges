...
The R0 artifact keeps the square's row frame at the kernel exit: the dispatch at 4757
sends a square to `sq_exit` (4824), which
...
* if it is still non-zero, jumps to `more` (4896), which calls the unchanged final
  ...
  CSUB sees only `[pdst, again]` above the retained frame) and comes back at `again` (4905);
* `again` (4905) re-stages the operand (`MCOPY 0x800 → 0x2300`), re-zeroes the accumulator
  (`CALLDATACOPY` of the calldata tail) and resets the frame's pointer, first-loop entry and
  previous-limb slots, then falls into `sq_row` (4899) — exactly the row-0 state that
  `Cios2Dispatch.gasSteps_commonSetupInput` produces for a fresh call;
* if the counter reached zero, `last` (4841) overwrites the frame's `ret` slot with
  `after_sq` (3278) and leaves through `nx` (4766), the 14 `POP`s and the CSUB.
-/
...
def pcNx : Nat := 4777
def pcSqExit : Nat := 4835
def pcLast : Nat := 4851
def pcMore : Nat := 4860
def pcAgain : Nat := 4869
...
def frameStack (n : Nat) (pbi ent tl inv m0 m96 m64 m32 aprev pdst ret : UInt256) :
  ...
  [pbi, UInt256.ofNat 4902, UInt256.ofNat (2048 - 32), ent, negative32, allOnes, l2Target n,
...
theorem frameAt_eq_sqExitState (s : State) (mem : ByteArray) (n : Nat)
    ...
    frameAt pcSqExit s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest =
      CiosCachedTailDefs.sqExitState s mem pbi 2048 n (UInt256.ofNat 4902) ent inv m0
...
theorem frameAt_eq_nxJdState (s : State) (mem : ByteArray) (n : Nat)
    ...
    frameAt pcNx s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest =
      CiosCachedTailDefs.nxJdState s mem pbi 2048 n (UInt256.ofNat 4902) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest) := rfl

/-! ## Programs -/

/-- `sq_exit` (4824): load the counter, decrement, store it back, branch to `more`. -/
def sqExitProgram : List Instr :=
  [.op .JUMPDEST, .push 2 9280, .op .MLOAD, .op (.Dup ⟨6, by decide⟩), .op .ADD,
   .op (.Dup ⟨0, by decide⟩), .push 2 9280, .op .MSTORE, .push 2 4860, .op .JUMPI]

/-- `last` (4841): the frame's `ret` slot becomes `after_sq`, then leave through `nx`. -/
def lastProgram : List Instr :=
  [.push 2 3266, .op (.Swap ⟨15, by decide⟩), .op .POP, .push 2 4777, .op .JUMP]

/-- `more` (4896): call the CSUB as a subroutine returning to `again`. -/
def moreProgram : List Instr :=
  [.op .JUMPDEST, .push 2 4869, .op (.Dup ⟨15, by decide⟩), .push 2 4792, .op .JUMP]

/-- `again` (4905): re-stage, re-zero, reset three frame slots, fall into `sq_row`. -/
...
def sqExitBlock : Block Artifact.submissionArtifact .Osaka 4835 sqExitProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3742 10 4835 sqExitProgram
    (by decide) (by rfl) (by rfl) (by decide)

def lastBlock : Block Artifact.submissionArtifact .Osaka 4851 lastProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3752 5 4851 lastProgram
    (by decide) (by rfl) (by rfl) (by decide)

def moreBlock : Block Artifact.submissionArtifact .Osaka 4860 moreProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3757 5 4860 moreProgram
    (by decide) (by rfl) (by rfl) (by decide)

def againBlock : Block Artifact.submissionArtifact .Osaka 4869 againProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3762 22 4869 againProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem jumpDest4762 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4869 = true :=
