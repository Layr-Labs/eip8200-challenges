...
* decrements the square counter in memory word 2720 (`0x2440`, written by the caller) and
  stores it back;
...
def sqExitProgram : List Instr :=
  [.op .JUMPDEST, .push 2 2720, .op .MLOAD, .push 1 1, .op (.Swap ⟨0, by decide⟩), .op .SUB,
   .op (.Dup ⟨0, by decide⟩), .push 2 2720, .op .MSTORE, .push 2 4739, .op .JUMPI]

/-- `last` (4719): the frame's `ret` slot becomes `after_sq`, then leave through `nx`. -/
def lastProgram : List Instr :=
  [.push 2 3231, .op (.Swap ⟨15, by decide⟩), .op .POP, .push 2 4650, .op .JUMP]
...
def againProgram : List Instr :=
  [.op .JUMPDEST, .push 2 2784, .op .MLOAD, .op (.Dup ⟨0, by decide⟩), .push 2 512,
   .push 2 2400, .op .MCOPY,
   .op (.Dup ⟨0, by decide⟩), .push 1 64, .op .ADD, .op .CALLDATASIZE, .push 2 2048,
   .op .CALLDATACOPY,
...
/-- The memory after `sq_exit`'s `MSTORE`: the counter word 2720 holds `c`. -/
def countMem (mem : ByteArray) (c : Nat) : ByteArray :=
  MachineState.writeBytes mem (Data.Bytes.natToBytesPadded c 32) 2720

theorem readWord_countMem (mem : ByteArray) (c : Nat) (hc : c < 2 ^ 256) :
    MachineState.readWord (countMem mem c) 2720 = UInt256.ofNat c := by
  have h : (UInt256.ofNat c).toNat = c := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt hc]
  calc MachineState.readWord (countMem mem c) 2720
      = MachineState.readWord
          (MachineState.writeBytes mem
            (Data.Bytes.natToBytesPadded (UInt256.ofNat c).toNat 32) 2720) 2720 := by
        rw [countMem, h]
    _ = UInt256.ofNat c := Challenge.EvmProof.Memory.readWord_writeWord mem 2720 (UInt256.ofNat c)

theorem readWord_countMem_disjoint (mem : ByteArray) (c addr : Nat)
    (hd : addr + 32 ≤ 2720 ∨ 2752 ≤ addr) :
