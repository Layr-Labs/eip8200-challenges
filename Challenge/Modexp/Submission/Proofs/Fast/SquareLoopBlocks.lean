...
def pcNx : Nat := 4322
def pcSqExit : Nat := 4378
def pcLast : Nat := 4400
def pcAgain : Nat := 4422
/-- H2：CSUB 返回点（R4 挂钩）。 -/
def pcH2 : Nat := 4412
...
def frameStack (n : Nat) (pbi ent tl inv m0 m96 m64 m32 aprev pdst ret : UInt256) :
  ...
  [pbi, UInt256.ofNat 4447, UInt256.ofNat (2368 - 32), ent, negative32, allOnes, l2Target n,
...
theorem frameAt_eq_sqExitState (s : State) (mem : ByteArray) (n : Nat)
    ...
    frameAt pcSqExit s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest =
      CiosCachedTailDefs.sqExitState s mem pbi 2368 n (UInt256.ofNat 4447) ent inv m0
...
theorem frameAt_eq_nxJdState (s : State) (mem : ByteArray) (n : Nat)
    ...
    frameAt pcNx s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest =
      CiosCachedTailDefs.nxJdState s mem pbi 2368 n (UInt256.ofNat 4447) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest) := rfl
...
def sqExitProgram : List Instr :=
  [.op .JUMPDEST, .push 2 4412, .push 2 2368, .push 2 2624, .op .MLOAD,
   .op (.Dup ⟨8, by decide⟩), .op .ADD,
   .op (.Dup ⟨0, by decide⟩), .push 2 2624, .op .MSTORE, .push 2 4336, .op .JUMPI]

/-- `last`: drop the unused call pair, then call the CSUB as a subroutine returning to the
post-loop block. -/
def lastProgram : List Instr :=
  [.op .POP, .op .POP, .push 2 3672, .push 2 2368, .push 2 4336, .op .JUMP]
...
def sqExitBlock : Block Artifact.submissionArtifact .Osaka 4378 sqExitProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3318 12 4378 sqExitProgram
    (by decide) (by rfl) (by rfl) (by decide)

def lastBlock : Block Artifact.submissionArtifact .Osaka 4400 lastProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3330 6 4400 lastProgram
    (by decide) (by rfl) (by rfl) (by decide)

def againBlock : Block Artifact.submissionArtifact .Osaka 4422 againProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3342 16 4422 againProgram
    (by decide) (by rfl) (by rfl) (by decide)

/-! ## Jump destinations of the loop -/

theorem jumpDest4683 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4336 = true :=
