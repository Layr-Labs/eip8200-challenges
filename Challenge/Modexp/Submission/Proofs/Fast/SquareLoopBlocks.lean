...
def frameStack (n : Nat) (pbi ent tl inv m0 m96 m64 m32 aprev pdst ret : UInt256) :
  ...
  [pbi, UInt256.ofNat 4699, UInt256.ofNat (2048 - 32), ent, negative32, allOnes, l2Target n,
...
theorem frameAt_eq_sqExitState (s : State) (mem : ByteArray) (n : Nat)
    ...
    frameAt pcSqExit s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest =
      CiosCachedTailDefs.sqExitState s mem pbi 2048 n (UInt256.ofNat 4699) ent inv m0
...
theorem frameAt_eq_nxJdState (s : State) (mem : ByteArray) (n : Nat)
    ...
    frameAt pcNx s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest =
      CiosCachedTailDefs.nxJdState s mem pbi 2048 n (UInt256.ofNat 4699) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest) := rfl
...
def sqExitProgram : List Instr :=
  [.op .JUMPDEST, .push 3 9280, .op .MLOAD, .op (.Dup ⟨6, by decide⟩), .op .JUMPDEST, .op .ADD,
   .op (.Dup ⟨0, by decide⟩), .push 2 9280, .op .MSTORE, .push 2 4650, .op .JUMPI]

/-- `last` (4719): the frame's `ret` slot becomes `after_sq`, then leave through `nx`. -/
def lastProgram : List Instr :=
  [.push 2 3231, .op (.Swap ⟨15, by decide⟩), .op .POP, .push 2 4517, .op .JUMP]

/-- `more` (4728): call the CSUB as a subroutine returning to `again`. -/
def moreProgram : List Instr :=
  [.op .JUMPDEST, .push 2 4755, .op (.Dup ⟨15, by decide⟩), .push 2 4536, .op .JUMP]
...
def sqExitBlock : Block Artifact.submissionArtifact .Osaka 4623 sqExitProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3509 11 4623 sqExitProgram
    (by decide) (by rfl) (by rfl) (by decide)

def lastBlock : Block Artifact.submissionArtifact .Osaka 4641 lastProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3520 5 4641 lastProgram
    (by decide) (by rfl) (by rfl) (by decide)

def moreBlock : Block Artifact.submissionArtifact .Osaka 4650 moreProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3525 5 4650 moreProgram
    (by decide) (by rfl) (by rfl) (by decide)

def againBlock : Block Artifact.submissionArtifact .Osaka 4666 againProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3537 22 4666 againProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem jumpDest4753 :
  ...
  Artifact.isValidJumpDest_index 3525 (by rfl)

theorem jumpDest4762 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4666 = true :=
...
theorem jumpDest4683 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4536 = true :=
