...
The multiply is called at the `mul entry` (pc 4077) with `[pa, pb, pdst, ret] ++ rest`;
it pushes its row head `hd = 4274` and falls into `common` (pc 4163), which the square
call enters directly with `hd = 4960`.  Widths of four and eight limbs continue at the
kernel `setup` (pc 4187, `CiosCached.setupState`, `hd` on top); every other width drops
`hd` and enters the generic `MONPRO` at pc 1749 (`Monpro.mpEntryState`).

The 62-instruction `setup` (instructions 3115..3176, `StagedOperand.fullEntryProgram`,
...
`commonState s mem 4960 2048 2048 (UInt256.ofNat 2048) ret tail`), `dispatchState` (= `Exp.mpCall`),
`gasSteps_mulEntry`, `gasSteps_common`, `gasSteps_commonFallback(OfWidth)`, `gasSteps_setup`,
`gasSteps_commonSetup(Input)`, `gasSteps_mulSetup`, and the jump destinations 4077/4163/4187/4274/4960.
-/
...
/-- The multiply call state: `mul entry`, pc 4077, `[pa, pb, pdst, ret] ++ rest`. -/
...
/-- The shared `common` block, pc 4163, with the row head `hd` above the call frame
(`hd = 4274` after the `mul entry`, `hd = 4960` for the square). -/
...
/-- The `mul entry` JUMPDEST (instruction 3098, pc 4077 = 0x0f50). -/
theorem jumpDest4012 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4151 = true := by
  exact Artifact.isValidJumpDest_index 3140 (by rfl)

/-- The `common` JUMPDEST (instruction 3100, pc 4163 = 0x0f54). -/
theorem jumpDestCommon :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4155 = true := by
  exact Artifact.isValidJumpDest_index 3142 (by rfl)

/-- The kernel `setup` JUMPDEST (instruction 3115, pc 4187 = 0x0f6c). -/
theorem jumpDestSetup :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4179 = true := by
  exact Artifact.isValidJumpDest_index 3157 (by rfl)

/-- The multiply row head (instruction 3040, pc 4274 = 0x0fc5). -/
theorem jumpDestRowHead :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4266 = true := by
  exact Artifact.isValidJumpDest_index 3217 (by rfl)

/-- The square row head `sq_row` (instruction 3618, pc 4960 = 0x1266). -/
theorem jumpDestSqRow :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4995 = true := by
  exact Artifact.isValidJumpDest_index 3810 (by rfl)

/-- `jumpDestRowHead` in the `hd.toNat` form taken by `gasSteps_setup`/`gasSteps_commonSetup`. -/
theorem jumpDestRowHead' :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode (UInt256.ofNat 4266).toNat = true := by
  rw [show (UInt256.ofNat 4266).toNat = 4266 by decide]
  exact jumpDestRowHead

/-- `jumpDestSqRow` in the `hd.toNat` form taken by `gasSteps_setup`/`gasSteps_commonSetup`. -/
theorem jumpDestSqRow' :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode (UInt256.ofNat 4995).toNat = true := by
  rw [show (UInt256.ofNat 4995).toNat = 4995 by decide]
  exact jumpDestSqRow

/-- The square call state (`Exp.sqCall s mem ret tail`) is definitionally `commonState`
with `hd = 4960` and `pa = pb = pdst = 2048`. -/
example (s : State) (mem : ByteArray) (ret : UInt256) (tail : List UInt256) :
    ({ s with pc := UInt256.ofNat 4155
              ...
              memory := mem } : State) =
      commonState s mem 4995 512 512 (UInt256.ofNat 512) ret tail := rfl
...
private theorem activeWords9344 (s : State) (hact : 168 ≤ s.activeWords.toNat) :
    UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 5248 32) =
      s.activeWords := by
  have hnat : MachineState.activeWordsAfter s.activeWords.toNat 5248 32 =
...
theorem run_mulEntry (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    runInstructions mulEntryProgram (dispatchState s mem pa pb pdst ret rest) =
      some (commonState s mem (UInt256.ofNat 4266) pa pb pdst ret rest) := by
...
theorem run_commonGuard (s : State) (mem : ByteArray) (hd : UInt256) (pa pb : Nat)
    ...
    (hs32 : MachineState.readWord mem 5248 = UInt256.ofNat 128 ∨
      MachineState.readWord mem 5248 = UInt256.ofNat 256) :
  ...
  have hcond :
      UInt256.isTrue (UInt256.lor ((UInt256.ofNat 256).eq (MachineState.readWord mem 5248))
        ((UInt256.ofNat 128).eq (MachineState.readWord mem 5248))) := by
    rcases hs32 with h | h <;> rw [h] <;> decide
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4179 = true := by
...
set_option linter.unusedSimpArgs false in
/-- `common` with any other width falls through its `JUMPI` (pc 3904). -/
theorem run_commonGuardFallthrough (s : State) (mem : ByteArray) (hd : UInt256) (pa pb : Nat)
  ...
  have h128Nat : (UInt256.ofNat 128).toNat ≠
      (MachineState.readWord mem 5248).toNat :=
...
set_option linter.unusedSimpArgs false in
/-- The fallback drops `hd` and enters the generic `MONPRO` (pc 1749). -/
theorem run_commonFallback (s : State) (mem : ByteArray) (hd : UInt256) (pa pb : Nat)
  ...
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hjd : Decode.isValidJumpDest s.executionEnv.code 1756 = true := by
...
/-- The kernel `setup`: instructions 3115..3176 (pc 4187 = 0x0f6c .. 4273), 62 instructions. -/
def setup : Block Artifact.submissionArtifact .Osaka 4179 StagedOperand.fullEntryProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3157 60 4179 StagedOperand.fullEntryProgram
...
opaque gasSteps_mulEntry (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (commonState s mem (UInt256.ofNat 4266) pa pb pdst ret rest) :=
...
/-- `common` → generic `MONPRO` (pc 1749) for every other width (any `hd`). -/
...
/-- Multiply, four limbs: `mul entry` → `common` → `setup` with `hd = 4274`. -/
opaque gasSteps_dispatch4 (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (dispatchState s mem pa pb pdst ret rest)
      (CiosCached.setupState s mem (UInt256.ofNat 4266) pa pb pdst ret rest) :=
  (gasSteps_mulEntry s mem pa pb pdst ret rest hcap hrun hcode hfork hnp).trans
    (gasSteps_common s mem (UInt256.ofNat 4266) pa pb pdst ret rest hcap hrun hcode hfork hnp
      hact (Or.inl hs32))

/-- Multiply, eight limbs: `mul entry` → `common` → `setup` with `hd = 4274`. -/
opaque gasSteps_dispatch8 (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat)
    (hs32 : MachineState.readWord mem 5248 = UInt256.ofNat 256) :
    Challenge.EvmProof.GasSteps
      (dispatchState s mem pa pb pdst ret rest)
      (CiosCached.setupState s mem (UInt256.ofNat 4266) pa pb pdst ret rest) :=
  (gasSteps_mulEntry s mem pa pb pdst ret rest hcap hrun hcode hfork hnp).trans
...
opaque gasSteps_dispatchFallback (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (mpEntryState s mem pa pb pdst ret rest) :=
  (gasSteps_mulEntry s mem pa pb pdst ret rest hcap hrun hcode hfork hnp).trans
    (gasSteps_commonFallback s mem (UInt256.ofNat 4266) pa pb pdst ret rest hcap hrun hcode
...
/-- The kernel `setup` (pc 4187 → `hd`): stages `a` at 4864, zeroes `t` and builds the
...
4274 for the multiply, 4960 for the square). -/
opaque gasSteps_commonSetup (s : State) (mem : ByteArray) (hd : UInt256) (pa pb n : Nat)
  ...
  have hguard : MachineState.readWord mem 5248 = UInt256.ofNat 128 ∨
      MachineState.readWord mem 5248 = UInt256.ofNat 256 := by
...
/-- Multiply, four or eight limbs: `mul entry` → `common` → `setup` → row-0 head at 4274
...
opaque gasSteps_mulSetup (s : State) (mem : ByteArray) (pa pb n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (CiosCached.outState s (mpZeroed s (StagedOperand.stage mem pa n) n) pb n 0
        (UInt256.ofNat 4266) (CiosCached.l1Target n)
        (MachineState.readWord mem 5280) (MachineState.readWord mem (32 * n - 32))
        (MachineState.readWord mem 5344 :: MachineState.readWord mem 96 ::
          MachineState.readWord mem 64 :: MachineState.readWord mem 32 ::
          UInt256.ofNat (pa + 32 * n - 32) :: pdst :: ret :: rest)) :=
  (gasSteps_mulEntry s mem pa pb pdst ret rest (by omega) hrun hcode hfork hnp).trans
    (gasSteps_commonSetup s mem (UInt256.ofNat 4266) pa pb n pdst ret rest hcap hrun hcode hfork
