...
theorem jumpDestSqRow :
  ...
  exact Artifact.isValidJumpDest_index 3604 (by rfl)
...
example (s : State) (mem : ByteArray) (ret : UInt256) (tail : List UInt256) :
    ({ s with pc := UInt256.ofNat 3924
              ...
              memory := mem } : State) =
      commonState s mem 4710 2048 2048 (UInt256.ofNat 2048) ret tail := rfl
...
def setup : Block Artifact.submissionArtifact .Osaka 3948 StagedOperand.fullEntryProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 2978 62 3948 StagedOperand.fullEntryProgram
