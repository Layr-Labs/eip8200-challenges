...
theorem jumpDestRowHead' :
  ...
  rw [show (UInt256.ofNat 3461).toNat = 3461 by decide]
...
theorem jumpDestSqRow' :
  ...
  rw [show (UInt256.ofNat 4175).toNat = 4175 by decide]
  exact jumpDestSqRow
...
example (s : State) (mem : ByteArray) (ret : UInt256) (tail : List UInt256) :
    ({ s with pc := UInt256.ofNat 3323
              ...
              memory := mem } : State) =
      commonState s mem 4175 512 512 (UInt256.ofNat 512) ret tail := rfl
