...
kernel `setup` (pc 3909, `CiosCached.setupState`, `hd` on top); every other width drops
`hd` and enters the generic `MONPRO` at pc 1667 (`Monpro.mpEntryState`).

The 62-instruction `setup` (instructions 2974..3035, `StagedOperand.fullEntryProgram`,
...
`gasSteps_commonSetup(Input)`, `gasSteps_mulSetup`, and the jump destinations 3881/3885/3909/3996/4710.
-/
...
(`hd = 3996` after the `mul entry`, `hd = 4710` for the square). -/
...
theorem jumpDest4012 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 3873 = true := by
  exact Artifact.isValidJumpDest_index 2951 (by rfl)

/-- The `common` JUMPDEST (instruction 2959, pc 3885 = 0x0f54). -/
theorem jumpDestCommon :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 3877 = true := by
  exact Artifact.isValidJumpDest_index 2953 (by rfl)

/-- The kernel `setup` JUMPDEST (instruction 2974, pc 3909 = 0x0f6c). -/
theorem jumpDestSetup :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 3901 = true := by
  exact Artifact.isValidJumpDest_index 2968 (by rfl)

/-- The multiply row head (instruction 3040, pc 3996 = 0x0fc5). -/
theorem jumpDestRowHead :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 3988 = true := by
  exact Artifact.isValidJumpDest_index 3028 (by rfl)

/-- The square row head `sq_row` (instruction 3604, pc 4710 = 0x1266). -/
theorem jumpDestSqRow :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4734 = true := by
  exact Artifact.isValidJumpDest_index 3624 (by rfl)

/-- `jumpDestRowHead` in the `hd.toNat` form taken by `gasSteps_setup`/`gasSteps_commonSetup`. -/
theorem jumpDestRowHead' :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode (UInt256.ofNat 3988).toNat = true := by
  rw [show (UInt256.ofNat 3988).toNat = 3988 by decide]
  exact jumpDestRowHead

/-- `jumpDestSqRow` in the `hd.toNat` form taken by `gasSteps_setup`/`gasSteps_commonSetup`. -/
theorem jumpDestSqRow' :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode (UInt256.ofNat 4734).toNat = true := by
  rw [show (UInt256.ofNat 4734).toNat = 4734 by decide]
  exact jumpDestSqRow
...
example (s : State) (mem : ByteArray) (ret : UInt256) (tail : List UInt256) :
    ({ s with pc := UInt256.ofNat 3877
              ...
              memory := mem } : State) =
      commonState s mem 4734 2048 2048 (UInt256.ofNat 2048) ret tail := rfl
...
theorem run_mulEntry (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    runInstructions mulEntryProgram (dispatchState s mem pa pb pdst ret rest) =
      some (commonState s mem (UInt256.ofNat 3988) pa pb pdst ret rest) := by
...
theorem run_commonGuard (s : State) (mem : ByteArray) (hd : UInt256) (pa pb : Nat)
  ...
  have hjd : Decode.isValidJumpDest s.executionEnv.code 3901 = true := by
...
/-- The kernel `setup`: instructions 2974..3035 (pc 3909 = 0x0f6c .. 3995), 62 instructions. -/
def setup : Block Artifact.submissionArtifact .Osaka 3901 StagedOperand.fullEntryProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 2968 60 3901 StagedOperand.fullEntryProgram
...
opaque gasSteps_mulEntry (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (commonState s mem (UInt256.ofNat 3988) pa pb pdst ret rest) :=
...
opaque gasSteps_dispatch4 (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (dispatchState s mem pa pb pdst ret rest)
      (CiosCached.setupState s mem (UInt256.ofNat 3988) pa pb pdst ret rest) :=
  (gasSteps_mulEntry s mem pa pb pdst ret rest hcap hrun hcode hfork hnp).trans
    (gasSteps_common s mem (UInt256.ofNat 3988) pa pb pdst ret rest hcap hrun hcode hfork hnp
      hact (Or.inl hs32))
...
opaque gasSteps_dispatchFallback (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (mpEntryState s mem pa pb pdst ret rest) :=
  (gasSteps_mulEntry s mem pa pb pdst ret rest hcap hrun hcode hfork hnp).trans
    (gasSteps_commonFallback s mem (UInt256.ofNat 3988) pa pb pdst ret rest hcap hrun hcode
...
/-- The kernel `setup` (pc 3909 → `hd`): stages `a` at 8960, zeroes `t` and builds the
...
3996 for the multiply, 4710 for the square). -/
...
opaque gasSteps_mulSetup (s : State) (mem : ByteArray) (pa pb n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (CiosCached.outState s (mpZeroed s (StagedOperand.stage mem pa n) n) pb n 0
        (UInt256.ofNat 3988) (CiosCached.l1Target n)
        (MachineState.readWord mem 9376) (MachineState.readWord mem (32 * n - 32))
        (MachineState.readWord mem 9440 :: MachineState.readWord mem 96 ::
          MachineState.readWord mem 64 :: MachineState.readWord mem 32 ::
          UInt256.ofNat (pa + 32 * n - 32) :: pdst :: ret :: rest)) :=
  (gasSteps_mulEntry s mem pa pb pdst ret rest (by omega) hrun hcode hfork hnp).trans
    (gasSteps_commonSetup s mem (UInt256.ofNat 3988) pa pb n pdst ret rest hcap hrun hcode hfork
