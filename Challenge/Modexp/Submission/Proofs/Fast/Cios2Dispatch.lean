...
def specializedEntryState (s : State) (mem : ByteArray) (pa pb : Nat)
    (pdst ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 4199
           stack := [UInt256.ofNat pa, UInt256.ofNat pb, pdst, ret] ++ rest
           memory := mem }

theorem jumpDest4057 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4176 = true := by
  exact Artifact.isValidJumpDest_index 2916 (by rfl)

theorem jumpDest4080 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4199 = true := by
  exact Artifact.isValidJumpDest_index 2930 (by rfl)
...
theorem run_dispatch4 (s : State) (mem : ByteArray) (pa pb : Nat)
  ...
  have h4080 : (4199 : UInt256).toNat = 4199 := by decide
  have h4080' : (4199 : UInt256) = UInt256.ofNat 4199 := by decide
  have hcond128 :
...
theorem run_dispatchFallback (s : State) (mem : ByteArray) (pa pb : Nat)
  ...
  have hc6 : rest.length + 6 < 1024 := by omega
  have hc7 : rest.length + 7 < 1024 := by omega
  have h1939 : (1930 : UInt256).toNat = 1930 := by decide
  have h1939' : (1930 : UInt256) = UInt256.ofNat 1930 := by decide
