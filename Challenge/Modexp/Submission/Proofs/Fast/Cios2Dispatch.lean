...
def specializedEntryState (s : State) (mem : ByteArray) (pa pb : Nat)
    (pdst ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 5010
           stack := [UInt256.ofNat pa, UInt256.ofNat pb, pdst, ret] ++ rest
           memory := mem }

theorem jumpDest4012 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4049 = true := by
  exact Artifact.isValidJumpDest_index 3066 (by rfl)

theorem jumpDest4035 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 5010 = true := by
  exact Artifact.isValidJumpDest_index 3809 (by rfl)
...
theorem run_dispatch4 (s : State) (mem : ByteArray) (pa pb : Nat)
  ...
  have h4080 : (5010 : UInt256).toNat = 5010 := by decide
  have h4080' : (5010 : UInt256) = UInt256.ofNat 5010 := by decide
  have hcond128 :
