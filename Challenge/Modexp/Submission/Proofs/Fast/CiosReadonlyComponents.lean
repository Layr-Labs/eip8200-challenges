...
def cacheStack (bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret : UInt256)
    (rest : List UInt256) : List UInt256 :=
  baseStack bi pbi pa pb flag target2 inv (m0 :: m128 :: m96 :: m64 :: m32 :: tl :: dst :: ret :: rest)

def entryPrelude : List Instr :=
  [.push 1 64, .op .MLOAD, .push 1 96, .op .MLOAD, .push 1 128, .op .MLOAD,
   .push 2 9408, .op .MLOAD, .op .MLOAD, .push 2 9376, .op .MLOAD,
   .push 1 32, .op .MLOAD, .push 2 9440, .op .MLOAD,
...
theorem run_entryPrelude (s : State) (pa pb dst ret : UInt256) (n : Nat)
    ...
    runInstructions entryPrelude
      (framed s (UInt256.ofNat 4161) ([pa,pb,dst,ret] ++ rest)) =
    some (framed s (UInt256.ofNat 4189)
      ([pa,pb,MachineState.readWord s.memory 9376,MachineState.readWord s.memory (32*n-32),
        MachineState.readWord s.memory 128,MachineState.readWord s.memory 96,
        MachineState.readWord s.memory 64,MachineState.readWord s.memory 32,
        MachineState.readWord s.memory 9440,dst,ret] ++ rest)) := by
  ...
  have hp128 : (128 : UInt256).toNat = 128 := by decide
  ...
  have hG := activeWords_fix s 128 32 (by decide) (by omega) hact
  ...
  simp [entryPrelude, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    framed, hc4, hc5, hc6, hc7, hc8, hc9, hc10, hc11, hp128, hp96, hp9440, hp9376, hp9408, hp64, hp32, hml,
    State.activeWordsAfterUInt256, Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod, Challenge.EvmProof.Word.word_toNat_ofNat,
    hmod, hA, hB, hC, hD, hE, hF, hG, hH, List.exchange]

def dropCache : List Instr := [.op .POP, .op .POP, .op .POP, .op .POP, .op .POP, .op .POP, .op .POP]

theorem run_dropCache (s : State) (tl inv m0 m128 m96 m64 m32 dst ret : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998) :
    runInstructions dropCache
      (framed s (UInt256.ofNat 4879) ([inv,m0,m128,m96,m64,m32,tl,dst,ret] ++ rest)) =
    some (framed s (UInt256.ofNat 4886) ([dst,ret] ++ rest)) := by
...
def cachedLoadLow : List Instr := [.op (.Dup ⟨14, by decide⟩), .op .MLOAD]
def cachedMakeMu : List Instr :=
  [.op (.Dup ⟨0, by decide⟩), .op (.Dup ⟨10, by decide⟩), .op .MUL,
   .op (.Swap ⟨0, by decide⟩)]
def cachedLoadMod : List Instr := [.op (.Dup ⟨11, by decide⟩)]

theorem run_cachedLoadLow (s : State) (bi pbi pa pb flag target2 inv m0 m128 m96 m64 m32 dst ret : UInt256)
    (n : Nat) (rest : List UInt256) (hcap : rest.length ≤ 998) (hn : n ≤ 32)
    (hact : 296 ≤ s.activeWords.toNat) :
    runInstructions cachedLoadLow
      (framed s (UInt256.ofNat 4566)
        (cacheStack bi pbi pa pb flag target2 (UInt256.ofNat (8224+32*n)) inv m0 m128 m96 m64 m32 dst ret rest)) =
    some (framed s (UInt256.ofNat 4568)
      ([MachineState.readWord s.memory (8224+32*n)] ++
        cacheStack bi pbi pa pb flag target2 (UInt256.ofNat (8224+32*n)) inv m0 m128 m96 m64 m32 dst ret rest)) := by
...
theorem run_cachedMakeMu (s : State) (bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret t0 : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998) :
    runInstructions cachedMakeMu
      (framed s (UInt256.ofNat 4568)
        ([t0] ++ cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) =
    some (framed s (UInt256.ofNat 4572)
      ([t0, inv*t0] ++ cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) := by
  have hc18 : rest.length + 18 < 1024 := by omega
...
theorem run_cachedLoadMod (s : State) (bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret mu t0 : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998) :
    runInstructions cachedLoadMod
      (framed s (UInt256.ofNat 4572)
        ([t0,mu] ++ cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) =
    some (framed s (UInt256.ofNat 4573)
      ([m0,t0,mu] ++ cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) := by
  have hc19 : rest.length + 19 < 1024 := by omega
  simp [cachedLoadMod, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    framed, cacheStack, baseStack, Nat.add_assoc, hc19, Challenge.EvmProof.Word.succ_ofNat_mod]

def makeModProduct : List Instr :=
  [.op (.Dup ⟨2, by decide⟩), .op (.Dup ⟨10, by decide⟩),
...
theorem run_cachedMakeModProduct (s : State)
    (bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret mu t0 : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998) :
    runInstructions makeModProduct
      (framed s (UInt256.ofNat 4573)
        ([m0,t0,mu] ++ cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) =
    some (framed s (UInt256.ofNat 4577)
      ([UInt256.mulMod m0 mu maxWord,t0,mu] ++
        cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) := by
  have hc20 : rest.length + 20 < 1024 := by omega
  ...
  simp [makeModProduct,
    ...
    allOnes_value, maxWord, List.exchange, Challenge.EvmProof.Word.succ_ofNat_mod]

theorem run_cachedFinishCarry (s : State)
    (bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret mm mu t0 : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998) :
    runInstructions finishCarry
      (framed s (UInt256.ofNat 4577)
        ([mm,t0,mu] ++ cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) =
    some (framed s (UInt256.ofNat 4583)
      ([endCarry t0 mm,mu] ++ cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) := by
  have hc20 : rest.length + 20 < 1024 := by omega
  have hc21 : rest.length + 21 < 1024 := by omega
  simp [finishCarry,
    runInstructions, Challenge.EvmProof.Stepper.runInstr,
...
def cachedProduct : List Instr :=
  (((cachedLoadLow ++ cachedMakeMu) ++ cachedLoadMod) ++
    makeModProduct) ++ finishCarry

theorem run_cachedProduct (s : State) (bi pbi pa pb flag target2 inv m0 m128 m96 m64 m32 dst ret : UInt256)
    (n : Nat) (rest : List UInt256) (hcap : rest.length ≤ 998) (hn : n ≤ 32)
    (hact : 296 ≤ s.activeWords.toNat) :
    ...
    runInstructions cachedProduct
      (framed s (UInt256.ofNat 4566)
        (cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) =
    some (framed s (UInt256.ofNat 4583)
      ([endCarry t0 (UInt256.mulMod m0 (inv*t0) maxWord),inv*t0] ++
        cacheStack bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) := by
  let tl := UInt256.ofNat (8224+32*n)
  let t0 := MachineState.readWord s.memory (8224+32*n)
  have h1 := run_cachedLoadLow s bi pbi pa pb flag target2 inv m0 m128 m96 m64 m32 dst ret n rest hcap hn hact
  have h2 := run_cachedMakeMu s bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret t0 rest hcap
  have h3 := run_cachedLoadMod s bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret (inv*t0) t0 rest hcap
  have h4 := run_cachedMakeModProduct s bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret (inv*t0) t0 rest hcap
  have h5 := run_cachedFinishCarry s bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret
...
theorem run_cachedProduct_model (s : State)
    (bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret : UInt256)
    (n : Nat) (rest : List UInt256) (hcap : rest.length ≤ 998) (hn : n ≤ 32)
    (hact : 296 ≤ s.activeWords.toNat)
  ...
  have hr := run_cachedProduct s bi pbi pa pb flag target2
    ...
    m128 m96 m64 m32 dst ret n rest hcap hn hact
