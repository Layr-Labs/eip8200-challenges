...
def cacheStack (bi pbi pa pb flag tl inv m0 dst ret : UInt256)
    (rest : List UInt256) : List UInt256 :=
  baseStack bi pbi pa pb flag tl inv (m0 :: dst :: ret :: rest)

def entryPrelude : List Instr :=
  [.push 2 9440, .op .MLOAD, .push 2 9376, .op .MLOAD,
   .push 2 9408, .op .MLOAD, .op .MLOAD,
   .push 1 64, .op .MLOAD, .push 1 32, .op .MLOAD,
   .op (.Swap ⟨5, by decide⟩), .op (.Swap ⟨0, by decide⟩),
   .op (.Swap ⟨4, by decide⟩), .op (.Swap ⟨1, by decide⟩),
   .op (.Swap ⟨3, by decide⟩), .op (.Swap ⟨1, by decide⟩)]

theorem run_entryPrelude (s : State) (pa pb dst ret : UInt256) (n : Nat)
    (rest : List UInt256) (hcap : rest.length ≤ 1001) (hn : n ≤ 32)
  ...
  simp [entryPrelude, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    framed, hc4, hc5, hc6, hc7, hc8, hc9, hp9440, hp9376, hp9408, hp64, hp32, hml,
    State.activeWordsAfterUInt256, Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod, Challenge.EvmProof.Word.word_toNat_ofNat,
    hmod, hA, hB, hC, hD, hE, hF, List.exchange]

def dropCache : List Instr := [.op .POP, .op .POP, .op .POP, .op .POP, .op .POP]
...
theorem run_dropCache (s : State) (tl inv m0 m64 m32 dst ret : UInt256)
  ...
  simp [dropCache, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    framed, hc3, hc4, hc5, hc6, hc7, Challenge.EvmProof.Word.succ_ofNat_mod]

def cachedLoadLow : List Instr := [.op (.Dup ⟨7, by decide⟩), .op .MLOAD]
def cachedMakeMu : List Instr :=
  [.op (.Dup ⟨0, by decide⟩), .op (.Dup ⟨10, by decide⟩), .op .MUL,
   .op (.Swap ⟨0, by decide⟩)]
def cachedLoadMod : List Instr := [.op (.Dup ⟨11, by decide⟩)]

theorem run_cachedLoadLow (s : State) (bi pbi pa pb flag inv m0 dst ret : UInt256)
  ...
  simp [cachedLoadLow, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    framed, cacheStack, baseStack, Nat.add_assoc, hc12, hc13, State.activeWordsAfterUInt256,
...
theorem run_cachedMakeMu (s : State) (bi pbi pa pb flag tl inv m0 dst ret t0 : UInt256)
  ...
  simp [cachedMakeMu, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    framed, cacheStack, baseStack, Nat.add_assoc, hc13, hc14, hc15, List.exchange,
    Challenge.EvmProof.Word.succ_ofNat_mod]
...
theorem run_cachedLoadMod (s : State) (bi pbi pa pb flag tl inv m0 dst ret mu t0 : UInt256)
  ...
  simp [cachedLoadMod, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    framed, cacheStack, baseStack, Nat.add_assoc, hc14, Challenge.EvmProof.Word.succ_ofNat_mod]
...
theorem run_cachedMakeModProduct (s : State)
  ...
  simp [makeModProduct,
    ...
    framed, cacheStack, baseStack, Nat.add_assoc, hc15, hc16, hc17,
...
theorem run_cachedFinishCarry (s : State)
  ...
  simp [finishCarry,
    runInstructions, Challenge.EvmProof.Stepper.runInstr,
    framed, cacheStack, baseStack, Nat.add_assoc, hc15, hc16, endCarry,
...
theorem run_cachedProduct (s : State) (bi pbi pa pb flag inv m0 dst ret : UInt256)
    (n : Nat) (rest : List UInt256) (hcap : rest.length ≤ 1003) (hn : n ≤ 32)
    (hact : 296 ≤ s.activeWords.toNat) :
  ...
  let tl := UInt256.ofNat (8224+32*n)
  let t0 := MachineState.readWord s.memory (8224+32*n)
  have h1 := run_cachedLoadLow s bi pbi pa pb flag inv m0 dst ret n rest hcap hn hact
  have h2 := run_cachedMakeMu s bi pbi pa pb flag tl inv m0 dst ret t0 rest hcap
  have h3 := run_cachedLoadMod s bi pbi pa pb flag tl inv m0 dst ret (inv*t0) t0 rest hcap
  have h4 := run_cachedMakeModProduct s bi pbi pa pb flag tl inv m0 dst ret (inv*t0) t0 rest hcap
  have h5 := run_cachedFinishCarry s bi pbi pa pb flag tl inv m0 dst ret
...
theorem run_cachedProduct_model (s : State)
    (bi pbi pa pb flag tl inv m0 dst ret : UInt256)
    (n : Nat) (rest : List UInt256) (hcap : rest.length ≤ 1003) (hn : n ≤ 32)
    (hact : 296 ≤ s.activeWords.toNat)
  ...
  have hr := run_cachedProduct s bi pbi pa pb flag
    (MachineState.readWord s.memory 9376) (MachineState.readWord s.memory (32*n-32))
