...
def entryPrelude : List Instr :=
  [.push 1 64, .op .MLOAD, .push 1 96, .op .MLOAD, .push 2 5344, .op .MLOAD,
   .push 2 5312, .op .MLOAD, .op .MLOAD, .push 2 5280, .op .MLOAD,
   .push 1 32, .op .MLOAD, .push 2 5312, .op .MLOAD,
...
theorem run_entryPrelude (s : State) (pa pb dst ret : UInt256) (n : Nat)
    ...
    (hml : MachineState.readWord s.memory 5312 = UInt256.ofNat (32*n-32)) :
    runInstructions entryPrelude
      (framed s (UInt256.ofNat 4291) ([pa,pb,dst,ret] ++ rest)) =
    some (framed s (UInt256.ofNat 4322)
      ([pa,pb,MachineState.readWord s.memory 5280,MachineState.readWord s.memory (32*n-32),
  ...
  have hp9440 : (5344 : UInt256).toNat = 5344 := by decide
  have hp9376 : (5280 : UInt256).toNat = 5280 := by decide
  have hp9408 : (5312 : UInt256).toNat = 5312 := by decide
  ...
  have hA := activeWords_fix s 5344 32 (by decide) (by omega) hact
  have hB := activeWords_fix s 5280 32 (by decide) (by omega) hact
  have hC := activeWords_fix s 5312 32 (by decide) (by omega) hact
...
theorem run_dropCache (s : State) (tl inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    ...
    runInstructions dropCache
      (framed s (UInt256.ofNat 4793) ([inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest)) =
    some (framed s (UInt256.ofNat 4800) ([dst,ret] ++ rest)) := by
...
theorem run_cachedLoadLow (s : State) (bi pbi pa pb flag target2 inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    ...
    runInstructions cachedLoadLow
      (framed s (UInt256.ofNat 4476)
        (cacheStack bi pbi pa pb flag target2 (UInt256.ofNat (4128+32*n)) inv m0 aEnd m96 m64 m32 dst ret rest)) =
    some (framed s (UInt256.ofNat 4478)
      ([MachineState.readWord s.memory (4128+32*n)] ++
        cacheStack bi pbi pa pb flag target2 (UInt256.ofNat (4128+32*n)) inv m0 aEnd m96 m64 m32 dst ret rest)) := by
  have hc17 : rest.length + 17 < 1024 := by omega
  have hc18 : rest.length + 18 < 1024 := by omega
  have hmod : (4128+32*n) %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      4128+32*n := Nat.mod_eq_of_lt (by omega)
  have hactQ := activeWords_fix s (4128+32*n) 32 (by decide) (by omega) hact
...
theorem run_cachedProduct (s : State) (bi pbi pa pb flag target2 inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    (n : Nat) (rest : List UInt256) (hcap : rest.length ≤ 998) (hn : n ≤ 32)
    (hact : 168 ≤ s.activeWords.toNat) :
    let tl := UInt256.ofNat (4128+32*n)
    let t0 := MachineState.readWord s.memory (4128+32*n)
...
structure ReadonlyCache (mem : ByteArray) (n : Nat) (tl inv m0 : UInt256) : Prop where
  lowAddress : tl = UInt256.ofNat (4128 + 32*n)
  inverse : inv = MachineState.readWord mem 5280
...
theorem ReadonlyCache.of_preserved {mem mem' : ByteArray} {n : Nat}
    ...
    (hinv : MachineState.readWord mem' 5280 = MachineState.readWord mem 5280)
...
theorem ReadonlyCache.l2 {mem : ByteArray} {n : Nat} {tl inv m0 : UInt256}
  ...
  h.of_preserved
    (readWord_l2Step mem mu c0 n 5280 k hn (Or.inr (by decide)))
...
theorem ReadonlyCache.middle {mem : ByteArray} {n : Nat} {tl inv m0 : UInt256}
  ...
  h.of_preserved
    (CiosCachedMidMemory.read_mid mem c 5280 (Or.inr (by decide)))
...
theorem run_cachedProduct_model (s : State)
  ...
  have hr := run_cachedProduct s bi pbi pa pb flag target2
    (MachineState.readWord s.memory 5280) (MachineState.readWord s.memory (32*n-32))
    aEnd m96 m64 m32 dst ret n rest hcap hn hact
  have hcarry := row_carry_swapped (MachineState.readWord s.memory (32*n-32))
    (MachineState.readWord s.memory 5280) (MachineState.readWord s.memory (4128+32*n)) hminv
