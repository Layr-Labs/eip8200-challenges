...
def entryPrelude : List Instr :=
  [.push 1 64, .op .MLOAD, .push 1 96, .op .MLOAD, .push 2 2880, .op .MLOAD,
   .push 2 2848, .op .MLOAD, .op .MLOAD, .push 2 2816, .op .MLOAD,
   .push 1 32, .op .MLOAD, .push 2 2848, .op .MLOAD,
...
theorem run_entryPrelude (s : State) (pa pb dst ret : UInt256) (n : Nat)
    ...
    some (framed s (UInt256.ofNat 4187)
      ([pa,pb,MachineState.readWord s.memory 2816,MachineState.readWord s.memory (32*n-32),
        MachineState.readWord s.memory 2880,MachineState.readWord s.memory 96,
  ...
  have hA := activeWords_fix s 2880 32 (by decide) (by omega) hact
  have hB := activeWords_fix s 2816 32 (by decide) (by omega) hact
  have hC := activeWords_fix s 2848 32 (by decide) (by omega) hact
...
structure ReadonlyCache (mem : ByteArray) (n : Nat) (tl inv m0 : UInt256) : Prop where
  ...
  inverse : inv = MachineState.readWord mem 2816
...
theorem ReadonlyCache.of_preserved {mem mem' : ByteArray} {n : Nat}
    ...
    (hinv : MachineState.readWord mem' 2816 = MachineState.readWord mem 2816)
...
theorem ReadonlyCache.l2 {mem : ByteArray} {n : Nat} {tl inv m0 : UInt256}
  ...
  h.of_preserved
    (readWord_l2Step mem mu c0 n 2816 k hn (Or.inr (by decide)))
...
theorem ReadonlyCache.middle {mem : ByteArray} {n : Nat} {tl inv m0 : UInt256}
  ...
  h.of_preserved
    (CiosCachedMidMemory.read_mid mem c 2816 (Or.inr (by decide)))
...
theorem run_cachedProduct_model (s : State)
  ...
  have hr := run_cachedProduct s bi pbi pa pb flag target2
    (MachineState.readWord s.memory 2816) (MachineState.readWord s.memory (32*n-32))
    aEnd m96 m64 m32 dst ret n rest hcap hn hact
  have hcarry := row_carry_swapped (MachineState.readWord s.memory (32*n-32))
    (MachineState.readWord s.memory 2816) (MachineState.readWord s.memory (2080+32*n)) hminv
