...
def program : List Instr :=
  [.op .JUMPDEST, .push 2 2688, .op .MLOAD, .push 1 64, .op .ADD,
...
theorem jumpDest :
  ...
  exact Artifact.isValidJumpDest_index 4388 (by rfl)
...
theorem run_entry (s : State) (mem : ByteArray) (pa pb n : Nat)
  ...
  simp (config := { maxSteps := 200000 })
    [program, input, legacyState, runInstructions, Challenge.EvmProof.Stepper.runInstr,
      mpZeroed, hs32, State.activeWordsAfterUInt256, hactS, hactC, hactN,
      h2688, h2048, h64, hsizeN, hcdsN, hc4, hc5, hc6, hc7,
      Challenge.EvmProof.Word.literal_eq_ofNat,
      Challenge.EvmProof.Word.word_toNat_ofNat,
      Challenge.EvmProof.Word.succ_ofNat_mod,
      Challenge.EvmProof.Word.ofNat_add_mod, htarget, List.exchange] <;>
...
def block : Block Artifact.submissionArtifact .Osaka 5454 program :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 4388 10 5454 program
...
def gasSteps_entry (s : State) (mem : ByteArray) (pa pb n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (legacyState s (mpZeroed s mem n) pa pb pdst ret rest) :=
