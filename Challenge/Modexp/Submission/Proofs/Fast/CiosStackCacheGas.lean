...
opaque l1Last (s : State) (env : Environment art .Osaka s) (blocks : KernelBlocks art)
    ...
    GasSteps (l1At 4887 s c r bi pa pb n i (n-1) dst ret rest)
...
opaque out (s : State) (env : Environment art .Osaka s) (blocks : KernelBlocks art)
    ...
    GasSteps (outAt s c r pa pb n i dst ret rest)
      (l1At 4625 s c r (rowBi c.virtual pb n i) pa pb n i 0 dst ret rest) :=
...
opaque mid (s : State) (env : Environment art .Osaka s) (blocks : KernelBlocks art)
    ...
    GasSteps (midAt s c r carry bi pa pb n i dst ret rest)
      (l2At 4953 s (cacheMid c carry) r bi (rowMu c.virtual n) (rowC0 c.virtual n)
...
opaque tail (s : State) (env : Environment art .Osaka s) (blocks : KernelBlocks art)
    ...
    GasSteps (tailAt s c r carry mu bi pa pb n i dst ret rest)
      (rowState s (if i+1 < n then 4620 else 5237) (CiosStackCacheModel.cacheTail c carry)
...
opaque exit (s : State) (env : Environment art .Osaka s) (blocks : KernelBlocks art)
    ...
    (pa pb n i : Nat) (dst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 998) (hact : 296 ≤ s.activeWords.toNat) :
    GasSteps (rowState s 5237 c r pa pb n i dst ret rest [])
...
opaque l1Dispatch (s : State) (env : Environment art .Osaka s) (blocks : KernelBlocks art)
    ...
    GasSteps (l1At 4625 s c r bi pa pb n i j dst ret rest)
      (l1At (if UInt256.isTrue (CiosCached.isFour n) then 4782 else 4630)
        s c r bi pa pb n i j dst ret rest) := by
  let p := cacheL1 c bi pa n j
  refine blocks.l1Dispatch.steps (s := l1At 4625 s c r bi pa pb n i j dst ret rest) (env.transfer rfl rfl) rfl ?_
  have h := run_dispatch { s with memory := p.cache.memory } 4625 4782 p.cache r
...
opaque l1Join (s : State) (env : Environment art .Osaka s) (blocks : KernelBlocks art)
    ...
    (pa pb n i j : Nat) (dst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 998) :
    GasSteps (l1At 4782 s c r bi pa pb n i j dst ret rest)
      (l1At 4783 s c r bi pa pb n i j dst ret rest) := by
  let p := cacheL1 c bi pa n j
  refine blocks.l1Join.steps (s := l1At 4782 s c r bi pa pb n i j dst ret rest) (env.transfer rfl rfl) rfl ?_
  have h := run_join { s with memory := p.cache.memory } 4782
...
opaque l2Dispatch (s : State) (env : Environment art .Osaka s) (blocks : KernelBlocks art)
    ...
    (pa pb n i j : Nat) (dst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 998) :
    GasSteps (l2At 4953 s c r bi mu c0 pa pb n i j dst ret rest)
      (l2At (if UInt256.isTrue (CiosCached.isFour n) then 5110 else 4958)
        s c r bi mu c0 pa pb n i j dst ret rest) := by
  let p := cacheL2 c mu c0 n j
  refine blocks.l2Dispatch.steps (s := l2At 4953 s c r bi mu c0 pa pb n i j dst ret rest) (env.transfer rfl rfl) rfl ?_
  have h := run_dispatch { s with memory := p.cache.memory } 4953 5110 p.cache r
    (UInt256.ofNat (ptrAt (pb+32*n-32) i)) (UInt256.ofNat (pa+32*n-32))
...
opaque l2Join (s : State) (env : Environment art .Osaka s) (blocks : KernelBlocks art)
    ...
    (pa pb n i j : Nat) (dst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 998) :
    GasSteps (l2At 5110 s c r bi mu c0 pa pb n i j dst ret rest)
      (l2At 5111 s c r bi mu c0 pa pb n i j dst ret rest) := by
  let p := cacheL2 c mu c0 n j
  refine blocks.l2Join.steps (s := l2At 5110 s c r bi mu c0 pa pb n i j dst ret rest) (env.transfer rfl rfl) rfl ?_
  have h := run_join { s with memory := p.cache.memory } 5110
