...
opaque gasSteps_monproCsub (L : RowLemmas) (E : EntryLemmas) (s : State) (mem : ByteArray) (pa pb n : Nat)
  ...
  (gasSteps_toCsub L E s mem pa pb n pdst ret rest (by omega) hrun hcode hfork hnp hact hn
    ...
    (Csub.gasSteps_csub s (selectedRows (mpZeroed s (inputMemory mem pa n) n) pa pb n n) n pdst ret rest
      ...
      ((readWord_selected_preserved s mem pa pb n n 2848 hn32 (by omega)).trans hml)
      ((readWord_selected_preserved s mem pa pb n n 2880 hn32 (by omega)).trans htl)
      ((Csub.csStep_readWord_disjoint (selectedRows (mpZeroed s (inputMemory mem pa n) n) pa pb n n) n 2784
            (by omega) (Or.inr (by omega)) n (Nat.le_refl n)).trans
        ((readWord_selected_preserved s mem pa pb n n 2784 hn32 (by omega)).trans hs32))
      hdstFit
      (by
