...
opaque gasSteps_fallback (s : State) (mem : ByteArray) (pa pb n : Nat)
  ...
  have h256 : MachineState.readWord mem 9344 ≠ UInt256.ofNat 256 := by
    ...
    rw [Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt h32n,
      Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt (by norm_num)] at hnat
    exact hn8 (by omega)
  have hf := (gasSteps_dispatchFallback s mem pa pb pdst ret rest (by omega) hrun hcode
  ...
  simpa only [selectedRows, if_neg (show ¬(n=8 ∧ pa=pb) by simp [hn8]), if_neg (show ¬(n=4 ∧ pa=pb) by simp [hn4]), if_neg (show ¬(n=4 ∨ n=8) by simp [hn4, hn8])] using hf
