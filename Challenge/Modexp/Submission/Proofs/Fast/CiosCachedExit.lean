...
theorem run_exit (s : State) (pbi paEnd pbEnd flag dst ret : UInt256) (rest : List UInt256)
    ...
    (htarget : Decode.isValidJumpDest s.executionEnv.code 2292 = true) :
    runInstructions exitProgram
      (framed s (UInt256.ofNat 4958) ([pbi, paEnd, pbEnd, flag, negative32, allOnes, dst, ret] ++ rest)) =
    some (framed s (UInt256.ofNat 2292) ([dst, ret] ++ rest)) := by
