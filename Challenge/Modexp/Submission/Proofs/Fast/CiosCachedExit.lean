...
theorem run_exit (s : State) (pbi paEnd pbEnd flag target2 cachedAddress cachedInverse cachedTailPointer cachedLow64 cachedLow32 dst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1006)
    (htarget : Decode.isValidJumpDest s.executionEnv.code 2220 = true) :
    runInstructions exitProgram
      (framed s (UInt256.ofNat 4865) ([pbi, paEnd, pbEnd, flag, negative32, allOnes, target2, cachedAddress, cachedInverse, cachedTailPointer, cachedLow64, cachedLow32, dst, ret] ++ rest)) =
