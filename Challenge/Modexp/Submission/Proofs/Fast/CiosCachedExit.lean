...
theorem run_exit (s : State) (pbi paEnd pbEnd flag target2 dst ret : UInt256) (rest : List UInt256)
    ...
    (htarget : Decode.isValidJumpDest s.executionEnv.code 4642 = true) :
    runInstructions exitProgram
      (framed s (UInt256.ofNat 4624) ([pbi, paEnd, pbEnd, flag, negative32, allOnes, target2, dst, ret] ++ rest)) =
    some (framed s (UInt256.ofNat 4642) ([dst, ret] ++ rest)) := by
