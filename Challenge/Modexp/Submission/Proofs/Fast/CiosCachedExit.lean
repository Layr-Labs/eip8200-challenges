...
theorem run_exit (s : State) (pbi paEnd pbEnd flag dst ret : UInt256) (rest : List UInt256)
    ...
    (htarget : Decode.isValidJumpDest s.executionEnv.code 2274 = true) :
    runInstructions exitProgram
      (framed s (UInt256.ofNat 5150) ([pbi, paEnd, pbEnd, flag, negative32, allOnes, dst, ret] ++ rest)) =
    some (framed s (UInt256.ofNat 2274) ([dst, ret] ++ rest)) := by
