...
namespace Challenge.Modexp.Submission.Proofs.Fast.TnCacheExtraTrace
open EvmSemantics EvmSemantics.EVM YulEvmCompiler
open Challenge.Modexp.Submission.Proofs.Bytecode
open Challenge.Modexp.Submission.Proofs.Fast
open CiosCachedMacCore CiosCached CiosCachedMidDefs WindowNibbleKernel Monpro

def extendedStack (carry mu bi pbi pa pb flag tn target2 tl inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    (rest : List UInt256) : List UInt256 :=
  [carry,mu,bi,pbi,pa,pb,flag,tn,allOnes,target2,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest
...
theorem run_extraLoad (slot : Fin 3) (s : State)
    (pc carry mu bi pbi pa pb flag tn target2 tl inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998) :
    runInstructions (extraLoad slot)
      (framed s pc (extendedStack carry mu bi pbi pa pb flag tn target2 tl inv m0 aEnd m96 m64 m32 dst ret rest)) =
    some (framed s pc.succ.succ
      ([allOnes,cacheWord slot m96 m64 m32] ++
        extendedStack carry mu bi pbi pa pb flag tn target2 tl inv m0 aEnd m96 m64 m32 dst ret rest)) := by
...
def legacyExtraProgram (slot : Fin 3) (tl ts : UInt256) : List Instr :=
  extraLoad slot ++ CiosCached.macFusedProgram tl ts

theorem run_legacyExtraStep (slot : Fin 3) (template : State) (pc : UInt256) (mem : ByteArray)
    ...
    (pbi paEnd pbEnd flag tn target2 cachedTL inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    ...
    runInstructions (legacyExtraProgram slot loadAddr storeAddr)
      (TnCacheL2Trace.state template pc mem bi mu c0 n k pbi paEnd pbEnd flag tn target2 inv
        (m0 :: cachedTL :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest)) =
    some (TnCacheL2Trace.state template (pc+UInt256.ofNat 33) mem bi mu c0 n (k+1)
      pbi paEnd pbEnd flag tn target2 inv (m0 :: cachedTL :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest)) := by
  ...
  have hl := run_extraLoad slot st pc (l2Step mem mu c0 n k).carry mu bi
    pbi paEnd pbEnd flag tn target2 cachedTL inv m0 aEnd m96 m64 m32 dst ret rest hrest
  ...
  have hf := CiosCachedFused.run_fused st (pc+UInt256.ofNat 2)
    ...
    ([bi,pbi,paEnd,pbEnd,flag,tn,allOnes,target2,inv,m0] ++
  ...
  simpa only [st,TnCacheL2Trace.state,framed,extendedStack,l2Step,hx,hloadAddr,hstoreAddr,hpc,
...
private theorem run_slotZeroHead (loadAddr storeAddr : UInt256) (s : State)
    ...
    runInstructions ((extraLoad 0 ++ CiosCached.macProductProgram.take 6) ++
      ...
      (framed s pc (extendedStack carry mu bi pbi pa pb flag tn target2 tl inv m0 aEnd m96 m64 m32 dst ret rest)) := by
  have hc19 : rest.length + 19 < 1024 := by omega
  have hc20 : rest.length + 20 < 1024 := by omega
...
theorem run_extraStep (slot : Fin 3) (template : State) (pc : UInt256) (mem : ByteArray)
  ...
  have hold := run_legacyExtraStep slot template pc mem bi mu c0 n k x loadAddr storeAddr
    hx hselect hloadAddr hstoreAddr pbi paEnd pbEnd flag tn target2 cachedTL inv m0 aEnd
    m96 m64 m32 dst ret rest hrest hactive hn hk hc
  by_cases hs : slot.val = 0
  · have hslot : slot = 0 := Fin.ext hs
    ...
    have hp := run_slotZeroHead loadAddr storeAddr st pc (l2Step mem mu c0 n k).carry mu bi pbi paEnd pbEnd
      flag tn target2 cachedTL inv m0 aEnd m96 m64 m32 dst ret rest hrest
    ...
    change (runInstructions (slotZeroHead loadAddr storeAddr)
      (framed st pc (extendedStack (l2Step mem mu c0 n k).carry mu bi pbi paEnd pbEnd
        flag tn target2 cachedTL inv m0 aEnd m96 m64 m32 dst ret rest))).bind _ = _
    rw [hp]
    exact hold
...
/-- Slot zero as the M128 artifact spells it: its address push is narrowed to `PUSH2`,
which makes the cell exactly the fused prefix followed by `macFusedPostProgram`, thirty-two
bytes rather than thirty-three.  The wider `slotZeroHead` above is unchanged, because the
other candidate artifact still carries the padded `PUSH3` there. -/
def extraProgramZeroNarrow (tl ts : UInt256) : List Instr :=
  (slotZeroPrefix ++ (CiosCached.macProductProgram.drop 4).take 2) ++
    CiosCached.macFusedPostProgram tl ts

/-- The fused prefix: `DUP2 DUP15 MUL DUP10 DUP16 DUP5 MULMOD` leaves exactly the frame
`run_post` consumes, seven bytes where the legacy load-and-product needs eight. -/
private theorem run_zeroPrefix (s : State) (w : UInt256)
    (pc carry mu bi pbi pa pb flag tn target2 tl inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998) (hw : w = m96) :
    runInstructions (slotZeroPrefix ++ (CiosCached.macProductProgram.drop 4).take 2)
      (framed s pc (extendedStack carry mu bi pbi pa pb flag tn target2 tl inv m0 aEnd m96 m64 m32 dst ret rest)) =
    some (framed s (pc + UInt256.ofNat 7)
      ([UInt256.mulMod mu w allOnes, w * mu, carry, mu] ++
        ([bi,pbi,pa,pb,flag,tn,allOnes,target2,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest))) := by
  subst hw
  have hc19 : rest.length + 19 < 1024 := by omega
  have hc20 : rest.length + 20 < 1024 := by omega
  have hc21 : rest.length + 21 < 1024 := by omega
  have hc22 : rest.length + 22 < 1024 := by omega
  have hc23 : rest.length + 23 < 1024 := by omega
  simp [slotZeroPrefix, CiosCached.macProductProgram, runInstructions,
    Challenge.EvmProof.Stepper.runInstr, framed, extendedStack, Nat.add_assoc,
    succ_eq_add, word_add_assoc, Challenge.EvmProof.Word.ofNat_add_mod,
    hc19, hc20, hc21, hc22, hc23]

/-- The whole slot-zero cell: seven-byte fused prefix, then `macFusedPostProgram`
through the public `run_post`.  Thirty-two bytes, one less than slots one and two. -/
theorem run_extraStepZeroNarrow (template : State) (pc : UInt256) (mem : ByteArray)
    (bi mu c0 : UInt256) (n k : Nat) (x loadAddr storeAddr : UInt256)
    (hx : x.toNat = 32*(n-2-k))
    (hselect : x.toNat = cacheAddress 0)
    (hloadAddr : loadAddr.toNat = 2112+32*(n-2-k))
    (hstoreAddr : storeAddr.toNat = 2112+32*(n-1-k))
    (pbi paEnd pbEnd flag tn target2 cachedTL inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    (rest : List UInt256) (hrest : rest.length ≤ 998)
    (hactive : 88 ≤ template.activeWords.toNat) (hn : n ≤ 8) (hk : k+1 < n)
    (hc : ExtraCache mem m96 m64 m32) :
    runInstructions (extraProgramZeroNarrow loadAddr storeAddr)
      (TnCacheL2Trace.state template pc mem bi mu c0 n k pbi paEnd pbEnd flag tn target2 inv
        (m0 :: cachedTL :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest)) =
    some (TnCacheL2Trace.state template (pc+UInt256.ofNat 32) mem bi mu c0 n (k+1)
      pbi paEnd pbEnd flag tn target2 inv (m0 :: cachedTL :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest)) := by
  have hactT : UInt256.ofNat (MachineState.activeWordsAfter template.activeWords.toNat
      (2112+32*(n-2-k)) 32) = template.activeWords :=
    activeWords_fix template _ 32 (by decide) (by omega) hactive
  ...
  have hW : UInt256.ofNat (MachineState.activeWordsAfter st.activeWords.toNat storeAddr.toNat 32) =
      st.activeWords := by simpa only [st,hstoreAddr] using hactW
  have hvalue : (cacheWord 0 m96 m64 m32) = MachineState.readWord st.memory x.toNat := by
    rw [hselect]
    exact (hc.l2 mu c0 n k hn).choose 0
  have hm96 : m96 = MachineState.readWord st.memory x.toNat := hvalue
  have hpre := run_zeroPrefix st (MachineState.readWord st.memory x.toNat) pc
    (l2Step mem mu c0 n k).carry mu bi pbi paEnd pbEnd
    flag tn target2 cachedTL inv m0 aEnd m96 m64 m32 dst ret rest hrest hm96.symm
  have hp := CiosCachedFused.run_post st (pc + UInt256.ofNat 7)
    (UInt256.mulMod mu (MachineState.readWord st.memory x.toNat) allOnes)
    ((MachineState.readWord st.memory x.toNat) * mu) (l2Step mem mu c0 n k).carry mu loadAddr storeAddr
    ([bi,pbi,paEnd,pbEnd,flag,tn,allOnes,target2,inv,m0,cachedTL,m96,m64,m32,aEnd,dst,ret] ++ rest)
    (by simp only [List.length_append, List.length_cons, List.length_nil]; omega) hT hW
  have hall := runInstructions_append_some _ _ _ _ _ hpre hp
  have hpc : (pc + UInt256.ofNat 7) + UInt256.ofNat 25 = pc + UInt256.ofNat 32 := by
    simp [word_add_assoc, Challenge.EvmProof.Word.ofNat_add_mod]
  rw [allOnes_value] at hall
  -- the two arithmetic normalisations `run_fused` performs, at this cell's arguments
  set X := MachineState.readWord st.memory x.toNat with hXdef
  set T := MachineState.readWord st.memory loadAddr.toNat with hTdef
  have hs : (X * mu + (l2Step mem mu c0 n k).carry) + T =
      macSum X mu T (l2Step mem mu c0 n k).carry := by
    rw [Challenge.EvmProof.Word.word_add_comm]
    exact CiosCachedMacCore.sum_eq X mu T (l2Step mem mu c0 n k).carry
  have hcarry : CiosCachedMacCore.partialCarry X mu (l2Step mem mu c0 n k).carry +
      UInt256.lt ((X * mu + (l2Step mem mu c0 n k).carry) + T)
        (X * mu + (l2Step mem mu c0 n k).carry) =
      macCarry X mu T (l2Step mem mu c0 n k).carry := by
    rw [Challenge.EvmProof.Word.word_add_comm (CiosCachedMacCore.partialCarry X mu _)]
    simpa only [UInt256.gt, UInt256.lt,
      Challenge.EvmProof.Word.word_add_comm (X * mu + (l2Step mem mu c0 n k).carry) T] using
      CiosCachedMacCore.carry_eq X mu T (l2Step mem mu c0 n k).carry
  rw [CiosCachedMacCore.partialCarry] at hcarry
  rw [hcarry, hs] at hall
  simpa only [hXdef, hTdef, extraProgramZeroNarrow, st, TnCacheL2Trace.state, framed, extendedStack,
    l2Step, hx, hloadAddr, hstoreAddr, hpc, allOnes_value,
    List.cons_append, List.nil_append] using hall

#print axioms run_extraLoad
#print axioms run_extraStepZeroNarrow
#print axioms run_extraStep
end Challenge.Modexp.Submission.Proofs.Fast.TnCacheExtraTrace
