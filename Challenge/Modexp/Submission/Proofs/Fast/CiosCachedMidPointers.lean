...
private theorem cached_stride_add (x : UInt256) :
...
theorem run_words (s : State) (ml tl c0 mu bi pbi paEnd pbEnd flag dst ret : UInt256)
    ...
    runInstructions pointersProgram
      (framed s (UInt256.ofNat 4703) ([c0, mu] ++ baseStack bi pbi paEnd pbEnd flag dst ret rest)) =
  ...
  simp [pointersProgram, midProgram, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    baseStack, framed, hc10, hc11, hc12, hc13, h9408, h9440, hml, htl,
    hactML, hactTL, cached_stride_add, State.activeWordsAfterUInt256,
