...
theorem reduction_low (mem : ByteArray) (f : UInt256) :
    Csub.lowValue (reduction mem f) 2112 8 8 =
  ...
  rw [reduction, read_tail_top,
    readWord_l2Step_low _ _ _ 8 2080 7 (by decide)]
...
theorem reduction_equation (mem : ByteArray) (f : UInt256) (m : Nat)
    ...
    tValue (reduction mem f) 8 * Limbs.radix =
      (f.toNat*Limbs.radix+(MachineState.readWord mem 2080).toNat)*Limbs.radix^8 +
        Csub.lowValue mem 2112 8 8 + (rowMu mem 8).toNat*m := by
  let p := l2Step mem (rowMu mem 8) (rowC0 mem 8) 8 7
  ...
  have htn : (MachineState.readWord (reduction mem f) 2080).toNat = f.toNat+cv.toNat := by
    rw [reduction, read_tail_high, readWord_l2Step_low _ _ _ 8 2080 7 (by decide)]
  ...
  have hC : (rowC0 mem 8).toNat*Limbs.radix =
      (MachineState.readWord mem 2336).toNat +
        (MachineState.readWord mem 224).toNat*(rowMu mem 8).toNat :=
    c0_spec (MachineState.readWord mem 224) (MachineState.readWord mem 2816)
      (MachineState.readWord mem 2336) hinv
  have hD := l2_invariant mem (rowMu mem 8) (rowC0 mem 8) 8 (by decide) 7 (by decide)
  have hE : limbSum (fun k =>
          (MachineState.readWord mem (2112+32*(8-2-k))).toNat) 7*Limbs.radix +
        (MachineState.readWord mem 2336).toNat = Csub.lowValue mem 2112 8 8 := by
    have he := limbSum_shift (fun k =>
      (MachineState.readWord mem (2112+32*(8-1-k))).toNat) 7
    have haddr : (fun k => (MachineState.readWord mem (2112+32*(8-1-(k+1)))).toNat) =
        (fun k => (MachineState.readWord mem (2112+32*(8-2-k))).toNat) := by
      funext k
      congr 2
      omega
