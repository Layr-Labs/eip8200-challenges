...
opaque gasSteps_out (s : State) (mem : ByteArray) (pa pb n i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l1At 4244 s mem (rowBi mem pb n i) pa pb n i 0 pdst ret rest) :=
...
opaque gasSteps_mid (s : State) (mem : ByteArray) (c bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At 4579 s (midMem mem c) bi (rowMu mem n)
...
opaque gasSteps_tailLast (s : State) (mem : ByteArray) (c mu bi : UInt256)
  ...
  (exitBlock.steps (environment (CiosCachedTailDefs.exitState s (tailMem mem c)
    ...
    (CiosCachedExit.run_exit { s with memory := tailMem mem c }
      ...
      (UInt256.ofNat (pb-32)) (l1Target n) (l2Target n) (modulusValue (tailMem mem c) n) (inverseValue (tailMem mem c)) (tailPointerValue (tailMem mem c)) (low96Value (tailMem mem c)) (low64Value (tailMem mem c)) (low32Value (tailMem mem c)) pdst ret rest (by omega)
...
opaque gasSteps_l1Dispatch4 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4244 s mem bi pa pb 4 i j pdst ret rest)
      (l1At 4396 s mem bi pa pb 4 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4244 s mem bi pa pb 4 i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Dispatch8 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4244 s mem bi pa pb 8 i j pdst ret rest)
      (l1At 4247 s mem bi pa pb 8 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4244 s mem bi pa pb 8 i j pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l1Dispatch8 s mem bi pa pb i j pdst ret rest hcap
      (by rw [hcode]; exact jumpDestL1Eight)) |>.trans
  (l1Join8.steps (environment (l1At 4246 s mem bi pa pb 8 i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Join (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4396 s mem bi pa pb n i j pdst ret rest)
      (l1At 4397 s mem bi pa pb n i j pdst ret rest) :=
  l1Join.steps (environment (l1At 4396 s mem bi pa pb n i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Low64 (pc : Nat) (x tl ts : UInt256)
    (block : Block Artifact.submissionArtifact .Osaka pc (l2Cached64Program tl ts))
    (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    (pa pb n i k : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 999) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At (pc+34) s mem bi mu c0 pa pb n i (k+1) pdst ret rest) :=
  block.steps (environment (l2At pc s mem bi mu c0 pa pb n i k pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l2Low64 pc x tl ts s mem bi mu c0 pa pb n i k pdst ret rest hcap hact hn32 hk hx htl hts hselect)

opaque gasSteps_l2Low96 (pc : Nat) (x tl ts : UInt256)
    (block : Block Artifact.submissionArtifact .Osaka pc (l2Cached96Program tl ts))
    (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    (pa pb n i k : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 999) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At (pc+34) s mem bi mu c0 pa pb n i (k+1) pdst ret rest) :=
  block.steps (environment (l2At pc s mem bi mu c0 pa pb n i k pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l2Low96 pc x tl ts s mem bi mu c0 pa pb n i k pdst ret rest hcap hact hn32 hk hx htl hts hselect)
...
opaque gasSteps_l2Dispatch4 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4579 s mem bi mu c0 pa pb 4 i k pdst ret rest)
      (l2At 4731 s mem bi mu c0 pa pb 4 i k pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4579 s mem bi mu c0 pa pb 4 i k pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Dispatch8 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4579 s mem bi mu c0 pa pb 8 i k pdst ret rest)
      (l2At 4582 s mem bi mu c0 pa pb 8 i k pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4579 s mem bi mu c0 pa pb 8 i k pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l2Dispatch8 s mem bi mu c0 pa pb i k pdst ret rest hcap
      (by rw [hcode]; exact jumpDestL2Eight)) |>.trans
  (l2Join8.steps (environment (l2At 4581 s mem bi mu c0 pa pb 8 i k pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Join (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4731 s mem bi mu c0 pa pb n i k pdst ret rest)
      (l2At 4732 s mem bi mu c0 pa pb n i k pdst ret rest) :=
  l2Join.steps (environment (l2At 4731 s mem bi mu c0 pa pb n i k pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Final (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hn32 : n ≤ 32) (hn : 2 ≤ n) :
    Challenge.EvmProof.GasSteps
      (l2At 4800 s mem bi mu c0 pa pb n i (n-2) pdst ret rest)
      (tailState s (l2Step mem mu c0 n (n-1)).memory
        (l2Step mem mu c0 n (n-1)).carry mu bi pa pb n i pdst ret rest) := by
  have h := gasSteps_l2Mac 4800 0 0 8256 8288 l2Mac6 s mem bi mu c0 pa pb n i (n-2) pdst ret rest
  ...
  rw [hnn] at h
  have hpc : 4800 + ((0 : Fin 33).val + 35) = 4835 := by decide
  simpa only [l2At, tailState, modulusValue_l2 mem mu c0 n (n-1) hn32, inverseValue_l2 mem mu c0 n (n-1) hn32, tailPointerValue_l2 mem mu c0 n (n-1) hn32, low96Value_l2 mem mu c0 n (n-1) hn32, low64Value_l2 mem mu c0 n (n-1) hn32, low32Value_l2 mem mu c0 n (n-1) hn32, hpc] using h
