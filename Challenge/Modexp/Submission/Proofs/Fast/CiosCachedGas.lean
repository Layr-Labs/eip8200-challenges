...
opaque gasSteps_out (s : State) (mem : ByteArray) (pa pb n i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l1At 4227 s mem (rowBi mem pb n i) pa pb n i 0 pdst ret rest) :=
...
opaque gasSteps_mid (s : State) (mem : ByteArray) (c bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At 4568 s (midMem mem c) bi (rowMu mem n)
...
opaque gasSteps_tailLast (s : State) (mem : ByteArray) (c mu bi : UInt256)
  ...
  (exitBlock.steps (environment (CiosCachedTailDefs.exitState s (tailMem mem c)
    ...
    (CiosCachedExit.run_exit { s with memory := tailMem mem c }
      ...
      (UInt256.ofNat (pb-32)) (l1Target n) (l2Target n) (modulusValue (tailMem mem c) n) pdst ret rest (by omega)
...
opaque gasSteps_l1Last (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hpa : 32 ≤ pa) (hpaFit : pa+32*n ≤ 9472) :
    Challenge.EvmProof.GasSteps
      (l1At 4494 s mem bi pa pb n i (n-1) pdst ret rest)
      (midState s (l1Step mem bi pa n n).memory (l1Step mem bi pa n n).carry bi
        pa pb n i pdst ret rest) :=
  l1Mac7.steps (environment (l1At 4494 s mem bi pa pb n i (n-1) pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Dispatch4 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4227 s mem bi pa pb 4 i j pdst ret rest)
      (l1At 4379 s mem bi pa pb 4 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4227 s mem bi pa pb 4 i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Dispatch8 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4227 s mem bi pa pb 8 i j pdst ret rest)
      (l1At 4230 s mem bi pa pb 8 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4227 s mem bi pa pb 8 i j pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l1Dispatch8 s mem bi pa pb i j pdst ret rest hcap
      (by rw [hcode]; exact jumpDestL1Eight)) |>.trans
  (l1Join8.steps (environment (l1At 4229 s mem bi pa pb 8 i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Join (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4379 s mem bi pa pb n i j pdst ret rest)
      (l1At 4380 s mem bi pa pb n i j pdst ret rest) :=
  l1Join.steps (environment (l1At 4379 s mem bi pa pb n i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Dispatch4 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4568 s mem bi mu c0 pa pb 4 i k pdst ret rest)
      (l2At 4720 s mem bi mu c0 pa pb 4 i k pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4568 s mem bi mu c0 pa pb 4 i k pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Dispatch8 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At 4571 s mem bi mu c0 pa pb 8 i k pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4568 s mem bi mu c0 pa pb 8 i k pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l2Dispatch8 s mem bi mu c0 pa pb i k pdst ret rest hcap
      (by rw [hcode]; exact jumpDestL2Eight)) |>.trans
  (l2Join8.steps (environment (l2At 4570 s mem bi mu c0 pa pb 8 i k pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Final (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
  ...
  rw [hnn] at h
  have hpc : 4793 + ((0 : Fin 33).val + 35) = 4828 := by decide
  simpa only [l2At, tailState, modulusValue_l2 mem mu c0 n (n-1) hn32, hpc] using h
