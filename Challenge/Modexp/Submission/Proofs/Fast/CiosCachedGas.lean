...
opaque gasSteps_out (s : State) (mem : ByteArray) (pa pb n i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (firstAt 4250 s mem (rowBi mem pb n i) pa pb n i pdst ret rest) :=
...
opaque gasSteps_commonFirst (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (firstAt 4250 s mem bi pa pb n i inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
      (l1At 4276 s mem bi pa pb n i 1 inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) :=
...
opaque gasSteps_l1Dispatch4 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4276 s mem bi pa pb 4 i j pdst ret rest)
      (l1At 4431 s mem bi pa pb 4 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4276 s mem bi pa pb 4 i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Dispatch8 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4276 s mem bi pa pb 8 i j pdst ret rest)
      (l1At 4279 s mem bi pa pb 8 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4276 s mem bi pa pb 8 i j pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l1Dispatch8 s mem bi pa pb i j pdst ret rest hcap
      (by rw [hcode]; exact jumpDestL1Eight)) |>.trans
  (l1Join8.steps (environment (l1At 4278 s mem bi pa pb 8 i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Join (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4431 s mem bi pa pb n i j pdst ret rest)
      (l1At 4432 s mem bi pa pb n i j pdst ret rest) :=
  l1Join.steps (environment (l1At 4431 s mem bi pa pb n i j pdst ret rest) hcode hfork hrun hnp) rfl
