...
opaque gasSteps_out (s : State) (mem : ByteArray) (pa pb n i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l1At 4604 s mem (rowBi mem pb n i) pa pb n i 0 pdst ret rest) :=
...
opaque gasSteps_mid (s : State) (mem : ByteArray) (paj ptj c bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At 4975 s (midMem mem c) bi (rowMu mem n)
...
opaque gasSteps_l1Mac (pc : Nat) {c : Nat}
    (block : Block Artifact.submissionArtifact .Osaka pc (l1Program c))
    ...
    (hn32 : n ≤ 32) (hj : j < n)
    (hpa : 32 ≤ pa) (hpaFit : pa+32*n ≤ 9472)
    (hc : UInt256.ofNat c = UInt256.ofNat (ptrAt (8224 + 32*n) j - 32)) :
    Challenge.EvmProof.GasSteps
...
opaque gasSteps_l1Dispatch4 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4604 s mem bi pa pb 4 i j pdst ret rest)
      (l1At 4758 s mem bi pa pb 4 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4604 s mem bi pa pb 4 i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Dispatch8 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4604 s mem bi pa pb 8 i j pdst ret rest)
      (l1At 4610 s mem bi pa pb 8 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4604 s mem bi pa pb 8 i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Join (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4758 s mem bi pa pb n i j pdst ret rest)
      (l1At 4759 s mem bi pa pb n i j pdst ret rest) :=
  l1Join.steps (environment (l1At 4758 s mem bi pa pb n i j pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l1Join s mem bi pa pb n i j pdst ret rest hcap)

opaque gasSteps_l2Mac (pc : Nat) {c : Nat}
    (block : Block Artifact.submissionArtifact .Osaka pc (l2Program c))
    ...
    (hact : 296 ≤ s.activeWords.toNat)
    (hn32 : n ≤ 32) (hk : k+1 < n)
    (hc : UInt256.ofNat c = UInt256.ofNat (ptrAt (8192 + 32*n) k - 32)) :
    Challenge.EvmProof.GasSteps
  ...
  block.steps (environment (l2At pc s mem bi mu c0 pa pb n i k pdst ret rest) hcode hfork hrun hnp) rfl
    (by simpa only [l2Program, hc] using
      (run_l2Mac pc s mem bi mu c0 pa pb n i k pdst ret rest hcap hact hn32 hk))

opaque gasSteps_l2Last (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    (pa pb n i k : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1006) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hfork : s.fork = .Osaka)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (tailState s (l2Step mem mu c0 n (k+1)).memory
        (UInt256.ofNat (ptrAt (32*n - 64) k))
        (UInt256.ofNat (ptrAt (8192 + 32*n) k))
        (l2Step mem mu c0 n (k+1)).carry mu bi pa pb n i pdst ret rest) :=
  l2Mac6.steps (environment (l2At 5222 s mem bi mu c0 pa pb n i k pdst ret rest)
    hcode hfork hrun hnp) rfl (by
      have h := CiosCachedL2LastRun.run_step s (UInt256.ofNat 5222) mem bi mu c0 n k
        (UInt256.ofNat (ptrAt (pb+32*n-32) i)) (UInt256.ofNat (pa+32*n-32))
        (UInt256.ofNat (pb-32)) (isFour n) pdst ret rest hcap hact hn32 hk
      simpa only [CiosCachedL2LastRun.program_matches, CiosCachedL2.state,
        CiosCachedL2LastRun.outputState, l2At, tailState,
...
opaque gasSteps_l2Dispatch4 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4975 s mem bi mu c0 pa pb 4 i k pdst ret rest)
      (l2At 5141 s mem bi mu c0 pa pb 4 i k pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4975 s mem bi mu c0 pa pb 4 i k pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Dispatch8 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4975 s mem bi mu c0 pa pb 8 i k pdst ret rest)
      (l2At 4981 s mem bi mu c0 pa pb 8 i k pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4975 s mem bi mu c0 pa pb 8 i k pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Join (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 5141 s mem bi mu c0 pa pb n i k pdst ret rest)
      (l2At 5142 s mem bi mu c0 pa pb n i k pdst ret rest) :=
  l2Join.steps (environment (l2At 5141 s mem bi mu c0 pa pb n i k pdst ret rest) hcode hfork hrun hnp) rfl
