...
opaque gasSteps_out (s : State) (mem : ByteArray) (pa pb n i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l1At 4569 s mem (rowBi mem pb n i) pa pb n i 0 pdst ret rest) :=
...
opaque gasSteps_mid (s : State) (mem : ByteArray) (c bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At 4924 s (midMem mem c) bi (rowMu mem n)
...
opaque gasSteps_tailNext (s : State) (mem : ByteArray) (c mu bi : UInt256)
  ...
  tailLoop.steps (environment (tailState s mem c mu bi pa pb n i pdst ret rest) hcode hfork hrun hnp) rfl
    (CiosCachedTailRows.run_next s mem c mu bi pa pb n i pdst ret rest
      hcap hact hpb hpbFit hi (by rw [hcode]; exact jumpDest4564))
...
opaque gasSteps_tailLast (s : State) (mem : ByteArray) (c mu bi : UInt256)
  ...
  (tailLoop.steps (environment (tailState s mem c mu bi pa pb n i pdst ret rest) hcode hfork hrun hnp) rfl
    (CiosCachedTailRows.run_last s mem c mu bi pa pb n i pdst ret rest
      hcap hact hpb hpbFit hi (by rw [hcode]; exact jumpDest4564))).trans
...
opaque gasSteps_l1Last (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hpa : 32 ≤ pa) (hpaFit : pa+32*n ≤ 9472) :
    Challenge.EvmProof.GasSteps
      (l1At 4841 s mem bi pa pb n i (n-1) pdst ret rest)
      (midState s (l1Step mem bi pa n n).memory (l1Step mem bi pa n n).carry bi
        pa pb n i pdst ret rest) :=
  l1Mac7.steps (environment (l1At 4841 s mem bi pa pb n i (n-1) pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Dispatch4 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4569 s mem bi pa pb 4 i j pdst ret rest)
      (l1At 4726 s mem bi pa pb 4 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4569 s mem bi pa pb 4 i j pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l1Dispatch4 s mem bi pa pb i j pdst ret rest hcap
      (by rw [hcode]; exact jumpDest4726))
...
opaque gasSteps_l1Dispatch8 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4569 s mem bi pa pb 8 i j pdst ret rest)
      (l1At 4574 s mem bi pa pb 8 i j pdst ret rest) :=
  l1Dispatch.steps (environment (l1At 4569 s mem bi pa pb 8 i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Join (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1At 4726 s mem bi pa pb n i j pdst ret rest)
      (l1At 4727 s mem bi pa pb n i j pdst ret rest) :=
  l1Join.steps (environment (l1At 4726 s mem bi pa pb n i j pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Dispatch4 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4924 s mem bi mu c0 pa pb 4 i k pdst ret rest)
      (l2At 5081 s mem bi mu c0 pa pb 4 i k pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4924 s mem bi mu c0 pa pb 4 i k pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l2Dispatch4 s mem bi mu c0 pa pb i k pdst ret rest hcap
      (by rw [hcode]; exact jumpDest5081))
...
opaque gasSteps_l2Dispatch8 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4924 s mem bi mu c0 pa pb 8 i k pdst ret rest)
      (l2At 4929 s mem bi mu c0 pa pb 8 i k pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4924 s mem bi mu c0 pa pb 8 i k pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Join (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 5081 s mem bi mu c0 pa pb n i k pdst ret rest)
      (l2At 5082 s mem bi mu c0 pa pb n i k pdst ret rest) :=
  l2Join.steps (environment (l2At 5081 s mem bi mu c0 pa pb n i k pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Final (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hn32 : n ≤ 32) (hn : 2 ≤ n) :
    Challenge.EvmProof.GasSteps
      (l2At 5158 s mem bi mu c0 pa pb n i (n-2) pdst ret rest)
      (tailState s (l2Step mem mu c0 n (n-1)).memory
        (l2Step mem mu c0 n (n-1)).carry mu bi pa pb n i pdst ret rest) := by
  have h := gasSteps_l2Last 5158 l2Mac6 s mem bi mu c0 pa pb n i (n-2) pdst ret rest
