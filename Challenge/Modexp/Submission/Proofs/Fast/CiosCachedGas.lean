...
opaque gasSteps_l2Mac (pc p : Nat)
    (block : Block Artifact.submissionArtifact .Osaka pc (l2ConstProgram p))
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At (pc+40) s mem bi mu c0 pa pb n i (k+1) pdst ret rest) := by
  subst p
  exact block.steps
