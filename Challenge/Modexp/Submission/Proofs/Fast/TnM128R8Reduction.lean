...
namespace Challenge.Modexp.Submission.Proofs.Fast.TnM128R8Reduction
...
noncomputable def reduction_steps (s : State)
    ...
    GasSteps
      (framed {s with memory := q.memory} (UInt256.ofNat 3647)
  ...
  have hmid := TnCacheRowTrace.run_middle 3647 st q.carry bi pbi hd pb ent tn
  ...
  have g0 := TnM128CandidateBlocks.middle.steps
    (s := framed st (UInt256.ofNat 3647)
  ...
  have hpc : (TnCacheLastTrace.lastState st
      ...
      UInt256.ofNat 3912 := by
    change UInt256.ofNat (l2PC+TnMod128Chain.fullSize 8) = UInt256.ofNat 3912
    rfl
...
end Challenge.Modexp.Submission.Proofs.Fast.TnM128R8Reduction
