...
Block 0 carries the zero-carry schedule (`headZeroProgram`, instructions 1992–1993 /
pc 2699–2700 are `JUMPDEST JUMPDEST`), not the general `headProgram`: it is the row head of
every chain that reaches it, so its incoming carry is the literal zero `entryProgram` pushes
at pc 2676, and `lo + 0 = lo` makes the `PUSH0 ADD` a two-byte no-op.  `block4` keeps
`headProgram` — pc 2827 is also *fallen into* from block 3, whose outgoing carry is not zero. -/
