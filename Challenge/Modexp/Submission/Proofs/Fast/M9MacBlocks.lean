...
Block 0 carries the folded schedule (`headZeroProgram`, the `JUMPDEST JUMPDEST` pair at
instructions 1992-1993 / pc 2699-2700), not the general `headProgram`: it is the row head of
every chain that reaches it, so its incoming carry is the literal zero `entryProgram` pushes
at pc 2676, and `x + 0` needs no instructions.  `block4` keeps `headProgram` — pc 2827 is
