...
def gasSteps_ebChainFromHead (s : State) {n bsize mm minv R : Nat}
    ...
    (hinv : Exp.EbInv (Exp.mcopyMem mem 256 1024 (32 * n)) n mm bM
    ...
    Challenge.EvmProof.GasSteps
      ...
      (Exp.returnedState s
        (sub.mpMem 256 768 256
          (Exp.storeWord
            (Exp.ebMems sub.mpMem input bsize n
              (Exp.mcopyMem mem 256 1024 (32 * n)) esize)
            (736 + 32 * n) (UInt256.ofNat 1)))
        n bsize esize msize) := by
  have hframe0 : Exp.Frame (Exp.mcopyMem mem 256 1024 (32 * n))
      n bsize minv := Exp.frame_mcopyMem (by omega) hframe
  have hloop := Exp.ebMems_inv spec hm hcop hn hn32 bM hbM input bsize
    (Exp.mcopyMem mem 256 1024 (32 * n)) hframe0.minvW hinv esize
  have hframeE : Exp.Frame
      (Exp.ebMems sub.mpMem input bsize n
        (Exp.mcopyMem mem 256 1024 (32 * n)) esize) n bsize minv :=
    Exp.ebMems_frame sub input (Exp.mcopyMem mem 256 1024 (32 * n))
      hn32 hframe0 esize
  have hframeS : Exp.Frame
      (Exp.storeWord
        (Exp.ebMems sub.mpMem input bsize n
          (Exp.mcopyMem mem 256 1024 (32 * n)) esize)
        (736 + 32 * n) (UInt256.ofNat 1)) n bsize minv :=
    Exp.frame_storeWord (UInt256.ofNat 1) (by omega) hframeE
  have hone : Model.FastRepresents
      (Exp.storeWord
        (Exp.ebMems sub.mpMem input bsize n
          (Exp.mcopyMem mem 256 1024 (32 * n)) esize)
        (736 + 32 * n) (UInt256.ofNat 1)) 768 n 1 := by
  ...
  have hmodS := Exp.storeWord_frame
    (Exp.ebMems sub.mpMem input bsize n
      (Exp.mcopyMem mem 256 1024 (32 * n)) esize)
    (736 + 32 * n) 0 n mm (UInt256.ofNat 1) (Or.inr (by omega))
    hloop.modulus
  ...
  exact (((Exp.gasSteps_ebLoop s sub spec
      (Exp.mcopyMem mem 256 1024 (32 * n)) input esize msize bM
    ...
    (sub.monpro 256 768 256 (UInt256.ofNat 1721)
...
theorem handled_of_ebHead (input : ByteArray) (s : State) (mem : ByteArray)
  ...
  have hframeC : Exp.Frame (Exp.mcopyMem mem 256 1024 (32 * n))
      n bsize minv := Exp.frame_mcopyMem (by omega) hframe
  ...
  have hfinal := Exp.ebMem_final spec hmpos hn hn32 hcop hradix hbMlt hbMform
    input bsize esize rfl (Exp.mcopyMem mem 256 1024 (32 * n))
    hframeC.minvW hEb
