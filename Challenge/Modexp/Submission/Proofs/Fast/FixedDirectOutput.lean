...
in memory word `0x2440 = 2720` (the loop head's `MSTORE`, read by the kernel's
own square loop) and then calls the kernel. -/
def fixedDirectMems
    (sq : ByteArray → ByteArray) : ByteArray → Nat → ByteArray
  | mem, 0 => mem
  | mem, k + 1 =>
      fixedDirectMems sq (sq (Exp.storeWord mem 2720 (UInt256.ofNat (k + 1)))) k
...
theorem countStore_frame {mem : ByteArray} {n bsize minv : Nat} (c : Nat)
    ...
    Exp.Frame (Exp.storeWord mem 2720 (UInt256.ofNat c)) n bsize minv :=
...
theorem countStore_inv {mem : ByteArray} {n mm rawBase squareBase : Nat} (c : Nat)
    ...
    Inv (Exp.storeWord mem 2720 (UInt256.ofNat c)) n mm rawBase squareBase := by
  obtain ⟨one, honeLt, honeRep⟩ := hinv.oneBlock
  exact ⟨Exp.storeWord_frame mem 2720 0 n mm _ (Or.inr (by omega)) hinv.modulus,
    Exp.storeWord_frame mem 2720 1024 n rawBase _ (Or.inr (by omega)) hinv.rawAcc,
    Exp.storeWord_frame mem 2720 2048 n squareBase _ (Or.inr (by omega))
      hinv.squareBase,
    ⟨one, honeLt,
      Exp.storeWord_frame mem 2720 3072 n one _ (Or.inr (by omega)) honeRep⟩⟩
...
theorem fixedDirectMems_inv {s : State} {n bsize mm minv : Nat}
  ...
  | succ t ih =>
      ...
      have hstep : Inv (sub.sqMem (Exp.storeWord mem 2720 (UInt256.ofNat (t + 1))))
