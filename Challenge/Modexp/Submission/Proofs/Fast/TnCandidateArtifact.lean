import Challenge.EvmProof.Program
import Challenge.EvmProof.Stepper
import Challenge.EvmProof.Word
import Challenge.Modexp.Submission.Proofs.Fast.TnCandidateBytes

attribute [simp]
  Challenge.EvmProof.Word.word_toNat_add
  Challenge.EvmProof.Word.word_toNat_sub
  Challenge.EvmProof.Word.word_toNat_lt
  Challenge.EvmProof.Word.word_toNat_isZero
  Challenge.EvmProof.Word.word_toNat_lor
  Challenge.EvmProof.Word.word_toNat_land

set_option warningAsError true
set_option maxRecDepth 20000
set_option maxHeartbeats 2000000

namespace Challenge.Modexp.Submission.Proofs.Fast.TnCandidateArtifact

open EvmSemantics
open EvmSemantics.EVM
open YulEvmCompiler

def submissionInstructions : List Instr :=
[.push 0 0,
 .op .CALLDATALOAD,
 .push 1 32,
 .op .CALLDATALOAD,
 .push 1 64,
 .op .CALLDATALOAD,
 .push 1 32,
 .op (.Dup { idx := 3 }),
 .op .GT,
 .op (.Dup { idx := 2 }),
 .push 1 32,
 .op .XOR,
 .op .OR,
 .op (.Dup { idx := 1 }),
 .push 1 32,
 .op .XOR,
 .op .OR,
 .push 1 126,
 .op .JUMPI,
 .op .JUMPDEST,
 .op (.Dup { idx := 2 }),
 .push 1 96,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op .CALLDATALOAD,
 .push 1 32,
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op .CALLDATALOAD,
 .op (.Dup { idx := 1 }),
 .op .NOT,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .push 2 1775,
 .op .JUMPI,
 .op (.Dup { idx := 0 }),
 .push 32 21888242871839275222246405745257275088696311157297823662689037894645226208583,
 .op .EQ,
 .op (.Dup { idx := 1 }),
 .push 5 4294968272,
 .op .NOT,
 .op .EQ,
 .op .OR,
 .op .ISZERO,
 .push 2 1775,
 .op .JUMPI,
 .push 1 96,
 .op .CALLDATALOAD,
 .op (.Dup { idx := 6 }),
 .push 1 32,
 .op .SUB,
 .push 1 3,
 .op .SHL,
 .op .SHR,
 .op .MOD,
 .push 0 0,
 .op .LT,
 .push 0 0,
 .op .MSTORE,
 .push 1 32,
 .push 0 0,
 .op .RETURN,
 .op .JUMPDEST,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .push 1 25,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op .POP,
 .op .POP,
 .push 2 709,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .CALLDATALOAD,
 .push 0 0,
 .op .BYTE,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .op .POP,
 .op (.Swap { idx := 0 }),
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 5 }),
 .op .CALLDATALOAD,
 .op (.Dup { idx := 3 }),
 .push 1 32,
 .op .SUB,
 .push 1 3,
 .op .SHL,
 .op .SHR,
 .op (.Dup { idx := 0 }),
 .push 1 162,
 .op .JUMPI,
 .op (.Dup { idx := 3 }),
 .push 0 0,
 .op .RETURN,
 .op .JUMPDEST,
 .push 0 0,
 .push 0 0,
 .op .JUMPDEST,
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .ISZERO,
 .push 1 199,
 .op .JUMPI,
 .op (.Dup { idx := 2 }),
 .push 1 183,
 .push 0 0,
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 10 }),
 .op .ADD,
 .push 1 134,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 4 }),
 .push 2 256,
 .op (.Dup { idx := 5 }),
 .op .MULMOD,
 .op .ADDMOD,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .push 1 1,
 .op .ADD,
 .push 1 165,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op (.Dup { idx := 1 }),
 .push 1 1,
 .op .LT,
 .push 0 0,
 .op .JUMPDEST,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 1 }),
 .op .EQ,
 .push 1 235,
 .op .JUMPI,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 9 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op .CALLDATALOAD,
 .push 0 0,
 .op .BYTE,
 .push 0 0,
 .push 2 2500,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op .POP,
 .op .POP,
 .push 1 1,
 .op .ADD,
 .push 1 206,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 6 }),
 .push 1 32,
 .op .SUB,
 .push 1 3,
 .op .SHL,
 .op .SHL,
 .push 0 0,
 .op .MSTORE,
 .op (.Dup { idx := 5 }),
 .push 0 0,
 .op .RETURN,
 .op .JUMPDEST,
 .push 2 9248,
 .op .CALLDATASIZE,
 .push 0 0,
 .op .CALLDATACOPY,
 .push 1 64,
 .op .CALLDATALOAD,
 .op (.Dup { idx := 0 }),
 .push 2 9216,
 .op .MSTORE,
 .push 1 32,
 .op .CALLDATALOAD,
 .push 0 0,
 .op .CALLDATALOAD,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 1 96,
 .op .ADD,
 .push 2 1024,
 .op .CALLDATACOPY,
 .op (.Dup { idx := 0 }),
 .push 1 96,
 .push 2 5120,
 .op .CALLDATACOPY,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .push 1 96,
 .op .ADD,
 .push 2 6144,
 .op .CALLDATACOPY,
 .push 0 0,
 .op (.Dup { idx := 3 }),
 .op .JUMPDEST,
 .push 0 0,
 .op .NOT,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 1024,
 .op .ADD,
 .op .MLOAD,
 .push 0 0,
 .op .BYTE,
 .op (.Swap { idx := 0 }),
 .op (.Swap { idx := 1 }),
 .op .OR,
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .push 2 300,
 .op .JUMPI,
 .op .POP,
 .push 2 331,
 .op .JUMPI,
 .op (.Dup { idx := 2 }),
 .push 2 8192,
 .op .RETURN,
 .op .JUMPDEST,
 .push 1 1,
 .op (.Dup { idx := 3 }),
 .push 2 7167,
 .op .ADD,
 .op .MSTORE8,
 .op (.Dup { idx := 0 }),
 .push 2 5120,
 .push 2 7168,
 .push 2 354,
 .push 2 476,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .push 2 9216,
 .op .MLOAD,
 .push 0 0,
 .push 2 2048,
 .op .MCOPY,
 .op (.Dup { idx := 1 }),
 .push 2 7168,
 .push 2 3072,
 .op .MCOPY,
 .push 0 0,
 .op .JUMPDEST,
 .op (.Dup { idx := 1 }),
 .push 1 3,
 .op .SHL,
 .op (.Dup { idx := 1 }),
 .op .EQ,
 .push 2 455,
 .op .JUMPI,
 .op (.Dup { idx := 2 }),
 .push 2 3072,
 .op (.Dup { idx := 0 }),
 .push 2 397,
 .push 2 476,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 2 }),
 .push 0 0,
 .push 2 3072,
 .op .MCOPY,
 .op (.Dup { idx := 0 }),
 .push 1 3,
 .op .SHR,
 .push 2 6144,
 .op .ADD,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .push 1 7,
 .op .AND,
 .op .SHL,
 .push 1 255,
 .op .SHR,
 .op .ISZERO,
 .push 2 447,
 .op .JUMPI,
 .op (.Dup { idx := 2 }),
 .push 2 2048,
 .push 2 3072,
 .push 2 440,
 .push 2 476,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 2 }),
 .push 0 0,
 .push 2 3072,
 .op .MCOPY,
 .op .JUMPDEST,
 .push 1 1,
 .op .ADD,
 .push 2 374,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 2 }),
 .push 2 3072,
 .push 0 0,
 .op .MCOPY,
 .push 2 8192,
 .push 2 472,
 .push 2 548,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 2 }),
 .push 0 0,
 .op .RETURN,
 .op .JUMPDEST,
 .push 2 9216,
 .op .MLOAD,
 .op .CALLDATASIZE,
 .push 0 0,
 .op .CALLDATACOPY,
 .push 0 0,
 .op .JUMPDEST,
 .op (.Dup { idx := 4 }),
 .push 1 3,
 .op .SHL,
 .op (.Dup { idx := 1 }),
 .op .EQ,
 .push 2 541,
 .op .JUMPI,
 .push 0 0,
 .push 2 504,
 .push 2 548,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .push 1 3,
 .op .SHR,
 .op (.Dup { idx := 4 }),
 .op .ADD,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .push 1 7,
 .op .AND,
 .op .SHL,
 .push 1 255,
 .op .SHR,
 .op .ISZERO,
 .push 2 533,
 .op .JUMPI,
 .op (.Dup { idx := 2 }),
 .push 2 533,
 .push 2 548,
 .op .JUMP,
 .op .JUMPDEST,
 .push 1 1,
 .op .ADD,
 .push 2 485,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op (.Swap { idx := 2 }),
 .op .POP,
 .op .POP,
 .op .POP,
 .op .JUMP,
 .op .JUMPDEST,
 .push 0 0,
 .push 2 9216,
 .op .MLOAD,
 .op .JUMPDEST,
 .push 0 0,
 .op .NOT,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 4 }),
 .op .ADD,
 .op .MLOAD,
 .push 0 0,
 .op .BYTE,
 .op (.Dup { idx := 1 }),
 .op .MLOAD,
 .push 0 0,
 .op .BYTE,
 .op .ADD,
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MSTORE8,
 .push 1 8,
 .op .SHR,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .op (.Dup { idx := 0 }),
 .push 2 554,
 .op .JUMPI,
 .op .POP,
 .push 0 0,
 .push 2 9216,
 .op .MLOAD,
 .op .JUMPDEST,
 .push 0 0,
 .op .NOT,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 1024,
 .op .ADD,
 .op .MLOAD,
 .push 0 0,
 .op .BYTE,
 .op (.Dup { idx := 1 }),
 .op .MLOAD,
 .push 0 0,
 .op .BYTE,
 .op .SUB,
 .op (.Dup { idx := 2 }),
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .push 2 4096,
 .op .ADD,
 .op .MSTORE8,
 .push 1 255,
 .op .SHR,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .op (.Dup { idx := 0 }),
 .push 2 590,
 .op .JUMPI,
 .op .POP,
 .op .ISZERO,
 .op .OR,
 .op .ISZERO,
 .push 2 644,
 .op .JUMPI,
 .push 2 9216,
 .op .MLOAD,
 .push 2 4096,
 .push 0 0,
 .op .MCOPY,
 .op .JUMPDEST,
 .op (.Swap { idx := 0 }),
 .op .POP,
 .op .JUMP,
 .op .JUMPDEST,
 .push 0 0,
 .op .CALLDATALOAD,
 .push 1 32,
 .op .CALLDATALOAD,
 .push 1 64,
 .op .CALLDATALOAD,
 .op (.Dup { idx := 0 }),
 .push 2 665,
 .op .JUMPI,
 .push 0 0,
 .push 0 0,
 .op .RETURN,
 .op .JUMPDEST,
 .op (.Dup { idx := 2 }),
 .push 1 96,
 .op .ADD,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .push 1 32,
 .op (.Dup { idx := 3 }),
 .op .GT,
 .push 2 695,
 .op .JUMPI,
 .push 2 1186,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .push 1 96,
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 10 }),
 .push 2 1752,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 1190,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .push 1 96,
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 10 }),
 .push 1 251,
 .op .JUMP,
 .op .JUMPDEST,
 .push 1 64,
 .op .CALLDATALOAD,
 .push 1 33,
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .push 1 223,
 .op .LT,
 .push 2 1158,
 .op .JUMPI,
 .push 1 32,
 .op .CALLDATALOAD,
 .push 0 0,
 .op .CALLDATALOAD,
 .push 1 31,
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 1 5,
 .op .SHR,
 .op (.Dup { idx := 0 }),
 .push 1 5,
 .op .SHL,
 .push 1 96,
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 4 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op .CALLDATALOAD,
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 3 }),
 .op .SUB,
 .push 1 3,
 .op .SHL,
 .op .SHR,
 .op .ISZERO,
 .push 2 1164,
 .op .JUMPI,
 .push 1 32,
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op .SUB,
 .op .CALLDATALOAD,
 .push 1 1,
 .op .AND,
 .op .ISZERO,
 .push 2 1164,
 .op .JUMPI,
 .op (.Dup { idx := 1 }),
 .push 2 2688,
 .op .MSTORE,
 .op (.Dup { idx := 3 }),
 .push 1 96,
 .op .ADD,
 .push 2 2816,
 .op .MSTORE,
 .push 1 32,
 .op (.Dup { idx := 2 }),
 .op .SUB,
 .op (.Dup { idx := 0 }),
 .push 2 2752,
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .push 2 2080,
 .op .ADD,
 .push 2 2784,
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .op .CALLDATASIZE,
 .push 0 0,
 .op .CALLDATACOPY,
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 5 }),
 .op .SUB,
 .op .CALLDATACOPY,
 .op (.Swap { idx := 0 }),
 .op .POP,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .push 1 3,
 .op .MUL,
 .push 1 2,
 .op .XOR,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .push 0 0,
 .op .SUB,
 .push 2 2720,
 .op .MSTORE,
 .op .POP,
 .op .POP,
 .push 2 2788,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2688,
 .op .MLOAD,
 .push 2 1024,
 .push 2 1280,
 .op .MCOPY,
 .push 2 2394,
 .push 2 1280,
 .push 2 2747,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 918,
 .push 2 1536,
 .op (.Dup { idx := 0 }),
 .push 2 1536,
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .SHR,
 .push 1 1,
 .op .AND,
 .push 1 8,
 .op .SHL,
 .push 2 1024,
 .op .ADD,
 .push 2 1728,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op (.Dup { idx := 0 }),
 .op .ISZERO,
 .push 2 951,
 .op .JUMPI,
 .push 0 0,
 .op .NOT,
 .op .ADD,
 .push 2 903,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op (.Dup { idx := 2 }),
 .op .ISZERO,
 .push 2 2631,
 .op .JUMPI,
 .push 2 2429,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .EQ,
 .push 2 1030,
 .op .JUMPI,
 .push 2 987,
 .push 2 256,
 .push 2 1280,
 .push 2 256,
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .SUB,
 .push 1 5,
 .op .SHL,
 .op (.Dup { idx := 5 }),
 .op .SUB,
 .push 1 96,
 .op .ADD,
 .op .CALLDATALOAD,
 .op (.Dup { idx := 3 }),
 .push 2 736,
 .op .ADD,
 .op .MSTORE,
 .push 2 1022,
 .push 2 256,
 .push 2 768,
 .push 2 256,
 .push 2 1475,
 .op .JUMP,
 .op .JUMPDEST,
 .push 1 1,
 .op .ADD,
 .push 2 963,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op .POP,
 .push 2 2631,
 .push 2 512,
 .push 2 1536,
 .push 2 256,
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 1 }),
 .op .EQ,
 .push 2 1120,
 .op .JUMPI,
 .push 2 1679,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 1076,
 .push 2 256,
 .op (.Dup { idx := 0 }),
 .push 2 256,
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .AND,
 .op .ISZERO,
 .push 2 1102,
 .op .JUMPI,
 .push 2 1101,
 .push 2 256,
 .push 2 512,
 .push 2 256,
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .op .JUMPDEST,
 .push 1 1,
 .op .SHR,
 .op (.Dup { idx := 0 }),
 .push 2 1061,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .push 1 1,
 .op .ADD,
 .push 2 1049,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .push 1 1,
 .op (.Dup { idx := 1 }),
 .push 2 736,
 .op .ADD,
 .op .MSTORE,
 .push 2 1148,
 .push 2 256,
 .push 2 768,
 .push 2 256,
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op .JUMPDEST,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .push 2 256,
 .op .ADD,
 .op .SUB,
 .op .RETURN,
 .op .JUMPDEST,
 .op .POP,
 .push 2 648,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .push 2 648,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 256,
 .op .JUMPDEST,
 .push 2 1190,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .push 2 1475,
 .op .JUMP,
 .op .JUMPDEST,
 .push 1 1,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op (.Dup { idx := 0 }),
 .push 2 1179,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2688,
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .push 1 64,
 .op .ADD,
 .op .CALLDATASIZE,
 .push 2 2048,
 .op .CALLDATACOPY,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 1 32,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .push 1 32,
 .op (.Dup { idx := 4 }),
 .op .SUB,
 .op (.Swap { idx := 3 }),
 .op .POP,
 .op (.Swap { idx := 0 }),
 .op .POP,
 .push 1 32,
 .op (.Dup { idx := 2 }),
 .op .SUB,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .push 0 0,
 .push 2 2784,
 .op .MLOAD,
 .op (.Dup { idx := 4 }),
 .push 2 2688,
 .op .MLOAD,
 .op .ADD,
 .op .JUMPDEST,
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 1 }),
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .push 0 0,
 .op .NOT,
 .op (.Swap { idx := 1 }),
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op (.Dup { idx := 3 }),
 .op .MLOAD,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 2 }),
 .op .LT,
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 4 }),
 .op .ADD,
 .op (.Swap { idx := 3 }),
 .op (.Dup { idx := 4 }),
 .op .LT,
 .op .ADD,
 .op (.Swap { idx := 2 }),
 .op (.Dup { idx := 2 }),
 .op .MSTORE,
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .push 2 1252,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .op (.Dup { idx := 0 }),
 .push 2 2080,
 .op .MLOAD,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2080,
 .op .MSTORE,
 .op .LT,
 .push 2 2048,
 .op .MSTORE,
 .push 2 2784,
 .op .MLOAD,
 .op .MLOAD,
 .push 2 2720,
 .op .MLOAD,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .push 2 2752,
 .op .MLOAD,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .push 0 0,
 .op .NOT,
 .op (.Swap { idx := 1 }),
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op (.Swap { idx := 0 }),
 .push 0 0,
 .op .LT,
 .op .ADD,
 .push 2 2784,
 .op .MLOAD,
 .push 1 32,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .push 2 2752,
 .op .MLOAD,
 .push 1 32,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .push 0 0,
 .op .NOT,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 6 }),
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 5 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 3 }),
 .op .MLOAD,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 4 }),
 .op .GT,
 .op .ADD,
 .op (.Swap { idx := 2 }),
 .push 1 32,
 .op (.Dup { idx := 3 }),
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 3 }),
 .op .ADD,
 .op .MSTORE,
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .push 2 2080,
 .op (.Dup { idx := 2 }),
 .op .GT,
 .push 2 1376,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .op .POP,
 .op (.Dup { idx := 0 }),
 .push 2 2080,
 .op .MLOAD,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2112,
 .op .MSTORE,
 .op .LT,
 .push 2 2048,
 .op .MLOAD,
 .op .ADD,
 .push 2 2080,
 .op .MSTORE,
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .push 2 1238,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .op .POP,
 .push 2 4320,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2688,
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .push 1 32,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .ADD,
 .push 1 32,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op (.Swap { idx := 2 }),
 .op .POP,
 .op (.Swap { idx := 2 }),
 .op .POP,
 .op .POP,
 .push 2 2784,
 .op .MLOAD,
 .push 0 0,
 .op (.Swap { idx := 2 }),
 .op (.Swap { idx := 0 }),
 .op .JUMPDEST,
 .op (.Dup { idx := 1 }),
 .op .MLOAD,
 .op (.Dup { idx := 3 }),
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 5 }),
 .op .ADD,
 .op (.Swap { idx := 4 }),
 .op (.Dup { idx := 5 }),
 .op .LT,
 .op .OR,
 .op (.Swap { idx := 3 }),
 .op (.Dup { idx := 1 }),
 .op .MSTORE,
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .op (.Swap { idx := 1 }),
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 1 }),
 .push 2 2080,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .push 2 1506,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .op .POP,
 .push 2 2080,
 .op .MSTORE,
 .push 2 4320,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .op (.Dup { idx := 2 }),
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 6 }),
 .op .GT,
 .op (.Swap { idx := 0 }),
 .op (.Swap { idx := 1 }),
 .op .OR,
 .op (.Swap { idx := 4 }),
 .op .POP,
 .op (.Dup { idx := 3 }),
 .op .MSTORE,
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .op (.Swap { idx := 1 }),
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 1 }),
 .push 2 2080,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .push 2 1562,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .ISZERO,
 .push 2 2080,
 .op .MLOAD,
 .op .OR,
 .push 2 319,
 .op .NOT,
 .op .MUL,
 .push 2 2112,
 .op .ADD,
 .push 2 2688,
 .op .MLOAD,
 .op (.Swap { idx := 1 }),
 .op .MCOPY,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 1646,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .push 0 0,
 .op .NOT,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 1635,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .op .JUMP,
 .op .JUMPDEST,
 .push 0 0,
 .op .MLOAD,
 .push 1 255,
 .op .SHR,
 .op .ISZERO,
 .push 2 1175,
 .op .JUMPI,
 .push 1 1,
 .push 2 2080,
 .op .MSTORE,
 .push 2 4320,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .push 2 2816,
 .op .MLOAD,
 .op .ADD,
 .op .CALLDATALOAD,
 .push 0 0,
 .op .BYTE,
 .op (.Dup { idx := 1 }),
 .op .ISZERO,
 .push 2 1701,
 .op .JUMPI,
 .push 1 128,
 .push 2 1061,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .push 1 1,
 .op .SHR,
 .op .OR,
 .op (.Dup { idx := 0 }),
 .push 1 2,
 .op .SHR,
 .op .OR,
 .op (.Dup { idx := 0 }),
 .push 1 4,
 .op .SHR,
 .op .OR,
 .push 1 1,
 .op .SHR,
 .push 1 1,
 .op .ADD,
 .push 2 2609,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .push 2 1024,
 .op .EQ,
 .push 2 936,
 .op .JUMPI,
 .push 2 936,
 .push 2 1536,
 .op (.Dup { idx := 2 }),
 .push 2 1536,
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .push 1 32,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Dup { idx := 2 }),
 .push 1 32,
 .op .XOR,
 .op .OR,
 .op (.Dup { idx := 3 }),
 .push 1 32,
 .op .XOR,
 .op .OR,
 .push 1 144,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .push 1 115,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .push 1 96,
 .op .JUMPDEST,
 .op .JUMPDEST,
 .op .JUMPDEST,
 .op .JUMPDEST,
 .op .CALLDATALOAD,
 .op (.Dup { idx := 7 }),
 .push 1 32,
 .op .SUB,
 .push 1 3,
 .op .SHL,
 .op .SHR,
 .push 1 1,
 .push 0 0,
 .op .MSTORE,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 3 }),
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 14 }),
 .op (.Dup { idx := 15 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 14 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 13 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 12 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 11 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 10 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 9 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 8 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 7 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 6 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 5 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 3 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MSIZE,
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .op .MULMOD,
 .op .MSIZE,
 .op .MSTORE,
 .op .POP,
 .push 2 480,
 .op (.Dup { idx := 1 }),
 .push 1 247,
 .op .SHR,
 .op (.Dup { idx := 1 }),
 .op .AND,
 .op .MLOAD,
 .op (.Swap { idx := 1 }),
 .push 1 1,
 .op .SHL,
 .op (.Dup { idx := 3 }),
 .push 1 2,
 .op (.Swap { idx := 3 }),
 .op .JUMPDEST,
 .op .JUMPDEST,
 .op (.Dup { idx := 2 }),
 .push 1 4,
 .op .SHR,
 .push 2 512,
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .push 2 544,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 14 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 482,
 .op .MLOAD,
 .op (.Dup { idx := 15 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 514,
 .op .MLOAD,
 .op (.Dup { idx := 10 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 483,
 .op .MLOAD,
 .op (.Dup { idx := 5 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 14 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 515,
 .op .MLOAD,
 .op (.Dup { idx := 15 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 484,
 .op .MLOAD,
 .op (.Dup { idx := 10 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 516,
 .op .MLOAD,
 .op (.Dup { idx := 5 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 14 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 485,
 .op .MLOAD,
 .op (.Dup { idx := 15 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 517,
 .op .MLOAD,
 .op (.Dup { idx := 10 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 486,
 .op .MLOAD,
 .op (.Dup { idx := 5 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 14 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 518,
 .op .MLOAD,
 .op (.Dup { idx := 15 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 487,
 .op .MLOAD,
 .op (.Dup { idx := 10 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 519,
 .op .MLOAD,
 .op (.Dup { idx := 5 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 14 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 488,
 .op .MLOAD,
 .op (.Dup { idx := 15 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 520,
 .op .MLOAD,
 .op (.Dup { idx := 10 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 489,
 .op .MLOAD,
 .op (.Dup { idx := 5 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 14 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 521,
 .op .MLOAD,
 .op (.Dup { idx := 15 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 490,
 .op .MLOAD,
 .op (.Dup { idx := 10 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 522,
 .op .MLOAD,
 .op (.Dup { idx := 5 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 14 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 491,
 .op .MLOAD,
 .op (.Dup { idx := 15 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 523,
 .op .MLOAD,
 .op (.Dup { idx := 10 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .push 2 492,
 .op .MLOAD,
 .op (.Dup { idx := 5 }),
 .op .AND,
 .op .MLOAD,
 .op .MULMOD,
 .op (.Swap { idx := 1 }),
 .push 1 84,
 .op .SHL,
 .op (.Swap { idx := 1 }),
 .push 1 1,
 .op (.Dup { idx := 5 }),
 .op .SUB,
 .op (.Swap { idx := 4 }),
 .push 2 1904,
 .op .JUMPI,
 .push 0 0,
 .op .MSTORE,
 .push 1 32,
 .push 0 0,
 .op .RETURN,
 .op .JUMPDEST,
 .push 0 0,
 .push 0 0,
 .op .MSTORE,
 .push 1 32,
 .push 0 0,
 .op .RETURN,
 .op .JUMPDEST,
 .push 2 2688,
 .op .MLOAD,
 .push 2 1280,
 .push 2 1536,
 .op .MCOPY,
 .op (.Dup { idx := 1 }),
 .push 1 3,
 .op .LT,
 .op (.Dup { idx := 2 }),
 .push 1 7,
 .op .LT,
 .op (.Dup { idx := 3 }),
 .push 1 15,
 .op .LT,
 .op (.Dup { idx := 4 }),
 .push 1 31,
 .op .LT,
 .op .ADD,
 .op .ADD,
 .op .ADD,
 .push 2 903,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 3 }),
 .op .EQ,
 .push 0 0,
 .op .MLOAD,
 .push 1 255,
 .op .SHR,
 .op .AND,
 .op .ISZERO,
 .push 2 2467,
 .op .JUMPI,
 .op (.Dup { idx := 0 }),
 .push 1 96,
 .push 2 256,
 .op .CALLDATACOPY,
 .push 2 2631,
 .push 2 512,
 .push 2 256,
 .push 2 1536,
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 2 }),
 .push 1 31,
 .op .ADD,
 .push 1 5,
 .op .SHR,
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 1 }),
 .push 1 5,
 .op .SHL,
 .op .SUB,
 .push 1 3,
 .op .SHL,
 .push 1 96,
 .op .CALLDATALOAD,
 .op (.Swap { idx := 0 }),
 .op .SHR,
 .op (.Dup { idx := 2 }),
 .push 1 224,
 .op .ADD,
 .op .MSTORE,
 .push 1 1,
 .push 2 963,
 .op .JUMP,
 .op .JUMPDEST,
 .push 1 1,
 .op (.Dup { idx := 6 }),
 .op .SUB,
 .op .JUMPDEST,
 .op (.Dup { idx := 7 }),
 .push 1 1,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 4 }),
 .push 1 7,
 .op .SUB,
 .op .SHR,
 .op .AND,
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 1,
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op .MULMOD,
 .op (.Swap { idx := 5 }),
 .op .POP,
 .op (.Dup { idx := 7 }),
 .push 1 1,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 4 }),
 .push 1 6,
 .op .SUB,
 .op .SHR,
 .op .AND,
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 1,
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op .MULMOD,
 .op (.Swap { idx := 5 }),
 .op .POP,
 .op (.Dup { idx := 7 }),
 .push 1 1,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 4 }),
 .push 1 5,
 .op .SUB,
 .op .SHR,
 .op .AND,
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 1,
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op .MULMOD,
 .op (.Swap { idx := 5 }),
 .op .POP,
 .op (.Dup { idx := 7 }),
 .push 1 1,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 4 }),
 .push 1 4,
 .op .SUB,
 .op .SHR,
 .op .AND,
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 1,
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op .MULMOD,
 .op (.Swap { idx := 5 }),
 .op .POP,
 .op (.Dup { idx := 1 }),
 .op .ISZERO,
 .op (.Swap { idx := 1 }),
 .push 1 4,
 .op .ADD,
 .op (.Swap { idx := 1 }),
 .push 2 2505,
 .op .JUMPI,
 .op .POP,
 .push 1 225,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 1 }),
 .op .ISZERO,
 .push 2 1061,
 .op .JUMPI,
 .push 2 2688,
 .op .MLOAD,
 .push 2 512,
 .push 2 256,
 .op .MCOPY,
 .push 2 1102,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 3 }),
 .push 1 3,
 .op .EQ,
 .push 2 2668,
 .op .JUMPI,
 .op (.Dup { idx := 3 }),
 .push 1 1,
 .op .XOR,
 .push 2 2733,
 .op .JUMPI,
 .push 2 2816,
 .op .MLOAD,
 .op .CALLDATALOAD,
 .push 0 0,
 .op .BYTE,
 .push 1 3,
 .op .XOR,
 .push 2 2733,
 .op .JUMPI,
 .push 1 1,
 .push 2 2688,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2816,
 .op .MLOAD,
 .op .CALLDATALOAD,
 .push 1 232,
 .op .SHR,
 .push 3 65537,
 .op .XOR,
 .push 2 2733,
 .op .JUMPI,
 .push 1 16,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .push 2 2624,
 .op .MSTORE,
 .push 2 2709,
 .push 2 512,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .push 2 4785,
 .push 2 3545,
 .op .JUMP,
 .op .JUMPDEST,
 .push 0 0,
 .op .NOT,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2688,
 .op .JUMPI,
 .op .POP,
 .push 2 1148,
 .push 2 256,
 .op (.Dup { idx := 0 }),
 .push 2 512,
 .push 2 3541,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .push 2 1024,
 .push 2 256,
 .op .MCOPY,
 .push 0 0,
 .push 2 1049,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2688,
 .op .MLOAD,
 .push 1 128,
 .op .LT,
 .op (.Dup { idx := 0 }),
 .push 1 8,
 .op (.Swap { idx := 0 }),
 .op .SHL,
 .op .JUMPDEST,
 .push 2 2771,
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .push 2 1475,
 .op .JUMP,
 .op .JUMPDEST,
 .push 0 0,
 .op .NOT,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2760,
 .op .JUMPI,
 .op .POP,
 .push 1 5,
 .op .SUB,
 .push 2 1635,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 3 }),
 .op .EQ,
 .push 0 0,
 .op .MLOAD,
 .push 1 255,
 .op .SHR,
 .op .AND,
 .op .ISZERO,
 .push 2 2822,
 .op .JUMPI,
 .op (.Dup { idx := 0 }),
 .push 1 96,
 .push 2 2112,
 .op .CALLDATACOPY,
 .push 0 0,
 .push 2 2080,
 .op .MSTORE,
 .push 2 2839,
 .push 2 4521,
 .op .JUMP,
 .op .JUMPDEST,
 .push 1 1,
 .push 2 1024,
 .op .MSTORE,
 .push 2 881,
 .push 2 1024,
 .push 2 1658,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .push 2 2112,
 .push 2 256,
 .op .MCOPY,
 .push 1 1,
 .push 2 2752,
 .op .MLOAD,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .op .NOT,
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 2 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .push 2 1280,
 .op .ADD,
 .op .MSTORE,
 .op (.Dup { idx := 0 }),
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .push 2 2854,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .push 0 0,
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .push 0 0,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op .AND,
 .op (.Dup { idx := 0 }),
 .push 2 1536,
 .op .MSTORE,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .DIV,
 .op (.Dup { idx := 0 }),
 .push 2 1568,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .push 0 0,
 .op .SUB,
 .op .DIV,
 .push 1 1,
 .op .ADD,
 .push 2 1600,
 .op .MSTORE,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .push 0 0,
 .op .SUB,
 .op .MOD,
 .push 2 1632,
 .op .MSTORE,
 .op (.Dup { idx := 0 }),
 .push 1 3,
 .op .MUL,
 .push 1 2,
 .op .XOR,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .push 1 2,
 .op .SUB,
 .op .MUL,
 .push 2 1664,
 .op .MSTORE,
 .op .POP,
 .op .POP,
 .op .POP,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .push 0 0,
 .op .SUB,
 .push 1 3,
 .op .AND,
 .push 1 39,
 .op .MUL,
 .push 2 3139,
 .op .ADD,
 .push 2 1698,
 .op .MSTORE,
 .op (.Dup { idx := 4 }),
 .push 1 1,
 .op .EQ,
 .push 2 2816,
 .op .MLOAD,
 .op .CALLDATALOAD,
 .push 0 0,
 .op .BYTE,
 .push 1 3,
 .op .EQ,
 .op .AND,
 .op (.Dup { idx := 1 }),
 .push 1 3,
 .op .AND,
 .op .ISZERO,
 .op .AND,
 .op (.Dup { idx := 0 }),
 .push 2 1760,
 .op .MSTORE,
 .op .SHR,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .ISZERO,
 .push 2 3488,
 .op .JUMPI,
 .op (.Dup { idx := 1 }),
 .push 2 2112,
 .push 2 2080,
 .op .MCOPY,
 .push 0 0,
 .push 2 2784,
 .op .MLOAD,
 .op .MSTORE,
 .push 2 2080,
 .op .MLOAD,
 .push 2 1536,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .DIV,
 .op (.Swap { idx := 0 }),
 .push 2 1600,
 .op .MLOAD,
 .op .MUL,
 .push 2 1536,
 .op .MLOAD,
 .push 2 2112,
 .op .MLOAD,
 .op .DIV,
 .op .ADD,
 .push 2 1568,
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .push 2 1632,
 .op .MLOAD,
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 2 }),
 .op .ADDMOD,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .push 2 1664,
 .op .MLOAD,
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .push 0 0,
 .op .MLOAD,
 .op .MUL,
 .push 2 2112,
 .op .MLOAD,
 .op .SUB,
 .push 1 32,
 .op .MLOAD,
 .push 1 128,
 .op .SHR,
 .op (.Dup { idx := 2 }),
 .push 1 128,
 .op .SHR,
 .op .MUL,
 .op .GT,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op (.Swap { idx := 0 }),
 .push 2 1568,
 .op .MLOAD,
 .op .GT,
 .op .ISZERO,
 .push 0 0,
 .op .SUB,
 .op .OR,
 .push 0 0,
 .push 1 31,
 .op .NOT,
 .push 0 0,
 .op .NOT,
 .push 2 2784,
 .op .MLOAD,
 .push 2 1698,
 .op .MLOAD,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 832,
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op .MLOAD,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 7 }),
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 6 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 2 }),
 .op .MLOAD,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 5 }),
 .op .GT,
 .op .ADD,
 .op (.Swap { idx := 3 }),
 .op (.Dup { idx := 1 }),
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op .JUMPDEST,
 .push 2 832,
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op .MLOAD,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 7 }),
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 6 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 2 }),
 .op .MLOAD,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 5 }),
 .op .GT,
 .op .ADD,
 .op (.Swap { idx := 3 }),
 .op (.Dup { idx := 1 }),
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op .JUMPDEST,
 .push 2 832,
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op .MLOAD,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 7 }),
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 6 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 2 }),
 .op .MLOAD,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 5 }),
 .op .GT,
 .op .ADD,
 .op (.Swap { idx := 3 }),
 .op (.Dup { idx := 1 }),
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op .JUMPDEST,
 .push 2 832,
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op .MLOAD,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 7 }),
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 6 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 2 }),
 .op .MLOAD,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 5 }),
 .op .GT,
 .op .ADD,
 .op (.Swap { idx := 3 }),
 .op (.Dup { idx := 1 }),
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .push 2 2080,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .push 2 3139,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .op .POP,
 .push 2 2080,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 1 }),
 .op .GT,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .GT,
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op (.Dup { idx := 0 }),
 .push 2 2080,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .OR,
 .push 2 3344,
 .op .JUMPI,
 .op .JUMPDEST,
 .op .NOT,
 .op .ADD,
 .push 2 3024,
 .push 2 4521,
 .op .JUMP,
 .op .JUMPDEST,
 .op .ISZERO,
 .push 2 3429,
 .op .JUMPI,
 .op .JUMPDEST,
 .push 0 0,
 .push 2 2784,
 .op .MLOAD,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .push 2 2112,
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 4 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .POP,
 .op (.Dup { idx := 2 }),
 .op .MSTORE,
 .op (.Swap { idx := 0 }),
 .op (.Swap { idx := 1 }),
 .op .OR,
 .op (.Swap { idx := 0 }),
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .push 2 2111,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .push 2 3356,
 .op .JUMPI,
 .op .POP,
 .push 2 2080,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2080,
 .op .MSTORE,
 .op .LT,
 .op .ISZERO,
 .push 2 3350,
 .op .JUMPI,
 .op .JUMPDEST,
 .push 2 2080,
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .op .ISZERO,
 .push 2 3334,
 .op .JUMPI,
 .op .POP,
 .op .JUMPDEST,
 .push 0 0,
 .push 2 2784,
 .op .MLOAD,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .push 2 2112,
 .op (.Dup { idx := 2 }),
 .op .SUB,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 4 }),
 .op (.Swap { idx := 0 }),
 .op .SUB,
 .op (.Dup { idx := 3 }),
 .op .MSTORE,
 .op .OR,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .push 2 2111,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .push 2 3435,
 .op .JUMPI,
 .op .POP,
 .push 2 2080,
 .op .MLOAD,
 .op .SUB,
 .push 2 2080,
 .op .MSTORE,
 .push 2 3417,
 .op .JUMP,
 .op .JUMPDEST,
 .op .POP,
 .push 2 1760,
 .op .MLOAD,
 .op .ISZERO,
 .push 2 3520,
 .op .JUMPI,
 .push 0 0,
 .push 2 1760,
 .op .MSTORE,
 .op (.Dup { idx := 0 }),
 .push 2 2112,
 .push 2 256,
 .op .MCOPY,
 .op (.Dup { idx := 1 }),
 .push 1 2,
 .op .SHR,
 .push 2 3024,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .push 2 2112,
 .push 2 512,
 .op .MCOPY,
 .op (.Dup { idx := 0 }),
 .push 2 1280,
 .push 2 1024,
 .op .MCOPY,
 .push 2 2631,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 3711,
 .op .JUMPDEST,
 .push 2 2688,
 .op .MLOAD,
 .push 1 127,
 .op .AND,
 .op .ISZERO,
 .push 1 1,
 .push 2 2720,
 .op .MLOAD,
 .op .GT,
 .op .AND,
 .push 2 3571,
 .op .JUMPI,
 .op .POP,
 .push 2 1203,
 .op .JUMP,
 .op .JUMPDEST,
 .push 1 96,
 .op .MLOAD,
 .push 2 2784,
 .op .MLOAD,
 .push 2 2752,
 .op .MLOAD,
 .op .MLOAD,
 .push 2 2720,
 .op .MLOAD,
 .push 2 2752,
 .op .MLOAD,
 .op (.Dup { idx := 6 }),
 .op .ADD,
 .push 1 32,
 .op .MLOAD,
 .push 0 0,
 .push 2 2688,
 .op .MLOAD,
 .push 1 128,
 .op .EQ,
 .push 1 148,
 .op .MUL,
 .push 2 3740,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 283,
 .op .ADD,
 .op (.Swap { idx := 3 }),
 .op (.Swap { idx := 10 }),
 .push 0 0,
 .op .NOT,
 .op (.Swap { idx := 3 }),
 .op (.Swap { idx := 10 }),
 .push 1 64,
 .op .MLOAD,
 .op (.Swap { idx := 10 }),
 .push 2 2688,
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 2 }),
 .push 2 2368,
 .op .MCOPY,
 .op (.Dup { idx := 1 }),
 .push 1 64,
 .op .ADD,
 .op .CALLDATASIZE,
 .push 2 2048,
 .op .CALLDATACOPY,
 .op (.Swap { idx := 1 }),
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Swap { idx := 1 }),
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op .JUMP,
 .op .JUMPDEST,
 .push 0 0,
 .op (.Swap { idx := 4 }),
 .op .POP,
 .op (.Dup { idx := 9 }),
 .push 2 256,
 .op .ADD,
 .op (.Swap { idx := 13 }),
 .op .POP,
 .push 2 1856,
 .op (.Dup { idx := 10 }),
 .op .SUB,
 .op (.Swap { idx := 0 }),
 .op .POP,
 .push 2 3711,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .push 1 224,
 .op (.Swap { idx := 2 }),
 .op .POP,
 .push 2 283,
 .op (.Dup { idx := 7 }),
 .op .SUB,
 .op (.Swap { idx := 3 }),
 .op .POP,
 .push 2 256,
 .op (.Swap { idx := 14 }),
 .op .POP,
 .push 2 1146,
 .op (.Swap { idx := 15 }),
 .op .POP,
 .push 2 2016,
 .op (.Dup { idx := 10 }),
 .op .SUB,
 .op .CALLDATASIZE,
 .push 2 2048,
 .op .CALLDATACOPY,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .op (.Dup { idx := 14 }),
 .op .MLOAD,
 .op (.Dup { idx := 7 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .MULMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 13 }),
 .op .MLOAD,
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 14 }),
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op (.Dup { idx := 5 }),
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2560,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2304,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2304,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op .JUMPDEST,
 .push 2 2528,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2272,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2272,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op .JUMPDEST,
 .push 2 2496,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2240,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2240,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op .JUMPDEST,
 .push 2 2464,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2208,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2208,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op .JUMPDEST,
 .push 2 2432,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2176,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2176,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op .JUMPDEST,
 .push 2 2400,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2144,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2144,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op .JUMPDEST,
 .push 2 2368,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2112,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2112,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 7 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 7 }),
 .op .POP,
 .op .LT,
 .op (.Swap { idx := 0 }),
 .op .POP,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 11 }),
 .op .MLOAD,
 .op .MUL,
 .op (.Dup { idx := 7 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 13 }),
 .op .MULMOD,
 .op (.Dup { idx := 13 }),
 .op .MLOAD,
 .op .ADDMOD,
 .op (.Dup { idx := 9 }),
 .op .JUMP,
 .op .JUMPDEST,
 .push 10 192,
 .op .MLOAD,
 .op (.Dup { idx := 9 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2304,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2336,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .push 1 160,
 .op .MLOAD,
 .op (.Dup { idx := 9 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2272,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2304,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .push 1 128,
 .op .MLOAD,
 .op (.Dup { idx := 9 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2240,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2272,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 14 }),
 .op .MUL,
 .op (.Dup { idx := 9 }),
 .op (.Dup { idx := 15 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 3 2208,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2240,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op .JUMPDEST,
 .op (.Dup { idx := 14 }),
 .op (.Dup { idx := 9 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2176,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2208,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op (.Dup { idx := 15 }),
 .op (.Dup { idx := 9 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2144,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2176,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op (.Dup { idx := 8 }),
 .push 0 0,
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 4 }),
 .op .MUL,
 .op (.Swap { idx := 3 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .push 2 2112,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2144,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op .ADD,
 .op .SUB,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 7 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2112,
 .op .MSTORE,
 .op .LT,
 .op .ADD,
 .op (.Swap { idx := 4 }),
 .op .POP,
 .push 1 31,
 .op .NOT,
 .op .ADD,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Dup { idx := 2 }),
 .op .JUMPI,
 .op (.Dup { idx := 4 }),
 .push 2 2080,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .push 2 4471,
 .op .EQ,
 .push 2 4362,
 .op .JUMPI,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .JUMPDEST,
 .push 0 0,
 .op .MLOAD,
 .push 2 2112,
 .op .MLOAD,
 .op .LT,
 .push 2 2080,
 .op .MLOAD,
 .op .LT,
 .push 2 4398,
 .op .JUMPI,
 .push 2 4536,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2208,
 .op .MLOAD,
 .push 1 96,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .push 2 1888,
 .op .MSTORE,
 .push 2 4689,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 4409,
 .push 2 2368,
 .push 2 2624,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .push 2 2624,
 .op .MSTORE,
 .push 2 4389,
 .op .JUMPI,
 .push 2 3657,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .op .JUMPDEST,
 .push 2 2080,
 .op .MLOAD,
 .push 2 4536,
 .op .JUMPI,
 .op .JUMPDEST,
 .push 2 2112,
 .push 2 2688,
 .op .MLOAD,
 .op (.Swap { idx := 1 }),
 .op .MCOPY,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 6 }),
 .push 2 4171,
 .op .EQ,
 .push 2 4809,
 .op .JUMPI,
 .push 0 0,
 .op (.Swap { idx := 4 }),
 .op .POP,
 .push 2 256,
 .push 2 320,
 .op .CALLDATASIZE,
 .push 2 2048,
 .op .CALLDATACOPY,
 .op .ADD,
 .push 2 283,
 .op (.Dup { idx := 7 }),
 .op .SUB,
 .op (.Swap { idx := 3 }),
 .op .POP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 15 }),
 .op .POP,
 .op .ADD,
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 15 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 15 }),
 .op (.Dup { idx := 0 }),
 .op .MUL,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .LT,
 .op (.Dup { idx := 1 }),
 .op (.Swap { idx := 2 }),
 .op .SUB,
 .op .SUB,
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 5 }),
 .op .MSTORE,
 .push 2 3777,
 .op (.Swap { idx := 5 }),
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 14 }),
 .push 0 0,
 .op .SGT,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 5 }),
 .op .MUL,
 .op (.Swap { idx := 4 }),
 .op .LT,
 .op (.Swap { idx := 2 }),
 .op .MULMOD,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .push 2 256,
 .op (.Dup { idx := 4 }),
 .op .SUB,
 .op (.Dup { idx := 0 }),
 .op .MLOAD,
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 1 }),
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .push 1 37,
 .op (.Dup { idx := 6 }),
 .op .ADD,
 .op (.Swap { idx := 5 }),
 .op .JUMP,
 .op .JUMPDEST,
 .push 0 0,
 .op .MLOAD,
 .push 2 2112,
 .op .MLOAD,
 .op .LT,
 .push 2 5304,
 .op .JUMPI,
 .push 2 2112,
 .op .JUMPDEST,
 .push 2 2688,
 .op .MLOAD,
 .push 2 256,
 .op .EQ,
 .push 2 4580,
 .op .JUMPI,
 .push 2 2688,
 .op .MLOAD,
 .push 1 128,
 .op .EQ,
 .push 2 4341,
 .op .JUMPI,
 .push 2 2784,
 .op .MLOAD,
 .push 2 2752,
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .push 2 1792,
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .push 0 0,
 .op (.Swap { idx := 2 }),
 .push 2 1562,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2336,
 .op .MLOAD,
 .push 1 224,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .push 2 2016,
 .op .MSTORE,
 .push 2 2304,
 .op .MLOAD,
 .push 1 192,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op (.Swap { idx := 2 }),
 .op .GT,
 .op .OR,
 .op (.Swap { idx := 0 }),
 .push 2 1984,
 .op .MSTORE,
 .push 2 2272,
 .op .MLOAD,
 .push 1 160,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op (.Swap { idx := 2 }),
 .op .GT,
 .op .OR,
 .op (.Swap { idx := 0 }),
 .push 2 1952,
 .op .MSTORE,
 .push 2 2240,
 .op .MLOAD,
 .push 1 128,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op (.Swap { idx := 2 }),
 .op .GT,
 .op .OR,
 .op (.Swap { idx := 0 }),
 .push 2 1920,
 .op .MSTORE,
 .push 2 2208,
 .op .MLOAD,
 .push 1 96,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op (.Swap { idx := 2 }),
 .op .GT,
 .op .OR,
 .op (.Swap { idx := 0 }),
 .push 2 1888,
 .op .MSTORE,
 .op .JUMPDEST,
 .push 2 2176,
 .op .MLOAD,
 .push 1 64,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op (.Swap { idx := 2 }),
 .op .GT,
 .op .OR,
 .op (.Swap { idx := 0 }),
 .push 2 1856,
 .op .MSTORE,
 .push 2 2144,
 .op .MLOAD,
 .push 1 32,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op (.Swap { idx := 2 }),
 .op .GT,
 .op .OR,
 .op (.Swap { idx := 0 }),
 .push 2 1824,
 .op .MSTORE,
 .push 2 2112,
 .op .MLOAD,
 .push 0 0,
 .op .MLOAD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 1 }),
 .op .SUB,
 .op (.Swap { idx := 2 }),
 .op .GT,
 .op .OR,
 .op (.Swap { idx := 0 }),
 .push 2 1792,
 .op .MSTORE,
 .op .ISZERO,
 .push 2 2080,
 .op .MLOAD,
 .op .OR,
 .push 2 319,
 .op .NOT,
 .push 2 4772,
 .op .JUMP,
 .op .JUMPDEST,
 .op .MUL,
 .push 2 2112,
 .op .ADD,
 .push 2 2688,
 .op .MLOAD,
 .op (.Swap { idx := 1 }),
 .op .MCOPY,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 4471,
 .op (.Swap { idx := 1 }),
 .op .POP,
 .push 2 2336,
 .op (.Swap { idx := 2 }),
 .op .POP,
 .push 2 1856,
 .op .ADD,
 .op (.Dup { idx := 6 }),
 .push 2 4171,
 .op .XOR,
 .push 2 4441,
 .op .JUMPI,
 .op .JUMPDEST,
 .op (.Dup { idx := 7 }),
 .push 0 0,
 .op .MLOAD,
 .op (.Dup { idx := 14 }),
 .op (.Dup { idx := 14 }),
 .op (.Dup { idx := 12 }),
 .push 2 2464,
 .op .MLOAD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 0 }),
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 0 }),
 .op .MUL,
 .op (.Dup { idx := 13 }),
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 3 }),
 .op (.Dup { idx := 0 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op (.Dup { idx := 2 }),
 .op (.Swap { idx := 1 }),
 .op .SUB,
 .op .SUB,
 .push 2 2432,
 .op .MLOAD,
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 4 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 5 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .push 2 2400,
 .op .MLOAD,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 6 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .push 2 2368,
 .op .MLOAD,
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 7 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 0 }),
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op (.Swap { idx := 4 }),
 .op .POP,
 .push 0 0,
 .push 2 4917,
 .push 2 5157,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2432,
 .op .MLOAD,
 .push 2 2464,
 .op .MLOAD,
 .push 0 0,
 .op .SGT,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 5 }),
 .op .MUL,
 .op (.Swap { idx := 4 }),
 .op .LT,
 .op (.Swap { idx := 2 }),
 .op .MULMOD,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 6 }),
 .op .ADD,
 .op (.Swap { idx := 5 }),
 .op (.Dup { idx := 6 }),
 .op .LT,
 .op .SUB,
 .op .SUB,
 .push 2 2400,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 6 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 6 }),
 .op (.Dup { idx := 7 }),
 .op .LT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .push 2 2368,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 5 }),
 .op (.Dup { idx := 6 }),
 .op .LT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op (.Dup { idx := 6 }),
 .op .ADD,
 .op (.Swap { idx := 5 }),
 .op (.Dup { idx := 6 }),
 .op .LT,
 .op (.Swap { idx := 0 }),
 .op .POP,
 .push 2 5029,
 .push 2 5157,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2400,
 .op .MLOAD,
 .push 2 2432,
 .op .MLOAD,
 .push 0 0,
 .op .SGT,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 5 }),
 .op .MUL,
 .op (.Swap { idx := 4 }),
 .op .LT,
 .op (.Swap { idx := 2 }),
 .op .MULMOD,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 5 }),
 .op .ADD,
 .op (.Swap { idx := 4 }),
 .op (.Dup { idx := 5 }),
 .op .LT,
 .op .SUB,
 .op .SUB,
 .push 2 2368,
 .op .MLOAD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 3 }),
 .op (.Dup { idx := 2 }),
 .op .MUL,
 .op (.Swap { idx := 1 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 5 }),
 .op (.Dup { idx := 6 }),
 .op .LT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op (.Dup { idx := 6 }),
 .op .ADD,
 .op (.Swap { idx := 5 }),
 .op (.Dup { idx := 6 }),
 .op .LT,
 .op (.Swap { idx := 0 }),
 .op .POP,
 .push 2 5112,
 .push 2 5157,
 .op .JUMP,
 .op .JUMPDEST,
 .push 2 2368,
 .op .MLOAD,
 .push 2 2400,
 .op .MLOAD,
 .push 0 0,
 .op .SGT,
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 0 }),
 .op (.Dup { idx := 7 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 5 }),
 .op .MUL,
 .op (.Swap { idx := 4 }),
 .op .LT,
 .op (.Swap { idx := 2 }),
 .op .MULMOD,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 1 }),
 .op .LT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Swap { idx := 2 }),
 .op (.Dup { idx := 3 }),
 .op .LT,
 .op .SUB,
 .op .SUB,
 .op (.Dup { idx := 5 }),
 .op .ADD,
 .op (.Swap { idx := 4 }),
 .op (.Dup { idx := 5 }),
 .op .LT,
 .push 2 5263,
 .op .JUMPDEST,
 .op (.Dup { idx := 5 }),
 .op (.Dup { idx := 13 }),
 .op .MUL,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 12 }),
 .op .MULMOD,
 .op (.Dup { idx := 8 }),
 .op .ADDMOD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 12 }),
 .op .MUL,
 .op (.Dup { idx := 10 }),
 .op (.Dup { idx := 13 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 9 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 10 }),
 .op .POP,
 .op (.Dup { idx := 10 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 13 }),
 .op .MUL,
 .op (.Dup { idx := 10 }),
 .op (.Dup { idx := 14 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 9 }),
 .op .POP,
 .op (.Dup { idx := 9 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 14 }),
 .op .MUL,
 .op (.Dup { idx := 10 }),
 .op (.Dup { idx := 15 }),
 .op (.Dup { idx := 4 }),
 .op .MULMOD,
 .op (.Dup { idx := 0 }),
 .op (.Dup { idx := 2 }),
 .op .GT,
 .op .SUB,
 .op (.Dup { idx := 1 }),
 .op (.Dup { idx := 3 }),
 .op .ADD,
 .op (.Dup { idx := 7 }),
 .op (.Dup { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 8 }),
 .op .POP,
 .op (.Dup { idx := 8 }),
 .op (.Dup { idx := 1 }),
 .op .GT,
 .op (.Swap { idx := 3 }),
 .op .GT,
 .op .SUB,
 .op .SUB,
 .op .ADD,
 .op (.Dup { idx := 8 }),
 .op .ADD,
 .op (.Swap { idx := 3 }),
 .op .POP,
 .op .POP,
 .op (.Dup { idx := 2 }),
 .op (.Dup { idx := 7 }),
 .op .GT,
 .op (.Swap { idx := 0 }),
 .op (.Swap { idx := 1 }),
 .op .ADD,
 .op (.Swap { idx := 5 }),
 .op .POP,
 .op .JUMP,
 .op .JUMPDEST,
 .op (.Dup { idx := 3 }),
 .push 2 2208,
 .op .MSTORE,
 .op (.Dup { idx := 2 }),
 .push 2 2176,
 .op .MSTORE,
 .op (.Dup { idx := 1 }),
 .push 2 2144,
 .op .MSTORE,
 .op (.Dup { idx := 0 }),
 .push 2 2112,
 .op .MSTORE,
 .op (.Dup { idx := 4 }),
 .push 2 2080,
 .op .MSTORE,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .op .POP,
 .push 2 4362,
 .op .JUMP,
 .op .JUMPDEST,
 .op .JUMP]

theorem submissionInstructions_count : submissionInstructions.length = 4084 := by
  decide

theorem assemble_submissionInstructions :
    assemble submissionInstructions = TnCandidate.bytecode := by
  rfl

def submissionArtifact : Challenge.EvmProof.ProgramArtifact where
  code := TnCandidate.bytecode
  instructions := submissionInstructions
  assembly_eq := assemble_submissionInstructions

theorem allWellFormed :
    Challenge.EvmProof.Stepper.AllWellFormed submissionArtifact .Osaka := by
  simp (config := { maxSteps := 4000000 })
    [Challenge.EvmProof.Stepper.AllWellFormed, submissionArtifact,
    submissionInstructions, Challenge.EvmProof.Stepper.WellFormed,
    YulEvmCompiler.plainOp]
  repeat' apply And.intro
  all_goals decide

def instructionPC (index : Nat) : Nat := submissionArtifact.instructionPC index

theorem decodeAt_op_index (index : Nat) (op : Operation)
    (hget : submissionInstructions[index]? = some (.op op))
    (hopcode : Decode.opcodeOf (Instr.opByte op) = some op)
    (hplain : YulEvmCompiler.plainOp op) :
    Decode.decodeAt TnCandidate.bytecode (instructionPC index) = some (op, none) :=
  Challenge.EvmProof.ProgramArtifact.decodeAt_op_index
    submissionArtifact index op hget hopcode hplain

theorem decodeAt_push_index (index : Nat) (width : Fin 33) (value : UInt256)
    (hget : submissionInstructions[index]? = some (.push width value))
    (hfit : value.toNat < 256 ^ width.val) :
    Decode.decodeAt TnCandidate.bytecode (instructionPC index) =
      some (.Push ⟨width⟩, some (value, width.val)) :=
  Challenge.EvmProof.ProgramArtifact.decodeAt_push_index
    submissionArtifact index width value hget hfit

theorem isValidJumpDest_index (index : Nat)
    (hget : submissionInstructions[index]? = some (.op .JUMPDEST)) :
    Decode.isValidJumpDest TnCandidate.bytecode (instructionPC index) = true :=
  Challenge.EvmProof.ProgramArtifact.isValidJumpDest_index
    submissionArtifact index hget

end Challenge.Modexp.Submission.Proofs.Fast.TnCandidateArtifact
