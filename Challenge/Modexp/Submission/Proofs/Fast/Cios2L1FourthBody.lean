...
theorem run_l1FourthMac (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock fourthMac
      ...
      some (l1At 4448 s mem bi pa pb n i (j + 1) pdst ret rest) := by
...
def gasSteps_l1FourthMac (s : State) (mem : ByteArray) (bi : UInt256)
    (pa pb n i j : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hfork : s.fork = .Osaka)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 4379 s mem bi pa pb n i j pdst ret rest)
      (l1At 4448 s mem bi pa pb n i (j + 1) pdst ret rest) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka fourthMac hcode hfork
    (run_l1FourthMac s mem bi pa pb n i j pdst ret rest hcap hrun hcode hact hn32 hj
      hpa hpaFit) hrun hnp
