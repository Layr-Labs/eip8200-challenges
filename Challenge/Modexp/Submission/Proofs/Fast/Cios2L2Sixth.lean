...
theorem run_sixth (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    (pa pb n i k : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    (hact : 296 ≤ s.activeWords.toNat)
    (hn32 : n ≤ 32) (hk : k + 1 < n) :
    Challenge.EvmProof.Stepper.runLocatedBlock sixthMac
      (l2At 5213 s mid bi mu c0 pa pb n i k pdst ret rest) =
      some (l2At 5254 s mid bi mu c0 pa pb n i (k + 1) pdst ret rest) := by
  ...
  simp (config := { maxSteps := 800000 })
    [sixthMac, Cios2Paths.L2Pair.sixthPC, Cios2Paths.L2Pair.sixthStartIndex,
      Cios2Paths.L2Pair.sixthOpAt, Cios2Paths.L2Pair.sixthPushAt, wfOp,
...
def gasSteps_sixth (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    (pa pb n i k : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    ...
    Challenge.EvmProof.GasSteps
      (l2At 5213 s mid bi mu c0 pa pb n i k pdst ret rest)
      (l2At 5254 s mid bi mu c0 pa pb n i (k + 1) pdst ret rest) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka sixthMac hcode hfork
    (run_sixth s mid bi mu c0 pa pb n i k pdst ret rest hcap hrun hact hn32 hk)
      hrun hnp
