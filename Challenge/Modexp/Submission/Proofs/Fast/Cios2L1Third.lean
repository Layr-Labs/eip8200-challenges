...
theorem run_l1ThirdMac (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock thirdMac
      (l1At 4310 s mem bi pa pb n i j pdst ret rest) =
      some (l1At 4379 s mem bi pa pb n i (j + 1) pdst ret rest) := by
  ...
  simp (config := { maxSteps := 800000 })
    [thirdMac, Cios2Paths.L1.thirdPC, Cios2Paths.L1.thirdStartIndex,
      Cios2Paths.L1.thirdOpAt, Cios2Paths.L1.thirdPushAt, wfOp,
...
def gasSteps_l1ThirdMac (s : State) (mem : ByteArray) (bi : UInt256)
    (pa pb n i j : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hfork : s.fork = .Osaka)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 4310 s mem bi pa pb n i j pdst ret rest)
      (l1At 4379 s mem bi pa pb n i (j + 1) pdst ret rest) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka thirdMac hcode hfork
    (run_l1ThirdMac s mem bi pa pb n i j pdst ret rest hcap hrun hcode hact hn32 hj
      hpa hpaFit) hrun hnp
