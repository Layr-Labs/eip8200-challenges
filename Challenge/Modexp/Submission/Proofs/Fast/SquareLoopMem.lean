...
the rows and the final conditional subtraction:
    ...
    sqRound s n c mem
      = CSUB (2720 := c) (rows (re-stage, re-zero) mem)
...
model already differs from the Monpro-style one) and the counter word `[2720, 2752)`.
...
`[2720, 2752)` (the squaring counter). -/
def Agree (a b : ByteArray) : Prop :=
  ∀ i, i < 2048 ∨ (2080 ≤ i ∧ i < 2720) ∨ 2752 ≤ i → a[i]?.getD 0 = b[i]?.getD 0
...
/-- Storing the counter at 2720 changes nothing that `Agree` sees. -/
...
theorem readPadded_eq {a b : ByteArray} (h : Agree a b) (start count : Nat)
    (hout : start + count ≤ 2048 ∨ (2080 ≤ start ∧ start + count ≤ 2720) ∨ 2752 ≤ start) :
...
theorem readWord_eq {a b : ByteArray} (h : Agree a b) (start : Nat)
    (hout : start + 32 ≤ 2048 ∨ (2080 ≤ start ∧ start + 32 ≤ 2720) ∨ 2752 ≤ start) :
...
theorem csResult_agree (a b : ByteArray) (h : Agree a b) (n dst : Nat)
  ...
  ·
    ...
    have hout : (Csub.csSrc b n n).toNat + 32 * n ≤ 2048 ∨
        (2080 ≤ (Csub.csSrc b n n).toNat ∧ (Csub.csSrc b n n).toNat + 32 * n ≤ 2720) ∨
        2752 ≤ (Csub.csSrc b n n).toNat := by
      rw [hsrc]
      split <;> omega
...
theorem fastRepresents_iff (a b : ByteArray) (h : Agree a b) (ptr n value : Nat)
    (hout : ptr + 32 * n ≤ 2048 ∨ (2080 ≤ ptr ∧ ptr + 32 * n ≤ 2720) ∨ 2752 ≤ ptr) :
...
rows, store the decremented counter `c` at 2720, and subtract conditionally. -/
...
/-- The row-0 memory of a square: the operand staged at 2400 and the accumulator zeroed. -/
theorem tn_le_one (s : State) (mem : ByteArray) (p a mm : Nat) (hn32 : p + 2 ≤ 32)
    ...
    (MachineState.readWord
      (sqRowsCarry (mpZeroed s (stage mem 512 (p + 2)) (p + 2)) (p + 2) (p + 2)) 2080).toNat
        ≤ 1 := by
  have hread (addr : Nat) (hd : addr + 32 ≤ 2048 ∨ 2720 ≤ addr) :
  ...
  have hminv0 : ((MachineState.readWord (mpZeroed s (stage mem 512 (p + 2)) (p + 2))
      (32 * (p + 2) - 32)).toNat *
      (MachineState.readWord (mpZeroed s (stage mem 512 (p + 2)) (p + 2)) 2816).toNat + 1) %
        2 ^ 256 = 0 := by
    rw [hread (32 * (p + 2) - 32) (Or.inl (by omega)), hread 2816 (Or.inr (by decide))]
    exact hminv
...
/-- Everything at or above 2752 survives a round. -/
theorem sqRound_readWord_high (s : State) (mem : ByteArray) (n c addr : Nat)
    (hn : 1 ≤ n) (hn32 : n ≤ 32) (hfast : n = 4 ∨ n = 8) (haddr : 2752 ≤ addr) :
...
theorem sqRound_count (s : State) (mem : ByteArray) (n c : Nat) (hn : 1 ≤ n) (hn32 : n ≤ 32)
    (hc : c < 2 ^ 256) :
    MachineState.readWord (sqRound s n c mem) 2720 = UInt256.ofNat c := by
  rw [sqRound, csResultMemory_readWord_outside _ n 512 2720 hn (Or.inr (by omega))
    (Or.inr (by omega))]
...
theorem sqRound_fastRepresents_outside (s : State) (mem : ByteArray) (n c ptr cnt v : Nat)
    ...
    (hcount : ptr + 32 * cnt ≤ 2720 ∨ 2752 ≤ ptr)
...
theorem sqLoopMem_readWord_high (s : State) (mem : ByteArray) (n k addr : Nat)
    (hfast : n = 4 ∨ n = 8) (hn : 1 ≤ n) (hn32 : n ≤ 32) (haddr : 2752 ≤ addr) :
...
theorem sqLoopMem_frame (s : State) (mem : ByteArray) (n k : Nat) (hfast : n = 4 ∨ n = 8)
    ...
    MachineState.readWord (sqLoopMem s n k mem) 2784 = MachineState.readWord mem 2784 ∧
      ...
      MachineState.readWord (sqLoopMem s n k mem) 2848 = MachineState.readWord mem 2848 ∧
      MachineState.readWord (sqLoopMem s n k mem) 2880 = MachineState.readWord mem 2880 ∧
      MachineState.readWord (sqLoopMem s n k mem) 2912 = MachineState.readWord mem 2912 :=
  ⟨sqLoopMem_readWord_high s mem n k 2784 hfast hn hn32 (by omega),
   ...
   sqLoopMem_readWord_high s mem n k 2848 hfast hn hn32 (by omega),
   sqLoopMem_readWord_high s mem n k 2880 hfast hn hn32 (by omega),
   sqLoopMem_readWord_high s mem n k 2912 hfast hn hn32 (by omega)⟩
...
theorem sqLoopMem_represents (s : State) (mem : ByteArray) (p a mm k : Nat)
  ...
  | succ k ih =>
      ...
      have hminv' : ((MachineState.readWord (sqRound s (p + 2) k mem)
          (32 * (p + 2) - 32)).toNat *
          (MachineState.readWord (sqRound s (p + 2) k mem) 2816).toNat + 1) % 2 ^ 256 = 0 := by
        rw [sqRound_readWord_outside s mem (p + 2) k (32 * (p + 2) - 32) (by omega) hn32 hfast
          ...
          sqRound_readWord_high s mem (p + 2) k 2816 (by omega) hn32 hfast (by omega)]
        exact hminv
