...
def value (mem : ByteArray) (c extra : UInt256) : UInt256 :=
  MachineState.readWord mem 2080 + (c+extra)
...
def memory (mem : ByteArray) (c extra : UInt256) : ByteArray :=
  storeWord mem 2080 (value mem c extra)
...
theorem top_spec (mem : ByteArray) (c extra : UInt256) :
    (flag mem c extra).toNat*Limbs.radix + (value mem c extra).toNat =
      (MachineState.readWord mem 2080).toNat+c.toNat+extra.toNat := by
  have h1 := add_carry_split extra c
  rw [word_add_comm extra c] at h1
  have h2 := add_carry_split (MachineState.readWord mem 2080) (c+extra)
  rw [flag_toNat, value, radix_eq]
  calc
    _ = (UInt256.lt (c+extra) c).toNat*2^256 +
        ((UInt256.lt (MachineState.readWord mem 2080+(c+extra)) (c+extra)).toNat*2^256 +
          (MachineState.readWord mem 2080+(c+extra)).toNat) := by ring
    _ = (UInt256.lt (c+extra) c).toNat*2^256 +
        ((MachineState.readWord mem 2080).toNat+(c+extra).toNat) := by rw [h2]
    _ = (MachineState.readWord mem 2080).toNat +
        ((UInt256.lt (c+extra) c).toNat*2^256+(c+extra).toNat) := by ring
    _ = (MachineState.readWord mem 2080).toNat + (extra.toNat+c.toNat) := by rw [h1]
    _ = _ := by ring
...
theorem flag_zero (mem : ByteArray) (c : UInt256) :
    flag mem c (UInt256.ofNat 0) = UInt256.lt (MachineState.readWord mem 2080+c) c := by
  simp only [flag, value, add_zero, gt_self]

/-- The exact shared TOP block, including its common MU cleanup. -/
def topBlock : Block Artifact.submissionArtifact .Osaka 4466 CarryRowPrograms.middleStore :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3396 24 4466 CarryRowPrograms.middleStore
...
theorem run_first (s : State) (mem : ByteArray) (c bi : UInt256) (rest : List UInt256)
    ...
    runInstructions firstProgram (stateAt s mem 4466 (c :: bi :: rest)) =
      some (stateAt s mem 4480
        ((c+bi*MachineState.readWord mem 2368) ::
         UInt256.gt c (c+bi*MachineState.readWord mem 2368) :: bi :: rest)) := by
...
theorem run_second (s : State) (mem : ByteArray) (v f bi : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1016) (hact : 91 ≤ s.activeWords.toNat) :
    runInstructions secondProgram (stateAt s mem 4480 (v :: f :: bi :: rest)) =
      some (stateAt s (storeWord mem 2080 (MachineState.readWord mem 2080+v)) 4496
        ((UInt256.lt (MachineState.readWord mem 2080+v) v+f) :: rest)) := by
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have hc4 : rest.length+4 < 1024 := by omega
  have hc5 : rest.length+5 < 1024 := by omega
  have hc6 : rest.length+6 < 1024 := by omega
  have haN := EarlyCsub.activeWords_fix s 2080 32 (by decide) (by decide) hact
...
theorem run_top (s : State) (mem : ByteArray) (c bi : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1016) (hact : 91 ≤ s.activeWords.toNat) :
    runInstructions CarryRowPrograms.middleStore (stateAt s mem 4466 (c :: bi :: rest)) =
      some (stateAt s (memory mem c (bi*MachineState.readWord mem 2368)) 4496
        (flag mem c (bi*MachineState.readWord mem 2368) :: rest)) := by
  exact runInstructions_append_some firstProgram secondProgram _ _ _
    (run_first s mem c bi rest hcap hact)
    (run_second s mem (c+bi*MachineState.readWord mem 2368)
      (UInt256.gt c (c+bi*MachineState.readWord mem 2368)) bi rest hcap hact)
...
def gasSteps_top (s : State) (mem : ByteArray) (c bi : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.GasSteps (stateAt s mem 4466 (c :: bi :: rest))
      (stateAt s (memory mem c (bi*MachineState.readWord mem 2368)) 4496
        (flag mem c (bi*MachineState.readWord mem 2368) :: rest)) :=
  topBlock.steps (EarlyCsub.environment (stateAt s mem 4466 (c :: bi :: rest)) hcode hfork hrun hnp) rfl
