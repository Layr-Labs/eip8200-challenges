...
* `blk1362` (idx 1362..1368, pc 1906..1925) — the loop head `DBL`, which
  pushes the call frame `[px, px, px, 1917]` and jumps to `ADDMOD` (pc 2224);
* `blk1369` (idx 1369..1375, pc 1917..1935) — the return point, which
  decrements the counter and jumps back to pc 1906 while it is nonzero;
...
/-- The loop head `DBL`, pc 1906, with the counter at `k`. -/
def loopState (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 1906
           stack := loopStack px k ret rest
           memory := mem }

/-- The `ADDMOD` call, pc 2224, with the frame `[px, px, px, 1917]` pushed. -/
def callState (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
  ...
  { s with pc := UInt256.ofNat 2203
           stack := [UInt256.ofNat px, UInt256.ofNat px, UInt256.ofNat px,
                     UInt256.ofNat 1917] ++ loopStack px k ret rest
           memory := mem }

/-- The return point, pc 1917, with the counter still at `k`. -/
def retState (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 1917
           stack := loopStack px k ret rest
           memory := mem }
...
set_option linter.unusedSimpArgs false in
/-- `blk1362` (pc 1906..1925): the loop head pushes the `ADDMOD` frame
`[px, px, px, 1917]` and jumps to pc 2224. -/
theorem run_call (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
  ...
  have h1926 : (1917 : UInt256) = UInt256.ofNat 1917 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1369` (pc 1917..1935), counter above one: decrement and loop. -/
theorem run_ret (s : State) (mem : ByteArray) (px k k' : Nat) (ret : UInt256)
  ...
  have h1915 : (1906 : UInt256) = UInt256.ofNat 1906 := by decide
  have h1915Nat : (UInt256.ofNat 1906).toNat = 1906 := by decide
  ...
  simp (config := { maxSteps := 400000 }) [blk1369, opAt, pushAt, wfOp,
    ...
    hone, h1915, h1915Nat, hsub, htrue, jumpDest1906, List.exchange,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat]
...
set_option linter.unusedSimpArgs false in
/-- `blk1369` (pc 1917..1935), counter one: fall through to the exit. -/
...
theorem jump1926 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 1917).toNat = true := by
  rw [show (UInt256.ofNat 1917).toNat = 1917 by decide]
  exact jumpDest1917
...
def gasSteps_double256_addmod (s : State) (mem : ByteArray) (px n : Nat)
  ...
  gasSteps_double256 s px ret rest (iterMem (dblStep px n) mem)
    (fun i _ => Challenge.EvmProof.GasSteps.cast
      (gasSteps_addmodStep s (iterMem (dblStep px n) mem i) px n (UInt256.ofNat 1917)
