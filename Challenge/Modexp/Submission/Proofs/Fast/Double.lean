...
* `blk1362` (idx 1362..1368, pc 1841..1925) — the loop head `DBL`, which
  pushes the call frame `[px, px, px, 1852]` and jumps to `ADDMOD` (pc 2224);
* `blk1369` (idx 1369..1375, pc 1852..1935) — the return point, which
  decrements the counter and jumps back to pc 1841 while it is nonzero;
...
/-- The loop head `DBL`, pc 1841, with the counter at `k`. -/
...
/-- The `ADDMOD` call, pc 2224, with the frame `[px, px, px, 1852]` pushed. -/
def callState (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
  ...
  { s with pc := UInt256.ofNat 2056
           stack := [UInt256.ofNat px, UInt256.ofNat px, UInt256.ofNat px,
                     UInt256.ofNat 1771] ++ loopStack px k ret rest
           memory := mem }

/-- The return point, pc 1852, with the counter still at `k`. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1362` (pc 1841..1925): the loop head pushes the `ADDMOD` frame
`[px, px, px, 1852]` and jumps to pc 2224. -/
theorem run_call (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
  ...
  have h1926 : (1771 : UInt256) = UInt256.ofNat 1771 := by decide
  have h2467 : (2056 : UInt256) = UInt256.ofNat 2056 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1369` (pc 1852..1935), counter above one: decrement and loop. -/
theorem run_ret (s : State) (mem : ByteArray) (px k k' : Nat) (ret : UInt256)
  ...
  have h1915 : (1760 : UInt256) = UInt256.ofNat 1760 := by decide
  have h1915Nat : (UInt256.ofNat 1760).toNat = 1760 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1369` (pc 1852..1935), counter one: fall through to the exit. -/
...
theorem readWord_amResultMemory_high (mem : ByteArray) (pa pb n addr : Nat)
    (hn : 1 ≤ n) (hn32 : n ≤ 8) (haddr : 2784 ≤ addr) :
...
theorem readWord_csResultMemory_high (mem : ByteArray) (n pdst addr : Nat)
    (hn : 1 ≤ n) (hn32 : n ≤ 8) (hpdst : pdst + 32 * n ≤ 2784) (haddr : 2784 ≤ addr) :
...
theorem vars_dblStep (mem : ByteArray) (px n : Nat) (hn : 2 ≤ n) (hn32 : n ≤ 8)
  ...
  · rw [dblStep, readWord_csResultMemory_high _ n px 2784 (by omega) hn32 (by omega)
    ...
    exact hv.s32
  · rw [dblStep, readWord_csResultMemory_high _ n px 2848 (by omega) hn32 (by omega)
      (by omega), readWord_amResultMemory_high _ px px n 2848 (by omega) hn32 (by omega)]
    exact hv.ml
  · rw [dblStep, readWord_csResultMemory_high _ n px 2880 (by omega) hn32 (by omega)
      (by omega), readWord_amResultMemory_high _ px px n 2880 (by omega) hn32 (by omega)]
    exact hv.tl
...
def gasSteps_addmodStep (s : State) (mem : ByteArray) (px n : Nat) (ret' : UInt256)
  ...
  have hml : MachineState.readWord (Csub.amResultMemory mem px px n) 2848 =
      UInt256.ofNat (32 * n - 32) := by
    rw [readWord_amResultMemory_high _ px px n 2848 (by omega) hn32 (by omega)]
    exact hv.ml
  have htl : MachineState.readWord (Csub.amResultMemory mem px px n) 2880 =
      UInt256.ofNat (2080 + 32 * n) := by
    rw [readWord_amResultMemory_high _ px px n 2880 (by omega) hn32 (by omega)]
    exact hv.tl
  have hs32' : MachineState.readWord
      (Csub.csStep (Csub.amResultMemory mem px px n) n n).memory 2784 =
      UInt256.ofNat (32 * n) := by
    rw [Csub.csStep_readWord_disjoint _ n 2784 (by omega) (Or.inr (by omega)) n le_rfl,
      readWord_amResultMemory_high _ px px n 2784 (by omega) hn32 (by omega)]
    exact hv.s32
  ...
  have hdstFit : (UInt256.ofNat px).toNat + 32 * n ≤ 2912 := by
...
theorem jump1926 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 1771).toNat = true := by
  rw [show (UInt256.ofNat 1771).toNat = 1771 by decide]
  exact jumpDest1852
...
def gasSteps_double256_addmod (s : State) (mem : ByteArray) (px n : Nat)
  ...
  gasSteps_double256 s px ret rest (iterMem (dblStep px n) mem)
    (fun i _ => Challenge.EvmProof.GasSteps.cast
      (gasSteps_addmodStep s (iterMem (dblStep px n) mem i) px n (UInt256.ofNat 1771)
