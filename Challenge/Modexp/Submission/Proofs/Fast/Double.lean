...
theorem vars_dblStep (mem : ByteArray) (px n : Nat) (hn : 2 ≤ n) (hn32 : n ≤ 8)
    (hpxFit : px + 32 * n ≤ 2048) (hv : Vars mem n) : Vars (dblStep px n mem) n := by
  refine ⟨?_, ?_, ?_⟩
  · rw [dblStep, readWord_csResultMemory_high _ n px 2688 (by omega) hn32 (by omega)
      (by omega), readWord_amResultMemory_high _ px px n 2688 (by omega) hn32 (by omega)]
    exact hv.s32
  · rw [dblStep, readWord_csResultMemory_high _ n px 2752 (by omega) hn32 (by omega)
      (by omega), readWord_amResultMemory_high _ px px n 2752 (by omega) hn32 (by omega)]
    exact hv.ml
  · rw [dblStep, readWord_csResultMemory_high _ n px 2784 (by omega) hn32 (by omega)
      (by omega), readWord_amResultMemory_high _ px px n 2784 (by omega) hn32 (by omega)]
    exact hv.tl

theorem vars_iterMem (mem : ByteArray) (px n : Nat) (hn : 2 ≤ n) (hn32 : n ≤ 8)
    (hpxFit : px + 32 * n ≤ 2048) (hv : Vars mem n) (i : Nat) :
...
def gasSteps_addmodStep (s : State) (mem : ByteArray) (px n : Nat) (ret' : UInt256)
  ...
  have hml : MachineState.readWord (Csub.amResultMemory mem px px n) 2752 =
      UInt256.ofNat (32 * n - 32) := by
    rw [readWord_amResultMemory_high _ px px n 2752 (by omega) hn32 (by omega)]
    exact hv.ml
  have htl : MachineState.readWord (Csub.amResultMemory mem px px n) 2784 =
      UInt256.ofNat (2080 + 32 * n) := by
    rw [readWord_amResultMemory_high _ px px n 2784 (by omega) hn32 (by omega)]
    exact hv.tl
  have hs32' : MachineState.readWord
      (Csub.csStep (Csub.amResultMemory mem px px n) n n).memory 2688 =
      UInt256.ofNat (32 * n) := by
    rw [Csub.csStep_readWord_disjoint _ n 2688 (by omega) (Or.inr (by omega)) n le_rfl,
      readWord_amResultMemory_high _ px px n 2688 (by omega) hn32 (by omega)]
    exact hv.s32
  have htn : (MachineState.readWord
      (Csub.csStep (Csub.amResultMemory mem px px n) n n).memory 2080).toNat ≤ 1 := by
  ...
  have hdstFit : (UInt256.ofNat px).toNat + 32 * n ≤ 2816 := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt (Nat.lt_of_le_of_lt (show px ≤ 2048 by omega) (by norm_num))]
    omega
...
def gasSteps_double256_addmod (s : State) (mem : ByteArray) (px n : Nat)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 88 ≤ s.activeWords.toNat) (hn : 2 ≤ n) (hn32 : n ≤ 8)
    (hpx : 32 ≤ px) (hpxFit : px + 32 * n ≤ 2048) (hv : Vars mem n) :
    Challenge.EvmProof.GasSteps (entryState s mem px ret rest)
  ...
  gasSteps_double256 s px ret rest (iterMem (dblStep px n) mem)
    (fun i _ => Challenge.EvmProof.GasSteps.cast
      ...
      (csReturned_eq s (iterMem (dblStep px n) mem i) px n (256 - i) ret rest
        (Nat.lt_of_le_of_lt (show px ≤ 2048 by omega) (by norm_num))))
...
theorem dblStep_represents (mem : ByteArray) (px n mm x : Nat)
    (hn : 2 ≤ n) (hn32 : n ≤ 8) (hpxFit : px + 32 * n ≤ 2048) (_hpxLow : 32 * n ≤ px)
...
theorem iterMem_dblStep_invariant (mem : ByteArray) (px n mm x : Nat)
    (hn : 2 ≤ n) (hn32 : n ≤ 8) (hpxFit : px + 32 * n ≤ 2048) (hpxLow : 32 * n ≤ px)
