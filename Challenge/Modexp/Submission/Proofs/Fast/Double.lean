...
/-- The wrapped decrement that `PUSH0; NOT; ADD` computes: adding the
all-ones word is subtraction of one on every nonzero counter value. -/
private theorem dec_ofNat (i : Nat) (hi : 1 ≤ i) (hi' : i < 2 ^ 256) :
...
theorem run_ret (s : State) (mem : ByteArray) (px k k' : Nat) (ret : UInt256)
  ...
  have hsub : UInt256.lnot (UInt256.ofNat 0) + UInt256.ofNat (k' + 1) =
      UInt256.ofNat k' := by
    have h := dec_ofNat (k' + 1) (by omega) (by omega)
  ...
  simp (config := { maxSteps := 400000 }) [blk1369, opAt, pushAt, wfOp,
    ...
    hzero, h1915, h1915Nat, hsub, htrue, jumpDest1841, List.exchange,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat]
...
theorem run_retLast (s : State) (mem : ByteArray) (px : Nat) (ret : UInt256)
  ...
  have hzero : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have h1915 : (1178 : UInt256) = UInt256.ofNat 1178 := by decide
  have hsub : UInt256.lnot (UInt256.ofNat 0) + UInt256.ofNat 1 = UInt256.ofNat 0 :=
    dec_ofNat 1 (by omega) (by norm_num)
  have hfalse : ¬ UInt256.isTrue (UInt256.ofNat 0) := by decide
  simp (config := { maxSteps := 400000 }) [blk1369, opAt, pushAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    retState, exitState, loopStack, fastPC9, hc3, hc4, hc5, hrun,
    hzero, h1915, hsub, hfalse, List.exchange,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat]
