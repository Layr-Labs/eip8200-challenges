...
* `blk1362` (idx 1362..1368, pc 1915..1925) — the loop head `DBL`, which
  pushes the call frame `[px, px, px, 1926]` and jumps to `ADDMOD` (pc 2511);
...
/-- The `ADDMOD` call, pc 2511, with the frame `[px, px, px, 1926]` pushed. -/
...
`[px, px, px, 1926]` and jumps to pc 2511. -/
theorem run_call (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
  ...
  simp (config := { maxSteps := 400000 }) [blk1362, opAt, pushAt, wfOp,
    ...
    hcode, hrun,
    h1926, h2511, h2511Nat, jumpDest2467,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat]
...
`Fast.Csub` proves `ADDMOD` from its entry at pc 2511 down to the `CSUB` entry
at pc 2686, and `CSUB` from there to the return jump.  Composing the two gives
