...
def csResultMemory (memory : ByteArray) (n pdst : Nat) : ByteArray :=
  if Skip memory then
    MachineState.writeBytes memory (MachineState.readPadded memory 4160 (32*n)) pdst
...
def csReturnedState (s : State) (memory : ByteArray) (n j : Nat)
  ...
  { s with
    ...
    memory := if Skip memory then
      MachineState.writeBytes memory (MachineState.readPadded memory 4160 (32*n)) pdst.toNat
...
theorem csub_correct (memory : ByteArray) (n tlow mm tn pdst : Nat)
  ...
  · rename_i hskip
    ...
    exact fastRepresents_mcopy memory 4160 pdst n tlow (by omega) ht
...
theorem csub_preserves_region (memory : ByteArray) (n pdst ptr cnt v : Nat)
  ...
  unfold csResultMemory
  split
  · exact fastRepresents_mcopy_disjoint memory 4160 pdst (32*n) ptr cnt v hdisjDst hrep
...
theorem guarded_readWord_outside (memory : ByteArray) (n pdst addr : Nat)
    (hn : 1 ≤ n) (hsubb : addr+32 ≤ 3072 ∨ 3072+32*n ≤ addr)
