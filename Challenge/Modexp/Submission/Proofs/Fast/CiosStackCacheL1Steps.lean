...
theorem run_l1Last (s : State) (c : CachedMemory) (r : ReadOnlyCache) (bi : UInt256)
    ...
    runInstructions l1Last (l1At 4887 s c r bi pa pb n i (n-1) dst ret rest) =
  ...
  have hb := run_l1Body 4887 s c r bi pa pb n i (n-1) dst ret rest hcap hact hn (by omega) hpa hfit
  rw [hn0] at hb
  let p := cacheL1 c bi pa n n
  have hd := run_drop { s with memory := p.cache.memory } (UInt256.ofNat 4918) p.cache r
    (UInt256.ofNat (ptrAt (pb+32*n-32) i)) (UInt256.ofNat (pa+32*n-32))
    (UInt256.ofNat (pb-32)) (CiosCached.isFour n) dst ret
