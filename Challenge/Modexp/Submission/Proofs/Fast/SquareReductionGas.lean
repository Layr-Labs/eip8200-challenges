...
def qBlock : Block Artifact.submissionArtifact .Osaka 4496 CiosReadonly.cachedProduct :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3420 16 4496 CiosReadonly.cachedProduct
...
def gasSteps_reduce (s : State) (mem : ByteArray) (f pbi pa pb tag tl inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (stateAt s mem 4496 (f :: base pbi pa pb tag (UInt256.ofNat 4514) inv
        (m0 :: tl :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest)))
      (stateAt s (SquareReduce.reduction mem f)
        (if UInt256.isTrue (UInt256.gt (negative32+pbi) pb) then 5186 else 4790)
        (base (negative32+pbi) pa pb tag (UInt256.ofNat 4514) inv
  ...
  have hq : Challenge.EvmProof.GasSteps
      (stateAt s mem 4496 (f :: base pbi pa pb tag (UInt256.ofNat 4514) inv
        (m0 :: tl :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest))) (S 4512 0) := by
    apply qBlock.steps (s := stateAt s mem 4496 (f :: base pbi pa pb tag (UInt256.ofNat 4514) inv
        (m0 :: tl :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest))) (env.transfer rfl rfl) rfl
    exact CiosReadonly.run_cachedProduct_model {s with memory := mem} f pbi pa pb tag (UInt256.ofNat 4514)
  ...
  apply CarryRowBlocks.tailLoop.steps (s := S 4761 7) (env.transfer rfl rfl) rfl
  have hr : MachineState.readWord (l2Step mem mu c0 8 7).memory 2720 = UInt256.ofNat 5186 :=
    (SquareReduce.read_l2_outside mem mu c0 2720 (Or.inr (by decide)) 7 (by decide)).trans hroute
  have h := CarryRowRun.run_tail {s with memory := (l2Step mem mu c0 8 7).memory}
    (l2Step mem mu c0 8 7).carry mu f pbi pa pb tag (UInt256.ofNat 4514) inv
    (m0 :: tl :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest)
    (by simp only [List.length_cons]; omega) hact (UInt256.ofNat 5186) hr
    (by change Decode.isValidJumpDest s.executionEnv.code 5186 = true; rw [env.code]; exact SquareParts.jump_row)
