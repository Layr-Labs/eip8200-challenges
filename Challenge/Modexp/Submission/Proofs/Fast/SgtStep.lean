...
theorem sqRowSgt_index : Artifact.submissionInstructions[3697]? = some (.op .SGT) := by
  rfl

/-- ... at program counter 4716 (0x126c). -/
theorem sqRowSgt_pc : Artifact.submissionArtifact.instructionPC 3697 = 4929 := by
  rfl
...
theorem decodedOp_sqRowSgt (s : State)
  ...
  have hpcNat : s.pc.toNat = Artifact.submissionArtifact.instructionPC 3697 := by
    rw [hpc, sqRowSgt_pc]; decide
  have hwf : Stepper.WellFormed s.fork (.op .SGT) := by
    rw [hfork]
    exact ⟨by decide, trivial, rfl⟩
  exact Stepper.decodes_of_artifact Artifact.submissionArtifact s 3697 (.op .SGT)
