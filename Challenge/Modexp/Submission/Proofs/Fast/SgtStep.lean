...
only `SGT` of the candidate bytecode: instruction 3601 at pc 4716 (0x126c) in the
square row `sq_row` (`JUMPDEST DUP1 MLOAD DUP1 SWAP15 PUSH0 SGT ...`).

Use: split `sq_row` at the `SGT` — a `Block` for idx 3604..3600 (pc 4664..4669),
`gasSteps_sqRowSgt`, then a `Block` for idx 3602..3644 (pc 4703..).
-/
...
/-- The candidate's only `SGT` is instruction 3601. -/
theorem sqRowSgt_index : Artifact.submissionInstructions[3626]? = some (.op .SGT) := by
  rfl

/-- ... at program counter 4716 (0x126c). -/
theorem sqRowSgt_pc : Artifact.submissionArtifact.instructionPC 3626 = 4730 := by
  rfl

/-- Decoder fact at pc 4716 of the submission bytecode. -/
theorem decodedOp_sqRowSgt (s : State)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hfork : s.fork = .Osaka) (hpc : s.pc = UInt256.ofNat 4730) :
    s.decodedOp = some .SGT := by
  have hpcNat : s.pc.toNat = Artifact.submissionArtifact.instructionPC 3626 := by
    rw [hpc, sqRowSgt_pc]; decide
  have hwf : Stepper.WellFormed s.fork (.op .SGT) := by
    rw [hfork]
    exact ⟨by decide, trivial, rfl⟩
  exact Stepper.decodes_of_artifact Artifact.submissionArtifact s 3626 (.op .SGT)
    hcode hpcNat sqRowSgt_index hwf

theorem succ_4716 : (UInt256.ofNat 4730).succ = UInt256.ofNat 4731 := by
  decide

/-- The `SGT` of `sq_row` (pc 4716 → 4703): pops `a, b`, pushes `UInt256.sgt a b`.
...
def gasSteps_sqRowSgt (s : State) (a b : UInt256) (rest : List UInt256)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hfork : s.fork = .Osaka)
    (hpc : s.pc = UInt256.ofNat 4730)
    (hstack : s.stack = a :: b :: rest)
    (hcap : rest.length ≤ 1021)
    (hrun : s.halt = .Running)
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig s.executionEnv.fork
      s.executionEnv.codeAddr = false) :
    GasSteps s { s with stack := UInt256.sgt a b :: rest, pc := UInt256.ofNat 4731 } := by
...
def gasSteps_sqRowSgt_framed (t : State) (m : ByteArray) (a b : UInt256)
    ...
    GasSteps { t with pc := UInt256.ofNat 4730, stack := a :: b :: rest, memory := m }
      { t with pc := UInt256.ofNat 4731, stack := UInt256.sgt a b :: rest, memory := m } :=
  gasSteps_sqRowSgt { t with pc := UInt256.ofNat 4730, stack := a :: b :: rest, memory := m }
