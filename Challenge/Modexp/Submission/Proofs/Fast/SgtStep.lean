...
theorem sqRowSgt_index : Artifact.submissionInstructions[3052]? = some (.op .SGT) := by
  rfl

/-- ... at program counter 4807 (0x126c). -/
theorem sqRowSgt_pc : Artifact.submissionArtifact.instructionPC 3052 = 4071 := by
...
theorem decodedOp_sqRowSgt (s : State)
  ...
  have hpcNat : s.pc.toNat = Artifact.submissionArtifact.instructionPC 3052 := by
    rw [hpc, sqRowSgt_pc]; decide
  have hwf : Stepper.WellFormed s.fork (.op .SGT) := by
    rw [hfork]
    exact ⟨by decide, trivial, rfl⟩
  exact Stepper.decodes_of_artifact Artifact.submissionArtifact s 3052 (.op .SGT)
