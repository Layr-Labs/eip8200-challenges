...
theorem sqRowSgt_index : Artifact.submissionInstructions[3347]? = some (.op .SGT) := by
  rfl

/-- ... at program counter 4807 (0x126c). -/
theorem sqRowSgt_pc : Artifact.submissionArtifact.instructionPC 3347 = 4442 := by
...
theorem decodedOp_sqRowSgt (s : State)
  ...
  have hpcNat : s.pc.toNat = Artifact.submissionArtifact.instructionPC 3347 := by
    rw [hpc, sqRowSgt_pc]; decide
  have hwf : Stepper.WellFormed s.fork (.op .SGT) := by
    rw [hfork]
    exact ⟨by decide, trivial, rfl⟩
  exact Stepper.decodes_of_artifact Artifact.submissionArtifact s 3347 (.op .SGT)
