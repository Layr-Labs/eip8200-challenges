...
theorem sqRowSgt_index : Artifact.submissionInstructions[3399]? = some (.op .SGT) := by
  rfl

/-- ... at program counter 4835 (0x126c). -/
theorem sqRowSgt_pc : Artifact.submissionArtifact.instructionPC 3399 = 4264 := by
...
theorem decodedOp_sqRowSgt (s : State)
  ...
  have hpcNat : s.pc.toNat = Artifact.submissionArtifact.instructionPC 3399 := by
    rw [hpc, sqRowSgt_pc]; decide
  have hwf : Stepper.WellFormed s.fork (.op .SGT) := by
    rw [hfork]
    exact ⟨by decide, trivial, rfl⟩
  exact Stepper.decodes_of_artifact Artifact.submissionArtifact s 3399 (.op .SGT)
