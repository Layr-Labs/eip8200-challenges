...
theorem sqRowSgt_index : Artifact.submissionInstructions[3361]? = some (.op .SGT) := by
  rfl

/-- ... at program counter 4803 (0x126c). -/
theorem sqRowSgt_pc : Artifact.submissionArtifact.instructionPC 3361 = 4450 := by
  rw [Challenge.Modexp.Submission.Proofs.Bytecode.PCFast.instructionPC_eq_byteLength]
  rfl

theorem decodedOp_sqRowSgt (s : State)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hfork : s.fork = .Osaka) (hpc : s.pc = UInt256.ofNat 4450) :
    s.decodedOp = some .SGT := by
  have hpcNat : s.pc.toNat = Artifact.submissionArtifact.instructionPC 3361 := by
    rw [hpc, sqRowSgt_pc]; decide
  have hwf : Stepper.WellFormed s.fork (.op .SGT) := by
    rw [hfork]
    exact ⟨by decide, trivial, rfl⟩
  exact Stepper.decodes_of_artifact Artifact.submissionArtifact s 3361 (.op .SGT)
    hcode hpcNat sqRowSgt_index hwf

theorem succ_4716 : (UInt256.ofNat 4450).succ = UInt256.ofNat 4451 := by
  decide
...
def gasSteps_sqRowSgt (s : State) (a b : UInt256) (rest : List UInt256)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hfork : s.fork = .Osaka)
    (hpc : s.pc = UInt256.ofNat 4450)
    (hstack : s.stack = a :: b :: rest)
    (hcap : rest.length ≤ 1021)
    (hrun : s.halt = .Running)
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig s.executionEnv.fork
      s.executionEnv.codeAddr = false) :
    GasSteps s { s with stack := UInt256.sgt a b :: rest, pc := UInt256.ofNat 4451 } := by
...
def gasSteps_sqRowSgt_framed (t : State) (m : ByteArray) (a b : UInt256)
    ...
    GasSteps { t with pc := UInt256.ofNat 4450, stack := a :: b :: rest, memory := m }
      { t with pc := UInt256.ofNat 4451, stack := UInt256.sgt a b :: rest, memory := m } :=
  gasSteps_sqRowSgt { t with pc := UInt256.ofNat 4450, stack := a :: b :: rest, memory := m }
