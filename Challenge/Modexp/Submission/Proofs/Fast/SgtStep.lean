...
theorem sqRowSgt_index : Artifact.submissionInstructions[3063]? = some (.op .SGT) := by
  rfl

/-- ... at program counter 4807 (0x126c). -/
theorem sqRowSgt_pc : Artifact.submissionArtifact.instructionPC 3063 = 4071 := by
...
theorem decodedOp_sqRowSgt (s : State)
  ...
  have hpcNat : s.pc.toNat = Artifact.submissionArtifact.instructionPC 3063 := by
    rw [hpc, sqRowSgt_pc]; decide
  have hwf : Stepper.WellFormed s.fork (.op .SGT) := by
    rw [hfork]
    exact ⟨by decide, trivial, rfl⟩
  exact Stepper.decodes_of_artifact Artifact.submissionArtifact s 3063 (.op .SGT)
