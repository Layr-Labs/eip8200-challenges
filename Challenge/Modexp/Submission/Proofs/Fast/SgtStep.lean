...
only `SGT` of the candidate bytecode: instruction 3737 at pc 4819 (0x126c) in the
square row `sq_row` (`JUMPDEST DUP1 MLOAD DUP1 SWAP15 PUSH0 SGT ...`).

Use: split `sq_row` at the `SGT` — a `Block` for idx 3554..3696 (pc 2464..2464),
`gasSteps_sqRowSgt`, then a `Block` for idx 3700..3840 (pc 4934..).
-/
...
(`{ t with pc := 4819, stack := a :: b :: rest, memory := m }`), the form used by
the kernel frames. -/
