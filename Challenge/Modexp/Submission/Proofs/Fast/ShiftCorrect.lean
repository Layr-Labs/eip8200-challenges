...
pc 4077.  When the base is exactly `n` words wide and the modulus has its top
...
theorem jumpD4643 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
    (UInt256.ofNat 4052).toNat = true :=
  Exp.jumpD 4052 (by decide) jumpDest4643
...
theorem handled_of_dispatch (input : ByteArray) (s : State) (mem : ByteArray)
    ...
    ∃ final : State,
      Nonempty (Challenge.EvmProof.GasSteps
        (Exp.r1Call s mem 4096 (UInt256.ofNat 4052) n bsize esize msize) final) ∧
        final.isDone = true ∧
