...
pc 4686.  When the base is exactly `n` words wide and the modulus has its top
...
theorem jumpD4643 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
    (UInt256.ofNat 4686).toNat = true :=
  Exp.jumpD 4686 (by decide) jumpDest4643
...
theorem handled_of_dispatch (input : ByteArray) (s : State) (mem : ByteArray)
    ...
    ∃ final : State,
      Nonempty (Challenge.EvmProof.GasSteps
        (Exp.r1Call s mem 4096 (UInt256.ofNat 4686) n bsize esize msize) final) ∧
        final.isDone = true ∧
