...
theorem run_jump_taken (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocated branchJumpLocated
      (jumpStackState input (UInt256.ofNat 1)) = some (branchState input 5183) := by
  have htrue : UInt256.isTrue (UInt256.ofNat 1) := by decide
  have hjump : Decode.isValidJumpDest submissionBytecode 5183 = true :=
    Artifact.isValidJumpDest_index 2231 (by rfl)
  have hpc : (jumpStackState input (UInt256.ofNat 1)).pc.toNat =
      Artifact.submissionArtifact.instructionPC 2228 := by
    simp [jumpStackState, initialState, guardBN254PC1,
      Challenge.EvmProof.Word.word_toNat_ofNat]
  unfold Challenge.EvmProof.Stepper.runLocated
  change (if (jumpStackState input (UInt256.ofNat 1)).pc.toNat =
      Artifact.submissionArtifact.instructionPC 2228 then
  ...
  rw [if_pos htrue]
  rw [show (UInt256.ofNat 5183).toNat = 5183 by decide]
