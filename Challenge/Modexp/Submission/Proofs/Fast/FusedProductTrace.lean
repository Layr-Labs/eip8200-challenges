...
def gasSteps_rows (s : State) (mem : ByteArray) (n : Nat)
    ...
    Challenge.EvmProof.GasSteps (rowReady s (mpZeroed s mem n) n tl inv m0 m96 m64 m32 rest)
      (mpCsubState s (rowsCarry (mpZeroed s mem n) 2368 256 n n)
        (UInt256.ofNat 256) (UInt256.ofNat 1162) rest) := by
  by_cases h4 : n = 4
  · subst n
    exact CarryFull.gasSteps_rowsFour CarryIface.rowLemmas s mem 2368 256 tl inv m0
      (UInt256.ofNat (2368+32*4-32)) m96 m64 m32 (UInt256.ofNat 256) (UInt256.ofNat 1162)
  ...
  · have h8 : n = 8 := hn.resolve_left h4
    ...
    exact CarryFull.gasSteps_rowsEight CarryIface.rowLemmas s mem 2368 256 tl inv m0
      (UInt256.ofNat (2368+32*8-32)) m96 m64 m32 (UInt256.ofNat 256) (UInt256.ofNat 1162)
      rest hcap hrun hcode hfork hnp hact (Or.inr rfl) (by decide) (by decide)
      hminv hc he rfl (by intro _ _; rfl)
...
def gasSteps_product (s : State) (mem : ByteArray) (p a mm : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (frameAt 3667 s mem (p+2) pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      {s with pc := UInt256.ofNat 1162, stack := rest, memory := StagedProduct.memory s mem (p+2)} := by
  ...
  have hj : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 1162).toNat = true := by
    change Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 1162 = true
    exact Artifact.isValidJumpDest_index 782 (by rfl)
  ...
  have g3 := gasSteps_csubAt s (rowsCarry (mpZeroed s mem (p+2)) 2368 256 (p+2) (p+2))
    (p+2) 256 (UInt256.ofNat 1162) rest (by omega) hrun hcode hfork hnp hact (by omega) hn8
    (by omega) hj
