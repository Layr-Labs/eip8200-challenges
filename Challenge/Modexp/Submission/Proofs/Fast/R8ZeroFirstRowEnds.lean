...
def finishStore : List Instr := [.push 2 2080, .op .MSTORE, .op .POP, .push 0 0]

theorem run_finishStore (s : State) (pc c bi : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 1020) (hact : 88 ≤ s.activeWords.toNat) :
    runInstructions finishStore
      { s with pc := pc, stack := c :: bi :: rest } =
    some { s with pc := advancePC 6 pc, stack := UInt256.ofNat 0 :: rest,
                  memory := MachineState.writeBytes s.memory (Data.Bytes.natToBytesPadded c.toNat 32) 2080 } := by
  ...
  simp [finishStore, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    List.exchange, h0, h1, h2, h3, State.activeWordsAfterUInt256, ht, hT, hz, advancePC]
  simp only [succ_eq_add, word_add_assoc]
  rfl

#print axioms run_diagonal
