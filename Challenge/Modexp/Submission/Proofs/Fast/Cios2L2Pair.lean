...
theorem jumpDest4913 :
  ...
  exact Artifact.isValidJumpDest_index 2882 (by rfl)
...
theorem run_first (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock firstMac
      (l2At 4427 s mid bi mu c0 pa pb n i k pdst ret rest) =
      some (l2At 4473 s mid bi mu c0 pa pb n i (k + 1) pdst ret rest) := by
...
theorem run_secondBody (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    (hact : 296 ≤ s.activeWords.toNat)
    (hn32 : n ≤ 32) (hk : k + 2 < n) :
    Challenge.EvmProof.Stepper.runLocatedBlock secondMacBody
      (l2At 4473 s mid bi mu c0 pa pb n i k pdst ret rest) =
      some (l2At 4427 s mid bi mu c0 pa pb n i (k + 1) pdst ret rest) := by
  have hc10 : rest.length + 10 < 1024 := by omega
  have hc11 : rest.length + 11 < 1024 := by omega
  have hc12 : rest.length + 12 < 1024 := by omega
  have hc13 : rest.length + 13 < 1024 := by omega
  ...
  have h32 : (32 : UInt256) = UInt256.ofNat 32 := by decide
  have h8224 : (8224 : UInt256).toNat = 8224 := by decide
  have h4913 : (4427 : UInt256).toNat = 4427 := by decide
  have h4913' : (4427 : UInt256) = UInt256.ofNat 4427 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (4427 : UInt256).toNat = true := by
    rw [h4913]
    exact jumpDest4913
  have hpmj : ptrAt (32 * n - 64) k %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
    ...
    rw [ptrAt_mod _ _ (by omega) (by omega)]
    omega
  have hpmNonzero : 32 * (n - 2 - k) ≠ 0 := by omega
  have hptj : ptrAt (8192 + 32 * n) k %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      8256 + 32 * (n - 2 - k) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]
  ...
  have hactW : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat
    ...
    activeWords_fix s _ 32 (by decide) (by omega) hact
  simp (config := { maxSteps := 800000 })
    [secondMacBody, secondMac, Cios2Paths.L2Pair.secondPC, Cios2Paths.L2Pair.secondStartIndex,
      ...
      h4913, h4913', hjump, jumpDest4913,
      hpmj, hpmNonzero, hptj, hwr, hwr1, hnextT, hgt, hactM, hactT, hactW,
...
theorem run_secondExit (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
  ...
  have hpmj : ptrAt (32 * n - 64) k %
    ...
    rw [ptrAt_mod _ _ (by omega) (by omega)]
    omega
  have hpmZero : 32 * (n - 2 - k) = 0 := by omega
  have hptj : ptrAt (8192 + 32 * n) k %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      8256 + 32 * (n - 2 - k) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]
  ...
  have hactW : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat
      (8256 + 32 * (n - 1 - k)) 32) = s.activeWords :=
    activeWords_fix s _ 32 (by decide) (by omega) hact
  have hact0 : MachineState.activeWordsAfter s.activeWords.toNat 0 32 =
      s.activeWords.toNat :=
    activeWordsAfter_fix _ 0 32 (by decide) (by decide) hact
  have hact8256 : MachineState.activeWordsAfter s.activeWords.toNat 8256 32 =
      s.activeWords.toNat :=
    activeWordsAfter_fix _ 8256 32 (by decide) (by decide) hact
  have hactWN : MachineState.activeWordsAfter s.activeWords.toNat
      (8256 + 32 * (n - 1 - k)) 32 = s.activeWords.toNat :=
    activeWordsAfter_fix _ _ 32 (by decide) (by omega) hact
  have hactMod : s.activeWords.toNat %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      s.activeWords.toNat :=
  ...
  simp (config := { maxSteps := 800000 })
    [secondMac, Cios2Paths.L2Pair.secondPC, Cios2Paths.L2Pair.secondStartIndex,
      ...
      fastPC13, fastPC14,
      hc10, hc11, hc12, hc13, hc14, hrun, hK, h32, h8224,
      hpmj, hpmZero, hptj, hwr, hwr1, hnextT, hactM, hactT, hactW,
      ptrAt_succ, ptrAt_shift32, UInt256.gt, UInt256.isTrue,
      State.activeWordsAfterUInt256,
      Challenge.EvmProof.Word.succ_ofNat_mod,
      Challenge.EvmProof.Word.ofNat_add_mod,
  ...
  constructor
  · constructor
    · rw [hact0, hactMod, hact8256, hactMod, hactWN]
...
def gasSteps_first (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l2At 4427 s mid bi mu c0 pa pb n i k pdst ret rest)
      (l2At 4473 s mid bi mu c0 pa pb n i (k + 1) pdst ret rest) :=
...
def gasSteps_secondBody (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l2At 4473 s mid bi mu c0 pa pb n i k pdst ret rest)
      (l2At 4427 s mid bi mu c0 pa pb n i (k + 1) pdst ret rest) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka secondMacBody hcode hfork
...
def gasSteps_pairBody (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l2At 4427 s mid bi mu c0 pa pb n i k pdst ret rest)
      (l2At 4427 s mid bi mu c0 pa pb n i (k + 2) pdst ret rest) :=
