...
opaque gasSteps_l2Four (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (CiosCachedTailDefs.cleaned
        { s with memory := (l2Step mid mu c0 4 3).memory }
        (l2Step mid mu c0 4 3).carry
        (UInt256.ofNat (ptrAt (pb + 32 * 4 - 32) i))
        (UInt256.ofNat (pa + 32 * 4 - 32)) (UInt256.ofNat (pb - 32))
        (isFour 4) pdst ret rest) :=
  ...
  gasSteps_l2LastCleanup s mid bi mu c0 pa pb 4 i 2 pdst ret rest hcap hrun
    hcode hfork hnp hact (by decide) (by decide)
...
opaque gasSteps_l2Eight (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (CiosCachedTailDefs.cleaned
        { s with memory := (l2Step mid mu c0 8 7).memory }
        (l2Step mid mu c0 8 7).carry
        (UInt256.ofNat (ptrAt (pb + 32 * 8 - 32) i))
        (UInt256.ofNat (pa + 32 * 8 - 32)) (UInt256.ofNat (pb - 32))
        (isFour 8) pdst ret rest) :=
  ...
  gasSteps_l2LastCleanup s mid bi mu c0 pa pb 8 i 6 pdst ret rest hcap hrun
    hcode hfork hnp hact (by decide) (by decide)

/-- One complete four-limb row, stopping at the common cleaned state. -/
opaque gasSteps_rowFourToTail (s : State) (mem : ByteArray) (pa pb i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (CiosCachedTailDefs.cleaned
        { s with memory := (rowL2 mem pa pb 4 i).memory }
        (rowL2 mem pa pb 4 i).carry
        (UInt256.ofNat (ptrAt (pb + 32 * 4 - 32) i))
        (UInt256.ofNat (pa + 32 * 4 - 32)) (UInt256.ofNat (pb - 32))
        (isFour 4) pdst ret rest) := by
...
/-- One complete eight-limb row, stopping at the common cleaned state. -/
opaque gasSteps_rowEightToTail (s : State) (mem : ByteArray) (pa pb i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (CiosCachedTailDefs.cleaned
        { s with memory := (rowL2 mem pa pb 8 i).memory }
        (rowL2 mem pa pb 8 i).carry
        (UInt256.ofNat (ptrAt (pb + 32 * 8 - 32) i))
        (UInt256.ofNat (pa + 32 * 8 - 32)) (UInt256.ofNat (pb - 32))
        (isFour 8) pdst ret rest) := by
...
opaque gasSteps_rowFourNext (s : State) (mem : ByteArray) (pa pb i : Nat)
  ...
  (gasSteps_rowFourToTail s mem pa pb i pdst ret rest hcap hrun hcode hfork hnp
    ...
    by simpa only [rowMem] using
      gasSteps_tailNextAfterCleanup s (rowL2 mem pa pb 4 i).memory
        (rowL2 mem pa pb 4 i).carry pa pb 4 i pdst ret rest
        hcap hrun hcode hfork hnp hact hi hpb hpbFit
...
opaque gasSteps_rowFourLast (s : State) (mem : ByteArray) (pa pb i : Nat)
  ...
  (gasSteps_rowFourToTail s mem pa pb i pdst ret rest hcap hrun hcode hfork hnp
      hact (by omega) hpa hpaFit hpb hpbFit hs32 htl hml).trans <|
    by simpa only [rowMem] using
      gasSteps_tailLastAfterCleanup s (rowL2 mem pa pb 4 i).memory
        (rowL2 mem pa pb 4 i).carry pa pb 4 i pdst ret rest
        hcap hrun hcode hfork hnp hact hi hpb hpbFit
...
opaque gasSteps_rowEightNext (s : State) (mem : ByteArray) (pa pb i : Nat)
  ...
  (gasSteps_rowEightToTail s mem pa pb i pdst ret rest hcap hrun hcode hfork hnp
      hact (by omega) hpa hpaFit hpb hpbFit hs32 htl hml).trans <|
    by simpa only [rowMem] using
      gasSteps_tailNextAfterCleanup s (rowL2 mem pa pb 8 i).memory
        (rowL2 mem pa pb 8 i).carry pa pb 8 i pdst ret rest
        hcap hrun hcode hfork hnp hact hi hpb hpbFit
...
opaque gasSteps_rowEightLast (s : State) (mem : ByteArray) (pa pb i : Nat)
  ...
  (gasSteps_rowEightToTail s mem pa pb i pdst ret rest hcap hrun hcode hfork hnp
      hact (by omega) hpa hpaFit hpb hpbFit hs32 htl hml).trans <|
    by simpa only [rowMem] using
      gasSteps_tailLastAfterCleanup s (rowL2 mem pa pb 8 i).memory
        (rowL2 mem pa pb 8 i).carry pa pb 8 i pdst ret rest
        hcap hrun hcode hfork hnp hact hi hpb hpbFit
