...
The first copied MAC ends at pc 4179.  Two no-test middle copies follow, and
the final copied MAC starts at pc 4263 and owns the shared four-way loop test.
...
/-- Compact literal encodings used by the CIOS2 instruction traces. -/
@[simp] theorem compact_lnot_zero :
    UInt256.lnot ({ val := 0 } : UInt256) = maxWord := by decide

@[simp] theorem compact_lnot_31 :
    UInt256.lnot (31 : UInt256) = UInt256.ofNat
...
theorem run_l1FirstMac (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock firstMac
      ...
      some (l1At 4179 s mem bi pa pb n i (j + 1) pdst ret rest) := by
...
theorem run_l1SecondMacBody (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock secondMac
      (l1At 4263 s mem bi pa pb n i k pdst ret rest) =
...
def gasSteps_l1FirstMac (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 4136 s mem bi pa pb n i j pdst ret rest)
      (l1At 4179 s mem bi pa pb n i (j + 1) pdst ret rest) :=
