...
def midState (s : State) (mem : ByteArray) (paj ptj c bi : UInt256)
  ...
  { s with pc := UInt256.ofNat 4790
           stack := [paj, ptj, c, bi, UInt256.ofNat (ptrAt (pb + 32 * n - 32) i),
                     UInt256.ofNat (pa - 32), UInt256.ofNat (pb - 32), pdst, ret] ++ rest
           memory := mem }

theorem jumpDest4136 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4250 = true := by
...
theorem run_l1FirstMac (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock firstMac
      (l1At 4250 s mem bi pa pb n i j pdst ret rest) =
      some (l1At 4384 s mem bi pa pb n i (j + 1) pdst ret rest) := by
...
theorem run_l1SecondMacBody (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock secondMacBody
      (l1At 4650 s mem bi pa pb n i k pdst ret rest) =
      some (l1At 4250 s mem bi pa pb n i (k + 1) pdst ret rest) := by
  have hc9 : rest.length + 9 < 1024 := by omega
  have hc10 : rest.length + 10 < 1024 := by omega
  have hc11 : rest.length + 11 < 1024 := by omega
  have hc12 : rest.length + 12 < 1024 := by omega
  ...
  have hK :
      (115792089237316195423570985008687907853269984665640564039457584007913129639904 :
        UInt256) = UInt256.ofNat
          115792089237316195423570985008687907853269984665640564039457584007913129639904 := by
    decide
  have h4136 : (4250 : UInt256).toNat = 4250 := by decide
  have h4136' : (4250 : UInt256) = UInt256.ofNat 4250 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (4250 : UInt256).toNat = true := by
    rw [h4136]
    exact jumpDest4136
...
opaque gasSteps_l1FirstMac (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 4250 s mem bi pa pb n i j pdst ret rest)
      (l1At 4384 s mem bi pa pb n i (j + 1) pdst ret rest) :=
...
opaque gasSteps_l1SecondMacBody (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 4650 s mem bi pa pb n i k pdst ret rest)
      (l1At 4250 s mem bi pa pb n i (k + 1) pdst ret rest) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
