...
theorem run_jump_taken (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocated branchJumpLocated
      (jumpStackState input (UInt256.ofNat 1)) = some (branchState input 4986) := by
  have htrue : UInt256.isTrue (UInt256.ofNat 1) := by decide
  have hjump : Decode.isValidJumpDest submissionBytecode 4986 = true :=
  ...
  rw [if_pos htrue]
  rw [show (UInt256.ofNat 4986).toNat = 4986 by decide]
