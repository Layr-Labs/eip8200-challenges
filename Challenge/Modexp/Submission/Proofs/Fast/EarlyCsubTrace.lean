...
def gasSteps_csub (s : State) (memory : ByteArray) (n : Nat)
    ...
    (hs32 : MachineState.readWord (csStep memory n n).memory 2784 = UInt256.ofNat (32*n))
  ...
  · rw [if_pos hskip] at hc
    have hj := jumpBlock.steps
      (environment (atState s memory 4685 pdst ret rest) hcode hfork hrun hnp) rfl
    ...
    have hk := copyBlock.steps
      (environment (atState s memory 5087 pdst ret rest) hcode hfork hrun hnp) rfl
