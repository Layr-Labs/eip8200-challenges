...
`LZ` (pc 2556) is the head of the exponent-byte loop body.  It loads exponent
...
* `blk1781` (idx 1781..1792, pc 2556..2933) — the byte load and the `i = 0`
  test;
* `blk1793` (idx 1793..1795, pc 2572..2939) — `PUSH1 128` and the jump back
  into the bit loop;
* `blk1796` (idx 1796..1815, pc 2578..2966) — the fold
...
/-- Instructions 1781..1792, pc 2556..2933: load exponent byte `i`, test `i = 0`. -/
def blk1781 :
...
/-- Instructions 1793..1795, pc 2572..2939: every byte after the first. -/
def blk1793 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 1577 1 128,
   pushAt 1578 2 1486,
...
/-- Instructions 1796..1815, pc 2578..2966: the highest set bit of byte `0`. -/
def blk1796 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 1580 .JUMPDEST,
