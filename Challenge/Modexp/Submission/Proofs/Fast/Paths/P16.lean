...
`LZ` (pc 2683) is the head of the exponent-byte loop body.  It loads exponent
...
* `blk1781` (idx 1911..1792, pc 2683..3060) — the byte load and the `i = 0`
  test;
* `blk1793` (idx 1793..1925, pc 2699..3066) — `PUSH1 128` and the jump back
  into the bit loop;
* `blk1796` (idx 1878..1945, pc 2705..3093) — the fold
...
/-- Instructions 1911..1792, pc 2683..3060: load exponent byte `i`, test `i = 0`. -/
def blk1781 :
...
/-- Instructions 1793..1925, pc 2699..3066: every byte after the first. -/
def blk1793 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 1650 1 128,
   pushAt 1651 2 1616,
...
/-- Instructions 1878..1945, pc 2705..3093: the highest set bit of byte `0`. -/
def blk1796 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 1653 .JUMPDEST,
