import Challenge.Modexp.Submission.Proofs.Fast.Defs
set_option warningAsError true
set_option maxRecDepth 40000
set_option maxHeartbeats 4000000
/-! Basic-block instruction paths, group 0 (instructions 977..1027). -/

namespace Challenge.Modexp.Submission.Proofs.Fast

open EvmSemantics
open EvmSemantics.EVM
open Challenge.Modexp.Submission.Proofs.Bytecode

/-- Instructions 977..985, pc 1314..1326. -/
def blk977 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 495 .JUMPDEST,
   pushAt 496 1 64,
   opAt 497 .CALLDATALOAD,
   opAt 498 (.Dup ⟨0, by decide⟩),
   pushAt 499 1 33,
   opAt 500 (.Swap ⟨0, by decide⟩),
   opAt 501 .SUB,
   pushAt 502 1 223,
   opAt 503 .LT,
   pushAt 504 2 1174,
   opAt 505 .JUMPI]

/-- Instructions 986..989, pc 1133..1137.  The EIP-7823 oversize test that used to
follow the two header loads (13 instructions, `DUP3 PUSH2 1024 LT DUP3 PUSH2 1024 LT OR
DUP2 PUSH2 1024 LT OR PUSH2 <BAIL3> JUMPI`) is gone from the bytecode: `ValidInput`
bounds every declared size by 1024, so its `JUMPI` was never taken.  The block now falls
straight through to the top-limb block at instruction 872, pc 1138. -/
def blk986 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 506 1 32,
   opAt 507 .CALLDATALOAD,
   pushAt 508 0 0,
   opAt 509 .CALLDATALOAD]

/-- Instructions 1003..1027, pc 1353..1384. -/
def blk1003 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 510 (.Dup ⟨2, by decide⟩),
   pushAt 511 1 31,
   opAt 512 .ADD,
   pushAt 513 1 5,
   opAt 514 .SHR,
   opAt 515 (.Dup ⟨0, by decide⟩),
   pushAt 516 1 5,
   opAt 517 .SHL,
   opAt 518 (.Dup ⟨3, by decide⟩),
   opAt 519 (.Dup ⟨3, by decide⟩),
   opAt 520 .ADD,
   pushAt 521 1 96,
   opAt 522 .ADD,
   opAt 523 (.Dup ⟨5, by decide⟩),
   opAt 524 (.Dup ⟨2, by decide⟩),
   opAt 525 .SUB,
   pushAt 526 1 3,
   opAt 527 .SHL,
   opAt 528 (.Dup ⟨1, by decide⟩),
   opAt 529 .CALLDATALOAD,
   opAt 530 (.Swap ⟨0, by decide⟩),
   opAt 531 .SHR,
   opAt 532 .ISZERO,
   pushAt 533 2 1180,
   opAt 534 .JUMPI]

end Challenge.Modexp.Submission.Proofs.Fast
