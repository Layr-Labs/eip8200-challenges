...
`R1B` (pc 2662) sits in front of the first `DOUBLE256` call.  When the
...
* `blk1768` (idx 1898..1905, pc 2662..3034) — `JUMPDEST`, the top-bit test
  `MLOAD 0; PUSH1 255; SHR; ISZERO` and the `JUMPI` back to `DOUBLE256`;
* `blk1776` (idx 1824..1910, pc 2673..3003) — `MSTORE TN 1` and the tail call
  into `CSUB` (pc 2436).
...
/-- Instructions 1898..1905, pc 2662..3034: the top-bit test and the branch
back into `DOUBLE256`. -/
def blk1768 :
...
/-- Instructions 1824..1910, pc 2673..3003: `t[n] := 1` and the tail call into
`CSUB`. -/
def blk1776 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 1633 1 1,
