...
`R1B` (pc 2535) sits in front of the first `DOUBLE256` call.  When the
...
* `blk1768` (idx 1768..1775, pc 2535..2907) — `JUMPDEST`, the top-bit test
  `MLOAD 0; PUSH1 255; SHR; ISZERO` and the `JUMPI` back to `DOUBLE256`;
* `blk1776` (idx 1776..1780, pc 2546..2921) — `MSTORE TN 1` and the tail call
  into `CSUB` (pc 2309).
...
/-- Instructions 1768..1775, pc 2535..2907: the top-bit test and the branch
back into `DOUBLE256`. -/
def blk1768 :
...
/-- Instructions 1776..1780, pc 2546..2921: `t[n] := 1` and the tail call into
`CSUB`. -/
def blk1776 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 1560 1 1,
