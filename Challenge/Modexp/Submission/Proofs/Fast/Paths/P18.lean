...
`LZBASE` (pc 4034) is reached from `LZ`'s byte-0 arm with `[mask, w, i]`.  The
...
* `blk2557` (idx 2557..2561, pc 4034..3635) — the `w = 0` test;
* `blk2562` (idx 2562..2568, pc 3841..3650) — the copy and the resume;
...
/-- Instructions 2557..2561, pc 4034..3635: the `w = 0` test. -/
def blk2557 :
...
/-- Instructions 2562..2568, pc 3841..3650: `ACC := BASE`, then the shift. -/
def blk2562 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [pushAt 2544 2 2784,
   opAt 2545 .MLOAD,
