...
The live pc-2467 entry jumps to pc 5299.  The appended routine walks the input
...
def entryState (s : State) (memory : ByteArray) (pa pb : Nat)
  ...
  { s with pc := UInt256.ofNat 5299
           stack := [UInt256.ofNat pa, UInt256.ofNat pb, pd, ret] ++ rest
           memory := memory }

/-- Fused loop head at pc 5321 after `j` completed limbs. -/
def loopState (s : State) (memory : ByteArray) (pa pb n j : Nat)
    (pd ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 5321
...
/-- Fused tail at pc 5385 after all `n` limbs. -/
def tailState (s : State) (memory : ByteArray) (pa pb n j : Nat)
    (pd ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 5385
           stack := [UInt256.ofNat (8224 + 32 * (n - j)),
                     (limbStep memory pa pb n j).carry,
                     (limbStep memory pa pb n j).borrow,
                     addrDelta pa, addrDelta pb, pd, ret] ++ rest
...
theorem run_trampoline (s : State) (memory : ByteArray) (pa pb : Nat)
  ...
  have h4057 : (5299 : UInt256).toNat = 5299 := by decide
  have h4057' : (5299 : UInt256) = UInt256.ofNat 5299 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (5299 : UInt256).toNat = true := by
    rw [h4057]
    exact jumpDest5299
  simp (config := { maxSteps := 100000 })
    [blk1600, opAt, pushAt, wfOp,
      ...
      h4057', hjump,
      jumpDest5299, Challenge.EvmProof.Word.succ_ofNat_mod,
...
theorem run_entry (s : State) (memory : ByteArray) (pa pb n : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk3398
...
theorem run_loop_body (s : State) (memory : ByteArray) (pa pb n j : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk3414
  ...
  have h4079 : (5321 : UInt256).toNat = 5321 := by decide
  have h4079' : (5321 : UInt256) = UInt256.ofNat 5321 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (5321 : UInt256).toNat = true := by
    rw [h4079]
    exact jumpDest5321
  ...
  simp (config := { maxSteps := 800000 })
    [blk3414, opAt, pushAt, wfOp,
      ...
      jumpDest5321, hactA, hactB, hactM, hactD, hactT,
...
theorem run_loop_exit (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  have h4143' : (5385 : UInt256) = UInt256.ofNat 5385 := by decide
  have haddrA : (pa + 32 * (n - 1 - j)) % 2 ^ 256 =
      pa + 32 * (n - 1 - j) := Nat.mod_eq_of_lt (by omega)
  have haddrB : (pb + 32 * (n - 1 - j)) % 2 ^ 256 =
      pb + 32 * (n - 1 - j) := Nat.mod_eq_of_lt (by omega)
...
theorem run_tail (s : State) (memory : ByteArray) (pa pb n j : Nat)
    ...
    (hcarry : (limbStep memory pa pb n j).carry.toNat ≤ 1) :
    Challenge.EvmProof.Stepper.runLocatedBlock blk3469
  ...
  have h4169 : (5411 : UInt256).toNat = 5411 := by decide
  have h4170 : (5412 : UInt256).toNat = 5412 := by decide
  have h4171 : (5413 : UInt256).toNat = 5413 := by decide
  have h4172 : (5414 : UInt256).toNat = 5414 := by decide
  have h4173 : (5415 : UInt256).toNat = 5415 := by decide
...
def gasSteps_entry (s : State) (memory : ByteArray) (pa pb n : Nat)
  ...
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka blk3398
...
def gasSteps_iteration (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka blk3414
...
def gasSteps_tail (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka blk3469
