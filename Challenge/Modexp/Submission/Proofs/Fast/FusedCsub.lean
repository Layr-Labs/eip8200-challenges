...
The live pc-2467 entry jumps to pc 4569.  The appended routine walks the input
...
def entryState (s : State) (memory : ByteArray) (pa pb : Nat)
  ...
  { s with pc := UInt256.ofNat 4569
           stack := [UInt256.ofNat pa, UInt256.ofNat pb, pd, ret] ++ rest
           memory := memory }

/-- Fused loop head at pc 4591 after `j` completed limbs. -/
def loopState (s : State) (memory : ByteArray) (pa pb n j : Nat)
    (pd ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 4591
...
/-- Fused tail at pc 4655 after all `n` limbs. -/
def tailState (s : State) (memory : ByteArray) (pa pb n j : Nat)
    (pd ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 4655
           stack := [UInt256.ofNat (8224 + 32 * (n - j)),
                     (limbStep memory pa pb n j).carry,
                     (limbStep memory pa pb n j).borrow,
                     addrDelta pa, addrDelta pb, pd, ret] ++ rest
...
theorem run_trampoline (s : State) (memory : ByteArray) (pa pb : Nat)
  ...
  have h4057 : (4569 : UInt256).toNat = 4569 := by decide
  have h4057' : (4569 : UInt256) = UInt256.ofNat 4569 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (4569 : UInt256).toNat = true := by
    rw [h4057]
    exact jumpDest5267
  simp (config := { maxSteps := 100000 })
    [blk1600, opAt, pushAt, wfOp,
      ...
      h4057', hjump,
      jumpDest5267, Challenge.EvmProof.Word.succ_ofNat_mod,
...
theorem run_entry (s : State) (memory : ByteArray) (pa pb n : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk3085
...
theorem run_loop_body (s : State) (memory : ByteArray) (pa pb n j : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk3101
  ...
  have h4079 : (4591 : UInt256).toNat = 4591 := by decide
  have h4079' : (4591 : UInt256) = UInt256.ofNat 4591 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (4591 : UInt256).toNat = true := by
    rw [h4079]
    exact jumpDest5289
  ...
  simp (config := { maxSteps := 800000 })
    [blk3101, opAt, pushAt, wfOp,
      ...
      jumpDest5289, hactA, hactB, hactM, hactD, hactT,
...
theorem run_loop_exit (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  have h4143' : (4655 : UInt256) = UInt256.ofNat 4655 := by decide
  have haddrA : (pa + 32 * (n - 1 - j)) % 2 ^ 256 =
      pa + 32 * (n - 1 - j) := Nat.mod_eq_of_lt (by omega)
  have haddrB : (pb + 32 * (n - 1 - j)) % 2 ^ 256 =
      pb + 32 * (n - 1 - j) := Nat.mod_eq_of_lt (by omega)
...
theorem run_tail (s : State) (memory : ByteArray) (pa pb n j : Nat)
    ...
    (hcarry : (limbStep memory pa pb n j).carry.toNat ≤ 1) :
    Challenge.EvmProof.Stepper.runLocatedBlock blk3156
  ...
  have h4169 : (4719 : UInt256).toNat = 4719 := by decide
  have h4170 : (4720 : UInt256).toNat = 4720 := by decide
  have h4171 : (5419 : UInt256).toNat = 5419 := by decide
  have h4172 : (5420 : UInt256).toNat = 5420 := by decide
  have h4173 : (4723 : UInt256).toNat = 4723 := by decide
...
def gasSteps_entry (s : State) (memory : ByteArray) (pa pb n : Nat)
  ...
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka blk3085
...
def gasSteps_iteration (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka blk3101
...
def gasSteps_tail (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka blk3156
