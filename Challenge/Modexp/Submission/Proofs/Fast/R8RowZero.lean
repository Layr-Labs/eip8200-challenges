...
def program : List Instr := R8ZeroFirstRow.program (UInt256.ofNat 3527)

def block : Block Artifact.submissionArtifact .Osaka 4999 program :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 4026 213 4999 program
    (by decide) (by rw [PCFast.instructionPC_eq_byteLength]; rfl) (by rfl) (by decide)

theorem jumpDest : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4999 = true :=
  Artifact.isValidJumpDest_index 4026 (by rfl)

/-- The second-loop entry the row jumps to (`l2Target 8`). -/
theorem jumpDestL2 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 3778 = true :=
  Artifact.isValidJumpDest_index 3050 (by rfl)

theorem l2Target_eight_toNat : (l2Target 8).toNat = 3778 := by decide
...
theorem run_rowZero (s : State) (mem : ByteArray) (pc hd : UInt256) (e : Nat)
    ...
    (hjump : Decode.isValidJumpDest s.executionEnv.code 3778 = true) :
    runInstructions program
      { outState s mem 2368 8 0 hd (UInt256.ofNat e) inv m0
        (UInt256.ofNat 2336 :: m96 :: m64 :: m32 :: aprev :: rest) with pc := pc } =
    some (R8ZeroFirstRow.result { s with memory := mem } hd (UInt256.ofNat 3527) negative32
  ...
  exact R8ZeroFirstRow.run_program { s with memory := mem } pc hd (UInt256.ofNat e)
    (UInt256.ofNat 3527) negative32 (l2Target 8) inv m0 m96 m64 m32 aprev rest hcap hact hj
...
def gasSteps_prologue (s : State) (mem : ByteArray) (e : Nat)
    ...
    Challenge.EvmProof.GasSteps
      { outState s mem 2368 8 0 (UInt256.ofNat 4175) (UInt256.ofNat e) inv m0
        (UInt256.ofNat 2336 :: m96 :: m64 :: m32 :: aprev :: rest) with pc := UInt256.ofNat 4999 }
      (R8ZeroFirstRow.result { s with memory := mem } (UInt256.ofNat 4175) (UInt256.ofNat 3527)
        negative32 (l2Target 8) inv m0 m96 m64 m32 rest) :=
  SquareRow.stepsOf block
    (run_rowZero s mem (UInt256.ofNat 4999) (UInt256.ofNat 4175) e inv m0 m96 m64 m32 aprev rest
