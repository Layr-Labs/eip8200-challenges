...
def program : List Instr := R8ZeroFirstRow.program (UInt256.ofNat 3582)

def block : Block Artifact.submissionArtifact .Osaka 5090 program :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3907 214 5090 program
    (by decide) (by rw [PCFast.instructionPC_eq_byteLength]; rfl) (by rfl) (by decide)

theorem jumpDest : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 5090 = true :=
  Artifact.isValidJumpDest_index 3907 (by rfl)

/-- The second-loop entry the row jumps to (`l2Target 8`). -/
theorem jumpDestL2 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 3833 = true :=
  Artifact.isValidJumpDest_index 2891 (by rfl)

theorem l2Target_eight_toNat : (l2Target 8).toNat = 3833 := by decide
...
theorem run_rowZero (s : State) (mem : ByteArray) (pc hd : UInt256) (e : Nat)
    ...
    (hjump : Decode.isValidJumpDest s.executionEnv.code 3833 = true) :
    runInstructions program
      { outState s mem 2368 8 0 hd (UInt256.ofNat e) inv m0
        (UInt256.ofNat 2336 :: m96 :: m64 :: m32 :: aprev :: rest) with pc := pc } =
    some (R8ZeroFirstRow.result { s with memory := mem } hd (UInt256.ofNat 3582) negative32
  ...
  exact R8ZeroFirstRow.run_program { s with memory := mem } pc hd (UInt256.ofNat e)
    (UInt256.ofNat 3582) negative32 (l2Target 8) inv m0 m96 m64 m32 aprev rest hcap hact hj
...
def gasSteps_prologue (s : State) (mem : ByteArray) (e : Nat)
    ...
    Challenge.EvmProof.GasSteps
      { outState s mem 2368 8 0 (UInt256.ofNat 4230) (UInt256.ofNat e) inv m0
        (UInt256.ofNat 2336 :: m96 :: m64 :: m32 :: aprev :: rest) with pc := UInt256.ofNat 5090 }
      (R8ZeroFirstRow.result { s with memory := mem } (UInt256.ofNat 4230) (UInt256.ofNat 3582)
        negative32 (l2Target 8) inv m0 m96 m64 m32 rest) :=
  SquareRow.stepsOf block
    (run_rowZero s mem (UInt256.ofNat 5090) (UInt256.ofNat 4230) e inv m0 m96 m64 m32 aprev rest
