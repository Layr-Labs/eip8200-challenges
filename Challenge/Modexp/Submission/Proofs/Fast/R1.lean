...
`R1B` occupies instruction indices 1763..1775 (pc 2894..2914).  It is entered
at pc 2894 with stack `[px, ret]`, exactly the calling convention of
...
* otherwise it stores `1` at `TN = 0x2020` and jumps to `CSUB` (pc 2635) with
...
/-- Subroutine entry, pc 2894, stack `[px, ret]`. -/
...
/-- Between the test and the store, pc 2905. -/
def fastState (s : State) (mem : ByteArray) (px : Nat) (ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2905
           stack := [UInt256.ofNat px, ret] ++ rest
           memory := mem }

/-- The `CSUB` entry, pc 2635, stack `[px, ret]`, with `t[n] = 1` stored.
