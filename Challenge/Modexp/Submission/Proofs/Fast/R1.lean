...
`R1B` occupies instruction indices 1793..1805 (pc 2930..2950).  It is entered
at pc 2930 with stack `[px, ret]`, exactly the calling convention of
...
* otherwise it stores `1` at `TN = 0x2020` and jumps to `CSUB` (pc 2671) with
...
/-- Subroutine entry, pc 2930, stack `[px, ret]`. -/
...
/-- Between the test and the store, pc 2941. -/
def fastState (s : State) (mem : ByteArray) (px : Nat) (ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2941
           stack := [UInt256.ofNat px, ret] ++ rest
           memory := mem }

/-- The `CSUB` entry, pc 2671, stack `[px, ret]`, with `t[n] = 1` stored.
