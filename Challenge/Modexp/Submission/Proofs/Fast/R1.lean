...
`R1B` occupies instruction indices 1898..1910 (pc 2677..3018).  It is entered
at pc 2677 with stack `[px, ret]`, exactly the calling convention of
...
* otherwise it stores `1` at `TN = 0x2020` and jumps to `CSUB` (pc 2435) with
...
/-- Subroutine entry, pc 2677, stack `[px, ret]`. -/
...
/-- The `CSUB` entry, pc 2435, stack `[px, ret]`, with `t[n] = 1` stored.
