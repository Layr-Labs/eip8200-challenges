...
`ADDMOD` occupies instruction indices 1625..1691 (pc 2496..2670) and `CSUB`
indices 1692..1766 (pc 2671..2891); `ADDMOD` falls through into `CSUB`.
...
/-- Subroutine entry (pc 2496) with stack `[pa, pb, pd, ret]`. -/
...
/-- The `ADDMOD` loop head (pc 2529) after `j` limb steps. -/
def amLoopState (s : State) (memory : ByteArray) (pa pb n j : Nat)
    (pd ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2529
...
theorem run_amEntry (s : State) (memory : ByteArray) (pa pb n : Nat)
  ...
  simp (config := { maxSteps := 800000 })
    [blk1600, opAt, pushAt, wfOp,
      ...
      amEntryState, amLoopState, amStep, fastPC16,
...
theorem run_amLoopBody (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  have h2529 : (2529 : UInt256).toNat = 2529 := by decide
  have h2529' : (2529 : UInt256) = UInt256.ofNat 2529 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (2529 : UInt256).toNat = true := by
    rw [h2529]; exact jumpDest2500
  ...
  simp (config := { maxSteps := 400000 })
    [blk1627, opAt, pushAt, wfOp,
      ...
      hc6, hc7, hc8, hc9, hrun, hcode, hK, h8224, h2529, h2529', hjump, jumpDest2500,
...
/-- Entry of `CSUB` (pc 2671) with stack `[pd, ret]`. -/
...
/-- The `CSUB` loop head (pc 2695) after `j` limb steps. -/
def csLoopState (s : State) (memory : ByteArray) (n j : Nat)
    (pdst ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2695
           stack := [UInt256.ofNat (ptrAt (8224 + 32 * n) j),
...
/-- The `CSUB` loop exit (pc 2836). -/
def csTailState (s : State) (memory : ByteArray) (n j : Nat)
    (pdst ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2836
           stack := [UInt256.ofNat (ptrAt (8224 + 32 * n) j),
                     UInt256.ofNat (ptrAt (32 * n - 32) j),
                     UInt256.ofNat (ptrAt (7136 + 32 * n) j),
                     (csStep memory n j).flag, pdst, ret] ++ rest
...
theorem run_csEntry (s : State) (memory : ByteArray) (n : Nat)
  ...
  simp (config := { maxSteps := 400000 })
    [blk1667, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      csEntryState, csLoopState, csStep, fastPC17, fastPC18,
...
theorem run_csLoopBody (s : State) (memory : ByteArray) (n j : Nat)
  ...
  have hK : (115792089237316195423570985008687907853269984665640564039457584007913129639904 :
    ...
    decide
  have h8224 : (8224 : UInt256).toNat = 8224 := by decide
  have h2695 : (2695 : UInt256).toNat = 2695 := by decide
  have h2695' : (2695 : UInt256) = UInt256.ofNat 2695 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (2695 : UInt256).toNat = true := by
    rw [h2695]; exact jumpDest2666
  ...
  simp (config := { maxSteps := 400000 })
    [blk1683, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      csLoopState, csStep, fastPC18, fastPC19,
      hc6, hc7, hc8, hc9, hc10, hrun, hcode, hK, h8224, h2695, h2695', hjump,
...
theorem run_csLoopExit (s : State) (memory : ByteArray) (n j : Nat)
  ...
  simp (config := { maxSteps := 400000 })
    [blk1683, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      csLoopState, csTailState, csStep, fastPC18, fastPC19,
...
theorem run_csTail (s : State) (memory : ByteArray) (n j : Nat)
  ...
  simp (config := { maxSteps := 400000 })
    [blk1724, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      csTailState, csReturnedState, hsrcEq, fastPC19,
