...
def loadProgram : List Instr :=
  [.op .JUMPDEST, .push 2 9408, .op .MLOAD, .op .MLOAD,
   .push 1 31, .op .NOT, .push 2 9344, .op .MLOAD,
   .push 1 128, .op .EQ, .push 1 150, .op .MUL,
   .op (.Dup ⟨0, by decide⟩)]

def shuffleProgram : List Instr :=
  [.push 2 4229, .op .ADD, .op (.Swap ⟨0, by decide⟩),
   .push 2 4570, .op .ADD, .op (.Swap ⟨3, by decide⟩),
   .op (.Swap ⟨4, by decide⟩), .push 0 0, .op .NOT,
   .op (.Swap ⟨3, by decide⟩), .op (.Swap ⟨5, by decide⟩)]
...
theorem run_load (s : State) (pa pb dst ret : UInt256) (rest : List UInt256)
    (addr : Nat) (hcap : rest.length ≤ 1004) (hact : 296 ≤ s.activeWords.toNat)
    (haddr : addr + 32 ≤ 9472)
    ...
    some {s with
        pc := UInt256.ofNat 4180
        stack := [displacement s.memory, displacement s.memory, negative32,
        MachineState.readWord s.memory addr, pa, pb, dst, ret] ++ rest} := by
  ...
  simp [loadProgram, runInstructions,
    ...
    ← negative32_not, hml, htonat, hc4, hc5, hc6, hc7, hc8, hc9, hc10, hc11, hc12, h9408, h9344,
    State.activeWordsAfterUInt256, hact1, hact2, hact3,
    Challenge.EvmProof.Word.succ_ofNat_mod, Challenge.EvmProof.Word.ofNat_add_mod]
  exact ⟨rfl, rfl⟩

theorem run_shuffle (s : State) (pa pb dst ret value delta : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 1004) :
    runInstructions shuffleProgram
      {s with
        pc := UInt256.ofNat 4180
        stack := [delta, delta, negative32, value, pa, pb, dst, ret] ++ rest} =
    some {s with
        pc := UInt256.ofNat 4195
        stack := [pa, pb, UInt256.ofNat 4229 + delta, negative32, allOnes,
        UInt256.ofNat 4570 + delta, value, dst, ret] ++ rest} := by
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  have hc7 : rest.length + 7 < 1024 := by omega
  ...
  simp [shuffleProgram, runInstructions,
    Challenge.EvmProof.Stepper.runInstr, hc4, hc5, hc6, hc7, hc8, hc9, hc10, hc11, hc12, ← allOnes_not, List.exchange,
    Challenge.EvmProof.Word.succ_ofNat_mod, Challenge.EvmProof.Word.ofNat_add_mod]
  exact ⟨rfl, rfl, rfl⟩
