...
theorem run_load (s : State) (pa pb dst ret : UInt256) (rest : List UInt256)
    ...
    some {s with
        ...
        MachineState.readWord s.memory 9376, MachineState.readWord s.memory 9440, MachineState.readWord s.memory addr, pa, pb, dst, ret] ++ rest} := by
...
theorem run_shuffle (s : State) (pa pb dst ret value inverse tailPointer delta : UInt256)
    ...
    runInstructions shuffleProgram
      {s with
        pc := UInt256.ofNat 4188
        stack := [delta, delta, negative32, inverse, tailPointer, value, pa, pb, dst, ret] ++ rest} =
    some {s with
        pc := UInt256.ofNat 4203
        stack := [pa, pb, UInt256.ofNat 4237 + delta, negative32, allOnes,
        UInt256.ofNat 4572 + delta, value, inverse, tailPointer, dst, ret] ++ rest} := by
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  have hc7 : rest.length + 7 < 1024 := by omega
