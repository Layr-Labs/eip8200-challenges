...
namespace Challenge.Modexp.Submission.Proofs.Fast.TnR8Reduction
...
noncomputable def reduction_steps (s : State)
    ...
    (hc : ReadonlyCache q.memory 8 tl inv m0)
    (he : TnCacheExtraTrace.ExtraCache q.memory m96 m64 m32)
    (hi : inverseInvariant q.memory 8)
    (hjd : Decode.isValidJumpDest s.executionEnv.code hd.toNat = true) :
    GasSteps
      (framed {s with memory := q.memory} (UInt256.ofNat 3643)
      ...
      (if UInt256.isTrue (UInt256.gt (negative32+pbi) pb) then hd
       else UInt256.ofNat ((l2PC)+TnCacheL2Chain.fullSize 8+51))
      (TnCacheFrameOps.frame (negative32+pbi) hd pb ent
        (TnCacheSquareModel.fromL1 q tn 8).tn (UInt256.ofNat (l2PC)) inv
  ...
  have hmid := TnCacheRowTrace.run_middle 3643 st q.carry bi pbi hd pb ent tn
    (UInt256.ofNat (l2PC)) tl inv m0 aEnd m96 m64 m32 dst ret 8 rest hcap (by omega) hact hc hi
  have hmidRun : runInstructions TnCacheRowTrace.middle
      (framed st (UInt256.ofNat 3643)
        ([q.carry,bi,pbi,hd,pb,ent,tn,allOnes,UInt256.ofNat l2PC,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest)) =
    ...
    simpa only [TnCacheL2Trace.state, l2Step, tail, st, framed, l2PC,
      List.cons_append, List.nil_append] using hmid
  have hreduce := TnCacheL2Chain.run_full st (l2PC) q.memory flag (rowMu q.memory 8)
    (rowC0 q.memory 8) 8 (Or.inr rfl) pbi hd pb ent (tn+q.carry) (UInt256.ofNat (l2PC))
    tl inv m0 aEnd m96 m64 m32 dst ret rest hcap hact he
  have htailrun := TnCacheRowTrace.run_tail ((l2PC)+TnCacheL2Chain.fullSize 8)
  ...
  have hlast : runInstructions TnCacheRowTrace.tail
      (TnCacheLastTrace.lastState st (UInt256.ofNat ((l2PC)+TnCacheL2Chain.fullSize 8))
      ...
      some (framed {s with memory := (TnCacheSquareModel.fromL1 q tn 8).memory}
        ...
        (TnCacheFrameOps.frame (negative32+pbi) hd pb ent
          (TnCacheSquareModel.fromL1 q tn 8).tn (UInt256.ofNat (l2PC)) inv tail)) := by
  ...
  have g0 := TnCandidateBlocks.middle.steps
    (s := framed st (UInt256.ofNat 3643)
      ([q.carry,bi,pbi,hd,pb,ent,tn,allOnes,UInt256.ofNat (l2PC),inv,
  ...
  have g1 : GasSteps
      (TnCacheL2Trace.state st (UInt256.ofNat (l2PC)) q.memory flag
        (rowMu q.memory 8) (rowC0 q.memory 8) 8 0 pbi hd pb ent (tn+q.carry)
        (UInt256.ofNat (l2PC)) inv tail)
      (TnCacheLastTrace.lastState st (UInt256.ofNat (l2PC+TnCacheL2Chain.fullSize 8))
        q.memory flag (rowMu q.memory 8) (rowC0 q.memory 8) 8 (8-1)
        pbi hd pb ent (tn+q.carry) (UInt256.ofNat (l2PC)) inv tail) := by
    exact TnCandidateBlocks.l2Eight.steps (env.transfer rfl rfl) rfl hreduce
  have hpc : (TnCacheLastTrace.lastState st
      (UInt256.ofNat (l2PC+TnCacheL2Chain.fullSize 8)) q.memory flag
      (rowMu q.memory 8) (rowC0 q.memory 8) 8 (8-1)
      pbi hd pb ent (tn+q.carry) (UInt256.ofNat (l2PC)) inv tail).pc =
      UInt256.ofNat 3910 := by
    change UInt256.ofNat (l2PC+TnCacheL2Chain.fullSize 8) = UInt256.ofNat 3910
    rfl
...
end Challenge.Modexp.Submission.Proofs.Fast.TnR8Reduction
