...
theorem jumpD3970 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
    (UInt256.ofNat 3497).toNat = true :=
  Exp.jumpD 3497 (by decide) FixedDirectPaths.jumpDest3970

theorem jumpD3997 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
    (UInt256.ofNat 3524).toNat = true :=
  Exp.jumpD 3524 (by decide) FixedDirectPaths.jumpDest3954
...
def gasSteps_squareLoop (s : State) {n bsize mm minv R : Nat}
  ...
  | zero => omega
  | succ k ih =>
      have hcall := sub.monpro 2048 2048 2048 (UInt256.ofNat 3497)
