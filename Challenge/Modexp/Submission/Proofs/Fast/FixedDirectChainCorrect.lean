...
memory word `0x2440 = 2720` and calls the kernel; from there the chain runs one
...
theorem readWord_countStore (mem : ByteArray) (count : Nat) :
    MachineState.readWord (Exp.storeWord mem 2720 (UInt256.ofNat count)) 2720 =
      UInt256.ofNat count :=
  Challenge.EvmProof.Memory.readWord_writeWord mem 2720 (UInt256.ofNat count)
...
def gasSteps_squareLoop (s : State) {n bsize mm minv : Nat}
  ...
  | succ k ih =>
      ...
      have hcall := sub.square (UInt256.ofNat 3221)
        ...
        (Exp.storeWord memory 2720 (UInt256.ofNat (k + 1))) bM
...
def gasSteps_squareLoopFast (s : State) {n bsize mm minv : Nat}
    ...
    Challenge.EvmProof.GasSteps
      ...
      (product s
        (sub.sqLoopMem count
          (Exp.storeWord memory 2720 (UInt256.ofNat count)))
  ...
  (FixedDirectChainTrace.gasSteps_squareCall s memory
    ...
    (sub.squareLoop count (UInt256.ofNat 3221)
      ...
      (Exp.storeWord memory 2720 (UInt256.ofNat count)) bM
...
def chain_of_fixed (s : State) {n bsize mm minv : Nat}
  ...
  · have hframe0 := countStore_frame count hframe
    ...
    have hinvFast : Inv
        (sub.sqLoopMem count (Exp.storeWord memory 2720 (UInt256.ofNat count)))
