...
* `n ∈ {4, 8}`: the kernel keeps its row frame and performs all `count` squares
  itself (`Exp.Subroutines.squareLoop`), returning once to `after_sq` (pc 3278)
  with the pushed count still on the stack;
* other widths: the kernel returns after every square
  (`Exp.Subroutines.square`) and the caller's own loop at pc 3248 counts down,
  falling through to pc 3278 with the count at `0`.
...
theorem jumpD3997 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
    (UInt256.ofNat 1619).toNat = true :=
  Exp.jumpD 1619 (by decide) jumpDest1802
...
def gasSteps_squareLoopFast (s : State) {n bsize mm minv : Nat}
  ...
  (FixedDirectChainTrace.gasSteps_squareCall s memory
    ...
    (sub.squareLoop count (UInt256.ofNat 3257)
...
/-- What either square chain delivers at `after_sq` (pc 3278): the memory, the
