...
namespace Challenge.Modexp.Submission.Proofs.Fast.CiosCarryL2
...
def state (template : State) (pc : UInt256) (mem : ByteArray)
  ...
  { template with
    ...
    stack := [(l2Step mem mu c0 n k).carry, mu, bi, pbi, paEnd, pbEnd, flag,
      carrySlot, allOnes, destination, returnPC] ++ rest
...
theorem run_load (w : Fin 33)
    ...
    runInstructions (loadProgram w x)
      (framed template pc
        ([carry, mu, bi, pbi, paEnd, pbEnd, flag, carrySlot, allOnes, destination, returnPC] ++ rest)) =
    some (framed template (pc + UInt256.ofNat (w.val + 3))
      ([maxWord, MachineState.readWord template.memory x.toNat, carry, mu, bi,
        pbi, paEnd, pbEnd, flag, carrySlot, allOnes, destination, returnPC] ++ rest)) := by
...
theorem run_step (w : Fin 33) (template : State) (pc : UInt256) (mem : ByteArray)
    ...
    runInstructions (l2Program w x tl ts)
      (state (carrySlot := carrySlot) template pc mem bi mu c0 n k pbi paEnd pbEnd flag destination returnPC rest) =
    some (state (carrySlot := carrySlot) template (pc + UInt256.ofNat (w.val + 35)) mem bi mu c0 n (k+1)
  ...
  have hl := run_load (carrySlot := carrySlot) w st pc x (l2Step mem mu c0 n k).carry mu bi
  ...
  have hp := L2.run_product st (pc + UInt256.ofNat (w.val + 3))
    ...
    ([bi, pbi, paEnd, pbEnd, flag, carrySlot, allOnes, destination, returnPC] ++ rest)
...
end Challenge.Modexp.Submission.Proofs.Fast.CiosCarryL2
