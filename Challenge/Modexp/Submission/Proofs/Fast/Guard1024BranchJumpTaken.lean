...
theorem run_jump_taken (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocated branchJumpLocated
      (jumpStackState input (UInt256.ofNat 1)) = some (branchState input 4690) := by
  have htrue : UInt256.isTrue (UInt256.ofNat 1) := by decide
  have hjump : Decode.isValidJumpDest submissionBytecode 4690 = true :=
  ...
  have hpc : (jumpStackState input (UInt256.ofNat 1)).pc.toNat =
    ...
    simp [jumpStackState, initialState, guard1024PC3,
  ...
  rw [if_pos htrue]
  rw [show (UInt256.ofNat 4690).toNat = 4690 by decide]
