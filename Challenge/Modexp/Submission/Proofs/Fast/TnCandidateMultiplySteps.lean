import Challenge.Modexp.Submission.Proofs.Fast.TnCandidateRowsSteps
...
namespace Challenge.Modexp.Submission.Proofs.Fast.TnCandidateMultiplySteps
...
open TnCandidateRowSteps TnCandidateRowsSteps TnCandidateReductionSteps
...
noncomputable def finish_steps (s : State)
    ...
    GasSteps (rowState s z pb n n tl inv m0 aEnd m96 m64 m32 dst ret rest)
      (framed {s with memory := lift z.memory z.tn} (UInt256.ofNat 4320) (dst :: ret :: rest)) := by
  ...
  let stack := TnCacheFrameOps.frame (pointer pb n n) (UInt256.ofNat 3711)
    (UInt256.ofNat (pb-32)) (UInt256.ofNat (l1PC n)) z.tn (UInt256.ofNat (l2PC n)) inv tail
  have htail : tail.length ≤ 1006 := by simp only [tail, List.length_cons]; omega
  have hf := TnCacheFrameOps.run_flush 4292 st (pointer pb n n) (UInt256.ofNat 3711)
    (UInt256.ofNat (pb-32)) (UInt256.ofNat (l1PC n)) z.tn (UInt256.ofNat (l2PC n)) inv
    tail htail hact
  have g0 := TnCandidateBlocks.flush.steps
    (s := framed st (UInt256.ofNat 4292) stack) (env.transfer rfl rfl) rfl hf
  have hg := TnCacheExitTrace.run_guard aft 4297 (pointer pb n n)
    (UInt256.ofNat (pb-32)) (UInt256.ofNat (l1PC n)) z.tn (UInt256.ofNat (l2PC n)) inv tail htail
  have g1 := TnCandidateExitBlocks.guard.steps
    (s := framed aft (UInt256.ofNat 4297) stack) (env.transfer rfl rfl) rfl hg
  have hd := TnCacheExitTrace.run_drop aft 4306 (pointer pb n n) (UInt256.ofNat 3711)
    (UInt256.ofNat (pb-32)) (UInt256.ofNat (l1PC n)) z.tn (UInt256.ofNat (l2PC n)) inv
    m0 tl m96 m64 m32 aEnd dst ret rest hcap
  have g2 := TnCandidateExitBlocks.drop.steps
    (s := framed aft (UInt256.ofNat 4306) stack) (env.transfer rfl rfl) rfl hd
...
noncomputable def multiply_steps (s : State)
    ...
    GasSteps (rowState s z pb n 0 tl inv m0 aEnd m96 m64 m32 dst ret rest)
      (framed {s with memory := CarryRowModel.rowsCarry (lift z.memory z.tn) pa pb n n}
        (UInt256.ofNat 4320) (dst :: ret :: rest)) := by
...
end Challenge.Modexp.Submission.Proofs.Fast.TnCandidateMultiplySteps
