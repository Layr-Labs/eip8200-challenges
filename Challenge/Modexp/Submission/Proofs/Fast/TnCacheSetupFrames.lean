...
def l1Target (n : Nat) : UInt256 := UInt256.ofNat 3740 + UInt256.ofNat 148 * isFour n

def l2Target (n : Nat) : UInt256 := UInt256.ofNat 4023 + UInt256.ofNat 148 * isFour n

def setupState (s : State) (mem : ByteArray) (hd : UInt256) (pa pb : Nat)
    (dst ret : UInt256) (rest : List UInt256) : State :=
  { s with
      pc := UInt256.ofNat 3571
      memory := mem
...
def outState (s : State) (mem : ByteArray) (pb n i : Nat)
  ...
  { s with
      ...
      stack := [UInt256.ofNat (ptrAt (pb+32*n-32) i), hd, UInt256.ofNat (pb-32),
        ent, zeroTn, allOnes, l2Target n, inv, m0] ++ rest }
