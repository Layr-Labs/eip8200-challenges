...
/-!
# `again` (4956): the next square's row-0 state

The loop's staging block, in two halves so that neither elaboration is large:

* `againStageProgram` (13 instructions, pc 4956 -> 4970) re-stages the operand
  ...
  `Cios2Dispatch.gasSteps_commonSetupInput` produces, `mpZeroed s (stage mem 2368 n) n`;
* `againFixProgram` (9 instructions, pc 4970 -> falls into `sq_row` 4981) resets the
...
/-- Between the two halves (pc 4970): the width word sits above the untouched frame and
...
`2368 + 32 * n - 32`, the entry slot back to `sqEnt n 0`, and slot 14 is cleared. -/
theorem run_againFix (s : State) (mem : ByteArray) (n : Nat)
    ...
    (hn : n = 4 ∨ n = 8)
    (hhd : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4428 = true) :
    ...
    some (outState s (mpZeroed s mem n) 2368 n 0
      (UInt256.ofNat 4428) (UInt256.ofNat (sqEnt n 0)) inv m0
...
/-- `again` (4956): the loop's next row-0 state, with the operand re-staged, the
