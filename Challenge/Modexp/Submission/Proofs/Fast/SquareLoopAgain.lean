...
/-!
# Repeated staging followed by the specialized first row

The first eight instructions clear the accumulator and preserve its width.
The remaining six instructions reset the operand pointer and computed chain
entry, then fall through into the row-zero helper at PC 4453. That helper
overwrites the preserved previous-limb slot before ordinary rows observe it.
-/
...
/-- Between the two halves (pc 4411): the width word sits above the untouched frame and
...
theorem run_againFix (s : State) (mem : ByteArray) (n : Nat)
    ...
    (hn : n = 4 ∨ n = 8)
    (hhd : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4453 = true) :
    ...
    some { outState s (mpZeroed s mem n) 2368 n 0
      (UInt256.ofNat 4483) (UInt256.ofNat (sqEnt n 0)) inv m0
      (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest) with pc := UInt256.ofNat 4453 } := by
  have hc16' : rest.length + 16 < 1024 := by omega
  have hc17 : rest.length + 17 < 1024 := by omega
  have hc18 : rest.length + 18 < 1024 := by omega
  have hc19 : rest.length + 19 < 1024 := by omega
