...
5272. The previous-limb slot is preserved: the new entry overwrites it with the
...
theorem run_againFix (s : State) (mem : ByteArray) (n : Nat)
    ...
    (hn : n = 4 ∨ n = 8)
    (hhd : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 5283 = true) :
    ...
    some { outState s (mpZeroed s mem n) 2368 n 0
      (UInt256.ofNat 4441) (UInt256.ofNat (sqEnt n 0)) inv m0
      (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest) with pc := UInt256.ofNat 5283 } := by
  have hc16' : rest.length + 16 < 1024 := by omega
  have hc17 : rest.length + 17 < 1024 := by omega
  have hc18 : rest.length + 18 < 1024 := by omega
  have hc19 : rest.length + 19 < 1024 := by omega
