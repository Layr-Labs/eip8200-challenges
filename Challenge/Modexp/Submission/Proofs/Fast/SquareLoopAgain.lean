...
theorem run_againFix (s : State) (mem : ByteArray) (n : Nat)
    ...
    (hn : n = 4 ∨ n = 8)
    (hhd : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4972 = true) :
    ...
    some (outState s (mpZeroed s mem n) 2368 n 0
      (UInt256.ofNat 4972) (UInt256.ofNat (sqEnt n 0)) inv m0
  ...
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4972 = true := by
    rw [hcode]; exact hhd
  simp (config := { maxSteps := 200000 })
