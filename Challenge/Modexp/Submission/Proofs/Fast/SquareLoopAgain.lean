...
/-!
# `again` (4701): the next square's row-0 state

The loop's staging block, in two halves so that neither elaboration is large:

* `againStageProgram` (13 instructions, pc 4701 -> 4723) re-stages the operand
...
* `againFixProgram` (9 instructions, pc 4723 -> falls into `sq_row` 4734) resets the
...
/-- Between the two halves (pc 4723): the width word sits above the untouched frame and
...
theorem run_againFix (s : State) (mem : ByteArray) (n : Nat)
    ...
    (hn : n = 4 ∨ n = 8)
    (hhd : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4734 = true) :
    ...
    some (outState s (mpZeroed s (StagedOperand.stage mem 2048 n) n) 2048 n 0
      (UInt256.ofNat 4734) (UInt256.ofNat (sqEnt n 0)) inv m0
  ...
  have hptr : UInt256.ofNat (32 * n) +
      UInt256.ofNat (ptrAt (2048 + 32 * n - 32) n) =
      UInt256.ofNat (2048 + 32 * n - 32) := by
    rw [Challenge.EvmProof.Word.word_add_comm]
    exact ptr_wrap (2048 + 32 * n - 32) n
  have hent := entry_from_reduction n hn
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4734 = true := by
    rw [hcode]; exact hhd
  simp (config := { maxSteps := 200000 })
...
/-- `again` (4701): the loop's next row-0 state, with the operand re-staged, the
