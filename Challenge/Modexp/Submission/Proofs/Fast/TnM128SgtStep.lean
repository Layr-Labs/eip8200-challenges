...
SGT is instruction 3065 at pc 4099. The separate R4 path has its own SGT sites.
-/
...
theorem sqRowSgt_index : TnM128CandidateArtifact.submissionInstructions[3065]? = some (.op .SGT) := by
  rfl

/-- Its exact byte address. -/
theorem sqRowSgt_pc : TnM128CandidateArtifact.submissionArtifact.instructionPC 3065 = 4099 := by
...
theorem decodedOp_sqRowSgt (s : State)
  ...
  have hpcNat : s.pc.toNat = TnM128CandidateArtifact.submissionArtifact.instructionPC 3065 := by
    rw [hpc, sqRowSgt_pc]; decide
  have hwf : Stepper.WellFormed s.fork (.op .SGT) := by
    rw [hfork]
    exact ⟨by decide, trivial, rfl⟩
  exact Stepper.decodes_of_artifact TnM128CandidateArtifact.submissionArtifact s 3065 (.op .SGT)
