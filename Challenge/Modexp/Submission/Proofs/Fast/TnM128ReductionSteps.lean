...
def middlePC (n : Nat) : Nat := if n = 4 then 5291 else 3647
def l2PC (n : Nat) : Nat := if n = 4 then 3812 else 3666
...
noncomputable def reduction_steps (s : State)
    ...
    GasSteps
      ...
      (framed {s with memory := (TnCacheSquareModel.fromL1 q tn n).memory}
        (if UInt256.isTrue (UInt256.gt (negative32+pbi) pb) then hd else UInt256.ofNat 3963)
