...
def program : List Instr :=
  [.op .JUMPDEST, .push 2 2080, .op .MLOAD, .push 2 4295, .op .JUMPI]
...
theorem run_gate (s : State) (mem : ByteArray) (dst ret : UInt256) (rest : List UInt256)
    ...
    (hjump : Decode.isValidJumpDest s.executionEnv.code 4295 = true) :
    runInstructions program (atState s mem 4531 dst ret rest) =
      some (atState s mem
        (if (MachineState.readWord mem 2080).toNat = 0 then 4540 else 4295)
        dst ret rest) := by
  have hc2 : rest.length+2 < 1024 := by omega
  have hc3 : rest.length+3 < 1024 := by omega
  have hc4 : rest.length+4 < 1024 := by omega
  have h2080 : (2080 : UInt256).toNat = 2080 := by decide
  have h4491 : (4295 : UInt256).toNat = 4295 := by decide
  have heq4491 : (4295 : UInt256) = UInt256.ofNat 4295 := by decide
