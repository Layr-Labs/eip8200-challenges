...
theorem run_next (s : State) (mem : ByteArray) (c mu bi : UInt256)
  ...
  have trace := run_tail { s with memory := mem } c mu bi
    ...
    (l1Target n) (l2Target n) (modulusValue mem n) (inverseValue mem :: tailPointerValue mem :: low64Value mem :: low32Value mem :: dst :: ret :: rest) (by simp only [List.length_cons]; omega) hact htarget
  simpa only [List.cons_append, List.nil_append, input, result, baseStack, framed, CiosCached.tailState,
    CiosCached.outState, hp, if_pos hcond, List.cons_append, List.nil_append, modulusValue_tail mem c n hn32, inverseValue_tail mem c, tailPointerValue_tail mem c, low64Value_tail mem c, low32Value_tail mem c] using trace
...
theorem run_last (s : State) (mem : ByteArray) (c mu bi : UInt256)
  ...
  have trace := run_tail { s with memory := mem } c mu bi
    ...
    (l1Target n) (l2Target n) (modulusValue mem n) (inverseValue mem :: tailPointerValue mem :: low64Value mem :: low32Value mem :: dst :: ret :: rest) (by simp only [List.length_cons]; omega) hact htarget
  simpa only [List.cons_append, List.nil_append, input, result, baseStack, framed, CiosCached.tailState,
    exitState, hp, if_neg hcond, List.cons_append, List.nil_append, modulusValue_tail mem c n hn32, inverseValue_tail mem c, tailPointerValue_tail mem c, low64Value_tail mem c, low32Value_tail mem c] using trace
