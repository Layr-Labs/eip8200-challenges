...
theorem run_next (s : State) (mem : ByteArray) (c mu bi : UInt256)
    ...
    (htarget : Decode.isValidJumpDest s.executionEnv.code 4223 = true) :
  ...
  have trace := run_tail { s with memory := mem } c mu bi
    ...
    (l1Target n) (l2Target n) (modulusValue mem n) (dst :: ret :: rest) (by simp only [List.length_cons]; omega) hact htarget
  simpa only [List.cons_append, List.nil_append, input, result, baseStack, framed, CiosCached.tailState,
    CiosCached.outState, hp, if_pos hcond, List.cons_append, List.nil_append, modulusValue_tail mem c n hn32] using trace
...
theorem run_last (s : State) (mem : ByteArray) (c mu bi : UInt256)
  ...
  have trace := run_tail { s with memory := mem } c mu bi
    ...
    (l1Target n) (l2Target n) (modulusValue mem n) (dst :: ret :: rest) (by simp only [List.length_cons]; omega) hact htarget
  simpa only [List.cons_append, List.nil_append, input, result, baseStack, framed, CiosCached.tailState,
    exitState, hp, if_neg hcond, List.cons_append, List.nil_append, modulusValue_tail mem c n hn32] using trace
