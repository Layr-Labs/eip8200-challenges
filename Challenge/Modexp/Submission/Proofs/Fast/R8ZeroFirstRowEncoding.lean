...
theorem encoded_frozen : assembleBytes (program (UInt256.ofNat 3417)) = frozenBytes := by decide

theorem program_length : (program (UInt256.ofNat 3417)).length = 213 := by decide

theorem encoded_length : (assembleBytes (program (UInt256.ofNat 3417))).length = 246 := by decide
