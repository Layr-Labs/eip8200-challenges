...
def input (s : State) (paj ptj c bi pbi paEnd pbEnd flag dst ret : UInt256)
  ...
  framed s (UInt256.ofNat 4601) ([paj, ptj, c] ++ baseStack bi pbi paEnd pbEnd flag dst ret rest)

def stored (s : State) (c bi pbi paEnd pbEnd flag dst ret : UInt256)
    (rest : List UInt256) : State :=
  framed { s with memory := midMem s.memory c } (UInt256.ofNat 4619)
