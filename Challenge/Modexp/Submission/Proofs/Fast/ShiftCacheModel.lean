...
def cacheMem (mem : ByteArray) (n : Nat) : ByteArray :=
  Exp.storeWord mem 1696 (entryWord n)

theorem read_cache (mem : ByteArray) (n : Nat) :
    MachineState.readWord (cacheMem mem n) 1696 = entryWord n := by
  exact Challenge.EvmProof.Memory.readWord_writeWord _ _ _

theorem read_disjoint (mem : ByteArray) (n addr : Nat)
    (h : addr + 32 ≤ 1696 ∨ 1728 ≤ addr) :
    MachineState.readWord (cacheMem mem n) addr = MachineState.readWord mem addr := by
  exact Exp.storeWord_readWord_disjoint mem 1696 addr (entryWord n) h

theorem represents_cache (mem : ByteArray) (n ptr cnt value : Nat)
    (h : ptr + 32 * cnt ≤ 1696 ∨ 1728 ≤ ptr)
    (hr : Model.FastRepresents mem ptr cnt value) :
    Model.FastRepresents (cacheMem mem n) ptr cnt value := by
  exact Model.fastRepresents_writeWord_disjoint mem 1696 ptr cnt value (entryWord n).toNat
    (Or.symm h) hr

theorem cache_survives_step (mem : ByteArray) (n mm : Nat) (hn : 1 ≤ n) (hcap : n ≤ 8)
    (h : MachineState.readWord mem 1696 = entryWord n) :
    MachineState.readWord (Shift.stepMem mem n mm) 1696 = entryWord n := by
  rw [Shift.stepMem_readWord_disjoint mem n mm 1696 hn
    ⟨Or.inr (by omega), Or.inl (by omega), Or.inl (by omega)⟩]
  exact h

theorem cache_survives_u (mem : ByteArray) (n : Nat)
    (h : MachineState.readWord mem 1696 = entryWord n) :
    MachineState.readWord (Shift.uMem mem n) 1696 = entryWord n := by
  rw [Shift.uMem_readWord_disjoint mem n 1696 (Or.inl (by omega))]
  exact h
