...
def decodeProgram : List Instr :=
  [.op .JUMPDEST,
   ...
   .op (.Dup ⟨1, by decide⟩),
   .push ⟨2, by decide⟩ (UInt256.ofNat 736),
   .op .ADD,
   .op .MSTORE,
...
theorem run_startProgram (s : State) (memory : ByteArray)
    ...
    (hactive : 93 ≤ s.activeWords.toNat) (hrun : s.halt = .Running) :
  ...
  have hfix : UInt256.ofNat (MachineState.activeWordsAfter
      ...
      512 (32 * n)) = s.activeWords :=
    Exp.activeWords_fix2 s 256 (32 * n) 512 (32 * n)
...
theorem run_decodeProgram (s : State) (memory : ByteArray)
  ...
  have haddr : (736 + 32 * n) %
      ...
      736 + 32 * n :=
    Exp.mod_word_self
      (Nat.lt_of_le_of_lt (show 736 + 32 * n ≤ 3973 by omega) (by norm_num))
  have hfix : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat
      (736 + 32 * n) 32) = s.activeWords :=
    Exp.activeWords_fix s (736 + 32 * n) 32 (by omega) (by omega) hactive
  simp (config := { maxSteps := 600000 })
