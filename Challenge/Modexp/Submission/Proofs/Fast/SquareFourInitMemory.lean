...
def digitAddr (j : Nat) : Nat := 8960+32*(3-j)
...
theorem read_init_high (mem : ByteArray) :
    MachineState.readWord (initMemory mem) 8928 = (doubleWords mem 4).carry := by
...
theorem init_represents (mem : ByteArray) (a : Nat)
    (ha : Model.FastRepresents mem 8960 4 a) :
    Model.FastRepresents (initMemory mem) 8928 5 (2*a) := by
  have hv := doubleWords_invariant mem 4 le_rfl
  have hsrc : limbSum (fun k => (MachineState.readWord mem (digitAddr k)).toNat) 4 = a :=
    limbSum_fastRepresents (mem := mem) (ptr := 8960) (count := 4) (value := a) ha
  rw [hsrc] at hv
  have hval : Csub.lowValue (initMemory mem) 8928 5 5 = 2*a := by
    rw [← limbSum_eq_lowValue, limbSum_succ]
    have hlo : limbSum (fun k =>
          (MachineState.readWord (initMemory mem) (8928+32*(5-1-k))).toNat) 4 =
        limbSum (fun k => (doubleDigit mem k).toNat) 4 := by
      apply limbSum_congr
      intro k hk
      have hp : 8928+32*(5-1-k) = digitAddr k := by simp only [digitAddr]; omega
