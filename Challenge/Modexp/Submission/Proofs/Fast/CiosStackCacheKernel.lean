...
opaque kernel {art : ProgramArtifact} (s : State) (env : Environment art .Osaka s)
    ...
    GasSteps (CiosCached.entryState s mem pa pb dst ret rest)
  ...
  have he' : runInstructions CiosStackCachePrograms.entry
      (CiosCached.entryState s mem pa pb dst ret rest) = some (outAt s c r pa pb n 0 dst ret rest) := by
    simpa only [outAt, rowState, c, r, initial] using he
  have entrySteps := blocks.entry.steps
    (s := CiosCached.entryState s mem pa pb dst ret rest) (env.transfer rfl rfl) rfl he'
