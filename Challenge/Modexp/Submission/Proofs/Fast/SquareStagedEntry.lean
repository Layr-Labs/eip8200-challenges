...
def program : List Instr :=
  [.op .JUMPDEST, .push 2 4234, .op (.Swap ⟨1, by decide⟩), .op .POP,
   .push 2 2336, .op (.Swap ⟨2, by decide⟩), .op .POP,
   .push 2 1856, .op .ADD]

def block : Block Artifact.submissionArtifact .Osaka 4568 program :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3435 9 4568 program
...
def gasSteps_entry (s : State) (mem : ByteArray) (n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (outState s mem 512 n 0 (UInt256.ofNat 4568) ent inv m0 rest)
      { outState s mem 2368 n 0 (UInt256.ofNat 4234) ent inv m0 rest with
        pc := UInt256.ofNat 4583 } :=
