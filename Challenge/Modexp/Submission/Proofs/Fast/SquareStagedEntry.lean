...
def program : List Instr :=
  [.op .JUMPDEST, .push 2 4428, .op (.Swap ⟨1, by decide⟩), .op .POP,
   .push 2 2336, .op (.Swap ⟨2, by decide⟩), .op .POP,
   .push 2 1856, .op .ADD, .push 2 4758, .op .JUMP]

def block : Block Artifact.submissionArtifact .Osaka 4739 program :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3565 11 4739 program
    (by decide) (by rfl) (by rfl) (by decide)

theorem jumpDest : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
    (UInt256.ofNat 4739).toNat = true :=
...
theorem run_entry (s : State) (mem : ByteArray) (n : Nat)
    ...
    runInstructions program
      (outState s mem 512 n 0 (UInt256.ofNat 4739) ent inv m0 rest) =
    some { outState s mem 2368 n 0 (UInt256.ofNat 4428) ent inv m0 rest with
      pc := UInt256.ofNat 4758 } := by
  ...
  have hp : UInt256.ofNat 1856 + UInt256.ofNat (512 + 32 * n - 32) =
    ...
    congr 1
    omega
  have hj : Decode.isValidJumpDest s.executionEnv.code 4758 = true := by
    rw [hcode]; exact Artifact.isValidJumpDest_index 3576 (by rfl)
...
def gasSteps_entry (s : State) (mem : ByteArray) (n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (outState s mem 512 n 0 (UInt256.ofNat 4739) ent inv m0 rest)
      { outState s mem 2368 n 0 (UInt256.ofNat 4428) ent inv m0 rest with
        pc := UInt256.ofNat 4758 } :=
