...
theorem run_l2Last (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    runInstructions (l2LastProgram 0 0 2112 2144)
      (l2At 4239 s mid bi mu c0 pb n i (n-2) hd ent pdst ret rest) =
      some (tailState s (l2Step mid mu c0 n (n-1)).memory
        (l2Step mid mu c0 n (n-1)).carry mu bi pb n i hd ent pdst ret rest) := by
  have h := run_stepLast 0 s (UInt256.ofNat 4239) mid bi mu c0 n (n-2) 0 2112 2144
  ...
  have hpc : UInt256.ofNat 4239 + UInt256.ofNat ((0 : Fin 33).val + 34) = UInt256.ofNat 4273 := by
    decide
  rw [hnn, hpc] at h
