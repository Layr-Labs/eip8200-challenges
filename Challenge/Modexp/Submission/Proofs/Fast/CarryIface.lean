...
structure EntryLemmas : Type where
  /-- Statement of WP-K2 `Cios2Dispatch.gasSteps_mulEntry`: `mul entry` (pc 4015) →
  `common` (pc 4019) with the multiply row head `hd = 4027`. -/
  gasSteps_mulEntry : ∀ (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (Cios2Dispatch.commonState s mem (UInt256.ofNat 4256) pa pb pdst ret rest)
...
/-- The multiply row head `hd = 4027` (instruction 1760, pc 0x0fc5) is a jump destination. -/
theorem jumpDest_rowHead :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode (UInt256.ofNat 4256).toNat = true := by
  rw [show (UInt256.ofNat 4256).toNat = 4256 from by decide]
