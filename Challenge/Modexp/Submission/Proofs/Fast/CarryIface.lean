...
structure EntryLemmas : Type where
  /-- Statement of WP-K2 `Cios2Dispatch.gasSteps_mulEntry`: `mul entry` (pc 3890) →
  `common` (pc 3894) with the multiply row head `hd = 4005`. -/
  gasSteps_mulEntry : ∀ (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (Cios2Dispatch.commonState s mem (UInt256.ofNat 3986) pa pb pdst ret rest)
...
/-- The multiply row head `hd = 4005` (instruction 3040, pc 0x0fc5) is a jump destination. -/
theorem jumpDest_rowHead :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode (UInt256.ofNat 3986).toNat = true := by
  rw [show (UInt256.ofNat 3986).toNat = 3986 from by decide]
