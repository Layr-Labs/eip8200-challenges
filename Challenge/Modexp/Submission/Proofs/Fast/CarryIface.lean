...
structure EntryLemmas : Type where
  /-- Statement of WP-K2 `Cios2Dispatch.gasSteps_mulEntry`: `mul entry` (pc 4025) →
  `common` (pc 4029) with the multiply row head `hd = 4037`. -/
  gasSteps_mulEntry : ∀ (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (Cios2Dispatch.commonState s mem (UInt256.ofNat 4266) pa pb pdst ret rest)
...
theorem jumpDest_rowHead :
  ...
  rw [show (UInt256.ofNat 4266).toNat = 4266 from by decide]
