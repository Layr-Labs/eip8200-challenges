...
structure EntryLemmas : Type where
  /-- Statement of WP-K2 `Cios2Dispatch.gasSteps_mulEntry`: `mul entry` (pc 3959) →
  `common` (pc 3963) with the multiply row head `hd = 3971`. -/
  gasSteps_mulEntry : ∀ (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (Cios2Dispatch.commonState s mem (UInt256.ofNat 3711) pa pb pdst ret rest)
...
/-- The multiply row head `hd = 3971` (instruction 1760, pc 0x0fc5) is a jump destination. -/
