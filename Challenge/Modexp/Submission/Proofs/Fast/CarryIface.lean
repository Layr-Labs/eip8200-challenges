...
structure EntryLemmas : Type where
  /-- Statement of WP-K2 `Cios2Dispatch.gasSteps_mulEntry`: `mul entry` (pc 3961) →
  `common` (pc 3965) with the multiply row head `hd = 3973`. -/
  gasSteps_mulEntry : ∀ (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (Cios2Dispatch.commonState s mem (UInt256.ofNat 3713) pa pb pdst ret rest)
...
theorem jumpDest_rowHead :
  ...
  rw [show (UInt256.ofNat 3713).toNat = 3713 from by decide]
