...
theorem run_genericShim (s : State) (memory : ByteArray)
  ...
  simp [genericShimProgram, returnInput, genericShimOutput, frame16, atState,
    ...
    List.exchange, hc8, hc9, hc10, hc11, hc12, hc13, hc14, hc15, hc16, hc17, hjump,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod]
...
theorem run_genericReturnTail (s : State) (memory : ByteArray)
  ...
  simp [genericReturnProgram, genericReturnInput, returnOutput, atState,
    runInstructions, Challenge.EvmProof.Stepper.runInstr,
    List.exchange, hc1, hc2, hc3, hc4, hc5, hc6, hc7, hc8, hret, hjump,
    Challenge.EvmProof.Word.succ_ofNat_mod]
...
opaque gasSteps_genericReturn (s : State) (memory : ByteArray)
  ...
  have hpath0 : Challenge.EvmProof.Stepper.runLocatedBlock genericShimBlock.path input =
      some middle := by
    apply WindowTwentyOneLocated.run_linear genericShimBlock.path _ input middle
      env0.sizeBound genericShimBlock.layout rfl env0.running
    rw [genericShimBlock.instructions_eq]
    exact hraw0
  have g0 := genericShimBlock.steps env0 rfl hpath0
  ...
  have hpath1 : Challenge.EvmProof.Stepper.runLocatedBlock genericReturnBlock.path middle =
      some output := by
    apply WindowTwentyOneLocated.run_linear genericReturnBlock.path _ middle output
      env1.sizeBound genericReturnBlock.layout rfl env1.running
    rw [genericReturnBlock.instructions_eq]
    exact hraw1
  have g1 := genericReturnBlock.steps env1 rfl hpath1
  exact g0.trans g1
