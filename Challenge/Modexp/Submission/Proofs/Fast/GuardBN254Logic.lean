import Challenge.Modexp.Submission.Proofs.Fast.GuardBN254Data
...
namespace Challenge.Modexp.Submission.Proofs.Fast.GuardBN254Logic
...
/-- Sequential accumulator used by the bytecode's twelve comparisons. -/
...
def guardDiff (input : ByteArray) : UInt256 :=
  scanDiff input checks
    (UInt256.lor 0
      (UInt256.xor (UInt256.ofNat 192) (UInt256.ofNat input.size)))
...
theorem guardDiff_eq_zero_iff (input : ByteArray) (hbound : input.size < 2 ^ 256) :
  ...
  simp only [wordOr_eq_zero_iff, wordXor_eq_zero_iff, Matches]
  constructor
  · rintro ⟨⟨-, hsize⟩, hwords⟩
    constructor
    · have := congrArg UInt256.toNat hsize.symm
  ...
  · rintro ⟨hsize, hwords⟩
    constructor
    · exact ⟨trivial, by rw [hsize]⟩
    · exact hwords

end Challenge.Modexp.Submission.Proofs.Fast.GuardBN254Logic
