...
def displacement (mem : ByteArray) : UInt256 :=
  UInt256.ofNat 152 * UInt256.eq (UInt256.ofNat 128) (MachineState.readWord mem 2784)
...
theorem run_reads (s : State) (hd pa pb dst ret : UInt256) (rest : List UInt256)
    (addr : Nat) (hcap : rest.length ≤ 998) (hact : 93 ≤ s.activeWords.toNat)
    (haddr : addr + 32 ≤ 2912)
    ...
    some {s with
        ...
        stack := [MachineState.readWord s.memory 32, (pa + UInt256.ofNat addr), MachineState.readWord s.memory 2816, MachineState.readWord s.memory addr, MachineState.readWord s.memory 2880, MachineState.readWord s.memory 96, hd, pa, pb, dst, ret] ++ rest} := by
...
theorem run_setup (s : State) (hd pa pb dst ret value inverse aEnd tailPointer low96 low32 : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998) (hact : 93 ≤ s.activeWords.toNat) :
...
theorem run_load (s : State) (hd pa pb dst ret : UInt256) (rest : List UInt256)
    ...
    some {s with
        pc := UInt256.ofNat 3979
        stack := [displacement s.memory, negative32,
        MachineState.readWord s.memory 32, (pa + UInt256.ofNat addr), MachineState.readWord s.memory 2816, MachineState.readWord s.memory addr, MachineState.readWord s.memory 2880, MachineState.readWord s.memory 96, hd, pa, pb, dst, ret] ++ rest} := by
...
theorem run_shuffle (s : State) (hd pa pb dst ret value inverse aEnd tailPointer low96 low32 delta : UInt256)
    ...
    some {s with
        ...
        stack := [pa, pb, UInt256.ofNat 4060 + delta, negative32, allOnes,
        UInt256.ofNat 4359 + delta, inverse, value, tailPointer, low96, hd, low32, aEnd, dst, ret] ++ rest} := by
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  have hc7 : rest.length + 7 < 1024 := by omega
