...
theorem run_next (s : State) (mem : ByteArray) (c mu bi : UInt256)
    ...
    (htarget : Decode.isValidJumpDest s.executionEnv.code 4165 = true)
  ...
  have trace := CarryRowRun.run_tail { s with memory := mem } c mu bi
    ...
    (l1Target n) (l2Target n) dst (ret :: rest) (by simp only [List.length_cons]; omega) hact (UInt256.ofNat 4165) hroute htarget
