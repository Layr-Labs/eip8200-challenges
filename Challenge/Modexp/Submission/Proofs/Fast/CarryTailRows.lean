...
theorem run_next (s : State) (mem : ByteArray) (c mu bi : UInt256)
    (pb n i : Nat) (hd ent dst ret w8 w9 w10 : UInt256) (rest : List UInt256)
    ...
    runInstructions CarryRowPrograms.tail
      (CiosCached.tailState s mem c mu bi pb n i hd ent dst ret
        (w8 :: w9 :: w10 :: negative32 :: rest)) =
    some (CiosCached.outState s (tailCarry mem c bi) pb n (i+1) hd ent dst ret
  ...
  have trace := CarryRowRun.run_tail { s with memory := mem } c bi
    ...
    ent dst (l2Target n) ret w8 w9 w10 rest (by omega) hact htarget
...
theorem run_last (s : State) (mem : ByteArray) (c mu bi : UInt256)
    ...
    some (exitState s (tailCarry mem c bi)
      (UInt256.ofNat (ptrAt (pb+32*n-32) (i+1))) pb n hd ent dst ret
      (w8 :: w9 :: w10 :: negative32 :: rest)) := by
  have hp := pointer_next (pb+32*n-32) i
  have hcond : ¬UInt256.isTrue
