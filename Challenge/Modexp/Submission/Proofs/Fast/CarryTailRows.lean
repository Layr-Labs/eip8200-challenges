...
/-- Not the last row: the writeback stores the limb, installs the row's carry out into
the cell and the `DUP3 JUMPI` returns to the row head `hd`. -/
theorem run_next (s : State) (mem : ByteArray) (c mu bi : UInt256)
    (pb n i : Nat) (hd ent cy dst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1005) (hact : 88 ≤ s.activeWords.toNat)
    (hpb : 32 ≤ pb) (hpbFit : pb+32*n ≤ 2816) (hi : i+1 < n)
    (htarget : Decode.isValidJumpDest s.executionEnv.code hd.toNat = true) :
    runInstructions CarryRowPrograms.tail
      (CiosCached.tailState s mem c mu bi pb n i hd ent cy dst ret rest) =
    some (CiosCached.outState s (tailMemS mem cy c) pb n (i+1) hd ent
      (slotOverflow cy c + bi) dst ret rest) := by
  ...
  have trace := CarryRowRun.run_tail { s with memory := mem } c bi
    ...
    ent cy dst (ret :: rest) (by simp only [List.length_cons]; omega) hact htarget
...
/-- The last row: the `DUP3 JUMPI` falls through into the kernel-exit dispatch. -/
theorem run_last (s : State) (mem : ByteArray) (c mu bi : UInt256)
    ...
    runInstructions CarryRowPrograms.tail
      (CiosCached.tailState s mem c mu bi pb n i hd ent cy dst ret rest) =
    some (exitState s (tailMemS mem cy c)
      (UInt256.ofNat (ptrAt (pb+32*n-32) (i+1))) pb n hd ent (slotOverflow cy c + bi)
      dst ret rest) := by
  have hp := pointer_next (pb+32*n-32) i
  have hcond : ¬UInt256.isTrue
