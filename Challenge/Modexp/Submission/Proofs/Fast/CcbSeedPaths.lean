...
private theorem seedPCAnchor :
    Artifact.submissionArtifact.instructionPC 2629 = 3455 := by rfl

@[simp] theorem seedPC (i : Nat)
    (hi : 2629 ≤ i) (hii : i ≤ 2656) :
    Artifact.submissionArtifact.instructionPC i =
      ([3455,3456,3459,3460,3462,3463,3464,3466,3467,3468,3469,3472,3473,3474,3475,3478,3479,3480,3481,3482,3483,3484,3487,3488,3489,3491,3492,3495] : List Nat)[i - 2629]! := by
...
def entryPath : List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2629 .JUMPDEST,
   ...
   pushAt 2635 1 8,
   opAt 2636 (.Swap ⟨0, by decide⟩),
   opAt 2637 .SHL]

def callPath : List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2638 .JUMPDEST,
   pushAt 2639 2 3479,
   opAt 2640 (.Dup ⟨3, by decide⟩),
...
theorem jumpDest4029 :
  ...
  Artifact.isValidJumpDest_index 2638 (by rfl)
