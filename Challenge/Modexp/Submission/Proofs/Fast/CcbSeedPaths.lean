...
private theorem seedPCAnchor :
    Artifact.submissionArtifact.instructionPC 2510 = 4011 := by rfl

@[simp] theorem seedPC (i : Nat)
    (hi : 2510 ≤ i) (hii : i ≤ 2537) :
    Artifact.submissionArtifact.instructionPC i =
      ([4011,4012,4015,4016,4018,4019,4020,4022,4023,4024,4025,4028,4029,4030,4031,4034,4035,4036,4037,4038,4039,4040,4043,4044,4045,4047,4048,4051] : List Nat)[i - 2510]! := by
...
def entryPath : List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2510 .JUMPDEST,
   ...
   pushAt 2516 1 8,
   opAt 2517 (.Swap ⟨0, by decide⟩),
   opAt 2518 .SHL]

def callPath : List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [opAt 2519 .JUMPDEST,
   pushAt 2520 2 4035,
   opAt 2521 (.Dup ⟨3, by decide⟩),
