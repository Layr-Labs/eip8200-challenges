...
theorem run_fallback (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock fallbackPath (branchState input 4902) =
      some (fallbackState input) := by
  have hjump : Decode.isValidJumpDest submissionBytecode 4919 = true :=
    Artifact.isValidJumpDest_index 2197 (by rfl)
  simp [fallbackPath, Main.opAt, Main.pushAt, Main.wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    fallbackState, Main.trampolineState, branchState, initialState, hjump, g257PC0, g257PC1, g257PC2,
