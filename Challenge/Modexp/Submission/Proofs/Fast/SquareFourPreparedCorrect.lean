...
theorem represents_double_four (s : State) (mem : ByteArray) (pa a : Nat)
    (hpa : pa+128 ≤ 2048) (ha : Model.FastRepresents mem pa 4 a) :
    Model.FastRepresents (prepared s mem pa pa 4) 2368 5 (2*a) := by
  ...
  have hs := Csub.fastRepresents_mcopy (selected mem pa pa 4) pa 2400 4 a (by decide) hsel
  have hb : Model.FastRepresents (before mem pa pa 4) 2400 4 a := by
  ...
  exact (Model.fastRepresents_congr (fun j hj =>
    mpZeroed_readWord_outside s (before mem pa pa 4) 4 (2400+32*j) (Or.inr (by omega))) a).2 hb

theorem tValue_zero_four (s : State) (mem : ByteArray) (pa : Nat) :
    tValue (prepared s mem pa pa 4) 4 = 0 := by
  rw [prepared, if_neg (by simp : ¬(4=8 ∧ pa=pa)), if_pos ⟨rfl,rfl⟩, tValue, SquareFourInit.read_init_outside _ 2080 (Or.inl (by decide)), readWord_mpZeroed_tn]
  have hrep : Model.FastRepresents (SquareFourInit.initMemory (mpZeroed s (before mem pa pa 4) 4)) 2112 4 0 :=
    (Model.fastRepresents_congr (fun j hj => SquareFourInit.read_init_outside _ (2112+32*j) (Or.inl (by omega))) 0).2
      (fastRepresents_mpZeroed s (before mem pa pa 4) 4)
  have hlow := Model.fastRepresents_value_unique (Csub.fastRepresents_lowValue _ 2112 4) hrep
  rw [hlow]
...
theorem init_high_clear_four (mem : ByteArray) :
    SquareWords.clearBit (MachineState.readWord (SquareFourInit.initMemory mem) 2368) = UInt256.ofNat 0 := by
  have ha := Csub.fastRepresents_lowValue mem 2400 4
  have hd := SquareFourInit.init_represents mem (Csub.lowValue mem 2400 4 4) ha
  have hc := SquareFourCoefficients.coefficient_high_le_one
    (SquareFourInit.initMemory mem) (Csub.lowValue mem 2400 4 4) (UInt256.ofNat 0) 0 hd ha.1 (by decide)
...
theorem square_spec_four (s : State) (mem : ByteArray) (pa pdst a m : Nat)
    ...
    (hinv : ((MachineState.readWord mem 96).toNat * (MachineState.readWord mem 2816).toNat+1) % 2^256 = 0) :
    (MachineState.readWord (SquareFourRowsModel.rows (prepared s mem pa pa 4) pa 4) 2080).toNat ≤ 1 ∧
  ...
  have hi : ((MachineState.readWord (prepared s mem pa pa 4) 96).toNat *
      (MachineState.readWord (prepared s mem pa pa 4) 2816).toNat+1) % 2^256 = 0 := by
    rw [read_prepared_outside s mem pa pa 4 96 (by decide) (Or.inl (by decide)),
      read_prepared_outside s mem pa pa 4 2816 (by decide) (Or.inr (by decide))]
    exact hinv
