...
theorem handled_of_fixed (input : ByteArray) (s : State) (memory : ByteArray)
    ...
    FixedExponentRoute.Handled input
      (square s memory n bsize esize msize count) := by
  ...
  have htraceSq := gasSteps_squareLoop s sub spec memory esize msize count
    bM rawBase hm hn32 hcount hcount16 hbMlt
    hframe ⟨hmod, hrawAcc, hbase, hone⟩ hcode hfork hrun hnp
  ...
  refine Exp.handled_of_trace input
    (square s memory n bsize esize msize count) s memOut
