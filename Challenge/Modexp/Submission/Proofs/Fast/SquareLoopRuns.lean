...
The four short blocks of the loop (`sq_exit` 4824, `last` 4841, `more` 4896) as
`runInstructions` lemmas and gas steps, plus the last square's exit (`nx` 4766, the 14
`POP`s and the jump to the CSUB guard) and the final conditional subtraction on an abstract
memory.  `again` (4905), the only memory-heavy block, lives in `SquareLoopAgain`; splitting
...
/-- `last` (4841): rewrite the frame's `ret` slot to `after_sq` and jump to `nx`. -/
theorem run_last (s : State) (mem : ByteArray) (n : Nat)
    ...
    some (frameAt pcNx s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst
      (UInt256.ofNat 3266) rest) := by
  have hc16' : rest.length + 16 < 1024 := by omega
  have hc17 : rest.length + 17 < 1024 := by omega
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4777 = true := by
...
theorem run_more (s : State) (mem : ByteArray) (n : Nat)
    ...
    some (mpCsubState s mem pdst (UInt256.ofNat 4869)
...
def gasSteps_last (s : State) (mem : ByteArray) (n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (frameAt pcLast s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      (frameAt pcNx s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst (UInt256.ofNat 3266) rest) :=
  lastBlock.steps
...
def gasSteps_more (s : State) (mem : ByteArray) (n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (frameAt pcMore s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      (mpCsubState s mem pdst (UInt256.ofNat 4869)
...
def gasSteps_nxExit (s : State) (mem : ByteArray) (n : Nat)
  ...
  (CarryRowGas.gasSteps_nxJd s mem pbi 2048 n (UInt256.ofNat 4902) ent inv m0
  ...
  (CarryRowBlocks.exitBlock.steps
    (CarryRowBlocks.environment (CiosCachedTailDefs.nxState s mem pbi 2048 n
      (UInt256.ofNat 4902) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest)) hcode hfork hrun hnp) rfl
    (CiosReadonly.run_exit { s with memory := mem } pbi (UInt256.ofNat 4902)
