...
theorem run_sqExit_more (s : State) (mem : ByteArray) (n c : Nat)
  ...
  have htrue : UInt256.isTrue (UInt256.ofNat c) := by
    ...
    rw [hcNat]; omega
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4894 = true := by
...
theorem run_last (s : State) (mem : ByteArray) (n : Nat)
  ...
  have hc16' : rest.length + 16 < 1024 := by omega
  have hc17 : rest.length + 17 < 1024 := by omega
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4805 = true := by
...
theorem run_more (s : State) (mem : ByteArray) (n : Nat)
    ...
    some (mpCsubState s mem pdst (UInt256.ofNat 4910)
...
def gasSteps_more (s : State) (mem : ByteArray) (n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (frameAt pcMore s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      (mpCsubState s mem pdst (UInt256.ofNat 4910)
...
def gasSteps_nxExit (s : State) (mem : ByteArray) (n : Nat)
  ...
  (CarryRowGas.gasSteps_nxJd s mem pbi 512 n (UInt256.ofNat 4943) ent inv m0
  ...
  (CarryRowBlocks.exitBlock.steps
    (CarryRowBlocks.environment (CiosCachedTailDefs.nxState s mem pbi 512 n
      (UInt256.ofNat 4943) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest)) hcode hfork hrun hnp) rfl
    (CiosReadonly.run_exit { s with memory := mem } pbi (UInt256.ofNat 4943)
