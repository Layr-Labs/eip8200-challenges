...
The four short blocks of the loop (`sq_exit` 4906, `last` 4923, `more` 4896) as
`runInstructions` lemmas and gas steps, plus the last square's exit (`nx` 4848, the 14
`POP`s and the jump to the CSUB guard) and the final conditional subtraction on an abstract
memory.  `again` (4987), the only memory-heavy block, lives in `SquareLoopAgain`; splitting
...
private theorem activeWords9280 (s : State) (hact : 168 ≤ s.activeWords.toNat) :
    UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 5184 32) =
      s.activeWords := by
  have hnat : MachineState.activeWordsAfter s.activeWords.toNat 5184 32 =
...
theorem run_sqExit_more (s : State) (mem : ByteArray) (n c : Nat)
    ...
    (hact : 168 ≤ s.activeWords.toNat) (hcpos : 0 < c) (hc16 : c + 1 ≤ 16)
    (hcount : MachineState.readWord mem 5184 = UInt256.ofNat (c + 1)) :
  ...
  have htrue : UInt256.isTrue (UInt256.ofNat c) := by
    ...
    rw [hcNat]; omega
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4942 = true := by
    rw [hcode]; exact jumpDest4753
  have h9280 : (5184 : UInt256).toNat = 5184 := by decide
...
theorem run_sqExit_last (s : State) (mem : ByteArray) (n : Nat)
    ...
    (hcount : MachineState.readWord mem 5184 = UInt256.ofNat 1) :
    runInstructions sqExitProgram
      (frameAt pcSqExit s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest) =
...
/-- `last` (4923): rewrite the frame's `ret` slot to `after_sq` and jump to `nx`. -/
theorem run_last (s : State) (mem : ByteArray) (n : Nat)
    ...
    some (frameAt pcNx s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst
      (UInt256.ofNat 3348) rest) := by
  have hc16' : rest.length + 16 < 1024 := by omega
  have hc17 : rest.length + 17 < 1024 := by omega
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4859 = true := by
...
/-- `more` (4896): enter the CSUB guard as a subroutine with `[pdst, again]` on top of the
retained frame. -/
theorem run_more (s : State) (mem : ByteArray) (n : Nat)
    ...
    some (mpCsubState s mem pdst (UInt256.ofNat 4951)
...
def gasSteps_last (s : State) (mem : ByteArray) (n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (frameAt pcLast s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      (frameAt pcNx s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst (UInt256.ofNat 3348) rest) :=
  lastBlock.steps
...
def gasSteps_more (s : State) (mem : ByteArray) (n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (frameAt pcMore s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      (mpCsubState s mem pdst (UInt256.ofNat 4951)
...
def gasSteps_nxExit (s : State) (mem : ByteArray) (n : Nat)
  ...
  (CarryRowGas.gasSteps_nxJd s mem pbi 512 n (UInt256.ofNat 4984) ent inv m0
  ...
  (CarryRowBlocks.exitBlock.steps
    (CarryRowBlocks.environment (CiosCachedTailDefs.nxState s mem pbi 512 n
      (UInt256.ofNat 4984) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest)) hcode hfork hrun hnp) rfl
    (CiosReadonly.run_exit { s with memory := mem } pbi (UInt256.ofNat 4984)
      (UInt256.ofNat (512 - 32)) ent (l2Target n) tl inv m0 aprev m96 m64 m32 pdst ret rest
      hcap))
...
def gasSteps_csubAt (s : State) (X : ByteArray) (n : Nat) (ret : UInt256)
    ...
    (hml : MachineState.readWord X 5312 = UInt256.ofNat (32 * n - 32))
    (htl : MachineState.readWord X 5344 = UInt256.ofNat (4128 + 32 * n))
    (hs32 : MachineState.readWord (Csub.csStep X n n).memory 5248 = UInt256.ofNat (32 * n))
    (htn : (MachineState.readWord (Csub.csStep X n n).memory 4128).toNat ≤ 1) :
    Challenge.EvmProof.GasSteps
      (mpCsubState s X (UInt256.ofNat 512) ret tail)
      { s with pc := ret, stack := tail, memory := Csub.csResultMemory X n 512 } := by
  have g := Csub.gasSteps_csub s X n (UInt256.ofNat 512) ret tail hcap hcode hfork hrun hnp
    hact hn hn32 hjump hml htl hs32
    (by rw [show (UInt256.ofNat 512).toNat = 512 from by decide]; omega) htn
  refine g.cast rfl ?_
  rw [show Csub.csReturnedState s X n n (UInt256.ofNat 512) ret tail =
      { s with pc := ret, stack := tail,
               memory := Csub.csResultMemory X n (UInt256.ofNat 512).toNat } from rfl,
    show (UInt256.ofNat 512).toNat = 512 from by decide]
