...
theorem run_sqExit_more (s : State) (mem : ByteArray) (n c : Nat)
  ...
  have hc16' : rest.length + 16 < 256 := by omega
  have hc17 : rest.length + 17 < 256 := by omega
  have hc18 : rest.length + 18 < 256 := by omega
  have hc19 : rest.length + 19 < 256 := by omega
...
def gasSteps_last (s : State) (mem : ByteArray) (n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (frameAt pcLast s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      (frameAt pcNx s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst (UInt256.ofNat 3231) rest) :=
  lastBlock.steps
...
def gasSteps_nxExit (s : State) (mem : ByteArray) (n : Nat)
  ...
  (CarryRowGas.gasSteps_nxJd s mem pbi 512 n (UInt256.ofNat 4788) ent inv m0
  ...
  (CarryRowBlocks.exitBlock.steps
    (CarryRowBlocks.environment (CiosCachedTailDefs.nxState s mem pbi 512 n
      (UInt256.ofNat 4788) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aprev :: pdst :: ret :: rest)) hcode hfork hrun hnp) rfl
    (CiosReadonly.run_exit { s with memory := mem } pbi (UInt256.ofNat 4788)
      (UInt256.ofNat (512 - 32)) ent (l2Target n) tl inv m0 aprev m96 m64 m32 pdst ret rest
      hcap))

/-- The final conditional subtraction on an abstract memory: `[512, ret]` above `tail`. -/
