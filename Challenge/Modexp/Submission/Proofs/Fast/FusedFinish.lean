...
def block : Block Artifact.submissionArtifact .Osaka 1696 program :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 1215 2 1696 program
    (by decide) (by rfl) (by rfl) (by decide)

theorem run (s : State) (mem : ByteArray) (count : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1000) :
    runInstructions program (Exp.retTo s mem (UInt256.ofNat 1696) (count::rest)) =
      some (Exp.retTo s mem (UInt256.ofNat 1698) rest) := by
...
def gasSteps (s : State) (mem : ByteArray) (count : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.GasSteps (Exp.retTo s mem (UInt256.ofNat 1696) (count::rest))
      (Exp.retTo s mem (UInt256.ofNat 1698) rest) :=
