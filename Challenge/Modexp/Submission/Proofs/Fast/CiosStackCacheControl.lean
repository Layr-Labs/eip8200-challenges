...
theorem run_outWords (s : State) (c : CachedMemory) (r : ReadOnlyCache)
    ...
    runInstructions CiosStackCachePrograms.out
      (framed s (UInt256.ofNat 4243) (rowFrame c r pbi paEnd pbEnd flag dst ret rest)) =
...
theorem run_out (s : State) (c : CachedMemory) (r : ReadOnlyCache)
    ...
    some (l1At 4248 s c r (rowBi c.virtual pb n i) pa pb n i 0 dst ret rest) := by
...
theorem dispatch_l1 : CiosStackCachePrograms.l1Dispatch = dispatchFor 4405 := rfl
theorem dispatch_l2 : CiosStackCachePrograms.l2Dispatch = dispatchFor 4729 := rfl
