...
theorem run_outWords (s : State) (c : CachedMemory) (r : ReadOnlyCache)
    ...
    runInstructions CiosStackCachePrograms.out
      (framed s (UInt256.ofNat 4595) (rowFrame c r pbi paEnd pbEnd flag dst ret rest)) =
    some (framed s (UInt256.ofNat 4600)
...
theorem run_out (s : State) (c : CachedMemory) (r : ReadOnlyCache)
    ...
    some (l1At 4600 s c r (rowBi c.virtual pb n i) pa pb n i 0 dst ret rest) := by
...
theorem dispatch_l1 : CiosStackCachePrograms.l1Dispatch = dispatchFor 4757 := rfl
theorem dispatch_l2 : CiosStackCachePrograms.l2Dispatch = dispatchFor 5085 := rfl
