...
theorem square_spec (s : State) (mem : ByteArray) (pa pdst a m : Nat)
    ...
    (hinv : ((MachineState.readWord mem 224).toNat * (MachineState.readWord mem 2816).toNat+1) % 2^256 = 0) :
    (MachineState.readWord (SquareRowsModel.rows (prepared s mem pa pa 8) pa 8) 2080).toNat ≤ 1 ∧
  ...
  have hi : ((MachineState.readWord (prepared s mem pa pa 8) 224).toNat *
      (MachineState.readWord (prepared s mem pa pa 8) 2816).toNat+1) % 2^256 = 0 := by
    rw [read_prepared_outside s mem pa pa 8 224 (by decide) (Or.inl (by decide)),
      read_prepared_outside s mem pa pa 8 2816 (by decide) (Or.inr (by decide))]
    exact hinv
