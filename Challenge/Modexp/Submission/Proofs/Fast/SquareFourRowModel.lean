...
import Challenge.Modexp.Submission.Proofs.Fast.SquareFourCoefficients
...
namespace Challenge.Modexp.Submission.Proofs.Fast.SquareFourRowModel
open EvmSemantics EvmSemantics.EVM
open Challenge.Modexp.Submission.Proofs.Fast
open Monpro SquareInit SquareFourProducts SquareFourCoefficients

def product (mem : ByteArray) (ai : UInt256) (i : Nat) : MacState :=
  products mem (coefficient mem ai i) ai i (4-i)

def extra (mem : ByteArray) (ai : UInt256) (i : Nat) : UInt256 :=
  ai * coefficient mem ai i 4
...
theorem low_mid (mem : ByteArray) (ai : UInt256) (i : Nat) :
    Csub.lowValue (mid mem ai i) 2112 4 4 = low (product mem ai i).memory := by
...
theorem extra_toNat (mem : ByteArray) (a : Nat) (ai : UInt256) (i : Nat)
    (hd : Model.FastRepresents mem 2368 5 (2*a)) (ha : a < Limbs.radix^4)
    (hi : i < 4) :
    (extra mem ai i).toNat = ai.toNat*(coefficient mem ai i 4).toNat := by
...
theorem row_equation (mem : ByteArray) (a m : Nat) (ai : UInt256) (i : Nat)
    (hd : Model.FastRepresents mem 2368 5 (2*a))
    (hm : Model.FastRepresents mem 0 4 m) (ha : a < Limbs.radix^4)
    (hai : ai.toNat = a / Limbs.radix^i % Limbs.radix) (hi : i < 4)
    (hinv : ((MachineState.readWord mem 96).toNat *
      (MachineState.readWord mem 2816).toNat+1) % 2^256 = 0) :
    tValue (row mem ai i) 4*Limbs.radix =
      tValue mem 4 + SquareArithmetic.rowTerm Limbs.radix a i +
        (rowMu (mid mem ai i) 4).toNat*m := by
  have hmid := represents_mid mem ai i 0 4 m (by omega) (Or.inl (by decide)) hm
  have hiv : ((MachineState.readWord (mid mem ai i) 96).toNat *
    ...
    exact hinv
  have red := SquareFourReduce.reduction_equation (mid mem ai i) (flag mem ai i) m
  ...
  have prod := products_invariant mem (coefficient mem ai i) ai i (4-i) (by omega)
  have hlen : i+(4-i) = 4 := by omega
  change low (product mem ai i).memory + (product mem ai i).carry.toNat*Limbs.radix^(i+(4-i)) =
    low mem + ai.toNat*weighted (fun j => (coefficient mem ai i j).toNat) i (4-i) at prod
  rw [hlen] at prod
  have coeff := weighted_coefficient mem a ai i hd hi
  have hlen' : 5-i = (4-i)+1 := by omega
  rw [hlen', weighted, hlen] at coeff
  change tValue (row mem ai i) 4*Limbs.radix = _ at red
  rw [low_mid] at red
  ...
  rw [top] at red
  have hlow : low mem = Csub.lowValue mem 2112 4 4 := limbSum_eq_lowValue mem 2112 4 4
  have heq : low (product mem ai i).memory +
      ((product mem ai i).carry.toNat + ai.toNat*(coefficient mem ai i 4).toNat)*Limbs.radix^4 =
      low mem + ai.toNat*(ai.toNat*Limbs.radix^i +
        2*(a/Limbs.radix^(i+1))*Limbs.radix^(i+1)) := by
    calc
      _ = (low (product mem ai i).memory+(product mem ai i).carry.toNat*Limbs.radix^4) +
        ai.toNat*(coefficient mem ai i 4).toNat*Limbs.radix^4 := by ring
      _ = low mem + ai.toNat*(weighted (fun j => (coefficient mem ai i j).toNat) i (4-i) +
          (coefficient mem ai i 4).toNat*Limbs.radix^4) := by rw [prod]; ring
      _ = _ := by rw [coeff]
  calc
    _ = (MachineState.readWord mem 2080).toNat*Limbs.radix^4 +
      (low (product mem ai i).memory +
       ((product mem ai i).carry.toNat + ai.toNat*(coefficient mem ai i 4).toNat)*Limbs.radix^4) +
      (rowMu (mid mem ai i) 4).toNat*m := by rw [red]; ring
    _ = _ := by rw [heq, hlow, hai]; simp only [tValue, SquareArithmetic.rowTerm]; ring

end Challenge.Modexp.Submission.Proofs.Fast.SquareFourRowModel
