...
theorem run_l1FifthMac (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock fifthMac
      (l1At 4455 s mem bi pa pb n i j pdst ret rest) =
      some (l1At 4524 s mem bi pa pb n i (j + 1) pdst ret rest) := by
  ...
  simp (config := { maxSteps := 800000 })
    [fifthMac, Cios2Paths.L1.fifthPC, Cios2Paths.L1.fifthStartIndex,
      Cios2Paths.L1.fifthOpAt, Cios2Paths.L1.fifthPushAt, wfOp,
...
def gasSteps_l1FifthMac (s : State) (mem : ByteArray) (bi : UInt256)
    (pa pb n i j : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hfork : s.fork = .Osaka)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 4455 s mem bi pa pb n i j pdst ret rest)
      (l1At 4524 s mem bi pa pb n i (j + 1) pdst ret rest) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka fifthMac hcode hfork
    (run_l1FifthMac s mem bi pa pb n i j pdst ret rest hcap hrun hcode hact hn32 hj
      hpa hpaFit) hrun hnp
