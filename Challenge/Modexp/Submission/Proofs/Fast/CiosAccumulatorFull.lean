import Challenge.Modexp.Submission.Proofs.Fast.CiosAccumulatorRows
import Challenge.Modexp.Submission.Proofs.Fast.Cios2Dispatch
import Challenge.Modexp.Submission.Proofs.Fast.CiosAccumulatorInvariants
...
namespace Challenge.Modexp.Submission.Proofs.Fast.CiosAccumulatorFull
...
open CiosCached CiosCachedMidMemory CiosAccumulatorInvariants CarryRowModel CarryResult CiosCarryFrames


def rowSlot (mem : ByteArray) (pa pb n : Nat) : Nat → UInt256
  | 0 => negative32
  | i+1 => rowTN (rowsCarry mem pa pb n i) pa pb n i
...
opaque gasSteps_rowsFour (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (outState (carrySlot := negative32) s (mpZeroed s mem 4) pa pb 4 0 inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
  ...
  refine (Challenge.EvmProof.GasSteps.iterateBounded
    (I := fun i => outState (carrySlot := rowSlot (mpZeroed s mem 4) pa pb 4 i) s (rowsCarry (mpZeroed s mem 4) pa pb 4 i)
      pa pb 4 i inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) 3 ?_).trans ?_
  · intro i hi
    exact gasSteps_rowFourNext (carrySlot := rowSlot (mpZeroed s mem 4) pa pb 4 i) s (rowsCarry (mpZeroed s mem 4) pa pb 4 i) pa pb i
  ...
  · simpa only [rowsCarry] using
      gasSteps_rowFourLast (carrySlot := rowSlot (mpZeroed s mem 4) pa pb 4 3) s (rowsCarry (mpZeroed s mem 4) pa pb 4 3) pa pb 3
...
opaque gasSteps_rowsEight (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (outState (carrySlot := negative32) s (mpZeroed s mem 8) pa pb 8 0 inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
  ...
  have hez := he.zeroed s (by decide) (by decide) hpaFit
  refine (Challenge.EvmProof.GasSteps.iterateBounded
    (I := fun i => outState (carrySlot := rowSlot (mpZeroed s mem 8) pa pb 8 i) s (rowsCarry (mpZeroed s mem 8) pa pb 8 i)
      pa pb 8 i inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) 7 ?_).trans ?_
  · intro i hi
    exact gasSteps_rowEightNext (carrySlot := rowSlot (mpZeroed s mem 8) pa pb 8 i) s (rowsCarry (mpZeroed s mem 8) pa pb 8 i) pa pb i
      tl inv m0 aEnd m96 m64 m32 pdst ret rest (by omega) hrun hcode hfork hnp hact (by omega) hpa hpaFit hpb
      hpbFit
      ...
      (operand_rows hez pb i (by decide) (by decide) hpaFit)
  · simpa only [rowsCarry] using
      gasSteps_rowEightLast (carrySlot := rowSlot (mpZeroed s mem 8) pa pb 8 7) s (rowsCarry (mpZeroed s mem 8) pa pb 8 7) pa pb 7
        tl inv m0 aEnd m96 m64 m32 pdst ret rest (by omega) hrun hcode hfork hnp hact (by decide) hpa hpaFit hpb
        hpbFit
        ((readWord_rowsCarry (mpZeroed s mem 8) pa pb 8 9344 (by decide) (by omega)
          7).trans hs32z)
...
end Challenge.Modexp.Submission.Proofs.Fast.CiosAccumulatorFull
