...
def guardProgram : List Instr :=
  [.op .JUMPDEST, .op (.Dup ⟨0, by decide⟩),
   ...
   .op .AND, .op .ISZERO,
   .push ⟨2, by decide⟩ (UInt256.ofNat 3096), .op .JUMPI]
...
theorem run_guard_word (s : State) (memory : ByteArray)
    (n bsize esize msize : Nat) (hactive : 93 ≤ s.activeWords.toNat)
    (hjump : Decode.isValidJumpDest s.executionEnv.code 3096 = true) :
