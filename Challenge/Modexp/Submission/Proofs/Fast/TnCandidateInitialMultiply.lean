import Challenge.Modexp.Submission.Proofs.Fast.TnCandidateMultiplySteps
...
namespace Challenge.Modexp.Submission.Proofs.Fast.TnCandidateInitialMultiply
...
open TnCandidateRowsSteps TnCandidateRowSteps TnCandidateReductionSteps
...
noncomputable def rows_steps (s : State)
    ...
    GasSteps
      (TnCacheSetup.outState s (mpZeroed s mem n) pb n 0 (UInt256.ofNat 3711)
        (TnCacheSetup.l1Target n) inv m0
  ...
  have ha : aEnd.toNat = pa+32*(n-1) := by
    ...
    omega
  have g := TnCandidateMultiplySteps.multiply_steps s env z pa pb n hn
    tl inv m0 aEnd m96 m64 m32 dst ret rest hcap hact hpa hpb (by omega)
    (Or.inl (by omega)) ha hz
  have hstart : TnCacheSetup.outState s (mpZeroed s mem n) pb n 0 (UInt256.ofNat 3711)
      (TnCacheSetup.l1Target n) inv m0
      (tl :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest) =
      rowState s z pb n 0 tl inv m0 aEnd m96 m64 m32 dst ret rest := by
    rcases hn with rfl | rfl <;>
      simp [TnCacheSetup.outState, TnCacheSetup.l1Target, TnCacheSetup.l2Target,
        TnCacheSetup.zeroTn, rowState, TnCacheRowPointers.pointer, l1PC, l2PC,
        isFour, z, TnCacheFrameOps.frame, framed] <;> decide
  rw [hstart]
  simpa only [z, TnCacheInitialMemory.lift_zeroed] using g

#print axioms rows_steps
end Challenge.Modexp.Submission.Proofs.Fast.TnCandidateInitialMultiply
