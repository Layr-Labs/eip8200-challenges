/- Kernel `setup` (pc 4116, 0x0f6c) of sqCP1m: the 3-slot variant of the entry permutation
...
namespace Challenge.Modexp.Submission.Proofs.Fast.TnCacheSetup.EntryPrefix
...
theorem run_reads (s : State) (hd pa pb dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions readsProgram
      {s with pc := UInt256.ofNat 3571, stack := [hd, pa, pb, dst, ret] ++ rest} =
    some {s with
...
theorem run_setup (s : State) (hd pa pb dst ret value inverse aEnd tailPointer low96 low32 : UInt256)
  ...
  simp [setupProgram, loadProgram, runInstructions, Challenge.EvmProof.Stepper.runInstr,
    displacement, zeroTn, ← negative32_not, h9344, State.activeWordsAfterUInt256, hact3,
...
(pc 4162 → 4170): `ent = 4286 + delta` and `l2T = 292 + ent` enter the frame; `hd`
...
theorem run_shuffle (s : State) (hd pa pb dst ret value inverse aEnd tailPointer low96 low32 delta : UInt256)
    ...
    some {s with
        pc := UInt256.ofNat 3623
        stack := [pa, pb, UInt256.ofNat 3740 + delta, zeroTn, allOnes,
        UInt256.ofNat 4023 + delta, inverse, value, tailPointer, low96, hd, low32, aEnd, dst, ret] ++ rest} := by
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  have hc7 : rest.length + 7 < 1024 := by omega
...
end Challenge.Modexp.Submission.Proofs.Fast.TnCacheSetup.EntryPrefix
