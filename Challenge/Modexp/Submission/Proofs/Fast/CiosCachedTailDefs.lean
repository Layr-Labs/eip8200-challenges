...
/-- The row loop's `JUMPI` (pc 4856) falls through into the R0 kernel-exit **dispatch**
(pc 4857), which sends a square (`hd = sq_row`) to `sq_exit` and lets every multiply
fall through to `nx` (pc 4866). -/
...
/-- The `nx` side of the R0 dispatch (pc 4867): the 14 `POP`s that drop the row frame. -/
...
/-- The `nx` `JUMPDEST` itself (pc 4866): the last square jumps here from `sq_exit`. -/
...
/-- `sq_exit` (pc 4925): the in-kernel squaring loop keeps the row frame. -/
...
def input (s : State) (c mu bi pbi paEnd pbEnd flag dst ret : UInt256)
  ...
  framed s (UInt256.ofNat 4271) ([c, mu, bi] ++ baseStack pbi paEnd pbEnd flag dst ret rest)
...
def stored (s : State) (c pbi paEnd pbEnd flag dst ret : UInt256) (rest : List UInt256) : State :=
  framed { s with memory := tailMem s.memory c } (UInt256.ofNat 4283)
