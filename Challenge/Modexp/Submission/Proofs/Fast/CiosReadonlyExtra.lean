...
def extendedStack (carry mu bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret : UInt256)
    (rest : List UInt256) : List UInt256 :=
  [carry,mu,bi,pbi,pa,pb,flag,negative32,allOnes,target2,inv,m0,m128,m96,m64,m32,tl,dst,ret] ++ rest

def cacheAddress (slot : Fin 4) : Nat := 128-32*slot.val
def cacheWord (slot : Fin 4) (m128 m96 m64 m32 : UInt256) : UInt256 :=
  if slot.val = 0 then m128 else if slot.val = 1 then m96 else if slot.val = 2 then m64 else m32

def extraLoad (slot : Fin 4) : List Instr :=
  [.op (.Dup ⟨12+slot.val, by omega⟩),
   .op (.Dup ⟨9, by decide⟩)]

theorem run_extraLoad (slot : Fin 4) (s : State)
    (pc carry mu bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998) :
    runInstructions (extraLoad slot)
      (framed s pc (extendedStack carry mu bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) =
    some (framed s pc.succ.succ
      ([allOnes,cacheWord slot m128 m96 m64 m32] ++
        extendedStack carry mu bi pbi pa pb flag target2 tl inv m0 m128 m96 m64 m32 dst ret rest)) := by
...
structure ExtraCache (mem : ByteArray) (m128 m96 m64 m32 : UInt256) : Prop where
  word128 : m128 = MachineState.readWord mem 128
  word96 : m96 = MachineState.readWord mem 96
  word64 : m64 = MachineState.readWord mem 64
  word32 : m32 = MachineState.readWord mem 32

theorem ExtraCache.of_preserved {mem mem' : ByteArray} {m128 m96 m64 m32 : UInt256}
    (hc : ExtraCache mem m128 m96 m64 m32)
    (h128 : MachineState.readWord mem' 128 = MachineState.readWord mem 128)
    ...
    ExtraCache mem' m128 m96 m64 m32 := ⟨hc.word128.trans h128.symm,hc.word96.trans h96.symm,hc.word64.trans h64.symm,hc.word32.trans h32.symm⟩

theorem ExtraCache.l1 {mem : ByteArray} {m128 m96 m64 m32 : UInt256}
    (hc : ExtraCache mem m128 m96 m64 m32) (bi : UInt256) (pa n j : Nat) (hn : n ≤ 32) :
    ExtraCache (l1Step mem bi pa n j).memory m128 m96 m64 m32 :=
  hc.of_preserved (readWord_l1Step mem bi pa n 128 j hn (Or.inl (by decide)))
...
theorem ExtraCache.l2 {mem : ByteArray} {m128 m96 m64 m32 : UInt256}
    (hc : ExtraCache mem m128 m96 m64 m32) (mu c0 : UInt256) (n k : Nat) (hn : n ≤ 32) :
    ExtraCache (l2Step mem mu c0 n k).memory m128 m96 m64 m32 :=
  hc.of_preserved (readWord_l2Step mem mu c0 n 128 k hn (Or.inl (by decide)))
...
theorem ExtraCache.rows {mem : ByteArray} {m128 m96 m64 m32 : UInt256}
    (hc : ExtraCache mem m128 m96 m64 m32) (pa pb n i : Nat) (hn : n ≤ 32) :
    ExtraCache (rowsMem mem pa pb n i) m128 m96 m64 m32 :=
  hc.of_preserved (readWord_rowsMem mem pa pb n 128 hn (Or.inl (by decide)) i)
    (readWord_rowsMem mem pa pb n 96 hn (Or.inl (by decide)) i)
    (readWord_rowsMem mem pa pb n 64 hn (Or.inl (by decide)) i)
    (readWord_rowsMem mem pa pb n 32 hn (Or.inl (by decide)) i)

theorem ExtraCache.middle {mem : ByteArray} {m128 m96 m64 m32 : UInt256}
    (hc : ExtraCache mem m128 m96 m64 m32) (c : UInt256) :
    ExtraCache (Monpro.midMem mem c) m128 m96 m64 m32 :=
  hc.of_preserved
    (CiosCachedMidMemory.read_mid mem c 128 (Or.inl (by decide)))
...
theorem ExtraCache.zeroed {mem : ByteArray} {m128 m96 m64 m32 : UInt256}
    (hc : ExtraCache mem m128 m96 m64 m32) (s : State) (n : Nat) (hn : n ≤ 32) :
    ExtraCache (mpZeroed s mem n) m128 m96 m64 m32 :=
  hc.of_preserved
    (readWord_mpZeroed s mem n 128 hn (Or.inl (by decide)))
...
theorem ExtraCache.choose {mem : ByteArray} {m128 m96 m64 m32 : UInt256}
    (hc : ExtraCache mem m128 m96 m64 m32) (slot : Fin 4) :
    (cacheWord slot m128 m96 m64 m32) = MachineState.readWord mem (cacheAddress slot) := by
  fin_cases slot
  · exact hc.word128
  · exact hc.word96
  · exact hc.word64
...
def extraProgram (slot : Fin 4) (tl ts : UInt256) : List Instr :=
  (extraLoad slot ++ L2.productProgram) ++ L2.finishProgram tl ts

theorem run_extraStep (slot : Fin 4) (template : State) (pc : UInt256) (mem : ByteArray)
    ...
    (pbi paEnd pbEnd flag target2 cachedTL inv m0 m128 m96 m64 m32 dst ret : UInt256)
    ...
    runInstructions (extraProgram slot loadAddr storeAddr)
      (CiosCachedL2.state template pc mem bi mu c0 n k pbi paEnd pbEnd flag target2 inv
        (m0 :: m128 :: m96 :: m64 :: m32 :: cachedTL :: dst :: ret :: rest)) =
    some (CiosCachedL2.state template (pc+UInt256.ofNat 34) mem bi mu c0 n (k+1)
      pbi paEnd pbEnd flag target2 inv (m0 :: m128 :: m96 :: m64 :: m32 :: cachedTL :: dst :: ret :: rest)) := by
  ...
  have hvalue : (cacheWord slot m128 m96 m64 m32) = MachineState.readWord st.memory x.toNat := by
    rw [hselect]
    exact (hc.l2 mu c0 n k hn).choose slot
  have hl := run_extraLoad slot st pc (l2Step mem mu c0 n k).carry mu bi
    pbi paEnd pbEnd flag target2 cachedTL inv m0 m128 m96 m64 m32 dst ret rest hrest
  ...
  have hp := L2.run_product st (pc+UInt256.ofNat 2)
    ...
    ([bi,pbi,paEnd,pbEnd,flag,negative32,allOnes,target2,inv,m0] ++
      (m128 :: m96 :: m64 :: m32 :: cachedTL :: dst :: ret :: rest))
