...
opaque gasSteps_specializedEight (s : State) (mem : ByteArray) (pa pb : Nat)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (mpCsubState s (rowsCarry (mpZeroed s (before mem pa pb 8) 8) pa pb 8 8) pdst ret rest) := by
  ...
  have hr : Challenge.EvmProof.GasSteps
      {SquareEntry.out s mem pa pb 8 pdst ret rest with pc := UInt256.ofNat 4160}
      (SquareEntry.out s mem pa pb 8 pdst ret rest) := by
    exact SquareEntry.gasSteps_route s (mpZeroed s (before mem pa pb 8) 8) (UInt256.ofNat 4165)
  ...
  exact gasSteps_rowsEight s (before mem pa pb 8) pa pb
    ...
    (MachineState.readWord mem (32*8-32)) (MachineState.readWord mem (pa+32*8-32))
    ...
    (hread (pa+32*8-32) (Or.inl (by omega))).symm hsz hctrl
