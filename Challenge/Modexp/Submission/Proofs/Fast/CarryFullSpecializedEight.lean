...
opaque gasSteps_specializedEight (s : State) (mem : ByteArray) (pa pb : Nat)
  ...
  have hr : Challenge.EvmProof.GasSteps
      {SquareEntry.out s mem pa pb 8 pdst ret rest with pc := UInt256.ofNat 4164}
      (SquareEntry.out s mem pa pb 8 pdst ret rest) := by
    exact SquareEntry.gasSteps_route s (mpZeroed s (before mem pa pb 8) 8) (UInt256.ofNat 4169)
