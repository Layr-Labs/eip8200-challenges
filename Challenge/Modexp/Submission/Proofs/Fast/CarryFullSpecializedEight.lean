...
opaque gasSteps_specializedEight (s : State) (mem : ByteArray) (pa pb : Nat)
  ...
  refine (gasSteps_dispatch8 s mem pa pb pdst ret rest (by omega) hrun hcode hfork hnp hact
    (by simpa using hs32)).trans ?_
  refine (gasSteps_entry s mem pa pb 8 pdst ret rest (by omega) hrun hcode hfork hnp hact
