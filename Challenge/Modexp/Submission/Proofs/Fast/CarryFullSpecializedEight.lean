...
/-- An eight-limb multiply: `mul entry` (pc 4063) → `common` → `setup` → the eight rows (row head
`hd = 4260`) → the final subtraction (pc 4917). -/
opaque gasSteps_specializedEight (L : RowLemmas) (E : EntryLemmas) (s : State) (mem : ByteArray)
    ...
    (hpa : 32 ≤ pa) (hpaFit : pa + 32 * 8 ≤ 4096)
    (hpb : 32 ≤ pb) (hpbFit : pb + 32 * 8 ≤ 5376)
    (hcds : s.executionEnv.calldata.size < 2 ^ 256)
    (hs32 : MachineState.readWord mem 5248 = UInt256.ofNat (32 * 8))
    (htl : MachineState.readWord mem 5344 = UInt256.ofNat (4128 + 32 * 8))
    (hml : MachineState.readWord mem 5312 = UInt256.ofNat (32 * 8 - 32))
  ...
  refine (E.gasSteps_commonSetup s mem (UInt256.ofNat 4252) pa pb 8 pdst ret rest hcap hrun hcode
  ...
  exact gasSteps_rowsEight L s (stage mem pa 8) pa pb
    ...
    ⟨htl, (hread 5280 (Or.inr (by decide))).symm,
