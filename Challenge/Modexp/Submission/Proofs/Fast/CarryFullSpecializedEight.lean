...
/-- An eight-limb multiply: `mul entry` (pc 3881) → `common` → `setup` → the eight rows (row head
`hd = 3996`) → the final subtraction (pc 4667). -/
opaque gasSteps_specializedEight (L : RowLemmas) (E : EntryLemmas) (s : State) (mem : ByteArray)
  ...
  refine (E.gasSteps_commonSetup s mem (UInt256.ofNat 3988) pa pb 8 pdst ret rest hcap hrun hcode
