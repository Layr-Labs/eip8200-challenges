...
def phaseGuardProgram : List Instr :=
  [.push 2 1760, .op .MLOAD, .op .ISZERO, .push 2 3369, .op .JUMPI]
...
def phaseSwitchProgram : List Instr :=
  [.push 0 0, .push 2 1760, .op .MSTORE,
   ...
   .op (.Dup ⟨1, by decide⟩), .push 1 2, .op .SHR, .push 2 2760, .op .JUMP]
...
private theorem run_phaseGuard (s : State) (mem : ByteArray) (rest : List UInt256)
    ...
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3369 = true) :
    runInstructions phaseGuardProgram (frame s mem 3339 rest) =
      some (frame s mem (if (MachineState.readWord mem 1760).toNat = 0 then 3369 else 3348) rest) := by
...
theorem run_phaseExitHead (s : State) (mem : ByteArray) (value : UInt256)
    ...
    runInstructions phaseExitHeadProgram (frame s mem 3337 (value :: rest)) =
      some (frame s mem 3339 rest) := by
...
theorem run_phaseSwitch (s : State) (mem : ByteArray) (n : Nat) (rest : List UInt256)
    ...
    (hdest : Decode.isValidJumpDest s.executionEnv.code 2760 = true) :
    runInstructions phaseSwitchProgram
      (frame s mem 3348 ([UInt256.ofNat (32 * n), UInt256.ofNat n] ++ rest)) =
      some (frame s (phaseSwitchMemory mem n) 2760
...
/-- Exact relocated guard bytes at PCs 3389 through 3397. -/
theorem phaseGuardProgram_assembly : assemble phaseGuardProgram =
    ByteArray.mk #[0x61, 0x06, 0xe0, 0x51, 0x15, 0x61, 0x0d, 0x29, 0x57] := by rfl

/-- Exact relocated switch bytes at PCs 3398 through 3418; PUSH0 uses Instr.push0. -/
theorem phaseSwitchProgram_assembly : assemble phaseSwitchProgram =
    ByteArray.mk #[0x5f, 0x61, 0x06, 0xe0, 0x52, 0x80, 0x61, 0x08, 0x40, 0x61, 0x01, 0x00, 0x5e, 0x81, 0x60, 0x02, 0x1c, 0x61, 0x0a, 0xc8, 0x56] := by rfl
...
theorem run_phaseDone (s : State) (mem : ByteArray) (rest : List UInt256)
    ...
    runInstructions phaseDoneProgram (frame s mem 3369 rest) =
      some (frame s mem 3370 rest) := by
...
theorem run_phaseExitZero (s : State) (mem : ByteArray) (value : UInt256)
    ...
    runInstructions (phaseExitHeadProgram ++ phaseGuardProgram)
      (frame s mem 3337 (value :: rest)) = some (frame s mem 3369 rest) := by
...
theorem run_phaseExitOne (s : State) (mem : ByteArray) (value : UInt256)
    ...
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3369 = true) :
    runInstructions (phaseExitHeadProgram ++ phaseGuardProgram)
      (frame s mem 3337 (value :: rest)) = some (frame s mem 3348 rest) := by
  rw [runInstructions_append, run_phaseExitHead s mem value rest hrest]
...
structure PhaseBlocks (artifact : ProgramArtifact) (fork : Fork) where
  exitHead : Block artifact fork 3337 phaseExitHeadProgram
  guard : Block artifact fork 3339 phaseGuardProgram
  switch : Block artifact fork 3348 phaseSwitchProgram
  done : Block artifact fork 3369 phaseDoneProgram
...
def PhaseBlocks.guardZeroSteps {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hflag : MachineState.readWord mem 1760 = UInt256.ofNat 0)
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3369 = true)
    (env : Environment artifact fork (frame s mem 3339 rest)) :
    GasSteps (frame s mem 3339 rest) (frame s mem 3369 rest) :=
...
def PhaseBlocks.guardOneSteps {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3369 = true)
    (env : Environment artifact fork (frame s mem 3339 rest)) :
    GasSteps (frame s mem 3339 rest) (frame s mem 3348 rest) :=
...
def PhaseBlocks.switchSteps {artifact : ProgramArtifact} {fork : Fork}
    ...
    (env : Environment artifact fork
      (frame s mem 3348 ([UInt256.ofNat (32 * n), UInt256.ofNat n] ++ rest))) :
    GasSteps (frame s mem 3348 ([UInt256.ofNat (32 * n), UInt256.ofNat n] ++ rest))
      (frame s (phaseSwitchMemory mem n) 2760
