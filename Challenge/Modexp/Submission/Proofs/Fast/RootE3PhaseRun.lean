...
def phaseGuardProgram : List Instr :=
  [.push 2 1760, .op .MLOAD, .op .ISZERO, .push 2 3534, .op .JUMPI]

def phaseExitHeadProgram : List Instr := [.op .JUMPDEST, .op .POP]

def phaseSwitchProgram : List Instr :=
  [.push 0 0, .push 2 1760, .op .MSTORE,
   .op (.Dup ⟨0, by decide⟩), .push 2 512, .push 2 256, .op .MCOPY,
   .op (.Dup ⟨1, by decide⟩), .push 1 2, .op .SHR, .push 2 3033, .op .JUMP]

/-- The flag is cleared before MCOPY reads the old BASE bytes. -/
...
private theorem run_phaseGuard (s : State) (mem : ByteArray) (rest : List UInt256)
    ...
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3534 = true) :
    runInstructions phaseGuardProgram (frame s mem 3504 rest) =
      some (frame s mem (if (MachineState.readWord mem 1760).toNat = 0 then 3534 else 3513) rest) := by
...
theorem run_phaseGuardOne (s : State) (mem : ByteArray) (rest : List UInt256)
    ...
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3534 = true) :
    runInstructions phaseGuardProgram (frame s mem 3504 rest) =
      some (frame s mem 3513 rest) := by
  simpa [hflag] using run_phaseGuard s mem rest hrest hact hdest

theorem run_phaseExitHead (s : State) (mem : ByteArray) (value : UInt256)
    (rest : List UInt256) (hrest : rest.length ≤ 1000) :
    runInstructions phaseExitHeadProgram (frame s mem 3502 (value :: rest)) =
      some (frame s mem 3504 rest) := by
...
theorem run_phaseSwitch (s : State) (mem : ByteArray) (n : Nat) (rest : List UInt256)
    ...
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3033 = true) :
    runInstructions phaseSwitchProgram
      (frame s mem 3513 ([UInt256.ofNat (32 * n), UInt256.ofNat n] ++ rest)) =
      some (frame s (phaseSwitchMemory mem n) 3033
  ...
  have hactive2 := Exp.activeWords_fix2 s 256 (32 * n) 512 (32 * n)
...
/-- Exact relocated guard bytes at PCs 3507 through 3514. -/
theorem phaseGuardProgram_assembly : assemble phaseGuardProgram =
    ByteArray.mk #[0x61, 0x06, 0xe0, 0x51, 0x15, 0x61, 0x0d, 0xce, 0x57] := by rfl

/-- Exact relocated switch bytes at PCs 3515 through 3535; PUSH0 uses Instr.push0. -/
theorem phaseSwitchProgram_assembly : assemble phaseSwitchProgram =
    ByteArray.mk #[0x5f, 0x61, 0x06, 0xe0, 0x52, 0x80, 0x61, 0x02, 0x00, 0x61, 0x01, 0x00, 0x5e, 0x81, 0x60, 0x02, 0x1c, 0x61, 0x0b, 0xd9, 0x56] := by rfl
...
theorem run_phaseDone (s : State) (mem : ByteArray) (rest : List UInt256)
    ...
    runInstructions phaseDoneProgram (frame s mem 3534 rest) =
      some (frame s mem 3535 rest) := by
...
theorem run_phaseExitZero (s : State) (mem : ByteArray) (value : UInt256)
    ...
    runInstructions (phaseExitHeadProgram ++ phaseGuardProgram)
      (frame s mem 3502 (value :: rest)) = some (frame s mem 3534 rest) := by
...
theorem run_phaseExitOne (s : State) (mem : ByteArray) (value : UInt256)
    ...
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3534 = true) :
    runInstructions (phaseExitHeadProgram ++ phaseGuardProgram)
      (frame s mem 3502 (value :: rest)) = some (frame s mem 3513 rest) := by
  rw [runInstructions_append, run_phaseExitHead s mem value rest hrest]
...
The instruction indices in v4 are2639/2641/2646/2658 respectively. -/
structure PhaseBlocks (artifact : ProgramArtifact) (fork : Fork) where
  exitHead : Block artifact fork 3502 phaseExitHeadProgram
  guard : Block artifact fork 3504 phaseGuardProgram
  switch : Block artifact fork 3513 phaseSwitchProgram
  done : Block artifact fork 3534 phaseDoneProgram
...
def PhaseBlocks.guardZeroSteps {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hflag : MachineState.readWord mem 1760 = UInt256.ofNat 0)
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3534 = true)
    (env : Environment artifact fork (frame s mem 3504 rest)) :
    GasSteps (frame s mem 3504 rest) (frame s mem 3534 rest) :=
...
def PhaseBlocks.guardOneSteps {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hdest : Decode.isValidJumpDest s.executionEnv.code 3534 = true)
    (env : Environment artifact fork (frame s mem 3504 rest)) :
    GasSteps (frame s mem 3504 rest) (frame s mem 3513 rest) :=
...
def PhaseBlocks.switchSteps {artifact : ProgramArtifact} {fork : Fork}
    ...
    (env : Environment artifact fork
      (frame s mem 3513 ([UInt256.ofNat (32 * n), UInt256.ofNat n] ++ rest))) :
    GasSteps (frame s mem 3513 ([UInt256.ofNat (32 * n), UInt256.ofNat n] ++ rest))
      (frame s (phaseSwitchMemory mem n) 3033
