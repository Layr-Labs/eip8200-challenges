...
theorem run_entry (s : State) (mem : ByteArray) (hd : UInt256) (pa pb n : Nat)
  ...
  have hlow := EntryPrefix.run_low { s with memory := mem } hd
    ...
    (UInt256.ofNat 3490 + EntryPrefix.displacement mem)
    (UInt256.ofNat 3778 + EntryPrefix.displacement mem)
  ...
  have hdisp1 : UInt256.ofNat 3490 + EntryPrefix.displacement mem = l1Target n := by
    rcases hn4 with rfl | rfl <;>
      rw [EntryPrefix.displacement, hs32] <;> decide
  have hdisp2 : UInt256.ofNat 3778 + EntryPrefix.displacement mem = l2Target n := by
    rcases hn4 with rfl | rfl <;>
      rw [EntryPrefix.displacement, hs32] <;> decide
  have hmasked :
