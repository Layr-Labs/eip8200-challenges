...
/-- The whole `setup`: from `setupState` (pc 4173, `[hd, pa, pb, dst, ret] ++ rest`) to the
...
theorem run_entry (s : State) (mem : ByteArray) (hd : UInt256) (pa pb n : Nat)
  ...
  have hlow := EntryPrefix.run_low { s with memory := mem } hd
    ...
    (UInt256.ofNat 4283 + EntryPrefix.displacement mem)
    (UInt256.ofNat 4575 + EntryPrefix.displacement mem)
