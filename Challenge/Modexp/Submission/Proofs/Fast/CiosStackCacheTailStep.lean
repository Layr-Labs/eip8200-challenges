...
theorem run_tail (s : State) (c : CachedMemory) (r : ReadOnlyCache) (carry mu bi : UInt256)
    ...
    (hjump : Decode.isValidJumpDest s.executionEnv.code 4620 = true) :
    runInstructions tailProgram (tailAt s c r carry mu bi pa pb n i dst ret rest) =
    some (rowState s (if i+1 < n then 4620 else 5237) (CiosStackCacheModel.cacheTail c carry)
...
theorem run_exit (s : State) (c : CachedMemory) (r : ReadOnlyCache)
    ...
    runInstructions CiosStackCachePrograms.exit (rowState s 5237 c r pa pb n i dst ret rest []) =
