...
namespace Challenge.Modexp.Submission.Proofs.Fast.TnR4Reduction
...
def l2PC : Nat := 3810
def frameTarget : Nat := 5454

/-- The private four-limb middle, its literal jump into the three-cell reduction
suffix, and the long-immediate tail implement the carry-cache model. The
frame's sentinel5454 is preserved separately from the actual L2 entry3810. -/
noncomputable def reduction_steps (s : State)
    ...
    GasSteps
      (framed {s with memory := q.memory} (UInt256.ofNat 5287)
        ([q.carry,bi,pbi,hd,pb,ent,tn,allOnes,UInt256.ofNat frameTarget,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest))
      (framed {s with memory := (TnCacheSquareModel.fromL1 q tn 4).memory}
      (if UInt256.isTrue (UInt256.gt (negative32+pbi) pb) then hd
       else UInt256.ofNat ((l2PC)+TnCacheL2Chain.fullSize 4+51))
      (TnCacheFrameOps.frame (negative32+pbi) hd pb ent
        (TnCacheSquareModel.fromL1 q tn 4).tn (UInt256.ofNat frameTarget) inv
  ...
  have hmid := TnCacheRowTrace.run_middle 5287 st q.carry bi pbi hd pb ent tn
    (UInt256.ofNat frameTarget) tl inv m0 aEnd m96 m64 m32 dst ret 4 rest hcap (by omega) hact hc hi
  have hmidRun : runInstructions (TnCacheRowTrace.middle ++ [.push 2 3810, .op .JUMP])
      (framed st (UInt256.ofNat 5287)
        ([q.carry,bi,pbi,hd,pb,ent,tn,allOnes,UInt256.ofNat frameTarget,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest)) =
      some (TnCacheL2Trace.state st (UInt256.ofNat l2PC) q.memory flag
        ...
        (UInt256.ofNat frameTarget) inv tail) := by
    have hj : Decode.isValidJumpDest s.executionEnv.code 3810 = true := by
      rw [env.code]
      exact TnCandidateArtifact.isValidJumpDest_index 2869 (by rfl)
    have h19 : rest.length+19 < 1024 := by omega
    have h20 : rest.length+20 < 1024 := by omega
    have hjump : runInstructions [.push 2 3810, .op .JUMP]
        (framed st (UInt256.ofNat 5306)
          ([rowC0 q.memory 4,rowMu q.memory 4,flag,pbi,hd,pb,ent,tn+q.carry,allOnes,
            UInt256.ofNat frameTarget,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest)) =
        some (TnCacheL2Trace.state st (UInt256.ofNat l2PC) q.memory flag
          (rowMu q.memory 4) (rowC0 q.memory 4) 4 0 pbi hd pb ent (tn+q.carry)
          (UInt256.ofNat frameTarget) inv tail) := by
  ...
  have hreduce := TnCacheL2Chain.run_full st (l2PC) q.memory flag (rowMu q.memory 4)
    (rowC0 q.memory 4) 4 (Or.inl rfl) pbi hd pb ent (tn+q.carry) (UInt256.ofNat frameTarget)
    tl inv m0 aEnd m96 m64 m32 dst ret rest hcap hact he
  have htailrun := TnCacheRowTrace.run_tail ((l2PC)+TnCacheL2Chain.fullSize 4)
    {s with memory := mem2} carry2 flag pbi hd pb ent (tn+q.carry)
    (UInt256.ofNat frameTarget) inv tail htail hact hjd
  ...
  have hlast : runInstructions TnCacheRowTrace.tail
      (TnCacheLastTrace.lastState st (UInt256.ofNat ((l2PC)+TnCacheL2Chain.fullSize 4))
        q.memory flag (rowMu q.memory 4) (rowC0 q.memory 4) 4 (4-1)
        pbi hd pb ent (tn+q.carry) (UInt256.ofNat frameTarget) inv tail) =
      some (framed {s with memory := (TnCacheSquareModel.fromL1 q tn 4).memory}
        (if UInt256.isTrue (UInt256.gt (negative32+pbi) pb) then hd
         else UInt256.ofNat ((l2PC)+TnCacheL2Chain.fullSize 4+51))
        (TnCacheFrameOps.frame (negative32+pbi) hd pb ent
          (TnCacheSquareModel.fromL1 q tn 4).tn (UInt256.ofNat frameTarget) inv tail)) := by
    simpa only [TnCacheLastTrace.lastState, st, framed, mem2, carry2,
      TnCacheSquareModel.fromL1, flag] using htailrun
  have g0 := TnCandidateBlocks.middleCopy.steps
    (s := framed st (UInt256.ofNat 5287)
      ([q.carry,bi,pbi,hd,pb,ent,tn,allOnes,UInt256.ofNat frameTarget,inv,
  ...
  have g1 : GasSteps
      (TnCacheL2Trace.state st (UInt256.ofNat l2PC) q.memory flag
        (rowMu q.memory 4) (rowC0 q.memory 4) 4 0 pbi hd pb ent (tn+q.carry)
        (UInt256.ofNat frameTarget) inv tail)
      (TnCacheLastTrace.lastState st (UInt256.ofNat (l2PC+TnCacheL2Chain.fullSize 4))
        q.memory flag (rowMu q.memory 4) (rowC0 q.memory 4) 4 (4-1)
        pbi hd pb ent (tn+q.carry) (UInt256.ofNat frameTarget) inv tail) := by
    exact TnCandidateBlocks.l2Four.steps (env.transfer rfl rfl) rfl hreduce
  have hpc : (TnCacheLastTrace.lastState st
      (UInt256.ofNat (l2PC+TnCacheL2Chain.fullSize 4)) q.memory flag
      (rowMu q.memory 4) (rowC0 q.memory 4) 4 (4-1)
      pbi hd pb ent (tn+q.carry) (UInt256.ofNat frameTarget) inv tail).pc =
      UInt256.ofNat 3910 := by
    change UInt256.ofNat (l2PC+TnCacheL2Chain.fullSize 4) = UInt256.ofNat 3910
    rfl
  have g2 := TnCandidateBlocks.tail.steps
...
end Challenge.Modexp.Submission.Proofs.Fast.TnR4Reduction
