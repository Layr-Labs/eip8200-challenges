...
def gasSteps_lazy {artifact : Challenge.EvmProof.ProgramArtifact}
    (gateBlock : Block artifact .Osaka 4737 program)
    (copyBlock : Block artifact .Osaka 4746 copyProgram)
    ...
    (hsubJump : Decode.isValidJumpDest s.executionEnv.code 4501 = true)
    (subTrace : Challenge.EvmProof.GasSteps (atState s mem 4501 dst ret rest)
      (returnedState s (Csub.subResultMemory mem n dst.toNat) ret rest)) :
    Challenge.EvmProof.GasSteps (atState s mem 4737 dst ret rest)
      (returnedState s (LazyCsub.resultMemory mem n dst.toNat) ret rest) := by
  have hc := gateBlock.steps (env.transfer (t := atState s mem 4737 dst ret rest) rfl rfl) rfl
    (run_gate s mem dst ret rest hcap hact hsubJump)
  by_cases hz : (MachineState.readWord mem 2080).toNat = 0
  · rw [if_pos hz] at hc
    have hk := copyBlock.steps (env.transfer (t := atState s mem 4746 dst ret rest) rfl rfl) rfl
