...
theorem run_l1SeventhMac (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock seventhMac
      (l1At 4593 s mem bi pa pb n i j pdst ret rest) =
      some (l1At 4662 s mem bi pa pb n i (j + 1) pdst ret rest) := by
  ...
  simp (config := { maxSteps := 800000 })
    [seventhMac, Cios2Paths.L1.seventhPC, Cios2Paths.L1.seventhStartIndex,
      Cios2Paths.L1.seventhOpAt, Cios2Paths.L1.seventhPushAt, wfOp,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      l1At, l1Step, macSum, macCarry, mulHi, maxWord_literal,
      hc9, hc10, hc11, hc12, hc13, hc14, hrun, hcode, hK,
      hpaj, hptj, hactA, hactT, ptrAt_succ,
...
def gasSteps_l1SeventhMac (s : State) (mem : ByteArray) (bi : UInt256)
    (pa pb n i j : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hfork : s.fork = .Osaka)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 4593 s mem bi pa pb n i j pdst ret rest)
      (l1At 4662 s mem bi pa pb n i (j + 1) pdst ret rest) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka seventhMac hcode hfork
    (run_l1SeventhMac s mem bi pa pb n i j pdst ret rest hcap hrun hcode hact hn32 hj
      hpa hpaFit) hrun hnp
