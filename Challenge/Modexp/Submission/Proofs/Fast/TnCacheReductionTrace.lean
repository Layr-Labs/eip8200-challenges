import Challenge.Modexp.Submission.Proofs.Fast.TnCacheL2Chain
import Challenge.Modexp.Submission.Proofs.Fast.TnCacheRowTrace
import Challenge.Modexp.Submission.Proofs.Fast.TnCacheSquareModel

set_option warningAsError true
set_option linter.unusedSimpArgs false
set_option maxRecDepth 40000
set_option maxHeartbeats 2000000

namespace Challenge.Modexp.Submission.Proofs.Fast.TnCacheReductionTrace

open EvmSemantics EvmSemantics.EVM YulEvmCompiler
open Challenge.Modexp.Submission.Proofs.Bytecode WindowNibbleKernel
open Challenge.Modexp.Submission.Proofs.Fast Monpro CiosCached CiosCachedMacCore
open CiosReadonly CiosCachedMidMemory

def dispatch : List Instr := [.op (.Dup ⟨9, by decide⟩), .op .JUMP]

def program (n : Nat) : List Instr :=
  ((TnCacheRowTrace.middle ++ dispatch) ++ TnCacheL2Chain.fullProgram n) ++
    TnCacheRowTrace.tailShort

/-- The complete reduction half of a row computes the cache model, including
the real next-row branch. The bytes are bound separately from this theorem. -/
theorem run_reduction (s : State) (pc l2pc : Nat) (q : MacState) (bi tn : UInt256)
    (n : Nat) (hn : n = 4 ∨ n = 8)
    (pbi hd pb ent tl inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 998)
    (hact : 88 ≤ s.activeWords.toNat)
    (hc : ReadonlyCache q.memory n tl inv m0)
    (he : TnCacheExtraTrace.ExtraCache q.memory m96 m64 m32)
    (hi : inverseInvariant q.memory n)
    (hj : Decode.isValidJumpDest s.executionEnv.code l2pc = true)
    (hjd : Decode.isValidJumpDest s.executionEnv.code hd.toNat = true)
    (hl2 : l2pc < 2^256) :
    runInstructions (program n)
      (framed {s with memory := q.memory} (UInt256.ofNat pc)
        ([q.carry,bi,pbi,hd,pb,ent,tn,allOnes,UInt256.ofNat l2pc,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest)) =
    some (framed {s with memory := (TnCacheSquareModel.fromL1 q tn n).memory}
      (if UInt256.isTrue (UInt256.gt (negative32+pbi) pb) then hd
       else UInt256.ofNat (l2pc+TnCacheL2Chain.fullSize n+21))
      (TnCacheFrameOps.frame (negative32+pbi) hd pb ent
        (TnCacheSquareModel.fromL1 q tn n).tn (UInt256.ofNat l2pc) inv
        (m0 :: tl :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest))) := by
  let st : State := {s with memory := q.memory}
  let flag := UInt256.lt (tn+q.carry) q.carry
  let mem2 := (l2Step q.memory (rowMu q.memory n) (rowC0 q.memory n) n (n-1)).memory
  let carry2 := (l2Step q.memory (rowMu q.memory n) (rowC0 q.memory n) n (n-1)).carry
  let tail := m0 :: tl :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest
  have htail : tail.length ≤ 1006 := by simp only [tail, List.length_cons]; omega
  have hmid := TnCacheRowTrace.run_middle pc st q.carry bi pbi hd pb ent tn
    (UInt256.ofNat l2pc) tl inv m0 aEnd m96 m64 m32 dst ret n rest hcap (by omega) hact hc hi
  have htarget : (UInt256.ofNat l2pc).toNat = l2pc := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt hl2]
  have hjump : runInstructions dispatch
      (framed st (UInt256.ofNat (pc+21))
        ([rowC0 q.memory n,rowMu q.memory n,flag,pbi,hd,pb,ent,tn+q.carry,allOnes,
          UInt256.ofNat l2pc,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest)) =
      some (TnCacheL2Trace.state st (UInt256.ofNat l2pc) q.memory flag
        (rowMu q.memory n) (rowC0 q.memory n) n 0 pbi hd pb ent (tn+q.carry)
        (UInt256.ofNat l2pc) inv tail) := by
    have h19 : rest.length+19 < 1024 := by omega
    have h20 : rest.length+20 < 1024 := by omega
    simp [dispatch, runInstructions, Challenge.EvmProof.Stepper.runInstr,
      framed, TnCacheL2Trace.state, l2Step, tail, st, h19, h20, htarget, hj]
  have hreduce := TnCacheL2Chain.run_full st l2pc q.memory flag (rowMu q.memory n)
    (rowC0 q.memory n) n hn pbi hd pb ent (tn+q.carry) (UInt256.ofNat l2pc)
    tl inv m0 aEnd m96 m64 m32 dst ret rest hcap hact he
  have htailrun := TnCacheRowTrace.run_tailShort (l2pc+TnCacheL2Chain.fullSize n)
    {s with memory := mem2} carry2 flag pbi hd pb ent (tn+q.carry)
    (UInt256.ofNat l2pc) inv tail htail hact hjd
  have hflag : UInt256.lt (tn+q.carry+carry2) carry2+flag =
      flag+UInt256.lt (tn+q.carry+carry2) carry2 := by
    apply Challenge.EvmProof.Word.word_ext
    simp only [Challenge.EvmProof.Word.word_toNat_add, Nat.add_comm]
  have h01 := runInstructions_append_some _ _ _ _ _ hmid hjump
  have h012 := runInstructions_append_some _ _ _ _ _ h01 hreduce
  rw [hflag] at htailrun
  have hlast : runInstructions TnCacheRowTrace.tailShort
      (TnCacheLastTrace.lastState st (UInt256.ofNat (l2pc+TnCacheL2Chain.fullSize n))
        q.memory flag (rowMu q.memory n) (rowC0 q.memory n) n (n-1)
        pbi hd pb ent (tn+q.carry) (UInt256.ofNat l2pc) inv tail) =
      some (framed {s with memory := (TnCacheSquareModel.fromL1 q tn n).memory}
        (if UInt256.isTrue (UInt256.gt (negative32+pbi) pb) then hd
         else UInt256.ofNat (l2pc+TnCacheL2Chain.fullSize n+21))
        (TnCacheFrameOps.frame (negative32+pbi) hd pb ent
          (TnCacheSquareModel.fromL1 q tn n).tn (UInt256.ofNat l2pc) inv tail)) := by
    simpa only [TnCacheLastTrace.lastState, st, framed, mem2, carry2,
      TnCacheSquareModel.fromL1, flag] using htailrun
  have hall := runInstructions_append_some _ _ _ _ _ h012 hlast
  simpa only [program, st, tail] using hall

#print axioms run_reduction
end Challenge.Modexp.Submission.Proofs.Fast.TnCacheReductionTrace
