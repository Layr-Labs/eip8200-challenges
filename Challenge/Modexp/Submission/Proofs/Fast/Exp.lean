...
/-- pc 3578, `LZBASE` past its zero test with the copy still to run. -/
def lzBaseCopy (s : State) (mem : ByteArray) (n bsize esize msize i w mask : Nat) :
    State :=
  { s with pc := UInt256.ofNat 3578
           stack := bitStack n bsize esize msize i w mask
           memory := mem }

/-- pc 3593, `LZBASE`'s zero-byte arm. -/
def lzBaseSkip (s : State) (mem : ByteArray) (n bsize esize msize i w mask : Nat) :
    State :=
  { s with pc := UInt256.ofNat 3593
           stack := bitStack n bsize esize msize i w mask
           memory := mem }
...
set_option linter.unusedSimpArgs false in
/-- `LZBASE`'s test, pc 3571, with a zero leading byte: take the untouched arm. -/
theorem run_lzBase_zero (s : State) (mem : ByteArray)
    (n bsize esize msize i w mask : Nat) (hw : w = 0)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock blk2338
  ...
  simp (config := { maxSteps := 400000 }) [blk2338, opAt, pushAt,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    Lz.lzBase, lzBaseSkip, bitStack, hrun, hcode, hz, jumpDest3593,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat]
...
set_option linter.unusedSimpArgs false in
/-- `blk2343` (pc 3578..3592): `ACC := BASE`, then resume at the mask shift. -/
theorem run_lzBaseCopy (s : State) (mem : ByteArray)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock blk2343
  ...
  simp (config := { maxSteps := 600000 }) [blk2343, opAt, pushAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
...
theorem run_lzBaseSkip (s : State) (mem : ByteArray)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock blk2350
      (lzBaseSkip s mem n bsize esize msize i w mask) =
      some (ebitHead s mem n bsize esize msize i w mask) := by
  simp (config := { maxSteps := 400000 }) [blk2350, opAt, pushAt,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
...
def gasSteps_ebLoad (s : State) (mem input : ByteArray)
  ...
  if h : i = 0 then
    ...
    have g3 : Challenge.EvmProof.GasSteps
      ...
      Challenge.EvmProof.Stepper.runLocatedBlock_sound
        Artifact.submissionArtifact .Osaka blk2338 hcode hfork
    ...
    have g4 : Challenge.EvmProof.GasSteps
        ...
        (ebitHead s mem n bsize esize msize i (expByte input bsize i)
          (Lz.topBit (expByte input bsize i))) :=
      Challenge.EvmProof.Stepper.runLocatedBlock_sound
        Artifact.submissionArtifact .Osaka blk2350 hcode hfork
...
/- The nonzero first byte copies `BASE` to `ACC` before resuming the bit loop. -/
def gasSteps_ebLoadCopy (s : State) (mem input : ByteArray)
  ...
  have g4 : Challenge.EvmProof.GasSteps
      ...
      (ebitNext s (mcopyMem mem 1024 2048 (32 * n)) n bsize esize msize i
        (expByte input bsize i) (Lz.topBit (expByte input bsize i))) :=
    Challenge.EvmProof.Stepper.runLocatedBlock_sound
      Artifact.submissionArtifact .Osaka blk2343 hcode hfork
...
/- The first nonzero exponent byte copies BASE to ACC before the bit loop;
   later bytes use the ordinary in-place bit chain. -/
...
/- Leading zeros then the leading one: the accumulator lands on `Mont BASE`. -/
...
/- The first nonzero byte consumes its leading one after the copy. -/
theorem expAcc_skipOne {mm R bM : Nat} (hm : 0 < mm) (hcop : Nat.Coprime R mm)
  ...
  have htop7 : Lz.topExp (expByte input bsize 0) ≤ 7 :=
...
/- `LZBASE` copies `BASE` onto `ACC`; other named blocks are disjoint. -/
...
/- From an arbitrary bit of one exponent byte to the next byte head. -/
