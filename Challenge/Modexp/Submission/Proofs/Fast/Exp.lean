...
/-- The accelerated `SQUARE` call: the remaining count is the destination word
at the kernel entry, and the caller's memory is unchanged. -/
def sqLoopCall (s : State) (mem : ByteArray) (count : Nat) (ret : UInt256)
    (tail : List UInt256) : State :=
  { s with pc := UInt256.ofNat 3394
           stack := UInt256.ofNat 4561 :: UInt256.ofNat 512 :: UInt256.ofNat 512 ::
             UInt256.ofNat count :: ret :: tail
           memory := mem }
...
structure Subroutines (s : State) (n bsize mm minv : Nat) where
  ...
  squareLoop : ∀ (k : Nat) (ret : UInt256) (tail : List UInt256)
    ...
    Frame mem n bsize minv → Model.FastRepresents mem 0 n mm →
    Model.FastRepresents mem 512 n a → a < mm →
    Challenge.EvmProof.GasSteps (sqLoopCall s mem k ret tail)
  ...
  /-- The concrete dispatcher admission carried by this internal caller. -/
  hfast : n = 4 ∨ n = 8
  hminv1 : minv ≠ 1
