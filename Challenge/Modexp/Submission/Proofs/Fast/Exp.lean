...
theorem shl8_bit_ofNat (v : Nat) (hv : v < 2) :
    UInt256.shiftLeft (UInt256.ofNat v) (UInt256.ofNat 8) =
      UInt256.ofNat (256 * v) := by
  have h := Challenge.EvmProof.Word.shiftLeft_ofNat (value := v) (shift := 8)
    (by omega) (by norm_num) (by omega)
  rw [h, show v * 2 ^ 8 = 256 * v from by ring]
...
theorem run_rrMid (s : State) (mem : ByteArray) (n bsize esize msize k : Nat)
  ...
  have hshl : UInt256.shiftLeft (UInt256.ofNat (n / 2 ^ k % 2)) (UInt256.ofNat 8) =
      UInt256.ofNat (256 * (n / 2 ^ k % 2)) :=
    shl8_bit_ofNat _ (Nat.mod_lt _ (by norm_num))
