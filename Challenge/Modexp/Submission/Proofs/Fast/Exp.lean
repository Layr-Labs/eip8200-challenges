...
import Challenge.Modexp.Submission.Proofs.Fast.Paths.LeadingBit
...
/-- pc 3702, `LZBASE` past its zero test with the copy still to run. -/
def lzBaseCopy (s : State) (mem : ByteArray) (n bsize esize msize i w mask : Nat) :
    State :=
  { s with pc := UInt256.ofNat 3702
           stack := bitStack n bsize esize msize i w mask
           memory := mem }

/-- pc 3717, `LZBASE`'s zero-byte arm. -/
...
set_option linter.unusedSimpArgs false in
/-- `LZBASE`'s test, pc 3695, with a zero leading byte: take the untouched arm. -/
theorem run_lzBase_zero (s : State) (mem : ByteArray)
    (n bsize esize msize i w mask : Nat) (hw : w = 0)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock blkLeadingBitTest
  ...
  simp (config := { maxSteps := 400000 }) [blkLeadingBitTest, opAt, pushAt,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    Lz.lzBase, lzBaseSkip, bitStack, hrun, hcode, hz, jumpDest3717,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat]
...
set_option linter.unusedSimpArgs false in
/-- `blkLeadingBitCopy` (pc 3702..3716): `ACC := BASE`, then resume at the mask shift. -/
theorem run_lzBaseCopy (s : State) (mem : ByteArray)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock blkLeadingBitCopy
  ...
  simp (config := { maxSteps := 600000 }) [blkLeadingBitCopy, opAt, pushAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
...
theorem run_lzBaseSkip (s : State) (mem : ByteArray)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock blkLeadingBitZero
      (lzBaseSkip s mem n bsize esize msize i w mask) =
      some (ebitHead s mem n bsize esize msize i w mask) := by
  simp (config := { maxSteps := 400000 }) [blkLeadingBitZero, opAt, pushAt,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
...
def gasSteps_ebLoad (s : State) (mem input : ByteArray)
  ...
  if h : i = 0 then
    ...
    have g3 : Challenge.EvmProof.GasSteps
      ...
      Challenge.EvmProof.Stepper.runLocatedBlock_sound
        Artifact.submissionArtifact .Osaka blkLeadingBitTest hcode hfork
    ...
    have g4 : Challenge.EvmProof.GasSteps
        ...
        (ebitHead s mem n bsize esize msize i (expByte input bsize i)
          (Lz.topBit (expByte input bsize i))) :=
      Challenge.EvmProof.Stepper.runLocatedBlock_sound
        Artifact.submissionArtifact .Osaka blkLeadingBitZero hcode hfork
...
def gasSteps_ebLoadCopy (s : State) (mem input : ByteArray)
  ...
  have g4 : Challenge.EvmProof.GasSteps
      ...
      (ebitNext s (mcopyMem mem 1024 2048 (32 * n)) n bsize esize msize i
        (expByte input bsize i) (Lz.topBit (expByte input bsize i))) :=
    Challenge.EvmProof.Stepper.runLocatedBlock_sound
      Artifact.submissionArtifact .Osaka blkLeadingBitCopy hcode hfork
