...
1. **the `RR` chain** (idx 1203..1242, pc 1569..1638) — six iterations of
   square-and-multiply computing `RR = φ(radix ^ n) = R² mod m`;
2. **the base chain** (idx 1243..1264, pc 1687..1745) — a Horner loop over the
   base limbs producing `ACC = b mod m`, then `BASE = MonPro(ACC, RR)`;
3. **the exponent loop** (idx 1313..1332, pc 1804..1923) — `8 * esize`
   flagless square-and-multiply steps producing `ACC = φ(b ^ e)`, the final
   `MonPro(ACC, 1)` and the `RETURN`.

`MONPRO` (pc 4590) and `ADDMOD` (pc 2269) are developed in `Fast.Monpro` and
...
def sqCall (s : State) (mem : ByteArray) (ret : UInt256)
    (tail : List UInt256) : State :=
  { s with pc := UInt256.ofNat 4059
           stack := UInt256.ofNat 4902 :: UInt256.ofNat 2048 :: UInt256.ofNat 2048 ::
...
`Fast.Csub` splits the routine at pc 2392 into `gasSteps_addmod` (the schoolbook
...
structure Subroutines (s : State) (n bsize mm minv : Nat) where
  ...
  `0x2440 = 9280` (which the caller stores before the call), re-stages the
  operand and re-enters its rows, and on the last square rewrites the frame's
  return slot, so control comes back to the caller's `after_sq` (pc 3278) with
  ...
  squareLoop : ∀ (k : Nat) (ret : UInt256) (tail : List UInt256)
    ...
    Model.FastRepresents mem 2048 n a → a < mm →
    Challenge.EvmProof.GasSteps (sqCall s mem ret tail)
      (retTo s (sqLoopMem k mem) (UInt256.ofNat 3266) tail)
...
/-- pc 1634, back from the squaring `MonPro(RR, RR) → RR`. -/
def rrMid (s : State) (mem : ByteArray) (n bsize esize msize k : Nat) : State :=
  { s with pc := UInt256.ofNat 1385
           stack := UInt256.ofNat k :: outer n bsize esize msize
           memory := mem }

/-- pc 2650, with the selector computed but the multiply not yet taken. -/
...
/-- pc 1640, back from the selected multiply, with the selector still live. -/
...
/-- `BDONE`, pc 1804, where the base chain rejoins. -/
...
theorem run_rrHead (s : State) (mem : ByteArray) (n bsize esize msize k : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1155
      ...
      some (mpCall s mem 6144 6144 6144 (UInt256.ofNat 1385)
        (UInt256.ofNat k :: outer n bsize esize msize)) := by
  have h1939Nat : (UInt256.ofNat 4055).toNat = 4055 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1162` (pc 1634..1662): pick `R1` or `CC` by bit `k` of `n` and call
`MonPro(RR, sel) → RR`. -/
...
/-- pc 3022, the multiply's call frame about to be pushed. -/
def rrCallSel (s : State) (mem : ByteArray) (n bsize esize msize k : Nat) : State :=
  { s with pc := UInt256.ofNat 2209
           stack := UInt256.ofNat (selOf n k) :: UInt256.ofNat k ::
             outer n bsize esize msize
           memory := mem }
...
theorem run_rrCallSel (s : State) (mem : ByteArray) (n bsize esize msize k : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1822
      ...
      some (mpCall s mem 6144 (selOf n k) 6144 (UInt256.ofNat 1404)
...
set_option linter.unusedSimpArgs false in
/-- `blk1178` (pc 1640..1647) with the counter at zero: leave the chain. -/
theorem run_rrPost_exit (s : State) (mem : ByteArray) (n bsize esize msize : Nat)
  ...
  have h1631Nat : (UInt256.ofNat 1419).toNat = 1419 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1184` (pc 1671..1655): decrement the counter and loop. -/
theorem run_rrNext (s : State) (mem : ByteArray)
  ...
  have h1569Nat : (UInt256.ofNat 1370).toNat = 1370 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1189` (pc 1679..1638) with a nonempty base: enter the base chain. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1189` (pc 1679..1638) with an empty base: skip the base chain. -/
...
def gasSteps_rrHead (s : State) (mem : ByteArray) (n bsize esize msize k : Nat)
    ...
    Challenge.EvmProof.GasSteps (rrHead s mem n bsize esize msize k)
      (mpCall s mem 6144 6144 6144 (UInt256.ofNat 1385)
...
def gasSteps_rrCallSel (s : State) (mem : ByteArray) (n bsize esize msize k : Nat)
    ...
    Challenge.EvmProof.GasSteps (rrCallSel s mem n bsize esize msize k)
      (mpCall s mem 6144 (selOf n k) 6144 (UInt256.ofNat 1404)
        (UInt256.ofNat (selOf n k) :: UInt256.ofNat k ::
          outer n bsize esize msize)) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
...
`BL` (pc 1667) walks `j` from `1` to `pb = ⌈bsize / 32⌉`, alternating
...
set_option linter.unusedSimpArgs false in
/-- `blk1216` (pc 1667..1724) with `j = pb`: leave the loop. -/
theorem run_blHead_exit (s : State) (mem : ByteArray)
  ...
  have h1736Nat : (UInt256.ofNat 1498).toNat = 1498 := by decide
  simp (config := { maxSteps := 400000 }) [blk1216, opAt, pushAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
...
set_option linter.unusedSimpArgs false in
/-- `blk1223` (pc 1725..1740): call `MonPro(ACC, CC) → ACC`. -/
theorem run_blMul (s : State) (mem : ByteArray) (n bsize esize msize pb j : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1223
      ...
      some (mpCall s mem 1024 5120 1024 (UInt256.ofNat 1455)
...
theorem run_blNext (s : State) (mem : ByteArray) (n bsize esize msize pb j : Nat)
  ...
  have h1668Nat : (UInt256.ofNat 1431).toNat = 1431 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1255` (pc 1784..1754): drop the loop variables and call
`MonPro(ACC, RR) → BASE`. -/
theorem run_blExit (s : State) (mem : ByteArray) (n bsize esize msize pb j : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1255
      ...
      some (mpCall s mem 1024 6144 2048 (UInt256.ofNat 1517)
        (outer n bsize esize msize)) := by
  have h1939Nat : (UInt256.ofNat 4055).toNat = 4055 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1264` (pc 1745): the one-instruction rejoin. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1195` (pc 1687..1666): compute the limb count, store the top partial
...
theorem run_baseHead (s : State) (mem input : ByteArray) (n bsize esize msize : Nat)
  ...
  have hshr : UInt256.shiftRight (UInt256.ofNat (31 + bsize)) (UInt256.ofNat 5) =
    ...
    shr_ofNat _ 5 (Nat.lt_of_le_of_lt (show 31 + bsize ≤ 1108 by omega) (by norm_num))
      (by omega)
...
set_option linter.unusedSimpArgs false in
/-- `blk1229` (pc 1741..1726): store base limb `pb - 1 - j` into `ONE` and call
`AddMod(ACC, ONE) → ACC`. -/
theorem run_blAdd (s : State) (mem input : ByteArray)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1229
      ...
      some (amCall s (storeWord mem (3040 + 32 * n) (baseLimbWord input bsize pb j))
        1024 3072 1024 (UInt256.ofNat 1490)
        (UInt256.ofNat j :: UInt256.ofNat pb :: outer n bsize esize msize)) := by
  ...
  have h2467Nat : (UInt256.ofNat 1946).toNat = 1946 := by decide
...
def gasSteps_blMul (s : State) (mem : ByteArray) (n bsize esize msize pb j : Nat)
    ...
    Challenge.EvmProof.GasSteps (blMul s mem n bsize esize msize pb j)
      (mpCall s mem 1024 5120 1024 (UInt256.ofNat 1455)
...
def gasSteps_blExit (s : State) (mem : ByteArray) (n bsize esize msize pb j : Nat)
    ...
    Challenge.EvmProof.GasSteps (blExit s mem n bsize esize msize pb j)
      (mpCall s mem 1024 6144 2048 (UInt256.ofNat 1517)
        (outer n bsize esize msize)) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
...
`EB` (pc 1759) walks the `esize` exponent bytes; for each byte `EBIT`
(pc 1837) walks its eight bits from the most significant, squaring `ACC` on
...
def ebitTest (s : State) (mem : ByteArray) (n bsize esize msize i w mask : Nat) :
    State :=
  { s with pc := UInt256.ofNat 1549
           stack := bitStack n bsize esize msize i w mask
           memory := mem }

/-- pc 1863, the multiply branch. -/
def ebitMul (s : State) (mem : ByteArray) (n bsize esize msize i w mask : Nat) :
    State :=
  { s with pc := UInt256.ofNat 1558
           stack := bitStack n bsize esize msize i w mask
           memory := mem }

/-- pc 1879, back from the multiply. -/
def ebitJoin (s : State) (mem : ByteArray) (n bsize esize msize i w mask : Nat) :
    State :=
  { s with pc := UInt256.ofNat 1574
           stack := bitStack n bsize esize msize i w mask
           memory := mem }
...
set_option linter.unusedSimpArgs false in
/-- `blk1272` (pc 1759..1825) with `i < esize`: process byte `i`. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1272` (pc 1759..1825) with `i = esize`: leave the byte loop. -/
theorem run_ebHead_exit (s : State) (mem : ByteArray) (n bsize esize msize i : Nat)
  ...
  have h1850Nat : (UInt256.ofNat 1593).toNat = 1593 := by decide
  simp (config := { maxSteps := 400000 }) [blk1272, opAt, pushAt, wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
...
/-- `blk1279` (pc 1826..1829): the tail call into `LZ`, which loads exponent
...
set_option linter.unusedSimpArgs false in
/-- `LZBASE`'s test, pc 4260, with a zero leading byte: take the untouched arm. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk2562` (pc 3872..4194): `ACC := BASE`, then resume at the mask shift. -/
theorem run_lzBaseCopy (s : State) (mem : ByteArray)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk2562
      ...
      some (ebitNext s (mcopyMem mem 1024 2048 (32 * n))
  ...
  have hfix2 : UInt256.ofNat (MachineState.activeWordsAfter
    ...
    activeWords_fix2 s 1024 (32 * n) 2048 (32 * n) (by omega) (by omega) (by omega)
      (by omega) hact
  have h1832Nat : (UInt256.ofNat 1575).toNat = 1575 := by decide
...
set_option linter.unusedSimpArgs false in
/-- The zero-byte arm rejoins the loop head at pc 1837. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1287` (pc 1837..1853): square `ACC`. -/
theorem run_ebitHead (s : State) (mem : ByteArray)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1287
      ...
      some (mpCall s mem 1024 1024 1024 (UInt256.ofNat 1549)
...
set_option linter.unusedSimpArgs false in
/-- `blk1294` (pc 1741..1862) with the bit clear: skip the multiply. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1294` (pc 1741..1862) with the bit set: fall into the multiply. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1301` (pc 1863..1878): multiply `ACC` by `BASE`. -/
theorem run_ebitMul (s : State) (mem : ByteArray)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1301
      ...
      some (mpCall s mem 1024 2048 1024 (UInt256.ofNat 1574)
        (bitStack n bsize esize msize i w mask)) := by
  have h1939Nat : (UInt256.ofNat 4055).toNat = 4055 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1307` (pc 1879): the one-instruction rejoin. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1308` (pc 1815..1888) with more bits to go: shift the mask and loop. -/
theorem run_ebitNext_loop (s : State) (mem : ByteArray)
  ...
  have htrue : UInt256.isTrue (UInt256.ofNat (mask / 2)) :=
    isTrue_ofNat (Nat.lt_of_le_of_lt (show mask / 2 ≤ 128 by omega) (by norm_num))
      (by omega)
  have h1789Nat : (UInt256.ofNat 1534).toNat = 1534 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1308` (pc 1815..1888) after the last bit: leave the bit loop. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1314` (pc 1824..1897): advance to the next exponent byte. -/
theorem run_ebTail (s : State) (mem : ByteArray) (n bsize esize msize i w : Nat)
  ...
  have h1769Nat : (UInt256.ofNat 1522).toNat = 1522 := by decide
...
set_option linter.unusedSimpArgs false in
/-- `blk1320` (pc 1898..1923): set `ONE` to one and call `MonPro(ACC, ONE)`. -/
theorem run_ebEnd (s : State) (mem : ByteArray) (n bsize esize msize i : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1320
      ...
      some (mpCall s (storeWord mem (3040 + 32 * n) (UInt256.ofNat 1))
        1024 3072 1024 (UInt256.ofNat 1619) (outer n bsize esize msize)) := by
  have hmod : (3040 + 32 * n) %
      115792089237316195423570985008687907853269984665640564039457584007913129639936
      = 3040 + 32 * n :=
...
set_option linter.unusedSimpArgs false in
/-- `blk1333` (pc 1859..1933): `RETURN(ACC + s32 - msize, msize)`. -/
...
def gasSteps_ebLoadCopy (s : State) (mem input : ByteArray)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps (ebLoad s mem n bsize esize msize i)
      (ebitNext s (mcopyMem mem 1024 2048 (32 * n)) n bsize esize msize i
...
def gasSteps_ebitHead (s : State) (mem : ByteArray)
    ...
    Challenge.EvmProof.GasSteps (ebitHead s mem n bsize esize msize i w mask)
      (mpCall s mem 1024 1024 1024 (UInt256.ofNat 1549)
...
def gasSteps_ebitMul (s : State) (mem : ByteArray)
    ...
    Challenge.EvmProof.GasSteps (ebitMul s mem n bsize esize msize i w mask)
      (mpCall s mem 1024 2048 1024 (UInt256.ofNat 1574)
        (bitStack n bsize esize msize i w mask)) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
...
def gasSteps_ebEnd (s : State) (mem : ByteArray) (n bsize esize msize i : Nat)
    ...
    Challenge.EvmProof.GasSteps (ebEnd s mem n bsize esize msize i)
      (mpCall s (storeWord mem (3040 + 32 * n) (UInt256.ofNat 1))
        1024 3072 1024 (UInt256.ofNat 1619) (outer n bsize esize msize)) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
...
def byteMemAt (mpMem : Nat → Nat → Nat → ByteArray → ByteArray) (input : ByteArray)
  ...
  if i = 0 ∧ expByte input bsize i ≠ 0 then
    bitMemsFrom mpMem (expByte input bsize i) (mcopyMem mem 1024 2048 (32 * n))
...
theorem readWord_ebMems (mpMem : Nat → Nat → Nat → ByteArray → ByteArray)
  ...
  | succ i ih =>
      ...
      · rw [readWord_bitMemsFrom mpMem addr hkeep,
          readWord_mcopyMem_disjoint _ 1024 2048 (32 * n) addr (Or.inr (by omega)), ih]
...
`Fast.Setup` stops at pc 1959 — the `DOUBLE256` entry — with stack
`[R1, 1533] ++ OUTER`.  Two `DOUBLE256` calls with the two `MCOPY` blocks `r0`
(pc 1533) and `r1` (pc 1603) between and after them lead into the `RR` chain at
...
/-- The `DOUBLE256` entry, pc 1959, stack `[px, ret] ++ OUTER`. -/
...
/-- The `R1B` entry, pc 2580, stack `[px, ret] ++ OUTER`.  Same calling
...
set_option linter.unusedSimpArgs false in
/-- `blk1138` (pc 1533..1602): `MCOPY(CC, R1, s32)` then `CCB(CC)`. -/
theorem run_r0 (s : State) (mem : ByteArray) (n bsize esize msize : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1138
      ...
      some (ccCall s (mcopyMem mem 5120 4096 (32 * n)) 5120 (UInt256.ofNat 2878)
...
def gasSteps_r0 (s : State) (mem : ByteArray) (n bsize esize msize : Nat)
    ...
    Challenge.EvmProof.GasSteps (r0State s mem n bsize esize msize)
      (ccCall s (mcopyMem mem 5120 4096 (32 * n)) 5120 (UInt256.ofNat 2878)
        n bsize esize msize) :=
  Challenge.EvmProof.Stepper.runLocatedBlock_sound
...
theorem ebInv_accCopy {mm bM n acc : Nat} (hn : 2 ≤ n) (hn32 : n ≤ 32)
    ...
    EbInv (mcopyMem mem 1024 2048 (32 * n)) n mm bM bM where
  modulus := fastRepresents_mcopyMem_disjoint mem 1024 2048 (32 * n) 0 n mm
    (Or.inr (by omega)) hinv.modulus
  accBlock := fastRepresents_mcopyMem mem 1024 2048 n bM (by omega) hinv.baseBlock
  baseBlock := fastRepresents_mcopyMem_disjoint mem 1024 2048 (32 * n) 2048 n bM
    (Or.inl (by omega)) hinv.baseBlock
  oneBlock := by
    obtain ⟨one, h1, h2⟩ := hinv.oneBlock
    exact ⟨one, h1, fastRepresents_mcopyMem_disjoint mem 1024 2048 (32 * n) 3072 n
...
theorem ebMems_inv {mpMem amMem : Nat → Nat → Nat → ByteArray → ByteArray}
  ...
  | succ i ih =>
      ...
      have hminvI : MachineState.readWord (ebMems mpMem input bsize n mem i) 9376
          = UInt256.ofNat minv :=
        (readWord_ebMems mpMem 9376
          (fun pa pb m => spec.mpMinv pa pb 1024 m (by omega)) input bsize n mem i
      ...
      · rename_i hc
        ...
        have hminvC : MachineState.readWord
            (mcopyMem (ebMems mpMem input bsize n mem 0) 1024 2048 (32 * n)) 9376
            = UInt256.ofNat minv :=
          (readWord_mcopyMem_disjoint _ 1024 2048 (32 * n) 9376
            (Or.inr (by omega))).trans hminvI
        have hinvC : EbInv
            (mcopyMem (ebMems mpMem input bsize n mem 0) 1024 2048 (32 * n)) n mm bM
        ...
        have h := bitMemsFrom_inv spec hm hn32 bM hbM (expByte input bsize 0)
          ...
          (fun k hk => by
            ...
            exact hh)
          (mcopyMem (ebMems mpMem input bsize n mem 0) 1024 2048 (32 * n))
...
theorem ebMem_final {mpMem amMem : Nat → Nat → Nat → ByteArray → ByteArray}
    ...
    Model.FastRepresents
      (mpMem 1024 3072 1024
        (storeWord (ebMems mpMem input bsize n mem esize) (3040 + 32 * n)
  ...
  have hmi : MachineState.readWord
    ...
    exact (readWord_ebMems mpMem 9376
      (fun pa pb m => spec.mpMinv pa pb 1024 m (by omega)) input bsize n mem esize
      (by omega) hn32).trans hminv
  ...
  have hmod := storeWord_frame (ebMems mpMem input bsize n mem esize) (3040 + 32 * n)
    0 n mm (UInt256.ofNat 1) (Or.inr (by omega)) hloop.modulus
  have hacc := storeWord_frame (ebMems mpMem input bsize n mem esize) (3040 + 32 * n)
...
`entry` is the state the entry hop produces (`Main.trampolineState input 1281`),
...
theorem jumpD1876 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
    (UInt256.ofNat 1619).toNat = true := jumpD 1619 (by decide) jumpDest1802
...
def gasSteps_rrBody (s : State) {n bsize mm minv R : Nat}
  ...
  have hmid : Challenge.EvmProof.GasSteps (rrHead s mem n bsize esize msize k)
    ...
    ((gasSteps_rrHead s mem n bsize esize msize k hcode hfork hrun hnp).trans
      (sub.monpro 6144 6144 6144 (UInt256.ofNat 1385)
      ...
      exact Challenge.EvmProof.GasSteps.cast
        (((gasSteps_rrSelCall s (sub.mpMem 6144 6144 6144 mem) n bsize esize msize k h0
            ...
            (gasSteps_rrCallSel s (sub.mpMem 6144 6144 6144 mem) n bsize esize msize k
              hcode hfork hrun hnp)).trans
          (sub.monpro 6144 (selOf n k) 6144 (UInt256.ofNat 1404)
      ...
      exact Challenge.EvmProof.GasSteps.cast
        (((gasSteps_rrSelCall s (sub.mpMem 6144 6144 6144 mem) n bsize esize msize 0 h0
            ...
            (gasSteps_rrCallSel s (sub.mpMem 6144 6144 6144 mem) n bsize esize msize 0
              hcode hfork hrun hnp)).trans
          (sub.monpro 6144 (selOf n 0) 6144 (UInt256.ofNat 1404)
...
`[5] ++ OUTER` at pc 1569 and ending at `RRE` (pc 1679). -/
...
def gasSteps_blBody (s : State) {n bsize mm minv R : Nat}
  ...
  ((((gasSteps_blBodyHead s mem n bsize esize msize pb j hpb hjpb hcode hfork hrun
    ...
    (gasSteps_blMul s mem n bsize esize msize pb j hcode hfork hrun hnp)).trans
      (sub.monpro 1024 5120 1024 (UInt256.ofNat 1455)
    ...
    (gasSteps_blAdd s (sub.mpMem 1024 5120 1024 mem) input n bsize esize msize pb j
      ...
      ((sub.addmod 1024 3072 1024 (UInt256.ofNat 1490)
        (UInt256.ofNat j :: UInt256.ofNat pb :: outer n bsize esize msize)
        (storeWord (sub.mpMem 1024 5120 1024 mem) (3040 + 32 * n)
...
def gasSteps_baseChain_fallback (s : State) {n bsize mm minv R : Nat}
  ...
  (((Challenge.EvmProof.GasSteps.cast
    ...
    ((sub.monpro 1024 6144 2048 (UInt256.ofNat 1517) (outer n bsize esize msize)
        (blMems sub.mpMem sub.amMem input n bsize (pbOf bsize)
          (storeWord mem (992 + 32 * n) (UInt256.ofNat (topLimbOf input bsize)))
          (pbOf bsize - 1))
  ...
  if h : bitAt w r = 0 then
    Challenge.EvmProof.GasSteps.cast
      (((gasSteps_ebitHead s mem n bsize esize msize i w (2 ^ r) hcode hfork hrun
          hnp).trans
        (sub.monpro 1024 1024 1024 (UInt256.ofNat 1549)
    ...
    Challenge.EvmProof.GasSteps.cast
      (((((gasSteps_ebitHead s mem n bsize esize msize i w (2 ^ r) hcode hfork hrun
        ...
        ((gasSteps_ebitMul s (sub.mpMem 1024 1024 1024 mem) n bsize esize msize i w
          ...
          (sub.monpro 1024 2048 1024 (UInt256.ofNat 1574)
    ...
    if h7 : lzSkip input bsize i = 7 then
      Challenge.EvmProof.GasSteps.cast
        ((Challenge.EvmProof.GasSteps.cast hcopy rfl
          ...
          ((gasSteps_ebitNextExit s (mcopyMem mem 1024 2048 (32 * n)) n bsize esize
              msize i (expByte input bsize i) hcode hfork hrun hnp).trans
            (gasSteps_ebTail s (mcopyMem mem 1024 2048 (32 * n)) n bsize esize msize
      ...
      Challenge.EvmProof.GasSteps.cast
        ((hcopy.trans
            (Challenge.EvmProof.GasSteps.cast
              (gasSteps_ebitNextLoop s (mcopyMem mem 1024 2048 (32 * n)) n bsize
              ...
              (by
                have hh : Lz.topBit (expByte input bsize i) / 2
                  ...
                  omega
                rw [hh]))).trans
          (gasSteps_byteFrom s sub spec (mcopyMem mem 1024 2048 (32 * n)) esize msize
...
def gasSteps_setupToRR (s : State) {n bsize mm minv : Nat}
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (r1Call s mem 4096 (UInt256.ofNat 1348) n bsize esize msize)
  ...
  (((dbl (UInt256.ofNat 1348) mem jumpD1533 hframe).trans
...
theorem handled_of_baseFallback (input : ByteArray) (s : State) (mem : ByteArray)
    ...
    (hcontinue : ∀ (mem' : ByteArray) (bM : Nat),
      Frame mem' n bsize minv →
      Model.FastRepresents mem' 0 n mm →
      Model.FastRepresents mem' 2048 n bM →
      (∃ one, one < Limbs.radix ∧ Model.FastRepresents mem' 3072 n one) →
      EbInv (mcopyMem mem' 1024 4096 (32 * n)) n mm bM
        (expAcc mm (Limbs.radix ^ n) bM (expBits input bsize) 0) →
      bM < mm →
      bM ≡ Precompile.bytesToNatPadded input 96 bsize * Limbs.radix ^ n [MOD mm] →
      Model.FastRepresents mem' 1024 n
  ...
  let finalMem := sub.mpMem 1024 6144 2048
    (blMems sub.mpMem sub.amMem input n bsize (pbOf bsize)
      (storeWord mem (992 + 32 * n) (UInt256.ofNat (topLimbOf input bsize)))
      (pbOf bsize - 1))
  ...
  have hmod7 : Model.FastRepresents finalMem 0 n mm := by
    dsimp only [finalMem]
    exact spec.mpFrame 1024 6144 2048 0 mm _ (by omega) (by omega) hblInv.modulus
  have hone7 : ∃ one, one < Limbs.radix ∧ Model.FastRepresents finalMem 3072 n one := by
    obtain ⟨one, honelt, honerep⟩ := hblInv.oneBlock
    exact ⟨one, honelt, by
      dsimp only [finalMem]
      exact spec.mpFrame 1024 6144 2048 3072 one _ (by omega) (by omega) honerep⟩
  obtain ⟨final, ⟨tr⟩, hdone, hres⟩ := hcontinue finalMem baseM (by
    ...
    (by
      dsimp only [finalMem]
      exact spec.mpFrame 1024 6144 2048 1024
...
`R1B` (pc 2580) stands in front of the first `DOUBLE256` call.  When the
...
/-- **The guarded `R1` block against the real `CSUB`.**  Entering pc 2580 with
...
`CCB` (pc 2542) replaces the second `DOUBLE256`.  It doubles the block at
...
theorem ccSeedMem_inv {mpMem amMem : Nat → Nat → Nat → ByteArray → ByteArray}
    ...
    Model.FastRepresents (ccSeedMem amMem px mem i) 0 n mm ∧
      MachineState.readWord (ccSeedMem amMem px mem i) 9376 = UInt256.ofNat minv ∧
...
theorem jump2888 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 2117).toNat = true := by
  rw [show (UInt256.ofNat 2117).toNat = 2117 by decide]
  exact jumpDest2310
...
def gasSteps_ccbFull (s : State) {n bsize mm minv R : Nat}
  ...
  · intro i _hi
    ...
    exact sub.monpro px px px (UInt256.ofNat 2117)
...
theorem setupToDirect_facts {s : State} {bsize minv : Nat} (n mm : Nat)
    ...
    Model.FastRepresents
      ...
      Model.FastRepresents
        (setupToDirectMem (r1Mem n) (ccbMem n sub.mpMem sub.amMem) n mem) 4096 n
        (Limbs.radix ^ n % mm) ∧
      Model.FastRepresents
        (setupToDirectMem (r1Mem n) (ccbMem n sub.mpMem sub.amMem) n mem) 5120 n
...
theorem setupToRR_preserves {s : State} {n bsize mm minv R : Nat}
  ...
  have h3 : Model.FastRepresents
      (ccbMem n sub.mpMem sub.amMem 5120
...
theorem readWord_setupMem_high (input : ByteArray) (m0 target : Nat)
    (hm : Challenge.Modexp.modulusSize input ≤ 1024)
    (hlo : 4096 ≤ target) (hhi : target + 32 ≤ 9344) :
    MachineState.readWord (Setup.setupMem ByteArray.empty input m0) target =
      UInt256.ofNat 0 := by
  have hS := Setup.s32_le_1024 input hm
  have hms := Setup.modulusSize_le_s32 input
...
/-- The setup block ends at the dispatcher (pc 3337) with the plain outer frame,
...
theorem fastSetup_entry_eq (input : ByteArray) :
    Setup.fastSetupState input =
      retTo (Setup.fastSetupState input) (Setup.fastSetupMemory input) (UInt256.ofNat 3337)
...
theorem handled_of_handover (input : ByteArray) (s : State) (mem : ByteArray)
    ...
    ∃ final : State,
      Nonempty (Challenge.EvmProof.GasSteps
        (r1Call s mem 4096 (UInt256.ofNat 1348) n bsize esize msize) final) ∧
        final.isDone = true ∧
        final.toResult = .returned (Challenge.Modexp.spec input) := by
