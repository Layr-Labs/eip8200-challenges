...
/-- The **ladder** entry, pc 4016, stack `[px, ret] ++ OUTER`.  Same calling
...
theorem run_r0 (s : State) (mem : ByteArray) (n bsize esize msize : Nat)
  ...
  simp (config := { maxSteps := 600000 }) [blk1138, opAt, pushAt, wfOp,
    ...
    hfix2, h4016, h4016Nat, Ladder.jumpDest4016, State.activeWordsAfterUInt256,
    State.activeWordsAfterUInt256_2,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
...
theorem jump4040 :
  ...
  exact Ladder.jumpDest4040

/-- **The ladder against the real `ADDMOD` and `MONPRO`.**  Entering pc 4016
...
def gasSteps_ladderFull (s : State) {n bsize mm minv R : Nat}
  ...
  (Ladder.gasSteps_ladder s px n ret (outer n bsize esize msize)
      ...
      hn32 hact hf.s32
      (fun i _ =>
        sub.addmod px px px (UInt256.ofNat 4040)
