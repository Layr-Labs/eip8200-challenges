...
theorem run_push (input : ByteArray) (condition : UInt256) :
    Challenge.EvmProof.Stepper.runLocated branchPushLocated
      (conditionState input 5073 condition) =
      some (jumpStackState input condition) := by
  have hpc : (conditionState input 5073 condition).pc.toNat =
  ...
  have hop : Challenge.EvmProof.Stepper.runLocated branchPushLocated
    ...
    change (if (conditionState input 5073 condition).pc.toNat =
        Artifact.submissionArtifact.instructionPC 2176 then
        Challenge.EvmProof.Stepper.runInstr (.push 2 (UInt256.ofNat 5081))
          (conditionState input 5073 condition) else none) = _
    rw [if_pos hpc]
    have hcap : (conditionState input 5073 condition).stack.length < 1024 := by
      simp [conditionState]
    have hwidth : ¬ (2 : Fin 33).val = 0 := by decide
    have hthree : (2 : Fin 33).val + 1 = 3 := by decide
    have hadd : UInt256.ofNat 5073 + UInt256.ofNat 3 = UInt256.ofNat 5076 :=
