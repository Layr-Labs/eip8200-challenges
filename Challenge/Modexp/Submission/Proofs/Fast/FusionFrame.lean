...
theorem run_a (hcap : rest.length ≤ 1005) :
    runInstructions (chunkA) { s with pc := 4201, stack := [p,oldHead,oldEnd,ent,neg,mask,ent2,inv,m0,tl,m96,m64,m32,aprev,dst,ret] ++ rest } =
      some { s with pc := 4209, stack := [p,oldHead,oldEnd,ent,neg,mask,ent2,inv,m0,tl,m96,m64,m32,tl+256,dst,ret] ++ rest } := by
...
theorem run_b (hcap : rest.length ≤ 1005) :
    runInstructions (chunkB head) { s with pc := 4209, stack := [p,oldHead,oldEnd,ent,neg,mask,ent2,inv,m0,tl,m96,m64,m32,tl+256,dst,ret] ++ rest } =
      some { s with pc := 4225, stack := [tl-1856,head,224,ent,neg,mask,ent2,inv,m0,tl,m96,m64,m32,tl+256,dst,ret] ++ rest } := by
  have h16 : rest.length + 16 < 1024 := by omega
  have h17 : rest.length + 17 < 1024 := by omega
  have h18 : rest.length + 18 < 1024 := by omega
...
theorem run_c (hcap : rest.length ≤ 1005) :
    runInstructions (chunkC finish) { s with pc := 4225, stack := [tl-1856,head,224,ent,neg,mask,ent2,inv,m0,tl,m96,m64,m32,tl+256,dst,ret] ++ rest } =
      some { s with pc := 4242, stack := [tl-1856,head,224,ent2-292,neg,mask,ent2,inv,m0,tl,m96,m64,m32,tl+256,256,finish] ++ rest } := by
  have h16 : rest.length + 16 < 1024 := by omega
  have h17 : rest.length + 17 < 1024 := by omega
  have h18 : rest.length + 18 < 1024 := by omega
...
theorem run_prefix (hcap : rest.length ≤ 1005) :
    runInstructions (frameProgram head finish) { s with pc := 4201, stack := [p,oldHead,oldEnd,ent,neg,mask,ent2,inv,m0,tl,m96,m64,m32,aprev,dst,ret] ++ rest } =
      some { s with pc := 4242, stack := [tl-1856,head,224,ent2-292,neg,mask,ent2,inv,m0,tl,m96,m64,m32,tl+256,256,finish] ++ rest } := by
