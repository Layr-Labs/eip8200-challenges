...
The early wrapper handles matching headers first. Every other header reaches
the fast entry at pc 599 with all three decoded lengths retained. The
Montgomery success proof is then reused. If that fast path declines, Setup
preserves the same header frame for the route-aware reference-body proof.
-/
