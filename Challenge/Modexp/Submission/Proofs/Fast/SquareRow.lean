...
def blockB : Block Artifact.submissionArtifact .Osaka 4795 programB :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3645 41 4795 programB
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_B4 (s : State) (C b2 P hd w3 ent : UInt256)
    ...
    runInstructions programB4
      { s with pc := UInt256.ofNat 4833, stack := C :: b2 :: P :: hd :: w3 :: ent :: rest } =
    some { s with pc := ent,
...
theorem run_B23b (s : State) (tA d lo b2 P : UInt256)
    ...
    runInstructions programB23b
      { s with pc := UInt256.ofNat 4822, stack := tA :: d :: lo :: b2 :: P :: rest } =
