...
* computes `f = x + tb`, `b2 = f + x`, `lo = x * f`, `hi` (512-bit high word via
  `MULMOD … 2^256-1`), stores `s = lo + t_i` at `P + 0x0e40 = tAddr n i` and leaves the
...
/-- `SWAP1 DUP4 PUSH2 0x0e40 ADD` (idx 3636..3639). -/
...
theorem run_B (s : State) (tb x P hd w3 ent w5 M : UInt256)
  ...
  have h4 := run_B3b s ((3648 : UInt256) + P) (loOf x tb) (hiOf x tb M) ((x + tb) + x) P
...
/-- ... and `P + 0x0e40` is the address of `t_i`. -/
theorem tptr_toNat (n i : Nat) (hi : i < n) (hn : n ≤ 32) :
  ...
  have h3648 : (3648 : UInt256).toNat = 3648 := rfl
  rw [h3648, Nat.mod_eq_of_lt (by omega)]
  omega
