...
/-- Separate square setup. The ordinary multiply keeps its full clear. -/
...
def cached (s : State) (mem : ByteArray) (hd : UInt256) (pa pb n : Nat)
  ...
  { cachedSetupState s mem hd pa pb n inv m0 rest with pc := UInt256.ofNat 5511 }

def staged (s : State) (mem : ByteArray) (hd : UInt256) (pb n : Nat)
    (inv m0 : UInt256) (rest : List UInt256) : State :=
  { s with
      pc := UInt256.ofNat 5521
      memory := mem
...
theorem run_stage (s : State) (mem : ByteArray) (hd : UInt256) (pa pb n : Nat)
  ...
  have hpaN : pa % 2^256 = pa := Nat.mod_eq_of_lt (by omega)
  have hszN : (32*n) % 2^256 = 32*n := Nat.mod_eq_of_lt (by omega)
  ...
  have hactN : s.activeWords.toNat % 2^256 = s.activeWords.toNat :=
...
theorem run_entry (s : State) (mem : ByteArray) (hd : UInt256) (pa pb n : Nat)
  ...
  have h0 : runInstructions [.op .JUMPDEST] (input s mem hd pa pb dst ret rest) =
      some { input s mem hd pa pb dst ret rest with pc := UInt256.ofNat 5455 } := by
  ...
  have hsh := EntryPrefix.run_shuffle {s with memory := mem} hd
    ...
    (MachineState.readWord mem 2784) (MachineState.readWord mem 96)
    (MachineState.readWord mem 32) (EntryPrefix.displacement mem) rest hcap hact 5494
  have hlo := EntryPrefix.run_low {s with memory := mem} hd
    ...
    (MachineState.readWord mem 96) (MachineState.readWord mem 32) rest hcap hact 5507
  ...
  have hprefix' : runInstructions
      ((EntryPrefix.loadProgram ++ EntryPrefix.shuffleProgram) ++ EntryPrefix.lowProgram)
      { input s mem hd pa pb dst ret rest with pc := UInt256.ofNat 5455 } =
...
def block : Block Artifact.submissionArtifact .Osaka 5454 program :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 4388 57 5454 program
