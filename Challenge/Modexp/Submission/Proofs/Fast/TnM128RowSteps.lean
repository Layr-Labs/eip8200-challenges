...
def l1PC (n : Nat) : Nat := if n = 4 then 5319 else 3572

theorem l1_jump (n : Nat) (hn : n = 4 ∨ n = 8) :
    Decode.isValidJumpDest TnM128Candidate.bytecode (UInt256.ofNat (l1PC n)).toNat = true := by
  rcases hn with rfl | rfl
  · exact TnM128CandidateArtifact.isValidJumpDest_index 4267 (by rfl)
  · exact TnM128CandidateArtifact.isValidJumpDest_index 2843 (by rfl)

theorem row_jump :
    Decode.isValidJumpDest TnM128Candidate.bytecode (UInt256.ofNat 3543).toNat = true :=
  TnM128CandidateArtifact.isValidJumpDest_index 2814 (by rfl)
...
noncomputable def row_steps (s : State)
    ...
    GasSteps
      (framed s (UInt256.ofNat 3543)
        (TnCacheFrameOps.frame pbi (UInt256.ofNat 3543) pb (UInt256.ofNat (l1PC n))
          tn m128 inv
      ...
      (framed {s with memory := (result s.memory tn pa n pbi).memory}
        (if UInt256.isTrue (UInt256.gt (negative32+pbi) pb)
         then UInt256.ofNat 3543 else UInt256.ofNat 4138)
        (TnCacheFrameOps.frame (negative32+pbi) (UInt256.ofNat 3543) pb
  ...
  have hhd : Decode.isValidJumpDest s.executionEnv.code
      (UInt256.ofNat 3543).toNat = true := by rw [env.code]; exact row_jump
  have hhead := TnCacheHeadTrace.run_head s 3543 pa n pbi (UInt256.ofNat 3543) pb
  ...
  have g1 : GasSteps
      (TnCacheL1Trace.qState s (UInt256.ofNat (l1PC n)) q1 bi
        pbi (UInt256.ofNat 3543) pb (UInt256.ofNat (l1PC n)) tn m128 inv tail)
      (TnCacheL1Trace.qState s (UInt256.ofNat (middlePC n)) (l1Run q1 bi pa n 1 (n-1)) bi
        pbi (UInt256.ofNat 3543) pb (UInt256.ofNat (l1PC n)) tn m128 inv tail) := by
    by_cases h4 : n = 4
    · subst n
      exact TnM128L1Steps.private_steps s env q1 bi pa 4 1 3 (by decide)
        pbi (UInt256.ofNat 3543) pb (UInt256.ofNat (l1PC 4)) tn m128 inv tail
    ...
    · have h8 : n = 8 := hn.resolve_left h4
      ...
      exact TnM128L1Steps.suffix_steps s env q1 bi pa 8 1 7 (by decide)
        pbi (UInt256.ofNat 3543) pb (UInt256.ofNat (l1PC 8)) tn m128 inv tail
        (by simp only [tail, List.length_cons]; omega) hact (by decide) (by decide) hpa hs1
  ...
  have hm128 : m128 = MachineState.readWord qn.memory 128 := by
    ...
    exact hcache
  have g2 := reduction_steps s env qn bi tn n hn pbi (UInt256.ofNat 3543) pb
