...
/-- One straight MAC block on a known-zero incoming carry: `macFusedZeroProgram` spends two
`JUMPDEST`s where `macFusedProgram` spends the `DUP4` that reproduces the carry plus the
`PUSH0; ADD` that added it.  Same 36 bytes, four gas cheaper (30 instructions). -/
