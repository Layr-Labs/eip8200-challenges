...
/-- Blocks 0 and 4, `JUMPDEST` included (30 instructions, 37 bytes). -/
...
carry `DUP4; ADD` of zero with `JUMPDEST`+`PUSH3`.  Same 36 bytes, four gas cheaper. -/
def blockZeroProgram (a t : UInt256) : List Instr :=
  loadProgram a ++ CiosCached.macFusedZeroProgram t t

/-- Block 0 only, `JUMPDEST` included (30 instructions, 37 bytes).  Block 4 must *not* use this
