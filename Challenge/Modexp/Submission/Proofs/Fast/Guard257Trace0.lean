...
theorem run_prelude (input : ByteArray) :
  ...
  simp [preludePath, Main.opAt, Main.pushAt, Main.wfOp,
    ...
    entryState, Main.trampolineState, accState, acc0, initialState, g257PC0, g257PC1, g257PC2,
...
theorem run_check0 (input : ByteArray) :
  ...
  simp [check0Path, Main.opAt, Main.pushAt, Main.wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    accState, acc1, Guard257State.chunk0, scanDiff, Guard257Data.checks, initialState, g257PC0, g257PC1, g257PC2,
    Challenge.EvmProof.Word.literal_eq_ofNat, Challenge.EvmProof.Word.succ_ofNat_mod, Challenge.EvmProof.Word.ofNat_add_mod, Challenge.EvmProof.Word.word_toNat_ofNat, hzeroWord, hzeroNat]

set_option linter.unusedSimpArgs false in
theorem run_check1 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock check1Path (accState input 4865 (acc1 input)) =
      some (accState input 4897 (acc2 input)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
  simp [check1Path, Main.opAt, Main.pushAt, Main.wfOp,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    accState, acc2, Guard257State.chunk1, scanDiff, Guard257Data.checks, initialState, g257PC0, g257PC1, g257PC2,
    Challenge.EvmProof.Word.literal_eq_ofNat, Challenge.EvmProof.Word.succ_ofNat_mod, Challenge.EvmProof.Word.ofNat_add_mod, Challenge.EvmProof.Word.word_toNat_ofNat, hzeroWord, hzeroNat]
