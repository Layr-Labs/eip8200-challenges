...
def pcH1 : Nat := 4769
def pcH1Fall : Nat := 4778
def pcR4 : Nat := 4782

/-- 判别块。 -/
def hookProgram : List Instr :=
  [.op .JUMPDEST, .op (.Dup ⟨6, by decide⟩), .push 2 4186, .op .EQ, .push 2 4782, .op .JUMPI]

def h1Program : List Instr :=
  [.op (.Dup ⟨6, by decide⟩), .push 2 4186, .op .EQ, .push 2 4782, .op .JUMPI]

def h1FallProgram : List Instr := [.push 2 5280, .op .JUMP]

def h1Block : Block Artifact.submissionArtifact .Osaka 4769 h1Program :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3599 5 4769 h1Program
    (by decide) (by rfl) (by rfl) (by decide)

def h1FallBlock : Block Artifact.submissionArtifact .Osaka 4778 h1FallProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3604 2 4778 h1FallProgram
    (by decide) (by rfl) (by rfl) (by decide)

def h2Block : Block Artifact.submissionArtifact .Osaka 4409 hookProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3333 6 4409 hookProgram
    (by decide) (by rfl) (by rfl) (by decide)

theorem jumpDestR4 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4782 = true :=
...
theorem jumpDestRow : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4444 = true :=
  Artifact.isValidJumpDest_index 3355 (by rfl)

theorem cond_four : UInt256.isTrue ((UInt256.ofNat 4186).eq (l2Target 4)) := by decide

theorem cond_eight : ¬ UInt256.isTrue ((UInt256.ofNat 4186).eq (l2Target 8)) := by decide

theorem cond_four' : UInt256.isTrue ((UInt256.ofNat 4186).eq (UInt256.ofNat 4186)) := by decide

theorem cond_eight' : ¬ UInt256.isTrue ((UInt256.ofNat 4186).eq (UInt256.ofNat 4038)) := by decide
...
theorem run_hookTaken (pc0 : Nat) (s : State) (mem : ByteArray)
  ...
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4782 = true := by
...
theorem run_h1FallJump (s : State) (mem : ByteArray)
    ...
    some (frameAt 5280 s mem 8 pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest) := by
  have hc16 : rest.length + 16 < 1024 := by omega
  have hc17 : rest.length + 17 < 1024 := by omega
...
def gasSteps_h1Fall (n : Nat) (hn : n = 8) :
    Challenge.EvmProof.GasSteps
      (frameAt pcH1 s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      (frameAt 5280 s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest) := by
  subst hn
  exact (SquareRow.stepsOf h1Block
