...
def h1FallProgram : List Instr := [.push 2 5314, .op .JUMP]
...
theorem jumpDest5314 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 5314 = true :=
...
theorem run_h1FallJump (s : State) (mem : ByteArray)
    ...
    some (frameAt 5314 s mem 8 pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest) := by
  have hc16 : rest.length + 16 < 1024 := by omega
  have hc17 : rest.length + 17 < 1024 := by omega
  have hjd : Decode.isValidJumpDest s.executionEnv.code 5314 = true := by
    rw [hcode]; exact jumpDest5314
...
def gasSteps_h1Fall (n : Nat) (hn : n = 8) :
    Challenge.EvmProof.GasSteps
      (frameAt pcH1 s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      (frameAt 5314 s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest) := by
  subst hn
  exact (SquareRow.stepsOf h1Block
