...
def pcH1 : Nat := 4752
def pcH1Fall : Nat := 4761
def pcR4 : Nat := 4765

/-- 判别块。 -/
def hookProgram : List Instr :=
  [.op .JUMPDEST, .op (.Dup ⟨6, by decide⟩), .push 2 4168, .op .EQ, .push 2 4765, .op .JUMPI]

def h1Program : List Instr :=
  [.op (.Dup ⟨6, by decide⟩), .push 2 4168, .op .EQ, .push 2 4765, .op .JUMPI]

def h1FallProgram : List Instr := [.push 2 5265, .op .JUMP]

def h1Block : Block Artifact.submissionArtifact .Osaka 4752 h1Program :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3570 5 4752 h1Program
    (by decide) (by rfl) (by rfl) (by decide)

def h1FallBlock : Block Artifact.submissionArtifact .Osaka 4761 h1FallProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3575 2 4761 h1FallProgram
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem jumpDestR4 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4765 = true :=
...
theorem run_hookTaken (pc0 : Nat) (s : State) (mem : ByteArray)
  ...
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4765 = true := by
...
theorem run_h1FallJump (s : State) (mem : ByteArray)
    ...
    some (frameAt 5265 s mem 8 pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest) := by
  have hc16 : rest.length + 16 < 1024 := by omega
  have hc17 : rest.length + 17 < 1024 := by omega
  have hjd : Decode.isValidJumpDest s.executionEnv.code 5265 = true := by
...
def gasSteps_h1Fall (n : Nat) (hn : n = 8) :
    Challenge.EvmProof.GasSteps
      (frameAt pcH1 s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest)
      (frameAt 5265 s mem n pbi ent tl inv m0 m96 m64 m32 aprev pdst ret rest) := by
  subst hn
  exact (SquareRow.stepsOf h1Block
