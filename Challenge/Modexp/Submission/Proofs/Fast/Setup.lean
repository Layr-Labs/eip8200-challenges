...
def setupPathD :
  ...
  [pushAt 1125 0 0, opAt 1126 .SUB, pushAt 1127 2 9376, opAt 1128 .MSTORE,
   ...
   pushAt 1136 2 1911, opAt 1137 .JUMP]
...
/-- State at the `DOUBLE256` entry `JUMPDEST` (pc 1911). -/
...
theorem fastSetupState_pc (input : ByteArray) :
    (fastSetupState input).pc = UInt256.ofNat 1911 := rfl
...
runs from the state the retargeted entry produces to the first `DOUBLE256`
call, with the modulus loaded, `minv` computed and `R1` initialised. -/
