...
namespace Challenge.Modexp.Submission.Proofs.Fast.SquareFourCoefficients
open EvmSemantics EvmSemantics.EVM
open Challenge.Modexp.Submission.Proofs.Fast
open Monpro SquareFourProducts SquareWords

def dWord (mem : ByteArray) (j : Nat) : UInt256 :=
  MachineState.readWord mem (2368+32*(4-j))
...
theorem weighted_dWords (mem : ByteArray) (v i : Nat)
    (hv : Model.FastRepresents mem 2368 5 v) (hi : i ≤ 5) :
    weighted (fun j => (dWord mem j).toNat) i (5-i) =
      (v / Limbs.radix^i)*Limbs.radix^i := by
  have he := weighted_congr (fun j => (dWord mem j).toNat)
    (fun j => v / Limbs.radix^j % Limbs.radix) i (5-i) (fun j hj =>
      Model.readLimb_of_fastRepresents (memory := mem) (ptr := 2368) (count := 5) (value := v) (k := i+j) hv (by omega))
  rw [he, weighted_digits]
  have hbound : v / Limbs.radix^i < Limbs.radix^(5-i) := by
...
theorem weighted_coefficient (mem : ByteArray) (a : Nat) (ai : UInt256) (i : Nat)
    (hd : Model.FastRepresents mem 2368 5 (2*a)) (hi : i < 4) :
    weighted (fun j => (coefficient mem ai i j).toNat) i (5-i) =
  ...
  have htail : weighted c (i+2) (3-i) = weighted d (i+2) (3-i) := by
  ...
  have hdi : d (i+1) = (2*a/P) % Limbs.radix :=
    Model.readLimb_of_fastRepresents (memory := mem) (ptr := 2368) (count := 5) (value := 2*a) (k := i+1) hd (by omega)
  ...
  have hlen0 : 5-i = (4-i)+1 := by omega
  have hlen1 : 4-i = (3-i)+1 := by omega
  have hlen2 : 5-(i+1) = 4-i := by omega
  rw [hlen2, hlen1, weighted_cons] at hw
  have hsum : weighted c i (5-i) = ai.toNat*Limbs.radix^i +
      (d (i+1)/2*2)*P + weighted d (i+2) (3-i) := by
  ...
  change d (i+1)*P + weighted d (i+1+1) (3-i) = (2*a/P)*P at hw
  have hp : i+1+1 = i+2 := by omega
  rw [hp] at hw
  change ai.toNat*Limbs.radix^i + (d (i+1)/2*2)*P + weighted d (i+2) (3-i) =
...
theorem coefficient_high_le_one (mem : ByteArray) (a : Nat) (ai : UInt256) (i : Nat)
    (hd : Model.FastRepresents mem 2368 5 (2*a)) (ha : a < Limbs.radix^4) (hi : i < 4) :
    (coefficient mem ai i 4).toNat ≤ 1 := by
  have hword : (dWord mem 4).toNat = (2*a / Limbs.radix^4) % Limbs.radix :=
    Model.readLimb_of_fastRepresents (memory := mem) (ptr := 2368) (count := 5) (value := 2*a) (k := 4) hd (by decide)
  have hdiv : 2*a / Limbs.radix^4 < 2 := by
    rw [Nat.div_lt_iff_lt_mul (pow_pos Limbs.radix_pos 4)]
    omega
...
end Challenge.Modexp.Submission.Proofs.Fast.SquareFourCoefficients
