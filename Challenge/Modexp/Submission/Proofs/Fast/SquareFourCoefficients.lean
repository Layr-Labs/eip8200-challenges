...
def dWord (mem : ByteArray) (j : Nat) : UInt256 :=
  MachineState.readWord mem (8928+32*(4-j))
...
theorem weighted_dWords (mem : ByteArray) (v i : Nat)
    (hv : Model.FastRepresents mem 8928 5 v) (hi : i ≤ 5) :
  ...
  have he := weighted_congr (fun j => (dWord mem j).toNat)
    (fun j => v / Limbs.radix^j % Limbs.radix) i (5-i) (fun j hj =>
      Model.readLimb_of_fastRepresents (memory := mem) (ptr := 8928) (count := 5) (value := v) (k := i+j) hv (by omega))
...
theorem weighted_coefficient (mem : ByteArray) (a : Nat) (ai : UInt256) (i : Nat)
    (hd : Model.FastRepresents mem 8928 5 (2*a)) (hi : i < 4) :
  ...
  have hdi : d (i+1) = (2*a/P) % Limbs.radix :=
    Model.readLimb_of_fastRepresents (memory := mem) (ptr := 8928) (count := 5) (value := 2*a) (k := i+1) hd (by omega)
...
theorem coefficient_high_le_one (mem : ByteArray) (a : Nat) (ai : UInt256) (i : Nat)
    (hd : Model.FastRepresents mem 8928 5 (2*a)) (ha : a < Limbs.radix^4) (hi : i < 4) :
    (coefficient mem ai i 4).toNat ≤ 1 := by
  have hword : (dWord mem 4).toNat = (2*a / Limbs.radix^4) % Limbs.radix :=
    Model.readLimb_of_fastRepresents (memory := mem) (ptr := 8928) (count := 5) (value := 2*a) (k := 4) hd (by decide)
