...
theorem diag1Part0_run (s : State) (x0 x1 x2 x3 x4 x5 x6 x7 x8 x9 x10 x11 x12 : UInt256) (rest : List UInt256)
    ...
    runInstructions diag1Part0 { s with pc := UInt256.ofNat 4928, stack := [x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12] ++ rest } =
    some { s with pc := UInt256.ofNat 4934, stack := [x1, (x1 + x0), ((x1 + x0) + x1), x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12] ++ rest } := by
