...
/-- `macFusedPostProgram` with the incoming-carry `DUP4` folded away: on a frame whose
incoming carry is already zero the `DUP4` would reproduce `0`, and the `PUSH0; ADD` that
added it is a two-byte no-op, so the schedule spends two `JUMPDEST`s instead — same byte
length, three gas cheaper, and equivalent *only* where `c = 0` — the row-head cell of a
conversion chain, never a cell fed by a predecessor. -/
