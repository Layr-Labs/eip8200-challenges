...
/-- `macFusedPostProgram` with the incoming-carry `DUP4; ADD` pair replaced by two `JUMPDEST`s:
same byte length, three gas cheaper, and equivalent *only* on a frame whose incoming carry is
already zero.  The `DUP4` reproduces the cell's incoming carry `c` and the `ADD` folds it in, so
where `c = 0` the pair is the identity `x + 0 = x` on the stack and both bytes may be spent on
no-ops instead.  Sound exactly where `c = 0` — the row-head cell of a conversion chain, never a
cell fed by a predecessor. -/
