...
/-- `macFusedPostProgram` with the incoming-carry `DUP4; ADD` of zero replaced by one `JUMPDEST`
plus a `PUSH3` load address (second pad absorbed): same byte length, four gas cheaper per head. -/
