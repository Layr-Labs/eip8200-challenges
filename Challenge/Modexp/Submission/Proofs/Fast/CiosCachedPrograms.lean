...
/-- `macFusedPostProgram` with the incoming-carry `DUP4; ADD` of zero replaced by a `PUSH4`
load address: same byte length, five gas cheaper per head, sound only when `c = 0`. -/
