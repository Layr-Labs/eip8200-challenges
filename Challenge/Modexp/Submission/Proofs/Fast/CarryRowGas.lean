...
opaque gasSteps_out (s : State) (mem : ByteArray) (pb n i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (outState s mem pb n i (UInt256.ofNat 3722) ent pdst ret rest)
      (firstAt 3725 s mem (rowBi mem pb n i) pb n i (UInt256.ofNat 3722) ent pdst ret rest) :=
  CarryRowBlocks.out.steps
    (environment (outState s mem pb n i (UInt256.ofNat 3722) ent pdst ret rest) hcode hfork hrun hnp)
    rfl
...
opaque gasSteps_commonFirst (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (firstAt 3725 s mem bi pb n i hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
      (l1At 3749 s mem bi pa pb n i 1 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) :=
...
opaque gasSteps_l1Dispatch4 (s : State) (q : MacState) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1Q 3749 s q bi pb 4 i hd (l1Target 4) pdst ret rest)
      (l1Q 3899 s q bi pb 4 i hd (l1Target 4) pdst ret rest) :=
  l1Dispatch.steps (environment (l1Q 3749 s q bi pb 4 i hd (l1Target 4) pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Dispatch8 (s : State) (q : MacState) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1Q 3749 s q bi pb 8 i hd (l1Target 8) pdst ret rest)
      (l1Q 3751 s q bi pb 8 i hd (l1Target 8) pdst ret rest) :=
  l1Dispatch.steps (environment (l1Q 3749 s q bi pb 8 i hd (l1Target 8) pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1MulFour (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 3749 s mem bi pa pb 4 i 1 hd (l1Target 4) pdst ret rest)
...
opaque gasSteps_l1MulEight (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 3749 s mem bi pa pb 8 i 1 hd (l1Target 8) pdst ret rest)
...
opaque gasSteps_mid (s : State) (mem : ByteArray) (c bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At 4037 s (midMem1 mem c) (overflow mem c) (rowMu mem n)
...
opaque gasSteps_l2Dispatch4 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4037 s mem bi mu c0 pb 4 i k hd ent pdst ret rest)
      (l2At 4188 s mem bi mu c0 pb 4 i k hd ent pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4037 s mem bi mu c0 pb 4 i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Dispatch8 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4037 s mem bi mu c0 pb 8 i k hd ent pdst ret rest)
      (l2At 4041 s mem bi mu c0 pb 8 i k hd ent pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4037 s mem bi mu c0 pb 8 i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Four (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l2At 4037 s mid bi mu c0 pb 4 i 0 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
...
opaque gasSteps_l2Eight (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    (hc : CiosReadonlyExtra.ExtraCache mid m96 m64 m32) :
    Challenge.EvmProof.GasSteps
      (l2At 4037 s mid bi mu c0 pb 8 i 0 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
...
theorem jumpDest4726 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4377 = true :=
...
def dispatchProgram : List Instr :=
  [.op (.Dup ⟨1, by decide⟩), .push 2 4446, .op .EQ, .push 2 4377, .op .JUMPI]
...
def dispatchBlock : Block Artifact.submissionArtifact .Osaka 4312 dispatchProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3271 5 4312 dispatchProgram
    (by decide) (by rfl) (by rfl) (by decide)

/-- The `nx` `JUMPDEST` (reached by the dispatch's fall-through and by `sq_exit`'s last
square). -/
def nxJd : Block Artifact.submissionArtifact .Osaka 4321 [] :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3276 0 4321 []
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_dispatchMul (s : State) (mem : ByteArray) (pbi : UInt256) (pb n : Nat)
  ...
  have hcond : ¬ UInt256.isTrue ((UInt256.ofNat 4446).eq hd) := by
    rw [UInt256.eq]
...
theorem run_dispatchSq (s : State) (mem : ByteArray) (pbi : UInt256) (pb n : Nat)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions dispatchProgram
      (CiosCachedTailDefs.exitState s mem pbi pb n (UInt256.ofNat 4446) ent dst ret rest) =
    some (CiosCachedTailDefs.sqExitState s mem pbi pb n (UInt256.ofNat 4446) ent dst ret rest) := by
  have hc9 : rest.length + 9 < 1024 := by omega
  have hc10 : rest.length + 10 < 1024 := by omega
  have hc11 : rest.length + 11 < 1024 := by omega
  have hcond : UInt256.isTrue ((UInt256.ofNat 4446).eq (UInt256.ofNat 4446)) := by decide
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4377 = true := by
...
def gasSteps_dispatchSq (s : State) (mem : ByteArray) (pbi : UInt256) (pb n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (CiosCachedTailDefs.exitState s mem pbi pb n (UInt256.ofNat 4446) ent dst ret rest)
      (CiosCachedTailDefs.sqExitState s mem pbi pb n (UInt256.ofNat 4446) ent dst ret rest) :=
  dispatchBlock.steps
    (environment (CiosCachedTailDefs.exitState s mem pbi pb n (UInt256.ofNat 4446) ent dst ret rest)
      hcode hfork hrun hnp) rfl
...
def gasSteps_tailLastSq (s : State) (mem : ByteArray) (c mu bi : UInt256)
    ...
    (hhd : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 4446).toNat = true) :
    Challenge.EvmProof.GasSteps
      (tailState s mem c mu bi pb n i (UInt256.ofNat 4446) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
      (CiosCachedTailDefs.sqExitState s (tailCarry mem c bi)
        (UInt256.ofNat (ptrAt (pb+32*n-32) (i+1))) pb n (UInt256.ofNat 4446) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) :=
  (tailLoop.steps (environment (tailState s mem c mu bi pb n i (UInt256.ofNat 4446) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) hcode hfork hrun hnp) rfl
    (CarryTailRows.run_last s mem c mu bi pb n i (UInt256.ofNat 4446) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)
