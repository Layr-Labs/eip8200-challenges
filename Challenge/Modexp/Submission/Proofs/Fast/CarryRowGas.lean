...
opaque gasSteps_out (s : State) (mem : ByteArray) (pb n i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (outState s mem pb n i (UInt256.ofNat 3516) ent pdst ret rest)
      (firstAt 3519 s mem (rowBi mem pb n i) pb n i (UInt256.ofNat 3516) ent pdst ret rest) :=
  CarryRowBlocks.out.steps
    (environment (outState s mem pb n i (UInt256.ofNat 3516) ent pdst ret rest) hcode hfork hrun hnp)
    rfl
...
opaque gasSteps_commonFirst (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (firstAt 3519 s mem bi pb n i hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
      (l1At 3543 s mem bi pa pb n i 1 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) :=
...
opaque gasSteps_l1Dispatch4 (s : State) (q : MacState) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1Q 3543 s q bi pb 4 i hd (l1Target 4) pdst ret rest)
      (l1Q 3693 s q bi pb 4 i hd (l1Target 4) pdst ret rest) :=
  l1Dispatch.steps (environment (l1Q 3543 s q bi pb 4 i hd (l1Target 4) pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Dispatch8 (s : State) (q : MacState) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1Q 3543 s q bi pb 8 i hd (l1Target 8) pdst ret rest)
      (l1Q 3545 s q bi pb 8 i hd (l1Target 8) pdst ret rest) :=
  l1Dispatch.steps (environment (l1Q 3543 s q bi pb 8 i hd (l1Target 8) pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1MulFour (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 3543 s mem bi pa pb 4 i 1 hd (l1Target 4) pdst ret rest)
...
opaque gasSteps_l1MulEight (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 3543 s mem bi pa pb 8 i 1 hd (l1Target 8) pdst ret rest)
...
opaque gasSteps_mid (s : State) (mem : ByteArray) (c bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At 3831 s (midMem1 mem c) (overflow mem c) (rowMu mem n)
...
opaque gasSteps_l2Dispatch4 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 3831 s mem bi mu c0 pb 4 i k hd ent pdst ret rest)
      (l2At 3981 s mem bi mu c0 pb 4 i k hd ent pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 3831 s mem bi mu c0 pb 4 i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Dispatch8 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 3831 s mem bi mu c0 pb 8 i k hd ent pdst ret rest)
      (l2At 3834 s mem bi mu c0 pb 8 i k hd ent pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 3831 s mem bi mu c0 pb 8 i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l2Dispatch8 s mem bi mu c0 pb i k hd ent pdst ret rest hcap
      (by rw [hcode]; exact jumpDestL2Eight)) |>.trans
  (l2Join8.steps (environment (l2At 3833 s mem bi mu c0 pb 8 i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Join (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 3981 s mem bi mu c0 pb n i k hd ent pdst ret rest)
      (l2At 3982 s mem bi mu c0 pb n i k hd ent pdst ret rest) :=
  l2Join.steps (environment (l2At 3981 s mem bi mu c0 pb n i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Final (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hn32 : n ≤ 8) (hn : 2 ≤ n) :
    Challenge.EvmProof.GasSteps
      (l2At 4048 s mem bi mu c0 pb n i (n-2) hd ent pdst ret rest)
      (tailState s (l2Step mem mu c0 n (n-1)).memory
        (l2Step mem mu c0 n (n-1)).carry mu bi pb n i hd ent pdst ret rest) := by
  exact l2Mac6.steps (environment (l2At 4048 s mem bi mu c0 pb n i (n-2) hd ent pdst ret rest)
...
opaque gasSteps_l2Four (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l2At 3831 s mid bi mu c0 pb 4 i 0 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
  ...
  (gasSteps_extraL2 1 3982 64 2176 2208 l2Mac4 s mid bi mu c0 pb 4 i 0 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
  (gasSteps_extraL2 2 4015 32 2144 2176 l2Mac5 s mid bi mu c0 pb 4 i 1 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
...
opaque gasSteps_l2Eight (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    (hc : CiosReadonlyExtra.ExtraCache mid m96 m64 m32) :
    Challenge.EvmProof.GasSteps
      (l2At 3831 s mid bi mu c0 pb 8 i 0 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
  ...
  (gasSteps_l2Mac 3834 10 192 2304 2336 l2Mac0 s mid bi mu c0 pb 8 i 0 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest) hcap' hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)).trans <|
  (gasSteps_l2Mac 3878 1 160 2272 2304 l2Mac1 s mid bi mu c0 pb 8 i 1 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest) hcap' hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)).trans <|
  (gasSteps_l2Mac 3913 1 128 2240 2272 l2Mac2 s mid bi mu c0 pb 8 i 2 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest) hcap' hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)).trans <|
  (gasSteps_extraL2 0 3948 96 2208 2240 l2Mac3 s mid bi mu c0 pb 8 i 3 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
  (gasSteps_l2Join s mid bi mu c0 pb 8 i 4 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest) hcap' hrun hcode hfork hnp).trans <|
  (gasSteps_extraL2 1 3982 64 2176 2208 l2Mac4 s mid bi mu c0 pb 8 i 4 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
  (gasSteps_extraL2 2 4015 32 2144 2176 l2Mac5 s mid bi mu c0 pb 8 i 5 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
...
def dispatchProgram : List Instr :=
  [.op (.Dup ⟨1, by decide⟩), .push 2 4230, .op .EQ, .push 2 4170, .op .JUMPI]
...
def dispatchBlock : Block Artifact.submissionArtifact .Osaka 4105 dispatchProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3116 5 4105 dispatchProgram
    (by decide) (by rfl) (by rfl) (by decide)

/-- The `nx` `JUMPDEST` (reached by the dispatch's fall-through and by `sq_exit`'s last
square). -/
def nxJd : Block Artifact.submissionArtifact .Osaka 4114 [] :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3121 0 4114 []
    (by decide) (by rfl) (by rfl) (by decide)
...
theorem run_dispatchMul (s : State) (mem : ByteArray) (pbi : UInt256) (pb n : Nat)
  ...
  have hcond : ¬ UInt256.isTrue ((UInt256.ofNat 4230).eq hd) := by
    rw [UInt256.eq]
...
theorem run_dispatchSq (s : State) (mem : ByteArray) (pbi : UInt256) (pb n : Nat)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions dispatchProgram
      (CiosCachedTailDefs.exitState s mem pbi pb n (UInt256.ofNat 4230) ent dst ret rest) =
    some (CiosCachedTailDefs.sqExitState s mem pbi pb n (UInt256.ofNat 4230) ent dst ret rest) := by
  have hc9 : rest.length + 9 < 1024 := by omega
  have hc10 : rest.length + 10 < 1024 := by omega
  have hc11 : rest.length + 11 < 1024 := by omega
  have hcond : UInt256.isTrue ((UInt256.ofNat 4230).eq (UInt256.ofNat 4230)) := by decide
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4170 = true := by
...
def gasSteps_dispatchSq (s : State) (mem : ByteArray) (pbi : UInt256) (pb n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (CiosCachedTailDefs.exitState s mem pbi pb n (UInt256.ofNat 4230) ent dst ret rest)
      (CiosCachedTailDefs.sqExitState s mem pbi pb n (UInt256.ofNat 4230) ent dst ret rest) :=
  dispatchBlock.steps
    (environment (CiosCachedTailDefs.exitState s mem pbi pb n (UInt256.ofNat 4230) ent dst ret rest)
      hcode hfork hrun hnp) rfl
...
def gasSteps_tailLastSq (s : State) (mem : ByteArray) (c mu bi : UInt256)
    ...
    (hhd : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 4230).toNat = true) :
    Challenge.EvmProof.GasSteps
      (tailState s mem c mu bi pb n i (UInt256.ofNat 4230) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
      (CiosCachedTailDefs.sqExitState s (tailCarry mem c bi)
        (UInt256.ofNat (ptrAt (pb+32*n-32) (i+1))) pb n (UInt256.ofNat 4230) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) :=
  (tailLoop.steps (environment (tailState s mem c mu bi pb n i (UInt256.ofNat 4230) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) hcode hfork hrun hnp) rfl
    (CarryTailRows.run_last s mem c mu bi pb n i (UInt256.ofNat 4230) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)
