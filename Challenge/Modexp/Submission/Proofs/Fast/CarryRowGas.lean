...
`hd = 4274`, `ent = l1Target n`; the square rows `hd = 4805` and their own `ent`), and in
the memory/carry.  Multiply-only pieces: the row head `out` (pc 4274), `commonFirst`,
and `gasSteps_l1MulFour/Eight` (the `DUP6 JUMP` dispatch + the uniform chain).
Shared: `gasSteps_mid` (4563 → 4460), `gasSteps_l2Four/Eight` (4460 → 4832),
`gasSteps_tailNext` (4832 → next row head `hd`), `gasSteps_tailLast` (4832 → CSUB 4885).
-/
...
/-- Multiply row head (`hd = 4274`): `JUMPDEST; DUP1; MLOAD` loads `b_i`. -/
opaque gasSteps_out (s : State) (mem : ByteArray) (pb n i : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (outState s mem pb n i (UInt256.ofNat 4266) ent pdst ret rest)
      (firstAt 4269 s mem (rowBi mem pb n i) pb n i (UInt256.ofNat 4266) ent pdst ret rest) :=
  CarryRowBlocks.out.steps
    (environment (outState s mem pb n i (UInt256.ofNat 4266) ent pdst ret rest) hcode hfork hrun hnp)
    rfl
    (run_out s mem pb n i ent pdst ret rest hcap hrun hact hn hn32 hi hpb hpbFit)

/-- The first product of a multiply row (carry zero), pc 4277 → 4302. -/
opaque gasSteps_commonFirst (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (firstAt 4269 s mem bi pb n i hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
      (l1At 4293 s mem bi pa pb n i 1 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) :=
...
opaque gasSteps_l1Dispatch4 (s : State) (q : MacState) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1Q 4293 s q bi pb 4 i hd (l1Target 4) pdst ret rest)
      (l1Q 4444 s q bi pb 4 i hd (l1Target 4) pdst ret rest) :=
  l1Dispatch.steps (environment (l1Q 4293 s q bi pb 4 i hd (l1Target 4) pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l1Dispatch8 (s : State) (q : MacState) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l1Q 4293 s q bi pb 8 i hd (l1Target 8) pdst ret rest)
      (l1Q 4296 s q bi pb 8 i hd (l1Target 8) pdst ret rest) :=
  l1Dispatch.steps (environment (l1Q 4293 s q bi pb 8 i hd (l1Target 8) pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l1Dispatch8 s q bi pb i hd pdst ret rest hcap
      (by rw [hcode]; exact KernelChain.jumpDest4068))

/-- Four-limb multiply row: dispatch to block `k = 5` (pc 4452) and run steps `1..3`. -/
opaque gasSteps_l1MulFour (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 4293 s mem bi pa pb 4 i 1 hd (l1Target 4) pdst ret rest)
...
/-- Eight-limb multiply row: dispatch to block `k = 1` (pc 4304) and run steps `1..7`. -/
opaque gasSteps_l1MulEight (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l1At 4293 s mem bi pa pb 8 i 1 hd (l1Target 8) pdst ret rest)
...
/-! ## The middle block (pc 4563 → 4460), shared -/
...
opaque gasSteps_mid (s : State) (mem : ByteArray) (c bi : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (l2At 4586 s (midMem1 mem c) (overflow mem c) (rowMu mem n)
...
/-! ## Second loop (pc 4460 → 4832), shared -/
...
opaque gasSteps_l2Dispatch4 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4586 s mem bi mu c0 pb 4 i k hd ent pdst ret rest)
      (l2At 4736 s mem bi mu c0 pb 4 i k hd ent pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4586 s mem bi mu c0 pb 4 i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Dispatch8 (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4586 s mem bi mu c0 pb 8 i k hd ent pdst ret rest)
      (l2At 4589 s mem bi mu c0 pb 8 i k hd ent pdst ret rest) :=
  l2Dispatch.steps (environment (l2At 4586 s mem bi mu c0 pb 8 i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
    (run_l2Dispatch8 s mem bi mu c0 pb i k hd ent pdst ret rest hcap
      (by rw [hcode]; exact jumpDestL2Eight)) |>.trans
  (l2Join8.steps (environment (l2At 4588 s mem bi mu c0 pb 8 i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Join (s : State) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    Challenge.EvmProof.GasSteps
      (l2At 4736 s mem bi mu c0 pb n i k hd ent pdst ret rest)
      (l2At 4737 s mem bi mu c0 pb n i k hd ent pdst ret rest) :=
  l2Join.steps (environment (l2At 4736 s mem bi mu c0 pb n i k hd ent pdst ret rest) hcode hfork hrun hnp) rfl
...
opaque gasSteps_l2Four (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.GasSteps
      (l2At 4586 s mid bi mu c0 pb 4 i 0 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
  ...
  (gasSteps_extraL2 1 4737 64 4224 4256 l2Mac4 s mid bi mu c0 pb 4 i 0 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
  (gasSteps_extraL2 2 4770 32 4192 4224 l2Mac5 s mid bi mu c0 pb 4 i 1 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
...
/-- The whole eight-limb second loop (dispatch to 4462, seven cells). -/
opaque gasSteps_l2Eight (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    (hc : CiosReadonlyExtra.ExtraCache mid m96 m64 m32) :
    Challenge.EvmProof.GasSteps
      (l2At 4586 s mid bi mu c0 pb 8 i 0 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
  ...
  (gasSteps_l2Mac 4589 10 192 4352 4384 l2Mac0 s mid bi mu c0 pb 8 i 0 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest) hcap' hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)).trans <|
  (gasSteps_l2Mac 4633 1 160 4320 4352 l2Mac1 s mid bi mu c0 pb 8 i 1 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest) hcap' hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)).trans <|
  (gasSteps_l2Mac 4668 1 128 4288 4320 l2Mac2 s mid bi mu c0 pb 8 i 2 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest) hcap' hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)).trans <|
  (gasSteps_extraL2 0 4703 96 4256 4288 l2Mac3 s mid bi mu c0 pb 8 i 3 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
  (gasSteps_l2Join s mid bi mu c0 pb 8 i 4 hd ent inv m0 (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest) hcap' hrun hcode hfork hnp).trans <|
  (gasSteps_extraL2 1 4737 64 4224 4256 l2Mac4 s mid bi mu c0 pb 8 i 4 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
  (gasSteps_extraL2 2 4770 32 4192 4224 l2Mac5 s mid bi mu c0 pb 8 i 5 hd ent tl inv m0 aEnd m96 m64 m32 pdst ret rest hcap hrun hcode hfork hnp hact (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) hc).trans <|
...
/-! ## R0: the kernel-exit dispatch (pc 4850)
...
theorem jumpDest4726 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4928 = true :=
  Artifact.isValidJumpDest_index 3768 (by rfl)

theorem jumpDest4664 :
    Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4870 = true :=
  Artifact.isValidJumpDest_index 3725 (by rfl)

/-! ## R0: the kernel-exit dispatch (pc 4850)

`DUP2 PUSH2 sq_row EQ PUSH2 sq_exit JUMPI`: the frame's row head decides.  A square
(`hd = sq_row = 4984`) jumps to `sq_exit` (4917) with the frame retained; every multiply
falls through the `JUMPI` into the `nx` `JUMPDEST` (4859) and on to the 14 `POP`s. -/

def dispatchProgram : List Instr :=
  [.op (.Dup ⟨1, by decide⟩), .push 2 4995, .op .EQ, .push 2 4928, .op .JUMPI]

/-- The dispatch block: the `JUMPI` ends it, taken for a square and not taken for a
multiply (which then continues at the `nx` `JUMPDEST` 4859). -/
def dispatchBlock : Block Artifact.submissionArtifact .Osaka 4861 dispatchProgram :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3720 5 4861 dispatchProgram
    (by decide) (by rfl) (by rfl) (by decide)

/-- The `nx` `JUMPDEST` (reached by the dispatch's fall-through and by `sq_exit`'s last
square). -/
def nxJd : Block Artifact.submissionArtifact .Osaka 4870 [.op .JUMPDEST] :=
  WindowTwentyOneSlice.block Artifact.allWellFormed 3725 1 4870 [.op .JUMPDEST]
    (by decide) (by rfl) (by rfl) (by decide)

set_option linter.unusedSimpArgs false in
/-- A multiply falls through the dispatch into the `nx` `JUMPDEST` (4859 → 4640). -/
theorem run_dispatchMul (s : State) (mem : ByteArray) (pbi : UInt256) (pb n : Nat)
  ...
  have hcond : ¬ UInt256.isTrue ((UInt256.ofNat 4995).eq hd) := by
    rw [UInt256.eq]
...
set_option linter.unusedSimpArgs false in
/-- A square takes the dispatch's `JUMPI` to `sq_exit` (4917) with the frame retained. -/
theorem run_dispatchSq (s : State) (mem : ByteArray) (pbi : UInt256) (pb n : Nat)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode) :
    runInstructions dispatchProgram
      (CiosCachedTailDefs.exitState s mem pbi pb n (UInt256.ofNat 4995) ent dst ret rest) =
    some (CiosCachedTailDefs.sqExitState s mem pbi pb n (UInt256.ofNat 4995) ent dst ret rest) := by
  have hc9 : rest.length + 9 < 1024 := by omega
  have hc10 : rest.length + 10 < 1024 := by omega
  have hc11 : rest.length + 11 < 1024 := by omega
  have hcond : UInt256.isTrue ((UInt256.ofNat 4995).eq (UInt256.ofNat 4995)) := by decide
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4928 = true := by
...
set_option linter.unusedSimpArgs false in
/-- The `nx` `JUMPDEST` alone (4859 → 4640), used by the last square's exit. -/
...
def gasSteps_dispatchSq (s : State) (mem : ByteArray) (pbi : UInt256) (pb n : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (CiosCachedTailDefs.exitState s mem pbi pb n (UInt256.ofNat 4995) ent dst ret rest)
      (CiosCachedTailDefs.sqExitState s mem pbi pb n (UInt256.ofNat 4995) ent dst ret rest) :=
  dispatchBlock.steps
    (environment (CiosCachedTailDefs.exitState s mem pbi pb n (UInt256.ofNat 4995) ent dst ret rest)
      hcode hfork hrun hnp) rfl
...
def gasSteps_tailLastSq (s : State) (mem : ByteArray) (c mu bi : UInt256)
    ...
    (hhd : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 4995).toNat = true) :
    Challenge.EvmProof.GasSteps
      (tailState s mem c mu bi pb n i (UInt256.ofNat 4995) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest))
      (CiosCachedTailDefs.sqExitState s (tailCarry mem c bi)
        (UInt256.ofNat (ptrAt (pb+32*n-32) (i+1))) pb n (UInt256.ofNat 4995) ent inv m0
        (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) :=
  (tailLoop.steps (environment (tailState s mem c mu bi pb n i (UInt256.ofNat 4995) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)) hcode hfork hrun hnp) rfl
    (CarryTailRows.run_last s mem c mu bi pb n i (UInt256.ofNat 4995) ent inv m0
      (tl :: m96 :: m64 :: m32 :: aEnd :: pdst :: ret :: rest)
...
through as well), drop the frame, jump to the CSUB guard (4878). -/
