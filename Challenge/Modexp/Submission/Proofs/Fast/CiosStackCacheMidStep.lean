...
theorem run_mid (s : State) (c : CachedMemory) (r : ReadOnlyCache) (carry bi : UInt256)
    ...
    some (l2At 4576 s (cacheMid c carry) r bi (rowMu c.virtual n) (rowC0 c.virtual n)
  ...
  have h2 := Mid.run_words { s with memory := d.memory } (UInt256.ofNat 4558) d r bi
    (UInt256.ofNat (ptrAt (pb+32*n-32) i)) (UInt256.ofNat (pa+32*n-32))
  ...
  rw [hc, hmu] at h2
  have hpc : advancePC 18 (UInt256.ofNat 4558) = UInt256.ofNat 4576 := by decide
  rw [hpc] at h2
