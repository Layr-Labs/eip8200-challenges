...
The seven uniform first-loop blocks `k = 1..7` sit at instruction `3185 + 32(k-1)`,
pc `4286 + 37(k-1)`; block `k` performs limb step `j = k + n - 8` (offset
`32(7-k)`, accumulator word `2112 + 32(7-k)`, for both admitted widths).  The
middle block starts with a `JUMPDEST` at pc 4590 (instruction 3367), the entry of the
empty chain.

* `gasSteps_l1Step k`  : one block, on any MAC state `q`.
* `gasSteps_l1Suffix`  : blocks `k..7` from the chain entry `4286 + 37(k-1)` to the
  ...
  dispatch; the square rows enter at `ent = 4286 + 37(k-1)` directly.
-/
...
def gasSteps_l1Step (k : Nat) (hk1 : 1 ≤ k) (hk7 : k ≤ 7)
    ...
    Challenge.EvmProof.GasSteps
      (l1Q (3740 + 37 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q (3740 + 37 * k) s (SquareModel.l1StepOn q bi pa n j) bi pb n i hd ent pdst ret rest) := by
  ...
  interval_cases k
  · exact l1Block1.steps (environment (l1Q 3740 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 3740 192 2304 (by decide) (by decide))
  · exact l1Block2.steps (environment (l1Q 3777 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 3777 160 2272 (by decide) (by decide))
  · exact l1Block3.steps (environment (l1Q 3814 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 3814 128 2240 (by decide) (by decide))
  · exact l1Block4.steps (environment (l1Q 3851 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 3851 96 2208 (by decide) (by decide))
  · exact l1Block5.steps (environment (l1Q 3888 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 3888 64 2176 (by decide) (by decide))
  · exact l1Block6.steps (environment (l1Q 3925 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 3925 32 2144 (by decide) (by decide))
  · exact l1Block7.steps (environment (l1Q 3962 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 3962 0 2112 (by decide) (by decide))
...
def gasSteps_l1Run : (m k : Nat) → 1 ≤ k → k + m = 8 →
    ...
    Challenge.EvmProof.GasSteps
      (l1Q (3740 + 37 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q 3999 s (SquareModel.l1Run q bi pa n j m) bi pb n i hd ent pdst ret rest)
...
`4286 + 37(k-1)`) at limb step `j = k + n - 8` to the middle block's `JUMPDEST`,
...
def gasSteps_l1SuffixMul (k : Nat) (hk1 : 1 ≤ k) (hk8 : k ≤ 8)
    ...
    Challenge.EvmProof.GasSteps
      (l1At (3740 + 37 * (k - 1)) s mem bi pa pb n i j hd ent pdst ret rest)
