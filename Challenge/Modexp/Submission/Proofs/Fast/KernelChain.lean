...
The seven uniform first-loop blocks `k = 1..7` sit at instruction `3207 + 32(k-1)`,
pc `4294 + 37(k-1)`; block `k` performs limb step `j = k + n - 8` (offset
`32(7-k)`, accumulator word `4160 + 32(7-k)`, for both admitted widths).  The
middle block starts with a `JUMPDEST` at pc 4553 (instruction 3416), the entry of the
empty chain.

* `gasSteps_l1Step k`  : one block, on any MAC state `q`.
* `gasSteps_l1Suffix`  : blocks `k..7` from the chain entry `4294 + 37(k-1)` to the
  ...
  dispatch; the square rows enter at `ent = 4294 + 37(k-1)` directly.
-/
...
def gasSteps_l1Step (k : Nat) (hk1 : 1 ≤ k) (hk7 : k ≤ 7)
    ...
    Challenge.EvmProof.GasSteps
      (l1Q (4286 + 37 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q (4286 + 37 * k) s (SquareModel.l1StepOn q bi pa n j) bi pb n i hd ent pdst ret rest) := by
  ...
  interval_cases k
  · exact l1Block1.steps (environment (l1Q 4286 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4286 192 4352 (by decide) (by decide))
  · exact l1Block2.steps (environment (l1Q 4323 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4323 160 4320 (by decide) (by decide))
  · exact l1Block3.steps (environment (l1Q 4360 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4360 128 4288 (by decide) (by decide))
  · exact l1Block4.steps (environment (l1Q 4397 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4397 96 4256 (by decide) (by decide))
  · exact l1Block5.steps (environment (l1Q 4434 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4434 64 4224 (by decide) (by decide))
  · exact l1Block6.steps (environment (l1Q 4471 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4471 32 4192 (by decide) (by decide))
  · exact l1Block7.steps (environment (l1Q 4508 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4508 0 4160 (by decide) (by decide))
...
def gasSteps_l1Run : (m k : Nat) → 1 ≤ k → k + m = 8 →
    ...
    Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false →
    168 ≤ s.activeWords.toNat → n ≤ 8 → (pa + 32 * n ≤ 4096 ∨ pa = 4864) → j + 8 = k + n →
    Snapshot q.memory pa n →
    Challenge.EvmProof.GasSteps
      (l1Q (4286 + 37 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q 4545 s (SquareModel.l1Run q bi pa n j m) bi pb n i hd ent pdst ret rest)
...
`4294 + 37(k-1)`) at limb step `j = k + n - 8` to the middle block's `JUMPDEST`,
...
def gasSteps_l1Suffix (k : Nat) (hk1 : 1 ≤ k) (hk8 : k ≤ 8)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hn : n ≤ 8) (hpaFit : pa + 32 * n ≤ 4096 ∨ pa = 4864)
    (hjk : j + 8 = k + n)
    (hsnapshot : Snapshot q.memory pa n) :
    Challenge.EvmProof.GasSteps
      (l1Q (4286 + 37 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
...
def gasSteps_l1SuffixMul (k : Nat) (hk1 : 1 ≤ k) (hk8 : k ≤ 8)
    ...
    Challenge.EvmProof.GasSteps
      (l1At (4286 + 37 * (k - 1)) s mem bi pa pb n i j hd ent pdst ret rest)
  ...
  have h := gasSteps_l1Suffix k hk1 hk8 s (l1Step mem bi pa n j) bi pa pb n i j hd ent pdst ret rest
    hcap hrun hcode hfork hnp hact hn (Or.inl hpaFit) hjk (hsnapshot.l1 bi j hn hpaFit)
