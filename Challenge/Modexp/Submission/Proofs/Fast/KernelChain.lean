...
The seven uniform first-loop blocks `k = 1..7` sit at instruction `3067 + 32(k-1)`,
pc `4027 + 38(k-1)`; block `k` performs limb step `j = k + n - 8` (offset
`32(7-k)`, accumulator word `8256 + 32(7-k)`, for both admitted widths).  The
middle block starts with a `JUMPDEST` at pc 4293 (instruction 3295), the entry of the
empty chain.

* `gasSteps_l1Step k`  : one block, on any MAC state `q`.
* `gasSteps_l1Suffix`  : blocks `k..7` from the chain entry `4027 + 38(k-1)` to the
  ...
  dispatch; the square rows enter at `ent = 4027 + 38(k-1)` directly.
-/
...
def gasSteps_l1Step (k : Nat) (hk1 : 1 ≤ k) (hk7 : k ≤ 7)
    ...
    Challenge.EvmProof.GasSteps
      (l1Q (4008 + 38 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q (4008 + 38 * k) s (SquareModel.l1StepOn q bi pa n j) bi pb n i hd ent pdst ret rest) := by
  ...
  interval_cases k
  · exact l1Block1.steps (environment (l1Q 4008 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4008 192 8448 (by decide) (by decide))
  · exact l1Block2.steps (environment (l1Q 4046 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4046 160 8416 (by decide) (by decide))
  · exact l1Block3.steps (environment (l1Q 4084 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4084 128 8384 (by decide) (by decide))
  · exact l1Block4.steps (environment (l1Q 4122 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4122 96 8352 (by decide) (by decide))
  · exact l1Block5.steps (environment (l1Q 4160 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4160 64 8320 (by decide) (by decide))
  · exact l1Block6.steps (environment (l1Q 4198 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4198 32 8288 (by decide) (by decide))
  · exact l1Block7.steps (environment (l1Q 4236 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4236 0 8256 (by decide) (by decide))
...
def gasSteps_l1Run : (m k : Nat) → 1 ≤ k → k + m = 8 →
    ...
    Challenge.EvmProof.GasSteps
      (l1Q (4008 + 38 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q 4274 s (SquareModel.l1Run q bi pa n j m) bi pb n i hd ent pdst ret rest)
...
`4027 + 38(k-1)`) at limb step `j = k + n - 8` to the middle block's `JUMPDEST`,
...
def gasSteps_l1SuffixMul (k : Nat) (hk1 : 1 ≤ k) (hk8 : k ≤ 8)
    ...
    Challenge.EvmProof.GasSteps
      (l1At (4008 + 38 * (k - 1)) s mem bi pa pb n i j hd ent pdst ret rest)
