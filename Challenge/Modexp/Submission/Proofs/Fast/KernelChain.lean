...
def gasSteps_l1Step (k : Nat) (hk1 : 1 ≤ k) (hk7 : k ≤ 7)
    ...
    (hact : 91 ≤ s.activeWords.toNat) (hn : n ≤ 8) (hjk : j + 8 = k + n)
    (hsnapshot : Snapshot q.memory pa n) :
    Challenge.EvmProof.GasSteps
      (l1Q (4068 + 38 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q (4068 + 38 * k) s (SquareModel.l1StepOn q bi pa n j) bi pb n i hd ent pdst ret rest) := by
  ...
  interval_cases k
  · exact l1Block1.steps (environment (l1Q 4068 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4068 192 2304 (by decide) (by decide))
  · exact l1Block2.steps (environment (l1Q 4106 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4106 160 2272 (by decide) (by decide))
  · exact l1Block3.steps (environment (l1Q 4144 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4144 128 2240 (by decide) (by decide))
  · exact l1Block4.steps (environment (l1Q 4182 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4182 96 2208 (by decide) (by decide))
  · exact l1Block5.steps (environment (l1Q 4220 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4220 64 2176 (by decide) (by decide))
  · exact l1Block6.steps (environment (l1Q 4258 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4258 32 2144 (by decide) (by decide))
  · exact l1Block7.steps (environment (l1Q 4296 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4296 0 2112 (by decide) (by decide))
...
def gasSteps_l1Run : (m k : Nat) → 1 ≤ k → k + m = 8 →
    ...
    Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false →
    91 ≤ s.activeWords.toNat → n ≤ 8 → pa + 32 * n ≤ 2048 → j + 8 = k + n →
    Snapshot q.memory pa n →
    Challenge.EvmProof.GasSteps
      (l1Q (4068 + 38 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q 4334 s (SquareModel.l1Run q bi pa n j m) bi pb n i hd ent pdst ret rest)
...
def gasSteps_l1Suffix (k : Nat) (hk1 : 1 ≤ k) (hk8 : k ≤ 8)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 91 ≤ s.activeWords.toNat) (hn : n ≤ 8) (hpaFit : pa + 32 * n ≤ 2048)
    (hjk : j + 8 = k + n)
    (hsnapshot : Snapshot q.memory pa n) :
    Challenge.EvmProof.GasSteps
      (l1Q (4068 + 38 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
...
def gasSteps_l1SuffixMul (k : Nat) (hk1 : 1 ≤ k) (hk8 : k ≤ 8)
    ...
    Challenge.EvmProof.GasSteps
      (l1At (4068 + 38 * (k - 1)) s mem bi pa pb n i j hd ent pdst ret rest)
