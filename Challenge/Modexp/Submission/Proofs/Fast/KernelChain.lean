...
pc `4036 + 38(k-1)`; block `k` performs limb step `j = k + n - 8` (offset
`32(7-k)`, accumulator word `8256 + 32(7-k)`, for both admitted widths).  The
middle block starts with a `JUMPDEST` at pc 4302 (instruction 3295), the entry of the
empty chain.

* `gasSteps_l1Step k`  : one block, on any MAC state `q`.
* `gasSteps_l1Suffix`  : blocks `k..7` from the chain entry `4036 + 38(k-1)` to the
  ...
  dispatch; the square rows enter at `ent = 4036 + 38(k-1)` directly.
-/
...
def gasSteps_l1Step (k : Nat) (hk1 : 1 ≤ k) (hk7 : k ≤ 7)
    ...
    Challenge.EvmProof.GasSteps
      (l1Q (4017 + 38 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q (4017 + 38 * k) s (SquareModel.l1StepOn q bi pa n j) bi pb n i hd ent pdst ret rest) := by
  ...
  interval_cases k
  · exact l1Block1.steps (environment (l1Q 4017 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4017 192 8448 (by decide) (by decide))
  · exact l1Block2.steps (environment (l1Q 4055 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4055 160 8416 (by decide) (by decide))
  · exact l1Block3.steps (environment (l1Q 4093 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4093 128 8384 (by decide) (by decide))
  · exact l1Block4.steps (environment (l1Q 4131 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4131 96 8352 (by decide) (by decide))
  · exact l1Block5.steps (environment (l1Q 4169 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4169 64 8320 (by decide) (by decide))
  · exact l1Block6.steps (environment (l1Q 4207 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4207 32 8288 (by decide) (by decide))
  · exact l1Block7.steps (environment (l1Q 4245 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4245 0 8256 (by decide) (by decide))
...
def gasSteps_l1Run : (m k : Nat) → 1 ≤ k → k + m = 8 →
    ...
    Challenge.EvmProof.GasSteps
      (l1Q (4017 + 38 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q 4283 s (SquareModel.l1Run q bi pa n j m) bi pb n i hd ent pdst ret rest)
...
`4036 + 38(k-1)`) at limb step `j = k + n - 8` to the middle block's `JUMPDEST`,
...
def gasSteps_l1SuffixMul (k : Nat) (hk1 : 1 ≤ k) (hk8 : k ≤ 8)
    ...
    Challenge.EvmProof.GasSteps
      (l1At (4017 + 38 * (k - 1)) s mem bi pa pb n i j hd ent pdst ret rest)
