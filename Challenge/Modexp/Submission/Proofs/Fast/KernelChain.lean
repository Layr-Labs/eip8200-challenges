...
middle block starts with a `JUMPDEST` at pc 4293 (instruction 3286), the entry of the
empty chain.
...
def gasSteps_l1Step (k : Nat) (hk1 : 1 ≤ k) (hk7 : k ≤ 7)
    ...
    Challenge.EvmProof.GasSteps
      (l1Q (4019 + 38 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q (4019 + 38 * k) s (SquareModel.l1StepOn q bi pa n j) bi pb n i hd ent pdst ret rest) := by
  ...
  interval_cases k
  · exact l1Block1.steps (environment (l1Q 4019 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4019 192 8448 (by decide) (by decide))
  · exact l1Block2.steps (environment (l1Q 4057 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4057 160 8416 (by decide) (by decide))
  · exact l1Block3.steps (environment (l1Q 4095 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4095 128 8384 (by decide) (by decide))
  · exact l1Block4.steps (environment (l1Q 4133 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4133 96 8352 (by decide) (by decide))
  · exact l1Block5.steps (environment (l1Q 4171 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4171 64 8320 (by decide) (by decide))
  · exact l1Block6.steps (environment (l1Q 4209 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4209 32 8288 (by decide) (by decide))
  · exact l1Block7.steps (environment (l1Q 4247 s q bi pb n i hd ent pdst ret rest)
      hcode hfork hrun hnp) rfl (hrunQ 4247 0 8256 (by decide) (by decide))
...
def gasSteps_l1Run : (m k : Nat) → 1 ≤ k → k + m = 8 →
    ...
    Challenge.EvmProof.GasSteps
      (l1Q (4019 + 38 * (k - 1)) s q bi pb n i hd ent pdst ret rest)
      (l1Q 4285 s (SquareModel.l1Run q bi pa n j m) bi pb n i hd ent pdst ret rest)
...
def gasSteps_l1SuffixMul (k : Nat) (hk1 : 1 ≤ k) (hk8 : k ≤ 8)
    ...
    Challenge.EvmProof.GasSteps
      (l1At (4019 + 38 * (k - 1)) s mem bi pa pb n i j hd ent pdst ret rest)
