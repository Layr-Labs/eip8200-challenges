...
storing it at `T_ = 8192`.  `sqRowsCarry` is that machine model; it agrees with
the Monpro-style `sqRowsMem` everywhere except the scratch word `[8192, 8224)`.

`sqMem s mem n` is the memory a whole `SQUARE(2048) → 2048` call leaves behind:
for `n ∈ {4, 8}` the square rows followed by the conditional subtraction, and for
other widths the generic `MONPRO(2048, 2048) → 2048` fallback.
-/
...
theorem rowFrom_agree (a b : MacState) (h : Agree a.memory b.memory) (hc : a.carry = b.carry)
  ...
  have hf : MachineState.readWord (rowFromL2 b n).memory 8192 = overflow b.memory b.carry := by
...
theorem l1Run_agree (qa qb : MacState) (h : Agree qa.memory qb.memory) (hc : qa.carry = qb.carry)
    (bi : UInt256) (pa n j0 : Nat) (hpa : pa + 32 * n ≤ 8192) :
    ∀ k, j0 + k ≤ n →
  ...
  | succ k ih =>
      ...
      have ht := readWord_eq prev.1 (8256 + 32 * (n - 1 - (j0 + k))) (Or.inr (by omega))
...
theorem sqL1_agree (a b : ByteArray) (h : Agree a b) (n i : Nat) (tb : UInt256)
  ...
  unfold sqL1
  rw [hx]
  exact l1Run_agree _ _ hp.1 hp.2 _ 2048 n (i + 1) (by omega) (n - 1 - i) (by omega)
...
/-- One 32-byte store at `dst ∈ [8192, 8256 + 32 n)` leaves every word outside
that region alone. -/
theorem readWord_store_far (mem : ByteArray) (w dst n addr : Nat)
    (hdst : 8192 ≤ dst) (hdstHi : dst + 32 ≤ 8256 + 32 * n)
    (haddr : addr + 32 ≤ 8192 ∨ 8256 + 32 * n ≤ addr) :
...
theorem readWord_l2Step_far (mem : ByteArray) (mu c0 : UInt256) (n addr : Nat) (hn : 1 ≤ n)
  ...
  | succ k ih =>
      simp only [l2Step]
      rw [readWord_store_far _ _ (8256 + 32 * (n - 1 - k)) n addr (by omega) (by omega) haddr]
      exact ih
...
theorem readWord_rowFromCarry_far (q : MacState) (n addr : Nat) (hn : 1 ≤ n)
  ...
  rw [readWord_store_far _ _ 8224 n addr (by omega) (by omega) haddr,
    readWord_store_far _ _ 8256 n addr (by omega) (by omega) haddr,
    readWord_l2Step_far _ _ _ n addr hn haddr (n - 1),
    readWord_store_far _ _ 8224 n addr (by omega) (by omega) haddr]
...
/-- The machine-carry square rows write only inside `[8192, 8256 + 32 n)`. -/
...
/-- The machine-carry square rows write only inside `[8192, 9280)`. -/
theorem readWord_sqRowsCarry (mem : ByteArray) (n addr : Nat) (hn : n ≤ 32)
    (haddr : addr + 32 ≤ 8192 ∨ 9280 ≤ addr) (i : Nat) (hi : i ≤ n) :
...
theorem sqRowsCarry_tn_le_one (m0 : ByteArray) (p a mm : Nat) (hn32 : p + 2 ≤ 32)
    ...
    (MachineState.readWord (sqRowsCarry m0 (p + 2) (p + 2)) 8224).toNat ≤ 1 := by
  rw [readWord_eq (sqRows_agree m0 (p + 2) hn32 (p + 2) le_rfl) 8224 (Or.inr (by decide))]
...
def sqMem (s : State) (mem : ByteArray) (n : Nat) : ByteArray :=
  if n = 4 ∨ n = 8 then
    Csub.csResultMemory (sqRowsCarry (mpZeroed s (inputMemory mem 2048 n) n) n n) n 2048
  else CarryResult.monproMem s mem 2048 2048 n 2048

theorem sqMem_of_fast (s : State) (mem : ByteArray) (n : Nat) (h : n = 4 ∨ n = 8) :
    sqMem s mem n =
      Csub.csResultMemory (sqRowsCarry (mpZeroed s (inputMemory mem 2048 n) n) n n) n 2048 := by
  unfold sqMem
  rw [if_pos h]

theorem sqMem_of_not_fast (s : State) (mem : ByteArray) (n : Nat) (h : ¬(n = 4 ∨ n = 8)) :
    sqMem s mem n = Monpro.monproMem s mem 2048 2048 n 2048 := by
...
theorem csReturnedState_memory_sqMem (s : State) (mem : ByteArray) (n : Nat) (h : n = 4 ∨ n = 8)
    ...
    (Csub.csReturnedState s (sqRowsCarry (mpZeroed s (inputMemory mem 2048 n) n) n n) n n
      (UInt256.ofNat 2048) ret rest).memory = sqMem s mem n := by
...
theorem sqMem_represents (s : State) (mem : ByteArray) (p a mm : Nat)
    ...
    Model.FastRepresents (sqMem s mem (p + 2)) 2048 (p + 2)
      (Model.montMul mm (Limbs.radix ^ (p + 2)) a a) := by
  unfold sqMem
  split
  · let prepared := inputMemory mem 2048 (p + 2)
    have ha' : Model.FastRepresents prepared 2048 (p + 2) a :=
      (fastRepresents_inputMemory mem 2048 (p + 2) 2048 (p + 2) a (by omega)).2 ha
    have hm' : Model.FastRepresents prepared 0 (p + 2) mm :=
      (fastRepresents_inputMemory mem 2048 (p + 2) 0 (p + 2) mm (by omega)).2 hm
    have hminv' : ((MachineState.readWord prepared (32 * (p + 2) - 32)).toNat *
        (MachineState.readWord prepared 9376).toNat + 1) % 2 ^ 256 = 0 := by
      simpa only [prepared,
        read_inputMemory_outside mem 2048 (p + 2) (32 * (p + 2) - 32) (Or.inl (by omega)),
        read_inputMemory_outside mem 2048 (p + 2) 9376 (Or.inr (by decide))] using hminv
    have ha0 : Model.FastRepresents (mpZeroed s prepared (p + 2)) 2048 (p + 2) a := by
      refine (Model.fastRepresents_congr (a := prepared) ?_ a).1 ha'
      intro j hj
      rw [readWord_mpZeroed s prepared (p + 2) (2048 + 32 * j) hn32 (Or.inl (by omega))]
    ...
    have hminv0 : ((MachineState.readWord (mpZeroed s prepared (p + 2)) (32 * (p + 2) - 32)).toNat *
        (MachineState.readWord (mpZeroed s prepared (p + 2)) 9376).toNat + 1) % 2 ^ 256 = 0 := by
      rw [readWord_mpZeroed s prepared (p + 2) (32 * (p + 2) - 32) hn32 (Or.inl (by omega)),
        readWord_mpZeroed s prepared (p + 2) 9376 hn32 (Or.inr (by omega))]
      exact hminv'
    exact sqRowsCarry_represents (mpZeroed s prepared (p + 2)) p a mm 2048 hn32 (by omega)
      ha0 hm0 hodd ham hminv0 (tValue_mpZeroed s prepared (p + 2))
  · exact CarryResult.monproMem_represents s mem 2048 2048 p 2048 a a mm hn32
...
theorem sqMem_readWord_outside (s : State) (mem : ByteArray) (n addr : Nat)
  ...
  unfold sqMem
  split
  · rw [csResultMemory_readWord_outside _ n 2048 addr hn hsubb hdst,
      readWord_sqRowsCarry _ n addr hn32 hscratch n le_rfl,
      readWord_mpZeroed s _ n addr hn32 hscratch,
      read_inputMemory_outside mem 2048 n addr hscratch]
  · exact CarryResult.monproMem_readWord_outside s mem 2048 2048 n 2048 addr hn hn32
      hsubb hscratch hdst

/-- Everything at or above `9280` survives a `SQUARE` call. -/
theorem sqMem_readWord_high (s : State) (mem : ByteArray) (n addr : Nat)
    (hn : 1 ≤ n) (hn32 : n ≤ 32) (haddr : 9280 ≤ addr) :
...
theorem sqMem_frame (s : State) (mem : ByteArray) (n : Nat) (hn : 1 ≤ n) (hn32 : n ≤ 32) :
    MachineState.readWord (sqMem s mem n) 9344 = MachineState.readWord mem 9344 ∧
      MachineState.readWord (sqMem s mem n) 9376 = MachineState.readWord mem 9376 ∧
      MachineState.readWord (sqMem s mem n) 9408 = MachineState.readWord mem 9408 ∧
      MachineState.readWord (sqMem s mem n) 9440 = MachineState.readWord mem 9440 ∧
      MachineState.readWord (sqMem s mem n) 9472 = MachineState.readWord mem 9472 :=
  ⟨sqMem_readWord_high s mem n 9344 hn hn32 (by omega),
   sqMem_readWord_high s mem n 9376 hn hn32 (by omega),
   sqMem_readWord_high s mem n 9408 hn hn32 (by omega),
   sqMem_readWord_high s mem n 9440 hn hn32 (by omega),
   sqMem_readWord_high s mem n 9472 hn hn32 (by omega)⟩
...
theorem Snapshot.of_far {mem mem' : ByteArray} {pa n : Nat} (h : Snapshot mem pa n)
    (hn : n ≤ 8) (hpa : pa + 32 * n ≤ 8192)
    (hpres : ∀ addr, (addr + 32 ≤ 8192 ∨ 8256 + 32 * n ≤ addr) →
      MachineState.readWord mem' addr = MachineState.readWord mem addr) :
    Snapshot mem' pa n := by
  intro k hk
  rw [hpres (8960 + 32 * k) (Or.inr (by omega)), hpres (pa + 32 * k) (Or.inl (by omega))]
  exact h k hk

theorem Snapshot.sqPro {mem : ByteArray} {n : Nat} (h : Snapshot mem 2048 n) (i : Nat)
    (tb : UInt256) (hi : i < n) (hn : n ≤ 8) :
    Snapshot (SquareModel.sqPro mem n i tb).memory 2048 n :=
  h.of_far hn (by omega) (fun addr haddr => readWord_sqPro mem n i addr tb hi (by omega))

theorem Snapshot.l1Run {q : MacState} {n : Nat} (h : Snapshot q.memory 2048 n) (bi : UInt256)
    (j0 k : Nat) (hk : j0 + k ≤ n) (hn : n ≤ 8) :
    Snapshot (SquareModel.l1Run q bi 2048 n j0 k).memory 2048 n :=
  h.of_far hn (by omega) (fun addr haddr => readWord_l1Run q bi 2048 n j0 addr (by omega) k hk)

theorem Snapshot.sqL1 {mem : ByteArray} {n : Nat} (h : Snapshot mem 2048 n) (i : Nat)
    (tb : UInt256) (hi : i < n) (hn : n ≤ 8) :
    Snapshot (SquareModel.sqL1 mem n i tb).memory 2048 n :=
  h.of_far hn (by omega) (fun addr haddr => readWord_sqL1 mem n i addr tb hi (by omega))

theorem Snapshot.sqRowCarry {mem : ByteArray} {n : Nat} (h : Snapshot mem 2048 n) (i : Nat)
    (tb : UInt256) (hi : i < n) (hn : n ≤ 8) :
    Snapshot (SquareResult.sqRowCarry mem n i tb) 2048 n :=
  h.of_far hn (by omega) (fun addr haddr => readWord_sqRowCarry_far mem n i addr tb hi haddr)

theorem Snapshot.sqRowsCarry {mem : ByteArray} {n : Nat} (h : Snapshot mem 2048 n) (i : Nat)
    (hi : i ≤ n) (hn : n ≤ 8) :
    Snapshot (SquareResult.sqRowsCarry mem n i) 2048 n :=
