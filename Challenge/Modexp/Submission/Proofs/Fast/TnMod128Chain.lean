...
theorem run_prefix (s : State) (pc : Nat) (mem : ByteArray) (bi mu c0 : UInt256)
    ...
    some (TnCacheL2Trace.state s (UInt256.ofNat (pc+146)) mem bi mu c0 8 4
  ...
  have h2 := TnCacheL2Trace.run_step 1 s (UInt256.ofNat (pc+45)) mem bi mu c0 8 1
    ...
    pbi hd pb ent tn m128 inv (m0 :: tl :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest)
    hrest hact (by decide) (by decide) (by decide)
  have h3 := TnMod128Trace.run_step s (UInt256.ofNat (pc+80)) mem bi mu c0
    pbi hd pb ent tn m128 inv (m0 :: tl :: m96 :: m64 :: m32 :: aEnd :: dst :: ret :: rest)
    hrest hact hcache
  have h4 := TnCacheExtraTrace.run_extraStep 0 s (UInt256.ofNat (pc+113)) mem bi mu c0 8 3
...
def fullSize (n : Nat) : Nat := if n = 4 then 100 else 246
...
theorem run_full (s : State) (pc : Nat) (mem : ByteArray) (bi mu c0 : UInt256)
  ...
  · have hp := run_prefix s pc mem bi mu c0 pbi hd pb ent tn m128 tl inv m0 aEnd m96 m64 m32 dst ret
    ...
    have hs := TnCacheL2Chain.run_suffix s (pc+146) mem bi mu c0 8 4 pbi hd pb ent tn m128 tl inv m0 aEnd m96 m64 m32 dst ret
      rest hcap hact (by decide) (by decide) hc
