...
namespace Challenge.Modexp.Submission.Proofs.Fast.CiosReadonly.EntryPrefix
...
def loadProgram : List Instr :=
  [.op .JUMPDEST,
   ...
   .push 1 128, .op .EQ, .push 1 153, .op .MUL]
...
def displacement (mem : ByteArray) : UInt256 :=
  UInt256.ofNat 153 * UInt256.eq (UInt256.ofNat 128) (MachineState.readWord mem 9344)
...
theorem run_reads (s : State) (pa pb dst ret : UInt256) (rest : List UInt256)
    ...
    runInstructions readsProgram
      {s with pc := UInt256.ofNat 4163, stack := [pa, pb, dst, ret] ++ rest} =
    some {s with
        pc := UInt256.ofNat 4192
...
theorem run_shuffle (s : State) (pa pb dst ret value inverse aEnd tailPointer low96 low64 low32 delta : UInt256)
    ...
    some {s with
        pc := UInt256.ofNat 4222
        stack := [pa, pb, UInt256.ofNat 4281 + delta, negative32, allOnes,
        UInt256.ofNat 4580 + delta, inverse, value, tailPointer, low96, low64, low32, aEnd, dst, ret] ++ rest} := by
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  have hc7 : rest.length + 7 < 1024 := by omega
...
end Challenge.Modexp.Submission.Proofs.Fast.CiosReadonly.EntryPrefix
