...
theorem run_push (input : ByteArray) (condition : UInt256) :
  ...
  have hop : Challenge.EvmProof.Stepper.runLocated branchPushLocated
    ...
    change (if (conditionState input 4682 condition).pc.toNat =
        Artifact.submissionArtifact.instructionPC 2119 then
        Challenge.EvmProof.Stepper.runInstr (.push 2 (UInt256.ofNat 4690))
          (conditionState input 4682 condition) else none) = _
    rw [if_pos hpc]
    have hcap : (conditionState input 4682 condition).stack.length < 1024 := by
      simp [conditionState]
    have hwidth : ¬ (2 : Fin 33).val = 0 := by decide
    have hthree : (2 : Fin 33).val + 1 = 3 := by decide
    have hadd : UInt256.ofNat 4682 + UInt256.ofNat 3 = UInt256.ofNat 4685 :=
