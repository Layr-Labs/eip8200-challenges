...
/-- The CSUB of one round: from `[512, ret]` above any stack `tail` to `ret` with the
round's memory. -/
...
def gasSteps_roundLast (s : State) (M : ByteArray) (p a mm : Nat)
  ...
  have g4 := gasSteps_nxExit s (countMem (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) 0)
    (p + 2) (UInt256.ofNat (ptrAt (512 + 32 * (p + 2) - 32) (p + 2)))
    (UInt256.ofNat (sqEnt (p + 2) (p + 2))) tl inv m0 m96 m64 m32
    (sqLast (rowZero s M (p + 2)) (p + 2)) (UInt256.ofNat 512) (UInt256.ofNat 3231) rest
    (by omega) hrun hcode hfork hnp
...
`k` Montgomery squares of the block at 512 and returns to `after_sq` (3243). -/
def gasSteps_squareLoop (s : State) (mem : ByteArray) (p a mm k : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (Cios2Dispatch.commonState s mem 4788 512 512 (UInt256.ofNat 512) ret rest)
      { s with pc := UInt256.ofNat 3231, stack := rest,
  ...
  have g1 := Cios2Dispatch.gasSteps_commonSetupInput s mem (UInt256.ofNat 4788) 512 512 (p + 2)
