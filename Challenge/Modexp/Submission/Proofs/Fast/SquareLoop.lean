...
`after_sq` (3278) and the call leaves through the ordinary exit.
...
frame drop and the CSUB, returning to `after_sq` (3278). -/
def gasSteps_roundLast (s : State) (M : ByteArray) (p a mm : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (rowState s a0 (rowZero s M (p + 2)) (p + 2) tl inv m0 m96 m64 m32
        (UInt256.ofNat 2048) ret rest 0)
      { s with pc := UInt256.ofNat 3266, stack := rest,
  ...
  have g4 := gasSteps_nxExit s (countMem (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) 0)
    (p + 2) (UInt256.ofNat (ptrAt (2048 + 32 * (p + 2) - 32) (p + 2)))
    (UInt256.ofNat (sqEnt (p + 2) (p + 2))) tl inv m0 m96 m64 m32
    (sqLast (rowZero s M (p + 2)) (p + 2)) (UInt256.ofNat 2048) (UInt256.ofNat 3266) rest
    (by omega) hrun hcode hfork hnp
  have hjumpAfter : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 3266).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt (by decide)]
    exact jumpDest3272
  have g5 := gasSteps_csubRound s M p 0 a mm (UInt256.ofNat 3266) rest tl inv m0 m96 m64 m32
...
`k` Montgomery squares of the block at 2048 and returns to `after_sq` (3278). -/
def gasSteps_squareLoop (s : State) (mem : ByteArray) (p a mm k : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (Cios2Dispatch.commonState s mem 4902 2048 2048 (UInt256.ofNat 2048) ret rest)
      { s with pc := UInt256.ofNat 3266, stack := rest,
  ...
  have g1 := Cios2Dispatch.gasSteps_commonSetupInput s mem (UInt256.ofNat 4902) 2048 2048 (p + 2)
