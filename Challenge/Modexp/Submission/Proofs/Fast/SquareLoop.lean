...
For `n ∈ {4, 8}` the caller stores the number of squares in memory word 2720 and enters the
...
staged at 2400 and the accumulator zeroed. -/
...
theorem read_prefix (s : State) (M : ByteArray) (n c addr : Nat) (hn32 : n ≤ 32)
    (hfast : n = 4 ∨ n = 8) (hd : addr + 32 ≤ 2048 ∨ 2752 ≤ addr) :
...
theorem read_rowZero (s : State) (M : ByteArray) (n addr : Nat) (hn32 : n ≤ 32)
    (hfast : n = 4 ∨ n = 8) (hd : addr + 32 ≤ 2048 ∨ 2720 ≤ addr) :
...
structure Entry (s : State) (mem : ByteArray) (p a mm : Nat)
  ...
  s32 : MachineState.readWord mem 2784 = UInt256.ofNat (32 * (p + 2))
  tlw : MachineState.readWord mem 2880 = tl
  tlv : tl = UInt256.ofNat (2080 + 32 * (p + 2))
  ml : MachineState.readWord mem 2848 = UInt256.ofNat (32 * (p + 2) - 32)
  invw : MachineState.readWord mem 2816 = inv
...
theorem minv' (h : Entry s mem p a mm tl inv m0 m96 m64 m32) :
    ((MachineState.readWord mem (32 * (p + 2) - 32)).toNat *
      (MachineState.readWord mem 2816).toNat + 1) % 2 ^ 256 = 0 := by
...
theorem cache (h : Entry s mem p a mm tl inv m0 m96 m64 m32) (hn32 : p + 2 ≤ 32)
  ...
  ⟨h.tlv, by
    rw [read_rowZero s mem (p + 2) 2816 hn32 hfast (Or.inr (by decide)), h.invw], by
...
theorem inverse (h : Entry s mem p a mm tl inv m0 m96 m64 m32) (hn32 : p + 2 ≤ 32)
  ...
  rw [read_rowZero s mem (p + 2) (32 * (p + 2) - 32) hn32 hfast (Or.inl (by omega)),
    read_rowZero s mem (p + 2) 2816 hn32 hfast (Or.inr (by decide))]
  exact h.minv'
...
theorem round (h : Entry s mem p a mm tl inv m0 m96 m64 m32) (c : Nat) (hn32 : p + 2 ≤ 32)
  ...
  s32 := by
    ...
    exact h.s32
  tlw := by
    rw [SquareLoopMem.sqRound_readWord_high s mem (p + 2) c 2880 (by omega) hn32 hfast (by omega)]
    exact h.tlw
  tlv := h.tlv
  ml := by
    rw [SquareLoopMem.sqRound_readWord_high s mem (p + 2) c 2848 (by omega) hn32 hfast (by omega)]
    exact h.ml
  invw := by
    rw [SquareLoopMem.sqRound_readWord_high s mem (p + 2) c 2816 (by omega) hn32 hfast (by omega)]
    exact h.invw
  m0w := by
...
def gasSteps_csubRound (s : State) (M : ByteArray) (p c a mm : Nat)
  ...
  (gasSteps_csubAt s (countMem (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) c) (p + 2)
    ...
    (by rw [read_prefix s M (p + 2) c 2848 hn32 hfast (Or.inr (by decide))]; exact h.ml)
    (by
      rw [read_prefix s M (p + 2) c 2880 hn32 hfast (Or.inr (by decide)), h.tlw]
      exact h.tlv)
    (by
      rw [Csub.csStep_readWord_disjoint _ (p + 2) 2784 (by omega) (Or.inr (by omega)) (p + 2)
        le_rfl, read_prefix s M (p + 2) c 2784 hn32 hfast (Or.inr (by decide))]
      exact h.s32)
    (by
...
def gasSteps_roundMore (s : State) (M : ByteArray) (p c a mm : Nat)
    ...
    (hcount : MachineState.readWord M 2720 = UInt256.ofNat (c + 1))
    (h : Entry s M p a mm tl inv m0 m96 m64 m32) :
    Challenge.EvmProof.GasSteps
  ...
  have hcountRows : MachineState.readWord
      (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) 2720 = UInt256.ofNat (c + 1) := by
    rw [readWord_sqRowsCarry _ (p + 2) 2720 hn32 (Or.inr (by omega)) (p + 2) le_rfl,
      read_rowZero s M (p + 2) 2720 hn32 hfast (Or.inr (by omega))]
    exact hcount
  have hs32' : MachineState.readWord (SquareLoopMem.sqRound s (p + 2) c M) 2784 =
      UInt256.ofNat (32 * (p + 2)) := by
    rw [SquareLoopMem.sqRound_readWord_high s M (p + 2) c 2784 (by omega) hn32 hfast (by omega)]
    exact h.s32
...
def gasSteps_roundLast (s : State) (M : ByteArray) (p a mm : Nat)
    ...
    (hfast : p + 2 = 4 ∨ p + 2 = 8)
    (ha0 : UInt256.sgt (UInt256.ofNat 0) a0 = UInt256.ofNat 0)
    (hcount : MachineState.readWord M 2720 = UInt256.ofNat 1)
    (h : Entry s M p a mm tl inv m0 m96 m64 m32) :
    Challenge.EvmProof.GasSteps
      (rowState s a0 (rowZero s M (p + 2)) (p + 2) tl inv m0 m96 m64 m32
        (UInt256.ofNat 512) ret rest 0)
  ...
  have hcountRows : MachineState.readWord
      (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) 2720 = UInt256.ofNat 1 := by
    rw [readWord_sqRowsCarry _ (p + 2) 2720 hn32 (Or.inr (by omega)) (p + 2) le_rfl,
      read_rowZero s M (p + 2) 2720 hn32 hfast (Or.inr (by omega))]
    exact hcount
  have g1 := gasSteps_rowsToExit s a0 (rowZero s M (p + 2)) (p + 2) tl inv m0 m96 m64 m32
...
def gasSteps_loop (s : State) (p mm : Nat) (tl inv m0 m96 m64 m32 ret : UInt256)
    ...
    ∀ (k : Nat) (M : ByteArray) (a : Nat) (a0 : UInt256), 1 ≤ k → k ≤ 16 →
      ...
      MachineState.readWord M 2720 = UInt256.ofNat k →
...
`common` block with the row head `sq_row` and the counter `k` in memory word 2720, it runs
...
def gasSteps_squareLoop (s : State) (mem : ByteArray) (p a mm k : Nat)
    ...
    (hcount : MachineState.readWord mem 2720 = UInt256.ofNat k)
    (hcds : s.executionEnv.calldata.size < 2 ^ 256)
  ...
  have hentry : Entry s mem p a mm (MachineState.readWord mem 2880)
      (MachineState.readWord mem 2816) (MachineState.readWord mem (32 * (p + 2) - 32))
  ...
  exact g1.trans (gasSteps_loop s p mm (MachineState.readWord mem 2880)
    (MachineState.readWord mem 2816) (MachineState.readWord mem (32 * (p + 2) - 32))
    (MachineState.readWord mem 96) (MachineState.readWord mem 64)
