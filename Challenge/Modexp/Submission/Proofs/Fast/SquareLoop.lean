...
For `n ∈ {4, 8}` the caller stores the number of squares in memory word 5184 and enters the
...
staged at 4864 and the accumulator zeroed. -/
...
theorem read_prefix (s : State) (M : ByteArray) (n c addr : Nat) (hn32 : n ≤ 32)
    (_hfast : n = 4 ∨ n = 8) (hd : addr + 32 ≤ 4096 ∨ 5216 ≤ addr) :
...
theorem read_rowZero (s : State) (M : ByteArray) (n addr : Nat) (hn32 : n ≤ 32)
    (_hfast : n = 4 ∨ n = 8) (hd : addr + 32 ≤ 4096 ∨ 5184 ≤ addr) :
...
structure Entry (s : State) (mem : ByteArray) (p a mm : Nat)
  ...
  s32 : MachineState.readWord mem 5248 = UInt256.ofNat (32 * (p + 2))
  tlw : MachineState.readWord mem 5344 = tl
  tlv : tl = UInt256.ofNat (4128 + 32 * (p + 2))
  ml : MachineState.readWord mem 5312 = UInt256.ofNat (32 * (p + 2) - 32)
  invw : MachineState.readWord mem 5280 = inv
  ...
  arep : Model.FastRepresents mem 4864 (p + 2) a
...
theorem minv' (h : Entry s mem p a mm tl inv m0 m96 m64 m32) :
    ((MachineState.readWord mem (32 * (p + 2) - 32)).toNat *
      (MachineState.readWord mem 5280).toNat + 1) % 2 ^ 256 = 0 := by
...
theorem cache (h : Entry s mem p a mm tl inv m0 m96 m64 m32) (hn32 : p + 2 ≤ 32)
  ...
  ⟨h.tlv, by
    rw [read_rowZero s mem (p + 2) 5280 hn32 hfast (Or.inr (by decide)), h.invw], by
...
theorem inverse (h : Entry s mem p a mm tl inv m0 m96 m64 m32) (hn32 : p + 2 ≤ 32)
  ...
  rw [read_rowZero s mem (p + 2) (32 * (p + 2) - 32) hn32 hfast (Or.inl (by omega)),
    read_rowZero s mem (p + 2) 5280 hn32 hfast (Or.inr (by decide))]
  exact h.minv'

theorem snapshot (s : State) (mem : ByteArray) (p : Nat)
    (_hfast : p + 2 = 4 ∨ p + 2 = 8) :
    StagedOperand.Snapshot (rowZero s mem (p + 2)) 4864 (p + 2) := by
  intro k _; rfl

/-- Every round re-establishes the invariant, with the operand squared. -/
theorem round (h : Entry s mem p a mm tl inv m0 m96 m64 m32) (c : Nat) (hc : 0 < c) (hn32 : p + 2 ≤ 32)
  ...
  s32 := by
    rw [SquareLoopMem.sqRound_readWord_high s mem (p + 2) c 5248 (by omega) hn32 hfast (by omega)]
    exact h.s32
  tlw := by
    rw [SquareLoopMem.sqRound_readWord_high s mem (p + 2) c 5344 (by omega) hn32 hfast (by omega)]
    exact h.tlw
  tlv := h.tlv
  ml := by
    rw [SquareLoopMem.sqRound_readWord_high s mem (p + 2) c 5312 (by omega) hn32 hfast (by omega)]
    exact h.ml
  invw := by
    rw [SquareLoopMem.sqRound_readWord_high s mem (p + 2) c 5280 (by omega) hn32 hfast (by omega)]
    exact h.invw
  m0w := by
  ...
  arep := by
    ...
    simpa only [SquareLoopMem.roundDst, if_neg (by omega : c ≠ 0)] using hrep
...
def gasSteps_csubRound (s : State) (M : ByteArray) (p c a mm : Nat)
  ...
  (gasSteps_csubAt s (countMem (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) c) (p + 2) (SquareLoopMem.roundDst c)
    ret tail hcap hrun hcode hfork hnp hact (by omega) hn32 (by unfold SquareLoopMem.roundDst; split <;> omega) hjump
    (by rw [read_prefix s M (p + 2) c 5312 hn32 hfast (Or.inr (by decide))]; exact h.ml)
    (by
      rw [read_prefix s M (p + 2) c 5344 hn32 hfast (Or.inr (by decide)), h.tlw]
      exact h.tlv)
    (by
      rw [Csub.csStep_readWord_disjoint _ (p + 2) 5248 (by omega) (Or.inr (by omega)) (p + 2)
        le_rfl, read_prefix s M (p + 2) c 5248 hn32 hfast (Or.inr (by decide))]
      exact h.s32)
    (by
      rw [Csub.csStep_readWord_disjoint _ (p + 2) 4128 (by omega) (Or.inr (by omega)) (p + 2)
          le_rfl,
        readWord_countMem_disjoint _ c 4128 (Or.inl (by omega))]
...
def gasSteps_roundMore (s : State) (M : ByteArray) (p c a mm : Nat)
    ...
    (hcount : MachineState.readWord M 5184 = UInt256.ofNat (c + 1))
    (h : Entry s M p a mm tl inv m0 m96 m64 m32) :
    Challenge.EvmProof.GasSteps
      (rowState s a0 (rowZero s M (p + 2)) (p + 2) tl inv m0 m96 m64 m32
        (UInt256.ofNat 512) ret rest 0)
      (rowState s (UInt256.ofNat 0) (rowZero s (SquareLoopMem.sqRound s (p + 2) c M) (p + 2)) (p + 2)
        tl inv m0 m96 m64 m32 (UInt256.ofNat 512) ret rest 0) := by
  have hcountRows : MachineState.readWord
      (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) 5184 = UInt256.ofNat (c + 1) := by
    rw [readWord_sqRowsCarry _ (p + 2) 5184 hn32 (Or.inr (by omega)) (p + 2) le_rfl,
      read_rowZero s M (p + 2) 5184 hn32 hfast (Or.inr (by omega))]
    exact hcount
  have hs32' : MachineState.readWord (SquareLoopMem.sqRound s (p + 2) c M) 5248 =
      UInt256.ofNat (32 * (p + 2)) := by
    rw [SquareLoopMem.sqRound_readWord_high s M (p + 2) c 5248 (by omega) hn32 hfast (by omega)]
    exact h.s32
  have g1 := gasSteps_rowsToExit s a0 (rowZero s M (p + 2)) (p + 2) tl inv m0 m96 m64 m32
    (UInt256.ofNat 512) ret rest (by omega) hrun hcode hfork hnp hact hfast ha0
  ...
  have g2 := gasSteps_sqExitMore s (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) (p + 2) c
    (UInt256.ofNat (ptrAt (4864 + 32 * (p + 2) - 32) (p + 2)))
  ...
  have g3 := gasSteps_more s (countMem (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) c)
    (p + 2) (UInt256.ofNat (ptrAt (4864 + 32 * (p + 2) - 32) (p + 2)))
    (UInt256.ofNat (sqEnt (p + 2) (p + 2))) tl inv m0 m96 m64 m32
    (sqLast (rowZero s M (p + 2)) (p + 2)) (UInt256.ofNat 512) ret rest
  ...
  have g4 := gasSteps_csubRound s M p c a mm (UInt256.ofNat pcAgain)
    (frameStack (p + 2) (UInt256.ofNat (ptrAt (4864 + 32 * (p + 2) - 32) (p + 2)))
      (UInt256.ofNat (sqEnt (p + 2) (p + 2))) tl inv m0 m96 m64 m32
      (sqLast (rowZero s M (p + 2)) (p + 2)) (UInt256.ofNat 512) ret rest)
  ...
  simp only [SquareLoopMem.roundDst, if_neg (by omega : c ≠ 0)] at g4
...
def gasSteps_roundLast (s : State) (M : ByteArray) (p a mm : Nat)
    ...
    (hfast : p + 2 = 4 ∨ p + 2 = 8)
    (ha0 : UInt256.sgt (UInt256.ofNat 0) a0 = UInt256.ofNat 0)
    (hcount : MachineState.readWord M 5184 = UInt256.ofNat 1)
    (h : Entry s M p a mm tl inv m0 m96 m64 m32) :
    Challenge.EvmProof.GasSteps
      (rowState s a0 (rowZero s M (p + 2)) (p + 2) tl inv m0 m96 m64 m32
        (UInt256.ofNat 512) ret rest 0)
      { s with pc := UInt256.ofNat 3352, stack := rest,
               memory := SquareLoopMem.sqRound s (p + 2) 0 M } := by
  have hcountRows : MachineState.readWord
      (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) 5184 = UInt256.ofNat 1 := by
    rw [readWord_sqRowsCarry _ (p + 2) 5184 hn32 (Or.inr (by omega)) (p + 2) le_rfl,
      read_rowZero s M (p + 2) 5184 hn32 hfast (Or.inr (by omega))]
    exact hcount
  have g1 := gasSteps_rowsToExit s a0 (rowZero s M (p + 2)) (p + 2) tl inv m0 m96 m64 m32
  ...
  have g4 := gasSteps_nxExit s (countMem (sqRowsCarry (rowZero s M (p + 2)) (p + 2) (p + 2)) 0)
    (p + 2) (UInt256.ofNat (ptrAt (4864 + 32 * (p + 2) - 32) (p + 2)))
    (UInt256.ofNat (sqEnt (p + 2) (p + 2))) tl inv m0 m96 m64 m32
    (sqLast (rowZero s M (p + 2)) (p + 2)) (UInt256.ofNat 512) (UInt256.ofNat 3352) rest
    (by omega) hrun hcode hfork hnp
  have hjumpAfter : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 3352).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt (by decide)]
    exact jumpDest3272
  have g5 := gasSteps_csubRound s M p 0 a mm (UInt256.ofNat 3352) rest tl inv m0 m96 m64 m32
...
def gasSteps_loop (s : State) (p mm : Nat) (tl inv m0 m96 m64 m32 ret : UInt256)
    ...
    ∀ (k : Nat) (M : ByteArray) (a : Nat) (a0 : UInt256), 1 ≤ k → k ≤ 16 →
      ...
      MachineState.readWord M 5184 = UInt256.ofNat k →
      Entry s M p a mm tl inv m0 m96 m64 m32 →
      Challenge.EvmProof.GasSteps
        (rowState s a0 (rowZero s M (p + 2)) (p + 2) tl inv m0 m96 m64 m32
          (UInt256.ofNat 512) ret rest 0)
        { s with pc := UInt256.ofNat 3352, stack := rest,
                 memory := SquareLoopMem.sqRunMem s (p + 2) k M } := by
  intro k
  induction k with
      ...
      | (i + 1), ih =>
          exact (gasSteps_roundMore s M p (i + 1) a mm a0 tl inv m0 m96 m64 m32 ret rest hcap
            ...
            (ih (SquareLoopMem.sqRound s (p + 2) (i + 1) M)
              ...
              (h.round (i + 1) (by omega) hn32 hfast hodd hmpos))
...
`common` block with the row head `sq_row` and the counter `k` in memory word 5184, it runs
...
def gasSteps_squareLoop (s : State) (mem : ByteArray) (p a mm k : Nat)
    ...
    (hcount : MachineState.readWord mem 5184 = UInt256.ofNat k)
    (hcds : s.executionEnv.calldata.size < 2 ^ 256)
  ...
  have hs (addr : Nat) (hd : addr + 32 ≤ 4096 ∨ 5184 ≤ addr) :
      MachineState.readWord (stage mem 512 (p + 2)) addr = MachineState.readWord mem addr :=
    read_stage_outside mem 512 (p + 2) addr (by omega)
  have hentry : Entry s (stage mem 512 (p + 2)) p a mm (MachineState.readWord mem 5344)
      (MachineState.readWord mem 5280) (MachineState.readWord mem (32 * (p + 2) - 32))
      (MachineState.readWord mem 96) (MachineState.readWord mem 64)
      (MachineState.readWord mem 32) := by
    refine ⟨(hs 5248 (Or.inr (by decide))).trans hs32, hs 5344 (Or.inr (by decide)), htl,
      (hs 5312 (Or.inr (by decide))).trans hml, hs 5280 (Or.inr (by decide)),
  ...
  have g1 := Cios2Dispatch.gasSteps_commonSetupInput s mem (UInt256.ofNat 5294) 512 512 (p + 2)
  ...
  have g2 := SquareStagedEntry.gasSteps_entry s (rowZero s (stage mem 512 (p + 2)) (p + 2)) (p + 2)
    (UInt256.ofNat (sqEnt (p + 2) 0)) (MachineState.readWord mem 5280)
    (MachineState.readWord mem (32 * (p + 2) - 32))
    (MachineState.readWord mem 5344 :: MachineState.readWord mem 96 :: MachineState.readWord mem 64 ::
  ...
  have hcall : sqLoopMem s (p + 2) k mem =
      SquareLoopMem.sqRunMem s (p + 2) k (stage mem 512 (p + 2)) := by
    cases k with
    | zero => omega
    | succ k => rfl
  rw [hcall]
  exact (g1.trans g2).trans (gasSteps_loop s p mm (MachineState.readWord mem 5344)
    (MachineState.readWord mem 5280) (MachineState.readWord mem (32 * (p + 2) - 32))
    (MachineState.readWord mem 96) (MachineState.readWord mem 64)
    (MachineState.readWord mem 32) ret rest hcap hrun hcode hfork hnp hact hn32 hfast hcds
    hodd hmpos k (stage mem 512 (p + 2)) a (UInt256.ofNat (512 + 32 * (p + 2) - 32)) hk hk16
    (sgt_zero_setup (p + 2) (by omega)) ((hs 5184 (Or.inr (by decide))).trans hcount) hentry)
