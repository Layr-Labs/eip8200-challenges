...
def gasSteps_roundMore (s : State) (M : ByteArray) (p c a mm : Nat)
  ...
  exact (((g1.trans g2).trans g4).trans g5)
...
def gasSteps_roundLast (s : State) (M : ByteArray) (p a mm : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (rowState s a0 (rowZero s M (p + 2)) (p + 2) tl inv m0 m96 m64 m32
        (UInt256.ofNat 512) ret rest 0)
      { s with pc := UInt256.ofNat 1696, stack := rest,
  ...
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (UInt256.ofNat 4201).toNat = true := by
    change Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 4201 = true
    exact Artifact.isValidJumpDest_index 3165 (by rfl)
  have g4 := gasSteps_csubRound s M p 0 a mm (UInt256.ofNat 4201) retained tl inv m0 m96 m64 m32
...
def gasSteps_squareLoop (s : State) (mem : ByteArray) (p a mm k : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (Cios2Dispatch.commonState s mem 5283 512 512 (UInt256.ofNat 512) ret rest)
      { s with pc := UInt256.ofNat 1696, stack := rest,
  ...
  have g1 := Cios2Dispatch.gasSteps_commonSetupInput s mem (UInt256.ofNat 5283) 512 512 (p + 2)
  ...
  change Challenge.EvmProof.GasSteps _
    { s with
      pc := UInt256.ofNat 1696
      stack := rest
      memory := StagedProduct.memory s (sqLoopMem s (p + 2) k mem) (p + 2) }
  rw [hcall]
  exact (g1.trans g2).trans (gasSteps_loop s p mm (MachineState.readWord mem 2784)
    (MachineState.readWord mem 2720) (MachineState.readWord mem (32 * (p + 2) - 32))
    (MachineState.readWord mem 96) (MachineState.readWord mem 64)
