...
* `n ∈ {4, 8}`: the call does **not** return to `ret` on the R0 artifact — the kernel keeps
  its frame, loops over the counter in memory word 2720 and leaves through `after_sq`
...
def gasSteps_squareFull (s : State) (mem : ByteArray) (p a mm : Nat)
  ...
  have g1 := Cios2Dispatch.gasSteps_commonFallbackOfWidth s mem 4788 512 512 (p + 2)
