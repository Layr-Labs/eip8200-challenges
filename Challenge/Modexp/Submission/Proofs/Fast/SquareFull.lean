...
The fixed-exponent caller enters the kernel's `common` block (pc 4149) with the row
...
* other widths: `common` falls back (`POP PUSH2 0x683 JUMP`) to the generic `MONPRO`
  at 1749 (`Cios2Dispatch.gasSteps_commonFallbackOfWidth`, `Monpro.gasSteps_monproCsub`),
...
def gasSteps_squareFull (s : State) (mem : ByteArray) (p a mm : Nat)
  ...
  have g1 := Cios2Dispatch.gasSteps_commonFallbackOfWidth s mem 4984 512 512 (p + 2)
