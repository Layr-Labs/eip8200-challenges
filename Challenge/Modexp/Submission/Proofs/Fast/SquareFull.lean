...
The fixed-exponent caller enters the kernel's `common` block (pc 4163) with the row
...
def gasSteps_squareFull (s : State) (mem : ByteArray) (p a mm : Nat)
  ...
  have g1 := Cios2Dispatch.gasSteps_commonFallbackOfWidth s mem 4995 512 512 (p + 2)
