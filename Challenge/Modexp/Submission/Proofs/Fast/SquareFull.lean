...
The fixed-exponent caller enters the kernel's `common` block (pc 4067) with the row
head `hd = sq_row = 4878` on top of the call frame `[2048, 2048, 2048, ret]`
(`Cios2Dispatch.commonState s mem 4878 2048 2048 (ofNat 2048) ret rest`, definitionally
`Exp.sqCall`).
...
* `n ∈ {4, 8}`: the call does **not** return to `ret` on the R0 artifact — the kernel keeps
  ...
  (3278).  That path is `SquareLoop.gasSteps_squareLoop`; this lemma therefore carries
  `hslow : ¬(n = 4 ∨ n = 8)`.
...
def gasSteps_squareFull (s : State) (mem : ByteArray) (p a mm : Nat)
  ...
  have g1 := Cios2Dispatch.gasSteps_commonFallbackOfWidth s mem 4902 2048 2048 (p + 2)
