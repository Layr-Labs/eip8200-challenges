...
The fixed-exponent caller enters the kernel's `common` block (pc 4097) with the row
...
def gasSteps_squareFull (s : State) (mem : ByteArray) (p a mm : Nat)
  ...
  have g1 := Cios2Dispatch.gasSteps_commonFallbackEligible s mem 4785 512 512 (p + 2)
