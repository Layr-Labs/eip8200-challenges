...
/-!
# `SQUARE(512) → 512` through the sqCP1m kernel

The fixed-exponent caller enters the kernel's `common` block (pc 3924) with the row
head `hd = sq_row = 4710` on top of the call frame `[512, 512, 512, ret]`
(`Cios2Dispatch.commonState s mem 4710 512 512 (ofNat 512) ret rest`, definitionally
WP-C's `Exp.sqCall`).
...
* `n ∈ {4, 8}`: the width check passes and the 3-slot `setup` stages the operand and
  ...
  `M0 = mpZeroed s (inputMemory mem 512 n) n`); the `n` square rows run
...
theorem word512_toNat : (UInt256.ofNat 512).toNat = 512 := by decide

/-- **The square subroutine** `SQUARE(512) → 512` of the sqCP1m artifact: from the
...
def gasSteps_squareFull (s : State) (mem : ByteArray) (p a mm : Nat)
    ...
    Challenge.EvmProof.GasSteps
      (Cios2Dispatch.commonState s mem 4710 512 512 (UInt256.ofNat 512) ret rest)
  ...
  · -- the square kernel
    ...
    have hread (addr : Nat) (hd : addr + 32 ≤ 2048 ∨ 2720 ≤ addr) :
        MachineState.readWord (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) addr =
          MachineState.readWord mem addr :=
      (readWord_mpZeroed s _ (p + 2) addr hn32 hd).trans
        (read_inputMemory_outside mem 512 (p + 2) addr hd)
    -- setup: row head 0 of the square rows
    have g1 := Cios2Dispatch.gasSteps_commonSetupInput s mem (UInt256.ofNat 4710) 512 512 (p + 2)
      (UInt256.ofNat 512) ret rest hcap hrun hcode hfork hnp hact hfast (by omega) (by decide)
      (by omega) hcds hs32 hml Cios2Dispatch.jumpDestSqRow'
    rw [l1Target_eq_sqEnt (p + 2) hfast] at g1
    -- the rows
    have hcM : CiosReadonly.ReadonlyCache (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) (p + 2)
        (MachineState.readWord mem 2880) (MachineState.readWord mem 2816)
        (MachineState.readWord mem (32 * (p + 2) - 32)) :=
      ⟨htl, (hread 2816 (Or.inr (by decide))).symm, (hread (32 * (p + 2) - 32) (Or.inl (by omega))).symm⟩
    have heM : CiosReadonlyExtra.ExtraCache (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2))
    ...
    have hminvM : inverseInvariant (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) (p + 2) := by
      unfold inverseInvariant
      rw [hread (32 * (p + 2) - 32) (Or.inl (by omega)), hread 2816 (Or.inr (by decide))]
      exact hminv
    have hin : inputMemory mem 512 (p + 2) = stage mem 512 (p + 2) := by
      unfold inputMemory; rw [if_pos hfast]
    have hsnapM : StagedOperand.Snapshot (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) 512 (p + 2) := by
      rw [hin]; exact (snapshot_stage mem 512 (p + 2) (by omega)).zeroed s hn8 (by omega)
    have g2 := gasSteps_rows s (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) (p + 2)
      (MachineState.readWord mem 2880) (MachineState.readWord mem 2816)
      (MachineState.readWord mem (32 * (p + 2) - 32))
      (MachineState.readWord mem 96) (MachineState.readWord mem 64) (MachineState.readWord mem 32)
      (UInt256.ofNat 512) ret rest hcap hrun hcode hfork hnp hact hfast hminvM hcM heM hsnapM
    -- the final conditional subtraction
    have hrowsRead (addr : Nat) (hd : addr + 32 ≤ 2048 ∨ 2720 ≤ addr) :
        MachineState.readWord (sqRowsCarry (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) (p + 2) (p + 2))
          addr = MachineState.readWord mem addr :=
      (readWord_sqRowsCarry _ (p + 2) addr hn32 hd (p + 2) le_rfl).trans (hread addr hd)
    have htn : (MachineState.readWord (Csub.csStep (sqRowsCarry (mpZeroed s (inputMemory mem 512 (p + 2))
        (p + 2)) (p + 2) (p + 2)) (p + 2) (p + 2)).memory 2080).toNat ≤ 1 := by
      rw [Csub.csStep_readWord_disjoint _ (p + 2) 2080 (by omega) (Or.inr (by omega)) (p + 2) le_rfl]
      have ha0 : Model.FastRepresents (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) 512 (p + 2) a := by
        refine (Model.fastRepresents_congr (a := mem) ?_ a).1 ha
        intro j hj
        rw [hread (512 + 32 * j) (Or.inl (by omega))]
      have hm0 : Model.FastRepresents (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) 0 (p + 2) mm := by
        refine (Model.fastRepresents_congr (a := mem) ?_ mm).1 hm
        intro j hj
        rw [hread (0 + 32 * j) (Or.inl (by omega))]
      have hminv0 : ((MachineState.readWord (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2))
          (32 * (p + 2) - 32)).toNat *
          (MachineState.readWord (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) 2816).toNat + 1) %
            2 ^ 256 = 0 := by
        rw [hread (32 * (p + 2) - 32) (Or.inl (by omega)), hread 2816 (Or.inr (by decide))]
        exact hminv
      exact sqRowsCarry_tn_le_one _ p a mm hn32 ha0 hm0 hminv0 (tValue_mpZeroed s _ (p + 2)) ham
    have g3 := Csub.gasSteps_csub s
      (sqRowsCarry (mpZeroed s (inputMemory mem 512 (p + 2)) (p + 2)) (p + 2) (p + 2)) (p + 2)
      (UInt256.ofNat 512) ret rest (by omega) hcode hfork hrun hnp hact (by omega) hn32 hjump
      ((hrowsRead 2848 (Or.inr (by decide))).trans hml)
      ((hrowsRead 2880 (Or.inr (by decide))).trans htl)
      ((Csub.csStep_readWord_disjoint _ (p + 2) 2784 (by omega) (Or.inr (by omega)) (p + 2) le_rfl).trans
        ((hrowsRead 2784 (Or.inr (by decide))).trans hs32))
      (by rw [Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt (by decide)]; omega)
      htn
    refine (g1.trans (g2.trans g3)).cast rfl ?_
    rw [csReturnedState_eq, sqMem_of_fast s mem (p + 2) hfast, word512_toNat]
  · -- the generic MONPRO fallback
    have g1 := Cios2Dispatch.gasSteps_commonFallbackOfWidth s mem 4710 512 512 (p + 2)
