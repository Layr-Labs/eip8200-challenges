...
theorem row_step (mem : ByteArray) (pa a m i : Nat)
  ...
  have hd' := represents_rows mem pa 2368 9 (2*a) (Or.inr (by decide)) hd i (by omega)
...
theorem final_high_le_one (mem : ByteArray) (pa a m : Nat)
    ...
    (hinv : ((MachineState.readWord mem 224).toNat *
      (MachineState.readWord mem 2816).toNat+1) % 2^256 = 0) :
    (MachineState.readWord (rows mem pa 8) 2080).toNat ≤ 1 := by
...
theorem represents_result (mem : ByteArray) (pa pdst a m : Nat)
  ...
  exact Csub.csub_correct (rows mem pa 8) 8 (Csub.lowValue (rows mem pa 8) 2112 8 8) m
    (MachineState.readWord (rows mem pa 8) 2080).toNat pdst (by decide) (by decide)
    (Csub.fastRepresents_lowValue (rows mem pa 8) 2112 8) hm' rfl htn (by omega) hlt
