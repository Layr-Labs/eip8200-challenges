...
theorem run_entry (s : State) (mem : ByteArray) (pa pb n : Nat)
  ...
  have hskip : runInstructions [.op .JUMPDEST]
    ...
    some (framed { s with memory := mem } (UInt256.ofNat 4199)
  ...
  have hstart : runInstructions [.op .JUMPDEST] (entryState s mem pa pb dst ret rest) =
    some (framed { s with memory := mem } (UInt256.ofNat 4168)
...
theorem run_middle (s : State) (mem : ByteArray) (c bi : UInt256)
    ...
    some (CiosCached.l2At 4582 s (midMem mem c) bi (rowMu mem n) (rowC0 mem n)
...
theorem run_exit (s : State) (pbi paEnd pbEnd flag target2 tl inv m0 aEnd m96 m64 m32 dst ret : UInt256)
    ...
    (htarget : Decode.isValidJumpDest s.executionEnv.code 3815 = true) :
    runInstructions fullExitProgram
      (framed s (UInt256.ofNat 4874)
        ([pbi,paEnd,pbEnd,flag,negative32,allOnes,target2,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest)) =
    some (framed s (UInt256.ofNat 3815) ([dst,ret] ++ rest)) := by
