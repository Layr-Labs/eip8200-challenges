...
/-- Redirected base-chain head, pc3650. -/
def entryState (s : State) (memory : ByteArray)
    (n bsize esize msize : Nat) : State :=
  { s with pc := UInt256.ofNat 3650
           stack := outer n bsize esize msize
           memory := memory }

/-- Guard hit, immediately before the calldata copy, pc3665. -/
def copyState (s : State) (memory : ByteArray)
    (n bsize esize msize : Nat) : State :=
  { s with pc := UInt256.ofNat 3665
           stack := outer n bsize esize msize
           memory := memory }
...
def addCallState (s : State) (memory input : ByteArray)
  ...
  { s with pc := UInt256.ofNat 2511
           stack := [UInt256.ofNat 1024, UInt256.ofNat 3072,
             UInt256.ofNat 1024, UInt256.ofNat 3688] ++
             outer n bsize esize msize
           memory := copyBaseMem memory input n }

/-- Return from ADDMOD, pc3688. The memory argument is its abstract result. -/
def afterAddState (s : State) (memory : ByteArray)
    (n bsize esize msize : Nat) : State :=
  { s with pc := UInt256.ofNat 3688
           stack := outer n bsize esize msize
           memory := memory }
...
/-- Guard miss, pc3705, with the original stack and memory. -/
def fallbackState (s : State) (memory : ByteArray)
    (n bsize esize msize : Nat) : State :=
  { s with pc := UInt256.ofNat 3705
           stack := outer n bsize esize msize
           memory := memory }
