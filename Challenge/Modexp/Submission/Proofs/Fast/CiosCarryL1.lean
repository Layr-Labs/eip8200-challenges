...
namespace Challenge.Modexp.Submission.Proofs.Fast.CiosCarryL1
...
def state (template : State) (pc : UInt256) (mem : ByteArray)
  ...
  { template with
    ...
    stack := [(l1Step mem bi pa n j).carry, bi, pbi, UInt256.ofNat pa,
      pbEnd, flag, carrySlot, allOnes, destination, returnPC] ++ rest
...
def doneState (template : State) (pc : UInt256) (mem : ByteArray)
    ...
    (rest : List UInt256) : State :=
  state (carrySlot := carrySlot) template pc mem bi pa n n pbi pbEnd flag destination returnPC rest
...
theorem run_step (template : State) (pc : UInt256) (mem : ByteArray)
    ...
    some (state (carrySlot := carrySlot) template (pc + UInt256.ofNat 38) mem bi pa n (j+1)
  ...
  have hl := run_load (carrySlot := carrySlot) st pc off (l1Step mem bi pa n j).carry bi pbi (UInt256.ofNat pa)
...
theorem run_first (template : State) (pc : UInt256) (mem : ByteArray)
    ...
    runInstructions (l1FirstProgram off t)
      (state (carrySlot := carrySlot) template pc mem bi pa n 0 pbi pbEnd flag destination returnPC rest) =
    some (state (carrySlot := carrySlot) template (pc + UInt256.ofNat 35) mem bi pa n 1
      pbi pbEnd flag destination returnPC rest) := by
  ...
  have hT : UInt256.ofNat (MachineState.activeWordsAfter st.activeWords.toNat t.toNat 32) =
      st.activeWords := by simpa only [st, ht] using hactT
  have hl := run_load (carrySlot := carrySlot) st pc off (UInt256.ofNat 0) bi pbi (UInt256.ofNat pa)
    pbEnd flag destination returnPC rest hrest hA
...
theorem run_last (template : State) (pc : UInt256) (mem : ByteArray)
    ...
    runInstructions (l1LastProgram t)
      (state (carrySlot := carrySlot) template pc mem bi pa n (n-1) pbi pbEnd flag destination returnPC rest) =
    some (doneState (carrySlot := carrySlot) template (pc + UInt256.ofNat 35) mem bi pa n
      pbi pbEnd flag destination returnPC rest) := by
  ...
  have hT : UInt256.ofNat (MachineState.activeWordsAfter st.activeWords.toNat t.toNat 32) =
      st.activeWords := by simpa only [st, ht] using hactT
  have hl := run_last_load (carrySlot := carrySlot) st pc (l1Step mem bi pa n (n-1)).carry bi pbi
  ...
  have hfinal : runInstructions (l1LastProgram t)
      (state (carrySlot := carrySlot) template pc mem bi pa n (n-1) pbi pbEnd flag destination returnPC rest) =
      some (state (carrySlot := carrySlot) template (pc + UInt256.ofNat 35) mem bi pa n (n-1+1)
        pbi pbEnd flag destination returnPC rest) := by
...
end Challenge.Modexp.Submission.Proofs.Fast.CiosCarryL1
