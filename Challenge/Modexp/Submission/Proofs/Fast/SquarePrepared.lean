...
theorem read_prepared_outside (s : State) (mem : ByteArray) (pa pb n addr : Nat)
    (hn : n ≤ 32) (hd : addr+32 ≤ 8192 ∨ 9312 ≤ addr) :
...
theorem represents_prepared (s : State) (mem : ByteArray) (pa pb n ptr count v : Nat)
    (hn : n ≤ 32) (hd : ptr+32*count ≤ 8192 ∨ 9312 ≤ ptr)
...
theorem represents_double (s : State) (mem : ByteArray) (pa a : Nat)
    ...
    Model.FastRepresents (prepared s mem pa pa 8) 8928 9 (2*a) := by
  ...
  have hs := Csub.fastRepresents_mcopy (selected mem pa pa 8) pa 8960 8 a (by decide) hsel
  have hb : Model.FastRepresents (before mem pa pa 8) 8960 8 a := by
  ...
  exact (Model.fastRepresents_congr (fun j hj =>
    mpZeroed_readWord_outside s (before mem pa pa 8) 8 (8960+32*j) (Or.inr (by omega))) a).2 hb

theorem tValue_zero (s : State) (mem : ByteArray) (pa : Nat) :
    tValue (prepared s mem pa pa 8) 8 = 0 := by
  rw [prepared, if_pos ⟨rfl,rfl⟩, tValue, read_init_outside _ 8224 (Or.inl (by decide)), readWord_mpZeroed_tn]
  have hrep : Model.FastRepresents (initMemory (mpZeroed s (before mem pa pa 8) 8)) 8256 8 0 :=
    (Model.fastRepresents_congr (fun j hj => read_init_outside _ (8256+32*j) (Or.inl (by omega))) 0).2
      (fastRepresents_mpZeroed s (before mem pa pa 8) 8)
  have hlow := Model.fastRepresents_value_unique (Csub.fastRepresents_lowValue _ 8256 8) hrep
  rw [hlow]
...
theorem init_high_clear (mem : ByteArray) :
    SquareWords.clearBit (MachineState.readWord (initMemory mem) 8928) = UInt256.ofNat 0 := by
  have ha := Csub.fastRepresents_lowValue mem 8960 8
  have h := SquareProductGas.last_coefficient_zero (initMemory mem) (UInt256.ofNat 0)
    (Csub.lowValue mem 8960 8 8) (init_represents mem _ ha) ha.1
