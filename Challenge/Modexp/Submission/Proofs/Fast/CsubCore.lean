...
`ADDMOD` starts at PC 2262 and falls through into `CSUB` at PC 2220.
`CSUB` dispatches at PC 5152 to the generic loop at PC 2225 or the fixed
eight/four-limb paths. The fixed paths share the suffix beginning at PC 5238.
...
def amStep (memory : ByteArray) (pa pb n : Nat) : Nat → LimbState
  ...
  | j + 1 =>
      ...
      { memory := MachineState.writeBytes prev.memory
          (Data.Bytes.natToBytesPadded total.toNat 32) (4160 + 32 * (n - 1 - j))
...
/-- Subroutine entry (pc 2262) with stack `[pa, pb, pd, ret]`. -/
...
/-- The `ADDMOD` loop head (pc 2293) after `j` limb steps. -/
def amLoopState (s : State) (memory : ByteArray) (pa pb n j : Nat)
    (pd ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2054
           stack := [UInt256.ofNat (ptrAt (4128 + 32 * n) j),
...
theorem run_amEntry (s : State) (memory : ByteArray) (pa pb n : Nat)
    ...
    (hn : 2 ≤ n) (hpa : 32 ≤ pa) (hpaFit : pa + 32 * n ≤ 5376)
    (hpb : 32 ≤ pb) (hpbFit : pb + 32 * n ≤ 5376)
    (hs32 : MachineState.readWord memory 5248 = UInt256.ofNat (32 * n))
  ...
  have hactA : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 5248 32) =
      s.activeWords := activeWords_fix s 5248 32 (by decide) (by omega) hact
  have hactB : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 5344 32) =
      s.activeWords := activeWords_fix s 5344 32 (by decide) (by omega) hact
...
theorem run_amLoopBody (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  have hbig : (5376 : Nat) < 2 ^ 256 := by norm_num
  have hc6 : rest.length + 6 < 1024 := by omega
  have hc7 : rest.length + 7 < 1024 := by omega
  have hc8 : rest.length + 8 < 1024 := by omega
  ...
  have h2500 : (2054 : UInt256).toNat = 2054 := by decide
  have h2500' : (2054 : UInt256) = UInt256.ofNat 2054 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (2054 : UInt256).toNat = true := by
  ...
  have htb : ptrAt (pb + 32 * n - 32) j %
      ...
      pb + 32 * (n - 1 - j) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  have htt : ptrAt (4128 + 32 * n) j %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      4160 + 32 * (n - 1 - j) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  have hnext : ptrAt (4128 + 32 * n) (j + 1) %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      4128 + 32 * (n - 1 - j) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  have hgt : 4128 < 4128 + 32 * (n - 1 - j) := by omega
...
def csEntryState (s : State) (memory : ByteArray) (pdst ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 4875, stack := [pdst, ret] ++ rest, memory := memory }
...
theorem run_amLoopExit (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  have hactT : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 4160 32) =
      s.activeWords := activeWords_fix s 4160 32 (by decide) (by omega) hact
  simp (config := { maxSteps := 400000 })
    [blk1627, opAt, pushAt, wfOp,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated,
...
theorem run_amTail (s : State) (memory : ByteArray) (pa pb n j : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1662
      ...
      some (csEntryState s
        (MachineState.writeBytes (amStep memory pa pb n j).memory
          (Data.Bytes.natToBytesPadded (amStep memory pa pb n j).flag.toNat 32) 4128)
        pd ret rest) := by
  ...
  have hc6 : rest.length + 6 < 1024 := by omega
  have h8224 : (4128 : UInt256).toNat = 4128 := by decide
  have hactT : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 4128 32) =
      s.activeWords := activeWords_fix s 4128 32 (by decide) (by omega) hact
  have h3811 : (4875 : UInt256).toNat = 4875 := by decide
  have hjd : Decode.isValidJumpDest s.executionEnv.code 4875 = true := by
...
def csStep (memory : ByteArray) (n : Nat) : Nat → LimbState
  ...
  | j + 1 =>
      ...
      let d2 := d1 - prev.flag
      { memory := MachineState.writeBytes prev.memory
          (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (n - 1 - j))
...
theorem run_csEntry (s : State) (memory : ByteArray) (n : Nat)
  ...
  have h9440 : (5344 : UInt256).toNat = 5344 := by decide
  have hzero : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hadd : (3072 : UInt256) + UInt256.ofNat (32 * n - 32) =
      UInt256.ofNat (3040 + 32 * n) := by
    rw [show (3072 : UInt256) = UInt256.ofNat 3072 from by decide,
      Challenge.EvmProof.Word.ofNat_add_mod]
    congr 1
    omega
  have hactA : UInt256.ofNat
      ...
      s.activeWords := activeWords_fix s 5344 32 (by decide) (by omega) hact
  have hactB : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 5312 32) =
      s.activeWords := activeWords_fix s 5312 32 (by decide) (by omega) hact
  have hactS := activeWords_fix s 5248 32 (by decide) (by decide) hact
  ...
  have hword9344 : (5248 : UInt256).toNat = 5248 := by decide
  have hword128 : (128 : UInt256).toNat = 128 := by decide
  have hword256 : (256 : UInt256).toNat = 256 := by decide
  have hword4972 : (5030 : UInt256).toNat = 5030 := by decide
  have hword5022 : (5078 : UInt256).toNat = 5078 := by decide
  have hword5131 : (5164 : UInt256).toNat = 5164 := by decide
  have hword2225 : (2110 : UInt256).toNat = 2110 := by decide
  have hwordEq4972 : (5030 : UInt256) = UInt256.ofNat 5030 := by decide
  have hwordEq5022 : (5078 : UInt256) = UInt256.ofNat 5078 := by decide
  have hwordEq5131 : (5164 : UInt256) = UInt256.ofNat 5164 := by decide
  have hwordEq2225 : (2110 : UInt256) = UInt256.ofNat 2110 := by decide
  ...
  simp (config := { maxSteps := 400000 })
    [csGenericPath, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      subEntryState, csLoopState, csStep, fastPC14, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
...
theorem run_csLoopBody (s : State) (memory : ByteArray) (n j : Nat)
  ...
  have hK : (115792089237316195423570985008687907853269984665640564039457584007913129639904 :
    ...
    decide
  have h8224 : (4128 : UInt256).toNat = 4128 := by decide
  have h2666 : (2110 : UInt256).toNat = 2110 := by decide
  have h2666' : (2110 : UInt256) = UInt256.ofNat 2110 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (2110 : UInt256).toNat = true := by
    rw [h2666]; exact jumpDest2225
  have ht : ptrAt (4128 + 32 * n) j %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      4160 + 32 * (n - 1 - j) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  ...
  have hactD : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat
      (3072 + 32 * (n - 1 - j)) 32) = s.activeWords :=
    activeWords_fix s _ 32 (by decide) (by omega) hact
  simp (config := { maxSteps := 400000 })
...
theorem run_csLoopExit (s : State) (memory : ByteArray) (n j : Nat)
  ...
  have hactD : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 3072 32) =
      s.activeWords := activeWords_fix s 3072 32 (by decide) (by omega) hact
  simp (config := { maxSteps := 400000 })
    [blk1683, opAt, pushAt, wfOp,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated,
...
def csUse (memory : ByteArray) (n j : Nat) : UInt256 :=
  UInt256.lor (MachineState.readWord (csStep memory n j).memory 4128)
...
theorem run_csTail (s : State) (memory : ByteArray) (n j : Nat)
    ...
    (hs32 : MachineState.readWord (csStep memory n j).memory 5248 =
      UInt256.ofNat (32 * n))
    (hdstFit : pdst.toNat + 32 * n ≤ 5376)
    (hsrcFit : (csSrc memory n j).toNat + 32 * n ≤ 5376) :
  ...
  have hactN : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 4128 32) =
      s.activeWords := activeWords_fix s 4128 32 (by decide) (by omega) hact
  have hactS : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 5248 32) =
      s.activeWords := activeWords_fix s 5248 32 (by decide) (by omega) hact
...
theorem amStep_readWord_disjoint (memory : ByteArray) (pa pb n addr : Nat)
    (_hn : 1 ≤ n) (hdisj : addr + 32 ≤ 4160 ∨ 4160 + 32 * n ≤ addr) :
...
theorem amStep_lowValue_stable (memory : ByteArray) (pa pb n j : Nat) (hj : j < n) :
    lowValue (amStep memory pa pb n (j + 1)).memory 4160 n j =
      lowValue (amStep memory pa pb n j).memory 4160 n j := by
...
theorem amStep_invariant (memory : ByteArray) (pa pb n : Nat)
    (hn : 1 ≤ n) (hpa : pa + 32 * n ≤ 4160) (hpb : pb + 32 * n ≤ 4160) :
    ∀ j, j ≤ n →
      lowValue (amStep memory pa pb n j).memory 4160 n j +
  ...
  | succ j ih =>
      ...
      rw [lowValue_succ (amStep memory pa pb n (j + 1)).memory 4160 n j,
...
theorem csStep_readWord_disjoint (memory : ByteArray) (n addr : Nat)
    (_hn : 1 ≤ n) (hdisj : addr + 32 ≤ 3072 ∨ 3072 + 32 * n ≤ addr) :
...
theorem csStep_lowValue_stable (memory : ByteArray) (n j : Nat) (hj : j < n) :
    lowValue (csStep memory n (j + 1)).memory 3072 n j =
      lowValue (csStep memory n j).memory 3072 n j := by
  apply lowValue_congr
  intro k hk
  simp only [csStep]
  exact readWord_write_disjoint _ _ _ _ (by omega)
...
theorem csStep_readWord_new (memory : ByteArray) (n j : Nat) :
    MachineState.readWord (csStep memory n (j + 1)).memory (3072 + 32 * (n - 1 - j)) =
      MachineState.readWord (csStep memory n j).memory (4160 + 32 * (n - 1 - j)) -
...
theorem csStep_flag_succ (memory : ByteArray) (n j : Nat) :
    (csStep memory n (j + 1)).flag =
      UInt256.lor
        (UInt256.lt (MachineState.readWord (csStep memory n j).memory (4160 + 32 * (n - 1 - j)))
          (MachineState.readWord (csStep memory n j).memory (32 * (n - 1 - j))))
        (UInt256.lt
          (MachineState.readWord (csStep memory n j).memory (4160 + 32 * (n - 1 - j)) -
            MachineState.readWord (csStep memory n j).memory (32 * (n - 1 - j)))
...
theorem csStep_invariant (memory : ByteArray) (n : Nat) (hn : 1 ≤ n) (hn32 : n ≤ 32) :
    ∀ j, j ≤ n →
      lowValue (csStep memory n j).memory 3072 n j + lowValue memory 0 n j =
          lowValue memory 4160 n j +
  ...
  | succ j ih =>
      ...
      have hxt : MachineState.readWord (csStep memory n j).memory
          (4160 + 32 * (n - 1 - j)) =
          MachineState.readWord memory (4160 + 32 * (n - 1 - j)) :=
      ...
      have hxm : MachineState.readWord (csStep memory n j).memory (32 * (n - 1 - j)) =
        ...
        csStep_readWord_disjoint memory n _ hn (by omega) j (by omega)
      have hlimb := subLimb_spec (MachineState.readWord memory (4160 + 32 * (n - 1 - j)))
        (MachineState.readWord memory (32 * (n - 1 - j)))
        (csStep memory n j).flag ihLe
      rw [lowValue_succ (csStep memory n (j + 1)).memory 3072 n j,
        csStep_lowValue_stable memory n j (by omega),
        csStep_readWord_new, csStep_flag_succ,
        lowValue_succ memory 0 n j, lowValue_succ memory 4160 n j, pow_succ]
...
theorem fastRepresents_amStep (memory : ByteArray) (pa pb n ptr cnt v : Nat)
    (hn : 1 ≤ n)
    (hdisj : ptr + 32 * cnt ≤ 4160 ∨ 4160 + 32 * n ≤ ptr)
...
theorem fastRepresents_csStep (memory : ByteArray) (n ptr cnt v : Nat)
    (hn : 1 ≤ n)
    (hdisj : ptr + 32 * cnt ≤ 3072 ∨ 3072 + 32 * n ≤ ptr)
    (hrep : Model.FastRepresents memory ptr cnt v) (j : Nat) (hj : j ≤ n) :
...
theorem csUse_le_one (memory : ByteArray) (n j : Nat)
    (htn : (MachineState.readWord (csStep memory n j).memory 4128).toNat ≤ 1) :
...
theorem csUse_toNat (memory : ByteArray) (n j : Nat)
    ...
    (csUse memory n j).toNat =
      max (MachineState.readWord (csStep memory n j).memory 4128).toNat
...
theorem csSrc_toNat (memory : ByteArray) (n j : Nat)
    ...
    (csSrc memory n j).toNat =
      if (csUse memory n j).toNat = 0 then 4160 else 3072 := by
...
def gasSteps_addmod (s : State) (memory : ByteArray) (pa pb n : Nat)
    ...
    Challenge.EvmProof.GasSteps (amEntryState s memory pa pb pd ret rest)
      (csEntryState s (MachineState.writeBytes (amStep memory pa pb n n).memory
        (Data.Bytes.natToBytesPadded (amStep memory pa pb n n).flag.toNat 32) 4128)
        pd ret rest) := by
...
def gasSteps_csIteration (s : State) (memory : ByteArray) (n j : Nat)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hj : j + 1 < n) (hn32 : n ≤ 32) :
...
def gasSteps_csLoop (s : State) (memory : ByteArray) (n : Nat)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hn32 : n ≤ 32) :
...
theorem run_csFixedEntry8 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hact : 168 ≤ s.activeWords.toNat)
    (hs32 : MachineState.readWord memory 5248 = UInt256.ofNat (32 * 8)) :
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedEntry8
      (subEntryState s memory pdst ret rest) =
      some (csFixedState s memory 8 0 5079 pdst ret rest) := by
  have hactS := activeWords_fix s 5248 32 (by decide) (by decide) hact
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
...
theorem run_csFixedStep8_0 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_0
      (csFixedState s memory 8 0 5079 pdst ret rest) =
      some (csFixedState s memory 8 1 5095 pdst ret rest) := by
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 4384 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 3296 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 1 =
      (let prev := csStep memory 8 0
       let t := MachineState.readWord prev.memory (4160 + 32 * (8 - 1 - 0))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 0))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (8 - 1 - 0))
  ...
  have hword8480 : (4384 : UInt256).toNat = 4384 := by decide
  have hword7392 : (3296 : UInt256).toNat = 3296 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_1 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_1
      (csFixedState s memory 8 1 5095 pdst ret rest) =
      some (csFixedState s memory 8 2 5118 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 4352 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 3264 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 2 =
      (let prev := csStep memory 8 1
       let t := MachineState.readWord prev.memory (4160 + 32 * (8 - 1 - 1))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 1))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (8 - 1 - 1))
  ...
  have hword8448 : (4352 : UInt256).toNat = 4352 := by decide
  have hword7360 : (3264 : UInt256).toNat = 3264 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_2 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_2
      (csFixedState s memory 8 2 5118 pdst ret rest) =
      some (csFixedState s memory 8 3 5141 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 4320 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 3232 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 3 =
      (let prev := csStep memory 8 2
       let t := MachineState.readWord prev.memory (4160 + 32 * (8 - 1 - 2))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 2))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (8 - 1 - 2))
  ...
  have hword8416 : (4320 : UInt256).toNat = 4320 := by decide
  have hword7328 : (3232 : UInt256).toNat = 3232 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_3 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_3
      (csFixedState s memory 8 3 5141 pdst ret rest) =
      some (csFixedState s memory 8 4 5164 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 4288 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 3200 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 4 =
      (let prev := csStep memory 8 3
       let t := MachineState.readWord prev.memory (4160 + 32 * (8 - 1 - 3))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 3))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (8 - 1 - 3))
         flag := UInt256.lor (UInt256.lt t md) (UInt256.lt d1 prev.flag) } : LimbState) := csStep.eq_2 memory 8 3
  have hword128 : (128 : UInt256).toNat = 128 := by decide
  have hword8384 : (4288 : UInt256).toNat = 4288 := by decide
  have hword7296 : (3200 : UInt256).toNat = 3200 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_4 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_4
      (csFixedState s memory 8 4 5164 pdst ret rest) =
      some (csFixedState s memory 8 5 5187 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 4256 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 3168 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 5 =
      (let prev := csStep memory 8 4
       let t := MachineState.readWord prev.memory (4160 + 32 * (8 - 1 - 4))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 4))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (8 - 1 - 4))
  ...
  have hword8352 : (4256 : UInt256).toNat = 4256 := by decide
  have hword7264 : (3168 : UInt256).toNat = 3168 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_5 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_5
      (csFixedState s memory 8 5 5187 pdst ret rest) =
      some (csFixedState s memory 8 6 5211 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 4224 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 3136 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 6 =
      (let prev := csStep memory 8 5
       let t := MachineState.readWord prev.memory (4160 + 32 * (8 - 1 - 5))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 5))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (8 - 1 - 5))
  ...
  have hword8320 : (4224 : UInt256).toNat = 4224 := by decide
  have hword7232 : (3136 : UInt256).toNat = 3136 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_6 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_6
      (csFixedState s memory 8 6 5211 pdst ret rest) =
      some (csFixedState s memory 8 7 5234 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 4192 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 3104 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 7 =
      (let prev := csStep memory 8 6
       let t := MachineState.readWord prev.memory (4160 + 32 * (8 - 1 - 6))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 6))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (8 - 1 - 6))
  ...
  have hword8288 : (4192 : UInt256).toNat = 4192 := by decide
  have hword7200 : (3104 : UInt256).toNat = 3104 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_7 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_7
      (csFixedState s memory 8 7 5234 pdst ret rest) =
      some (csFixedState s memory 8 8 5256 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 4160 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 3072 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 8 =
      (let prev := csStep memory 8 7
       let t := MachineState.readWord prev.memory (4160 + 32 * (8 - 1 - 7))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 7))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (8 - 1 - 7))
  ...
  have hword8256 : (4160 : UInt256).toNat = 4160 := by decide
  have hword7168 : (3072 : UInt256).toNat = 3072 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedTail8 (s : State) (memory : ByteArray)
    ...
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hact : 168 ≤ s.activeWords.toNat) (_hn : 2 ≤ 8) (_hn32 : 8 ≤ 32)
    (hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode ret.toNat = true)
    (hs32 : MachineState.readWord (csStep memory 8 8).memory 5248 =
      UInt256.ofNat (32 * 8))
    (hdstFit : pdst.toNat + 32 * 8 ≤ 5376)
    (hsrcFit : (csSrc memory 8 8).toNat + 32 * 8 ≤ 5376) :
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedTail8
      (csFixedState s memory 8 8 5256 pdst ret rest) =
      some (subReturnedState s memory 8 8 pdst ret rest) := by
  have h5243 : (5281 : UInt256).toNat = 5281 := by decide
  have heq5243 : (5281 : UInt256) = UInt256.ofNat 5281 := by decide
  have hbig : (20000 : Nat) < 2 ^ 256 := by norm_num
  have hc1 : rest.length + 1 < 1024 := by omega
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  ...
  have hsrcEq : (4160 : UInt256) +
      (115792089237316195423570985008687907853269984665640564039457584007913129638848 :
        UInt256) *
        UInt256.lor (MachineState.readWord (csStep memory 8 8).memory 4128)
...
def gasSteps_csFixed8 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hs : MachineState.readWord memory 5248 = UInt256.ofNat (32 * 8))
    (hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode ret.toNat = true)
    (hs32 : MachineState.readWord (csStep memory 8 8).memory 5248 = UInt256.ofNat (32 * 8))
    (hdstFit : pdst.toNat + 32 * 8 ≤ 5376)
    (hsrcFit : (csSrc memory 8 8).toNat + 32 * 8 ≤ 5376) :
    Challenge.EvmProof.GasSteps (subEntryState s memory pdst ret rest)
...
theorem run_csFixedEntry4 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hact : 168 ≤ s.activeWords.toNat)
    (hs32 : MachineState.readWord memory 5248 = UInt256.ofNat (32 * 4)) :
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedEntry4
      (subEntryState s memory pdst ret rest) =
      some (csFixedState s memory 4 0 4898 pdst ret rest) := by
  have hactS := activeWords_fix s 5248 32 (by decide) (by decide) hact
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  ...
  have hword4972 : (5030 : UInt256).toNat = 5030 := by decide
  have hword5022 : (5078 : UInt256).toNat = 5078 := by decide
  have hword5131 : (4897 : UInt256).toNat = 4897 := by decide
  have hword2225 : (2110 : UInt256).toNat = 2110 := by decide
  have hwordEq4972 : (5030 : UInt256) = UInt256.ofNat 5030 := by decide
  have hwordEq5022 : (5078 : UInt256) = UInt256.ofNat 5078 := by decide
  have hwordEq5131 : (4897 : UInt256) = UInt256.ofNat 4897 := by decide
  have hwordEq2225 : (2110 : UInt256) = UInt256.ofNat 2110 := by decide
  have hwordZero : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  simp (config := { maxSteps := 800000 })
    [csFixedEntry4, subEntryState, csFixedState, csStep, fastPC14, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      hrun, hcode, hs32, hactS, hc2, hc3, hc4, hc5,
      UInt256.eq, UInt256.isTrue, opAt, pushAt, wfOp, Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
      State.activeWordsAfterUInt256, Challenge.EvmProof.Word.succ_ofNat_mod,
...
theorem run_csFixedStep4_0 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep4_0
      (csFixedState s memory 4 0 4898 pdst ret rest) =
      some (csFixedState s memory 4 1 5187 pdst ret rest) := by
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  ...
  have hstep : csStep memory 4 1 =
      (let prev := csStep memory 4 0
       let t := MachineState.readWord prev.memory (4160 + 32 * (4 - 1 - 0))
       let md := MachineState.readWord prev.memory (32 * (4 - 1 - 0))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (4 - 1 - 0))
         flag := UInt256.lor (UInt256.lt t md) (UInt256.lt d1 prev.flag) } : LimbState) := csStep.eq_2 memory 4 0
  have hword96 : (96 : UInt256).toNat = 96 := by decide
  have hword8352 : (4256 : UInt256).toNat = 4256 := by decide
  have hword7264 : (3168 : UInt256).toNat = 3168 := by decide
  have hword5117 : (5187 : UInt256).toNat = 5187 := by decide
  have hwordEq5117 : (5187 : UInt256) = UInt256.ofNat 5187 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep4_1 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep4_1
      (csFixedState s memory 4 1 5187 pdst ret rest) =
      some (csFixedState s memory 4 2 5211 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have hstep : csStep memory 4 2 =
      (let prev := csStep memory 4 1
       let t := MachineState.readWord prev.memory (4160 + 32 * (4 - 1 - 1))
       let md := MachineState.readWord prev.memory (32 * (4 - 1 - 1))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (4 - 1 - 1))
...
theorem run_csFixedStep4_2 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep4_2
      (csFixedState s memory 4 2 5211 pdst ret rest) =
      some (csFixedState s memory 4 3 5234 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have hstep : csStep memory 4 3 =
      (let prev := csStep memory 4 2
       let t := MachineState.readWord prev.memory (4160 + 32 * (4 - 1 - 2))
       let md := MachineState.readWord prev.memory (32 * (4 - 1 - 2))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (4 - 1 - 2))
...
theorem run_csFixedStep4_3 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep4_3
      (csFixedState s memory 4 3 5234 pdst ret rest) =
      some (csFixedState s memory 4 4 5256 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have hstep : csStep memory 4 4 =
      (let prev := csStep memory 4 3
       let t := MachineState.readWord prev.memory (4160 + 32 * (4 - 1 - 3))
       let md := MachineState.readWord prev.memory (32 * (4 - 1 - 3))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (3072 + 32 * (4 - 1 - 3))
...
theorem run_csFixedTail4 (s : State) (memory : ByteArray)
    ...
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hact : 168 ≤ s.activeWords.toNat) (_hn : 2 ≤ 4) (_hn32 : 4 ≤ 32)
    (hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode ret.toNat = true)
    (hs32 : MachineState.readWord (csStep memory 4 4).memory 5248 =
      UInt256.ofNat (32 * 4))
    (hdstFit : pdst.toNat + 32 * 4 ≤ 5376)
    (hsrcFit : (csSrc memory 4 4).toNat + 32 * 4 ≤ 5376) :
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedTail4
      (csFixedState s memory 4 4 5256 pdst ret rest) =
  ...
  have hsrcEq : (4160 : UInt256) +
      (115792089237316195423570985008687907853269984665640564039457584007913129638848 :
        UInt256) *
        UInt256.lor (MachineState.readWord (csStep memory 4 4).memory 4128)
          (UInt256.isZero (csStep memory 4 4).flag) = csSrc memory 4 4 := rfl
  simp (config := { maxSteps := 400000 })
    [h5243, heq5243, jumpDestCopyResume, csFixedTail4, opAt, pushAt, wfOp,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      csFixedState, subReturnedState, hsrcEq, fastPC14, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
...
def gasSteps_csFixed4 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hs : MachineState.readWord memory 5248 = UInt256.ofNat (32 * 4))
    (hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode ret.toNat = true)
    (hs32 : MachineState.readWord (csStep memory 4 4).memory 5248 = UInt256.ofNat (32 * 4))
    (hdstFit : pdst.toNat + 32 * 4 ≤ 5376)
    (hsrcFit : (csSrc memory 4 4).toNat + 32 * 4 ≤ 5376) :
    Challenge.EvmProof.GasSteps (subEntryState s memory pdst ret rest)
...
def gasSteps_csub_sub (s : State) (memory : ByteArray) (n : Nat)
    ...
    (hml : MachineState.readWord memory 5312 = UInt256.ofNat (32 * n - 32))
    (htl : MachineState.readWord memory 5344 = UInt256.ofNat (4128 + 32 * n))
    (hs32 : MachineState.readWord (csStep memory n n).memory 5248 =
      UInt256.ofNat (32 * n))
    (hdstFit : pdst.toNat + 32 * n ≤ 5376)
  ...
  have hnn : n - 1 + 1 = n := by omega
  have hsrcFit : (csSrc memory n n).toNat + 32 * n ≤ 5376 := by
...
theorem amResultMemory_def (memory : ByteArray) (pa pb n : Nat) :
    amResultMemory memory pa pb n =
      MachineState.writeBytes (amStep memory pa pb n n).memory
        (Data.Bytes.natToBytesPadded (amStep memory pa pb n n).flag.toNat 32) 4128 := rfl
...
theorem addmod_carry_le_one (memory : ByteArray) (pa pb n : Nat) (hn : 2 ≤ n)
    (hpa : pa + 32 * n ≤ 4160) (hpb : pb + 32 * n ≤ 4160) :
...
theorem addmod_value (memory : ByteArray) (pa pb n a b : Nat) (hn : 2 ≤ n)
    ...
    lowValue (amStep memory pa pb n n).memory 4160 n n +
...
theorem addmod_represents (memory : ByteArray) (pa pb n : Nat) :
    Model.FastRepresents (amResultMemory memory pa pb n) 4160 n
      (lowValue (amStep memory pa pb n n).memory 4160 n n) := by
  unfold amResultMemory
  exact Model.fastRepresents_writeWord_disjoint _ 4128 4160 n _ _ (by omega)
    (fastRepresents_lowValue _ _ _)

theorem addmod_tn (memory : ByteArray) (pa pb n : Nat) :
    MachineState.readWord (amResultMemory memory pa pb n) 4128 =
...
theorem addmod_preserves_region (memory : ByteArray) (pa pb n ptr cnt v : Nat)
  ...
  unfold amResultMemory
  exact Model.fastRepresents_writeWord_disjoint _ 4128 ptr cnt v _ (by omega)
...
theorem csub_sub_correct (memory : ByteArray) (n tlow mm tn pdst : Nat)
    (hn : 2 ≤ n) (hn32 : n ≤ 32)
    (ht : Model.FastRepresents memory 4160 n tlow)
    (hm : Model.FastRepresents memory 0 n mm)
    (htnv : (MachineState.readWord memory 4128).toNat = tn) (htn1 : tn ≤ 1)
    (_hmpos : 0 < mm)
  ...
  have hT : lowValue memory 4160 n n = tlow := by
    rw [lowValue_full]; exact Model.value_of_fastRepresents ht
  rw [hM, hT] at hinv
  have hDlt : lowValue (csStep memory n n).memory 3072 n n < Limbs.radix ^ n :=
    lowValue_lt _ _ _ _
  have hmmlt : mm < Limbs.radix ^ n := hm.1
  have htnStep : MachineState.readWord (csStep memory n n).memory 4128 =
      MachineState.readWord memory 4128 :=
    csStep_readWord_disjoint memory n 4128 hn1 (by omega) n le_rfl
  have htnStep' : (MachineState.readWord (csStep memory n n).memory 4128).toNat ≤ 1 := by
  ...
  have hsubb : Model.FastRepresents (csStep memory n n).memory 3072 n
      (lowValue (csStep memory n n).memory 3072 n n) := fastRepresents_lowValue _ _ _
  have htsblk : Model.FastRepresents (csStep memory n n).memory 4160 n tlow :=
    fastRepresents_csStep memory n 4160 n tlow hn1 (by omega) ht n le_rfl
  ...
  · have huse1' : max tn (if (csStep memory n n).flag.toNat = 0 then 1 else 0) = 1 := by
    ...
    have hcases : tn = 1 ∨ (csStep memory n n).flag.toNat = 0 := by
      ...
      · rw [if_neg hb] at huse1'
        left; omega
    have hval : lowValue (csStep memory n n).memory 3072 n n + mm =
    ...
    have hmod : (tn * Limbs.radix ^ n + tlow) % mm =
        lowValue (csStep memory n n).memory 3072 n n := by
