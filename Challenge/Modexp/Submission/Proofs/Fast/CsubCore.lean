...
`ADDMOD` starts at PC 2264 and falls through into `CSUB` at PC 2220.
`CSUB` dispatches at PC 5156 to the generic loop at PC 2225 or the fixed
eight/four-limb paths. The fixed paths share the suffix beginning at PC 5236.
...
/- One downward step, in the shape a `PUSH1 32; SUB` pair produces.  The
unsigned decrement and the wrapped increment by `2 ^ 256 - 32` agree on every
word, because the word subtraction already reduces modulo `2 ^ 256`; no range
side condition on `base` or `j` is needed. -/
...
/-- Subroutine entry (pc 2264) with stack `[pa, pb, pd, ret]`. -/
...
/-- The `ADDMOD` loop head (pc 2295) after `j` limb steps. -/
def amLoopState (s : State) (memory : ByteArray) (pa pb n j : Nat)
    (pd ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 1128
...
theorem run_amEntry (s : State) (memory : ByteArray) (pa pb n : Nat)
    ...
    (hn : 2 ≤ n) (hpa : 32 ≤ pa) (hpaFit : pa + 32 * n ≤ 2816)
    (hpb : 32 ≤ pb) (hpbFit : pb + 32 * n ≤ 2816)
    (hs32 : MachineState.readWord memory 2688 = UInt256.ofNat (32 * n))
  ...
  have hactA : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 2688 32) =
      s.activeWords := activeWords_fix s 2688 32 (by decide) (by omega) hact
  ...
  simp (config := { maxSteps := 800000 })
    [blk1600, opAt, pushAt, wfOp,
      ...
      amEntryState, amLoopState, amStep, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
...
theorem run_amLoopBody (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  have hbig : (2816 : Nat) < 2 ^ 256 := by norm_num
  have hc6 : rest.length + 6 < 1024 := by omega
  have hc7 : rest.length + 7 < 1024 := by omega
  have hc8 : rest.length + 8 < 1024 := by omega
  ...
  have h2500 : (1128 : UInt256).toNat = 1128 := by decide
  have h2500' : (1128 : UInt256) = UInt256.ofNat 1128 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (1128 : UInt256).toNat = true := by
  ...
  have hnext : ptrAt (2080 + 32 * n) (j + 1) %
      ...
      2080 + 32 * (n - 1 - j) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  have hgt : 2080 < 2080 + 32 * (n - 1 - j) := by omega
  ...
  simp (config := { maxSteps := 400000 })
    [blk1627, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      amLoopState, amStep, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
      hc6, hc7, hc8, hc9, hrun, hcode, hK, h32w, h8224, h2500, h2500', hjump, jumpDest2168,
      hta, htb, htt, hnext, hgt, hactA, hactB, hactT, ptrAt_succ, ptrAt_pred,
...
def csEntryState (s : State) (memory : ByteArray) (pdst ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 3963, stack := [pdst, ret] ++ rest, memory := memory }
...
theorem run_amLoopExit (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  simp (config := { maxSteps := 400000 })
    [blk1627, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      amLoopState, amTailState, amStep, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
      hc6, hc7, hc8, hc9, hrun, hK, h32w, h8224, hnj,
      hta, htb, htt, hnext, hactA, hactB, hactT, ptrAt_succ, ptrAt_pred,
      UInt256.gt, UInt256.isTrue,
      State.activeWordsAfterUInt256,
      Challenge.EvmProof.Word.succ_ofNat_mod,
      Challenge.EvmProof.Word.ofNat_add_mod,
...
theorem run_amTail (s : State) (memory : ByteArray) (pa pb n j : Nat)
  ...
  have h3811 : (3963 : UInt256).toNat = 3963 := by decide
  have hjd : Decode.isValidJumpDest s.executionEnv.code 3963 = true := by
  ...
  simp (config := { maxSteps := 400000 })
    [blk1662, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      amTailState, csEntryState, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
...
theorem run_csLoopBody (s : State) (memory : ByteArray) (n j : Nat)
  ...
  have hK : (115792089237316195423570985008687907853269984665640564039457584007913129639904 :
    ...
    decide
  have h8224 : (2080 : UInt256).toNat = 2080 := by decide
  have h2666 : (1182 : UInt256).toNat = 1182 := by decide
  have h2666' : (1182 : UInt256) = UInt256.ofNat 1182 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (1182 : UInt256).toNat = true := by
  ...
  have hactD : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat
      (1792 + 32 * (n - 1 - j)) 32) = s.activeWords :=
    activeWords_fix s _ 32 (by decide) (by omega) hact
  simp (config := { maxSteps := 400000 })
    [blk1683, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      csLoopState, csStep, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
...
theorem run_csLoopExit (s : State) (memory : ByteArray) (n j : Nat)
  ...
  simp (config := { maxSteps := 400000 })
    [blk1683, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      csLoopState, csTailState, csStep, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
...
theorem run_csTail (s : State) (memory : ByteArray) (n j : Nat)
    ...
    (hs32 : MachineState.readWord (csStep memory n j).memory 2688 =
      UInt256.ofNat (32 * n))
    (hdstFit : pdst.toNat + 32 * n ≤ 2816)
    (hsrcFit : (csSrc memory n j).toNat + 32 * n ≤ 2816) :
  ...
  have hactN : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 2080 32) =
      s.activeWords := activeWords_fix s 2080 32 (by decide) (by omega) hact
  have hactS : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 2688 32) =
      s.activeWords := activeWords_fix s 2688 32 (by decide) (by omega) hact
  ...
  simp (config := { maxSteps := 400000 })
    [blk1724, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      csTailState, subReturnedState, hsrcEq, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
...
def gasSteps_csIteration (s : State) (memory : ByteArray) (n j : Nat)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 88 ≤ s.activeWords.toNat) (hj : j + 1 < n) (hn32 : n ≤ 8) :
...
-- the eight-limb entry now falls through the dispatch instead of jumping, so it no longer needs
-- the code hypothesis; the binder stays so the call sites are unchanged
...
theorem run_csFixedEntry8 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedEntry8
      ...
      some (csFixedState s memory 8 0 4142 pdst ret rest) := by
  ...
  have hword4972 : (4130 : UInt256).toNat = 4130 := by decide
  have hword5022 : (3984 : UInt256).toNat = 3984 := by decide
  have hword5131 : (4230 : UInt256).toNat = 4230 := by decide
  have hword2225 : (1182 : UInt256).toNat = 1182 := by decide
  have hwordEq4972 : (4130 : UInt256) = UInt256.ofNat 4130 := by decide
  have hwordEq5022 : (3984 : UInt256) = UInt256.ofNat 3984 := by decide
  have hwordEq5131 : (4230 : UInt256) = UInt256.ofNat 4230 := by decide
  have hwordEq2225 : (1182 : UInt256) = UInt256.ofNat 1182 := by decide
  have hwordZero : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  simp (config := { maxSteps := 800000 })
    [csFixedEntry8, subEntryState, csFixedState, csStep, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
...
theorem run_csFixedStep8_0 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_0
      (csFixedState s memory 8 0 4142 pdst ret rest) =
      some (csFixedState s memory 8 1 4158 pdst ret rest) := by
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
...
theorem run_csFixedStep8_1 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_1
      (csFixedState s memory 8 1 4158 pdst ret rest) =
      some (csFixedState s memory 8 2 4181 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 2304 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 1984 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 2 =
      (let prev := csStep memory 8 1
       let t := MachineState.readWord prev.memory (2112 + 32 * (8 - 1 - 1))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 1))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (1792 + 32 * (8 - 1 - 1))
  ...
  have hword8448 : (2304 : UInt256).toNat = 2304 := by decide
  have hword7360 : (1984 : UInt256).toNat = 1984 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_2 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_2
      (csFixedState s memory 8 2 4181 pdst ret rest) =
      some (csFixedState s memory 8 3 4204 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 2272 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 1952 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 3 =
      (let prev := csStep memory 8 2
       let t := MachineState.readWord prev.memory (2112 + 32 * (8 - 1 - 2))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 2))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (1792 + 32 * (8 - 1 - 2))
  ...
  have hword8416 : (2272 : UInt256).toNat = 2272 := by decide
  have hword7328 : (1952 : UInt256).toNat = 1952 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_3 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_3
      (csFixedState s memory 8 3 4204 pdst ret rest) =
      some (csFixedState s memory 8 4 4227 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have ha1 := activeWords_fix s 2240 32 (by decide) (by decide) hact
  have ha2 := activeWords_fix s 1920 32 (by decide) (by decide) hact
  have hstep : csStep memory 8 4 =
      (let prev := csStep memory 8 3
       let t := MachineState.readWord prev.memory (2112 + 32 * (8 - 1 - 3))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 3))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (1792 + 32 * (8 - 1 - 3))
         flag := UInt256.lor (UInt256.lt t md) (UInt256.lt d1 prev.flag) } : LimbState) := csStep.eq_2 memory 8 3
  have hword128 : (128 : UInt256).toNat = 128 := by decide
  have hword8384 : (2240 : UInt256).toNat = 2240 := by decide
  have hword7296 : (1920 : UInt256).toNat = 1920 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep8_4 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_4
      (csFixedState s memory 8 4 4227 pdst ret rest) =
      some (csFixedState s memory 8 5 4250 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have hstep : csStep memory 8 5 =
      (let prev := csStep memory 8 4
       let t := MachineState.readWord prev.memory (2112 + 32 * (8 - 1 - 4))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 4))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (1792 + 32 * (8 - 1 - 4))
...
theorem run_csFixedStep8_5 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_5
      (csFixedState s memory 8 5 4250 pdst ret rest) =
      some (csFixedState s memory 8 6 4274 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have hstep : csStep memory 8 6 =
      (let prev := csStep memory 8 5
       let t := MachineState.readWord prev.memory (2112 + 32 * (8 - 1 - 5))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 5))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (1792 + 32 * (8 - 1 - 5))
...
theorem run_csFixedStep8_6 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_6
      (csFixedState s memory 8 6 4274 pdst ret rest) =
      some (csFixedState s memory 8 7 4297 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have hstep : csStep memory 8 7 =
      (let prev := csStep memory 8 6
       let t := MachineState.readWord prev.memory (2112 + 32 * (8 - 1 - 6))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 6))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (1792 + 32 * (8 - 1 - 6))
...
theorem run_csFixedStep8_7 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep8_7
      (csFixedState s memory 8 7 4297 pdst ret rest) =
      some (csFixedState s memory 8 8 4319 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
  ...
  have hstep : csStep memory 8 8 =
      (let prev := csStep memory 8 7
       let t := MachineState.readWord prev.memory (2112 + 32 * (8 - 1 - 7))
       let md := MachineState.readWord prev.memory (32 * (8 - 1 - 7))
       let d1 := t - md
       let d2 := d1 - prev.flag
       { memory := MachineState.writeBytes prev.memory
           (Data.Bytes.natToBytesPadded d2.toNat 32) (1792 + 32 * (8 - 1 - 7))
...
theorem run_csFixedTail8 (s : State) (memory : ByteArray)
    ...
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hact : 88 ≤ s.activeWords.toNat) (_hn : 2 ≤ 8) (_hn32 : 8 ≤ 32)
    (hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode ret.toNat = true)
    (hs32 : MachineState.readWord (csStep memory 8 8).memory 2688 =
      UInt256.ofNat (32 * 8))
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedTail8
      (csFixedState s memory 8 8 4319 pdst ret rest) =
      some (subReturnedState s memory 8 8 pdst ret rest) := by
  have h5243 : (4353 : UInt256).toNat = 4353 := by decide
  have heq5243 : (4353 : UInt256) = UInt256.ofNat 4353 := by decide
  have hbig : (20000 : Nat) < 2 ^ 256 := by norm_num
  have hc1 : rest.length + 1 < 1024 := by omega
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  ...
  simp (config := { maxSteps := 400000 })
    [h5243, heq5243, jumpDestCopyResume, csFixedTail8, opAt, pushAt, wfOp,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      csFixedState, subReturnedState, hsrcEq, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
      hc1, hc2, hc3, hc4, hc5, hc6, hrun, hcode, h8224, h9344, hjump, hs32,
      hsz, hactN, hactS, hactC1, hactC2,
...
def gasSteps_csFixed8 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hs : MachineState.readWord memory 2688 = UInt256.ofNat (32 * 8))
    (hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode ret.toNat = true)
    (hs32 : MachineState.readWord (csStep memory 8 8).memory 2688 = UInt256.ofNat (32 * 8))
    (hdstFit : pdst.toNat + 32 * 8 ≤ 2816)
    (hsrcFit : (csSrc memory 8 8).toNat + 32 * 8 ≤ 2816) :
...
theorem run_csFixedEntry4 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hact : 88 ≤ s.activeWords.toNat)
    (hs32 : MachineState.readWord memory 2688 = UInt256.ofNat (32 * 4)) :
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedEntry4
      (subEntryState s memory pdst ret rest) =
      some (csFixedState s memory 4 0 3985 pdst ret rest) := by
  have hactS := activeWords_fix s 2688 32 (by decide) (by decide) hact
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  ...
  have hword4972 : (4130 : UInt256).toNat = 4130 := by decide
  have hword5022 : (3984 : UInt256).toNat = 3984 := by decide
  have hword5131 : (3984 : UInt256).toNat = 3984 := by decide
  have hword2225 : (1182 : UInt256).toNat = 1182 := by decide
  have hwordEq4972 : (4130 : UInt256) = UInt256.ofNat 4130 := by decide
  have hwordEq5022 : (3984 : UInt256) = UInt256.ofNat 3984 := by decide
  have hwordEq5131 : (3984 : UInt256) = UInt256.ofNat 3984 := by decide
  have hwordEq2225 : (1182 : UInt256) = UInt256.ofNat 1182 := by decide
  have hwordZero : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  simp (config := { maxSteps := 800000 })
    [csFixedEntry4, subEntryState, csFixedState, csStep, fastPC15, fastPC16, fastPC17, fastPC18, fastPC19,
      fixedPC3777, fixedPC3114, fixedPC3115, fixedPC3116,
      fixedPC3117, fixedPC3118, fixedPC3119, fixedPC3805,
      hrun, hcode, hs32, hactS, hc2, hc3, hc4, hc5,
      UInt256.eq, UInt256.isTrue, opAt, pushAt, wfOp, Challenge.EvmProof.Stepper.runLocatedBlock,
...
theorem run_csFixedStep4_0 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep4_0
      (csFixedState s memory 4 0 3985 pdst ret rest) =
      some (csFixedState s memory 4 1 4250 pdst ret rest) := by
  have hc2 : rest.length + 2 < 1024 := by omega
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  ...
  have hword8352 : (2208 : UInt256).toNat = 2208 := by decide
  have hword7264 : (1888 : UInt256).toNat = 1888 := by decide
  have hword5117 : (4250 : UInt256).toNat = 4250 := by decide
  have hwordEq5117 : (4250 : UInt256) = UInt256.ofNat 4250 := by decide
  simp (config := { maxSteps := 800000 })
...
theorem run_csFixedStep4_1 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep4_1
      (csFixedState s memory 4 1 4250 pdst ret rest) =
      some (csFixedState s memory 4 2 4274 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
...
theorem run_csFixedStep4_2 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep4_2
      (csFixedState s memory 4 2 4274 pdst ret rest) =
      some (csFixedState s memory 4 3 4297 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
...
theorem run_csFixedStep4_3 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedStep4_3
      (csFixedState s memory 4 3 4297 pdst ret rest) =
      some (csFixedState s memory 4 4 4319 pdst ret rest) := by
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have hc6 : rest.length + 6 < 1024 := by omega
...
theorem run_csFixedTail4 (s : State) (memory : ByteArray)
    ...
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hact : 88 ≤ s.activeWords.toNat) (_hn : 2 ≤ 4) (_hn32 : 4 ≤ 32)
    (hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode ret.toNat = true)
    (hs32 : MachineState.readWord (csStep memory 4 4).memory 2688 =
      UInt256.ofNat (32 * 4))
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock csFixedTail4
      (csFixedState s memory 4 4 4319 pdst ret rest) =
  ...
  have hsrcEq : (2112 : UInt256) +
      (115792089237316195423570985008687907853269984665640564039457584007913129639616 :
        UInt256) *
        UInt256.lor (MachineState.readWord (csStep memory 4 4).memory 2080)
...
def gasSteps_csFixed4 (s : State) (memory : ByteArray) (pdst ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hs : MachineState.readWord memory 2688 = UInt256.ofNat (32 * 4))
    (hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode ret.toNat = true)
    (hs32 : MachineState.readWord (csStep memory 4 4).memory 2688 = UInt256.ofNat (32 * 4))
    (hdstFit : pdst.toNat + 32 * 4 ≤ 2816)
    (hsrcFit : (csSrc memory 4 4).toNat + 32 * 4 ≤ 2816) :
    Challenge.EvmProof.GasSteps (subEntryState s memory pdst ret rest)
...
def gasSteps_csub_sub (s : State) (memory : ByteArray) (n : Nat)
    ...
    (htl : MachineState.readWord memory 2784 = UInt256.ofNat (2080 + 32 * n))
    (hs32 : MachineState.readWord (csStep memory n n).memory 2688 =
      UInt256.ofNat (32 * n))
    (hdstFit : pdst.toNat + 32 * n ≤ 2816)
    (htn : (MachineState.readWord (csStep memory n n).memory 2080).toNat ≤ 1)
    -- the rewritten dispatcher admits only four or eight limbs, so the general-width
    -- conditional subtraction below it is unreachable
    (hfast : n = 4 ∨ n = 8) :
    Challenge.EvmProof.GasSteps (subEntryState s memory pdst ret rest)
      (subReturnedState s memory n n pdst ret rest) := by
  have hnn : n - 1 + 1 = n := by omega
  have hsrcFit : (csSrc memory n n).toNat + 32 * n ≤ 2816 := by
  ...
  exact absurd hfast (fun h => h.elim hn4 hn8)
