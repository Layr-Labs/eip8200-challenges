...
`2 ^ 256 - 32`.  The modulus is rewritten to a literal before `omega`,
which does not evaluate `2 ^ 256` inside `Nat.mod`. -/
...
theorem run_csLoopBody (s : State) (memory : ByteArray) (n j : Nat)
  ...
  simp (config := { maxSteps := 400000 })
    [blk1683, opAt, pushAt, wfOp,
      ...
      hc6, hc7, hc8, hc9, hc10, hrun, hcode, hK, h32, h8224, h2666, h2666', hjump,
...
theorem run_csLoopExit (s : State) (memory : ByteArray) (n j : Nat)
  ...
  simp (config := { maxSteps := 400000 })
    [blk1683, opAt, pushAt, wfOp,
      ...
      hc6, hc7, hc8, hc9, hc10, hrun, hK, h32, h8224, hnj,
