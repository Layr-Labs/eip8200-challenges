import Challenge.Modexp.Submission.Proofs.Fast.TnCandidateRowSteps
...
namespace Challenge.Modexp.Submission.Proofs.Fast.TnCandidateRowsSteps
open EvmSemantics EvmSemantics.EVM
open Challenge.EvmProof
open Challenge.Modexp.Submission.Proofs.Bytecode WindowTwentyOneBinding
open Challenge.Modexp.Submission.Proofs.Fast Monpro CiosCached CiosCachedMacCore
open TnCacheRowModel TnCacheRowPreserves TnCacheRowPointers TnCandidateRowSteps
...
def rowState (s : State) (z : CacheState) (pb n i : Nat)
  ...
  framed {s with memory := z.memory}
    (if i < n then UInt256.ofNat 3711 else UInt256.ofNat 4292)
    (TnCacheFrameOps.frame (pointer pb n i) (UInt256.ofNat 3711) (UInt256.ofNat (pb-32))
      (UInt256.ofNat (l1PC n)) z.tn (UInt256.ofNat (l2PC n)) inv
...
noncomputable def step (s : State)
  ...
  have hr := row_steps {s with memory := z.memory}
    ...
    tl inv m0 aEnd m96 m64 m32 dst ret rest hcap hact hpbi hpa ha
    hc.readonly hc.extra hc.inverse hc.snapshot
  have he := result_eq_row z pa pb n i hpb hfit hi
  have hcond := pointer_condition pb n i hpb hfit hi
  rw [pointer_succ] at hcond
  simpa only [rowState, if_pos hi, he, hcond, pointer_succ] using hr
...
end Challenge.Modexp.Submission.Proofs.Fast.TnCandidateRowsSteps
