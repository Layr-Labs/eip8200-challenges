...
import Challenge.Modexp.Submission.Proofs.Fast.Guard257Trace1
...
def gasSteps_match (input : ByteArray) (h : guardDiff input = 0) :
  ...
  (((sound preludePath rfl (Guard257Trace0.run_prelude input) rfl rfl deployAddress_not_precompile).trans
    ...
    (sound check1Path rfl (Guard257Trace1.run_check1 input) rfl rfl deployAddress_not_precompile)).trans
