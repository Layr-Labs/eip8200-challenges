...
theorem run_addBodyA (s : State) (mem : ByteArray) (p : UInt256) (n bsize esize msize k j : Nat)
    (hn32 : n ≤ 32) (hj : j < n) (hpv : p.toNat = 2112 + 32 * (n - 1 - j))
    (hact : 93 ≤ s.activeWords.toNat)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hrun : s.halt = .Running) :
...
theorem run_shiftDone (s : State) (mem : ByteArray) (n bsize esize msize : Nat)
  ...
  -- This block's high-water mark is fixed THREE times, not once: the `MLOAD` of `V_S32` at 2784
  ...
  have haw9344 : MachineState.activeWordsAfter s.activeWords.toNat 2784 32 =
      s.activeWords.toNat :=
    Monpro.activeWordsAfter_fix s.activeWords.toNat 2784 32 (by decide) (by omega) hact
