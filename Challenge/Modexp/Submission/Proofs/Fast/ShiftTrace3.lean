...
theorem run_mid_pos (s : State) (um : ByteArray) (q : UInt256) (n bsize esize msize k : Nat)
  ...
  have hTN : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat 4128 32) =
...
theorem run_addBodyA (s : State) (mem : ByteArray) (p : UInt256) (n bsize esize msize k j : Nat)
    (hn32 : n ≤ 32) (hj : j < n) (hpv : p.toNat = 4160 + 32 * (n - 1 - j))
    (hact : 168 ≤ s.activeWords.toNat)
    (hcode : s.executionEnv.code = Challenge.Modexp.submissionBytecode)
    (hrun : s.halt = .Running) :
  ...
  have hsub : (115792089237316195423570985008687907853269984665640564039457584007913129639936 + (4160 + 32 * (n - 1 - j)) - 4160) % 115792089237316195423570985008687907853269984665640564039457584007913129639936 = 32 * (n - 1 - j) := by
    rw [show 115792089237316195423570985008687907853269984665640564039457584007913129639936 + (4160 + 32 * (n - 1 - j)) - 4160 = 115792089237316195423570985008687907853269984665640564039457584007913129639936 + 32 * (n - 1 - j) by omega,
      Nat.add_mod_left]
...
theorem run_shiftDone (s : State) (mem : ByteArray) (n bsize esize msize : Nat)
    (hn : 1 ≤ n) (hn32 : n ≤ 32) (hact : 168 ≤ s.activeWords.toNat)
  ...
  -- This block's high-water mark is fixed THREE times, not once: the `MLOAD` of `V_S32` at 5248
  ...
  -- is the ready-made two-region lemma but wants `170 <= activeWords` where this block has 168;
  -- `Monpro.activeWordsAfter_fix` carries the 168 bound.
  have hactN : s.activeWords.toNat %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      s.activeWords.toNat := Nat.mod_eq_of_lt s.activeWords.val.isLt
  have haw9344 : MachineState.activeWordsAfter s.activeWords.toNat 5248 32 =
      s.activeWords.toNat :=
    Monpro.activeWordsAfter_fix s.activeWords.toNat 5248 32 (by decide) (by omega) hact
