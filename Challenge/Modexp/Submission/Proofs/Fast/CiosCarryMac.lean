...
import Challenge.Modexp.Submission.Proofs.Fast.CiosCarryL2
...
theorem run_l1Mac (pc : Nat) (off t : UInt256) (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    runInstructions (l1Program off t) (l1At (carrySlot := carrySlot) pc s mem bi pa pb n i j pdst ret rest) =
      some (l1At (carrySlot := carrySlot) (pc+38) s mem bi pa pb n i (j+1) pdst ret rest) := by
  have h := CiosCarryL1.run_step (carrySlot := carrySlot) s (UInt256.ofNat pc) mem bi pa n j off t hoff ht
...
theorem run_l1First (pc : Nat) (off t : UInt256) (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hpa : 32 ≤ pa) (hpaFit : pa + 32*n ≤ 9472) :
    runInstructions (l1FirstProgram off t) (l1At (carrySlot := carrySlot) pc s mem bi pa pb n i 0 pdst ret rest) =
      some (l1At (carrySlot := carrySlot) (pc+35) s mem bi pa pb n i 1 pdst ret rest) := by
  have h := CiosCarryL1.run_first (carrySlot := carrySlot) s (UInt256.ofNat pc) mem bi pa n off t hoff ht
    (UInt256.ofNat (ptrAt (pb+32*n-32) i))
...
theorem run_l1Last (t : UInt256) (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hpa : 32 ≤ pa) (hpaFit : pa + 32*n ≤ 9472) :
    runInstructions (l1LastProgram t) (l1At (carrySlot := carrySlot) 4502 s mem bi pa pb n i (n-1) pdst ret rest) =
      some (midState (carrySlot := carrySlot) s (l1Step mem bi pa n n).memory (l1Step mem bi pa n n).carry bi
        pa pb n i pdst ret rest) := by
  have h := CiosCarryL1.run_last (carrySlot := carrySlot) s (UInt256.ofNat 4502) mem bi pa n t ht
    (UInt256.ofNat (ptrAt (pb+32*n-32) i))
    (UInt256.ofNat (pb-32)) (l1Target n) (l2Target n) pdst (ret :: rest) (by simp only [List.length_cons]; omega) hact hn32 hpos hpa hpaFit
  have hpc : UInt256.ofNat 4502 + UInt256.ofNat 35 = UInt256.ofNat 4537 := by decide
  simpa only [List.cons_append, List.nil_append, CiosCarryL1.state, CiosCarryL1.doneState, l1At, midState, hpc] using h
...
theorem run_l2Mac (pc : Nat) (w : Fin 33) (x tl ts : UInt256)
    ...
    runInstructions (l2Program w x tl ts) (l2At (carrySlot := carrySlot) pc s mid bi mu c0 pa pb n i k pdst ret rest) =
      some (l2At (carrySlot := carrySlot) (pc+(w.val+35)) s mid bi mu c0 pa pb n i (k+1) pdst ret rest) := by
  have h := CiosCarryL2.run_step (carrySlot := carrySlot) w s (UInt256.ofNat pc) mid bi mu c0 n k x tl ts hx htl hts
  ...
  simpa only [List.cons_append, List.nil_append, CiosCarryL2.state, l2At,
    Challenge.EvmProof.Word.ofNat_add_mod] using h
