...
theorem run_peel (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock cios2L2Peel
      (l2At 4916 s mid bi mu c0 pa pb n i 0 pdst ret rest) =
      some (l2At 5053 s mid bi mu c0 pa pb n i 1 pdst ret rest) := by
...
opaque gasSteps_peel (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    (pa pb n i : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1008) (hrun : s.halt = .Running)
    ...
    Challenge.EvmProof.GasSteps
      (l2At 4916 s mid bi mu c0 pa pb n i 0 pdst ret rest)
      (l2At 5053 s mid bi mu c0 pa pb n i 1 pdst ret rest) :=
