...
theorem row_step (mem : ByteArray) (pa a m i : Nat)
  ...
  have hd' := represents_rows mem pa 2368 5 (2*a) (Or.inr (by decide)) hd i (by omega)
  have hm' := represents_rows mem pa 0 4 m (Or.inl (by decide)) hm i (by omega)
  have hiv : ((MachineState.readWord (rows mem pa i) 96).toNat *
      (MachineState.readWord (rows mem pa i) 2816).toNat+1) % 2^256 = 0 := by
...
theorem final_high_le_one (mem : ByteArray) (pa a m : Nat)
    ...
    (hinv : ((MachineState.readWord mem 96).toNat *
      (MachineState.readWord mem 2816).toNat+1) % 2^256 = 0) :
    (MachineState.readWord (rows mem pa 4) 2080).toNat ≤ 1 := by
...
theorem represents_result (mem : ByteArray) (pa pdst a m : Nat)
  ...
  exact Csub.csub_correct (rows mem pa 4) 4 (Csub.lowValue (rows mem pa 4) 2112 4 4) m
    (MachineState.readWord (rows mem pa 4) 2080).toNat pdst (by decide) (by decide)
    (Csub.fastRepresents_lowValue (rows mem pa 4) 2112 4) hm' rfl htn (by omega) hlt
