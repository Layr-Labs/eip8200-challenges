...
def Agree (a b : ByteArray) : Prop :=
  ∀ i, i < 4096 ∨ 4128 ≤ i → a[i]?.getD 0 = b[i]?.getD 0
...
theorem readPadded_eq {a b : ByteArray} (h : Agree a b) (start count : Nat)
    (hout : start + count ≤ 4096 ∨ 4128 ≤ start) :
...
theorem scratch_write (a : ByteArray) (bytes : ByteArray) (hlen : bytes.size = 32) :
    Agree (MachineState.writeBytes a bytes 4096) a := by
  intro i hi
...
theorem scratch_word (a : ByteArray) (word : UInt256) :
    Agree (MachineState.writeBytes a (Data.Bytes.natToBytesPadded word.toNat 32) 4096) a := by
  apply scratch_write
