...
private theorem r4Mem_eq_of_base (s : State) (mem : ByteArray) (W : R4Math.W5) :
  ...
  simp only [R4Bridge.r4Mem, MachineState.writeBytes_getElem?_getD,
    YulEvmCompiler.BytesLemmas.natToBytesPadded_size]
  by_cases h4 : 2080 ≤ i ∧ i < 2080 + 32
  · simp only [if_pos h4]
  simp only [if_neg h4]
  by_cases h3 : 2112 ≤ i ∧ i < 2112 + 32
  · simp only [if_pos h3]
  simp only [if_neg h3]
  by_cases h2 : 2144 ≤ i ∧ i < 2144 + 32
  · simp only [if_pos h2]
  simp only [if_neg h2]
  by_cases h1 : 2176 ≤ i ∧ i < 2176 + 32
  · simp only [if_pos h1]
  simp only [if_neg h1]
  by_cases h0 : 2208 ≤ i ∧ i < 2208 + 32
  · simp only [if_pos h0]
  simp only [if_neg h0]
  exact entry_byte_eq s mem i (by omega)
