...
theorem run_push (input : ByteArray) (condition : UInt256) :
    Challenge.EvmProof.Stepper.runLocated branchPushLocated
      (conditionState input 4970 condition) =
      some (jumpStackState input condition) := by
  have hpc : (conditionState input 4970 condition).pc.toNat =
      Artifact.submissionArtifact.instructionPC 2231 := by
    simp [conditionState, initialState, gEip2PC0, gEip2PC1, gEip2PC2, Challenge.EvmProof.Word.word_toNat_ofNat]
  unfold Challenge.EvmProof.Stepper.runLocated
  change (if (conditionState input 4970 condition).pc.toNat =
      Artifact.submissionArtifact.instructionPC 2231 then
      Challenge.EvmProof.Stepper.runInstr (.push 2 (UInt256.ofNat 4978))
        (conditionState input 4970 condition) else none) = _
  rw [if_pos hpc]
  have hcap : (conditionState input 4970 condition).stack.length < 1024 := by
    simp [conditionState]
  have hwidth : ¬ (2 : Fin 33).val = 0 := by decide
  have hthree : (2 : Fin 33).val + 1 = 3 := by decide
  have hadd : UInt256.ofNat 4970 + UInt256.ofNat 3 = UInt256.ofNat 4973 :=
