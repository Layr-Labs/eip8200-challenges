...
/-- Kernel `setup` entry (pc 4177, 0x0f6c), reached from `common` with the row head
...
/-- After the first loop, at the middle block's `JUMPDEST` (pc 4553): the carry and
