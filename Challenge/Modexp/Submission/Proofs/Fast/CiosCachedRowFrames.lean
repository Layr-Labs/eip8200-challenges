...
/-- Kernel `setup` entry (pc 4187, 0x0f6c), reached from `common` with the row head
...
/-- After the first loop, at the middle block's `JUMPDEST` (pc 4563): the carry and
