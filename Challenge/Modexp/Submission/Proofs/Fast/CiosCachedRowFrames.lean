...
/-- Kernel `setup` entry (pc 4173, 0x0f6c), reached from `common` with the row head
...
/-- After the first loop, at the middle block's `JUMPDEST` (pc 4550): the carry and
