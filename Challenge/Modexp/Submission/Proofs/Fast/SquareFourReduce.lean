...
theorem reduction_low (mem : ByteArray) (f : UInt256) :
    Csub.lowValue (reduction mem f) 8256 4 4 =
  ...
  rw [reduction, read_tail_top,
    readWord_l2Step_low _ _ _ 4 8224 3 (by decide)]
...
theorem reduction_equation (mem : ByteArray) (f : UInt256) (m : Nat)
    ...
    tValue (reduction mem f) 4 * Limbs.radix =
      (f.toNat*Limbs.radix+(MachineState.readWord mem 8224).toNat)*Limbs.radix^4 +
        Csub.lowValue mem 8256 4 4 + (rowMu mem 4).toNat*m := by
  let p := l2Step mem (rowMu mem 4) (rowC0 mem 4) 4 3
  ...
  have htn : (MachineState.readWord (reduction mem f) 8224).toNat = f.toNat+cv.toNat := by
    rw [reduction, read_tail_high, readWord_l2Step_low _ _ _ 4 8224 3 (by decide)]
  ...
  have hC : (rowC0 mem 4).toNat*Limbs.radix =
      (MachineState.readWord mem 8352).toNat +
        (MachineState.readWord mem 96).toNat*(rowMu mem 4).toNat :=
    c0_spec (MachineState.readWord mem 96) (MachineState.readWord mem 9376)
      (MachineState.readWord mem 8352) hinv
  have hD := l2_invariant mem (rowMu mem 4) (rowC0 mem 4) 4 (by decide) 3 (by decide)
  have hE : limbSum (fun k =>
          (MachineState.readWord mem (8256+32*(4-2-k))).toNat) 3*Limbs.radix +
        (MachineState.readWord mem 8352).toNat = Csub.lowValue mem 8256 4 4 := by
    have he := limbSum_shift (fun k =>
      (MachineState.readWord mem (8256+32*(4-1-k))).toNat) 3
    have haddr : (fun k => (MachineState.readWord mem (8256+32*(4-1-(k+1)))).toNat) =
        (fun k => (MachineState.readWord mem (8256+32*(4-2-k))).toNat) := by
      funext k
      congr 2
      omega
