...
theorem frame_stepMem {mem : ByteArray} {n bsize mm minv : Nat} (hn : 1 ≤ n) (hn32 : n ≤ 8)
  ...
  ml := by
    rw [stepMem_readWord_disjoint mem n mm 2848 hn
      ⟨Or.inr (by omega), Or.inr (by omega), Or.inr (by omega)⟩]; exact hf.ml
  tl := by
    rw [stepMem_readWord_disjoint mem n mm 2880 hn
      ⟨Or.inr (by omega), Or.inr (by omega), Or.inr (by omega)⟩]; exact hf.tl
  eoff := by
    rw [stepMem_readWord_disjoint mem n mm 2912 hn
...
theorem stepInv_stepMems {mem : ByteArray} {n bsize mm minv : Nat} (hn : 1 ≤ n) (hn32 : n ≤ 8)
...
theorem m1_base (mem input : ByteArray) (n mm : Nat) (hn : 2 ≤ n) (hn32 : n ≤ 8)
  ...
  have h := Csub.csub_correct (hitMem mem input n) n _ mm 0 512 hn hn32 hts hmodH htn0 (Nat.zero_le 1)
    hmpos hbound
...
theorem m2_stepInv (mem input : ByteArray) (n bsize mm minv : Nat)
  ...
  have hframe2 : Exp.Frame (m2Of mem input n) n bsize minv :=
    ⟨by rw [hm2high 2784 le_rfl]; exact hframe.s32,
     by rw [hm2high 2816 (by omega)]; exact hframe.minvW,
     by rw [hm2high 2848 (by omega)]; exact hframe.ml,
     by rw [hm2high 2880 (by omega)]; exact hframe.tl,
     by rw [hm2high 2912 (by omega)]; exact hframe.eoff⟩
...
def gasSteps_hitPath (s : State) (mem input : ByteArray) (n bsize esize msize mm minv : Nat)
    ...
    Challenge.EvmProof.GasSteps (dispState s mem n bsize esize msize)
      { Exp.bDone s (hitFinalMem mem input n mm) n bsize esize msize with pc := UInt256.ofNat 3179 } := by
  ...
  have hpro := gasSteps_prologue s (m1Of mem input n) n bsize esize msize (by omega) hn32 e
    (by rw [hm1high 2848 (by omega)]; exact hframe.ml)
  ...
  have hexit : Challenge.EvmProof.GasSteps
      ...
      { Exp.bDone s (hitFinalMem mem input n mm) n bsize esize msize with pc := UInt256.ofNat 3179 } :=
    (soundEnv blk3013 e
