...
def gasSteps_hitPath (s : State) (mem input : ByteArray) (n bsize esize msize mm minv : Nat)
    ...
    Challenge.EvmProof.GasSteps (dispState s mem n bsize esize msize)
      { Exp.bDone s (hitFinalMem mem input n mm) n bsize esize msize with pc := UInt256.ofNat 3146 } := by
  ...
  have hexit : Challenge.EvmProof.GasSteps
      ...
      { Exp.bDone s (hitFinalMem mem input n mm) n bsize esize msize with pc := UInt256.ofNat 3146 } :=
    (soundEnv blk3013 e
