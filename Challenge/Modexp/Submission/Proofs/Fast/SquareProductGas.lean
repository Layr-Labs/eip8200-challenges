...
def gasSteps_product (s : State) (mem : ByteArray) (ai : UInt256) (i : Nat)
    ...
    Challenge.EvmProof.GasSteps (stateAt s mem 5186 (base pbi pa pb tag dst ret rest))
      (stateAt s (SquareRowModel.mid mem ai i) 4496
  ...
  have hguard : Challenge.EvmProof.GasSteps
      ...
      (partState s mem ai i 1 (if i = 7 then 5290 else 5231) pbi pa pb (delta i) dst ret rest) := by
    apply SquareControls.guardBlock.steps (s := partState s mem ai i 1 5225 pbi pa pb (delta i) dst ret rest)
    ...
    have h := SquareControls.run_guard {s with memory := (P 1).memory} (P 1).carry ai pbi pa pb (delta i)
      ...
      (by change Decode.isValidJumpDest s.executionEnv.code 5290 = true; rw [env.code]; exact jump_last)
  ...
  · subst i
    ...
    have hl : Challenge.EvmProof.GasSteps (stateAt s (P 1).memory 5290 ((P 1).carry :: ai :: bs))
        (stateAt s (storeWord (P 1).memory 2080 (MachineState.readWord (P 1).memory 2080+(P 1).carry)) 4493
          (UInt256.lt (MachineState.readWord (P 1).memory 2080+(P 1).carry) (P 1).carry :: ai :: bs)) :=
    ...
    have hm := SquareControls.muBlock.steps
      (env.transfer (t := stateAt s (storeWord (P 1).memory 2080 (MachineState.readWord (P 1).memory 2080+(P 1).carry))
        4493 (UInt256.lt (MachineState.readWord (P 1).memory 2080+(P 1).carry) (P 1).carry :: ai :: bs)) rfl rfl) rfl
  ...
  · have hin : i < 7 := by omega
    have hj : Decode.isValidJumpDest s.executionEnv.code (SquareControls.target (delta i)).toNat = true := by
      ...
      · exact Artifact.isValidJumpDest_index 3241 (by rfl)
      · exact Artifact.isValidJumpDest_index 3272 (by rfl)
      · exact Artifact.isValidJumpDest_index 3303 (by rfl)
      · exact Artifact.isValidJumpDest_index 3334 (by rfl)
      · exact Artifact.isValidJumpDest_index 3365 (by rfl)
      · exact Artifact.isValidJumpDest_index 3396 (by rfl)
    have hd : Challenge.EvmProof.GasSteps
        (partState s mem ai i 2 5278 pbi pa pb (delta i) dst ret rest)
        (partState s mem ai i 2 (4162+38*(i+2)) pbi pa pb (delta i) dst ret rest) := by
      apply SquareControls.dispatchBlock.steps (s := partState s mem ai i 2 5278 pbi pa pb (delta i) dst ret rest)
        (env.transfer rfl rfl) rfl
    ...
    have he : ai*MachineState.readWord (P (8-i)).memory 2368 = SquareRowModel.extra mem ai i := by
      rw [read_products_outside mem _ ai i 2368 (Or.inr (by decide)) (8-i) (by omega)]
    ...
    have ht' : Challenge.EvmProof.GasSteps
        (partState s mem ai i (8-i) 4466 pbi pa pb (delta i) dst ret rest)
        (stateAt s (SquareRowModel.mid mem ai i) 4496
