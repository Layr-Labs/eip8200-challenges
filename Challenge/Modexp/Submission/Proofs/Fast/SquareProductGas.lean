...
def gasSteps_product (s : State) (mem : ByteArray) (ai : UInt256) (i : Nat)
  ...
  · subst i
    ...
    have hl : Challenge.EvmProof.GasSteps (stateAt s (P 1).memory 5290 ((P 1).carry :: ai :: bs))
        (stateAt s (storeWord (P 1).memory 8224 (MachineState.readWord (P 1).memory 8224+(P 1).carry)) 4493
    ...
    have hm := SquareControls.muBlock.steps
      (env.transfer (t := stateAt s (storeWord (P 1).memory 8224 (MachineState.readWord (P 1).memory 8224+(P 1).carry))
        4493 (UInt256.lt (MachineState.readWord (P 1).memory 8224+(P 1).carry) (P 1).carry :: ai :: bs)) rfl rfl) rfl
