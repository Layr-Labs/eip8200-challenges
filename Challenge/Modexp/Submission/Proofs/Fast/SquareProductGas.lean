...
def gasSteps_product (s : State) (mem : ByteArray) (ai : UInt256) (i : Nat)
  ...
  have hguard : Challenge.EvmProof.GasSteps
      ...
      (partState s mem ai i 1 (if i = 7 then 5294 else 5235) pbi pa pb (delta i) dst ret rest) := by
    apply SquareControls.guardBlock.steps (s := partState s mem ai i 1 5229 pbi pa pb (delta i) dst ret rest)
    ...
    have h := SquareControls.run_guard {s with memory := (P 1).memory} (P 1).carry ai pbi pa pb (delta i)
      ...
      (by change Decode.isValidJumpDest s.executionEnv.code 5294 = true; rw [env.code]; exact jump_last)
  ...
  · subst i
    ...
    have hl : Challenge.EvmProof.GasSteps (stateAt s (P 1).memory 5294 ((P 1).carry :: ai :: bs))
        (stateAt s (storeWord (P 1).memory 2080 (MachineState.readWord (P 1).memory 2080+(P 1).carry)) 4497
    ...
    have hm := SquareControls.muBlock.steps
      (env.transfer (t := stateAt s (storeWord (P 1).memory 2080 (MachineState.readWord (P 1).memory 2080+(P 1).carry))
        4497 (UInt256.lt (MachineState.readWord (P 1).memory 2080+(P 1).carry) (P 1).carry :: ai :: bs)) rfl rfl) rfl
  ...
  · have hin : i < 7 := by omega
    have hj : Decode.isValidJumpDest s.executionEnv.code (SquareControls.target (delta i)).toNat = true := by
      ...
      interval_cases i
      · exact Artifact.isValidJumpDest_index 3215 (by rfl)
      · exact Artifact.isValidJumpDest_index 3247 (by rfl)
      · exact Artifact.isValidJumpDest_index 3279 (by rfl)
      · exact Artifact.isValidJumpDest_index 3311 (by rfl)
      · exact Artifact.isValidJumpDest_index 3343 (by rfl)
      · exact Artifact.isValidJumpDest_index 3375 (by rfl)
      · exact Artifact.isValidJumpDest_index 3407 (by rfl)
