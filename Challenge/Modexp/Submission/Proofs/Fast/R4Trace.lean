...
* `gasSteps_red`：共享约化子程序（入口 5159，返回到栈上的 `ret`），对任意窗口 `P`、
  溢出位 `v` 成立。
...
theorem jd5158 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 5136 = true :=
...
/-- **RED**：`[ret, v] ++ win P ++ K` 在 5159 → `win (red P v) ++ K` 在 `ret`。 -/
def gasSteps_red (ret v : UInt256) (P : W5) (k n0 n1 n2 n3 np : UInt256) (rest : List UInt256)
    ...
    GasSteps { s with pc := UInt256.ofNat 5136,
...
/-- 行 0：`K` 在 4804（序言之后）→ `[4907, 0] ++ win (row0 …) ++ K` 在 5159。 -/
def gasSteps_row0 (k n0 n1 n2 n3 np a0 a1 a2 a3 : UInt256) (rest : List UInt256)
  ...
  have hjd : Decode.isValidJumpDest s.executionEnv.code 5136 = true := by rw [hcode]; exact jd5158
  simp only [win]
...
/-- 行 1：`win P ++ K` 在 4907 → `[5023, v] ++ win (row1 …).1 ++ K` 在 5159。 -/
def gasSteps_row1 (P : W5) (k n0 n1 n2 n3 np a0 a1 a2 a3 : UInt256) (rest : List UInt256) (hk : k = Monpro.maxWord)
    ...
    GasSteps { s with pc := UInt256.ofNat 4896, stack := win P (k :: n0 :: n1 :: n2 :: n3 :: np :: rest) }
      { s with pc := UInt256.ofNat 5136,
               stack := (5008 : UInt256) :: (row1 a0 a1 a2 a3 k P).2 ::
                 win (row1 a0 a1 a2 a3 k P).1 (k :: n0 :: n1 :: n2 :: n3 :: np :: rest) } := by
  have hjd : Decode.isValidJumpDest s.executionEnv.code 5136 = true := by rw [hcode]; exact jd5158
  simp only [win]
  seg block_r1h (run_r1h s (hcap := by lenOK) (ha1 := ha1) (ha0 := ha0) (hact := hact) ..)
  refine GasSteps.trans (gasSteps_sgtAt 3713 4906 (by rfl) (by rfl) (by norm_num) s _ _ _
...
/-- 行 2：`win P ++ K` 在 5023 → `[5110, v] ++ win (row2 …).1 ++ K` 在 5159。 -/
def gasSteps_row2 (P : W5) (k n0 n1 n2 n3 np a1 a2 a3 : UInt256) (rest : List UInt256) (hk : k = Monpro.maxWord)
    (hcap : rest.length ≤ 1003) (hact : 88 ≤ s.activeWords.toNat)
    (ha1 : MachineState.readWord s.memory 2432 = a1)
    (ha2 : MachineState.readWord s.memory 2400 = a2)
    (ha3 : MachineState.readWord s.memory 2368 = a3) :
    GasSteps { s with pc := UInt256.ofNat 5008, stack := win P (k :: n0 :: n1 :: n2 :: n3 :: np :: rest) }
      { s with pc := UInt256.ofNat 5136,
               stack := (5091 : UInt256) :: (row2 a1 a2 a3 k P).2 ::
                 win (row2 a1 a2 a3 k P).1 (k :: n0 :: n1 :: n2 :: n3 :: np :: rest) } := by
  have hjd : Decode.isValidJumpDest s.executionEnv.code 5136 = true := by rw [hcode]; exact jd5158
  simp only [win]
  seg block_r2h (run_r2h s (hcap := by lenOK) (ha2 := ha2) (ha1 := ha1) (hact := hact) ..)
  refine GasSteps.trans (gasSteps_sgtAt 3813 5018 (by rfl) (by rfl) (by norm_num) s _ _ _
    hcode hfork (by lenOK) hrun hnp) ?_
...
/-- 行 3：`win P ++ K` 在 5110 → `[5275, v] ++ win (row3 …).1 ++ K` 落入 5159。 -/
def gasSteps_row3 (P : W5) (k n0 n1 n2 n3 np a2 a3 : UInt256) (rest : List UInt256) (hk : k = Monpro.maxWord)
    (hcap : rest.length ≤ 1003) (hact : 88 ≤ s.activeWords.toNat)
    (ha2 : MachineState.readWord s.memory 2400 = a2)
    (ha3 : MachineState.readWord s.memory 2368 = a3) :
    GasSteps { s with pc := UInt256.ofNat 5091, stack := win P (k :: n0 :: n1 :: n2 :: n3 :: np :: rest) }
      { s with pc := UInt256.ofNat 5136,
               stack := (5242 : UInt256) :: (row3 a2 a3 k P).2 ::
                 win (row3 a2 a3 k P).1 (k :: n0 :: n1 :: n2 :: n3 :: np :: rest) } := by
  simp only [win]
  seg block_r3h (run_r3h s (hcap := by lenOK) (ha3 := ha3) (ha2 := ha2) (hact := hact) ..)
  refine GasSteps.trans (gasSteps_sgtAt 3886 5101 (by rfl) (by rfl) (by norm_num) s _ _ _
    hcode hfork (by lenOK) hrun hnp) ?_
...
def gasSteps_r4 (s : State) (e0 e1 e2 e3 e4 e5 e6 e7 e8 e9 e10 e11 e12 : UInt256)
  ...
  have g4 := gasSteps_red s hcode hfork hrun hnp 5008 R1.2 R1.1 e5 e8 e11 e12 n3 e7 E hE (hj _ jd5020) hk hminv hguard
  have g5 := gasSteps_row2 (hk := hk) s hcode hfork hrun hnp W2 e5 e8 e11 e12 n3 e7 a1 a2 a3 E hE hact ha1 ha2 ha3
  have g6 := gasSteps_red s hcode hfork hrun hnp 5091 R2.2 R2.1 e5 e8 e11 e12 n3 e7 E hE (hj _ jd5108) hk hminv hguard
  have g7 := gasSteps_row3 (hk := hk) s hcode hfork hrun hnp W3 e5 e8 e11 e12 n3 e7 a2 a3 E hE hact ha2 ha3
  have g8 := gasSteps_red s hcode hfork hrun hnp 5242 R3.2 R3.1 e5 e8 e11 e12 n3 e7 E hE (hj _ jd5274) hk hminv hguard
