...
Instruction 0 enters the early one-word dispatcher at pc 5243. Matching headers
...
private noncomputable def chosenData (F : FastPath) (input : ByteArray) (hvalid : ValidInput input) :
  ...
  if hmatch : WindowTwentyOneInput.Matches input then
    ...
    if hzero : EarlyWordSmallExp.ZeroExpGuard input then
      ⟨Classical.choose (EarlyWordCorrect.smallExp input hvalid hmatch hzero),
        Classical.choose_spec
          (EarlyWordCorrect.smallExp input hvalid hmatch hzero)⟩
    else
      if h : F.Handles input then
        ⟨Classical.choose (F.handled input hvalid h),
          ⟨⟨(EarlyWordCorrect.legacy input hmatch hzero).trans
        ...
        let completed := WindowBodyCorrect.handledOf WindowTwentyOneCorrect.route
          input hvalid
          ((EarlyWordCorrect.legacy input hmatch hzero).trans (F.bail input hvalid h))
