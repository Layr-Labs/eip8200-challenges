...
`CCB` occupies instruction indices 1737..1762 (pc 2856..2893).  It is entered
at pc 2856 with stack `[px, ret]`, exactly the calling convention of
...
* `blk1742` (idx 1737..1743, pc 2856..2866) — `JUMPDEST`, then the call frame
  `[px, px, px, 2867]` and a jump to `ADDMOD` (pc 2460);
* `blk1749` (idx 1744..1745, pc 2867..2869) — `JUMPDEST; PUSH1 8`;
* `blk1751` (idx 1746..1752, pc 2870..2880) — the loop head `CCL`, which
  pushes the call frame `[px, px, px, 2881]` and jumps to `MONPRO` (pc 1939);
* `blk1758` (idx 1753..1759, pc 2881..2890) — the return point, which
  decrements the counter and jumps back to pc 2870 while it is nonzero;
* `blk1765` (idx 1760..1762, pc 2891..2893) — `POP; POP; JUMP ret`.
...
/-- Subroutine entry, pc 2856, stack `[px, ret]`. -/
...
/-- The prologue `ADDMOD` call, pc 2460, frame `[px, px, px, 2867]`. -/
def amCallState (s : State) (mem : ByteArray) (px : Nat) (ret : UInt256)
  ...
  { s with pc := UInt256.ofNat 2460
           stack := [UInt256.ofNat px, UInt256.ofNat px, UInt256.ofNat px,
                     UInt256.ofNat 2867] ++ ([UInt256.ofNat px, ret] ++ rest)
           memory := mem }

/-- The prologue return point, pc 2867, stack `[px, ret]`. -/
def postState (s : State) (mem : ByteArray) (px : Nat) (ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2867
           stack := [UInt256.ofNat px, ret] ++ rest
           memory := mem }

/-- The loop head `CCL`, pc 2870, with the counter at `k`. -/
def loopState (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2870
           stack := loopStack px k ret rest
           memory := mem }

/-- The `MONPRO` call, pc 1939, with the frame `[px, px, px, 2881]` pushed. -/
def mpCallState (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 1939
           stack := [UInt256.ofNat px, UInt256.ofNat px, UInt256.ofNat px,
                     UInt256.ofNat 2881] ++ loopStack px k ret rest
           memory := mem }

/-- The return point, pc 2881, with the counter still at `k`. -/
def retState (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2881
           stack := loopStack px k ret rest
           memory := mem }

/-- The loop exit, pc 2891, with the counter at zero. -/
def exitState (s : State) (mem : ByteArray) (px : Nat) (ret : UInt256)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2891
...
/-- `blk1742` (pc 2856..2866): push the `ADDMOD` frame and jump to pc 2460. -/
theorem run_entry (s : State) (mem : ByteArray) (px : Nat) (ret : UInt256)
  ...
  have h2867 : (2867 : UInt256) = UInt256.ofNat 2867 := by decide
  ...
  simp (config := { maxSteps := 400000 }) [blk1742, opAt, pushAt, wfOp,
    ...
    h2867, h2460, h2460Nat, jumpDest2467,
...
set_option linter.unusedSimpArgs false in
/-- `blk1749` (pc 2867..2869): push the squaring counter. -/
...
set_option linter.unusedSimpArgs false in
/-- `blk1751` (pc 2870..2880): push the `MONPRO` frame and jump to pc 1939. -/
theorem run_call (s : State) (mem : ByteArray) (px k : Nat) (ret : UInt256)
  ...
  have h2881 : (2881 : UInt256) = UInt256.ofNat 2881 := by decide
  ...
  simp (config := { maxSteps := 400000 }) [blk1751, opAt, pushAt, wfOp,
    ...
    hcode, hrun, h2881, h1939, h1939Nat, jumpDest1939,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat]
...
set_option linter.unusedSimpArgs false in
/-- `blk1758` (pc 2881..2890), counter above one: decrement and loop. -/
theorem run_ret (s : State) (mem : ByteArray) (px k k' : Nat) (ret : UInt256)
  ...
  have h2870 : (2870 : UInt256) = UInt256.ofNat 2870 := by decide
  have h2870Nat : (UInt256.ofNat 2870).toNat = 2870 := by decide
  ...
  simp (config := { maxSteps := 400000 }) [blk1758, opAt, pushAt, wfOp,
    ...
    hzero, h2870, h2870Nat, hdec, htrue, jumpDest2877, List.exchange,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat]
...
set_option linter.unusedSimpArgs false in
/-- `blk1758` (pc 2881..2890), counter one: fall through to the exit. -/
theorem run_retLast (s : State) (mem : ByteArray) (px : Nat) (ret : UInt256)
  ...
  simp (config := { maxSteps := 400000 }) [blk1758, opAt, pushAt, wfOp,
    ...
    hzero, h2870, hdec, hfalse, List.exchange,
    Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat]
...
set_option linter.unusedSimpArgs false in
/-- `blk1765` (pc 2891..2893): pop the frame and return to the caller. -/
