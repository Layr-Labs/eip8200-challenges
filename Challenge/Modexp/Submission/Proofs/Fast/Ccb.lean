...
* `blk1742` (idx 1742..1748, pc 2863..2873) — `JUMPDEST`, then the call frame
  `[px, px, px, 2874]` and a jump to `ADDMOD` (pc 5267);
...
/-- The prologue `ADDMOD` call, pc 5267, frame `[px, px, px, 2874]`. -/
...
/-- `blk1742` (pc 2863..2873): push the `ADDMOD` frame and jump to pc 5267. -/
theorem run_entry (s : State) (mem : ByteArray) (px : Nat) (ret : UInt256)
  ...
  simp (config := { maxSteps := 400000 }) [blk1742, opAt, pushAt, wfOp,
    ...
    h2874, h5267, h5267Nat, jumpDest5267,
