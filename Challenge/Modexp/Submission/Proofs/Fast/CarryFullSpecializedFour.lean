...
opaque gasSteps_specializedFour (E : EntryLemmas) (s : State) (mem : ByteArray)
    ...
    Challenge.EvmProof.GasSteps
      ...
      (mpCsubState s (rowsCarry (mpZeroed s (stage mem pa 4) 4) pa pb 4 4) pdst
        (UInt256.ofNat GenericReturnAdapter.genericShimPC)
        (TnM128InitialMultiply.retainedFrame s (stage mem pa 4) pa pb 4
  ...
  refine (E.gasSteps_mulEntry s mem pa pb 4 pdst ret rest hcap hrun hcode hfork hnp
    hact (by decide) hcds hs32).trans ?_
  have htail : (ret :: rest).length ≤ 998 := by simp; omega
  refine (E.gasSteps_commonSetup s mem (UInt256.ofNat 3543) pa pb 4 pdst
