...
/-- A four-limb multiply: `mul entry` (pc 4077) → `common` → `setup` → the four rows (row head
...
opaque gasSteps_specializedFour (L : RowLemmas) (E : EntryLemmas) (s : State) (mem : ByteArray)
  ...
  refine (E.gasSteps_commonSetup s mem (UInt256.ofNat 4266) pa pb 4 pdst ret rest hcap hrun hcode
