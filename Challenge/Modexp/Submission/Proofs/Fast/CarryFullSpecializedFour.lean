...
/-- A four-limb multiply: `mul entry` (pc 4006) → `common` → `setup` → the four rows (row head
...
opaque gasSteps_specializedFour (E : EntryLemmas) (s : State) (mem : ByteArray)
  ...
  refine (E.gasSteps_commonSetup s mem (UInt256.ofNat 3711) pa pb 4 pdst ret rest hcap hrun hcode
