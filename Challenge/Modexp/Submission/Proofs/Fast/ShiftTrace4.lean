...
theorem Env.act296 {s : State} (e : Env s) : 91 ≤ s.activeWords.toNat :=
...
def gasSteps_addRounds (s : State) (mem : ByteArray) (n bsize esize msize k c : Nat)
  ...
  have htl' : ∀ i, MachineState.readWord (addRounds mem n i) 2880 =
      UInt256.ofNat (2080 + 32 * n) := fun i => by
    rw [addRounds_readWord_disjoint mem n 2880 (Or.inr (by omega)) i]; exact htl
  Challenge.EvmProof.GasSteps.cast
    ((Challenge.EvmProof.GasSteps.iterateBounded
...
def gasSteps_subRounds (s : State) (mem : ByteArray) (n bsize esize msize k c : Nat)
  ...
  have htl' : ∀ i, MachineState.readWord (subRounds mem n i) 2880 =
      UInt256.ofNat (2080 + 32 * n) := fun i => by
    rw [subRounds_readWord_disjoint mem n 2880 (Or.inr (by omega)) i]; exact htl
  (Challenge.EvmProof.GasSteps.iterateBounded
...
def gasSteps_csubStep (s : State) (mem : ByteArray) (n bsize esize msize k : Nat)
  ...
  have hs32' : MachineState.readWord (Csub.csStep mem n n).memory 2784 =
      UInt256.ofNat (32 * n) := by
    rw [Csub.csStep_readWord_disjoint mem n 2784 (by omega) (Or.inr (by omega)) n le_rfl]
    exact hs32
...
def gasSteps_hitCsub (s : State) (mem input : ByteArray) (n bsize esize msize : Nat)
  ...
  have hs32' : MachineState.readWord (Csub.csStep (hitMem mem input n) n n).memory 2784 =
      UInt256.ofNat (32 * n) := by
  ...
  Challenge.EvmProof.GasSteps.cast
    ((soundEnv blk2874 e
      ...
      (Csub.gasSteps_csub s (hitMem mem input n) n (UInt256.ofNat 512)
        ...
        (by rw [hhigh 2848 (by omega)]; exact hml)
        (by rw [hhigh 2880 (by omega)]; exact htl) hs32'
        (by rw [show (UInt256.ofNat 512).toNat = 512 by decide]; omega) htn'))
    rfl (by
      rw [Csub.csReturnedState_eq_result,
        show (UInt256.ofNat 512).toNat = 512 by decide]
...
def gasSteps_step (s : State) (mem : ByteArray) (n bsize esize msize k mm minv : Nat)
  ...
  have htl0 : MachineState.readWord mem 2880 = UInt256.ofNat (2080 + 32 * n) := inv.frame.tl
  have hml0 : MachineState.readWord mem 2848 = UInt256.ofNat (32 * n - 32) := inv.frame.ml
  have hs320 : MachineState.readWord mem 2784 = UInt256.ofNat (32 * n) := inv.frame.s32
  have hhighU : ∀ addr, 2784 ≤ addr →
    ...
    (((soundEnv blk3013 e
      ...
      (soundEnv blk3069 e
        (run_macSetup s mem n bsize esize msize k (by omega) hn32 e.act296
          (by rw [hhighU 2880 (by omega)]; exact htl0)
          (by rw [hhighU 2848 (by omega)]; exact hml0) e.code e.run))
  ...
  have htlMid : MachineState.readWord (stepMid mem n) 2880 = UInt256.ofNat (2080 + 32 * n) := by
    rw [hhighMid 2880 (by omega)]; exact htl0
  have htlA : MachineState.readWord
      (addRounds (stepMid mem n) n (addCount (stepMid mem n) n mm (stepNeg mem n))) 2880 =
      UInt256.ofNat (2080 + 32 * n) := by
    rw [addRounds_readWord_disjoint (stepMid mem n) n 2880 (Or.inr (by omega))]; exact htlMid
  ...
  have g4 : Challenge.EvmProof.GasSteps
    ...
    gasSteps_csubStep s (fixMem (stepMid mem n) n mm (stepNeg mem n)) n bsize esize msize k
      hn hn32 e
      (by rw [hhighFix 2848 (by omega)]; exact hml0)
      (by rw [hhighFix 2880 (by omega)]; exact htl0)
      (by rw [hhighFix 2784 le_rfl]; exact hs320)
      (by rw [rf'.subDoneTn]; decide)
  have g5 : Challenge.EvmProof.GasSteps
      (afterCsubState s (Csub.csResultMemory (fixMem (stepMid mem n) n mm (stepNeg mem n)) n 512)
        n bsize esize msize k)
      (shiftLoopState s (Csub.csResultMemory (fixMem (stepMid mem n) n mm (stepNeg mem n)) n 512)
