...
noncomputable def reduction_steps (s : State)
    ...
    GasSteps
      (framed {s with memory := q.memory} (UInt256.ofNat 5291)
  ...
  have hmid := TnCacheRowTrace.run_middle 5291 st q.carry bi pbi hd pb ent tn
    m128 tl inv m0 aEnd m96 m64 m32 dst ret 4 rest hcap (by omega) hact hc hi
  have hmidRun : runInstructions (TnCacheRowTrace.middle ++ [.push 2 3812, .op .JUMP])
      (framed st (UInt256.ofNat 5291)
        ([q.carry,bi,pbi,hd,pb,ent,tn,allOnes,m128,inv,m0,tl,m96,m64,m32,aEnd,dst,ret] ++ rest)) =
  ...
  have g0 := TnM128CandidateBlocks.middleCopy.steps
    (s := framed st (UInt256.ofNat 5291)
  ...
  have hpc : (TnCacheLastTrace.lastState st
    ...
    change UInt256.ofNat (l2PC+TnMod128Chain.fullSize 4) = UInt256.ofNat 3912
    rfl
