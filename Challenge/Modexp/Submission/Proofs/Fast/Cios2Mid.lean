...
import Challenge.Modexp.Submission.Proofs.Bytecode.Cios2RoundedCarry
...
theorem run_mid (s : State) (mem : ByteArray) (paj ptj c bi : UInt256)
  ...
  have hactM0 : UInt256.ofNat
      ...
      s.activeWords := activeWords_fix s _ 32 (by decide) (by omega) hact
  have hrounded (mm lo : UInt256) :
      UInt256.gt mm (({ val := 0 } : UInt256).lnot + lo) + mm - lo =
        UInt256.isZero (UInt256.isZero lo) + (mm - (lo + UInt256.lt mm lo)) := by
    rw [← zero_lt_eq_double_isZero]
    exact (Cios2RoundedCarry.rounded_high mm lo).symm
  simp (config := { maxSteps := 800000 })
    [cios2Mid, Cios2Paths.Mid.midPC, Cios2Paths.Mid.startIndex,
      ...
      zero_lt_eq_double_isZero, hrounded,
