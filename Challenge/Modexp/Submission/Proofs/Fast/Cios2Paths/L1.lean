...
@[simp] theorem secondPC (index : Nat) (hlo : secondStartIndex ≤ index)
    ...
    Artifact.submissionArtifact.instructionPC index =
       ...
       4676, 4679, 4680, 4681, 4682, 4683, 4684, 4685, 4686, 4687, 4688, 4689, 4690, 4691,
       4692, 4693, 4694, 4695][index - secondStartIndex]! := by
  calc
    Artifact.submissionArtifact.instructionPC index =
        Artifact.submissionArtifact.instructionPC
...
/- Second MAC: instructions 2853..2897, pc 4536..4695. -/
def secondMac :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
...
/-- The exit trace jumps directly to the final landing and skips dead padding. -/
def secondMacExit :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  secondMac ++
  [secondPushAt 42 2 4695,
   secondOpAt 43 .JUMP,
