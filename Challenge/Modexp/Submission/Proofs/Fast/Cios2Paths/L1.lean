...
@[simp] theorem firstPC (index : Nat) (hlo : firstStartIndex ≤ index)
    ...
    Artifact.submissionArtifact.instructionPC index =
      [4136,4137,4138,4139,4172,4173,4174,4175,4176,4177,4178,4179,4180,4181,4182,4183,4184,4185,4186,4187,4188,4189,4190,4191,4192,4193,4194,4195,4196,4197,4198,4199,4200,4233,4234,4235,4236,4269][index - firstStartIndex]! := by
  calc
    Artifact.submissionArtifact.instructionPC index =
...
/-- First MAC: instructions 2726..2763, pc 4136..4269. -/
def firstMac :
...
@[simp] theorem middleOnePC (index : Nat) (hlo : middleOneStartIndex ≤ index)
    (hhi : index ≤ 2800) :
    Artifact.submissionArtifact.instructionPC index =
      [4270,4271,4272,4305,4306,4307,4308,4309,4310,4311,4312,4313,4314,4315,4316,4317,4318,4319,4320,4321,4322,4323,4324,4325,4326,4327,4328,4329,4330,4331,4332,4333,4366,4367,4368,4369,4402][index - middleOneStartIndex]! := by
  calc
    Artifact.submissionArtifact.instructionPC index =
        Artifact.submissionArtifact.instructionPC
...
/-- MiddleOne MAC: instructions 2764..2800, pc 4270..4402. -/
def middleOneMac :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
...
@[simp] theorem middleTwoPC (index : Nat) (hlo : middleTwoStartIndex ≤ index)
    (hhi : index ≤ 2837) :
    Artifact.submissionArtifact.instructionPC index =
      [4403,4404,4405,4438,4439,4440,4441,4442,4443,4444,4445,4446,4447,4448,4449,4450,4451,4452,4453,4454,4455,4456,4457,4458,4459,4460,4461,4462,4463,4464,4465,4466,4499,4500,4501,4502,4535][index - middleTwoStartIndex]! := by
  calc
    Artifact.submissionArtifact.instructionPC index =
        Artifact.submissionArtifact.instructionPC
...
/-- MiddleTwo MAC: instructions 2801..2837, pc 4403..4535. -/
def middleTwoMac :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
...
@[simp] theorem secondPC (index : Nat) (hlo : secondStartIndex ≤ index)
    (hhi : index ≤ 2879) :
    Artifact.submissionArtifact.instructionPC index =
      [4536,4537,4538,4571,4572,4573,4574,4575,4576,4577,4578,4579,4580,4581,4582,4583,4584,4585,4586,4587,4588,4589,4590,4591,4592,4593,4594,4595,4596,4597,4598,4599,4632,4633,4634,4635,4668,4669,4670,4671,4672,4675][index - secondStartIndex]! := by
  calc
    Artifact.submissionArtifact.instructionPC index =
        Artifact.submissionArtifact.instructionPC
...
/-- Final MAC and four-way loop test: instructions 2838..2879, pc 4536..4675. -/
def secondMac :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
