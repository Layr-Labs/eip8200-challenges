...
theorem run_l1Dispatch4 (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (htarget : Decode.isValidJumpDest s.executionEnv.code 4745 = true) :
    runInstructions l1DispatchProgram (l1At 4591 s mem bi pa pb 4 i j pdst ret rest) =
      some (l1At 4745 s mem bi pa pb 4 i j pdst ret rest) := by
...
theorem run_l1Dispatch8 (s : State) (mem : ByteArray) (bi : UInt256)
    (pa pb i j : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1006)
    (_htarget : Decode.isValidJumpDest s.executionEnv.code 4745 = true) :
    runInstructions l1DispatchProgram (l1At 4591 s mem bi pa pb 8 i j pdst ret rest) =
      some (l1At 4596 s mem bi pa pb 8 i j pdst ret rest) := by
  have hc1 : rest.length+10 < 1024 := by omega
  have hc2 : rest.length+11 < 1024 := by omega
  have hc3 : rest.length+12 < 1024 := by omega
  simp [l1DispatchProgram, runInstructions, Challenge.EvmProof.Stepper.runInstr,
...
theorem run_l1Join (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hcap : rest.length ≤ 1006) :
    runInstructions joinProgram (l1At 4745 s mem bi pa pb n i j pdst ret rest) =
      some (l1At 4746 s mem bi pa pb n i j pdst ret rest) := by
...
theorem run_l2Dispatch4 (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    (hcap : rest.length ≤ 1006)
    (htarget : Decode.isValidJumpDest s.executionEnv.code 5077 = true) :
    runInstructions l2DispatchProgram (l2At 4928 s mid bi mu c0 pa pb 4 i k pdst ret rest) =
      some (l2At 5077 s mid bi mu c0 pa pb 4 i k pdst ret rest) := by
...
theorem run_l2Dispatch8 (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    (pa pb i k : Nat) (pdst ret : UInt256) (rest : List UInt256)
    (hcap : rest.length ≤ 1006)
    (_htarget : Decode.isValidJumpDest s.executionEnv.code 5077 = true) :
    runInstructions l2DispatchProgram (l2At 4928 s mid bi mu c0 pa pb 8 i k pdst ret rest) =
      some (l2At 4933 s mid bi mu c0 pa pb 8 i k pdst ret rest) := by
  have hc1 : rest.length+11 < 1024 := by omega
  have hc2 : rest.length+12 < 1024 := by omega
  have hc3 : rest.length+13 < 1024 := by omega
  simp [l2DispatchProgram, runInstructions, Challenge.EvmProof.Stepper.runInstr,
...
theorem run_l2Join (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    (hcap : rest.length ≤ 1006) :
    runInstructions joinProgram (l2At 5077 s mid bi mu c0 pa pb n i k pdst ret rest) =
      some (l2At 5078 s mid bi mu c0 pa pb n i k pdst ret rest) := by
