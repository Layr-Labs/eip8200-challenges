...
theorem loadActiveWords_eq (template : State)
    (hactive : 170 ≤ template.activeWords.toNat) :
...
theorem copiedActiveWords_eq (template : State) (n : Nat)
    (hn32 : n ≤ 32) (hactive : 170 ≤ template.activeWords.toNat) :
...
/-- A word at or above byte 3072 is disjoint from the copied RR destination. -/
theorem copiedMemory_readWord_above (mem : ByteArray) (n addr : Nat)
    (hn32 : n ≤ 32) (haddr : 3072 ≤ addr) :
...
theorem copiedMemory_exponentSizeWord (mem : ByteArray) (n : Nat) (hn32 : n ≤ 32) :
    MachineState.readWord (copiedMemory mem n) 5408 =
      MachineState.readWord mem 5408 :=
  copiedMemory_readWord_above mem n 5408 hn32 (by omega)
