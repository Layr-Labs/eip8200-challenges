...
`MONPRO` occupies instruction indices 1509..1724 (pc 2064..2344).  It is
...
scratch area, and tail-calls `CSUB` at pc 2430 with stack `[pc, ret]`.
...
`T_ = 4096` holds `t[n+1]`, `TN = 4128` holds `t[n]`, `TS = 4160` holds the
`n` limbs `t[n-1..0]` most significant first, so `t[k]` sits at
`4160 + 32 * (n - 1 - k)` and `TL = 4128 + 32 * n` is the address of `t[0]`.
`V_S32 = 5248`, `V_MINV = 5280`, `V_ML = 5312`, `V_TL = 5344`. -/
...
def midMem1 (mem : ByteArray) (c : UInt256) : ByteArray :=
  MachineState.writeBytes mem
    (Data.Bytes.natToBytesPadded (MachineState.readWord mem 4128 + c).toNat 32) 4128

/-- `t[n] := t[n] + C`, then `t[n+1] := carry`. -/
def midMem (mem : ByteArray) (c : UInt256) : ByteArray :=
  MachineState.writeBytes (midMem1 mem c)
    (Data.Bytes.natToBytesPadded
      (UInt256.lt (MachineState.readWord mem 4128 + c) c).toNat 32) 4096

/-- `mu = minv * t[0]` truncated to one limb. -/
def rowMu (mem : ByteArray) (n : Nat) : UInt256 :=
  MachineState.readWord mem 5280 * MachineState.readWord mem (4128 + 32 * n)
...
def l2Step (mem : ByteArray) (mu c0 : UInt256) (n : Nat) : Nat → MacState
  ...
  | k + 1 =>
      ...
      let t := MachineState.readWord prev.memory (4160 + 32 * (n - 2 - k))
      { memory := MachineState.writeBytes prev.memory
...
def tailMem1 (mem : ByteArray) (c : UInt256) : ByteArray :=
  MachineState.writeBytes mem
    (Data.Bytes.natToBytesPadded (MachineState.readWord mem 4128 + c).toNat 32) 4160

/-- `t[n-1] := t[n] + C`, then `t[n] := t[n+1] + carry`. -/
def tailMem (mem : ByteArray) (c : UInt256) : ByteArray :=
  MachineState.writeBytes (tailMem1 mem c)
    (Data.Bytes.natToBytesPadded
      (MachineState.readWord (tailMem1 mem c) 4096 +
        UInt256.lt (MachineState.readWord mem 4128 + c) c).toNat 32) 4128
...
/-- Subroutine entry, pc 2064, stack `[pa, pb, pd, ret]`. -/
...
/-- The subroutine exit, pc 2593, after all `n` rows. -/
def mpExitState (s : State) (mem : ByteArray) (pbi : UInt256) (pa pb : Nat)
    (pdst ret : UInt256) (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 2016
           stack := [pbi, UInt256.ofNat (pa - 32), UInt256.ofNat (pb - 32),
                     pdst, ret] ++ rest
           memory := mem }

/-- `CSUB` entry, pc 2430, with stack `[pd, ret]`. -/
...
theorem run_mpEntry (s : State) (mem : ByteArray) (pa pb n : Nat)
  ...
  have h8192 : (4096 : UInt256).toNat = 4096 := by decide
  ...
  have hactS : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 5248 32) = s.activeWords :=
    activeWords_fix s 5248 32 (by decide) (by omega) hact
  have hactC : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 4096 (64 + 32 * n)) =
      s.activeWords := activeWords_fix s 4096 (64 + 32 * n) (by omega) (by omega) hact
...
theorem run_mpOut (s : State) (mem : ByteArray) (pa pb n i : Nat)
  ...
  have hactT : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 5344 32) = s.activeWords :=
    activeWords_fix s 5344 32 (by decide) (by omega) hact
  have hactS : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 5248 32) = s.activeWords :=
    activeWords_fix s 5248 32 (by decide) (by omega) hact
  simp (config := { maxSteps := 800000 })
...
theorem run_mpL1Body (s : State) (mem : ByteArray) (bi : UInt256) (pa pb n i j : Nat)
  ...
  have hK : (115792089237316195423570985008687907853269984665640564039457584007913129639904 :
    ...
    decide
  have h1995 : (1800 : UInt256).toNat = 1800 := by decide
  have h1995' : (1800 : UInt256) = UInt256.ofNat 1800 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (1800 : UInt256).toNat = true := by
    rw [h1995]; exact jumpDest1914
  have hpaj : ptrAt (pa + 32 * n - 32) j %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      pa + 32 * (n - 1 - j) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  have hptj : ptrAt (4128 + 32 * n) j %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      4160 + 32 * (n - 1 - j) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
...
theorem run_mpL1Exit (s : State) (mem : ByteArray) (bi : UInt256) (pa pb n i j : Nat)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1421
      ...
      some (mpMidState s (l1Step mem bi pa n (j + 1)).memory
        ...
        (UInt256.ofNat (ptrAt (4128 + 32 * n) (j + 1)))
...
/-- The two `MSTORE`s of the row middle land at `4128` and `4096`, so every
...
theorem run_mpMid (s : State) (mem : ByteArray) (paj ptj c bi : UInt256)
  ...
  have hsc1 : 4160 ≤ 4128 + 32 * n := by omega
  have hsc2 : 32 * n - 32 + 32 ≤ 4096 := by omega
  have hsc3 : 32 * n ≤ 4096 := by omega
  have hsubTL : UInt256.ofNat (4128 + 32 * n) - UInt256.ofNat 32 =
      UInt256.ofNat (4096 + 32 * n) := by
    rw [Challenge.EvmProof.Word.ofNat_sub_ofNat (by omega) (by omega)]
    exact congrArg UInt256.ofNat (by omega)
  ...
  have hactN : UInt256.ofNat
    ...
    activeWords_fix s 4128 32 (by decide) (by omega) hact
  have hactP : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 4096 32) = s.activeWords :=
    activeWords_fix s 4096 32 (by decide) (by omega) hact
  have hactTL : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 5344 32) = s.activeWords :=
    activeWords_fix s 5344 32 (by decide) (by omega) hact
  have hactT0 : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat (4128 + 32 * n) 32) =
      s.activeWords := activeWords_fix s _ 32 (by decide) (by omega) hact
  have hactMI : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 5280 32) = s.activeWords :=
    activeWords_fix s 5280 32 (by decide) (by omega) hact
  have hactML : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 5312 32) = s.activeWords :=
    activeWords_fix s 5312 32 (by decide) (by omega) hact
...
theorem ptrAt_shift32 (n k : Nat) :
    32 + ptrAt (4096 + 32 * n) k = ptrAt (4128 + 32 * n) k := by
  simp only [ptrAt]
  omega
...
theorem run_mpL2Body (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
  ...
  have h32 : (32 : UInt256) = UInt256.ofNat 32 := by decide
  have h8224 : (4128 : UInt256).toNat = 4128 := by decide
  have h2241 : (1924 : UInt256).toNat = 1924 := by decide
  have h2241' : (1924 : UInt256) = UInt256.ofNat 1924 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (1924 : UInt256).toNat = true := by
    rw [h2241]; exact jumpDest2038
  have hpmj : ptrAt (32 * n - 64) k %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      32 * (n - 2 - k) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  have hptj : ptrAt (4096 + 32 * n) k %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      4160 + 32 * (n - 2 - k) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  have hwr : ptrAt (4128 + 32 * n) k %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      4160 + 32 * (n - 1 - k) := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  have hnextT : ptrAt (4096 + 32 * n) (k + 1) %
      115792089237316195423570985008687907853269984665640564039457584007913129639936 =
      4064 + 32 * n - 32 * k := by
    rw [ptrAt_mod _ _ (by omega) (by omega)]; omega
  have hgt : 4128 < 4064 + 32 * n - 32 * k := by omega
  have hactM : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat
      (32 * (n - 2 - k)) 32) = s.activeWords :=
    activeWords_fix s _ 32 (by decide) (by omega) hact
  have hactT : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat
      (4160 + 32 * (n - 2 - k)) 32) = s.activeWords :=
    activeWords_fix s _ 32 (by decide) (by omega) hact
  have hactW : UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat
      (4160 + 32 * (n - 1 - k)) 32) = s.activeWords :=
    activeWords_fix s _ 32 (by decide) (by omega) hact
  simp (config := { maxSteps := 800000 })
  ...
  rw [MacAlt.macSumNat]
  rw [Nat.add_comm (ptrAt (4096 + 32 * n) k) 32, ptrAt_shift32, hwr]
  exact ⟨hactW, rfl⟩
...
theorem run_mpL2Exit (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock blk1519
      ...
      some (mpTailState s (l2Step mid mu c0 n (k + 1)).memory
        ...
        (UInt256.ofNat (ptrAt (4096 + 32 * n) (k + 1)))
...
theorem run_mpTailNext (s : State) (mem : ByteArray) (pmj ptj c mu bi : UInt256)
  ...
  have h1974 : (1786 : UInt256).toNat = 1786 := by decide
  have h1974' : (1786 : UInt256) = UInt256.ofNat 1786 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (1786 : UInt256).toNat = true := by
    rw [h1974]; exact jumpDest1900
  ...
  have hactN : UInt256.ofNat
    ...
    activeWords_fix s 4128 32 (by decide) (by omega) hact
  have hactS : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 4160 32) = s.activeWords :=
    activeWords_fix s 4160 32 (by decide) (by omega) hact
  have hactP : UInt256.ofNat
      (MachineState.activeWordsAfter s.activeWords.toNat 4096 32) = s.activeWords :=
    activeWords_fix s 4096 32 (by decide) (by omega) hact
  simp (config := { maxSteps := 800000 })
    [blk1569, opAt, pushAt, wfOp,
      ...
      h1974, h1974', hjump, jumpDest1900, hnextB, hpbmN, hgt,
...
theorem run_mpExit (s : State) (mem : ByteArray) (pbi : UInt256) (pa pb : Nat)
  ...
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
  have h2642 : (4875 : UInt256).toNat = 4875 := by decide
  have h2642' : (4875 : UInt256) = UInt256.ofNat 4875 := by decide
  have hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode
      (4875 : UInt256).toNat = true := by
...
Every word `MONPRO` writes lies in `[4096, 5184)`: the `CALLDATACOPY` prologue
writes `64 + 32 * n ≤ 1088` bytes from `4096`, and every limb store lands at
`4160 + 32 * (n - 1 - j) ≤ 5152`.  So every word below `T_ = 0x2000` — that is
every named block `M`, `ACC`, `BASE`, `ONE`, `R1`, `CC`, `RR`, `SUBB` of the
memory map — and every word at or above `5184` — `V_S32 = 5248`,
`V_MINV = 5280`, `V_ML = 5312`, `V_TL = 5344`, `V_EOFF`, `V_N` — survive the
subroutine unchanged. -/

theorem readWord_writeLimb (mem : ByteArray) (w dst addr : Nat)
    (hdstLo : 4096 ≤ dst) (hdstHi : dst + 32 ≤ 5184)
    (haddr : addr + 32 ≤ 4096 ∨ 5184 ≤ addr) :
    MachineState.readWord
...
theorem readWord_l1Step (mem : ByteArray) (bi : UInt256) (pa n addr j : Nat)
    (hn : n ≤ 32) (haddr : addr + 32 ≤ 4096 ∨ 5184 ≤ addr) :
...
theorem readWord_rowsMem (mem : ByteArray) (pa pb n addr : Nat)
    (hn : n ≤ 32) (haddr : addr + 32 ≤ 4096 ∨ 5184 ≤ addr) : ∀ i,
...
def gasSteps_mpOut (s : State) (mem : ByteArray) (pa pb n i : Nat)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hn : 2 ≤ n) (hn32 : n ≤ 32) (hi : i < n)
    (hpa : 32 ≤ pa) (hpaFit : pa + 32 * n ≤ 5376)
    (hpb : 32 ≤ pb) (hpbFit : pb + 32 * n ≤ 5376)
    (hs32 : MachineState.readWord mem 5248 = UInt256.ofNat (32 * n))
    (htl : MachineState.readWord mem 5344 = UInt256.ofNat (4128 + 32 * n)) :
...
def gasSteps_mpL1Body (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hn32 : n ≤ 32) (hj : j + 1 < n)
    (hpa : 32 ≤ pa) (hpaFit : pa + 32 * n ≤ 5376) :
    Challenge.EvmProof.GasSteps
      (mpL1State s mem bi pa pb n i j pdst ret rest)
...
def gasSteps_mpL1Loop (s : State) (mem : ByteArray) (bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hn32 : m + 2 ≤ 32)
    (hpa : 32 ≤ pa) (hpaFit : pa + 32 * (m + 2) ≤ 5376) :
    Challenge.EvmProof.GasSteps
...
def gasSteps_mpL2Body (s : State) (mid : ByteArray) (bi mu c0 : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hn32 : n ≤ 32) (hk : k + 2 < n) :
    Challenge.EvmProof.GasSteps
      (mpL2State s mid bi mu c0 pa pb n i k pdst ret rest)
...
def gasSteps_mpTailNext (s : State) (mem : ByteArray) (pmj ptj c mu bi : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hn32 : n ≤ 32) (hi : i + 1 < n)
    (hpb : 32 ≤ pb) (hpbFit : pb + 32 * n ≤ 5376) :
    Challenge.EvmProof.GasSteps
      (mpTailState s mem pmj ptj c mu bi pa pb n i pdst ret rest)
...
def gasSteps_mpRowToTail (s : State) (mem : ByteArray) (pa pb m i : Nat)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hn32 : m + 2 ≤ 32) (hi : i < m + 2)
    (hpa : 32 ≤ pa) (hpaFit : pa + 32 * (m + 2) ≤ 5376)
    (hpb : 32 ≤ pb) (hpbFit : pb + 32 * (m + 2) ≤ 5376)
    (hs32 : MachineState.readWord mem 5248 = UInt256.ofNat (32 * (m + 2)))
    (htl : MachineState.readWord mem 5344 = UInt256.ofNat (4128 + 32 * (m + 2)))
    (hml : MachineState.readWord mem 5312 = UInt256.ofNat (32 * (m + 2) - 32)) :
    Challenge.EvmProof.GasSteps
      (mpOutState s mem pa pb (m + 2) i pdst ret rest)
      (mpTailState s (rowL2 mem pa pb (m + 2) i).memory
        (UInt256.ofNat (ptrAt (32 * (m + 2) - 64) (m + 1)))
        (UInt256.ofNat (ptrAt (4096 + 32 * (m + 2)) (m + 1)))
  ...
  (gasSteps_mpMid s
      ...
      (UInt256.ofNat (ptrAt (4128 + 32 * (m + 2)) (m + 1 + 1)))
      (l1Step mem (rowBi mem pb (m + 2) i) pa (m + 2) (m + 1 + 1)).carry
      (rowBi mem pb (m + 2) i) pa pb (m + 2) i pdst ret rest hcap hrun hcode hfork
      hnp hact (by omega) hn32
      ((readWord_l1Step mem (rowBi mem pb (m + 2) i) pa (m + 2) 5312 (m + 1 + 1)
          hn32 (by omega)).trans hml)
      ((readWord_l1Step mem (rowBi mem pb (m + 2) i) pa (m + 2) 5344 (m + 1 + 1)
...
def gasSteps_mpRowNext (s : State) (mem : ByteArray) (pa pb m i : Nat)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false)
    (hact : 168 ≤ s.activeWords.toNat) (hn32 : m + 2 ≤ 32) (hi : i + 1 < m + 2)
    (hpa : 32 ≤ pa) (hpaFit : pa + 32 * (m + 2) ≤ 5376)
    (hpb : 32 ≤ pb) (hpbFit : pb + 32 * (m + 2) ≤ 5376)
    (hs32 : MachineState.readWord mem 5248 = UInt256.ofNat (32 * (m + 2)))
    (htl : MachineState.readWord mem 5344 = UInt256.ofNat (4128 + 32 * (m + 2)))
...
def gasSteps_mpRows (s : State) (memory : ByteArray) (pa pb m : Nat)
    ...
    (hpa : 32 ≤ pa) (hpaFit : pa + 32 * (m + 2) ≤ 5376)
    (hpb : 32 ≤ pb) (hpbFit : pb + 32 * (m + 2) ≤ 5376)
    (hs32 : MachineState.readWord memory 5248 = UInt256.ofNat (32 * (m + 2)))
    (htl : MachineState.readWord memory 5344 = UInt256.ofNat (4128 + 32 * (m + 2)))
    (hml : MachineState.readWord memory 5312 =
  ...
  Challenge.EvmProof.GasSteps.iterateBounded
    ...
    (fun i hi => gasSteps_mpRowNext s (rowsMem memory pa pb (m + 2) i) pa pb m i
      ...
      hpbFit
      ((readWord_rowsMem memory pa pb (m + 2) 5248 hn32 (by omega) i).trans hs32)
      ((readWord_rowsMem memory pa pb (m + 2) 5344 hn32 (by omega) i).trans htl)
      ((readWord_rowsMem memory pa pb (m + 2) 5312 hn32 (by omega) i).trans hml))
...
def gasSteps_monproToCsub (s : State) (mem : ByteArray) (pa pb m : Nat)
  ...
  (gasSteps_mpRows s (mpZeroed s mem (m + 2)) pa pb m pdst ret rest hcap hrun
      ...
      ((readWord_mpZeroed s mem (m + 2) 5248 hn32 (by omega)).trans hs32)
      ((readWord_mpZeroed s mem (m + 2) 5344 hn32 (by omega)).trans htl)
      ((readWord_mpZeroed s mem (m + 2) 5312 hn32 (by omega)).trans hml)).trans
  (gasSteps_mpRowLast s (rowsMem (mpZeroed s mem (m + 2)) pa pb (m + 2) (m + 1))
      ...
      hpaFit hpb hpbFit
      ((readWord_rowsMem (mpZeroed s mem (m + 2)) pa pb (m + 2) 5248 hn32
          (by omega) (m + 1)).trans
        ((readWord_mpZeroed s mem (m + 2) 5248 hn32 (by omega)).trans hs32))
      ((readWord_rowsMem (mpZeroed s mem (m + 2)) pa pb (m + 2) 5344 hn32
          (by omega) (m + 1)).trans
        ((readWord_mpZeroed s mem (m + 2) 5344 hn32 (by omega)).trans htl))
      ((readWord_rowsMem (mpZeroed s mem (m + 2)) pa pb (m + 2) 5312 hn32
          (by omega) (m + 1)).trans
        ((readWord_mpZeroed s mem (m + 2) 5312 hn32 (by omega)).trans hml)))

/-! ## Preservation of the named memory blocks

Every named block of the memory map except the CIOS scratch area lies below
`T_ = 0x2000 = 4096`: `M = 0`, `ACC = 0x400`, `BASE = 0x800`, `ONE = 0xC00`,
...
`MONPRO` is entered at pc 2064 with stack `[pa, pb, pd, ret]` and tail-calls
`CSUB` at pc 2430 with stack `[pd, ret]`; `CSUB` (proved in `Fast.Csub`) copies
...
`t[k]` lives at `tAddr n k = 4160 + 32 * (n - 1 - k)`.  The source blocks at
`pa`, `pb` and the modulus at `0` all lie below `T_ = 4096`, so no limb store
of `MONPRO` disturbs them. -/

theorem radix_eq : Limbs.radix = 2 ^ 256 := rfl

/-- The address of `t[k]`, counted from the least significant limb. -/
def tAddr (n k : Nat) : Nat := 4160 + 32 * (n - 1 - k)
...
theorem l1Step_succ_carry (mem : ByteArray) (bi : UInt256) (pa n j : Nat)
    (hn : n ≤ 32) (hj : j < n) (hpaFit : pa + 32 * n ≤ 4096) :
...
theorem readWord_l1Step_val (mem : ByteArray) (bi : UInt256) (pa n k : Nat)
    (hn : n ≤ 32) (hkn : k < n) (hpaFit : pa + 32 * n ≤ 4096) : ∀ j, k < j → j ≤ n →
    MachineState.readWord (l1Step mem bi pa n j).memory (tAddr n k) =
...
theorem l1_invariant (mem : ByteArray) (bi : UInt256) (pa n : Nat)
    (hn : n ≤ 32) (hpaFit : pa + 32 * n ≤ 4096) : ∀ j, j ≤ n →
...
`4160 + 32 * (n - 2 - k)`, and stores into `t[k]` at `tAddr n k`. -/

/-- Nothing below `TS = 4160` is written by the first loop. -/
...
/-- Nothing below `TS = 4160` is written by the second loop. -/
...
theorem readWord_l2Step_keep (mem : ByteArray) (mu c0 : UInt256) (n k : Nat)
    (hkn : k + 1 < n) : ∀ j, j ≤ k →
    MachineState.readWord (l2Step mem mu c0 n j).memory (4160 + 32 * (n - 2 - k)) =
      MachineState.readWord mem (4160 + 32 * (n - 2 - k)) := by
  intro j
  induction j with
  | zero => intro _; rfl
  | succ j ih =>
...
def l2Val (mem : ByteArray) (mu c0 : UInt256) (n k : Nat) : UInt256 :=
  macSum (MachineState.readWord mem (32 * (n - 2 - k))) mu
    (MachineState.readWord mem (4160 + 32 * (n - 2 - k))) (l2Step mem mu c0 n k).carry
...
theorem l2Step_succ_carry (mem : ByteArray) (mu c0 : UInt256) (n k : Nat)
    ...
    (l2Step mem mu c0 n (k + 1)).carry =
      macCarry (MachineState.readWord mem (32 * (n - 2 - k))) mu
        (MachineState.readWord mem (4160 + 32 * (n - 2 - k)))
...
theorem l2_invariant (mem : ByteArray) (mu c0 : UInt256) (n : Nat)
    ...
    limbSum (fun k => (l2Val mem mu c0 n k).toNat) j +
      ...
      c0.toNat +
        (limbSum
            (fun k => (MachineState.readWord mem (4160 + 32 * (n - 2 - k))).toNat) j +
          mu.toNat *
  ...
  | succ j ih =>
      ...
      have hmac := macSpec (MachineState.readWord mem (32 * (n - 2 - j))) mu
        (MachineState.readWord mem (4160 + 32 * (n - 2 - j)))
...
theorem l1_row (mem : ByteArray) (bi : UInt256) (pa n tlow a : Nat)
    (hn : n ≤ 32) (hpaFit : pa + 32 * n ≤ 4096)
    (hta : Model.FastRepresents mem pa n a)
    (htt : Model.FastRepresents mem 4160 n tlow) :
...
`MONPRO` enters `CSUB` at pc 2430 with stack `[pd, ret]`, `t[n]` at
...
/-- The whole `MONPRO` call: from the subroutine entry at pc 2064 with stack
...
def gasSteps_monproCsub (s : State) (mem : ByteArray) (pa pb n : Nat)
    ...
    (htn : (MachineState.readWord (rowsMem (mpZeroed s mem n) pa pb n n) 4128).toNat
      ≤ 1) :
    Challenge.EvmProof.GasSteps
      (mpEntryState s mem pa pb pdst ret rest)
  ...
  (gasSteps_monpro s mem pa pb n pdst ret rest hcap hrun hcode hfork hnp hact hn
    ...
    (Csub.gasSteps_csub s (rowsMem (mpZeroed s mem n) pa pb n n) n pdst ret rest
      ...
      ((readWord_monpro_preserved s mem pa pb n n 5312 hn32 (by omega)).trans hml)
      ((readWord_monpro_preserved s mem pa pb n n 5344 hn32 (by omega)).trans htl)
      ((Csub.csStep_readWord_disjoint (rowsMem (mpZeroed s mem n) pa pb n n) n 5248
            (by omega) (Or.inr (by omega)) n (Nat.le_refl n)).trans
        ((readWord_monpro_preserved s mem pa pb n n 5248 hn32 (by omega)).trans hs32))
      hdstFit
      (by
...
/-- Reads at or above `TS = 4160` see through the row middle. -/
...
/-- Reads below `T_ = 4096` see through the row middle. -/
theorem readWord_midMem_low' (mem : ByteArray) (c : UInt256) (r : Nat)
    (hr : r + 32 ≤ 4096) :
    MachineState.readWord (midMem mem c) r = MachineState.readWord mem r :=
  readWord_midMem_peel mem _ _ r (Or.inl hr)

/-- The two `MSTORE`s of the row tail land at `4160` and `4128`. -/
theorem readWord_tailMem_peel (mem : ByteArray) (v w r : Nat)
    (hr : r + 32 ≤ 4128 ∨ 4192 ≤ r) :
    MachineState.readWord
        (MachineState.writeBytes
          (MachineState.writeBytes mem (Data.Bytes.natToBytesPadded v 32) 4160)
          (Data.Bytes.natToBytesPadded w 32) 4128) r =
      MachineState.readWord mem r := by
  have h1 : MachineState.readWord
      (MachineState.writeBytes
        (MachineState.writeBytes mem (Data.Bytes.natToBytesPadded v 32) 4160)
        (Data.Bytes.natToBytesPadded w 32) 4128) r =
      MachineState.readWord
        (MachineState.writeBytes mem (Data.Bytes.natToBytesPadded v 32) 4160) r := by
    apply Challenge.EvmProof.Memory.readWord_writeBytes_disjoint
    rw [YulEvmCompiler.BytesLemmas.natToBytesPadded_size]
    omega
  have h2 : MachineState.readWord
      (MachineState.writeBytes mem (Data.Bytes.natToBytesPadded v 32) 4160) r =
      MachineState.readWord mem r := by
    apply Challenge.EvmProof.Memory.readWord_writeBytes_disjoint
    rw [YulEvmCompiler.BytesLemmas.natToBytesPadded_size]
    omega
...
theorem readWord_tailMem_ts (mem : ByteArray) (c : UInt256) :
    MachineState.readWord (tailMem mem c) 4160 =
      MachineState.readWord mem 4128 + c := by
  have h1 : MachineState.readWord (tailMem mem c) 4160 =
      MachineState.readWord (tailMem1 mem c) 4160 := by
    apply Challenge.EvmProof.Memory.readWord_writeBytes_disjoint
    rw [YulEvmCompiler.BytesLemmas.natToBytesPadded_size]
    omega
...
theorem readWord_tailMem_tn (mem : ByteArray) (c : UInt256) :
    MachineState.readWord (tailMem mem c) 4128 =
      MachineState.readWord mem 4096 +
        UInt256.lt (MachineState.readWord mem 4128 + c) c := by
  have hlow : MachineState.readWord (tailMem1 mem c) 4096 =
      MachineState.readWord mem 4096 := by
    apply Challenge.EvmProof.Memory.readWord_writeBytes_disjoint
    rw [YulEvmCompiler.BytesLemmas.natToBytesPadded_size]
    omega
  simp only [tailMem]
...
theorem readWord_midMem_tn (mem : ByteArray) (c : UInt256) :
    MachineState.readWord (midMem mem c) 4128 =
      MachineState.readWord mem 4128 + c := by
  have h1 : MachineState.readWord (midMem mem c) 4128 =
      MachineState.readWord (midMem1 mem c) 4128 := by
    apply Challenge.EvmProof.Memory.readWord_writeBytes_disjoint
    rw [YulEvmCompiler.BytesLemmas.natToBytesPadded_size]
    omega
...
theorem readWord_midMem_tnp (mem : ByteArray) (c : UInt256) :
    MachineState.readWord (midMem mem c) 4096 =
      UInt256.lt (MachineState.readWord mem 4128 + c) c := by
  simp only [midMem]
  exact Challenge.EvmProof.Memory.readWord_writeWord _ _ _
...
theorem readWord_rowMem_top (mem : ByteArray) (pa pb n i : Nat) :
    MachineState.readWord (rowMem mem pa pb n i) 4160 =
      MachineState.readWord (rowL2 mem pa pb n i).memory 4128 +
...
theorem readWord_rowMem_tn (mem : ByteArray) (pa pb n i : Nat) :
    MachineState.readWord (rowMem mem pa pb n i) 4128 =
      MachineState.readWord (rowL2 mem pa pb n i).memory 4096 +
        UInt256.lt (MachineState.readWord (rowL2 mem pa pb n i).memory 4128 +
...
theorem lowValue_rowMem (mem : ByteArray) (pa pb p i : Nat) (hn32 : p + 2 ≤ 32) :
    Csub.lowValue (rowMem mem pa pb (p + 2) i) 4160 (p + 2) (p + 2) =
      limbSum (fun k => (l2Val (rowMid mem pa pb (p + 2) i)
          ...
          (p + 1) +
        (MachineState.readWord (rowMem mem pa pb (p + 2) i) 4160).toNat *
          Limbs.radix ^ (p + 1) := by
  rw [← limbSum_eq_lowValue, limbSum_succ]
  have hlast : (MachineState.readWord (rowMem mem pa pb (p + 2) i)
      (4160 + 32 * (p + 2 - 1 - (p + 1)))).toNat =
      (MachineState.readWord (rowMem mem pa pb (p + 2) i) 4160).toNat := by
    have h0 : 4160 + 32 * (p + 2 - 1 - (p + 1)) = 4160 := by omega
    rw [h0]
  rw [hlast]
  congr 1
  apply limbSum_congr
  ...
  simp only [tAddr] at h
  have haddr : 4160 + 32 * (p + 2 - 1 - k) = 4160 + 32 * (p + 2 - 1 - k) := rfl
  rw [haddr, h]
...
theorem row_equation (mem : ByteArray) (pa pb p i : Nat) (a mm tlow : Nat)
    (hn32 : p + 2 ≤ 32) (hpaFit : pa + 32 * (p + 2) ≤ 4096)
    (ha : Model.FastRepresents mem pa (p + 2) a)
    (hm : Model.FastRepresents mem 0 (p + 2) mm)
    (ht : Model.FastRepresents mem 4160 (p + 2) tlow)
    (hminv : ((MachineState.readWord mem (32 * (p + 2) - 32)).toNat *
        (MachineState.readWord mem 5280).toNat + 1) % 2 ^ 256 = 0) :
    ((MachineState.readWord (rowMem mem pa pb (p + 2) i) 4128).toNat *
          Limbs.radix ^ (p + 2) +
        Csub.lowValue (rowMem mem pa pb (p + 2) i) 4160 (p + 2) (p + 2)) *
        Limbs.radix =
      (MachineState.readWord mem 4128).toNat * Limbs.radix ^ (p + 2) + tlow +
  ...
  have hA1 : MachineState.readWord (rowL1 mem pa pb (p + 2) i).memory 4128 =
      MachineState.readWord mem 4128 :=
    readWord_l1Step_low mem (rowBi mem pb (p + 2) i) pa (p + 2) 4128 (p + 2) (by omega)
  ...
  have hMinvR : MachineState.readWord (rowL1 mem pa pb (p + 2) i).memory 5280 =
      MachineState.readWord mem 5280 :=
    readWord_l1Step mem (rowBi mem pb (p + 2) i) pa (p + 2) 5280 (p + 2) hn32
      (Or.inr (by omega))
  have hT0 : MachineState.readWord (rowL1 mem pa pb (p + 2) i).memory
    ...
    simp only [tAddr] at h
    have haddr : 4160 + 32 * (p + 2 - 1 - 0) = 4128 + 32 * (p + 2) := by omega
    rw [haddr] at h
    exact h
  have hMDtn : MachineState.readWord (rowMid mem pa pb (p + 2) i) 4128 =
      MachineState.readWord mem 4128 + (rowL1 mem pa pb (p + 2) i).carry := by
    rw [rowMid_def, readWord_midMem_tn, hA1]
  have hMDtnp : MachineState.readWord (rowMid mem pa pb (p + 2) i) 4096 =
      UInt256.lt (MachineState.readWord mem 4128 + (rowL1 mem pa pb (p + 2) i).carry)
        (rowL1 mem pa pb (p + 2) i).carry := by
    rw [rowMid_def, readWord_midMem_tnp, hA1]
  have hL2tn : MachineState.readWord (rowL2 mem pa pb (p + 2) i).memory 4128 =
      MachineState.readWord (rowMid mem pa pb (p + 2) i) 4128 :=
    readWord_l2Step_low _ _ _ (p + 2) 4128 (p + 2 - 1) (by omega)
  have hL2tnp : MachineState.readWord (rowL2 mem pa pb (p + 2) i).memory 4096 =
      MachineState.readWord (rowMid mem pa pb (p + 2) i) 4096 :=
    readWord_l2Step_low _ _ _ (p + 2) 4096 (p + 2 - 1) (by omega)
  have hcu : (UInt256.lt (MachineState.readWord mem 4128 +
      (rowL1 mem pa pb (p + 2) i).carry)
      (rowL1 mem pa pb (p + 2) i).carry).toNat ≤ 1 := by
    rw [word_toNat_lt']
    split <;> omega
  have hcv : (UInt256.lt (MachineState.readWord (rowMid mem pa pb (p + 2) i) 4128 +
      (rowL2 mem pa pb (p + 2) i).carry)
      (rowL2 mem pa pb (p + 2) i).carry).toNat ≤ 1 := by
    rw [word_toNat_lt']
    split <;> omega
  have hFtn : (MachineState.readWord (rowMem mem pa pb (p + 2) i) 4128).toNat =
      (UInt256.lt (MachineState.readWord mem 4128 + (rowL1 mem pa pb (p + 2) i).carry)
        (rowL1 mem pa pb (p + 2) i).carry).toNat +
      (UInt256.lt (MachineState.readWord (rowMid mem pa pb (p + 2) i) 4128 +
        (rowL2 mem pa pb (p + 2) i).carry)
  ...
  have hFts : (MachineState.readWord (rowMem mem pa pb (p + 2) i) 4160).toNat =
      (MachineState.readWord (rowMid mem pa pb (p + 2) i) 4128 +
        (rowL2 mem pa pb (p + 2) i).carry).toNat := by
    rw [readWord_rowMem_top, hL2tn]
  have hA : (UInt256.lt (MachineState.readWord mem 4128 +
        (rowL1 mem pa pb (p + 2) i).carry)
        (rowL1 mem pa pb (p + 2) i).carry).toNat * Limbs.radix +
      (MachineState.readWord (rowMid mem pa pb (p + 2) i) 4128).toNat =
      (MachineState.readWord mem 4128).toNat +
        (rowL1 mem pa pb (p + 2) i).carry.toNat := by
    rw [hMDtn, radix_eq]
    exact add_carry_split (MachineState.readWord mem 4128)
      (rowL1 mem pa pb (p + 2) i).carry
  have hB : (UInt256.lt (MachineState.readWord (rowMid mem pa pb (p + 2) i) 4128 +
        (rowL2 mem pa pb (p + 2) i).carry)
      ...
      (MachineState.readWord (rowMid mem pa pb (p + 2) i) 4128).toNat +
        (rowL2 mem pa pb (p + 2) i).carry.toNat := by
    rw [radix_eq]
    exact add_carry_split (MachineState.readWord (rowMid mem pa pb (p + 2) i) 4128)
      (rowL2 mem pa pb (p + 2) i).carry
  have hC : (rowC0 (rowL1 mem pa pb (p + 2) i).memory (p + 2)).toNat * Limbs.radix =
    ...
    exact c0_spec
      ...
      (MachineState.readWord (rowL1 mem pa pb (p + 2) i).memory 5280)
      (MachineState.readWord (rowL1 mem pa pb (p + 2) i).memory (4128 + 32 * (p + 2)))
  ...
  have hstF : ∀ k, k < p + 1 →
    ...
    have h2 : MachineState.readWord (rowL1 mem pa pb (p + 2) i).memory
      ...
      have haddr : 4160 + 32 * (p + 2 - 1 - (k + 1)) = 4160 + 32 * (p - k) := by omega
      rw [haddr] at h3
      exact h3
    rw [h1, h2]
  have hE : limbSum (fun k => (MachineState.readWord (rowMid mem pa pb (p + 2) i)
          (4160 + 32 * (p - k))).toNat) (p + 1) * Limbs.radix +
        (l1Val mem (rowBi mem pb (p + 2) i) pa (p + 2) 0).toNat =
...
theorem readWord_mpZeroed_zero (s : State) (mem : ByteArray) (n a : Nat)
    (hlo : 4096 ≤ a) (hhi : a + 32 ≤ 4160 + 32 * n) :
    MachineState.readWord (mpZeroed s mem n) a = UInt256.ofNat 0 := by
  simp only [mpZeroed]
  exact readWord_calldata_zero mem s.executionEnv.calldata (64 + 32 * n) 4096 a hlo
    (by omega)

theorem fastRepresents_mpZeroed (s : State) (mem : ByteArray) (n : Nat) :
    Model.FastRepresents (mpZeroed s mem n) 4160 n 0 := by
  rw [Model.fastRepresents_zero_iff]
  intro j hj
  rw [readWord_mpZeroed_zero s mem n (4160 + 32 * j) (by omega) (by omega)]
  simp

theorem readWord_mpZeroed_tn (s : State) (mem : ByteArray) (n : Nat) :
    MachineState.readWord (mpZeroed s mem n) 4128 = UInt256.ofNat 0 :=
  readWord_mpZeroed_zero s mem n 4128 (by omega) (by omega)

/-! ## The outer loop -/

/-- The `(n+1)`-limb CIOS accumulator `t = t[n]·rad^n + t_low`. -/
def tValue (mem : ByteArray) (n : Nat) : Nat :=
  (MachineState.readWord mem 4128).toNat * Limbs.radix ^ n +
    Csub.lowValue mem 4160 n n
...
theorem rows_invariant (s : State) (mem : ByteArray) (pa pb p : Nat) (a b mm : Nat)
    (hn32 : p + 2 ≤ 32)
    (hpaFit : pa + 32 * (p + 2) ≤ 4096) (hpbFit : pb + 32 * (p + 2) ≤ 4096)
    (ha : Model.FastRepresents mem pa (p + 2) a)
  ...
  | zero =>
      intro _
      have hlow0 : Csub.lowValue (mpZeroed s mem (p + 2)) 4160 (p + 2) (p + 2) = 0 :=
        Model.fastRepresents_value_unique
          (Csub.fastRepresents_lowValue (mpZeroed s mem (p + 2)) 4160 (p + 2))
  ...
  | succ i ih =>
      ...
      have hminvR : ((MachineState.readWord
            ...
            (32 * (p + 2) - 32)).toNat *
          (MachineState.readWord
            (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) i) 5280).toNat + 1) %
          2 ^ 256 = 0 := by
        rw [readWord_monpro_preserved s mem pa pb (p + 2) i (32 * (p + 2) - 32) hn32
            (Or.inl (by omega)),
          readWord_monpro_preserved s mem pa pb (p + 2) i 5280 hn32 (Or.inr (by omega))]
        exact hminv
      have hrow := row_equation (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) i)
        pa pb p i a mm
        (Csub.lowValue (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) i) 4160
          (p + 2) (p + 2))
        hn32 hpaFit hpaR hmR
        (Csub.fastRepresents_lowValue
          (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) i) 4160 (p + 2)) hminvR
      ...
      obtain ⟨hstep1, hstep2⟩ := Model.cios_step (β := Limbs.radix) (m := mm) (a := a)
        ...
        (t := (MachineState.readWord
            (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) i) 4128).toNat *
            Limbs.radix ^ (p + 2) +
          Csub.lowValue (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) i) 4160
            (p + 2) (p + 2))
        (Q := Q)
...
theorem monpro_tn_le_one (s : State) (mem : ByteArray) (pa pb p : Nat) (a b mm : Nat)
  ...
  have hlow : Csub.lowValue (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) (p + 2))
      4160 (p + 2) (p + 2) < Limbs.radix ^ (p + 2) := Csub.lowValue_lt _ _ _ _
  by_contra hcon
  have hge : 2 ≤ (MachineState.readWord
      (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) (p + 2)) 4128).toNat := by omega
  have hmul : 2 * Limbs.radix ^ (p + 2) ≤
      (MachineState.readWord (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) (p + 2))
        4128).toNat * Limbs.radix ^ (p + 2) := Nat.mul_le_mul_right _ hge
  omega
...
theorem monpro_represents (s : State) (mem : ByteArray) (pa pb p pdst : Nat)
  ...
  exact Csub.csub_correct (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) (p + 2))
    (p + 2)
    (Csub.lowValue (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) (p + 2)) 4160
      (p + 2) (p + 2))
    mm
    (MachineState.readWord (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) (p + 2))
      4128).toNat
    pdst (by omega) hn32
    (Csub.fastRepresents_lowValue
      (rowsMem (mpZeroed s mem (p + 2)) pa pb (p + 2) (p + 2)) 4160 (p + 2))
...
def gasSteps_monproFull (s : State) (mem : ByteArray) (pa pb p : Nat) (a b mm : Nat)
    ...
    (hpa : 32 ≤ pa) (hpaFit : pa + 32 * (p + 2) ≤ 4096)
    (hpb : 32 ≤ pb) (hpbFit : pb + 32 * (p + 2) ≤ 4096)
    (hcds : s.executionEnv.calldata.size < 2 ^ 256)
    ...
    (hjump : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode ret.toNat = true)
    (hdstFit : pdst.toNat + 32 * (p + 2) ≤ 5376)
    (ha : Model.FastRepresents mem pa (p + 2) a)
    (hb : Model.FastRepresents mem pb (p + 2) b)
    (hm : Model.FastRepresents mem 0 (p + 2) mm)
    (ham : a < mm) (hmpos : 0 < mm)
...
The configuration words `V_S32 = 5248`, `V_MINV = 5280`, `V_ML = 5312`,
`V_TL = 5344` and `V_EOFF = 5376` are written once by `Fast.Setup` and read by
every subroutine.  No transformer a `MONPRO` call applies writes at or above
`5184 = 4160 + 32 * 32`, so all five survive a whole run; the lemmas below say
...
/-- The `CALLDATACOPY` prologue writes exactly `[4096, 4160 + 32 * n)`. -/
theorem mpZeroed_readWord_outside (s : State) (mem : ByteArray) (n addr : Nat)
    (hout : addr + 32 ≤ 4096 ∨ 4160 + 32 * n ≤ addr) :
    MachineState.readWord (mpZeroed s mem n) addr = MachineState.readWord mem addr := by
  simp only [mpZeroed]
  apply Challenge.EvmProof.Memory.readWord_writeBytes_disjoint
  rw [Challenge.EvmProof.Memory.readPadded_size]
...
/-- The `n` CIOS rows write only inside `[4096, 5184)`. -/
