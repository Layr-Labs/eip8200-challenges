...
theorem readWord_mpZeroed_zero (s : State) (mem : ByteArray) (n a : Nat)
    (hlo : 8224 ≤ a) (hhi : a + 32 ≤ 8256 + 32 * n) :
    MachineState.readWord (mpZeroed s mem n) a = UInt256.ofNat 0 := by
  simp only [mpZeroed]
  exact readWord_calldata_zero mem s.executionEnv.calldata (32 + 32 * n) 8224 a hlo
    (by omega)
...
/-- The `CALLDATACOPY` prologue writes exactly `[8224, 8256 + 32 * n)`;
the carry flag at `8192` is written by the first row middle before it is read. -/
