...
`LZ` occupies instruction indices 1806..1840 (pc 2966..3014).  It is entered
at pc 2966 with the byte index `i` on top of the driver frame, loads exponent
...
/-- The `LZ` entry, pc 2966.  The driver frame below the byte index is left
...
/-- pc 2982, the arm every byte after the first takes. -/
...
/- The first-byte arm hands to `LZBASE`, whose relocated entry is pc 3909. -/
def lzBase (s : State) (mem : ByteArray) (i w mask : Nat)
    (rest : List UInt256) : State :=
  { s with pc := UInt256.ofNat 3909
           stack := UInt256.ofNat mask :: UInt256.ofNat w :: UInt256.ofNat i :: rest
           memory := mem }
...
theorem run_lzFirst (s : State) (mem : ByteArray) (i w : Nat)
  ...
  have h3909 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 3909 = true :=
    Artifact.isValidJumpDest_index 2588 (by rfl)
  simp (config := { maxSteps := 600000 }) [blk1796, opAt, pushAt,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    lzFirst, lzBase, hrun, hcode, hc2, hc3, hc4, hc5, hcomm, h3909,
