...
/- The first-byte arm hands to `LZBASE`, whose relocated entry is pc 3858. -/
...
theorem run_lzFirst (s : State) (mem : ByteArray) (i w : Nat)
  ...
  have h3858 : Decode.isValidJumpDest Challenge.Modexp.submissionBytecode 3858 = true :=
    Artifact.isValidJumpDest_index 2552 (by rfl)
  simp (config := { maxSteps := 600000 }) [blk1796, opAt, pushAt,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    lzFirst, lzBase, hrun, hcode, hc2, hc3, hc4, hc5, hcomm, h3858,
