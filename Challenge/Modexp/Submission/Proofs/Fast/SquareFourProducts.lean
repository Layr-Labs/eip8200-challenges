...
namespace Challenge.Modexp.Submission.Proofs.Fast.SquareFourProducts
open EvmSemantics EvmSemantics.EVM
open Challenge.Modexp.Submission.Proofs.Fast
open Monpro SquareInit

/-- The current value of the four low accumulator words. -/
def low (mem : ByteArray) : Nat :=
  limbSum (fun k => (MachineState.readWord mem (tAddr 4 k)).toNat) 4
...
theorem low_storeWord (mem : ByteArray) (j : Nat) (w : UInt256) (hj : j < 4) :
    low (storeWord mem (tAddr 4 j) w) +
        (MachineState.readWord mem (tAddr 4 j)).toNat*Limbs.radix^j =
      low mem + w.toNat*Limbs.radix^j := by
  have hsum : low (storeWord mem (tAddr 4 j) w) =
      limbSum (fun k => if k = j then w.toNat else
        (MachineState.readWord mem (tAddr 4 k)).toNat) 4 := by
    apply limbSum_congr
    intro k hk
    by_cases he : k = j
    ...
    · rw [if_neg he, read_storeWord_outside]
      ...
      omega
  rw [hsum]
  exact limbSum_replace _ 4 j w.toNat hj
...
def products (mem : ByteArray) (x : Nat → UInt256) (y : UInt256) (i : Nat) : Nat → MacState
  ...
  | k+1 =>
    ...
    let t := MachineState.readWord p.memory (tAddr 4 (i+k))
    ⟨storeWord p.memory (tAddr 4 (i+k)) (macSum (x (i+k)) y t p.carry),
...
theorem products_invariant (mem : ByteArray) (x : Nat → UInt256) (y : UInt256) (i : Nat) :
  ...
  | succ k ih =>
    ...
    change low (storeWord p.memory (tAddr 4 (i+k)) (macSum (x (i+k)) y t p.carry)) +
...
theorem read_products_outside (mem : ByteArray) (x : Nat → UInt256) (y : UInt256)
    (i addr : Nat) (hd : addr+32 ≤ 8256 ∨ 8384 ≤ addr) :
    ∀ k, i+k ≤ 4 →
...
end Challenge.Modexp.Submission.Proofs.Fast.SquareFourProducts
