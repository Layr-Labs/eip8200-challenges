...
namespace Challenge.Modexp.Submission.Proofs.Fast.TnCacheStores
...
theorem run_middle (s : State) (c bi pbi hd pb ent tn target inv : UInt256)
    (rest : List UInt256) (hcap : rest.length ≤ 1006) :
    runInstructions middle
      (framed s (UInt256.ofNat 0)
        ([c,bi,pbi,hd,pb,ent,tn,allOnes,target,inv] ++ rest)) =
    some (framed s (UInt256.ofNat 9)
...
theorem run_tail (s : State) (c f pbi hd pb ent tn target inv : UInt256)
    ...
    some (framed {s with memory := (MachineState.writeBytes s.memory
      (Data.Bytes.natToBytesPadded (tn+c).toNat 32) 2112)} (UInt256.ofNat 12)
...
end Challenge.Modexp.Submission.Proofs.Fast.TnCacheStores
