...
# Adjacent no-op transposition at the 710 run-tail landing pad

Prepared: 2026-09-17T21:35Z
Parent: da5f2e264c53290ff01a2ab18fef52baa7fe4874 (current promoted frontier).
Artifact: raw-byte SHA-256
  8b807bd83e580ae17758603a1cf797619b3895ad1813acb3cb449e08e2abdd4a
Artifact size: 5439 bytes, 4393 instructions.

Executable change: PRESENT, two bytes at one position. The window [710, 712)
reads `JUMPDEST; DUP7` in the parent and `DUP7; JUMPDEST` in this image. The
JUMPDEST at 710 is the tail of the four-destination run at 707-710 left by the
setup unroller; no push immediate anywhere in the image names 710 or 711, no
computed target formula reaches the offset, and no `isValidJumpDest` certificate
or `Block` anchor names either pc. The only located witness covering the window
is the setup path segment in `Proofs/Fast/Setup.lean` (indices 504-505), which is
reached only by fall-through. On every input the two orders execute the same
instructions in the same order with the same stack, memory and program counter,
so the gas of every path through the window is unchanged; the image differs only
in which byte carries the marker.

Proof changes: PRESENT, in three files under this directory. The byte-level chunk
that carries the window is regenerated; the instruction list in
`Proofs/Bytecode/Artifact.lean` swaps the two entries at indices 504 and 505 to
match; and the located witness in `Proofs/Fast/Setup.lean` is updated in the same
way. No program counter moves, so every `instructionPC` table, every
`isValidJumpDest` certificate and every gas constant is unchanged.
