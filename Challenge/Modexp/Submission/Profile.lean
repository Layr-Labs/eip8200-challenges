...
def main (args : List String) : IO Unit := do
  let path := (args.find? (fun arg => !arg.startsWith "--")).getD
    "Challenge/Modexp/Submission/bytecode.hex"
  let code := Hex.hexToBytes (← IO.FS.readFile path).trimAscii.copy
  ...
  let sites := (Array.range code.size).filter fun pc =>
    !args.contains "--jumps" || code[pc]! == 0x5b
  let ranked := sites.qsort (fun a b => costs[a]! > costs[b]!)
