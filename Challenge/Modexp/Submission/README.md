...
## This submission: the slot-15 carry channel image (5,444 bytes, 4,387 instructions)

`bytecode.hex` in this directory is NOT the inherited 5,439-byte artifact
described below (that text is kept because it documents the lineage this image
is derived from). Two mechanical changes are stacked on the inherited image:

1. CUT-P "padding swallow" (-608 gas/seed, constant): two live 18-byte JUMPDEST
   spacers are repacked as `PUSH16` immediates, so the spacer bytes are skipped
   in one instruction instead of being executed one JUMPDEST at a time.
2. The slot-15 carry channel (-870 gas/seed, constant): the CIOS row-carry
