...
bit and jumps directly to the select preamble on a clear bit. The three
limb-loop counter tails in the masked-add helper are shortened
(`PUSH1 1; ADD` instead of `PUSH1 1; DUP2; ADD; SWAP1; POP`, padding with
unreachable `PUSH0`), the helper's select uses an XOR formulation that
consumes its temporaries, and the one-word path stores and returns from
memory offset zero instead of `0x1800`. -/
