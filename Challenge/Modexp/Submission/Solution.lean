...
uses an equivalent XOR expression that consumes its temporary words. The
word-sized return paths use memory offset zero rather than `0x1800`. These
...
step still runs, and the proof tracks the conditional memory state. -/
