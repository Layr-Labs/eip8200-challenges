...
/- Superseded standalone Ch composition. -/
/-
theorem t1_cost_potential (s : State) (msgOff returnDest : UInt256)
    ...
    (Compression.gasSteps_t1 s msgOff returnDest rest j hj hcap hcode hfork
      ...
      254 + MachineState.memCost
        (Compression.afterT1 s msgOff returnDest rest j).activeWords.toNat := by
  have hcond := blockCost_potential_of_static Compression.conditionPath 24
  ...
  have hfinish := blockCost_potential_of_static Compression.finishT1Path 1
...
theorem t1_cost_potential (s : State) (msgOff returnDest : UInt256)
  ...
  have hsetupK := blockCost_potential_of_static Compression.setupKPath 24
    ...
    (by simpa [Compression.gotW, Compression.loadedE,
      Accessors.loadReturned, State.fork] using hfork)
    (by simp [Compression.setupKPath, CopyFree]) (by rfl)
  have hsetupT1 := blockCost_potential_of_static Compression.setupT1Path 26
    (Compression.run_setupT1 s msgOff returnDest rest j (by omega) hcode hrun)
    (by simpa [Compression.gotK, Compression.gotW, Compression.loadedE,
      Accessors.kAtReturned, Accessors.loadReturned, State.fork] using hfork)
    (by simp [Compression.setupT1Path, CopyFree]) (by rfl)
  have hb1 := ArithmeticGas.gasSteps_bigSigma1_cost_potential
    (Compression.loadedT1Inputs s msgOff returnDest rest j)
    (Compression.hValue s 4) (Compression.hValue s 5) (Compression.hValue s 6)
    (Compression.kValue s j) (Compression.wValue s j)
    ([Compression.hValue s 4, UInt256.ofNat j, msgOff, returnDest] ++ rest)
    (by simp; omega)
    (by simpa [Compression.loadedT1Inputs, Compression.gotK, Compression.gotW,
    ...
    (by simpa [Compression.loadedT1Inputs, Compression.gotK, Compression.gotW,
      ...
      using hnp)
  have haw :
      (Compression.callIntegratedT1 s msgOff returnDest rest j).activeWords =
        (Compression.loadedT1Inputs s msgOff returnDest rest j).activeWords := by
    rfl
  simp only [Compression.gasSteps_t1, Compression.gasSteps_condition,
    Challenge.EvmProof.GasSteps.trans_cost,
    Challenge.EvmProof.Stepper.runLocatedBlock_sound_cost, id_eq]
  rw [haw] at hsetupT1
  change _ = 108 + MachineState.memCost
    (Compression.afterT1 s msgOff returnDest rest j).activeWords.toNat at hb1
  omega
