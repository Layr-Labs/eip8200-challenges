import Challenge.Sha256.Submission.Proofs.Bytecode.Reference
...
namespace Challenge.Sha256.Submission.Proofs.Bytecode.Main
...
def gasSteps_initStore (s : State) (w : Artifact.InitStore)
  ...
  have pcBound (i : Nat) : Artifact.instructionPC i < 2 ^ 256 := by
    ...
    change Artifact.instructionPC i ≤ submissionBytecode.size at hle
    rw [submissionBytecode_size] at hle
    omega
...
def gasSteps_initialize (calldata : ByteArray) :
    Challenge.EvmProof.GasSteps
      (initialState submissionBytecode calldata 0) (initializedState calldata) := by
...
end Challenge.Sha256.Submission.Proofs.Bytecode.Main
