...
namespace Challenge.Sha256.Submission.Proofs.Bytecode.Driver
...
theorem run_call (s : State) (input : ByteArray)
  ...
  have hdest : Decode.isValidJumpDest submissionBytecode 612 = true := by decide
...
private theorem run_increment_core (q : State) (input : ByteArray) (i : Nat)
  ...
  have hdest : Decode.isValidJumpDest submissionBytecode 1369 = true := by decide
  ...
  simp [incrementPath, Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    hpc, hstack, hcode, hrun, hadd, hadd', hdest, List.exchange]
...
theorem run_increment (s : State) (input : ByteArray)
  ...
  have qcode : (afterCompression s input i).executionEnv.code =
      submissionBytecode := by simpa using hcode
...
def gasSteps_reference (input : ByteArray) (hfit : CalldataFits input) :
  ...
  have finalCode : final.executionEnv.code = submissionBytecode := by
    simp [final]
...
end Challenge.Sha256.Submission.Proofs.Bytecode.Driver
