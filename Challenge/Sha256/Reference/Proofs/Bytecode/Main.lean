...
theorem gasSteps_initStore (s : State) (w : Artifact.InitStore)
    ...
    Challenge.BytecodeProof.GasSteps s (applyInitStore s w) := by
  ...
  have pc₁ : s₁.pc = UInt256.ofNat (Artifact.instructionPC (w.index + 1)) := by
    ...
    congr 1
    omega
  have g₁ : Challenge.BytecodeProof.GasSteps s s₁ := by
    apply Challenge.BytecodeProof.GasStep.pushN w.valueWidth w.value w.valueWidth.val
      valuePos
      decValue (by simp [hstack]) hrun hnp
  have pc₁ToNat : s₁.pc.toNat = Artifact.instructionPC (w.index + 1) := by
    rw [pc₁, Challenge.BytecodeProof.Word.word_toNat_ofNat,
  ...
  have pc₂ : s₂.pc = UInt256.ofNat (Artifact.instructionPC (w.index + 2)) := by
    ...
    omega
  have g₂ : Challenge.BytecodeProof.GasSteps s₁ s₂ := by
    apply Challenge.BytecodeProof.GasStep.pushN w.offsetWidth w.offset w.offsetWidth.val
      offsetPos
  ...
  have decStore : s₂.decodedOp = some .MSTORE := by
    ...
    · exact Artifact.decodeAt_op_index (w.index + 2) .MSTORE hv.2.2.2.2.1
        (by decide) trivial
    · rfl
  have g₃ : Challenge.BytecodeProof.GasSteps s₂ (applyInitStore s w) := by
    have pc₃ : s₂.pc.succ = UInt256.ofNat (Artifact.instructionPC (w.index + 3)) := by
      rw [pc₂, Challenge.BytecodeProof.Word.succ_ofNat (by
        rw [← hv.2.2.2.2.2]
        exact pcBound (w.index + 3))]
      congr 1
      omega
    have gm := Challenge.BytecodeProof.GasStep.mstore (s := s₂) w.offset w.value []
...
theorem gasSteps_initStores (s : State) (ws : List Artifact.InitStore)
    ...
    (hnp : Precompile.isPrecompile s.executionEnv.fork
      s.executionEnv.codeAddr = false) :
    Challenge.BytecodeProof.GasSteps s (ws.foldl applyInitStore s) := by
  induction ws generalizing s with
  | nil => exact Challenge.BytecodeProof.GasSteps.refl s
  | cons w ws ih =>
