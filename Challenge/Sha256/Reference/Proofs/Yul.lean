...
`Reference/reference.yul` is the challenge's reference implementation:
SHA-256 in the verified Yul fragment, reading the message from calldata and
returning the 32-byte digest. It is the incumbent on the leaderboard —
deliberately written for provability rather than gas, so beating it is easy
and *proving* you beat it is the challenge.
