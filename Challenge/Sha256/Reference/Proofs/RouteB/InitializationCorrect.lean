import Challenge.Sha256.Reference.Proofs.RouteB.Main
...
namespace Challenge.Sha256.RouteB.InitializationCorrect
...
def ConstantsCorrect (memory : ByteArray) : Prop :=
  (∀ j : Nat, j < 64 →
    kWord memory j = Challenge.RouteB.Word.ofUInt32 Sha256.K[j]!) ∧
  (∀ i : Nat, i < 8 →
    hWord memory i = Challenge.RouteB.Word.ofUInt32 Sha256.H0[i]!)
...
private theorem initializedMemory_k_0_8 (j : Nat)
  ...
  interval_cases j <;>
    norm_num [Sha256.K, Challenge.RouteB.Word.ofUInt32,
      Challenge.RouteB.Word.word_toNat_ofNat] <;> decide
...
private theorem initializedMemory_h_fin (i : Fin 8) :
    hWord initializedMemory i.val =
      Challenge.RouteB.Word.ofUInt32 Sha256.H0[i.val]! := by
  fin_cases i <;>
  ...
  all_goals
    repeat'
      ...
      | rw [Challenge.RouteB.Memory.readWord_writeBytes_disjoint _ _ _ _
...
theorem initializedState_hWord (input : ByteArray) (i : Nat) (hi : i < 8) :
    hWord (Main.initializedState input).memory i =
      Challenge.RouteB.Word.ofUInt32 Sha256.H0[i]! := by
  rw [initializedState_memory]
...
end Challenge.Sha256.RouteB.InitializationCorrect
