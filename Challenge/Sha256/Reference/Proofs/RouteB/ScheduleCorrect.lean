import Challenge.Sha256.Reference.Proofs.RouteB.Schedule
...
namespace Challenge.Sha256.RouteB.ScheduleCorrect
...
private theorem recurrence_of_words (a b c d : UInt32) :
    Challenge.RouteB.Word.mask32
        ((Word.evmSmallSigma1 (Challenge.RouteB.Word.ofUInt32 a) +
            Challenge.RouteB.Word.ofUInt32 b) +
          (Word.evmSmallSigma0 (Challenge.RouteB.Word.ofUInt32 c) +
            Challenge.RouteB.Word.ofUInt32 d)) =
  ...
  rw [Word.evmSmallSigma1_ofUInt32, Word.evmSmallSigma0_ofUInt32,
    mask32_add_distrib, Challenge.RouteB.Word.mask32_add,
    Challenge.RouteB.Word.mask32_add, Challenge.RouteB.Word.mask32_add]
...
theorem recurrenceWord_eq (s : State) (msgOff returnDest : UInt256)
    ...
    (h2 : Schedule.wValue s (j - 2) =
      Challenge.RouteB.Word.ofUInt32 (scheduleWord padded blockOff (j - 2)))
    (h7 : Schedule.wValue s (j - 7) =
      Challenge.RouteB.Word.ofUInt32 (scheduleWord padded blockOff (j - 7)))
    (h15 : Schedule.wValue s (j - 15) =
      Challenge.RouteB.Word.ofUInt32 (scheduleWord padded blockOff (j - 15)))
    (h16 : Schedule.wValue s (j - 16) =
      Challenge.RouteB.Word.ofUInt32 (scheduleWord padded blockOff (j - 16))) :
    Schedule.recurrenceWord s msgOff returnDest rest j =
      Challenge.RouteB.Word.ofUInt32 (scheduleWord padded blockOff j) := by
...
def SlotsCorrect (s : State) (padded : ByteArray) (blockOff upto : Nat) : Prop :=
  ∀ k, k < upto → Schedule.wValue s k =
    Challenge.RouteB.Word.ofUInt32 (scheduleWord padded blockOff k)
...
theorem firstLoopState_slots (s : State) (msgOff returnDest : UInt256)
  ...
  | succ n ih =>
      ...
      by_cases hkn : k = n
      · subst k
        rw [Challenge.RouteB.Memory.readWord_writeWord]
...
end Challenge.Sha256.RouteB.ScheduleCorrect
