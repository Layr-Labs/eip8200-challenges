import Challenge.Sha256.Reference.Proofs.RouteB
import Challenge.Sha256.Reference.Proofs.RouteB.Artifact
...
@[simp] theorem withGas_atPC (calldata : ByteArray) (pc gas : Nat) :
    Challenge.RouteB.withGas (atPC calldata pc) gas =
...
theorem gasSteps_jumpdest_at (calldata : ByteArray) (pc : Nat)
    ...
    Challenge.RouteB.GasSteps (atPC calldata pc) (atPC calldata (pc + 1)) := by
    ...
    have hd : (atPC calldata pc).decoded = some (.JUMPDEST, none) := by
      ...
      rw [show (atPC calldata pc).pc.toNat = pc by
        rw [show (atPC calldata pc).pc = UInt256.ofNat pc by rfl,
          Challenge.RouteB.Word.word_toNat_ofNat,
          Nat.mod_eq_of_lt (by omega : pc < 2 ^ 256)]]
      rw [hdecode]
      rfl
    rw [hd]
    rfl
  have hsucc := Challenge.RouteB.Word.succ_ofNat hpc
  apply Challenge.RouteB.GasStep.of_running 1 rfl deployAddress_not_precompile
  intro gas hgas
  have hstep := StepRunning.jumpdest
    (s := Challenge.RouteB.withGas (atPC calldata pc) gas)
    hop (by simpa [atPC, frame, State.fork, Gas.baseCost] using hgas)
    (by simp [Challenge.RouteB.withGas, atPC, frame,
      Operation.pushArity, Operation.popArity])
  simpa [Challenge.RouteB.withGas, atPC, hsucc, Gas.baseCost] using hstep
...
theorem gasSteps_push2_at (calldata : ByteArray) (pc dest : Nat)
    ...
    Challenge.RouteB.GasSteps (atPC calldata pc)
      (atPCStack calldata (pc + 3) [UInt256.ofNat dest]) := by
  have hpcadd := Challenge.RouteB.Word.ofNat_add_ofNat hpc
  apply Challenge.RouteB.GasStep.of_running 3 rfl deployAddress_not_precompile
  intro gas hgas
  have hstep := StepRunning.pushN
    ...
    (by decide) (by
      unfold State.decoded
      rw [show (Challenge.RouteB.withGas (atPC calldata pc) gas).executionEnv.code =
        referenceBytecode by rfl]
      rw [show (Challenge.RouteB.withGas (atPC calldata pc) gas).pc.toNat = pc by
        rw [show (Challenge.RouteB.withGas (atPC calldata pc) gas).pc =
    ...
    (by simp [Challenge.RouteB.withGas, atPC, frame])
  simpa [Challenge.RouteB.withGas, atPC, atPCStack, hpcadd,
...
theorem gasSteps_jump_at (calldata : ByteArray) (pc dest : Nat)
  ...
  have hop : (atPCStack calldata pc [UInt256.ofNat dest]).decodedOp =
    ...
    rw [hd]
    rfl
  apply Challenge.RouteB.GasStep.of_running 8 rfl deployAddress_not_precompile
  intro gas hgas
  have hstep := StepRunning.jump
    (s := Challenge.RouteB.withGas
    ...
    (by simp [Challenge.RouteB.withGas, atPCStack, frame])
    (by
    ...
    (by simp [Challenge.RouteB.withGas, atPCStack, frame,
      Operation.pushArity, Operation.popArity])
  simpa [Challenge.RouteB.withGas, atPC, atPCStack, frame,
...
theorem gasSteps_trampoline (calldata : ByteArray) (src dest : Nat)
    ...
    (hvalid : Decode.isValidJumpDest referenceBytecode dest = true) :
    Challenge.RouteB.GasSteps (atPC calldata src) (atPC calldata dest) := by
...
def entryTargets : List Nat :=
  Challenge.Sha256.RouteB.Artifact.entryTargets

def entryLinks : List (Nat × Nat) :=
  Challenge.Sha256.RouteB.Artifact.entryTrampolines.map
    (fun t => (t.src, t.dest))
...
theorem reference_decode_entry_push :
  ...
  have hv := Challenge.Sha256.RouteB.Artifact.initialEntry_valid
  simpa [hv.1] using
    Challenge.Sha256.RouteB.Artifact.decodeAt_push_index 0
...
theorem reference_decode_entry_jump :
  ...
  simpa [hv.2.2.1] using
    Challenge.Sha256.RouteB.Artifact.decodeAt_op_index 1 .JUMP
...
theorem reference_entryTarget_isValid {pc : Nat} (hpc : pc ∈ entryTargets) :
  ...
  exact Challenge.Sha256.RouteB.Artifact.entryTarget_isValid hpc
...
theorem reference_first_target_is_jumpdest :
  ...
  simp [entryTargets, Challenge.Sha256.RouteB.Artifact.entryTargets]
...
theorem gasSteps_to_firstTarget (calldata : ByteArray) :
    Challenge.RouteB.GasSteps (frame referenceBytecode calldata 0)
      (atPC calldata 0x001b) := by
  refine ⟨11, fun gas hgas => ?_⟩
  have hpush : 3 ≤ gas := by omega
  simpa [Challenge.RouteB.withGas, atPC, frame] using
...
theorem gasSteps_entryLink (calldata : ByteArray) (src dest : Nat)
  ...
  subst src
  subst dest
  have hv := Challenge.Sha256.RouteB.Artifact.entryTrampoline_valid t ht
  have hb : t.src + 5 < 2 ^ 256 ∧ t.dest < 2 ^ 256 := by
    simp only [Challenge.Sha256.RouteB.Artifact.entryTrampolines,
  ...
  · simpa [hv.1] using
      Challenge.Sha256.RouteB.Artifact.decodeAt_op_index t.srcIndex .JUMPDEST
        hv.2.1 (by decide) trivial
  · have hfit : (UInt256.ofNat t.dest).toNat < 256 ^ 2 := by
      rw [Challenge.RouteB.Word.word_toNat_ofNat, Nat.mod_eq_of_lt hb.2]
      simp only [Challenge.Sha256.RouteB.Artifact.entryTrampolines,
        List.mem_cons, List.not_mem_nil, or_false] at ht
      rcases ht with rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl |
        rfl | rfl | rfl | rfl | rfl | rfl <;> decide
    simpa [hv.2.2.1] using
      Challenge.Sha256.RouteB.Artifact.decodeAt_push_index (t.srcIndex + 1)
        ⟨2, by decide⟩ (UInt256.ofNat t.dest) hv.2.2.2.1 hfit
  · simpa [hv.2.2.2.2.1] using
      Challenge.Sha256.RouteB.Artifact.decodeAt_op_index (t.srcIndex + 2) .JUMP
        hv.2.2.2.2.2.1 (by decide) trivial
  · simpa [hv.2.2.2.2.2.2.1] using
      Challenge.Sha256.RouteB.Artifact.isValidJumpDest_index t.destIndex
        hv.2.2.2.2.2.2.2
...
theorem gasSteps_to_main (calldata : ByteArray) :
  ...
  have l₁ := gasSteps_entryLink calldata 0x001b 0x0044
    (by simp [entryLinks, Challenge.Sha256.RouteB.Artifact.entryTrampolines])
  ...
  have l₁₄ := gasSteps_entryLink calldata 0x025f mainPC
    (by simp [entryLinks, mainPC,
      Challenge.Sha256.RouteB.Artifact.entryTrampolines])
