import Challenge.Sha256.Reference.Proofs.RouteB.Compression
import Challenge.Sha256.Reference.Proofs.RouteB.ScheduleCorrect
import Challenge.Sha256.Reference.Proofs.RouteB.InitializationCorrect
import Challenge.Sha256.Reference.Proofs.RouteB.HashSpecBridge
...
namespace Challenge.Sha256.RouteB.CompressionCorrect
...
def Represents (s : State) (x : Working) : Prop :=
  Compression.hValue s 0 = Challenge.RouteB.Word.ofUInt32 x.a ∧
  Compression.hValue s 1 = Challenge.RouteB.Word.ofUInt32 x.b ∧
  Compression.hValue s 2 = Challenge.RouteB.Word.ofUInt32 x.c ∧
  Compression.hValue s 3 = Challenge.RouteB.Word.ofUInt32 x.d ∧
  Compression.hValue s 4 = Challenge.RouteB.Word.ofUInt32 x.e ∧
  Compression.hValue s 5 = Challenge.RouteB.Word.ofUInt32 x.f ∧
  Compression.hValue s 6 = Challenge.RouteB.Word.ofUInt32 x.g ∧
  Compression.hValue s 7 = Challenge.RouteB.Word.ofUInt32 x.h

private theorem mask32_add_distrib (x y : UInt256) :
    Challenge.RouteB.Word.mask32 (x + y) =
      Challenge.RouteB.Word.mask32
        (Challenge.RouteB.Word.mask32 x + Challenge.RouteB.Word.mask32 y) := by
  rw [Challenge.RouteB.Word.mask32_eq_ofUInt32,
    Challenge.RouteB.Word.mask32_eq_ofUInt32 x,
    Challenge.RouteB.Word.mask32_eq_ofUInt32 y,
    Challenge.RouteB.Word.mask32_add]
  congr 1
  apply UInt32.toNat_inj.mp
  simp only [Challenge.RouteB.Word.toUInt32_toNat, UInt32.toNat_add]
...
private theorem toUInt32_add (x y : UInt256) :
    Challenge.RouteB.Word.toUInt32 (x + y) =
      Challenge.RouteB.Word.toUInt32 x + Challenge.RouteB.Word.toUInt32 y := by
  apply UInt32.toNat_inj.mp
  simp only [Challenge.RouteB.Word.toUInt32_toNat, UInt32.toNat_add]
  change ((x.val + y.val).val % 2 ^ 32) =
    (x.toNat % 2 ^ 32 + y.toNat % 2 ^ 32) % 2 ^ 32
...
theorem t1_eq (s : State) (j : Nat) (x : Working) (k w : UInt32)
    (hx : Represents s x)
    (hk : Compression.kValue s j = Challenge.RouteB.Word.ofUInt32 k)
    (hw : Compression.wValue s j = Challenge.RouteB.Word.ofUInt32 w) :
    Compression.t1 s j = Challenge.RouteB.Word.ofUInt32
  ...
  rw [Challenge.RouteB.Word.mask32_eq_ofUInt32]
  congr 1
  simp only [toUInt32_add, Challenge.RouteB.Word.toUInt32_ofUInt32]
  ac_rfl

theorem t2_eq (s : State) (x : Working) (hx : Represents s x) :
    Compression.t2 s = Challenge.RouteB.Word.ofUInt32
...
private theorem hSlot_eq (i : Nat) (hi : i < 8) :
  ...
  rw [Challenge.RouteB.Word.shiftLeft_ofNat (by omega) (by decide) (by omega)]
  rw [Challenge.RouteB.Word.ofNat_add_ofNat (by omega),
    Challenge.RouteB.Word.word_toNat_ofNat, Nat.mod_eq_of_lt (by omega)]
  omega
...
private theorem readH_writeH_same (memory : ByteArray) (i : Nat)
  ...
  exact Challenge.RouteB.Memory.readWord_writeWord memory (288 + i * 32) value
...
private theorem readH_writeH_ne (memory : ByteArray) (read write : Nat)
  ...
  apply Challenge.RouteB.Memory.readWord_writeBytes_disjoint
...
theorem afterSecondIteration_represents (s : State)
  ...
  let T1 := x.h + Sha256.bigSigma1 x.e + Sha256.Ch x.e x.f x.g + k + w
  let T2 := Sha256.bigSigma0 x.a + Sha256.Maj x.a x.b x.c
  have hnewA : Challenge.RouteB.Word.mask32
      (Compression.t1 s j + Compression.t2 s) =
      Challenge.RouteB.Word.ofUInt32 (T1 + T2) := by
    rw [ht1, ht2, Challenge.RouteB.Word.mask32_add]
  have hnewE : Compression.newH4 s msgOff returnDest rest j =
      Challenge.RouteB.Word.ofUInt32 (x.d + T1) := by
    unfold Compression.newH4
    change Challenge.RouteB.Word.mask32
      (Compression.hValue q5 3 + Compression.t1 s j) = _
    rw [hq5d]
    rcases hx with ⟨ha, hb, hc, hd, he, hf, hg, hh⟩
    rw [hd, ht1, Challenge.RouteB.Word.mask32_add]
...
def RoundInputsCorrect (s : State) (msgOff returnDest : UInt256)
  ...
  ∀ n, n < 64 →
    Compression.kValue
      ...
      Challenge.RouteB.Word.ofUInt32 Sha256.K[n]! ∧
    Compression.wValue
        (Compression.roundLoopState s msgOff returnDest rest n) n =
      Challenge.RouteB.Word.ofUInt32
        (ScheduleCorrect.scheduleWord padded blockOff n)
...
private theorem readBE32_writeH (memory : ByteArray) (j write : Nat)
  ...
  · have hw : idx < (writeH memory write value).size := by
    ...
    rw [Challenge.RouteB.Memory.getD0_eq_getElem _ _ hw,
      Challenge.RouteB.Memory.getD0_eq_getElem _ _ hm] at hg
    exact hg
  · by_cases hw : idx < (writeH memory write value).size
    · rw [dif_pos hw, dif_neg hm]
      ...
      change (writeH memory write value)[idx]?.getD 0 = memory[idx]?.getD 0 at hg
      rw [Challenge.RouteB.Memory.getD0_eq_getElem _ _ hw,
        Challenge.RouteB.Memory.getElem?_getD_eq_zero_of_size_le _ _
          (by omega)] at hg
...
theorem represents_get (s : State) (x : Working) (hx : Represents s x)
    (i : Nat) (hi : i < 8) :
    Compression.hValue s i = Challenge.RouteB.Word.ofUInt32 (x.get i) := by
  rcases hx with ⟨ha, hb, hc, hd, he, hf, hg, hh⟩
  interval_cases i
  · exact ha
  · exact hb
...
def SavedRepresents (s : State) (H : Array UInt32) : Prop :=
  ∀ i, i < 8 → Compression.savedValue s i =
    Challenge.RouteB.Word.ofUInt32 H[i]!
...
theorem foldLoopState_correct (s : State) (msgOff returnDest : UInt256)
  ...
  | succ n ih =>
      ...
      have hv : Compression.foldedValue q msgOff returnDest rest n =
          Challenge.RouteB.Word.ofUInt32 (H[n]! + x.get n) := by
...
private theorem readPadded_write_inside (memory bytes : ByteArray)
  ...
  · intro i hleft hright
    ...
    rw [← Challenge.RouteB.Memory.getD0_eq_getElem _ _ hleft,
      ← Challenge.RouteB.Memory.getD0_eq_getElem _ _ hright,
      Challenge.RouteB.Memory.readPadded_getElem?_getD,
      Challenge.RouteB.Memory.readPadded_getElem?_getD,
      if_pos hi, if_pos hi]
...
private theorem readPadded_readPadded (memory : ByteArray)
  ...
  · intro i hleft hright
    ...
    rw [← Challenge.RouteB.Memory.getD0_eq_getElem _ _ hleft,
      ...
      if_pos hi]
    rw [if_pos (by omega)]
    rw [Challenge.RouteB.Memory.readPadded_getElem?_getD]
    rw [if_pos hi]
    congr 2
    omega
...
theorem copyHashState_kValue (s : State) (k : Nat) (hk : k < 64) :
  ...
  rw [Challenge.RouteB.Memory.readWord_writeBytes_disjoint]
  left
  omega
...
theorem copyHashState_wValue (s : State) (j : Nat) (hj : j < 64) :
  ...
  rw [ScheduleCorrect.scheduleSlot_eq j hj,
    Challenge.RouteB.Memory.readPadded_size]
  simp
...
theorem compressResult_eq_compressBlock (s : State)
    ...
    (hK : ∀ j, j < 64 → Compression.kValue s j =
      Challenge.RouteB.Word.ofUInt32 Sha256.K[j]!)
  ...
  have hpK : ∀ j, j < 64 → Compression.kValue prepared j =
      Challenge.RouteB.Word.ofUInt32 Sha256.K[j]! := by
    intro j hj
...
end Challenge.Sha256.RouteB.CompressionCorrect
