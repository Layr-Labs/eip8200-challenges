import Challenge.Sha256.Reference.Proofs.RouteB.PaddingTrace
import Challenge.Sha256.Reference.Proofs.RouteB.Compression
import Challenge.Sha256.Reference.Proofs.RouteB.Output
...
namespace Challenge.Sha256.RouteB.Driver
...
private theorem run_setup (input : ByteArray) :
    Challenge.RouteB.Stepper.runLocatedBlock setupPath
      (PaddingTrace.padReturned input) = some (blockLoopState input 0) := by
  simp [setupPath, Challenge.RouteB.Stepper.runLocatedBlock,
...
theorem gasSteps_setup (input : ByteArray) :
    Challenge.RouteB.GasSteps (PaddingTrace.padReturned input)
...
private theorem run_condition_continue (s : State) (input : ByteArray)
    (hfit : CalldataFits input) (i : Nat) (hi : i < blockCount input)
    (hrun : s.halt = .Running) :
    Challenge.RouteB.Stepper.runLocatedBlock conditionPath (loopAt s input i) =
...
private theorem run_call (s : State) (input : ByteArray)
    ...
    (hrun : s.halt = .Running) :
    Challenge.RouteB.Stepper.runLocatedBlock callPath
...
private theorem run_increment_core (q : State) (input : ByteArray) (i : Nat)
    ...
    (hadd : blockOffsetWord i + UInt256.ofNat 64 =
      blockOffsetWord (i + 1)) :
    Challenge.RouteB.Stepper.runLocatedBlock incrementPath q =
      some { q with
        pc := UInt256.ofNat 1369
  ...
  simp [incrementPath, Challenge.RouteB.Stepper.runLocatedBlock,
    Challenge.RouteB.Stepper.runLocated, Challenge.RouteB.Stepper.runInstr,
...
private theorem run_increment (s : State) (input : ByteArray)
    ...
    (hcode : s.executionEnv.code = referenceBytecode)
    (hrun : s.halt = .Running) :
    Challenge.RouteB.Stepper.runLocatedBlock incrementPath
  ...
  have hadd : blockOffsetWord i + UInt256.ofNat 64 =
    ...
    simpa [blockOffsetWord, blockOffset, Nat.add_mul] using
      Challenge.RouteB.Word.ofNat_add_ofNat (a := i * 64) (b := 64) haddBound
...
theorem gasSteps_iteration (s : State) (input : ByteArray)
    ...
    Challenge.RouteB.GasSteps (loopAt s input i) (afterIteration s input i) := by
  have gCondition : Challenge.RouteB.GasSteps (loopAt s input i)
    ...
    · exact hrun
    · exact hnp
  have gCall : Challenge.RouteB.GasSteps (afterCondition s input i)
  ...
  have qnp : Precompile.isPrecompile
    ...
    simpa using hnp
  have gIncrement : Challenge.RouteB.GasSteps (afterCompression s input i)
    ...
    · exact qrun
    · exact qnp
  have gCompress' : Challenge.RouteB.GasSteps
      (Compression.compressEntry (loopAt s input i) (messageOffsetWord i)
        (UInt256.ofNat 1390) [blockOffsetWord i, Padding.paddedWord input])
...
theorem gasSteps_blockLoop (input : ByteArray) (hfit : CalldataFits input) :
    Challenge.RouteB.GasSteps (blockLoopState input 0)
      (blockLoopState input (blockCount input)) := by
  apply Challenge.RouteB.GasSteps.iterateBounded (count := blockCount input)
  intro i hi
...
theorem gasSteps_exit (input : ByteArray) (hfit : CalldataFits input) :
    Challenge.RouteB.GasSteps (blockLoopState input (blockCount input))
...
end Challenge.Sha256.RouteB.Driver
