...
namespace Challenge.Sha256.RouteB.PaddedBlockBridge

open EvmSemantics
open EvmSemantics.Crypto
open EvmSemantics.EVM

private theorem shl8_or_byte (w : UInt32) (b : UInt8) :
    (w <<< 8) ||| b.toUInt32 = w * 256 + b.toUInt32 := by
  bv_decide
...
private theorem readPadded_four_eq (bs : ByteArray) (off : Nat) :
  ...
  · rw [Challenge.RouteB.Memory.readPadded_size]
    rfl
  · intro i hi₁ hi₂
    have hi : i < 4 := by simpa using hi₁
    rw [← Challenge.RouteB.Memory.getD0_eq_getElem _ _ hi₁,
      Challenge.RouteB.Memory.readPadded_getElem?_getD, if_pos hi]
...
private theorem bytesToBigEndianNat_readPadded_four (bs : ByteArray)
  ...
  simp [Data.Bytes.bytesToBigEndianNat,
    Challenge.RouteB.Bytecode.toList_eq_data]
...
private theorem readPadded_thirtyTwo_split (bs : ByteArray) (off : Nat) :
  ...
  · intro i hi₁ hi₂
    ...
    rw [if_pos hi, Challenge.RouteB.Memory.getElem?_getD_append]
    simp only [Challenge.RouteB.Memory.readPadded_size]
    by_cases h4 : i < 4
    · rw [if_pos h4,
        Challenge.RouteB.Memory.readPadded_getElem?_getD, if_pos h4]
    · rw [if_neg h4,
        Challenge.RouteB.Memory.readPadded_getElem?_getD, if_pos (by omega)]
      congr 2
      omega
...
theorem shiftRight_readWord_224 (bs : ByteArray) (off : Nat) :
    UInt256.shiftRight (MachineState.readWord bs off) (UInt256.ofNat 224) =
      Challenge.RouteB.Word.ofUInt32 (Sha256.readBE32 bs off) := by
  ...
  rw [Challenge.RouteB.Word.shiftRight_ofNat hvalue (by omega)]
  ...
  unfold Challenge.RouteB.Word.ofUInt32
  apply Challenge.RouteB.Word.word_ext
  rw [Challenge.RouteB.Word.word_toNat_ofNat,
    Challenge.RouteB.Word.word_toNat_ofNat, UInt32.toNat_ofNat']
...
theorem readPadded_paddedMemory_shift (base input : ByteArray)
  ...
  · intro i hi₁ hi₂
    ...
    · rw [if_pos (by omega)]
      ...
      omega
    · rw [if_neg (by omega)]
      rw [Challenge.RouteB.Memory.getElem?_getD_eq_zero_of_size_le base _
        (by omega)]
      exact (Challenge.RouteB.Memory.getElem?_getD_eq_zero_of_size_le
...
private theorem loadOffset_eq (input : ByteArray) (blockOff k : Nat)
  ...
  rw [Challenge.RouteB.Word.shiftLeft_ofNat (by omega) (by omega) (by omega),
    Challenge.RouteB.Word.ofNat_add_ofNat (by omega),
    Challenge.RouteB.Word.word_toNat_ofNat, Nat.mod_eq_of_lt]
  · omega
  · omega
...
end Challenge.Sha256.RouteB.PaddedBlockBridge
