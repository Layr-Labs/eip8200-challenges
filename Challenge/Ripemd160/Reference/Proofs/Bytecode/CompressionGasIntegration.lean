...
/- The remaining composition declarations move to `CompressionGasComposition`
so this expensive round-by-round layer can be cached independently.
...
theorem rightLoopAndTail_cost_potential (s : State)
  ...
  have h012 := potential_trans _ _ _ _ _ _ _ h01 h2
  have hall := potential_trans _ _ _ _ _ _ _ h012 h3
  simpa [CompressionTailTrace.gasSteps_rightLoopAndTail,
    rightLoopAndTailWork, q, GasSteps.trans_cost, Nat.add_assoc] using hall
...
theorem compressToRight_cost_potential (s : State)
  ...
  have h3 := blockCost_potential leftInitLocated
    ...
    (by simpa [copiesReturned] using hsFork)
  ...
  have h5 := blockCost_potential leftTestLocated
    ...
    (leftExitTested final messageOffset returnDest rest)
  ...
  have h6 := blockCost_potential leftExitLocated
    ...
    (by simpa [leftExitTested] using hfFork)
  ...
  have h7 := blockCost_potential rightInitLocated
    ...
    (run_rightInit final messageOffset returnDest rest (by omega) hfRun)
  ...
  have hall := potential_trans _ _ _ _ _ _ _ h0123456 h7
  simpa [gasSteps_compressToRight, compressToRightWork, tail, scheduled,
    initial, final, GasSteps.cast_cost, GasSteps.trans_cost,
    Nat.add_assoc] using hall
