...
def leftInlineSite (k : Fin 20) (hk : 16 ≤ k.val) :
...
theorem leftInlineSite_start (k : Fin 20) (hk : 16 ≤ k.val) :
    (leftInlineSite k hk).startPC = leftInlinePC k.val := by
  fin_cases k <;> first | omega | rfl

theorem leftInlineSite_end (k : Fin 20) (hk : 16 ≤ k.val) :
    (leftInlineSite k hk).endPC = leftInlinePC (k.val + 1) := by
  fin_cases k <;> first | omega | rfl

theorem leftghost_pc (k : Fin 20) (hk : 16 ≤ k.val) :
...
def leftenterPath : List (Stepper.Located Artifact .Osaka) :=
  [⟨1123, .push 3 5300, by rfl, by decide⟩,
   ⟨1124, .op .JUMP, by rfl, by decide⟩,
...
def left_enter (s : State)
    ...
    GasSteps {s with pc := UInt256.ofNat 1785}
  ...
  have hp1123 : Artifact.instructionPC 1123 = 1785 := by rfl
  have hp1124 : Artifact.instructionPC 1124 = 1789 := by rfl
  ...
  · simp [Stepper.runLocatedBlock, Stepper.runLocated, Stepper.runInstr,
      leftenterPath, hp1123, hp1124, hp2918, hrun, hstack, hv]
  · exact hrun
  · exact hnp

def leftexitPath : List (Stepper.Located Artifact .Osaka) :=
  [⟨3435, .push 2 1896, by rfl, by decide⟩,
   ⟨3436, .op .JUMP, by rfl, by decide⟩,
...
def left_exit (s : State)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 6081}
  ...
  have hp3435 : Artifact.instructionPC 3435 = 6081 := by rfl
  have hp3436 : Artifact.instructionPC 3436 = 6084 := by rfl
  have hp1170 : Artifact.instructionPC 1170 = 1896 := by rfl
  apply Stepper.runLocatedBlock_sound Artifact .Osaka leftexitPath
  · exact hcode
  · exact hfork
  · simp [Stepper.runLocatedBlock, Stepper.runLocated, Stepper.runInstr,
      leftexitPath, hp3435, hp3436, hp1170, hrun, hstack, hv]
  · exact hrun
  · exact hnp
