...
theorem run_tail_fallback_acc (input : ByteArray) (hneAcc : finalAcc input ≠ 0) :
  ...
  have htrue : UInt256.isTrue (finalAcc input) := by
    ...
    simpa using hz
  have hdest : Decode.isValidJumpDest submissionBytecode 358 = true :=
...
def gasSteps_direct_return (input : ByteArray) :
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 68 .MSIZE
    (by rfl) (by decide) trivial
  have hp : (storedReturnState input).pc.toNat =
      Artifact.submissionArtifact.instructionPC 68 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
  have hop : (storedReturnState input).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedReturnState input) 68
