...
def actualOutput (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v3 (UInt256.shiftRight (UInt256.mul x.v10 x.v9) (UInt256.ofNat 28))),
    ...
    x.v7,
    x.v0,
    (UInt256.land x.v3 (UInt256.add x.v5 (UInt256.shiftRight (UInt256.mul x.v10 (UInt256.land x.v3 (UInt256.add x.v7 (UInt256.add (MachineState.readWord memory 594) (UInt256.add (UInt256.xor x.v9 (UInt256.land (UInt256.xor x.v6 (UInt256.lor x.v4 x.v9)) (UInt256.xor (UInt256.land x.v4 x.v9) x.v0))) x.v8))))) (UInt256.ofNat 24)))),
    x.v10,
    x.v11,
    x.v12,
    x.v13,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2547).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2547 actual_slice
    (by change 2547 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3289 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2547) = UInt256.ofNat 3289
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3289, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3329, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3289) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3289) template = UInt256.ofNat 3329 := by decide
  rw [hend] at hraw
