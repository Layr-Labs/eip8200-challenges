...
private theorem template_slice :
    (A.instructions.drop 119).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 119 template_slice (by
    change 119 + template.length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) StackRoundData.artifact_code_bound template_wellFormed (by decide)

theorem site_start : site.startPC = UInt256.ofNat 201 := by rfl
theorem site_end : site.endPC = UInt256.ofNat 209 := by rfl
...
def gasSteps_fullWord (s : State) (rho : List UInt256) (hstack : rho.length < 1021)
    ...
    GasSteps {s with pc := UInt256.ofNat 201, stack := rho}
      {s with pc := UInt256.ofNat 209, stack := KnownInputData.fullWord :: rho} := by
