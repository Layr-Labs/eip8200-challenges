...
private theorem template_slice :
    (A.instructions.drop 3277).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 3277 template_slice (by
    change 3277 + template.length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) QuadLayout.code_bound template_wellFormed (by decide)

theorem site_start : site.startPC = UInt256.ofNat 5067 := by rfl
theorem site_end : site.endPC = UInt256.ofNat 5075 := by rfl
...
def gasSteps_fullWord (s : State) (rho : List UInt256) (hstack : rho.length < 1021)
    ...
    GasSteps {s with pc := UInt256.ofNat 5067, stack := rho}
      {s with pc := UInt256.ofNat 5075, stack := KnownInputData.fullWord :: rho} := by
