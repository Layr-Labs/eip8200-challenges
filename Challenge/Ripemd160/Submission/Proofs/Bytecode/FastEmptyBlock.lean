...
theorem run_decision_nonempty (s : State) (input : ByteArray) (i : Nat)
  ...
  have htrue : UInt256.isTrue (UInt256.ofNat input.size) := by
    exact hmod
  have hdest : Decode.isValidJumpDest submissionBytecode 422 = true := by
    have hpc : Artifact.submissionArtifact.instructionPC 257 = 422 := by
      rw [ArtifactByteLength.instructionPC_eq_byteLength]
      decide
    have h := Artifact.submissionArtifact.isValidJumpDest_index 257 (by rfl)
    rw [hpc] at h
    exact h
  have hpc2792 : Artifact.submissionArtifact.instructionPC 248 = 390 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hpc2793 : Artifact.submissionArtifact.instructionPC 248 = 390 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hpc2794 : Artifact.submissionArtifact.instructionPC 249 = 391 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hpc2795 : Artifact.submissionArtifact.instructionPC 250 = 394 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
