...
def decisionPath : List Located :=
  [⟨77, .op .CALLDATASIZE, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨78, .push ⟨2, by decide⟩ (UInt256.ofNat 5027), by rfl, by decide⟩,
...
theorem run_decision_nonempty (s : State) (input : ByteArray) (i : Nat)
  ...
  have hdest : Decode.isValidJumpDest submissionBytecode 5027 = true := by
    have hpc : Artifact.submissionArtifact.instructionPC 4092 = 5027 := by
      rw [ArtifactByteLength.instructionPC_eq_byteLength]
      decide
    have h := Artifact.submissionArtifact.isValidJumpDest_index 4092 (by rfl)
    rw [hpc] at h
    exact h
