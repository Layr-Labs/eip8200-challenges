...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 810, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 882, stack := outputStack s.memory x rho, memory := outputMemory s.memory x} := by
  have hraw := run_actual s (UInt256.ofNat 810) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 810) template = UInt256.ofNat 882 := by decide
  rw [hend] at hraw
