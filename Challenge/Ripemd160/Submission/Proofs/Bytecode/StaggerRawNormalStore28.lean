...
theorem site_pc : site.startPC = UInt256.ofNat 964 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 572) = UInt256.ofNat 964
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 964, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1036, stack := outputStack s.memory x rho, memory := outputMemory s.memory x} := by
  have hraw := run_actual s (UInt256.ofNat 964) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 964) template = UInt256.ofNat 1036 := by decide
  rw [hend] at hraw
