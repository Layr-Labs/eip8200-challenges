...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Serialized675316Endian8.Stage8
...
theorem run_endian (s : State) (startPC value : UInt256)
    (a b : UInt256) (rest : List UInt256) (hstack : rest.length < 1010)
    (hrun : s.halt = .Running) :
    runInstrSeq code {s with pc := startPC, stack := value :: a :: b :: mask8 :: rest} =
      some {s with
        pc := pcAfter startPC code
...
theorem run_located {artifact : DataProgramArtifact} {fork : Fork}
    ...
    (s : State) (value : UInt256) (a b : UInt256) (rest : List UInt256)
    (hstack : rest.length < 1010)
    (hrun : s.halt = .Running) :
    DataStepper.runLocatedBlock site.path {s with pc := site.startPC, stack := value :: a :: b :: mask8 :: rest} =
      some {s with pc := site.endPC, stack := packedStage value 8 mask8 :: a :: b :: mask8 :: rest} := by
  ...
  have hraw : DataStepper.runLocatedBlock site.path
      {s with pc := site.startPC, stack := value :: a :: b :: mask8 :: rest} =
      runInstrSeq code {s with pc := site.startPC, stack := value :: a :: b :: mask8 :: rest} := by
  ...
  rw [hraw, run_endian s site.startPC value a b rest hstack hrun, ← hend]
...
def gasSteps_endian {artifact : DataProgramArtifact} {fork : Fork}
    ...
    GasSteps {s with pc := site.startPC, stack := value :: a :: b :: mask8 :: rest}
      {s with pc := site.endPC, stack := packedStage value 8 mask8 :: a :: b :: mask8 :: rest} := by
  apply DataStepper.runLocatedBlock_sound artifact fork site.path
  · exact hcode
  · exact hfork
  · exact run_located site s value a b rest hstack hrun
  · exact hrun
  · exact hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Serialized675316Endian8.Stage8

namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Serialized675316Endian8.Stage16
open EvmSemantics EvmSemantics.EVM YulEvmCompiler Challenge.EvmProof
open Challenge.EvmProof.Word
open StackRoundTrace DenseScheduleTemplate DenseScheduleTrace
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Serialized675316Endian8.Stage16

namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Serialized675316Endian8
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Serialized675316Endian8
