...
private theorem code_bound : Artifact.submissionArtifact.code.size < 2^256 := by
  ...
  decide
namespace selector
abbrev template : List Instr := RecognitionSelectorRaw.prefixTemplate (UInt256.ofNat 4940)
...
theorem site_pc : site.startPC = UInt256.ofNat 303 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 202) = UInt256.ofNat 303
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 303) template = UInt256.ofNat 324 := by decide
...
theorem site_pc : site.startPC = UInt256.ofNat 326 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 215) = UInt256.ofNat 326
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
