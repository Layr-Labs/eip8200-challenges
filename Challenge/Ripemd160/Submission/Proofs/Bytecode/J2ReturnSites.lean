...
theorem site_pc : site.startPC = UInt256.ofNat 302 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 198) = UInt256.ofNat 302
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 302) template = UInt256.ofNat 321 := by decide
...
def lift (s t : State) (e : Env s) (stack : List UInt256)
    (h : runInstrSeq template (atState s 302 stack) = some t) : GasSteps (atState s 302 stack) t :=
...
theorem site_pc : site.startPC = UInt256.ofNat 323 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 211) = UInt256.ofNat 323
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
