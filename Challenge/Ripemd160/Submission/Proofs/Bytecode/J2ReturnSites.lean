...
theorem site_pc : site.startPC = UInt256.ofNat 543 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 288) = UInt256.ofNat 543
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 543) template = UInt256.ofNat 561 := by decide
...
def lift (s t : State) (e : Env s) (stack : List UInt256)
    (h : runInstrSeq template (atState s 543 stack) = some t) : GasSteps (atState s 543 stack) t :=
...
theorem actual_slice : (Artifact.submissionArtifact.instructions.drop 301).take template.length = template := by rfl
def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 301 actual_slice
    (by change 301 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    code_bound (StackRoundData.templateWellFormed_mem (instructions := template) (by decide)) (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 563 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 301) = UInt256.ofNat 563
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 563) template = UInt256.ofNat 565 := by decide
theorem form : ∀ instruction ∈ template.dropLast, RecognitionLift.Advances instruction :=
  RecognitionLift.advancesAll_sound _ (by decide)
def lift (s t : State) (e : Env s) (stack : List UInt256)
    (h : runInstrSeq template (atState s 563 stack) = some t) : GasSteps (atState s 563 stack) t :=
  RecognitionLift.gasSteps_of_raw site _ _ e.code e.fork e.run e.np site_pc.symm form h
end returned
