...
def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpTemplate :=
  StackSiteBuilder.ofSlice jumpTemplate 3834 jump_slice
    ...
    (by decide)
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4854 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3834) = UInt256.ofNat 4854
...
def gasSteps_jump (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1022)
    ...
    GasSteps {s with pc := UInt256.ofNat 4854, stack := rho}
      {s with pc := UInt256.ofNat 920, stack := rho} := by
  apply PadLift.gasSteps_of_raw jumpSite {s with pc := UInt256.ofNat 4854, stack := rho} _ hcode hfork hrun hnp jump_pc.symm jump_advances
  exact PadJump.run_template s (UInt256.ofNat 4854) rho 920 hstack hrun (valid_merge s hcode)
