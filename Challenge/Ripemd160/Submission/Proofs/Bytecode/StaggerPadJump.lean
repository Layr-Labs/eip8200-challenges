...
def jumpTemplate : List Instr := PadJump.template 969
theorem jump_slice :
...
def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpTemplate :=
  StackSiteBuilder.ofSlice jumpTemplate 279 jump_slice
    (by change 279 + jumpTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := jumpTemplate) (by decide))
    (by decide)
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 461 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 279) = UInt256.ofNat 461
...
theorem valid_merge (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 969).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 578 = 969 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 584 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 969 = true
  rw [hcode]
  exact h
...
def gasSteps_jump (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1022)
    ...
    GasSteps {s with pc := UInt256.ofNat 461, stack := rho}
      {s with pc := UInt256.ofNat 969, stack := rho} := by
  ...
  exact PadJump.run_template s (UInt256.ofNat 461) rho 969 hstack hrun (valid_merge s hcode)
