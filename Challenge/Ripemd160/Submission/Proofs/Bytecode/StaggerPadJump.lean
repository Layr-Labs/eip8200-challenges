...
theorem jump_slice :
    (Artifact.submissionArtifact.instructions.drop 3811).take jumpTemplate.length = jumpTemplate := by rfl
def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpTemplate :=
  StackSiteBuilder.ofSlice jumpTemplate 3811 jump_slice
    (by change 3811 + jumpTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4801 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3811) = UInt256.ofNat 4801
...
def gasSteps_jump (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1022)
    ...
    GasSteps {s with pc := UInt256.ofNat 4801, stack := rho}
      {s with pc := UInt256.ofNat 925, stack := rho} := by
  apply PadLift.gasSteps_of_raw jumpSite {s with pc := UInt256.ofNat 4801, stack := rho} _ hcode hfork hrun hnp jump_pc.symm jump_advances
  exact PadJump.run_template s (UInt256.ofNat 4801) rho 925 hstack hrun (valid_merge s hcode)
