...
theorem jump_slice :
    (Artifact.submissionArtifact.instructions.drop 3765).take jumpTemplate.length = jumpTemplate := by rfl
def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpTemplate :=
  StackSiteBuilder.ofSlice jumpTemplate 3765 jump_slice
    (by change 3765 + jumpTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4836 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3765) = UInt256.ofNat 4836
...
def gasSteps_jump (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1022)
  ...
  exact PadJump.run_template s (UInt256.ofNat 4836) rho 904 hstack hrun (valid_merge s hcode)
