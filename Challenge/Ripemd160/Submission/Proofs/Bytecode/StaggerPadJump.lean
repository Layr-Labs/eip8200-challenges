...
def jumpTemplate : List Instr := PadJump.template 915
theorem jump_slice :
...
def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpTemplate :=
  StackSiteBuilder.ofSlice jumpTemplate 3834 jump_slice
    ...
    (by decide)
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4849 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3834) = UInt256.ofNat 4849
...
theorem valid_merge (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 915).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 559 = 915 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 559 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 915 = true
  rw [hcode]
  exact h
...
def gasSteps_jump (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1022)
    ...
    GasSteps {s with pc := UInt256.ofNat 4849, stack := rho}
      {s with pc := UInt256.ofNat 915, stack := rho} := by
  apply PadLift.gasSteps_of_raw jumpSite {s with pc := UInt256.ofNat 4849, stack := rho} _ hcode hfork hrun hnp jump_pc.symm jump_advances
  exact PadJump.run_template s (UInt256.ofNat 4849) rho 915 hstack hrun (valid_merge s hcode)
