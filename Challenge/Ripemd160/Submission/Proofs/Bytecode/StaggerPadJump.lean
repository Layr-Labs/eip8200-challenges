...
def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpTemplate :=
  StackSiteBuilder.ofSlice jumpTemplate 3736 jump_slice
    ...
    (by decide)
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4864 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3733) = UInt256.ofNat 4864
...
def gasSteps_jump (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1022)
    ...
    GasSteps {s with pc := UInt256.ofNat 4864, stack := rho}
      {s with pc := UInt256.ofNat 922, stack := rho} := by
  apply PadLift.gasSteps_of_raw jumpSite {s with pc := UInt256.ofNat 4864, stack := rho} _ hcode hfork hrun hnp jump_pc.symm jump_advances
  exact PadJump.run_template s (UInt256.ofNat 4864) rho 894 hstack hrun (valid_merge s hcode)
