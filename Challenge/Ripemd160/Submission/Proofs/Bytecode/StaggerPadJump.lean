...
def jumpTemplate : List Instr := PadJump.template 922
theorem jump_slice :
    (Artifact.submissionArtifact.instructions.drop 3733).take jumpTemplate.length = jumpTemplate := by rfl
...
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4836 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3733) = UInt256.ofNat 4836
