...
theorem jump_slice :
    (Artifact.submissionArtifact.instructions.drop 3810).take jumpTemplate.length = jumpTemplate := by rfl
def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpTemplate :=
  StackSiteBuilder.ofSlice jumpTemplate 3810 jump_slice
    (by change 3810 + jumpTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4801 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3810) = UInt256.ofNat 4801
