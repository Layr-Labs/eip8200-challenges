...
remaining compressor accesses are to the fixed scratch area below word 62.
-/
...
private theorem tableAtReturned_activeWords (s : State) (base i : Nat)
    (hi : i < 80) (hlo : 31 ≤ base) (hactive : 62 ≤ s.activeWords.toNat)
    (hbase : base + 96 + 32 ≤ 62 * 32) (returnDest : UInt256)
    (rest : List UInt256) :
...
private theorem afterConstantLoad_activeWords (s : State) (base i : Nat)
    (hi : i < 80) (hactive : 62 ≤ s.activeWords.toNat)
    (hbase : base + 4 * 32 + 32 ≤ 62 * 32) :
...
private theorem afterLoads_activeWords (s : State) (base : Nat)
    (hactive : 62 ≤ s.activeWords.toNat)
    (hbase : base + 0x80 + 32 ≤ 62 * 32) :
...
private theorem storedWord_activeWords (s : State) (base index : Nat)
    (hindex : index < 16) (hactive : 62 ≤ s.activeWords.toNat)
    (hbase : base + 32 * index + 32 ≤ 62 * 32) (value : UInt256) :
...
theorem roundReturned_activeWords (s : State) (base : Nat)
    (hbase : base + 0x80 + 32 ≤ 62 * 32) (j : Nat)
    (wordIndex rotation k returnDest : UInt256) (hword : wordIndex.toNat < 16)
    (rest : List UInt256) (hactive : 62 ≤ s.activeWords.toNat) :
...
theorem resultState_activeWords (s : State) (input : ByteArray)
    ...
    simpa [scheduledState] using
      scheduledState_activeWords s input hfit block hblock
        (UInt256.ofNat 630) (DriverTrace.messageOffsetWord block ::
          returnDest :: rest)
  have hscheduledActive : 62 ≤ scheduled.activeWords.toNat := by
    rw [hscheduled]
    exact le_trans (by omega : 62 ≤ 62 + 2 * block)
  ...
  have hinitialActive : 62 ≤ initial.activeWords.toNat := by
    rw [hinitial]
  ...
  have hleft : leftFinal.activeWords = initial.activeWords := by
    ...
    exact leftStates_activeWords _ messageOffset returnDest rest hinitialTables
      hinitialActive
  have hleftActive : 62 ≤ leftFinal.activeWords.toNat := by
    rw [hleft]
  ...
  calc
    ...
    _ = max s.activeWords.toNat (62 + 2 * block) := hscheduled
