...
theorem lowPart_bound (positive : Bool) (a k : Nat) (ha : a < 2 ^ 32)
    (hk0 : 1 ≤ k) (hk1 : k ≤ 7) : a * coefficient positive k < 2 ^ 143 := by
  have hc : k = 1 ∨ k = 2 ∨ k = 3 ∨ k = 4 ∨ k = 5 ∨ k = 6 ∨ k = 7 := by omega
  rcases hc with h | h | h | h | h | h | h <;> subst k <;> cases positive <;>
...
theorem canonical_bound (positive : Bool) (a b k : Nat)
    (ha : a < 2 ^ 32) (hb : b < 2 ^ 32) (hk0 : 1 ≤ k) (hk1 : k ≤ 7) :
...
theorem minus_residue (a b k : Nat) (ha : a < 2 ^ 32) (hb : b < 2 ^ 32)
  ...
  have hsplit : (a + b * base) * km k = canonical false a b k +
    ...
    have hc : k = 1 ∨ k = 2 ∨ k = 3 ∨ k = 4 ∨ k = 5 ∨ k = 6 ∨ k = 7 := by omega
    rcases hc with h | h | h | h | h | h | h <;> subst k <;>
...
theorem low_shifted (positive : Bool) (a b : BitVec 32) (k n : Nat)
    (hk0 : 1 ≤ k) (hk1 : k ≤ 7) (hn0 : 8 ≤ n) (hn1 : n ≤ 31) :
    low (rawProduct positive a b k >>> n) = a.rotateLeft (32 + loScale positive k - n) := by
  have hu : loScale positive k ≤ 7 := by cases positive <;> simp [loScale] <;> omega
...
theorem high_shifted (positive : Bool) (a b : BitVec 32) (k n : Nat)
  ...
  have hv : hiScale positive k ≤ 7 := by cases positive <;> simp [hiScale] <;> omega
...
theorem coefficient_small (positive : Bool) (k : Nat) (hk0 : 1 ≤ k) (hk1 : k ≤ 7) :
...
theorem bits_fused_product (x : UInt256) (a b : BitVec 32) (positive : Bool) (k : Nat)
    (hx : normalize (bits x) = pack a b) (hk0 : 1 ≤ k) (hk1 : k ≤ 7) :
...
theorem compact_bounds (r s : Nat) (hc : Paired144WordRound.usesCompact r s) :
    (r < s ∧ s - r ≤ 7) ∨ (s < r ∧ r - s ≤ 7) := by
  simp [Paired144WordRound.usesCompact] at hc
  omega

theorem normalize_wordFusedRotate_add (x : UInt256) (a b e f : BitVec 32) (r s : Nat)
    (hx : normalize (bits x) = pack a b)
    (hr0 : 5 ≤ r) (hr1 : r ≤ 15) (hs0 : 5 ≤ s) (hs1 : s ≤ 15)
    (hc : Paired144WordRound.usesCompact r s) :
    normalize (bits (wordFusedRotate x r s) + pack e f) =
      pack (a.rotateLeft r + e) (b.rotateLeft s + f) := by
  rcases compact_bounds r s hc with ⟨hrs, hk⟩ | ⟨hsr, hk⟩
...
#print axioms normalize_wordFusedRotate_add
end RootFusedD65
