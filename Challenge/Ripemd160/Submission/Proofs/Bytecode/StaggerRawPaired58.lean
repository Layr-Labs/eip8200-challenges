...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2978 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3748 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2978) = UInt256.ofNat 3748
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3748, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3788, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3748) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3748) template = UInt256.ofNat 3788 := by decide
  rw [hend] at hraw
