...
/-! Delay the temporary mask32 copy until after endian conversion. The persistent
mask32 below the return word is preserved. -/
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.DeferredNormalInitial
...
def cachedInitial : List Instr :=
  [op .JUMPDEST,
    .push ⟨6, by decide⟩ (UInt256.ofNat 257), .push 0 0, op .NOT, op .DIV,
    .push ⟨32, by decide⟩ mask16,
...
theorem run_cachedInitial (s : State) (pc messageOffset returnPC : UInt256)
    ...
    runInstrSeq cachedInitial (scheduleEntry s pc messageOffset returnPC (maskWord :: rest)) =
      some {s with
...
theorem cachedInitial_length : cachedInitial.length = 13 := rfl
theorem cachedInitial_byteLength : (assembleBytes cachedInitial).length = 52 := by decide
theorem cachedInitial_gas : staticGas cachedInitial = 38 := by decide
