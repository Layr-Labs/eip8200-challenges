...
import Challenge.Ripemd160.Submission.Proofs.Bytecode.TinyABCSpec
...
theorem leadWord_empty : leadWord ByteArray.empty = 0 := by
  ...
  norm_num [byte, YulSemantics.EVM.byteFrom, YulEvmCompiler.ByteArray.toList_eq_data,
    TinyABCSpec.inputBytes, Word.word_toNat_ofNat, UInt256.toNat]
  decide
theorem leadWord_abc : leadWord TinyABCSpec.inputBytes = UInt256.ofNat 0x616263 := by
  apply Word.word_ext
  rw [prefix_toNat]
  norm_num [byte, YulSemantics.EVM.byteFrom, YulEvmCompiler.ByteArray.toList_eq_data,
    TinyABCSpec.inputBytes, Word.word_toNat_ofNat, UInt256.toNat]
...
theorem condition_empty : condition ByteArray.empty = 0 := by
  ...
  rw [leadWord_empty]
  decide
theorem condition_abc : condition TinyABCSpec.inputBytes = 0 := by
  unfold condition
  rw [leadWord_abc]
  decide

theorem zero_cases (input : ByteArray) (hfit : CalldataFits input)
    (hz : condition input = 0) : input = ByteArray.empty ∨ input = TinyABCSpec.inputBytes := by
