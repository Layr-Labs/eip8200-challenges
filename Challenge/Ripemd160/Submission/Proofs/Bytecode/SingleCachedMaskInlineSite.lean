...
private theorem template_slice :
    (A.instructions.drop 1989).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 1989 template_slice (by
    change 1989 + template.length ≤ Artifact.submissionInstructions.length
