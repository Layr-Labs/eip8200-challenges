...
private theorem template_slice :
    (A.instructions.drop 2154).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2154 template_slice (by
    change 2154 + template.length ≤ Artifact.submissionInstructions.length
