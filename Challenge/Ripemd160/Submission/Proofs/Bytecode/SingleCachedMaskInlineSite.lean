...
private theorem template_slice :
    (A.instructions.drop 2168).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2168 template_slice (by
    change 2168 + template.length ≤ Artifact.submissionInstructions.length
