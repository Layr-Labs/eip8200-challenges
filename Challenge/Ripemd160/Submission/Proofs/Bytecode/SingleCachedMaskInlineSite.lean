...
private theorem template_slice :
    (A.instructions.drop 1984).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 1984 template_slice (by
    change 1982 + template.length ≤ Artifact.submissionInstructions.length
