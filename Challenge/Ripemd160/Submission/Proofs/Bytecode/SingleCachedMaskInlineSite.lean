...
private theorem template_slice :
    (A.instructions.drop 2002).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2002 template_slice (by
    change 1984 + template.length ≤ Artifact.submissionInstructions.length
