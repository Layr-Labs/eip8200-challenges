...
private theorem template_slice :
    (A.instructions.drop 2008).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2008 template_slice (by
    change 2006 + template.length ≤ Artifact.submissionInstructions.length
