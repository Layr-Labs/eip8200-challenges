import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 theorem actual_slice :
     (Artifact.submissionArtifact.instructions.drop 2178).take template.length = template := by rfl
 def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
   StackSiteBuilder.ofSlice template 2178 actual_slice
     (by change 2178 + template.length ≤ Artifact.submissionInstructions.length
         rw [Artifact.referenceInstructions_count]; decide)
     StackRoundData.artifact_code_bound
     (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
     (by decide)
 theorem site_pc : site.startPC = UInt256.ofNat 2740 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2178) = UInt256.ofNat 2740
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 2740, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 2779, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 2740) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 2740) template = UInt256.ofNat 2779 := by decide
   rw [hend] at hraw
