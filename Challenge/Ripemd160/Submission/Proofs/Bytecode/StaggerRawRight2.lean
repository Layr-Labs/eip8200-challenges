...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 654).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 654 actual_slice
    (by change 654 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1073 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 654) = UInt256.ofNat 1073
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1073, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1105, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1073) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1073) template = UInt256.ofNat 1105 := by decide
  rw [hend] at hraw
