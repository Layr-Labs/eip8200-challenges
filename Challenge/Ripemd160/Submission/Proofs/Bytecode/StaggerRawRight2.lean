...
theorem site_pc : site.startPC = UInt256.ofNat 1097 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 651) = UInt256.ofNat 1097
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1097, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1129, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1097) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1097) template = UInt256.ofNat 1129 := by decide
  rw [hend] at hraw
