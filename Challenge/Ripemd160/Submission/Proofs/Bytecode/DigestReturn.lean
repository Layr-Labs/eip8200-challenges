...
def storedState (input : ByteArray) (word : UInt256) : State :=
  { stS input 455 [] with memory := answerMemory word, activeWords := UInt256.ofNat 1 }

def sizedState (input : ByteArray) (word : UInt256) : State :=
  { storedState input word with pc := UInt256.ofNat 456, stack := [UInt256.ofNat 32] }

def returnedState (input : ByteArray) (word : UInt256) : State :=
  { storedState input word with pc := UInt256.ofNat 457, halt := .Returned, hReturn := MachineState.readPadded (answerMemory word) 0 32 }

def storePath : List Located := [pushAt 252 0 0, opAt 253 .MSTORE]
def finishPath : List Located := [pushAt 255 0 0, opAt 256 .RETURN]

theorem run_store (input : ByteArray) (word : UInt256) :
    run storePath (stS input 453 [word]) = some (storedState input word) := by
...
def gasSteps_return (input : ByteArray) (word : UInt256) :
    GasSteps (stS input 453 [word]) (returnedState input word) := by
  ...
  have hop : (storedState input word).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState input word) 254
