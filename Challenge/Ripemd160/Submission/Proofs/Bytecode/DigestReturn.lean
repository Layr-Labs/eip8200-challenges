...
def storedState (input : ByteArray) (word : UInt256) : State :=
  { stS input 474 [] with memory := answerMemory word, activeWords := UInt256.ofNat 1 }

def sizedState (input : ByteArray) (word : UInt256) : State :=
  { storedState input word with pc := UInt256.ofNat 475, stack := [UInt256.ofNat 32] }

def returnedState (input : ByteArray) (word : UInt256) : State :=
  { storedState input word with pc := UInt256.ofNat 476, halt := .Returned, hReturn := MachineState.readPadded (answerMemory word) 0 32 }

def storePath : List Located := [pushAt 260 0 0, opAt 261 .MSTORE]
def finishPath : List Located := [pushAt 263 0 0, opAt 264 .RETURN]

theorem run_store (input : ByteArray) (word : UInt256) :
    run storePath (stS input 472 [word]) = some (storedState input word) := by
...
def gasSteps_return (input : ByteArray) (word : UInt256) :
    GasSteps (stS input 472 [word]) (returnedState input word) := by
  have gs := sound storePath (run_store input word)
  have hd := Artifact.submissionArtifact.decodeAt_op_index 262 .MSIZE
    (by rfl) (by decide) trivial
  have hp : (storedState input word).pc.toNat =
      Artifact.submissionArtifact.instructionPC 262 := by
    rw [prefixDigestPC262]
    rfl
  have hop : (storedState input word).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState input word) 262
