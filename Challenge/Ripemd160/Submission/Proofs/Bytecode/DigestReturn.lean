...
def storedState (input : ByteArray) (word : UInt256) (rest : List UInt256) : State :=
  { stS input 351 rest with memory := answerMemory word, activeWords := UInt256.ofNat 1 }

def sizedState (input : ByteArray) (word : UInt256) (rest : List UInt256) : State :=
  { storedState input word rest with pc := UInt256.ofNat 352, stack := UInt256.ofNat 32 :: rest }

def returnedState (input : ByteArray) (word : UInt256) (rest : List UInt256) : State :=
  { storedState input word rest with pc := UInt256.ofNat 353, halt := .Returned, hReturn := MachineState.readPadded (answerMemory word) 0 32 }

def storePath : List Located := [pushAt 186 0 0, opAt 187 .MSTORE]
...
theorem run_store (input : ByteArray) (word : UInt256) (rest : List UInt256) (hlen : rest.length < 1020) :
    run storePath (stS input 349 (word :: rest)) = some (storedState input word rest) := by
...
def gasSteps_return (input : ByteArray) (word : UInt256) (rest : List UInt256) (hlen : rest.length < 1020) :
    GasSteps (stS input 349 (word :: rest)) (returnedState input word rest) := by
  ...
  have hop : (storedState input word rest).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState input word rest) 188
