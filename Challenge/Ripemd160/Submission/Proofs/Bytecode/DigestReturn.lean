...
def storedState (input : ByteArray) (word : UInt256) : State :=
  { stS input 460 [] with memory := answerMemory word, activeWords := UInt256.ofNat 1 }

def sizedState (input : ByteArray) (word : UInt256) : State :=
  { storedState input word with pc := UInt256.ofNat 461, stack := [UInt256.ofNat 32] }

def returnedState (input : ByteArray) (word : UInt256) : State :=
  { storedState input word with pc := UInt256.ofNat 462, halt := .Returned, hReturn := MachineState.readPadded (answerMemory word) 0 32 }

def storePath : List Located := [pushAt 258 0 0, opAt 259 .MSTORE]
def finishPath : List Located := [pushAt 261 0 0, opAt 262 .RETURN]

theorem run_store (input : ByteArray) (word : UInt256) :
    run storePath (stS input 458 [word]) = some (storedState input word) := by
...
def gasSteps_return (input : ByteArray) (word : UInt256) :
    GasSteps (stS input 458 [word]) (returnedState input word) := by
  ...
  have hop : (storedState input word).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState input word) 260
