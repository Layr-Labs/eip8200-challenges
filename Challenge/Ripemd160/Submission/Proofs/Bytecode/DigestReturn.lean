...
def storedState (input : ByteArray) (word : UInt256) : State :=
  { stS input 492 [] with memory := answerMemory word, activeWords := UInt256.ofNat 1 }

def sizedState (input : ByteArray) (word : UInt256) : State :=
  { storedState input word with pc := UInt256.ofNat 493, stack := [UInt256.ofNat 32] }

def returnedState (input : ByteArray) (word : UInt256) : State :=
  { storedState input word with pc := UInt256.ofNat 494, halt := .Returned, hReturn := MachineState.readPadded (answerMemory word) 0 32 }

def storePath : List Located := [pushAt 283 0 0, opAt 284 .MSTORE]
def finishPath : List Located := [pushAt 286 0 0, opAt 287 .RETURN]

theorem run_store (input : ByteArray) (word : UInt256) :
    run storePath (stS input 490 [word]) = some (storedState input word) := by
...
def gasSteps_return (input : ByteArray) (word : UInt256) :
    GasSteps (stS input 490 [word]) (returnedState input word) := by
  have gs := sound storePath (run_store input word)
  have hd := Artifact.submissionArtifact.decodeAt_op_index 285 .MSIZE
    (by rfl) (by decide) trivial
  have hp : (storedState input word).pc.toNat =
      Artifact.submissionArtifact.instructionPC 285 := by
  ...
  have hop : (storedState input word).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState input word) 285
