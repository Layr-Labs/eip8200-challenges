import Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRawRight0
...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixCoreRight0
...
theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
    PadPrefixRawRight0.outputStack memory (input memory h4 q right (UInt256.ofNat 1352829926)) rho =
  ...
  simp only [PadPrefixRawRight0.outputStack, input, stack, List.map_cons, List.map_nil,
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4988, stack := stack s.memory h4 [ .a, .k, .b, .c, .d, .e, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] q right (UInt256.ofNat 1352829926) rho}
      {s with pc := UInt256.ofNat 5019, stack := stack s.memory h4 [ .d, .k, .c, .b, .e, .a, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (StaggerCoreModel.right0 s.memory q) (right) (UInt256.ofNat 1352829926) rho} := by
  have h := PadPrefixRawRight0.gasSteps s (input s.memory h4 q right (UInt256.ofNat 1352829926)) rho hs halias hr ha hcode hfork hnp
  rw [output_eq] at h
  exact h
#print axioms gasSteps
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixCoreRight0
