...
abbrev entryState (input : ByteArray) : State := atPC input 0x14ad
...
def jumpStackState (input : ByteArray) (condition : UInt256) : State :=
  { initialState submissionBytecode input 0 with
      pc := UInt256.ofNat 0x1500
      stack := [UInt256.ofNat 0x1505, condition] }
