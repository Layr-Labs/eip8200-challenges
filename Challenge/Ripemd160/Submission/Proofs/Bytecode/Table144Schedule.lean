import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144EndianSetup
import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144PoolSetup
import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144StoreSetup
...
open PairTable144Active PairTable144Scratch
open PairTable144Memory (scratchMemory erase_scratch)
...
theorem run_normal (s : State) (pc returnPC : UInt256) (p : Nat) (rest : List UInt256)
    ...
    runInstrSeq normalTemplate (scheduleEntry s pc (UInt256.ofNat p) returnPC rest) =
      some {s with pc := pcAfter pc normalTemplate, stack := returnPC :: rest, memory := PairTable144Layout.resultMemory s.memory (PairedScheduleData.extractedWord s.memory p), activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  ...
  let s1 : State := {s with activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)}
  let s2 : State := {s1 with memory := scratch}
  have ha : 53 ≤ s1.activeWords.toNat := loaded_active_ge53 s p hp hbound
  ...
  have hm : PairTable144Layout.resultMemory scratch words = PairTable144Layout.resultMemory s.memory words :=
    erase_scratch s.memory (PairTable144Layout.tableWords words) _ _
