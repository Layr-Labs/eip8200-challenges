...
def lookupNat (n : Nat) : Nat := (105838886210502912 >>> (n &&& 53)) &&& 1016
...
def lookup (x : UInt256) : UInt256 :=
  UInt256.land (UInt256.ofNat 1016)
    (UInt256.shiftRight (UInt256.ofNat 105838886210502912)
      (UInt256.land (UInt256.ofNat 53) x))

theorem lookup_toNat (x : UInt256) : (lookup x).toNat = lookupNat x.toNat := by
  let index := UInt256.land (UInt256.ofNat 53) x
  have hi : index.toNat = x.toNat &&& 53 := by
    rw [show index = UInt256.land (UInt256.ofNat 53) x by rfl, Word.word_toNat_land]
    change 53 &&& x.toNat = x.toNat &&& 53
    exact Nat.and_comm _ _
  have hs : index.toNat < 256 := by
    rw [hi]
    have h : x.toNat &&& 53 ≤ 53 := Nat.and_le_right
    omega
  have hsr := Word.shiftRight_toNat (UInt256.ofNat 105838886210502912) hs
  rw [← Word.word_eq_ofNat_toNat index] at hsr
  unfold lookup
  rw [Word.word_toNat_land, hsr]
  change 1016 &&& (105838886210502912 >>> index.toNat) = lookupNat x.toNat
  rw [hi]
  exact Nat.and_comm _ _
