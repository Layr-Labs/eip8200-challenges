...
def steppedState (input : ByteArray) (s : State) (frame : List UInt256) (i : Nat) : State :=
  {loopState input s frame (i + 1) with pc := UInt256.ofNat 236}

def exitState (input : ByteArray) (s : State) (frame : List UInt256) (i : Nat) : State :=
  {loopState input s frame i with pc := UInt256.ofNat 240}
...
private theorem valid_loop : Decode.isValidJumpDest submissionBytecode 224 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 130 = 224 := by
