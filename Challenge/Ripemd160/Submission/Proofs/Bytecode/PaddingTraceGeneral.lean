...
def steppedState (input : ByteArray) (s : State) (frame : List UInt256) (i : Nat) : State :=
  {loopState input s frame (i + 1) with pc := UInt256.ofNat 4751}

def exitState (input : ByteArray) (s : State) (frame : List UInt256) (i : Nat) : State :=
  {loopState input s frame i with pc := UInt256.ofNat 4756}
...
private theorem valid_loop : Decode.isValidJumpDest submissionBytecode 4739 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3639 = 4739 := by
...
private theorem run_sentinel (hfit : CalldataFits input) :
    DataStepper.runLocatedBlock lengthSentinelPath
      {s with pc := UInt256.ofNat 4722, stack := frame} =
...
def gasSteps_setup (hfit : CalldataFits input) :
  ...
  have gs := DataStepper.runLocatedBlock_sound Artifact.submissionArtifact .Osaka
    lengthSentinelPath (s := {s with pc := UInt256.ofNat 4722, stack := frame}) hcode hfork (run_sentinel input s frame hframe hcal hrun hfit) hrun hnp
