...
def steppedState (input : ByteArray) (s : State) (frame : List UInt256) (i : Nat) : State :=
  {loopState input s frame (i + 1) with pc := UInt256.ofNat 4811}

def exitState (input : ByteArray) (s : State) (frame : List UInt256) (i : Nat) : State :=
  {loopState input s frame i with pc := UInt256.ofNat 4816}
...
private theorem valid_loop : Decode.isValidJumpDest submissionBytecode 4799 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3624 = 4799 := by
...
private theorem run_sentinel (hfit : CalldataFits input) :
    DataStepper.runLocatedBlock lengthSentinelPath
      {s with pc := UInt256.ofNat 4782, stack := frame} =
...
def gasSteps_setup (hfit : CalldataFits input) :
    GasSteps {s with pc := UInt256.ofNat 4782, stack := frame}
      (loopState input s frame 0) := by
  have gs := DataStepper.runLocatedBlock_sound Artifact.submissionArtifact .Osaka
    lengthSentinelPath (s := {s with pc := UInt256.ofNat 4782, stack := frame}) hcode hfork (run_sentinel input s frame hframe hcal hrun hfit) hrun hnp
