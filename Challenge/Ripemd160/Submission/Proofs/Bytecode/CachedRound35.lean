...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2104).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2104 actual_slice
    (by change 2104 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2672 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2104) = UInt256.ofNat 2672
...
theorem run_actual (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq actualTemplate {s with pc := UInt256.ofNat 2672, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 2715, stack := outputStack s.memory x rho} := by
  have hcap (n : Nat) (hn : n ≤ 26) : rho.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (haddress : address ≤ 704) :
      UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat address 32) = s.activeWords :=
    active_schedule_preserved s.activeWords address hactive haddress
...
def gasSteps (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2672, stack := coreStack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] f (cache s.memory ++ rho)}
      {s with pc := UInt256.ofNat 2715, stack := coreStack [.d, .k, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (PairedAllInlineCoreTrace.inline35Block.eval s.memory f) (cache s.memory ++ rho)} := by
