...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2116).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2116 actual_slice
    (by change 2116 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2664 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2116) = UInt256.ofNat 2664
