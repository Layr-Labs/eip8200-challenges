...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2003).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2003 actual_slice
    (by change 2003 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2506 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2003) = UInt256.ofNat 2506
