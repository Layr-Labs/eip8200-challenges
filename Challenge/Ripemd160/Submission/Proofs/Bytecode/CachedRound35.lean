...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2106).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2106 actual_slice
    (by change 2106 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2672 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2106) = UInt256.ofNat 2672
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 2556, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 2602, stack := outputStack s.memory x rho} := by
