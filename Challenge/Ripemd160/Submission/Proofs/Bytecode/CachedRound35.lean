...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2112).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2112 actual_slice
    (by change 2112 + actualTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualTemplate) (by decide))
    (by decide)
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2671 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2112) = UInt256.ofNat 2671
...
theorem run_actual (s : State) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 996) (hrun : s.halt = .Running)
    (hactive : 23 ≤ s.activeWords.toNat) :
    runInstrSeq actualTemplate {s with pc := UInt256.ofNat 2671, stack := inputStack s.memory x rho} =
...
def gasSteps (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2671, stack := coreStack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] f (cache s.memory ++ rho)}
      {s with pc := UInt256.ofNat 2714, stack := coreStack [.d, .k, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (PairedAllInlineCoreTrace.inline35Block.eval s.memory f) (cache s.memory ++ rho)} := by
  ...
  have g := gasSteps_terminal_of_raw actualSite
    {s with pc := UInt256.ofNat 2671, stack := inputStack s.memory x rho} _
