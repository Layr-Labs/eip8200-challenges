...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2113).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2113 actual_slice
    (by change 2113 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2671 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2113) = UInt256.ofNat 2671
