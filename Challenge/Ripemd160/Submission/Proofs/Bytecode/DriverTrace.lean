...
def postCheckPath : List Located :=
  [⟨64, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ...
   ⟨70, .push ⟨2, by decide⟩ (UInt256.ofNat 4914), by rfl, by decide⟩,
...
theorem run_postCheck_exit (s : State) (input : ByteArray)
  ...
  have hdest : Decode.isValidJumpDest submissionBytecode 0x1332 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 4018 (by rfl)
  simp [postCheckPath, Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
...
/-- One dispatcher execution that consumes four blocks (the depth-4 prefix
ladder): the call, the four-block certificate, then the post-check for
block 3. -/
def gasSteps_iteration4_of_compress (s next : State) (input : ByteArray)
    (hfit : Challenge.Ripemd160.CalldataFits input)
    (h3 : 3 < blockCount input)
    (hcodeS : s.executionEnv.code = submissionBytecode)
    (hforkS : s.fork = .Osaka) (hrunS : s.halt = .Running)
    (hnpS : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig s.executionEnv.fork
      s.executionEnv.codeAddr = false)
...
/-- Iterate the driver over all padded blocks.  When `quadruple` is set,
the first dispatcher execution consumes blocks 0, 1, 2 and 3; `triple`
consumes blocks 0, 1 and 2; `double` consumes blocks 0 and 1. -/
