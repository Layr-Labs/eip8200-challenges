...
/-- The one-time empty-input test at the driver entry (pc 368): nonempty
calldata jumps to the block setup at pc 506; later blocks skip it. -/
def enterPath : List Located :=
  [⟨234, .op .CALLDATASIZE, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨235, .push ⟨2, by decide⟩ (UInt256.ofNat 506), by rfl, by decide⟩,
...
@[simp] private theorem pcD237 : Artifact.submissionArtifact.instructionPC 235 = 369 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
