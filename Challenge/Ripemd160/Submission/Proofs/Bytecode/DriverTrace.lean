...
def callPath : List Located :=
  [⟨777, .push ⟨2, by decide⟩ (UInt256.ofNat 0x643), by rfl, by decide⟩,
   ...
   ⟨781, .push ⟨2, by decide⟩ (UInt256.ofNat 0x13e2), by rfl, by decide⟩,
...
/-- State at the appended empty-input dispatcher.  Its stack is deliberately
identical to the legacy compressor's entry stack, so the nonempty branch can
jump to `compressEntry` without any shuffling. -/
def dispatchEntry (s : State) (input : ByteArray) (i : Nat) : State :=
  { s with
    pc := UInt256.ofNat 0x13e2
...
theorem run_call (s : State) (input : ByteArray)
  ...
  have hdest : Decode.isValidJumpDest submissionBytecode 0x13e2 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 2565 (by rfl)
