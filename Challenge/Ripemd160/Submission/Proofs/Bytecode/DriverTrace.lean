...
def callPath : List Located :=
  [⟨777, .push ⟨2, by decide⟩ (UInt256.ofNat 0x643), by rfl, by decide⟩,
   ...
   ⟨781, .push ⟨2, by decide⟩ (UInt256.ofNat 0x708), by rfl, by decide⟩,
...
theorem run_increment (s : State) (input : ByteArray) (i : Nat)
  ...
  -- DriverFold5 consumes the offset instead of duplicating and discarding it.
  -- ADD now sees 64 above the old offset; discharge the new operand order.
  have hadd : UInt256.ofNat 64 + blockOffsetWord i =
      blockOffsetWord (i + 1) := by
    have haddBound' : 64 + i * 64 < 2 ^ 256 := by omega
    simpa [blockOffsetWord, blockOffset, Nat.add_mul, Nat.add_comm] using
      Challenge.EvmProof.Word.ofNat_add_ofNat
        (a := 64) (b := i * 64) haddBound'
