...
private theorem returnPC : Artifact.submissionArtifact.instructionPC 64 = 0x69 := by
...
noncomputable def gasSteps_legacyBlock (s : State) (input : ByteArray) (i : Nat)
  ...
  have hvalid : Decode.isValidJumpDest q.executionEnv.code
      (UInt256.ofNat 0x69).toNat = true := by
    have hdest := Artifact.submissionArtifact.isValidJumpDest_index 64 (by rfl)
    rw [returnPC] at hdest
    change Decode.isValidJumpDest q.executionEnv.code 0x69 = true
    rw [qcode]
    exact hdest
  have gtail := CachedMaskTailSite.actualTailGasSteps q left right (UInt256.ofNat 0x69)
  ...
  have tailSeam : stateAt q (QuadLayout.rightPC 20) right rightRest =
      CachedMaskOrderedTail.entry q left right (UInt256.ofNat 0x69) (driverRest input i) := by
  ...
  have hright : right = rightWorking s input i := by
    exact congrArg (StackCompression.rightRounds (blockWords input i) 80)
      (initialWorking_scheduled s input i)
  have tailEnd : QuadTailTemplate.finalResult q left right (UInt256.ofNat 0x69)
      (driverRest input i) =
      DriverTrace.compressReturned (resultState s input i) input i := by
    change StackTail.tailResult q left right (UInt256.ofNat 0x69)
      (driverRest input i) = _
    exact (congrArg₂ (fun l r => StackTail.tailResult q l r (UInt256.ofNat 0x69)
