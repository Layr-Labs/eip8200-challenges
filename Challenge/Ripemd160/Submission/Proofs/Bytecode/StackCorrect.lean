...
noncomputable def gasSteps_block (s : State) (input : ByteArray) (i : Nat)
  ...
  have restBound : rest.length < 1015 := by
    simp [rest, StackFrame.frameRest, driverRest]
  have rightRestBound : rightRest.length < 1015 := by
  ...
  have gload2 := StackLoadTrace.gasSteps_load StackFrame.loadSite4644 q rightRest qactive
    (by omega) qcode qfork qrun qnp
