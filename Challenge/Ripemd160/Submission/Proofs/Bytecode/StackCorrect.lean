...
noncomputable def gasSteps_block (s : State) (input : ByteArray) (i : Nat)
  ...
  have restBound : rest.length < 1000 := by
    simp [rest, StackFrame.frameRest, driverRest]
  have rightRestBound : rightRest.length < 1000 := by
  ...
  have gload1 := StackLoadTrace.gasSteps_load StackFrame.loadSite848 q rest qactive
    (by omega) qcode qfork qrun qnp
  have gload1' : GasSteps (StackFrame.frameLoadEntry s input i)
      (stateAt q (QuadSites.leftPC 0) w rest) := by
  ...
  have gload2 := StackLoadTrace.gasSteps_load StackFrame.loadSite1098 q rightRest qactive
    (by omega) qcode qfork qrun qnp
  have gload2' : GasSteps (stateAt q (QuadSites.leftPC 20) left rest)
      (stateAt q (QuadSites.rightPC 0) w rightRest) := by
  ...
  have tailSeam : stateAt q (QuadSites.rightPC 20) right rightRest =
