...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v3 (UInt256.shiftRight (UInt256.mul x.v10 x.v9) (UInt256.ofNat 28))),
    ...
    x.v0,
    x.v8,
    (UInt256.land x.v3 (UInt256.add x.v8 (UInt256.shiftRight (UInt256.mul x.v10 (UInt256.land x.v3 (UInt256.add x.v5 (UInt256.add (MachineState.readWord memory 756) (UInt256.add (UInt256.xor (UInt256.lor (UInt256.lnot x.v9) x.v6) x.v0) x.v7))))) (UInt256.ofNat 25)))),
    x.v10,
    x.v11,
    x.v12,
    x.v13,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2216).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2216 actual_slice
    (by change 2216 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2921 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2216) = UInt256.ofNat 2921
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2921, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2954, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2921) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2921) template = UInt256.ofNat 2954 := by decide
  rw [hend] at hraw
