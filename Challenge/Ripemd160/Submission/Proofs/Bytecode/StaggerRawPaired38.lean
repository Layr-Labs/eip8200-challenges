...
theorem site_pc : site.startPC = UInt256.ofNat 2881 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2163) = UInt256.ofNat 2881
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2881, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2915, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2881) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2881) template = UInt256.ofNat 2915 := by decide
  rw [hend] at hraw
