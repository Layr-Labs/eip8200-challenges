...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v2 (UInt256.shiftRight (UInt256.mul x.v9 x.v8) (UInt256.ofNat 28))),
    ...
    x.v0,
    x.v7,
    (UInt256.land x.v2 (UInt256.add x.v7 (UInt256.shiftRight (UInt256.mul x.v9 (UInt256.land x.v2 (UInt256.add x.v4 (UInt256.add (MachineState.readWord memory 756) (UInt256.add (UInt256.xor x.v0 (UInt256.lor (UInt256.lnot x.v8) x.v5)) x.v6))))) (UInt256.ofNat 25)))),
    x.v9,
    x.v10,
    x.v11,
    x.v12,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2176).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2176 actual_slice
    (by change 2176 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2876 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2176) = UInt256.ofNat 2876
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2876, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2910, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2876) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2876) template = UInt256.ofNat 2910 := by decide
  rw [hend] at hraw
