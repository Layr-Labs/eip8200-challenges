...
theorem site_pc : site.startPC = UInt256.ofNat 1037 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 609) = UInt256.ofNat 1037
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1037, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1012, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1037) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1037) template = UInt256.ofNat 1012 := by decide
  rw [hend] at hraw
