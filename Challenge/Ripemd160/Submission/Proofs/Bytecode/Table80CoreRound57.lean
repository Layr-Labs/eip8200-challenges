...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound57
...
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound57.outputStack memory (input q k) rho =
      stack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 270) k q) (nextKey k) rho := by
  simp only [Table80RawRound57.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired80Algorithm.physicalKey 57) =
     Paired80Algorithm.physicalKey 58 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired80Algorithm.physicalKey 57) q = Paired80Algorithm.step 57 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 3613, stack := stack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 3660, stack :=
          stack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 270) k q) (nextKey k) rho} := by
   have gs := Table80RawRound57.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound57
