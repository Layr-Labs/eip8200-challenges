...
theorem states_final_activeWords_of_step (input : ByteArray)
  ...
  have hstates : ∀ n, n ≤ blocks →
    ...
    | succ n ih =>
        ...
        rw [Nat.max_eq_right (by omega : 60 + 2 * n ≤ 62 + 2 * n)]
        exact congrArg (Nat.max (59 + 2 * blocks)) (by omega)
