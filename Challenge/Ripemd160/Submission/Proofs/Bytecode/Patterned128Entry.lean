...
Entered at pc 4870 when the first calldata byte is 7, or when a short or
...
private theorem pc3836 : Artifact.submissionArtifact.instructionPC 3836 = 4870 := GuardInstructionWindow.pc 182
private theorem pc3837 : Artifact.submissionArtifact.instructionPC 3837 = 4871 := GuardInstructionWindow.pc 183
private theorem pc3843 : Artifact.submissionArtifact.instructionPC 3843 = 4896 := GuardInstructionWindow.pc 189
private theorem pc3844 : Artifact.submissionArtifact.instructionPC 3844 = 4897 := GuardInstructionWindow.pc 190
private theorem pc3845 : Artifact.submissionArtifact.instructionPC 3845 = 4898 := GuardInstructionWindow.pc 191
private theorem pc3846 : Artifact.submissionArtifact.instructionPC 3846 = 4901 := GuardInstructionWindow.pc 192
private theorem pc3847 : Artifact.submissionArtifact.instructionPC 3847 = 4902 := GuardInstructionWindow.pc 193
private theorem pc3848 : Artifact.submissionArtifact.instructionPC 3848 = 4903 := GuardInstructionWindow.pc 194
private theorem pc3849 : Artifact.submissionArtifact.instructionPC 3849 = 4906 := GuardInstructionWindow.pc 195
private theorem pc3850 : Artifact.submissionArtifact.instructionPC 3850 = 4907 := GuardInstructionWindow.pc 196
private theorem pc3851 : Artifact.submissionArtifact.instructionPC 3851 = 4908 := GuardInstructionWindow.pc 197
private theorem pc3852 : Artifact.submissionArtifact.instructionPC 3852 = 4911 := GuardInstructionWindow.pc 198
private theorem pc3853 : Artifact.submissionArtifact.instructionPC 3853 = 4912 := GuardInstructionWindow.pc 199
private theorem pc3854 : Artifact.submissionArtifact.instructionPC 3854 = 4913 := GuardInstructionWindow.pc 200
private theorem pc3855 : Artifact.submissionArtifact.instructionPC 3855 = 4914 := GuardInstructionWindow.pc 201
private theorem pc3856 : Artifact.submissionArtifact.instructionPC 3856 = 4916 := GuardInstructionWindow.pc 202

@[simp] private theorem sizeHitPC3838 : Artifact.submissionArtifact.instructionPC 3838 = 4889 :=
  GuardInstructionWindow.pc 184
@[simp] private theorem sizeHitPC3839 : Artifact.submissionArtifact.instructionPC 3839 = 4890 :=
  GuardInstructionWindow.pc 185
@[simp] private theorem sizeHitPC3840 : Artifact.submissionArtifact.instructionPC 3840 = 4891 :=
  GuardInstructionWindow.pc 186
@[simp] private theorem sizeHitPC3841 : Artifact.submissionArtifact.instructionPC 3841 = 4893 :=
  GuardInstructionWindow.pc 187
@[simp] private theorem sizeHitPC3842 : Artifact.submissionArtifact.instructionPC 3842 = 4894 :=
  GuardInstructionWindow.pc 188

/-! ## The size mask -/

private def maskEntry : List Located :=
  [DirectGuard.opAt 3836 .JUMPDEST (by exact GuardInstructionWindow.get 182)]

private def sizePrefix : List Located :=
  [DirectGuard.pushAt 3837 17 342276208914615837337402008677671501826 (by exact GuardInstructionWindow.get 183),
   DirectGuard.opAt 3838 .CALLDATASIZE (by exact GuardInstructionWindow.get 184),
   DirectGuard.opAt 3839 .SHR (by exact GuardInstructionWindow.get 185),
   DirectGuard.pushAt 3840 1 1 (by exact GuardInstructionWindow.get 186),
   DirectGuard.opAt 3841 .AND (by exact GuardInstructionWindow.get 187),
   DirectGuard.pushAt 3842 1 115 (by exact GuardInstructionWindow.get 188)]

private def maskJump : List Located :=
  [DirectGuard.opAt 3843 .JUMPI (by exact GuardInstructionWindow.get 189)]
...
private theorem run_mask_entry (input : ByteArray) :
    DirectGuard.run maskEntry (PatternedScan.stS input 4870 []) =
      some (PatternedScan.stS input 4871 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 3836 4870 [] (by norm_num) pc3836)
    (PatternedScan.stepS_jumpdest input 4870 [] (by simp) (by norm_num))

private theorem run_size_prefix (input : ByteArray) :
    DirectGuard.run sizePrefix (PatternedScan.stS input 4871 []) =
      some (PatternedScan.stS input 4896 [115, sizeBit input]) := by
  have hpc : Artifact.submissionArtifact.instructionPC 3837 = 4871 := pc3837
...
private theorem run_mask_match (input : ByteArray) (hc : UInt256.isTrue (sizeBit input)) :
    DirectGuard.run maskJump (PatternedScan.stS input 4896 [115, sizeBit input]) =
      some (PatternedScan.stS input 115 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 3843 4896 [115, sizeBit input] (by norm_num) pc3843)
    (PatternedScan.stepS_jumpi_taken input 4896 115 115 (sizeBit input) []
...
private theorem run_mask_fall (input : ByteArray) (hc : ¬ UInt256.isTrue (sizeBit input)) :
    DirectGuard.run maskJump (PatternedScan.stS input 4896 [115, sizeBit input]) =
      some (PatternedScan.stS input 4897 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 3843 4896 [115, sizeBit input] (by norm_num) pc3843)
    (PatternedScan.stepS_jumpi_fall input 4896 115 (sizeBit input) []
...
private def gasSteps_mask_prefix (input : ByteArray) :
    GasSteps (DirectGuard.guardEntry input) (PatternedScan.stS input 4896 [115, sizeBit input]) :=
...
private def gasSteps_mask_fail (input : ByteArray) (hfit : CalldataFits input)
    ...
    GasSteps (DirectGuard.guardEntry input) (PatternedScan.stS input 4897 []) := by
...
private def lengthPrefix : List Located :=
  [DirectGuard.opAt 3844 .CALLDATASIZE (by exact GuardInstructionWindow.get 190),
   DirectGuard.pushAt 3845 2 256 (by exact GuardInstructionWindow.get 191),
   DirectGuard.opAt 3846 .EQ (by exact GuardInstructionWindow.get 192),
   DirectGuard.opAt 3847 .CALLDATASIZE (by exact GuardInstructionWindow.get 193),
   DirectGuard.pushAt 3848 2 376 (by exact GuardInstructionWindow.get 194),
   DirectGuard.opAt 3849 .EQ (by exact GuardInstructionWindow.get 195),
   DirectGuard.opAt 3850 .CALLDATASIZE (by exact GuardInstructionWindow.get 196),
   DirectGuard.pushAt 3851 2 1000 (by exact GuardInstructionWindow.get 197),
   DirectGuard.opAt 3852 .EQ (by exact GuardInstructionWindow.get 198),
   DirectGuard.opAt 3853 .OR (by exact GuardInstructionWindow.get 199),
   DirectGuard.opAt 3854 .OR (by exact GuardInstructionWindow.get 200),
   DirectGuard.pushAt 3855 1 115 (by exact GuardInstructionWindow.get 201)]

private def lengthJump : List Located :=
  [DirectGuard.opAt 3856 .JUMPI (by exact GuardInstructionWindow.get 202)]

private theorem run_length_prefix (input : ByteArray) :
    DirectGuard.run lengthPrefix (PatternedScan.stS input 4897 []) =
      some (PatternedScan.stS input 4916 [115, lengthCond input]) := by
  let n := UInt256.ofNat input.size
  let e256 := UInt256.eq 256 n
  let e376 := UInt256.eq 376 n
  let e1000 := UInt256.eq 1000 n
  let l0 : Located := DirectGuard.opAt 3844 .CALLDATASIZE (by exact GuardInstructionWindow.get 190)
  have h0 := PatternedScan.blockOfS l0
    (PatternedScan.pcFactS input 3844 4897 [] (by norm_num) pc3844)
    (calldatasize_step input 4897 [] (by simp) (by norm_num))
  let l1 : Located := DirectGuard.pushAt 3845 2 256 (by exact GuardInstructionWindow.get 191)
  have h1 := PatternedScan.blockOfS l1
    (PatternedScan.pcFactS input 3845 4898 [n] (by norm_num) pc3845)
    (PatternedScan.stepS_push input 4898 2 256 [n] (by simp) (by decide) (by decide) (by norm_num))
  let l2 : Located := DirectGuard.opAt 3846 .EQ (by exact GuardInstructionWindow.get 192)
  have h2 := PatternedScan.blockOfS l2
    (PatternedScan.pcFactS input 3846 4901 [256, n] (by norm_num) pc3846)
    (PatternedScan.stepS_eq input 4901 256 n [] (by simp) (by norm_num))
  let l3 : Located := DirectGuard.opAt 3847 .CALLDATASIZE (by exact GuardInstructionWindow.get 193)
  have h3 := PatternedScan.blockOfS l3
    (PatternedScan.pcFactS input 3847 4902 [e256] (by norm_num) pc3847)
    (calldatasize_step input 4902 [e256] (by simp) (by norm_num))
  let l4 : Located := DirectGuard.pushAt 3848 2 376 (by exact GuardInstructionWindow.get 194)
  have h4 := PatternedScan.blockOfS l4
    (PatternedScan.pcFactS input 3848 4903 [n, e256] (by norm_num) pc3848)
    (PatternedScan.stepS_push input 4903 2 376 [n, e256] (by simp) (by decide) (by decide) (by norm_num))
  let l5 : Located := DirectGuard.opAt 3849 .EQ (by exact GuardInstructionWindow.get 195)
  have h5 := PatternedScan.blockOfS l5
    (PatternedScan.pcFactS input 3849 4906 [376, n, e256] (by norm_num) pc3849)
    (PatternedScan.stepS_eq input 4906 376 n [e256] (by simp) (by norm_num))
  let l6 : Located := DirectGuard.opAt 3850 .CALLDATASIZE (by exact GuardInstructionWindow.get 196)
  have h6 := PatternedScan.blockOfS l6
    (PatternedScan.pcFactS input 3850 4907 [e376, e256] (by norm_num) pc3850)
    (calldatasize_step input 4907 [e376, e256] (by simp) (by norm_num))
  let l7 : Located := DirectGuard.pushAt 3851 2 1000 (by exact GuardInstructionWindow.get 197)
  have h7 := PatternedScan.blockOfS l7
    (PatternedScan.pcFactS input 3851 4908 [n, e376, e256] (by norm_num) pc3851)
    (PatternedScan.stepS_push input 4908 2 1000 [n, e376, e256] (by simp) (by decide) (by decide) (by norm_num))
  let l8 : Located := DirectGuard.opAt 3852 .EQ (by exact GuardInstructionWindow.get 198)
  have h8 := PatternedScan.blockOfS l8
    (PatternedScan.pcFactS input 3852 4911 [1000, n, e376, e256] (by norm_num) pc3852)
    (PatternedScan.stepS_eq input 4911 1000 n [e376, e256] (by simp) (by norm_num))
  let l9 : Located := DirectGuard.opAt 3853 .OR (by exact GuardInstructionWindow.get 199)
  have h9 := PatternedScan.blockOfS l9
    (PatternedScan.pcFactS input 3853 4912 [e1000, e376, e256] (by norm_num) pc3853)
    (PatternedScan.stepS_or input 4912 e1000 e376 [e256] (by simp) (by norm_num))
  let l10 : Located := DirectGuard.opAt 3854 .OR (by exact GuardInstructionWindow.get 200)
  have h10 := PatternedScan.blockOfS l10
    (PatternedScan.pcFactS input 3854 4913 [UInt256.lor e1000 e376, e256] (by norm_num) pc3854)
    (PatternedScan.stepS_or input 4913 (UInt256.lor e1000 e376) e256 [] (by simp) (by norm_num))
  let l11 : Located := DirectGuard.pushAt 3855 1 115 (by exact GuardInstructionWindow.get 201)
  have h11 := PatternedScan.blockOfS l11
    (PatternedScan.pcFactS input 3855 4914 [UInt256.lor (UInt256.lor e1000 e376) e256] (by norm_num) pc3855)
    (PatternedScan.stepS_push input 4914 1 115 [UInt256.lor (UInt256.lor e1000 e376) e256]
...
private def gasSteps_length_match (input : ByteArray)
    ...
    GasSteps (PatternedScan.stS input 4897 []) (PatternedScan.patternedEntry input) := by
  have hj : DirectGuard.run lengthJump (PatternedScan.stS input 4916 [115, lengthCond input]) =
      some (PatternedScan.stS input 115 []) :=
    PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 3856 4916 _ (by norm_num) pc3856)
      (PatternedScan.stepS_jumpi_taken input 4916 115 115 (lengthCond input) []
...
private def gasSteps_length_fail (input : ByteArray) (hfit : CalldataFits input)
    (h256 : input.size ≠ 256) (h376 : input.size ≠ 376) (h1000 : input.size ≠ 1000) :
    GasSteps (PatternedScan.stS input 4897 []) (AbcArm.armEntry input) := by
  have hj : DirectGuard.run lengthJump (PatternedScan.stS input 4916 [115, lengthCond input]) =
      some (AbcArm.armEntry input) :=
    PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 3856 4916 _ (by norm_num) pc3856)
      (PatternedScan.stepS_jumpi_fall input 4916 115 (lengthCond input) []
