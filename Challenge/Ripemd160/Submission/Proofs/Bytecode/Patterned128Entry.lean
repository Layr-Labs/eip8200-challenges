...
private theorem pc3836 : Artifact.submissionArtifact.instructionPC 3774 = 4803 := GuardInstructionWindow.pc 180
private theorem pc3837 : Artifact.submissionArtifact.instructionPC 3775 = 4804 := GuardInstructionWindow.pc 181
private theorem pc3843 : Artifact.submissionArtifact.instructionPC 3781 = 4829 := GuardInstructionWindow.pc 187
private theorem pc3844 : Artifact.submissionArtifact.instructionPC 3782 = 4830 := GuardInstructionWindow.pc 188
private theorem pc3845 : Artifact.submissionArtifact.instructionPC 3783 = 4831 := GuardInstructionWindow.pc 189
private theorem pc3846 : Artifact.submissionArtifact.instructionPC 3784 = 4834 := GuardInstructionWindow.pc 190
private theorem pc3847 : Artifact.submissionArtifact.instructionPC 3785 = 4835 := GuardInstructionWindow.pc 191
private theorem pc3848 : Artifact.submissionArtifact.instructionPC 3786 = 4836 := GuardInstructionWindow.pc 192
private theorem pc3849 : Artifact.submissionArtifact.instructionPC 3787 = 4839 := GuardInstructionWindow.pc 193
private theorem pc3850 : Artifact.submissionArtifact.instructionPC 3788 = 4840 := GuardInstructionWindow.pc 194
private theorem pc3851 : Artifact.submissionArtifact.instructionPC 3789 = 4841 := GuardInstructionWindow.pc 195
private theorem pc3852 : Artifact.submissionArtifact.instructionPC 3790 = 4844 := GuardInstructionWindow.pc 196
private theorem pc3853 : Artifact.submissionArtifact.instructionPC 3791 = 4845 := GuardInstructionWindow.pc 197
private theorem pc3854 : Artifact.submissionArtifact.instructionPC 3792 = 4846 := GuardInstructionWindow.pc 198
private theorem pc3855 : Artifact.submissionArtifact.instructionPC 3793 = 4847 := GuardInstructionWindow.pc 199
private theorem pc3856 : Artifact.submissionArtifact.instructionPC 3794 = 4849 := GuardInstructionWindow.pc 200

@[simp] private theorem sizeHitPC3838 : Artifact.submissionArtifact.instructionPC 3776 = 4822 :=
  GuardInstructionWindow.pc 182
@[simp] private theorem sizeHitPC3839 : Artifact.submissionArtifact.instructionPC 3777 = 4823 :=
  GuardInstructionWindow.pc 183
@[simp] private theorem sizeHitPC3840 : Artifact.submissionArtifact.instructionPC 3778 = 4824 :=
  GuardInstructionWindow.pc 184
@[simp] private theorem sizeHitPC3841 : Artifact.submissionArtifact.instructionPC 3779 = 4826 :=
  GuardInstructionWindow.pc 185
@[simp] private theorem sizeHitPC3842 : Artifact.submissionArtifact.instructionPC 3780 = 4827 :=
  GuardInstructionWindow.pc 186

/-! ## The size mask -/

private def maskEntry : List Located :=
  [DirectGuard.opAt 3774 .JUMPDEST (by exact GuardInstructionWindow.get 180)]

private def sizePrefix : List Located :=
  [DirectGuard.pushAt 3775 17 342276208914615837337402008677671501826 (by exact GuardInstructionWindow.get 181),
   DirectGuard.opAt 3776 .CALLDATASIZE (by exact GuardInstructionWindow.get 182),
   DirectGuard.opAt 3777 .SHR (by exact GuardInstructionWindow.get 183),
   DirectGuard.pushAt 3778 1 1 (by exact GuardInstructionWindow.get 184),
   DirectGuard.opAt 3779 .AND (by exact GuardInstructionWindow.get 185),
   DirectGuard.pushAt 3780 1 115 (by exact GuardInstructionWindow.get 186)]

private def maskJump : List Located :=
  [DirectGuard.opAt 3781 .JUMPI (by exact GuardInstructionWindow.get 187)]
...
private theorem run_mask_entry (input : ByteArray) :
    DirectGuard.run maskEntry (PatternedScan.stS input 4803 []) =
      some (PatternedScan.stS input 4804 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 3774 4803 [] (by norm_num) pc3836)
    (PatternedScan.stepS_jumpdest input 4803 [] (by simp) (by norm_num))

private theorem run_size_prefix (input : ByteArray) :
    DirectGuard.run sizePrefix (PatternedScan.stS input 4804 []) =
      some (PatternedScan.stS input 4829 [115, sizeBit input]) := by
  have hpc : Artifact.submissionArtifact.instructionPC 3775 = 4804 := pc3837
...
private theorem run_mask_match (input : ByteArray) (hc : UInt256.isTrue (sizeBit input)) :
    DirectGuard.run maskJump (PatternedScan.stS input 4829 [115, sizeBit input]) =
      some (PatternedScan.stS input 115 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 3781 4829 [115, sizeBit input] (by norm_num) pc3843)
    (PatternedScan.stepS_jumpi_taken input 4829 115 115 (sizeBit input) []
...
private theorem run_mask_fall (input : ByteArray) (hc : ¬ UInt256.isTrue (sizeBit input)) :
    DirectGuard.run maskJump (PatternedScan.stS input 4829 [115, sizeBit input]) =
      some (PatternedScan.stS input 4830 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 3781 4829 [115, sizeBit input] (by norm_num) pc3843)
    (PatternedScan.stepS_jumpi_fall input 4829 115 (sizeBit input) []
...
private def gasSteps_mask_prefix (input : ByteArray) :
    GasSteps (DirectGuard.guardEntry input) (PatternedScan.stS input 4829 [115, sizeBit input]) :=
...
private def gasSteps_mask_fail (input : ByteArray) (hfit : CalldataFits input)
    ...
    GasSteps (DirectGuard.guardEntry input) (PatternedScan.stS input 4830 []) := by
...
private def lengthPrefix : List Located :=
  [DirectGuard.opAt 3782 .CALLDATASIZE (by exact GuardInstructionWindow.get 188),
   DirectGuard.pushAt 3783 2 256 (by exact GuardInstructionWindow.get 189),
   DirectGuard.opAt 3784 .EQ (by exact GuardInstructionWindow.get 190),
   DirectGuard.opAt 3785 .CALLDATASIZE (by exact GuardInstructionWindow.get 191),
   DirectGuard.pushAt 3786 2 376 (by exact GuardInstructionWindow.get 192),
   DirectGuard.opAt 3787 .EQ (by exact GuardInstructionWindow.get 193),
   DirectGuard.opAt 3788 .CALLDATASIZE (by exact GuardInstructionWindow.get 194),
   DirectGuard.pushAt 3789 2 1000 (by exact GuardInstructionWindow.get 195),
   DirectGuard.opAt 3790 .EQ (by exact GuardInstructionWindow.get 196),
   DirectGuard.opAt 3791 .OR (by exact GuardInstructionWindow.get 197),
   DirectGuard.opAt 3792 .OR (by exact GuardInstructionWindow.get 198),
   DirectGuard.pushAt 3793 1 115 (by exact GuardInstructionWindow.get 199)]

private def lengthJump : List Located :=
  [DirectGuard.opAt 3794 .JUMPI (by exact GuardInstructionWindow.get 200)]

private theorem run_length_prefix (input : ByteArray) :
    DirectGuard.run lengthPrefix (PatternedScan.stS input 4830 []) =
      some (PatternedScan.stS input 4849 [115, lengthCond input]) := by
  let n := UInt256.ofNat input.size
  let e256 := UInt256.eq 256 n
  let e376 := UInt256.eq 376 n
  let e1000 := UInt256.eq 1000 n
  let l0 : Located := DirectGuard.opAt 3782 .CALLDATASIZE (by exact GuardInstructionWindow.get 188)
  have h0 := PatternedScan.blockOfS l0
    (PatternedScan.pcFactS input 3782 4830 [] (by norm_num) pc3844)
    (calldatasize_step input 4830 [] (by simp) (by norm_num))
  let l1 : Located := DirectGuard.pushAt 3783 2 256 (by exact GuardInstructionWindow.get 189)
  have h1 := PatternedScan.blockOfS l1
    (PatternedScan.pcFactS input 3783 4831 [n] (by norm_num) pc3845)
    (PatternedScan.stepS_push input 4831 2 256 [n] (by simp) (by decide) (by decide) (by norm_num))
  let l2 : Located := DirectGuard.opAt 3784 .EQ (by exact GuardInstructionWindow.get 190)
  have h2 := PatternedScan.blockOfS l2
    (PatternedScan.pcFactS input 3784 4834 [256, n] (by norm_num) pc3846)
    (PatternedScan.stepS_eq input 4834 256 n [] (by simp) (by norm_num))
  let l3 : Located := DirectGuard.opAt 3785 .CALLDATASIZE (by exact GuardInstructionWindow.get 191)
  have h3 := PatternedScan.blockOfS l3
    (PatternedScan.pcFactS input 3785 4835 [e256] (by norm_num) pc3847)
    (calldatasize_step input 4835 [e256] (by simp) (by norm_num))
  let l4 : Located := DirectGuard.pushAt 3786 2 376 (by exact GuardInstructionWindow.get 192)
  have h4 := PatternedScan.blockOfS l4
    (PatternedScan.pcFactS input 3786 4836 [n, e256] (by norm_num) pc3848)
    (PatternedScan.stepS_push input 4836 2 376 [n, e256] (by simp) (by decide) (by decide) (by norm_num))
  let l5 : Located := DirectGuard.opAt 3787 .EQ (by exact GuardInstructionWindow.get 193)
  have h5 := PatternedScan.blockOfS l5
    (PatternedScan.pcFactS input 3787 4839 [376, n, e256] (by norm_num) pc3849)
    (PatternedScan.stepS_eq input 4839 376 n [e256] (by simp) (by norm_num))
  let l6 : Located := DirectGuard.opAt 3788 .CALLDATASIZE (by exact GuardInstructionWindow.get 194)
  have h6 := PatternedScan.blockOfS l6
    (PatternedScan.pcFactS input 3788 4840 [e376, e256] (by norm_num) pc3850)
    (calldatasize_step input 4840 [e376, e256] (by simp) (by norm_num))
  let l7 : Located := DirectGuard.pushAt 3789 2 1000 (by exact GuardInstructionWindow.get 195)
  have h7 := PatternedScan.blockOfS l7
    (PatternedScan.pcFactS input 3789 4841 [n, e376, e256] (by norm_num) pc3851)
    (PatternedScan.stepS_push input 4841 2 1000 [n, e376, e256] (by simp) (by decide) (by decide) (by norm_num))
  let l8 : Located := DirectGuard.opAt 3790 .EQ (by exact GuardInstructionWindow.get 196)
  have h8 := PatternedScan.blockOfS l8
    (PatternedScan.pcFactS input 3790 4844 [1000, n, e376, e256] (by norm_num) pc3852)
    (PatternedScan.stepS_eq input 4844 1000 n [e376, e256] (by simp) (by norm_num))
  let l9 : Located := DirectGuard.opAt 3791 .OR (by exact GuardInstructionWindow.get 197)
  have h9 := PatternedScan.blockOfS l9
    (PatternedScan.pcFactS input 3791 4845 [e1000, e376, e256] (by norm_num) pc3853)
    (PatternedScan.stepS_or input 4845 e1000 e376 [e256] (by simp) (by norm_num))
  let l10 : Located := DirectGuard.opAt 3792 .OR (by exact GuardInstructionWindow.get 198)
  have h10 := PatternedScan.blockOfS l10
    (PatternedScan.pcFactS input 3792 4846 [UInt256.lor e1000 e376, e256] (by norm_num) pc3854)
    (PatternedScan.stepS_or input 4846 (UInt256.lor e1000 e376) e256 [] (by simp) (by norm_num))
  let l11 : Located := DirectGuard.pushAt 3793 1 115 (by exact GuardInstructionWindow.get 199)
  have h11 := PatternedScan.blockOfS l11
    (PatternedScan.pcFactS input 3793 4847 [UInt256.lor (UInt256.lor e1000 e376) e256] (by norm_num) pc3855)
    (PatternedScan.stepS_push input 4847 1 115 [UInt256.lor (UInt256.lor e1000 e376) e256]
...
private def gasSteps_length_match (input : ByteArray)
    ...
    GasSteps (PatternedScan.stS input 4830 []) (PatternedScan.patternedEntry input) := by
  have hj : DirectGuard.run lengthJump (PatternedScan.stS input 4849 [115, lengthCond input]) =
      some (PatternedScan.stS input 115 []) :=
    PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 3794 4849 _ (by norm_num) pc3856)
      (PatternedScan.stepS_jumpi_taken input 4849 115 115 (lengthCond input) []
...
private def gasSteps_length_fail (input : ByteArray) (hfit : CalldataFits input)
    (h256 : input.size ≠ 256) (h376 : input.size ≠ 376) (h1000 : input.size ≠ 1000) :
    GasSteps (PatternedScan.stS input 4830 []) (AbcArm.armEntry input) := by
  have hj : DirectGuard.run lengthJump (PatternedScan.stS input 4849 [115, lengthCond input]) =
      some (AbcArm.armEntry input) :=
    PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 3794 4849 _ (by norm_num) pc3856)
      (PatternedScan.stepS_jumpi_fall input 4849 115 (lengthCond input) []
