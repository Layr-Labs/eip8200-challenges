...
@[simp] private theorem entryPC61 : Artifact.submissionArtifact.instructionPC 70 = 118 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

private theorem guard_match_dest : Decode.isValidJumpDest submissionBytecode 118 = true :=
...
private theorem pc3836 : Artifact.submissionArtifact.instructionPC 246 = 425 := J2EntryWindow.pc 0
private theorem pc3837 : Artifact.submissionArtifact.instructionPC 247 = 426 := J2EntryWindow.pc 1
private theorem pc3843 : Artifact.submissionArtifact.instructionPC 253 = 451 := J2EntryWindow.pc 7
private theorem packedPC3735 : Artifact.submissionArtifact.instructionPC 254 = 452 := J2EntryWindow.pc 8
private theorem packedPC3736 : Artifact.submissionArtifact.instructionPC 255 = 455 := J2EntryWindow.pc 9
private theorem packedPC3737 : Artifact.submissionArtifact.instructionPC 256 = 461 := J2EntryWindow.pc 10
private theorem packedPC3738 : Artifact.submissionArtifact.instructionPC 257 = 462 := J2EntryWindow.pc 11
private theorem packedPC3739 : Artifact.submissionArtifact.instructionPC 258 = 464 := J2EntryWindow.pc 12
private theorem packedPC3740 : Artifact.submissionArtifact.instructionPC 259 = 465 := J2EntryWindow.pc 13
private theorem packedPC3741 : Artifact.submissionArtifact.instructionPC 260 = 466 := J2EntryWindow.pc 14
private theorem packedPC3742 : Artifact.submissionArtifact.instructionPC 261 = 467 := J2EntryWindow.pc 15
private theorem packedPC3743 : Artifact.submissionArtifact.instructionPC 262 = 468 := J2EntryWindow.pc 16
private theorem packedPC3744 : Artifact.submissionArtifact.instructionPC 263 = 469 := J2EntryWindow.pc 17
private theorem packedPC3745 : Artifact.submissionArtifact.instructionPC 264 = 471 := J2EntryWindow.pc 18

@[simp] private theorem sizeHitPC3838 : Artifact.submissionArtifact.instructionPC 248 = 428 :=
  J2EntryWindow.pc 2
@[simp] private theorem sizeHitPC3839 : Artifact.submissionArtifact.instructionPC 249 = 446 :=
  J2EntryWindow.pc 3
@[simp] private theorem sizeHitPC3840 : Artifact.submissionArtifact.instructionPC 250 = 447 :=
  J2EntryWindow.pc 4
@[simp] private theorem sizeHitPC3841 : Artifact.submissionArtifact.instructionPC 251 = 448 :=
  J2EntryWindow.pc 5
@[simp] private theorem sizeHitPC3842 : Artifact.submissionArtifact.instructionPC 252 = 449 :=
  J2EntryWindow.pc 6

/-! ## The size mask -/

private def maskEntry : List Located :=
  [DirectGuard.opAt 246 .JUMPDEST (by exact J2EntryWindow.get 0)]

private def sizePrefix : List Located :=
  [DirectGuard.pushAt 247 1 1 (by exact J2EntryWindow.get 1),
   DirectGuard.pushAt 248 17 342276208914615837337402008677671501826 (by exact J2EntryWindow.get 2),
   DirectGuard.opAt 249 .CALLDATASIZE (by exact J2EntryWindow.get 3),
   DirectGuard.opAt 250 .SHR (by exact J2EntryWindow.get 4),
   DirectGuard.opAt 251 .AND (by exact J2EntryWindow.get 5),
   DirectGuard.pushAt 252 1 118 (by exact J2EntryWindow.get 6)]

private def maskJump : List Located :=
  [DirectGuard.opAt 253 .JUMPI (by exact J2EntryWindow.get 7)]

private def guardMatchTail : List Located :=
  [DirectGuard.opAt 70 .JUMPDEST]
...
private theorem run_mask_entry (input : ByteArray) :
    DirectGuard.run maskEntry (PatternedScan.stS input 425 []) =
      some (PatternedScan.stS input 426 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 246 425 [] (by norm_num) pc3836)
    (PatternedScan.stepS_jumpdest input 425 [] (by simp) (by norm_num))

private theorem run_size_prefix (input : ByteArray) :
    DirectGuard.run sizePrefix (PatternedScan.stS input 426 []) =
      some (PatternedScan.stS input 451 [118, sizeBit input]) := by
  have hpc : Artifact.submissionArtifact.instructionPC 247 = 426 := pc3837
...
private theorem run_mask_match (input : ByteArray) (hc : UInt256.isTrue (sizeBit input)) :
    DirectGuard.run maskJump (PatternedScan.stS input 451 [118, sizeBit input]) =
      some (PatternedScan.stS input 118 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 253 451 [118, sizeBit input] (by norm_num) pc3843)
    (PatternedScan.stepS_jumpi_taken input 451 118 118 (sizeBit input) []
...
private theorem run_mask_fall (input : ByteArray) (hc : ¬ UInt256.isTrue (sizeBit input)) :
    DirectGuard.run maskJump (PatternedScan.stS input 451 [118, sizeBit input]) =
      some (PatternedScan.stS input 452 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 253 451 [118, sizeBit input] (by norm_num) pc3843)
    (PatternedScan.stepS_jumpi_fall input 451 118 (sizeBit input) []
      (by simp) (by norm_num) hc)

private theorem run_guard_match_tail (input : ByteArray) :
    DirectGuard.run guardMatchTail (PatternedScan.stS input 118 []) =
...
private def gasSteps_mask_prefix (input : ByteArray) :
    GasSteps (DirectGuard.guardEntry input) (PatternedScan.stS input 451 [118, sizeBit input]) :=
...
private def gasSteps_mask_fail (input : ByteArray) (hfit : CalldataFits input)
    ...
    GasSteps (DirectGuard.guardEntry input) (PatternedScan.stS input 452 []) := by
...
private def lengthPrefix : List Located :=
  [DirectGuard.pushAt 254 2 1016 (by exact J2EntryWindow.get 8),
   DirectGuard.pushAt 255 5 6308489473 (by exact J2EntryWindow.get 9),
   DirectGuard.opAt 256 .CALLDATASIZE (by exact J2EntryWindow.get 10),
   DirectGuard.pushAt 257 1 24 (by exact J2EntryWindow.get 11),
   DirectGuard.opAt 258 .AND (by exact J2EntryWindow.get 12),
   DirectGuard.opAt 259 .SHR (by exact J2EntryWindow.get 13),
   DirectGuard.opAt 260 .AND (by exact J2EntryWindow.get 14),
   DirectGuard.opAt 261 .CALLDATASIZE (by exact J2EntryWindow.get 15),
   DirectGuard.opAt 262 .EQ (by exact J2EntryWindow.get 16),
   DirectGuard.pushAt 263 1 118 (by exact J2EntryWindow.get 17)]

private def lengthJump : List Located :=
  [DirectGuard.opAt 264 .JUMPI (by exact J2EntryWindow.get 18)]

private theorem run_length_prefix (input : ByteArray) :
    DirectGuard.run lengthPrefix (PatternedScan.stS input 452 []) =
      some (PatternedScan.stS input 471 [118, lengthCond input]) := by
...
private def gasSteps_length_match (input : ByteArray)
    ...
    GasSteps (PatternedScan.stS input 452 []) (PatternedScan.patternedEntry input) := by
  have hj : DirectGuard.run lengthJump (PatternedScan.stS input 471 [118, lengthCond input]) =
      some (PatternedScan.stS input 118 []) :=
    PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 264 471 _ (by norm_num) packedPC3745)
      (PatternedScan.stepS_jumpi_taken input 471 118 118 (lengthCond input) []
...
private def gasSteps_length_fail (input : ByteArray) (hfit : CalldataFits input)
    (h256 : input.size ≠ 256) (h376 : input.size ≠ 376) (h1000 : input.size ≠ 1000) :
    GasSteps (PatternedScan.stS input 452 []) (AbcArm.armEntry input) := by
  have hj : DirectGuard.run lengthJump (PatternedScan.stS input 471 [118, lengthCond input]) =
      some (AbcArm.armEntry input) :=
    PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 264 471 _ (by norm_num) packedPC3745)
      (PatternedScan.stepS_jumpi_fall input 471 118 (lengthCond input) []
