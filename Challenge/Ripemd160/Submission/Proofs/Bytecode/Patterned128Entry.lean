...
private theorem guard_fallback_dest : Decode.isValidJumpDest submissionBytecode 387 = true :=
...
private def byteState (input : ByteArray) : State :=
  PatternedScan.stS input 287 [387, byteValue input]
private def sizeEntry (input : ByteArray) : State := PatternedScan.stS input 288 []
private def sizeState (input : ByteArray) : State := PatternedScan.stS input 315 [387, flag input]
