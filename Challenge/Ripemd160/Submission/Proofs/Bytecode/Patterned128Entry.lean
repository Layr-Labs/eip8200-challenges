...
@[simp] private theorem entryPC61 : Artifact.submissionArtifact.instructionPC 3558 = 4795 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

private theorem guard_match_dest : Decode.isValidJumpDest submissionBytecode 4795 = true :=
...
private theorem pc3836 : Artifact.submissionArtifact.instructionPC 3519 = 4681 := J2EntryWindow.pc 0
private theorem pc3837 : Artifact.submissionArtifact.instructionPC 3520 = 4682 := J2EntryWindow.pc 1
private theorem pc3843 : Artifact.submissionArtifact.instructionPC 3526 = 4708 := J2EntryWindow.pc 7
private theorem packedPC3735 : Artifact.submissionArtifact.instructionPC 3527 = 4709 := J2EntryWindow.pc 8
private theorem packedPC3736 : Artifact.submissionArtifact.instructionPC 3528 = 4712 := J2EntryWindow.pc 9
private theorem packedPC3737 : Artifact.submissionArtifact.instructionPC 3529 = 4718 := J2EntryWindow.pc 10
private theorem packedPC3738 : Artifact.submissionArtifact.instructionPC 3530 = 4719 := J2EntryWindow.pc 11
private theorem packedPC3739 : Artifact.submissionArtifact.instructionPC 3531 = 4721 := J2EntryWindow.pc 12
private theorem packedPC3740 : Artifact.submissionArtifact.instructionPC 3532 = 4722 := J2EntryWindow.pc 13
private theorem packedPC3741 : Artifact.submissionArtifact.instructionPC 3533 = 4723 := J2EntryWindow.pc 14
private theorem packedPC3742 : Artifact.submissionArtifact.instructionPC 3534 = 4724 := J2EntryWindow.pc 15
private theorem packedPC3743 : Artifact.submissionArtifact.instructionPC 3535 = 4725 := J2EntryWindow.pc 16
private theorem packedPC3744 : Artifact.submissionArtifact.instructionPC 3536 = 4726 := J2EntryWindow.pc 17
private theorem packedPC3745 : Artifact.submissionArtifact.instructionPC 3537 = 4729 := J2EntryWindow.pc 18

@[simp] private theorem sizeHitPC3838 : Artifact.submissionArtifact.instructionPC 3521 = 4684 :=
  J2EntryWindow.pc 2
@[simp] private theorem sizeHitPC3839 : Artifact.submissionArtifact.instructionPC 3522 = 4702 :=
  J2EntryWindow.pc 3
@[simp] private theorem sizeHitPC3840 : Artifact.submissionArtifact.instructionPC 3523 = 4703 :=
  J2EntryWindow.pc 4
@[simp] private theorem sizeHitPC3841 : Artifact.submissionArtifact.instructionPC 3524 = 4704 :=
  J2EntryWindow.pc 5
@[simp] private theorem sizeHitPC3842 : Artifact.submissionArtifact.instructionPC 3525 = 4705 :=
  J2EntryWindow.pc 6

/-! ## The size mask -/

private def maskEntry : List Located :=
  [DirectGuard.opAt 3519 .JUMPDEST (by exact J2EntryWindow.get 0)]

private def sizePrefix : List Located :=
  [DirectGuard.pushAt 3520 1 1 (by exact J2EntryWindow.get 1),
   DirectGuard.pushAt 3521 17 342276208914615837337402008677671501826 (by exact J2EntryWindow.get 2),
   DirectGuard.opAt 3522 .CALLDATASIZE (by exact J2EntryWindow.get 3),
   DirectGuard.opAt 3523 .SHR (by exact J2EntryWindow.get 4),
   DirectGuard.opAt 3524 .AND (by exact J2EntryWindow.get 5),
   DirectGuard.pushAt 3525 2 4795 (by exact J2EntryWindow.get 6)]

private def maskJump : List Located :=
  [DirectGuard.opAt 3526 .JUMPI (by exact J2EntryWindow.get 7)]
...
private theorem run_mask_entry (input : ByteArray) :
    DirectGuard.run maskEntry (PatternedScan.stS input 4681 []) =
      some (PatternedScan.stS input 4682 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 3519 4681 [] (by norm_num) pc3836)
    (PatternedScan.stepS_jumpdest input 4681 [] (by simp) (by norm_num))

private theorem run_size_prefix (input : ByteArray) :
    DirectGuard.run sizePrefix (PatternedScan.stS input 4682 []) =
      some (PatternedScan.stS input 4708 [4795, sizeBit input]) := by
  have hpc : Artifact.submissionArtifact.instructionPC 3520 = 4682 := pc3837
...
private theorem run_mask_match (input : ByteArray) (hc : UInt256.isTrue (sizeBit input)) :
    DirectGuard.run maskJump (PatternedScan.stS input 4708 [4795, sizeBit input]) =
      some (PatternedScan.stS input 4795 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 3526 4708 [4795, sizeBit input] (by norm_num) pc3843)
    (PatternedScan.stepS_jumpi_taken input 4708 4795 4795 (sizeBit input) []
...
private theorem run_mask_fall (input : ByteArray) (hc : ¬ UInt256.isTrue (sizeBit input)) :
    DirectGuard.run maskJump (PatternedScan.stS input 4708 [4795, sizeBit input]) =
      some (PatternedScan.stS input 4709 []) :=
  PatternedScan.blockOfS _
    (PatternedScan.pcFactS input 3526 4708 [4795, sizeBit input] (by norm_num) pc3843)
    (PatternedScan.stepS_jumpi_fall input 4708 4795 (sizeBit input) []
      (by simp) (by norm_num) hc)

private theorem run_guard_match_tail (input : ByteArray) :
    DirectGuard.run guardMatchTail (PatternedScan.stS input 4795 []) =
...
private def gasSteps_mask_prefix (input : ByteArray) :
    GasSteps (DirectGuard.guardEntry input) (PatternedScan.stS input 4708 [4795, sizeBit input]) :=
...
private def gasSteps_mask_fail (input : ByteArray) (hfit : CalldataFits input)
    ...
    GasSteps (DirectGuard.guardEntry input) (PatternedScan.stS input 4709 []) := by
...
private def lengthPrefix : List Located :=
  [DirectGuard.pushAt 3527 2 1016 (by exact J2EntryWindow.get 8),
   DirectGuard.pushAt 3528 5 6308489473 (by exact J2EntryWindow.get 9),
   DirectGuard.opAt 3529 .CALLDATASIZE (by exact J2EntryWindow.get 10),
   DirectGuard.pushAt 3530 1 24 (by exact J2EntryWindow.get 11),
   DirectGuard.opAt 3531 .AND (by exact J2EntryWindow.get 12),
   DirectGuard.opAt 3532 .SHR (by exact J2EntryWindow.get 13),
   DirectGuard.opAt 3533 .AND (by exact J2EntryWindow.get 14),
   DirectGuard.opAt 3534 .CALLDATASIZE (by exact J2EntryWindow.get 15),
   DirectGuard.opAt 3535 .EQ (by exact J2EntryWindow.get 16),
   DirectGuard.pushAt 3536 2 4795 (by exact J2EntryWindow.get 17)]

private def lengthJump : List Located :=
  [DirectGuard.opAt 3537 .JUMPI (by exact J2EntryWindow.get 18)]

private theorem run_length_prefix (input : ByteArray) :
    DirectGuard.run lengthPrefix (PatternedScan.stS input 4709 []) =
      some (PatternedScan.stS input 4729 [4795, lengthCond input]) := by
...
private def gasSteps_length_match (input : ByteArray)
    ...
    GasSteps (PatternedScan.stS input 4709 []) (PatternedScan.patternedEntry input) := by
  have hj : DirectGuard.run lengthJump (PatternedScan.stS input 4729 [4795, lengthCond input]) =
      some (PatternedScan.stS input 4795 []) :=
    PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 3537 4729 _ (by norm_num) packedPC3745)
      (PatternedScan.stepS_jumpi_taken input 4729 4795 4795 (lengthCond input) []
...
private def gasSteps_length_fail (input : ByteArray) (hfit : CalldataFits input)
    (h256 : input.size ≠ 256) (h376 : input.size ≠ 376) (h1000 : input.size ≠ 1000) :
    GasSteps (PatternedScan.stS input 4709 []) (AbcArm.armEntry input) := by
  have hj : DirectGuard.run lengthJump (PatternedScan.stS input 4729 [4795, lengthCond input]) =
      some (AbcArm.armEntry input) :=
    PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 3537 4729 _ (by norm_num) packedPC3745)
      (PatternedScan.stepS_jumpi_fall input 4729 4795 (lengthCond input) []
