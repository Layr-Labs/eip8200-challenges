...
`instructionPC` over the chunked instruction list and scan the 5317-byte array
...
theorem runLocated_of_pcS {artifact : Challenge.EvmProof.DataProgramArtifact}
...
theorem runLocatedBlock_singleS {artifact : Challenge.EvmProof.DataProgramArtifact}
...
theorem stepS_jumpdest (input : ByteArray) (pc : Nat) (stk : List UInt256)
    ...
    Challenge.EvmProof.DataStepper.runInstr (.op .JUMPDEST) (stS input pc stk) =
...
theorem stepS_push (input : ByteArray) (pc w : Nat) (v : UInt256) (stk : List UInt256)
    ...
    (hpc : pc + (w + 1) < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.push ⟨w, by omega⟩ v) (stS input pc stk) =
...
theorem stepS_push0 (input : ByteArray) (pc : Nat) (stk : List UInt256)
    (hlen : stk.length < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.push ⟨0, by omega⟩ 0) (stS input pc stk) =
...
theorem stepS_pop (input : ByteArray) (pc : Nat) (a : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.DataStepper.runInstr (.op .POP) (stS input pc (a :: rest)) =
...
theorem stepS_dup (input : ByteArray) (pc n : Nat) (hn : n < 16) (stk : List UInt256)
    ...
    (hlen : stk.length < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op (.Dup ⟨n, hn⟩)) (stS input pc stk) =
...
theorem stepS_swap (input : ByteArray) (pc n : Nat) (hn : n < 16)
    ...
    (hlen : stk.length < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op (.Swap ⟨n, hn⟩)) (stS input pc stk) =
...
theorem stepS_calldataload (input : ByteArray) (pc : Nat) (off : UInt256)
    ...
    Challenge.EvmProof.DataStepper.runInstr (.op .CALLDATALOAD)
...
theorem stepS_add (input : ByteArray) (pc : Nat) (a b : UInt256) (rest : List UInt256)
    ...
    Challenge.EvmProof.DataStepper.runInstr (.op .ADD) (stS input pc (a :: b :: rest)) =
...
theorem stepS_mul (input : ByteArray) (pc : Nat) (a b : UInt256) (rest : List UInt256)
    (hlen : rest.length + 2 < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op .MUL) (stS input pc (a :: b :: rest)) =
      some (stS input (pc + 1) ((a * b) :: rest)) := by
  unfold Challenge.EvmProof.DataStepper.runInstr
  rw [if_pos (by simpa using hlen)]
  simp only [stS, Challenge.EvmProof.Word.succ_ofNat hpc]
...
theorem stepS_sub (input : ByteArray) (pc : Nat) (a b : UInt256) (rest : List UInt256)
    (hlen : rest.length + 2 < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op .SUB) (stS input pc (a :: b :: rest)) =
      some (stS input (pc + 1) ((a - b) :: rest)) := by
  unfold Challenge.EvmProof.DataStepper.runInstr
  rw [if_pos (by simpa using hlen)]
  simp only [stS, Challenge.EvmProof.Word.succ_ofNat hpc]
...
theorem stepS_and (input : ByteArray) (pc : Nat) (a b : UInt256) (rest : List UInt256)
    (hlen : rest.length + 2 < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op .AND) (stS input pc (a :: b :: rest)) =
...
theorem stepS_or (input : ByteArray) (pc : Nat) (a b : UInt256) (rest : List UInt256)
    (hlen : rest.length + 2 < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op .OR) (stS input pc (a :: b :: rest)) =
...
theorem stepS_byte (input : ByteArray) (pc : Nat) (a b : UInt256) (rest : List UInt256)
    (hlen : rest.length + 2 < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op .BYTE) (stS input pc (a :: b :: rest)) =
...
theorem stepS_xor (input : ByteArray) (pc : Nat) (a b : UInt256) (rest : List UInt256)
    (hlen : rest.length + 2 < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op .XOR) (stS input pc (a :: b :: rest)) =
...
theorem stepS_gt (input : ByteArray) (pc : Nat) (a b : UInt256) (rest : List UInt256)
    (hlen : rest.length + 2 < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op .GT) (stS input pc (a :: b :: rest)) =
...
theorem stepS_eq (input : ByteArray) (pc : Nat) (a b : UInt256) (rest : List UInt256)
    (hlen : rest.length + 2 < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op .EQ) (stS input pc (a :: b :: rest)) =
...
theorem stepS_shr (input : ByteArray) (pc : Nat) (shift value : UInt256)
    ...
    Challenge.EvmProof.DataStepper.runInstr (.op .SHR)
...
theorem stepS_shl (input : ByteArray) (pc : Nat) (shift value : UInt256)
    (rest : List UInt256) (hlen : rest.length + 2 < 1024) (hpc : pc + 1 < 2 ^ 256) :
    Challenge.EvmProof.DataStepper.runInstr (.op .SHL)
        (stS input pc (shift :: value :: rest)) =
...
theorem stepS_jump (input : ByteArray) (pc target : Nat) (d : UInt256)
    ...
    Challenge.EvmProof.DataStepper.runInstr (.op .JUMP) (stS input pc (d :: rest)) =
...
theorem stepS_jumpi_taken (input : ByteArray) (pc target : Nat) (d c : UInt256)
    ...
    (hvalid : Decode.isValidJumpDest submissionBytecode target = true) :
    Challenge.EvmProof.DataStepper.runInstr (.op .JUMPI) (stS input pc (d :: c :: rest)) =
      some (stS input target rest) := by
  subst hd
  unfold Challenge.EvmProof.DataStepper.runInstr
  rw [if_pos (by simpa using hlen)]
