...
theorem middleTemplateUnmasked_length (shift address : Nat) (slot : Fin 16) :
    (middleTemplateUnmasked shift address).length + 2 =
...
theorem fullTemplate_length : fullTemplate.length = 151 := by
...
theorem fullTemplate_byteLength : (assembleBytes fullTemplate).length = 270 := by
...
theorem fullTemplate_staticGas : staticGas fullTemplate = 454 := by
...
private theorem storeAddress_advances (address : Nat) :
    DenseScheduleLift.Advances
      (if address = 0 then Instr.push ⟨0, by decide⟩ (UInt256.ofNat address)
       else if address < 256 then DenseScheduleTemplate.push1 (UInt256.ofNat address)
