...
def gasSteps_padAll (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hsmall : s.executionEnv.calldata.size < 5225)
  ...
  -- The branch is taken exactly when `n < 5225`, matching the candidate CODESIZE value;
...
def gasSteps_prepare (s : State) (input : ByteArray) (i : Nat) (h : Compression.HashState)
    ...
    (hordinary : input.size = DriverTrace.blockOffset i → input.size < 5225)
    (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) (hfork : s.fork = .Osaka)
    (hr : s.halt = .Running)
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
