...
private theorem template_slice :
    (A.instructions.drop 2710).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2710 template_slice (by
    change 2692 + template.length ≤ Artifact.submissionInstructions.length
