...
private theorem template_slice :
    (A.instructions.drop 2718).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2718 template_slice (by
    change 2700 + template.length ≤ Artifact.submissionInstructions.length
