...
private theorem template_slice :
    (A.instructions.drop 2714).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2714 template_slice (by
    change 2696 + template.length ≤ Artifact.submissionInstructions.length
