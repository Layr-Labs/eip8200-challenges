...
private theorem template_slice :
    (A.instructions.drop 2708).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2708 template_slice (by
    change 2708 + template.length ≤ Artifact.submissionInstructions.length
