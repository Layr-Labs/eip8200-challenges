...
theorem upper_value :
    UInt256.shiftLeft (UInt256.ofNat 0x50a28be6) (UInt256.ofNat 156) =
      UInt256.ofNat 460344169260758029377710773882198039553172832284 := by decide
...
theorem mixed_value :
    UInt256.lor (UInt256.ofNat 0x5a827999)
      ...
      UInt256.ofNat 526962527014005041256681316140890030896371104181 := by decide
...
def replaceTemplate : List Instr :=
  [.op (.Swap ⟨0, by decide⟩), .op .POP,
   .push ⟨20, by decide⟩
     (UInt256.ofNat 526962527014005041256681316140890030896371104181)]
...
theorem run_upperTemplate (s : State) (pc : UInt256) (rho : List UInt256)
    ...
    runInstrSeq upperTemplate {s with pc := pc, stack := rho} =
      some {s with
        ...
        stack := UInt256.ofNat 460344169260758029377710773882198039553172832284 :: rho} := by
...
theorem run_replaceTemplate (s : State) (pc value discarded : UInt256) (rho : List UInt256)
    ...
    runInstrSeq replaceTemplate {s with pc := pc, stack := value :: discarded :: rho} =
      some {s with
        ...
        stack := UInt256.ofNat 526962527014005041256681316140890030896371104181 :: value :: rho} := by
