...
private theorem nat_mask (n : Nat) (hn : n < 2 ^ 256) :
    (2 ^ 256 - 64) &&& n = n / 64 * 64 := by
  have hmask : (2 ^ 256 - 64) = (2 ^ 250 - 1) <<< 6 := by
    rw [Nat.shiftLeft_eq]; norm_num
  have hrhs : n / 64 * 64 = (n >>> 6) <<< 6 := by
  ...
  by_cases h6 : 6 ≤ i
  · simp only [h6, decide_true, Bool.true_and]
    rw [show 6 + (i - 6) = i from by omega]
    by_cases h256 : i < 256
    · have hlt : i - 6 < 250 := by omega
      simp [hlt]
    · have h1 : ¬ (i - 6 < 250) := by omega
      have hpow : (2 : Nat) ^ 256 ≤ 2 ^ i :=
        Nat.pow_le_pow_right (by norm_num) (by omega)
      have h2 : n.testBit i = false :=
        Nat.testBit_lt_two_pow (Nat.lt_of_lt_of_le hn hpow)
      simp [h1, h2]
  · simp [h6]

/-- The bytecode clears the low six bits with a mask; the padding model
states the same value as a right shift followed by a left shift. -/
theorem lnot63_land (v : UInt256) :
    (UInt256.ofNat 63).lnot.land v =
      (v.shiftRight (UInt256.ofNat 6)).shiftLeft (UInt256.ofNat 6) := by
  have hv : v.toNat < 2 ^ 256 := v.val.isLt
  have hsr : (v.shiftRight (UInt256.ofNat 6)).toNat = v.toNat / 64 := by
    rw [Challenge.EvmProof.Word.shiftRight_toNat v (by norm_num),
      Nat.shiftRight_eq_div_pow]
  have hq : v.toNat / 64 < 2 ^ 256 := Nat.lt_of_le_of_lt (Nat.div_le_self _ _) hv
  have hres : v.toNat / 64 * 2 ^ 6 < 2 ^ 256 := by
    have h1 : v.toNat / 64 * 64 ≤ v.toNat := Nat.div_mul_le_self _ _
    norm_num at h1 ⊢
    omega
  have hrhs : (v.shiftRight (UInt256.ofNat 6)).shiftLeft (UInt256.ofNat 6)
      = UInt256.ofNat (v.toNat / 64 * 2 ^ 6) := by
    rw [Challenge.EvmProof.Word.word_eq_ofNat_toNat
      (v.shiftRight (UInt256.ofNat 6)), hsr]
    exact Challenge.EvmProof.Word.shiftLeft_ofNat hq (by norm_num) hres
  have hlnot : ((UInt256.ofNat 63).lnot).toNat = 2 ^ 256 - 64 := by
    unfold EvmSemantics.UInt256.lnot
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Challenge.EvmProof.Word.word_toNat_ofNat]
    norm_num [EvmSemantics.UInt256.size]
  rw [hrhs]
  apply Challenge.EvmProof.Word.word_ext
  rw [Challenge.EvmProof.Word.word_toNat_land, hlnot,
...
private theorem run_paddedLength (input : ByteArray) :
  ...
  simp [paddedLengthPath, Artifact.padLengthPath, Challenge.EvmProof.Stepper.runLocatedBlock,
    ...
    Padding.paddedWord, List.exchange, lnot63_land]
...
@[simp] private theorem validPadReturn :
    Decode.isValidJumpDest submissionBytecode 0x62c = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 768 = 0x62c := by rfl
  rw [← hpc]
  exact Artifact.submissionArtifact.isValidJumpDest_index 768 (by rfl)
...
def gasSteps_lengthExit (input : ByteArray) :
  ...
  have g2a := Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka lengthExitZeroPath (by rfl) (by rfl)
