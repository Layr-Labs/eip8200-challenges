...
def lengthIterationPath : List
    (Challenge.EvmProof.DataStepper.Located Artifact.submissionArtifact .Osaka) :=
  [⟨3620, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3621, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3622, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3623, .op .MSTORE8, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3624, .push ⟨1, by decide⟩ (UInt256.ofNat 1), by rfl, by decide⟩,
   ⟨3625, .op .ADD, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3626, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3627, .push ⟨1, by decide⟩ (UInt256.ofNat 8), by rfl, by decide⟩,
   ⟨3628, .op .SHR, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3629, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3630, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3631, .push ⟨2, by decide⟩ (UInt256.ofNat 4795), by rfl, by decide⟩,
   ⟨3632, .op .JUMPI, by rfl, wfOp (by decide) trivial rfl⟩]
...
def lengthBranchReady (input : ByteArray) (i : Nat) : State :=
  { lengthSteppedState input i with
    ...
    stack := [UInt256.ofNat 4795, lengthShift input (i + 1)] ++
...
@[simp] private theorem validLengthLoopHead :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 3620 = 4795 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
  exact Artifact.submissionArtifact.isValidJumpDest_index 3620 (by rfl)
