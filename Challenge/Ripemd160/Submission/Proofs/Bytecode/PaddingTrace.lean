...
@[simp] private theorem padGuardMiss_pc (input : ByteArray) :
    (padGuardMiss input).pc = UInt256.ofNat 4771 := by rfl
...
@[simp] private theorem padSentinel_pcToNat (input : ByteArray) :
    (padSentinel input).pc.toNat = 4779 := by rfl

@[simp] private theorem padSentinel_pc (input : ByteArray) :
    (padSentinel input).pc = UInt256.ofNat 4779 := by rfl

@[simp] private theorem padSentinel_pcSucc (input : ByteArray) :
    (padSentinel input).pc.succ = UInt256.ofNat 4780 := by
...
@[simp] private theorem padSentinelAddressReady_pc (input : ByteArray) :
    (padSentinelAddressReady input).pc = UInt256.ofNat 4778 := by rfl
...
def padGuardTaken (input : ByteArray) : State :=
  {padGuardMiss input with pc := UInt256.ofNat 4753, stack := initialFrame input}
...
def lengthIterationPath : List
    (Challenge.EvmProof.DataStepper.Located Artifact.submissionArtifact .Osaka) :=
  [⟨3613, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3614, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3615, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3616, .op .MSTORE8, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3617, .push ⟨1, by decide⟩ (UInt256.ofNat 1), by rfl, by decide⟩,
   ⟨3618, .op .ADD, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3619, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3620, .push ⟨1, by decide⟩ (UInt256.ofNat 8), by rfl, by decide⟩,
   ⟨3621, .op .SHR, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3622, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3623, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨3624, .push ⟨2, by decide⟩ (UInt256.ofNat 4788), by rfl, by decide⟩,
   ⟨3625, .op .JUMPI, by rfl, wfOp (by decide) trivial rfl⟩]
...
@[simp] private theorem lengthLoopState_pc (input : ByteArray) (i : Nat) :
    (lengthLoopState input i).pc = UInt256.ofNat 4788 := by rfl
...
def lengthBranchReady (input : ByteArray) (i : Nat) : State :=
  { lengthSteppedState input i with
    ...
    stack := [UInt256.ofNat 4788, lengthShift input (i + 1)] ++
...
@[simp] private theorem lengthSteppedState_pc (input : ByteArray) (i : Nat) :
    (lengthSteppedState input i).pc = UInt256.ofNat 4800 := by rfl
...
@[simp] private theorem lengthBranchReady_pc (input : ByteArray) (i : Nat) :
    (lengthBranchReady input i).pc = UInt256.ofNat 4804 := by rfl
...
@[simp] private theorem validLengthLoopHead :
    Decode.isValidJumpDest submissionBytecode 4788 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3613 = 4788 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
  exact Artifact.submissionArtifact.isValidJumpDest_index 3613 (by rfl)
...
def lengthExitEntered (input : ByteArray) (i : Nat) : State :=
  { lengthLoopState input i with pc := UInt256.ofNat 4805 }
...
@[simp] private theorem lengthExitEntered_pc (input : ByteArray) (i : Nat) :
    (lengthExitEntered input i).pc = UInt256.ofNat 4805 := by rfl
