...
def pushedReturn (input : ByteArray) : State :=
  { Main.initializedState input with
    ...
    stack := UInt256.ofNat 483 :: (Main.initializedState input).stack }
...
@[simp] private theorem initializedPC764 :
    Artifact.instructionPC 222 = 408 := by change Artifact.submissionArtifact.instructionPC 222 = 408; rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
...
@[simp] private theorem padLengthReady_pcToNat (input : ByteArray) :
    (padLengthReady input).pc.toNat = 420 := by rfl

@[simp] private theorem padLengthReady_pc (input : ByteArray) :
    (padLengthReady input).pc = UInt256.ofNat 420 := by rfl

@[simp] private theorem padLengthReady_pcSucc (input : ByteArray) :
    (padLengthReady input).pc.succ = UInt256.ofNat 421 := by
...
@[simp] private theorem padLengthReady_stack (input : ByteArray) :
    (padLengthReady input).stack =
      [UInt256.ofNat input.size, Padding.paddedWord input, UInt256.ofNat 483] := by
  rfl
...
@[simp] private theorem padCopied_pcToNat (input : ByteArray) :
    (padCopied input).pc.toNat = 426 := by rfl

@[simp] private theorem padCopied_pc (input : ByteArray) :
    (padCopied input).pc = UInt256.ofNat 426 := by rfl
...
@[simp] private theorem padSentinel_pcToNat (input : ByteArray) :
    (padSentinel input).pc.toNat = 434 := by rfl

@[simp] private theorem padSentinel_pc (input : ByteArray) :
    (padSentinel input).pc = UInt256.ofNat 434 := by rfl

@[simp] private theorem padSentinel_pcSucc (input : ByteArray) :
    (padSentinel input).pc.succ = UInt256.ofNat 435 := by
...
@[simp] private theorem padSentinelAddressReady_pc (input : ByteArray) :
    (padSentinelAddressReady input).pc = UInt256.ofNat 433 := by rfl
...
def lengthLoopState (input : ByteArray) (i : Nat) : State :=
  { padSentinel input with
    pc := UInt256.ofNat (Artifact.instructionPC 251)
    stack := [lengthAddr input i, lengthShift input i,
      Padding.paddedWord input, UInt256.ofNat 483]
...
def lengthIterationPath : List
  ...
  [⟨251, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ...
   ⟨254, .op .MSTORE8, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨255, .push ⟨1, by decide⟩ (UInt256.ofNat 1), by rfl, by decide⟩,
   ⟨256, .op .ADD, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨257, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨258, .push ⟨1, by decide⟩ (UInt256.ofNat 8), by rfl, by decide⟩,
   ⟨259, .op .SHR, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨260, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨261, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨262, .push ⟨2, by decide⟩ (UInt256.ofNat 451), by rfl, by decide⟩,
   ⟨263, .op .JUMPI, by rfl, wfOp (by decide) trivial rfl⟩]
...
@[simp] private theorem lengthLoopState_pc (input : ByteArray) (i : Nat) :
    (lengthLoopState input i).pc = UInt256.ofNat 451 := by rfl
...
@[simp] private theorem lengthLoopState_stack (input : ByteArray) (i : Nat) :
    (lengthLoopState input i).stack =
      [lengthAddr input i, lengthShift input i,
        Padding.paddedWord input, UInt256.ofNat 483] := by rfl
...
@[simp] private theorem lengthSteppedState_pc (input : ByteArray) (i : Nat) :
    (lengthSteppedState input i).pc = UInt256.ofNat 463 := by rfl
...
@[simp] private theorem lengthBranchReady_pc (input : ByteArray) (i : Nat) :
    (lengthBranchReady input i).pc = UInt256.ofNat 467 := by rfl
...
@[simp] private theorem validLengthLoopHead :
    Decode.isValidJumpDest submissionBytecode 451 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 251 = 451 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
...
def lengthExitPath : List
    (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [⟨264, .op .POP, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨265, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨266, .op (.Swap ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨267, .op .JUMP, by rfl, wfOp (by decide) trivial rfl⟩]

@[simp] private theorem validPadReturn :
    Decode.isValidJumpDest submissionBytecode 483 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 276 = 483 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
  exact Artifact.submissionArtifact.isValidJumpDest_index 276 (by rfl)
...
def lengthExitEntered (input : ByteArray) (i : Nat) : State :=
  { lengthLoopState input i with pc := UInt256.ofNat 468 }
...
def lengthExitSwapped (input : ByteArray) (i : Nat) : State :=
  { lengthExitEntered input i with
    ...
    stack := [UInt256.ofNat 483, lengthShift input i, Padding.paddedWord input] }
...
@[simp] private theorem lengthExitEntered_pc (input : ByteArray) (i : Nat) :
    (lengthExitEntered input i).pc = UInt256.ofNat 468 := by rfl
...
@[simp] private theorem lengthExitSwapped_pc (input : ByteArray) (i : Nat) :
    (lengthExitSwapped input i).pc = UInt256.ofNat 471 := by rfl
...
private theorem jump_step
    ...
    (hl : l.instruction = YulEvmCompiler.Instr.op .JUMP) (hi : l.index = 267)
    (hcode : s.executionEnv.code = submissionBytecode)
    (hstack : s.stack = t :: rest)
    (hpc : s.pc.toNat = Artifact.submissionArtifact.instructionPC 267)
...
private theorem run_lengthExitJump (input : ByteArray) (i : Nat) :
  ...
  have hstep := jump_step (lengthExitPath[3]'(by simp [lengthExitPath]))
    (lengthExitSwapped input i) (UInt256.ofNat 483)
