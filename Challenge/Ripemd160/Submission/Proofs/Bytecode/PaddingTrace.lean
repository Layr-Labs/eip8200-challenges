...
def pushedReturn (input : ByteArray) : State :=
  { Main.initializedState input with
    ...
    stack := UInt256.ofNat 396 :: (Main.initializedState input).stack }
...
def padEntry (input : ByteArray) : State :=
  { pushedPad input with
    pc := UInt256.ofNat 324
    stack := [UInt256.ofNat 396] }
...
@[simp] private theorem initializedPC764 :
    Artifact.instructionPC 200 = 321 := by change Artifact.submissionArtifact.instructionPC 200 = 321; rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
...
@[simp] private theorem padLengthReady_stack (input : ByteArray) :
    (padLengthReady input).stack =
      [UInt256.ofNat input.size, Padding.paddedWord input, UInt256.ofNat 396] := by
  rfl
...
def lengthLoopState (input : ByteArray) (i : Nat) : State :=
  { padSentinel input with
    ...
    stack := [lengthAddr input i, lengthShift input i,
      Padding.paddedWord input, UInt256.ofNat 396]
    memory := lengthLoopMemory input i
    activeWords := topByteActiveWords input }

def lengthIterationPath : List
    (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [⟨229, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨230, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨231, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨232, .op .MSTORE8, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨233, .push ⟨1, by decide⟩ (UInt256.ofNat 1), by rfl, by decide⟩,
   ⟨234, .op .ADD, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨235, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨236, .push ⟨1, by decide⟩ (UInt256.ofNat 8), by rfl, by decide⟩,
   ⟨237, .op .SHR, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨238, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨239, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨240, .push ⟨2, by decide⟩ (UInt256.ofNat 364), by rfl, by decide⟩,
   ⟨241, .op .JUMPI, by rfl, wfOp (by decide) trivial rfl⟩]
...
@[simp] private theorem lengthLoopState_pc (input : ByteArray) (i : Nat) :
    (lengthLoopState input i).pc = UInt256.ofNat 364 := by rfl
...
@[simp] private theorem lengthLoopState_stack (input : ByteArray) (i : Nat) :
    (lengthLoopState input i).stack =
      [lengthAddr input i, lengthShift input i,
        Padding.paddedWord input, UInt256.ofNat 396] := by rfl
...
@[simp] private theorem validLengthLoopHead :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 229 = 364 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
  exact Artifact.submissionArtifact.isValidJumpDest_index 229 (by rfl)
...
def lengthExitPath : List
  ...
  [⟨242, .op .POP, by rfl, wfOp (by decide) trivial rfl⟩,
   ...
   ⟨244, .op (.Swap ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨245, .op .JUMP, by rfl, wfOp (by decide) trivial rfl⟩]

@[simp] private theorem validPadReturn :
    Decode.isValidJumpDest submissionBytecode 396 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 254 = 396 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
...
def lengthExitEntered (input : ByteArray) (i : Nat) : State :=
  { lengthLoopState input i with pc := UInt256.ofNat 381 }
...
def lengthExitSwapped (input : ByteArray) (i : Nat) : State :=
  { lengthExitEntered input i with
    ...
    stack := [UInt256.ofNat 396, lengthShift input i, Padding.paddedWord input] }
...
private theorem run_lengthExitJump (input : ByteArray) (i : Nat) :
  ...
  have hstep := jump_step (lengthExitPath[3]'(by simp [lengthExitPath]))
    (lengthExitSwapped input i) (UInt256.ofNat 396)
