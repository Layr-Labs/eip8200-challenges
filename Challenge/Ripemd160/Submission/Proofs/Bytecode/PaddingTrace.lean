...
@[simp] private theorem initializedPC764 :
    Artifact.instructionPC 219 = 0x18d := by change Artifact.submissionArtifact.instructionPC 219 = 0x18d; rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
...
def lengthIterationPath : List
  ...
  [⟨248, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ...
   ⟨259, .push ⟨2, by decide⟩ (UInt256.ofNat 440), by rfl, by decide⟩,
...
@[simp] private theorem validLengthLoopHead :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 248 = 0x1b8 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
...
def lengthExitPath : List
  ...
  [⟨261, .op .POP, by rfl, wfOp (by decide) trivial rfl⟩,
   ...
   ⟨263, .op (.Swap ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
...
@[simp] private theorem validPadReturn :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 273 = 0x1d8 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
  exact Artifact.submissionArtifact.isValidJumpDest_index 273 (by rfl)
...
private theorem jump_step
    ...
    (hl : l.instruction = YulEvmCompiler.Instr.op .JUMP) (hi : l.index = 264)
    (hcode : s.executionEnv.code = submissionBytecode)
    (hstack : s.stack = t :: rest)
    (hpc : s.pc.toNat = Artifact.submissionArtifact.instructionPC 264)
