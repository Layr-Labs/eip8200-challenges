...
def lengthIterationPath : List
  ...
  [⟨222, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ...
   ⟨233, .push ⟨2, by decide⟩ (UInt256.ofNat 354), by rfl, by decide⟩,
...
@[simp] private theorem validLengthLoopHead :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 222 = 354 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
...
@[simp] private theorem pcExitPop :
    Artifact.submissionArtifact.instructionPC 235 = 371 := by
