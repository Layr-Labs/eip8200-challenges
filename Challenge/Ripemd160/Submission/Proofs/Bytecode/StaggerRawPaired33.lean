...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2006).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2006 actual_slice
    (by change 2006 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2685 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2006) = UInt256.ofNat 2685
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2685, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2726, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2685) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2685) template = UInt256.ofNat 2726 := by decide
  rw [hend] at hraw
