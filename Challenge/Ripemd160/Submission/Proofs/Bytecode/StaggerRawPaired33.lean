...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2000 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2679 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2000) = UInt256.ofNat 2679
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2679, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2720, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2679) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2679) template = UInt256.ofNat 2720 := by decide
  rw [hend] at hraw
