...
theorem site_pc : site.startPC = UInt256.ofNat 2553 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1960) = UInt256.ofNat 2553
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2553, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2605, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2553) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2553) template = UInt256.ofNat 2605 := by decide
  rw [hend] at hraw
