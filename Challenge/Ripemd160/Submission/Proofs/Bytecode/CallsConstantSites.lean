...
private theorem left2_slice :
    (A.instructions.drop 944).take left2Code.length = left2Code := by
  rfl
...
def left2 : GenericRoundSite A .Osaka left2Code :=
  StackSiteBuilder.ofSlice _ 944 left2_slice (by
    change 944 + left2Code.length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound left2_wellFormed (by decide)

theorem left2_start : left2.startPC = UInt256.ofNat 1414 := by
  rfl

theorem left2_end : left2.endPC = UInt256.ofNat 1915 := by
  rfl

private theorem left4_slice :
    (A.instructions.drop 1566).take left4Code.length = left4Code := by
  rfl
...
def left4 : GenericRoundSite A .Osaka left4Code :=
  StackSiteBuilder.ofSlice _ 1566 left4_slice (by
    change 1566 + left4Code.length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound left4_wellFormed (by decide)

theorem left4_start : left4.startPC = UInt256.ofNat 2234 := by
  rfl

theorem left4_end : left4.endPC = UInt256.ofNat 2735 := by
  rfl

private theorem right0_slice :
    (A.instructions.drop 2026).take right0Code.length = right0Code := by
  rfl
...
def right0 : GenericRoundSite A .Osaka right0Code :=
  StackSiteBuilder.ofSlice _ 2026 right0_slice (by
    change 2026 + right0Code.length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound right0_wellFormed (by decide)

theorem right0_start : right0.startPC = UInt256.ofNat 2751 := by
  rfl

theorem right0_end : right0.endPC = UInt256.ofNat 3252 := by
  rfl

private theorem right1_slice :
    (A.instructions.drop 2475).take right1Code.length = right1Code := by
  rfl
...
def right1 : GenericRoundSite A .Osaka right1Code :=
  StackSiteBuilder.ofSlice _ 2475 right1_slice (by
    change 2475 + right1Code.length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound right1_wellFormed (by decide)

theorem right1_start : right1.startPC = UInt256.ofNat 3252 := by
  rfl

theorem right1_end : right1.endPC = UInt256.ofNat 3769 := by
  rfl

private theorem right2_slice :
    (A.instructions.drop 2940).take right2Code.length = right2Code := by
  rfl
...
def right2 : GenericRoundSite A .Osaka right2Code :=
  StackSiteBuilder.ofSlice _ 2940 right2_slice (by
    change 2940 + right2Code.length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound right2_wellFormed (by decide)

theorem right2_start : right2.startPC = UInt256.ofNat 3769 := by
  rfl

theorem right2_end : right2.endPC = UInt256.ofNat 4270 := by
  rfl
