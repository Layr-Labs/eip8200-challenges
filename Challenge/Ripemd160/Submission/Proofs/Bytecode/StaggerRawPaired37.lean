...
theorem site_pc : site.startPC = UInt256.ofNat 2760 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2088) = UInt256.ofNat 2760
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2760, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2798, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2760) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2760) template = UInt256.ofNat 2798 := by decide
  rw [hend] at hraw
