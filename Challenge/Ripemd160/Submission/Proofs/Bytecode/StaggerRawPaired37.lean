...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2129).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2129 actual_slice
    (by change 2129 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2843 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2129) = UInt256.ofNat 2843
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 2843) template = UInt256.ofNat 2881 := by decide
  rw [hend] at hraw
