...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2175).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2175 actual_slice
    (by change 2175 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2901 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2175) = UInt256.ofNat 2901
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 2901) template = UInt256.ofNat 2938 := by decide
  rw [hend] at hraw
