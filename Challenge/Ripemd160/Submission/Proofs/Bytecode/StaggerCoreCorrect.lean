...
theorem paired_crypto (memory : ByteArray) (words : Nat → UInt32) (q : CryptoLane)
    (hm : StaggerMessage.Ready memory words) :
    unpackLeft (paired memory (embed q)) = leftFold words 77 q ∧
      unpackRight (paired memory (embed q)) = rightFold words 80 q := by
  unfold paired
  rw [StaggerLastStep.project_left, StaggerLastStep.project_right]
  change unpackLeft (StaggerAlgorithm.fold (message memory) 77 (pair (embed q) (prologue memory (embed q)))) = _ ∧
    unpackRight (StaggerAlgorithm.fold (message memory) 77 (pair (embed q) (prologue memory (embed q)))) = _
  rw [prologue_crypto memory words q hm, pair_embed,
    StaggerAlgorithm.fold_crypto (message memory) words 77 (by decide) q q hm.paired,
    unpackLeft_packCrypto, unpackRight_packCrypto]
  exact ⟨rfl, rfl⟩
