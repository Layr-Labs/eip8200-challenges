...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1767).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1767 actual_slice
    (by change 1767 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2265 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1767) = UInt256.ofNat 2265
