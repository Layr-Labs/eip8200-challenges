...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1664).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1664 actual_slice
    (by change 1664 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2099 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1664) = UInt256.ofNat 2099
