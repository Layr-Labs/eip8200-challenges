...
noncomputable def gasSteps_start (input : ByteArray) (hfit : CalldataFits input)
  ...
  · have gx : GasSteps (StackTail.append s rho)
      ...
      have hs : StackTail.append s rho = {s with pc := UInt256.ofNat 485,
          stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0)
            (UInt256.ofNat input.size) (maskRho rho)} := by
        dsimp [s]
  ...
  · have gx : GasSteps (StackTail.append s rho)
        {s with pc := UInt256.ofNat 486,
          stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0)
            (LoopCompletionControl.limit input) (maskRho rho)} := by
      have hs : StackTail.append s rho = {s with pc := UInt256.ofNat 486,
          stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0)
