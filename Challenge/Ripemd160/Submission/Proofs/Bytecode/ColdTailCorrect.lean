...
noncomputable def gasSteps_start (input : ByteArray) (hfit : CalldataFits input)
  ...
  have gx : GasSteps (StackTail.append s rho)
      {s with pc := UInt256.ofNat 453, stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0) (LoopCompletionControl.limit input) (maskRho rho)} := by
    by_cases hz : input.size % 64 = 0
    · have hs : StackTail.append s rho = {s with pc := UInt256.ofNat 453, stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0) (UInt256.ofNat input.size) (maskRho rho)} := by
        dsimp [s]
    ...
    · have hs : StackTail.append s rho = {s with pc := UInt256.ofNat 453, stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0) (Padding.paddedWord input) (maskRho rho)} := by
        dsimp [s]
