...
theorem site_pc : site.startPC = UInt256.ofNat 4081 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3351) = UInt256.ofNat 4081
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4081, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4117, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4081) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4081) template = UInt256.ofNat 4117 := by decide
  rw [hend] at hraw
