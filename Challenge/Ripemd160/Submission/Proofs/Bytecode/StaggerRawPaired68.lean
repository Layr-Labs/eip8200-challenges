...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3302).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3302 actual_slice
    (by change 3302 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4242 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3302) = UInt256.ofNat 4242
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 4242) template = UInt256.ofNat 4281 := by decide
  rw [hend] at hraw
