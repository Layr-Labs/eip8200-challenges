...
theorem site_pc : site.startPC = UInt256.ofNat 546 := by
  ...
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 546) template = UInt256.ofNat 882 := by decide
...
abbrev template : List Instr := [ .op .CALLDATASIZE,
    ...
    .op .JUMPI ]
theorem actual_slice : (Artifact.submissionArtifact.instructions.drop 3600).take template.length = template := by rfl
def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3600 actual_slice
    ...
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide)) (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4784 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3600) = UInt256.ofNat 4784
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 4784) template = UInt256.ofNat 4792 := by decide
def lift (s t : State) (e : Env s) (stack : List UInt256)
    (h : runInstrSeq template (atState s 4784 stack) = some t) :
    GasSteps (atState s 4784 stack) t := by
  apply PadLift.gasSteps_of_raw site (atState s 4784 stack) t e.code e.fork e.run e.np site_pc.symm
  · apply PadLift.advancesAll_sound; decide
  · exact h
end guard
