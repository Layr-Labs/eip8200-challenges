...
theorem site_pc : site.startPC = UInt256.ofNat 4756 := by
  ...
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 4756) template = UInt256.ofNat 4764 := by decide
def lift (s t : State) (e : Env s) (stack : List UInt256)
    (h : runInstrSeq template (atState s 4756 stack) = some t) :
    GasSteps (atState s 4756 stack) t := by
  apply PadLift.gasSteps_of_raw site (atState s 4756 stack) t e.code e.fork e.run e.np site_pc.symm
  · apply PadLift.advancesAll_sound; decide
  · exact h
end guard
