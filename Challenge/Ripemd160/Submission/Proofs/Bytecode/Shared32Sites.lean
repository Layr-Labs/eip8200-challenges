...
def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 282 actual_slice
    ...
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide)) (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 539 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 282) = UInt256.ofNat 539
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 539) template = UInt256.ofNat 540 := by decide
def lift (s t : State) (e : Env s) (stack : List UInt256)
    (h : runInstrSeq template (atState s 539 stack) = some t) :
    GasSteps (atState s 539 stack) t := by
  apply PadLift.gasSteps_of_raw site (atState s 539 stack) t e.code e.fork e.run e.np site_pc.symm
  · apply PadLift.advancesAll_sound; decide
  · exact h
end lower
...
namespace jump
abbrev template : List Instr := PadJump.template 539
...
theorem site_pc : site.startPC = UInt256.ofNat 540 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 283) = UInt256.ofNat 540
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 540) template = UInt256.ofNat 877 := by decide
...
def lift (s t : State) (e : Env s) (stack : List UInt256)
    (h : runInstrSeq template (atState s 540 stack) = some t) :
    GasSteps (atState s 540 stack) t := by
  let u := atState s 540 stack
  apply DataStepper.runLocatedBlock_sound Artifact.submissionArtifact .Osaka site.path (s := u) (t := t) e.code e.fork
  · have hl := runLocatedBlock_eq_raw_terminal_sites site.sites template
      site.instruction_eq site.contiguous u
      (by rw [site.head_eq]; exact congrArg some site_pc) (by
...
def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3604 actual_slice
    ...
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide)) (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4779 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3604) = UInt256.ofNat 4779
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem end_pc : pcAfter (UInt256.ofNat 4779) template = UInt256.ofNat 4787 := by decide
def lift (s t : State) (e : Env s) (stack : List UInt256)
    (h : runInstrSeq template (atState s 4779 stack) = some t) :
    GasSteps (atState s 4779 stack) t := by
  apply PadLift.gasSteps_of_raw site (atState s 4779 stack) t e.code e.fork e.run e.np site_pc.symm
  · apply PadLift.advancesAll_sound; decide
  · exact h
end guard
...
theorem valid_lower (s : State) (e : Env s) :
    Decode.isValidJumpDest s.executionEnv.code 539 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 282 = 539 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
