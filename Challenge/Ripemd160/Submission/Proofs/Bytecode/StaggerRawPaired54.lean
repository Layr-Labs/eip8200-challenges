...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2811 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3620 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2811) = UInt256.ofNat 3620
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3620, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3660, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3620) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3620) template = UInt256.ofNat 3660 := by decide
  rw [hend] at hraw
