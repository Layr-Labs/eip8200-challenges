...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2819 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3510 := by
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3510, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3550, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3510) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3510) template = UInt256.ofNat 3550 := by decide
  rw [hend] at hraw
