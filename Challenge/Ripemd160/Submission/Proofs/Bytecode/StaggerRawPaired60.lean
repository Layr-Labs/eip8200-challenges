...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3043 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3890 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3043) = UInt256.ofNat 3890
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3890, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3930, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3890) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3890) template = UInt256.ofNat 3930 := by decide
  rw [hend] at hraw
