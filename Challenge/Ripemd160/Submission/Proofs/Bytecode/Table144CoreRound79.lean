import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144RawRound79
...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound79
...
def eval (message k : UInt256) (q : WordLane) : WordLane := rawFinish 6 11 message k q
def nextKey (k : UInt256) : UInt256 := k
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table144RawRound79.outputStack memory (input q k) rho =
      stack [.d, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 180) k q) (nextKey k) rho := by
  simp only [Table144RawRound79.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 4581, stack := stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 4620, stack :=
          stack [.d, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 180) k q) (nextKey k) rho} := by
   have gs := Table144RawRound79.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound79
