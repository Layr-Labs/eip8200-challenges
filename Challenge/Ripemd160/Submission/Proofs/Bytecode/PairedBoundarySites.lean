...
theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 3992 = 5108 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4061 = 5192 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def tailPrefixSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedTailTrace.prefixTemplate :=
  StackSiteBuilder.ofSlice PairedTailTrace.prefixTemplate 3992 tailPrefix_slice
    (by
      change 3992 + PairedTailTrace.prefixTemplate.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 5108 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3992) =
    UInt256.ofNat 5108
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 5192 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4061) =
    UInt256.ofNat 5192
...
def tailJump : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4061)
  pc_eq := instructionPC_toNat 4061

theorem tailJump_pc : tailJump.pc = UInt256.ofNat 5192 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4061) =
    UInt256.ofNat 5192
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 5108, stack := PairedTailTrace.entryStack q ret rho}
