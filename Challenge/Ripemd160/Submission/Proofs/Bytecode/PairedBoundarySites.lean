...
theorem tailPrefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3989).take
      PairedTailTrace.prefixTemplate.length = PairedTailTrace.prefixTemplate := by
  rfl

theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 3989 = 5105 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4058 = 5190 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 5105 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3989) =
    UInt256.ofNat 5105
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 5189 := by
...
theorem tailJump_pc : tailJump.pc = UInt256.ofNat 5189 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4058) =
    UInt256.ofNat 5189
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 5105, stack := PairedTailTrace.entryStack q ret rho}
