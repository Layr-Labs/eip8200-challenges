...
/-- Exact schedule, startup, and tail windows of the frozen 5225-byte artifact. -/
...
theorem schedule_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 421 = 852 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem scheduleSite_endPC : scheduleSite.endPC = UInt256.ofNat 852 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 421) =
    UInt256.ofNat 852
  rw [schedule_endInstructionPC]

theorem startup_slice :
    (Artifact.submissionArtifact.instructions.drop 421).take
...
def startupSite :
  ...
  StackSiteBuilder.ofSlice PairedStartupTrace.template 421 startup_slice
    (by
      change 421 + PairedStartupTrace.template.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem startupSite_startPC : startupSite.startPC = UInt256.ofNat 852 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 421) =
    UInt256.ofNat 852
  rw [startup_instructionPC]

theorem startupSite_endPC : startupSite.endPC = UInt256.ofNat 955 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 465) =
    UInt256.ofNat 955
  rw [startup_endInstructionPC]

theorem tailPrefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3801).take
      PairedTailTrace.prefixTemplate.length = PairedTailTrace.prefixTemplate := by
  rfl

theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 3801 = 4921 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 3870 = 5005 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def tailPrefixSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedTailTrace.prefixTemplate :=
  StackSiteBuilder.ofSlice PairedTailTrace.prefixTemplate 3801 tailPrefix_slice
    (by
      change 3801 + PairedTailTrace.prefixTemplate.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 4921 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3801) =
    UInt256.ofNat 4921
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 5005 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3870) =
    UInt256.ofNat 5005
...
def tailJump : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3870)
  pc_eq := instructionPC_toNat 3873

theorem tailJump_pc : tailJump.pc = UInt256.ofNat 5005 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3870) =
    UInt256.ofNat 5005
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_startup (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1002)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 852, stack := rho}
      {s with pc := UInt256.ofNat 955, stack := PairedStartupTrace.resultStack s.memory rho} := by
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4921, stack := PairedTailTrace.entryStack q ret rho}
