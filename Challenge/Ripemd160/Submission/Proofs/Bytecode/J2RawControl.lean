...
theorem run_first (s : State) (pc : UInt256) (f : Frame) (rho : List UInt256)
    ...
    (hvalid : Decode.isValidJumpDest s.executionEnv.code 4920 = true) :
    runInstrSeq firstTemplate {s with pc := pc, stack := frame f rho} =
      some {s with pc := if 219 < f.full.toNat then UInt256.ofNat 4920 else pcAfter pc firstTemplate,
...
theorem run_normalGuard (s : State) (pc : UInt256) (f : Frame) (rho : List UInt256)
    (hstack : rho.length ≤ 990) (hrun : s.halt = .Running)
    (hvalid : Decode.isValidJumpDest s.executionEnv.code 4893 = true) :
    runInstrSeq normalGuardTemplate {s with pc := pc, stack := frame f rho} =
      some {s with pc := if f.off.toNat < f.full.toNat then UInt256.ofNat 4893 else pcAfter pc normalGuardTemplate,
                   stack := frame f rho} := by
  have hbase : rho.length < 1024 := by omega
  have hcap (n : Nat) (hn : n ≤ 30) : rho.length + n < 1024 := by omega
...
theorem sub_toNat_zero (a b : UInt256) : (UInt256.sub a b).toNat = 0 ↔ a.toNat = b.toNat := by
  unfold UInt256.sub UInt256.toNat
  constructor
  · intro h
    have h1 : a.val - b.val = 0 := Fin.ext h
    exact congrArg Fin.val (sub_eq_zero.mp h1)
  · intro h
    have h1 : a.val = b.val := Fin.ext h
    rw [h1, sub_self]; rfl

theorem hsub_toNat_zero (a b : UInt256) : (a - b).toNat = 0 ↔ a.toNat = b.toNat := sub_toNat_zero a b

theorem run_finish (s : State) (pc : UInt256) (f : Frame) (rho : List UInt256)
    (hstack : rho.length ≤ 990) (hrun : s.halt = .Running)
    (hlen : f.len = UInt256.ofNat s.executionEnv.calldata.size)
    (hvalid : Decode.isValidJumpDest s.executionEnv.code 4633 = true) :
    runInstrSeq finishTemplate {s with pc := pc, stack := frame f rho} =
      some {s with pc := if f.stop.toNat = f.len.toNat then pcAfter pc finishTemplate else UInt256.ofNat 4633,
                   stack := frame f rho} := by
  have hbase : rho.length < 1024 := by omega
  have hcap (n : Nat) (hn : n ≤ 30) : rho.length + n < 1024 := by omega
  ...
  all_goals simp (discharger := omega) [finishTemplate, frame, runInstrSeq, DataStepper.runInstr,
    pcAfter, UInt256.succ, Instr.size, List.exchange, List.getElem?_cons_zero,
    Nat.add_assoc, hrun, hbase, hcap, ← hlen, UInt256.isTrue, sub_toNat_zero, hsub_toNat_zero,
...
theorem run_transitionGuard (s : State) (pc : UInt256) (f : Frame) (rho : List UInt256)
    ...
    runInstrSeq transitionGuardTemplate {s with pc := pc, stack := frame f rho} =
      some {s with pc := if f.off.toNat < f.full.toNat then UInt256.ofNat 4893 else pcAfter pc transitionGuardTemplate,
                   stack := frame f rho} := by
  have hbase : rho.length < 1024 := by omega
  have hcap (n : Nat) (hn : n ≤ 30) : rho.length + n < 1024 := by omega
  by_cases hc : f.off.toNat < f.full.toNat
...
theorem run_result (s : State) (pc : UInt256) (f : Frame) (rho : List UInt256)
    (hstack : rho.length ≤ 990) (hrun : s.halt = .Running)
    (hvalid : Decode.isValidJumpDest s.executionEnv.code 246 = true) :
    runInstrSeq resultTemplate {s with pc := pc, stack := frame f rho} =
      some {s with pc := if f.acc.toNat = 0 then pcAfter pc resultTemplate else UInt256.ofNat 246,
...
theorem run_toTail (s : State) (pc : UInt256) (stack : List UInt256)
    ...
    runInstrSeq toTailTemplate {s with pc := pc, stack := stack} =
      some {s with pc := UInt256.ofNat 4920, stack := stack} := by
