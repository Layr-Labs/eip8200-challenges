...
noncomputable def gasSteps_start (input : ByteArray) (hfit : CalldataFits input) (hpositive : 0 < input.size)
    (entryPrefix : GasSteps (initialState submissionBytecode input 0) (Execution.atPC input 349)) :
  ...
  have gx : GasSteps s {s with pc := UInt256.ofNat 481, stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0) (LoopCompletionControl.limit input) []} := by
    by_cases hz : input.size % 64 = 0
    · have hs : s = {s with pc := UInt256.ofNat 372, stack := [Padding.paddedWord input]} := by
    ...
    · have hs : s = {s with pc := UInt256.ofNat 374, stack := [Padding.paddedWord input]} := by
