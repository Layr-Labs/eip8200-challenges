...
noncomputable def gasSteps_start (input : ByteArray) (hfit : CalldataFits input) (hpositive : 0 < input.size) (hn32 : input.size ≠ 32)
    (entryPrefix : GasSteps (initialState submissionBytecode input 0) (Execution.atPC input 338)) :
  ...
  have gx : GasSteps s {s with pc := UInt256.ofNat 498, stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0) (LoopCompletionControl.limit input) maskRho} := by
    by_cases hz : input.size % 64 = 0
    · have hs : s = {s with pc := UInt256.ofNat 498, stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0) (UInt256.ofNat input.size) maskRho} := by
    ...
    · have hs : s = {s with pc := UInt256.ofNat 498, stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0) (Padding.paddedWord input) maskRho} := by
