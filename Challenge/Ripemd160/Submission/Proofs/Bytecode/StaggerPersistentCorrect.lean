...
noncomputable def gasSteps_start (input : ByteArray) (hfit : CalldataFits input) (hpositive : 0 < input.size)
    (entryPrefix : GasSteps (initialState submissionBytecode input 0) (Execution.atPC input 357)) :
  ...
  have gx : GasSteps s {s with pc := UInt256.ofNat 490, stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0) (LoopCompletionControl.limit input) maskRho} := by
    ...
    · have hs : s = {s with pc := UInt256.ofNat 393, stack := Padding.paddedWord input :: maskRho} := by
    ...
    · have hs : s = {s with pc := UInt256.ofNat 395, stack := Padding.paddedWord input :: maskRho} := by
