...
noncomputable def gasSteps_start (input : ByteArray) (hfit : CalldataFits input) (hpositive : 0 < input.size)
  ...
  have gx : GasSteps s {s with pc := UInt256.ofNat 490, stack := StaggerPersistentFrame.frame h (UInt256.ofNat 0) (LoopCompletionControl.limit input) []} := by
    by_cases hz : input.size % 64 = 0
    · have hs : s = {s with pc := UInt256.ofNat 381, stack := [Padding.paddedWord input]} := by
    ...
    · have hs : s = {s with pc := UInt256.ofNat 383, stack := [Padding.paddedWord input]} := by
