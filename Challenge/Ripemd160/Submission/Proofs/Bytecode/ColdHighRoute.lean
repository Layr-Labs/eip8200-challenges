...
theorem slice : (Artifact.submissionArtifact.instructions.drop 3719).take template.length=template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3719 slice
...
theorem pc : site.startPC=UInt256.ofNat 4831 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3719)=UInt256.ofNat 4831
  rw [ArtifactByteLength.instructionPC_eq_byteLength];decide

theorem valid (s : State) (hcode : s.executionEnv.code=Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code 4720=true := by
  have hp : Artifact.submissionArtifact.instructionPC 3657=4720 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength];decide
  have h:=Artifact.submissionArtifact.isValidJumpDest_index 3657 (by rfl)
  rw [hp] at h
  rw [hcode]
  exact h
...
def gasSteps (s : State) (h : Compression.HashState) (off lim : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc:=UInt256.ofNat 4831, stack:=frame h off lim rho}
      {s with pc:=UInt256.ofNat 4720, stack:=frame h off (lim+UInt256.ofNat 64) rho} := by
  apply PadLift.gasSteps_of_raw site {s with pc:=UInt256.ofNat 4831, stack:=frame h off lim rho} _ hc hf hr hnp pc.symm
