...
theorem slice : (Artifact.submissionArtifact.instructions.drop 3704).take template.length=template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3704 slice
...
theorem pc : site.startPC=UInt256.ofNat 4823 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3704)=UInt256.ofNat 4823
...
theorem valid (s : State) (hcode : s.executionEnv.code=Artifact.submissionArtifact.code) :
  ...
  have hp : Artifact.submissionArtifact.instructionPC 3636=4705 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength];decide
