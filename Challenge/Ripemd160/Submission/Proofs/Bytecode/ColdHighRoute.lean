...
theorem slice : (Artifact.submissionArtifact.instructions.drop 3662).take template.length=template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3662 slice
...
theorem pc : site.startPC=UInt256.ofNat 4813 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3662) = UInt256.ofNat 4841
...
theorem valid (s : State) (hcode : s.executionEnv.code=Artifact.submissionArtifact.code) :
  ...
  have hp : Artifact.submissionArtifact.instructionPC 3594 = 4723 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength];decide
...
def gasSteps (s : State) (h : Compression.HashState) (off lim : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc:=UInt256.ofNat 4813, stack:=frame h off lim rho}
      {s with pc:=UInt256.ofNat 4723, stack:=frame h off lim rho} := by
