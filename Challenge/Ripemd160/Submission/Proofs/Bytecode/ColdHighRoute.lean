...
theorem slice : (Artifact.submissionArtifact.instructions.drop 3683).take template.length=template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3683 slice
...
theorem pc : site.startPC=UInt256.ofNat 4822 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3683)=UInt256.ofNat 4822
...
theorem valid (s : State) (hcode : s.executionEnv.code=Artifact.submissionArtifact.code) :
  ...
  have hp : Artifact.submissionArtifact.instructionPC 3615=4704 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength];decide
  have h:=Artifact.submissionArtifact.isValidJumpDest_index 3615 (by rfl)
  rw [hp] at h
  rw [hcode]
  exact h
...
def gasSteps (s : State) (h : Compression.HashState) (off lim : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc:=UInt256.ofNat 4822, stack:=frame h off lim rho}
      {s with pc:=UInt256.ofNat 4704, stack:=frame h off lim rho} := by
  apply PadLift.gasSteps_of_raw site {s with pc:=UInt256.ofNat 4822, stack:=frame h off lim rho} _ hc hf hr hnp pc.symm
