...
def gasSteps_select (input : ByteArray) (rest : List UInt256) (hlen : rest.length < 1020) :
  ...
  have t1 := soundS (opAt 166 .CALLDATASIZE)
    (blockOfS _ (pcFactS input 166 261 (UInt256.ofNat input.size :: rest) (by norm_num) (by rfl))
      (stepS_calldatasize input 261 (UInt256.ofNat input.size :: rest) (by simp only [List.length_cons]; omega) (by norm_num)))
...
def gasSteps_skip128 (input : ByteArray) (rest : List UInt256)
  ...
  have t3 := soundS (pushAt 163 2 5206)
    (blockOfS _ (pcFactS input 163 256 (UInt256.gt 129 (UInt256.ofNat input.size) :: rest) (by norm_num) (by rfl))
      (stepS_push input 256 2 5206 (UInt256.gt 129 (UInt256.ofNat input.size) :: rest)
        (by simp only [List.length_cons]; omega) (by decide) (by decide) (by norm_num)))
  have t4 := soundS (opAt 164 .JUMPI)
    (blockOfS _ (pcFactS input 164 259 (5206 :: UInt256.gt 129 (UInt256.ofNat input.size) :: rest) (by norm_num) (by rfl))
      (stepS_jumpi_fall input 259 5206 (UInt256.gt 129 (UInt256.ofNat input.size)) rest
