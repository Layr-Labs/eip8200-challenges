...
def gasSteps_skip128 (input : ByteArray) (rest : List UInt256)
  ...
  have t3 := soundS (pushAt 165 2 5072)
    (blockOfS _ (pcFactS input 165 259 (UInt256.gt 129 (UInt256.ofNat input.size) :: rest) (by norm_num) (by rfl))
      (stepS_push input 259 2 5072 (UInt256.gt 129 (UInt256.ofNat input.size) :: rest)
        (by simp only [List.length_cons]; omega) (by decide) (by decide) (by norm_num)))
  have t4 := soundS (opAt 166 .JUMPI)
    (blockOfS _ (pcFactS input 166 262 (5072 :: UInt256.gt 129 (UInt256.ofNat input.size) :: rest) (by norm_num) (by rfl))
