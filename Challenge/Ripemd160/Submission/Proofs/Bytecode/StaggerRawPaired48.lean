...
theorem site_pc : site.startPC = UInt256.ofNat 3184 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2469) = UInt256.ofNat 3184
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3184, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3231, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3184) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3184) template = UInt256.ofNat 3231 := by decide
  rw [hend] at hraw
