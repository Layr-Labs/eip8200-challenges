...
theorem site_pc : site.startPC = UInt256.ofNat 3337 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2571) = UInt256.ofNat 3337
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3337, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3401, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3337) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3337) template = UInt256.ofNat 3401 := by decide
  rw [hend] at hraw
