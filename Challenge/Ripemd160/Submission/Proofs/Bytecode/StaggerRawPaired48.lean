...
theorem site_pc : site.startPC = UInt256.ofNat 3286 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2521) = UInt256.ofNat 3286
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3286, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3351, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3286) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3286) template = UInt256.ofNat 3351 := by decide
  rw [hend] at hraw
