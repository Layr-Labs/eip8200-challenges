...
def dispatchTemplate : List Instr := PadDispatch.template 400
...
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 243 prefix_slice
    (by change 243 + prefixTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := prefixTemplate) (by decide))
    (by decide)
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 400 := by
...
theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 400).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 243 = 400 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 243 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 400 = true
  rw [hcode]
  exact h
...
def gasSteps_miss (s : State) (ptr ret off : UInt256) (rho : List UInt256)
  ...
  have h := PadDispatch.run_miss s (UInt256.ofNat 526) ptr ret off rho 400 hstack hrun hfit hmiss
  have hp : pcAfter (UInt256.ofNat 526) (PadDispatch.template 400) = UInt256.ofNat 533 := by rfl
  rw [hp] at h
  exact h
...
def gasSteps_hit (s : State) (ptr ret off : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 526, stack := ptr :: ret :: off :: rho}
      {s with pc := UInt256.ofNat 400, stack := ptr :: ret :: off :: rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 526, stack := ptr :: ret :: off :: rho} _ hcode hfork hrun hnp dispatch_pc.symm dispatch_advances
  have h := PadDispatch.run_hit s (UInt256.ofNat 526) ptr ret off rho 400 hstack hrun hfit hhit (valid_pad s hcode)
  exact h
...
def gasSteps_prefix (s : State) (ret : UInt256) (p : Nat) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 400, stack := UInt256.ofNat p :: ret :: rest}
      {s with pc := UInt256.ofNat 406, stack := ret :: rest, activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  apply PadLift.gasSteps_of_raw prefixSite {s with pc := UInt256.ofNat 400, stack := UInt256.ofNat p :: ret :: rest} _ hcode hfork hrun hnp prefix_pc.symm prefix_advances
  have h := PadPrefix.run_template s (UInt256.ofNat 400) ret p 32 rest hstack hrun hbound halign (by decide)
  have hp : pcAfter (UInt256.ofNat 400) (PadPrefix.template 32) = UInt256.ofNat 406 := by rfl
  rw [hp] at h
  exact h
