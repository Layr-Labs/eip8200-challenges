...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTableScratch
...
@[simp] theorem scratch_read_low (memory : ByteArray) (low high : UInt256) :
    MachineState.readWord (scratchMemory memory low high) 32 = low :=
  read_writeWord _ _ _

@[simp] theorem scratch_read_high (memory : ByteArray) (low high : UInt256) :
    MachineState.readWord (scratchMemory memory low high) 64 = high := by
...
def poolWord (memory : ByteArray) (i : Nat) : UInt256 :=
  Word.mask32 (MachineState.readWord memory (4 + 4 * i))
...
theorem poolWord_eq (memory : ByteArray) (low high : UInt256) (i : Nat) (hi : i < 16) :
  ...
  · rw [if_pos hlow, Nat.mod_eq_of_lt hlow]
    have h := mask32_window (scratchMemory memory low high) 32 i (by decide) hlow
    rw [scratch_read_low] at h
    have ha : 32 + 4 * i - 28 = 4 + 4 * i := by omega
    simpa only [poolWord, ha] using h
  · rw [if_neg hlow]
    have hmod : i % 8 = i - 8 := by omega
    rw [hmod]
    have h := mask32_window (scratchMemory memory low high) 64 (i - 8) (by decide) (by omega)
    rw [scratch_read_high] at h
    have ha : 64 + 4 * (i - 8) - 28 = 4 + 4 * i := by omega
    simpa only [poolWord, ha] using h
...
#print axioms mask32_window
#print axioms poolWord_eq_extracted
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTableScratch
