...
def dispatchCode : List Instr := dispatchTemplate 4699
theorem dispatch_slice :
    (Artifact.submissionArtifact.instructions.drop 3520).take dispatchCode.length = dispatchCode := by rfl
def dispatchSite : GenericRoundSite Artifact.submissionArtifact .Osaka dispatchCode :=
  StackSiteBuilder.ofSlice dispatchCode 3520 dispatch_slice
    (by change 3520 + dispatchCode.length ≤ Artifact.submissionInstructions.length
...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 4636 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3520) = UInt256.ofNat 4636
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 4699).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3568 = 4699 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3568 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 4699 = true
  rw [hcode]
  exact h
...
def gasSteps_hit (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4636, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 4699, stack := frame h off limit rho} := by
  ...
  · have hr := run_hit s (UInt256.ofNat 4636) off limit h rho 4699 hstack hrun hfit hhit (valid_pad s hcode)
    exact hr
...
def gasSteps_miss (s : State) (off limit : UInt256) (h : Compression.HashState)
  ...
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 4636, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_miss s (UInt256.ofNat 4636) off limit h rho 4699 hstack hrun hfit hmiss
    have he : pcAfter (UInt256.ofNat 4636) (dispatchTemplate 4699) = UInt256.ofNat 4643 := by decide
    rw [he] at hr
    exact hr
