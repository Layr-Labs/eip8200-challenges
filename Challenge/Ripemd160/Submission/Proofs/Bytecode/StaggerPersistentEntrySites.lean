...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 600 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 335) = UInt256.ofNat 600
...
theorem call_pc : callSite.startPC = UInt256.ofNat 607 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 340) = UInt256.ofNat 607
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 285 = 521 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
