...
theorem dispatch_slice :
    (Artifact.submissionArtifact.instructions.drop 282).take dispatchCode.length = dispatchCode := by rfl
def dispatchSite : GenericRoundSite Artifact.submissionArtifact .Osaka dispatchCode :=
  StackSiteBuilder.ofSlice dispatchCode 282 dispatch_slice
    (by change 282 + dispatchCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := dispatchCode) (by decide)) (by decide)
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 466 := by
...
theorem call_pc : callSite.startPC = UInt256.ofNat 473 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 287) = UInt256.ofNat 473
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_hit (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 466, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 386, stack := frame h off limit rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 466, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_hit s (UInt256.ofNat 466) off limit h rho 386 hstack hrun hfit hhit (valid_pad s hcode)
    exact hr
...
def gasSteps_miss (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 466, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 473, stack := frame h off limit rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 466, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_miss s (UInt256.ofNat 466) off limit h rho 386 hstack hrun hfit hmiss
    have he : pcAfter (UInt256.ofNat 466) (dispatchTemplate 386) = UInt256.ofNat 473 := by decide
    rw [he] at hr
    exact hr
...
def gasSteps_call (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 473, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 478, stack := pointer off :: frame h off limit rho} := by
  apply DenseScheduleLift.gasSteps_of_raw callSite {s with pc := UInt256.ofNat 473, stack := frame h off limit rho} _ hcode hfork hrun hnp call_pc.symm
  · exact Table80SiteCommon.coreAdvancesAll_sound _ (by decide)
  · have hr := run_call s (UInt256.ofNat 473) off limit h rho hstack hrun
    have he : pcAfter (UInt256.ofNat 473) callTemplate = UInt256.ofNat 478 := by decide
    rw [he] at hr
    exact hr
