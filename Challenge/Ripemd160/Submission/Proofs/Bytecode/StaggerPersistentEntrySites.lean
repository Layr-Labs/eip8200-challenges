...
theorem dispatch_slice :
    (Artifact.submissionArtifact.instructions.drop 3723).take dispatchCode.length = dispatchCode := by rfl
def dispatchSite : GenericRoundSite Artifact.submissionArtifact .Osaka dispatchCode :=
  StackSiteBuilder.ofSlice dispatchCode 3723 dispatch_slice
    (by change 3723 + dispatchCode.length ≤ Artifact.submissionInstructions.length
...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 4626 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3723) = UInt256.ofNat 4626
...
theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 3801 = 4767 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3801 (by rfl)
  rw [hpc] at h
