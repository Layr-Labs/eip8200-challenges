...
theorem dispatch_slice :
    (Artifact.submissionArtifact.instructions.drop 3683).take dispatchCode.length = dispatchCode := by rfl
def dispatchSite : GenericRoundSite Artifact.submissionArtifact .Osaka dispatchCode :=
  StackSiteBuilder.ofSlice dispatchCode 3683 dispatch_slice
    (by change 3683 + dispatchCode.length ≤ Artifact.submissionInstructions.length
...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 4584 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3683) = UInt256.ofNat 4584
...
theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 3761 = 4725 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3761 (by rfl)
  rw [hpc] at h
