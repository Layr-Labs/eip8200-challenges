...
def dispatchCode : List Instr := dispatchTemplate 4772
...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 4631 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3712) = UInt256.ofNat 4631
...
theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 4772).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3790 = 4772 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3790 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 4772 = true
  rw [hcode]
  exact h
...
def gasSteps_hit (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4631, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 4772, stack := frame h off limit rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 4631, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_hit s (UInt256.ofNat 4631) off limit h rho 4772 hstack hrun hfit hhit (valid_pad s hcode)
    exact hr
...
def gasSteps_miss (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4631, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 4638, stack := frame h off limit rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 4631, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_miss s (UInt256.ofNat 4631) off limit h rho 4772 hstack hrun hfit hmiss
    have he : pcAfter (UInt256.ofNat 4631) (dispatchTemplate 4772) = UInt256.ofNat 4638 := by decide
    rw [he] at hr
    exact hr
...
def gasSteps_call (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 484, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 502, stack := pointer off :: DenseScheduleTemplate.mask8 :: DenseScheduleTemplate.mask16 :: frame h off limit rho} := by
  apply call_gasSteps_of_raw {s with pc := UInt256.ofNat 484, stack := frame h off limit rho} _ hcode hfork hrun hnp call_pc.symm
  have hr := run_call s (UInt256.ofNat 484) off limit h rho hstack hrun
  have he : pcAfter (UInt256.ofNat 484) callTemplate = UInt256.ofNat 502 := by decide
  rw [he] at hr
  exact hr
