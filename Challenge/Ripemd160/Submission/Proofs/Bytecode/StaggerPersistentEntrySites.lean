...
def dispatchCode : List Instr := dispatchTemplate 4723
...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 4660 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3524) = UInt256.ofNat 4660
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 4723).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3572 = 4723 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3572 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 4723 = true
  rw [hcode]
  exact h
...
def gasSteps_hit (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4660, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 4723, stack := frame h off limit rho} := by
...
def gasSteps_miss (s : State) (off limit : UInt256) (h : Compression.HashState)
  ...
  · have hr := run_miss s (UInt256.ofNat 4638) off limit h rho 4701 hstack hrun hfit hmiss
    have he : pcAfter (UInt256.ofNat 4660) (dispatchTemplate 4723) = UInt256.ofNat 4667 := by decide
    rw [he] at hr
    exact hr
