...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 4628 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3713) = UInt256.ofNat 4628
...
theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 3791 = 4769 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_hit (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4628, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 4769, stack := frame h off limit rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 4628, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_hit s (UInt256.ofNat 4628) off limit h rho 4769 hstack hrun hfit hhit (valid_pad s hcode)
    exact hr
...
def gasSteps_miss (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4628, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 4635, stack := frame h off limit rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 4628, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_miss s (UInt256.ofNat 4628) off limit h rho 4769 hstack hrun hfit hmiss
    have he : pcAfter (UInt256.ofNat 4628) (dispatchTemplate 4769) = UInt256.ofNat 4635 := by decide
    rw [he] at hr
    exact hr
