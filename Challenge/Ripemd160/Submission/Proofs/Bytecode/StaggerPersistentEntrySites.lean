...
def dispatchCode : List Instr := dispatchTemplate 4763
theorem dispatch_slice :
    (Artifact.submissionArtifact.instructions.drop 3567).take dispatchCode.length = dispatchCode := by rfl
def dispatchSite : GenericRoundSite Artifact.submissionArtifact .Osaka dispatchCode :=
  StackSiteBuilder.ofSlice dispatchCode 3567 dispatch_slice
    (by change 3567 + dispatchCode.length ≤ Artifact.submissionInstructions.length
...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 4642 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3567) = UInt256.ofNat 4642
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 4763).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3656 = 4763 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3656 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 4763 = true
  rw [hcode]
  exact h
...
def gasSteps_hit (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4642, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 4763, stack := frame h off limit rho} := by
  ...
  · have hr := run_hit s (UInt256.ofNat 4642) off limit h rho 4763 hstack hrun hfit hhit (valid_pad s hcode)
    exact hr
...
def gasSteps_miss (s : State) (off limit : UInt256) (h : Compression.HashState)
  ...
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 4642, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_miss s (UInt256.ofNat 4642) off limit h rho 4763 hstack hrun hfit hmiss
    have he : pcAfter (UInt256.ofNat 4642) (dispatchTemplate 4763) = UInt256.ofNat 4649 := by decide
    rw [he] at hr
    exact hr
