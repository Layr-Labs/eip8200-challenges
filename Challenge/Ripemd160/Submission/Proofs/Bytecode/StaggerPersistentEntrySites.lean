...
theorem dispatch_slice :
    (Artifact.submissionArtifact.instructions.drop 3516).take dispatchCode.length = dispatchCode := by rfl
def dispatchSite : GenericRoundSite Artifact.submissionArtifact .Osaka dispatchCode :=
  StackSiteBuilder.ofSlice dispatchCode 3516 dispatch_slice
    (by change 3516 + dispatchCode.length ≤ Artifact.submissionInstructions.length
...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 4632 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3516) = UInt256.ofNat 4632
...
theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 3564 = 4695 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3564 (by rfl)
  rw [hpc] at h
...
def gasSteps_hit (s : State) (off limit : UInt256) (h : Compression.HashState)
  ...
  · have hr := run_hit s (UInt256.ofNat 4632) off limit h rho 4695 hstack hrun hfit hhit (valid_pad s hcode)
    exact hr
...
def gasSteps_miss (s : State) (off limit : UInt256) (h : Compression.HashState)
  ...
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 4632, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_miss s (UInt256.ofNat 4632) off limit h rho 4695 hstack hrun hfit hmiss
    have he : pcAfter (UInt256.ofNat 4632) (dispatchTemplate 4695) = UInt256.ofNat 4639 := by decide
    rw [he] at hr
    exact hr
