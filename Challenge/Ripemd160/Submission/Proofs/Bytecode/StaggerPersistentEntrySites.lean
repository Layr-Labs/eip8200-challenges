...
def dispatchCode : List Instr := dispatchTemplate 4767
...
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 4626 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3713) = UInt256.ofNat 4626
...
theorem call_pc : callSite.startPC = UInt256.ofNat 482 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 270) = UInt256.ofNat 482
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 4767).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3791 = 4767 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3791 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 4767 = true
  rw [hcode]
  exact h
...
def gasSteps_hit (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4626, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 4767, stack := frame h off limit rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 4626, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_hit s (UInt256.ofNat 4626) off limit h rho 4767 hstack hrun hfit hhit (valid_pad s hcode)
    exact hr
...
def gasSteps_miss (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4626, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 4633, stack := frame h off limit rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 4626, stack := frame h off limit rho} _ hcode hfork hrun hnp dispatch_pc.symm
  · exact PadLift.advancesAll_sound _ (by decide)
  · have hr := run_miss s (UInt256.ofNat 4626) off limit h rho 4767 hstack hrun hfit hmiss
    have he : pcAfter (UInt256.ofNat 4626) (dispatchTemplate 4767) = UInt256.ofNat 4633 := by decide
    rw [he] at hr
    exact hr
...
def gasSteps_call (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 482, stack := frame h off limit rho}
      {s with pc := UInt256.ofNat 487, stack := pointer off :: frame h off limit rho} := by
  apply DenseScheduleLift.gasSteps_of_raw callSite {s with pc := UInt256.ofNat 482, stack := frame h off limit rho} _ hcode hfork hrun hnp call_pc.symm
  · exact Table80SiteCommon.coreAdvancesAll_sound _ (by decide)
  · have hr := run_call s (UInt256.ofNat 482) off limit h rho hstack hrun
    have he : pcAfter (UInt256.ofNat 482) callTemplate = UInt256.ofNat 487 := by decide
    rw [he] at hr
    exact hr
