...
theorem template_slice :
    (Artifact.submissionArtifact.instructions.drop 3719).take template.length = template := by
  rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3719 template_slice
    (by
      change 3719 + template.length ≤ Artifact.submissionInstructions.length
...
theorem run_template (s : State) (q : PairedHelperBooleanTrace.Frame)
  ...
  let post : PairedHelperBooleanTrace.Frame :=
    {q with
      ...
      d := modifiedC10 q
      e := q.d}
  have h0 := run_modifiedTemplate_raw s (UInt256.ofNat 4589) q (ret :: rho) hstack hrun hactive
  rw [modifiedTemplate_pc] at h0
  have h1 := run_coreExitTemplate s (UInt256.ofNat 4636) post (ret :: rho) hstack hrun
  have hpc : pcAfter (UInt256.ofNat 4636) coreExitTemplate = UInt256.ofNat 4638 := by decide
  rw [hpc] at h1
