...
theorem template_slice :
    (Artifact.submissionArtifact.instructions.drop 3981).take template.length = template := by
  rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3981 template_slice
    (by
      change 3981 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_startPC : site.startPC = UInt256.ofNat 4797 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3981) = UInt256.ofNat 4797
