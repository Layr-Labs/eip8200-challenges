...
theorem template_slice :
    (Artifact.submissionArtifact.instructions.drop 3966).take template.length = template := by
  rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3966 template_slice
    (by
      change 3966 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_startPC : site.startPC = UInt256.ofNat 4812 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3966) = UInt256.ofNat 4812
