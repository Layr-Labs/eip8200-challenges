...
theorem site_startPC : site.startPC = UInt256.ofNat 4805 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3931) = UInt256.ofNat 4805
...
theorem run_template (s : State) (q : PairedHelperBooleanTrace.Frame)
    ...
    runInstrSeq template {s with pc := UInt256.ofNat 4805, stack := inline79Entry q (ret :: rho)} =
      some {s with
  ...
  let post : PairedHelperBooleanTrace.Frame :=
    {q with
      ...
      d := modifiedC10 q
      e := q.d}
  have h0 := run_modifiedTemplate_raw s (UInt256.ofNat 4805) q (ret :: rho) hstack hrun hactive
  rw [modifiedTemplate_pc] at h0
  have h1 := run_coreExitTemplate s (UInt256.ofNat 4852) post (ret :: rho) hstack hrun
  have hpc : pcAfter (UInt256.ofNat 4852) coreExitTemplate = UInt256.ofNat 4854 := by decide
  rw [hpc] at h1
...
def gasSteps_round (s : State) (q : PairedHelperBooleanTrace.Frame)
    ...
    GasSteps {s with pc := UInt256.ofNat 4805, stack := inline79Entry q (ret :: rho)}
      {s with
        pc := UInt256.ofNat 4854
        stack := PairedAllInlineTail.entryStack (modifiedFrame s.memory q) ret rho} := by
