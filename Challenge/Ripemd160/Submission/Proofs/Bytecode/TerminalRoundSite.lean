...
theorem template_slice :
    (Artifact.submissionArtifact.instructions.drop 3964).take template.length = template := by
  rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3964 template_slice
    (by
      change 3964 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_startPC : site.startPC = UInt256.ofNat 4806 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3964) = UInt256.ofNat 4806
