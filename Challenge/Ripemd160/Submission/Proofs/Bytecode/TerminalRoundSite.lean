...
theorem template_slice :
    (Artifact.submissionArtifact.instructions.drop 3856).take template.length = template := by
  rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3856 template_slice
    (by
      change 3856 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_startPC : site.startPC = UInt256.ofNat 4572 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3856) = UInt256.ofNat 4572
...
theorem run_template (s : State) (q : PairedHelperBooleanTrace.Frame)
    ...
    runInstrSeq template {s with pc := UInt256.ofNat 4572, stack := inline79Entry q (cache s.memory ++ (ret :: rho))} =
      some {s with pc := UInt256.ofNat 4619, stack := PairedAllInlineBoundarySites.cachedTailEntry s.memory (modifiedFrame s.memory q) ret rho} := by
  ...
  have h0 := run_modifiedTemplate_raw s (UInt256.ofNat 4572) q (cache s.memory ++ (ret :: rho)) hs hrun hactive
  have hmodified_pc : pcAfter (UInt256.ofNat 4572) modifiedTemplate = UInt256.ofNat 4617 := by decide
  rw [hmodified_pc] at h0
  have h1 := run_coreExitTemplate s (UInt256.ofNat 4617) post (cache s.memory ++ (ret :: rho)) hs hrun
  have hpc : pcAfter (UInt256.ofNat 4617) coreExitTemplate = UInt256.ofNat 4619 := by decide
  rw [hpc] at h1
...
def gasSteps_round (s : State) (q : PairedHelperBooleanTrace.Frame)
    ...
    GasSteps {s with pc := UInt256.ofNat 4572, stack := inline79Entry q (cache s.memory ++ (ret :: rho))}
      {s with pc := UInt256.ofNat 4619, stack := PairedAllInlineBoundarySites.cachedTailEntry s.memory (modifiedFrame s.memory q) ret rho} := by
