...
theorem template_slice :
    (Artifact.submissionArtifact.instructions.drop 3932).take template.length = template := by
  rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3932 template_slice
    (by
      change 3932 + template.length ≤ Artifact.submissionInstructions.length
