...
theorem site_pc : site.startPC = UInt256.ofNat 3028 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2352) = UInt256.ofNat 3028
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3028, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3061, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3028) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3028) template = UInt256.ofNat 3061 := by decide
  rw [hend] at hraw
