...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1491 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1989 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1491) = UInt256.ofNat 1989
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1989, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2037, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1989) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1989) template = UInt256.ofNat 2037 := by decide
  rw [hend] at hraw
