...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3116).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3116 actual_slice
    (by change 3116 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3989 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3116) = UInt256.ofNat 3989
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 3989) template = UInt256.ofNat 4030 := by decide
  rw [hend] at hraw
