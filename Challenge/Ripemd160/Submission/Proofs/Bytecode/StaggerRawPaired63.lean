...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3058).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3058 actual_slice
    (by change 3058 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3897 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3058) = UInt256.ofNat 3897
