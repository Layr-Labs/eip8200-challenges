...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3292).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3292 actual_slice
    (by change 3292 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4034 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3292) = UInt256.ofNat 4034
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 4034) template = UInt256.ofNat 4072 := by decide
  rw [hend] at hraw
