...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1487).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1487 actual_slice
    (by change 1487 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 1898 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1487) = UInt256.ofNat 1898
