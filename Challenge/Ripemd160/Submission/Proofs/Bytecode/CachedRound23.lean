...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1588).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1588 actual_slice
    (by change 1588 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2064 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1588) = UInt256.ofNat 2064
