...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1597).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1597 actual_slice
    (by change 1597 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2062 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1597) = UInt256.ofNat 2062
