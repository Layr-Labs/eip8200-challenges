...
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1588 actual_slice
    ...
    (by decide)
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2058 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1588) = UInt256.ofNat 2058
...
theorem run_actual (s : State) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 996) (hrun : s.halt = .Running)
    (hactive : 23 ≤ s.activeWords.toNat) :
    runInstrSeq actualTemplate {s with pc := UInt256.ofNat 2058, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 2108, stack := outputStack s.memory x rho} := by
  have hcap (n : Nat) (hn : n ≤ 26) : rho.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (haddress : address ≤ 704) :
      UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat address 32) = s.activeWords :=
    active_schedule_preserved s.activeWords address hactive haddress
...
def gasSteps (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2058, stack := coreStack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] f (cache s.memory ++ rho)}
      {s with pc := UInt256.ofNat 2108, stack := coreStack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] (PairedAllInlineCoreTrace.inline23Block.eval s.memory f) (cache s.memory ++ rho)} := by
  ...
  have g := gasSteps_terminal_of_raw actualSite
    {s with pc := UInt256.ofNat 2058, stack := inputStack s.memory x rho} _
