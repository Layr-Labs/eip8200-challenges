...
def stage8 (wide : Bool) : List Instr :=
  [ .op (.Dup ⟨0, by decide⟩),
    (if wide then .push ⟨2, by decide⟩ (UInt256.ofNat 8) else .push ⟨1, by decide⟩ (UInt256.ofNat 8)),
    .op .SHR,
    .op (.Dup ⟨1, by decide⟩),
    .op .XOR,
    .op (.Dup ⟨15, by decide⟩),
...
theorem run_load (s : State) (pc ret mw a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim : UInt256)
  ...
  simp (discharger := omega) [loadTemplate, stk, activeAfterWord,
    runInstrSeq, DataStepper.runInstr, pcAfter, UInt256.succ, Instr.size, hrun, hcap,
...
def lowStore : List Instr := [ .push ⟨2, by decide⟩ (UInt256.ofNat 28), .op .MSTORE ]
...
theorem run_highStore (s : State) (pc value : UInt256) (rest : List UInt256)
  ...
  PairedSchedulePrimitives.run_storeTemplate s pc value 60 rest hstack (by decide) hrun
...
def normalTemplate : List Instr := (template ++ StaggerRawNormalPool.template) ++ StaggerNormal.storesTemplate
...
theorem run_normal (s : State) (pc ret a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim : UInt256)
  ...
  have h3 := StaggerNormal.run_pool s2 (pcAfter pc template) F hF (by rfl) hrun (by omega)
  ...
  have h4 := StaggerNormal.run_stores s2 (pcAfter (pcAfter pc template) StaggerRawNormalPool.template)
