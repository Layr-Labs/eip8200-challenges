...
270 to scratch word 0.  Branch proofs compose via `gasSteps_codecopy`.
-/
...
def preCopyState (s : State) (rho : List UInt256) : State :=
  { s with
    ...
    stack := [UInt256.ofNat 0, UInt256.ofNat 270, UInt256.ofNat 32] ++ rho }
...
def gasSteps_codecopy (s : State) (rho : List UInt256)
  ...
  have hpc : (withGas pre gas).pc.toNat =
    ...
    show (UInt256.ofNat 5057).toNat = _
  ...
  · have hstack' : (withGas pre gas).stack =
        UInt256.ofNat 0 :: UInt256.ofNat 270 :: UInt256.ofNat 32 :: rho := by
      rfl
    ...
    have hstep := StepRunning.codecopy (withGas pre gas)
      (UInt256.ofNat 0) (UInt256.ofNat 270) (UInt256.ofNat 32) rho
      hdec hstack' hgas hcap
    have mz : (0 : Nat) % 2 ^ 256 = 0 := Nat.mod_eq_of_lt (by norm_num)
    have mz262 : (270 : Nat) % 2 ^ 256 = 270 := Nat.mod_eq_of_lt (by norm_num)
    ...
    simpa [pre, cost, preCopyState, copiedState, PrefixStateMemory.copied,
      ...
      Challenge.EvmProof.Word.succ_ofNat (n := 5057) (by norm_num),
      hcode] using hstep
