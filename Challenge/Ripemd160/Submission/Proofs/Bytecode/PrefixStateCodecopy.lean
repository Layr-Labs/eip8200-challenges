...
/-! Generic `CODECOPY` step for the H8 checked prefix (index 4279, pc 5211).
...
def gasSteps_codecopy (s : State) (rho : List UInt256)
  ...
  have hpc : (withGas pre gas).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4279 := by
    show (UInt256.ofNat 5211).toNat = _
    rw [PrefixStatePaths.pc4022]
    decide
  have hdec := Stepper.decodes_of_artifact
    Artifact.submissionArtifact (withGas pre gas) 4279 (.op .CODECOPY)
