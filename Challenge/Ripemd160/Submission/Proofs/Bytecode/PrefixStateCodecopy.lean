...
/-! Generic `CODECOPY` step for the H8 checked prefix (index 4088, pc 5007).
...
def gasSteps_codecopy (s : State) (rho : List UInt256)
  ...
  have hdec := Stepper.decodes_of_artifact
    Artifact.submissionArtifact (withGas pre gas) 4095 (.op .CODECOPY)
  ...
  · have hstack' : (withGas pre gas).stack =
    ...
    simpa [pre, cost, preCopyState, copiedState, PrefixStateMemory.copied,
      ...
      Challenge.EvmProof.Word.succ_ofNat (n := 5041) (by norm_num),
      hcode] using hstep
