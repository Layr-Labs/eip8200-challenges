...
/-! Generic `CODECOPY` step for the H8 checked prefix (index 4280, pc 5226).
...
261 to scratch word 0.  Branch proofs compose via `gasSteps_codecopy`.
-/
...
def preCopyState (s : State) (rho : List UInt256) : State :=
  { s with
    ...
    stack := [UInt256.ofNat 0, UInt256.ofNat 261, UInt256.ofNat 32] ++ rho }
...
def gasSteps_codecopy (s : State) (rho : List UInt256)
  ...
  have hpc : (withGas pre gas).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4280 := by
    show (UInt256.ofNat 5226).toNat = _
    rw [PrefixStatePaths.pc4022]
    decide
  have hdec := Stepper.decodes_of_artifact
    Artifact.submissionArtifact (withGas pre gas) 4280 (.op .CODECOPY)
  ...
  · have hstack' : (withGas pre gas).stack =
        UInt256.ofNat 0 :: UInt256.ofNat 261 :: UInt256.ofNat 32 :: rho := by
      rfl
    ...
    have hstep := StepRunning.codecopy (withGas pre gas)
      (UInt256.ofNat 0) (UInt256.ofNat 261) (UInt256.ofNat 32) rho
      hdec hstack' hgas hcap
    have mz : (0 : Nat) % 2 ^ 256 = 0 := Nat.mod_eq_of_lt (by norm_num)
    have mz261 : (261 : Nat) % 2 ^ 256 = 261 := Nat.mod_eq_of_lt (by norm_num)
    ...
    simpa [pre, cost, preCopyState, copiedState, PrefixStateMemory.copied,
      ...
      Challenge.EvmProof.Word.word_toNat_ofNat, mz, mz261, mz32,
      Challenge.EvmProof.Word.succ_ofNat (n := 5226) (by norm_num),
      hcode] using hstep
