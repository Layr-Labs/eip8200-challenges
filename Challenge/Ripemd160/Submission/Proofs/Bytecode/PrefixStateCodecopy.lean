...
/-! Generic `CODECOPY` step for the H8 checked prefix (index 4110, pc 5002).
...
276 to scratch word 0.  Branch proofs compose via `gasSteps_codecopy`.
-/
...
def preCopyState (s : State) (rho : List UInt256) : State :=
  { s with
    ...
    stack := [UInt256.ofNat 0, UInt256.ofNat 276, UInt256.ofNat 32] ++ rho }
...
def gasSteps_codecopy (s : State) (rho : List UInt256)
  ...
  have hpc : (withGas pre gas).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4097 := by
    show (UInt256.ofNat 5047).toNat = _
    rw [PrefixStatePaths.pc4079]
    decide
  have hdec := Stepper.decodes_of_artifact
    Artifact.submissionArtifact (withGas pre gas) 4097 (.op .CODECOPY)
  ...
  · have hstack' : (withGas pre gas).stack =
        UInt256.ofNat 0 :: UInt256.ofNat 276 :: UInt256.ofNat 32 :: rho := by
      rfl
    ...
    have hstep := StepRunning.codecopy (withGas pre gas)
      (UInt256.ofNat 0) (UInt256.ofNat 276) (UInt256.ofNat 32) rho
      hdec hstack' hgas hcap
    have mz : (0 : Nat) % 2 ^ 256 = 0 := Nat.mod_eq_of_lt (by norm_num)
    have mz262 : (276 : Nat) % 2 ^ 256 = 276 := Nat.mod_eq_of_lt (by norm_num)
    ...
    simpa [pre, cost, preCopyState, copiedState, PrefixStateMemory.copied,
      ...
      Challenge.EvmProof.Word.succ_ofNat (n := 5002) (by norm_num),
      hcode] using hstep
