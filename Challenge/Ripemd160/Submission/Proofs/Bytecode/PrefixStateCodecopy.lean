...
/-! Generic `CODECOPY` step for the H8 checked prefix (index 4283, pc 5213).
...
def gasSteps_codecopy (s : State) (rho : List UInt256)
  ...
  have hpc : (withGas pre gas).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4283 := by
    show (UInt256.ofNat 5213).toNat = _
    rw [PrefixStatePaths.pc4022]
    decide
  have hdec := Stepper.decodes_of_artifact
    Artifact.submissionArtifact (withGas pre gas) 4283 (.op .CODECOPY)
  ...
  · have hstack' : (withGas pre gas).stack =
    ...
    simpa [pre, cost, preCopyState, copiedState, PrefixStateMemory.copied,
      ...
      Challenge.EvmProof.Word.succ_ofNat (n := 5213) (by norm_num),
      hcode] using hstep
