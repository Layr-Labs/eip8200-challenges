...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1622).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1622 actual_slice
    (by change 1622 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2192 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1622) = UInt256.ofNat 2196
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2192, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2232, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2192) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2192) template = UInt256.ofNat 2232 := by decide
  rw [hend] at hraw
