...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1570 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2157 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1570) = UInt256.ofNat 2157
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2157, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2198, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2157) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2157) template = UInt256.ofNat 2198 := by decide
  rw [hend] at hraw
