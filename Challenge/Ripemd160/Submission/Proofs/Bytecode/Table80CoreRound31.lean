...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound31
...
def nextKey (k : UInt256) : UInt256 := (UInt256.ofNat 7411972540165114580685751011896225)
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound31.outputStack memory (input q k) rho =
      stack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 540) k q) (nextKey k) rho := by
  simp only [Table80RawRound31.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired80Algorithm.physicalKey 31) =
     Paired80Algorithm.physicalKey 32 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired80Algorithm.physicalKey 31) q = Paired80Algorithm.step 31 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 2518, stack := stack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 2580, stack :=
          stack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 540) k q) (nextKey k) rho} := by
   have gs := Table80RawRound31.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound31
