...
theorem generic_dest : Decode.isValidJumpDest submissionBytecode 357 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 245 = 357 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; rfl
...
theorem run_gate_prefix (input : ByteArray) :
    run gatePrefix (PatternedScan.stS input 11 []) =
      some (PatternedScan.stS input 24 [357, EntryGateLogic.gateWord input.size]) := by
  ...
  let l7 : Located := pushAt 15 2 357
  have h7 := PatternedScan.blockOfS l7
    (PatternedScan.pcFactS input 15 21 [s * x] (by norm_num) pc_g15)
    (PatternedScan.stepS_push input 21 2 357 [s * x] (by simp) (by decide) (by decide) (by norm_num))
...
theorem run_gate_taken (input : ByteArray) (hfit : CalldataFits input)
    ...
    run gatePath (PatternedScan.stS input 11 []) = some (PatternedScan.stS input 357 []) := by
  have hj : run [opAt 16 .JUMPI]
      (PatternedScan.stS input 24 [357, EntryGateLogic.gateWord input.size]) =
      some (PatternedScan.stS input 357 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 16 24 _ (by norm_num) pc_g16)
      (PatternedScan.stepS_jumpi_taken input 24 357 357 _ []
...
theorem run_gate_fall (input : ByteArray) (hfit : CalldataFits input)
  ...
  have hj : run [opAt 16 .JUMPI]
    ...
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 16 24 _ (by norm_num) pc_g16)
      (PatternedScan.stepS_jumpi_fall input 24 357 _ []
