...
theorem run_size_fail_legacy (input : ByteArray) (hfit : CalldataFits input)
  ...
  -- rw idiom rather than term mode: the term form forces instructionPC 3850 and
  ...
  have hpc4131 : Artifact.submissionArtifact.instructionPC 3850 = 4837 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 4837 = true := by
    have h := Artifact.submissionArtifact.isValidJumpDest_index 3850 (by rfl)
    rw [hpc4131] at h
    exact h
  have hprefix :
      run
