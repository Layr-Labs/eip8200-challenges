...
theorem run_gate_prefix (input : ByteArray) :
    run gatePrefix (PatternedScan.stS input 12 []) =
      some (PatternedScan.stS input 26 [351, EntryGateLogic.gateWord input.size]) := by
  ...
  have h7 := PatternedScan.blockOfS l7
    ...
    (PatternedScan.stepS_push input 23 2 351 [x * s] (by simp) (by decide) (by decide) (by norm_num))
...
theorem run_gate_taken (input : ByteArray) (hfit : CalldataFits input)
    ...
    run gatePath (PatternedScan.stS input 12 []) = some (PatternedScan.stS input 351 []) := by
  have hj : run [opAt 16 .JUMPI]
      (PatternedScan.stS input 26 [351, EntryGateLogic.gateWord input.size]) =
      some (PatternedScan.stS input 351 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 16 26 _ (by norm_num) pc_g16)
      (PatternedScan.stepS_jumpi_taken input 26 351 351 _ []
...
theorem run_gate_fall (input : ByteArray) (hfit : CalldataFits input)
  ...
  have hj : run [opAt 16 .JUMPI]
    ...
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 16 26 _ (by norm_num) pc_g16)
      (PatternedScan.stepS_jumpi_fall input 26 351 _ []
