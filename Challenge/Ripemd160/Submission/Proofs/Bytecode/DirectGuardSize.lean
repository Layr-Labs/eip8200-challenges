...
theorem run_size_fail (input : ByteArray) (hfit : CalldataFits input)
  ...
  change run sizePath (PatternedScan.stS input 0 []) =
    some (PatternedScan.stS input 5265 [])
  have hdest : Decode.isValidJumpDest submissionBytecode 5265 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 4116 (by rfl)
  have hprefix :
      run
    ...
    simp (config := { decide := true })
      [opAt, pushAt, wfOp, PatternedScan.stS,
       ...
       Challenge.EvmProof.Word.word_toNat_ofNat]
  have hpush :
      run [pushAt 14 2 5265] (PatternedScan.stS input 20 [UInt256.ofNat 1]) =
        some (PatternedScan.stS input 23 [UInt256.ofNat 5265, UInt256.ofNat 1]) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 14 20 _ (by norm_num) pc_classifier_112)
      (PatternedScan.stepS_push input 20 2 5265 [UInt256.ofNat 1]
        (by norm_num) (by decide) (by decide) (by norm_num))
  have htrue : UInt256.isTrue (UInt256.ofNat 1) := by decide
  have hjump :
      run [opAt 15 .JUMPI]
          (PatternedScan.stS input 23 [UInt256.ofNat 5265, UInt256.ofNat 1]) =
        some (PatternedScan.stS input 5265 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 15 23 _ (by norm_num) pc_classifier_113)
      (PatternedScan.stepS_jumpi_taken input 23 5265 5265 (UInt256.ofNat 1) []
  ...
  have hprefix_push :
    ...
    exact Challenge.EvmProof.Stepper.runLocatedBlock_append
      [opAt 0 .CALLDATASIZE, opAt 1 .JUMPDEST,
       ...
       opAt 8 .CALLDATASIZE, pushAt 9 2 1000,
       opAt 10 .EQ, opAt 11 .OR, opAt 12 .OR, opAt 13 .ISZERO]
      [pushAt 14 2 5265] _ _ _ hprefix rfl hpush
  have hfull :
      run
          [opAt 0 .CALLDATASIZE, opAt 1 .JUMPDEST,
           pushAt 2 2 256, opAt 3 .EQ,
           ...
           opAt 8 .CALLDATASIZE, pushAt 9 2 1000,
           opAt 10 .EQ, opAt 11 .OR, opAt 12 .OR, opAt 13 .ISZERO,
           pushAt 14 2 5265, opAt 15 .JUMPI]
        (PatternedScan.stS input 0 []) =
      some (PatternedScan.stS input 5265 []) := by
    exact Challenge.EvmProof.Stepper.runLocatedBlock_append
      [opAt 0 .CALLDATASIZE, opAt 1 .JUMPDEST,
