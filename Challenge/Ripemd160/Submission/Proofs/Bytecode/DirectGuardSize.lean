...
def guardEntry (input : ByteArray) : State := atPC input 4782
...
theorem run_size_fail (input : ByteArray) (hfit : CalldataFits input)
    ...
    run sizePath (Execution.atPC input 0) = some (guardEntry input) := by
  ...
  change run sizePath (PatternedScan.stS input 0 []) =
    some (PatternedScan.stS input 4782 [])
  -- rw idiom rather than term mode: the term form forces instructionPC 4049 and
  ...
  have hpc4131 : Artifact.submissionArtifact.instructionPC 4049 = 4782 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 4782 = true := by
    have h := Artifact.submissionArtifact.isValidJumpDest_index 4049 (by rfl)
    rw [hpc4131] at h
    exact h
  have hprefix :
      run
    ...
    simp (config := { decide := true })
      [opAt, pushAt, wfOp, PatternedScan.stS,
       ...
       Challenge.EvmProof.Word.word_toNat_ofNat]
  have hpush :
      run [pushAt 12 2 4782] (PatternedScan.stS input 18 [UInt256.ofNat 1]) =
        some (PatternedScan.stS input 21 [UInt256.ofNat 4782, UInt256.ofNat 1]) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 12 18 _ (by norm_num) pc_classifier_112)
      (PatternedScan.stepS_push input 18 2 4782 [UInt256.ofNat 1]
        (by norm_num) (by decide) (by decide) (by norm_num))
  have htrue : UInt256.isTrue (UInt256.ofNat 1) := by decide
  have hjump :
      run [opAt 13 .JUMPI]
          (PatternedScan.stS input 21 [UInt256.ofNat 4782, UInt256.ofNat 1]) =
        some (PatternedScan.stS input 4782 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 13 21 _ (by norm_num) pc_classifier_113)
      (PatternedScan.stepS_jumpi_taken input 21 4782 4782 (UInt256.ofNat 1) []
  ...
  have hprefix_push :
    ...
    exact Challenge.EvmProof.Stepper.runLocatedBlock_append
      [opAt 0 .CALLDATASIZE, pushAt 1 2 256, opAt 2 .EQ,
       ...
       opAt 6 .CALLDATASIZE, pushAt 7 2 1000,
       opAt 8 .EQ, opAt 9 .OR, opAt 10 .OR, opAt 11 .ISZERO]
      [pushAt 12 2 4782] _ _ _ hprefix rfl hpush
  have hfull :
      run
          [opAt 0 .CALLDATASIZE, pushAt 1 2 256, opAt 2 .EQ,
           opAt 3 .CALLDATASIZE, pushAt 4 2 376, opAt 5 .EQ,
           opAt 6 .CALLDATASIZE, pushAt 7 2 1000,
           opAt 8 .EQ, opAt 9 .OR, opAt 10 .OR, opAt 11 .ISZERO,
           pushAt 12 2 4782, opAt 13 .JUMPI]
        (PatternedScan.stS input 0 []) =
      some (PatternedScan.stS input 4782 []) := by
    exact Challenge.EvmProof.Stepper.runLocatedBlock_append
      [opAt 0 .CALLDATASIZE, pushAt 1 2 256, opAt 2 .EQ,
...
theorem run_size_match (input : ByteArray) (hsize : input.size = 1000) :
    run sizePath (Execution.atPC input 0) = some (sizeMatched input) := by
