...
theorem run_size_fail_legacy (input : ByteArray) (hfit : CalldataFits input)
  ...
  -- rw idiom rather than term mode: the term form forces instructionPC 3823 and
  ...
  have hpc4131 : Artifact.submissionArtifact.instructionPC 3823 = 4788 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 4788 = true := by
    have h := Artifact.submissionArtifact.isValidJumpDest_index 3823 (by rfl)
    rw [hpc4131] at h
    exact h
  have hprefix :
      run
