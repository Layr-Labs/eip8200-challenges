...
def guardEntry (input : ByteArray) : State := atPC input 4793
...
theorem run_size_fail_legacy (input : ByteArray) (hfit : CalldataFits input)
  ...
  -- rw idiom rather than term mode: the term form forces instructionPC 3822 and
  ...
  have hpc4131 : Artifact.submissionArtifact.instructionPC 3822 = 4793 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 4793 = true := by
    have h := Artifact.submissionArtifact.isValidJumpDest_index 3822 (by rfl)
    rw [hpc4131] at h
    exact h
  have hprefix :
      run
    ...
    simp (config := { decide := true })
      [opAt, pushAt, wfOp, PatternedScan.stS,
       ...
       Challenge.EvmProof.Word.word_toNat_ofNat]
  have hpush :
      run [pushAt 17 2 4793] (PatternedScan.stS input 26 [UInt256.ofNat 1]) =
        some (PatternedScan.stS input 29 [UInt256.ofNat 4793, UInt256.ofNat 1]) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 17 26 _ (by norm_num) pc_classifier_112)
      (PatternedScan.stepS_push input 26 2 4793 [UInt256.ofNat 1]
        (by norm_num) (by decide) (by decide) (by norm_num))
  have htrue : UInt256.isTrue (UInt256.ofNat 1) := by decide
  have hjump :
      run [opAt 18 .JUMPI]
          (PatternedScan.stS input 29 [UInt256.ofNat 4793, UInt256.ofNat 1]) =
        some (PatternedScan.stS input 4793 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 18 29 _ (by norm_num) pc_classifier_113)
      (PatternedScan.stepS_jumpi_taken input 29 4793 4793 (UInt256.ofNat 1) []
  ...
  have hprefix_push :
    ...
    exact Challenge.EvmProof.Stepper.runLocatedBlock_append
      [opAt 5 .CALLDATASIZE, pushAt 6 2 256, opAt 7 .EQ,
       ...
       opAt 11 .CALLDATASIZE, pushAt 12 2 1000,
       opAt 13 .EQ, opAt 14 .OR, opAt 15 .OR, opAt 16 .ISZERO]
      [pushAt 17 2 4793] _ _ _ hprefix rfl hpush
  have hfull :
      run
          [opAt 5 .CALLDATASIZE, pushAt 6 2 256, opAt 7 .EQ,
           opAt 8 .CALLDATASIZE, pushAt 9 2 376, opAt 10 .EQ,
           opAt 11 .CALLDATASIZE, pushAt 12 2 1000,
           opAt 13 .EQ, opAt 14 .OR, opAt 15 .OR, opAt 16 .ISZERO,
           pushAt 17 2 4793, opAt 18 .JUMPI]
        (PatternedScan.stS input 8 []) =
      some (PatternedScan.stS input 4793 []) := by
    exact Challenge.EvmProof.Stepper.runLocatedBlock_append
      [opAt 5 .CALLDATASIZE, pushAt 6 2 256, opAt 7 .EQ,
