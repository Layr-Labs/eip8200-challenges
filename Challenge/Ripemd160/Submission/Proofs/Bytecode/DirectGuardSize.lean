...
def guardEntry (input : ByteArray) : State := atPC input 427
...
theorem generic_dest : Decode.isValidJumpDest submissionBytecode 568 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 326 = 568 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; rfl
...
theorem run_gate_prefix (input : ByteArray) :
    run gatePrefix (PatternedScan.stS input 24 []) =
      some (PatternedScan.stS input 37 [568, EntryGateLogic.gateWord input.size]) := by
  ...
  let l7 : Located := pushAt 21 2 568
  have h7 := PatternedScan.blockOfS l7
    (PatternedScan.pcFactS input 21 34 [s * x] (by norm_num) pc_g15)
    (PatternedScan.stepS_push input 34 2 568 [s * x] (by simp) (by decide) (by decide) (by norm_num))
...
theorem run_gate_taken (input : ByteArray) (hfit : CalldataFits input)
    ...
    run gatePath (PatternedScan.stS input 24 []) = some (PatternedScan.stS input 569 []) := by
  have hj : run [opAt 22 .JUMPI]
      (PatternedScan.stS input 37 [568, EntryGateLogic.gateWord input.size]) =
      some (PatternedScan.stS input 568 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 22 37 _ (by norm_num) pc_g16)
      (PatternedScan.stepS_jumpi_taken input 37 568 568 _ []
  ...
  have hd : run [entryDest] (PatternedScan.stS input 568 []) =
      some (PatternedScan.stS input 569 []) := by
    exact PatternedScan.blockOfS entryDest
      (PatternedScan.pcFactS input 326 568 [] (by norm_num)
...
theorem run_gate_fall (input : ByteArray) (hfit : CalldataFits input)
  ...
  have hj : run [opAt 22 .JUMPI]
    ...
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 22 37 _ (by norm_num) pc_g16)
      (PatternedScan.stepS_jumpi_fall input 37 568 _ []
