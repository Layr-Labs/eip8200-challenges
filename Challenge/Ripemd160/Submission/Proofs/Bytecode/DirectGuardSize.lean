...
theorem run_size_fail (input : ByteArray) (hfit : CalldataFits input)
    ...
    run sizePath (Execution.atPC input 0x8) = some (fallbackState input) := by
  ...
  have hdest : Decode.isValidJumpDest submissionBytecode 0x154 = true :=
...
theorem run_size_match (input : ByteArray) (hsize : input.size = 1000) :
    run sizePath (Execution.atPC input 0x8) = some (sizeMatched input) := by
