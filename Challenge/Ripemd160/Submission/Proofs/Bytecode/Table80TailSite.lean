...
theorem prefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3918).take prefixTemplate.length =
      prefixTemplate := by rfl

def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 3918 prefix_slice
    (by change 3918 + prefixTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 4615 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3918) = UInt256.ofNat 4615
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem prefix_endPC : prefixSite.endPC = UInt256.ofNat 4714 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3991) = UInt256.ofNat 4714
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def jumpSite : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc_eq := by
    change (UInt256.ofNat 4714).toNat = Artifact.submissionArtifact.instructionPC 3991
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
