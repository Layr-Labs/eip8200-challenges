...
theorem prefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3954).take prefixTemplate.length =
      prefixTemplate := by rfl

def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 3954 prefix_slice
    (by change 3954 + prefixTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := prefixTemplate) (by decide))
    (by decide)

theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 4684 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3954) = UInt256.ofNat 4684
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem prefix_endPC : prefixSite.endPC = UInt256.ofNat 4783 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4027) = UInt256.ofNat 4783
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def jumpSite : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc_eq := by
    change (UInt256.ofNat 4783).toNat = Artifact.submissionArtifact.instructionPC 4027
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps (s : State) (ret : UInt256) (q : WordLane) (factor : UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4684, stack := entryStack factor q ret rho}
