...
private theorem template_slice :
    (A.instructions.drop 2117).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2117 template_slice (by
    change 2117 + template.length ≤ Artifact.submissionInstructions.length
