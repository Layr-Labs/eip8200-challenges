...
private theorem template_slice :
    (A.instructions.drop 2280).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2280 template_slice (by
    change 2280 + template.length ≤ Artifact.submissionInstructions.length
