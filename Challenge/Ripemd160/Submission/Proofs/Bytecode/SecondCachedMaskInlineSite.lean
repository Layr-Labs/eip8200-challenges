...
private theorem template_slice :
    (A.instructions.drop 2114).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2114 template_slice (by
    change 2096 + template.length ≤ Artifact.submissionInstructions.length
