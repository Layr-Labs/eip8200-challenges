...
private theorem template_slice :
    (A.instructions.drop 2102).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2102 template_slice (by
    change 2102 + template.length ≤ Artifact.submissionInstructions.length
...
theorem destination_next : destination.pc.succ = QuadSites.rightPC 2 := by rfl
...
def gasSteps_right1 (s : State) (word : Nat → UInt32)
  ...
  let w := (rightParams 0 1).apply s working
