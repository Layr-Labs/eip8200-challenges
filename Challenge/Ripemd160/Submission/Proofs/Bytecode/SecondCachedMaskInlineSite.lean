...
private theorem template_slice :
    (A.instructions.drop 2092).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2092 template_slice (by
    change 2092 + template.length ≤ Artifact.submissionInstructions.length
