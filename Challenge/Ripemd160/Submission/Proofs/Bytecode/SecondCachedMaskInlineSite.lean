...
private theorem template_slice :
    (A.instructions.drop 2105).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2105 template_slice (by
    change 2105 + template.length ≤ Artifact.submissionInstructions.length
