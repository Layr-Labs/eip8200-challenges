...
private theorem template_slice :
    (A.instructions.drop 2267).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2267 template_slice (by
    change 2267 + template.length ≤ Artifact.submissionInstructions.length
