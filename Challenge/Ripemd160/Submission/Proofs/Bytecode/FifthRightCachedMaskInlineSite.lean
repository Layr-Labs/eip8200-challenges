...
private theorem template_slice :
    (A.instructions.drop 2616).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2616 template_slice (by
    change 2616 + template.length ≤ Artifact.submissionInstructions.length
