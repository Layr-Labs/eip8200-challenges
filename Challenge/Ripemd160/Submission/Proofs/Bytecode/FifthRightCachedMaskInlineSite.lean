...
private theorem template_slice :
    (A.instructions.drop 2435).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2435 template_slice (by
    change 2435 + template.length ≤ Artifact.submissionInstructions.length
