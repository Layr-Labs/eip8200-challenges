...
private theorem template_slice :
    (A.instructions.drop 2441).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2441 template_slice (by
    change 2441 + template.length ≤ Artifact.submissionInstructions.length
