...
private theorem template_slice :
    (A.instructions.drop 2606).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2606 template_slice (by
    change 2606 + template.length ≤ Artifact.submissionInstructions.length
