...
private theorem template_slice :
    (A.instructions.drop 2453).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2453 template_slice (by
    change 2453 + template.length ≤ Artifact.submissionInstructions.length
