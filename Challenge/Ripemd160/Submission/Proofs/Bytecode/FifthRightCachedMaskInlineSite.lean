...
private theorem template_slice :
    (A.instructions.drop 2432).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2432 template_slice (by
    change 2432 + template.length ≤ Artifact.submissionInstructions.length
