...
private theorem template_slice :
    (A.instructions.drop 2442).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2442 template_slice (by
    change 2442 + template.length ≤ Artifact.submissionInstructions.length
