...
private theorem template_slice :
    (A.instructions.drop 2456).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2456 template_slice (by
    change 2454 + template.length ≤ Artifact.submissionInstructions.length
