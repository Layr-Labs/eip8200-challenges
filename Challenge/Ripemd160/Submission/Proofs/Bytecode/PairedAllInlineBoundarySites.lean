...
theorem schedule_instructionPC :
    Artifact.submissionArtifact.instructionPC 298 = 529 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem schedule_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 463 = 764 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def scheduleSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedDivMaskCache.fullTemplate :=
  StackSiteBuilder.ofSlice PairedDivMaskCache.fullTemplate 298 schedule_slice
    (by
      change 298 + PairedDivMaskCache.fullTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem scheduleSite_startPC : scheduleSite.startPC = UInt256.ofNat 529 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 298) =
    UInt256.ofNat 529
  rw [schedule_instructionPC]

theorem scheduleSite_endPC : scheduleSite.endPC = UInt256.ofNat 764 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 463) =
    UInt256.ofNat 764
  rw [schedule_endInstructionPC]

theorem startup_slice :
    (Artifact.submissionArtifact.instructions.drop 463).take
...
def startupSite :
  ...
  StackSiteBuilder.ofSlice SCanonicalStartup.template 463 startup_slice
    (by
      change 463 + SCanonicalStartup.template.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem startupSite_startPC : startupSite.startPC = UInt256.ofNat 764 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 463) =
    UInt256.ofNat 764
  rw [startup_instructionPC]

theorem startupSite_endPC : startupSite.endPC = UInt256.ofNat 812 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 496) =
    UInt256.ofNat 812
  rw [startup_endInstructionPC]

theorem tailPrefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3977).take
      PairedAllInlineTail.prefixTemplate.length = PairedAllInlineTail.prefixTemplate := by
  rfl

theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 3977 = 4859 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4046 = 4943 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def tailPrefixSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedAllInlineTail.prefixTemplate :=
  StackSiteBuilder.ofSlice PairedAllInlineTail.prefixTemplate 3977 tailPrefix_slice
    (by
      change 3977 + PairedAllInlineTail.prefixTemplate.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 4859 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3977) =
    UInt256.ofNat 4859
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 4943 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4046) =
    UInt256.ofNat 4943
...
def tailJump : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4046)
  pc_eq := instructionPC_toNat 4046

theorem tailJump_pc : tailJump.pc = UInt256.ofNat 4943 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4046) =
    UInt256.ofNat 4943
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_schedule (s : State) (returnPC : UInt256) (p : Nat)
    ...
    GasSteps (DenseScheduleTemplate.scheduleEntry s (UInt256.ofNat 529) (UInt256.ofNat p) returnPC rest)
      { s with
        pc := UInt256.ofNat 764
        stack := returnPC :: rest
...
def gasSteps_startup (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1002)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 764, stack := rho}
      {s with pc := UInt256.ofNat 812, stack := PairedStartupTrace.resultStack s.memory rho} := by
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4859, stack := PairedAllInlineTail.entryStack q ret rho}
