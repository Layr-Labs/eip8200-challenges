...
def scheduleSite : GenericRoundSite Artifact.submissionArtifact .Osaka scheduleTemplate :=
  StackSiteBuilder.ofSlice scheduleTemplate 258 schedule_slice
    ...
    (by decide)
theorem schedule_pc : scheduleSite.startPC = UInt256.ofNat 425 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 258) = UInt256.ofNat 425
...
def gasSteps_schedule (s : State) (returnPC : UInt256) (p : Nat)
    ...
    GasSteps (DenseScheduleTemplate.scheduleEntry s (UInt256.ofNat 425) (UInt256.ofNat p) returnPC rest)
      {s with pc := UInt256.ofNat 658, stack := cache (PairedScheduleMemory.normalizedMemory s.memory (PairedScheduleData.extractedWord s.memory p)) ++ (returnPC :: rest), memory := PairedScheduleMemory.normalizedMemory s.memory (PairedScheduleData.extractedWord s.memory p), activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  have h := CachedSchedule.run_fullTemplate_natural s returnPC p rest hstack hrun hp hbound hsentinel
  have hpct : pcAfter (UInt256.ofNat 425) CachedSchedule.fullTemplate = UInt256.ofNat 658 := by rfl
  rw [hpct] at h
  have hl := runLocatedBlock_eq_runInstrSeq_site scheduleSite
    (DenseScheduleTemplate.scheduleEntry s (UInt256.ofNat 425) (UInt256.ofNat p) returnPC rest)
    schedule_pc.symm (by
...
theorem startup_slice :
    (Artifact.submissionArtifact.instructions.drop 430).take startupTemplate.length = startupTemplate := by rfl
def startupSite : GenericRoundSite Artifact.submissionArtifact .Osaka startupTemplate :=
  StackSiteBuilder.ofSlice startupTemplate 430 startup_slice
    (by change 430 + startupTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := startupTemplate) (by decide))
    (by decide)
theorem startup_pc : startupSite.startPC = UInt256.ofNat 658 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 430) = UInt256.ofNat 658
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem startup_endPC : startupSite.endPC = UInt256.ofNat 711 := by
...
def gasSteps_startup (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1002)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 658, stack := rho}
      {s with pc := UInt256.ofNat 711, stack := PairedStartupTrace.resultStack s.memory rho} := by
...
def tailTemplate : List Instr :=
  [ .op (.Dup ⟨2, by decide⟩),
    ...
    .op .JUMP ]
theorem tail_slice :
    (Artifact.submissionArtifact.instructions.drop 3898).take tailTemplate.length = tailTemplate := by rfl
def tailSite : GenericRoundSite Artifact.submissionArtifact .Osaka tailTemplate :=
  StackSiteBuilder.ofSlice tailTemplate 3898 tail_slice
    (by change 3898 + tailTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := tailTemplate) (by decide))
    (by decide)
theorem tail_pc : tailSite.startPC = UInt256.ofNat 4619 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3898) = UInt256.ofNat 4619
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem run_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    runInstrSeq tailTemplate {s with pc := UInt256.ofNat 4619, stack := cachedTailEntry s.memory q ret rho} =
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4619, stack := cachedTailEntry s.memory q ret rho}
