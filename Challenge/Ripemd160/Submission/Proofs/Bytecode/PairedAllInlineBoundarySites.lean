...
theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 3949 = 4995 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4018 = 5079 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 4995 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3949) =
    UInt256.ofNat 4995
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 5079 := by
...
theorem tailJump_pc : tailJump.pc = UInt256.ofNat 5079 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4018) =
    UInt256.ofNat 5079
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4995, stack := PairedAllInlineTail.entryStack q ret rho}
