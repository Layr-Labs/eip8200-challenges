...
theorem schedule_instructionPC :
    Artifact.submissionArtifact.instructionPC 297 = 536 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem schedule_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 462 = 771 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem scheduleSite_startPC : scheduleSite.startPC = UInt256.ofNat 536 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 297) =
    UInt256.ofNat 536
...
theorem startup_slice :
    (Artifact.submissionArtifact.instructions.drop 462).take
...
def startupSite :
  ...
  StackSiteBuilder.ofSlice SCanonicalStartup.template 462 startup_slice
    (by
      change 462 + SCanonicalStartup.template.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem startupSite_endPC : startupSite.endPC = UInt256.ofNat 819 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 495) =
    UInt256.ofNat 819
...
theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 3972 = 4862 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4041 = 4946 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def tailPrefixSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedAllInlineTail.prefixTemplate :=
  StackSiteBuilder.ofSlice PairedAllInlineTail.prefixTemplate 3972 tailPrefix_slice
    (by
      change 3972 + PairedAllInlineTail.prefixTemplate.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 4946 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4041) =
    UInt256.ofNat 4946
...
def tailJump : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4041)
  pc_eq := instructionPC_toNat 4041
