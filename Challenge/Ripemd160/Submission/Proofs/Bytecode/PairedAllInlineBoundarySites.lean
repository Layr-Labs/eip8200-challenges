...
def tailTemplate : List Instr :=
  [ .op (.Dup ⟨2, by decide⟩),
    ...
    .op .JUMP ]
theorem tail_slice :
    (Artifact.submissionArtifact.instructions.drop 3895).take tailTemplate.length = tailTemplate := by rfl
def tailSite : GenericRoundSite Artifact.submissionArtifact .Osaka tailTemplate :=
  StackSiteBuilder.ofSlice tailTemplate 3895 tail_slice
    (by change 3895 + tailTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := tailTemplate) (by decide))
    (by decide)
theorem tail_pc : tailSite.startPC = UInt256.ofNat 4629 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3895) = UInt256.ofNat 4629
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem run_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    runInstrSeq tailTemplate {s with pc := UInt256.ofNat 4629, stack := cachedTailEntry s.memory q ret rho} =
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4629, stack := cachedTailEntry s.memory q ret rho}
