...
theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 3962 = 4878 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4031 = 4962 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 4878 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3962) =
    UInt256.ofNat 4878
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 4962 := by
...
theorem tailJump_pc : tailJump.pc = UInt256.ofNat 4962 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4031) =
    UInt256.ofNat 4962
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4878, stack := PairedAllInlineTail.entryStack q ret rho}
