...
theorem schedule_slice :
    (Artifact.submissionArtifact.instructions.drop 290).take
      PairedDivMaskCache.fullTemplate.length = PairedDivMaskCache.fullTemplate := by
  rfl

theorem schedule_instructionPC :
    Artifact.submissionArtifact.instructionPC 290 = 499 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem schedule_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 455 = 736 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def scheduleSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedDivMaskCache.fullTemplate :=
  StackSiteBuilder.ofSlice PairedDivMaskCache.fullTemplate 290 schedule_slice
    (by
      change 290 + PairedDivMaskCache.fullTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem scheduleSite_startPC : scheduleSite.startPC = UInt256.ofNat 499 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 290) =
    UInt256.ofNat 499
  rw [schedule_instructionPC]

theorem scheduleSite_endPC : scheduleSite.endPC = UInt256.ofNat 736 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 455) =
    UInt256.ofNat 736
  rw [schedule_endInstructionPC]

theorem startup_slice :
    (Artifact.submissionArtifact.instructions.drop 455).take
...
def startupSite :
  ...
  StackSiteBuilder.ofSlice PairedNormalizedStartup.template 455 startup_slice
    (by
      change 455 + PairedNormalizedStartup.template.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem startupSite_startPC : startupSite.startPC = UInt256.ofNat 736 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 455) =
    UInt256.ofNat 736
  rw [startup_instructionPC]

theorem startupSite_endPC : startupSite.endPC = UInt256.ofNat 794 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 493) =
    UInt256.ofNat 794
  rw [startup_endInstructionPC]

theorem tailPrefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3960).take
      PairedAllInlineTail.prefixTemplate.length = PairedAllInlineTail.prefixTemplate := by
  rfl

theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 3960 = 4874 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4029 = 4958 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def tailPrefixSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedAllInlineTail.prefixTemplate :=
  StackSiteBuilder.ofSlice PairedAllInlineTail.prefixTemplate 3960 tailPrefix_slice
    (by
      change 3960 + PairedAllInlineTail.prefixTemplate.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 4874 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3960) =
    UInt256.ofNat 4874
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 4958 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4029) =
    UInt256.ofNat 4958
...
def tailJump : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4029)
  pc_eq := instructionPC_toNat 4029

theorem tailJump_pc : tailJump.pc = UInt256.ofNat 4958 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4029) =
    UInt256.ofNat 4958
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_schedule (s : State) (returnPC : UInt256) (p : Nat)
    ...
    GasSteps (DenseScheduleTemplate.scheduleEntry s (UInt256.ofNat 499) (UInt256.ofNat p) returnPC rest)
      { s with
        pc := UInt256.ofNat 736
        stack := returnPC :: rest
...
def gasSteps_startup (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1002)
    ...
    GasSteps {s with pc := UInt256.ofNat 736, stack := rho}
      {s with pc := UInt256.ofNat 794, stack := PairedStartupTrace.resultStack s.memory rho} := by
