...
theorem tailPrefix_slice :
    (Artifact.submissionArtifact.instructions.drop 4143).take
      PairedAllInlineTail.prefixTemplate.length = PairedAllInlineTail.prefixTemplate := by
  rfl

theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 4143 = 5032 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4212 = 5116 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def tailPrefixSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedAllInlineTail.prefixTemplate :=
  StackSiteBuilder.ofSlice PairedAllInlineTail.prefixTemplate 4143 tailPrefix_slice
    (by
      change 4143 + PairedAllInlineTail.prefixTemplate.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 5032 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4143) =
    UInt256.ofNat 5032
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 5116 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4212) =
    UInt256.ofNat 5116
...
def tailJump : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4212)
  pc_eq := instructionPC_toNat 4212

theorem tailJump_pc : tailJump.pc = UInt256.ofNat 5116 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4212) =
    UInt256.ofNat 5116
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 5032, stack := PairedAllInlineTail.entryStack q ret rho}
