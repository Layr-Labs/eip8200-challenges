...
/-- Exact schedule, startup, and tail windows of the frozen 5293-byte artifact. -/
...
theorem schedule_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 437 = 701 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def startupSite :
  ...
  StackSiteBuilder.ofSlice SCanonicalStartup.template 437 startup_slice
    (by
      change 437 + SCanonicalStartup.template.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem startupSite_endPC : startupSite.endPC = UInt256.ofNat 764 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 468) =
    UInt256.ofNat 764
  rw [startup_endInstructionPC]

theorem tailPrefix_slice :
    (Artifact.submissionArtifact.instructions.drop 4137).take
      PairedAllInlineTail.prefixTemplate.length = PairedAllInlineTail.prefixTemplate := by
  rfl

theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 4137 = 5027 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4206 = 5111 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def tailPrefixSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedAllInlineTail.prefixTemplate :=
  StackSiteBuilder.ofSlice PairedAllInlineTail.prefixTemplate 4137 tailPrefix_slice
    (by
      change 4137 + PairedAllInlineTail.prefixTemplate.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 5027 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4137) =
    UInt256.ofNat 5027
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 5111 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4206) =
    UInt256.ofNat 5111
...
def tailJump : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4206)
  pc_eq := instructionPC_toNat 4206

theorem tailJump_pc : tailJump.pc = UInt256.ofNat 5111 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4206) =
    UInt256.ofNat 5111
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_startup (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1002)
    ...
    GasSteps {s with pc := UInt256.ofNat 701, stack := rho}
      {s with pc := UInt256.ofNat 764, stack := PairedStartupTrace.resultStack s.memory rho} := by
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 5027, stack := PairedAllInlineTail.entryStack q ret rho}
