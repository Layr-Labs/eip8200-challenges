...
theorem schedule_slice :
    (Artifact.submissionArtifact.instructions.drop 273).take
      PairedDivMaskCache.fullTemplate.length = PairedDivMaskCache.fullTemplate := by
  rfl

theorem schedule_instructionPC :
    Artifact.submissionArtifact.instructionPC 273 = 522 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem schedule_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 432 = 746 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

def scheduleSite :
    GenericRoundSite Artifact.submissionArtifact .Osaka PairedDivMaskCache.fullTemplate :=
  StackSiteBuilder.ofSlice PairedDivMaskCache.fullTemplate 273 schedule_slice
    (by
      change 273 + PairedDivMaskCache.fullTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem scheduleSite_startPC : scheduleSite.startPC = UInt256.ofNat 522 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 273) =
    UInt256.ofNat 522
  rw [schedule_instructionPC]

theorem scheduleSite_endPC : scheduleSite.endPC = UInt256.ofNat 746 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 432) = UInt256.ofNat 746
  rw [schedule_endInstructionPC]

theorem startup_slice :
    (Artifact.submissionArtifact.instructions.drop 432).take
...
def startupSite :
  ...
  StackSiteBuilder.ofSlice SCanonicalStartup.template 432 startup_slice
    (by
      change 432 + SCanonicalStartup.template.length ≤ Artifact.submissionInstructions.length
      rw [Artifact.referenceInstructions_count]
      decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem
...
theorem startupSite_startPC : startupSite.startPC = UInt256.ofNat 746 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 432) = UInt256.ofNat 746
  rw [startup_instructionPC]

theorem startupSite_endPC : startupSite.endPC = UInt256.ofNat 789 := by
...
theorem tailPrefix_instructionPC :
    Artifact.submissionArtifact.instructionPC 4129 = 5059 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

theorem tailPrefix_endInstructionPC :
    Artifact.submissionArtifact.instructionPC 4198 = 5143 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem tailPrefixSite_startPC : tailPrefixSite.startPC = UInt256.ofNat 5059 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4129) = UInt256.ofNat 5059
  rw [tailPrefix_instructionPC]

theorem tailPrefixSite_endPC : tailPrefixSite.endPC = UInt256.ofNat 5143 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4198) = UInt256.ofNat 5143
...
theorem tailJump_pc : tailJump.pc = UInt256.ofNat 5143 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4198) = UInt256.ofNat 5143
  rw [tailPrefix_endInstructionPC]
...
def gasSteps_schedule (s : State) (returnPC : UInt256) (p : Nat)
    ...
    GasSteps (DenseScheduleTemplate.scheduleEntry s (UInt256.ofNat 522) (UInt256.ofNat p) returnPC (a :: b :: DenseScheduleTemplate.mask16 :: DenseScheduleTemplate.mask8 :: tail))
      { s with
        pc := UInt256.ofNat 746
...
def gasSteps_startup (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1002)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 746, stack := PairedDerivedStartup.lowerWord :: rho}
      {s with pc := UInt256.ofNat 789, stack := PairedStartupTrace.resultStack s.memory rho} := by
...
def gasSteps_tail (s : State) (ret : UInt256) (q : PairedTailTrace.Frame)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 5059, stack := PairedAllInlineTail.entryStack q ret rho}
