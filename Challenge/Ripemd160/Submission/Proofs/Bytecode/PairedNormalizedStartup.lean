...
unconditional equivalence for arbitrary memory is asserted. Narrowing each
address push preserves every eight-byte load window without padding operations.
...
def loadTemplate (address : Nat) : List Instr :=
  [.push ⟨1, by decide⟩ (UInt256.ofNat address), .op .MLOAD,
...
theorem template_bytes : (template.map Instr.size).sum = 58 := by
  norm_num [template, cacheTemplate, loadTemplate, push1, dup1, Instr.size]

theorem load_bytes (address : Nat) :
    ((loadTemplate address).map Instr.size).sum = 8 := by
...
theorem template_exactBytes : assembleBytes template =
  [0x63, 0xff, 0xff, 0xff, 0xff, 0x80, 0x60, 0x80, 0x1b, 0x80, 0x82, 0x17,
   ...
   0x60, 0xa0, 0x51, 0x80, 0x60, 0x80, 0x1b, 0x17,
   0x60, 0x80, 0x51, 0x80, 0x60, 0x80, 0x1b, 0x17,
   0x60, 0x60, 0x51, 0x80, 0x60, 0x80, 0x1b, 0x17,
   0x60, 0x40, 0x51, 0x80, 0x60, 0x80, 0x1b, 0x17,
   0x60, 0x20, 0x51, 0x80, 0x60, 0x80, 0x1b, 0x17] := by decide
