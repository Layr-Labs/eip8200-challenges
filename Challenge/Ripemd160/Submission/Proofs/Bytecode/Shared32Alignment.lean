...
theorem site_pc : site.startPC = UInt256.ofNat 457 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 250) = UInt256.ofNat 457
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_padding (s : State) (e : Env s) :
    Decode.isValidJumpDest s.executionEnv.code 4709 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3605 = 4737 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 457 F) (atState s 4709 F) := by
  apply PadLift.gasSteps_of_raw site (atState s 457 F) (atState s 4709 F)
