...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 248).take template.length = template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 248 actual_slice
    (by change 248 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 465 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 248) = UInt256.ofNat 465
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_padding (s : State) (e : Env s) :
    Decode.isValidJumpDest s.executionEnv.code 4763 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3619 = 4763 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3619 (by rfl)
  rw [hpc] at h
  rw [e.code]
  exact h
