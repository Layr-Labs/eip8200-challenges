...
abbrev template : List Instr :=
  [ .push ⟨1, by decide⟩ (UInt256.ofNat 63),
    .op .CALLDATASIZE,
    .op .AND,
    .push ⟨2, by decide⟩ (UInt256.ofNat 4763),
    .op .JUMPI ]

theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 249).take template.length = template := by rfl
...
theorem site_pc : site.startPC = UInt256.ofNat 465 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 249) = UInt256.ofNat 465
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_padding (s : State) (e : Env s) :
    Decode.isValidJumpDest s.executionEnv.code 4763 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3620 = 4763 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3620 (by rfl)
  rw [hpc] at h
  rw [e.code]
  exact h
...
def gasSteps (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 465 F) (atState s 4763 F) := by
  apply PadLift.gasSteps_of_raw site (atState s 465 F) (atState s 4763 F)
