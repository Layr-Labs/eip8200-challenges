...
abbrev template : List Instr :=
  [ .push ⟨1, by decide⟩ (UInt256.ofNat 63),
    .op .CALLDATASIZE,
    .op .AND,
    .push ⟨2, by decide⟩ (UInt256.ofNat 4778),
    .op .JUMPI ]
...
theorem site_pc : site.startPC = UInt256.ofNat 494 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 250) = UInt256.ofNat 494
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_padding (s : State) (e : Env s) :
    Decode.isValidJumpDest s.executionEnv.code 4778 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3603 = 4778 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 494 F) (atState s 4778 F) := by
  apply PadLift.gasSteps_of_raw site (atState s 494 F) (atState s 4778 F)
