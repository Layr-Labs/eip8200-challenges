...
abbrev template : List Instr :=
  [ .push ⟨1, by decide⟩ (UInt256.ofNat 63),
    .op .CALLDATASIZE,
    .op .AND,
    .push ⟨2, by decide⟩ (UInt256.ofNat 4757),
    .op .JUMPI ]
...
theorem valid_padding (s : State) (e : Env s) :
    Decode.isValidJumpDest s.executionEnv.code 4757 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3593 = 4757 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 500 F) (atState s 4757 F) := by
  apply PadLift.gasSteps_of_raw site (atState s 500 F) (atState s 4757 F)
