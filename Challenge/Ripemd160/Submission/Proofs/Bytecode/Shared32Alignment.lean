...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 275).take template.length = template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 275 actual_slice
    (by change 275 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide)) (by decide)

theorem site_pc : site.startPC = UInt256.ofNat 478 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 275) = UInt256.ofNat 478
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_padding (s : State) (e : Env s) :
    Decode.isValidJumpDest s.executionEnv.code 4705 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3660 = 4705 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
