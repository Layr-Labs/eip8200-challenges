...
abbrev template : List Instr :=
  [ .push ⟨1, by decide⟩ (UInt256.ofNat 63),
    .op .CALLDATASIZE,
    .op .AND,
    .push ⟨2, by decide⟩ (UInt256.ofNat 367),
    .op .JUMPI ]

theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 323).take template.length = template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 323 actual_slice
    (by change 323 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide)) (by decide)

theorem site_pc : site.startPC = UInt256.ofNat 765 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 323) = UInt256.ofNat 765
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_padding (s : State) (e : Env s) :
    Decode.isValidJumpDest s.executionEnv.code 367 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 205 = 367 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 765 F) (atState s 367 F) := by
  apply PadLift.gasSteps_of_raw site (atState s 765 F) (atState s 367 F)
