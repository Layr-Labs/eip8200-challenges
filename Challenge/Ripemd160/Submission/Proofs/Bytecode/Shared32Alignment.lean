...
abbrev template : List Instr :=
  [ .push ⟨1, by decide⟩ (UInt256.ofNat 63),
    .op .CALLDATASIZE,
    .op .AND,
    .push ⟨2, by decide⟩ (UInt256.ofNat 4754),
    .op .JUMPI ]
...
theorem valid_padding (s : State) (e : Env s) :
    Decode.isValidJumpDest s.executionEnv.code 4754 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3590 = 4754 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3590 (by rfl)
  rw [hpc] at h
  rw [e.code]
  exact h
...
def gasSteps (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 500 F) (atState s 4754 F) := by
  apply PadLift.gasSteps_of_raw site (atState s 500 F) (atState s 4754 F)
