...
def gasSteps_size_test (input : ByteArray) (sv ov : UInt256) (rest : List UInt256)
    ...
    GasSteps (stS input 236 (sv :: ov :: rest))
      (stS input 242 ((5180 : UInt256) ::
        UInt256.eq ov (UInt256.ofNat input.size) :: sv :: ov :: rest)) := by
  have a := soundS (opAt 123 .CALLDATASIZE)
    (blockOfS _ (pcFactS input 123 236 _ (by norm_num) (by rfl))
      (stepS_calldatasize input 236 (sv :: ov :: rest) (by simp; omega) (by norm_num)))
  have b := soundS (opAt 124 (.Dup ⟨2, by decide⟩))
    (blockOfS _ (pcFactS input 124 237 _ (by norm_num) (by rfl))
      (stepS_dup input 237 2 (by decide)
        (UInt256.ofNat input.size :: sv :: ov :: rest) ov
        (by rfl) (by simp; omega) (by norm_num)))
  have c := soundS (opAt 125 .EQ)
    (blockOfS _ (pcFactS input 125 238 _ (by norm_num) (by rfl))
      (stepS_eq input 238 ov (UInt256.ofNat input.size) (sv :: ov :: rest)
        (by simp; omega) (by norm_num)))
  have d := soundS (pushAt 126 2 5180)
    (blockOfS _ (pcFactS input 126 239 _ (by norm_num) (by rfl))
      (stepS_push input 239 2 5180
...
def gasSteps_size_skip (input : ByteArray) (sv ov : UInt256) (rest : List UInt256)
  ...
  exact (gasSteps_size_test input sv ov rest hlen).trans
    (soundS (opAt 127 .JUMPI)
      (blockOfS _ (pcFactS input 127 242 _ (by norm_num) (by rfl))
        (stepS_jumpi_fall input 242 5180 (UInt256.eq ov (UInt256.ofNat input.size))
...
def gasSteps_size_done (input : ByteArray) (sv ov : UInt256) (rest : List UInt256)
    ...
    GasSteps (stS input 236 (sv :: ov :: rest))
      (stS input 5180 (sv :: ov :: rest)) := by
  have hc : UInt256.isTrue (UInt256.eq ov (UInt256.ofNat input.size)) := by
    simp [UInt256.eq, UInt256.isTrue, heq]
  have hd : Decode.isValidJumpDest submissionBytecode 5180 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 4061 (by rfl)
  exact (gasSteps_size_test input sv ov rest hlen).trans
    (soundS (opAt 127 .JUMPI)
      (blockOfS _ (pcFactS input 127 242 _ (by norm_num) (by rfl))
        (stepS_jumpi_taken input 242 5180 5180 (UInt256.eq ov (UInt256.ofNat input.size))
