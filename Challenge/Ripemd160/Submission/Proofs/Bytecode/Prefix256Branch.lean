...
def gasSteps_size_test (input : ByteArray) (sv ov : UInt256) (rest : List UInt256)
    ...
    GasSteps (stS input 248 (sv :: ov :: rest))
      (stS input 253 ((155 : UInt256) ::
        UInt256.lt ov (UInt256.ofNat input.size) :: sv :: ov :: rest)) := by
  have a := soundS (opAt 157 .CALLDATASIZE)
    (blockOfS _ (pcFactS input 157 248 _ (by norm_num) (by rfl))
      (stepS_calldatasize input 248 (sv :: ov :: rest) (by simp; omega) (by norm_num)))
  have b := soundS (opAt 158 (.Dup ⟨2, by decide⟩))
  ...
  have c := soundS (opAt 159 .LT)
    (blockOfS _ (pcFactS input 159 250 _ (by norm_num) (by rfl))
      (stepS_lt input 250 ov (UInt256.ofNat input.size) (sv :: ov :: rest)
        (by simp; omega) (by norm_num)))
  have d := soundS (pushAt 160 1 155)
    (blockOfS _ (pcFactS input 160 251 _ (by norm_num) (by rfl))
      (stepS_push input 251 1 155
...
def gasSteps_size_more (input : ByteArray) (sv ov : UInt256) (rest : List UInt256)
  ...
  exact (gasSteps_size_test input sv ov rest hlen).trans
    (soundS (opAt 161 .JUMPI)
      (blockOfS _ (pcFactS input 161 253 _ (by norm_num) (by rfl))
        (stepS_jumpi_taken input 253 155 155 (UInt256.lt ov (UInt256.ofNat input.size))
          (sv :: ov :: rest) (by simp; omega) (by norm_num) rfl hc hd)))

def gasSteps_size_end (input : ByteArray) (sv ov : UInt256) (rest : List UInt256)
    (hlen : rest.length < 1020) (hc : ¬ UInt256.isTrue (UInt256.lt ov (UInt256.ofNat input.size))) :
    GasSteps (stS input 248 (sv :: ov :: rest))
      (stS input 254 (sv :: ov :: rest)) := by
  exact (gasSteps_size_test input sv ov rest hlen).trans
    (soundS (opAt 161 .JUMPI)
      (blockOfS _ (pcFactS input 161 253 _ (by norm_num) (by rfl))
        (stepS_jumpi_fall input 253 155 (UInt256.lt ov (UInt256.ofNat input.size))
