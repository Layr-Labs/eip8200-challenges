...
def eval (memory : ByteArray) (q : WordLane) : WordLane :=
  StaggerAlgorithm.step 42 (MachineState.readWord memory 900) q

theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired42.outputStack memory (input memory h4 q right (StaggerAlgorithm.physicalKey 41)) rho =
      stack memory h4 [ .d, .late 252, .late 342, .pair, .upper, .k, .c, .e, .a, .b, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] (eval memory q) right (StaggerAlgorithm.physicalKey 42) rho := by
  have hkb : StaggerAlgorithm.physicalKey 41 = UInt256.ofNat 136726760529788758926252426176837954510549114783656865 := by decide
  have hka : StaggerAlgorithm.physicalKey 42 = UInt256.ofNat 136726760529788758926252426176837954510549114783656865 := by decide
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3033, stack := stack s.memory h4 [ .d, .late 252, .late 342, .pair, .upper, .k, .b, .a, .e, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] q right (StaggerAlgorithm.physicalKey 41) rho}
      {s with pc := UInt256.ofNat 3067, stack := stack s.memory h4 [ .d, .late 252, .late 342, .pair, .upper, .k, .c, .e, .a, .b, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 42) rho} := by
  have g := StaggerRawPaired42.gasSteps s (input s.memory h4 q right (StaggerAlgorithm.physicalKey 41))
