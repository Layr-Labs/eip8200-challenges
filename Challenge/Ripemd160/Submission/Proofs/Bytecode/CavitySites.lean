...
private theorem leftSecond_slice :
    (A.instructions.drop 960).take (CavityParams.leftCode.drop 320).length =
...
def leftSecond : GenericRoundSite A .Osaka (CavityParams.leftCode.drop 320) :=
  StackSiteBuilder.ofSlice _ 960 leftSecond_slice (by
...
private theorem rightFirst_slice :
    (A.instructions.drop 407).take (CavityParams.rightCode.take 298).length =
...
def rightFirst : GenericRoundSite A .Osaka (CavityParams.rightCode.take 298) :=
  StackSiteBuilder.ofSlice _ 407 rightFirst_slice (by
...
private theorem rightSecond_slice :
    (A.instructions.drop 2502).take (CavityParams.rightCode.drop 298).length =
...
def rightSecond : GenericRoundSite A .Osaka (CavityParams.rightCode.drop 298) :=
  StackSiteBuilder.ofSlice _ 2502 rightSecond_slice (by
