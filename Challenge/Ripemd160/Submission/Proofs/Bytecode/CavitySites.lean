...
private theorem leftSecond_slice :
    (A.instructions.drop 966).take (CavityParams.leftCode.drop 320).length =
...
def leftSecond : GenericRoundSite A .Osaka (CavityParams.leftCode.drop 320) :=
  StackSiteBuilder.ofSlice _ 966 leftSecond_slice (by
...
private theorem rightFirst_slice :
    (A.instructions.drop 413).take (CavityParams.rightCode.take 298).length =
...
def rightFirst : GenericRoundSite A .Osaka (CavityParams.rightCode.take 298) :=
  StackSiteBuilder.ofSlice _ 413 rightFirst_slice (by
...
private theorem rightSecond_slice :
    (A.instructions.drop 2496).take (CavityParams.rightCode.drop 298).length =
...
def rightSecond : GenericRoundSite A .Osaka (CavityParams.rightCode.drop 298) :=
  StackSiteBuilder.ofSlice _ 2496 rightSecond_slice (by
