...
theorem site_pc : site.startPC = UInt256.ofNat 1384 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 978) = UInt256.ofNat 1384
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1384, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1422, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1384) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1384) template = UInt256.ofNat 1422 := by decide
  rw [hend] at hraw
