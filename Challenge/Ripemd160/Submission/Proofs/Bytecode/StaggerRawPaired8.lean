...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1063).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1063 actual_slice
    (by change 1063 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1592 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1063) = UInt256.ofNat 1592
