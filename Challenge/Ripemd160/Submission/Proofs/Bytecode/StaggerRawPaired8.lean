...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v1 (UInt256.shiftRight (UInt256.mul x.v8 x.v7) (UInt256.ofNat 28))),
    ...
    x.v5,
    x.v0,
    (UInt256.land x.v1 (UInt256.add x.v3 (UInt256.shiftRight (UInt256.mul x.v8 (UInt256.land x.v1 (UInt256.add x.v5 (UInt256.add (MachineState.readWord memory 756) (UInt256.add (UInt256.xor (UInt256.lor (UInt256.land x.v2 x.v7) x.v0) (UInt256.xor x.v2 (UInt256.xor x.v4 x.v7))) x.v6))))) (UInt256.ofNat 27)))),
    x.v8,
    x.v9,
    x.v10,
    x.v11,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1006).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1006 actual_slice
    (by change 1006 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1545 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1006) = UInt256.ofNat 1545
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1545, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1584, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1545) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1545) template = UInt256.ofNat 1584 := by decide
  rw [hend] at hraw
