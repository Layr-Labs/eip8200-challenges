...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1021).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1021 actual_slice
    (by change 1021 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1507 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1021) = UInt256.ofNat 1507
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 1507) template = UInt256.ofNat 1546 := by decide
  rw [hend] at hraw
