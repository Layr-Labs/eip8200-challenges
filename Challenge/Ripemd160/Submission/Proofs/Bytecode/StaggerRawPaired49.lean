...
theorem site_pc : site.startPC = UInt256.ofNat 3345 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2560) = UInt256.ofNat 3345
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3345, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3391, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3345) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3345) template = UInt256.ofNat 3391 := by decide
  rw [hend] at hraw
