...
theorem tail_result (h : Compression.HashState) (q : StaggerPersistentTailRaw.Input)
    (rho : List UInt256) :
...
theorem run_tail (s : State) (pc : UInt256) (h : Compression.HashState)
    ...
    runInstrSeq StaggerPersistentTailRaw.template
      {s with pc := pc, stack := StaggerPersistentTailRaw.stack0 (bind h q) rho} =
      some {s with
        pc := pcAfter pc StaggerPersistentTailRaw.template
        stack := frame (combine h q) q.off q.limit rho} := by
  simpa only [tail_result] using StaggerPersistentTailRaw.run_template s pc (bind h q) rho hstack hrun
