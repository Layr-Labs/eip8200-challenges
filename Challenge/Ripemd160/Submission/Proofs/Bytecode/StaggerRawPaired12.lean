...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1214).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1214 actual_slice
    (by change 1214 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1708 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1214) = UInt256.ofNat 1712
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 1708) template = UInt256.ofNat 1746 := by decide
  rw [hend] at hraw
