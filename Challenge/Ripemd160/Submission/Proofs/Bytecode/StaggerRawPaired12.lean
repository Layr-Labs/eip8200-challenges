...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1505, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1544, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1505) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1505) template = UInt256.ofNat 1544 := by decide
  rw [hend] at hraw
