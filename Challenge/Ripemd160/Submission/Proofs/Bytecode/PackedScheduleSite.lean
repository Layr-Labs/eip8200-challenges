...
private theorem packedSchedule_slice :
    (Artifact.submissionArtifact.instructions.drop 3281).take
...
def packedScheduleSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    DenseScheduleTemplate.denseBeforeJumpTemplate 3281
    packedSchedule_slice
    (by
      change 3281 + DenseScheduleTemplate.denseBeforeJumpTemplate.length ≤
...
private theorem packedSchedule_start_instructionPC :
    Artifact.submissionArtifact.instructionPC 3281 = 0x1239 :=
  QuadLayout.schedule_pc

private theorem packedSchedule_end_instructionPC :
    Artifact.submissionArtifact.instructionPC 3332 = 0x12f9 :=
  QuadLayout.scheduleJump_pc

@[simp] theorem packedScheduleSite_startPC :
    packedScheduleSite.startPC = UInt256.ofNat 0x1239 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3281) =
    UInt256.ofNat 0x1239
  rw [packedSchedule_start_instructionPC]

@[simp] theorem packedScheduleSite_endPC :
    packedScheduleSite.endPC = UInt256.ofNat 0x12f9 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3332) =
    UInt256.ofNat 0x12f9
...
def packedScheduleFinalJump :
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3332)
  pc_eq := pc_toNat_instructionPC 3332
...
@[simp] theorem packedScheduleFinalJump_pc :
    packedScheduleFinalJump.pc = UInt256.ofNat 0x12f9 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3332) =
    UInt256.ofNat 0x12f9
  rw [packedSchedule_end_instructionPC]
...
theorem packedScheduleFinalJump_site_end :
  ...
  calc
    packedScheduleFinalJump.pc = UInt256.ofNat 0x12f9 := packedScheduleFinalJump_pc
