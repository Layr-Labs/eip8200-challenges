...
private theorem packedSchedule_start_instructionPC :
  ...
  have hlength : byteLength
      (DenseScheduleTemplate.denseFullTemplate ++ packedScheduleAfter) = 1083 := by
    decide
  rw [packedScheduleBefore_length, hlength] at h
  have hsize : Artifact.submissionArtifact.code.size = 5338 := by
    change Challenge.Ripemd160.submissionBytecode.size = 5338
