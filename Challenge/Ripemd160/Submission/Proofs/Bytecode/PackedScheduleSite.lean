...
private theorem packedSchedule_slice :
    (Artifact.submissionArtifact.instructions.drop 3187).take
...
def packedScheduleSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    DenseScheduleTemplate.denseBeforeJumpTemplate 3187
    packedSchedule_slice
    (by
      change 3187 + DenseScheduleTemplate.denseBeforeJumpTemplate.length ≤
...
private theorem packedSchedule_start_instructionPC :
    Artifact.submissionArtifact.instructionPC 3187 = 0x118a :=
  QuadLayout.schedule_pc

private theorem packedSchedule_end_instructionPC :
    Artifact.submissionArtifact.instructionPC 3238 = 0x1250 :=
  QuadLayout.scheduleJump_pc

@[simp] theorem packedScheduleSite_startPC :
    packedScheduleSite.startPC = UInt256.ofNat 0x118a := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3187) =
    UInt256.ofNat 0x118a
  rw [packedSchedule_start_instructionPC]

@[simp] theorem packedScheduleSite_endPC :
    packedScheduleSite.endPC = UInt256.ofNat 0x1250 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3238) =
    UInt256.ofNat 0x1250
...
def packedScheduleFinalJump :
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3238)
  pc_eq := pc_toNat_instructionPC 3238
...
@[simp] theorem packedScheduleFinalJump_pc :
    packedScheduleFinalJump.pc = UInt256.ofNat 0x1250 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3238) =
    UInt256.ofNat 0x1250
  rw [packedSchedule_end_instructionPC]
...
theorem packedScheduleFinalJump_site_end :
  ...
  calc
    packedScheduleFinalJump.pc = UInt256.ofNat 0x1250 := packedScheduleFinalJump_pc
