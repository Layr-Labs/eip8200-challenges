...
private theorem packedSchedule_slice :
    (Artifact.submissionArtifact.instructions.drop 275).take
...
def packedScheduleSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    DenseScheduleTemplate.denseBeforeJumpTemplate 275
    packedSchedule_slice
    (by
      change 275 + DenseScheduleTemplate.denseBeforeJumpTemplate.length ≤
...
private theorem packedSchedule_start_instructionPC :
    Artifact.submissionArtifact.instructionPC 275 = 0x1dd :=
  QuadLayout.schedule_pc

private theorem packedSchedule_end_instructionPC :
    Artifact.submissionArtifact.instructionPC 326 = 0x29d :=
  QuadLayout.scheduleJump_pc

@[simp] theorem packedScheduleSite_startPC :
    packedScheduleSite.startPC = UInt256.ofNat 0x1dd := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 275) =
    UInt256.ofNat 0x1dd
  rw [packedSchedule_start_instructionPC]

@[simp] theorem packedScheduleSite_endPC :
    packedScheduleSite.endPC = UInt256.ofNat 0x29d := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 326) =
    UInt256.ofNat 0x29d
...
def packedScheduleFinalJump :
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 326)
  pc_eq := pc_toNat_instructionPC 326
...
@[simp] theorem packedScheduleFinalJump_pc :
    packedScheduleFinalJump.pc = UInt256.ofNat 0x29d := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 326) =
    UInt256.ofNat 0x29d
  rw [packedSchedule_end_instructionPC]
...
theorem packedScheduleFinalJump_site_end :
  ...
  calc
    packedScheduleFinalJump.pc = UInt256.ofNat 0x29d := packedScheduleFinalJump_pc
