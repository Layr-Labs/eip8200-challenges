...
private theorem packedSchedule_slice :
    (Artifact.submissionArtifact.instructions.drop 264).take
...
def packedScheduleSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    DenseScheduleTemplate.denseBeforeJumpTemplate 264
    packedSchedule_slice
    (by
      change 264 + DenseScheduleTemplate.denseBeforeJumpTemplate.length ≤
...
private theorem packedSchedule_start_instructionPC :
    Artifact.submissionArtifact.instructionPC 264 = 0x1e8 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

private theorem packedSchedule_end_instructionPC :
    Artifact.submissionArtifact.instructionPC 315 = 0x2a6 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] theorem packedScheduleSite_startPC :
    packedScheduleSite.startPC = UInt256.ofNat 0x1e8 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 264) =
    UInt256.ofNat 0x1e8
  rw [packedSchedule_start_instructionPC]

@[simp] theorem packedScheduleSite_endPC :
    packedScheduleSite.endPC = UInt256.ofNat 0x2a6 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 315) =
    UInt256.ofNat 0x2a6
