...
private theorem packedSchedule_slice :
    (Artifact.submissionArtifact.instructions.drop 3474).take
...
def packedScheduleSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    DenseScheduleTemplate.denseBeforeJumpTemplate 3474
    packedSchedule_slice
    (by
      change 3474 + DenseScheduleTemplate.denseBeforeJumpTemplate.length ≤
...
private theorem packedSchedule_start_instructionPC :
    Artifact.submissionArtifact.instructionPC 3474 = 0x1278 :=
  QuadLayout.schedule_pc

private theorem packedSchedule_end_instructionPC :
    Artifact.submissionArtifact.instructionPC 3525 = 0x1336 :=
  QuadLayout.scheduleJump_pc

@[simp] theorem packedScheduleSite_startPC :
    packedScheduleSite.startPC = UInt256.ofNat 0x1278 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3474) =
    UInt256.ofNat 0x1278
  rw [packedSchedule_start_instructionPC]

@[simp] theorem packedScheduleSite_endPC :
    packedScheduleSite.endPC = UInt256.ofNat 0x1336 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3525) =
    UInt256.ofNat 0x1336
...
def packedScheduleFinalJump :
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3525)
  pc_eq := pc_toNat_instructionPC 3525
...
@[simp] theorem packedScheduleFinalJump_pc :
    packedScheduleFinalJump.pc = UInt256.ofNat 0x1336 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3525) =
    UInt256.ofNat 0x1336
  rw [packedSchedule_end_instructionPC]
...
theorem packedScheduleFinalJump_site_end :
  ...
  calc
    packedScheduleFinalJump.pc = UInt256.ofNat 0x1336 := packedScheduleFinalJump_pc
