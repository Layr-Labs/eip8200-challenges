...
def poolWordV2 (memory : ByteArray) (i : Nat) : UInt256 :=
  if i ∈ [11] then UInt256.land poolMask (rawLoadV2 memory i) else rawLoadV2 memory i
        ...
        some {s with
          ...
          stack :=
            [ MachineState.readWord (copiedV2 s.memory) 138,
              ...
              UInt256.land poolMask (MachineState.readWord (copiedV2 s.memory) 146),
