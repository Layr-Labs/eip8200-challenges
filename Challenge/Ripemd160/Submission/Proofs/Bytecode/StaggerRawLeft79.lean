...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4664, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4692, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4664) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4664) template = UInt256.ofNat 4692 := by decide
  rw [hend] at hraw
