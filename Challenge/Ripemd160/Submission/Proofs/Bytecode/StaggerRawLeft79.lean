...
theorem site_pc : site.startPC = UInt256.ofNat 4595 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3753) = UInt256.ofNat 4595
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4595, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4623, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4595) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4595) template = UInt256.ofNat 4623 := by decide
  rw [hend] at hraw
