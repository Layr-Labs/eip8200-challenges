...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3528 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4566 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3528) = UInt256.ofNat 4566
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4566, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4595, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4566) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4566) template = UInt256.ofNat 4595 := by decide
  rw [hend] at hraw
