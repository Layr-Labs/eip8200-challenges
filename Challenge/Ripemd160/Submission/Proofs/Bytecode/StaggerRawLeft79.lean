...
theorem site_pc : site.startPC = UInt256.ofNat 4591 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3751) = UInt256.ofNat 4595
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4591, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4619, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4591) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4591) template = UInt256.ofNat 4619 := by decide
  rw [hend] at hraw
