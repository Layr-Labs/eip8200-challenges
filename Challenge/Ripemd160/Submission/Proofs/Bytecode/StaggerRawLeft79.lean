...
def actualOutput (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.shiftRight (UInt256.mul x.v13 x.v1) (UInt256.ofNat 28)),
    (UInt256.add x.v3 (UInt256.shiftRight (UInt256.mul x.v13 (UInt256.land x.v14 (UInt256.add x.v6 (UInt256.add (MachineState.readWord memory 198) (UInt256.add (UInt256.xor x.v7 (UInt256.lor x.v1 (UInt256.lnot x.v0))) x.v2))))) (UInt256.ofNat 32))),
    x.v0,
    x.v3,
    (UInt256.ofNat 28),
    x.v5,
...
theorem site_pc : site.startPC = UInt256.ofNat 4540 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3631) = UInt256.ofNat 4540
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4540, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4568, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4540) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4540) template = UInt256.ofNat 4568 := by decide
  rw [hend] at hraw
