...
theorem site_pc : site.startPC = UInt256.ofNat 4540 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3434) = UInt256.ofNat 4540
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4540, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4570, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4540) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4540) template = UInt256.ofNat 4570 := by decide
  rw [hend] at hraw
