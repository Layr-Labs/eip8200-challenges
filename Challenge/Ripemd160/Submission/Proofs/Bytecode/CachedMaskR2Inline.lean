...
/-- Left rounds 16--31 as one out-of-line sequence of four cached-mask quads. -/
def l1Code : List Instr := CachedMaskParams.leftCode 1

def l1Result (s : State) (w : Compression.EvmWorking) :
    Compression.EvmWorking :=
  fourResult (CachedMaskParams.left 1) s w

theorem l1_advances :
    ∀ instruction ∈ l1Code, PairMultiplyLift.Advances instruction :=
  four_advances (CachedMaskParams.left 1) 0

theorem l1_fits (s : State) (hactive : 25 ≤ s.activeWords.toNat) :
    ∀ k, (CachedMaskParams.left 1 k).Fits s :=
  CachedMaskParams.left_fits s 1 hactive

theorem run_l1 (s : State) (pc : UInt256)
    (w : Compression.EvmWorking) (rho : List UInt256)
    (hfit : ∀ k, (CachedMaskParams.left 1 k).Fits s)
    (hstack : rho.length < 1006) (hrun : s.halt = .Running) :
    runInstrSeq l1Code (stateAt s pc w (mask :: rho)) =
      some (stateAt s (pcAfter pc l1Code) (l1Result s w) (mask :: rho)) :=
  CachedMaskParams.run_leftCode 1 s pc w rho hfit hstack hrun

theorem l1_result_after16 (s : State) (word : Nat → UInt32)
    (w : Compression.EvmWorking) (hwords : DenseWordsAt s word) :
    l1Result s (leftRounds word 16 w) = leftRounds word 32 w :=
  CachedMaskParams.left_result_after s word w 1 hwords

theorem runLocated_l1 {artifact : ProgramArtifact} {fork : Fork}
    (site : GenericRoundSite artifact fork l1Code)
    (s : State) (w : Compression.EvmWorking) (rho : List UInt256)
    (hactive : 25 ≤ s.activeWords.toNat) (hstack : rho.length < 1006)
    (hrun : s.halt = .Running) :
    Stepper.runLocatedBlock site.path
      (stateAt s site.startPC w (mask :: rho)) =
      some (stateAt s site.endPC (l1Result s w) (mask :: rho)) := by
  have hend : site.endPC = pcAfter site.startPC l1Code := by
    have h := endPC_eq_pcAfter_sites site.sites site.startPC site.endPC
      site.head_eq site.end_eq site.contiguous
    rwa [site.instruction_eq] at h
  rw [PairMultiplyLift.runLocatedBlock_eq_raw site l1_advances _ rfl]
  rw [run_l1 s site.startPC w rho (l1_fits s hactive) hstack hrun, ← hend]

def gasSteps_l1_after16 {artifact : ProgramArtifact} {fork : Fork}
    (site : GenericRoundSite artifact fork l1Code)
    (s : State) (word : Nat → UInt32) (w : Compression.EvmWorking)
    (rho : List UInt256) (hwords : DenseWordsAt s word)
    (hactive : 25 ≤ s.activeWords.toNat) (hstack : rho.length < 1006)
    (hrun : s.halt = .Running)
    (hcode : s.executionEnv.code = artifact.code) (hfork : s.fork = fork)
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps
      (stateAt s site.startPC (leftRounds word 16 w) (mask :: rho))
      (stateAt s site.endPC (leftRounds word 32 w) (mask :: rho)) := by
  have h := runLocated_l1 site s (leftRounds word 16 w) rho
    hactive hstack hrun
  rw [l1_result_after16 s word w hwords] at h
  exact Stepper.runLocatedBlock_sound artifact fork site.path hcode hfork
    h hrun hnp

/-- The wide entry transfer uses PUSH7 because the appended site is past 0xfff. -/
structure WideBridge (artifact : ProgramArtifact) (fork : Fork) where
  push : LocatedSite artifact fork
  jump : LocatedSite artifact fork
  destination : LocatedSite artifact fork
  push_instr : push.located.instruction = .push ⟨7, by decide⟩ destination.pc
  jump_instr : jump.located.instruction = .op .JUMP
  destination_instr : destination.located.instruction = .op .JUMPDEST
  jump_at : jump.pc = push.pc + 8

def WideBridge.path {artifact : ProgramArtifact} {fork : Fork}
    (b : WideBridge artifact fork) : List (Stepper.Located artifact fork) :=
  [b.push.located, b.jump.located, b.destination.located]

theorem run_wide_bridge {artifact : ProgramArtifact} {fork : Fork}
    (b : WideBridge artifact fork) (s : State) (stack : List UInt256)
  ...
  have hjump : (b.push.pc + UInt256.ofNat 8).toNat =
      artifact.instructionPC b.jump.located.index := by
    change (b.push.pc + 8).toNat = _
    rw [← b.jump_at]
    exact b.jump.pc_eq
  have hcap : stack.length < 1024 := by omega
  have hcapPush : stack.length + 1 < 1024 := by omega
  simp [WideBridge.path, Stepper.runLocatedBlock, Stepper.runLocated,
...
def gasSteps_wide_bridge {artifact : ProgramArtifact} {fork : Fork}
    (b : WideBridge artifact fork) (s : State) (stack : List UInt256)
    (hstack : stack.length < 1023) (hcode : s.executionEnv.code = artifact.code)
  ...
  exact Stepper.runLocatedBlock_sound artifact fork b.path hcode hfork
    (run_wide_bridge b s stack hstack hcode hrun) hrun hnp

theorem l1Code_bytes :
    (l1Code.map Instr.size).sum = 576 := by
  decide

theorem l1Code_length : l1Code.length = 464 := by
  decide
