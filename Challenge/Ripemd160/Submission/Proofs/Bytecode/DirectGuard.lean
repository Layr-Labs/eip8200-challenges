...
theorem correct : Correct submissionBytecode := by
  ...
  · by_cases hp : input = PatternedInputData.patternedInput
    ...
    · by_cases hsize56 : input.size = 56
      · exact ShortPatternCorrect.correct56_from_patternedEntry input hfit hsize56
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize1 : input.size = 1
      · exact ShortPatternCorrect.correct1_from_patternedEntry input hfit hsize1
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize31 : input.size = 31
      · exact ShortPatternCorrect.correct31_from_patternedEntry input hfit hsize31
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize32 : input.size = 32
      · exact ShortPatternCorrect.correct32_from_patternedEntry input hfit hsize32
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize55 : input.size = 55
      · exact ShortPatternCorrect.correct55_from_patternedEntry input hfit hsize55
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize120 : input.size = 120
      · exact ShortPatternCorrect.correct120_from_patternedEntry input hfit hsize120
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize119 : input.size = 119
      · exact ShortPatternCorrect.correct119_from_patternedEntry input hfit hsize119
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize63 : input.size = 63
      · exact Patterned128Correct.correct_from_patternedEntry input hfit hsize63
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize64 : input.size = 64
      · exact ShortPatternCorrect.correct64_from_patternedEntry input hfit hsize64
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize65 : input.size = 65
      · exact ShortPatternCorrect.correct65_from_patternedEntry input hfit hsize65
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      by_cases hsize128 : input.size = 128
      · exact ShortPatternCorrect.correct128_from_patternedEntry input hfit hsize128
          (Patterned128Entry.gasSteps_hit input hfit (by omega))
      exact StackCorrect.correct input hfit
        (gasSteps_fallback input hfit h hp
          ⟨hsize56, hsize120, hsize63, hsize64, hsize65, hsize128, hsize119, hsize55, hsize1, hsize31, hsize32⟩ h256 hshort)
