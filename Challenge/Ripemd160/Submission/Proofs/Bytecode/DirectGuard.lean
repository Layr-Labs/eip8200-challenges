...
def checkEarlyPath : List Located :=
  [pushAt 2819 0 0, opAt 2820 .CALLDATALOAD,
   ...
   pushAt 2862 2 5300, opAt 2863 .JUMP]
...
theorem run_checkEarly (input : ByteArray)
  ...
  have hpatterned : Decode.isValidJumpDest submissionBytecode 5300 = true :=
  ...
  simp (config := { maxSteps := 1000000 })
    ...
    PatternedScan.atPC, atPC,
    referenceWord, htrue, hcond, hcleanup, hpatterned, BooleanSelect.xor_comm,
    Challenge.EvmProof.Stepper.runLocatedBlock, Challenge.EvmProof.Stepper.runLocated,
    Challenge.EvmProof.Stepper.runInstr, initialState,
    Challenge.EvmProof.Word.literal_eq_ofNat, Challenge.EvmProof.Word.succ_ofNat_mod,
    Challenge.EvmProof.Word.ofNat_add_mod, Challenge.EvmProof.Word.word_toNat_ofNat]
    ...
    · exact (Execution.gasSteps_start input).trans
        ((sound sizePath (run_size_match input hsize)).trans
          ((sound checkEarlyPath (run_checkEarly input href)).trans
            (by
              change GasSteps (PatternedScan.patternedEntry input) (fallbackState input)
              simpa [fallbackState, atPC, PatternedScan.fallbackState,
                PatternedScan.atPC] using
...
private def gasSteps_patterned :
    GasSteps (initialState submissionBytecode PatternedInputData.patternedInput 0)
      (PatternedScan.returnedState PatternedInputData.patternedInput) :=
  ...
  (Execution.gasSteps_start PatternedInputData.patternedInput).trans
    ((sound sizePath (run_size_match PatternedInputData.patternedInput
      PatternedInputData.patternedInput_size)).trans
