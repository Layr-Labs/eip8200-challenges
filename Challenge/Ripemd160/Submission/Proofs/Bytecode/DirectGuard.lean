...
def gasSteps_target :
  ...
  (Prefix256Entry.gasSteps_skip KnownInputData.targetInput
    (by unfold CalldataFits; rw [KnownInputData.targetInput_size]; decide)
...
def gasSteps_fallback (input : ByteArray) (hfit : CalldataFits input)
  ...
  · by_cases href : referenceWord input = KnownInputData.fullWord
    · exact (Prefix256Entry.gasSteps_skip input hfit h256).trans
...
theorem correct : Correct submissionBytecode := by
  ...
  · exact Prefix256Correct.correct input hfit h256
  ...
  · by_cases hp : input = PatternedInputData.patternedInput
    · subst input
      ...
      let trace :=
        (Prefix256Entry.gasSteps_skip PatternedInputData.patternedInput
          (by unfold CalldataFits; rw [PatternedInputData.patternedInput_size]; decide)
          (by rw [PatternedInputData.patternedInput_size]; decide)).trans
