...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 703).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 703 actual_slice
    (by change 703 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1366 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 703) = UInt256.ofNat 1366
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1366, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1408, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1366) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1366) template = UInt256.ofNat 1408 := by decide
  rw [hend] at hraw
