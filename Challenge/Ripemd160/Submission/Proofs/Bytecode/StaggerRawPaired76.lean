...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v1 (UInt256.shiftRight (UInt256.mul x.v8 x.v7) (UInt256.ofNat 22))),
    ...
    x.v5,
    x.v6,
    (UInt256.land x.v1 (UInt256.add x.v5 (UInt256.shiftRight (UInt256.mul x.v8 (UInt256.land x.v1 (UInt256.add x.v6 (UInt256.add (MachineState.readWord memory 70) (UInt256.add (UInt256.xor (UInt256.lor (UInt256.land x.v9 x.v7) x.v0) (UInt256.xor x.v9 (UInt256.xor x.v7 x.v4))) x.v3))))) (UInt256.ofNat 21)))),
    x.v8,
    x.v9,
    x.v10,
    x.v11,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3654).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3654 actual_slice
    (by change 3654 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4426 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3654) = UInt256.ofNat 4426
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4426, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4464, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4426) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4426) template = UInt256.ofNat 4464 := by decide
  rw [hend] at hraw
