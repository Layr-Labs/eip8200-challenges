...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3543 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4425 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3543) = UInt256.ofNat 4425
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4425, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4461, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4425) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4425) template = UInt256.ofNat 4461 := by decide
  rw [hend] at hraw
