...
def actualOutput (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.shiftRight (UInt256.mul x.v10 x.v9) (UInt256.ofNat 28)),
    ...
    x.v7,
    x.v8,
    (UInt256.land x.v3 (UInt256.add x.v7 (UInt256.shiftRight (UInt256.mul x.v10 (UInt256.land x.v3 (UInt256.add x.v8 (UInt256.add (MachineState.readWord memory 126) (UInt256.add (UInt256.xor (UInt256.xor x.v11 x.v6) (UInt256.xor x.v9 (UInt256.lor (UInt256.land x.v11 x.v9) x.v0))) x.v5))))) (UInt256.ofNat 27)))),
    x.v10,
    x.v11,
    x.v12,
    x.v13,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3645).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3645 actual_slice
    (by change 3645 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4536 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3645) = UInt256.ofNat 4536
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4536, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4571, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4536) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4536) template = UInt256.ofNat 4571 := by decide
  rw [hend] at hraw
