...
theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired32.outputStack memory (input memory h4 q right (StaggerAlgorithm.physicalKey 31)) rho =
      stack memory h4 [ .d, .literal 28, .literal 72, .pair, .upper, .k, .c, .e, .a, .b, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval memory q) right (StaggerAlgorithm.physicalKey 32) rho := by
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2645, stack := stack s.memory h4 [ .d, .literal 28, .literal 72, .pair, .upper, .a, .b, .k, .e, .c, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] q right (StaggerAlgorithm.physicalKey 31) rho}
      {s with pc := UInt256.ofNat 2704, stack := stack s.memory h4 [ .d, .literal 28, .literal 72, .pair, .upper, .k, .c, .e, .a, .b, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 32) rho} := by
