...
open EvmSemantics PairedHelperBooleanTrace
open PairedSynthCoreTrace (hoistedAlgorithmFold physicalKey)
def corePenultimateChain : CoreChain 727 [.a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] 4628 [.d, .k, .b, .c, .a, .e, .factor, .pair, .upper, .lower] :=
  .cons group0Block (
  .cons inline0Block (
  .cons inline1Block (
  .cons inline2Block (
...
theorem corePenultimateChain_eval (memory : ByteArray) (f : CoreFrame) :
    corePenultimateChain.eval memory f =
  ...
  let f86 : CoreFrame := ⟨hoistedAlgorithmFold memory 0 78 f.lane, physicalKey 4⟩
  ...
  have hc : CoreEvalCert corePenultimateChain memory f0 f83 :=
...
theorem prefix_after_penultimate (memory : ByteArray) (f : CoreFrame) :
    inline78Block.eval memory (corePenultimateChain.eval memory f) = corePrefixChain.eval memory f := by
  rw [corePenultimateChain_eval, corePrefixChain_eval]
  exact inline78Block_eval memory _
#print axioms corePenultimateChain_eval
#print axioms prefix_after_penultimate
