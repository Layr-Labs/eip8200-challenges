...
theorem site_pc : site.startPC = UInt256.ofNat 629 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 403) = UInt256.ofNat 629
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 629, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 708, stack := outputStack s.memory x rho, memory := outputMemory s.memory x} := by
  have hraw := run_actual s (UInt256.ofNat 629) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 629) template = UInt256.ofNat 708 := by decide
  rw [hend] at hraw
