...
theorem site_pc : site.startPC = UInt256.ofNat 708 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 471) = UInt256.ofNat 708
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 708, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 787, stack := outputStack s.memory x rho, memory := outputMemory s.memory x} := by
  have hraw := run_actual s (UInt256.ofNat 708) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 708) template = UInt256.ofNat 787 := by decide
  rw [hend] at hraw
