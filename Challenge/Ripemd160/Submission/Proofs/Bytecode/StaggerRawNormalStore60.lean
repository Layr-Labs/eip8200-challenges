...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 395).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 395 actual_slice
    (by change 395 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 652 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 395) = UInt256.ofNat 652
