...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound55
...
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound55.outputStack memory (input q k) rho =
      stack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 610) k q) (nextKey k) rho := by
  simp only [Table80RawRound55.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired80Algorithm.physicalKey 55) =
     Paired80Algorithm.physicalKey 56 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired80Algorithm.physicalKey 55) q = Paired80Algorithm.step 55 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 3509, stack := stack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 3554, stack :=
          stack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 610) k q) (nextKey k) rho} := by
   have gs := Table80RawRound55.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound55
