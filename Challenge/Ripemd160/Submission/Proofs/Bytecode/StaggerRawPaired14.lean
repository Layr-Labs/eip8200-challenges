...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1210).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1210 actual_slice
    (by change 1210 + template.length ≤ Artifact.submissionInstructions.length
