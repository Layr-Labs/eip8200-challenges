...
theorem site_pc : site.startPC = UInt256.ofNat 1666 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1207) = UInt256.ofNat 1666
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1666, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1724, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1666) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1666) template = UInt256.ofNat 1724 := by decide
  rw [hend] at hraw
