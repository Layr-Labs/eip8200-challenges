...
theorem site_pc : site.startPC = UInt256.ofNat 2333 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1824) = UInt256.ofNat 2333
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2333, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2371, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2333) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2333) template = UInt256.ofNat 2371 := by decide
  rw [hend] at hraw
