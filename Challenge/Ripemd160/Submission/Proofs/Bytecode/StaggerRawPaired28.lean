...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawPaired28
...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1763 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2330 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1763) = UInt256.ofNat 2330
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2330, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2367, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2330) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2330) template = UInt256.ofNat 2367 := by decide
  rw [hend] at hraw
  exact DenseScheduleLift.gasSteps_of_raw site _ _ hcode hfork hrun hnp site_pc.symm advances hraw
#print axioms gasSteps
end Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawPaired28
