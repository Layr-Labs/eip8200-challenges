...
private theorem template_slice :
    (A.instructions.drop 2323).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2323 template_slice (by
    change 2323 + template.length ≤ Artifact.submissionInstructions.length
