...
private theorem template_slice :
    (A.instructions.drop 2330).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2330 template_slice (by
    change 2312 + template.length ≤ Artifact.submissionInstructions.length
