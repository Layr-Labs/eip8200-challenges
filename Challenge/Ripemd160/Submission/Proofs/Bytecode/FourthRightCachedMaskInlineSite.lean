...
private theorem template_slice :
    (A.instructions.drop 2334).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2334 template_slice (by
    change 2316 + template.length ≤ Artifact.submissionInstructions.length
