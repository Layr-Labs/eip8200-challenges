...
private theorem template_slice :
    (A.instructions.drop 2329).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2329 template_slice (by
    change 2329 + template.length ≤ Artifact.submissionInstructions.length
