...
private theorem template_slice :
    (A.instructions.drop 2493).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2493 template_slice (by
    change 2493 + template.length ≤ Artifact.submissionInstructions.length
