...
private theorem template_slice :
    (A.instructions.drop 2504).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2504 template_slice (by
    change 2504 + template.length ≤ Artifact.submissionInstructions.length
