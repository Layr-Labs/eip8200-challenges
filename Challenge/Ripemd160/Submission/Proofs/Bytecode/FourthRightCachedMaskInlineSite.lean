...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.FourthRightCachedMaskInlineSite
...
private theorem template_slice :
    (A.instructions.drop 2316).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2316 template_slice (by
    change 2318 + template.length ≤ Artifact.submissionInstructions.length
...
def gasSteps_right3 (s : State) (word : Nat → UInt32)
    ...
    GasSteps (CachedMaskRoundCertificates.stateAt s (QuadSites.rightPC 3) working
      (a :: b :: c :: d :: e :: mask :: rho))
      (CachedMaskRoundCertificates.stateAt s (QuadSites.rightPC 4)
        (CachedMaskRoundCertificates.right4 word 3 working) (a :: b :: c :: d :: e :: mask :: rho)) := by
  have core := SingleCachedMaskInline.gasSteps_right (rightParams 0 3) site
...
#print axioms gasSteps_right3

end Challenge.Ripemd160.Submission.Proofs.Bytecode.FourthRightCachedMaskInlineSite
