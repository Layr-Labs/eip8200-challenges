...
private theorem template_slice :
    (A.instructions.drop 2341).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2341 template_slice (by
    change 2341 + template.length ≤ Artifact.submissionInstructions.length
