...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1371).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1371 actual_slice
    (by change 1371 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1965 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1371) = UInt256.ofNat 1965
