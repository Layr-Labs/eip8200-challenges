...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1379).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1379 actual_slice
    (by change 1379 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1908 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1379) = UInt256.ofNat 1912
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1908, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1973, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1908) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1908) template = UInt256.ofNat 1973 := by decide
  rw [hend] at hraw
