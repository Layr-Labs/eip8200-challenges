...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v1 (UInt256.shiftRight (UInt256.mul x.v8 x.v7) (UInt256.ofNat 22))),
    ...
    (UInt256.land x.v1 (UInt256.add x.v5 (UInt256.shiftRight (UInt256.mul x.v8 (UInt256.land x.v1 (UInt256.add (UInt256.ofNat 1872146977937934200243865289652633) (UInt256.add x.v13 (UInt256.add (UInt256.lor (UInt256.xor (UInt256.land (UInt256.lor x.v0 x.v7) (UInt256.xor x.v2 x.v4)) x.v0) (UInt256.land x.v7 x.v4)) x.v6))))) (UInt256.ofNat 25)))),
    x.v8,
    x.v9,
    x.v10,
    x.v11,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1372).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1372 actual_slice
    (by change 1372 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1847 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1372) = UInt256.ofNat 1847
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1847, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1902, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1847) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1847) template = UInt256.ofNat 1902 := by decide
  rw [hend] at hraw
