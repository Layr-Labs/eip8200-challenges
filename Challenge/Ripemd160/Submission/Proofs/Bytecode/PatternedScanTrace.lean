...
def tailState (input : ByteArray) (a : UInt256) : State :=
  { initialState submissionBytecode input 0 with
    pc := UInt256.ofNat 0xe0
    stack := UInt256.ofNat (scalarAt 31) :: UInt256.ofNat 992 :: a :: frame }

/-- The stub jumps here, and the guard answers or falls through.  -/
def patternedEntry (input : ByteArray) : State := atPC input 0x74

def hitState (input : ByteArray) : State := atPC input 0x102
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 0x74) = some (loopState input 0 0) := by
  have hpc3229 : Artifact.submissionArtifact.instructionPC 51 = 119 := by rfl
  have hpc3230 : Artifact.submissionArtifact.instructionPC 52 = 120 := by rfl
  have hpc3231 : Artifact.submissionArtifact.instructionPC 53 = 121 := by rfl
  have hpc3233 : Artifact.submissionArtifact.instructionPC 55 = 155 := by rfl
  have hpc3234 : Artifact.submissionArtifact.instructionPC 56 = 156 := by rfl
  have hpc3235 : Artifact.submissionArtifact.instructionPC 57 = 158 := by rfl
  have hpc3236 : Artifact.submissionArtifact.instructionPC 58 = 159 := by rfl
  have hpc3238 : Artifact.submissionArtifact.instructionPC 60 = 161 := by rfl
...
theorem run_word_straddle (input : ByteArray) (k : Nat) (a : UInt256) (hk : k < 31)
  ...
  have hcond : UInt256.isTrue ((UInt256.ofNat 224).eq
    ...
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 0x11d = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 140 (by rfl)
  have hdestN : Decode.isValidJumpDest submissionBytecode
      (UInt256.ofNat 285).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt (by norm_num : 285 < 2 ^ 256)]
    exact hdest
  have hdestL : Decode.isValidJumpDest submissionBytecode
      ((285 : UInt256)).toNat = true := by
    rw [show ((285 : UInt256)).toNat = 285 from by decide]
    exact hdest
  simp (config := { maxSteps := 800000 })
