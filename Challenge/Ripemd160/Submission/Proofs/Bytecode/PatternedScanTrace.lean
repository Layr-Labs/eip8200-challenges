...
def patternedEntry (input : ByteArray) : State := atPC input 0x1345
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 0x1345) = some (loopState input 0 0) := by
...
theorem run_word_straddle (input : ByteArray) (k : Nat) (a : UInt256) (hk : k < 31)
  ...
  have hdest : Decode.isValidJumpDest submissionBytecode 0x1448 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 3113 (by rfl)
