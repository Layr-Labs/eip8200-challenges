...
def patternedEntry (input : ByteArray) : State := atPC input 0x13ba

def hitState (input : ByteArray) : State := atPC input 0x144c
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 0x13ba) = some (loopState input 0 0) := by
  have hpc3326 : Artifact.submissionArtifact.instructionPC 3466 = 5053 := by rfl
  have hpc3327 : Artifact.submissionArtifact.instructionPC 3467 = 5054 := by rfl
  have hpc3328 : Artifact.submissionArtifact.instructionPC 3468 = 5055 := by rfl
  have hpc3330 : Artifact.submissionArtifact.instructionPC 3470 = 5089 := by rfl
  have hpc3331 : Artifact.submissionArtifact.instructionPC 3471 = 5090 := by rfl
  have hpc3332 : Artifact.submissionArtifact.instructionPC 3472 = 5092 := by rfl
  have hpc3333 : Artifact.submissionArtifact.instructionPC 3473 = 5093 := by rfl
  have hpc3335 : Artifact.submissionArtifact.instructionPC 3475 = 5095 := by rfl
...
theorem run_word_straddle (input : ByteArray) (k : Nat) (a : UInt256) (hk : k < 31)
  ...
  have hcond : UInt256.isTrue ((UInt256.ofNat 224).eq
    ...
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 0x1467 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 3558 (by rfl)
  have hdestN : Decode.isValidJumpDest submissionBytecode
      (UInt256.ofNat 5223).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt (by norm_num : 5223 < 2 ^ 256)]
    exact hdest
  have hdestL : Decode.isValidJumpDest submissionBytecode
      ((5223 : UInt256)).toNat = true := by
    rw [show ((5223 : UInt256)).toNat = 5223 from by decide]
    exact hdest
  simp (config := { maxSteps := 800000 })
