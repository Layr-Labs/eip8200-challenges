...
never normalizes the 5257-byte array. -/
...
def tailState (input : ByteArray) (a : UInt256) : State :=
  { initialState submissionBytecode input 0 with
    pc := UInt256.ofNat 0x1423
    stack := UInt256.ofNat (scalarAt 31) :: UInt256.ofNat 992 :: a :: frame }

/-- The stub jumps here, and the guard answers or falls through.  -/
def patternedEntry (input : ByteArray) : State := atPC input 0x13b3

def hitState (input : ByteArray) : State := atPC input 0x1445
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 0x13b3) = some (loopState input 0 0) := by
  have hpc3326 : Artifact.submissionArtifact.instructionPC 3461 = 5046 := by rfl
  have hpc3327 : Artifact.submissionArtifact.instructionPC 3462 = 5047 := by rfl
  have hpc3328 : Artifact.submissionArtifact.instructionPC 3463 = 5048 := by rfl
  have hpc3330 : Artifact.submissionArtifact.instructionPC 3465 = 5082 := by rfl
  have hpc3331 : Artifact.submissionArtifact.instructionPC 3466 = 5083 := by rfl
  have hpc3332 : Artifact.submissionArtifact.instructionPC 3467 = 5085 := by rfl
  have hpc3333 : Artifact.submissionArtifact.instructionPC 3468 = 5086 := by rfl
  have hpc3335 : Artifact.submissionArtifact.instructionPC 3470 = 5088 := by rfl
...
theorem run_word_straddle (input : ByteArray) (k : Nat) (a : UInt256) (hk : k < 31)
  ...
  have hcond : UInt256.isTrue ((UInt256.ofNat 224).eq
    ...
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 0x1460 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 3553 (by rfl)
  have hdestN : Decode.isValidJumpDest submissionBytecode
      (UInt256.ofNat 5216).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt (by norm_num : 5216 < 2 ^ 256)]
    exact hdest
  have hdestL : Decode.isValidJumpDest submissionBytecode
      ((5216 : UInt256)).toNat = true := by
    rw [show ((5216 : UInt256)).toNat = 5216 from by decide]
    exact hdest
  simp (config := { maxSteps := 800000 })
