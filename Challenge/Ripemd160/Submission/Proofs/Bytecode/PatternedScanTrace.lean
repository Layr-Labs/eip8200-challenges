...
def patternedEntry (input : ByteArray) : State := atPC input 0x13d6

def hitState (input : ByteArray) : State := atPC input 0x1468
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 0x13d6) = some (loopState input 0 0) := by
  have hpc3326 : Artifact.submissionArtifact.instructionPC 3466 = 5081 := by rfl
  have hpc3327 : Artifact.submissionArtifact.instructionPC 3467 = 5082 := by rfl
  have hpc3328 : Artifact.submissionArtifact.instructionPC 3468 = 5083 := by rfl
  have hpc3330 : Artifact.submissionArtifact.instructionPC 3470 = 5117 := by rfl
  have hpc3331 : Artifact.submissionArtifact.instructionPC 3471 = 5118 := by rfl
  have hpc3332 : Artifact.submissionArtifact.instructionPC 3472 = 5120 := by rfl
  have hpc3333 : Artifact.submissionArtifact.instructionPC 3473 = 5121 := by rfl
  have hpc3335 : Artifact.submissionArtifact.instructionPC 3475 = 5123 := by rfl
...
theorem run_word_straddle (input : ByteArray) (k : Nat) (a : UInt256) (hk : k < 31)
  ...
  have hdestN : Decode.isValidJumpDest submissionBytecode
      (UInt256.ofNat 5251).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt (by norm_num : 5251 < 2 ^ 256)]
    exact hdest
  have hdestL : Decode.isValidJumpDest submissionBytecode
      ((5251 : UInt256)).toNat = true := by
    rw [show ((5251 : UInt256)).toNat = 5251 from by decide]
    exact hdest
  simp (config := { maxSteps := 800000 })
