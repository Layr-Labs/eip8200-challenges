...
def tailState (input : ByteArray) (a : UInt256) : State :=
  { initialState submissionBytecode input 0 with
    pc := UInt256.ofNat 0xe5
    stack := UInt256.ofNat (scalarAt 31) :: UInt256.ofNat 992 :: a :: frame }

/-- The stub jumps here, and the guard answers or falls through.  -/
def patternedEntry (input : ByteArray) : State := atPC input 0x76

def hitState (input : ByteArray) : State := atPC input 0x107
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 0x76) = some (loopState input 0 0) := by
  have hpc3229 : Artifact.submissionArtifact.instructionPC 53 = 121 := by rfl
  have hpc3230 : Artifact.submissionArtifact.instructionPC 54 = 122 := by rfl
  have hpc3231 : Artifact.submissionArtifact.instructionPC 55 = 123 := by rfl
  have hpc3233 : Artifact.submissionArtifact.instructionPC 57 = 157 := by rfl
  have hpc3234 : Artifact.submissionArtifact.instructionPC 58 = 158 := by rfl
  have hpc3235 : Artifact.submissionArtifact.instructionPC 59 = 160 := by rfl
  have hpc3236 : Artifact.submissionArtifact.instructionPC 60 = 161 := by rfl
  have hpc3238 : Artifact.submissionArtifact.instructionPC 62 = 163 := by rfl
