...
def tailState (input : ByteArray) (a : UInt256) : State :=
  { initialState submissionBytecode input 0 with
    pc := UInt256.ofNat 0x19d
    stack := UInt256.ofNat (scalarAt 31) :: UInt256.ofNat 992 :: a :: frame }

/-- The stub jumps here, and the guard answers or falls through.  -/
def patternedEntry (input : ByteArray) : State := atPC input 0x12c

def hitState (input : ByteArray) : State := atPC input 0x1bf
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 0x12c) = some (loopState input 0 0) := by
  have hpc3326 : Artifact.submissionArtifact.instructionPC 161 = 302 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3327 : Artifact.submissionArtifact.instructionPC 162 = 303 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3328 : Artifact.submissionArtifact.instructionPC 163 = 304 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3330 : Artifact.submissionArtifact.instructionPC 165 = 338 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3331 : Artifact.submissionArtifact.instructionPC 166 = 339 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3332 : Artifact.submissionArtifact.instructionPC 167 = 341 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3333 : Artifact.submissionArtifact.instructionPC 168 = 342 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3335 : Artifact.submissionArtifact.instructionPC 170 = 344 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
...
theorem run_word_straddle (input : ByteArray) (k : Nat) (a : UInt256) (hk : k < 31)
  ...
  have hcond : UInt256.isTrue ((UInt256.ofNat 224).eq
    ...
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 0x1f5 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 261 (by rfl)
  have hdestN : Decode.isValidJumpDest submissionBytecode
      (UInt256.ofNat 501).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt (by norm_num : 501 < 2 ^ 256)]
    exact hdest
  have hdestL : Decode.isValidJumpDest submissionBytecode
      ((501 : UInt256)).toNat = true := by
    rw [show ((501 : UInt256)).toNat = 501 from by decide]
    exact hdest
  simp (config := { maxSteps := 800000 })
