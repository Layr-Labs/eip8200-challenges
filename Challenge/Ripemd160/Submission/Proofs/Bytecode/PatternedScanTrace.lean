...
def tailState (input : ByteArray) (a : UInt256) : State :=
  { initialState submissionBytecode input 0 with
    pc := UInt256.ofNat 0x176
    stack := UInt256.ofNat (scalarAt 31) :: UInt256.ofNat 992 :: a :: frame }

/-- The stub jumps here, and the guard answers or falls through.  -/
def patternedEntry (input : ByteArray) : State := atPC input 0x10b

def hitState (input : ByteArray) : State := atPC input 0x195
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 0x10b) = some (loopState input 0 0) := by
  have hpc3326 : Artifact.submissionArtifact.instructionPC 159 = 269 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3327 : Artifact.submissionArtifact.instructionPC 160 = 270 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3328 : Artifact.submissionArtifact.instructionPC 161 = 271 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3330 : Artifact.submissionArtifact.instructionPC 163 = 305 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3331 : Artifact.submissionArtifact.instructionPC 164 = 306 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3332 : Artifact.submissionArtifact.instructionPC 165 = 308 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3333 : Artifact.submissionArtifact.instructionPC 166 = 309 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  have hpc3335 : Artifact.submissionArtifact.instructionPC 168 = 311 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
...
theorem run_word_straddle (input : ByteArray) (k : Nat) (a : UInt256) (hk : k < 31)
  ...
  have hcond : UInt256.isTrue ((UInt256.ofNat 224).eq
    ...
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 0x1af = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 247 (by rfl)
  have hdestN : Decode.isValidJumpDest submissionBytecode
      (UInt256.ofNat 431).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt (by norm_num : 431 < 2 ^ 256)]
    exact hdest
  have hdestL : Decode.isValidJumpDest submissionBytecode
      ((431 : UInt256)).toNat = true := by
    rw [show ((431 : UInt256)).toNat = 431 from by decide]
    exact hdest
  simp (config := { maxSteps := 800000 })
