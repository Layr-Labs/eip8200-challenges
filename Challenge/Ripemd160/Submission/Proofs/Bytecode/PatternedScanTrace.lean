...
def tailState (input : ByteArray) (a : UInt256) : State :=
  { initialState submissionBytecode input 0 with
    pc := UInt256.ofNat 0x143d
    stack := UInt256.ofNat (scalarAt 31) :: UInt256.ofNat 992 :: a :: frame }

/-- The stub jumps here, and the guard answers or falls through.  -/
def patternedEntry (input : ByteArray) : State := atPC input 0x13d2

def hitState (input : ByteArray) : State := atPC input 0x145f
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 0x13d2) = some (loopState input 0 0) := by
  have hpc3326 : Artifact.submissionArtifact.instructionPC 3463 = 5076 := by rfl
  have hpc3327 : Artifact.submissionArtifact.instructionPC 3464 = 5077 := by rfl
  have hpc3328 : Artifact.submissionArtifact.instructionPC 3465 = 5078 := by rfl
  have hpc3330 : Artifact.submissionArtifact.instructionPC 3467 = 5112 := by rfl
  have hpc3331 : Artifact.submissionArtifact.instructionPC 3468 = 5113 := by rfl
  have hpc3332 : Artifact.submissionArtifact.instructionPC 3469 = 5115 := by rfl
  have hpc3333 : Artifact.submissionArtifact.instructionPC 3470 = 5116 := by rfl
  have hpc3335 : Artifact.submissionArtifact.instructionPC 3472 = 5118 := by rfl
...
theorem run_word_straddle (input : ByteArray) (k : Nat) (a : UInt256) (hk : k < 31)
  ...
  have hcond : UInt256.isTrue ((UInt256.ofNat 224).eq
    ...
    decide
  have hdest : Decode.isValidJumpDest submissionBytecode 0x147a = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 3551 (by rfl)
  have hdestN : Decode.isValidJumpDest submissionBytecode
      (UInt256.ofNat 5242).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt (by norm_num : 5242 < 2 ^ 256)]
    exact hdest
  have hdestL : Decode.isValidJumpDest submissionBytecode
      ((5242 : UInt256)).toNat = true := by
    rw [show ((5242 : UInt256)).toNat = 5242 from by decide]
    exact hdest
  simp (config := { maxSteps := 800000 })
