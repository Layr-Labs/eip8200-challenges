...
never normalizes the 5328-byte array. -/
...
def patternedEntry (input : ByteArray) : State := atPC input 5028

def hitState (input : ByteArray) : State := atPC input 5260
...
theorem run_setup (input : ByteArray) :
    run setupPath (atPC input 5028) = some (loopState input 0 0) := by
...
theorem run_word_straddle (input : ByteArray) (k : Nat) (a : UInt256) (hk : k < 31)
  ...
  have hdest : Decode.isValidJumpDest submissionBytecode 5287 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 3095 (by rfl)
  have hdestN : Decode.isValidJumpDest submissionBytecode
      (UInt256.ofNat 5287).toNat = true := by
    rw [Challenge.EvmProof.Word.word_toNat_ofNat,
      Nat.mod_eq_of_lt (by norm_num : 5287 < 2 ^ 256)]
    exact hdest
  have hdestL : Decode.isValidJumpDest submissionBytecode
      ((5287 : UInt256)).toNat = true := by
    rw [show ((5287 : UInt256)).toNat = 5287 from by decide]
    exact hdest
  simp (config := { maxSteps := 800000 })
