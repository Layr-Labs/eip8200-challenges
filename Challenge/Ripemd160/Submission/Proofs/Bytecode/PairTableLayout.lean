...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTableLayout
open EvmSemantics EvmSemantics.EVM Challenge.EvmProof
open PairTableMemory
...
theorem read_round (memory : ByteArray) (words : Nat → UInt256)
    ...
    (MachineState.readWord (resultMemory memory words) (10 * pairIndices[r]!)).toNat % 2 ^ 112 =
      (words Crypto.Ripemd160.r[r]!).toNat + (words Crypto.Ripemd160.rP[r]!).toNat * 2 ^ 80 := by
  obtain ⟨hpos, hlt, hleft, hright⟩ := layout_valid ⟨r, hr⟩
  change 1 ≤ pairIndices[r]! at hpos
  change pairIndices[r]! < 78 at hlt
  have h := read_pair memory (tableWords words) 0 78 pairIndices[r]!
    (by omega) (by omega)
...
theorem resultMemory_size (memory : ByteArray) (words : Nat → UInt256) :
    (resultMemory memory words).size = max memory.size 802 := by
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTableLayout
