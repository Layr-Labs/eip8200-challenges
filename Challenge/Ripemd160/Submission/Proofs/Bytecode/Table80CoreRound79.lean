...
def eval (message k : UInt256) (q : WordLane) : WordLane := Paired80FinalWord.rawStep 6 11 message k q
...
#print axioms output_eq
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 4565, stack := stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 4603, stack :=
