...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound79
...
def eval (message k : UInt256) (q : WordLane) : WordLane := Table80WideFinalWord.raw79 message k q
def nextKey (k : UInt256) : UInt256 := k
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound79.outputStack memory (input q k) rho =
      stack [.d, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 100) k q) (nextKey k) rho := by
  simp only [Table80RawRound79.outputStack, input, stack, List.map_cons, List.map_nil,
    ...
    Paired80WordGroupTwoHoist.hoistedBoolean, Table80WideFinalWord.raw79, Table80WideCoreBridge.wideWordShift,
...
#print axioms output_eq
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 4582, stack := stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 4612, stack :=
          stack [.d, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 100) k q) (nextKey k) rho} := by
   have gs := Table80RawRound79.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound79
