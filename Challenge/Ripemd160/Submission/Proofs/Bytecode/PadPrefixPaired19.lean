...
import Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRaw19
...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixPaired19
...
theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
    PadPrefixRaw19.outputStack memory (input memory h4 q right (StaggerAlgorithm.physicalKey 18)) rho =
      stack memory h4 [ .d, .literal 28, .cachedMessage 360, .pair, .upper, .a, .b, .e, .k, .c, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval memory q) right (StaggerAlgorithm.physicalKey 19) rho := by
  ...
  simp only [PadPrefixRaw19.outputStack, input, stack, List.map_cons, List.map_nil,
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 5894, stack := stack s.memory h4 [ .d, .literal 28, .cachedMessage 360, .pair, .upper, .e, .c, .a, .k, .b, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] q right (StaggerAlgorithm.physicalKey 18) rho}
      {s with pc := UInt256.ofNat 5947, stack := stack s.memory h4 [ .d, .literal 28, .cachedMessage 360, .pair, .upper, .a, .b, .e, .k, .c, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 19) rho} := by
  have g := PadPrefixRaw19.gasSteps s (input s.memory h4 q right (StaggerAlgorithm.physicalKey 18))
    rho hstack hrun hactive hz hcode hfork hnp
  rw [output_eq] at g
  exact g
#print axioms gasSteps
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixPaired19
