...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1804).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1804 actual_slice
    (by change 1804 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2464 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1804) = UInt256.ofNat 2464
