...
import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80ScratchZeroPool
import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80MaskConsumeStores
...
def normalTemplate : List Instr := ((Table80Setup.initialTemplate ++ endianTemplate) ++ poolTemplate) ++ Table80MaskConsume.storesTemplate
...
theorem run_normal (s : State) (pc returnPC : UInt256) (p : Nat) (rest : List UInt256)
  ...
  have h2 := run_endian s1 (pcAfter pc Table80Setup.initialTemplate)
  ...
  have h3 := run_pool s2 (pcAfter (pcAfter pc Table80Setup.initialTemplate) endianTemplate) returnPC rest hstack hrun ha
  ...
  have h4 := Table80MaskConsume.run_stores s2
    (pcAfter (pcAfter (pcAfter pc Table80Setup.initialTemplate) endianTemplate) poolTemplate)
