...
def gasSteps_prepare (s : State) (input : ByteArray) (i : Nat) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 529, stack := frame h (DriverTrace.blockOffsetWord i) limit rho}
      {scheduledState s i with pc := UInt256.ofNat 982, stack := frame h (DriverTrace.blockOffsetWord i) limit rho} := by
  ...
  · have he : s.executionEnv.calldata.size = off.toNat := by
    ...
    have gj := StaggerPadJump.gasSteps_jump q (frame h off limit rho)
      (by simp only [frame, List.length_append, List.length_cons, List.length_nil]; omega)
      hr hcode hfork hnp
    have gb' : GasSteps {s with pc := UInt256.ofNat 451, stack := frame h off limit rho}
        {q with pc := UInt256.ofNat 524, stack := frame h off limit rho} := by
      apply gb.cast rfl
