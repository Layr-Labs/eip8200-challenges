...
def gasSteps_prepare (s : State) (input : ByteArray) (i : Nat) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 524, stack := frame h (DriverTrace.blockOffsetWord i) limit rho}
      {scheduledState s i with pc := UInt256.ofNat 977, stack := frame h (DriverTrace.blockOffsetWord i) limit rho} := by
  ...
  · have he : s.executionEnv.calldata.size = off.toNat := by
    ...
    have gj := StaggerPadJump.gasSteps_jump q (frame h off limit rho)
      ...
      hr hcode hfork hnp
    have gb' : GasSteps {s with pc := UInt256.ofNat 446, stack := frame h off limit rho}
        {q with pc := UInt256.ofNat 519, stack := frame h off limit rho} := by
      apply gb.cast rfl
