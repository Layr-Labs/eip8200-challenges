...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1507).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1507 actual_slice
    (by change 1507 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 1962 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1507) = UInt256.ofNat 1962
