...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1389).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1389 actual_slice
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 1768 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1389) = UInt256.ofNat 1768
