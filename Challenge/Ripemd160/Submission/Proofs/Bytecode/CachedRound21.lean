...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1504).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1504 actual_slice
    (by change 1504 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 1969 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1504) = UInt256.ofNat 1969
