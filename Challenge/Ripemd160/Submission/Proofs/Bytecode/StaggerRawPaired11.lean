...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1132 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1628 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1132) = UInt256.ofNat 1628
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1628, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1672, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1628) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1628) template = UInt256.ofNat 1672 := by decide
  rw [hend] at hraw
