...
theorem site_pc : site.startPC = UInt256.ofNat 1589 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1155) = UInt256.ofNat 1589
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1589, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1635, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1589) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1589) template = UInt256.ofNat 1635 := by decide
  rw [hend] at hraw
