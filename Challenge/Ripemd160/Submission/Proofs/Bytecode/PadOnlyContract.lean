import Challenge.Ripemd160.Submission.Proofs.Bytecode.PadOnlyWords
...
theorem normalized_memory (memory : ByteArray) (input : ByteArray) (p : Nat)
    (hfit : input.size < 2^64) (hn : input.size % 64 = 0)
    (hbound : p + 64 < 2^256)
    (hblock : ScheduleCorrect.MessageBlockAt memory (UInt256.ofNat p)
      (Padding.paddedMessage input) input.size) (hs : SentinelCore.SentinelOK memory) :
    PairedScheduleMemory.normalizedMemory memory (PairedScheduleData.extractedWord memory p) =
      resultMemory memory (UInt256.ofNat input.size) := by
  rw [PairedScheduleContract.normalizedMemory_congr memory _ (padWords (UInt256.ofNat input.size))
    (extracted_words memory input p hfit hn hbound hblock)]
  exact memory_equiv memory (UInt256.ofNat input.size) hs

theorem resultMemory_read_cell (memory : ByteArray) (n : UInt256)
    (hs : SentinelCore.SentinelOK memory) (k : Nat) (hk : k < 16) :
    MachineState.readWord (resultMemory memory n) (PairedScheduleMemory.cell k) = padWords n k := by
  rw [← memory_equiv memory n hs, PairedScheduleMemory.read_normalized_cell _ _ _ (by omega),
    if_neg (by omega)]

theorem resultMemory_cache_tail (memory : ByteArray) (n : UInt256)
    (hs : SentinelCore.SentinelOK memory) :
    [MachineState.readWord (resultMemory memory n) 256,
     MachineState.readWord (resultMemory memory n) 384,
     MachineState.readWord (resultMemory memory n) 416,
     MachineState.readWord (resultMemory memory n) 352,
     MachineState.readWord (resultMemory memory n) 320] =
      [UInt256.ofNat 0, UInt256.ofNat 0, UInt256.ofNat 0, UInt256.ofNat 0, UInt256.ofNat 0] := by
  have h2 := resultMemory_read_cell memory n hs 2 (by decide)
  have h4 := resultMemory_read_cell memory n hs 4 (by decide)
  have h5 := resultMemory_read_cell memory n hs 5 (by decide)
  have h6 := resultMemory_read_cell memory n hs 6 (by decide)
  have h7 := resultMemory_read_cell memory n hs 7 (by decide)
  norm_num only [PairedScheduleMemory.cell, ScheduleLayout.perm_eval_2,
    ScheduleLayout.perm_eval_4, ScheduleLayout.perm_eval_5, ScheduleLayout.perm_eval_6,
    ScheduleLayout.perm_eval_7, padWords] at h2 h4 h5 h6 h7
  rw [h2, h4, h5, h6, h7]
  simp only [ite_false]

#print axioms resultMemory_cache_tail

#print axioms lowLength_ofNat
#print axioms highLength_ofNat
#print axioms padWords_eq_cryptoWords
#print axioms extracted_words
#print axioms normalized_memory
