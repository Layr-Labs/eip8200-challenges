...
def words (memory : ByteArray) (k : Nat) : UInt256 :=
  if k < 8 then StaggerScratch.dirtyWord memory 1087 k else UInt256.ofNat (highScalar k)

def scalars (memory : ByteArray) (k : Nat) : UInt32 :=
  UInt32.ofNat (if k < 8 then (PairedScheduleData.extractedWord memory 1087 k).toNat
    else highScalar k)
...
theorem words_split (memory : ByteArray) (k : Nat) (hk : k < 16) :
  ...
  by_cases hl : k < 8
  · have hs := StaggerScratch.dirtyWord_split memory 1087 k
    have hb := PairedScheduleData.extractedWord_bound memory 1087 k
...
theorem words_clean (memory : ByteArray) (k : Nat) (hk0 : 3 ≤ k) (hk1 : k < 16) :
  ...
  · have hd : ¬(k = 1 ∨ k = 2) := by omega
    ...
    exact PairedScheduleData.extractedWord_bound memory 1087 k
...
def wordsRaw (memory : ByteArray) (k : Nat) : UInt256 :=
  StaggerScratch.poolWordD
    (StaggerScratch.scratchMemory memory
      (PairedScheduleData.reversedWord (MachineState.readWord memory 1087)) highWord) k
...
theorem wordsRaw_eq_high (memory : ByteArray) (k : Nat) (hk0 : 3 ≤ k) (hk1 : k < 16) :
  ...
  by_cases hl : k < 8
  · rw [wordsRaw, words, if_pos hl, pool_lower_high_irrelevant memory 1087 k highWord hl]
    exact StaggerScratch.poolWordD_eq_dirty_high memory 1087 k hk1 (by omega)
...
theorem wordsRaw_mod (memory : ByteArray) (k : Nat) (hk : k < 16)
  ...
  · rw [wordsRaw, words, if_pos hl]
    exact pool_lower memory 1087 k highWord hl hzero
...
theorem writer_memory (memory : ByteArray) (hgap : PairStoreGap.GapClear memory)
    ...
    Pair13WriterRaw.writerMemory
      (fanMemory memory
        (PairedScheduleData.reversedWord (MachineState.readWord memory 1087)) highWord)
...
theorem copied_writer_memory (input : ByteArray) :
    Pair13WriterRaw.writerMemory
      (fanMemory (copiedMemory input)
        (PairedScheduleData.reversedWord (MachineState.readWord (copiedMemory input) 1087)) highWord)
