...
def inputStack (x : Input) (rho : List UInt256) : List UInt256 :=
  [ x.v0, x.v1, x.v2, x.v3, (UInt256.ofNat 28), (UInt256.ofNat 72), x.v6, x.v7, x.v8, x.v9, x.v10, x.v11, x.v12, x.v13, x.v14, x.v15, x.v16, x.v17, x.v18, x.v19 ] ++ rho
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3723).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3723 actual_slice
    (by change 3723 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4606 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3723) = UInt256.ofNat 4606
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4606, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4634, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4606) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4606) template = UInt256.ofNat 4634 := by decide
  rw [hend] at hraw
