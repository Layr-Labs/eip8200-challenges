...
def actualOutput (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.shiftRight (UInt256.mul x.v13 x.v7) (UInt256.ofNat 28)),
    ...
    x.v5,
    x.v6,
    (UInt256.add x.v2 (UInt256.shiftRight (UInt256.mul x.v13 (UInt256.land x.v14 (UInt256.add x.v6 (UInt256.add (MachineState.readWord memory 612) (UInt256.add (UInt256.xor x.v1 (UInt256.lor (UInt256.lnot x.v0) x.v7)) x.v3))))) (UInt256.ofNat 33))),
    x.v8,
    x.v9,
    x.v10,
    x.v11,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3705).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3705 actual_slice
    (by change 3705 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4635 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3705) = UInt256.ofNat 4635
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4635, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4664, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4635) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4635) template = UInt256.ofNat 4664 := by decide
  rw [hend] at hraw
