...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3426 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4519 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3426) = UInt256.ofNat 4519
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4519, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4547, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4519) x rho hstack haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4519) template = UInt256.ofNat 4547 := by decide
  rw [hend] at hraw
