...
theorem site_pc : site.startPC = UInt256.ofNat 4511 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3408) = UInt256.ofNat 4511
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4511, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4539, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4511) x rho hstack haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4511) template = UInt256.ofNat 4539 := by decide
  rw [hend] at hraw
