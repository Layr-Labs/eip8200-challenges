...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.shiftRight (UInt256.mul x.v12 x.v6) (UInt256.ofNat 28)),
    x.v1,
    x.v2,
    x.v0,
    x.v4,
    x.v5,
    (UInt256.add x.v2 (UInt256.shiftRight (UInt256.mul x.v12 (UInt256.land x.v13 (UInt256.add x.v5 (UInt256.add (MachineState.readWord memory 144) (UInt256.add (UInt256.xor (UInt256.lor (UInt256.lnot x.v0) x.v6) x.v1) x.v3))))) (UInt256.ofNat 33))),
    x.v7,
    x.v8,
    x.v9,
    x.v10,
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4609, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4638, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4609) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4609) template = UInt256.ofNat 4638 := by decide
  rw [hend] at hraw
