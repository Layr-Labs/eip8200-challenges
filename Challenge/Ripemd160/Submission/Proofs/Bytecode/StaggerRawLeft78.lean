...
theorem site_pc : site.startPC = UInt256.ofNat 4522 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3419) = UInt256.ofNat 4522
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4522, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4550, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4522) x rho hstack haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4522) template = UInt256.ofNat 4550 := by decide
  rw [hend] at hraw
