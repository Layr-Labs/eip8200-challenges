...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.shiftRight (UInt256.mul x.v11 x.v5) (UInt256.ofNat 22)),
    x.v1,
    x.v2,
    x.v0,
    x.v4,
    (UInt256.land x.v12 (UInt256.add x.v2 (UInt256.shiftRight (UInt256.mul x.v11 (UInt256.land x.v12 (UInt256.add x.v4 (UInt256.add (MachineState.readWord memory 80) (UInt256.add (UInt256.xor (UInt256.lor (UInt256.lnot x.v0) x.v5) x.v1) x.v3))))) (UInt256.ofNat 27)))),
    x.v6,
    x.v7,
    x.v8,
    x.v9,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3739).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3739 actual_slice
    (by change 3739 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4520 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3739) = UInt256.ofNat 4520
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4520, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4551, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4520) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4520) template = UInt256.ofNat 4551 := by decide
  rw [hend] at hraw
