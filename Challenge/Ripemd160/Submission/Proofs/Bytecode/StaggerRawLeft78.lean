...
theorem site_pc : site.startPC = UInt256.ofNat 4515 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3412) = UInt256.ofNat 4515
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4515, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4543, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4515) x rho hstack haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4515) template = UInt256.ofNat 4543 := by decide
  rw [hend] at hraw
