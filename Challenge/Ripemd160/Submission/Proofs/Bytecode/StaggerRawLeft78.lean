...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3480 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4772 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3480) = UInt256.ofNat 4772
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4772, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4800, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4772) x rho hstack haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4772) template = UInt256.ofNat 4800 := by decide
  rw [hend] at hraw
