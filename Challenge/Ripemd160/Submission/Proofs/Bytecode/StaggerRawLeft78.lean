...
theorem site_pc : site.startPC = UInt256.ofNat 4510 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3407) = UInt256.ofNat 4510
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4510, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4538, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4510) x rho hstack haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4510) template = UInt256.ofNat 4538 := by decide
  rw [hend] at hraw
