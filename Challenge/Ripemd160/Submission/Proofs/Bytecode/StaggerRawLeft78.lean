...
theorem site_pc : site.startPC = UInt256.ofNat 4632 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3704) = UInt256.ofNat 4632
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4632, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4661, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4632) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4632) template = UInt256.ofNat 4661 := by decide
  rw [hend] at hraw
