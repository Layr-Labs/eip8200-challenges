...
private theorem run_generated (s : State) (pc : UInt256) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 900) (haliasR : x.v8 = x.v3) (hrun : s.halt = .Running)
  ...
  simp (discharger := omega) [template, inputStack, actualOutput,
    ...
    List.exchange, List.getElem?_cons_zero, Nat.add_assoc, hrun, hbase, hzero, hcap, haliasR,
...
theorem run_actual (s : State) (pc : UInt256) (x : Input) (rho : List UInt256)
  ...
  simpa only [actualOutput_eq] using run_generated s pc x rho hstack haliasR hrun hactive
...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3406 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4507 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3406) = UInt256.ofNat 4507
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4507, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4535, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4507) x rho hstack haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4507) template = UInt256.ofNat 4535 := by decide
  rw [hend] at hraw
