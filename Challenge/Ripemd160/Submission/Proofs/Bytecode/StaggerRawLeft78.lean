...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3607).take template.length = template := by rfl
...
theorem site_pc : site.startPC = UInt256.ofNat 4524 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3607) = UInt256.ofNat 4524
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4524, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4553, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4524) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4524) template = UInt256.ofNat 4553 := by decide
  rw [hend] at hraw
