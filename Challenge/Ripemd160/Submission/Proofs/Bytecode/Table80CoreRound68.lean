...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound68
...
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound68.outputStack memory (input q k) rho =
      stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 570) k q) (nextKey k) rho := by
  simp only [Table80RawRound68.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired80Algorithm.physicalKey 68) =
     Paired80Algorithm.physicalKey 69 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired80Algorithm.physicalKey 68) q = Paired80Algorithm.step 68 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 4108, stack := stack [.d, .k, .b, .c, .a, .e, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 4152, stack :=
          stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 570) k q) (nextKey k) rho} := by
   have gs := Table80RawRound68.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound68
