...
theorem suffix_eq (s : State) (h : Compression.HashState) (q : WordLane)
    ...
    {s with pc := UInt256.ofNat 4572, stack := StaggerPersistentTailRaw.entryStack (StaggerPersistentFrame.bind h
...
def gasSteps (s : State) (h : Compression.HashState) (q : WordLane)
    ...
    GasSteps (StaggerCore.suffixState s (initial h).e q
      (StaggerPersistentPackBridge.suffix (initial h) off limit rho))
      {s with pc := UInt256.ofNat 4621, stack := StaggerPersistentFrame.frame (combine h (StaggerCoreModel.epilogue s.memory q) q) off limit rho} := by
  rw [suffix_eq]
