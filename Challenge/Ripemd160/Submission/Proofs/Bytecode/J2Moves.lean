...
structure Moves (s : State) (rho : List UInt256) where
  ...
  first (f : J2Raw.Frame) : GasSteps (atState s 188 (frame f rho))
    (atState s (if f.full.toNat=0 then 221 else 193) (frame f rho))
  ...
  normalGuard (f : J2Raw.Frame) : GasSteps (atState s 214 (frame f rho))
    (atState s (if f.off.toNat<f.full.toNat then 193 else 221) (frame f rho))
  tail (f : J2Raw.Frame) : GasSteps (atState s 221 (frame f rho))
    (atState s 238 (frame (tailResult s f) rho))
  finish (f : J2Raw.Frame) (hlen : f.len = UInt256.ofNat s.executionEnv.calldata.size) : GasSteps (atState s 238 (frame f rho))
    (atState s (if f.stop.toNat=f.len.toNat then 297 else 245) (frame f rho))
  transition (f : J2Raw.Frame) (hlen : f.len = UInt256.ofNat s.executionEnv.calldata.size) : GasSteps (atState s 245 (frame f rho))
    (atState s 288 (frame (transitionResult f) rho))
  transitionGuard (f : J2Raw.Frame) : GasSteps (atState s 288 (frame f rho))
    (atState s (if f.off.toNat<f.full.toNat then 193 else 294) (frame f rho))
  toTail (f : J2Raw.Frame) : GasSteps (atState s 294 (frame f rho)) (atState s 221 (frame f rho))
  result (f : J2Raw.Frame) : GasSteps (atState s 297 (frame f rho))
    (atState s (if f.acc.toNat=0 then 302 else 339) (finishRest f rho))
