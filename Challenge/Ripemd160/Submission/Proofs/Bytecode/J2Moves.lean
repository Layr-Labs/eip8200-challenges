...
structure Moves (s : State) (rho : List UInt256) where
  ...
  transition (f : J2Raw.Frame) (hlen : f.len = UInt256.ofNat s.executionEnv.calldata.size) : GasSteps (atState s 368 (frame f rho))
    (atState s 402 (frame (transitionResult f) rho))
  transitionGuard (f : J2Raw.Frame) : GasSteps (atState s 402 (frame f rho))
    (atState s (if f.off.toNat<f.full.toNat then 321 else 409) (frame f rho))
  toTail (f : J2Raw.Frame) : GasSteps (atState s 409 (frame f rho)) (atState s 348 (frame f rho))
