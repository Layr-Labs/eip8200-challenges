...
structure Moves (s : State) (rho : List UInt256) where
  ...
  finish (f : J2Raw.Frame) (hlen : f.len = UInt256.ofNat s.executionEnv.calldata.size) : GasSteps (atState s 237 (frame f rho))
    (atState s (if f.stop.toNat=f.len.toNat then 296 else 244) (frame f rho))
  ...
  result (f : J2Raw.Frame) : GasSteps (atState s 296 (frame f rho))
    (atState s (if f.acc.toNat=0 then 301 else 338) (finishRest f rho))
