...
def storePath : List Located :=
  [opAt 246 .CALLDATASIZE, pushAt 247 2 376, opAt 248 .EQ,
   ...
   pushAt 253 0 0, opAt 254 .MSTORE]

def finishPath : List Located := [pushAt 256 0 0, opAt 257 .RETURN]

def storedState (input : ByteArray) : State :=
  { atPC input 457 with memory := answerMemory, activeWords := UInt256.ofNat 1 }

def sizedState (input : ByteArray) : State :=
  { storedState input with pc := UInt256.ofNat 458, stack := [UInt256.ofNat 32] }
...
def gasSteps_return :
  ...
  have hop : (storedState patternedInput).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState patternedInput) 255
