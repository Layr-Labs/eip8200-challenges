...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1323).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1323 actual_slice
    (by change 1323 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 1765 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1323) = UInt256.ofNat 1765
