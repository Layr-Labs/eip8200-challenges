...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1217).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1217 actual_slice
    (by change 1217 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 1594 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1217) = UInt256.ofNat 1594
