...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3196).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3196 actual_slice
    (by change 3196 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3906 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3196) = UInt256.ofNat 3906
