...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3184).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3184 actual_slice
    (by change 3184 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3915 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3184) = UInt256.ofNat 3915
