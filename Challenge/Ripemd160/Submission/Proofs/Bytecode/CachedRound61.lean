...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3180).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3180 actual_slice
    (by change 3180 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3921 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3180) = UInt256.ofNat 3921
