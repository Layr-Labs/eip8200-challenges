...
def inputAddress (index : Nat) : Nat := 832 + 32 * index
...
theorem runInstrSeq_fastPackTemplate
  ...
  have h1raw := runInstrSeq_fastPackStep t0 t0.pc 864
    (inputWord s 0) rest (by norm_num) hstack (by simpa [t0] using hrun)
  have h1 :
      runInstrSeq (fastPackStep 864) t0 =
        some (afterFastPackStep t0 t0.pc 864 (inputWord s 0) rest) := by
    simpa [t0, afterFastLoad, inputWord] using h1raw
  let t1 : State := afterFastPackStep t0 t0.pc 864 (inputWord s 0) rest
  have hprefix1 :
      runInstrSeq (([DenseScheduleTemplate.op .JUMPDEST] ++ fastLoad0) ++
          fastPackStep 864)
  ...
  have h2raw := runInstrSeq_fastPackStep t1 t1.pc 896
    (packAppend (inputWord s 0) (inputWord s 1)) rest
    (by norm_num) hstack (by simpa [t1, t0] using hrun)
  have h2 :
      runInstrSeq (fastPackStep 896) t1 =
        some (afterFastPackStep t1 t1.pc 896
