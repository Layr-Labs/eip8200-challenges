...
def gasSteps_partial (s : State) (e : Env s) (f : RecognitionBodyRaw.Frame) (rho : List UInt256) (hs : rho.length ≤ 990) :
    GasSteps (atState s 297 (frame f rho)) (atState s 313 (frame (RecognitionControlRaw.partialResult s f) rho)) := by
  apply partialWord.lift s _ e (frame f rho)
  have h := RecognitionControlRaw.run_partial s (UInt256.ofNat 297) f rho hs e.run
...
def gasSteps_reset (s : State) (e : Env s) (f : RecognitionBodyRaw.Frame) (rho : List UInt256) (hs : rho.length ≤ 990) :
    GasSteps (atState s 282 (frame f rho)) (atState s 285 (frame ({f with stop := f.full}) rho)) := by
  apply reset.lift s _ e (frame f rho)
  have h := RecognitionBranchRaw.run_reset s (UInt256.ofNat 282) f rho hs e.run
...
def gasSteps_segment_yes (s : State) (e : Env s) (f : RecognitionBodyRaw.Frame) (rho : List UInt256)
    ...
    GasSteps (atState s 213 (frame f rho)) (atState s 289 (frame f rho)) := by
  apply segment.lift s _ e (frame f rho)
  have h := RecognitionBranchRaw.run_segment s (UInt256.ofNat 213) f rho 289 hs e.run (valid_291 s e)
...
def gasSteps_partialBranch_yes (s : State) (e : Env s) (f : RecognitionBodyRaw.Frame) (rho : List UInt256)
    ...
    GasSteps (atState s 289 (frame f rho)) (atState s 313 (frame f rho)) := by
  apply partialBranch.lift s _ e (frame f rho)
  have h := RecognitionBranchRaw.run_partial s (UInt256.ofNat 289) f rho 313 hs e.run (valid_315 s e)
...
def gasSteps_partialBranch_no (s : State) (e : Env s) (f : RecognitionBodyRaw.Frame) (rho : List UInt256)
    (hs : rho.length ≤ 990) (hc : ¬ (f.off.toNat = s.executionEnv.calldata.size % 2^256)) :
    GasSteps (atState s 289 (frame f rho)) (atState s 297 (frame f rho)) := by
  apply partialBranch.lift s _ e (frame f rho)
  have h := RecognitionBranchRaw.run_partial s (UInt256.ofNat 289) f rho 313 hs e.run (valid_315 s e)
...
def gasSteps_clamp (s : State) (e : Env s) (f : RecognitionBodyRaw.Frame) (rho : List UInt256) (hs : rho.length ≤ 990) :
  ...
  have h := RecognitionBranchRaw.run_clamp_width 2 (by decide) s (UInt256.ofNat 275) f rho 174 hs e.run (valid_174 s e)
  by_cases hc : f.stop.toNat < f.full.toNat
  ...
  · have g : GasSteps (atState s 275 (frame f rho))
    ...
    have g' := g.trans (gasSteps_reset s e f rho hs)
    simpa only [RecognitionFrame.clamp, if_neg hc] using g'.trans (gasSteps_back s e {f with stop := f.full} rho hs)
