...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTable144Sparse
open EvmSemantics EvmSemantics.EVM Challenge.EvmProof
open PairTable144Memory PairTable144Layout

def zeroBytes : ByteArray := ByteArray.mk (Array.replicate 1418 (0 : UInt8))
def zeroMemory (memory : ByteArray) : ByteArray := MachineState.writeBytes memory zeroBytes 0

@[simp] theorem zeroBytes_size : zeroBytes.size = 1418 := rfl
@[simp] theorem zeroBytes_getD (i : Nat) : zeroBytes[i]?.getD 0 = 0 := by
  change (Array.replicate 1418 (0 : UInt8))[i]?.getD 0 = 0
  by_cases hi : i < 1418
...
@[simp] theorem zeroMemory_size (memory : ByteArray) :
    (zeroMemory memory).size = max memory.size 1418 := by
  simp [zeroMemory, MachineState.writeBytes_size]

@[simp] theorem zeroMemory_getD (memory : ByteArray) (i : Nat) :
    (zeroMemory memory)[i]?.getD 0 = if i < 1418 then 0 else memory[i]?.getD 0 := by
...
theorem selected_eq_full (memory : ByteArray) (words : Nat → UInt256) (keep : Nat → Bool)
  ...
  | succ count ih =>
    ...
    · rename_i hkeep
      ...
      symm
      apply PairedScheduleMemory.writeWord_zero_noop
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTable144Sparse
