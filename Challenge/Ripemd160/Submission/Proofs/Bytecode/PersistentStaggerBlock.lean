...
def gasSteps (s : State) (input : ByteArray) (i : Nat) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 528, stack := frame h (DriverTrace.blockOffsetWord i) limit rho}
      {scheduledState s i with pc := UInt256.ofNat 4680, stack := frame (result (scheduledState s i).memory h) (DriverTrace.blockOffsetWord i) limit rho} := by
