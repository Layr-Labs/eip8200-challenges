...
def gasSteps (s : State) (input : ByteArray) (i : Nat) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 472, stack := frame h (DriverTrace.blockOffsetWord i) limit rho}
      {scheduledState s i with pc := UInt256.ofNat 4595, stack := frame (result (scheduledState s i).memory h) (DriverTrace.blockOffsetWord i) limit rho} := by
  ...
  have gb := StaggerPersistentBootstrapBridge.gasSteps_body q h off limit rho (by omega) hr
    (scheduled_active s input i hfit hi) hcode hfork hnp
