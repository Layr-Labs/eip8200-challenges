...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1773 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2301 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1773) = UInt256.ofNat 2301
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2301, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2342, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2301) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2301) template = UInt256.ofNat 2342 := by decide
  rw [hend] at hraw
