...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Prefix64Data

open EvmSemantics EvmSemantics.EVM
open PatternedInputData PatternedWordData

def data : ByteArray := patternedInput.extract 0 64

@[simp] theorem data_size : data.size = 64 := by
...
theorem byteFrom_data (i : Nat) (hi : i < 64) :
...
private theorem bytesToNatPadded_eq (off width : Nat) (hfit : off + width ≤ 64) :
...
theorem readWord_data (j : Nat) (hj : j < 2) :
...
theorem eq_data_of_words (input : ByteArray) (hsize : input.size = 64)
    (hw : ∀ j, j < 2 → MachineState.readWord input (32 * j) = expectedWordAt j) :
    input = data := by
  apply ByteArray.ext_getElem
  · rw [hsize, data_size]
  · intro j hj hjtarget
    have hj64 : j < 64 := by simpa [hsize] using hj
    have hi : j / 32 < 2 := by omega
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Prefix64Data
