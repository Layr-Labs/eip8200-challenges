...
def eval (memory : ByteArray) (q : WordLane) : WordLane :=
  StaggerAlgorithm.step 19 (MachineState.readWord memory 380) q

theorem output_eq (memory : ByteArray) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired19.outputStack memory (input memory q right (StaggerAlgorithm.physicalKey 18)) rho =
      stack memory [ .d, .pair, .upper, .a, .b, .e, .k, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] (eval memory q) right (StaggerAlgorithm.physicalKey 19) rho := by
  have hkb : StaggerAlgorithm.physicalKey 18 = UInt256.ofNat 1872146977937934200243865289652633 := by decide
  have hka : StaggerAlgorithm.physicalKey 19 = UInt256.ofNat 1872146977937934200243865289652633 := by decide
...
def gasSteps (s : State) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1974, stack := stack s.memory [ .d, .pair, .upper, .e, .c, .a, .k, .b, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] q right (StaggerAlgorithm.physicalKey 18) rho}
      {s with pc := UInt256.ofNat 2022, stack := stack s.memory [ .d, .pair, .upper, .a, .b, .e, .k, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 19) rho} := by
  have g := StaggerRawPaired19.gasSteps s (input s.memory q right (StaggerAlgorithm.physicalKey 18))
