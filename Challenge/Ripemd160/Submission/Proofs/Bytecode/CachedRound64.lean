...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3318).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3318 actual_slice
    (by change 3318 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 4076 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3318) = UInt256.ofNat 4076
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 3993, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 4041, stack := outputStack s.memory x rho} := by
