...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3216).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3216 actual_slice
    (by change 3216 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3910 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3216) = UInt256.ofNat 3910
