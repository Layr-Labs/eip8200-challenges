...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3247).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3247 actual_slice
    (by change 3247 + actualTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualTemplate) (by decide))
    (by decide)
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3850 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3247) = UInt256.ofNat 3850
...
theorem run_actual (s : State) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 996) (hrun : s.halt = .Running)
    (hactive : 23 ≤ s.activeWords.toNat) :
    runInstrSeq actualTemplate {s with pc := UInt256.ofNat 3850, stack := inputStack s.memory x rho} =
...
def gasSteps (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3850, stack := coreStack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] f (cache s.memory ++ rho)}
  ...
  have g := gasSteps_terminal_of_raw actualSite
    {s with pc := UInt256.ofNat 3850, stack := inputStack s.memory x rho} _
