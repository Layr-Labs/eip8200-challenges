...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Prefix256Data

open EvmSemantics EvmSemantics.EVM
open PatternedInputData PatternedWordData

def data : ByteArray := patternedInput.extract 0 256

@[simp] theorem data_size : data.size = 256 := by
...
theorem byteFrom_data (i : Nat) (hi : i < 256) :
...
private theorem bytesToNatPadded_eq (off width : Nat) (hfit : off + width ≤ 256) :
...
theorem readWord_data (j : Nat) (hj : j < 8) :
...
theorem eq_data_of_words (input : ByteArray) (hsize : input.size = 256)
    (hw : ∀ j, j < 8 → MachineState.readWord input (32 * j) = expectedWordAt j) :
    input = data := by
  apply ByteArray.ext_getElem
  · rw [hsize, data_size]
  · intro j hj hjtarget
    have hj256 : j < 256 := by simpa [hsize] using hj
    have hi : j / 32 < 8 := by omega
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Prefix256Data
