...
def data : ByteArray := patternedInput.extract 0 376

@[simp] theorem data_size : data.size = 376 := by
...
theorem byteFrom_data (i : Nat) (hi : i < 376) :
...
private theorem bytesToNatPadded_eq (off width : Nat) (hfit : off + width ≤ 376) :
...
theorem readWord_data (j : Nat) (hj : j < 11) :
