...
def digestEntryState (_n : Nat) (input : ByteArray) (sv ov : UInt256) : State :=
  stS input 4906 (returnRest sv ov)

def storedState (n : Nat) (input : ByteArray) (sv ov : UInt256) : State :=
  { stS input 4934 (returnRest sv ov) with
...
def tableOffset (n : Nat) : Nat := 5232 - 21 * (((n * (n + 71917)) / 32768) % 16)
...
def codeSizeEntryState (n : Nat) (input : ByteArray) (sv ov : UInt256) : State :=
  stS input 4929 ([UInt256.ofNat (21 * ((n * (n + 71917) / 32768) % 16)), 20] ++ returnRest sv ov)

def codeSizeAfterState (n : Nat) (input : ByteArray) (sv ov : UInt256) : State :=
  stS input 4930 ([UInt256.ofNat 5232, UInt256.ofNat (21 * ((n * (n + 71917) / 32768) % 16)), 20] ++ returnRest sv ov)
...
private theorem codePrefix_size : codePrefix.size = 4745 := by
...
private theorem tableRead (n : Nat) :
    MachineState.readPadded submissionBytecode (tableOffset n) 20 =
      MachineState.readPadded submissionByteChunk20 (487 - 21 * (((n * (n + 71917)) / 32768) % 16)) 20 := by
  have hj := Nat.mod_lt ((n * (n + 71917)) / 32768) (by decide : 0 < 16)
...
private theorem tablePayload (n : Nat)
    ...
    MachineState.readPadded submissionByteChunk20 (487 - 21 * (((n * (n + 71917)) / 32768) % 16)) 20 =
...
def selectorPath : List Located :=
  [pushAt 167 2 4906, opAt 168 .JUMP]
...
def digestAfterSizePath : List Located :=
  [opAt 4069 .SUB, pushAt 4070 1 12]

def digestFinishPath : List Located :=
  [pushAt 4073 0 0, opAt 4074 .RETURN]
...
@[simp] theorem pc258 : Artifact.submissionArtifact.instructionPC 168 = 266 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4843 : Artifact.submissionArtifact.instructionPC 4055 = 4906 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4844 : Artifact.submissionArtifact.instructionPC 4056 = 4907 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4846 : Artifact.submissionArtifact.instructionPC 4057 = 4909 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4848 : Artifact.submissionArtifact.instructionPC 4058 = 4911 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
@[simp] theorem pc4851 : Artifact.submissionArtifact.instructionPC 4060 = 4913 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4852 : Artifact.submissionArtifact.instructionPC 4061 = 4920 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4854 : Artifact.submissionArtifact.instructionPC 4062 = 4921 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4855 : Artifact.submissionArtifact.instructionPC 4063 = 4922 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4857 : Artifact.submissionArtifact.instructionPC 4064 = 4924 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4858 : Artifact.submissionArtifact.instructionPC 4065 = 4925 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4859 : Artifact.submissionArtifact.instructionPC 4066 = 4926 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4861 : Artifact.submissionArtifact.instructionPC 4067 = 4928 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4862 : Artifact.submissionArtifact.instructionPC 4068 = 4929 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4865 : Artifact.submissionArtifact.instructionPC 4069 = 4930 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4866 : Artifact.submissionArtifact.instructionPC 4070 = 4931 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4868 : Artifact.submissionArtifact.instructionPC 4071 = 4933 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4869 : Artifact.submissionArtifact.instructionPC 4072 = 4934 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4870 : Artifact.submissionArtifact.instructionPC 4073 = 4935 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
@[simp] theorem pc4871 : Artifact.submissionArtifact.instructionPC 4074 = 4936 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem run_selector (n : Nat) (input : ByteArray) (sv ov : UInt256) :
  ...
  change run selectorPath (stS input 263 (returnRest sv ov)) =
    some (stS input 4906 (returnRest sv ov))
  have hdest : Decode.isValidJumpDest submissionBytecode 4906 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 4055 (by rfl)
  have h0 : Stepper.runLocatedBlock [pushAt 167 2 4906]
      (stS input 263 (returnRest sv ov)) =
      some (stS input 266 (4906 :: returnRest sv ov)) := by
    exact blockOfS _ (pcFactS input 167 263 _ (by norm_num) pc255)
      (stepS_push input 263 2 4906 (returnRest sv ov)
        (by simp [returnRest]) (by decide) (by decide) (by norm_num))
  have h1 : Stepper.runLocatedBlock [opAt 168 .JUMP]
      (stS input 266 (4906 :: returnRest sv ov)) =
      some (stS input 4906 (returnRest sv ov)) := by
    exact blockOfS _ (pcFactS input 168 266 _ (by norm_num) pc258)
      (stepS_jump input 266 4906 4906 (returnRest sv ov)
        (by simp [returnRest]) (by norm_num) rfl hdest)
  exact Stepper.runLocatedBlock_append [pushAt 167 2 4906] [opAt 168 .JUMP]
    _ _ _ h0 (by rfl) h1
