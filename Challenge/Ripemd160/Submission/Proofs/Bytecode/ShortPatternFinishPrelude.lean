...
def tableOffset (n : Nat) : Nat := 5251 - 21 * (((n * (n + 12345089)) / 1048576) % 16)
...
def codeSizeAfterState (n : Nat) (input : ByteArray) (sv ov : UInt256) : State :=
  stS input 582 ([UInt256.ofNat 5251, UInt256.ofNat (21 * ((n * (n + 12345089) / 1048576) % 16)), 20] ++ returnRest sv ov)
...
private theorem codePrefix_size : codePrefix.size = 4764 := by
