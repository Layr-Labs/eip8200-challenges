...
private theorem returnDest_valid :
    Decode.isValidJumpDest submissionBytecode 5383 = true := by
  have h := Artifact.submissionArtifact.isValidJumpDest_index 2201 (by rfl)
  have hp : Artifact.submissionArtifact.instructionPC 2201 = 5383 := by
...
theorem run_branch_match (input : ByteArray)
    (h : guardDiff input = 0) :
    Stepper.runLocatedBlock branchPath (diffState input 5374 (guardDiff input)) =
      some (branchState input 5383) := by
...
theorem run_branch_fallback (input : ByteArray)
    (h : guardDiff input ≠ 0) :
    Stepper.runLocatedBlock branchPath (diffState input 5374 (guardDiff input)) =
      some (branchState input 5379) := by
...
theorem run_fallback (input : ByteArray) :
    Stepper.runLocatedBlock fallbackPath (branchState input 5379) =
...
theorem run_return (input : ByteArray) :
    Stepper.runLocatedBlock returnPath (branchState input 5383) =
