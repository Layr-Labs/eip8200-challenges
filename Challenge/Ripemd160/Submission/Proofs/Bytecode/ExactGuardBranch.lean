...
private theorem returnDest_valid :
    Decode.isValidJumpDest submissionBytecode 5348 = true := by
  have h := Artifact.submissionArtifact.isValidJumpDest_index 2966 (by rfl)
  have hp : Artifact.submissionArtifact.instructionPC 2966 = 5348 := by
...
theorem run_branch_match (input : ByteArray)
    (h : guardDiff input = 0) :
    Stepper.runLocatedBlock branchPath (diffState input 5344 (guardDiff input)) =
      some (branchState input 5348) := by
  have hz : ¬ UInt256.isTrue (guardDiff input) := by rw [h]; decide
  simp (config := { maxSteps := 1000000 }) [branchPath, branchIsZeroPath,
    branchPushPath, branchJumpPath, diffState, branchState, atPC, hz,
    fallbackDest_valid, Stepper.runLocatedBlock, Stepper.runLocated, Stepper.runInstr,
...
theorem run_branch_fallback (input : ByteArray)
    (h : guardDiff input ≠ 0) :
    Stepper.runLocatedBlock branchPath (diffState input 5344 (guardDiff input)) =
      some (branchState input 1006) := by
  have hn : (guardDiff input).toNat ≠ 0 := by
    intro heq
    apply h
    apply Challenge.EvmProof.Word.word_ext
    exact heq
  have hz : UInt256.isTrue (guardDiff input) := by
    simpa [UInt256.isTrue] using hn
  simp (config := { maxSteps := 1000000 }) [branchPath, branchIsZeroPath,
    branchPushPath, branchJumpPath, diffState, branchState, atPC, hz,
    fallbackDest_valid, Stepper.runLocatedBlock, Stepper.runLocated, Stepper.runInstr,
    initialState, Challenge.EvmProof.Word.literal_eq_ofNat,
...
theorem run_fallback (input : ByteArray) :
    Stepper.runLocatedBlock fallbackPath (branchState input 1006) =
...
theorem run_return (input : ByteArray) :
    Stepper.runLocatedBlock returnPath (branchState input 5348) =
