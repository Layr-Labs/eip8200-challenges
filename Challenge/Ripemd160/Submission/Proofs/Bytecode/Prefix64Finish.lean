...
private theorem pc3783 : Artifact.submissionArtifact.instructionPC 4103 = 5275 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
private theorem pc3784 : Artifact.submissionArtifact.instructionPC 4104 = 5276 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
private theorem pc3785 : Artifact.submissionArtifact.instructionPC 4105 = 5297 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
private theorem pc3786 : Artifact.submissionArtifact.instructionPC 4106 = 5298 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
private theorem pc3787 : Artifact.submissionArtifact.instructionPC 4107 = 5299 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
private theorem pc3788 : Artifact.submissionArtifact.instructionPC 4108 = 5300 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
private theorem pc3789 : Artifact.submissionArtifact.instructionPC 4109 = 5301 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl

private theorem returnDest : Decode.isValidJumpDest submissionBytecode 5275 = true :=
...
def gasSteps_select64 (input : ByteArray) (hsize : input.size = 64) :
    GasSteps (stS input 5205 []) (stS input 5275 []) := by
  ...
  exact (gasSteps_selector_test input).trans
    (soundS (opAt 4084 .JUMPI)
      (blockOfS _ (pcFactS input 4084 5212 _ (by norm_num) pc3776)
        (stepS_jumpi_taken input 5212 5275 5275
          (UInt256.eq 64 (UInt256.ofNat input.size)) []
          (by simp) (by norm_num) rfl hc returnDest)))

def gasSteps_return (input : ByteArray) :
    GasSteps (stS input 5275 []) (returnedState input) := by
  have a := soundS (opAt 4103 .JUMPDEST)
    (blockOfS _ (pcFactS input 4103 5275 _ (by norm_num) pc3783)
      (stepS_jumpdest input 5275 [] (by simp) (by norm_num)))
  have b := soundS (pushAt 4104 20 paddedDigestWord)
    (blockOfS _ (pcFactS input 4104 5276 _ (by norm_num) pc3784)
      (stepS_push input 5276 20 paddedDigestWord []
        (by simp) (by decide) (by decide) (by norm_num)))
  have c := soundS (pushAt 4105 0 0)
    (blockOfS _ (pcFactS input 4105 5297 _ (by norm_num) pc3785)
      (stepS_push0 input 5297 [paddedDigestWord] (by simp) (by norm_num)))
  have hd : Stepper.runInstr (.op .MSTORE)
      (stS input 5298 [0, paddedDigestWord]) = some (storedState input 5299 []) := by
    rfl
  have d := soundS (opAt 4106 .MSTORE)
    (blockOfS _ (pcFactS input 4106 5298 _ (by norm_num) pc3786) hd)
  have hop : (storedState input 5299 []).decodedOp = some .MSIZE := by
    apply Trace.decodedOpAt (storedState input 5299 []) 4107 .MSIZE
    · rfl
    ...
    · trivial
    · rfl
  have e : GasSteps (storedState input 5299 []) (storedState input 5300 [32]) := by
    exact Msize.step hop (by change 0 < 1024; decide) (by rfl) deployAddress_not_precompile
  have hf : Stepper.runInstr (.push 0 0) (storedState input 5300 [32]) =
      some (storedState input 5301 [0, 32]) := by rfl
  have f := soundS (pushAt 4108 0 0)
    (blockOfS _
      (show (storedState input 5300 [32]).pc.toNat =
        Artifact.submissionArtifact.instructionPC 4108 from
          pcFactS input 4108 5300 [32] (by norm_num) pc3788) hf)
  have hg : Stepper.runInstr (.op .RETURN) (storedState input 5301 [0, 32]) =
      some (returnedState input) := by rfl
  have g := soundS (opAt 4109 .RETURN)
    (blockOfS _
      (show (storedState input 5301 [0, 32]).pc.toNat =
        Artifact.submissionArtifact.instructionPC 4109 from
          pcFactS input 4109 5301 [0, 32] (by norm_num) pc3789) hg)
