...
def eval (memory : ByteArray) (q : WordLane) : WordLane :=
  StaggerModeSeven.step 46 (MachineState.readWord memory 1080) q
...
theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired46.outputStack memory (input memory h4 q right (StaggerModeSeven.physicalKey 45)) rho =
      stack memory h4 [ .d, .literal 28, .cachedMessage 450, .pair, .upper, .a, .c, .e, .k, .b, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval memory q) right (StaggerModeSeven.physicalKey 46) rho := by
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3093, stack := stack s.memory h4 [ .d, .literal 28, .cachedMessage 450, .pair, .upper, .e, .b, .a, .k, .c, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] q right (StaggerModeSeven.physicalKey 45) rho}
      {s with pc := UInt256.ofNat 3152, stack := stack s.memory h4 [ .d, .literal 28, .cachedMessage 450, .pair, .upper, .a, .c, .e, .k, .b, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval s.memory q) right (StaggerModeSeven.physicalKey 46) rho} := by
