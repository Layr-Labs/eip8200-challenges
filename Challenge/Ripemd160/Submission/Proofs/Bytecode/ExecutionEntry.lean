...
theorem destination : Decode.isValidJumpDest submissionBytecode 0x1363 = true := by
  have hget : Artifact.submissionArtifact.instructions[3222]? = some (.op .JUMPDEST) := by
    decide
  have hpc : Artifact.submissionArtifact.instructionPC 3222 = 0x1363 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3222 hget
  rw [hpc] at h
  exact h
...
def finish (s : State) : State := { s with pc := UInt256.ofNat 0x1363, stack := [] }
...
theorem run_entry (s : State)
  ...
  have hdestNat : (UInt256.ofNat 0x1363).toNat = 0x1363 := by decide
...
def initial_entry (input : ByteArray) :
    GasSteps (initialState submissionBytecode input 0)
      { initialState submissionBytecode input 0 with pc := UInt256.ofNat 0x1363 } :=
