...
theorem destination : Decode.isValidJumpDest submissionBytecode 0x12ce = true := by
  have hget : Artifact.submissionArtifact.instructions[3025]? = some (.op .JUMPDEST) := by
    decide
  have hpc : Artifact.submissionArtifact.instructionPC 3025 = 0x12ce := by
...
def finish (s : State) : State := { s with pc := UInt256.ofNat 0x12ce, stack := [] }
...
theorem run_entry (s : State)
  ...
  have hdestNat : (UInt256.ofNat 0x12ce).toNat = 0x12ce := by decide
...
def initial_entry (input : ByteArray) :
    GasSteps (initialState submissionBytecode input 0)
      { initialState submissionBytecode input 0 with pc := UInt256.ofNat 0x12ce } :=
