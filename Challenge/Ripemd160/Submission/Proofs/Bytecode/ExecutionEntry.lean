...
theorem destination : Decode.isValidJumpDest submissionBytecode 0xb4 = true := by
  have hget : Artifact.submissionArtifact.instructions[105]? = some (.op .JUMPDEST) := by
    decide
  have hpc : Artifact.submissionArtifact.instructionPC 105 = 0xb4 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 105 hget
  rw [hpc] at h
  exact h
...
def finish (s : State) : State := { s with pc := UInt256.ofNat 0xb4, stack := [] }
...
theorem run_entry (s : State)
  ...
  have hdestNat : (UInt256.ofNat 0xb4).toNat = 0xb4 := by decide
...
def initial_entry (input : ByteArray) :
    GasSteps (initialState submissionBytecode input 0)
      { initialState submissionBytecode input 0 with pc := UInt256.ofNat 0xb4 } :=
