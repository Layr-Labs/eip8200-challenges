import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144RawRound63
...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound63
...
def eval (message k : UInt256) (q : WordLane) : WordLane := wordStep 3 12 8 message k q
def nextKey (k : UInt256) : UInt256 := (UInt256.ofNat 2840853838)
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table144RawRound63.outputStack memory (input q k) rho =
      stack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 522) k q) (nextKey k) rho := by
  simp only [Table144RawRound63.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired144Algorithm.physicalKey 63) =
     Paired144Algorithm.physicalKey 64 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired144Algorithm.physicalKey 63) q = Paired144Algorithm.step 63 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 3870, stack := stack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 3923, stack :=
          stack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 522) k q) (nextKey k) rho} := by
   have gs := Table144RawRound63.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound63
