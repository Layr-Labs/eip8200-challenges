...
opaque gasSteps_prepare_hit (s : State) (input : ByteArray) (i : Nat)
    ...
    GasSteps (DriverTrace.compressEntry s input i)
      {scheduledState s i with pc := UInt256.ofNat 1048, stack := Table80Raw.cache ++ driverRest input i} := by
  ...
  have gtouch := Table80Dispatch.gasSteps_prefix s (DriverTrace.blockOffsetWord i) (messagePointer i)
  ...
  have gbody := Table80SetupSites.gasSteps_pad a (DriverTrace.blockOffsetWord i) [Padding.paddedWord input]
...
opaque gasSteps_prepare_miss (s : State) (input : ByteArray) (i : Nat)
  ...
  have gnormal := Table80SetupSites.gasSteps_normal s (DriverTrace.blockOffsetWord i)
