...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 538 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 877 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 538) = UInt256.ofNat 877
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 877, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 919, stack := outputStack s.memory x rho, memory := outputMemory s.memory x} := by
  have hraw := run_actual s (UInt256.ofNat 877) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 877) template = UInt256.ofNat 919 := by decide
  rw [hend] at hraw
