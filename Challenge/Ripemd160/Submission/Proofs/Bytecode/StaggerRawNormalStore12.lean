...
theorem site_pc : site.startPC = UInt256.ofNat 939 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 607) = UInt256.ofNat 939
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 939, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 982, stack := outputStack s.memory x rho, memory := outputMemory s.memory x} := by
  have hraw := run_actual s (UInt256.ofNat 939) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 939) template = UInt256.ofNat 982 := by decide
  rw [hend] at hraw
