...
def outputMemory (memory : ByteArray) (x : Input) : ByteArray :=
  (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord (PairedScheduleMemory.writeWord memory 216 x.v0) 198 x.v0) 180 x.v1) 162 x.v2) 144 x.v3) 126 x.v3) 108 x.v5) 90 x.v4) 72 x.v4) 54 x.v5) 36 x.v5) 18 x.v6) 0 x.v6)
...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 528 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 873 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 528) = UInt256.ofNat 873
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 873, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 917, stack := outputStack s.memory x rho, memory := outputMemory s.memory x} := by
  have hraw := run_actual s (UInt256.ofNat 873) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 873) template = UInt256.ofNat 917 := by decide
  rw [hend] at hraw
