...
theorem run_exit_miss (input : ByteArray) (hne : scanAcc input 376 ≠ 0) :
    run exitPath (loopExitState input) = some (fallbackState input) := by
  have htrue : UInt256.isTrue (scanAcc input 376) := by
    intro hz
    apply hne
    apply Challenge.EvmProof.Word.word_ext
    simpa only [show (0 : UInt256).toNat = 0 by rfl] using hz
  have hneNat : (scanAcc input 376).toNat ≠ 0 := htrue
