...
opaque gasSteps_prepare_hit (s : State) (input : ByteArray) (i : Nat)
    ...
    GasSteps (DriverTrace.compressEntry s input i)
      {scheduledState s i with pc := UInt256.ofNat 1222, stack := (UInt256.ofNat 563 :: driverRest input i)} := by
  let q := scheduledState s i
  let rho := UInt256.ofNat 563 :: driverRest input i
  ...
  have ghit := StaggerDispatch.gasSteps_hit s (DriverTrace.messageOffsetWord i)
    (UInt256.ofNat 563) (DriverTrace.blockOffsetWord i) [Padding.paddedWord input]
    (by simp) hrun hfit256 heq hcode hfork hnp
  have gtouch := StaggerDispatch.gasSteps_prefix s (UInt256.ofNat 563) (messagePointer i)
  ...
  have gbody := StaggerSetupSites.gasSteps_pad a (UInt256.ofNat 563) (driverRest input i)
  ...
  have gb : GasSteps {a with pc := UInt256.ofNat 406, stack := rho}
      {q with pc := UInt256.ofNat 641, stack := rho} := by
...
opaque gasSteps_prepare_miss (s : State) (input : ByteArray) (i : Nat)
  ...
  have gnormal := StaggerSetupSites.gasSteps_normal s (UInt256.ofNat 563)
