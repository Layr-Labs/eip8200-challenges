import Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerPrepareModel
...
opaque gasSteps_prepare_hit (s : State) (input : ByteArray) (i : Nat)
    ...
    GasSteps (DriverTrace.compressEntry s input i)
      {scheduledState s i with pc := UInt256.ofNat 1048, stack := (UInt256.ofNat 591 :: driverRest input i)} := by
  let q := scheduledState s i
  let rho := UInt256.ofNat 591 :: driverRest input i
  ...
  have ghit := StaggerDispatch.gasSteps_hit s (DriverTrace.messageOffsetWord i)
    (UInt256.ofNat 591) (DriverTrace.blockOffsetWord i) [Padding.paddedWord input]
    (by simp) hrun hfit256 heq hcode hfork hnp
  have gtouch := StaggerDispatch.gasSteps_prefix s (UInt256.ofNat 591) (messagePointer i)
  ...
  have ga : GasSteps (DriverTrace.compressEntry s input i)
      {a with pc := UInt256.ofNat 434, stack := rho} := by
    rw [ha]
    exact ghit.trans gtouch
  have gbody := StaggerSetupSites.gasSteps_pad a (UInt256.ofNat 591) (driverRest input i)
  ...
  have gb : GasSteps {a with pc := UInt256.ofNat 434, stack := rho}
      {q with pc := UInt256.ofNat 587, stack := rho} := by
    apply gbody.cast rfl
    rw [hmem]
    rfl
  have gjump := StaggerPadJump.gasSteps_jump q (rho)
    (by simp [rho, driverRest]) hrun hcode hfork hnp
...
opaque gasSteps_prepare_miss (s : State) (input : ByteArray) (i : Nat)
  ...
  have gnormal := StaggerSetupSites.gasSteps_normal s (UInt256.ofNat 591)
