...
def gasSteps_guard (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 4761 F) (atState s 327 F) := by
...
def gasSteps_sparse (s : State) (e : Env s)
    ...
    GasSteps
      (atState s 327 (stk ret mw a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := sparseMemory s.memory} 546
  ...
  have g0 : GasSteps (atState s 327 F) (atState s 328 F) := by
    apply special.lift s (atState s 328 F) e F
    simpa only [special.template, atState, show UInt256.ofNat 327 + UInt256.ofNat 1 = UInt256.ofNat 328 by decide] using
      PadJump.run_merge s (UInt256.ofNat 327) F (by omega) e.run
  have g1 : GasSteps (atState s 328 F) (atState s 330 (UInt256.ofNat 128 :: F)) := by
    apply marker.lift s (atState s 330 (UInt256.ofNat 128 :: F)) e F
    ...
    decide
  have g2 : GasSteps (atState s 330 (UInt256.ofNat 128 :: F)) (atState sM 337 F) := by
    apply sparse.lift s (atState sM 337 F) e (UInt256.ofNat 128 :: F)
  ...
  have g3 : GasSteps (atState sM 337 F) (atState sM 545 F) := by
    apply jump.lift sM (atState sM 545 F) eM F
    exact PadJump.run_template sM (UInt256.ofNat 337) F 545 (by omega) e.run
      (by simpa only [show (UInt256.ofNat 545).toNat = 545 by decide] using valid_lower sM eM)
  have g4 : GasSteps (atState sM 545 F) (atState sM 546 F) := by
    apply lower.lift sM (atState sM 546 F) eM F
    simpa only [lower.template, atState, show UInt256.ofNat 545 + UInt256.ofNat 1 = UInt256.ofNat 546 by decide] using
      PadJump.run_merge sM (UInt256.ofNat 545) F (by omega) e.run
...
def gasSteps_table (s : State) (e : Env s)
    ...
    (hactive : s.activeWords = UInt256.ofNat 34) :
    GasSteps
      (atState {s with memory := writeWord s.memory 162 highWord} 546
        (stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := Shared32Table.tableMemory s.memory, activeWords := UInt256.ofNat 35} 863
  ...
  have h := Shared32Run.run_table s (UInt256.ofNat 546) ret a2 a3 a4 a5 a6 a7
