...
def gasSteps_guard (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 4762 F) (atState s 324 F) := by
...
def gasSteps_sparse (s : State) (e : Env s)
    ...
    GasSteps
      ...
      (atState {s with memory := sparseMemory s.memory} 558
  ...
  have g3 : GasSteps (atState sM 334 F) (atState sM 557 F) := by
    apply jump.lift sM (atState sM 557 F) eM F
    exact PadJump.run_template sM (UInt256.ofNat 334) F 557 (by omega) e.run
      (by simpa only [show (UInt256.ofNat 557).toNat = 557 by decide] using valid_lower sM eM)
  have g4 : GasSteps (atState sM 557 F) (atState sM 558 F) := by
    apply lower.lift sM (atState sM 558 F) eM F
    simpa only [lower.template, atState, show UInt256.ofNat 557 + UInt256.ofNat 1 = UInt256.ofNat 558 by decide] using
      PadJump.run_merge sM (UInt256.ofNat 557) F (by omega) e.run
...
def gasSteps_table (s : State) (e : Env s)
    ...
    (hactive : s.activeWords = UInt256.ofNat 34) :
    GasSteps
      (atState {s with memory := writeWord s.memory 162 highWord} 558
        (stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := Shared32Table.tableMemory s.memory, activeWords := UInt256.ofNat 35} 875
  ...
  have h := Shared32Run.run_table s (UInt256.ofNat 558) ret a2 a3 a4 a5 a6 a7
