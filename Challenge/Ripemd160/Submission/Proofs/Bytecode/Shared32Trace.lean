...
def gasSteps_guard (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 368 F) (atState s 4954 F) := by
  apply guard.lift s (atState s 4954 F) e F
...
def gasSteps_sparse (s : State) (e : Env s)
    ...
    GasSteps
      (atState s 4954 (stk ret mw a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := sparseMemory s.memory} 811
  ...
  have g0 : GasSteps (atState s 4954 F) (atState s 4955 F) := by
    apply special.lift s (atState s 4955 F) e F
    simpa only [special.template, atState, show UInt256.ofNat 4954 + UInt256.ofNat 1 = UInt256.ofNat 4955 by decide] using
      PadJump.run_merge s (UInt256.ofNat 4954) F (by omega) e.run
  have g1 : GasSteps (atState s 4955 F) (atState s 4957 (UInt256.ofNat 128 :: F)) := by
    apply marker.lift s (atState s 4957 (UInt256.ofNat 128 :: F)) e F
    ...
    decide
  have g2 : GasSteps (atState s 4957 (UInt256.ofNat 128 :: F)) (atState sM 4964 F) := by
    apply sparse.lift s (atState sM 4964 F) e (UInt256.ofNat 128 :: F)
    have h := Shared32SparseRun.run_sparse s (UInt256.ofNat 4957) ret mw a2 a3 a4
      a5 a6 a7 a8 a9 a10 lim rho hstack e.run hactive hbyte
    simpa only [sparse.template, sparse.end_pc, atState, sM, F] using h
  have g3 : GasSteps (atState sM 4964 F) (atState sM 810 F) := by
    apply jump.lift sM (atState sM 810 F) eM F
    exact PadJump.run_template sM (UInt256.ofNat 4964) F 810 (by omega) e.run
      (by simpa only [show (UInt256.ofNat 810).toNat = 810 by decide] using valid_lower sM eM)
  have g4 : GasSteps (atState sM 810 F) (atState sM 811 F) := by
    apply lower.lift sM (atState sM 811 F) eM F
    simpa only [lower.template, atState, show UInt256.ofNat 810 + UInt256.ofNat 1 = UInt256.ofNat 811 by decide] using
      PadJump.run_merge sM (UInt256.ofNat 810) F (by omega) e.run
...
def gasSteps_table (s : State) (e : Env s)
    ...
    (hactive : s.activeWords = UInt256.ofNat 34) :
    GasSteps
      (atState {s with memory := writeWord s.memory 162 highWord} 811
        (stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := Shared32Table.tableMemory s.memory, activeWords := UInt256.ofNat 35} 1128
  ...
  have h := Shared32Run.run_table s (UInt256.ofNat 811) ret a2 a3 a4 a5 a6 a7
