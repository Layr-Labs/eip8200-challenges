...
def gasSteps_guard (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 4709 F) (atState s 322 F) := by
...
def gasSteps_sparse (s : State) (e : Env s)
    ...
    GasSteps
      ...
      (atState {s with memory := sparseMemory s.memory} 531
  ...
  have g2 : GasSteps (atState s 324 (UInt256.ofNat 1152 :: F)) (atState sM 331 F) := by
    apply sparse.lift s (atState sM 331 F) e (UInt256.ofNat 1152 :: F)
  ...
  have g3 : GasSteps (atState sM 331 F) (atState sM 530 F) := by
    apply jump.lift sM (atState sM 530 F) eM F
    exact PadJump.run_template sM (UInt256.ofNat 331) F 530 (by omega) e.run
      (by simpa only [show (UInt256.ofNat 530).toNat = 530 by decide] using valid_lower sM eM)
  have g4 : GasSteps (atState sM 530 F) (atState sM 531 F) := by
    apply lower.lift sM (atState sM 531 F) eM F
    simpa only [lower.template, atState, show UInt256.ofNat 530 + UInt256.ofNat 1 = UInt256.ofNat 531 by decide] using
...
def gasSteps_table (s : State) (e : Env s)
    ...
    GasSteps
      (atState {s with memory := writeWord s.memory 60 highWord} 531
        (stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := StaggerTableLayout.resultMemory s.memory (Shared32Table.words s.memory)} 898
  ...
  have h := Shared32Run.run_table s (UInt256.ofNat 531) ret a2 a3 a4 a5 a6 a7
