...
def gasSteps_sparse (s : State) (e : Env s)
    ...
    GasSteps
      ...
      (atState {s with memory := sparseMemory s.memory} 681
  ...
  have g3 : GasSteps (atState sM 128 F) (atState sM 680 F) := by
    apply jump.lift sM (atState sM 680 F) eM F
    exact PadJump.run_template sM (UInt256.ofNat 128) F 680 (by omega) e.run
      (by simpa only [show (UInt256.ofNat 680).toNat = 680 by decide] using valid_lower sM eM)
  have g4 : GasSteps (atState sM 680 F) (atState sM 681 F) := by
    apply lower.lift sM (atState sM 681 F) eM F
    simpa only [lower.template, atState, show UInt256.ofNat 680 + UInt256.ofNat 1 = UInt256.ofNat 681 by decide] using
      PadJump.run_merge sM (UInt256.ofNat 680) F (by omega) e.run
...
def gasSteps_table (s : State) (e : Env s)
    ...
    (hactive : s.activeWords = UInt256.ofNat 34) :
    GasSteps
      (atState {s with memory := writeWord s.memory 162 highWord} 681
        (stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := Shared32Table.tableMemory s.memory, activeWords := UInt256.ofNat 35} 998
  ...
  have h := Shared32Run.run_table s (UInt256.ofNat 681) ret a2 a3 a4 a5 a6 a7
