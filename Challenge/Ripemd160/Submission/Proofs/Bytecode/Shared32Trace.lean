...
def gasSteps_guard (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 4775 F) (atState s 324 F) := by
...
def gasSteps_sparse (s : State) (e : Env s)
    ...
    GasSteps
      ...
      (atState {s with memory := sparseMemory s.memory} 536
  ...
  have g1 : GasSteps (atState s 325 F) (atState s 327 (UInt256.ofNat 128 :: F)) := by
    apply marker.lift s (atState s 327 (UInt256.ofNat 128 :: F)) e F
    ...
    decide
  have g2 : GasSteps (atState s 327 (UInt256.ofNat 128 :: F)) (atState sM 334 F) := by
    apply sparse.lift s (atState sM 334 F) e (UInt256.ofNat 128 :: F)
  have h := Shared32SparseRun.run_sparse s (UInt256.ofNat 327) ret mw a2 a3 a4
      a5 a6 a7 a8 a9 a10 lim rho hstack e.run hactive hbyte
    simpa only [sparse.template, sparse.end_pc, atState, sM, F] using h
  have g3 : GasSteps (atState sM 334 F) (atState sM 535 F) := by
    apply jump.lift sM (atState sM 535 F) eM F
    exact PadJump.run_template sM (UInt256.ofNat 334) F 535 (by omega) e.run
      (by simpa only [show (UInt256.ofNat 535).toNat = 535 by decide] using valid_lower sM eM)
  have g4 : GasSteps (atState sM 535 F) (atState sM 536 F) := by
    apply lower.lift sM (atState sM 536 F) eM F
    simpa only [lower.template, atState, show UInt256.ofNat 535 + UInt256.ofNat 1 = UInt256.ofNat 536 by decide] using
      PadJump.run_merge sM (UInt256.ofNat 535) F (by omega) e.run
...
def gasSteps_table (s : State) (e : Env s)
    ...
    (hactive : s.activeWords = UInt256.ofNat 34) :
    GasSteps
      (atState {s with memory := writeWord s.memory 162 highWord} 536
  ...
  have h := Shared32Run.run_table s (UInt256.ofNat 536) ret a2 a3 a4 a5 a6 a7
