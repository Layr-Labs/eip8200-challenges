...
def gasSteps_guard (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 4706 F) (atState s 328 F) := by
...
def gasSteps_sparse (s : State) (e : Env s)
  ...
  have g1 : GasSteps (atState s 329 F) (atState s 330 (UInt256.ofNat 1167 :: F)) := by
  ...
  have g2 : GasSteps (atState s 330 (UInt256.ofNat 1167 :: F)) (atState sM 337 F) := by
    apply sparse.lift s (atState sM 337 F) e (UInt256.ofNat 1167 :: F)
  ...
  have g4 : GasSteps (atState sM 526 F) (atState sM 527 F) := by
    ...
    simpa only [lower.template, atState, show UInt256.ofNat 541 + UInt256.ofNat 1 = UInt256.ofNat 542 by decide] using
      PadJump.run_merge sM (UInt256.ofNat 526) F (by omega) e.run
...
def gasSteps_table (s : State) (e : Env s)
    ...
    GasSteps
      (atState {s with memory := writeWord s.memory 60 highWord} 527
        (stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := StaggerTableLayout.resultMemory s.memory (Shared32Table.words s.memory)} 894
  ...
  have h := Shared32Run.run_table s (UInt256.ofNat 527) ret a2 a3 a4 a5 a6 a7
