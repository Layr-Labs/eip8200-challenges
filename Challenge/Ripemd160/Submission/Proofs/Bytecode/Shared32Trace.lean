...
def gasSteps_guard (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 191 F) (atState s 115 F) := by
  apply guard.lift s (atState s 115 F) e F
...
def gasSteps_sparse (s : State) (e : Env s)
  ...
  have g0 : GasSteps (atState s 115 F) (atState s 116 F) := by
    apply special.lift s (atState s 116 F) e F
    simpa only [special.template, atState, show UInt256.ofNat 115 + UInt256.ofNat 1 = UInt256.ofNat 116 by decide] using
      PadJump.run_merge s (UInt256.ofNat 115) F (by omega) e.run
  have g1 : GasSteps (atState s 116 F) (atState s 118 (UInt256.ofNat 128 :: F)) := by
    apply marker.lift s (atState s 118 (UInt256.ofNat 128 :: F)) e F
    ...
    decide
  have g2 : GasSteps (atState s 118 (UInt256.ofNat 128 :: F)) (atState sM 125 F) := by
    apply sparse.lift s (atState sM 125 F) e (UInt256.ofNat 128 :: F)
