...
def gasSteps_guard (s : State) (e : Env s) (F : List UInt256)
    ...
    GasSteps (atState s 4771 F) (atState s 296 F) := by
  apply guard.lift s (atState s 296 F) e F
...
def gasSteps_sparse (s : State) (e : Env s)
    ...
    GasSteps
      (atState s 296 (stk ret mw a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := sparseMemory s.memory} 556
  ...
  have g0 : GasSteps (atState s 296 F) (atState s 297 F) := by
    apply special.lift s (atState s 297 F) e F
    simpa only [special.template, atState, show UInt256.ofNat 296 + UInt256.ofNat 1 = UInt256.ofNat 297 by decide] using
      PadJump.run_merge s (UInt256.ofNat 296) F (by omega) e.run
  have g1 : GasSteps (atState s 297 F) (atState s 299 (UInt256.ofNat 128 :: F)) := by
    apply marker.lift s (atState s 299 (UInt256.ofNat 128 :: F)) e F
    ...
    decide
  have g2 : GasSteps (atState s 299 (UInt256.ofNat 128 :: F)) (atState sM 306 F) := by
    apply sparse.lift s (atState sM 306 F) e (UInt256.ofNat 128 :: F)
    have h := Shared32SparseRun.run_sparse s (UInt256.ofNat 299) ret mw a2 a3 a4
      a5 a6 a7 a8 a9 a10 lim rho hstack e.run hactive hbyte
    simpa only [sparse.template, sparse.end_pc, atState, sM, F] using h
  have g3 : GasSteps (atState sM 306 F) (atState sM 555 F) := by
    apply jump.lift sM (atState sM 555 F) eM F
    exact PadJump.run_template sM (UInt256.ofNat 306) F 555 (by omega) e.run
      (by simpa only [show (UInt256.ofNat 555).toNat = 555 by decide] using valid_lower sM eM)
  have g4 : GasSteps (atState sM 555 F) (atState sM 556 F) := by
    apply lower.lift sM (atState sM 556 F) eM F
    simpa only [lower.template, atState, show UInt256.ofNat 555 + UInt256.ofNat 1 = UInt256.ofNat 556 by decide] using
      PadJump.run_merge sM (UInt256.ofNat 555) F (by omega) e.run
...
def gasSteps_table (s : State) (e : Env s)
    ...
    (hactive : s.activeWords = UInt256.ofNat 34) :
    GasSteps
      (atState {s with memory := writeWord s.memory 162 highWord} 556
        (stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 (UInt256.ofNat 0) lim rho))
      (atState {s with memory := Shared32Table.tableMemory s.memory, activeWords := UInt256.ofNat 35} 873
  ...
  have h := Shared32Run.run_table s (UInt256.ofNat 556) ret a2 a3 a4 a5 a6 a7
