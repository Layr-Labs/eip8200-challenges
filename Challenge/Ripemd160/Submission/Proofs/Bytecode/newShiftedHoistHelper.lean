...
/-- Inlined from accepted `RotationParameter` (absent in this tree):
full wide-value identity `(u * F) >> (32 - r) = (u * (F << r)) >> 32`
for `F = 2^32 + 1`, `u < 2^32`, `0 < r < 32`. Both products fit in 256 bits,
so neither EVM multiplication wraps. -/
private theorem nat_factor_shift_eq (u r : Nat)
...
private theorem nat_factor_shift_products_lt (u r : Nat)
...
private theorem shift_mul_toNat (a b : UInt256) :
...
private theorem uint256_factor_shift_mod_bounds (u : UInt256) (r : Nat)
...
private theorem factor_shift_eq (u : UInt256) (r : Nat)
    (hu : u.toNat < 2 ^ 32) (hr0 : 0 < r) (hr32 : r < 32) :
    UInt256.shiftRight
    ...
    calc
      (UInt256.mul (UInt256.ofNat (0x100000001 : Nat)) u).toNat =
          ((0x100000001 : Nat) * u.toNat) % 2 ^ 256 := by
            rw [shift_mul_toNat, Challenge.EvmProof.Word.word_toNat_ofNat]
  ...
  have hmulRight :
    ...
    rw [hfactorShift, shift_mul_toNat,
...
MUL; PUSH1 32; SHR. Caller SWAP8 replaces the first SWAP1; SWAP9 pair.
Cached-mask accesses and fixed C rotations are retained. -/
