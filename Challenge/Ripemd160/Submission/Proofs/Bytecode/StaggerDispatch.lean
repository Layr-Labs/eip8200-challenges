...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerDispatch
...
def dispatchSite : GenericRoundSite Artifact.submissionArtifact .Osaka dispatchTemplate :=
  StackSiteBuilder.ofSlice dispatchTemplate 305 dispatch_slice
    ...
    (by decide)
theorem dispatch_pc : dispatchSite.startPC = UInt256.ofNat 531 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 302) = UInt256.ofNat 531
...
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 246 prefix_slice
    ...
    (by decide)
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 432 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 243) = UInt256.ofNat 432
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_pad (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 432).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 240 = 432 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_miss (s : State) (ptr ret off : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 531, stack := ptr :: ret :: off :: rho}
      {s with pc := UInt256.ofNat 538, stack := ptr :: ret :: off :: rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 531, stack := ptr :: ret :: off :: rho} _ hcode hfork hrun hnp dispatch_pc.symm dispatch_advances
  have h := PadDispatch.run_miss s (UInt256.ofNat 531) ptr ret off rho 404 hstack hrun hfit hmiss
  have hp : pcAfter (UInt256.ofNat 531) (PadDispatch.template 404) = UInt256.ofNat 538 := by rfl
  rw [hp] at h
  exact h
...
def gasSteps_hit (s : State) (ptr ret off : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 531, stack := ptr :: ret :: off :: rho}
      {s with pc := UInt256.ofNat 432, stack := ptr :: ret :: off :: rho} := by
  apply PadLift.gasSteps_of_raw dispatchSite {s with pc := UInt256.ofNat 531, stack := ptr :: ret :: off :: rho} _ hcode hfork hrun hnp dispatch_pc.symm dispatch_advances
  have h := PadDispatch.run_hit s (UInt256.ofNat 531) ptr ret off rho 404 hstack hrun hfit hhit (valid_pad s hcode)
  exact h
...
def gasSteps_prefix (s : State) (ret : UInt256) (p : Nat) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 432, stack := UInt256.ofNat p :: ret :: rest}
      {s with pc := UInt256.ofNat 434, stack := ret :: rest} := by
  apply PadLift.gasSteps_of_raw prefixSite {s with pc := UInt256.ofNat 432, stack := UInt256.ofNat p :: ret :: rest} _ hcode hfork hrun hnp prefix_pc.symm prefix_advances
  have h := PadPrefix.run_drop s (UInt256.ofNat 432) ret p rest hstack hrun
  have hp : pcAfter (UInt256.ofNat 432) (PadPrefix.dropTemplate) = UInt256.ofNat 434 := by rfl
  rw [hp] at h
  exact h
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerDispatch
