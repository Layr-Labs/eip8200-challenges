...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Paired80GapCarry
open Paired80Core

theorem add_small_no_carry80 (x : BitVec 80) (e : Nat)
    (he : e < 2 ^ 47) (hgap : x.getLsbD 48 = false) :
    x.toNat + e < 2 ^ 80 := by
  have hx := x.isLt
...
theorem high_add_general (x y : Nat)
    (hcarry : x % 2 ^ 80 + y % 2 ^ 80 < 2 ^ 80) :
    ((x + y) % 2 ^ 256) / 2 ^ 80 % 2 ^ 32 =
      (x / 2 ^ 80 % 2 ^ 32 + y / 2 ^ 80 % 2 ^ 32) % 2 ^ 32 := by
  simp only [Nat.reducePow] at *
  omega

theorem normalize_add_gap (x y : BitVec 256)
    (hy : y.toNat % 2 ^ 80 < 2 ^ 47) (hgap : x.getLsbD 48 = false) :
  ...
  have hcarry : x.toNat % 2 ^ 80 + y.toNat % 2 ^ 80 < 2 ^ 80 := by
    simpa only [BitVec.extractLsb'_toNat, Nat.shiftRight_zero] using
      add_small_no_carry80 (x.extractLsb' 0 80) (y.toNat % 2 ^ 80) hy hgap'
  have hnorm : (normalize x).toNat % 2 ^ 80 < 2 ^ 32 := by
    rw [normalize, pack_toNat]
    have hlo := (low x).isLt
    simp only [Nat.reducePow] at *
    omega
  have hcarry' : (normalize x).toNat % 2 ^ 80 + y.toNat % 2 ^ 80 < 2 ^ 80 := by
    simp only [Nat.reducePow] at *
    omega
  ...
  have hhi : high (x + y) = high (normalize x + y) := by
    ...
    have hn : (normalize x).toNat / 2 ^ 80 % 2 ^ 32 = x.toNat / 2 ^ 80 % 2 ^ 32 := by
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Paired80GapCarry
