...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawPaired21
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1485).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1485 actual_slice
    (by change 1485 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1996 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1485) = UInt256.ofNat 1996
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1996, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2054, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1996) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1996) template = UInt256.ofNat 2054 := by decide
  rw [hend] at hraw
  exact DenseScheduleLift.gasSteps_of_raw site _ _ hcode hfork hrun hnp site_pc.symm advances hraw
#print axioms gasSteps
end Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawPaired21
