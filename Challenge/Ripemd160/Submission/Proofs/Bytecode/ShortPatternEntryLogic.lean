...
def sizeFlag (input : ByteArray) : UInt256 :=
  UInt256.isZero (UInt256.lor (UInt256.eq (UInt256.ofNat 56) (UInt256.ofNat input.size)) (UInt256.lor (UInt256.eq (UInt256.ofNat 120) (UInt256.ofNat input.size)) (UInt256.lor (UInt256.eq (UInt256.ofNat 63) (UInt256.ofNat input.size)) (UInt256.lor (UInt256.eq (UInt256.ofNat 64) (UInt256.ofNat input.size)) (UInt256.lor (UInt256.eq (UInt256.ofNat 65) (UInt256.ofNat input.size)) (UInt256.eq (UInt256.ofNat 128) (UInt256.ofNat input.size)))))))

theorem sizeFlag_hit (input : ByteArray)
    (hsize : input.size = 56 ∨ input.size = 120 ∨ input.size = 63 ∨ input.size = 64 ∨ input.size = 65 ∨ input.size = 128) : sizeFlag input = UInt256.ofNat 0 := by
  rcases hsize with h | h | h | h | h | h <;> unfold sizeFlag <;> rw [h] <;> decide

theorem sizeFlag_fail (input : ByteArray) (hfit : CalldataFits input)
    (hsize : input.size ≠ 56 ∧ input.size ≠ 120 ∧ input.size ≠ 63 ∧ input.size ≠ 64 ∧ input.size ≠ 65 ∧ input.size ≠ 128) : sizeFlag input = UInt256.ofNat 1 := by
  ...
  have e128 := DirectGuard.size_eq_zero input 128 hlt (by norm_num) hsize.2.2.2.2.2
  unfold sizeFlag
  rw [e56, e120, e63, e64, e65, e128]
  decide
