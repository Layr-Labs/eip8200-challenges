...
def prefixTemplate : List Instr :=
  [ .op (.Swap ⟨0, by decide⟩),
    ...
    .push ⟨2, by decide⟩ (UInt256.ofNat 3452),
...
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 2714 prefix_slice
    ...
    (by decide)
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 3404 := by
...
def suffixSite : GenericRoundSite Artifact.submissionArtifact .Osaka suffixTemplate :=
  StackSiteBuilder.ofSlice suffixTemplate 2752 suffix_slice
    ...
    (by decide)
theorem suffix_pc : suffixSite.startPC = UInt256.ofNat 3452 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2752) = UInt256.ofNat 3452
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem return_valid (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code 3452 = true := by
  have h := Artifact.submissionArtifact.isValidJumpDest_index 2757 (by rfl)
  have hp : Artifact.submissionArtifact.instructionPC 2757 = 3452 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  rw [hp] at h
  rw [hcode]
  exact h
theorem run_prefix (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq prefixTemplate {s with pc := UInt256.ofNat 3404, stack := inputStack x rho} =
...
theorem run_helper (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq helperTemplate {s with pc := UInt256.ofNat 5176, stack := prefixStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 3452, stack := helperStack s.memory x rho} := by
  have hcap (n : Nat) (hn : n ≤ 20) : rho.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (haddress : address ≤ 704) :
      UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat address 32) = s.activeWords :=
    active_schedule_preserved s.activeWords address hactive haddress
...
theorem run_suffix (s : State) (x : Input) (rho : List UInt256)
    ...
    (hactive : 23 ≤ s.activeWords.toNat)
    (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    runInstrSeq suffixTemplate {s with pc := UInt256.ofNat 3452, stack := helperStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 3490, stack := outputStack s.memory x rho} := by
  have hcap (n : Nat) (hn : n ≤ 20) : rho.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (haddress : address ≤ 704) :
      UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat address 32) = s.activeWords :=
    active_schedule_preserved s.activeWords address hactive haddress
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    (hactive : 23 ≤ s.activeWords.toNat)
    (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 3461, stack := inputStack x rho} =
      some {s with pc := UInt256.ofNat 3566, stack := outputStack s.memory x rho} := by
  have hcap (n : Nat) (hn : n ≤ 20) : rho.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (haddress : address ≤ 704) :
      UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat address 32) = s.activeWords :=
    active_schedule_preserved s.activeWords address hactive haddress
...
def gasSteps_raw (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3404, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3490, stack := outputStack s.memory x rho} := by
  have g0 := gasSteps_terminal_of_raw prefixSite {s with pc := UInt256.ofNat 3404, stack := inputStack x rho} _ hcode hfork hrun hnp prefix_pc.symm prefix_advances
  ...
  have g2 := gasSteps_terminal_of_raw suffixSite {s with pc := UInt256.ofNat 3452, stack := helperStack s.memory x rho} _ hcode hfork hrun hnp suffix_pc.symm suffix_advances
...
def gasSteps_pair (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 3404, stack := coreStack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] f rho}
      {s with pc := UInt256.ofNat 3490, stack := coreStack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] (pairEval s.memory f) rho} := by
