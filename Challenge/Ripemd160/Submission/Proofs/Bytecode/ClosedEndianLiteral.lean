...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.ClosedEndianLiteral
...
/-- The mask is fixed, so materialize it without a runtime division. -/
def maskPush (shift : Nat) : Instr :=
  .push ⟨32, by decide⟩ (if shift = 8 then mask8 else mask16)

def code (shift : Nat) : List Instr :=
  [factorPush shift, .op (.Dup ⟨1, by decide⟩), dup1,
   push1 (UInt256.ofNat shift), op .SHR, op .XOR,
   maskPush shift, op .AND, op .MUL, op .XOR]
...
theorem run_endian (s : State) (startPC value : UInt256) (shift : Nat)
  ...
  all_goals
    simp (config := { maxSteps := 1000000 })
      [code, factorPush, maskPush, endianFactorPush, endianFactor,
    ...
    all_goals simp [UInt256.mul, Fin.mul_def, Nat.mul_comm]
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.ClosedEndianLiteral
