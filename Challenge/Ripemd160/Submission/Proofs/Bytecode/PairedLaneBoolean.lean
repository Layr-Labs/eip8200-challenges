...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PairedLaneBoolean
...
theorem pack_and (a b c d : BitVec 32) :
  ...
  unfold pack
  rw [BitVec.and_append (x₁ := b.setWidth 128) (x₂ := d.setWidth 128)
    (y₁ := a.setWidth 128) (y₂ := c.setWidth 128)]
  rw [BitVec.setWidth_and, BitVec.setWidth_and]

theorem pack_or (a b c d : BitVec 32) :
    pack a b ||| pack c d = pack (a ||| c) (b ||| d) := by
  unfold pack
  rw [BitVec.or_append (x₁ := b.setWidth 128) (x₂ := d.setWidth 128)
    (y₁ := a.setWidth 128) (y₂ := c.setWidth 128)]
...
theorem pack_xor (a b c d : BitVec 32) :
    pack a b ^^^ pack c d = pack (a ^^^ c) (b ^^^ d) := by
  unfold pack
  rw [BitVec.xor_append (x₁ := b.setWidth 128) (x₂ := d.setWidth 128)
    (y₁ := a.setWidth 128) (y₂ := c.setWidth 128)]
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PairedLaneBoolean
