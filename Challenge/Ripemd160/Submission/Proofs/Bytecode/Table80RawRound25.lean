import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 theorem actual_slice :
     (Artifact.submissionArtifact.instructions.drop 1738).take template.length = template := by rfl
 def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
   StackSiteBuilder.ofSlice template 1738 actual_slice
     (by change 1738 + template.length ≤ Artifact.submissionInstructions.length
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 2253, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 2299, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 2253) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 2253) template = UInt256.ofNat 2299 := by decide
   rw [hend] at hraw
