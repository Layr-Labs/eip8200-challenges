import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 theorem site_pc : site.startPC = UInt256.ofNat 2473 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1959) = UInt256.ofNat 2473
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 2473, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 2512, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 2473) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 2473) template = UInt256.ofNat 2512 := by decide
   rw [hend] at hraw
