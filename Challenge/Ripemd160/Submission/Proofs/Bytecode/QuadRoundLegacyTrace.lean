...
/-! Legacy four-round evaluator retained as the semantic reference. -/
...
theorem runInstrSeq_legacy_quad (j : Nat) (hj : j < 5)
  ...
  have hcap (m : Nat) (hm : m ≤ 11) : rho.length + m < 1024 := by omega
  have hcap11 : rho.length + 11 < 1024 := by omega
  have hsecondPC : firstPC.add (UInt256.ofNat 1) = secondPC := by rfl
    ...
    calc
      pcAfter startPC (legacyQuadBeforeJumpTemplate j constant) =
          pcAfter (pcAfter startPC
            ...
            (cachedTailFTemplate j constant) := by
              rw [legacyQuadBeforeJumpTemplate, QuadRoundState.pcAfter_append]
      _ = pcAfter
      ...
      _ = pcAfter (pcAfter startPC (firstFTemplate j constant))
            ([swap1] ++ cachedTailFTemplate j constant) := by
              rw [QuadRoundState.pcAfter_append]
      _ = pcAfter secondPC (cachedTailFTemplate j constant) := by rfl
  rw [← hpc] at hswapTail
  have hall := runInstrSeq_append hfirst hgapRunning hswapTail
  simpa [legacyQuadBeforeJumpTemplate, List.append_assoc,
