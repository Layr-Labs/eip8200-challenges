...
@[simp] private theorem toNat544 : (832 : UInt256).toNat = 832 := by decide
@[simp] private theorem toNat576 : (864 : UInt256).toNat = 864 := by decide
@[simp] private theorem toNat608 : (896 : UInt256).toNat = 896 := by decide
@[simp] private theorem toNat640 : (928 : UInt256).toNat = 928 := by decide
@[simp] private theorem toNat672 : (960 : UInt256).toNat = 960 := by decide
...
private theorem initialized_h (i : Nat) (hi : i < 5) :
    slotWord initializedMemory 0x340 i =
...
theorem initializedState_hash (input : ByteArray) :
    ∀ i, i < 5 →
      slotWord (Main.initializedState input).memory 0x340 i =
        ofUInt32 (Crypto.Ripemd160.H0[i]!) := by
