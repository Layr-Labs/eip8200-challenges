...
/-- The writer multiplies the RAW pool word, which carries junk above bit 32 but stays below
`2 ^ 112`.  At that width the packed word is `[w's low 14 bytes][0000][w's low 14 bytes]`, so
the high-lane window is bytes `[0,14)` rather than `[10,14)`; the two agree for `w < 2 ^ 32`,
which is why the thirteen existing merges are unaffected. -/
theorem pack_nat_bound' (n : Nat) (hn : n < 2 ^ 112) :
    n * (2 ^ 144 + 1) < 2 ^ 256 := by
  calc
    n * (2 ^ 144 + 1) ≤ (2 ^ 112 - 1) * (2 ^ 144 + 1) :=
      Nat.mul_le_mul_right _ (by omega)
    _ < 2 ^ 256 := by norm_num

theorem packed_toNat' (w : UInt256) (hw : w.toNat < 2 ^ 112) :
    (packed w).toNat = w.toNat * (2 ^ 144 + 1) := by
  have hb : w.toNat * (2 ^ 144 + 1) < 2 ^ 256 := pack_nat_bound' _ hw
  unfold packed
  rw [mul_toNat, Word.word_toNat_ofNat,
    Nat.mod_eq_of_lt (by norm_num : (2 : Nat) ^ 144 + 1 < 2 ^ 256),
    Nat.mod_eq_of_lt hb]

theorem byte_pack_nat' (n i : Nat) (hn : n < 2 ^ 112) (hi : i < 32) :
    n * (2 ^ 144 + 1) / 256 ^ (32 - 1 - i) % 256 =
      if i < 14 then n / 256 ^ (32 - 1 - (i + 18)) % 256
      else n / 256 ^ (32 - 1 - i) % 256 := by
  by_cases h14 : i < 14
  · have hdiv : n * (2 ^ 144 + 1) / 2 ^ 144 = n := by
      have hnR : n < 2 ^ 144 := hn.trans (by norm_num)
    ...
    have hexp : (256 : Nat) ^ (32 - 1 - i) = 2 ^ 144 * 256 ^ (13 - i) := by
      interval_cases i <;> norm_num
    have hidx : 32 - 1 - (i + 18) = 13 - i := by omega
    rw [if_pos h14, hexp, ← Nat.div_div_eq_div_mul, hdiv, hidx]
  · rw [if_neg h14]
    have hc : (2 : Nat) ^ 144 % (256 ^ (32 - 1 - i) * 256) = 0 := by
      interval_cases i <;> norm_num
    rw [← Nat.mod_mul_right_div_self, ← Nat.mod_mul_right_div_self n]
...
theorem encoded_packed' (w : UInt256) (hw : w.toNat < 2 ^ 112)
    (i : Nat) (hi : i < 32) :
    (Data.Bytes.natToBytesPadded (packed w).toNat 32)[i]?.getD 0 =
      if i < 14 then
        (Data.Bytes.natToBytesPadded w.toNat 32)[i + 18]?.getD 0
...
theorem encoded_prefix_zero' (w : UInt256) (hw : w.toNat < 2 ^ 112)
    (i : Nat) (hi : i < 18) :
    (Data.Bytes.natToBytesPadded w.toNat 32)[i]?.getD 0 = 0 := by
  rw [YulEvmCompiler.BytesLemmas.natToBytesPadded_getElem?_getD _ _ _ (by omega)]
...
/-- Pointwise off the four exposed gap bytes, the middle store may be dropped with no
hypothesis about the gap at all: `three_store_merge`'s only use of `hgap` is the branch
covering exactly `i` in `[b + 14, a)`. -/
theorem two_store_residual (memory : ByteArray) (a b c : Nat) (w v : UInt256) (i : Nat)
    (hba : b + 18 = a) (hcb : c + 18 = b) (hw : w.toNat < 2 ^ 112)
    (hfree : i < b + 14 ∨ a ≤ i) :
    (writeWord (writeWord (writeWord memory a w) b w) c v)[i]?.getD 0 =
      (writeWord (writeWord memory a (UInt256.mul w (UInt256.ofNat (2 ^ 144 + 1))))
        c v)[i]?.getD 0 := by
  change (writeWord (writeWord (writeWord memory a w) b w) c v)[i]?.getD 0 =
    (writeWord (writeWord memory a (packed w)) c v)[i]?.getD 0
  simp only [writeWord, MachineState.writeBytes_getElem?_getD,
    YulEvmCompiler.BytesLemmas.natToBytesPadded_size]
  by_cases hlow : c ≤ i ∧ i < c + 32
  · simp only [if_pos hlow]
  · simp only [if_neg hlow]
    by_cases hmid : b ≤ i ∧ i < b + 32
    · rw [if_pos hmid]
      by_cases hin : a ≤ i ∧ i < a + 32
      · rw [if_pos hin, encoded_packed' w hw (i - a) (by omega), if_pos (by omega)]
        congr 2
        omega
      · exfalso
        rcases hfree with h | h <;> omega
    · rw [if_neg hmid]
      by_cases hin : a ≤ i ∧ i < a + 32
...
theorem writeWord_getD_congr (m1 m2 : ByteArray) (a : Nat) (v : UInt256) (i : Nat)
    (h : m1[i]?.getD 0 = m2[i]?.getD 0) :
    (writeWord m1 a v)[i]?.getD 0 = (writeWord m2 a v)[i]?.getD 0 := by
  simp only [writeWord, MachineState.writeBytes_getElem?_getD,
    YulEvmCompiler.BytesLemmas.natToBytesPadded_size]
  by_cases hin : a ≤ i ∧ i < a + 32
...
theorem prefix_zero_at (w : UInt256) (i : Nat) (hi : i < 32)
    (hw : w.toNat < 256 ^ (32 - 1 - i)) :
    (Data.Bytes.natToBytesPadded w.toNat 32)[i]?.getD 0 = 0 := by
  rw [YulEvmCompiler.BytesLemmas.natToBytesPadded_getElem?_getD _ _ _ hi,
    Nat.div_eq_of_lt hw]
  rfl
