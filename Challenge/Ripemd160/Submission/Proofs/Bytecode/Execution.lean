...
def mainStart (input : ByteArray) : State := atPC input 0x14b
...
def path_3ee : List
    (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [⟨178, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩]
...
def gasSteps_3ee (input : ByteArray) :
    Challenge.EvmProof.GasSteps (atPC input 0x14a) (mainStart input) := by
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_3ee
      (atPC input 0x14a) = some (mainStart input) := by
