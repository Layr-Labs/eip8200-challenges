...
def mainStart (input : ByteArray) : State := atPC input 353
...
def path_3ee : List
  ...
  [⟨237, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩]
...
def gasSteps_3ee (input : ByteArray) :
    Challenge.EvmProof.GasSteps (atPC input 352) (mainStart input) := by
  have hrun : Challenge.EvmProof.DataStepper.runLocatedBlock path_3ee
      (atPC input 352) = some (mainStart input) := by
    simp [path_3ee, Challenge.EvmProof.DataStepper.runLocatedBlock,
      ...
      atPC, mainStart, initialState, Artifact.instructionPC_237]
