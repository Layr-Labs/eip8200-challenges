...
def mainStart (input : ByteArray) : State := atPC input 463
...
def gasSteps_3ee (input : ByteArray) :
    Challenge.EvmProof.GasSteps (atPC input 462) (mainStart input) := by
  have hpc : Artifact.submissionArtifact.instructionPC 279 = 462 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_3ee
      (atPC input 462) = some (mainStart input) := by
