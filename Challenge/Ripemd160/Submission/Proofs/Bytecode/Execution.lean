...
def mainStart (input : ByteArray) : State := atPC input 0x16d
...
def gasSteps_3ee (input : ByteArray) :
    Challenge.EvmProof.GasSteps (atPC input 0x16c) (mainStart input) := by
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_3ee
      (atPC input 0x16c) = some (mainStart input) := by
