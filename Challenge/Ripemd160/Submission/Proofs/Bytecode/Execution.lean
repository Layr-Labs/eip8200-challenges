...
def mainStart (input : ByteArray) : State := atPC input 0x171

def path_start : List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) := []

def path_3ee : List
    (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [⟨204, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩]
...
def gasSteps_3ee (input : ByteArray) :
    Challenge.EvmProof.GasSteps (atPC input 0x170) (mainStart input) := by
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_3ee
      (atPC input 0x170) = some (mainStart input) := by
