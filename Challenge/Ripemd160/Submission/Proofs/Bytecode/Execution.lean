...
def path_start : List
  ...
  [⟨0, .push ⟨2, by decide⟩ (UInt256.ofNat 0x14ad), by rfl, by decide⟩,
...
def gasSteps_start (input : ByteArray) :
    Challenge.EvmProof.GasSteps (initialState submissionBytecode input 0) (atPC input 0x14ad) := by
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_start
      (atPC input 0) = some (atPC input 0x14ad) := by
    simp [path_start, Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
      atPC, initialState]
  have g : Challenge.EvmProof.GasSteps (atPC input 0) (atPC input 0x14ad) := by
