...
theorem jumpDest_12ce : Decode.isValidJumpDest submissionBytecode 0x1489 = true :=
  Artifact.submissionArtifact.isValidJumpDest_index 2780 (by rfl)
...
def loc0 :
  ...
  ⟨0, .push ⟨2, by decide⟩ (UInt256.ofNat 0x1489), by rfl, by decide⟩

theorem step0 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock [loc0]
      (st input 0x0 []) = some (st input 0x3 [UInt256.ofNat 0x1489]) := by
  rw [runLocatedBlock_single,
    runLocated_of_pc loc0 (pcFact input 0 0x0 [] (by norm_num) pc0)]
  exact runInstr_push input 0 2 0x1489 [] (by simp) (by decide) (by decide) (by norm_num)
...
theorem step1 (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock [loc1]
      (st input 0x3 [UInt256.ofNat 0x1489]) = some (st input 0x1489 []) := by
  rw [runLocatedBlock_single,
    runLocated_of_pc loc1 (pcFact input 1 0x3 [UInt256.ofNat 0x1489] (by norm_num) pc1)]
  exact runInstr_jump input 3 0x1489 [] (by simp) (by norm_num) jumpDest_12ce

def gasSteps_start (input : ByteArray) :
    Challenge.EvmProof.GasSteps (initialState submissionBytecode input 0) (atPC input 0x1489) := by
  rw [atPC_eq_st]
  exact
