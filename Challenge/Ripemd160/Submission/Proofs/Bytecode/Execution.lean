...
def mainStart (input : ByteArray) : State := atPC input 469
...
def gasSteps_3ee (input : ByteArray) :
    Challenge.EvmProof.GasSteps (atPC input 468) (mainStart input) := by
  have hpc : Artifact.submissionArtifact.instructionPC 280 = 468 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_3ee
      (atPC input 468) = some (mainStart input) := by
    simp [path_3ee, Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
      atPC, mainStart, initialState, hpc]
