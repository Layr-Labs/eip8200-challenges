...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2806).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2806 actual_slice
    (by change 2806 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3447 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2806) = UInt256.ofNat 3447
