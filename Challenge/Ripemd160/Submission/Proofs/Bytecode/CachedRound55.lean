...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2920).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2920 actual_slice
    (by change 2920 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3605 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2920) = UInt256.ofNat 3605
