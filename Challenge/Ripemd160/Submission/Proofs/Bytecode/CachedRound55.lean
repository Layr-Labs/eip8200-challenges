...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2923).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2923 actual_slice
    (by change 2923 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3598 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2923) = UInt256.ofNat 3598
