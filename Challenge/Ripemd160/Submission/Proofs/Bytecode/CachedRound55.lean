...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2907).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2907 actual_slice
    (by change 2907 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3613 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2907) = UInt256.ofNat 3613
