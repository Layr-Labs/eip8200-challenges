...
theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
  ...
  have hkb : StaggerAlgorithm.physicalKey 20 = UInt256.ofNat 34535016170389834383797866734609369064725061767952821 := by decide
  have hka : StaggerAlgorithm.physicalKey 21 = UInt256.ofNat 34535016170389834383797866734609369064725061767952821 := by decide
  ...
  have hrescale (x y : UInt256) :
      UInt256.land pairWord (UInt256.add y (UInt256.shiftRight (UInt256.mul (UInt256.ofNat 10141204804187018453417038184478) x) (UInt256.ofNat 24))) =
      UInt256.land pairWord (UInt256.add y (UInt256.shiftRight (UInt256.mul (UInt256.ofNat 20282409608374036906834076368928) x) (UInt256.ofNat 25))) := by
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2042, stack := stack s.memory h4 [ .d, .a, .cachedMessage 360, .pair, .upper, .e, .c, .literal 23, .k, .b, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] q right (StaggerAlgorithm.physicalKey 20) rho}
      {s with pc := UInt256.ofNat 2097, stack := stack s.memory h4 [ .d, .e, .cachedMessage 360, .pair, .upper, .a, .b, .literal 23, .k, .c, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 21) rho} := by
