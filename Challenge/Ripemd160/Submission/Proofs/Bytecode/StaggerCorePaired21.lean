...
theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired21.outputStack memory (input memory h4 q right (StaggerAlgorithm.physicalKey 20)) rho =
      stack memory h4 [ .d, .late, .pair, .upper, .a, .b, .e, .k, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] (eval memory q) right (StaggerAlgorithm.physicalKey 21) rho := by
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2121, stack := stack s.memory h4 [ .d, .late, .pair, .upper, .e, .c, .a, .k, .b, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] q right (StaggerAlgorithm.physicalKey 20) rho}
      {s with pc := UInt256.ofNat 2168, stack := stack s.memory h4 [ .d, .late, .pair, .upper, .a, .b, .e, .k, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 21) rho} := by
