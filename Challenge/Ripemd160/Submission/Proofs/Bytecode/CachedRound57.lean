...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3002).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3002 actual_slice
    (by change 3002 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3709 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3002) = UInt256.ofNat 3709
