...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3003).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3003 actual_slice
    (by change 3003 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3709 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3003) = UInt256.ofNat 3709
