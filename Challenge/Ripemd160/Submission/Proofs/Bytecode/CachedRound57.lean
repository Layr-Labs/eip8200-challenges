...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2998).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2998 actual_slice
    (by change 2998 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3709 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2998) = UInt256.ofNat 3709
