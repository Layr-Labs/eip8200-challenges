...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3000).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3000 actual_slice
    (by change 3000 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3715 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3000) = UInt256.ofNat 3715
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 3617, stack := inputStack s.memory x rho} =
