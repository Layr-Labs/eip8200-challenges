...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3010).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3010 actual_slice
    (by change 3010 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3707 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3010) = UInt256.ofNat 3707
