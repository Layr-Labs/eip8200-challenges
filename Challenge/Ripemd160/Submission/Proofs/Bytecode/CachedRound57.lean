...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2928).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2928 actual_slice
    (by change 2928 + actualTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualTemplate) (by decide))
    (by decide)
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3497 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2928) = UInt256.ofNat 3497
...
theorem run_actual (s : State) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 996) (hrun : s.halt = .Running)
    (hactive : 23 ≤ s.activeWords.toNat) :
    runInstrSeq actualTemplate {s with pc := UInt256.ofNat 3497, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 3548, stack := outputStack s.memory x rho} := by
  have hcap (n : Nat) (hn : n ≤ 26) : rho.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (haddress : address ≤ 704) :
      UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat address 32) = s.activeWords :=
    active_schedule_preserved s.activeWords address hactive haddress
...
def gasSteps (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3497, stack := coreStack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] f (cache s.memory ++ rho)}
      {s with pc := UInt256.ofNat 3548, stack := coreStack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] (PairedAllInlineCoreTrace.inline57Block.eval s.memory f) (cache s.memory ++ rho)} := by
  ...
  have g := gasSteps_terminal_of_raw actualSite
    {s with pc := UInt256.ofNat 3497, stack := inputStack s.memory x rho} _
