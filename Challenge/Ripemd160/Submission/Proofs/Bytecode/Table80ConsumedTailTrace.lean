...
theorem runLocatedBlock_tail_prefix {artifact : ProgramArtifact} {fork : Fork}
    ...
    (s : State) (ret : UInt256) (q : WordLane) (factor : UInt256) (rho : List UInt256)
    (hstack : rho.length ≤ 996) (hrun : s.halt = .Running)
    (hactive : 34 ≤ s.activeWords.toNat) :
    Stepper.runLocatedBlock site.path {s with pc := site.startPC, stack := entryStack factor q ret rho} =
  ...
  rw [DenseScheduleLift.runLocatedBlock_eq_raw site tail_prefix_advances
    {s with pc := site.startPC, stack := entryStack factor q ret rho} rfl]
  have h := run_prefix s site.startPC ret q factor rho hstack hrun hactive
  rw [← hend] at h
  exact h
...
theorem runLocatedBlock_tail {artifact : ProgramArtifact} {fork : Fork}
    (site : TailSite artifact fork) (s : State) (ret : UInt256) (q : WordLane) (factor : UInt256)
    ...
    Stepper.runLocatedBlock site.path
      {s with pc := site.prefixSite.startPC, stack := entryStack factor q ret rho} =
  ...
  · exact runLocatedBlock_tail_prefix site.prefixSite s ret q factor rho hstack hrun hactive
  · exact hrun
...
def gasSteps_tail {artifact : ProgramArtifact} {fork : Fork}
    ...
    GasSteps {s with pc := site.prefixSite.startPC, stack := entryStack factor q ret rho}
      {s with pc := ret, stack := rho, memory := resultMemory s.memory q} := by
  apply Stepper.runLocatedBlock_sound artifact fork site.path
  · exact hcode
  · exact hfork
  · exact runLocatedBlock_tail site s ret q factor rho hstack hrun hactive hvalid
  · exact hrun
  · exact hnp
