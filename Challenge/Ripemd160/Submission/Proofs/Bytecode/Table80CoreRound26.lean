...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound26
...
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound26.outputStack memory (input q k) rho =
      stack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 550) k q) (nextKey k) rho := by
  simp only [Table80RawRound26.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired80Algorithm.physicalKey 26) =
     Paired80Algorithm.physicalKey 27 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired80Algorithm.physicalKey 26) q = Paired80Algorithm.step 26 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 2309, stack := stack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 2356, stack :=
          stack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 550) k q) (nextKey k) rho} := by
   have gs := Table80RawRound26.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound26
