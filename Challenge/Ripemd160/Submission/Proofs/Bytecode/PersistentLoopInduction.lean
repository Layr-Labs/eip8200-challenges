...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentLoopInduction
open EvmSemantics EvmSemantics.EVM YulEvmCompiler Challenge.EvmProof
open PersistentLoopRaw
...
def loopState (s : State) (h : Compression.HashState) (i count : Nat) (rho : List UInt256) : State :=
  {s with pc := UInt256.ofNat 505, stack := PersistentFrame.frame h (offsetWord i) (limitWord count) rho}

def postState (s : State) (h : Compression.HashState) (i count : Nat) (rho : List UInt256) : State :=
  {s with pc := UInt256.ofNat 490, stack := PersistentFrame.frame h (offsetWord i) (limitWord count) rho}

def exitState (s : State) (h : Compression.HashState) (count : Nat) (rho : List UInt256) : State :=
  {s with pc := UInt256.ofNat 4704, stack := PersistentFrame.frame h (limitWord count) (limitWord count) rho}
...
def run_blocks (states : Nat → State) (hashes : Nat → Compression.HashState)
  ...
  have hb : ∀ i, i < count → GasSteps (I i) (I (i + 1)) := by
    ...
    · have gp := PersistentLoopSites.gasSteps_continue (states (i + 1)) (hashes (i + 1))
    ...
    · have he : i + 1 = count := by omega
      have gp := PersistentLoopSites.gasSteps_exit (states (i + 1)) (hashes (i + 1))
        (offsetWord i) (limitWord count) rho (by omega) a.running
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentLoopInduction
