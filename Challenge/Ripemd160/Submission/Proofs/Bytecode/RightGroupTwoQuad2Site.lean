...
private theorem template_slice :
    (A.instructions.drop 2942).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2942 template_slice (by
    change 2924 + template.length ≤ Artifact.submissionInstructions.length
