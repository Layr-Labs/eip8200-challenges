...
private theorem template_slice :
    (A.instructions.drop 2952).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2952 template_slice (by
    change 2950 + template.length ≤ Artifact.submissionInstructions.length
