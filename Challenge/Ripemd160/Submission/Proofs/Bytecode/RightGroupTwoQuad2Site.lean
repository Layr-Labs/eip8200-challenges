...
private theorem template_slice :
    (A.instructions.drop 2928).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2928 template_slice (by
    change 2926 + template.length ≤ Artifact.submissionInstructions.length
