...
theorem runInstrSeq_denseHalf
  ...
  have h8 := runInstrSeq_endianStage s startPC value 8 mask8
    (other :: rest) (by simp; omega) (Or.inl ⟨rfl, rfl⟩) hrun (half == 1)
  have h16 := runInstrSeq_endianStage s
    (pcAfter startPC (endianStage8 (half == 1)))
    (packedStage value 8 mask8) 16 mask16 (other :: rest)
    (by simp; omega) (Or.inr ⟨rfl, rfl⟩) hrun (half == 1)
  have hstore := runInstrSeq_denseStore s
    (pcAfter (pcAfter startPC (endianStage8 (half == 1))) (endianStage16 (half == 1)))
  ...
  have hhalf :
      runInstrSeq ((endianStage8 (half == 1)) ++ ((endianStage16 (half == 1)) ++
        [push1 (UInt256.ofNat (denseStoreAddress half)), op .MSTORE]))
        { s with pc := startPC, stack := value :: other :: rest } =
      some { s with
        pc := pcAfter (pcAfter (pcAfter startPC (endianStage8 (half == 1))) (endianStage16 (half == 1)))
          [push1 (UInt256.ofNat (denseStoreAddress half)), op .MSTORE]
        stack := other :: rest
  ...
  have hpc :
      pcAfter startPC (denseHalfTemplate half) =
        pcAfter (pcAfter (pcAfter startPC (endianStage8 (half == 1))) (endianStage16 (half == 1)))
