...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v3 (UInt256.shiftRight (UInt256.mul x.v10 x.v9) (UInt256.ofNat 28))),
    ...
    x.v7,
    x.v8,
    (UInt256.land x.v3 (UInt256.add x.v7 (UInt256.shiftRight (UInt256.mul x.v10 (UInt256.land x.v3 (UInt256.add x.v8 (UInt256.add x.v2 (UInt256.add (UInt256.xor (UInt256.lor (UInt256.land x.v9 x.v11) x.v0) (UInt256.xor x.v11 (UInt256.xor x.v6 x.v9))) x.v5))))) (UInt256.ofNat 25)))),
    x.v10,
    x.v11,
    x.v12,
    x.v13,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3374).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3374 actual_slice
    (by change 3374 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4280 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3374) = UInt256.ofNat 4280
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4280, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4316, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4280) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4280) template = UInt256.ofNat 4316 := by decide
  rw [hend] at hraw
