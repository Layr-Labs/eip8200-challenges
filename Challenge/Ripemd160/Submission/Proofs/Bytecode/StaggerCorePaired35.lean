...
def eval (memory : ByteArray) (q : WordLane) : WordLane :=
  StaggerAlgorithm.step 35 (MachineState.readWord memory 110) q

theorem output_eq (memory : ByteArray) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired35.outputStack memory (input memory q right (StaggerAlgorithm.physicalKey 34)) rho =
      stack memory [ .d, .pair, .upper, .k, .b, .a, .e, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500, .cache 230 ] (eval memory q) right (StaggerAlgorithm.physicalKey 35) rho := by
  have hkb : StaggerAlgorithm.physicalKey 34 = UInt256.ofNat 7411972540165114580685751011896225 := by decide
  have hka : StaggerAlgorithm.physicalKey 35 = UInt256.ofNat 7411972540165114580685751011896225 := by decide
...
def gasSteps (s : State) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2708, stack := stack s.memory [ .d, .pair, .upper, .k, .c, .e, .a, .b, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500, .cache 230 ] q right (StaggerAlgorithm.physicalKey 34) rho}
      {s with pc := UInt256.ofNat 2745, stack := stack s.memory [ .d, .pair, .upper, .k, .b, .a, .e, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500, .cache 230 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 35) rho} := by
  have g := StaggerRawPaired35.gasSteps s (input s.memory q right (StaggerAlgorithm.physicalKey 34))
