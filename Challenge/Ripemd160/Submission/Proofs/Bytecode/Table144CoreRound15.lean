import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144RawRound15
...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound15
...
def eval (message k : UInt256) (q : WordLane) : WordLane := wordStep 0 8 6 message k q
def nextKey (k : UInt256) : UInt256 := (UInt256.lor (UInt256.ofNat 1518500249) (UInt256.shiftLeft (UInt256.ofNat 1548603684) (UInt256.ofNat 144)))
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table144RawRound15.outputStack memory (input q k) rho =
      stack [.k, .d, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 864) k q) (nextKey k) rho := by
  simp only [Table144RawRound15.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired144Algorithm.physicalKey 15) =
     Paired144Algorithm.physicalKey 16 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired144Algorithm.physicalKey 15) q = Paired144Algorithm.step 15 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 1783, stack := stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 1839, stack :=
          stack [.k, .d, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 864) k q) (nextKey k) rho} := by
   have gs := Table144RawRound15.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound15
