...
theorem tail_eq : (Artifact.submissionArtifact.instructions.drop 3619).take tail.length = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3619 = 4898 := by
  rw [instructionPC_eq_byteLength]
  rfl

theorem get (index : Nat) (h : index < tail.length := by decide) :
    Artifact.submissionArtifact.instructions[3619 + index]? = tail[index]? := by
...
theorem pc (index : Nat) (h : index ≤ tail.length := by decide) :
    Artifact.submissionArtifact.instructionPC (3619 + index) =
      4898 + byteLength (tail.take index) := by
  have ht : (Artifact.submissionArtifact.instructions.drop 3619).take index = tail.take index := by
