...
theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3624 = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3624 = 4536 := by
...
theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3624 + index) =
      4536 + byteLength (tail.take index) := by
