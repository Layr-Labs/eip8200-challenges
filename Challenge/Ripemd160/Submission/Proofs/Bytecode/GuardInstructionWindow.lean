...
theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3731 = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3731 = 4664 := by
  rw [instructionPC_eq_byteLength]
  rfl

theorem get (index : Nat) :
    Artifact.submissionArtifact.instructions[3731 + index]? = tail[index]? := by
  rw [← InstructionWindow.get_drop, tail_eq]

theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3731 + index) =
      4664 + byteLength (tail.take index) := by
