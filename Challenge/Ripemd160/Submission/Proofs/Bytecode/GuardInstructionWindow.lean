...
theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3645 = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3645 = 4852 := by
  rw [instructionPC_eq_byteLength]
  rfl

theorem get (index : Nat) :
    Artifact.submissionArtifact.instructions[3645 + index]? = tail[index]? := by
  rw [← InstructionWindow.get_drop, tail_eq]

theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3645 + index) =
      4852 + byteLength (tail.take index) := by
