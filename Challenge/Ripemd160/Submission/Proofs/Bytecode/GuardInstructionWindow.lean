import Challenge.Ripemd160.Submission.Proofs.Bytecode.Artifact
import Challenge.Ripemd160.Submission.Proofs.Bytecode.InstructionWindow
set_option warningAsError true
set_option maxRecDepth 100000
set_option maxHeartbeats 2000000
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.GuardInstructionWindow
open EvmSemantics YulEvmCompiler Challenge.EvmProof ArtifactByteLength
open Artifact (op)
def tail : List Instr :=
[
  op 0x93,
  op 0x01,
  op 0x97,
  .push 1 144,
  op 0x1c,
  op 0x98,
  .push 1 144,
  op 0x1c,
  op 0x01,
  op 0x99,
  .push 1 144,
  op 0x1c,
  op 0x01,
  op 0x97,
  .push 1 144,
  op 0x1c,
  op 0x94,
  .push 1 144,
  op 0x1c,
  op 0x01,
  op 0x91,
  op 0x50,
  op 0x9f,
  op 0x01,
  op 0x88,
  op 0x16,
  op 0x9d,
  op 0x01,
  op 0x01,
  op 0x86,
  op 0x16,
  op 0x9e,
  op 0x01,
  op 0x01,
  op 0x84,
  op 0x16,
  op 0x98,
  op 0x01,
  op 0x83,
  op 0x16,
  op 0x9a,
  op 0x01,
  op 0x82,
  op 0x16,
  op 0x98,
  op 0x50,
  op 0x9a,
  .push 1 64,
  op 0x01,
  op 0x9a,
  op 0x8c,
  op 0x8c,
  op 0x10,
  .push 2 503,
  op 0x57,
  op 0x36,
  op 0x8c,
  op 0x14,
  .push 2 4695,
  op 0x57,
  op 0x50,
  op 0x50,
  op 0x50,
  op 0x50,
  op 0x50,
  op 0x50,
  op 0x93,
  .push 1 32,
  op 0x1b,
  op 0x17,
  .push 1 32,
  op 0x1b,
  op 0x17,
  .push 1 32,
  op 0x1b,
  op 0x17,
  .push 1 32,
  op 0x1b,
  op 0x17,
  op 0x80,
  op 0x80,
  .push 1 8,
  op 0x1c,
  op 0x18,
  op 0x84,
  op 0x16,
  .push 2 257,
  op 0x02,
  op 0x18,
  op 0x80,
  op 0x80,
  .push 1 16,
  op 0x1c,
  op 0x18,
  op 0x85,
  op 0x16,
  .push 3 65537,
  op 0x02,
  op 0x18,
  .push 0 0,
  op 0x52,
  .push 1 32,
  .push 0 0,
  op 0xf3,
  op 0x5b,
  .push 1 128,
  op 0x36,
  .push 2 1120,
  op 0x01,
  op 0x53,
  op 0x36,
  .push 1 3,
  op 0x1b,
  op 0x81,
  .push 2 1112,
  op 0x01,
  op 0x5b,
  op 0x81,
  op 0x81,
  op 0x53,
  .push 1 1,
  op 0x01,
  op 0x90,
  .push 1 8,
  op 0x1c,
  op 0x90,
  op 0x81,
  .push 2 4672,
  op 0x57,
  op 0x50,
  op 0x50,
  .push 2 396,
  op 0x56,
  op 0x5b,
  .push 2 1112,
  op 0x36,
  .push 0 0,
  op 0x37,
  .push 1 128,
  op 0x36,
  .push 1 3,
  op 0x1b,
  op 0x80,
  .push 1 162,
  op 0x52,
  op 0x80,
  .push 2 666,
  op 0x52,
  .push 1 144,
  op 0x52,
  op 0x80,
  .push 2 522,
  op 0x52,
  op 0x80,
  .push 1 54,
  op 0x52,
  .push 1 36,
  op 0x52,
  op 0x36,
  .push 1 29,
  op 0x1c,
  op 0x15,
  .push 2 925,
  op 0x57,
  op 0x36,
  .push 1 29,
  op 0x1c,
  op 0x80,
  .push 2 1008,
  op 0x52,
  op 0x80,
  .push 2 990,
  op 0x52,
  op 0x80,
  .push 2 648,
  op 0x52,
  op 0x80,
  .push 2 612,
  op 0x52,
  .push 2 270,
  op 0x52,
  .push 2 925,
  op 0x56,
  op 0x5b,
  .push 17 342276208914615837337402008677671501826,
  op 0x36,
  op 0x1c,
  .push 1 1,
  op 0x16,
  .push 1 115,
  op 0x57,
  op 0x36,
  .push 2 256,
  op 0x14,
  op 0x36,
  .push 2 376,
  op 0x14,
  op 0x36,
  .push 2 1000,
  op 0x14,
  op 0x17,
  op 0x17,
  .push 1 115,
  op 0x57,
  .push 3 2127393,
  op 0x36,
  op 0x02,
  .push 0 0,
  op 0x35,
  .push 1 232,
  op 0x1c,
  op 0x18,
  .push 2 358,
  op 0x57,
  op 0x36,
  .push 20 25448770637332498804579667936807160623886401639,
  op 0x02,
  .push 20 890993315260586290631548281360202943075753233713,
  op 0x03,
  .push 0 0,
  op 0x52,
  op 0x59,
  .push 0 0,
  op 0xf3
]
theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3619 = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3619 = 4528 := by
  rw [instructionPC_eq_byteLength]
  rfl

theorem get (index : Nat) :
    Artifact.submissionArtifact.instructions[3619 + index]? = tail[index]? := by
  rw [← InstructionWindow.get_drop, tail_eq]

theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3619 + index) =
      4528 + byteLength (tail.take index) := by
  rw [InstructionWindow.pc_drop, pc_base, tail_eq]

#print axioms get
#print axioms pc
end Challenge.Ripemd160.Submission.Proofs.Bytecode.GuardInstructionWindow
