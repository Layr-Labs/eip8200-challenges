...
theorem tail_eq : (Artifact.submissionArtifact.instructions.drop 265).take tail.length = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 265 = 472 := by
  rw [instructionPC_eq_byteLength]
  rfl

theorem get (index : Nat) (h : index < tail.length := by decide) :
    Artifact.submissionArtifact.instructions[265 + index]? = tail[index]? := by
...
theorem pc (index : Nat) (h : index ≤ tail.length := by decide) :
    Artifact.submissionArtifact.instructionPC (265 + index) =
      472 + byteLength (tail.take index) := by
  have ht : (Artifact.submissionArtifact.instructions.drop 265).take index = tail.take index := by
