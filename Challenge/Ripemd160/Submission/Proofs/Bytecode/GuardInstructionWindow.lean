...
theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3697 = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3697 = 4878 := by
  rw [instructionPC_eq_byteLength]
  rfl

theorem get (index : Nat) :
    Artifact.submissionArtifact.instructions[3700 + index]? = tail[index]? := by
  rw [← InstructionWindow.get_drop, tail_eq]

theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3700 + index) =
      4878 + byteLength (tail.take index) := by
