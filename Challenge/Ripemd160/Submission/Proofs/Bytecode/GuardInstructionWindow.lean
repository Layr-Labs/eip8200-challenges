...
theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3646 = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3646 = 4853 := by
...
theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3646 + index) =
      4853 + byteLength (tail.take index) := by
