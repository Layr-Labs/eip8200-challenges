import Challenge.Ripemd160.Submission.Proofs.Bytecode.Artifact
import Challenge.Ripemd160.Submission.Proofs.Bytecode.InstructionWindow
set_option warningAsError true
set_option maxRecDepth 100000
set_option maxHeartbeats 2000000
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.GuardInstructionWindow
open EvmSemantics YulEvmCompiler Challenge.EvmProof ArtifactByteLength
open Artifact (op)
def tail : List Instr :=
[
  op 0x98,
  .push 1 144,
  op 0x1c,
  op 0x01,
  op 0x94,
  op 0x50,
  op 0x99,
  .push 1 144,
  op 0x1c,
  op 0x01,
  op 0x9b,
  op 0x50,
  op 0x50,
  op 0x50,
  op 0x94,
  .push 1 144,
  op 0x1c,
  op 0x01,
  op 0x9a,
  op 0x50,
  .push 1 144,
  op 0x1c,
  op 0x01,
  op 0x97,
  op 0x50,
  .push 1 144,
  op 0x1c,
  op 0x9c,
  op 0x01,
  op 0x83,
  op 0x16,
  op 0x9b,
  op 0x01,
  op 0x93,
  op 0x50,
  op 0x50,
  op 0x94,
  op 0x01,
  op 0x84,
  op 0x16,
  op 0x96,
  op 0x01,
  op 0x83,
  op 0x16,
  op 0x94,
  op 0x01,
  op 0x82,
  op 0x16,
  op 0x92,
  op 0x01,
  op 0x16,
  op 0x94,
  .push 2 64,
  op 0x01,
  op 0x94,
  op 0x86,
  op 0x86,
  op 0x18,
  .push 2 519,
  op 0x57,
  .push 2 32,
  op 0x1b,
  op 0x17,
  .push 1 32,
  op 0x1b,
  op 0x17,
  .push 1 32,
  op 0x1b,
  op 0x17,
  .push 1 32,
  op 0x1b,
  op 0x17,
  .push 2 257,
  .push 19 5686776798952929642815894290724836652357714175,
  op 0x82,
  op 0x83,
  .push 1 8,
  op 0x1c,
  op 0x18,
  op 0x16,
  op 0x02,
  op 0x18,
  op 0x80,
  .push 1 16,
  op 0x1c,
  op 0x81,
  op 0x18,
  .push 18 22300404921355919834653475635385858670002175,
  op 0x16,
  .push 3 65537,
  op 0x02,
  op 0x18,
  .push 0 0,
  op 0x52,
  .push 1 32,
  .push 0 0,
  op 0xf3,
  op 0x5b,
  .push 0 0,
  op 0x35,
  .push 0 0,
  op 0x1a,
  .push 1 7,
  op 0x18,
  .push 2 5207,
  op 0x57,
  .push 0 0,
  .push 17 342276208914615837337402008677671501826,
  op 0x36,
  op 0x1c,
  .push 1 1,
  op 0x16,
  .push 1 106,
  op 0x57,
  op 0x50,
  .push 2 329,
  op 0x56,
  op 0x5b,
  .push 1 20,
  .push 1 15,
  op 0x36,
  .push 3 2377101,
  op 0x04,
  op 0x16,
  .push 1 21,
  op 0x02,
  .push 2 4914,
  op 0x01,
  .push 1 12,
  op 0x39,
  op 0x59,
  .push 0 0,
  op 0xf3,
  .push 20 1238202210714422317976759695583289414507727951107,
  .push 20 444633252455795925629383436637892360005217141692,
  .push 20 1409020389675646681432984939370312113762541159597,
  .push 20 317028691540587235129640223973284367834059040749,
  .push 20 51536278723262616553055226583496568135069946185,
  .push 20 1134778670259348587079032227229296800638025290378,
  .push 20 788302141291229731098395206502327271368262842391,
  .push 20 248818459742486850486840648156276084652376766343,
  .push 20 1251998741523656777300485297472400595618864439423,
  .push 20 766350606435067737561421097975693824639675460820,
  .push 20 407177000021581765662970131450951124156487664282,
  .push 20 233347948783734465632298330963582576039387513209,
  .push 20 153055736697849525067355378316411867211147587005,
  .push 20 524717510892769952200770787354467678713137771213,
  op 0x5b,
  op 0x36,
  .push 1 2,
  op 0x1c,
  .push 2 329,
  op 0x57,
  .push 3 2127393,
  op 0x36,
  op 0x02,
  .push 0 0,
  op 0x35,
  .push 1 232,
  op 0x1c,
  op 0x18,
  .push 2 329,
  op 0x57,
  .push 20 890993315260586290631548281360202943075753233713,
  op 0x36,
  .push 2 411,
  op 0x56
]
theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3761 = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3761 = 4688 := by
  rw [instructionPC_eq_byteLength]
  rfl

theorem get (index : Nat) :
    Artifact.submissionArtifact.instructions[3761 + index]? = tail[index]? := by
  rw [← InstructionWindow.get_drop, tail_eq]

theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3761 + index) =
      4688 + byteLength (tail.take index) := by
  rw [InstructionWindow.pc_drop, pc_base, tail_eq]

#print axioms get
#print axioms pc
end Challenge.Ripemd160.Submission.Proofs.Bytecode.GuardInstructionWindow
