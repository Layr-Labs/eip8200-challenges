...
theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3668 = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3668 = 4563 := by
...
theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3668 + index) =
      4563 + byteLength (tail.take index) := by
