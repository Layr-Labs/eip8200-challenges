...
theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3745 = tail := by rfl

theorem pc_base : Artifact.submissionArtifact.instructionPC 3745 = 4872 := by
  rw [instructionPC_eq_byteLength]
  rfl

theorem get (index : Nat) :
    Artifact.submissionArtifact.instructions[3745 + index]? = tail[index]? := by
  rw [← InstructionWindow.get_drop, tail_eq]

theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3745 + index) =
      4872 + byteLength (tail.take index) := by
