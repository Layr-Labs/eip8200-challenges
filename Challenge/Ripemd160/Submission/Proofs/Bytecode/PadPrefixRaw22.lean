...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRaw22
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRaw22
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 4805).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 4805 actual_slice
    (by change 4805 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 6040 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4805) = UInt256.ofNat 6040
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 6040, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 6080, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 6040) x rho hstack hrun hactive hz
  have hend : pcAfter (UInt256.ofNat 6040) template = UInt256.ofNat 6080 := by decide
  rw [hend] at hraw
