...
theorem site_pc : site.startPC = UInt256.ofNat 2708 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2119) = UInt256.ofNat 2708
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2708, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2745, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2708) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2708) template = UInt256.ofNat 2745 := by decide
  rw [hend] at hraw
