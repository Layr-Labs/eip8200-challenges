...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawPaired35
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2026).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2026 actual_slice
    (by change 2026 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2637 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2026) = UInt256.ofNat 2637
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2637, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2673, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2637) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2637) template = UInt256.ofNat 2673 := by decide
  rw [hend] at hraw
  exact DenseScheduleLift.gasSteps_of_raw site _ _ hcode hfork hrun hnp site_pc.symm advances hraw
#print axioms gasSteps
end Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawPaired35
