...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2019 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2659 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2019) = UInt256.ofNat 2659
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2659, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2695, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2659) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2659) template = UInt256.ofNat 2695 := by decide
  rw [hend] at hraw
