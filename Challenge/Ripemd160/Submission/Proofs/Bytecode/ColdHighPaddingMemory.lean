...
theorem finalMemory_poolClear (input : ByteArray) (hfit : CalldataFits input)
    (hpositive : 0 < input.size) (i : Nat) (hi : i ≤ DriverTrace.blockCount input) :
    PairStoreGap.PoolClear (finalMemory input i) := by
  have h := ColdHighMemory.padRealChain_poolClear (states input i).memory (UInt256.ofNat input.size)
    (size_word_lt input hfit) (states_context input hfit hpositive i hi).lowClear
  exact PairStoreGap.poolClear_congr _ _
    (fun a hA => lengthMemory_below input _ hfit _ (PaddingTrace.lengthStop_le input) a (by omega)) h

#print axioms finalMemory_blockAt
#print axioms finalMemory_lowClear
#print axioms finalMemory_gapClear
#print axioms finalMemory_poolClear
