...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawPack
...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.lor x.v7 (UInt256.shiftLeft x.v7 (UInt256.ofNat 80))),
    ...
    (UInt256.lor (MachineState.readWord memory 960) (UInt256.shiftLeft x.v4 (UInt256.ofNat 80))),
    (UInt256.lor (MachineState.readWord memory 864) (UInt256.shiftLeft x.v3 (UInt256.ofNat 80))),
    (UInt256.lor (MachineState.readWord memory 832) (UInt256.shiftLeft x.v5 (UInt256.ofNat 80))),
    (UInt256.lor (MachineState.readWord memory 928) (UInt256.shiftLeft x.v0 (UInt256.ofNat 80))),
    (UInt256.lor (MachineState.readWord memory 896) (UInt256.shiftLeft x.v2 (UInt256.ofNat 80))),
    x.v6,
    x.v7,
    x.v8,
    x.v9,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 720).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 720 actual_slice
    (by change 720 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1090 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 720) = UInt256.ofNat 1090
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1090, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1144, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1090) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1090) template = UInt256.ofNat 1144 := by decide
  rw [hend] at hraw
  exact DenseScheduleLift.gasSteps_of_raw site _ _ hcode hfork hrun hnp site_pc.symm advances hraw
#print axioms gasSteps
end Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawPack
