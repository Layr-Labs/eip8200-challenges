...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1976 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2631 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1976) = UInt256.ofNat 2631
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2631, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2691, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2631) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2631) template = UInt256.ofNat 2691 := by decide
  rw [hend] at hraw
