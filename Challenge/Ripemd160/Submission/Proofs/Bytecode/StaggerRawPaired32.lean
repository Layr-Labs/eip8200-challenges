...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2021 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2641 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2021) = UInt256.ofNat 2645
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2641, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2700, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2641) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2641) template = UInt256.ofNat 2700 := by decide
  rw [hend] at hraw
