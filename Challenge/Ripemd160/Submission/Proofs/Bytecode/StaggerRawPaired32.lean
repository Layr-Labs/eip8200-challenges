...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v2 (UInt256.shiftRight (UInt256.mul x.v9 x.v8) (UInt256.ofNat 28))),
    ...
    x.v0,
    x.v7,
    (UInt256.land x.v2 (UInt256.add x.v7 (UInt256.shiftRight (UInt256.mul x.v9 (UInt256.land x.v2 (UInt256.add (UInt256.ofNat 136726760529788758926252426176837954510549114783656865) (UInt256.add (MachineState.readWord memory 972) (UInt256.add (UInt256.xor x.v0 (UInt256.lor (UInt256.lnot x.v8) x.v5)) x.v4))))) (UInt256.ofNat 27)))),
    x.v9,
    x.v10,
    x.v11,
    x.v12,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1979).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1979 actual_slice
    (by change 1979 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2632 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1979) = UInt256.ofNat 2632
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2632, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2692, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2632) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2632) template = UInt256.ofNat 2692 := by decide
  rw [hend] at hraw
