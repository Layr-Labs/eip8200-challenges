...
theorem site_pc : site.startPC = UInt256.ofNat 2586 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2021) = UInt256.ofNat 2586
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2586, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2638, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2586) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2586) template = UInt256.ofNat 2638 := by decide
  rw [hend] at hraw
