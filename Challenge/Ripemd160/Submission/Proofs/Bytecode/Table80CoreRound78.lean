...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound78
...
def eval (message k : UInt256) (q : WordLane) : WordLane := Table80WideFinalWord.raw78 message k q
def nextKey (k : UInt256) : UInt256 := k
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound78.outputStack memory (input q k) rho =
      stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 400) k q) (nextKey k) rho := by
  simp only [Table80RawRound78.outputStack, input, stack, List.map_cons, List.map_nil,
    ...
    Paired80WordGroupTwoHoist.hoistedBoolean, Table80WideFinalWord.raw78, Table80WideCoreBridge.wideWordShift,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired80Algorithm.physicalKey 78) =
     Paired80Algorithm.physicalKey 79 := by decide
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 4548, stack := stack [.d, .k, .b, .c, .a, .e, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 4588, stack :=
          stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 400) k q) (nextKey k) rho} := by
   have gs := Table80RawRound78.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound78
