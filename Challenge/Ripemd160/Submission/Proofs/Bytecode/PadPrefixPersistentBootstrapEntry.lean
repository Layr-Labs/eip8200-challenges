import Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixPersistentBootstrapSite
...
def entry (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  {s with pc := UInt256.ofNat 4977, stack := StaggerPersistentFrame.frame h off limit rho}
...
def gasSteps (s : State) (h : Compression.HashState) (off limit : UInt256)
    ...
    GasSteps (entry s h off limit rho)
      (PadPrefixPersistentCoreRight.initialState s (Word.ofUInt32 h.h4) (initial h)
        (StaggerPersistentFrame.coreRest h off limit rho)) := by
  have g := PadPrefixPersistentBootstrapSite.gasSteps s (input h off limit) rho hs hr ha hcode hfork hnp
