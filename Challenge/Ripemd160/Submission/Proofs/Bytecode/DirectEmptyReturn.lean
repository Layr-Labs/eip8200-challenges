...
theorem run_decision_empty (s : State) (input : ByteArray) (i : Nat)
  ...
  have hpc231 : Artifact.submissionArtifact.instructionPC 340 = 549 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hpc232 : Artifact.submissionArtifact.instructionPC 341 = 550 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hpc233 : Artifact.submissionArtifact.instructionPC 342 = 553 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
...
def bodyTemplate : List Instr :=
  [ .push ⟨20, by decide⟩ (UInt256.ofNat 890993315260586290631548281360202943075753233713),
    ...
    .op .RETURN ]
theorem body_slice :
    (Artifact.submissionArtifact.instructions.drop 343).take bodyTemplate.length = bodyTemplate := by rfl
def bodySite : GenericRoundSite Artifact.submissionArtifact .Osaka bodyTemplate :=
  StackSiteBuilder.ofSlice bodyTemplate 343 body_slice
    (by change 343 + bodyTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := bodyTemplate) (by decide))
    (by decide)
theorem body_pc : bodySite.startPC = UInt256.ofNat 554 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 343) = UInt256.ofNat 554
...
def finalState (s : State) (input : ByteArray) (i : Nat) : State :=
  {bodyEntry s input i with pc := UInt256.ofNat 580, memory := outputMemory s.memory, activeWords := UInt256.ofNat (MachineState.activeWordsAfter
