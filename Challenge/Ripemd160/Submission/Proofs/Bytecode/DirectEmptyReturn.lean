...
def bodySite : GenericRoundSite Artifact.submissionArtifact .Osaka bodyTemplate :=
  StackSiteBuilder.ofSlice bodyTemplate 239 body_slice
    ...
    (by decide)
theorem body_pc : bodySite.startPC = UInt256.ofNat 378 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 239) = UInt256.ofNat 378
...
def finalState (s : State) (input : ByteArray) (i : Nat) : State :=
  {FastEmptyBlock.bodyEntry s input i with pc := UInt256.ofNat 404, memory := outputMemory s.memory, activeWords := UInt256.ofNat (MachineState.activeWordsAfter
...
def gasSteps_return (s : State) (input : ByteArray) (i : Nat) (hempty : input.size = 0)
  ...
  have gd := Stepper.runLocatedBlock_sound Artifact.submissionArtifact .Osaka
    FastEmptyBlock.decisionPath (s := DriverTrace.setupEntry s input) hcode hfork
...
theorem correct_empty (input : ByteArray) (hfit : CalldataFits input) (hempty : input.size = 0)
    (entryPrefix : GasSteps (initialState submissionBytecode input 0) (Execution.atPC input 277)) :
    ∃ g₀ : Nat, ∀ gas : Nat, g₀ ≤ gas →
