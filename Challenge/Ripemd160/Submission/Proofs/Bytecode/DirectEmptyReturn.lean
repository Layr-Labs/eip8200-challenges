...
def bodyTemplate : List Instr :=
  [ .push ⟨20, by decide⟩ (UInt256.ofNat 890993315260586290631548281360202943075753233713),
    ...
    .op .RETURN ]
theorem body_slice :
    (Artifact.submissionArtifact.instructions.drop 251).take bodyTemplate.length = bodyTemplate := by rfl
def bodySite : GenericRoundSite Artifact.submissionArtifact .Osaka bodyTemplate :=
  StackSiteBuilder.ofSlice bodyTemplate 251 body_slice
    (by change 251 + bodyTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := bodyTemplate) (by decide))
    (by decide)
theorem body_pc : bodySite.startPC = UInt256.ofNat 395 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 251) = UInt256.ofNat 395
...
def finalState (s : State) (input : ByteArray) (i : Nat) : State :=
  {FastEmptyBlock.bodyEntry s input i with pc := UInt256.ofNat 421, memory := outputMemory s.memory, activeWords := UInt256.ofNat (MachineState.activeWordsAfter
...
theorem run_body (s : State) (input : ByteArray) (i : Nat) (hrun : s.halt = .Running) :
    runInstrSeq bodyTemplate (FastEmptyBlock.bodyEntry s input i) = some (finalState s input i) := by
  have hbytes : Data.Bytes.natToBytesPadded (UInt256.ofNat EmptySpec.digestNat).toNat 32 =
    ...
    decide
  simp [bodyTemplate, FastEmptyBlock.bodyEntry, finalState, outputMemory,
...
def gasSteps_return (s : State) (input : ByteArray) (i : Nat) (hempty : input.size = 0)
    ...
    GasSteps (DriverTrace.dispatchEntry s input i) (finalState s input i) := by
  have gd := Stepper.runLocatedBlock_sound Artifact.submissionArtifact .Osaka
    FastEmptyBlock.decisionPath (s := FastEmptyBlock.legacyDispatchEntry s input i) hcode hfork
    (FastEmptyBlock.run_decision_empty s input i hempty hcalldata hrun) hrun hnp
  have gb := gasSteps_terminal_of_raw bodySite (FastEmptyBlock.bodyEntry s input i) _
...
theorem correct_empty (input : ByteArray) (hfit : CalldataFits input) (hempty : input.size = 0)
  ...
  have hs : s = DriverTrace.loopAt s input 0 := by
    have hpc := PaddingTrace.padReturned_pc input
    have hstack := PaddingTrace.padReturned_stack input
    dsimp only [s, DriverTrace.loopAt, DriverTrace.blockOffsetWord, DriverTrace.blockOffset]
    rw [← hpc, ← hstack]
  have hc := DriverTrace.gasSteps_call s input hfit 0 (DriverTrace.blockCount_pos input)
  ...
  have hr := gasSteps_return s input 0 hempty (by rfl)
    (PaddingTrace.padReturned_code input) (PaddingTrace.padReturned_fork input)
    (PaddingTrace.padReturned_halt input) (PaddingTrace.padReturned_noPrecompile input)
  let trace := hp.trans ((hc.cast hs.symm rfl).trans hr)
  ...
  have heval := eval_of_steps (trace.trace gas hgas) (by
    simp [withGas, finalState, FastEmptyBlock.bodyEntry, s, State.isDone, State.isHalted, State.isRunning]
    rfl)
