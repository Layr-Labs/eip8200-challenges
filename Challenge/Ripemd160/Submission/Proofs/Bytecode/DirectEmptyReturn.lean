...
theorem run_decision_empty (s : State) (input : ByteArray) (i : Nat)
  ...
  have hpc231 : Artifact.submissionArtifact.instructionPC 237 = 372 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hpc232 : Artifact.submissionArtifact.instructionPC 238 = 373 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have hpc233 : Artifact.submissionArtifact.instructionPC 239 = 376 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
...
def bodyTemplate : List Instr :=
  [ .push ⟨20, by decide⟩ (UInt256.ofNat 890993315260586290631548281360202943075753233713),
    ...
    .op .RETURN ]
theorem body_slice :
    (Artifact.submissionArtifact.instructions.drop 240).take bodyTemplate.length = bodyTemplate := by rfl
def bodySite : GenericRoundSite Artifact.submissionArtifact .Osaka bodyTemplate :=
  StackSiteBuilder.ofSlice bodyTemplate 240 body_slice
    (by change 240 + bodyTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem body_pc : bodySite.startPC = UInt256.ofNat 377 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 240) = UInt256.ofNat 377
