...
address, so each half has 24 instructions and the helper has 62.
-/
...
def initialTemplate : List Instr :=
  [ op .JUMPDEST, dup1, push1 (UInt256.ofNat 60), op .ADD, op .MLOAD,
    op .POP, dup1, push1 (UInt256.ofNat 32), op .ADD, op .MLOAD, swap1,
    op .MLOAD, op .JUMPDEST ]
...
def denseBeforeJumpTemplate : List Instr :=
  initialTemplate ++ denseHalfTemplate 0 ++ denseHalfTemplate 1
...
theorem denseFullTemplate_staticGas :
    staticGas denseFullTemplate = 186 := by
...
def denseExpectedMemory (s : State) (messageOffset : UInt256) : ByteArray :=
  writeDenseWord
    (writeDenseWord s.memory (denseStoreOffset 0)
      (packedInput0 s messageOffset))
    (denseStoreOffset 1) (packedInput1 s messageOffset)
...
def warmupActiveWords (s : State) (messageOffset : UInt256) : UInt256 :=
  ...
  let a1 := activeAfterWord a0 (messageOffset + UInt256.ofNat 32)
  activeAfterWord a1 messageOffset
...
def denseExpectedActiveWords (s : State) (messageOffset : UInt256) : UInt256 :=
  denseStoreActiveWords
    (denseStoreActiveWords (warmupActiveWords s messageOffset)
      [denseStoreOffset 0]) [denseStoreOffset 1]
