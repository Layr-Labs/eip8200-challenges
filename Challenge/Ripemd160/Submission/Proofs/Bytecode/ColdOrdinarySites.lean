...
theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3655).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3655 low_slice
    (by change 3655 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem low_pc : lowSite.startPC = UInt256.ofNat 4764 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3655) = UInt256.ofNat 4764
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_slice :
    (Artifact.submissionArtifact.instructions.drop 3679).take branchTemplate.length = branchTemplate := by rfl

def branchSite : GenericRoundSite Artifact.submissionArtifact .Osaka branchTemplate :=
  StackSiteBuilder.ofSlice branchTemplate 3679 branch_slice
    (by change 3679 + branchTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4819 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3679) = UInt256.ofNat 4819
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 514 = 884 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
