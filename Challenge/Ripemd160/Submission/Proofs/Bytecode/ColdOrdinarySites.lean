...
theorem normal_slice :
    (Artifact.submissionArtifact.instructions.drop 251).take actualNormalTemplate.length = actualNormalTemplate := by rfl

def normalSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualNormalTemplate :=
  StackSiteBuilder.ofSlice actualNormalTemplate 251 normal_slice
    (by change 251 + actualNormalTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualNormalTemplate) (by decide))
    (by decide)
theorem normal_pc : normalSite.startPC = UInt256.ofNat 521 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 251) = UInt256.ofNat 521
...
theorem normal_end : pcAfter (UInt256.ofNat 521) actualNormalTemplate = UInt256.ofNat 875 := by decide

theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3557).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3557 low_slice
    (by change 3557 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem low_pc : lowSite.startPC = UInt256.ofNat 4702 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3557) = UInt256.ofNat 4702
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4757 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3581) = UInt256.ofNat 4757
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 875).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 474 = 875 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 474 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 875 = true
  rw [hcode]
  exact h
...
def gasSteps_normal (s : State) (ret a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 521, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho}
      {s with pc := UInt256.ofNat 875, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho, memory := PoolReference.dataMemory s.memory p, activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  apply normal_gasSteps_of_raw {s with pc := UInt256.ofNat 521, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho} _ hcode hfork hrun hnp normal_pc.symm
  have h := PersistentMaskEndian.run_normal s (UInt256.ofNat 521) ret a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho p hstack hrun hp hbound hq1 hq0
  have hend : pcAfter (UInt256.ofNat 521) PersistentMaskEndian.normalTemplate = UInt256.ofNat 875 := by decide
  rw [hend] at h
  exact h
...
def gasSteps_branch_taken (s : State) (c : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4757, stack := c :: rho}
      {s with pc := UInt256.ofNat 875, stack := rho} := by
