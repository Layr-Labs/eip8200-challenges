...
theorem normal_pc : normalSite.startPC = UInt256.ofNat 494 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 258) = UInt256.ofNat 494
...
theorem normal_end : pcAfter (UInt256.ofNat 494) actualNormalTemplate = UInt256.ofNat 898 := by decide

theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3675).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3675 low_slice
    (by change 3675 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem low_pc : lowSite.startPC = UInt256.ofNat 4767 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3675) = UInt256.ofNat 4767
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_slice :
    (Artifact.submissionArtifact.instructions.drop 3699).take branchTemplate.length = branchTemplate := by rfl

def branchSite : GenericRoundSite Artifact.submissionArtifact .Osaka branchTemplate :=
  StackSiteBuilder.ofSlice branchTemplate 3699 branch_slice
    (by change 3699 + branchTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4822 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3699) = UInt256.ofNat 4822
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 898).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 525 = 898 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 525 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 898 = true
  rw [hcode]
  exact h
...
def gasSteps_normal (s : State) (ret a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim : UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 494, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho}
      {s with pc := UInt256.ofNat 898, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho, memory := StaggerTableLayout.resultMemory s.memory (StaggerScratch.dirtyWord s.memory p), activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  ...
  have hend : pcAfter (UInt256.ofNat 494) PersistentMaskEndian.normalTemplate = UInt256.ofNat 898 := by decide
  rw [hend] at h
  exact h
...
def gasSteps_branch_taken (s : State) (c : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4822, stack := c :: rho}
      {s with pc := UInt256.ofNat 898, stack := rho} := by
