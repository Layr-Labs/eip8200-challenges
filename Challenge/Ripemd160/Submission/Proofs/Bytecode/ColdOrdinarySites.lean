...
theorem low_pc : lowSite.startPC = UInt256.ofNat 4764 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3657) = UInt256.ofNat 4764
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4819 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3681) = UInt256.ofNat 4819
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 516 = 884 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 516 (by rfl)
  rw [hpc] at h
