...
theorem normal_end : pcAfter (UInt256.ofNat 509) actualNormalTemplate = UInt256.ofNat 882 := by decide

theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3573).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3573 low_slice
    (by change 3573 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.lowTemplate) (by decide))
    (by decide)
theorem low_pc : lowSite.startPC = UInt256.ofNat 4724 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3573) = UInt256.ofNat 4724
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem low_end : pcAfter (UInt256.ofNat 4724) StaggerPad.lowTemplate = UInt256.ofNat 4779 := by decide

open StaggerPad (branchTemplate)

theorem branch_slice :
    (Artifact.submissionArtifact.instructions.drop 3597).take branchTemplate.length = branchTemplate := by rfl

def branchSite : GenericRoundSite Artifact.submissionArtifact .Osaka branchTemplate :=
  StackSiteBuilder.ofSlice branchTemplate 3597 branch_slice
    (by change 3597 + branchTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := branchTemplate) (by decide))
    (by decide)
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4779 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3597) = UInt256.ofNat 4779
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_end : pcAfter (UInt256.ofNat 4779) branchTemplate = UInt256.ofNat 4783 := by decide

theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 882).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 475 = 882 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 475 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 882 = true
  rw [hcode]
  exact h
...
def gasSteps_normal (s : State) (ret a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim : UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 509, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho}
      {s with pc := UInt256.ofNat 882, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho, memory := PoolReference.dataMemory s.memory p, activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  ...
  have hend : pcAfter (UInt256.ofNat 509) PersistentMaskEndian.normalTemplate = UInt256.ofNat 882 := by decide
  rw [hend] at h
  exact h
...
def gasSteps_low (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4724, stack := ret :: rest}
      {s with
  ...
  | cons mask tailRest =>
    ...
    apply PadLift.gasSteps_of_raw lowSite {s with pc := UInt256.ofNat 4724, stack := ret :: UInt256.ofNat 4294967295 :: tailRest} _ hcode hfork hrun hnp low_pc.symm low_advances
...
def gasSteps_branch_taken (s : State) (c : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4779, stack := c :: rho}
      {s with pc := UInt256.ofNat 884, stack := rho} := by
  apply PadLift.gasSteps_of_raw branchSite {s with pc := UInt256.ofNat 4779, stack := c :: rho} _ hcode hfork hrun hnp branch_pc.symm branch_advances
...
def gasSteps_branch_fall (s : State) (c : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4779, stack := c :: rho}
      {s with pc := UInt256.ofNat 4783, stack := rho} := by
  apply PadLift.gasSteps_of_raw branchSite {s with pc := UInt256.ofNat 4779, stack := c :: rho} _ hcode hfork hrun hnp branch_pc.symm branch_advances
