...
theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3581).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3581 low_slice
    (by change 3581 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem low_pc : lowSite.startPC = UInt256.ofNat 4702 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3581) = UInt256.ofNat 4702
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_slice :
    (Artifact.submissionArtifact.instructions.drop 3605).take branchTemplate.length = branchTemplate := by rfl

def branchSite : GenericRoundSite Artifact.submissionArtifact .Osaka branchTemplate :=
  StackSiteBuilder.ofSlice branchTemplate 3605 branch_slice
    (by change 3605 + branchTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4757 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3605) = UInt256.ofNat 4757
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 483 = 860 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 483 (by rfl)
  rw [hpc] at h
