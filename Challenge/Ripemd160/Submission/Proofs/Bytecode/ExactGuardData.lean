...
abbrev targetInput : ByteArray := KnownInputData.targetInput

/-- A complete 32-byte chunk of the target calldata. -/
abbrev fullWord : UInt256 := KnownInputData.fullWord

/-- The final eight `a` bytes followed by 24 bytes of `CALLDATALOAD` padding. -/
abbrev tailWord : UInt256 := KnownInputData.finalWord

/-- The expected padded word at a block index in the 32-word guard. -/
abbrev expectedWord (block : Nat) : UInt256 := KnownInputData.expectedWord block
