...
theorem endian8_slice : (Artifact.submissionArtifact.instructions.drop 3731).take endian8Code.length = endian8Code := by rfl
def endian8Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian8Code :=
  StackSiteBuilder.ofSlice endian8Code 3731 endian8_slice
    (by change 3731 + endian8Code.length ≤ Artifact.submissionInstructions.length
...
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4858 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3731) = UInt256.ofNat 4858
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian8_end : endian8Site.endPC = UInt256.ofNat 4890 := by
...
theorem endian16_slice : (Artifact.submissionArtifact.instructions.drop 3741).take endian16Code.length = endian16Code := by rfl
def endian16Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian16Code :=
  StackSiteBuilder.ofSlice endian16Code 3741 endian16_slice
    (by change 3741 + endian16Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := endian16Code) (by decide)) (by decide)
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4890 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3741) = UInt256.ofNat 4890
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian16_end : endian16Site.endPC = UInt256.ofNat 4922 := by
...
theorem terminal_slice : (Artifact.submissionArtifact.instructions.drop 3751).take terminalCode.length = terminalCode := by rfl
def terminalSite : GenericRoundSite Artifact.submissionArtifact .Osaka terminalCode :=
  StackSiteBuilder.ofSlice terminalCode 3751 terminal_slice
    (by change 3751 + terminalCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := terminalCode) (by decide)) (by decide)
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4922 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3751) = UInt256.ofNat 4922
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def result (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  StaggerPersistentReturn.result s (UInt256.ofNat 4922) (value h) (off :: limit :: rho)
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4835, stack := StaggerPersistentFrame.frame h off limit rho}
