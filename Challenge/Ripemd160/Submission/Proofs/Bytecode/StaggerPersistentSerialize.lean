...
theorem endian8_slice : (Artifact.submissionArtifact.instructions.drop 3545).take endian8Code.length = endian8Code := by rfl
def endian8Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian8Code :=
  StackSiteBuilder.ofSlice endian8Code 3545 endian8_slice
    (by change 3545 + endian8Code.length ≤ Artifact.submissionInstructions.length
...
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4668 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3545) = UInt256.ofNat 4668
...
theorem endian16_slice : (Artifact.submissionArtifact.instructions.drop 3555).take endian16Code.length = endian16Code := by rfl
def endian16Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian16Code :=
  StackSiteBuilder.ofSlice endian16Code 3555 endian16_slice
    (by change 3555 + endian16Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4681 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3555) = UInt256.ofNat 4681
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem terminal_slice : (Artifact.submissionArtifact.instructions.drop 3565).take terminalCode.length = terminalCode := by rfl
def terminalSite : GenericRoundSite Artifact.submissionArtifact .Osaka terminalCode :=
  StackSiteBuilder.ofSlice terminalCode 3565 terminal_slice
    (by change 3565 + terminalCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4695 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3565) = UInt256.ofNat 4695
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
