...
import Challenge.Ripemd160.Submission.Proofs.Bytecode.ClosedEndianLiteral
...
theorem entry_slice : (Artifact.submissionArtifact.instructions.drop 3793).take entryCode.length = entryCode := by rfl
def entrySite : GenericRoundSite Artifact.submissionArtifact .Osaka entryCode :=
  StackSiteBuilder.ofSlice entryCode 3793 entry_slice
    (by change 3793 + entryCode.length ≤ Artifact.submissionInstructions.length
...
theorem entry_pc : entrySite.startPC = UInt256.ofNat 4734 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3793) = UInt256.ofNat 4734
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem entry_end : entrySite.endPC = UInt256.ofNat 4735 := by
...
def endian8Code : List Instr := ClosedEndianLiteral.code 8
theorem endian8_slice : (Artifact.submissionArtifact.instructions.drop 3806).take endian8Code.length = endian8Code := by rfl
def endian8Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian8Code :=
  StackSiteBuilder.ofSlice endian8Code 3806 endian8_slice
    (by change 3806 + endian8Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := endian8Code) (by decide)) (by decide)
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4751 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3806) = UInt256.ofNat 4751
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian8_end : endian8Site.endPC = UInt256.ofNat 4796 := by
...
def endian16Code : List Instr := ClosedEndianLiteral.code 16
theorem endian16_slice : (Artifact.submissionArtifact.instructions.drop 3816).take endian16Code.length = endian16Code := by rfl
def endian16Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian16Code :=
  StackSiteBuilder.ofSlice endian16Code 3816 endian16_slice
    (by change 3816 + endian16Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := endian16Code) (by decide)) (by decide)
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4796 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3816) = UInt256.ofNat 4796
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian16_end : endian16Site.endPC = UInt256.ofNat 4842 := by
...
theorem terminal_slice : (Artifact.submissionArtifact.instructions.drop 3826).take terminalCode.length = terminalCode := by rfl
def terminalSite : GenericRoundSite Artifact.submissionArtifact .Osaka terminalCode :=
  StackSiteBuilder.ofSlice terminalCode 3826 terminal_slice
    (by change 3826 + terminalCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := terminalCode) (by decide)) (by decide)
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4842 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3826) = UInt256.ofNat 4842
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def result (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  StaggerPersistentReturn.result s (UInt256.ofNat 4842) (value h) (off :: limit :: rho)
...
def gasSteps_entry (s : State) (rho : List UInt256) (hstack : rho.length < 1024)
    ...
    GasSteps {s with pc := UInt256.ofNat 4734, stack := rho}
      {s with pc := UInt256.ofNat 4735, stack := rho} := by
  apply DenseScheduleLift.gasSteps_of_raw entrySite
    {s with pc := UInt256.ofNat 4734, stack := rho}
    {s with pc := UInt256.ofNat 4735, stack := rho}
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4734, stack := StaggerPersistentFrame.frame h off limit rho}
  ...
  have g8 := ClosedEndianLiteral.gasSteps_endian 8 DenseScheduleTemplate.mask8 endian8Site
    s (StaggerPersistentOutput.packedHash h) (off::limit::rho) (by simp; omega) (Or.inl ⟨rfl,rfl⟩) hcode hfork hrun hnp
  have g16 := ClosedEndianLiteral.gasSteps_endian 16 DenseScheduleTemplate.mask16 endian16Site
