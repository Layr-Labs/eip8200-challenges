...
theorem entry_slice : (Artifact.submissionArtifact.instructions.drop 3795).take entryCode.length = entryCode := by rfl
def entrySite : GenericRoundSite Artifact.submissionArtifact .Osaka entryCode :=
  StackSiteBuilder.ofSlice entryCode 3795 entry_slice
    (by change 3795 + entryCode.length ≤ Artifact.submissionInstructions.length
...
theorem entry_pc : entrySite.startPC = UInt256.ofNat 4736 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3795) = UInt256.ofNat 4736
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem entry_end : entrySite.endPC = UInt256.ofNat 4737 := by
...
theorem endian8_slice : (Artifact.submissionArtifact.instructions.drop 3808).take endian8Code.length = endian8Code := by rfl
def endian8Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian8Code :=
  StackSiteBuilder.ofSlice endian8Code 3808 endian8_slice
    (by change 3808 + endian8Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := endian8Code) (by decide)) (by decide)
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4753 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3808) = UInt256.ofNat 4753
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian8_end : endian8Site.endPC = UInt256.ofNat 4769 := by
...
theorem endian16_slice : (Artifact.submissionArtifact.instructions.drop 3821).take endian16Code.length = endian16Code := by rfl
def endian16Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian16Code :=
  StackSiteBuilder.ofSlice endian16Code 3821 endian16_slice
    (by change 3821 + endian16Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := endian16Code) (by decide)) (by decide)
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4769 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3821) = UInt256.ofNat 4769
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian16_end : endian16Site.endPC = UInt256.ofNat 4786 := by
...
theorem terminal_slice : (Artifact.submissionArtifact.instructions.drop 3834).take terminalCode.length = terminalCode := by rfl
def terminalSite : GenericRoundSite Artifact.submissionArtifact .Osaka terminalCode :=
  StackSiteBuilder.ofSlice terminalCode 3834 terminal_slice
    (by change 3834 + terminalCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := terminalCode) (by decide)) (by decide)
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4786 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3834) = UInt256.ofNat 4786
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def result (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  StaggerPersistentReturn.result s (UInt256.ofNat 4786) (value h) (off :: limit :: rho)
...
def gasSteps_entry (s : State) (rho : List UInt256) (hstack : rho.length < 1024)
    ...
    GasSteps {s with pc := UInt256.ofNat 4736, stack := rho}
      {s with pc := UInt256.ofNat 4737, stack := rho} := by
  apply DenseScheduleLift.gasSteps_of_raw entrySite
    {s with pc := UInt256.ofNat 4736, stack := rho}
    {s with pc := UInt256.ofNat 4737, stack := rho}
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4736, stack := StaggerPersistentFrame.frame h off limit rho}
