...
theorem endian8_slice : (Artifact.submissionArtifact.instructions.drop 3701).take endian8Code.length = endian8Code := by rfl
def endian8Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian8Code :=
  StackSiteBuilder.ofSlice endian8Code 3701 endian8_slice
    (by change 3701 + endian8Code.length ≤ Artifact.submissionInstructions.length
...
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4621 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3701) = UInt256.ofNat 4621
...
theorem endian16_slice : (Artifact.submissionArtifact.instructions.drop 3711).take endian16Code.length = endian16Code := by rfl
def endian16Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian16Code :=
  StackSiteBuilder.ofSlice endian16Code 3711 endian16_slice
    (by change 3711 + endian16Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4653 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3711) = UInt256.ofNat 4653
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian16_end : endian16Site.endPC = UInt256.ofNat 4685 := by
...
theorem terminal_slice : (Artifact.submissionArtifact.instructions.drop 3721).take terminalCode.length = terminalCode := by rfl
def terminalSite : GenericRoundSite Artifact.submissionArtifact .Osaka terminalCode :=
  StackSiteBuilder.ofSlice terminalCode 3721 terminal_slice
    (by change 3721 + terminalCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := terminalCode) (by decide)) (by decide)
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4685 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3721) = UInt256.ofNat 4685
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def result (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  StaggerPersistentReturn.result s (UInt256.ofNat 4685) (value h) (off :: limit :: rho)
