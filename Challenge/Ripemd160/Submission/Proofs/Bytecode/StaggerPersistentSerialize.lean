...
theorem entry_slice : (Artifact.submissionArtifact.instructions.drop 3842).take entryCode.length = entryCode := by rfl
def entrySite : GenericRoundSite Artifact.submissionArtifact .Osaka entryCode :=
  StackSiteBuilder.ofSlice entryCode 3842 entry_slice
    (by change 3842 + entryCode.length ≤ Artifact.submissionInstructions.length
...
theorem entry_pc : entrySite.startPC = UInt256.ofNat 4618 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3842) = UInt256.ofNat 4618
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem entry_end : entrySite.endPC = UInt256.ofNat 4619 := by
...
theorem endian8_slice : (Artifact.submissionArtifact.instructions.drop 3855).take endian8Code.length = endian8Code := by rfl
def endian8Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian8Code :=
  StackSiteBuilder.ofSlice endian8Code 3855 endian8_slice
    (by change 3855 + endian8Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := endian8Code) (by decide)) (by decide)
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4635 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3855) = UInt256.ofNat 4635
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian8_end : endian8Site.endPC = UInt256.ofNat 4651 := by
...
theorem endian16_slice : (Artifact.submissionArtifact.instructions.drop 3868).take endian16Code.length = endian16Code := by rfl
def endian16Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian16Code :=
  StackSiteBuilder.ofSlice endian16Code 3868 endian16_slice
    (by change 3868 + endian16Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := endian16Code) (by decide)) (by decide)
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4651 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3868) = UInt256.ofNat 4651
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian16_end : endian16Site.endPC = UInt256.ofNat 4668 := by
...
theorem terminal_slice : (Artifact.submissionArtifact.instructions.drop 3881).take terminalCode.length = terminalCode := by rfl
def terminalSite : GenericRoundSite Artifact.submissionArtifact .Osaka terminalCode :=
  StackSiteBuilder.ofSlice terminalCode 3881 terminal_slice
    (by change 3881 + terminalCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := terminalCode) (by decide)) (by decide)
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4668 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3881) = UInt256.ofNat 4668
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def result (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  StaggerPersistentReturn.result s (UInt256.ofNat 4668) (value h) (off :: limit :: rho)
...
def gasSteps_entry (s : State) (rho : List UInt256) (hstack : rho.length < 1024)
    ...
    GasSteps {s with pc := UInt256.ofNat 4618, stack := rho}
      {s with pc := UInt256.ofNat 4619, stack := rho} := by
  apply DenseScheduleLift.gasSteps_of_raw entrySite
    {s with pc := UInt256.ofNat 4618, stack := rho}
    {s with pc := UInt256.ofNat 4619, stack := rho}
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4618, stack := StaggerPersistentFrame.frame h off limit rho}
