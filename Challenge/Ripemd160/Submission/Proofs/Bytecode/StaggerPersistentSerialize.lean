...
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4759 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3843) = UInt256.ofNat 4759
...
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4791 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3853) = UInt256.ofNat 4791
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem terminal_slice : (Artifact.submissionArtifact.instructions.drop 3863).take terminalCode.length = terminalCode := by rfl
def terminalSite : GenericRoundSite Artifact.submissionArtifact .Osaka terminalCode :=
  StackSiteBuilder.ofSlice terminalCode 3863 terminal_slice
    (by change 3863 + terminalCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4823 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3863) = UInt256.ofNat 4823
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
