...
theorem endian8_slice : (Artifact.submissionArtifact.instructions.drop 3831).take endian8Code.length = endian8Code := by rfl
def endian8Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian8Code :=
  StackSiteBuilder.ofSlice endian8Code 3831 endian8_slice
    (by change 3831 + endian8Code.length ≤ Artifact.submissionInstructions.length
...
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4778 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3831) = UInt256.ofNat 4778
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian8_end : endian8Site.endPC = UInt256.ofNat 4810 := by
...
theorem endian16_slice : (Artifact.submissionArtifact.instructions.drop 3841).take endian16Code.length = endian16Code := by rfl
def endian16Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian16Code :=
  StackSiteBuilder.ofSlice endian16Code 3841 endian16_slice
    (by change 3841 + endian16Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
    (StackRoundData.templateWellFormed_mem (instructions := endian16Code) (by decide)) (by decide)
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4810 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3841) = UInt256.ofNat 4810
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem terminal_slice : (Artifact.submissionArtifact.instructions.drop 3851).take terminalCode.length = terminalCode := by rfl
def terminalSite : GenericRoundSite Artifact.submissionArtifact .Osaka terminalCode :=
  StackSiteBuilder.ofSlice terminalCode 3851 terminal_slice
    (by change 3851 + terminalCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4842 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3851) = UInt256.ofNat 4842
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
