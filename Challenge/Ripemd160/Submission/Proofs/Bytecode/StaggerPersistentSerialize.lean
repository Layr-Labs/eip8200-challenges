...
theorem endian8_slice : (Artifact.submissionArtifact.instructions.drop 3608).take endian8Code.length = endian8Code := by rfl
def endian8Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian8Code :=
  StackSiteBuilder.ofSlice endian8Code 3608 endian8_slice
    (by change 3608 + endian8Code.length ≤ Artifact.submissionInstructions.length
...
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4675 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3608) = UInt256.ofNat 4675
...
theorem endian16_slice : (Artifact.submissionArtifact.instructions.drop 3618).take endian16Code.length = endian16Code := by rfl
def endian16Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian16Code :=
  StackSiteBuilder.ofSlice endian16Code 3618 endian16_slice
    (by change 3618 + endian16Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4688 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3618) = UInt256.ofNat 4688
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem terminal_slice : (Artifact.submissionArtifact.instructions.drop 3628).take terminalCode.length = terminalCode := by rfl
def terminalSite : GenericRoundSite Artifact.submissionArtifact .Osaka terminalCode :=
  StackSiteBuilder.ofSlice terminalCode 3628 terminal_slice
    (by change 3628 + terminalCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4702 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3628) = UInt256.ofNat 4702
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
