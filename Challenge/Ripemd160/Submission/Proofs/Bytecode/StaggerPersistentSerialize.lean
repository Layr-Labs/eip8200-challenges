...
theorem endian8_slice : (Artifact.submissionArtifact.instructions.drop 3825).take endian8Code.length = endian8Code := by rfl
def endian8Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian8Code :=
  StackSiteBuilder.ofSlice endian8Code 3825 endian8_slice
    (by change 3825 + endian8Code.length ≤ Artifact.submissionInstructions.length
...
theorem endian8_pc : endian8Site.startPC = UInt256.ofNat 4764 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3825) = UInt256.ofNat 4764
...
theorem endian16_slice : (Artifact.submissionArtifact.instructions.drop 3835).take endian16Code.length = endian16Code := by rfl
def endian16Site : GenericRoundSite Artifact.submissionArtifact .Osaka endian16Code :=
  StackSiteBuilder.ofSlice endian16Code 3835 endian16_slice
    (by change 3835 + endian16Code.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem endian16_pc : endian16Site.startPC = UInt256.ofNat 4796 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3835) = UInt256.ofNat 4796
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
theorem endian16_end : endian16Site.endPC = UInt256.ofNat 4828 := by
...
theorem terminal_pc : terminalSite.startPC = UInt256.ofNat 4828 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3845) = UInt256.ofNat 4828
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def result (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  StaggerPersistentReturn.result s (UInt256.ofNat 4828) (value h) (off :: limit :: rho)
