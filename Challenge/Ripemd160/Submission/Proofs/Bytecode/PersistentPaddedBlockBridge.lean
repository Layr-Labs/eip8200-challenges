...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentPaddedBlockBridge

open EvmSemantics
open EvmSemantics.Crypto
open EvmSemantics.EVM
open Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentPadding
...
private theorem padBase_memory (input : ByteArray) :
    (PersistentPaddingTrace.padLengthReady input).memory =
      (PersistentMain.initializedState input).memory := by
  rfl

private theorem padBase_size (input : ByteArray) :
    (PersistentPaddingTrace.padLengthReady input).memory.size ≤ messageOffset := by
  change 0 ≤ 1632
  decide
...
theorem padReturned_paddedBlockAt (input : ByteArray)
    ...
    (hblock : blockOff + 64 ≤ paddedLength input.size) :
    ScheduleCorrect.MessageBlockAt (PersistentPaddingTrace.padReturned input).memory
      (UInt256.ofNat (messageOffset + blockOff))
      (paddedMessage input) blockOff := by
  apply paddedBlockAt (PersistentPaddingTrace.padReturned input)
    (PersistentPaddingTrace.padLengthReady input).memory input
  · exact PersistentPaddingTrace.padReturned_memory input hfit
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentPaddedBlockBridge
