import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144RawRound73
...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound73
...
def eval (message k : UInt256) (q : WordLane) : WordLane := wordStep 4 12 13 message k q
def nextKey (k : UInt256) : UInt256 := k
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table144RawRound73.outputStack memory (input q k) rho =
      stack [.d, .k, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 1134) k q) (nextKey k) rho := by
  simp only [Table144RawRound73.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired144Algorithm.physicalKey 73) =
     Paired144Algorithm.physicalKey 74 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired144Algorithm.physicalKey 73) q = Paired144Algorithm.step 73 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 4316, stack := stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 4358, stack :=
          stack [.d, .k, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 1134) k q) (nextKey k) rho} := by
   have gs := Table144RawRound73.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound73
