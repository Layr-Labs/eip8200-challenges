...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRaw0
...
private theorem actualOutput_eq (memory : ByteArray) (x : Input) (rho : List UInt256) :
  ...
  have hkey : UInt256.shiftLeft (UInt256.ofNat 1352829926) (UInt256.ofNat 144) = UInt256.ofNat 30169115476673038213297653277143730720156734734729216 := by decide
...
private theorem run_generated (s : State) (pc : UInt256) (x : Input) (rho : List UInt256)
  ...
  have hz360 : MachineState.readWord s.memory 360 = UInt256.ofNat 0 :=
    PadPrefixContract.read_zero s hz ⟨20, by decide⟩ (by decide)
  ...
  simp (discharger := omega) [template, inputStack, actualOutput, hz360,
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRaw0
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 4002).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 4002 actual_slice
    (by change 4002 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 5115 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4002) = UInt256.ofNat 5115
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 5115, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 5163, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 5115) x rho hstack hrun hactive hz
  have hend : pcAfter (UInt256.ofNat 5115) template = UInt256.ofNat 5163 := by decide
  rw [hend] at hraw
