...
open StackCompression QuadSemantic CachedMaskRoundCertificates CavityQuadGroup
...
noncomputable def gasSteps_left80 (s : State) (word : Nat → UInt32)
  ...
  have entry := CachedMaskR2Inline.gasSteps_wide_bridge
    CachedMaskR2Sites.entryBridge s
    (working.a :: working.b :: working.c :: working.d :: working.e ::
      QuadRoundTemplate.factor :: StackRoundTemplate.mask :: rho)
    (by simp; omega) hcode hfork hrun hnp
  have entry' : GasSteps
      (stateAt s (QuadSites.leftPC 4) (leftRounds word 16 working)
        (StackRoundTemplate.mask :: rho))
      (stateAt s CachedMaskR2Sites.l1Site.startPC
        (leftRounds word 16 working) (StackRoundTemplate.mask :: rho)) := by
    rw [← CachedMaskR2Sites.entry_end]
    simpa [stateAt, roundEntry] using entry
  have secondCore := CachedMaskR2Inline.gasSteps_l1_after16
    CachedMaskR2Sites.l1Site s word working rho hwords hactive hstack hrun hcode hfork hnp
  have back := CavityQuadGroup.gasSteps_bridge CachedMaskR2Sites.backBridge s
    ((leftRounds word 32 working).a :: (leftRounds word 32 working).b ::
      (leftRounds word 32 working).c :: (leftRounds word 32 working).d ::
      (leftRounds word 32 working).e :: QuadRoundTemplate.factor ::
      StackRoundTemplate.mask :: rho)
    (by simp; omega) hcode hfork hrun hnp
  have back' : GasSteps
      (stateAt s CachedMaskR2Sites.l1Site.endPC
        (leftRounds word 32 working) (StackRoundTemplate.mask :: rho))
      (stateAt s (QuadSites.leftPC 8) (leftRounds word 32 working)
        (StackRoundTemplate.mask :: rho)) := by
    rw [← CachedMaskR2Sites.back_start, ← CachedMaskR2Sites.back_end]
    simpa [stateAt, roundEntry] using back
  have second := (entry'.trans secondCore).trans back'
