...
theorem site_pc : site.startPC = UInt256.ofNat 2289 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1689) = UInt256.ofNat 2289
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2289, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2335, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2289) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2289) template = UInt256.ofNat 2335 := by decide
  rw [hend] at hraw
