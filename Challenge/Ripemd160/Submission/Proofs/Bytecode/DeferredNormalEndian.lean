...
/-- Width-preserving closing frame: the two retired mask slots are replaced by
inert `JUMPDEST` bytes, so every later program counter and instruction index is
unchanged while the two `POP`s disappear from the executed stream. -/
