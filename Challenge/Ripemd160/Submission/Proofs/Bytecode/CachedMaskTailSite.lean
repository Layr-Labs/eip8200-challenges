...
private def tailAfter : List Instr :=
  submissionInstructionsChunk14.drop 64 ++ submissionInstructionsChunk15 ++
    submissionInstructionsChunk16
...
private theorem artifactChunk14_tail :
    submissionInstructionsChunk14 =
      submissionInstructionsChunk14.take 16 ++
        CachedMaskOrderedTail.template ++ submissionInstructionsChunk14.drop 64 := by
  rfl
...
private theorem tail_instruction_pc (i : Nat)
    ...
    Artifact.submissionArtifact.instructionPC (2816 + i) =
      0x101b + ArtifactByteLength.byteLength (CachedMaskOrderedTail.template.take i) := by
...
private theorem tail_instruction_pc_global (index : Nat)
    (hlo : 2816 ≤ index) (hhi : index ≤ 2864) :
    Artifact.submissionArtifact.instructionPC index =
      0x101b + ArtifactByteLength.byteLength
        (CachedMaskOrderedTail.template.take (index - 2816)) := by
  have hi : index - 2816 ≤ CachedMaskOrderedTail.template.length := by
