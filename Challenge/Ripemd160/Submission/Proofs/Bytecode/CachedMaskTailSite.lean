...
/-! Exact masked consume tail at instruction 2728, including its final JUMP. -/
...
private theorem tailBefore_length : tailBefore.length = 2728 := by
...
private theorem tail_instruction_at (i : Nat)
    ...
    Artifact.submissionArtifact.instructions[2728 + i]? =
...
private theorem tail_instruction_pc_global (index : Nat)
    (hlo : 2728 ≤ index) (hhi : index ≤ 2782) :
    Artifact.submissionArtifact.instructionPC index =
      0x10e6 + ArtifactByteLength.byteLength
        (CachedMaskTail.template.take (index - 2728)) := by
  have hi : index - 2728 ≤ CachedMaskTail.template.length := by
    rw [tailInstructions_length]
    omega
  have h := tail_instruction_pc (index - 2728) hi
