...
private def tailBefore : List Instr :=
  artifactPrefix ++ submissionInstructionsChunk15.take 97

private def tailAfter : List Instr :=
  submissionInstructionsChunk15.drop 145 ++
    submissionInstructionsChunk16 ++
    submissionInstructionsChunk17

private theorem tailBefore_length : tailBefore.length = 3097 := by
  simp [tailBefore, artifactPrefix]

private theorem artifactChunk_tail :
    submissionInstructionsChunk15 =
      submissionInstructionsChunk15.take 97 ++
        CachedMaskOrderedTail.template ++ submissionInstructionsChunk15.drop 145 := by rfl
...
private theorem tail_instruction_at (i : Nat)
    ...
    Artifact.submissionArtifact.instructions[3097 + i]? =
...
private theorem tail_instruction_pc_global (index : Nat)
    (hlo : 3097 ≤ index) (hhi : index ≤ 3145) :
    Artifact.submissionArtifact.instructionPC index =
      0x11d3 + ArtifactByteLength.byteLength
        (CachedMaskOrderedTail.template.take (index - 3097)) := by
  have hi : index - 3097 ≤ CachedMaskOrderedTail.template.length := by
    rw [tailInstructions_length]
    omega
  have h := tail_instruction_pc (index - 3097) hi
