...
theorem normal_slice :
    (Artifact.submissionArtifact.instructions.drop 294).take actualNormalTemplate.length = actualNormalTemplate := by rfl

def normalSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualNormalTemplate :=
  StackSiteBuilder.ofSlice actualNormalTemplate 294 normal_slice
    (by change 294 + actualNormalTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualNormalTemplate) (by decide))
    (by decide)
theorem normal_pc : normalSite.startPC = UInt256.ofNat 487 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 294) = UInt256.ofNat 487
...
theorem normal_end : pcAfter (UInt256.ofNat 487) actualNormalTemplate = UInt256.ofNat 906 := by decide

theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3722).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3722 low_slice
    (by change 3722 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.lowTemplate) (by decide))
    (by decide)
theorem low_pc : lowSite.startPC = UInt256.ofNat 4715 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3722) = UInt256.ofNat 4715
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem low_end : pcAfter (UInt256.ofNat 4715) StaggerPad.lowTemplate = UInt256.ofNat 4771 := by decide

open StaggerPad (branchTemplate)

theorem branch_slice :
    (Artifact.submissionArtifact.instructions.drop 3747).take branchTemplate.length = branchTemplate := by rfl

def branchSite : GenericRoundSite Artifact.submissionArtifact .Osaka branchTemplate :=
  StackSiteBuilder.ofSlice branchTemplate 3747 branch_slice
    (by change 3747 + branchTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := branchTemplate) (by decide))
    (by decide)
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4771 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3747) = UInt256.ofNat 4771
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_end : pcAfter (UInt256.ofNat 4771) branchTemplate = UInt256.ofNat 4775 := by decide

theorem high_slice :
    (Artifact.submissionArtifact.instructions.drop 3749).take StaggerPad.highTemplate.length = StaggerPad.highTemplate := by rfl

def highSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.highTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.highTemplate 3749 high_slice
    (by change 3749 + StaggerPad.highTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.highTemplate) (by decide))
    (by decide)
theorem high_pc : highSite.startPC = UInt256.ofNat 4775 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3749) = UInt256.ofNat 4775
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem high_end : pcAfter (UInt256.ofNat 4775) StaggerPad.highTemplate = UInt256.ofNat 4803 := by decide

theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 906).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 572 = 906 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 572 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 906 = true
  rw [hcode]
  exact h
...
def gasSteps_normal (s : State) (ret a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 487, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho}
      {s with pc := UInt256.ofNat 906, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho, memory := StaggerTableLayout.resultMemory s.memory (StaggerScratch.dirtyWord s.memory p), activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  apply normal_gasSteps_of_raw {s with pc := UInt256.ofNat 487, stack := PersistentMaskEndian.stk ret (UInt256.ofNat 4294967295) a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho} _ hcode hfork hrun hnp normal_pc.symm
  have h := PersistentMaskEndian.run_normal s (UInt256.ofNat 487) ret a2 a3 a4 a5 a6 a7 a8 a9 a10 off lim rho p hstack hrun hp hbound hq1 hq0 hlow
  have hend : pcAfter (UInt256.ofNat 487) PersistentMaskEndian.normalTemplate = UInt256.ofNat 906 := by decide
  rw [hend] at h
  exact h
...
def gasSteps_low (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4715, stack := ret :: rest}
      {s with
        pc := UInt256.ofNat 4771
  ...
  | cons mask tailRest =>
    ...
    apply PadLift.gasSteps_of_raw lowSite {s with pc := UInt256.ofNat 4715, stack := ret :: UInt256.ofNat 4294967295 :: tailRest} _ hcode hfork hrun hnp low_pc.symm low_advances
    have h := StaggerPad.run_low s (UInt256.ofNat 4715) ret tailRest (by omega) hrun hactive hfit
    rw [low_end] at h
    exact h
...
def gasSteps_branch_taken (s : State) (c : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4771, stack := c :: rho}
      {s with pc := UInt256.ofNat 906, stack := rho} := by
  apply PadLift.gasSteps_of_raw branchSite {s with pc := UInt256.ofNat 4771, stack := c :: rho} _ hcode hfork hrun hnp branch_pc.symm branch_advances
  exact StaggerPad.run_branch_taken s (UInt256.ofNat 4771) c rho hstack hrun hc (valid_rounds s hcode)
...
def gasSteps_branch_fall (s : State) (c : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4771, stack := c :: rho}
      {s with pc := UInt256.ofNat 4775, stack := rho} := by
  apply PadLift.gasSteps_of_raw branchSite {s with pc := UInt256.ofNat 4771, stack := c :: rho} _ hcode hfork hrun hnp branch_pc.symm branch_advances
  have h := StaggerPad.run_branch_fall s (UInt256.ofNat 4771) c rho hstack hrun hc
  rw [branch_end] at h
  exact h
...
def gasSteps_high (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4775, stack := ret :: rest}
      {s with
        pc := UInt256.ofNat 4803
        stack := ret :: rest
        memory := StaggerTablePad.highStores s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  apply PadLift.gasSteps_of_raw highSite {s with pc := UInt256.ofNat 4775, stack := ret :: rest} _ hcode hfork hrun hnp high_pc.symm high_advances
  have h := StaggerPad.run_high s (UInt256.ofNat 4775) ret rest (by omega) hrun hactive hfit
  rw [high_end] at h
  exact h
