...
theorem pad_slice :
    (Artifact.submissionArtifact.instructions.drop 3795).take StaggerPad.padTemplate.length = StaggerPad.padTemplate := by rfl

def padSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.padTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.padTemplate 3795 pad_slice
    (by change 3795 + StaggerPad.padTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.padTemplate) (by decide))
    (by decide)
theorem pad_pc : padSite.startPC = UInt256.ofNat 4785 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3795) = UInt256.ofNat 4785
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem pad_end : pcAfter (UInt256.ofNat 4785) StaggerPad.padTemplate = UInt256.ofNat 4855 := by decide
...
def gasSteps_pad (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4785, stack := ret :: rest}
      {s with pc := UInt256.ofNat 4855, stack := ret :: rest, memory := StaggerTablePad.resultMemory s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  apply PadLift.gasSteps_of_raw padSite {s with pc := UInt256.ofNat 4785, stack := ret :: rest} _ hcode hfork hrun hnp pad_pc.symm pad_advances
  have h := StaggerPad.run_pad s (UInt256.ofNat 4785) ret rest (by omega) hrun hactive hfit
  rw [pad_end] at h
  exact h
