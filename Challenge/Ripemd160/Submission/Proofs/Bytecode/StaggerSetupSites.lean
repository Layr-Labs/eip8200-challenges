...
def normalSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualNormalTemplate :=
  StackSiteBuilder.ofSlice actualNormalTemplate 342 normal_slice
    ...
    (by decide)
theorem normal_pc : normalSite.startPC = UInt256.ofNat 536 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 342) = UInt256.ofNat 540
...
theorem normal_end : pcAfter (UInt256.ofNat 536) actualNormalTemplate = UInt256.ofNat 977 := by decide
...
def padSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.padTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.padTemplate 285 pad_slice
    ...
    (by decide)
theorem pad_pc : padSite.startPC = UInt256.ofNat 446 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 285) = UInt256.ofNat 450
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem pad_end : pcAfter (UInt256.ofNat 446) StaggerPad.padTemplate = UInt256.ofNat 519 := by decide
...
def gasSteps_normal (s : State) (ret : UInt256) (p : Nat) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 536, stack := UInt256.ofNat p :: ret :: rest}
      {s with pc := UInt256.ofNat 977, stack := ret :: rest, memory := StaggerTableLayout.resultMemory s.memory (PairedScheduleData.extractedWord s.memory p), activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  apply normal_gasSteps_of_raw {s with pc := UInt256.ofNat 536, stack := UInt256.ofNat p :: ret :: rest} _ hcode hfork hrun hnp normal_pc.symm
  have h := StaggerNormal.run_normal s (UInt256.ofNat 535) ret p rest hstack hrun hp hbound
  have hfull : StaggerNormal.normalTemplate = .op .JUMPDEST :: actualNormalTemplate := by rfl
  have hend : pcAfter (UInt256.ofNat 535) StaggerNormal.normalTemplate = UInt256.ofNat 977 := by decide
  rw [hend, hfull] at h
...
def gasSteps_pad (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 446, stack := ret :: rest}
      {s with pc := UInt256.ofNat 519, stack := ret :: rest, memory := StaggerTablePad.resultMemory s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  apply PadLift.gasSteps_of_raw padSite {s with pc := UInt256.ofNat 446, stack := ret :: rest} _ hcode hfork hrun hnp pad_pc.symm pad_advances
  have h := StaggerPad.run_pad s (UInt256.ofNat 446) ret rest (by omega) hrun hactive hfit
  rw [pad_end] at h
  exact h
