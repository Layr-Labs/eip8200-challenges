...
def normalSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualNormalTemplate :=
  StackSiteBuilder.ofSlice actualNormalTemplate 281 normal_slice
    ...
    (by decide)
theorem normal_pc : normalSite.startPC = UInt256.ofNat 502 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 281) = UInt256.ofNat 502
...
theorem normal_end : pcAfter (UInt256.ofNat 502) actualNormalTemplate = UInt256.ofNat 920 := by decide

theorem pad_slice :
    (Artifact.submissionArtifact.instructions.drop 3791).take StaggerPad.padTemplate.length = StaggerPad.padTemplate := by rfl

def padSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.padTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.padTemplate 3791 pad_slice
    (by change 3791 + StaggerPad.padTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.padTemplate) (by decide))
    (by decide)
theorem pad_pc : padSite.startPC = UInt256.ofNat 4773 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3791) = UInt256.ofNat 4773
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem pad_end : pcAfter (UInt256.ofNat 4773) StaggerPad.padTemplate = UInt256.ofNat 4843 := by decide
...
def gasSteps_normal (s : State) (ret : UInt256) (p : Nat) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 502, stack := UInt256.ofNat p :: DenseScheduleTemplate.mask8 :: DenseScheduleTemplate.mask16 :: ret :: rest}
  ...
  | cons mask tailRest =>
    ...
    apply normal_gasSteps_of_raw {s with pc := UInt256.ofNat 502, stack := UInt256.ofNat p :: DenseScheduleTemplate.mask8 :: DenseScheduleTemplate.mask16 :: ret :: PairedMask32Cache.maskWord :: tailRest} _ hcode hfork hrun hnp normal_pc.symm
    have h := DeferredNormalSchedule.run_normal s (UInt256.ofNat 502) ret p tailRest (by omega) hrun hp hbound hlow
    have hend : pcAfter (UInt256.ofNat 502) DeferredNormalSchedule.normalTemplate = UInt256.ofNat 920 := by decide
    rw [hend] at h
    exact h
...
def gasSteps_pad (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4773, stack := ret :: rest}
      {s with pc := UInt256.ofNat 4843, stack := ret :: rest, memory := StaggerTablePad.resultMemory s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  apply PadLift.gasSteps_of_raw padSite {s with pc := UInt256.ofNat 4773, stack := ret :: rest} _ hcode hfork hrun hnp pad_pc.symm pad_advances
  have h := StaggerPad.run_pad s (UInt256.ofNat 4773) ret rest (by omega) hrun hactive hfit
  rw [pad_end] at h
  exact h
