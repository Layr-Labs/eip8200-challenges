...
theorem normal_slice :
    (Artifact.submissionArtifact.instructions.drop 291).take actualNormalTemplate.length = actualNormalTemplate := by rfl

def normalSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualNormalTemplate :=
  StackSiteBuilder.ofSlice actualNormalTemplate 291 normal_slice
    (by change 291 + actualNormalTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualNormalTemplate) (by decide))
    (by decide)
theorem normal_pc : normalSite.startPC = UInt256.ofNat 503 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 291) = UInt256.ofNat 503
...
theorem normal_end : pcAfter (UInt256.ofNat 503) actualNormalTemplate = UInt256.ofNat 919 := by decide

theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3740).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3740 low_slice
    (by change 3740 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.lowTemplate) (by decide))
    (by decide)
theorem low_pc : lowSite.startPC = UInt256.ofNat 4721 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3740) = UInt256.ofNat 4721
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem low_end : pcAfter (UInt256.ofNat 4721) StaggerPad.lowTemplate = UInt256.ofNat 4764 := by decide

open StaggerPad (branchTemplate)

theorem branch_slice :
    (Artifact.submissionArtifact.instructions.drop 3770).take branchTemplate.length = branchTemplate := by rfl

def branchSite : GenericRoundSite Artifact.submissionArtifact .Osaka branchTemplate :=
  StackSiteBuilder.ofSlice branchTemplate 3770 branch_slice
    (by change 3770 + branchTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := branchTemplate) (by decide))
    (by decide)
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4764 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3770) = UInt256.ofNat 4764
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_end : pcAfter (UInt256.ofNat 4764) branchTemplate = UInt256.ofNat 4768 := by decide

theorem high_slice :
    (Artifact.submissionArtifact.instructions.drop 3772).take StaggerPad.highTemplate.length = StaggerPad.highTemplate := by rfl

def highSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.highTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.highTemplate 3772 high_slice
    (by change 3772 + StaggerPad.highTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.highTemplate) (by decide))
    (by decide)
theorem high_pc : highSite.startPC = UInt256.ofNat 4768 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3772) = UInt256.ofNat 4768
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem high_end : pcAfter (UInt256.ofNat 4768) StaggerPad.highTemplate = UInt256.ofNat 4796 := by decide
...
theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 568 = 919 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 568 (by rfl)
  rw [hpc] at h
...
def gasSteps_normal (s : State) (ret : UInt256) (p : Nat) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 503, stack := UInt256.ofNat p :: DenseScheduleTemplate.mask8 :: DenseScheduleTemplate.mask16 :: ret :: rest}
      {s with pc := UInt256.ofNat 919, stack := ret :: rest, memory := StaggerTableLayout.resultMemory s.memory (StaggerScratch.dirtyWord s.memory p), activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  cases rest with
  | nil => simp at hmask
  | cons mask tailRest =>
    ...
    apply normal_gasSteps_of_raw {s with pc := UInt256.ofNat 503, stack := UInt256.ofNat p :: DenseScheduleTemplate.mask8 :: DenseScheduleTemplate.mask16 :: ret :: PairedMask32Cache.maskWord :: tailRest} _ hcode hfork hrun hnp normal_pc.symm
    have h := DeferredNormalSchedule.run_normal s (UInt256.ofNat 503) ret p tailRest (by omega) hrun hp hbound hlow hoff
    have hend : pcAfter (UInt256.ofNat 503) DeferredNormalSchedule.normalTemplate = UInt256.ofNat 919 := by decide
    rw [hend] at h
    exact h
...
def gasSteps_low (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4721, stack := ret :: rest}
      {s with
        pc := UInt256.ofNat 4764
  ...
  | cons mask tailRest =>
    ...
    subst mask
    simp only [List.length_cons] at hstack
    apply PadLift.gasSteps_of_raw lowSite {s with pc := UInt256.ofNat 4721, stack := ret :: UInt256.ofNat 4294967295 :: tailRest} _ hcode hfork hrun hnp low_pc.symm low_advances
    have h := StaggerPad.run_low s (UInt256.ofNat 4721) ret tailRest (by omega) hrun hactive hfit
    rw [low_end] at h
    exact h
...
def gasSteps_branch_taken (s : State) (c : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4764, stack := c :: rho}
      {s with pc := UInt256.ofNat 919, stack := rho} := by
  apply PadLift.gasSteps_of_raw branchSite {s with pc := UInt256.ofNat 4764, stack := c :: rho} _ hcode hfork hrun hnp branch_pc.symm branch_advances
  exact StaggerPad.run_branch_taken s (UInt256.ofNat 4764) c rho hstack hrun hc (valid_rounds s hcode)
...
def gasSteps_branch_fall (s : State) (c : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4764, stack := c :: rho}
      {s with pc := UInt256.ofNat 4768, stack := rho} := by
  apply PadLift.gasSteps_of_raw branchSite {s with pc := UInt256.ofNat 4764, stack := c :: rho} _ hcode hfork hrun hnp branch_pc.symm branch_advances
  have h := StaggerPad.run_branch_fall s (UInt256.ofNat 4764) c rho hstack hrun hc
  rw [branch_end] at h
  exact h
...
def gasSteps_high (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4768, stack := ret :: rest}
      {s with
        pc := UInt256.ofNat 4796
        stack := ret :: rest
        memory := StaggerTablePad.highStores s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  apply PadLift.gasSteps_of_raw highSite {s with pc := UInt256.ofNat 4768, stack := ret :: rest} _ hcode hfork hrun hnp high_pc.symm high_advances
  have h := StaggerPad.run_high s (UInt256.ofNat 4768) ret rest (by omega) hrun hactive hfit
  rw [high_end] at h
  exact h
