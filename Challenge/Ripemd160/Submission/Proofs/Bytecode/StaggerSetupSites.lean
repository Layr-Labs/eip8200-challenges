...
theorem normal_end : pcAfter (UInt256.ofNat 509) actualNormalTemplate = UInt256.ofNat 923 := by decide

theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3788).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3788 low_slice
    (by change 3788 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.lowTemplate) (by decide))
    (by decide)
theorem low_pc : lowSite.startPC = UInt256.ofNat 4776 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3788) = UInt256.ofNat 4776
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem low_end : pcAfter (UInt256.ofNat 4776) StaggerPad.lowTemplate = UInt256.ofNat 4832 := by decide
...
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4832 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3813) = UInt256.ofNat 4832
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem high_pc : highSite.startPC = UInt256.ofNat 4836 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3815) = UInt256.ofNat 4836
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem high_end : pcAfter (UInt256.ofNat 4836) StaggerPad.highTemplate = UInt256.ofNat 4866 := by decide

theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 923).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 562 = 923 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 562 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 923 = true
  rw [hcode]
  exact h
...
def gasSteps_normal (s : State) (ret : UInt256) (p : Nat) (rest : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 509, stack := UInt256.ofNat p :: DenseScheduleTemplate.mask8 :: DenseScheduleTemplate.mask16 :: ret :: rest}
      {s with pc := UInt256.ofNat 923, stack := ret :: rest, memory := StaggerTableLayout.resultMemory s.memory (StaggerScratch.dirtyWord s.memory p), activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  cases rest with
  | nil => simp at hmask
  | cons mask tailRest =>
    ...
    have hend : pcAfter (UInt256.ofNat 509) DeferredNormalSchedule.normalTemplate = UInt256.ofNat 923 := by decide
    rw [hend] at h
    exact h
...
def gasSteps_low (s : State) (ret : UInt256) (rest : List UInt256)
  ...
  | cons mask tailRest =>
    ...
    subst mask
    simp only [List.length_cons] at hstack
    apply PadLift.gasSteps_of_raw lowSite {s with pc := UInt256.ofNat 4776, stack := ret :: UInt256.ofNat 4294967295 :: tailRest} _ hcode hfork hrun hnp low_pc.symm low_advances
    have h := StaggerPad.run_low s (UInt256.ofNat 4776) ret tailRest (by omega) hrun hactive hfit
    rw [low_end] at h
    exact h
...
def gasSteps_branch_taken (s : State) (c : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4832, stack := c :: rho}
      {s with pc := UInt256.ofNat 923, stack := rho} := by
...
def gasSteps_high (s : State) (ret : UInt256) (rest : List UInt256)
  ...
  | cons mask tailRest =>
    ...
    subst mask
    simp only [List.length_cons] at hstack
    apply PadLift.gasSteps_of_raw highSite {s with pc := UInt256.ofNat 4836, stack := ret :: UInt256.ofNat 4294967295 :: tailRest} _ hcode hfork hrun hnp high_pc.symm high_advances
    have h := StaggerPad.run_high s (UInt256.ofNat 4836) ret tailRest (by omega) hrun hactive hfit
    rw [high_end] at h
    exact h
