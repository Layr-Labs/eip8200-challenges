...
theorem normal_pc : normalSite.startPC = UInt256.ofNat 504 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 285) = UInt256.ofNat 504
...
theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3787).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3787 low_slice
    (by change 3787 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem low_pc : lowSite.startPC = UInt256.ofNat 4784 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3787) = UInt256.ofNat 4784
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem low_end : pcAfter (UInt256.ofNat 4784) StaggerPad.lowTemplate = UInt256.ofNat 4825 := by decide

open StaggerPad (branchTemplate)

theorem branch_slice :
    (Artifact.submissionArtifact.instructions.drop 3815).take branchTemplate.length = branchTemplate := by rfl

def branchSite : GenericRoundSite Artifact.submissionArtifact .Osaka branchTemplate :=
  StackSiteBuilder.ofSlice branchTemplate 3815 branch_slice
    (by change 3815 + branchTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := branchTemplate) (by decide))
    (by decide)
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4825 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3815) = UInt256.ofNat 4825
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_end : pcAfter (UInt256.ofNat 4825) branchTemplate = UInt256.ofNat 4829 := by decide

theorem high_slice :
    (Artifact.submissionArtifact.instructions.drop 3817).take StaggerPad.highTemplate.length = StaggerPad.highTemplate := by rfl

def highSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.highTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.highTemplate 3817 high_slice
    (by change 3817 + StaggerPad.highTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.highTemplate) (by decide))
    (by decide)
theorem high_pc : highSite.startPC = UInt256.ofNat 4829 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3817) = UInt256.ofNat 4829
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem high_end : pcAfter (UInt256.ofNat 4829) StaggerPad.highTemplate = UInt256.ofNat 4857 := by decide

theorem valid_rounds (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 920).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 563 = 920 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 563 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 920 = true
  rw [hcode]
  exact h
...
def gasSteps_low (s : State) (ret : UInt256) (rest : List UInt256)
  ...
  | cons mask tailRest =>
    ...
    subst mask
    simp only [List.length_cons] at hstack
    apply PadLift.gasSteps_of_raw lowSite {s with pc := UInt256.ofNat 4784, stack := ret :: UInt256.ofNat 4294967295 :: tailRest} _ hcode hfork hrun hnp low_pc.symm low_advances
    have h := StaggerPad.run_low s (UInt256.ofNat 4784) ret tailRest (by omega) hrun hactive hfit
    rw [low_end] at h
    exact h
...
def gasSteps_branch_taken (s : State) (c : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4825, stack := c :: rho}
      {s with pc := UInt256.ofNat 920, stack := rho} := by
  apply PadLift.gasSteps_of_raw branchSite {s with pc := UInt256.ofNat 4825, stack := c :: rho} _ hcode hfork hrun hnp branch_pc.symm branch_advances
  exact StaggerPad.run_branch_taken s (UInt256.ofNat 4825) c rho hstack hrun hc (valid_rounds s hcode)
...
def gasSteps_branch_fall (s : State) (c : UInt256) (rho : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4825, stack := c :: rho}
      {s with pc := UInt256.ofNat 4829, stack := rho} := by
  apply PadLift.gasSteps_of_raw branchSite {s with pc := UInt256.ofNat 4825, stack := c :: rho} _ hcode hfork hrun hnp branch_pc.symm branch_advances
  have h := StaggerPad.run_branch_fall s (UInt256.ofNat 4825) c rho hstack hrun hc
  rw [branch_end] at h
  exact h
...
def gasSteps_high (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4829, stack := ret :: rest}
      {s with
        pc := UInt256.ofNat 4857
        stack := ret :: rest
        memory := StaggerTablePad.highStores s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  apply PadLift.gasSteps_of_raw highSite {s with pc := UInt256.ofNat 4829, stack := ret :: rest} _ hcode hfork hrun hnp high_pc.symm high_advances
  have h := StaggerPad.run_high s (UInt256.ofNat 4829) ret rest (by omega) hrun hactive hfit
  rw [high_end] at h
  exact h
