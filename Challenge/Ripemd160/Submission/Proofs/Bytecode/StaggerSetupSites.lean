...
theorem normal_slice :
    (Artifact.submissionArtifact.instructions.drop 290).take actualNormalTemplate.length = actualNormalTemplate := by rfl

def normalSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualNormalTemplate :=
  StackSiteBuilder.ofSlice actualNormalTemplate 290 normal_slice
    (by change 290 + actualNormalTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualNormalTemplate) (by decide))
    (by decide)
theorem normal_pc : normalSite.startPC = UInt256.ofNat 478 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 290) = UInt256.ofNat 478
...
theorem normal_end : pcAfter (UInt256.ofNat 478) actualNormalTemplate = UInt256.ofNat 943 := by decide
...
def padSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.padTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.padTemplate 233 pad_slice
    (by change 233 + StaggerPad.padTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := StaggerPad.padTemplate) (by decide))
    (by decide)
theorem pad_pc : padSite.startPC = UInt256.ofNat 388 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 233) = UInt256.ofNat 388
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem pad_end : pcAfter (UInt256.ofNat 388) StaggerPad.padTemplate = UInt256.ofNat 461 := by decide
...
def gasSteps_normal (s : State) (ret : UInt256) (p : Nat) (rest : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 478, stack := UInt256.ofNat p :: ret :: rest}
      {s with pc := UInt256.ofNat 943, stack := ret :: rest, memory := StaggerTableLayout.resultMemory s.memory (PairedScheduleData.extractedWord s.memory p), activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  ...
  have hend : pcAfter (UInt256.ofNat 477) StaggerNormal.normalTemplate = UInt256.ofNat 943 := by decide
  rw [hend, hfull] at h
  ...
  simpa only [DenseScheduleTemplate.scheduleEntry, List.cons_append, List.nil_append, show (UInt256.ofNat 477).succ = UInt256.ofNat 478 by decide] using ht
...
def gasSteps_pad (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 388, stack := ret :: rest}
      {s with pc := UInt256.ofNat 461, stack := ret :: rest, memory := StaggerTablePad.resultMemory s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  apply PadLift.gasSteps_of_raw padSite {s with pc := UInt256.ofNat 388, stack := ret :: rest} _ hcode hfork hrun hnp pad_pc.symm pad_advances
  have h := StaggerPad.run_pad s (UInt256.ofNat 388) ret rest (by omega) hrun hactive hfit
  rw [pad_end] at h
  exact h
