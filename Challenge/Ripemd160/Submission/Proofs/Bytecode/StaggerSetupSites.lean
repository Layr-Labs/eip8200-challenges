...
theorem normal_slice :
    (Artifact.submissionArtifact.instructions.drop 342).take actualNormalTemplate.length = actualNormalTemplate := by rfl

def normalSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualNormalTemplate :=
  StackSiteBuilder.ofSlice actualNormalTemplate 342 normal_slice
    (by change 342 + actualNormalTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem normal_pc : normalSite.startPC = UInt256.ofNat 612 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 342) = UInt256.ofNat 612
...
theorem pad_slice :
    (Artifact.submissionArtifact.instructions.drop 285).take StaggerPad.padTemplate.length = StaggerPad.padTemplate := by rfl

def padSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.padTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.padTemplate 285 pad_slice
    (by change 285 + StaggerPad.padTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem pad_pc : padSite.startPC = UInt256.ofNat 522 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 285) = UInt256.ofNat 522
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
