...
theorem low_slice :
    (Artifact.submissionArtifact.instructions.drop 3758).take StaggerPad.lowTemplate.length = StaggerPad.lowTemplate := by rfl

def lowSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.lowTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.lowTemplate 3758 low_slice
    (by change 3758 + StaggerPad.lowTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem low_pc : lowSite.startPC = UInt256.ofNat 4741 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3758) = UInt256.ofNat 4741
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_slice :
    (Artifact.submissionArtifact.instructions.drop 3783).take branchTemplate.length = branchTemplate := by rfl

def branchSite : GenericRoundSite Artifact.submissionArtifact .Osaka branchTemplate :=
  StackSiteBuilder.ofSlice branchTemplate 3783 branch_slice
    (by change 3783 + branchTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4797 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3783) = UInt256.ofNat 4797
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem high_slice :
    (Artifact.submissionArtifact.instructions.drop 3785).take StaggerPad.highTemplate.length = StaggerPad.highTemplate := by rfl

def highSite : GenericRoundSite Artifact.submissionArtifact .Osaka StaggerPad.highTemplate :=
  StackSiteBuilder.ofSlice StaggerPad.highTemplate 3785 high_slice
    (by change 3785 + StaggerPad.highTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem high_pc : highSite.startPC = UInt256.ofNat 4801 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3785) = UInt256.ofNat 4801
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem high_end : pcAfter (UInt256.ofNat 4801) StaggerPad.highTemplate = UInt256.ofNat 4831 := by decide
...
def gasSteps_high (s : State) (ret : UInt256) (rest : List UInt256)
  ...
  | cons mask tailRest =>
    ...
    subst mask
    simp only [List.length_cons] at hstack
    apply PadLift.gasSteps_of_raw highSite {s with pc := UInt256.ofNat 4801, stack := ret :: UInt256.ofNat 4294967295 :: tailRest} _ hcode hfork hrun hnp high_pc.symm high_advances
    have h := StaggerPad.run_high s (UInt256.ofNat 4801) ret tailRest (by omega) hrun hactive hfit
    rw [high_end] at h
    exact h
