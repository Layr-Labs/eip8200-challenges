...
theorem low_end : pcAfter (UInt256.ofNat 4784) StaggerPad.lowTemplate = UInt256.ofNat 4827 := by decide
...
theorem high_end : pcAfter (UInt256.ofNat 4831) StaggerPad.highTemplate = UInt256.ofNat 4859 := by decide
...
def gasSteps_branch_taken (s : State) (c : UInt256) (rho : List UInt256)
  ...
  exact StaggerPad.run_branch_taken s (UInt256.ofNat 4827) c rho hstack hrun hc (valid_rounds s hcode)
...
def gasSteps_branch_fall (s : State) (c : UInt256) (rho : List UInt256)
  ...
  apply PadLift.gasSteps_of_raw branchSite {s with pc := UInt256.ofNat 4827, stack := c :: rho} _ hcode hfork hrun hnp branch_pc.symm branch_advances
  have h := StaggerPad.run_branch_fall s (UInt256.ofNat 4827) c rho hstack hrun hc
  rw [branch_end] at h
  exact h
...
def gasSteps_high (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4831, stack := ret :: rest}
      {s with
        pc := UInt256.ofNat 4859
        stack := ret :: rest
        memory := StaggerTablePad.highStores s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  apply PadLift.gasSteps_of_raw highSite {s with pc := UInt256.ofNat 4831, stack := ret :: rest} _ hcode hfork hrun hnp high_pc.symm high_advances
  have h := StaggerPad.run_high s (UInt256.ofNat 4831) ret rest (by omega) hrun hactive hfit
  rw [high_end] at h
  exact h
