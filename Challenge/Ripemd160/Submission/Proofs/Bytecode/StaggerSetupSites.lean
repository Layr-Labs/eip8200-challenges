...
theorem low_pc : lowSite.startPC = UInt256.ofNat 4785 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3791) = UInt256.ofNat 4785
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem branch_pc : branchSite.startPC = UInt256.ofNat 4828 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3821) = UInt256.ofNat 4828
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem high_pc : highSite.startPC = UInt256.ofNat 4832 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3823) = UInt256.ofNat 4832
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem high_end : pcAfter (UInt256.ofNat 4832) StaggerPad.highTemplate = UInt256.ofNat 4862 := by decide
...
def gasSteps_high (s : State) (ret : UInt256) (rest : List UInt256)
  ...
  | cons mask tailRest =>
    ...
    subst mask
    simp only [List.length_cons] at hstack
    apply PadLift.gasSteps_of_raw highSite {s with pc := UInt256.ofNat 4832, stack := ret :: UInt256.ofNat 4294967295 :: tailRest} _ hcode hfork hrun hnp high_pc.symm high_advances
    have h := StaggerPad.run_high s (UInt256.ofNat 4832) ret tailRest (by omega) hrun hactive hfit
    rw [high_end] at h
    exact h
