...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3045).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3045 actual_slice
    (by change 3045 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3890 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3045) = UInt256.ofNat 3890
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 3890) template = UInt256.ofNat 3943 := by decide
  rw [hend] at hraw
