...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3078).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3078 actual_slice
    (by change 3078 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3938 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3078) = UInt256.ofNat 3938
