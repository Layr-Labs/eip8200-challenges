...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2974).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2974 actual_slice
    (by change 2974 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3819 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2974) = UInt256.ofNat 3819
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 3819) template = UInt256.ofNat 3870 := by decide
  rw [hend] at hraw
