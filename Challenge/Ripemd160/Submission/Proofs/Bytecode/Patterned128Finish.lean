...
def selectorPath : List Located :=
  [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT,
   ...
   opAt 4107 .JUMPDEST, opAt 4108 .CALLDATASIZE, pushAt 4109 1 63,
   opAt 4110 .EQ, pushAt 4111 2 5076, opAt 4112 .JUMPI]

def digestStorePath : List Located :=
  [opAt 4100 .JUMPDEST,
   pushAt 4101 20 paddedDigestWord,
   pushAt 4102 0 0,
   opAt 4103 .MSTORE]

def digestFinishPath : List Located :=
  [pushAt 4105 0 0, opAt 4106 .RETURN]
...
@[simp] private theorem digestStorePC4160 :
    Artifact.submissionArtifact.instructionPC 4100 = 5076 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4161 :
    Artifact.submissionArtifact.instructionPC 4101 = 5077 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4162 :
    Artifact.submissionArtifact.instructionPC 4102 = 5098 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4163 :
    Artifact.submissionArtifact.instructionPC 4103 = 5099 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4165 :
    Artifact.submissionArtifact.instructionPC 4105 = 5101 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4166 :
    Artifact.submissionArtifact.instructionPC 4106 = 5102 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem shortHelperPC4181 :
    Artifact.submissionArtifact.instructionPC 4107 = 5103 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4182 :
    Artifact.submissionArtifact.instructionPC 4108 = 5104 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4183 :
    Artifact.submissionArtifact.instructionPC 4109 = 5105 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4184 :
    Artifact.submissionArtifact.instructionPC 4110 = 5107 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4185 :
    Artifact.submissionArtifact.instructionPC 4111 = 5108 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4186 :
    Artifact.submissionArtifact.instructionPC 4112 = 5111 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl
...
theorem run_selector (input : ByteArray) (sv ov : UInt256)
  ...
  have ha : run [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT, pushAt 163 2 5103, opAt 164 .JUMPI] (stS input 252 (returnRest sv ov)) = some (stS input 5103 (returnRest sv ov)) := by
    ...
    exact h01234
  have hb : run [opAt 4107 .JUMPDEST] (stS input 5103 (returnRest sv ov)) = some (stS input 5104 (returnRest sv ov)) := by
    exact blockOfS _ (pcFactS input 4107 5103 _ (by norm_num) shortHelperPC4181)
      (stepS_jumpdest input 5103 (returnRest sv ov) (by simp [returnRest]) (by norm_num))
  have hc : run [opAt 4108 .CALLDATASIZE, pushAt 4109 1 63, opAt 4110 .EQ, pushAt 4111 2 5076, opAt 4112 .JUMPI] (stS input 5104 (returnRest sv ov)) = some (stS input 5076 (returnRest sv ov)) := by
    have hdest : Decode.isValidJumpDest submissionBytecode 5076 = true :=
      Artifact.submissionArtifact.isValidJumpDest_index 4100 (by rfl)
    ...
    have h0 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            ...
            (stS input 5104 (returnRest sv ov)) =
          some (stS input 5105 (UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4108 5104 _ (by norm_num) shortHelperPC4182)
        (stepS_calldatasize input 5104 (returnRest sv ov)
          (by simp [returnRest]) (by norm_num))
    have h1 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [pushAt 4109 1 63]
            (stS input 5105 (UInt256.ofNat input.size :: returnRest sv ov)) =
          some (stS input 5107 (63 :: UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4109 5105 _ (by norm_num) shortHelperPC4183)
        (stepS_push input 5105 1 63
          (UInt256.ofNat input.size :: returnRest sv ov)
          (by simp [returnRest]) (by decide) (by decide) (by norm_num))
    have h2 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4110 .EQ]
            (stS input 5107 (63 :: UInt256.ofNat input.size :: returnRest sv ov)) =
          some (stS input 5108
            (UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4110 5107 _ (by norm_num) shortHelperPC4184)
        (stepS_eq input 5107 63 (UInt256.ofNat input.size) (returnRest sv ov)
          (by simp [returnRest]) (by norm_num))
    have h3 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [pushAt 4111 2 5076]
          ...
          some (stS input 5111
            (5076 :: UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4111 5108 _ (by norm_num) shortHelperPC4185)
        (stepS_push input 5108 2 5076
          (UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)
          (by simp [returnRest]) (by decide) (by decide) (by norm_num))
    have h4 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4112 .JUMPI]
            (stS input 5111
              (5076 :: UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) =
          some (stS input 5076 (returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4112 5111 _ (by norm_num) shortHelperPC4186)
        (stepS_jumpi_taken input 5111 5076 5076
          (UInt256.eq 63 (UInt256.ofNat input.size)) (returnRest sv ov)
          (by simp [returnRest]) (by norm_num) rfl htrue hdest)
    have h01 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4108 .CALLDATASIZE, pushAt 4109 1 63]
            (stS input 5104 (returnRest sv ov)) =
          some (stS input 5107 (63 :: UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4108 .CALLDATASIZE] [pushAt 4109 1 63]
    ...
    have h012 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4108 .CALLDATASIZE, pushAt 4109 1 63, opAt 4110 .EQ]
            (stS input 5104 (returnRest sv ov)) =
          some (stS input 5108
            (UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
    ...
    have h0123 :
      ...
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4108 .CALLDATASIZE, pushAt 4109 1 63, opAt 4110 .EQ]
        [pushAt 4111 2 5076] _ _ _ h012 (by rfl) h3
    have h01234 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4108 .CALLDATASIZE, pushAt 4109 1 63, opAt 4110 .EQ,
              pushAt 4111 2 5076, opAt 4112 .JUMPI]
            (stS input 5104 (returnRest sv ov)) =
          some (stS input 5076 (returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4108 .CALLDATASIZE, pushAt 4109 1 63, opAt 4110 .EQ,
          pushAt 4111 2 5076] [opAt 4112 .JUMPI] _ _ _ h0123 (by rfl) h4
    exact h01234
  have hbc := Challenge.EvmProof.Stepper.runLocatedBlock_append
    [opAt 4107 .JUMPDEST] [opAt 4108 .CALLDATASIZE, pushAt 4109 1 63, opAt 4110 .EQ, pushAt 4111 2 5076, opAt 4112 .JUMPI] _ _ _ hb (by rfl) hc
  have habc := Challenge.EvmProof.Stepper.runLocatedBlock_append
    [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT, pushAt 163 2 5103, opAt 164 .JUMPI] ([opAt 4107 .JUMPDEST] ++ [opAt 4108 .CALLDATASIZE, pushAt 4109 1 63, opAt 4110 .EQ, pushAt 4111 2 5076, opAt 4112 .JUMPI]) _ _ _ ha (by rfl) hbc
  exact habc
...
def gasSteps_return (input : ByteArray) (sv ov : UInt256)
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 4104 .MSIZE
    (by rfl) (by decide) trivial
  have hp : (storedState input sv ov).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4104 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
  have hop : (storedState input sv ov).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState input sv ov) 4104
