...
def selectorPath : List Located :=
  [opAt 166 .CALLDATASIZE, pushAt 167 1 129, opAt 168 .GT,
   pushAt 169 2 5202, opAt 170 .JUMPI,
   opAt 4181 .JUMPDEST, opAt 4182 .CALLDATASIZE, pushAt 4183 1 128,
   opAt 4184 .EQ, pushAt 4185 2 5155, opAt 4186 .JUMPI]

def digestStorePath : List Located :=
  [opAt 4159 .JUMPDEST,
   pushAt 4160 20 paddedDigestWord,
   pushAt 4161 0 0,
   opAt 4162 .MSTORE]
...
@[simp] private theorem digestStorePC4160 :
    Artifact.submissionArtifact.instructionPC 4159 = 5155 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4161 :
    Artifact.submissionArtifact.instructionPC 4160 = 5156 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4162 :
    Artifact.submissionArtifact.instructionPC 4161 = 5177 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4163 :
    Artifact.submissionArtifact.instructionPC 4162 = 5178 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4165 :
    Artifact.submissionArtifact.instructionPC 4164 = 5180 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4166 :
    Artifact.submissionArtifact.instructionPC 4165 = 5181 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem shortHelperPC4181 :
    Artifact.submissionArtifact.instructionPC 4181 = 5202 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4182 :
    Artifact.submissionArtifact.instructionPC 4182 = 5203 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4183 :
    Artifact.submissionArtifact.instructionPC 4183 = 5204 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4184 :
    Artifact.submissionArtifact.instructionPC 4184 = 5206 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4185 :
    Artifact.submissionArtifact.instructionPC 4185 = 5207 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4186 :
    Artifact.submissionArtifact.instructionPC 4186 = 5210 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl
...
theorem run_selector (input : ByteArray) (sv ov : UInt256)
  ...
  have ha : run [opAt 166 .CALLDATASIZE, pushAt 167 1 129, opAt 168 .GT, pushAt 169 2 5202, opAt 170 .JUMPI] (stS input 260 (returnRest sv ov)) = some (stS input 5202 (returnRest sv ov)) := by
    have hdest : Decode.isValidJumpDest submissionBytecode 5202 = true :=
      Artifact.submissionArtifact.isValidJumpDest_index 4181 (by rfl)
    ...
    have h01 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 166 .CALLDATASIZE, pushAt 167 1 129]
            (stS input 260 (returnRest sv ov)) =
          some (stS input 263 (129 :: UInt256.ofNat input.size :: returnRest sv ov)) := by
    ...
    have h0123 :
      ...
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 166 .CALLDATASIZE, pushAt 167 1 129, opAt 168 .GT]
        [pushAt 169 2 5202] _ _ _ h012 (by rfl) h3
    have h01234 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 166 .CALLDATASIZE, pushAt 167 1 129, opAt 168 .GT,
              pushAt 169 2 5202, opAt 170 .JUMPI]
      ...
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 166 .CALLDATASIZE, pushAt 167 1 129, opAt 168 .GT,
          pushAt 169 2 5202] [opAt 170 .JUMPI] _ _ _ h0123 (by rfl) h4
    exact h01234
  have hb : run [opAt 4181 .JUMPDEST] (stS input 5202 (returnRest sv ov)) = some (stS input 5203 (returnRest sv ov)) := by
    exact blockOfS _ (pcFactS input 4181 5202 _ (by norm_num) shortHelperPC4181)
      (stepS_jumpdest input 5202 (returnRest sv ov) (by simp [returnRest]) (by norm_num))
  have hc : run [opAt 4182 .CALLDATASIZE, pushAt 4183 1 128, opAt 4184 .EQ, pushAt 4185 2 5155, opAt 4186 .JUMPI] (stS input 5203 (returnRest sv ov)) = some (stS input 5155 (returnRest sv ov)) := by
    ...
    have h1 :
      ...
      exact blockOfS _
        (pcFactS input 4183 5204 _ (by norm_num) shortHelperPC4183)
    ...
    have h2 :
      ...
      exact blockOfS _
        (pcFactS input 4184 5206 _ (by norm_num) shortHelperPC4184)
    ...
    have h3 :
      ...
      exact blockOfS _
        (pcFactS input 4185 5207 _ (by norm_num) shortHelperPC4185)
    ...
    have h4 :
      ...
      exact blockOfS _
        (pcFactS input 4186 5210 _ (by norm_num) shortHelperPC4186)
        (stepS_jumpi_taken input 5210 5155 5155
          (UInt256.eq 128 (UInt256.ofNat input.size)) (returnRest sv ov)
          (by simp [returnRest]) (by norm_num) rfl htrue hdest)
    have h01 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4182 .CALLDATASIZE, pushAt 4183 1 128]
            (stS input 5203 (returnRest sv ov)) =
          some (stS input 5206 (128 :: UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4182 .CALLDATASIZE] [pushAt 4183 1 128]
    ...
    have h012 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4182 .CALLDATASIZE, pushAt 4183 1 128, opAt 4184 .EQ]
            (stS input 5203 (returnRest sv ov)) =
          some (stS input 5207
            (UInt256.eq 128 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
    ...
    have h0123 :
      ...
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4182 .CALLDATASIZE, pushAt 4183 1 128, opAt 4184 .EQ]
        [pushAt 4185 2 5155] _ _ _ h012 (by rfl) h3
    have h01234 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4182 .CALLDATASIZE, pushAt 4183 1 128, opAt 4184 .EQ,
              pushAt 4185 2 5155, opAt 4186 .JUMPI]
            (stS input 5203 (returnRest sv ov)) =
          some (stS input 5155 (returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4182 .CALLDATASIZE, pushAt 4183 1 128, opAt 4184 .EQ,
          pushAt 4185 2 5155] [opAt 4186 .JUMPI] _ _ _ h0123 (by rfl) h4
    exact h01234
  have hbc := Challenge.EvmProof.Stepper.runLocatedBlock_append
    [opAt 4181 .JUMPDEST] [opAt 4182 .CALLDATASIZE, pushAt 4183 1 128, opAt 4184 .EQ, pushAt 4185 2 5155, opAt 4186 .JUMPI] _ _ _ hb (by rfl) hc
  have habc := Challenge.EvmProof.Stepper.runLocatedBlock_append
    [opAt 166 .CALLDATASIZE, pushAt 167 1 129, opAt 168 .GT, pushAt 169 2 5202, opAt 170 .JUMPI] ([opAt 4181 .JUMPDEST] ++ [opAt 4182 .CALLDATASIZE, pushAt 4183 1 128, opAt 4184 .EQ, pushAt 4185 2 5155, opAt 4186 .JUMPI]) _ _ _ ha (by rfl) hbc
  exact habc
...
def gasSteps_return (input : ByteArray) (sv ov : UInt256)
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 4163 .MSIZE
    (by rfl) (by decide) trivial
  have hp : (storedState input sv ov).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4163 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
  have hop : (storedState input sv ov).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState input sv ov) 4163
