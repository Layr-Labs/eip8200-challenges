...
def digestStorePath : List Located :=
  [opAt 4128 .JUMPDEST,
   pushAt 4129 20 paddedDigestWord,
   pushAt 4130 0 0,
   opAt 4131 .MSTORE]

def digestFinishPath : List Located :=
  [pushAt 4133 0 0, opAt 4134 .RETURN]
...
@[simp] private theorem digestStorePC4160 :
    Artifact.submissionArtifact.instructionPC 4128 = 5279 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4161 :
    Artifact.submissionArtifact.instructionPC 4129 = 5280 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4162 :
    Artifact.submissionArtifact.instructionPC 4130 = 5301 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4163 :
    Artifact.submissionArtifact.instructionPC 4131 = 5302 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4165 :
    Artifact.submissionArtifact.instructionPC 4133 = 5304 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4166 :
    Artifact.submissionArtifact.instructionPC 4134 = 5305 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem run_selector (input : ByteArray) (sv ov : UInt256)
  ...
  change run selectorPath (stS input 260 (returnRest sv ov)) =
    some (stS input 5279 (returnRest sv ov))
  have hdest : Decode.isValidJumpDest submissionBytecode 5279 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 4128 (by rfl)
  ...
  have h3 :
      Challenge.EvmProof.Stepper.runLocatedBlock
        ...
        some (stS input 267
          (5279 :: UInt256.eq 128 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
    exact blockOfS _
      (pcFactS input 169 264 _ (by norm_num) selectorPC169)
      (stepS_push input 264 2 5279
        (UInt256.eq 128 (UInt256.ofNat input.size) :: returnRest sv ov)
        (by simp [returnRest]) (by decide) (by decide) (by norm_num))
  have h4 :
      Challenge.EvmProof.Stepper.runLocatedBlock
          [opAt 170 .JUMPI]
          (stS input 267
            (5279 :: UInt256.eq 128 (UInt256.ofNat input.size) :: returnRest sv ov)) =
        some (stS input 5279 (returnRest sv ov)) := by
    exact blockOfS _
      (pcFactS input 170 267 _ (by norm_num) selectorPC170)
      (stepS_jumpi_taken input 267 5279 5279
        (UInt256.eq 128 (UInt256.ofNat input.size)) (returnRest sv ov)
  ...
  have h0123 :
    ...
    exact Challenge.EvmProof.Stepper.runLocatedBlock_append
      [opAt 166 .CALLDATASIZE, pushAt 167 1 128, opAt 168 .EQ]
      [pushAt 169 2 5279] _ _ _ h012 (by rfl) h3
  have h01234 :
      Challenge.EvmProof.Stepper.runLocatedBlock
          [opAt 166 .CALLDATASIZE, pushAt 167 1 128, opAt 168 .EQ,
            pushAt 169 2 5279, opAt 170 .JUMPI]
          (stS input 260 (returnRest sv ov)) =
        some (stS input 5279 (returnRest sv ov)) := by
    exact Challenge.EvmProof.Stepper.runLocatedBlock_append
      [opAt 166 .CALLDATASIZE, pushAt 167 1 128, opAt 168 .EQ,
        pushAt 169 2 5279] [opAt 170 .JUMPI] _ _ _ h0123 (by rfl) h4
...
def gasSteps_return (input : ByteArray) (sv ov : UInt256)
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 4132 .MSIZE
    (by rfl) (by decide) trivial
  have hp : (storedState input sv ov).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4132 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
  have hop : (storedState input sv ov).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState input sv ov) 4132
