...
def selectorPath : List Located :=
  [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT,
   pushAt 163 2 5105, opAt 164 .JUMPI,
   opAt 4105 .JUMPDEST, opAt 4106 .CALLDATASIZE, pushAt 4107 1 63,
   opAt 4108 .EQ, pushAt 4109 2 5078, opAt 4110 .JUMPI]

def digestStorePath : List Located :=
  [opAt 4098 .JUMPDEST,
   pushAt 4099 20 paddedDigestWord,
   pushAt 4100 0 0,
   opAt 4101 .MSTORE]

def digestFinishPath : List Located :=
  [pushAt 4103 0 0, opAt 4104 .RETURN]
...
@[simp] private theorem digestStorePC4160 :
    Artifact.submissionArtifact.instructionPC 4098 = 5078 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4161 :
    Artifact.submissionArtifact.instructionPC 4099 = 5079 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4162 :
    Artifact.submissionArtifact.instructionPC 4100 = 5100 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4163 :
    Artifact.submissionArtifact.instructionPC 4101 = 5101 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4165 :
    Artifact.submissionArtifact.instructionPC 4103 = 5103 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4166 :
    Artifact.submissionArtifact.instructionPC 4104 = 5104 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem shortHelperPC4181 :
    Artifact.submissionArtifact.instructionPC 4105 = 5105 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4182 :
    Artifact.submissionArtifact.instructionPC 4106 = 5106 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4183 :
    Artifact.submissionArtifact.instructionPC 4107 = 5107 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4184 :
    Artifact.submissionArtifact.instructionPC 4108 = 5109 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4185 :
    Artifact.submissionArtifact.instructionPC 4109 = 5110 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4186 :
    Artifact.submissionArtifact.instructionPC 4110 = 5113 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl
...
theorem run_selector (input : ByteArray) (sv ov : UInt256)
  ...
  have ha : run [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT, pushAt 163 2 5105, opAt 164 .JUMPI] (stS input 252 (returnRest sv ov)) = some (stS input 5105 (returnRest sv ov)) := by
    have hdest : Decode.isValidJumpDest submissionBytecode 5105 = true :=
      Artifact.submissionArtifact.isValidJumpDest_index 4105 (by rfl)
    ...
    have h3 :
        Challenge.EvmProof.Stepper.runLocatedBlock
          ...
          some (stS input 259
            (5105 :: UInt256.gt 129 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 163 256 _ (by norm_num) selectorPC169)
        (stepS_push input 256 2 5105
          (UInt256.gt 129 (UInt256.ofNat input.size) :: returnRest sv ov)
          (by simp [returnRest]) (by decide) (by decide) (by norm_num))
    have h4 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 164 .JUMPI]
            (stS input 259
              (5105 :: UInt256.gt 129 (UInt256.ofNat input.size) :: returnRest sv ov)) =
          some (stS input 5105 (returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 164 259 _ (by norm_num) selectorPC170)
        (stepS_jumpi_taken input 259 5105 5105
          (UInt256.gt 129 (UInt256.ofNat input.size)) (returnRest sv ov)
    ...
    have h0123 :
      ...
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT]
        [pushAt 163 2 5105] _ _ _ h012 (by rfl) h3
    have h01234 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT,
              pushAt 163 2 5105, opAt 164 .JUMPI]
      ...
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT,
          pushAt 163 2 5105] [opAt 164 .JUMPI] _ _ _ h0123 (by rfl) h4
    exact h01234
  have hb : run [opAt 4105 .JUMPDEST] (stS input 5105 (returnRest sv ov)) = some (stS input 5106 (returnRest sv ov)) := by
    exact blockOfS _ (pcFactS input 4105 5105 _ (by norm_num) shortHelperPC4181)
      (stepS_jumpdest input 5105 (returnRest sv ov) (by simp [returnRest]) (by norm_num))
  have hc : run [opAt 4106 .CALLDATASIZE, pushAt 4107 1 63, opAt 4108 .EQ, pushAt 4109 2 5078, opAt 4110 .JUMPI] (stS input 5106 (returnRest sv ov)) = some (stS input 5078 (returnRest sv ov)) := by
    have hdest : Decode.isValidJumpDest submissionBytecode 5078 = true :=
    ...
    have h0 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4106 .CALLDATASIZE]
            (stS input 5106 (returnRest sv ov)) =
          some (stS input 5113 (UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4106 5106 _ (by norm_num) shortHelperPC4182)
        (stepS_calldatasize input 5106 (returnRest sv ov)
          (by simp [returnRest]) (by norm_num))
    have h1 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [pushAt 4107 1 63]
            (stS input 5113 (UInt256.ofNat input.size :: returnRest sv ov)) =
          some (stS input 5109 (63 :: UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4107 5107 _ (by norm_num) shortHelperPC4183)
        (stepS_push input 5107 1 63
          (UInt256.ofNat input.size :: returnRest sv ov)
          (by simp [returnRest]) (by decide) (by decide) (by norm_num))
    have h2 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4108 .EQ]
            (stS input 5109 (63 :: UInt256.ofNat input.size :: returnRest sv ov)) =
          some (stS input 5110
            (UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4108 5109 _ (by norm_num) shortHelperPC4184)
        (stepS_eq input 5109 63 (UInt256.ofNat input.size) (returnRest sv ov)
          (by simp [returnRest]) (by norm_num))
    have h3 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [pushAt 4109 2 5078]
          ...
          some (stS input 5113
            (5078 :: UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4109 5110 _ (by norm_num) shortHelperPC4185)
        (stepS_push input 5110 2 5078
          (UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)
          (by simp [returnRest]) (by decide) (by decide) (by norm_num))
    have h4 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4110 .JUMPI]
            (stS input 5113
              (5078 :: UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) =
          some (stS input 5078 (returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4110 5113 _ (by norm_num) shortHelperPC4186)
        (stepS_jumpi_taken input 5113 5078 5078
          (UInt256.eq 63 (UInt256.ofNat input.size)) (returnRest sv ov)
          (by simp [returnRest]) (by norm_num) rfl htrue hdest)
    have h01 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4106 .CALLDATASIZE, pushAt 4107 1 63]
            (stS input 5106 (returnRest sv ov)) =
          some (stS input 5109 (63 :: UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4106 .CALLDATASIZE] [pushAt 4107 1 63]
    ...
    have h012 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4106 .CALLDATASIZE, pushAt 4107 1 63, opAt 4108 .EQ]
            (stS input 5106 (returnRest sv ov)) =
          some (stS input 5110
            (UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
    ...
    have h0123 :
      ...
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4106 .CALLDATASIZE, pushAt 4107 1 63, opAt 4108 .EQ]
        [pushAt 4109 2 5078] _ _ _ h012 (by rfl) h3
    have h01234 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4106 .CALLDATASIZE, pushAt 4107 1 63, opAt 4108 .EQ,
              pushAt 4109 2 5078, opAt 4110 .JUMPI]
            (stS input 5106 (returnRest sv ov)) =
          some (stS input 5078 (returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4106 .CALLDATASIZE, pushAt 4107 1 63, opAt 4108 .EQ,
          pushAt 4109 2 5078] [opAt 4110 .JUMPI] _ _ _ h0123 (by rfl) h4
    exact h01234
  have hbc := Challenge.EvmProof.Stepper.runLocatedBlock_append
    [opAt 4105 .JUMPDEST] [opAt 4106 .CALLDATASIZE, pushAt 4107 1 63, opAt 4108 .EQ, pushAt 4109 2 5078, opAt 4110 .JUMPI] _ _ _ hb (by rfl) hc
  have habc := Challenge.EvmProof.Stepper.runLocatedBlock_append
    [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT, pushAt 163 2 5105, opAt 164 .JUMPI] ([opAt 4105 .JUMPDEST] ++ [opAt 4106 .CALLDATASIZE, pushAt 4107 1 63, opAt 4108 .EQ, pushAt 4109 2 5078, opAt 4110 .JUMPI]) _ _ _ ha (by rfl) hbc
  exact habc
...
def gasSteps_return (input : ByteArray) (sv ov : UInt256)
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 4102 .MSIZE
  ...
  have hop : (storedState input sv ov).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState input sv ov) 4102
