...
def digestEntryState (input : ByteArray) (sv ov : UInt256) : State :=
  stS input 5162 (returnRest sv ov)

def storedState (input : ByteArray) (sv ov : UInt256) : State :=
  { stS input 5186 (returnRest sv ov) with
    memory := answerMemory
...
@[simp] private theorem digestStorePC4160 :
    Artifact.submissionArtifact.instructionPC 4155 = 5162 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4161 :
    Artifact.submissionArtifact.instructionPC 4156 = 5163 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4162 :
    Artifact.submissionArtifact.instructionPC 4157 = 5184 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestStorePC4163 :
    Artifact.submissionArtifact.instructionPC 4158 = 5185 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4165 :
    Artifact.submissionArtifact.instructionPC 4160 = 5187 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem digestFinishPC4166 :
    Artifact.submissionArtifact.instructionPC 4161 = 5188 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] private theorem shortHelperPC4181 :
    Artifact.submissionArtifact.instructionPC 4162 = 5189 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4182 :
    Artifact.submissionArtifact.instructionPC 4163 = 5190 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4183 :
    Artifact.submissionArtifact.instructionPC 4164 = 5191 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4184 :
    Artifact.submissionArtifact.instructionPC 4165 = 5193 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4185 :
    Artifact.submissionArtifact.instructionPC 4166 = 5194 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl

@[simp] private theorem shortHelperPC4186 :
    Artifact.submissionArtifact.instructionPC 4167 = 5197 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  rfl
...
theorem run_selector (input : ByteArray) (sv ov : UInt256)
  ...
  have ha : run [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT, pushAt 163 2 5189, opAt 164 .JUMPI] (stS input 261 (returnRest sv ov)) = some (stS input 5189 (returnRest sv ov)) := by
    ...
    exact h01234
  have hb : run [opAt 4162 .JUMPDEST] (stS input 5189 (returnRest sv ov)) = some (stS input 5190 (returnRest sv ov)) := by
    exact blockOfS _ (pcFactS input 4162 5189 _ (by norm_num) shortHelperPC4181)
      (stepS_jumpdest input 5189 (returnRest sv ov) (by simp [returnRest]) (by norm_num))
  have hc : run [opAt 4163 .CALLDATASIZE, pushAt 4164 1 63, opAt 4165 .EQ, pushAt 4166 2 5162, opAt 4167 .JUMPI] (stS input 5190 (returnRest sv ov)) = some (stS input 5162 (returnRest sv ov)) := by
    have hdest : Decode.isValidJumpDest submissionBytecode 5162 = true :=
    ...
    have h0 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4163 .CALLDATASIZE]
            (stS input 5190 (returnRest sv ov)) =
          some (stS input 5191 (UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4163 5190 _ (by norm_num) shortHelperPC4182)
        (stepS_calldatasize input 5190 (returnRest sv ov)
          (by simp [returnRest]) (by norm_num))
    have h1 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [pushAt 4164 1 63]
            (stS input 5191 (UInt256.ofNat input.size :: returnRest sv ov)) =
          some (stS input 5193 (63 :: UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4164 5191 _ (by norm_num) shortHelperPC4183)
        (stepS_push input 5191 1 63
          (UInt256.ofNat input.size :: returnRest sv ov)
          (by simp [returnRest]) (by decide) (by decide) (by norm_num))
    have h2 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4165 .EQ]
            (stS input 5193 (63 :: UInt256.ofNat input.size :: returnRest sv ov)) =
          some (stS input 5194
            (UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4165 5193 _ (by norm_num) shortHelperPC4184)
        (stepS_eq input 5193 63 (UInt256.ofNat input.size) (returnRest sv ov)
          (by simp [returnRest]) (by norm_num))
    have h3 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [pushAt 4166 2 5162]
          ...
          some (stS input 5197
            (5162 :: UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4166 5194 _ (by norm_num) shortHelperPC4185)
        (stepS_push input 5194 2 5162
          (UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)
          (by simp [returnRest]) (by decide) (by decide) (by norm_num))
    have h4 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4167 .JUMPI]
            (stS input 5197
              (5162 :: UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) =
          some (stS input 5162 (returnRest sv ov)) := by
      exact blockOfS _
        (pcFactS input 4167 5197 _ (by norm_num) shortHelperPC4186)
        (stepS_jumpi_taken input 5197 5162 5162
          (UInt256.eq 63 (UInt256.ofNat input.size)) (returnRest sv ov)
          (by simp [returnRest]) (by norm_num) rfl htrue hdest)
    have h01 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4163 .CALLDATASIZE, pushAt 4164 1 63]
            (stS input 5190 (returnRest sv ov)) =
          some (stS input 5193 (63 :: UInt256.ofNat input.size :: returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4163 .CALLDATASIZE] [pushAt 4164 1 63]
    ...
    have h012 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4163 .CALLDATASIZE, pushAt 4164 1 63, opAt 4165 .EQ]
            (stS input 5190 (returnRest sv ov)) =
          some (stS input 5194
            (UInt256.eq 63 (UInt256.ofNat input.size) :: returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
    ...
    have h0123 :
      ...
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4163 .CALLDATASIZE, pushAt 4164 1 63, opAt 4165 .EQ]
        [pushAt 4166 2 5162] _ _ _ h012 (by rfl) h3
    have h01234 :
        Challenge.EvmProof.Stepper.runLocatedBlock
            [opAt 4163 .CALLDATASIZE, pushAt 4164 1 63, opAt 4165 .EQ,
              pushAt 4166 2 5162, opAt 4167 .JUMPI]
            (stS input 5190 (returnRest sv ov)) =
          some (stS input 5162 (returnRest sv ov)) := by
      exact Challenge.EvmProof.Stepper.runLocatedBlock_append
        [opAt 4163 .CALLDATASIZE, pushAt 4164 1 63, opAt 4165 .EQ,
          pushAt 4166 2 5162] [opAt 4167 .JUMPI] _ _ _ h0123 (by rfl) h4
    exact h01234
  have hbc := Challenge.EvmProof.Stepper.runLocatedBlock_append
    [opAt 4162 .JUMPDEST] [opAt 4163 .CALLDATASIZE, pushAt 4164 1 63, opAt 4165 .EQ, pushAt 4166 2 5162, opAt 4167 .JUMPI] _ _ _ hb (by rfl) hc
  have habc := Challenge.EvmProof.Stepper.runLocatedBlock_append
    [opAt 160 .CALLDATASIZE, pushAt 161 1 129, opAt 162 .GT, pushAt 163 2 5189, opAt 164 .JUMPI] ([opAt 4162 .JUMPDEST] ++ [opAt 4163 .CALLDATASIZE, pushAt 4164 1 63, opAt 4165 .EQ, pushAt 4166 2 5162, opAt 4167 .JUMPI]) _ _ _ ha (by rfl) hbc
  exact habc
