...
The first calldata word differs from the 1000-a word. The taken branch
drops this word and falls through into the patterned guard.
-/
...
def gasSteps_checkEarly (input : ByteArray)
  ...
  rw [show sizeMatched input = stG input 0x9 [] from rfl,
    show PatternedScan.patternedEntry input = stG input 0x71 [] from rfl]
  refine ?_
