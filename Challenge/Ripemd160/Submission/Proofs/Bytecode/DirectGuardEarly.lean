...
def gasSteps_checkEarly (input : ByteArray)
    ...
    GasSteps (sizeMatched input) (stG input 4831 []) := by
  ...
  have step5 := soundG (opAt 27 .JUMPI)
    (blockOf _ (pcFactG input 27 39 [4831, UInt256.xor KnownInputData.fullWord (referenceWord input)]
      (by norm_num) pc_e27)
      (stepG_jumpi_taken input 39 4831 (UInt256.xor KnownInputData.fullWord (referenceWord input))
