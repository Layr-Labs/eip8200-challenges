...
def eval (memory : ByteArray) (q : WordLane) : WordLane :=
  StaggerAlgorithm.step 7 (MachineState.readWord memory 340) q

theorem output_eq (memory : ByteArray) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired7.outputStack memory (input memory q right (StaggerAlgorithm.physicalKey 6)) rho =
      stack memory [ .d, .pair, .upper, .e, .b, .k, .a, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500, .cache 230 ] (eval memory q) right (StaggerAlgorithm.physicalKey 7) rho := by
  have hkb : StaggerAlgorithm.physicalKey 6 = UInt256.ofNat 1635471027088748134935197149822976 := by decide
  have hka : StaggerAlgorithm.physicalKey 7 = UInt256.ofNat 1635471027088748134935197149822976 := by decide
...
def gasSteps (s : State) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1442, stack := stack s.memory [ .d, .pair, .upper, .a, .c, .k, .e, .b, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500, .cache 230 ] q right (StaggerAlgorithm.physicalKey 6) rho}
      {s with pc := UInt256.ofNat 1485, stack := stack s.memory [ .d, .pair, .upper, .e, .b, .k, .a, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500, .cache 230 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 7) rho} := by
  have g := StaggerRawPaired7.gasSteps s (input s.memory q right (StaggerAlgorithm.physicalKey 6))
