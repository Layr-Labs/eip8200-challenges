...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3284).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3284 actual_slice
    (by change 3284 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 4004 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3284) = UInt256.ofNat 4004
