...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3281).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3281 actual_slice
    (by change 3281 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 4011 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3281) = UInt256.ofNat 4011
