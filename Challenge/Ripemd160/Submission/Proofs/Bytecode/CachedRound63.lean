...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3270).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3270 actual_slice
    (by change 3270 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 4019 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3270) = UInt256.ofNat 4019
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 3933, stack := inputStack s.memory x rho} =
