...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.CachedRound63
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3157).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 3157 actual_slice
    (by change 3157 + actualTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualTemplate) (by decide))
    (by decide)
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3800 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3157) = UInt256.ofNat 3800
...
def originalTemplate : List Instr :=
  [ .op (.Swap ⟨4, by decide⟩),
    ...
    .op (.Dup ⟨7, by decide⟩),
    .op .AND ]
theorem original_eq : originalTemplate = PairedAllInlineCoreTrace.inline63Block.code := by rfl
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v7 (UInt256.shiftRight (UInt256.mul x.v6 x.v2) (UInt256.ofNat 22))),
    x.v1,
    (UInt256.land x.v7 (UInt256.add x.v1 (UInt256.shiftRight (UInt256.mul x.v6 (UInt256.add (UInt256.mul (UInt256.ofNat 15) (UInt256.land x.v9 (UInt256.land x.v7 (UInt256.add x.v4 (UInt256.add (UInt256.lor (MachineState.readWord memory 256) (MachineState.readWord memory 464)) (UInt256.add (UInt256.xor x.v2 (UInt256.land (UInt256.xor (UInt256.lor x.v2 x.v8) x.v3) (UInt256.xor (UInt256.land x.v8 x.v2) x.v0))) x.v5)))))) (UInt256.land x.v7 (UInt256.add x.v4 (UInt256.add (UInt256.lor (MachineState.readWord memory 256) (MachineState.readWord memory 464)) (UInt256.add (UInt256.xor x.v2 (UInt256.land (UInt256.xor (UInt256.lor x.v2 x.v8) x.v3) (UInt256.xor (UInt256.land x.v8 x.v2) x.v0))) x.v5)))))) (UInt256.ofNat 24)))),
    x.v3,
    x.v4,
    x.v0,
    x.v6,
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 3875, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 3928, stack := outputStack s.memory x rho} := by
...
theorem run_actual (s : State) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 996) (hrun : s.halt = .Running)
    (hactive : 23 ≤ s.activeWords.toNat) :
    runInstrSeq actualTemplate {s with pc := UInt256.ofNat 3800, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 3850, stack := outputStack s.memory x rho} := by
  have hcap (n : Nat) (hn : n ≤ 26) : rho.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (haddress : address ≤ 704) :
      UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat address 32) = s.activeWords :=
    active_schedule_preserved s.activeWords address hactive haddress
...
def gasSteps (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3800, stack := coreStack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] f (cache s.memory ++ rho)}
      {s with pc := UInt256.ofNat 3850, stack := coreStack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] (PairedAllInlineCoreTrace.inline63Block.eval s.memory f) (cache s.memory ++ rho)} := by
  ...
  have hold := PairedAllInlineCoreTrace.inline63Block.run s f (cache s.memory ++ rho) hs hrun hactive
  have hraw := run_original s x rho hstack hrun hactive
  rw [original_eq, hin] at hraw
  have he : outputStack s.memory x rho = coreStack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] (PairedAllInlineCoreTrace.inline63Block.eval s.memory f) (cache s.memory ++ rho) :=
    congrArg State.stack (Option.some.inj (hraw.symm.trans hold))
  have g := gasSteps_terminal_of_raw actualSite
    {s with pc := UInt256.ofNat 3800, stack := inputStack s.memory x rho} _
    hcode hfork hrun hnp actual_pc.symm actual_advances (run_actual s x rho hstack hrun hactive)
  rw [hin, he] at g
  exact g
#print axioms gasSteps
end Challenge.Ripemd160.Submission.Proofs.Bytecode.CachedRound63
