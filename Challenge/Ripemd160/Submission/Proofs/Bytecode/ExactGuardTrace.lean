...
private theorem loopDest_valid :
  ...
  have h := Artifact.submissionArtifact.isValidJumpDest_index 2170 (by rfl)
  have hp : Artifact.submissionArtifact.instructionPC 2170 = 5309 := by
...
theorem run_exit (input : ByteArray) :
    Stepper.runLocatedBlock exitPath (loopExitState input 992 (acc input 31)) = some (accState input 5333 (acc input 31)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
theorem run_tail (input : ByteArray) :
    Stepper.runLocatedBlock tailPath (accState input 5333 (acc input 31)) = some (accState input 5372 (tailAcc input)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
theorem run_cleanup (input : ByteArray) :
    Stepper.runLocatedBlock cleanupPath (accState input 5372 (tailAcc input)) = some (diffState input 5374 (guardDiff input)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
def gasSteps_scan (input : ByteArray) :
    GasSteps (entryState input) (diffState input 5374 (guardDiff input)) :=
