...
theorem run_tail (input : ByteArray) :
    Stepper.runLocatedBlock tailPath (accState input 5360 (acc input 31)) = some (accState input 5370 (tailAcc input)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
theorem run_cleanup (input : ByteArray) :
    Stepper.runLocatedBlock cleanupPath (accState input 5370 (tailAcc input)) = some (diffState input 5372 (guardDiff input)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
def gasSteps_scan (input : ByteArray) :
    GasSteps (entryState input) (diffState input 5372 (guardDiff input)) :=
