...
private theorem loopDest_valid :
    Decode.isValidJumpDest submissionBytecode 5309 = true := by
  have h := Artifact.submissionArtifact.isValidJumpDest_index 2937 (by rfl)
  have hp : Artifact.submissionArtifact.instructionPC 2937 = 5309 := by
    rw [StackPC.instructionPC_eq_byteLength]
    decide
  rw [hp] at h
  exact h

private theorem synthesizedFullWord :
    UInt256.ofNat 97 * (UInt256.lnot 0 / UInt256.ofNat 255) = fullWord := by decide

private theorem lor_zero (v : UInt256) : UInt256.lor v 0 = v := by
  apply Challenge.EvmProof.Word.word_ext
  rw [Challenge.EvmProof.Word.word_toNat_lor]
  change v.toNat ||| 0 = v.toNat
  exact Nat.or_zero _
...
theorem run_prelude (input : ByteArray) :
  ...
  simp (config := { maxSteps := 1000000 }) [preludePath, entryState, atPC, loopState, acc, acc0, lor_zero, synthesizedFullWord, fullWord,
...
theorem run_check0 (input : ByteArray) :
  ...
  simp (config := { maxSteps := 1000000 }) [loopPath, loopState, loopExitState,
    fullWord, loopDest_valid, UInt256.isTrue, UInt256.isZero, UInt256.gt,
    Stepper.runLocatedBlock, Stepper.runLocated, Stepper.runInstr,
    initialState, Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod, Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat, hzeroWord, hzeroNat]
...
theorem run_exit (input : ByteArray) :
    Stepper.runLocatedBlock exitPath (loopExitState input 992 (acc input 31)) = some (accState input 5332 (acc input 31)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
private theorem tailWord_shift :
    UInt256.shiftLeft fullWord (UInt256.ofNat 192) = tailWord := by decide

theorem run_tail (input : ByteArray) :
    Stepper.runLocatedBlock tailPath (accState input 5332 (acc input 31)) = some (accState input 5342 (tailAcc input)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
  simp (config := { maxSteps := 1000000 }) [tailPath, accState, tailAcc, tailWord_shift, fullWord, tailWord,
    Stepper.runLocatedBlock, Stepper.runLocated, Stepper.runInstr,
    initialState, Challenge.EvmProof.Word.literal_eq_ofNat,
    Challenge.EvmProof.Word.succ_ofNat_mod, Challenge.EvmProof.Word.ofNat_add_mod,
    Challenge.EvmProof.Word.word_toNat_ofNat, hzeroWord, hzeroNat]
...
theorem run_cleanup (input : ByteArray) :
    Stepper.runLocatedBlock cleanupPath (accState input 5342 (tailAcc input)) = some (diffState input 5344 (guardDiff input)) := by
  have hzeroWord : ({ val := 0 } : UInt256) = UInt256.ofNat 0 := by decide
  have hzeroNat : ({ val := 0 } : UInt256).toNat = 0 := rfl
...
def gasSteps_scan (input : ByteArray) :
    GasSteps (entryState input) (diffState input 5344 (guardDiff input)) :=
