...
def loopState (s : State) (n k : Nat) (rho : List UInt256) : State :=
  atState s (if isTail n k then 4920 else 4893) (frame (current s.executionEnv.calldata n k) rho)
...
def route_normal (s : State) (rho : List UInt256) (moves : Moves s rho)
    ...
    GasSteps (atState s 4913 (frame (current s.executionEnv.calldata n k) rho)) (loopState s n k rho) := by
...
def route_transition (s : State) (rho : List UInt256) (moves : Moves s rho)
    (n k : Nat) (hn : Allowed n) (hk : k≤last n) (hk0 : 0 < k) :
    GasSteps (atState s 4670 (frame (current s.executionEnv.calldata n k) rho)) (loopState s n k rho) := by
  have g := moves.transitionGuard (current s.executionEnv.calldata n k)
  have h := full_iff s.executionEnv.calldata n k hn hk hk0
  by_cases ht : isTail n k
  · have hc := h.not.mpr (not_not_intro ht)
    have g0 : GasSteps (atState s 4670 (frame (current s.executionEnv.calldata n k) rho))
        (atState s 4677 (frame (current s.executionEnv.calldata n k) rho)) := by
...
def one (s : State) (rho : List UInt256) (moves : Moves s rho)
  ...
  · have g0 := moves.tail (current s.executionEnv.calldata n k)
    ...
    have g01 : GasSteps (atState s 4920 (frame (current s.executionEnv.calldata n k) rho))
        (atState s 4633 (frame (tailResult s (current s.executionEnv.calldata n k)) rho)) := by
    ...
    have g012 : GasSteps (atState s 4920 (frame (current s.executionEnv.calldata n k) rho))
        (atState s 4670 (frame (current s.executionEnv.calldata n (k+1)) rho)) := by
  ...
  · have g0 := moves.normal (current s.executionEnv.calldata n k)
    have g1 : GasSteps (atState s 4893 (frame (current s.executionEnv.calldata n k) rho))
        (atState s 4913 (frame (current s.executionEnv.calldata n (k+1)) rho)) := by
...
def start (s : State) (rho : List UInt256) (moves : Moves s rho)
    ...
    GasSteps (atState s 4796 rho) (loopState s n 0 rho) := by
  have g0 : GasSteps (atState s 4796 rho) (atState s 4885 (frame (current s.executionEnv.calldata n 0) rho)) := by
...
def accumulate (s : State) (rho : List UInt256) (moves : Moves s rho)
    (n : Nat) (hn : Allowed n) (hsize : s.executionEnv.calldata.size=n) :
    GasSteps (atState s 4796 rho) (atState s 4940 (frame (endFrame s n) rho)) := by
  ...
  have g1 : GasSteps (loopState s n (last n) rho) (atState s 4933 (frame (endFrame s n) rho)) := by
  ...
  have g2 : GasSteps (atState s 4933 (frame (endFrame s n) rho)) (atState s 4940 (frame (endFrame s n) rho)) := by
...
def gasSteps_hit (s : State) (rho : List UInt256) (moves : Moves s rho)
    ...
    GasSteps (atState s 4796 rho) (atState s 4943 (finishRest (endFrame s n) rho)) := by
  have hc : (endFrame s n).acc.toNat=0 := by rw [end_acc s n hn, hzero]; rfl
  have g := moves.result (endFrame s n)
  have gr : GasSteps (atState s 4940 (frame (endFrame s n) rho)) (atState s 4943 (finishRest (endFrame s n) rho)) := by
...
def gasSteps_miss (s : State) (rho : List UInt256) (moves : Moves s rho)
    (n : Nat) (hn : Allowed n) (hsize : s.executionEnv.calldata.size=n)
    (hzero : J2Accumulator.resultAcc s.executionEnv.calldata n≠0) :
    GasSteps (atState s 4796 rho) (atState s 246 (finishRest (endFrame s n) rho)) := by
  ...
  have g := moves.result (endFrame s n)
  have gr : GasSteps (atState s 4940 (frame (endFrame s n) rho)) (atState s 246 (finishRest (endFrame s n) rho)) := by
    simpa only [if_neg hc] using g
  exact (accumulate s rho moves n hn hsize).trans gr
