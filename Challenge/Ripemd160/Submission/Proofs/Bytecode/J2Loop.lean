...
def loopState (s : State) (n k : Nat) (rho : List UInt256) : State :=
  atState s (if isTail n k then 349 else 321) (frame (current s.executionEnv.calldata n k) rho)
...
def one (s : State) (rho : List UInt256) (moves : Moves s rho)
  ...
  · have g0 := moves.tail (current s.executionEnv.calldata n k)
    ...
    have g01 : GasSteps (atState s 349 (frame (current s.executionEnv.calldata n k) rho))
        (atState s 373 (frame (tailResult s (current s.executionEnv.calldata n k)) rho)) := by
...
def accumulate (s : State) (rho : List UInt256) (moves : Moves s rho)
  ...
  have g1 : GasSteps (loopState s n (last n) rho) (atState s 366 (frame (endFrame s n) rho)) := by
  ...
  have g2 : GasSteps (atState s 366 (frame (endFrame s n) rho)) (atState s 540 (frame (endFrame s n) rho)) := by
