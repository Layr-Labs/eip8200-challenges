...
private theorem size_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  change (UInt256.ofNat 4811).toNat = Artifact.submissionArtifact.instructionPC 3916
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
    ...
    GasSteps (atState s 4789 rho) (output s rho) := by
  have hp : StackRoundTrace.runInstrSeq RecognitionSites.selector.template
      (atState s 4789 rho) = some (beforeCopy s rho) := by
    have h := run_prefix s (UInt256.ofNat 4789) (UInt256.ofNat 4819) rho (by omega) e.run
  ...
  have hf : StackRoundTrace.runInstrSeq RecognitionSites.returned.template
    ...
    run_finish (afterCopy s rho) (UInt256.ofNat 4812) rho (by omega) e.run
  have hpc : (UInt256.ofNat 4810).succ.succ = UInt256.ofNat 4812 := by decide
