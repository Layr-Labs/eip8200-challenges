...
def source (s : State) : UInt256 := selected (UInt256.ofNat 4916) s.executionEnv.calldata.size
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
  ...
  have hp : StackRoundTrace.runInstrSeq RecognitionSites.selector.template
    ...
    have h := run_prefix s (UInt256.ofNat 325) (UInt256.ofNat 4916) rho (by omega) e.run
