...
def source (s : State) : UInt256 := selected (UInt256.ofNat 4880) s.executionEnv.calldata.size

def beforeCopy (s : State) (rho : List UInt256) : State :=
  atState s 4875 (12 :: source s :: 20 :: rho)

def afterCopy (s : State) (rho : List UInt256) : State :=
  sized (beforeCopy s rho) (source s) rho

def output (s : State) (rho : List UInt256) : State :=
  RecognitionSelectorRaw.returned (afterCopy s rho) (UInt256.ofNat 4877) rho

private theorem copy_decoded (s : State) (e : Env s) (rho : List UInt256) :
    (beforeCopy s rho).decodedOp = some .CODECOPY := by
  have hd := Artifact.submissionArtifact.decodeAt_op_index 3903 .CODECOPY
    (by rfl) (by decide) trivial
  apply Artifact.submissionArtifact.state_decodedOp_of (beforeCopy s rho) 3903
    e.code ?_ .CODECOPY none hd (by change Operation.CODECOPY.availableInFork s.fork = true; rw [e.fork]; rfl)
  change (UInt256.ofNat 4875).toNat = Artifact.submissionArtifact.instructionPC 3903
...
private theorem size_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  apply Artifact.submissionArtifact.state_decodedOp_of
    (copied (beforeCopy s rho) (source s) rho) 3904 e.code ?_ .MSIZE none hd (by change Operation.MSIZE.availableInFork s.fork = true; rw [e.fork]; rfl)
  change (UInt256.ofNat 4876).toNat = Artifact.submissionArtifact.instructionPC 3904
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
    ...
    GasSteps (atState s 4854 rho) (output s rho) := by
  have hp : StackRoundTrace.runInstrSeq RecognitionSites.selector.template
      (atState s 4854 rho) = some (beforeCopy s rho) := by
    have h := run_prefix s (UInt256.ofNat 4854) (UInt256.ofNat 4880) rho (by omega) e.run
  ...
  have hf : StackRoundTrace.runInstrSeq RecognitionSites.returned.template
      (atState (afterCopy s rho) 4877 (32 :: rho)) = some (output s rho) :=
    run_finish (afterCopy s rho) (UInt256.ofNat 4877) rho (by omega) e.run
  have hpc : (UInt256.ofNat 4875).succ.succ = UInt256.ofNat 4877 := by decide
...
theorem output_spec (s : State) (e : Env s) (rho : List UInt256)
  ...
  apply returned_spec (beforeCopy s rho) (source s) (UInt256.ofNat 4877) rho
