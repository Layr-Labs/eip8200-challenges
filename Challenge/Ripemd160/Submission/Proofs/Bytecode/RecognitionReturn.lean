...
def source (s : State) : UInt256 := selected (UInt256.ofNat 4981) s.executionEnv.calldata.size

def beforeCopy (s : State) (rho : List UInt256) : State :=
  atState s 337 (12 :: source s :: 20 :: rho)

def afterCopy (s : State) (rho : List UInt256) : State :=
  sized (beforeCopy s rho) (source s) rho

def output (s : State) (rho : List UInt256) : State :=
  RecognitionSelectorRaw.returned (afterCopy s rho) (UInt256.ofNat 367) rho
...
private theorem size_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  change (UInt256.ofNat 366).toNat = Artifact.submissionArtifact.instructionPC 224
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
  ...
  have hp : StackRoundTrace.runInstrSeq RecognitionSites.selector.template
    ...
    have h := run_prefix s (UInt256.ofNat 344) (UInt256.ofNat 4981) rho (by omega) e.run
  ...
  have hf : StackRoundTrace.runInstrSeq RecognitionSites.returned.template
    ...
    run_finish (afterCopy s rho) (UInt256.ofNat 367) rho (by omega) e.run
  have hpc : (UInt256.ofNat 365).succ.succ = UInt256.ofNat 367 := by decide
...
theorem output_spec (s : State) (e : Env s) (rho : List UInt256)
  ...
  apply returned_spec (beforeCopy s rho) (source s) (UInt256.ofNat 367) rho
