...
def output (s : State) (rho : List UInt256) : State :=
  RecognitionSelectorRaw.returned (afterCopy s rho) (UInt256.ofNat 340) rho
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
    ...
    GasSteps (atState s 315 rho) (output s rho) := by
  have hp : StackRoundTrace.runInstrSeq RecognitionSites.selector.template
      (atState s 315 rho) = some (beforeCopy s rho) := by
    have h := run_prefix s (UInt256.ofNat 315) (UInt256.ofNat 4953) rho (by omega) e.run
  ...
  have hf : StackRoundTrace.runInstrSeq RecognitionSites.returned.template
      (atState (afterCopy s rho) 340 (32 :: rho)) = some (output s rho) :=
    run_finish (afterCopy s rho) (UInt256.ofNat 340) rho (by omega) e.run
  have hpc : (UInt256.ofNat 338).succ.succ = UInt256.ofNat 340 := by decide
...
theorem output_spec (s : State) (e : Env s) (rho : List UInt256)
  ...
  apply returned_spec (beforeCopy s rho) (source s) (UInt256.ofNat 339) rho
