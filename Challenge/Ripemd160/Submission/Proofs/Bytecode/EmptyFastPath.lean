...
def emptyFinal : State :=
  { Execution.atPC emptyInput 0x14b2 with
    stack := []
    halt := .Returned
    hReturn :=
...
def gasSteps_dispatch_empty :
    Challenge.EvmProof.GasSteps (Execution.atPC emptyInput 0x1492)
      (Execution.atPC emptyInput 0x1498) := by
  ...
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock Execution.path_dispatch
      (Execution.atPC emptyInput 0x1492) =
      some (Execution.atPC emptyInput 0x1498) := by
...
def s4_push1 : State :=
  { mstoreState with
    pc := UInt256.ofNat 0x14b1
    stack := [UInt256.ofNat 32] }

def s5_push0 : State :=
  { s4_push1 with
    pc := UInt256.ofNat 0x14b2
    stack := [UInt256.ofNat 0, UInt256.ofNat 32] }

private def path_2165 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  Execution.path_empty_tail.take 1

private def path_2166 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  (Execution.path_empty_tail.drop 1).take 1

private def path_2167 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  (Execution.path_empty_tail.drop 2).take 1

private def path_2168 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  (Execution.path_empty_tail.drop 3).take 1

private def path_2169 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  (Execution.path_empty_tail.drop 4).take 1

private def path_2170 :
    List (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  Execution.path_empty_tail.drop 5

private theorem path_tail_single_split :
    Execution.path_empty_tail =
      path_2165 ++ path_2166 ++ path_2167 ++ path_2168 ++ path_2169 ++ path_2170 := by
  rfl

def gasSteps_2165 :
    Challenge.EvmProof.GasSteps (Execution.atPC emptyInput 0x1498) s1_push20 := by
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_2165
      (Execution.atPC emptyInput 0x1498) = some s1_push20 := by
    simp [path_2165, Execution.path_empty_tail,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
      Execution.atPC, initialState, emptyInput, s1_push20]
  apply Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka path_2165
  · rfl
  · rfl
  · exact hrun
  · rfl
...
def gasSteps_2166 :
  ...
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_2166
      s1_push20 = some s2_push0 := by
    simp [path_2166, Execution.path_empty_tail, s1_push20, s2_push0,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
      Execution.atPC, initialState, emptyInput]
  apply Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka path_2166
  · rfl
  · rfl
  · exact hrun
  · rfl
...
def gasSteps_2167 :
  ...
  have hmod : Execution.emptyDigestNat % 115792089237316195423570985008687907853269984665640564039457584007913129639936 =
    ...
    hmod_empty
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_2167
      s2_push0 = some mstoreState := by
    simp [path_2167, Execution.path_empty_tail, s2_push0, mstoreState,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
      Execution.atPC, initialState, emptyInput,
      EmptySpec.emptyOutput,
      hmod, digest_padded_eq]
    decide
  apply Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka path_2167
  · rfl
  · rfl
  · exact hrun
  · rfl
...
def gasSteps_2168 :
  ...
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_2168
      mstoreState = some s4_push1 := by
    simp [path_2168, Execution.path_empty_tail, mstoreState, s4_push1,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
      Execution.atPC, initialState, emptyInput,
      EmptySpec.emptyOutput]
  apply Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka path_2168
  · rfl
  · rfl
  · exact hrun
  · rfl
...
def gasSteps_2169 :
  ...
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_2169
      s4_push1 = some s5_push0 := by
    simp [path_2169, Execution.path_empty_tail, s4_push1, s5_push0,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr]
  apply Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka path_2169
  · rfl
  · rfl
  · exact hrun
  · rfl
...
def gasSteps_2170 :
  ...
  have hrun : Challenge.EvmProof.Stepper.runLocatedBlock path_2170
      s5_push0 = some emptyFinal := by
    simp [path_2170, Execution.path_empty_tail, s5_push0, emptyFinal,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    ...
    decide
  apply Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka path_2170
  · rfl
  · rfl
  · exact hrun
  · rfl
...
def gasSteps_empty_tail :
    Challenge.EvmProof.GasSteps (Execution.atPC emptyInput 0x1498) emptyFinal :=
  (((((gasSteps_2165.trans gasSteps_2166).trans gasSteps_2167).trans
    gasSteps_2168).trans gasSteps_2169).trans gasSteps_2170)

def gasSteps_empty_dispatch :
    Challenge.EvmProof.GasSteps (Execution.atPC emptyInput 0x1492) emptyFinal :=
