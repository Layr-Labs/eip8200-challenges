...
private theorem rightWrapper_fits (k : Fin 20) :
  ...
  change 927 + 12 * k.val + 12 ≤ Artifact.submissionInstructions.length
...
private theorem rightCall_fits (k : Fin 20) :
    rightWrapperIndex k.val + (rightCallTemplate k).length ≤
      Artifact.instructions.length := by
  change 927 + 12 * k.val + 10 ≤ Artifact.submissionInstructions.length
  rw [Artifact.referenceInstructions_count]
  omega
...
@[simp] theorem rightPC_zero : rightPC 0 = UInt256.ofNat 0x796 := by
...
@[simp] theorem rightPC_end : rightPC 20 = UInt256.ofNat 0xb3e := by
...
@[simp] theorem rightStartPC_eq : rightStartPC = UInt256.ofNat 0x796 := by
  exact rightPC_zero

@[simp] theorem rightEndPC_eq : rightEndPC = UInt256.ofNat 0xb3e := by
  exact rightPC_end
