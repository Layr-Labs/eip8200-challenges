...
noncomputable opaque gasSteps_normal (input : ByteArray) (hfit : CalldataFits input)
    ...
    GasSteps {paddedState input i with pc:=UInt256.ofNat 507,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (Padding.paddedWord input) maskRho}
      {tableState input i with pc:=UInt256.ofNat 861,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (Padding.paddedWord input) maskRho} := by
