...
theorem site_pc : site.startPC = UInt256.ofNat 4737 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3446) = UInt256.ofNat 4737
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4737, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4742, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4737) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4737) template = UInt256.ofNat 4742 := by decide
  rw [hend] at hraw
