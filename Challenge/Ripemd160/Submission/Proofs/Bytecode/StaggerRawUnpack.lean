...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3627 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4604 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3627) = UInt256.ofNat 4604
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4604, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4620, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4604) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4604) template = UInt256.ofNat 4620 := by decide
  rw [hend] at hraw
