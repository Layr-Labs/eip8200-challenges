...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v1 (UInt256.shiftRight (UInt256.mul x.v8 x.v7) (UInt256.ofNat 28))),
    ...
    x.v0,
    x.v6,
    (UInt256.land x.v1 (UInt256.add x.v6 (UInt256.shiftRight (UInt256.mul x.v8 (UInt256.land x.v1 (UInt256.add x.v3 (UInt256.add (MachineState.readWord memory 252) (UInt256.add (UInt256.xor x.v0 (UInt256.lor (UInt256.lnot x.v7) x.v4)) x.v5))))) (UInt256.ofNat 33)))),
    x.v8,
    x.v9,
    x.v10,
    x.v11,
...
theorem site_pc : site.startPC = UInt256.ofNat 3084 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2360) = UInt256.ofNat 3084
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3084, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3117, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3084) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3084) template = UInt256.ofNat 3117 := by decide
  rw [hend] at hraw
