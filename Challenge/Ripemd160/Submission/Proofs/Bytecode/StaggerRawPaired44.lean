...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2399 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3155 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2399) = UInt256.ofNat 3155
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3155, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3187, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3155) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3155) template = UInt256.ofNat 3187 := by decide
  rw [hend] at hraw
