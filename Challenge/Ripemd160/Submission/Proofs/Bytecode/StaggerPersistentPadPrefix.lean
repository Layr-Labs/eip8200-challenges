...
theorem prefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3578).take prefixTemplate.length = prefixTemplate := by rfl
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 3578 prefix_slice
    (by change 3578 + prefixTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 4701 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3578) = UInt256.ofNat 4701
