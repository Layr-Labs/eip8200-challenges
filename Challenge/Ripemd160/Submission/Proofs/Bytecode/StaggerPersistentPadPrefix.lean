...
def gasSteps_prefix (s : State) (stack : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 386, stack := stack}
      {s with pc := UInt256.ofNat 388, stack := stack} := by
  apply PadLift.gasSteps_of_raw prefixSite {s with pc := UInt256.ofNat 386, stack := stack} _ hcode hfork hrun hnp prefix_pc.symm prefix_advances
  have h1 : stack.length < 1024 := by omega
  have h : runInstrSeq prefixTemplate {s with pc := UInt256.ofNat 386, stack := stack} =
      some {s with pc := pcAfter (UInt256.ofNat 386) prefixTemplate, stack := stack} := by
