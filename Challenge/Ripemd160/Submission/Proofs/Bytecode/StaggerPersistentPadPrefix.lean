...
theorem prefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3594).take prefixTemplate.length = prefixTemplate := by rfl
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 3594 prefix_slice
    (by change 3594 + prefixTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 4701 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3594) = UInt256.ofNat 4701
