...
theorem prefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3556).take prefixTemplate.length = prefixTemplate := by rfl
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 3556 prefix_slice
    (by change 3556 + prefixTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 4701 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3556) = UInt256.ofNat 4701
