...
theorem prefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3568).take prefixTemplate.length = prefixTemplate := by rfl
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 3568 prefix_slice
    (by change 3568 + prefixTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := prefixTemplate) (by decide))
    (by decide)
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 4699 := by
...
def gasSteps_prefix (s : State) (stack : List UInt256)
  ...
  apply PadLift.gasSteps_of_raw prefixSite {s with pc := UInt256.ofNat 4699, stack := stack} _ hcode hfork hrun hnp prefix_pc.symm prefix_advances
  have h1 : stack.length < 1024 := by omega
  have h : runInstrSeq prefixTemplate {s with pc := UInt256.ofNat 4699, stack := stack} =
      some {s with pc := pcAfter (UInt256.ofNat 4699) prefixTemplate, stack := stack} := by
    simp [prefixTemplate, runInstrSeq, DataStepper.runInstr, if_pos h1, hrun]
    decide
  have hp : pcAfter (UInt256.ofNat 4699) prefixTemplate = UInt256.ofNat 4700 := by rfl
  rw [hp] at h
  exact h
