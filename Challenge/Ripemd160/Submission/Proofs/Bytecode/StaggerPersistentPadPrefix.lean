...
theorem prefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3570).take prefixTemplate.length = prefixTemplate := by rfl
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 3570 prefix_slice
    (by change 3570 + prefixTemplate.length ≤ Artifact.submissionInstructions.length
