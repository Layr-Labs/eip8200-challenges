...
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 3693 prefix_slice
    ...
    (by decide)
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 4745 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3693) = UInt256.ofNat 4745
...
def gasSteps_prefix (s : State) (stack : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4745, stack := stack}
      {s with pc := UInt256.ofNat 4746, stack := stack} := by
  apply PadLift.gasSteps_of_raw prefixSite {s with pc := UInt256.ofNat 4745, stack := stack} _ hcode hfork hrun hnp prefix_pc.symm prefix_advances
  have h1 : stack.length < 1024 := by omega
  have h : runInstrSeq prefixTemplate {s with pc := UInt256.ofNat 4745, stack := stack} =
      some {s with pc := pcAfter (UInt256.ofNat 4745) prefixTemplate, stack := stack} := by
    simp [prefixTemplate, runInstrSeq, DataStepper.runInstr, if_pos h1, hrun]
    decide
  have hp : pcAfter (UInt256.ofNat 4745) prefixTemplate = UInt256.ofNat 4746 := by rfl
  rw [hp] at h
  exact h
