...
theorem prefix_slice :
    (Artifact.submissionArtifact.instructions.drop 3674).take prefixTemplate.length = prefixTemplate := by rfl
def prefixSite : GenericRoundSite Artifact.submissionArtifact .Osaka prefixTemplate :=
  StackSiteBuilder.ofSlice prefixTemplate 3674 prefix_slice
    (by change 3674 + prefixTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem prefix_pc : prefixSite.startPC = UInt256.ofNat 4766 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3674) = UInt256.ofNat 4766
