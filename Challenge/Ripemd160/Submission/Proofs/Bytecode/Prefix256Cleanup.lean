...
@[simp] private theorem branchPC0 : Artifact.submissionArtifact.instructionPC 261 = 425 := rfl
@[simp] private theorem branchPC1 : Artifact.submissionArtifact.instructionPC 262 = 426 := rfl
@[simp] private theorem branchPC2 : Artifact.submissionArtifact.instructionPC 263 = 429 := rfl
@[simp] private theorem cleanupDupPC : Artifact.submissionArtifact.instructionPC 284 = 514 := rfl
@[simp] private theorem cleanupDest : Decode.isValidJumpDest submissionBytecode 513 = true :=
  Artifact.submissionArtifact.isValidJumpDest_index 283 (by rfl)
...
def branchPath : List Located :=
  [opAt 261 (.Dup ⟨2, by decide⟩), pushAt 262 2 513, opAt 263 .JUMPI]

def cleanupPath : List Located :=
  [opAt 283 .JUMPDEST, opAt 284 (.Dup ⟨2, by decide⟩),
   opAt 285 (.Swap ⟨2, by decide⟩), opAt 286 .POP,
   opAt 287 (.Swap ⟨1, by decide⟩), opAt 288 (.Swap ⟨6, by decide⟩),
   opAt 289 .POP, opAt 290 .POP, opAt 291 .POP, opAt 292 .POP,
   opAt 293 .POP, opAt 294 .POP, opAt 295 .POP,
   pushAt 296 1 3, opAt 297 .JUMPI]

theorem run_branch (input : ByteArray) (sv ov acc : UInt256) :
    run branchPath (stS input 425 [sv, ov, acc, P7, M, m7, P, m8]) =
      some (stS input (if UInt256.isTrue acc then 513 else 430)
...
theorem run_cleanup (input : ByteArray) (sv ov acc : UInt256)
    ...
    run cleanupPath (stS input 513 [sv, ov, acc, P7, M, m7, P, m8]) =
...
def gasSteps_hit (input : ByteArray) (sv ov : UInt256) :
    GasSteps (stS input 425 [sv, ov, 0, P7, M, m7, P, m8])
      (stS input 430 [sv, ov, 0, P7, M, m7, P, m8]) := by
