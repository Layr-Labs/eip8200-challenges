...
@[simp] private theorem selectorDest : Decode.isValidJumpDest submissionBytecode 4829 = true :=
...
@[simp] private theorem e4PC163 : Artifact.submissionArtifact.instructionPC 163 = 258 := rfl
...
def branchPath : List Located :=
  [opAt 159 (.Dup ⟨2, by decide⟩), opAt 160 .ISZERO, pushAt 161 4 4829, opAt 162 .JUMPI]

def cleanupPath : List Located :=
  [opAt 163 .JUMPDEST, opAt 164 .JUMPDEST, opAt 165 .POP, opAt 166 .POP, opAt 167 .POP,
...
theorem run_branch (input : ByteArray) (sv ov acc : UInt256) :
    run branchPath (stS input 250 [sv, ov, acc, P7, M, m7, P, m8]) =
      some (stS input (if UInt256.isTrue acc then 258 else 4829)
        [sv, ov, acc, P7, M, m7, P, m8]) := by
  by_cases hc : acc.toNat = 0 <;>
    simp (config := { maxSteps := 400000 })
      [branchPath, opAt, pushAt, stS, atPC, hc, UInt256.isTrue, UInt256.isZero,
...
def gasSteps_hit (input : ByteArray) (sv ov : UInt256) :
    GasSteps (stS input 250 [sv, ov, 0, P7, M, m7, P, m8])
      (stS input 4829 [sv, ov, 0, P7, M, m7, P, m8]) := by
