...
@[simp] private theorem branchPC0 : Artifact.submissionArtifact.instructionPC 157 = 256 := rfl
@[simp] private theorem branchPC1 : Artifact.submissionArtifact.instructionPC 158 = 257 := rfl
@[simp] private theorem branchPC2 : Artifact.submissionArtifact.instructionPC 159 = 260 := rfl
@[simp] private theorem cleanupDupPC : Artifact.submissionArtifact.instructionPC 185 = 353 := rfl
@[simp] private theorem cleanupDest : Decode.isValidJumpDest submissionBytecode 352 = true :=
  Artifact.submissionArtifact.isValidJumpDest_index 184 (by rfl)
@[simp] private theorem fallbackDest : Decode.isValidJumpDest submissionBytecode 369 = true :=
  Artifact.submissionArtifact.isValidJumpDest_index 199 (by rfl)

def branchPath : List Located :=
  [opAt 157 (.Dup ⟨2, by decide⟩), pushAt 158 2 352, opAt 159 .JUMPI]
...
theorem run_branch (input : ByteArray) (sv ov acc : UInt256) :
    run branchPath (stS input 256 [sv, ov, acc, P7, M, m7, P, m8]) =
      some (stS input (if UInt256.isTrue acc then 352 else 261)
...
theorem run_cleanup (input : ByteArray) (sv ov acc : UInt256)
    ...
    run cleanupPath (stS input 352 [sv, ov, acc, P7, M, m7, P, m8]) =
...
def gasSteps_miss (input : ByteArray) (sv ov acc : UInt256) (hne : acc ≠ 0) :
    GasSteps (stS input 256 [sv, ov, acc, P7, M, m7, P, m8]) (fallbackState input) := by
...
def gasSteps_hit (input : ByteArray) (sv ov : UInt256) :
    GasSteps (stS input 256 [sv, ov, 0, P7, M, m7, P, m8])
      (stS input 261 [sv, ov, 0, P7, M, m7, P, m8]) := by
