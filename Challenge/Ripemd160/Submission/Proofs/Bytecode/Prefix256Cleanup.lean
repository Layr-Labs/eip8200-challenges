...
@[simp] private theorem cleanupDupPC : Artifact.submissionArtifact.instructionPC 192 = 355 := rfl
@[simp] private theorem cleanupDest : Decode.isValidJumpDest submissionBytecode 354 = true :=
  Artifact.submissionArtifact.isValidJumpDest_index 191 (by rfl)
@[simp] private theorem fallbackDest : Decode.isValidJumpDest submissionBytecode 371 = true :=
  Artifact.submissionArtifact.isValidJumpDest_index 206 (by rfl)

def branchPath : List Located :=
  [opAt 164 (.Dup ⟨2, by decide⟩), pushAt 165 2 354, opAt 166 .JUMPI]

def cleanupPath : List Located :=
  [opAt 191 .JUMPDEST, opAt 192 (.Dup ⟨2, by decide⟩),
   opAt 193 (.Swap ⟨2, by decide⟩), opAt 194 .POP,
   opAt 195 (.Swap ⟨1, by decide⟩), opAt 196 (.Swap ⟨6, by decide⟩),
   opAt 197 .POP, opAt 198 .POP, opAt 199 .POP, opAt 200 .POP,
   opAt 201 .POP, opAt 202 .POP, opAt 203 .POP,
...
theorem run_branch (input : ByteArray) (sv ov acc : UInt256) :
    run branchPath (stS input 258 [sv, ov, acc, P7, M, m7, P, m8]) =
      some (stS input (if UInt256.isTrue acc then 354 else 263)
...
theorem run_cleanup (input : ByteArray) (sv ov acc : UInt256)
    ...
    run cleanupPath (stS input 354 [sv, ov, acc, P7, M, m7, P, m8]) =
