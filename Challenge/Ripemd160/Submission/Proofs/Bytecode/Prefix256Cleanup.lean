...
@[simp] private theorem selectorDest : Decode.isValidJumpDest submissionBytecode 4798 = true :=
  Artifact.submissionArtifact.isValidJumpDest_index 3938 (by rfl)

def branchPath : List Located :=
  [opAt 164 (.Dup ⟨2, by decide⟩), opAt 165 .ISZERO,
   pushAt 166 2 4798, opAt 167 .JUMPI]
...
theorem run_branch (input : ByteArray) (sv ov acc : UInt256) :
    run branchPath (stS input 258 [sv, ov, acc, P7, M, m7, P, m8]) =
      some (stS input (if UInt256.isTrue acc then 264 else 4798)
...
def gasSteps_hit (input : ByteArray) (sv ov : UInt256) :
    GasSteps (stS input 258 [sv, ov, 0, P7, M, m7, P, m8])
      (stS input 4798 [sv, ov, 0, P7, M, m7, P, m8]) := by
