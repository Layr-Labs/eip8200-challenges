...
def actualOutput (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.shiftRight (UInt256.mul x.v13 x.v1) (UInt256.ofNat 28)),
    (UInt256.land x.v14 (UInt256.add x.v3 (UInt256.shiftRight (UInt256.mul x.v13 (UInt256.land x.v14 (UInt256.add x.v0 (UInt256.add (MachineState.readWord memory 0) (UInt256.add (UInt256.xor x.v7 (UInt256.lor (UInt256.lnot x.v2) x.v1)) x.v6))))) (UInt256.ofNat 30)))),
    x.v2,
    x.v3,
    (UInt256.ofNat 28),
    x.v5,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3683).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3683 actual_slice
    (by change 3683 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4595 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3683) = UInt256.ofNat 4595
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4595, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4624, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4595) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4595) template = UInt256.ofNat 4624 := by decide
  rw [hend] at hraw
