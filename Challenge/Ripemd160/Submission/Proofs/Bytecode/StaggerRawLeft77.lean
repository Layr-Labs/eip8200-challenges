...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3640 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4557 := by
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4557, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4587, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4557) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4557) template = UInt256.ofNat 4587 := by decide
  rw [hend] at hraw
