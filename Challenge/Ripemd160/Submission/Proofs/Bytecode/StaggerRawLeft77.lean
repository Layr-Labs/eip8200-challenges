...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3476).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3476 actual_slice
    (by change 3476 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4488 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3476) = UInt256.ofNat 4488
