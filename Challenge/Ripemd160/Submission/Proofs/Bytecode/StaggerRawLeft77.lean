...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3678).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3678 actual_slice
    (by change 3678 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4602 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3678) = UInt256.ofNat 4602
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4602, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4631, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4602) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4602) template = UInt256.ofNat 4631 := by decide
  rw [hend] at hraw
