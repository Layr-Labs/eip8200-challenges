...
theorem site_pc : site.startPC = UInt256.ofNat 4484 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3382) = UInt256.ofNat 4484
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4484, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4514, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4484) x rho hstack haliasA haliasE haliasD haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4484) template = UInt256.ofNat 4514 := by decide
  rw [hend] at hraw
