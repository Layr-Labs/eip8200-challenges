...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.shiftRight (UInt256.mul x.v12 x.v1) (UInt256.ofNat 28)),
    (UInt256.land x.v13 (UInt256.add x.v3 (UInt256.shiftRight (UInt256.mul x.v12 (UInt256.land x.v13 (UInt256.add x.v0 (UInt256.add (MachineState.readWord memory 0) (UInt256.add (UInt256.xor (UInt256.lor (UInt256.lnot x.v2) x.v1) x.v6) x.v5))))) (UInt256.ofNat 30)))),
    x.v2,
    x.v3,
    x.v4,
    x.v0,
...
theorem site_pc : site.startPC = UInt256.ofNat 4579 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3657) = UInt256.ofNat 4579
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4579, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4609, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4579) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4579) template = UInt256.ofNat 4609 := by decide
  rw [hend] at hraw
