...
theorem site_pc : site.startPC = UInt256.ofNat 4742 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3451) = UInt256.ofNat 4742
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4742, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4772, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4742) x rho hstack haliasA haliasE haliasD haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4742) template = UInt256.ofNat 4772 := by decide
  rw [hend] at hraw
