...
theorem site_pc : site.startPC = UInt256.ofNat 4501 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3392) = UInt256.ofNat 4501
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4501, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4532, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4501) x rho hstack haliasA haliasE haliasD haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4501) template = UInt256.ofNat 4532 := by decide
  rw [hend] at hraw
