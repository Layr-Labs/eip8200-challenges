...
theorem site_pc : site.startPC = UInt256.ofNat 4744 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3474) = UInt256.ofNat 4744
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4744, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4774, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4744) x rho hstack haliasA haliasE haliasD haliasR hrun hactive
  have hend : pcAfter (UInt256.ofNat 4744) template = UInt256.ofNat 4774 := by decide
  rw [hend] at hraw
