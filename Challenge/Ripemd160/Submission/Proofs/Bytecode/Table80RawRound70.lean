import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
   StackSiteBuilder.ofSlice template 3565 actual_slice
     ...
     (by decide)
 theorem site_pc : site.startPC = UInt256.ofNat 4270 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3565) = UInt256.ofNat 4270
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 4270, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 4312, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 4270) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 4270) template = UInt256.ofNat 4312 := by decide
   rw [hend] at hraw
