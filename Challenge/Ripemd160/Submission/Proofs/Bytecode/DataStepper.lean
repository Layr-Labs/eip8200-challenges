...
namespace Challenge.EvmProof.DataStepper
...
theorem decodes_of_artifact (artifact : DataProgramArtifact) (s : State)
...
def runAt (artifact : DataProgramArtifact) (index : Nat) (s : State) : Option State := do
...
def runAt_sound {artifact : DataProgramArtifact} {index : Nat} {s t : State}
    (hcode : s.executionEnv.code = artifact.code)
...
theorem runAt_executionEnv {artifact : DataProgramArtifact} {index : Nat}
...
def runBlock (artifact : DataProgramArtifact) : List Nat → State → Option State
  | [], s => some s
  | index :: rest, s =>
...
def runBlock_sound (artifact : DataProgramArtifact) (indices : List Nat)
    {s t : State}
    (hcode : s.executionEnv.code = artifact.code)
...
structure Located (artifact : DataProgramArtifact) (fork : Fork) where
  index : Nat
  instruction : Instr
...
def AllWellFormed (artifact : DataProgramArtifact) (fork : Fork) : Prop :=
  artifact.instructions.all (fun instruction =>
    decide (WellFormed fork instruction)) = true

theorem AllWellFormed.valid {artifact : DataProgramArtifact} {fork : Fork}
...
def Located.ofIndex {artifact : DataProgramArtifact} {fork : Fork}
...
def runLocated {artifact : DataProgramArtifact} {fork : Fork}
...
def runLocated_sound {artifact : DataProgramArtifact}
...
@[simp] theorem runLocated_sound_cost {artifact : DataProgramArtifact}
    {fork : Fork} {located : Located artifact fork} {s t : State}
    (hcode : s.executionEnv.code = artifact.code)
    (hfork : s.fork = fork)
    (hresult : runLocated located s = some t)
...
theorem runLocated_executionEnv {artifact : DataProgramArtifact}
    {fork : Fork} {located : Located artifact fork} {s t : State}
    (hresult : runLocated located s = some t) :
    t.executionEnv = s.executionEnv := by
  unfold runLocated at hresult
...
def runLocatedBlock {artifact : DataProgramArtifact} {fork : Fork} :
...
def runLocatedBlockCost {artifact : DataProgramArtifact} {fork : Fork} :
...
def runLocatedBlock_sound (artifact : DataProgramArtifact)
...
@[simp] theorem runLocatedBlock_sound_cost (artifact : DataProgramArtifact)
    (fork : Fork) (path : List (Located artifact fork)) {s t : State}
    (hcode : s.executionEnv.code = artifact.code)
    (hfork : s.fork = fork)
    (hresult : runLocatedBlock path s = some t)
...
end Challenge.EvmProof.DataStepper
