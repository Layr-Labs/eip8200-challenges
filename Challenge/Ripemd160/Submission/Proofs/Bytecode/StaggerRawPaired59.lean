...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2995).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2995 actual_slice
    (by change 2995 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3849 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2995) = UInt256.ofNat 3849
