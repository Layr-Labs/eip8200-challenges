...
theorem site_pc : site.startPC = UInt256.ofNat 3731 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2894) = UInt256.ofNat 3731
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3731, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3791, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3731) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3731) template = UInt256.ofNat 3791 := by decide
  rw [hend] at hraw
