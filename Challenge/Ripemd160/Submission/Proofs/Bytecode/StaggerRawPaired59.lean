...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2998).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2998 actual_slice
    (by change 2998 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3850 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2998) = UInt256.ofNat 3850
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3850, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3898, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3850) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3850) template = UInt256.ofNat 3898 := by decide
  rw [hend] at hraw
