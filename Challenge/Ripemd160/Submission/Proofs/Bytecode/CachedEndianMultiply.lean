...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.CachedEndianMultiply
...
def maskDup (shift : Nat) : Instr :=
  if shift = 8 then .op (.Dup ⟨4, by decide⟩) else .op (.Dup ⟨5, by decide⟩)

def code (shift : Nat) : List Instr :=
  [dup1, dup1, push1 (UInt256.ofNat shift), op .SHR, op .XOR,
   maskDup shift, op .AND, endianFactorPush shift, op .MUL, op .XOR]
...
theorem run_endian (s : State) (startPC value : UInt256) (shift : Nat)
    ...
    (hmask : rest[if shift = 8 then 2 else 3]? = some mask)
    (hrun : s.halt = .Running) :
  ...
  all_goals norm_num at hmask
  all_goals
    simp (config := { maxSteps := 1000000 })
      [code, maskDup, hmask, endianFactorPush, endianFactor, op, push1, push2, push3, dup1,
    ...
    rw [mul_op]
    exact ⟨word_add_ofNat_assoc startPC 4 _, hsemantic⟩
...
theorem run_located {artifact : ProgramArtifact} {fork : Fork}
  ...
  rw [hraw, run_endian s site.startPC value shift mask rest hstack hcase hmask hrun, ← hend]
...
def gasSteps_endian {artifact : ProgramArtifact} {fork : Fork}
  ...
  · exact hcode
  · exact hfork
  · exact run_located shift mask site s value rest hstack hcase hmask hrun
  · exact hrun
  · exact hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.CachedEndianMultiply
