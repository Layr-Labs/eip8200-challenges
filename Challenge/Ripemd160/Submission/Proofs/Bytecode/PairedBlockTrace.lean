...
theorem tail_stack (s : State) (input : ByteArray) (i : Nat) :
    coreStack [.d, .b, .c, .a, .e, .factor, .pair, .upper, .lower]
      ...
      (UInt256.ofNat 405 :: driverRest input i) =
      PairedAllInlineTail.entryStack (resultFrame s input i)
        (UInt256.ofNat 405) (driverRest input i) := by
  rfl

theorem valid_return (s : State) (hcode : s.executionEnv.code = submissionBytecode) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 405).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 245 = 405 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 245 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 405 = true
  rw [hcode]
  exact h
...
def gasSteps_compress (s : State) (input : ByteArray) (i : Nat)
  ...
  let rho := UInt256.ofNat 405 :: driverRest input i
  ...
  have gschedule := PairedAllInlineBoundarySites.gasSteps_schedule s (UInt256.ofNat 405)
  ...
  have gschedule' : GasSteps (DriverTrace.compressEntry s input i)
      {q with pc := UInt256.ofNat 658, stack := cache q.memory ++ rho} := gschedule
  ...
  have hentry :
      {q with pc := UInt256.ofNat 711, stack := coreStack [.a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] ⟨PairedLaneWordRound.packCrypto lane lane, 0⟩ (cache q.memory ++ rho)} =
      {q with pc := UInt256.ofNat 711, stack := PairedStartupTrace.resultStack q.memory (cache q.memory ++ rho)} := by
  ...
  have gsuffix := TerminalRoundSite.gasSteps_suffix q terminal.frame
    (UInt256.ofNat 405) (driverRest input i) hstack qrun qactive
    (valid_return q qcode) qcode qfork qnp
  rw [hmemory] at gsuffix
  have gsuffix' : GasSteps
      {q with pc := UInt256.ofNat 4572, stack := coreStack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] terminal (cache q.memory ++ rho)}
