...
theorem tail_stack (s : State) (input : ByteArray) (i : Nat) :
    coreStack [.d, .b, .c, .a, .e, .factor, .pair, .upper, .lower]
      ...
      (UInt256.ofNat 464 :: driverRest input i) =
      PairedAllInlineTail.entryStack (resultFrame s input i)
        (UInt256.ofNat 464) (driverRest input i) := by
  rfl

theorem valid_return (s : State) (hcode : s.executionEnv.code = submissionBytecode) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 464).toNat = true := by
  ...
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 464 = true
  rw [hcode]
  exact h
...
def gasSteps_compress (s : State) (input : ByteArray) (i : Nat)
  ...
  let rho := UInt256.ofNat 464 :: driverRest input i
  ...
  have gschedule := PairedAllInlineBoundarySites.gasSteps_schedule s (UInt256.ofNat 464)
  ...
  have gschedule' : GasSteps (DriverTrace.compressEntry s input i)
      {q with pc := UInt256.ofNat 766, stack := rho} := gschedule
  ...
  have hentry :
      ...
      {q with pc := UInt256.ofNat 814, stack := PairedStartupTrace.resultStack q.memory rho} := by
  ...
  have gsuffix := TerminalRoundSite.gasSteps_suffix q terminal.frame
    (UInt256.ofNat 464) (driverRest input i) hstack qrun qactive
