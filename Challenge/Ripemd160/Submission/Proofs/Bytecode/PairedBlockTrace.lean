...
def gasSteps_compress (s : State) (input : ByteArray) (i : Nat)
  ...
  have gschedule' : GasSteps (DriverTrace.compressEntry s input i)
      {q with pc := UInt256.ofNat 749, stack := PairedDerivedStartup.lowerWord :: rho} := gschedule
  ...
  have hentry :
      ...
      {q with pc := UInt256.ofNat 809, stack := PairedStartupTrace.resultStack q.memory rho} := by
    rw [startup_stack, scheduled_readLane]
  have htail :
      {q with
        pc := UInt256.ofNat 5072
