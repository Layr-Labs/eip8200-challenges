    ...
    have gprefix : GasSteps (DriverTrace.compressEntry s input i)
      ...
      · have heq : s.executionEnv.calldata.size = (DriverTrace.blockOffsetWord i).toNat := by
        ...
        have gb : GasSteps {a with pc := UInt256.ofNat 410, stack := rho}
            {q with pc := UInt256.ofNat 452, stack := cache q.memory ++ rho} := by
...
def gasSteps_compress (s : State) (input : ByteArray) (i : Nat)
  ...
  have hentry :
      {q with pc := UInt256.ofNat 762, stack := coreStack [.a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] ⟨PairedLaneWordRound.packCrypto lane lane, 0⟩ (cache q.memory ++ rho)} =
      {q with pc := UInt256.ofNat 762, stack := PairedStartupTrace.resultStack q.memory (cache q.memory ++ rho)} := by
