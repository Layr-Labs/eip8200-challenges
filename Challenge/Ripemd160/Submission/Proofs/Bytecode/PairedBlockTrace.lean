...
theorem tail_stack (s : State) (input : ByteArray) (i : Nat) :
    coreStack [.d, .b, .c, .a, .e, .factor, .pair, .upper, .lower]
      ...
      (UInt256.ofNat 377 :: driverRest input i) =
      PairedAllInlineTail.entryStack (resultFrame s input i)
        (UInt256.ofNat 377) (driverRest input i) := by
  rfl

theorem valid_return (s : State) (hcode : s.executionEnv.code = submissionBytecode) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 377).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 240 = 377 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 240 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 377 = true
  rw [hcode]
  exact h
...
def gasSteps_compress (s : State) (input : ByteArray) (i : Nat)
  ...
  let rho := UInt256.ofNat 377 :: driverRest input i
  ...
  have gsuffix := TerminalRoundSite.gasSteps_suffix q terminal.frame
    (UInt256.ofNat 377) (driverRest input i) hstack qrun qactive
