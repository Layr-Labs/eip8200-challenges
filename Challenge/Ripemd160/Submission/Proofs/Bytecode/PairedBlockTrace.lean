...
theorem valid_return (s : State) (hcode : s.executionEnv.code = submissionBytecode) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 355 = 589 := by
...
def gasSteps_compress (s : State) (input : ByteArray) (i : Nat)
  ...
  have gsuffix' : GasSteps
      {q with pc := UInt256.ofNat 4797, stack := coreStack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] terminal (cache q.memory ++ rho)}
