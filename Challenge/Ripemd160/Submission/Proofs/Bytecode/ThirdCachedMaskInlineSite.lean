...
private theorem template_slice :
    (A.instructions.drop 2395).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2395 template_slice (by
    change 2395 + template.length ≤ Artifact.submissionInstructions.length
