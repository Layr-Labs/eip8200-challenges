...
private theorem template_slice :
    (A.instructions.drop 2380).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2380 template_slice (by
    change 2380 + template.length ≤ Artifact.submissionInstructions.length
