...
private theorem template_slice :
    (A.instructions.drop 2217).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2217 template_slice (by
    change 2217 + template.length ≤ Artifact.submissionInstructions.length
