...
private theorem template_slice :
    (A.instructions.drop 2206).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2206 template_slice (by
    change 2206 + template.length ≤ Artifact.submissionInstructions.length
