...
private theorem template_slice :
    (A.instructions.drop 2212).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2212 template_slice (by
    change 2212 + template.length ≤ Artifact.submissionInstructions.length
