...
private theorem template_slice :
    (A.instructions.drop 2393).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2393 template_slice (by
    change 2393 + template.length ≤ Artifact.submissionInstructions.length
