...
private theorem template_slice :
    (A.instructions.drop 2392).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2392 template_slice (by
    change 2392 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_end : site.endPC = QuadSites.rightPC 3 := by rfl
