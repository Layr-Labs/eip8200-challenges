...
private theorem template_slice :
    (A.instructions.drop 2218).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2218 template_slice (by
    change 2200 + template.length ≤ Artifact.submissionInstructions.length
