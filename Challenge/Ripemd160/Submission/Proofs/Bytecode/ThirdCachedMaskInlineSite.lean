...
private theorem template_slice :
    (A.instructions.drop 2232).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2232 template_slice (by
    change 2230 + template.length ≤ Artifact.submissionInstructions.length
