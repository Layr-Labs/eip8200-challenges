...
private theorem template_slice :
    (A.instructions.drop 2222).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2222 template_slice (by
    change 2204 + template.length ≤ Artifact.submissionInstructions.length
