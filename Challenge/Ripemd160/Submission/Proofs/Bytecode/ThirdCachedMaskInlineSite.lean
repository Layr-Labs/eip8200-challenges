...
private theorem template_slice :
    (A.instructions.drop 2216).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2216 template_slice (by
    change 2218 + template.length ≤ Artifact.submissionInstructions.length
