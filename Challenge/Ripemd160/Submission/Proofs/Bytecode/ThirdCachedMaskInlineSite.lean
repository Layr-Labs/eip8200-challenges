...
private theorem template_slice :
    (A.instructions.drop 2229).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2229 template_slice (by
    change 2229 + template.length ≤ Artifact.submissionInstructions.length
