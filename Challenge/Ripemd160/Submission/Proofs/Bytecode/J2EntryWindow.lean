...
private theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3644 = tail := by rfl
private theorem pc_base : Artifact.submissionArtifact.instructionPC 3644 = 4819 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
theorem get (index : Nat) :
    Artifact.submissionArtifact.instructions[3644+index]? = tail[index]? := by
  rw [← InstructionWindow.get_drop, tail_eq]
theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3644+index) = 4819+byteLength (tail.take index) := by
