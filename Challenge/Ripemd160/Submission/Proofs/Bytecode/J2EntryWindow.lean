...
private theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3667 = tail := by rfl
private theorem pc_base : Artifact.submissionArtifact.instructionPC 3667 = 4819 := by
...
theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3667+index) = 4819+byteLength (tail.take index) := by
