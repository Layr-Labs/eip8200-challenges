...
private theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3628 = tail := by rfl
private theorem pc_base : Artifact.submissionArtifact.instructionPC 3628 = 4807 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
theorem get (index : Nat) :
    Artifact.submissionArtifact.instructions[3628+index]? = tail[index]? := by
  rw [← InstructionWindow.get_drop, tail_eq]
theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3628+index) = 4807+byteLength (tail.take index) := by
