...
private theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3683 = tail := by rfl
private theorem pc_base : Artifact.submissionArtifact.instructionPC 3683 = 4827 := by
...
theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3683+index) = 4827+byteLength (tail.take index) := by
