...
private theorem tail_eq : Artifact.submissionArtifact.instructions.drop 3643 = tail := by rfl
private theorem pc_base : Artifact.submissionArtifact.instructionPC 3643 = 4818 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
theorem get (index : Nat) :
    Artifact.submissionArtifact.instructions[3643+index]? = tail[index]? := by
  rw [← InstructionWindow.get_drop, tail_eq]
theorem pc (index : Nat) :
    Artifact.submissionArtifact.instructionPC (3643+index) = 4818+byteLength (tail.take index) := by
