...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 551 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 891 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 551) = UInt256.ofNat 891
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 891, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 928, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 891) x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
  have hend : pcAfter (UInt256.ofNat 891) template = UInt256.ofNat 928 := by decide
  rw [hend] at hraw
