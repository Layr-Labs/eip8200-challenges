...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 397).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 397 actual_slice
    (by change 397 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 809 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 397) = UInt256.ofNat 809
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 809, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 846, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 809) x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
  have hend : pcAfter (UInt256.ofNat 809) template = UInt256.ofNat 846 := by decide
  rw [hend] at hraw
