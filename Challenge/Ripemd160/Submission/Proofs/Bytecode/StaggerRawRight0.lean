...
theorem site_pc : site.startPC = UInt256.ofNat 1129 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 553) = UInt256.ofNat 1129
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1129, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1166, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1129) x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
  have hend : pcAfter (UInt256.ofNat 1129) template = UInt256.ofNat 1166 := by decide
  rw [hend] at hraw
