...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.shiftRight (UInt256.mul x.v6 x.v3) (UInt256.ofNat 28)),
    x.v0,
    x.v2,
    (UInt256.land x.v7 (UInt256.add x.v5 (UInt256.shiftRight (UInt256.mul x.v6 (UInt256.land x.v7 (UInt256.add x.v0 (UInt256.add (MachineState.readWord memory 540) (UInt256.add (UInt256.xor x.v2 (UInt256.lor (UInt256.lnot x.v4) x.v3)) x.v1))))) (UInt256.ofNat 30)))),
    x.v4,
    x.v5,
    x.v6,
    x.v7,
...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 567 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 928 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 567) = UInt256.ofNat 928
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 928, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 959, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 928) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 928) template = UInt256.ofNat 959 := by decide
  rw [hend] at hraw
