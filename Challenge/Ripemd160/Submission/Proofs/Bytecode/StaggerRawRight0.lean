...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 566).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 566 actual_slice
    (by change 566 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 931 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 566) = UInt256.ofNat 931
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hraw := run_actual s (UInt256.ofNat 931) x rho hstack hrun hactive
