...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawRight0
...
def inputStack (x : Input) (rho : List UInt256) : List UInt256 :=
  [ x.v6, x.v7, x.v8, x.v9, x.v10, x.v11, x.v5 ] ++ rho
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [     (UInt256.land x.v7 (UInt256.add x.v5 (UInt256.shiftRight (UInt256.mul x.v6 (UInt256.land x.v7 (UInt256.add x.v0 (UInt256.add (UInt256.add (UInt256.xor x.v2 (UInt256.lor x.v3 (UInt256.lnot x.v4))) (MachineState.readWord memory 486)) x.v1)))) (UInt256.ofNat 25)))),
    x.v4,
    (UInt256.shiftRight (UInt256.mul x.v6 x.v3) (UInt256.ofNat 23)),
    x.v6,
    x.v7,
    x.v8,
    x.v9,
...
private theorem run_generated (s : State) (pc : UInt256) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 900) (halias : rho[0]? = some x.v2) (haliasC : rho[1]? = some x.v3) (haliasD : rho[2]? = some x.v4) (haliasA : rho[3]? = some x.v1) (hkey : x.v0 = UInt256.ofNat 1352829926) (haliasE : x.v12 = x.v5) (hrun : s.halt = .Running)
  ...
  have hcap (n : Nat) (hn : n ≤ 36) : rho.length + n < 1024 := by omega
  ...
  simp (discharger := omega) [template, inputStack, actualOutput,
    ...
    List.exchange, List.getElem?_cons_zero, Nat.add_assoc, hrun, hbase, hzero, hcap, halias, haliasC, haliasD, haliasA, hkey, haliasE,
...
theorem run_actual (s : State) (pc : UInt256) (x : Input) (rho : List UInt256)
  ...
  simpa only [actualOutput_eq] using run_generated s pc x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
#print axioms run_actual
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 571).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 571 actual_slice
    (by change 571 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 905 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 571) = UInt256.ofNat 905
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 905, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 942, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 905) x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
  have hend : pcAfter (UInt256.ofNat 905) template = UInt256.ofNat 942 := by decide
  rw [hend] at hraw
  exact DenseScheduleLift.gasSteps_of_raw site _ _ hcode hfork hrun hnp site_pc.symm advances hraw
#print axioms gasSteps
end Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerRawRight0
