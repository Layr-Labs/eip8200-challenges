...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v7 (UInt256.shiftRight (UInt256.mul x.v6 x.v3) (UInt256.ofNat 28))),
    x.v0,
    x.v2,
    (UInt256.land x.v7 (UInt256.add x.v5 (UInt256.shiftRight (UInt256.mul x.v6 (UInt256.land x.v7 (UInt256.add x.v0 (UInt256.add (MachineState.readWord memory 90) (UInt256.add (UInt256.xor (UInt256.lor (UInt256.lnot x.v4) x.v3) x.v2) x.v1))))) (UInt256.ofNat 30)))),
    x.v4,
    x.v5,
    x.v6,
    x.v7,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 603).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 603 actual_slice
    (by change 603 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1016 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 603) = UInt256.ofNat 1016
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1016, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1049, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1016) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1016) template = UInt256.ofNat 1049 := by decide
  rw [hend] at hraw
