...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 480 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 878 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 480) = UInt256.ofNat 878
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 878, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 915, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 878) x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
  have hend : pcAfter (UInt256.ofNat 878) template = UInt256.ofNat 915 := by decide
  rw [hend] at hraw
