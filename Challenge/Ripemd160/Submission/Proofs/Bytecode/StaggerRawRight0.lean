...
private theorem run_generated (s : State) (pc : UInt256) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 900) (halias : rho[2]? = some x.v2) (haliasC : rho[1]? = some x.v3) (haliasD : rho[0]? = some x.v4) (haliasA : rho[3]? = some x.v1) (hkey : x.v0 = UInt256.ofNat 1352829926) (haliasE : x.v12 = x.v5) (hrun : s.halt = .Running)
...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 576 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1131 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 576) = UInt256.ofNat 1131
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1131, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1168, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1131) x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
  have hend : pcAfter (UInt256.ofNat 1131) template = UInt256.ofNat 1168 := by decide
  rw [hend] at hraw
