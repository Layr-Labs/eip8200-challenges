...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 477 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 864 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 477) = UInt256.ofNat 864
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 864, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 901, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 864) x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
  have hend : pcAfter (UInt256.ofNat 864) template = UInt256.ofNat 901 := by decide
  rw [hend] at hraw
