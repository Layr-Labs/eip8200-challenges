...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 512 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 897 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 512) = UInt256.ofNat 897
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 897, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 934, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 897) x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
  have hend : pcAfter (UInt256.ofNat 897) template = UInt256.ofNat 934 := by decide
  rw [hend] at hraw
