...
theorem site_pc : site.startPC = UInt256.ofNat 883 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 498) = UInt256.ofNat 883
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 883, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 920, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 855) x rho hstack halias haliasC haliasD haliasA hkey haliasE hrun hactive
  have hend : pcAfter (UInt256.ofNat 883) template = UInt256.ofNat 892 := by decide
  rw [hend] at hraw
