...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 517 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 787 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 517) = UInt256.ofNat 787
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 787, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 866, stack := outputStack s.memory x rho, memory := outputMemory s.memory x} := by
  have hraw := run_actual s (UInt256.ofNat 787) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 787) template = UInt256.ofNat 866 := by decide
  rw [hend] at hraw
