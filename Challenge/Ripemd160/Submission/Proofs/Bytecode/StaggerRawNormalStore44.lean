...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 440 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 781 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 440) = UInt256.ofNat 781
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 781, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 859, stack := outputStack s.memory x rho, memory := outputMemory s.memory x} := by
  have hraw := run_actual s (UInt256.ofNat 781) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 781) template = UInt256.ofNat 859 := by decide
  rw [hend] at hraw
