...
31 ops / 63 bytes. Keeps L/U/P cache construction, pushes `R = 1 + 2^128`,
...
The duplication arithmetic (`dupFactor`, `mul_dupFactor`) is included under
this module's namespace so the file compiles with tracked imports alone.
-/
...
theorem template_bytes : (template.map Instr.size).sum = 63 := by
...
/-- Flat 31-op expansion of `template` for the advancement case split. -/
