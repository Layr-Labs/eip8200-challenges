...
theorem jumpDest_generic : Decode.isValidJumpDest submissionBytecode 499 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 290 = 499 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 290 (by rfl)
  rw [hpc] at h
  exact h
...
theorem jumpDest_hit1 : Decode.isValidJumpDest submissionBytecode 5227 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 4138 = 5227 := PrefixStatePaths.pc4126
...
def gasSteps_finish (s : State) (input : ByteArray)
  ...
  have ghit2 : Challenge.EvmProof.GasSteps (hit2Entry s input)
    ...
    frameBlock PrefixStatePaths.hit2Path s input 5180 _ hcode hfork hrun hnp
  ...
  · rw [if_pos hw1]
    have gsecond : Challenge.EvmProof.GasSteps (entry s input) (rungEntry s input) :=
      frameBlock PrefixStatePaths.secondComparePath s input 5057 _ hcode hfork hrun hnp
    ...
    · have gthird : Challenge.EvmProof.GasSteps (rungEntry s input) (fourthEntry s input) :=
        frameBlock PrefixStatePaths.thirdComparePath s input 5098 _ hcode hfork hrun hnp
      ...
      · rw [if_pos ⟨hw2, hw3⟩]
        have gfourth : Challenge.EvmProof.GasSteps (fourthEntry s input) (hit2Entry s input) :=
          frameBlock PrefixStatePaths.fourthComparePath s input 5139 _ hcode hfork hrun hnp
  ...
  · rw [if_neg hw1]
    exact frameBlock PrefixStatePaths.secondComparePath s input 5057 _ hcode hfork hrun hnp
