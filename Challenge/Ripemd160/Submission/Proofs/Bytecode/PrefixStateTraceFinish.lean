...
def entry (s : State) (input : ByteArray) : State := frame s input 5056

/-- Entry at the `H1` install after a guard match. -/
def hit1Entry (s : State) (input : ByteArray) : State := frame s input 5097
...
theorem jumpDest_driver : Decode.isValidJumpDest submissionBytecode 461 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 265 = 461 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
...
def gasSteps_finish (s : State) (input : ByteArray)
  ...
  have ghit1 : Challenge.EvmProof.GasSteps (hit1Entry s input)
      (PrefixStateMemory.resultState s input 0) :=
    frameBlock PrefixStatePaths.hitPath s input 5097 _ hcode hfork hrun hnp
  ...
  · rw [if_pos hw1]
    have gsecond : Challenge.EvmProof.GasSteps (entry s input) (hit1Entry s input) :=
      frameBlock PrefixStatePaths.secondComparePath s input 5056 _ hcode hfork hrun hnp
        (run_secondCompare_hit s input hw1 hcalldata hcode hrun)
    exact gsecond.trans ghit1
  · rw [if_neg hw1]
    exact frameBlock PrefixStatePaths.secondComparePath s input 5056 _ hcode hfork hrun hnp
