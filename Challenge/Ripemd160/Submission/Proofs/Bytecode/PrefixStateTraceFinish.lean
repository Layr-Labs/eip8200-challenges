...
def hit2Entry (s : State) (input : ByteArray) : State := frame s input 5299

def word4Entry (s : State) (input : ByteArray) : State := frame s input 5135

def word5Entry (s : State) (input : ByteArray) : State := frame s input 5176

def word6Entry (s : State) (input : ByteArray) : State := frame s input 5217

def word7Entry (s : State) (input : ByteArray) : State := frame s input 5258

/-- Entry at the `H1` install (`JUMPDEST` target of the rung's guards). -/
def hit1Entry (s : State) (input : ByteArray) : State := frame s input 5346
...
theorem jumpDest_hit1 : Decode.isValidJumpDest submissionBytecode 5346 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 4150 = 5346 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 4150 (by rfl)
  rw [hpc] at h
  exact h
...
def gasSteps_finish (s : State) (input : ByteArray)
  ...
  have ghit1 : Challenge.EvmProof.GasSteps (hit1Entry s input)
    ...
    frameBlock PrefixStatePaths.hitPath s input 5346 _ hcode hfork hrun hnp
  ...
  have ghit2 : Challenge.EvmProof.GasSteps (hit2Entry s input)
    ...
    frameBlock PrefixStatePaths.hit2Path s input 5299 _ hcode hfork hrun hnp
    ...
    · have g2 : Challenge.EvmProof.GasSteps (rungEntry s input) (fourthEntry s input) :=
      ...
      · have g3 : Challenge.EvmProof.GasSteps (fourthEntry s input) (word4Entry s input) :=
        ...
        · have g4 : Challenge.EvmProof.GasSteps (word4Entry s input) (word5Entry s input) :=
            frameBlock PrefixStatePaths.word4ComparePath s input 5135 _ hcode hfork hrun hnp
          ...
          · have g5 : Challenge.EvmProof.GasSteps (word5Entry s input) (word6Entry s input) :=
              frameBlock PrefixStatePaths.word5ComparePath s input 5176 _ hcode hfork hrun hnp
            ...
            · have g6 : Challenge.EvmProof.GasSteps (word6Entry s input) (word7Entry s input) :=
                frameBlock PrefixStatePaths.word6ComparePath s input 5217 _ hcode hfork hrun hnp
              ...
              · have g7 : Challenge.EvmProof.GasSteps (word7Entry s input) (hit2Entry s input) :=
                  frameBlock PrefixStatePaths.word7ComparePath s input 5258 _ hcode hfork hrun hnp
