...
/-- Entry at the SWAR setup (pc 5044), right after the word-0 match. -/
...
theorem jumpDest_hit2 : Decode.isValidJumpDest submissionBytecode 5224 = true := by
  have h := Artifact.submissionArtifact.isValidJumpDest_index 4228 (by rfl)
  rw [PrefixStatePaths.pc4210] at h
  exact h

theorem jumpDest_hit1 : Decode.isValidJumpDest submissionBytecode 5276 = true := by
  have h := Artifact.submissionArtifact.isValidJumpDest_index 4254 (by rfl)
  rw [PrefixStatePaths.pc4236] at h
  exact h

theorem jumpDest_bail : Decode.isValidJumpDest submissionBytecode 5323 = true := by
  have h := Artifact.submissionArtifact.isValidJumpDest_index 4276 (by rfl)
...
theorem run_setup (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.setupPath (entry s input) =
      some (rungFrame s input 5073 (PatternedWordData.expectedWordAt 0)) := by
...
theorem run_rung1_hit (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung1Path
      (rungFrame s input 5073 (PatternedWordData.expectedWordAt 0)) =
      some (rungFrame s input 5093 (PatternedWordData.expectedWordAt 1)) := by
...
theorem run_rung1_miss (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung1Path
      (rungFrame s input 5073 (PatternedWordData.expectedWordAt 0)) =
      some (rungFrame s input 5323 (PatternedWordData.expectedWordAt 1)) := by
...
theorem run_rung2_hit (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung2Path
      (rungFrame s input 5093 (PatternedWordData.expectedWordAt 1)) =
      some (rungFrame s input 5113 (PatternedWordData.expectedWordAt 2)) := by
...
theorem run_rung2_miss (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung2Path
      (rungFrame s input 5093 (PatternedWordData.expectedWordAt 1)) =
      some (rungFrame s input 5276 (PatternedWordData.expectedWordAt 2)) := by
...
theorem run_rung3_hit (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung3Path
      (rungFrame s input 5113 (PatternedWordData.expectedWordAt 2)) =
      some (rungFrame s input 5133 (PatternedWordData.expectedWordAt 3)) := by
...
theorem run_rung3_miss (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung3Path
      (rungFrame s input 5113 (PatternedWordData.expectedWordAt 2)) =
      some (rungFrame s input 5276 (PatternedWordData.expectedWordAt 3)) := by
...
theorem run_rung4_hit (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung4Path
      (rungFrame s input 5133 (PatternedWordData.expectedWordAt 3)) =
      some (rungFrame s input 5153 (PatternedWordData.expectedWordAt 4)) := by
...
theorem run_rung4_miss (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung4Path
      (rungFrame s input 5133 (PatternedWordData.expectedWordAt 3)) =
      some (rungFrame s input 5224 (PatternedWordData.expectedWordAt 4)) := by
...
theorem run_rung5_hit (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung5Path
      (rungFrame s input 5153 (PatternedWordData.expectedWordAt 4)) =
      some (rungFrame s input 5173 (PatternedWordData.expectedWordAt 5)) := by
...
theorem run_rung5_miss (s : State) (input : ByteArray)
    ...
    Stepper.runLocatedBlock PrefixStatePaths.rung5Path
      (rungFrame s input 5153 (PatternedWordData.expectedWordAt 4)) =
      some (rungFrame s input 5224 (PatternedWordData.expectedWordAt 5)) := by
...
theorem run_hit3 (s : State) (input : ByteArray) (e : UInt256)
    (hcode : s.executionEnv.code = submissionBytecode)
    (hrun : s.halt = .Running) :
    Stepper.runLocatedBlock PrefixStatePaths.hit3Path (rungFrame s input 5173 e) =
...
theorem run_hit2 (s : State) (input : ByteArray) (e : UInt256)
    (hcode : s.executionEnv.code = submissionBytecode)
    (hrun : s.halt = .Running) :
    Stepper.runLocatedBlock PrefixStatePaths.hit2Path (rungFrame s input 5224 e) =
...
theorem run_hit1 (s : State) (input : ByteArray) (e : UInt256)
    (hcode : s.executionEnv.code = submissionBytecode)
    (hrun : s.halt = .Running) :
    Stepper.runLocatedBlock PrefixStatePaths.hit1Path (rungFrame s input 5276 e) =
...
theorem run_bail (s : State) (input : ByteArray) (e : UInt256)
    (hcode : s.executionEnv.code = submissionBytecode)
    (hrun : s.halt = .Running) :
    Stepper.runLocatedBlock PrefixStatePaths.bailPath (rungFrame s input 5323 e) =
...
def gasSteps_finish (s : State) (input : ByteArray)
  ...
  have ghit1 : ∀ e, GasSteps (rungFrame s input 5276 e)
      (PrefixStateMemory.resultState s input 0) := fun e =>
    rungBlock PrefixStatePaths.hit1Path s input 5276 e _ hcode hfork hrun hnp
      (run_hit1 s input e hcode hrun)
  have ghit2 : ∀ e, GasSteps (rungFrame s input 5224 e)
      (PrefixStateMemory.resultState2 s input) := fun e =>
    rungBlock PrefixStatePaths.hit2Path s input 5224 e _ hcode hfork hrun hnp
      (run_hit2 s input e hcode hrun)
  have ghit3 : ∀ e, GasSteps (rungFrame s input 5173 e)
      (PrefixStateMemory.resultState3 s input) := fun e =>
    rungBlock PrefixStatePaths.hit3Path s input 5173 e _ hcode hfork hrun hnp
      (run_hit3 s input e hcode hrun)
  have gbail : ∀ e, GasSteps (rungFrame s input 5323 e)
      (DriverTrace.compressEntry (PrefixStateMemory.scratchState s) input 0) := fun e =>
    rungBlock PrefixStatePaths.bailPath s input 5323 e _ hcode hfork hrun hnp
      (run_bail s input e hcode hrun)
  by_cases hw1 : MachineState.readWord input 32 = PatternedWordData.expectedWordAt 1
  · rw [if_pos hw1]
    have g1 : GasSteps (rungFrame s input 5073 (PatternedWordData.expectedWordAt 0))
        (rungFrame s input 5093 (PatternedWordData.expectedWordAt 1)) :=
      rungBlock PrefixStatePaths.rung1Path s input 5073 _ _ hcode hfork hrun hnp
        (run_rung1_hit s input hw1 hcalldata hrun)
    by_cases hw2 : MachineState.readWord input 64 = PatternedWordData.expectedWordAt 2
    · have g2 : GasSteps (rungFrame s input 5093 (PatternedWordData.expectedWordAt 1))
          (rungFrame s input 5113 (PatternedWordData.expectedWordAt 2)) :=
        rungBlock PrefixStatePaths.rung2Path s input 5093 _ _ hcode hfork hrun hnp
      ...
      · have hm2 : PrefixStateModel.Matched2 input := ⟨hw2, hw3⟩
        rw [if_pos hm2]
        have g3 : GasSteps (rungFrame s input 5113 (PatternedWordData.expectedWordAt 2))
            (rungFrame s input 5133 (PatternedWordData.expectedWordAt 3)) :=
          rungBlock PrefixStatePaths.rung3Path s input 5113 _ _ hcode hfork hrun hnp
            (run_rung3_hit s input hw3 hcalldata hrun)
        by_cases hw4 : MachineState.readWord input 128 = PatternedWordData.expectedWordAt 4
        · have g4 : GasSteps (rungFrame s input 5133 (PatternedWordData.expectedWordAt 3))
              (rungFrame s input 5153 (PatternedWordData.expectedWordAt 4)) :=
            rungBlock PrefixStatePaths.rung4Path s input 5133 _ _ hcode hfork hrun hnp
          ...
          · have hm3 : PrefixStateModel.Matched3 input := ⟨hw4, hw5⟩
            rw [if_pos hm3]
            have g5 : GasSteps (rungFrame s input 5153 (PatternedWordData.expectedWordAt 4))
                (rungFrame s input 5173 (PatternedWordData.expectedWordAt 5)) :=
              rungBlock PrefixStatePaths.rung5Path s input 5153 _ _ hcode hfork hrun hnp
          ...
          · rw [if_neg (fun h => hw5 h.2)]
            have g5 : GasSteps (rungFrame s input 5153 (PatternedWordData.expectedWordAt 4))
                (rungFrame s input 5224 (PatternedWordData.expectedWordAt 5)) :=
              rungBlock PrefixStatePaths.rung5Path s input 5153 _ _ hcode hfork hrun hnp
        ...
        · rw [if_neg (fun h => hw4 h.1)]
          have g4 : GasSteps (rungFrame s input 5133 (PatternedWordData.expectedWordAt 3))
              (rungFrame s input 5224 (PatternedWordData.expectedWordAt 4)) :=
            rungBlock PrefixStatePaths.rung4Path s input 5133 _ _ hcode hfork hrun hnp
      ...
      · rw [if_neg (fun h => hw3 h.2)]
        have g3 : GasSteps (rungFrame s input 5113 (PatternedWordData.expectedWordAt 2))
            (rungFrame s input 5276 (PatternedWordData.expectedWordAt 3)) :=
          rungBlock PrefixStatePaths.rung3Path s input 5113 _ _ hcode hfork hrun hnp
    ...
    · rw [if_neg (fun h => hw2 h.1)]
      have g2 : GasSteps (rungFrame s input 5093 (PatternedWordData.expectedWordAt 1))
          (rungFrame s input 5276 (PatternedWordData.expectedWordAt 2)) :=
        rungBlock PrefixStatePaths.rung2Path s input 5093 _ _ hcode hfork hrun hnp
          (run_rung2_miss s input hw2 hcalldata hcode hrun)
      exact gsetup.trans (g1.trans (g2.trans (ghit1 _)))
  · rw [if_neg hw1]
    have g1 : GasSteps (rungFrame s input 5073 (PatternedWordData.expectedWordAt 0))
        (rungFrame s input 5323 (PatternedWordData.expectedWordAt 1)) :=
      rungBlock PrefixStatePaths.rung1Path s input 5073 _ _ hcode hfork hrun hnp
