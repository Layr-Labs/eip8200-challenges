...
def entry (s : State) (input : ByteArray) : State := frame s input 5059

/-- Entry at the depth-2 rung (word-2 comparison). -/
def rungEntry (s : State) (input : ByteArray) : State := frame s input 5100

/-- Entry at the word-3 comparison. -/
def fourthEntry (s : State) (input : ByteArray) : State := frame s input 5141
...
theorem jumpDest_generic : Decode.isValidJumpDest submissionBytecode 501 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 291 = 501 := by
...
theorem jumpDest_hit1 : Decode.isValidJumpDest submissionBytecode 5229 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 4139 = 5229 := PrefixStatePaths.pc4126
  have h := Artifact.submissionArtifact.isValidJumpDest_index 4139 (by rfl)
  rw [hpc] at h
  exact h
...
def gasSteps_finish (s : State) (input : ByteArray)
  ...
  · rw [if_pos hw1]
    have gsecond : Challenge.EvmProof.GasSteps (entry s input) (rungEntry s input) :=
      frameBlock PrefixStatePaths.secondComparePath s input 5059 _ hcode hfork hrun hnp
    ...
    · have gthird : Challenge.EvmProof.GasSteps (rungEntry s input) (fourthEntry s input) :=
        frameBlock PrefixStatePaths.thirdComparePath s input 5100 _ hcode hfork hrun hnp
      ...
      · rw [if_pos ⟨hw2, hw3⟩]
        have gfourth : Challenge.EvmProof.GasSteps (fourthEntry s input) (hit2Entry s input) :=
          frameBlock PrefixStatePaths.fourthComparePath s input 5141 _ hcode hfork hrun hnp
  ...
  · rw [if_neg hw1]
    exact frameBlock PrefixStatePaths.secondComparePath s input 5059 _ hcode hfork hrun hnp
