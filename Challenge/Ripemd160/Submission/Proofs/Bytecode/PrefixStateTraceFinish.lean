...
def hit2Entry (s : State) (input : ByteArray) : State := frame s input 5225
def fifthEntry (s : State) (input : ByteArray) : State := frame s input 5273
def sixthEntry (s : State) (input : ByteArray) : State := frame s input 5315
def hit3Entry (s : State) (input : ByteArray) : State := frame s input 5356
...
private def fourthCompareMissPath : List PrefixStatePaths.Located :=
  PrefixStatePaths.fourthComparePath.take 6

private def fifthComparePath : List PrefixStatePaths.Located :=
  ⟨4165, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩ ::
...
theorem jumpDest_hit1 : Decode.isValidJumpDest submissionBytecode 5182 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 4125 = 5182 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 4125 (by rfl)
  rw [hpc] at h
  exact h

theorem jumpDest_hit2 : Decode.isValidJumpDest submissionBytecode 5225 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 4143 = 5225 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 4143 (by rfl)
  rw [hpc] at h
  exact h

theorem jumpDest_hit3 : Decode.isValidJumpDest submissionBytecode 5273 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 4165 = 5273 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 4165 (by rfl)
  rw [hpc] at h
  exact h
...
theorem run_fourthCompare_hit (s : State) (input : ByteArray)
    ...
    Challenge.EvmProof.Stepper.runLocatedBlock PrefixStatePaths.fourthComparePath
      (fourthEntry s input) = some (fifthEntry s input) := by
  ...
  simp (config := { maxSteps := 500000 })
    [PrefixStatePaths.fourthComparePath,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      fourthEntry, fifthEntry, frame, hcalldata, hcode, hrun, hzero, hzero', hcond, hcondLit,
      BooleanSelect.xor_comm, UInt256.isTrue,
...
/-- The `H2` stores return to the driver's `102` continuation with the
block-1 offset replacing the block-0 offset. -/
theorem run_hit3 (s : State) (input : ByteArray)
    (hcode : s.executionEnv.code = submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock PrefixStatePaths.hit3Path
      (hit3Entry s input) =
      some (PrefixStateMemory.resultState3 s input) := by
  have hbo : DriverTrace.blockOffsetWord 2 = UInt256.ofNat 128 := rfl
  simp (config := { maxSteps := 500000 })
    [PrefixStatePaths.hit3Path,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      hit3Entry, frame, PrefixStateMemory.resultState3,
...
/-- Word-4 guard on a match: the `JUMPI` is not taken. -/
theorem run_fifthCompare_hit (s : State) (input : ByteArray)
    ...
    (_hcode : s.executionEnv.code = submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock fifthComparePath
      (fifthEntry s input) = some (sixthEntry s input) := by
  ...
  simp (config := { maxSteps := 500000 })
    [fifthComparePath, PrefixStatePaths.fifthComparePath,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      fifthEntry, sixthEntry, frame, hcalldata, hrun, hzero, hzero', hcond, hcondLit,
      BooleanSelect.xor_comm, UInt256.isTrue,
...
/-- Word-4 guard on a mismatch: the `JUMPI` is taken. -/
theorem run_fifthCompare_miss (s : State) (input : ByteArray)
  ...
  simp (config := { maxSteps := 500000 })
    [fifthComparePath, PrefixStatePaths.fifthComparePath,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      fifthEntry, hit2Entry, frame, hcalldata, hcode, hrun,
      htrue, htrue', hcond, hcond', hcondLit, BooleanSelect.xor_comm, UInt256.isTrue,
      PrefixStatePaths.pc4165,
      PrefixStatePaths.pc4166,
      PrefixStatePaths.pc4167,
...
/-- Word-5 guard on a match: the `JUMPI` is not taken. -/
theorem run_sixthCompare_hit (s : State) (input : ByteArray)
    (hword : MachineState.readWord input 160 =
      PatternedWordData.expectedWordAt 5)
    (hcalldata : s.executionEnv.calldata = input)
    (_hcode : s.executionEnv.code = submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock PrefixStatePaths.sixthComparePath
      (sixthEntry s input) = some (hit3Entry s input) := by
  ...
  simp (config := { maxSteps := 500000 })
    [PrefixStatePaths.sixthComparePath,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      sixthEntry, hit3Entry, frame, hcalldata, hrun, hzero, hzero', hcond, hcondLit,
      BooleanSelect.xor_comm, UInt256.isTrue,
...
/-- Word-5 guard on a mismatch: the `JUMPI` is taken. -/
theorem run_sixthCompare_miss (s : State) (input : ByteArray)
    (hword : MachineState.readWord input 160 ≠
      PatternedWordData.expectedWordAt 5)
    (hcalldata : s.executionEnv.calldata = input)
    (hcode : s.executionEnv.code = submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock PrefixStatePaths.sixthComparePath
      (sixthEntry s input) = some (hit2Entry s input) := by
  ...
  simp (config := { maxSteps := 500000 })
    [PrefixStatePaths.sixthComparePath,
      ...
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
      sixthEntry, hit2Entry, frame, hcalldata, hcode, hrun,
      htrue, htrue', hcond, hcond', hcondLit, BooleanSelect.xor_comm, UInt256.isTrue,
      PrefixStatePaths.pc4172,
      PrefixStatePaths.pc4173,
      PrefixStatePaths.pc4174,
...
theorem run_fourthCompare_miss (s : State) (input : ByteArray)
    ...
    (hcode : s.executionEnv.code = submissionBytecode)
    (hrun : s.halt = .Running) :
    Challenge.EvmProof.Stepper.runLocatedBlock fourthCompareMissPath
  ...
  have hcondLit :
    ...
    simpa only [PatternedWordData.expectedWordAt_3] using hcond'
  simp (config := { maxSteps := 500000 })
    [fourthCompareMissPath, PrefixStatePaths.fourthComparePath,
      Challenge.EvmProof.Stepper.runLocatedBlock,
      Challenge.EvmProof.Stepper.runLocated,
      Challenge.EvmProof.Stepper.runInstr,
...
def gasSteps_finish (s : State) (input : ByteArray)
  ...
  have ghit2 : Challenge.EvmProof.GasSteps (hit2Entry s input)
    ...
    frameBlock PrefixStatePaths.hit2Path s input 5225 _ hcode hfork hrun hnp
      (run_hit2 s input hcode hrun)
  have ghit3 : Challenge.EvmProof.GasSteps (hit3Entry s input)
      (PrefixStateMemory.resultState3 s input) :=
    frameBlock PrefixStatePaths.hit3Path s input 5356 _ hcode hfork hrun hnp
      (run_hit3 s input hcode hrun)
    ...
    · have gthird : Challenge.EvmProof.GasSteps (rungEntry s input) (fourthEntry s input) :=
      ...
      · rw [if_pos ⟨hw2, hw3⟩]
        ...
        · by_cases hw5 : MachineState.readWord input 160 = PatternedWordData.expectedWordAt 5
          · rw [if_pos ⟨hw4, hw5⟩]
            ...
            have gfifth : Challenge.EvmProof.GasSteps (fifthEntry s input) (sixthEntry s input) :=
              frameBlock fifthComparePath s input 5273 _ hcode hfork hrun hnp
                (run_fifthCompare_hit s input hw4 hcalldata hcode hrun)
            have gsixth : Challenge.EvmProof.GasSteps (sixthEntry s input) (hit3Entry s input) :=
              frameBlock PrefixStatePaths.sixthComparePath s input 5315 _ hcode hfork hrun hnp
                (run_sixthCompare_hit s input hw5 hcalldata hcode hrun)
            exact gsecond.trans (gthird.trans (gfourth.trans (gfifth.trans (gsixth.trans ghit3))))
          · rw [if_neg (fun h => hw5 h.2)]
            ...
            have gfifth : Challenge.EvmProof.GasSteps (fifthEntry s input) (sixthEntry s input) :=
              frameBlock fifthComparePath s input 5273 _ hcode hfork hrun hnp
                (run_fifthCompare_hit s input hw4 hcalldata hcode hrun)
            have gsixth : Challenge.EvmProof.GasSteps (sixthEntry s input) (hit2Entry s input) :=
              frameBlock PrefixStatePaths.sixthComparePath s input 5315 _ hcode hfork hrun hnp
                (run_sixthCompare_miss s input hw5 hcalldata hcode hrun)
            exact gsecond.trans (gthird.trans (gfourth.trans (gfifth.trans (gsixth.trans ghit2))))
      ...
      · rw [if_neg (fun h => hw3 h.2)]
        have gfourth : Challenge.EvmProof.GasSteps (fourthEntry s input) (hit1Entry s input) :=
          frameBlock fourthCompareMissPath s input 5094 _ hcode hfork hrun hnp
