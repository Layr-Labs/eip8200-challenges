...
def entry (s : State) (input : ByteArray) : State := frame s input 5050

/-- Entry at the depth-2 rung (word-2 comparison). -/
def rungEntry (s : State) (input : ByteArray) : State := frame s input 5091

/-- Entry at the word-3 comparison. -/
def fourthEntry (s : State) (input : ByteArray) : State := frame s input 5132

/-- Entry at the `H2` install after both rung words matched. -/
def hit2Entry (s : State) (input : ByteArray) : State := frame s input 5173

/-- Entry at the `H1` install (`JUMPDEST` target of the rung's guards). -/
def hit1Entry (s : State) (input : ByteArray) : State := frame s input 5220
...
theorem jumpDest_hit1 : Decode.isValidJumpDest submissionBytecode 5220 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 4136 = 5220 := PrefixStatePaths.pc4126
...
def gasSteps_finish (s : State) (input : ByteArray)
  ...
  have ghit1 : Challenge.EvmProof.GasSteps (hit1Entry s input)
    ...
    frameBlock PrefixStatePaths.hitPath s input 5220 _ hcode hfork hrun hnp
  ...
  have ghit2 : Challenge.EvmProof.GasSteps (hit2Entry s input)
    ...
    frameBlock PrefixStatePaths.hit2Path s input 5173 _ hcode hfork hrun hnp
  ...
  · rw [if_pos hw1]
    have gsecond : Challenge.EvmProof.GasSteps (entry s input) (rungEntry s input) :=
      frameBlock PrefixStatePaths.secondComparePath s input 5050 _ hcode hfork hrun hnp
    ...
    · have gthird : Challenge.EvmProof.GasSteps (rungEntry s input) (fourthEntry s input) :=
        frameBlock PrefixStatePaths.thirdComparePath s input 5091 _ hcode hfork hrun hnp
      ...
      · rw [if_pos ⟨hw2, hw3⟩]
        have gfourth : Challenge.EvmProof.GasSteps (fourthEntry s input) (hit2Entry s input) :=
          frameBlock PrefixStatePaths.fourthComparePath s input 5132 _ hcode hfork hrun hnp
  ...
  · rw [if_neg hw1]
    exact frameBlock PrefixStatePaths.secondComparePath s input 5050 _ hcode hfork hrun hnp
