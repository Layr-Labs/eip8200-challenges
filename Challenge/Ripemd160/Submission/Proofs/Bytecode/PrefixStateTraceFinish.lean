...
private def frame (s : State) (input : ByteArray) (pc : Nat) : State :=
  { s with
    pc := UInt256.ofNat pc
    stack := [DriverTrace.messageOffsetWord 0, UInt256.ofNat 464,
...
def hit1Entry (s : State) (input : ByteArray) : State := frame s input 5100

/-- The generic compression target of the guard is a valid jump destination. -/
theorem jumpDest_generic : Decode.isValidJumpDest submissionBytecode 531 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 294 = 531 := by
...
theorem jumpDest_driver : Decode.isValidJumpDest submissionBytecode 464 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 261 = 464 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
...
def gasSteps_finish (s : State) (input : ByteArray)
  ...
  have ghit1 : Challenge.EvmProof.GasSteps (hit1Entry s input)
      (PrefixStateMemory.resultState s input 0) :=
    frameBlock PrefixStatePaths.hitPath s input 5100 _ hcode hfork hrun hnp
