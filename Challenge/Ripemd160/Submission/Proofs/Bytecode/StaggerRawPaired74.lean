...
theorem site_pc : site.startPC = UInt256.ofNat 4341 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3578) = UInt256.ofNat 4341
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4341, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4380, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4341) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4341) template = UInt256.ofNat 4380 := by decide
  rw [hend] at hraw
