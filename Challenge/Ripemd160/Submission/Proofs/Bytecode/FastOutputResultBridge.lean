...
def outputState (s : State) (input : ByteArray) : State :=
  FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1362) (driverRest input)
...
noncomputable def fullTrace (input : ByteArray) (hfit : CalldataFits input)
    ...
    (entryPrefix : GasSteps (initialState submissionBytecode input 0) (Execution.atPC input 0x16b)) :
