...
noncomputable def fullTrace (input : ByteArray) (hfit : CalldataFits input)
    ...
    (guardSteps : GasSteps (initialState submissionBytecode input 0) (Execution.atPC input 0x3ee)) :
  ...
  exact (PaddingTrace.gasSteps_pad input hfit guardSteps).trans
...
theorem correct_of_compression_trace
  ...
  let trace := fullTrace input hfit (seam input hfit) guardSteps
