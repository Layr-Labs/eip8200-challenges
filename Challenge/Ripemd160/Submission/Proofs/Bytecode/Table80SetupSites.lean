...
theorem normal_slice :
    (Artifact.submissionArtifact.instructions.drop 322).take actualNormalTemplate.length = actualNormalTemplate := by rfl

def normalSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualNormalTemplate :=
  StackSiteBuilder.ofSlice actualNormalTemplate 322 normal_slice
    (by change 322 + actualNormalTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualNormalTemplate) (by decide))
    (by decide)
theorem normal_pc : normalSite.startPC = UInt256.ofNat 527 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 322) = UInt256.ofNat 527
...
theorem normal_end : pcAfter (UInt256.ofNat 527) actualNormalTemplate = UInt256.ofNat 1052 := by decide

theorem pad_slice :
    (Artifact.submissionArtifact.instructions.drop 243).take padTemplate.length = padTemplate := by rfl

def padSite : GenericRoundSite Artifact.submissionArtifact .Osaka padTemplate :=
  StackSiteBuilder.ofSlice padTemplate 243 pad_slice
    (by change 243 + padTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := padTemplate) (by decide))
    (by decide)
theorem pad_pc : padSite.startPC = UInt256.ofNat 400 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 243) = UInt256.ofNat 400
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem pad_end : pcAfter (UInt256.ofNat 400) padTemplate = UInt256.ofNat 496 := by decide
...
def gasSteps_normal (s : State) (ret : UInt256) (p : Nat) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 527, stack := UInt256.ofNat p :: ret :: rest}
      {s with pc := UInt256.ofNat 1052, stack := Table80Raw.cache ++ (ret :: rest), memory := PairTableLayout.resultMemory s.memory (PairedScheduleData.extractedWord s.memory p), activeWords := DenseScheduleTemplate.loadedActiveWords s (UInt256.ofNat p)} := by
  apply normal_gasSteps_of_raw {s with pc := UInt256.ofNat 527, stack := UInt256.ofNat p :: ret :: rest} _ hcode hfork hrun hnp normal_pc.symm
  have h := Table80ScratchZero.run_normal s (UInt256.ofNat 526) ret p rest hstack hrun hp hbound
  have hfull : Table80ScratchZero.normalTemplate = .op .JUMPDEST :: actualNormalTemplate := by rfl
  have hend : pcAfter (UInt256.ofNat 526) Table80ScratchZero.normalTemplate = UInt256.ofNat 1052 := by decide
  rw [hend, hfull] at h
  have ht := run_without_jumpdest actualNormalTemplate
    (DenseScheduleTemplate.scheduleEntry s (UInt256.ofNat 526) (UInt256.ofNat p) ret rest) _ (by decide)
    (by simp only [DenseScheduleTemplate.scheduleEntry, List.length_append, List.length_cons, List.length_nil]; omega) hrun h
  simpa only [DenseScheduleTemplate.scheduleEntry, List.cons_append, List.nil_append, show (UInt256.ofNat 526).succ = UInt256.ofNat 527 by decide] using ht
...
def gasSteps_pad (s : State) (ret : UInt256) (rest : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 400, stack := ret :: rest}
      {s with pc := UInt256.ofNat 496, stack := Table80Raw.cache ++ (ret :: rest), memory := PairTablePad.resultMemory s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  apply PadLift.gasSteps_of_raw padSite {s with pc := UInt256.ofNat 400, stack := ret :: rest} _ hcode hfork hrun hnp pad_pc.symm pad_advances
  have h := run_pad s (UInt256.ofNat 400) ret rest hstack hrun hactive hfit
  rw [pad_end] at h
  exact h
