...
namespace Challenge.EvmProof.DataMeter
...
theorem runInstr_cost_potential {instruction : Instr} {s t : State}
    ...
    DataStepper.instrCost instruction s + MachineState.memCost s.activeWords.toNat =
      instrCostWithoutMemory instruction s +
        MachineState.memCost t.activeWords.toNat := by
  unfold DataStepper.runInstr at hresult
  split at hresult
  · split at hresult
    all_goals
      repeat' first | split at hresult | simp_all
    all_goals subst t
    all_goals simp_all [DataStepper.instrCost, instrCostWithoutMemory, Gas.totalCost,
...
theorem runLocated_cost_potential
    {artifact : Challenge.EvmProof.DataProgramArtifact} {fork : Fork}
    {located : DataStepper.Located artifact fork} {s t : State}
    (hresult : DataStepper.runLocated located s = some t) :
    DataStepper.instrCost located.instruction s +
        MachineState.memCost s.activeWords.toNat =
      instrCostWithoutMemory located.instruction s +
        MachineState.memCost t.activeWords.toNat := by
  unfold DataStepper.runLocated at hresult
  split at hresult
...
def runLocatedBlockCostWithoutMemory
    {artifact : Challenge.EvmProof.DataProgramArtifact} {fork : Fork} :
    List (DataStepper.Located artifact fork) → State → Nat
  | [], _ => 0
  | located :: rest, s =>
      instrCostWithoutMemory located.instruction s +
        match rest with
        | [] => 0
        | _ :: _ =>
            match DataStepper.runLocated located s with
            | some next =>
...
def runLocatedBlockStaticCost
    {artifact : Challenge.EvmProof.DataProgramArtifact} {fork : Fork}
    (path : List (DataStepper.Located artifact fork)) : Nat :=
...
theorem runLocatedBlockCostWithoutMemory_eq_static
    {artifact : Challenge.EvmProof.DataProgramArtifact} {fork : Fork}
    (path : List (DataStepper.Located artifact fork)) {s t : State}
    (hresult : DataStepper.runLocatedBlock path s = some t)
    (hfork : s.fork = fork)
  ...
  | cons located rest ih =>
      ...
      | cons nextLocated tail =>
          cases hnext : DataStepper.runLocated located s with
          | none =>
              simp [DataStepper.runLocatedBlock, hnext] at hresult
          | some next =>
              cases hrun : next.halt with
              | Running =>
                  simp [DataStepper.runLocatedBlock, hnext, hrun] at hresult
                  have hnextFork : next.fork = fork := by
                    have henv := DataStepper.runLocated_executionEnv hnext
...
theorem runLocatedBlock_cost_potential
    ...
    (path : List (DataStepper.Located artifact fork)) {s t : State}
    (hresult : DataStepper.runLocatedBlock path s = some t) :
    DataStepper.runLocatedBlockCost path s +
        MachineState.memCost s.activeWords.toNat =
      runLocatedBlockCostWithoutMemory path s +
        MachineState.memCost t.activeWords.toNat := by
  induction path generalizing s t with
  | nil =>
      simp [DataStepper.runLocatedBlock] at hresult
      subst t
      simp [DataStepper.runLocatedBlockCost, runLocatedBlockCostWithoutMemory]
  | cons located rest ih =>
      cases rest with
      | nil =>
          cases hnext : DataStepper.runLocated located s with
          ...
          | some next =>
              simp [DataStepper.runLocatedBlock, hnext] at hresult
              subst t
              simpa [DataStepper.runLocatedBlockCost,
      ...
      | cons nextLocated tail =>
          ...
          | some next =>
              ...
              | Running =>
                  ...
                  have hcost :
                      DataStepper.runLocatedBlockCost
                          (located :: nextLocated :: tail) s =
                        DataStepper.instrCost located.instruction s +
                          DataStepper.runLocatedBlockCost
                            (nextLocated :: tail) next := by
                    simp [DataStepper.runLocatedBlockCost, hnext, hrun]
                  have hwork :
                      runLocatedBlockCostWithoutMemory
                          (located :: nextLocated :: tail) s =
                        instrCostWithoutMemory located.instruction s +
...
theorem runLocatedBlock_cost_potential_of_copyFree
    {artifact : Challenge.EvmProof.DataProgramArtifact} {fork : Fork}
    (path : List (DataStepper.Located artifact fork)) {s t : State} (work : Nat)
    (hresult : DataStepper.runLocatedBlock path s = some t)
    (hfork : s.fork = fork)
...
end Challenge.EvmProof.DataMeter
