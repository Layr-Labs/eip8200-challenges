...
def branchTemplate : List Instr :=
  [ .push ⟨2, by decide⟩ (UInt256.ofNat 875),
    .op .JUMPI ]
...
theorem run_branch_taken (s : State) (pc c : UInt256) (rho : List UInt256)
    ...
    (hvalid : Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 875).toNat = true) :
    runInstrSeq branchTemplate {s with pc := pc, stack := c :: rho} =
      some {s with pc := UInt256.ofNat 875, stack := rho} := by
