...
def highZero (n : UInt256) : UInt256 := UInt256.lt n (UInt256.ofNat 5225)

theorem highZero_true_iff (n : UInt256) :
    UInt256.isTrue (highZero n) ↔ n.toNat < 5225 := by
  change (UInt256.lt n (UInt256.ofNat 5225)).toNat ≠ 0 ↔ n.toNat < 5225
  rw [Word.word_toNat_lt]
  have hc : (UInt256.ofNat 5225).toNat = 5225 := by decide
  rw [hc]
  by_cases hn : n.toNat < 5225 <;> simp [hn]
...
theorem run_low (s : State) (pc returnPC : UInt256) (rest : List UInt256)
    ...
    (hfit : s.executionEnv.calldata.size < 2 ^ 256) (hcode : s.executionEnv.code.size = 5225) :
