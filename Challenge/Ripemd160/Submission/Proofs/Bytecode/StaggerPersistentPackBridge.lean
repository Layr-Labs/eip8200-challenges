...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 547).take template.length = template := by rfl

def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 547 actual_slice
    (by change 547 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 938 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 547) = UInt256.ofNat 938
