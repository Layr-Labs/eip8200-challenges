...
def entry (s : State) (h q : WordLane) (off limit : UInt256) (rho : List UInt256) : State :=
  {s with pc := UInt256.ofNat 968, stack := stack s.memory h.e [.d, .k, .b, .c, .a, .e, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500] q h (UInt256.ofNat 1352829926) (suffix h off limit rho)}

theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 612).take template.length = template := by rfl

def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 612 actual_slice
    (by change 612 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 968 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 612) = UInt256.ofNat 968
...
def gasSteps (s : State) (h q : WordLane) (off limit : UInt256) (rho : List UInt256)
  ...
  have raw := JointRightPackRaw.run_actual s (UInt256.ofNat 968)
    (input s.memory h q off limit) rho hs hr ha
  have hend : StackRoundTrace.pcAfter (UInt256.ofNat 968) template = UInt256.ofNat 1033 := by decide
  rw [hend] at raw
  have g := DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 968, stack := JointRightPackRaw.inputStack (input s.memory h q off limit) rho}
    {s with pc := UInt256.ofNat 1033, stack := JointRightPackRaw.outputStack s.memory (input s.memory h q off limit) rho}
