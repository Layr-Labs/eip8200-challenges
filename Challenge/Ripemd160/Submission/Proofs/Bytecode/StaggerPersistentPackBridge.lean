...
def entry (s : State) (h q : WordLane) (off limit : UInt256) (rho : List UInt256) : State :=
  {s with pc := UInt256.ofNat 960, stack := stack s.memory h.e [.d, .k, .b, .c, .a, .e, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500] q h (UInt256.ofNat 1352829926) (suffix h off limit rho)}
...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 556 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 960 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 556) = UInt256.ofNat 960
...
def gasSteps (s : State) (h q : WordLane) (off limit : UInt256) (rho : List UInt256)
  ...
  have raw := JointRightPackRaw.run_actual s (UInt256.ofNat 932)
    (input s.memory h q off limit) rho hs hr ha (by rfl)
  have hend : StackRoundTrace.pcAfter (UInt256.ofNat 960) template = UInt256.ofNat 1036 := by decide
  rw [hend] at raw
  have g := DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 960, stack := JointRightPackRaw.inputStack (input s.memory h q off limit) rho}
    {s with pc := UInt256.ofNat 1064, stack := JointRightPackRaw.outputStack s.memory (input s.memory h q off limit) rho}
