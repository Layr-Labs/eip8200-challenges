...
theorem site_pc : site.startPC = UInt256.ofNat 1152 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 706) = UInt256.ofNat 1152
...
def gasSteps (s : State) (h q : WordLane) (off limit : UInt256) (rho : List UInt256)
  ...
  have hend : StackRoundTrace.pcAfter (UInt256.ofNat 1152) template = UInt256.ofNat 1218 := by decide
  rw [hend] at raw
  have g := DenseScheduleLift.gasSteps_of_raw site
    ...
    {s with pc := UInt256.ofNat 1218, stack := JointRightPackRaw.outputStack s.memory (input s.memory h q off limit) rho}
