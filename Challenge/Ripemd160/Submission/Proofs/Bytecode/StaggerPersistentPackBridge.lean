...
theorem site_pc : site.startPC = UInt256.ofNat 998 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 624) = UInt256.ofNat 998
...
def gasSteps (s : State) (h q : WordLane) (off limit : UInt256) (rho : List UInt256)
  ...
  have raw := JointRightPackRaw.run_actual s (UInt256.ofNat 998)
    (input s.memory h q off limit) rho hs hr ha
  have hend : StackRoundTrace.pcAfter (UInt256.ofNat 998) template = UInt256.ofNat 1063 := by decide
  rw [hend] at raw
  have g := DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 998, stack := JointRightPackRaw.inputStack (input s.memory h q off limit) rho}
    {s with pc := UInt256.ofNat 1063, stack := JointRightPackRaw.outputStack s.memory (input s.memory h q off limit) rho}
