...
theorem initial_slice :
    (Artifact.submissionArtifact.instructions.drop 268).take initialTemplate.length = initialTemplate := by rfl

def initialSite : GenericRoundSite Artifact.submissionArtifact .Osaka initialTemplate :=
  StackSiteBuilder.ofSlice initialTemplate 268 initial_slice
    (by change 268 + initialTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem initial_pc : initialSite.startPC = UInt256.ofNat 391 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 268) = UInt256.ofNat 391
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

def jumpCode := PadJump.template 391

theorem jump_slice :
    (Artifact.submissionArtifact.instructions.drop 3722).take jumpCode.length = jumpCode := by rfl

def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpCode :=
  StackSiteBuilder.ofSlice jumpCode 3722 jump_slice
    (by change 3722 + jumpCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4722 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3722) = UInt256.ofNat 4722
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_loop (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 391).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 268 = 391 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 268 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 391 = true
  rw [hcode]
  exact h
...
def gasSteps_push (s : State) (limit : UInt256) (rho : List UInt256)
  ...
  apply PadLift.gasSteps_of_raw initialSite {s with pc := UInt256.ofNat 391, stack := limit :: rho} _ hcode hfork hrun hnp initial_pc.symm
  · apply PadLift.advancesAll_sound; decide
  · have hr := run_initial s (UInt256.ofNat 391) limit rho (by omega) hrun
    have hp : pcAfter (UInt256.ofNat 391) initialTemplate = UInt256.ofNat 481 := by decide
    rw [hp] at hr
    exact hr
...
def gasSteps_jump (s : State) (frame : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4722, stack := frame}
      {s with pc := UInt256.ofNat 391, stack := frame} := by
  apply PadLift.gasSteps_of_raw jumpSite {s with pc := UInt256.ofNat 4722, stack := frame} _ hcode hfork hrun hnp jump_pc.symm
  · apply PadLift.advancesAll_sound; decide
  · exact PadJump.run_template s (UInt256.ofNat 4722) frame 391 (by omega) hrun (valid_loop s hcode)
...
def fullSite : GenericRoundSite Artifact.submissionArtifact .Osaka fullTemplate :=
  StackSiteBuilder.ofSlice fullTemplate 266 full_slice
    (by change 266 + fullTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := fullTemplate) (by decide)) (by decide)
theorem full_pc : fullSite.startPC = UInt256.ofNat 389 := by
...
def gasSteps_full (s : State) (limit : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 389, stack := limit :: rho}
      {s with pc := UInt256.ofNat 391, stack := UInt256.ofNat s.executionEnv.calldata.size :: rho} := by
  apply PadLift.gasSteps_of_raw fullSite {s with pc := UInt256.ofNat 389, stack := limit :: rho} _
...
theorem partial_pc : partialSite.startPC = UInt256.ofNat 4677 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3690) = UInt256.ofNat 4677
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_partial (s : State) (stack : List UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4677, stack := stack}
      {s with pc := UInt256.ofNat 4678, stack := stack} := by
  apply PadLift.gasSteps_of_raw partialSite {s with pc := UInt256.ofNat 4677, stack := stack} _
