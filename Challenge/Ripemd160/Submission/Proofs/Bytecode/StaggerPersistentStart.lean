...
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4763 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3789) = UInt256.ofNat 4763
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_push (s : State) (limit : UInt256) (rho : List UInt256)
  ...
  · have hr := run_initial s (UInt256.ofNat 374) limit rho (by omega) hrun
    have hp : pcAfter (UInt256.ofNat 374) initialTemplate = UInt256.ofNat 481 := by decide
    rw [hp] at hr
    exact hr
...
def gasSteps_jump (s : State) (frame : List UInt256)
  ...
  · apply PadLift.advancesAll_sound; decide
  · exact PadJump.run_template s (UInt256.ofNat 4763) frame 374 (by omega) hrun (valid_loop s hcode)
...
theorem partial_pc : partialSite.startPC = UInt256.ofNat 4726 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3762) = UInt256.ofNat 4726
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
