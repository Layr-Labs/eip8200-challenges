...
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4774 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3782) = UInt256.ofNat 4774
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_jump (s : State) (frame : List UInt256)
  ...
  · apply PadLift.advancesAll_sound; decide
  · exact PadJump.run_template s (UInt256.ofNat 4774) frame 376 (by omega) hrun (valid_loop s hcode)
...
theorem partial_pc : partialSite.startPC = UInt256.ofNat 4737 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3755) = UInt256.ofNat 4737
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
