...
theorem jump_slice :
    (Artifact.submissionArtifact.instructions.drop 3719).take jumpCode.length = jumpCode := by rfl

def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpCode :=
  StackSiteBuilder.ofSlice jumpCode 3719 jump_slice
    (by change 3719 + jumpCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4710 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3719) = UInt256.ofNat 4710
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem partial_slice :
    (Artifact.submissionArtifact.instructions.drop 3692).take partialTemplate.length = partialTemplate := by rfl
def partialSite : GenericRoundSite Artifact.submissionArtifact .Osaka partialTemplate :=
  StackSiteBuilder.ofSlice partialTemplate 3692 partial_slice
    (by change 3692 + partialTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem partial_pc : partialSite.startPC = UInt256.ofNat 4673 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3692) = UInt256.ofNat 4673
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
