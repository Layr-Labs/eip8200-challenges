...
/-- The resident minus-form modulus word is recovered from the plus-form seed by removing
the shifted increment, so the seed literal needs no leading zero bytes. -/
private theorem modulusMinusFromPlus :
    UInt256.sub (UInt256.ofNat 822752278660603021099785336477205875632903651089438293180284928)
...
theorem run_initial (s : State) (pc limit : UInt256) (rho : List UInt256)
  ...
  simp [initialTemplate, StaggerPersistentFrame.frame, StackRunBridge.initialHashState,
    ...
    FusedKeyReconstruction.modulusMinus, FusedKeyReconstruction.modulusPlus,
    FusedKeyReconstruction.modulusCombinedPlus, modulusMinusFromPlus, StaggerPersistentBootstrapRaw.fusedMinus_eq,
