...
theorem jump_slice :
    (Artifact.submissionArtifact.instructions.drop 3755).take jumpCode.length = jumpCode := by rfl

def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpCode :=
  StackSiteBuilder.ofSlice jumpCode 3755 jump_slice
    (by change 3755 + jumpCode.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    (by change submissionBytecode.size < 2^256; rw [referenceBytecode_size]; decide)
...
def gasSteps_jump (s : State) (frame : List UInt256)
  ...
  · apply PadLift.advancesAll_sound; decide
  · exact PadJump.run_template s (UInt256.ofNat 4736) frame 383 (by omega) hrun (valid_loop s hcode)
...
theorem partial_slice :
    (Artifact.submissionArtifact.instructions.drop 3728).take partialTemplate.length = partialTemplate := by rfl
def partialSite : GenericRoundSite Artifact.submissionArtifact .Osaka partialTemplate :=
  StackSiteBuilder.ofSlice partialTemplate 3728 partial_slice
    (by change 3728 + partialTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem partial_pc : partialSite.startPC = UInt256.ofNat 4699 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3728) = UInt256.ofNat 4699
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
