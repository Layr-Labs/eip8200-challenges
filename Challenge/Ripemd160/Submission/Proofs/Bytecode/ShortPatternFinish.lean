...
def gasSteps_return (n : Nat) (input : ByteArray) (sv ov : UInt256)
  ...
  have hfit : CalldataFits input := by
    unfold CalldataFits
    rcases hn with rfl | rfl <;> (rw [hsize]; decide)
  have hne : input.size ≠ 128 := by rw [hsize]; rcases hn with rfl | rfl <;> decide
  have gselect := sound selectorPath (run_selector n input sv ov hn hsize)
  have ghop := ShortPattern128Hop.gasSteps_sel_not128 input
    (returnRest sv ov) (by simp [returnRest]) hne hfit
  have gstore := sound digestStorePath (run_store n input sv ov hn hsize)
  have hd := Artifact.submissionArtifact.decodeAt_op_index 4165 .MSIZE
    (by rfl) (by decide) trivial
  have hp : (storedState n input sv ov).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4165 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
  have hop : (storedState n input sv ov).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState n input sv ov) 4165
  ...
  exact gselect.trans (ghop.trans (gstore.trans (gm.trans (sound digestFinishPath
