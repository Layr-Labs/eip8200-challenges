...
def gasSteps_return (n : Nat) (input : ByteArray) (sv ov : UInt256)
  ...
  have hc := Artifact.submissionArtifact.decodeAt_op_index 3865 .CODECOPY
  ...
  have hcopy : (copyReadyState n input sv ov).decodedOp = some .CODECOPY :=
    Artifact.submissionArtifact.state_decodedOp_of (copyReadyState n input sv ov) 3865
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 3866 .MSIZE
    (by rfl) (by decide) trivial
  ...
  have hop : (storedState n input sv ov).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState n input sv ov) 3866
