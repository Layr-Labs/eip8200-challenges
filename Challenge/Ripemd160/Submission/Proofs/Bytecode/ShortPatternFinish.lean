...
def gasSteps_return (n : Nat) (input : ByteArray) (sv ov : UInt256)
  ...
  have hcs := Artifact.submissionArtifact.decodeAt_op_index 4071 .CODESIZE
    (by rfl) (by decide) trivial
  have hcspc : (multipliedState n input sv ov).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4071 := by rw [pc4851]; rfl
  have hcsop : (multipliedState n input sv ov).decodedOp = some .CODESIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (multipliedState n input sv ov) 4071
  ...
  have hc := Artifact.submissionArtifact.decodeAt_op_index 4081 .CODECOPY
    (by rfl) (by decide) trivial
  have hpc : (copyReadyState n input sv ov).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4081 := by rw [pc4870]; rfl
  have hcopy : (copyReadyState n input sv ov).decodedOp = some .CODECOPY :=
    Artifact.submissionArtifact.state_decodedOp_of (copyReadyState n input sv ov) 4081
  ...
  have hoff : (UInt256.ofNat (tableOffset n)).toNat = tableOffset n := by
    ...
    unfold tableOffset
    have hlt := Nat.mod_lt (((259820 * n + 5208) / 65536)) (by decide : 0 < 14)
    omega
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 4082 .MSIZE
    (by rfl) (by decide) trivial
  have hp : (storedState n input sv ov).pc.toNat =
      Artifact.submissionArtifact.instructionPC 4082 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
  have hop : (storedState n input sv ov).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState n input sv ov) 4082
