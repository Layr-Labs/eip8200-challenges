...
def gasSteps_return (n : Nat) (input : ByteArray) (sv ov : UInt256)
  ...
  have hoff : (UInt256.ofNat (tableOffset n)).toNat = tableOffset n := by
    ...
    unfold tableOffset
    have hlt := Nat.mod_lt (((n * (n + 35704)) / 16384)) (by decide : 0 < 16)
    omega
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 4072 .MSIZE
    (by rfl) (by decide) trivial
  ...
  have hop : (storedState n input sv ov).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState n input sv ov) 4072
