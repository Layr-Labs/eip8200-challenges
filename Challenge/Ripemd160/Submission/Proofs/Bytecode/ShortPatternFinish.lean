...
def gasSteps_return (n : Nat) (input : ByteArray) (sv ov : UInt256)
  ...
  have hc := Artifact.submissionArtifact.decodeAt_op_index 3857 .CODECOPY
    (by rfl) (by decide) trivial
  have hpc : (copyReadyState n input sv ov).pc.toNat =
      Artifact.submissionArtifact.instructionPC 3857 := by rw [pc4870]; rfl
  have hcopy : (copyReadyState n input sv ov).decodedOp = some .CODECOPY :=
    Artifact.submissionArtifact.state_decodedOp_of (copyReadyState n input sv ov) 3857
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 3858 .MSIZE
    (by rfl) (by decide) trivial
  have hp : (storedState n input sv ov).pc.toNat =
      Artifact.submissionArtifact.instructionPC 3858 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
  have hop : (storedState n input sv ov).decodedOp = some .MSIZE :=
    Artifact.submissionArtifact.state_decodedOp_of (storedState n input sv ov) 3858
