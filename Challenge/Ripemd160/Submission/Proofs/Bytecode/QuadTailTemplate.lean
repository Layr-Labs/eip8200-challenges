...
/-!
# Cached-mask/factor consume tail template

The helper leaves `[right.a,b,c,d,e, mask, factor, left.b,c,d,e,a, ret] ++ rest`.
The consume tail permutes those live words onto the combine operands, writes the
five `evmCombine` stores, pops the leftover factor, and `JUMP`s. Nine following
...
def tailStartPC : UInt256 := UInt256.ofNat 0xb2b
def tailJumpPC : UInt256 := UInt256.ofNat 0xb7e

def swap5H : Instr := .op (.Swap ⟨4, by decide⟩)
def swap6H : Instr := .op (.Swap ⟨5, by decide⟩)
def swap7H : Instr := .op (.Swap ⟨6, by decide⟩)
def swap8H : Instr := .op (.Swap ⟨7, by decide⟩)
...
/-- 63-instruction window: 54 reachable + 9 `STOP`. Preserves later PCs. -/
...
def workingStack (left right : Compression.EvmWorking)
  ...
  [ right.a, right.b, right.c, right.d, right.e,
    mask, factor, left.b, left.c, left.d, left.e, left.a, ret ] ++ rest
...
@[simp] theorem quadTailWindow_length : quadTailWindow.length = 63 := by
  rfl
