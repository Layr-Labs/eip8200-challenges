...
def prefixPath : List Located :=
  [⟨652, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨653, .push ⟨2, by decide⟩ (UInt256.ofNat 0x3be), by rfl, by decide⟩,
   ⟨654, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨655, .push ⟨2, by decide⟩ (UInt256.ofNat 0x10e2), by rfl, by decide⟩,
   ⟨656, .op .JUMP, by rfl, wfOp (by decide) trivial rfl⟩]

def exitPath : List Located :=
  [⟨658, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨659, .push ⟨5, by decide⟩ QuadRoundTemplate.factor, by rfl, by decide⟩,
   ⟨660, .push ⟨4, by decide⟩ MaskProjection.mask, by rfl, by decide⟩]
...
def loadSite987 : GenericRoundSite Artifact.submissionArtifact .Osaka
  ...
  StackSiteBuilder.ofSlice (artifact := Artifact.submissionArtifact) (fork := .Osaka)
    StackLoadTrace.loadTemplate 661 (by rfl) (by decide)
...
def loadSite1238 : GenericRoundSite Artifact.submissionArtifact .Osaka
    StackLoadTrace.loadTemplate :=
  StackSiteBuilder.ofSlice (artifact := Artifact.submissionArtifact) (fork := .Osaka)
    StackLoadTrace.loadTemplate 917 (by rfl) (by decide)
    QuadLayout.code_bound
    (StackRoundData.templateWellFormed_mem
      (instructions := StackLoadTrace.loadTemplate) (by decide))
    (by simp [StackLoadTrace.loadTemplate])

@[simp] theorem loadSite987_startPC : loadSite987.startPC = UInt256.ofNat 0x3ca := by
  rfl

@[simp] theorem loadSite1238_startPC : loadSite1238.startPC = UInt256.ofNat 0x787 := by
  rfl

def frameRest (input : ByteArray) (i : Nat) : List UInt256 :=
  UInt256.ofNat 0x2d1 :: StackBlockModel.driverRest input i
...
theorem run_prefix (s : State) (input : ByteArray) (i : Nat)
    ...
    Stepper.runLocatedBlock prefixPath (DriverTrace.compressEntry s input i) =
      some (DenseScheduleTemplate.scheduleEntry s
        ...
        (DriverTrace.messageOffsetWord i) (UInt256.ofNat 0x3be)
        (StackBlockModel.scheduleRest input i)) := by
  have hpc979 : Artifact.submissionArtifact.instructionPC 652 = 0x3b4 := by rfl
  have hpc980 : Artifact.submissionArtifact.instructionPC 653 = 0x3b5 := by rfl
  have hpc981 : Artifact.submissionArtifact.instructionPC 654 = 0x3b8 := by rfl
  have hpc982 : Artifact.submissionArtifact.instructionPC 655 = 0x3b9 := by rfl
  have hpc983 : Artifact.submissionArtifact.instructionPC 656 = 0x3bc := by rfl
  have hdest12ac : Decode.isValidJumpDest submissionBytecode 0x10e2 = true :=
    Artifact.submissionArtifact.isValidJumpDest_index 2373 (by rfl)
...
theorem run_exit (s : State) (input : ByteArray) (i : Nat)
    ...
    Stepper.runLocatedBlock exitPath
        (Schedule.scheduleReturned (StackBlockModel.scheduledState s input i)
          (UInt256.ofNat 0x3be) (StackBlockModel.scheduleRest input i)) =
      some (frameLoadEntry s input i) := by
  have hpc985 : Artifact.submissionArtifact.instructionPC 658 = 0x3be := by rfl
  have hpc986 : Artifact.submissionArtifact.instructionPC 659 = 0x3bf := by rfl
  have hpc987 : Artifact.submissionArtifact.instructionPC 660 = 0x3c5 := by rfl
...
def gasSteps_schedule (s : State) (input : ByteArray) (i : Nat)
    ...
    GasSteps
        ...
        (Schedule.scheduleReturned (StackBlockModel.scheduledState s input i)
          (UInt256.ofNat 0x3be) (StackBlockModel.scheduleRest input i)) := by
  let rest := StackBlockModel.scheduleRest input i
  have hstate :
      Schedule.scheduleReturned
          (DenseScheduleTemplate.denseExpectedState s
            PackedScheduleSite.packedScheduleSite.startPC
            (DriverTrace.messageOffsetWord i) (UInt256.ofNat 0x3be) rest)
          (UInt256.ofNat 0x3be) rest =
        Schedule.scheduleReturned (StackBlockModel.scheduledState s input i)
          (UInt256.ofNat 0x3be) rest := by
  ...
  have hraw :
      StackRoundTrace.runInstrSeq DenseScheduleTemplate.denseBeforeJumpTemplate
        ...
        some (DenseScheduleTemplate.denseExpectedState s
          PackedScheduleSite.packedScheduleSite.startPC
          (DriverTrace.messageOffsetWord i) (UInt256.ofNat 0x3be) rest) := by
  ...
  have hvalid :
      Decode.isValidJumpDest s.executionEnv.code
        (UInt256.ofNat 0x3be).toNat = true := by
    rw [hcode]
    exact Artifact.submissionArtifact.isValidJumpDest_index 658 (by rfl)
...
def gasSteps_exit (s : State) (input : ByteArray) (i : Nat)
  ...
  have hqcode :
      (Schedule.scheduleReturned (StackBlockModel.scheduledState s input i)
        (UInt256.ofNat 0x3be) (StackBlockModel.scheduleRest input i)).executionEnv.code =
        submissionBytecode := by
    simp [StackBlockModel.scheduledState, Schedule.scheduleReturned, hcode]
  have hqfork :
      (Schedule.scheduleReturned (StackBlockModel.scheduledState s input i)
        (UInt256.ofNat 0x3be) (StackBlockModel.scheduleRest input i)).fork = .Osaka := by
    simpa [StackBlockModel.scheduledState, Schedule.scheduleReturned, State.fork] using hfork
  have hqrun :
      (Schedule.scheduleReturned (StackBlockModel.scheduledState s input i)
        (UInt256.ofNat 0x3be) (StackBlockModel.scheduleRest input i)).halt = .Running := by
    simp [StackBlockModel.scheduledState, Schedule.scheduleReturned, hrun]
  have hqnp :
      Precompile.isPrecompileWithConfig
          (Schedule.scheduleReturned (StackBlockModel.scheduledState s input i)
            (UInt256.ofNat 0x3be) (StackBlockModel.scheduleRest input i)).executionEnv.precompileConfig
          (Schedule.scheduleReturned (StackBlockModel.scheduledState s input i)
            (UInt256.ofNat 0x3be) (StackBlockModel.scheduleRest input i)).executionEnv.fork
          (Schedule.scheduleReturned (StackBlockModel.scheduledState s input i)
            (UInt256.ofNat 0x3be) (StackBlockModel.scheduleRest input i)).executionEnv.codeAddr = false := by
...
def routeReturned (s : State) (left : Compression.EvmWorking)
    (rest : List UInt256) : State :=
  StackLoadTrace.loadEntry s (UInt256.ofNat 0x787)
    (MaskProjection.mask :: QuadRoundTemplate.factor :: (savedLeft left ++ rest))

def routePath : List Located :=
  [⟨911, .op (.Swap ⟨5, by decide⟩), by rfl,
    wfOp (by decide) trivial rfl⟩,
   ⟨912, .op (.Swap ⟨0, by decide⟩), by rfl,
    wfOp (by decide) trivial rfl⟩,
   ⟨913, .op (.Swap ⟨1, by decide⟩), by rfl,
    wfOp (by decide) trivial rfl⟩,
   ⟨914, .op (.Swap ⟨2, by decide⟩), by rfl,
    wfOp (by decide) trivial rfl⟩,
   ⟨915, .op (.Swap ⟨3, by decide⟩), by rfl,
    wfOp (by decide) trivial rfl⟩,
   ⟨916, .op (.Swap ⟨4, by decide⟩), by rfl,
    wfOp (by decide) trivial rfl⟩]
...
theorem run_route (s : State) (left : Compression.EvmWorking)
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 911 = 0x781 := by rfl
  have hleft : (QuadLayout.leftPC 20).toNat = 0x781 := by rfl
  have hleft1 : (QuadLayout.leftPC 20).succ.toNat = 0x782 := by rfl
  have hleft2 : (QuadLayout.leftPC 20).succ.succ.toNat = 0x783 := by rfl
  have hleft3 : (QuadLayout.leftPC 20).succ.succ.succ.toNat = 0x784 := by rfl
  have hleft4 : (QuadLayout.leftPC 20).succ.succ.succ.succ.toNat = 0x785 := by rfl
  have hleft5 : (QuadLayout.leftPC 20).succ.succ.succ.succ.succ.toNat = 0x786 := by rfl
  have hleft6 : (QuadLayout.leftPC 20).succ.succ.succ.succ.succ.succ =
      UInt256.ofNat 0x787 := by rfl
  have hpc1173 : Artifact.submissionArtifact.instructionPC 912 = 0x782 := by rfl
  have hpc1174 : Artifact.submissionArtifact.instructionPC 913 = 0x783 := by rfl
  have hpc1175 : Artifact.submissionArtifact.instructionPC 914 = 0x784 := by rfl
  have hpc1176 : Artifact.submissionArtifact.instructionPC 915 = 0x785 := by rfl
  have hpc1177 : Artifact.submissionArtifact.instructionPC 916 = 0x786 := by rfl
