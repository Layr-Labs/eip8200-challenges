...
(index 273, pc 0x1fe) and ends at pc 0x2c4, where the mask and factor pushes
follow directly.  The raw suffix is the two-word driver stack; the preserved
outer return word 0x1bb heads `frameRest`.  The historical model return ghost
...
/-- N+L exit block: the mask push (index 324, pc 0x2c4) and the factor push
(index 325, pc 0x2c9).  The removed call prefix and the removed inner-return
JUMPDEST are no longer live instructions. -/
def exitPath : List Located :=
  [⟨324, .push ⟨4, by decide⟩ mask, by rfl, by decide⟩,
   ⟨325, .push ⟨5, by decide⟩ QuadRoundTemplate.factor, by rfl, by decide⟩]
...
def loadSite987 : GenericRoundSite Artifact.submissionArtifact .Osaka
  ...
  StackSiteBuilder.ofSlice (artifact := Artifact.submissionArtifact) (fork := .Osaka)
    StackLoadTrace.loadTemplate 326 (by rfl) (by decide)
...
@[simp] theorem loadSite987_startPC : loadSite987.startPC = UInt256.ofNat 0x2cf := by
  rfl

@[simp] theorem loadSite1238_startPC :
    loadSite1238.startPC = UInt256.ofNat 0xbac := by
...
def frameRest (input : ByteArray) (i : Nat) : List UInt256 :=
  UInt256.ofNat 0x1bb :: StackBlockModel.driverRest input i

/-- The frame the two exit pushes run from: the schedule body ends at pc 0x2c4
...
relocated site start 0x1fe, the message pointer, the preserved outer return
0x1bb, and the two-word driver suffix.  No prefix instructions run. -/
theorem frameEntry_eq_scheduleEntry (s : State) (input : ByteArray) (i : Nat) :
    DriverTrace.compressEntry s input i =
      DenseScheduleTemplate.scheduleEntry s
        ...
        (DriverTrace.messageOffsetWord i) (UInt256.ofNat 0x1bb)
...
sides by pc 0x2c4 and frameRest, without changing the pure loopState model. -/
theorem denseEnd_eq_frameSeam (s : State) (input : ByteArray) (i : Nat) :
  ...
  have hpc : PackedScheduleSite.packedScheduleSite.startPC = UInt256.ofNat 510 :=
  ...
  have h := DenseScheduleState.returned_eq_schedule_with_memory_active s
    (UInt256.ofNat 510) mo (UInt256.ofNat 1093) (UInt256.ofNat 0x424 :: dr)
    (DenseScheduleTemplate.denseExpectedMemory s mo) rfl
  have hu := congrArg (fun t : State =>
    { t with pc := UInt256.ofNat 708, stack := UInt256.ofNat 0x1bb :: dr }) h
  rw [hpc]
  show DenseScheduleTemplate.denseExpectedState s (UInt256.ofNat 510)
      mo (UInt256.ofNat 0x1bb) dr = frameSeam s input i
  calc DenseScheduleTemplate.denseExpectedState s (UInt256.ofNat 510)
        mo (UInt256.ofNat 0x1bb) dr =
      { Schedule.scheduleReturned
          (DenseScheduleTemplate.denseExpectedState s (UInt256.ofNat 510)
            mo (UInt256.ofNat 1093) (UInt256.ofNat 0x424 :: dr))
          (UInt256.ofNat 1093) (UInt256.ofNat 0x424 :: dr) with
        pc := UInt256.ofNat 708, stack := UInt256.ofNat 0x1bb :: dr } := by
    ...
    _ = { Schedule.scheduleReturned
            ...
            (UInt256.ofNat 1093) (UInt256.ofNat 0x424 :: dr) with
          pc := UInt256.ofNat 708, stack := UInt256.ofNat 0x1bb :: dr } := hu
...
theorem run_exit (s : State) (input : ByteArray) (i : Nat)
  ...
  have hpc940 : Artifact.submissionArtifact.instructionPC 324 = 0x2c4 := by rfl
  have hpc941 : Artifact.submissionArtifact.instructionPC 325 = 0x2c9 := by rfl
...
def gasSteps_schedule (s : State) (input : ByteArray) (i : Nat)
  ...
  have hraw :
      StackRoundTrace.runInstrSeq DenseScheduleTemplate.denseBeforeJumpTemplate
        (DenseScheduleTemplate.scheduleEntry s
          PackedScheduleSite.packedScheduleSite.startPC
          (DriverTrace.messageOffsetWord i) (UInt256.ofNat 0x1bb) rest) =
        some (DenseScheduleTemplate.denseExpectedState s
          PackedScheduleSite.packedScheduleSite.startPC
          (DriverTrace.messageOffsetWord i) (UInt256.ofNat 0x1bb) rest) := by
...
theorem run_route (s : State) (left : Compression.EvmWorking)
  ...
  have hpc : Artifact.submissionArtifact.instructionPC
      QuadLayout.routeIndex = 0xbab := QuadLayout.route_pc
