...
(index 283, pc 0x202) and ends at pc 0x2c0, where the mask and factor pushes
...
/-- The frame the two exit pushes run from: the schedule body ends at pc 0x2c0
...
def frameSeam (s : State) (input : ByteArray) (i : Nat) : State :=
  { StackBlockModel.scheduledState s input i with
    pc := UInt256.ofNat 0x2c0, stack := frameRest input i }
...
relocated site start 0x202, the message pointer, the preserved outer return
...
sides by pc 0x2c0 and frameRest, without changing the pure loopState model. -/
theorem denseEnd_eq_frameSeam (s : State) (input : ByteArray) (i : Nat) :
  ...
  have hu := congrArg (fun t : State =>
    { t with pc := UInt256.ofNat 704, stack := UInt256.ofNat 0x1bf :: dr }) h
  rw [hpc]
  ...
  calc DenseScheduleTemplate.denseExpectedState s (UInt256.ofNat 514)
      ...
      { Schedule.scheduleReturned
        ...
        pc := UInt256.ofNat 704, stack := UInt256.ofNat 0x1bf :: dr } := by
    ...
    _ = { Schedule.scheduleReturned
            ...
            (UInt256.ofNat 1093) (UInt256.ofNat 0x424 :: dr) with
          pc := UInt256.ofNat 704, stack := UInt256.ofNat 0x1bf :: dr } := hu
