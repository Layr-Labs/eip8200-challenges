...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.QuadFallthroughSitesLeft
...
def helperSite (group : Fin 5) :
  ...
  StackSiteBuilder.ofSlice _ (leftHelperStartIndex group.val)
    (helper_slice group)
...
private theorem helper_start (group : Fin 5) :
    (helperSite group).startPC = leftHelperPCOfGroup group.val := by
  fin_cases group <;> rfl
...
private theorem helper_end (group : Fin 5) :
  ...
  calc
    (helperJump group).pc =
        UInt256.ofNat (Artifact.instructionPC (leftHelperJumpIndex group.val)) := rfl
...
def fallthroughRound (group : Fin 5) :
    QuadFallthroughTrace.RoundSite Artifact .Osaka group.val
      (leftAddress0 (fallthroughK group)) (leftAddress1 (fallthroughK group))
      (leftAddress2 (fallthroughK group)) (leftAddress3 (fallthroughK group))
      (leftRotation0 (fallthroughK group)) (leftRotation1 (fallthroughK group))
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.QuadFallthroughSitesLeft
