...
private theorem leftWrapper_fits (k : Fin 20) :
  ...
  change 671 + 12 * k.val + 12 ≤ Artifact.submissionInstructions.length
...
private theorem leftCall_fits (k : Fin 20) :
    leftWrapperIndex k.val + (leftCallTemplate k).length ≤
      Artifact.instructions.length := by
  change 671 + 12 * k.val + 10 ≤ Artifact.submissionInstructions.length
  rw [Artifact.referenceInstructions_count]
  omega
...
@[simp] theorem leftPC_zero : leftPC 0 = UInt256.ofNat 0x3d9 := by rfl

@[simp] theorem leftPC_end : leftPC 20 = UInt256.ofNat 0x781 := by
...
@[simp] theorem leftStartPC_eq : leftStartPC = UInt256.ofNat 0x3d9 := by rfl

@[simp] theorem leftEndPC_eq : leftEndPC = UInt256.ofNat 0x781 := by
  exact leftPC_end
