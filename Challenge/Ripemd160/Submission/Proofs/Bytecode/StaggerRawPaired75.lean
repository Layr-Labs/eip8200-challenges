...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3615).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3615 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4443 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3615) = UInt256.ofNat 4447
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4443, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4483, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4443) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4443) template = UInt256.ofNat 4483 := by decide
  rw [hend] at hraw
