...
/-- Words above the complete 1418-byte table are unchanged. -/
...
/-- A full load sees fourteen suffix bytes of the previous word and eighteen
suffix bytes of the current word. Earlier stores end strictly before this load. -/
...
/-- Full UInt256 value, with no untracked high bits. -/
theorem read_pair_toNat (memory : ByteArray) (words : Nat → UInt256)
...
theorem read_pair (memory : ByteArray) (words : Nat → UInt256)
    (first count j : Nat) (hfirst : first < j) (hj : j < first + count)
    (hlo : (words j).toNat < 2 ^ 32) (hhi : (words (j - 1)).toNat < 2 ^ 32) :
    MachineState.readWord (storeDescending memory words first count) (18 * j) =
      UInt256.ofNat ((words j).toNat + (words (j - 1)).toNat * 2 ^ 144) := by
  apply Word.word_ext
  rw [read_pair_toNat _ _ _ _ _ hfirst hj hlo hhi, Word.word_toNat_ofNat]
  apply (Nat.mod_eq_of_lt ?_).symm
  norm_num at hlo hhi ⊢
  omega

#print axioms read_pair_toNat
