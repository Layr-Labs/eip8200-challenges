...
def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 672 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1052 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 672) = UInt256.ofNat 1052
...
def gasSteps (s : State) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1052, stack := cache ++ rho}
      {s with pc := UInt256.ofNat 1140, stack := resultStack s.memory rho} := by
  apply Stepper.runLocatedBlock_sound _ _ site.path
  · exact hcode
  · exact hfork
  · have hpc : ({s with pc := UInt256.ofNat 1052, stack := cache ++ rho} : State).pc = site.startPC := site_pc.symm
    ...
    have hraw := run_actual s (UInt256.ofNat 1052) rho hstack hrun hactive
    have hend : pcAfter (UInt256.ofNat 1052) template = UInt256.ofNat 1140 := by decide
    rw [hend] at hraw
    exact hraw
  · exact hrun
  · exact hnp
