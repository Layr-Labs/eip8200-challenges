...
theorem site_pc : site.startPC = UInt256.ofNat 4672 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3772) = UInt256.ofNat 4672
...
def gasSteps_prefix (s : State) (off limit : UInt256) (h : Compression.HashState)
  ...
  have hraw := ScheduledTailBridge.run_current s (UInt256.ofNat 4672) h {q with off := off, limit := limit} rho hstack hrun
  have hend : pcAfter (UInt256.ofNat 4672) template = UInt256.ofNat 4729 := by decide
  rw [hend] at hraw
  exact hraw
