...
theorem site_pc : site.startPC = UInt256.ofNat 4830 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3533) = UInt256.ofNat 4830
...
def gasSteps_prefix (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4830, stack := StaggerPersistentTailRaw.entryStack (StaggerPersistentFrame.bind h {q with off := off, limit := limit}) rho}
      {s with pc := UInt256.ofNat 4879, stack := StaggerPersistentFrame.frame (StaggerPersistentFrame.combine h q) off limit rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 4830, stack := StaggerPersistentTailRaw.entryStack (StaggerPersistentFrame.bind h {q with off := off, limit := limit}) rho}
    {s with pc := UInt256.ofNat 4879, stack := StaggerPersistentFrame.frame (StaggerPersistentFrame.combine h q) off limit rho}
    hcode hfork hrun hnp site_pc.symm advances
  have hraw := ScheduledTailBridge.run_current s (UInt256.ofNat 4830) h {q with off := off, limit := limit} rho hstack hrun
  have hend : pcAfter (UInt256.ofNat 4830) template = UInt256.ofNat 4879 := by decide
  rw [hend] at hraw
  exact hraw
