...
theorem site_pc : site.startPC = UInt256.ofNat 4563 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3668) = UInt256.ofNat 4563
...
def gasSteps_prefix (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4563, stack := StaggerPersistentTailRaw.stack0 (StaggerPersistentFrame.bind h {q with off := off, limit := limit}) rho}
  ...
  apply DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 4563, stack := StaggerPersistentTailRaw.stack0 (StaggerPersistentFrame.bind h {q with off := off, limit := limit}) rho}
  ...
  have hend : pcAfter (UInt256.ofNat 4563) template = UInt256.ofNat 4614 := by decide
  rw [hend] at hraw
  exact hraw
