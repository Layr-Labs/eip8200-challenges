...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3784).take template.length = template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3784 actual_slice
    (by change 3784 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide)) (by decide)

theorem site_pc : site.startPC = UInt256.ofNat 4555 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3784) = UInt256.ofNat 4555
...
def gasSteps_prefix (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4555, stack := StaggerPersistentTailRaw.stack0 (StaggerPersistentFrame.bind h {q with off := off, limit := limit}) rho}
      {s with pc := UInt256.ofNat 4620, stack := StaggerPersistentFrame.frame (StaggerPersistentFrame.combine h q) off limit rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 4555, stack := StaggerPersistentTailRaw.stack0 (StaggerPersistentFrame.bind h {q with off := off, limit := limit}) rho}
    {s with pc := UInt256.ofNat 4620, stack := StaggerPersistentFrame.frame (StaggerPersistentFrame.combine h q) off limit rho}
    hcode hfork hrun hnp site_pc.symm advances
  have hraw := StaggerPersistentFrame.run_tail s (UInt256.ofNat 4555) h {q with off := off, limit := limit} rho hstack hrun
  have hend : pcAfter (UInt256.ofNat 4555) template = UInt256.ofNat 4620 := by decide
  rw [hend] at hraw
  exact hraw
...
theorem jump_slice :
    (Artifact.submissionArtifact.instructions.drop 3844).take jumpTemplate.length = jumpTemplate := by rfl

def jumpSite : GenericRoundSite Artifact.submissionArtifact .Osaka jumpTemplate :=
  StackSiteBuilder.ofSlice jumpTemplate 3844 jump_slice
    (by change 3844 + jumpTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := jumpTemplate) (by decide)) (by decide)

theorem jump_pc : jumpSite.startPC = UInt256.ofNat 4620 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3844) = UInt256.ofNat 4620
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_jump (s : State) (rho : List UInt256) (hstack : rho.length ≤ 1022)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4620, stack := rho}
      {s with pc := UInt256.ofNat 471, stack := rho} := by
  apply PadLift.gasSteps_of_raw jumpSite {s with pc := UInt256.ofNat 4620, stack := rho} _
    hcode hfork hrun hnp jump_pc.symm jump_advances
  exact PadJump.run_template s (UInt256.ofNat 4620) rho 471 hstack hrun (valid_driver s hcode)
