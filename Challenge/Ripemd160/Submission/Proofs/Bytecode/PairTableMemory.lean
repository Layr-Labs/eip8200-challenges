...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTableMemory
...
def storeDescending (memory : ByteArray) (words : Nat → UInt256)
  ...
  | count + 1 => writeWord (storeDescending memory words (first + 1) count)
      (10 * first) (words first)

/-- Words above the complete 802-byte table are unchanged. -/
...
theorem getD_table_outside (memory : ByteArray) (words : Nat → UInt256)
    (address : Nat) (ha : 802 ≤ address) :
    (storeDescending memory words 0 78)[address]?.getD 0 =
      memory[address]?.getD 0 := by
  exact getD_storeDescending_outside _ _ _ _ _ (fun k hk hk' => Or.inr (by omega))

/-- The last fourteen bytes of each pair load survive all earlier stores.
The first four bytes belong to the previous slot; the final ten to this slot. -/
theorem getD_pair (memory : ByteArray) (words : Nat → UInt256)
    (first count j k : Nat) (hfirst : first < j) (hj : j < first + count)
    (hk : k < 14) :
    (storeDescending memory words first count)[10 * j + 18 + k]?.getD 0 =
      if k < 4 then
        (Data.Bytes.natToBytesPadded (words (j - 1)).toNat 32)[28 + k]?.getD 0
      else
        (Data.Bytes.natToBytesPadded (words j).toNat 32)[18 + k]?.getD 0 := by
  induction count generalizing first with
  | zero => omega
  | succ count ih =>
    rw [storeDescending]
    ...
    · have hfirstEq : first = j - 1 := by omega
      by_cases hk4 : k < 4
      · rw [if_pos (by omega), if_pos hk4, hfirstEq]
        congr 2
        omega
      · rw [if_neg (by omega), if_neg hk4]
...
theorem read_pair (memory : ByteArray) (words : Nat → UInt256)
    ...
    (MachineState.readWord (storeDescending memory words first count) (10 * j)).toNat % 2 ^ 112 =
      (words j).toNat + (words (j - 1)).toNat * 2 ^ 80 := by
  rw [show (2 : Nat) ^ 112 = 256 ^ 14 by norm_num, readWord_mod_pow _ _ _ (by omega)]
  have hp := Bytes.bytesToNatPadded_add (storeDescending memory words first count) (10 * j + 18) 4 10
  change Precompile.bytesToNatPadded (storeDescending memory words first count) (10 * j + 18) 14 = _
  rw [show 14 = 4 + 10 by rfl, hp]
  have hupper : Precompile.bytesToNatPadded (storeDescending memory words first count) (10 * j + 18) 4 =
      Precompile.bytesToNatPadded (Data.Bytes.natToBytesPadded (words (j - 1)).toNat 32) 28 4 := by
    apply bytesToNatPadded_congrOffset
    intro k hk
    rw [getD_pair _ _ _ _ _ _ hfirst hj (by omega), if_pos hk]
  have hlower : Precompile.bytesToNatPadded (storeDescending memory words first count) (10 * j + 18 + 4) 10 =
      Precompile.bytesToNatPadded (Data.Bytes.natToBytesPadded (words j).toNat 32) 22 10 := by
    apply bytesToNatPadded_congrOffset
    intro k hk
    have h := getD_pair memory words first count j (4 + k) hfirst hj (by omega)
    rw [if_neg (by omega : ¬4 + k < 4)] at h
    simpa only [← Nat.add_assoc, show 18 + 4 = 22 by rfl] using h
  rw [hupper, hlower]
  have hupperNat := readWord_mod_pow (Data.Bytes.natToBytesPadded (words (j - 1)).toNat 32) 0 4 (by omega)
  have hlowerNat := readWord_mod_pow (Data.Bytes.natToBytesPadded (words j).toNat 32) 0 10 (by omega)
  rw [readWord_encoded] at hupperNat hlowerNat
  norm_num only at hupperNat hlowerNat
  rw [← hupperNat, ← hlowerNat]
  rw [Nat.mod_eq_of_lt (by simpa using hhi), Nat.mod_eq_of_lt (by omega)]
  norm_num only
  omega
...
theorem storeDescending_size (memory : ByteArray) (words : Nat → UInt256)
    ...
    (storeDescending memory words first count).size =
      max memory.size (10 * (first + count - 1) + 32) := by
  induction count generalizing first with
  | zero => omega
  | succ count ih =>
...
theorem getD_storeDescending_inside (a b : ByteArray) (words : Nat → UInt256)
    ...
    (hlo : 10 * first ≤ address) (hhi : address < 10 * (first + count - 1) + 32) :
  ...
  | succ count ih =>
    ...
    simp only [writeWord, MachineState.writeBytes_getElem?_getD,
      YulEvmCompiler.BytesLemmas.natToBytesPadded_size]
    by_cases hin : 10 * first ≤ address ∧ address < 10 * first + 32
...
theorem read_table_outside (memory : ByteArray) (words : Nat → UInt256)
    (address : Nat) (ha : 802 ≤ address) :
    MachineState.readWord (storeDescending memory words 0 78) address =
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTableMemory
