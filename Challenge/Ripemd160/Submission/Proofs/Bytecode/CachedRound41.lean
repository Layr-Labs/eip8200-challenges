...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2347).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2347 actual_slice
    (by change 2347 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2930 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2347) = UInt256.ofNat 2930
