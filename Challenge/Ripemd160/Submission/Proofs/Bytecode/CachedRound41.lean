...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2336).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2336 actual_slice
    (by change 2336 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2938 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2336) = UInt256.ofNat 2938
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 2828, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 2874, stack := outputStack s.memory x rho} := by
