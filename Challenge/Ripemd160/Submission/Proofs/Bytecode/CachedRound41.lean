...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2233).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2233 actual_slice
    (by change 2233 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2772 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2233) = UInt256.ofNat 2772
