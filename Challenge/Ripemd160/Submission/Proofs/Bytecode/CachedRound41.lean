...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2338).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2338 actual_slice
    (by change 2338 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2932 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2338) = UInt256.ofNat 2932
