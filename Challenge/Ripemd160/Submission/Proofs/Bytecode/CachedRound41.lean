...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2343).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2343 actual_slice
    (by change 2343 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2937 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2343) = UInt256.ofNat 2937
