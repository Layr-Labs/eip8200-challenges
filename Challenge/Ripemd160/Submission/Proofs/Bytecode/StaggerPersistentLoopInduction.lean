...
def postState (input : ByteArray) (s : State) (h : Compression.HashState) (i _count : Nat) (rho : List UInt256) : State :=
  {s with pc := UInt256.ofNat 4579, stack := StaggerPersistentFrame.frame h (offsetWord i) (LoopCompletionControl.limit input) rho}

def exitState (input : ByteArray) (s : State) (h : Compression.HashState) (count : Nat) (rho : List UInt256) : State :=
  {s with pc := UInt256.ofNat 4598, stack := StaggerPersistentFrame.frame h (limitWord count) (LoopCompletionControl.limit input) rho}
