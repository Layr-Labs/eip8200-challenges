...
def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 217 slice
    ...
    (by decide)
theorem pc : site.startPC = UInt256.ofNat 4704 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 217) = UInt256.ofNat 4704
...
def gasSteps (s : State) (ret : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4704, stack := ret :: rho}
      {s with pc := ret, stack := rho} := by
  apply PadLift.gasSteps_of_raw site {s with pc := UInt256.ofNat 4704, stack := ret :: rho}
