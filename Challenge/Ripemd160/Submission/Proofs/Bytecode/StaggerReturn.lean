...
theorem slice : (Artifact.submissionArtifact.instructions.drop 216).take template.length = template := by rfl
def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 216 slice
...
theorem pc : site.startPC = UInt256.ofNat 4689 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 216) = UInt256.ofNat 4689
