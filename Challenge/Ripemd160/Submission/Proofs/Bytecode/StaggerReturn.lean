...
theorem slice : (Artifact.submissionArtifact.instructions.drop 3863).take template.length = template := by rfl
def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3863 slice
    (by change 3863 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem pc : site.startPC = UInt256.ofNat 4677 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3863) = UInt256.ofNat 4677
...
def gasSteps (s : State) (ret : UInt256) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4677, stack := ret :: rho}
      {s with pc := ret, stack := rho} := by
  apply PadLift.gasSteps_of_raw site {s with pc := UInt256.ofNat 4677, stack := ret :: rho}
