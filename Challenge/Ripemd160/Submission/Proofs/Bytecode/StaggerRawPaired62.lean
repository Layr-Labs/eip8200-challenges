...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3123).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3123 actual_slice
    (by change 3123 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3982 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3123) = UInt256.ofNat 3982
