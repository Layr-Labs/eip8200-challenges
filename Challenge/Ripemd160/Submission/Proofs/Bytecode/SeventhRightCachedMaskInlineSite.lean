...
private theorem template_slice :
    (A.instructions.drop 2849).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2849 template_slice (by
    change 2849 + template.length ≤ Artifact.submissionInstructions.length
