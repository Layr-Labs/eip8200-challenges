...
private theorem template_slice :
    (A.instructions.drop 2664).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2664 template_slice (by
    change 2664 + template.length ≤ Artifact.submissionInstructions.length
