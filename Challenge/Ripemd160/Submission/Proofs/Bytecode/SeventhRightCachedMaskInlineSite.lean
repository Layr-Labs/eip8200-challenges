...
private theorem template_slice :
    (A.instructions.drop 2671).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2671 template_slice (by
    change 2674 + template.length ≤ Artifact.submissionInstructions.length
