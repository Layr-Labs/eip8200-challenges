...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 976).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 976 actual_slice
    (by change 976 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1456 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 976) = UInt256.ofNat 1456
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1456, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1499, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1456) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1456) template = UInt256.ofNat 1499 := by decide
  rw [hend] at hraw
