...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.CompressionRightTrace
...
def gasSteps_rightRoundSetup (s : State)
  ...
  have gp := Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka rightRoundPrefixLocated
  ...
  have gm := Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka rightRoundMiddleLocated
  ...
  have gs := Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka rightRoundSuffixLocated
...
def gasSteps_rightTest_continue (s : State)
  ...
  apply Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka rightTestLocated
...
def gasSteps_rightExit (s : State) (messageOffset returnDest : UInt256)
  ...
  apply Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka rightExitLocated
...
theorem run_rightIncrement (s : State)
  ...
  have hdest : Decode.isValidJumpDest submissionBytecode 721 = true := by decide
  have hc3 : rest.length + 3 < 1024 := by omega
  have hc4 : rest.length + 4 < 1024 := by omega
  have hc5 : rest.length + 5 < 1024 := by omega
...
def gasSteps_rightIncrement (s : State)
  ...
  apply Challenge.EvmProof.Stepper.runLocatedBlock_sound
    Artifact.submissionArtifact .Osaka rightIncrementLocated
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.CompressionRightTrace
