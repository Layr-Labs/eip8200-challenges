...
def gasSteps_left80 (s : State) (word : Nat → UInt32)
  ...
  have firstStep (i : Nat) (hi : i < 18) :
    ...
    · simp only [firstStates, stateAt, StackRoundTrace.roundEntry, k]
      rw [hnext]
  have gf := GasSteps.iterateBounded 18 firstStep
  let lastStates := fun n => stateAt s (QuadSites.leftInlinePC (18 + n))
    (StackCompression.leftRounds word (4 * (18 + n)) working) rho
  have lastStep (i : Nat) (hi : i < 2) :
      GasSteps (lastStates i) (lastStates (i + 1)) := by
    let k : Fin 20 := ⟨18 + i, by omega⟩
    have g := QuadRoundCertificates.gasSteps_leftInlineQuad s word
      (StackCompression.leftRounds word (4 * (18 + i)) working) rho k (by omega)
      hwords hactive hstack hcode hfork hrun hnp
    have hnext : StackCompression.leftRounds word (4 * ((18 + i) + 1)) working =
        QuadRoundCertificates.left4 word k
          (StackCompression.leftRounds word (4 * (18 + i)) working) := by
      rw [QuadSemantic.leftRounds_quad word (18 + i) working]
      rfl
    apply g.cast
    · rfl
    · simp only [lastStates, stateAt, QuadRoundCertificates.stateAt,
        StackRoundTrace.roundEntry, k]
      rw [show 18 + (i + 1) = (18 + i) + 1 by omega, hnext]
  have gl := GasSteps.iterateBounded 2 lastStep
  have ge := QuadSites.left_enter
    (stateAt s 0 (StackCompression.leftRounds word 72 working) rho)
