...
def eval (memory : ByteArray) (q : WordLane) : WordLane :=
  StaggerAlgorithm.step 41 (MachineState.readWord memory 160) q

theorem output_eq (memory : ByteArray) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired41.outputStack memory (input memory q right (StaggerAlgorithm.physicalKey 40)) rho =
      stack memory [ .d, .pair, .upper, .k, .b, .a, .e, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500, .cache 230 ] (eval memory q) right (StaggerAlgorithm.physicalKey 41) rho := by
  have hkb : StaggerAlgorithm.physicalKey 40 = UInt256.ofNat 7411972540165114580685751011896225 := by decide
  have hka : StaggerAlgorithm.physicalKey 41 = UInt256.ofNat 7411972540165114580685751011896225 := by decide
...
def gasSteps (s : State) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2925, stack := stack s.memory [ .d, .pair, .upper, .k, .c, .e, .a, .b, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500, .cache 230 ] q right (StaggerAlgorithm.physicalKey 40) rho}
      {s with pc := UInt256.ofNat 2965, stack := stack s.memory [ .d, .pair, .upper, .k, .b, .a, .e, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500, .cache 230 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 41) rho} := by
  have g := StaggerRawPaired41.gasSteps s (input s.memory q right (StaggerAlgorithm.physicalKey 40))
