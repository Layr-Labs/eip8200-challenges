import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
   StackSiteBuilder.ofSlice template 3608 actual_slice
     ...
     (by decide)
 theorem site_pc : site.startPC = UInt256.ofNat 4285 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3608) = UInt256.ofNat 4285
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 4285, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 4328, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 4285) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 4285) template = UInt256.ofNat 4328 := by decide
   rw [hend] at hraw
