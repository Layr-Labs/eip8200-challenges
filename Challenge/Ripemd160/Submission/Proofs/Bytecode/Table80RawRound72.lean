import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 theorem actual_slice :
     (Artifact.submissionArtifact.instructions.drop 3604).take template.length = template := by rfl
 ...
 theorem site_pc : site.startPC = UInt256.ofNat 4287 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3604) = UInt256.ofNat 4287
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 4287, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 4330, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 4287) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 4287) template = UInt256.ofNat 4330 := by decide
   rw [hend] at hraw
