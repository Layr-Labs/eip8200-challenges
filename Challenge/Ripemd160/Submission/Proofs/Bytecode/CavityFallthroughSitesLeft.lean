...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.CavityFallthroughSitesLeft
...
private theorem normal_wellFormed (k : NormalIndex) :
    ∀ instruction ∈ QuadCallTrace.quadCallPushes
...
private theorem helper_slice (group : Fin 4) :
    (Artifact.instructions.drop (leftHelperStartIndex (group.val + 1))).take
        (leftHelperTemplate ⟨group.val + 1, by omega⟩).length = leftHelperTemplate ⟨group.val + 1, by omega⟩ := by
  fin_cases group <;> rfl

def helperSite (group : Fin 4) :
    GenericRoundSite Artifact .Osaka (leftHelperTemplate ⟨group.val + 1, by omega⟩) :=
  StackSiteBuilder.ofSlice _ (leftHelperStartIndex (group.val + 1))
    (helper_slice group)
...
def helperJump (group : Fin 4) : LocatedSite Artifact .Osaka where
  located :=
    { index := leftHelperJumpIndex (group.val + 1)
      instruction := .op .JUMP
      atIndex := by fin_cases group <;> rfl
      wellFormed := ⟨by decide, trivial, rfl⟩ }
  pc := UInt256.ofNat (Artifact.instructionPC (leftHelperJumpIndex (group.val + 1)))
  pc_eq := QuadLayout.pc_toNat_instructionPC _

private theorem helper_start (group : Fin 4) :
    (helperSite group).startPC = leftHelperPCOfGroup (group.val + 1) := by
  fin_cases group <;> rfl

private theorem helper_end (group : Fin 4) :
  ...
  calc
    (helperJump group).pc =
        UInt256.ofNat (Artifact.instructionPC (leftHelperJumpIndex (group.val + 1))) := rfl
    _ = StackRoundTrace.pcAfter (leftHelperPCOfGroup (group.val + 1))
        (leftHelperTemplate ⟨group.val + 1, by omega⟩) := by
      fin_cases group <;> rfl
    _ = (helperSite group).endPC := hend.symm

private theorem helper_valid (group : Fin 4) :
    Decode.isValidJumpDest Artifact.code
      (leftHelperPCOfGroup (group.val + 1)).toNat = true := by
  have hpc : (leftHelperPCOfGroup (group.val + 1)).toNat =
      Artifact.instructionPC (leftHelperStartIndex (group.val + 1)) := by
    fin_cases group <;> rfl
  rw [hpc]
  exact Artifact.isValidJumpDest_index (leftHelperStartIndex (group.val + 1))
...
def normalRound (k : NormalIndex) :
    QuadHelperTrace.RoundSite Artifact .Osaka
      ...
      (leftConstant (normalFin k)) where
...
def fallthroughK (group : Fin 4) : Fin 20 :=
  ⟨4 * (group.val + 1) + 3, by omega⟩

private theorem fallthrough_slice (group : Fin 4) :
    (Artifact.instructions.drop (leftWrapperIndex (fallthroughK group).val)).take
        (QuadFallthroughTrace.pushes
      ...
      QuadFallthroughTrace.pushes (leftReturnPC (fallthroughK group).val)
        (leftAddress0 (fallthroughK group)) (leftAddress1 (fallthroughK group))
        (leftAddress2 (fallthroughK group)) (leftAddress3 (fallthroughK group))
        (leftRotation0 (fallthroughK group)) (leftRotation1 (fallthroughK group))
        (leftRotation2 (fallthroughK group))
...
def fallthroughPushes (group : Fin 4) : GenericRoundSite Artifact .Osaka
    (QuadFallthroughTrace.pushes (leftReturnPC (fallthroughK group).val)
      (leftAddress0 (fallthroughK group)) (leftAddress1 (fallthroughK group))
      (leftAddress2 (fallthroughK group)) (leftAddress3 (fallthroughK group))
      (leftRotation0 (fallthroughK group)) (leftRotation1 (fallthroughK group))
  ...
  StackSiteBuilder.ofSlice _ (leftWrapperIndex (fallthroughK group).val)
    ...
    QuadLayout.code_bound
    (StackRoundData.templateWellFormed_mem (by fin_cases group <;> decide))
    (by simp [QuadFallthroughTrace.pushes])

def fallthroughReturn (group : Fin 4) : LocatedSite Artifact .Osaka where
  located :=
...
private theorem fallthrough_helper_start (group : Fin 4) :
    (helperSite group).startPC = (fallthroughPushes group).endPC := by
  fin_cases group <;> rfl

private theorem fallthrough_return_valid (group : Fin 4) :
    Decode.isValidJumpDest Artifact.code
...
def fallthroughRound (group : Fin 4) :
    QuadFallthroughTrace.RoundSite Artifact .Osaka (group.val + 1)
      (leftAddress0 (fallthroughK group)) (leftAddress1 (fallthroughK group))
      (leftAddress2 (fallthroughK group)) (leftAddress3 (fallthroughK group))
      (leftRotation0 (fallthroughK group)) (leftRotation1 (fallthroughK group))
      (leftRotation2 (fallthroughK group)) (leftRotation3 (fallthroughK group))
      (leftConstant (fallthroughK group)) where
...
theorem fallthrough_start (group : Fin 4) :
    (fallthroughRound group).callPushes.startPC =
      leftPC (fallthroughK group).val := rfl

theorem fallthrough_end (group : Fin 4) :
    (fallthroughRound group).returnSite.pc.succ =
      leftPC ((fallthroughK group).val + 1) := by
  fin_cases group <;> rfl

end Challenge.Ripemd160.Submission.Proofs.Bytecode.CavityFallthroughSitesLeft
