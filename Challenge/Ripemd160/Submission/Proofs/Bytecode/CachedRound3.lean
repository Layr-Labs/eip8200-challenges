...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 605).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 605 actual_slice
    (by change 605 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 878 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 605) = UInt256.ofNat 878
