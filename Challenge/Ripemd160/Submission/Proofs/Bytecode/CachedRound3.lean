...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 597).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 597 actual_slice
    (by change 597 + actualTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualTemplate) (by decide))
    (by decide)
theorem actual_pc : actualSite.startPC = UInt256.ofNat 866 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 597) = UInt256.ofNat 866
...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v7 (UInt256.shiftRight (UInt256.mul x.v6 x.v2) (UInt256.ofNat 22))),
    x.v1,
    (UInt256.land x.v7 (UInt256.add x.v4 (UInt256.shiftRight (UInt256.mul x.v6 (UInt256.add (UInt256.land x.v9 (UInt256.land x.v7 (UInt256.add x.v1 (UInt256.add (UInt256.lor (MachineState.readWord memory 16) (MachineState.readWord memory 288)) (UInt256.add (UInt256.xor x.v3 (UInt256.xor x.v2 (UInt256.xor x.v8 (UInt256.lor (UInt256.land x.v8 x.v2) x.v0)))) x.v5))))) (UInt256.land x.v7 (UInt256.add x.v1 (UInt256.add (UInt256.lor (MachineState.readWord memory 16) (MachineState.readWord memory 288)) (UInt256.add (UInt256.xor x.v3 (UInt256.xor x.v2 (UInt256.xor x.v8 (UInt256.lor (UInt256.land x.v8 x.v2) x.v0)))) x.v5)))))) (UInt256.ofNat 21)))),
    x.v3,
    x.v4,
    x.v0,
    x.v6,
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 885, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 932, stack := outputStack s.memory x rho} := by
...
theorem run_actual (s : State) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 996) (hrun : s.halt = .Running)
    (hactive : 23 ≤ s.activeWords.toNat) :
    runInstrSeq actualTemplate {s with pc := UInt256.ofNat 866, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 910, stack := outputStack s.memory x rho} := by
  have hcap (n : Nat) (hn : n ≤ 26) : rho.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (haddress : address ≤ 704) :
      UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat address 32) = s.activeWords :=
    active_schedule_preserved s.activeWords address hactive haddress
...
def gasSteps (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 866, stack := coreStack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] f (cache s.memory ++ rho)}
      {s with pc := UInt256.ofNat 910, stack := coreStack [.d, .k, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (PairedAllInlineCoreTrace.inline3Block.eval s.memory f) (cache s.memory ++ rho)} := by
  ...
  have g := gasSteps_terminal_of_raw actualSite
    {s with pc := UInt256.ofNat 866, stack := inputStack s.memory x rho} _
