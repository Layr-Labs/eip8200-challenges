...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2909).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2909 actual_slice
    (by change 2909 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3755 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2909) = UInt256.ofNat 3755
