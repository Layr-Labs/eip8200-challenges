...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRaw18
...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v3 (UInt256.shiftRight (UInt256.mul x.v10 x.v9) (UInt256.ofNat 28))),
    ...
    x.v7,
    x.v8,
    (UInt256.land x.v3 (UInt256.add x.v7 (UInt256.shiftRight (UInt256.mul x.v10 (UInt256.land x.v3 (UInt256.add x.v8 (UInt256.add (MachineState.readWord memory 198) (UInt256.add (UInt256.lor (UInt256.xor x.v0 (UInt256.land (UInt256.xor x.v4 x.v6) (UInt256.lor x.v0 x.v9))) (UInt256.land x.v9 x.v6)) x.v5))))) (UInt256.ofNat 30)))),
    x.v10,
    x.v11,
    x.v12,
    x.v13,
...
private theorem run_generated (s : State) (pc : UInt256) (x : Input) (rho : List UInt256)
  ...
  have hz198 : MachineState.readWord s.memory 198 = UInt256.ofNat 0 :=
    PadPrefixContract.read_zero s hz ⟨11, by decide⟩ (by decide)
  ...
  simp (discharger := omega) [template, inputStack, actualOutput, hz198,
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRaw18
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 4658).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 4658 actual_slice
    (by change 4658 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 5859 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4658) = UInt256.ofNat 5859
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 5859, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 5894, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 5859) x rho hstack hrun hactive hz
  have hend : pcAfter (UInt256.ofNat 5859) template = UInt256.ofNat 5894 := by decide
  rw [hend] at hraw
