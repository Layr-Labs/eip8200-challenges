...
def eval (memory : ByteArray) (q : WordLane) : WordLane :=
  StaggerAlgorithm.step 63 (MachineState.readWord memory 432) q
...
theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired63.outputStack memory (input memory h4 q right (StaggerAlgorithm.physicalKey 62)) rho =
      stack memory h4 [ .d, .literal 28, .cachedMessage 450, .pair, .upper, .k, .b, .e, .a, .c, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval memory q) right (StaggerAlgorithm.physicalKey 63) rho := by
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3904, stack := stack s.memory h4 [ .d, .literal 28, .cachedMessage 450, .pair, .upper, .k, .c, .a, .e, .b, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] q right (StaggerAlgorithm.physicalKey 62) rho}
      {s with pc := UInt256.ofNat 3944, stack := stack s.memory h4 [ .d, .literal 28, .cachedMessage 450, .pair, .upper, .k, .b, .e, .a, .c, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 63) rho} := by
