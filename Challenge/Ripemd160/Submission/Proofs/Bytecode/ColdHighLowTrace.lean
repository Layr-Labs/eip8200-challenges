...
noncomputable opaque gasSteps_lowRoute (input : ByteArray) (hfit : CalldataFits input) (hpositive : 0 < input.size)
    ...
    (hlarge : 5234 ≤ input.size) :
    GasSteps
      {states input i with pc:=UInt256.ofNat 4723, stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (LoopCompletionControl.limit input) maskRho}
      {lowState input i with pc:=UInt256.ofNat 4783, stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (UInt256.ofNat input.size) maskRho} := by
  ...
  rw [hcal] at gl
  have gl' : GasSteps {s with pc:=UInt256.ofNat 4724,stack:=frame h off lim maskRho}
      {lowState input i with pc:=UInt256.ofNat 4779, stack:=StaggerPad.highZero (UInt256.ofNat input.size)::frame h off lim maskRho} := gl
  ...
  -- the trampoline `PUSH2 4785 ; JUMP` is gone: the JUMPI at 4784 now FALLS THROUGH into the
  -- JUMPDEST at 4785 that it used to jump to, so `gf` already ends where `gr` used to.
