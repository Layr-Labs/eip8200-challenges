...
noncomputable opaque gasSteps_lowRoute (input : ByteArray) (hfit : CalldataFits input) (hpositive : 0 < input.size)
    ...
    (hlarge : 5200 ≤ input.size) :
    GasSteps
      {states input i with pc:=UInt256.ofNat 4689, stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (LoopCompletionControl.limit input) maskRho}
      {lowState input i with pc:=UInt256.ofNat 4749, stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (UInt256.ofNat input.size) maskRho} := by
  ...
  rw [hcal] at gl
  have gl' : GasSteps {s with pc:=UInt256.ofNat 4690,stack:=frame h off lim maskRho}
      {lowState input i with pc:=UInt256.ofNat 4745, stack:=StaggerPad.highZero (UInt256.ofNat input.size)::frame h off lim maskRho} := gl
