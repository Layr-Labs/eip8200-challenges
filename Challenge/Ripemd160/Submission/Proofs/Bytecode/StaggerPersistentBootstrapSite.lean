...
theorem site_pc : site.startPC = UInt256.ofNat 923 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 562) = UInt256.ofNat 923
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 923, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 934, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 923) x rho hs hr ha
  have hend : pcAfter (UInt256.ofNat 923) template = UInt256.ofNat 934 := by decide
  rw [hend] at hraw
