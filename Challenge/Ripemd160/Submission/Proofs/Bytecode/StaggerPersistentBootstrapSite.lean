...
theorem site_pc : site.startPC = UInt256.ofNat 1079 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 633) = UInt256.ofNat 1079
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1079, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1090, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1079) x rho hs hr ha
  have hend : pcAfter (UInt256.ofNat 1079) template = UInt256.ofNat 1090 := by decide
  rw [hend] at hraw
