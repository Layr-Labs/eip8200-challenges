...
theorem site_pc : site.startPC = UInt256.ofNat 910 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 497) = UInt256.ofNat 882
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 910, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 883, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 882) x rho hs hr ha
  have hend : pcAfter (UInt256.ofNat 910) template = UInt256.ofNat 855 := by decide
  rw [hend] at hraw
