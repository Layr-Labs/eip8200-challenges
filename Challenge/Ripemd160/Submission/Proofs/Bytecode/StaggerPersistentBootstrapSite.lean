...
theorem site_pc : site.startPC = UInt256.ofNat 1092 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 638) = UInt256.ofNat 1092
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1092, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1103, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1092) x rho hs hr ha
  have hend : pcAfter (UInt256.ofNat 1092) template = UInt256.ofNat 1103 := by decide
  rw [hend] at hraw
