...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 574).take template.length = template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 574 actual_slice
    (by change 574 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide)) (by decide)

theorem site_pc : site.startPC = UInt256.ofNat 908 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 574) = UInt256.ofNat 908
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 908, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 919, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 908) x rho hs hr ha
  have hend : pcAfter (UInt256.ofNat 908) template = UInt256.ofNat 919 := by decide
  rw [hend] at hraw
