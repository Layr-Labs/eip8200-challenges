...
theorem site_pc : site.startPC = UInt256.ofNat 896 := by
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 896, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 897, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 896) x rho hs hr ha
  have hend : pcAfter (UInt256.ofNat 896) template = UInt256.ofNat 897 := by decide
  rw [hend] at hraw
