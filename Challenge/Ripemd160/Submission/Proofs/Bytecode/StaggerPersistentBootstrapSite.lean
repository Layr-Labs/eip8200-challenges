...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 481).take template.length = template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 481 actual_slice
    (by change 481 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 860 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 481) = UInt256.ofNat 860
