...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 483).take template.length = template := by rfl

def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 483 actual_slice
    (by change 483 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 860 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 483) = UInt256.ofNat 860
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 860, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 861, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 860) x rho hs hr ha
  have hend : pcAfter (UInt256.ofNat 860) template = UInt256.ofNat 861 := by decide
  rw [hend] at hraw
