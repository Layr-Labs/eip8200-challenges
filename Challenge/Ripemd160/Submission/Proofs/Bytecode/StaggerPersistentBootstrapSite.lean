...
theorem site_pc : site.startPC = UInt256.ofNat 919 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 568) = UInt256.ofNat 919
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 919, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 930, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 919) x rho hs hr ha
  have hend : pcAfter (UInt256.ofNat 919) template = UInt256.ofNat 930 := by decide
  rw [hend] at hraw
