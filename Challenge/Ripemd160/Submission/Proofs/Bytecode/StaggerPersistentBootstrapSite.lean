...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 977, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1068, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 977) x rho hs hr ha
  have hend : pcAfter (UInt256.ofNat 977) template = UInt256.ofNat 1068 := by decide
  rw [hend] at hraw
