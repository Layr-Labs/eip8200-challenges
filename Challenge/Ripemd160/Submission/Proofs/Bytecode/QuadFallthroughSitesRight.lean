...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.QuadFallthroughSitesRight
...
private theorem helper_slice (group : Fin 5) :
...
def helperJump (group : Fin 5) : LocatedSite Artifact .Osaka where
  located :=
...
private theorem helper_start (group : Fin 5) :
    (helperSite group).startPC = rightHelperPCOfGroup group.val := by
  fin_cases group <;> rfl

private theorem helper_end (group : Fin 5) :
...
private theorem helper_valid (group : Fin 5) :
...
def normalRound (k : NormalIndex) :
  ...
  call := normalCall k
  helper := castTemplate (helperSite ⟨(normalFin k).val / 4, by omega⟩) (by rfl)
  helper_start := by rw [castTemplate_start, helper_start]
  helperJump := helperJump ⟨(normalFin k).val / 4, by omega⟩
  helper_jump_instr := rfl
  helper_end := by rw [castTemplate_end, helper_end]
  returnSite := normalReturn k
  return_instr := rfl
  return_at := rfl
  helper_valid := helper_valid ⟨(normalFin k).val / 4, by omega⟩
  return_valid := normalReturn_valid k

def fallthroughK (group : Fin 5) : Fin 20 :=
  ⟨4 * group.val + 3, by omega⟩

private theorem fallthrough_slice (group : Fin 5) :
...
def fallthroughPushes (group : Fin 5) : GenericRoundSite Artifact .Osaka
    (QuadFallthroughTrace.pushes (rightReturnPC (fallthroughK group).val)
      (rightAddress0 (fallthroughK group)) (rightAddress1 (fallthroughK group))
      (rightAddress2 (fallthroughK group)) (rightAddress3 (fallthroughK group))
      (rightRotation0 (fallthroughK group)) (rightRotation1 (fallthroughK group))
...
def fallthroughReturn (group : Fin 5) : LocatedSite Artifact .Osaka where
  located :=
...
private theorem fallthrough_helper_start (group : Fin 5) :
    (helperSite group).startPC = (fallthroughPushes group).endPC := by
  fin_cases group <;> rfl

private theorem fallthrough_return_valid (group : Fin 5) :
    Decode.isValidJumpDest Artifact.code
...
def fallthroughRound (group : Fin 5) :
