...
private theorem left0Second_slice :
    (A.instructions.drop 1096).take ((CachedMaskParams.leftCode 0).drop 399).length =
...
def left0Second : GenericRoundSite A .Osaka ((CachedMaskParams.leftCode 0).drop 399) :=
  StackSiteBuilder.ofSlice _ 1096 left0Second_slice (by
    change 1096 + ((CachedMaskParams.leftCode 0).drop 399).length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound wellFormed_left0Second (by decide)

private theorem left2First_slice :
    (A.instructions.drop 1100).take ((CachedMaskParams.leftCode 2).take 178).length =
...
def left2First : GenericRoundSite A .Osaka ((CachedMaskParams.leftCode 2).take 178) :=
  StackSiteBuilder.ofSlice _ 1100 left2First_slice (by
    change 1100 + ((CachedMaskParams.leftCode 2).take 178).length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound wellFormed_left2First (by decide)

private theorem left2Second_slice :
    (A.instructions.drop 1449).take (((CachedMaskParams.leftCode 2).drop 178).take 210).length =
...
def left2Second : GenericRoundSite A .Osaka (((CachedMaskParams.leftCode 2).drop 178).take 210) :=
  StackSiteBuilder.ofSlice _ 1449 left2Second_slice (by
    change 1449 + (((CachedMaskParams.leftCode 2).drop 178).take 210).length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound wellFormed_left2Second (by decide)

private theorem left2Third_slice :
    (A.instructions.drop 2679).take ((CachedMaskParams.leftCode 2).drop 388).length =
...
def left2Third : GenericRoundSite A .Osaka ((CachedMaskParams.leftCode 2).drop 388) :=
  StackSiteBuilder.ofSlice _ 2679 left2Third_slice (by
    change 2679 + ((CachedMaskParams.leftCode 2).drop 388).length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound wellFormed_left2Third (by decide)

private theorem rightFirst_slice :
    (A.instructions.drop 467).take (CachedMaskParams.rightCode.take 373).length =
...
def rightFirst : GenericRoundSite A .Osaka (CachedMaskParams.rightCode.take 373) :=
  StackSiteBuilder.ofSlice _ 467 rightFirst_slice (by
    change 467 + (CachedMaskParams.rightCode.take 373).length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound wellFormed_rightFirst (by decide)

private theorem rightSecond_slice :
    (A.instructions.drop 2648).take (CachedMaskParams.rightCode.drop 373).length =
...
def rightSecond : GenericRoundSite A .Osaka (CachedMaskParams.rightCode.drop 373) :=
  StackSiteBuilder.ofSlice _ 2648 rightSecond_slice (by
    change 2648 + (CachedMaskParams.rightCode.drop 373).length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
...
def left0Entry : Bridge A .Osaka where
  push := exactSite 1093 (.push 2 (UInt256.ofNat 4)) (by rfl) (by decide)
  jump := exactSite 1094 (.op .JUMP) (by rfl) (wfOp (by decide) trivial rfl)
...
def left0Bridge1 : Bridge A .Osaka where
  ...
  destination := exactSite 1095 (.op .JUMPDEST) (by rfl)
    (wfOp (by decide) trivial rfl)
  push_instr := by
    simp only [exactSite]
    apply congrArg (Instr.push ⟨2, by decide⟩)
...
def left0Return : Bridge A .Osaka where
  push := exactSite 1097 (.push 2 (UInt256.ofNat 1589)) (by rfl) (by decide)
  jump := exactSite 1098 (.op .JUMP) (by rfl) (wfOp (by decide) trivial rfl)
  destination := exactSite 1281 (.op .JUMPDEST) (by rfl)
    (wfOp (by decide) trivial rfl)
  push_instr := by
    simp only [exactSite]
    apply congrArg (Instr.push ⟨2, by decide⟩)
...
def left2Entry : Bridge A .Osaka where
  push := exactSite 1446 (.push 2 (UInt256.ofNat 1352)) (by rfl) (by decide)
  jump := exactSite 1447 (.op .JUMP) (by rfl) (wfOp (by decide) trivial rfl)
  destination := exactSite 1099 (.op .JUMPDEST) (by rfl)
    (wfOp (by decide) trivial rfl)
  push_instr := by
    simp only [exactSite]
    apply congrArg (Instr.push ⟨2, by decide⟩)
...
def left2Bridge1 : Bridge A .Osaka where
  push := exactSite 1278 (.push 2 (UInt256.ofNat 1919)) (by rfl) (by decide)
  jump := exactSite 1279 (.op .JUMP) (by rfl) (wfOp (by decide) trivial rfl)
  destination := exactSite 1448 (.op .JUMPDEST) (by rfl)
    (wfOp (by decide) trivial rfl)
  push_instr := by
    simp only [exactSite]
    apply congrArg (Instr.push ⟨2, by decide⟩)
...
def left2Bridge2 : Bridge A .Osaka where
  push := exactSite 1659 (.push 2 (UInt256.ofNat 4257)) (by rfl) (by decide)
  jump := exactSite 1660 (.op .JUMP) (by rfl) (wfOp (by decide) trivial rfl)
  destination := exactSite 2678 (.op .JUMPDEST) (by rfl)
    (wfOp (by decide) trivial rfl)
  push_instr := by
    simp only [exactSite]
    apply congrArg (Instr.push ⟨2, by decide⟩)
...
def left2Return : Bridge A .Osaka where
  push := exactSite 2739 (.push 2 (UInt256.ofNat 2195)) (by rfl) (by decide)
  jump := exactSite 2740 (.op .JUMP) (by rfl) (wfOp (by decide) trivial rfl)
  destination := exactSite 1661 (.op .JUMPDEST) (by rfl)
    (wfOp (by decide) trivial rfl)
  push_instr := by
    simp only [exactSite]
    apply congrArg (Instr.push ⟨2, by decide⟩)
...
def rightEntry : Bridge A .Osaka where
  push := exactSite 2645 (.push 2 (UInt256.ofNat 561)) (by rfl) (by decide)
  jump := exactSite 2646 (.op .JUMP) (by rfl) (wfOp (by decide) trivial rfl)
  destination := exactSite 466 (.op .JUMPDEST) (by rfl)
    (wfOp (by decide) trivial rfl)
  push_instr := by
    simp only [exactSite]
    apply congrArg (Instr.push ⟨2, by decide⟩)
...
def rightBridge1 : Bridge A .Osaka where
  push := exactSite 840 (.push 2 (UInt256.ofNat 4220)) (by rfl) (by decide)
  jump := exactSite 841 (.op .JUMP) (by rfl) (wfOp (by decide) trivial rfl)
  destination := exactSite 2647 (.op .JUMPDEST) (by rfl)
    (wfOp (by decide) trivial rfl)
  push_instr := by
    simp only [exactSite]
    apply congrArg (Instr.push ⟨2, by decide⟩)
...
def rightReturn : Bridge A .Osaka where
  push := exactSite 2675 (.push 2 (UInt256.ofNat 4339)) (by rfl) (by decide)
  jump := exactSite 2676 (.op .JUMP) (by rfl) (wfOp (by decide) trivial rfl)
  destination := exactSite 2741 (.op .JUMPDEST) (by rfl)
    (wfOp (by decide) trivial rfl)
  push_instr := by
    simp only [exactSite]
    apply congrArg (Instr.push ⟨2, by decide⟩)
