...
private theorem left2First_slice :
    (A.instructions.drop 914).take (CachedMaskParams.leftCode 2).length =
...
def left2First : GenericRoundSite A .Osaka (CachedMaskParams.leftCode 2) :=
  StackSiteBuilder.ofSlice _ 914 left2First_slice (by
    change 914 + (CachedMaskParams.leftCode 2).length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound wellFormed_left2First (by decide)

private theorem rightFirst_slice :
    (A.instructions.drop 3248).take CachedMaskParams.rightCode.length =
...
def rightFirst : GenericRoundSite A .Osaka CachedMaskParams.rightCode :=
  StackSiteBuilder.ofSlice _ 3248 rightFirst_slice (by
    change 3248 + CachedMaskParams.rightCode.length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
    decide) code_bound wellFormed_rightFirst (by decide)

private theorem left4First_slice :
    (A.instructions.drop 1535).take (CachedMaskParams.leftCode 4).length =
...
def left4First : GenericRoundSite A .Osaka (CachedMaskParams.leftCode 4) :=
  StackSiteBuilder.ofSlice _ 1535 left4First_slice (by
    change 1535 + (CachedMaskParams.leftCode 4).length ≤ Artifact.submissionInstructions.length
    rw [Artifact.referenceInstructions_count]
