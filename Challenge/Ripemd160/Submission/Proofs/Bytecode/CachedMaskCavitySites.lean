...
private theorem left0First_slice :
    (A.instructions.drop 329).take (CachedMaskParams.leftCode 0).length =
...
def left0First : GenericRoundSite A .Osaka (CachedMaskParams.leftCode 0) :=
  StackSiteBuilder.ofSlice _ 329 left0First_slice (by
...
private theorem left2First_slice :
    (A.instructions.drop 902).take (CachedMaskParams.leftCode 2).length =
...
def left2First : GenericRoundSite A .Osaka (CachedMaskParams.leftCode 2) :=
  StackSiteBuilder.ofSlice _ 902 left2First_slice (by
...
private theorem rightFirst_slice :
    (A.instructions.drop 3323).take CachedMaskParams.rightCode.length =
...
def rightFirst : GenericRoundSite A .Osaka CachedMaskParams.rightCode :=
  StackSiteBuilder.ofSlice _ 3323 rightFirst_slice (by
...
private theorem left4First_slice :
    (A.instructions.drop 1523).take (CachedMaskParams.leftCode 4).length =
...
def left4First : GenericRoundSite A .Osaka (CachedMaskParams.leftCode 4) :=
  StackSiteBuilder.ofSlice _ 1523 left4First_slice (by
