...
/-- One iteration. Slice files must apply this, not re-prove `run_loop_more`. -/
theorem gasSteps_one (input : ByteArray) (n : Nat) (hn : n < 999)
    (hfit : n < input.size) :
    GasSteps (loopState input n) (loopState input (n + 1)) :=
  sound loopPath (run_loop_more input n hn hfit)

/-- `iterateBounded` of `gasSteps_one`. Bound 29 matches 1000-a. -/
theorem gasSteps_loop_slice (input : ByteArray) (hsize : input.size = 1000)
  ...
  let step : ∀ n, n < len →
    ...
    fun n hn =>
      gasSteps_one input (start + n) (by omega) (by rw [hsize]; omega)
