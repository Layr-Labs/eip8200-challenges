...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 377 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 599 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 377) = UInt256.ofNat 599
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 599, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 678, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 599) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 599) template = UInt256.ofNat 678 := by decide
  rw [hend] at hraw
