...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 343).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 343 actual_slice
    (by change 343 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 573 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 343) = UInt256.ofNat 573
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hraw := run_actual s (UInt256.ofNat 573) x rho hstack halias hrun hactive
  have hend : pcAfter (UInt256.ofNat 573) template = UInt256.ofNat 646 := by decide
  rw [hend] at hraw
