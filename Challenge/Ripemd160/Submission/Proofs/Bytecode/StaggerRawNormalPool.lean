...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 338 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 575 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 338) = UInt256.ofNat 575
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 575, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 648, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 575) x rho hstack halias hrun hactive
  have hend : pcAfter (UInt256.ofNat 575) template = UInt256.ofNat 648 := by decide
  rw [hend] at hraw
