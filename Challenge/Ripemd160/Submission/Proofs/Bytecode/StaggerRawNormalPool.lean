...
theorem site_pc : site.startPC = UInt256.ofNat 558 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 347) = UInt256.ofNat 558
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 558, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 631, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 558) x rho hstack halias hrun hactive
  have hend : pcAfter (UInt256.ofNat 558) template = UInt256.ofNat 631 := by decide
  rw [hend] at hraw
