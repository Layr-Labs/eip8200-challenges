...
def scheduledState (s : State) (input : ByteArray) (i : Nat) : State :=
  { Schedule.loopState s (DriverTrace.messageOffsetWord i) (UInt256.ofNat 0x72f)
      (scheduleRest input i) 16 with
    memory := DenseScheduleTemplate.denseExpectedMemory s
      (DriverTrace.messageOffsetWord i)
    activeWords := DenseScheduleTemplate.denseExpectedActiveWords s
      (DriverTrace.messageOffsetWord i) }
...
theorem scheduledState_activeWords (s : State) (input : ByteArray)
  ...
  rw [scheduledState, DenseScheduleActiveWords.template_eq_expected]
