...
def scheduleRest (input : ByteArray) (_i : Nat) : List UInt256 :=
  [UInt256.ofNat 0x3e0] ++ [Padding.paddedWord input]
...
def scheduledState (s : State) (input : ByteArray) (i : Nat) : State :=
  withActiveWords
    (withMemory
      (Schedule.loopState s (DriverTrace.messageOffsetWord i) (UInt256.ofNat 0x401)
