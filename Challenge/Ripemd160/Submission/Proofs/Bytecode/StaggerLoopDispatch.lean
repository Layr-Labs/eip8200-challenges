...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerLoopDispatch
...
theorem run_miss (s : State) (pc ptr off : UInt256) (rho : List UInt256)
    (dest : Nat) (hstack : rho.length ≤ 1017) (hrun : s.halt = .Running)
    (hfit : s.executionEnv.calldata.size < 2 ^ 256)
    (hmiss : s.executionEnv.calldata.size ≠ off.toNat) :
    runInstrSeq (template dest) {s with pc := pc, stack := ptr :: off :: rho} =
      some {s with pc := pcAfter pc (template dest), stack := ptr :: off :: rho} := by
...
theorem run_hit (s : State) (pc ptr off : UInt256) (rho : List UInt256)
    (dest : Nat) (hstack : rho.length ≤ 1017) (hrun : s.halt = .Running)
    (hfit : s.executionEnv.calldata.size < 2 ^ 256)
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.StaggerLoopDispatch
