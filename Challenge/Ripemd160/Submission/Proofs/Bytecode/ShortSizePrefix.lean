...
/-! The entry byte gate: `PUSH1 7 PUSH0 CALLDATALOAD PUSH0 BYTE EQ PUSH2 4827 JUMPI`. -/
...
theorem guard_dest : Decode.isValidJumpDest submissionBytecode 4827 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3681 = 4827 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3681 (by rfl)
  rwa [hpc] at h

theorem run_byte_prefix (input : ByteArray) :
    run bytePrefix (PatternedScan.stS input 0 []) =
      some (PatternedScan.stS input 10 [4827, byteCondition input]) := by
  ...
  let l6 : Located := pushAt 6 2 4827
  have h6 := PatternedScan.blockOfS l6
    (PatternedScan.pcFactS input 6 7 [v] (by norm_num) pc_b6)
    (PatternedScan.stepS_push input 7 2 4827 [v] (by simp) (by decide) (by decide) (by norm_num))
...
theorem run_byte_taken (input : ByteArray) (hbyte : firstByte input = 7) :
    run bytePath (PatternedScan.stS input 0 []) = some (PatternedScan.stS input 4827 []) := by
  have hp := run_byte_prefix input
  rw [byteCondition_one input hbyte] at hp
  have hj : run [opAt 7 .JUMPI]
      (PatternedScan.stS input 10 [4827, UInt256.ofNat 1]) =
      some (PatternedScan.stS input 4827 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 7 10 _ (by norm_num) pc_b7)
      (PatternedScan.stepS_jumpi_taken input 10 4827 4827 (UInt256.ofNat 1) []
...
theorem run_byte_fall (input : ByteArray) (hbyte : firstByte input ≠ 7) :
  ...
  have hj : run [opAt 7 .JUMPI]
      (PatternedScan.stS input 10 [4827, UInt256.ofNat 0]) =
      some (PatternedScan.stS input 11 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 7 10 _ (by norm_num) pc_b7)
      (PatternedScan.stepS_jumpi_fall input 10 4827 (UInt256.ofNat 0) []
