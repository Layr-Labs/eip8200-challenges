...
/-! The entry byte gate: `PUSH1 7 PUSH0 CALLDATALOAD PUSH0 BYTE EQ PUSH2 4815 JUMPI`. -/
...
theorem guard_dest : Decode.isValidJumpDest submissionBytecode 4815 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3634 = 4815 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3634 (by rfl)
  rwa [hpc] at h

theorem run_byte_prefix (input : ByteArray) :
    run bytePrefix (PatternedScan.stS input 13 []) =
      some (PatternedScan.stS input 23 [4815, byteCondition input]) := by
  ...
  let l6 : Located := pushAt 12 2 4815
  have h6 := PatternedScan.blockOfS l6
    (PatternedScan.pcFactS input 12 20 [v] (by norm_num) pc_b6)
    (PatternedScan.stepS_push input 20 2 4815 [v] (by simp) (by decide) (by decide) (by norm_num))
...
theorem run_byte_fall (input : ByteArray) (hbyte : firstByte input ≠ 7) :
  ...
  have hj : run [opAt 13 .JUMPI]
      (PatternedScan.stS input 23 [4815, UInt256.ofNat 0]) =
      some (PatternedScan.stS input 24 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 13 23 _ (by norm_num) pc_b7)
      (PatternedScan.stepS_jumpi_fall input 23 4815 (UInt256.ofNat 0) []
