...
theorem guard_dest : Decode.isValidJumpDest submissionBytecode 4817 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3636 = 4817 := by
...
theorem run_byte_prefix (input : ByteArray) :
    run bytePrefix (PatternedScan.stS input 13 []) =
      some (PatternedScan.stS input 23 [4817, byteCondition input]) := by
  ...
  let l6 : Located := pushAt 12 2 4817
  have h6 := PatternedScan.blockOfS l6
    (PatternedScan.pcFactS input 12 20 [v] (by norm_num) pc_b6)
    (PatternedScan.stepS_push input 20 2 4817 [v] (by simp) (by decide) (by decide) (by norm_num))
...
theorem run_byte_fall (input : ByteArray) (hbyte : firstByte input ≠ 7) :
  ...
  have hj : run [opAt 13 .JUMPI]
      (PatternedScan.stS input 23 [4817, UInt256.ofNat 0]) =
      some (PatternedScan.stS input 24 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 13 23 _ (by norm_num) pc_b7)
      (PatternedScan.stepS_jumpi_fall input 23 4817 (UInt256.ofNat 0) []
