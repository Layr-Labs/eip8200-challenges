...
/-! The entry byte gate: `PUSH1 7 PUSH0 CALLDATALOAD PUSH0 BYTE EQ PUSH2 4841 JUMPI`. -/
...
theorem guard_dest : Decode.isValidJumpDest submissionBytecode 4841 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3640 = 4841 := by
...
theorem run_byte_prefix (input : ByteArray) :
    run bytePrefix (PatternedScan.stS input 13 []) =
      some (PatternedScan.stS input 23 [4841, byteCondition input]) := by
  ...
  let l6 : Located := pushAt 12 2 4841
...
theorem run_byte_fall (input : ByteArray) (hbyte : firstByte input ≠ 7) :
  ...
  have hj : run [opAt 13 .JUMPI]
      (PatternedScan.stS input 23 [4841, UInt256.ofNat 0]) =
      some (PatternedScan.stS input 24 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 13 23 _ (by norm_num) pc_b7)
      (PatternedScan.stepS_jumpi_fall input 23 4841 (UInt256.ofNat 0) []
