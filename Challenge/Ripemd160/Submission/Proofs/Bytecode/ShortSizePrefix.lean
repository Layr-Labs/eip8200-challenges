...
def shortSizePrefix : List Located :=
  [opAt 0 .CALLDATASIZE, pushAt 1 2 256, opAt 2 .GT, pushAt 3 2 4801]

def shortSizePath : List Located := shortSizePrefix ++ [opAt 4 .JUMPI]

def sizeDispatchPath (input : ByteArray) : List Located :=
  if input.size < 256 then shortSizePath else shortSizePath ++ sizePath

def shortSizeCondition (input : ByteArray) : UInt256 :=
  UInt256.gt (UInt256.ofNat 256) (UInt256.ofNat input.size)

private theorem prefix_pc4 : Artifact.submissionArtifact.instructionPC 4 = 8 := by
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

private theorem short_dest : Decode.isValidJumpDest submissionBytecode 4801 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 4028 = 4801 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
...
private theorem run_short_prefix (input : ByteArray) :
    run shortSizePrefix (Execution.atPC input 0) =
      some (PatternedScan.stS input 8 [4801, shortSizeCondition input]) := by
  ...
  have pc1 : Artifact.submissionArtifact.instructionPC 1 = 1 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
  have pc2 : Artifact.submissionArtifact.instructionPC 2 = 4 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
  have pc3 : Artifact.submissionArtifact.instructionPC 3 = 5 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    rfl
...
private theorem short_condition_one (input : ByteArray) (hsmall : input.size < 256) :
  ...
  rw [hmod]
  have h256 : 256 % UInt256.size = 256 := by norm_num [UInt256.size]
  simp [h256, hsmall]

private theorem short_condition_zero (input : ByteArray) (hfit : CalldataFits input)
    (hlarge : ¬ input.size < 256) :
...
theorem run_short_size_taken (input : ByteArray) (hsmall : input.size < 256) :
    run shortSizePath (Execution.atPC input 0) = some (atPC input 4801) := by
  have hp := run_short_prefix input
  rw [short_condition_one input hsmall] at hp
  have hj : run [opAt 4 .JUMPI]
      (PatternedScan.stS input 8 [4801, UInt256.ofNat 1]) =
      some (PatternedScan.stS input 4801 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 4 8 _ (by norm_num) prefix_pc4)
      (PatternedScan.stepS_jumpi_taken input 8 4801 4801 (UInt256.ofNat 1) []
...
theorem run_short_size_fall (input : ByteArray) (hfit : CalldataFits input)
    (hlarge : ¬ input.size < 256) :
    run shortSizePath (Execution.atPC input 0) = some (Execution.atPC input 9) := by
  have hp := run_short_prefix input
  rw [short_condition_zero input hfit hlarge] at hp
  have hj : run [opAt 4 .JUMPI]
      (PatternedScan.stS input 8 [4801, UInt256.ofNat 0]) =
      some (PatternedScan.stS input 9 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 4 8 _ (by norm_num) prefix_pc4)
      (PatternedScan.stepS_jumpi_fall input 8 4801 (UInt256.ofNat 0) []
