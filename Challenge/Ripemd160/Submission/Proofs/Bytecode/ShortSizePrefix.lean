...
def shortSizePrefix : List Located :=
  [opAt 0 .CALLDATASIZE, pushAt 1 1 255, opAt 2 .GT, pushAt 3 2 4794]
...
private theorem short_dest : Decode.isValidJumpDest submissionBytecode 4794 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 4048 = 4794 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
...
private theorem run_short_prefix (input : ByteArray) :
    run shortSizePrefix (Execution.atPC input 0) =
      some (PatternedScan.stS input 7 [4794, shortSizeCondition input]) := by
...
theorem run_short_size_taken (input : ByteArray) (hsmall : input.size < 255) :
    run shortSizePath (Execution.atPC input 0) = some (atPC input 4794) := by
  have hp := run_short_prefix input
  rw [short_condition_one input hsmall] at hp
  have hj : run [opAt 4 .JUMPI]
      (PatternedScan.stS input 7 [4794, UInt256.ofNat 1]) =
    ...
    exact PatternedScan.blockOfS _
      ...
      (PatternedScan.stepS_jumpi_taken input 7 4794 4794 (UInt256.ofNat 1) []
...
theorem run_short_size_fall (input : ByteArray) (hfit : CalldataFits input)
  ...
  have hj : run [opAt 4 .JUMPI]
      (PatternedScan.stS input 7 [4794, UInt256.ofNat 0]) =
      some (PatternedScan.stS input 8 []) := by
    exact PatternedScan.blockOfS _
      (PatternedScan.pcFactS input 4 7 _ (by norm_num) prefix_pc4)
      (PatternedScan.stepS_jumpi_fall input 7 4794 (UInt256.ofNat 0) []
