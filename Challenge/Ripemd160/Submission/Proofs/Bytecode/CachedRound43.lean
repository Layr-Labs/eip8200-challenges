...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2413).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2413 actual_slice
    (by change 2413 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3026 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2413) = UInt256.ofNat 3026
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 2919, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 2965, stack := outputStack s.memory x rho} := by
