...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2310).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2310 actual_slice
    (by change 2310 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2860 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2310) = UInt256.ofNat 2860
