...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2423).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 2423 actual_slice
    (by change 2423 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 3018 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2423) = UInt256.ofNat 3018
