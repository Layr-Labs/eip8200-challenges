...
def paddingWork : Nat := 1067

/-- Driver setup/final test plus the complete five-word output routine:
`3 + 26 + 2589`.  The last summand is exposed path-by-path in `OutputGas`. -/
def framingWork : Nat := 2618

theorem fixedWork_eq : paddingWork + framingWork = 3685 := by
  rfl
