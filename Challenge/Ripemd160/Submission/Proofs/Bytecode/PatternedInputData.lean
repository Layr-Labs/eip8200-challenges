...
/-- Compact kernel representation. Same shape as the 1000-a guard
(`ByteArray.mk` of a closed `Array` former). Do not put a 1000-byte
literal here, and do not `decide` `data = ofFn` — that is the
`92349360` Lean miss. -/
def patternedInput : ByteArray :=
  ByteArray.mk (Array.ofFn fun i : Fin 1000 => expectedByte i.val)
...
@[simp] theorem patternedInput_getElem (i : Nat) (hi : i < patternedInput.size) :
  ...
  simpa [patternedInput, expectedByte] using
    Array.getElem_ofFn
