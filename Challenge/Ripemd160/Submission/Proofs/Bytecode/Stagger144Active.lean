...
theorem loaded_active_ge37 (s : State) (p : Nat)
    (hp : 1120 ≤ p) (hbound : p + 64 < 2 ^ 256) :
    37 ≤ (loadedActiveWords s (UInt256.ofNat p)).toNat := by
  ...
  have hnew : MachineState.activeWordsAfter
    ...
    · exact hcur
    · omega
  change 37 ≤ (UInt256.ofNat (MachineState.activeWordsAfter
...
#print axioms loaded_active_ge37
