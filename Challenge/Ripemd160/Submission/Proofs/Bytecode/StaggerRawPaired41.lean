...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2092 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2887 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2092) = UInt256.ofNat 2887
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2887, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2934, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2887) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2887) template = UInt256.ofNat 2934 := by decide
  rw [hend] at hraw
