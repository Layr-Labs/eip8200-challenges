...
open PairTable144Active
open PairTable144Memory (scratchMemory)
...
theorem run_endian (s : State) (pc low high returnPC : UInt256) (rest : List UInt256)
  ...
  have h3 := run_reverse {s with memory := writeWord s.memory 60 (PairedScheduleData.reversedWord high)}
  ...
  simpa only [endianTemplate_eq, DenseScheduleTrace.pcAfter_append, scratchMemory, PairTable144Memory.writeWord] using h
