...
instead of the runtime quotient `(2^256 - 1) / 65537`.

The literal keeps only the low 160 bits of `mask16`. This is sound because the
stage only ever sees the packed five-word digest, which is below `2^160`.
The mask is applied to `(value >>> 16) ^^^ value`, which is then below `2^160` too,
and bits of `mask16` at or above bit 160 cannot survive the AND.
-/
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.ClosedEndianTrunc16
...
/-- The low 160 bits of `mask16`, as a 18-byte literal. -/
def mask16Low : UInt256 :=
  UInt256.ofNat 0xffff0000ffff0000ffff0000ffff0000ffff

theorem mask16Low_toNat : mask16Low.toNat = mask16.toNat % 2 ^ 160 := by
  simp only [mask16Low, mask16, Word.word_toNat_ofNat]
  norm_num
...
theorem land_mask16Low (value : UInt256) (hv : value.toNat < 2 ^ 160) :
    UInt256.land mask16Low
        (UInt256.xor (UInt256.shiftRight value (UInt256.ofNat 16)) value) =
      UInt256.land mask16
        (UInt256.xor (UInt256.shiftRight value (UInt256.ofNat 16)) value) := by
  apply Word.word_ext
  rw [Word.word_toNat_land, Word.word_toNat_land, xor_toNat,
    Word.shiftRight_toNat value (by norm_num), mask16Low_toNat]
  apply and_mod_of_lt
...
def code : List Instr :=
  [ClosedEndianReuse.factorPush 16, .op (.Dup ⟨1, by decide⟩), dup1,
   push1 (UInt256.ofNat 16), op .SHR, op .XOR,
   .push ⟨18, by decide⟩ mask16Low, op .AND, op .MUL, op .XOR]
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.ClosedEndianTrunc16
