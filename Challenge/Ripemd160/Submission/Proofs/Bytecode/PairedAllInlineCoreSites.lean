...
def gasSteps_core_prefix (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 755, stack := coreStack [.a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] f (cache s.memory ++ rho)}
      {s with pc := UInt256.ofNat 4516, stack := coreStack [.d, .k, .b, .c, .a, .e, .factor, .pair, .upper, .lower] (Strip78Prefix.corePrefix77Chain.eval s.memory f) (cache s.memory ++ rho)} := by
