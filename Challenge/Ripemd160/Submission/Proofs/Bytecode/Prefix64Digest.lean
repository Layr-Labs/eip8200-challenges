...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Prefix64Digest
...
def paddedDigestWord : UInt256 :=
  0x0000000000000000000000008a14b0c89287b39b1a2f73aa79a1ce95b04e7817
...
private theorem lengthBytes_eq :
    Padding.lengthBytes data = ByteArray.mk #[0, 2, 0, 0, 0, 0, 0, 0] := by
...
private theorem compress_data (h : Array UInt32) (off : Nat) (hoff : off + 64 ≤ 64) :
...
theorem hashAfter_one : SpecBridge.absorbBlocks Ripemd160.H0 data 0 1 =
    PatternedDigest.H1 := by
  change Ripemd160.compressBlock PatternedDigest.H0 data 0 = PatternedDigest.H1
  rw [compress_data _ 0 (by omega)]
  simp only [CompressionCorrect.compressBlock_eq_normalized,
    PatternedDigest.schedule0, PatternedDigestA.step0]
...
theorem step_final :
    CompressionCorrect.normalizedCompress PatternedDigest.H1 finalWords = finalHash := by
  decide

theorem hashAfter_two :
    SpecBridge.absorbBlocks Ripemd160.H0 (Padding.paddedMessage data) 0 2 =
      finalHash := by
  rw [paddedMessage_split]
  rw [show 2 = 1 + 1 from rfl,
    HashSpecBridge.absorbBlocks_append Ripemd160.H0 data finalBlock 1 1 (by
      rw [data_size]), hashAfter_one]
  change Ripemd160.compressBlock PatternedDigest.H1 finalBlock 0 = finalHash
...
theorem targetDigest_eq_literal : targetDigest = ByteArray.mk #[
    0x8a, 0x14, 0xb0, 0xc8, 0x92, 0x87, 0xb3, 0x9b, 0x1a, 0x2f,
    0x73, 0xaa, 0x79, 0xa1, 0xce, 0x95, 0xb0, 0x4e, 0x78, 0x17] := by
...
theorem hash_data_eq : Ripemd160.hash data = targetDigest := by
  ...
  change SpecBridge.emitDigest
    (SpecBridge.absorbBlocks Ripemd160.H0 (Padding.paddedMessage data) 0 2) = targetDigest
  rw [hashAfter_two]
  rfl
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Prefix64Digest
