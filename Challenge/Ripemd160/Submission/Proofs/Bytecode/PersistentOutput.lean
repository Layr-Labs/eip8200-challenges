...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentOutput
...
theorem run_template (s : State) (pc off limit : UInt256) (h : Compression.HashState)
  ...
  simpa only [PersistentFrame.frame, packedHash, join_reverse, List.cons_append, List.nil_append] using
    run_raw s pc (ofUInt32 h.h0) (ofUInt32 h.h1) (ofUInt32 h.h2)
      (ofUInt32 h.h3) (ofUInt32 h.h4) (off :: limit :: rho) hs hrun

theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3974).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3974 actual_slice
    (by change 3974 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 4705 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3974) = UInt256.ofNat 4705
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4705, stack := PersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4721, stack := packedHash h :: off :: limit :: rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 4705, stack := PersistentFrame.frame h off limit rho}
    {s with pc := UInt256.ofNat 4721, stack := packedHash h :: off :: limit :: rho}
    hcode hfork hrun hnp site_pc.symm advances
  have hraw := run_template s (UInt256.ofNat 4705) off limit h rho hstack hrun
  have hend : pcAfter (UInt256.ofNat 4705) template = UInt256.ofNat 4721 := by decide
  rw [hend] at hraw
  exact hraw
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentOutput
