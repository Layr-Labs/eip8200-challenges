...
theorem site_pc : site.startPC = UInt256.ofNat 2720 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2036) = UInt256.ofNat 2720
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2720, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2754, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2720) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2720) template = UInt256.ofNat 2754 := by decide
  rw [hend] at hraw
