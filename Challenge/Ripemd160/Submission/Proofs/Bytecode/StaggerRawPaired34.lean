...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2090 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2677 := by
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2677, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2708, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2677) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2677) template = UInt256.ofNat 2708 := by decide
  rw [hend] at hraw
