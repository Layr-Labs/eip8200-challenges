...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 2037).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2037 actual_slice
    (by change 2037 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2719 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2037) = UInt256.ofNat 2719
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2719, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2753, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2719) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2719) template = UInt256.ofNat 2753 := by decide
  rw [hend] at hraw
