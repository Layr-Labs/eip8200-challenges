...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3279 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4153 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3279) = UInt256.ofNat 4153
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4153, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4191, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4153) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4153) template = UInt256.ofNat 4191 := by decide
  rw [hend] at hraw
