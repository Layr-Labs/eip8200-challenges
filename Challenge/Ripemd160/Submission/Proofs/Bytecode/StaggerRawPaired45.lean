...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2442 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3064 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2442) = UInt256.ofNat 3064
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3064, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3131, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3064) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3064) template = UInt256.ofNat 3131 := by decide
  rw [hend] at hraw
