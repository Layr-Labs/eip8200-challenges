...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRaw32
...
private theorem run_generated (s : State) (pc : UInt256) (x : Input) (rho : List UInt256)
  ...
  have hz972 : MachineState.readWord s.memory 972 = UInt256.ofNat 0 :=
    PadPrefixContract.read_zero s hz ⟨54, by decide⟩ (by decide)
  ...
  simp (discharger := omega) [template, inputStack, actualOutput, hz972,
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PadPrefixRaw32
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 5183).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 5183 actual_slice
    (by change 5183 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 6475 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 5183) = UInt256.ofNat 6475
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 6475, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 6529, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 6475) x rho hstack hrun hactive hz
  have hend : pcAfter (UInt256.ofNat 6475) template = UInt256.ofNat 6529 := by decide
  rw [hend] at hraw
