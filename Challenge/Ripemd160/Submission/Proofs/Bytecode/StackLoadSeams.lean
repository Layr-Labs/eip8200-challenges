...
theorem secondLoad_start : StackFrame.loadSite1238.startPC = UInt256.ofNat 0xb27 := rfl
...
theorem tailEntry_eq_roundEntry (s : State) (left right : Compression.EvmWorking)
    ...
    QuadTailTemplate.tailEntry s left right ret rest =
      StackRoundTrace.roundEntry s (UInt256.ofNat 0x1155)
...
theorem compressReturned_eq_self (s : State) (input : ByteArray) (i : Nat)
    (hpc : s.pc = UInt256.ofNat 0x3f9) (hstack : s.stack = driverRest input i) :
    DriverTrace.compressReturned s input i = s := by
  change {s with pc := UInt256.ofNat 0x3f9, stack := driverRest input i} = s
  rw [← hpc, ← hstack]
