...
theorem firstLoad_end : StackFrame.loadSite848.endPC = QuadSites.leftPC 0 := rfl

theorem secondLoad_start : StackFrame.loadSite1098.startPC = QuadSites.leftPC 20 := rfl

theorem secondLoad_end : StackFrame.loadSite1098.endPC = QuadSites.rightPC 0 := rfl
...
theorem tailEntry_eq_roundEntry (s : State) (left right : Compression.EvmWorking)
    ...
    StackTail.tailEntry s left right ret rest =
      StackRoundTrace.roundEntry s (UInt256.ofNat 0xe81)
...
theorem firstLoad_entry (s : State) (input : ByteArray) (i : Nat) :
    StackLoadTrace.loadEntry (scheduledState s input i) StackFrame.loadSite848.startPC
      (StackFrame.frameRest input i) = StackFrame.frameLoadEntry s input i := by
  rw [StackFrame.loadSite848_startPC]
  rfl

theorem firstLoad_returned (s : State) (rest : List UInt256) :
    StackLoadTrace.loadReturned s StackFrame.loadSite848.endPC rest =
      StackRoundTrace.roundEntry s (QuadSites.leftPC 0)
...
theorem secondLoad_entry (s : State) (left : Compression.EvmWorking)
    (rest : List UInt256) :
    StackLoadTrace.loadEntry s StackFrame.loadSite1098.startPC
      (StackRoundTrace.roundWords left ++ rest) =
      StackRoundTrace.roundEntry s (QuadSites.leftPC 20)
...
theorem secondLoad_returned (s : State) (rest : List UInt256) :
    StackLoadTrace.loadReturned s StackFrame.loadSite1098.endPC rest =
      StackRoundTrace.roundEntry s (QuadSites.rightPC 0)
        (initialWorking s).a (initialWorking s).b (initialWorking s).c
        (initialWorking s).d (initialWorking s).e rest := by
  rw [secondLoad_end]
  exact loadReturned_eq_roundEntry s _ rest
...
theorem tailEntry_atLanePC (s : State) (left right : Compression.EvmWorking)
    (ret : UInt256) (rest : List UInt256) :
    StackTail.tailEntry s left right ret rest =
      StackRoundTrace.roundEntry s (QuadSites.rightPC 20)
        right.a right.b right.c right.d right.e
