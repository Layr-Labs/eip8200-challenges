...
theorem secondLoad_start : StackFrame.loadSite1238.startPC = UInt256.ofNat 0xab0 := rfl
...
theorem tailEntry_eq_roundEntry (s : State) (left right : Compression.EvmWorking)
    ...
    QuadTailTemplate.tailEntry s left right ret rest =
      StackRoundTrace.roundEntry s (UInt256.ofNat 0x13bd)
...
theorem routeEntry_atLanePC (s : State) (left : Compression.EvmWorking)
  ...
  have hpc : QuadLayout.leftPC 20 = UInt256.ofNat 0xaaf := by
...
theorem tailEntry_atLanePC (s : State) (left right : Compression.EvmWorking)
  ...
  have hpc : QuadLayout.rightPC 20 = UInt256.ofNat 0x13bd := by
    change UInt256.ofNat
...
theorem compressReturned_eq_self (s : State) (input : ByteArray) (i : Nat)
    (hpc : s.pc = UInt256.ofNat 0x1d1) (hstack : s.stack = driverRest input i) :
    DriverTrace.compressReturned s input i = s := by
  change {s with pc := UInt256.ofNat 0x1d1, stack := driverRest input i} = s
  rw [← hpc, ← hstack]
