...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 962).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 962 actual_slice
    (by change 962 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1358 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 962) = UInt256.ofNat 1358
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 1358) template = UInt256.ofNat 1396 := by decide
  rw [hend] at hraw
