...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 904 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1309 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 904) = UInt256.ofNat 1309
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1309, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1347, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1309) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1309) template = UInt256.ofNat 1347 := by decide
  rw [hend] at hraw
