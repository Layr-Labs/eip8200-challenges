...
theorem site_pc : site.startPC = UInt256.ofNat 1099 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 678) = UInt256.ofNat 1103
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1099, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1131, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1099) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1099) template = UInt256.ofNat 1131 := by decide
  rw [hend] at hraw
