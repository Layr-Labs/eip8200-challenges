...
theorem site_pc : site.startPC = UInt256.ofNat 937 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 554) = UInt256.ofNat 937
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 937, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 977, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 937) x rho hstack halias hkey hcache hrun hactive
  have hend : pcAfter (UInt256.ofNat 937) template = UInt256.ofNat 977 := by decide
  rw [hend] at hraw
