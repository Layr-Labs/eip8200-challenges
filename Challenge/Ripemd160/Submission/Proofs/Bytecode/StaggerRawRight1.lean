...
theorem site_pc : site.startPC = UInt256.ofNat 1104 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 680) = UInt256.ofNat 1104
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1104, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1136, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1104) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1104) template = UInt256.ofNat 1136 := by decide
  rw [hend] at hraw
