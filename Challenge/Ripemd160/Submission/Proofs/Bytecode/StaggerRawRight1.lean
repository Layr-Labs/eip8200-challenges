...
theorem site_pc : site.startPC = UInt256.ofNat 920 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 505) = UInt256.ofNat 920
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hraw := run_actual s (UInt256.ofNat 896) x rho hstack halias hkey hcache hrun hactive
  have hend : pcAfter (UInt256.ofNat 920) template = UInt256.ofNat 960 := by decide
  rw [hend] at hraw
