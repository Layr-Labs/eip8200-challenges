...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 582 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1166 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 582) = UInt256.ofNat 1166
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1166, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1206, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1166) x rho hstack halias hkey hcache hrun hactive
  have hend : pcAfter (UInt256.ofNat 1166) template = UInt256.ofNat 1206 := by decide
  rw [hend] at hraw
