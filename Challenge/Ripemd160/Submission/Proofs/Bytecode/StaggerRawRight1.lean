...
def actualOutput (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.shiftRight (UInt256.mul x.v6 x.v2) (UInt256.ofNat 28)),
    x.v1,
    (UInt256.land x.v7 (UInt256.add x.v4 (UInt256.shiftRight (UInt256.mul x.v6 (UInt256.land x.v7 (UInt256.add x.v1 (UInt256.add (MachineState.readWord memory 162) (UInt256.add (UInt256.xor x.v3 (UInt256.lor (UInt256.lnot x.v0) x.v2)) x.v5))))) (UInt256.ofNat 29)))),
    x.v3,
    x.v4,
    x.v0,
    x.v6,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 672).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 672 actual_slice
    (by change 672 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1134 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 672) = UInt256.ofNat 1134
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1134, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1165, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1134) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1134) template = UInt256.ofNat 1165 := by decide
  rw [hend] at hraw
