...
theorem site_pc : site.startPC = UInt256.ofNat 901 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 506) = UInt256.ofNat 901
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 901, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 941, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 901) x rho hstack halias hkey hcache hrun hactive
  have hend : pcAfter (UInt256.ofNat 901) template = UInt256.ofNat 941 := by decide
  rw [hend] at hraw
