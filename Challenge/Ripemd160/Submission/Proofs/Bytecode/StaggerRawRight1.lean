...
theorem site_pc : site.startPC = UInt256.ofNat 920 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 527) = UInt256.ofNat 920
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 920, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 960, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 892) x rho hstack halias hkey hcache hrun hactive
  have hend : pcAfter (UInt256.ofNat 920) template = UInt256.ofNat 932 := by decide
  rw [hend] at hraw
