import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
   StackSiteBuilder.ofSlice template 1161 actual_slice
     ...
     (by decide)
 theorem site_pc : site.startPC = UInt256.ofNat 1657 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1161) = UInt256.ofNat 1657
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 1657, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 1699, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 1657) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 1657) template = UInt256.ofNat 1699 := by decide
   rw [hend] at hraw
