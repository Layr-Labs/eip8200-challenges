...
def eval (memory : ByteArray) (q : WordLane) : WordLane :=
  StaggerTerminal75.step 76 (MachineState.readWord memory 594) q
...
theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
    StaggerRawPaired76.outputStack memory (input memory h4 q right (StaggerAlgorithm.physicalKey 75)) rho =
      stack memory h4 [ .d, .literal 23, .cachedMessage 360, .pair, .upper, .e, .c, .a, .k, .b, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval memory q) right (StaggerAlgorithm.physicalKey 76) rho := by
  ...
  simp only [StaggerRawPaired76.outputStack, input, stack, List.map_cons, List.map_nil,
    ...
    StaggerTerminal75.step, if_pos (Or.inr rfl), StaggerAlgorithm.step, StaggerAdaptiveWord.usesAdaptive, StaggerAdaptiveWord.step, StaggerAdaptiveWord.t,
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4446, stack := stack s.memory h4 [ .d, .literal 23, .cachedMessage 360, .pair, .upper, .a, .b, .e, .k, .c, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] q right (StaggerAlgorithm.physicalKey 75) rho}
      {s with pc := UInt256.ofNat 4482, stack := stack s.memory h4 [ .d, .literal 23, .cachedMessage 360, .pair, .upper, .e, .c, .a, .k, .b, .factor, .lower, .cache 140, .cache 350, .cache 310, .cache 190, .cache 500 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 76) rho} := by
