...
def eval (memory : ByteArray) (q : WordLane) : WordLane :=
  StaggerAlgorithm.stepU 76 (MachineState.readWord memory 126) q
...
theorem output_eq (memory : ByteArray) (h4 : UInt256) (q right : WordLane) (rho : List UInt256) :
  ...
  simp only [StaggerRawPaired76.outputStack, input, stack, List.map_cons, List.map_nil,
    ...
    StaggerAlgorithm.step, StaggerAlgorithm.stepU, hkb, hka, hm, hl, hr,
...
def gasSteps (s : State) (h4 : UInt256) (q right : WordLane) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4530, stack := stack s.memory h4 [ .d, .pair, .upper, .a, .b, .e, .k, .c, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] q right (StaggerAlgorithm.physicalKey 75) rho}
      {s with pc := UInt256.ofNat 4568, stack := stack s.memory h4 [ .d, .pair, .upper, .e, .c, .a, .k, .b, .factor, .lower, .cache 140, .cache 190, .cache 310, .cache 350, .cache 500 ] (eval s.memory q) right (StaggerAlgorithm.physicalKey 76) rho} := by
