...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1893).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 1893 actual_slice
    (by change 1893 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 2529 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1893) = UInt256.ofNat 2529
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2529, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2597, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2529) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2529) template = UInt256.ofNat 2597 := by decide
  rw [hend] at hraw
