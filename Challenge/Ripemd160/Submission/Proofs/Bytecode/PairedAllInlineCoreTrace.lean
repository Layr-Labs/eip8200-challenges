...
theorem group0Template_pc : pcAfter (UInt256.ofNat 794) group0Template = UInt256.ofNat 815 := rfl

#print axioms group0Template_pc

def group0Block : CoreBlock 794 815 [.a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] where
  code := group0Template
  ...
  run := by
    ...
    have h := run_group0Template s (UInt256.ofNat 794) f.frame rho hstack hrun
...
def wholeCoreChain : CoreChain 794 [.a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] 4874 [.d, .b, .c, .a, .e, .factor, .pair, .upper, .lower] :=
  .cons group0Block (
  .cons inline0Block (
  .cons inline1Block (
  .cons inline2Block (
...
theorem run_wholeCoreChain (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    runInstrSeq wholeCoreChain.code
      {s with pc := UInt256.ofNat 794, stack := coreStack [.a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] f rho} =
...
theorem run_wholeCore_crypto (s : State) (words : Nat → UInt32)
    ...
    runInstrSeq wholeCoreChain.code
      {s with pc := UInt256.ofNat 794, stack := coreStack [.a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] ⟨PairedLaneWordRound.packCrypto left right, 0⟩ rho} =
...
structure WholeCoreSites (artifact : ProgramArtifact) (fork : Fork) where
  group0 : {site : GenericRoundSite artifact fork group0Block.code // site.startPC = UInt256.ofNat 794}
...
def gasSteps_wholeCore {artifact : ProgramArtifact} {fork : Fork}
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 794, stack := coreStack [.a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] f rho}
