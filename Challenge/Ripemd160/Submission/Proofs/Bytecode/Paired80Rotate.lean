...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Paired80Rotate

open Paired80Core Paired80Product

theorem get_pack_low (a b : BitVec 32) (i : Nat) (hi : i < 80) :
...
theorem get_pack_high (a b : BitVec 32) (i : Nat)
    (hlo : 80 ≤ i) (hi : i < 256) :
    (pack a b).getLsbD i = b.getLsbD (i - 80) := by
  have hn : ¬ i < 80 := by omega
  have hs : i - 80 < 176 := by omega
...
theorem low_rotate_product (a b : BitVec 32) (r : Nat)
  ...
  intro i hi
  have ht : 32 - r + i < 80 := by omega
  have ht256 : 32 - r + i < 256 := by omega
  have hsub : 32 - r + i - 32 < 80 := by omega
...
/-- The high cell's preceding 48-bit spacer excludes the low lane. -/
theorem high_rotate_product (a b : BitVec 32) (r : Nat)
  ...
  apply BitVec.eq_of_getLsbD_eq
  intro i hi
  have hp0 : 80 ≤ 32 - r + (80 + i) := by omega
  have hp1 : 32 - r + (80 + i) < 256 := by omega
  have hp2 : ¬ 32 - r + (80 + i) < 32 := by omega
  have he : 32 - r + (80 + i) - 80 = 32 - r + i := by omega
  ...
  by_cases h : i < r
  · have hlow : 32 - r + (80 + i) - 32 < 80 := by omega
    rw [get_pack_low a b _ hlow,
      BitVec.getLsbD_of_ge a _ (by omega : 32 ≤ 32 - r + (80 + i) - 32)]
    simp [h]
  · have hhigh : 80 ≤ 32 - r + (80 + i) - 32 := by omega
    have hhigh' : 32 - r + (80 + i) - 32 < 256 := by omega
    have he' : 32 - r + (80 + i) - 32 - 80 = i - r := by omega
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Paired80Rotate
