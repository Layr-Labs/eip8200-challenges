...
theorem unpackRight_dirtyDE (l r : CryptoLane) (d e : UInt256)
  ...
  have hdh : high32 d = high32 (packCrypto l r).d :=
    (high32_pairMask d).symm.trans (congrArg high32 hd)
  have heh : high32 e = high32 (packCrypto l r).e :=
...
theorem resultMemory_eq_folds (memory : ByteArray) (words : Nat → UInt32)
  ...
  obtain ⟨d,e,hp,hd,he⟩ := StaggerCoreCorrect.paired_crypto memory words
  ...
  rw [StaggerRepresentation.initial_eq memory h hh,hp,
    ...
    StaggerCoreCorrect.epilogue_dirtyDE memory words _ _ hm d e hd he,
    unpackRight_dirtyDE _ _ d e hd he,StaggerCoreCorrect.leftFinish_fold]
