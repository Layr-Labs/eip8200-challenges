...
4053), whose pushed destination is 5154.  Size misses re-enter the generic
compressor at pc 276 (instruction 178); word misses fall through to the
`gen27` arm at pc 5203 (instruction 4115).
...
```
 idx    pc    instruction
  ...
  4099   5159  PUSH2 276
  4100   5162  JUMPI          ; size != 3  -> generic
  ...
  4106   5172  XOR
  4107   5173  PUSH2 5203
  4108   5176  JUMPI          ; word != abcWord -> gen27 arm
...
def sizePath : List Located :=
  [opAt 4095 .JUMPDEST, opAt 4096 .CALLDATASIZE, pushAt 4097 1 3, opAt 4098 .SUB, pushAt 4099 2 276]
...
theorem run_size (input : ByteArray) :
  ...
  let l4 : Located := pushAt 4099 2 276
...
theorem run_size_miss (input : ByteArray) (hc : UInt256.isTrue (sizeCond input)) :
    run [opAt 4100 .JUMPI] (PatternedScan.stS input 5162 [276, sizeCond input]) =
      some (fallbackState input) :=
  PatternedScan.blockOfS (opAt 4100 .JUMPI)
    (PatternedScan.pcFactS input 4100 5162 [276, sizeCond input] (by norm_num) (by
      rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide))
...
def wordPath : List Located :=
  [pushAt 4101 3 6382179, pushAt 4102 1 232, opAt 4103 .SHL, pushAt 4104 0 0, opAt 4105 .CALLDATALOAD,
   opAt 4106 .XOR, pushAt 4107 2 5203]

theorem run_word (input : ByteArray) :
    run wordPath (PatternedScan.stS input 5163 []) =
      some (PatternedScan.stS input 5176 [5203, wordCond input]) := by
  ...
  let l6 : Located := pushAt 4107 2 5203
  have h6 := PatternedScan.blockOfS l6
    (PatternedScan.pcFactS input 4107 5173 [wordCond input] (by norm_num) (by
      rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide))
    (PatternedScan.stepS_push input 5173 2 5203 [wordCond input] (by simp) (by decide) (by decide) (by norm_num))
  have s1 := Stepper.runLocatedBlock_append [l0] [l1] _ _ _ h0 rfl h1
  have s2 := Stepper.runLocatedBlock_append [l0, l1] [l2] _ _ _ s1 rfl h2
  have s3 := Stepper.runLocatedBlock_append [l0, l1, l2] [l3] _ _ _ s2 rfl h3
  have s4 := Stepper.runLocatedBlock_append [l0, l1, l2, l3] [l4] _ _ _ s3 rfl h4
...
theorem run_word_miss (input : ByteArray) (hc : UInt256.isTrue (wordCond input)) :
    run [opAt 4108 .JUMPI] (PatternedScan.stS input 5176 [5203, wordCond input]) =
      some (Gen27Arm.armEntry input) :=
  PatternedScan.blockOfS (opAt 4108 .JUMPI)
    (PatternedScan.pcFactS input 4108 5176 [5203, wordCond input] (by norm_num) (by
      rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide))
    (PatternedScan.stepS_jumpi_taken input 5176 5203 5203 (wordCond input) []
...
theorem run_word_hit (input : ByteArray) (hc : ¬ UInt256.isTrue (wordCond input)) :
  ...
  PatternedScan.blockOfS (opAt 4108 .JUMPI)
    (PatternedScan.pcFactS input 4108 5176 [5203, wordCond input] (by norm_num) (by
      rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide))
    (PatternedScan.stepS_jumpi_fall input 5176 5203 (wordCond input) []
      (by simp) (by norm_num) hc)
...
/-- **Miss.** Any input other than `"abc"` or `gen27Input` leaves the arm at the
generic entry (via the `gen27` arm for size-3 inputs). -/
def gasSteps_miss (input : ByteArray) (hfit : CalldataFits input) (hne : input ≠ abcInput)
  ...
  if hs : sizeCond input = 0 then
    have hsize : input.size = 3 := (sizeCond_zero_iff input hfit).1 hs
    have hw : wordCond input ≠ 0 := fun hw =>
      hne (AbcRecognition.input_eq_abc input hsize
    ...
    (sound sizePath (run_size input)).trans
      ((sound _ (run_size_pass input (not_isTrue_of_eq _ hs))).trans
        ((sound wordPath (run_word input)).trans
          ((sound _ (run_word_miss input (isTrue_of_ne _ hw))).trans
            (Gen27Arm.gasSteps_miss input hsize hgen))))
  else
    (sound sizePath (run_size input)).trans
      (sound _ (run_size_miss input (isTrue_of_ne _ hs)))

/-- **gen27 entry.** `gen27Input` passes the size test, misses the `abc` word
test, and lands on the `gen27` arm. -/
def gasSteps_gen27_entry (input : ByteArray) (hfit : CalldataFits input)
    (heq : input = Gen27InputData.gen27Input) :
    GasSteps (armEntry input) (Gen27Arm.armEntry input) := by
  have hs : sizeCond input = 0 := (sizeCond_zero_iff input hfit).2 (by rw [heq]; rfl)
  have hw : wordCond input ≠ 0 := by
    intro hz
    have hword := (wordCond_zero_iff input).1 hz
    rw [heq, Gen27Recognition.readWord_gen27Input] at hword
    revert hword; decide
  exact (sound sizePath (run_size input)).trans
    ((sound _ (run_size_pass input (not_isTrue_of_eq _ hs))).trans
      ((sound wordPath (run_word input)).trans
        (sound _ (run_word_miss input hw))))
