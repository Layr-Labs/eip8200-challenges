...
def gasSteps_leftStep (s : State) (word : Nat → UInt32)
  ...
  have hj : i.val / 16 < 5 := by omega
  have hzero : i.val / 16 = 0 → leftConstant i.val = 0 := by
    intro hzero
    simp only [leftConstant, hzero]
    rfl
  ...
  have ha : s.activeWordsAfterUInt256 (leftAddress i.val).toNat 32 = s.activeWords := by
    ...
    have hk := leftIndex_lt i
    omega
  have g := gasSteps_template (i.val / 16) hj (leftAddress i.val) (leftRotation i.val)
    (leftConstant i.val) hzero (StackSites.leftSite i) s w rest
    hstack hcode hfork hrun hnp
  rw [returned_eq_stateAt s _ w _ _ _ _ _ rest hw ha] at g
  simpa only [StackSites.leftSite_start, StackSites.leftSite_end,
    StackCompression.leftStep, leftRotation, leftConstant] using g
...
def gasSteps_rightStep (s : State) (word : Nat → UInt32)
  ...
  have hj : 4 - i.val / 16 < 5 := by omega
  have hzero : 4 - i.val / 16 = 0 → rightConstant i.val = 0 := by
    intro hzero
    have hgroup : i.val / 16 = 4 := by omega
    simp only [rightConstant, hgroup]
    rfl
  ...
  have ha : s.activeWordsAfterUInt256 (rightAddress i.val).toNat 32 = s.activeWords := by
    ...
    have hk := rightIndex_lt i
    omega
  have g := gasSteps_template (4 - i.val / 16) hj (rightAddress i.val) (rightRotation i.val)
    (rightConstant i.val) hzero (StackSites.rightSite i) s w rest
    hstack hcode hfork hrun hnp
  rw [returned_eq_stateAt s _ w _ _ _ _ _ rest hw ha] at g
  simpa only [StackSites.rightSite_start, StackSites.rightSite_end,
    StackCompression.rightStep, rightRotation, rightConstant] using g
