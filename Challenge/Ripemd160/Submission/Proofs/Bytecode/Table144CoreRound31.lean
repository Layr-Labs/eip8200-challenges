import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144RawRound31
...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound31
...
def eval (message k : UInt256) (q : WordLane) : WordLane := wordStep 1 12 11 message k q
def nextKey (k : UInt256) : UInt256 := (UInt256.lor (UInt256.ofNat 6154742689) (UInt256.shiftLeft (UInt256.ofNat 3065519993) (UInt256.ofNat 145)))
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table144RawRound31.outputStack memory (input q k) rho =
      stack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 972) k q) (nextKey k) rho := by
  simp only [Table144RawRound31.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired144Algorithm.physicalKey 31) =
     Paired144Algorithm.physicalKey 32 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired144Algorithm.physicalKey 31) q = Paired144Algorithm.step 31 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 2513, stack := stack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 2574, stack :=
          stack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 972) k q) (nextKey k) rho} := by
   have gs := Table144RawRound31.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
   have hin : Table144Raw.inputStack (input q k) rho = stack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] q k rho := rfl
   rw [hin, output_eq] at gs
   exact gs
 #print axioms gasSteps

end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table144CoreRound31
