...
def tailBefore : List Instr :=
  artifactPrefix ++ submissionInstructionsChunk7.take 8

def tailAfter : List Instr :=
  submissionInstructionsChunk7.drop 69 ++ submissionInstructionsChunk8 ++
    submissionInstructionsChunk9 ++ submissionInstructionsChunk10

private theorem artifactPrefix_length : artifactPrefix.length = 1340 := by
  simp [artifactPrefix]
...
private theorem artifactChunk7_tail :
    submissionInstructionsChunk7 =
      submissionInstructionsChunk7.take 8 ++
        StackTail.tailInstructions ++ submissionInstructionsChunk7.drop 69 := by
  rfl
...
private theorem artifact_tail_split :
  ...
  have hprefix : Artifact.submissionInstructions =
      artifactPrefix ++ submissionInstructionsChunk7 ++ submissionInstructionsChunk8 ++
        submissionInstructionsChunk9 ++ submissionInstructionsChunk10 := by
...
private theorem tail_instruction_at (i : Nat)
    ...
    Artifact.submissionArtifact.instructions[1348 + i]? =
...
private theorem tail_instruction_pc (i : Nat)
    (hi : i ≤ StackTail.tailInstructions.length) :
    Artifact.submissionArtifact.instructionPC (1348 + i) =
      0xe81 + StackPC.byteLength (StackTail.tailInstructions.take i) := by
  exact instructionPC_segment_of_byteLength Artifact.submissionArtifact
    tailBefore StackTail.tailInstructions tailAfter 1348 0xe81
    artifact_tail_split tailBefore_length StackPC.tailPC i hi

private theorem tail_instruction_pc_global (index : Nat)
    (hlo : 1348 ≤ index) (hhi : index ≤ 1409) :
    Artifact.submissionArtifact.instructionPC index =
      0xe81 +
        StackPC.byteLength (StackTail.tailInstructions.take (index - 1348)) := by
  have hi : index - 1348 ≤ StackTail.tailInstructions.length := by
    rw [tailInstructions_length]
    omega
  have h := tail_instruction_pc (index - 1348) hi
  simpa only [Nat.add_sub_of_le hlo] using h

private theorem finalJumpPC :
    Artifact.submissionArtifact.instructionPC 1408 = 0xedb := by
