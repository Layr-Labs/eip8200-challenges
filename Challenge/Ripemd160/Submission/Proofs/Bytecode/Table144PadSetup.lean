...
open PairTable144Active PairTable144Sparse PairTable144Layout PairTable144Memory
...
theorem run_pad (s : State) (pc returnPC : UInt256) (rest : List UInt256)
    (hstack : rest.length ≤ 996) (hrun : s.halt = .Running) (hactive : 53 ≤ s.activeWords.toNat)
    (hfit : s.executionEnv.calldata.size < 2 ^ 256) :
    runInstrSeq padTemplate {s with pc := pc, stack := returnPC :: rest} =
      some {s with pc := pcAfter pc padTemplate, stack := returnPC :: rest, memory := PairTable144Pad.resultMemory s.memory (UInt256.ofNat s.executionEnv.calldata.size)} := by
  have hcap (n : Nat) (hn : n ≤ 27) : rest.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (ha : address ≤ 1664) :
  ...
  simp (discharger := omega) [padTemplate, PairTable144Pad.resultMemory,
    storeSelected, PairTable144Pad.keepPad, tableWords, slots, zeroMemory,
    PairTable144Pad.padWords,
    PairTable144Pad.lowLength, PairTable144Pad.highLength,
    PadShiftDiet.low, Word.land_comm,
    writeWord,
    ...
    PairTable144Pad.readPadded_end, Word.word_toNat_ofNat, Word.literal_eq_ofNat]
