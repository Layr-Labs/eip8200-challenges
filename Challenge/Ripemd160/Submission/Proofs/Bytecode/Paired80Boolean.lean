...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Paired80Boolean
...
theorem pack_and (a b c d : BitVec 32) :
  ...
  unfold pack
  rw [BitVec.and_append (x₁ := b.setWidth 176) (x₂ := d.setWidth 176)
    (y₁ := a.setWidth 80) (y₂ := c.setWidth 80)]
  rw [BitVec.setWidth_and, BitVec.setWidth_and]

theorem pack_or (a b c d : BitVec 32) :
    pack a b ||| pack c d = pack (a ||| c) (b ||| d) := by
  unfold pack
  rw [BitVec.or_append (x₁ := b.setWidth 176) (x₂ := d.setWidth 176)
    (y₁ := a.setWidth 80) (y₂ := c.setWidth 80)]
...
theorem pack_xor (a b c d : BitVec 32) :
    pack a b ^^^ pack c d = pack (a ^^^ c) (b ^^^ d) := by
  unfold pack
  rw [BitVec.xor_append (x₁ := b.setWidth 176) (x₂ := d.setWidth 176)
    (y₁ := a.setWidth 80) (y₂ := c.setWidth 80)]
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Paired80Boolean
