...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound32
...
def input (q : WordLane) (k : UInt256) : Table80Raw.Input :=
  ⟨word .k q k, word .a q k, word .b q k, word .c q k, word .d q k, word .e q k, word .factor q k, word .pair q k, word .upper q k, word .lower q k⟩
def eval (message k : UInt256) (q : WordLane) : WordLane := Paired80WordGroupTwoHoist.rawWordStep2 11 9 message k q
def nextKey (k : UInt256) : UInt256 := k
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound32.outputStack memory (input q k) rho =
      stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 20) k q) (nextKey k) rho := by
  simp only [Table80RawRound32.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired80Algorithm.physicalKey 32) =
     Paired80Algorithm.physicalKey 33 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired80Algorithm.physicalKey 32) q = Paired80Algorithm.step 32 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 2663, stack := stack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 2702, stack :=
          stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 20) k q) (nextKey k) rho} := by
   have gs := Table80RawRound32.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
   have hin : Table80Raw.inputStack (input q k) rho = stack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] q k rho := rfl
   rw [hin, output_eq] at gs
   exact gs
 #print axioms gasSteps

end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound32
