import Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTableSparse
import Challenge.Ripemd160.Submission.Proofs.Bytecode.PadOnlyMemory
set_option warningAsError true
set_option maxRecDepth 10000
set_option maxHeartbeats 2000000
set_option linter.unusedSimpArgs false
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTablePad
open EvmSemantics EvmSemantics.EVM Challenge.EvmProof
open PairTableMemory PairTableLayout PairTableSparse

def keepPad (j : Nat) : Bool :=
  decide (slots[j]! = 0 ∨ slots[j]! = 14 ∨ slots[j]! = 15)

def resultMemory (memory : ByteArray) (n : UInt256) : ByteArray :=
  storeSelected (zeroMemory memory) (tableWords (PadOnlySchedule.padWords n)) keepPad 0 78
...
theorem resultMemory_eq_table (memory : ByteArray) (n : UInt256) :
    resultMemory memory n = PairTableLayout.resultMemory memory (PadOnlySchedule.padWords n) := by
  unfold resultMemory
  refine (selected_eq_full memory (tableWords (PadOnlySchedule.padWords n)) keepPad 0 78
...
theorem readPadded_end (input : ByteArray) :
    MachineState.readPadded input input.size 802 = PairTableSparse.zeroBytes := by
  simp [MachineState.readPadded, PairTableSparse.zeroBytes]

#print axioms padWords_bound
#print axioms resultMemory_eq_table
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PairTablePad
