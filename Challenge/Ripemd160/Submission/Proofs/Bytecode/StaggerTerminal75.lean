...
/-- The physical exception changes only the C output of round75 and round76:
the rotated `.d` field is left unmasked in both. -/
def step (i : Nat) (message : UInt256) (q : WordLane) : WordLane :=
  if i=75 then {StaggerAlgorithm.step i message q with d:=wordShift q.c 28}
  else if i=76 then {StaggerAlgorithm.step i message q with d:=wordShift q.c 28}
...
theorem fold_prefix (message : Nat → UInt256) (n : Nat) (hn : n≤75) (q : WordLane) :
  ...
  | succ n ih =>
    rw [fold,ih (by omega)]
    simp only [step,show n≠75 from by omega,show n≠76 from by omega,if_false,StaggerAlgorithm.fold]
...
theorem final_shape (message : Nat → UInt256) (words : Nat → UInt32)
    ...
    ∃ dd de : UInt256,
      fold message 77 (packCrypto l (rightFold words 3 r)) =
        {packCrypto (leftFold words 77 l) (rightFold words 80 r) with d:=dd, e:=de} ∧
      UInt256.land dd pairWord =
        (packCrypto (leftFold words 77 l) (rightFold words 80 r)).d ∧
      UInt256.land de pairWord =
  ...
  refine ⟨wordShift q76.c 28, d, ?_, ?_, ?_⟩
  · rw [fold,step,if_neg (by decide : ¬76=75),if_pos rfl,fold,step,if_pos rfl,
    ...
    change {StaggerAlgorithm.step 76 (message 76)
      {StaggerAlgorithm.step 75 (message 75) q75 with d:=d} with
        d:=wordShift {StaggerAlgorithm.step 75 (message 75) q75 with d:=d}.c 28} = _
    rw [h75]
    exact congrArg (fun q : WordLane => {q with d:=wordShift q76.c 28}) hlast
