...
/-- The last two paired rounds leave their C-rotation outputs unmasked. -/
...
theorem fold_prefix (message : Nat → UInt256) (n : Nat) (hn : n≤75) (q : WordLane) :
  ...
  | succ n ih =>
    rw [fold,ih (by omega)]
    simp only [step,show n≠75 from by omega,show n≠76 from by omega,
      if_false,StaggerAlgorithm.fold]
...
theorem final_shape (message : Nat → UInt256) (words : Nat → UInt32)
  ...
  have hgap : (bits e).getLsbD 121 = false := by
    ...
    exact c_gap _ _
  have he : UInt256.land e pairWord = q76.d := by
