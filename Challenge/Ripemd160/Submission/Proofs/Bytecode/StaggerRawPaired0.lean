...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v0 (UInt256.shiftRight (UInt256.mul x.v7 x.v6) (UInt256.ofNat 28))),
    (MachineState.readWord memory 252),
...
theorem site_pc : site.startPC = UInt256.ofNat 1144 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 715) = UInt256.ofNat 1144
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 1144, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 1213, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 1144) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 1144) template = UInt256.ofNat 1213 := by decide
  rw [hend] at hraw
