...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v0 (UInt256.shiftRight (UInt256.mul x.v7 x.v6) (UInt256.ofNat 28))),
    (MachineState.readWord memory 900),
    x.v0,
    x.v1,
    x.v2,
    x.v3,
...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 726).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 726 actual_slice
    (by change 726 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 1156 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 726) = UInt256.ofNat 1156
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
  ...
  have hend : pcAfter (UInt256.ofNat 1156) template = UInt256.ofNat 1219 := by decide
  rw [hend] at hraw
