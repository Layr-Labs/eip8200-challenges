...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 689).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 689 actual_slice
    (by change 689 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 970 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 689) = UInt256.ofNat 970
