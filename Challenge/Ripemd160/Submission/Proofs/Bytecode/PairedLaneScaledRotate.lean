...
carry garbage, except bit 64, which is proven zero so that the following
addition cannot carry into the high lane.
-/

namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PairedLaneScaledRotate
...
theorem toNat_pack_zero_left (b : BitVec 32) : (pack 0#32 b).toNat = b.toNat * 2 ^ 128 := by
  rw [pack_toNat]
  simp
...
theorem div_pow_add_mul_pow128 (X Y k : Nat) (hX : X < 2 ^ 128) :
    (X + Y * 2 ^ 128) / 2 ^ (k + 128) = Y / 2 ^ k := by
  rw [Nat.pow_add, Nat.mul_comm (2 ^ k), ← Nat.div_div_eq_div_mul,
    Nat.add_mul_div_right _ _ (Nat.two_pow_pos 128), Nat.div_eq_of_lt hX, Nat.zero_add]

theorem testBit_add_mul_pow128 (X Y i : Nat) (hi : i < 128) (hX : X < 2 ^ i) :
    (X + Y * 2 ^ 128).testBit i = false := by
  rw [Nat.testBit_eq_decide_div_mod_eq, div_pow_add_mul_pow X Y i 128 (by omega),
    Nat.div_eq_of_lt hX, Nat.zero_add]
  have h : 2 ^ (128 - i) = 2 ^ (127 - i) * 2 := by
    rw [← Nat.pow_succ]
    congr 1
    omega
  have h2 : Y * 2 ^ (128 - i) % 2 = 0 := by
    rw [h, ← Nat.mul_assoc]
    omega
  rw [h2]
  decide
...
theorem pack_factor_toNat (a b : BitVec 32) :
    (pack a b * factor).toNat = (a.toNat + b.toNat * 2 ^ 128) * (2 ^ 32 + 1) := by
  rw [BitVec.toNat_mul, pack_toNat, factor_toNat]
  apply Nat.mod_eq_of_lt
  calc (a.toNat + b.toNat * 2 ^ 128) * (2 ^ 32 + 1)
      ≤ ((2 ^ 32 - 1) + (2 ^ 32 - 1) * 2 ^ 128) * (2 ^ 32 + 1) :=
...
theorem scaleLow_toNat (a b : BitVec 32) (d : Nat) (hd : d ≤ 31) :
    (scaleLow (pack a b) d).toNat = a.toNat * 2 ^ d + b.toNat * 2 ^ 128 := by
  ...
  apply Nat.mod_eq_of_lt
  calc a.toNat * 2 ^ d + b.toNat * 2 ^ 128
      ≤ (2 ^ 32 - 1) * 2 ^ 31 + (2 ^ 32 - 1) * 2 ^ 128 :=
...
theorem scaleHigh_toNat (a b : BitVec 32) (d : Nat) (hd : d ≤ 31) :
    (scaleHigh (pack a b) d).toNat = a.toNat + b.toNat * 2 ^ 128 * 2 ^ d := by
  rw [scaleHigh, upperMask_and_pack, BitVec.toNat_add, BitVec.toNat_mul, BitVec.toNat_ofNat,
    toNat_pack_zero_left, pack_toNat]
  have h1 : (2 ^ d - 1) % 2 ^ 256 = 2 ^ d - 1 :=
    Nat.mod_eq_of_lt (Nat.lt_of_le_of_lt (pred_pow_le d hd) (by decide))
  have h2 : (2 ^ d - 1) * (b.toNat * 2 ^ 128) % 2 ^ 256 = (2 ^ d - 1) * (b.toNat * 2 ^ 128) :=
    Nat.mod_eq_of_lt (Nat.lt_of_le_of_lt
      (Nat.mul_le_mul (pred_pow_le d hd) (Nat.mul_le_mul_right _ (toNat_le_pred b)))
      (by decide))
  have e : (2 ^ d - 1) * (b.toNat * 2 ^ 128) + (a.toNat + b.toNat * 2 ^ 128) =
      a.toNat + ((2 ^ d - 1) * (b.toNat * 2 ^ 128) + b.toNat * 2 ^ 128) := by ring
  rw [h1, h2, e, scale_nat _ _ (Nat.two_pow_pos d), Nat.mul_comm (2 ^ d)]
  apply Nat.mod_eq_of_lt
  calc a.toNat + b.toNat * 2 ^ 128 * 2 ^ d
      ≤ (2 ^ 32 - 1) + (2 ^ 32 - 1) * 2 ^ 128 * 2 ^ 31 :=
...
theorem scaleLow_product_toNat (a b : BitVec 32) (d : Nat) (hd : d ≤ 31) :
    (scaleLow (pack a b) d * factor).toNat =
      a.toNat * (2 ^ 32 + 1) * 2 ^ d + (b.toNat * (2 ^ 32 + 1)) * 2 ^ 128 := by
  rw [BitVec.toNat_mul, scaleLow_toNat a b d hd, factor_toNat]
  have e : (a.toNat * 2 ^ d + b.toNat * 2 ^ 128) * (2 ^ 32 + 1) =
      a.toNat * (2 ^ 32 + 1) * 2 ^ d + (b.toNat * (2 ^ 32 + 1)) * 2 ^ 128 := by ring
  rw [e]
  apply Nat.mod_eq_of_lt
  calc a.toNat * (2 ^ 32 + 1) * 2 ^ d + (b.toNat * (2 ^ 32 + 1)) * 2 ^ 128
      ≤ (2 ^ 32 - 1) * (2 ^ 32 + 1) * 2 ^ 31 + ((2 ^ 32 - 1) * (2 ^ 32 + 1)) * 2 ^ 128 :=
        Nat.add_le_add
...
theorem scaleHigh_product_toNat (a b : BitVec 32) (d : Nat) (hd : d ≤ 31) :
    (scaleHigh (pack a b) d * factor).toNat =
      a.toNat * (2 ^ 32 + 1) + (b.toNat * (2 ^ 32 + 1) * 2 ^ d) * 2 ^ 128 := by
  rw [BitVec.toNat_mul, scaleHigh_toNat a b d hd, factor_toNat]
  have e : (a.toNat + b.toNat * 2 ^ 128 * 2 ^ d) * (2 ^ 32 + 1) =
      a.toNat * (2 ^ 32 + 1) + (b.toNat * (2 ^ 32 + 1) * 2 ^ d) * 2 ^ 128 := by ring
  rw [e]
  apply Nat.mod_eq_of_lt
  calc a.toNat * (2 ^ 32 + 1) + (b.toNat * (2 ^ 32 + 1) * 2 ^ d) * 2 ^ 128
      ≤ (2 ^ 32 - 1) * (2 ^ 32 + 1) + ((2 ^ 32 - 1) * (2 ^ 32 + 1) * 2 ^ 31) * 2 ^ 128 :=
...
theorem toNat_high (x : BitVec 256) : (high x).toNat = x.toNat / 2 ^ 128 % 2 ^ 32 := by
...
theorem aF_lt_128 (a : BitVec 32) : a.toNat * (2 ^ 32 + 1) < 2 ^ 128 :=
  Nat.lt_of_lt_of_le (aF_lt_64 a) (by decide)

theorem pack_factor_low_toNat (a b : BitVec 32) (k : Nat) (hk : k ≤ 96) :
    (low ((pack a b * factor) >>> k)).toNat = a.toNat * (2 ^ 32 + 1) / 2 ^ k % 2 ^ 32 := by
  rw [toNat_low, toNat_shift, pack_factor_toNat]
  have e : (a.toNat + b.toNat * 2 ^ 128) * (2 ^ 32 + 1) =
      a.toNat * (2 ^ 32 + 1) + (b.toNat * (2 ^ 32 + 1)) * 2 ^ 128 := by ring
  rw [e, div_pow_add_mul_pow _ _ k 128 (by omega), mod_pow32_add_mul_pow _ _ _ (by omega)]
...
theorem high_scaleLow (a b : BitVec 32) (r s : Nat)
  ...
  rw [pack_factor_high_toNat a b (32 - s), toNat_high, toNat_shift,
    ...
    div_pow_add_mul_pow128 _ _ _ hX]

theorem gap_scaleLow (a b : BitVec 32) (r s : Nat)
    (hs0 : 0 < s) (hsr : s < r) (hr : r < 32) :
    ((scaleLow (pack a b) (r - s) * factor) >>> (32 - s)).getLsbD 64 = false := by
  rw [BitVec.getLsbD_ushiftRight, ← BitVec.testBit_toNat,
    scaleLow_product_toNat a b (r - s) (by omega)]
  apply testBit_add_mul_pow128 _ _ _ (by omega)
  have hpow : 2 ^ (r - s) ≤ 2 ^ (32 - s) := Nat.pow_le_pow_right (by decide) (by omega)
  calc a.toNat * (2 ^ 32 + 1) * 2 ^ (r - s)
      < 2 ^ 64 * 2 ^ (32 - s) :=
        Nat.mul_lt_mul_of_lt_of_le (aF_lt_64 a) hpow (Nat.two_pow_pos _)
    _ = 2 ^ (32 - s + 64) := by rw [← Nat.pow_add, Nat.add_comm]

/-! ## The high lane is pre-scaled (`r < s`) -/

theorem low_scaleHigh (a b : BitVec 32) (r s : Nat)
    (hr0 : 0 < r) (hrs : r < s) (hs : s < 32) :
  ...
  rw [pack_factor_low_toNat a b (32 - r) (by omega), toNat_low, toNat_shift,
    ...
    div_pow_add_mul_pow _ _ (32 - r) 128 (by omega), mod_pow32_add_mul_pow _ _ _ (by omega)]

theorem high_scaleHigh (a b : BitVec 32) (r s : Nat)
    (hr0 : 0 < r) (hrs : r < s) (hs : s < 32) :
    high ((scaleHigh (pack a b) (s - r) * factor) >>> (32 - r)) = b.rotateLeft s := by
  rw [← high_rotate_product a b s (by omega) hs]
  apply BitVec.eq_of_toNat_eq
  rw [pack_factor_high_toNat a b (32 - s), toNat_high, toNat_shift,
    scaleHigh_product_toNat a b (s - r) (by omega), Nat.div_div_eq_div_mul, ← Nat.pow_add,
    div_pow_add_mul_pow128 _ _ _ (aF_lt_128 a), mul_pow_div_pow _ _ _ (by omega)]
  have e : 32 - r - (s - r) = 32 - s := by omega
  rw [e]

theorem gap_scaleHigh (a b : BitVec 32) (r s : Nat)
    (hr0 : 0 < r) (hrs : r < s) (hs : s < 32) :
    ((scaleHigh (pack a b) (s - r) * factor) >>> (32 - r)).getLsbD 64 = false := by
  rw [BitVec.getLsbD_ushiftRight, ← BitVec.testBit_toNat,
    scaleHigh_product_toNat a b (s - r) (by omega)]
  apply testBit_add_mul_pow128 _ _ _ (by omega)
  calc a.toNat * (2 ^ 32 + 1) < 2 ^ 64 := aF_lt_64 a
    _ ≤ 2 ^ (32 - r + 64) := Nat.pow_le_pow_right (by decide) (by omega)
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.PairedLaneScaledRotate
