import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 theorem actual_slice :
     (Artifact.submissionArtifact.instructions.drop 785).take template.length = template := by rfl
 def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
   StackSiteBuilder.ofSlice template 785 actual_slice
     (by change 785 + template.length ≤ Artifact.submissionInstructions.length
         rw [Artifact.referenceInstructions_count]; decide)
     StackRoundData.artifact_code_bound
     (StackRoundData.templateWellFormed_mem (instructions := template) (by decide))
     (by decide)
 theorem site_pc : site.startPC = UInt256.ofNat 1224 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 785) = UInt256.ofNat 1224
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 1224, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 1269, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 1224) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 1224) template = UInt256.ofNat 1269 := by decide
   rw [hend] at hraw
