import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
   StackSiteBuilder.ofSlice template 795 actual_slice
     ...
     (by decide)
 theorem site_pc : site.startPC = UInt256.ofNat 1222 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 795) = UInt256.ofNat 1222
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 1222, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 1266, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 1222) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 1222) template = UInt256.ofNat 1266 := by decide
   rw [hend] at hraw
