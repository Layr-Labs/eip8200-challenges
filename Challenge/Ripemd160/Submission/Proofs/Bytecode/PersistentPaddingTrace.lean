import Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentOuterPCs
import Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentPaddingArtifact
import Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentPadding
import Challenge.Ripemd160.Submission.Proofs.Bytecode.Trace
import Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentMain
...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentPaddingTrace
...
def padEntry (input : ByteArray) : State :=
  { PersistentMain.initializedState input with
    pc := UInt256.ofNat 277
    stack := [] }
...
@[simp] private theorem initializedPC764 :
    Artifact.instructionPC 179 = 277 := by change Artifact.submissionArtifact.instructionPC 179 = 277; rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl

@[simp] private theorem initializedCalldata (input : ByteArray) :
    (PersistentMain.initializedState input).executionEnv.calldata = input := by rfl

def enterPath : List
    (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  PersistentPaddingArtifact.padEnterPath

set_option maxHeartbeats 200000 in
private theorem run_enter (input : ByteArray) :
    Challenge.EvmProof.Stepper.runLocatedBlock enterPath
      (PersistentMain.initializedState input) = some (padEntry input) := by
  have hpc : (PersistentMain.initializedState input).pc = UInt256.ofNat 277 := by
    rw [PersistentMain.initializedState_pc, initializedPC764]
  have hstack := PersistentMain.initializedState_stack input
  simp only [enterPath, PersistentPaddingArtifact.padEnterPath, Challenge.EvmProof.Stepper.runLocatedBlock]
  rw [padEntry, ← hpc, ← hstack]

def gasSteps_enterPad (input : ByteArray) :
    Challenge.EvmProof.GasSteps (PersistentMain.initializedState input) (padEntry input) := by
...
def paddedLengthPath : List
    (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  PersistentPaddingArtifact.padLengthPath

def padLengthReady (input : ByteArray) : State :=
  { padEntry input with
    pc := UInt256.ofNat (Artifact.instructionPC 186)
    stack := [UInt256.ofNat input.size, PersistentPadding.paddedWord input] }
...
@[simp] private theorem padLengthReady_pcToNat (input : ByteArray) :
    (padLengthReady input).pc.toNat = 286 := by rfl

@[simp] private theorem padLengthReady_pc (input : ByteArray) :
    (padLengthReady input).pc = UInt256.ofNat 286 := by rfl

@[simp] private theorem padLengthReady_pcSucc (input : ByteArray) :
    (padLengthReady input).pc.succ = UInt256.ofNat 287 := by
...
@[simp] private theorem padLengthReady_stack (input : ByteArray) :
    (padLengthReady input).stack =
      [UInt256.ofNat input.size, PersistentPadding.paddedWord input] := by
  rfl
...
private theorem run_paddedLength (input : ByteArray) :
  ...
  simp [paddedLengthPath, PersistentPaddingArtifact.padLengthPath, Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    padEntry, padLengthReady,
    PersistentPadding.paddedWord, land_not63, Challenge.EvmProof.Word.word_add_comm]
...
def lengthOffsetWord (input : ByteArray) : UInt256 :=
  PersistentPadding.paddedWord input + UInt256.ofNat 1624

def padCopied (input : ByteArray) : State :=
  { padLengthReady input with
    pc := UInt256.ofNat (Artifact.instructionPC 190)
    memory := MachineState.writeBytes (padLengthReady input).memory
      (MachineState.readPadded input 0 input.size) PersistentPadding.messageOffset
    activeWords := (padLengthReady input).activeWordsAfterUInt256
      PersistentPadding.messageOffset input.size }

def padSentinel (input : ByteArray) : State :=
  { padCopied input with
    pc := UInt256.ofNat (Artifact.instructionPC 195)
    stack := [UInt256.ofNat input.size, PersistentPadding.paddedWord input]
    memory := MachineState.writeBytes (padCopied input).memory
      (ByteArray.mk #[0x80]) (PersistentPadding.messageOffset + input.size)
    activeWords := (padCopied input).activeWordsAfterUInt256
      (PersistentPadding.messageOffset + input.size) 1 }
...
@[simp] private theorem padCopied_pcToNat (input : ByteArray) :
    (padCopied input).pc.toNat = 292 := by rfl

@[simp] private theorem padCopied_pc (input : ByteArray) :
    (padCopied input).pc = UInt256.ofNat 292 := by rfl
...
@[simp] private theorem padSentinel_pcToNat (input : ByteArray) :
    (padSentinel input).pc.toNat = 300 := by rfl

@[simp] private theorem padSentinel_pc (input : ByteArray) :
    (padSentinel input).pc = UInt256.ofNat 300 := by rfl

@[simp] private theorem padSentinel_pcSucc (input : ByteArray) :
    (padSentinel input).pc.succ = UInt256.ofNat 301 := by
...
def lengthSetupPath : List
    (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  PersistentPaddingArtifact.padSetupPath
...
def padSentinelAddressReady (input : ByteArray) : State :=
  { padCopied input with
    pc := UInt256.ofNat (Artifact.instructionPC 194)
    stack := [UInt256.ofNat PersistentPadding.messageOffset + UInt256.ofNat input.size,
      UInt256.ofNat 128, UInt256.ofNat input.size, PersistentPadding.paddedWord input] }
...
def padSentinelStored (input : ByteArray) : State :=
  { padSentinelAddressReady input with
    ...
    memory := MachineState.writeBytes (padSentinelAddressReady input).memory
      (ByteArray.mk #[0x80])
      (UInt256.ofNat PersistentPadding.messageOffset + UInt256.ofNat input.size).toNat
    activeWords := (padSentinelAddressReady input).activeWordsAfterUInt256
      (UInt256.ofNat PersistentPadding.messageOffset + UInt256.ofNat input.size).toNat 1 }
...
@[simp] private theorem padSentinelAddressReady_pc (input : ByteArray) :
    (padSentinelAddressReady input).pc = UInt256.ofNat 299 := by rfl

@[simp] private theorem padSentinelAddressReady_stack (input : ByteArray) :
    (padSentinelAddressReady input).stack =
      [UInt256.ofNat PersistentPadding.messageOffset + UInt256.ofNat input.size,
        UInt256.ofNat 128, UInt256.ofNat input.size, PersistentPadding.paddedWord input] := by rfl
...
private theorem run_lengthCopy (input : ByteArray) (hfit : CalldataFits input) :
  ...
  simp [lengthCopyPath, lengthSetupPath, PersistentPaddingArtifact.padSetupPath,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    padCopied, padLengthReady, State.activeWordsAfterUInt256, PersistentPadding.messageOffset,
    hsizeWord, hzero]
...
private theorem run_lengthSentinelAddress (input : ByteArray) :
  ...
  simp [lengthSentinelAddressPath, lengthSentinelPath, lengthSetupPath,
    PersistentPaddingArtifact.padSetupPath, Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
    padSentinelAddressReady, padCopied, padLengthReady, PersistentPadding.messageOffset]
...
private theorem padSentinelStored_eq (input : ByteArray)
  ...
  have hsum : PersistentPadding.messageOffset + input.size < 2 ^ 256 := by
    unfold CalldataFits at hfit
    norm_num [PersistentPadding.messageOffset] at hfit ⊢
    omega
  have hadd : UInt256.ofNat PersistentPadding.messageOffset + UInt256.ofNat input.size =
      UInt256.ofNat (PersistentPadding.messageOffset + input.size) :=
    Challenge.EvmProof.Word.ofNat_add_ofNat hsum
  have haddNat : (UInt256.ofNat PersistentPadding.messageOffset +
      UInt256.ofNat input.size).toNat = PersistentPadding.messageOffset + input.size := by
  ...
  rw [show (UInt256.ofNat PersistentPadding.messageOffset +
    UInt256.ofNat input.size).toNat = PersistentPadding.messageOffset + input.size by
      exact haddNat]
...
def lengthLoopState (input : ByteArray) (i : Nat) : State :=
  { padSentinel input with
    ...
    stack := [lengthAddr input i, lengthShift input i,
      PersistentPadding.paddedWord input]
...
def lengthIterationPath : List
  ...
  [⟨207, .op .JUMPDEST, by rfl, wfOp (by decide) trivial rfl⟩,
   ...
   ⟨209, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨210, .op .MSTORE8, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨211, .push ⟨1, by decide⟩ (UInt256.ofNat 1), by rfl, by decide⟩,
   ⟨212, .op .ADD, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨213, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨214, .push ⟨1, by decide⟩ (UInt256.ofNat 8), by rfl, by decide⟩,
   ⟨215, .op .SHR, by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨216, .op (.Swap ⟨0, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨217, .op (.Dup ⟨1, by decide⟩), by rfl, wfOp (by decide) trivial rfl⟩,
   ⟨218, .push ⟨2, by decide⟩ (UInt256.ofNat 317), by rfl, by decide⟩,
   ⟨219, .op .JUMPI, by rfl, wfOp (by decide) trivial rfl⟩]
...
@[simp] private theorem lengthLoopState_pc (input : ByteArray) (i : Nat) :
    (lengthLoopState input i).pc = UInt256.ofNat 317 := by rfl
...
@[simp] private theorem lengthLoopState_stack (input : ByteArray) (i : Nat) :
    (lengthLoopState input i).stack =
      [lengthAddr input i, lengthShift input i,
        PersistentPadding.paddedWord input] := by rfl
...
def lengthSteppedState (input : ByteArray) (i : Nat) : State :=
  { lengthLoopState input i with
    pc := UInt256.ofNat (Artifact.instructionPC 217)
...
def lengthBranchReady (input : ByteArray) (i : Nat) : State :=
  { lengthSteppedState input i with
    pc := UInt256.ofNat (Artifact.instructionPC 219)
...
def lengthExitPending (input : ByteArray) (i : Nat) : State :=
  { lengthBranchReady input i with
    pc := UInt256.ofNat (Artifact.instructionPC 220)
    stack := (lengthSteppedState input i).stack }
...
@[simp] private theorem lengthSteppedState_pc (input : ByteArray) (i : Nat) :
    (lengthSteppedState input i).pc = UInt256.ofNat 329 := by rfl
...
@[simp] private theorem lengthBranchReady_pc (input : ByteArray) (i : Nat) :
    (lengthBranchReady input i).pc = UInt256.ofNat 333 := by rfl
...
@[simp] private theorem validLengthLoopHead :
    Decode.isValidJumpDest submissionBytecode 317 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 207 = 317 := by rw [Challenge.Ripemd160.Submission.Proofs.Bytecode.ArtifactByteLength.instructionPC_eq_byteLength]; rfl
  rw [← hpc]
  exact Artifact.submissionArtifact.isValidJumpDest_index 207 (by rfl)
...
theorem lengthOffsetWord_eq (input : ByteArray) (hfit : CalldataFits input) :
    (lengthOffsetWord input).toNat =
      PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 := by
  have hlt := PersistentPadding.paddedLength_lt input.size
  have hsum : PersistentPadding.paddedLength input.size + 1624 < 2 ^ 256 := by
    unfold CalldataFits at hfit
    norm_num at hfit ⊢
    omega
  rw [lengthOffsetWord, PersistentPadding.paddedWord_eq input hfit,
    Challenge.EvmProof.Word.ofNat_add_ofNat hsum,
    Challenge.EvmProof.Word.word_toNat_ofNat, Nat.mod_eq_of_lt hsum]
  unfold PersistentPadding.messageOffset
  omega

theorem lengthAddr_toNat (input : ByteArray) (hfit : CalldataFits input)
    (i : Nat) (hi : i ≤ 8) :
    (lengthAddr input i).toNat =
      PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + i := by
  induction i with
  | zero => simpa [lengthAddr] using lengthOffsetWord_eq input hfit
  | succ i ih =>
      have hlt := PersistentPadding.paddedLength_lt input.size
      have hbound : 1 + (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + i)
          < 2 ^ 256 := by
        unfold CalldataFits at hfit
        unfold PersistentPadding.messageOffset
        norm_num at hfit ⊢
...
theorem topByteAddr_toNat (input : ByteArray) (hfit : CalldataFits input) :
    (topByteAddr input).toNat =
      PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + 7 := by
  have hlt := PersistentPadding.paddedLength_lt input.size
  have hbound : 7 + (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8)
      < 2 ^ 256 := by
    unfold CalldataFits at hfit
    unfold PersistentPadding.messageOffset
    norm_num at hfit ⊢
...
private theorem topByteActiveWords_toNat (input : ByteArray)
    ...
    (topByteActiveWords input).toNat =
      Nat.max (padSentinel input).activeWords.toNat
        ((PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + 7) / 32 + 1) := by
  have hlt := PersistentPadding.paddedLength_lt input.size
  have hcur : (padSentinel input).activeWords.toNat < 2 ^ 256 :=
    (padSentinel input).activeWords.val.isLt
  have hsize : input.size < 2 ^ 256 := Nat.lt_trans hfit (by norm_num)
  have hdiv : (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + 7) / 32
      ≤ PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + 7 :=
    Nat.div_le_self _ _
  have hbig : PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + 7 + 1
      < 2 ^ 256 := by
    unfold CalldataFits at hfit
    unfold PersistentPadding.messageOffset
    norm_num at hfit ⊢
...
private theorem lengthLoopActiveWords_stable (input : ByteArray)
  ...
  have hmono : (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + i + 1 - 1) / 32
      ≤ (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + 7) / 32 :=
  ...
  rw [if_neg (by decide : (1 : Nat) ≠ 0)]
  dsimp only
  have hle : (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + i + 1 - 1) / 32 + 1
  ...
  have hmaxeq : Nat.max (topByteActiveWords input).toNat
      ((PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8 + i + 1 - 1) / 32 + 1)
...
def padFinalMemory (input : ByteArray) : ByteArray :=
  PersistentPadding.paddedMemory (padLengthReady input).memory input

def padReturned (input : ByteArray) : State :=
  { lengthLoopState input 0 with
    pc := UInt256.ofNat 335
    stack := [UInt256.ofNat 0, PersistentPadding.paddedWord input]
    memory := padFinalMemory input }

@[simp] theorem padReturned_pc (input : ByteArray) :
    (padReturned input).pc = UInt256.ofNat 335 := by rfl

@[simp] theorem padReturned_stack (input : ByteArray) :
    (padReturned input).stack = [UInt256.ofNat 0, PersistentPadding.paddedWord input] := by rfl
...
def lengthExitPath : List
    (Challenge.EvmProof.Stepper.Located Artifact.submissionArtifact .Osaka) :=
  [⟨220, .op .POP, by rfl, wfOp (by decide) trivial rfl⟩]

@[simp] private theorem pcExitPop :
    Artifact.submissionArtifact.instructionPC 220 = 334 := by
...
def lengthExitEntered (input : ByteArray) (i : Nat) : State :=
  { lengthLoopState input i with pc := UInt256.ofNat 334 }
...
def lengthExitReturned (input : ByteArray) (i : Nat) : State :=
  { lengthExitEntered input i with
    pc := UInt256.ofNat 335
    stack := [lengthShift input i, PersistentPadding.paddedWord input] }
...
@[simp] private theorem lengthExitEntered_pc (input : ByteArray) (i : Nat) :
    (lengthExitEntered input i).pc = UInt256.ofNat 334 := by rfl
...
private theorem padLengthReady_size_le (input : ByteArray) :
    (padLengthReady input).memory.size ≤ PersistentPadding.messageOffset := by
  change 0 ≤ 1632
  decide
...
private theorem sentinel_size_le (input : ByteArray) (_hfit : CalldataFits input) :
  ...
  have hlen : input.size + 9 ≤ PersistentPadding.paddedLength input.size := by
    unfold PersistentPadding.paddedLength
    omega
...
theorem lengthBytes_of_shift_zero (input : ByteArray) (hfit : CalldataFits input)
  ...
  rw [Challenge.EvmProof.Memory.getD0_eq_getElem _ _
    (by simpa using hj : j < (PersistentPadding.lengthBytes input).size),
    PersistentPadding.lengthByte input j hj]
...
theorem topByte_eq (input : ByteArray) (hfit : CalldataFits input) :
    UInt8.ofNat ((topByteWord input).toNat % 256) =
      (PersistentPadding.lengthBytes input)[7]?.getD 0 := by
  ...
  rw [hshift,
    Challenge.EvmProof.Memory.getD0_eq_getElem _ _
      (by simp : (7 : Nat) < (PersistentPadding.lengthBytes input).size),
    PersistentPadding.lengthByte input 7 (by norm_num)]
  congr 1
...
theorem lengthShift_byte_eq (input : ByteArray) (hfit : CalldataFits input)
    ...
    UInt8.ofNat ((lengthShift input i).toNat % 256) =
      (PersistentPadding.lengthBytes input)[i]?.getD 0 := by
  rw [lengthShift_toNat input hfit i,
    Challenge.EvmProof.Memory.getD0_eq_getElem _ _
      (by simpa using hi : i < (PersistentPadding.lengthBytes input).size),
    PersistentPadding.lengthByte input i hi]
  congr 1
...
theorem lengthLoopMemory_getD (input : ByteArray) (hfit : CalldataFits input)
    ...
    (lengthLoopMemory input i)[a]?.getD 0 =
      if (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) ≤ a ∧ a < (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + i then
        (PersistentPadding.lengthBytes input)[a - (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8)]?.getD 0
      else if a = (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + 7 then
        0
  ...
  | zero =>
      ...
      by_cases h : a = (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + 7
  ...
  | succ i ih =>
      ...
      simp only [oneByte_size]
      by_cases hin : (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + i ≤ a ∧ a < (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + i + 1
      · have haeq : a = (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + i := by omega
        subst haeq
        rw [if_pos (by omega), if_pos (by omega), Nat.sub_self,
          Nat.add_sub_cancel_left]
        rfl
      · rw [if_neg (by omega)]
        by_cases hlt : (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) ≤ a ∧ a < (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + i
        · rw [if_pos hlt, if_pos (by omega)]
        · have hne : ¬ ((PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) ≤ a ∧ a < (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + (i + 1)) := by
            omega
...
theorem padFinalMemory_getD (input : ByteArray) (_hfit : CalldataFits input) (a : Nat) :
    (padFinalMemory input)[a]?.getD 0 =
      if (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) ≤ a ∧ a < (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + 8 then
        (PersistentPadding.lengthBytes input)[a - (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8)]?.getD 0
      else (padSentinel input).memory[a]?.getD 0 := by
  have hsentinel : (padSentinel input).memory =
      PersistentPadding.sentinelMemory (padLengthReady input).memory input := by
    simp [padSentinel, padCopied, PersistentPadding.sentinelMemory,
      PersistentPadding.copiedMemory, Challenge.EvmProof.Memory.readPadded_zero_size]
  rw [padFinalMemory, PersistentPadding.paddedMemory,
    MachineState.writeBytes_getElem?_getD, hsentinel]
  simp only [PersistentPadding.lengthBytes_size]

theorem lengthLoopMemory_size (input : ByteArray) (hfit : CalldataFits input)
    (i : Nat) (hi : i ≤ 8) :
    (lengthLoopMemory input i).size = (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + 8 := by
...
theorem padFinalMemory_size (input : ByteArray) (hfit : CalldataFits input) :
    (padFinalMemory input).size = (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + 8 := by
  have hs := sentinel_size_le input hfit
  have hsentinel : (padSentinel input).memory =
      PersistentPadding.sentinelMemory (padLengthReady input).memory input := by
    simp [padSentinel, padCopied, PersistentPadding.sentinelMemory,
      PersistentPadding.copiedMemory, Challenge.EvmProof.Memory.readPadded_zero_size]
  rw [padFinalMemory, PersistentPadding.paddedMemory, MachineState.writeBytes_size,
    ← hsentinel]
  simp only [PersistentPadding.lengthBytes_size, if_neg (by decide : ¬ (8 = 0))]
  omega
...
theorem lengthLoopMemory_final (input : ByteArray) (hfit : CalldataFits input)
  ...
  · intro a h₁ h₂
    ...
    by_cases hin : (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) ≤ a ∧ a < (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + i
    · rw [if_pos hin, if_pos ⟨hin.1, by omega⟩]
    · rw [if_neg hin]
      by_cases htop : a = (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + 7
      · rw [if_pos htop, if_pos (by omega)]
        have h7 : a - (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) = 7 := by
          omega
        rw [h7, lengthBytes_of_shift_zero input hfit i 7 (by omega)
          (by norm_num) hz]
      · rw [if_neg htop]
        by_cases hwin : (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) ≤ a ∧ a < (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) + 8
        · rw [if_pos hwin]
          have hge : i ≤ a - (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8) := by omega
          rw [lengthBytes_of_shift_zero input hfit i (a - (PersistentPadding.messageOffset + PersistentPadding.paddedLength input.size - 8)) hge
            (by omega) hz]
...
def gasSteps_lengthExitEntered (input : ByteArray) (hfit : CalldataFits input)
  ...
  exact Challenge.EvmProof.GasSteps.cast g1raw rfl
...
private theorem run_lengthFooterSetup (input : ByteArray) :
  ...
  have haddressOrder : UInt256.ofNat 1624 + PersistentPadding.paddedWord input =
      PersistentPadding.paddedWord input + UInt256.ofNat 1624 := Challenge.EvmProof.Word.word_add_comm _ _
  simp [lengthFooterSetupPath, lengthSetupPath, PersistentPaddingArtifact.padSetupPath,
    Challenge.EvmProof.Stepper.runLocatedBlock,
    Challenge.EvmProof.Stepper.runLocated, Challenge.EvmProof.Stepper.runInstr,
...
private def gasSteps_padPrefix (input : ByteArray)
  ...
  (PersistentMain.gasSteps_initialize input entryPrefix).trans
...
theorem padReturned_memory (input : ByteArray) (_hfit : CalldataFits input) :
    (padReturned input).memory =
      PersistentPadding.paddedMemory (padLengthReady input).memory input := by
  rfl

#print axioms gasSteps_pad

end Challenge.Ripemd160.Submission.Proofs.Bytecode.PersistentPaddingTrace
