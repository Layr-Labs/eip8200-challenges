...
theorem actual_slice : (Artifact.submissionArtifact.instructions.drop 3900).take template.length = template := by rfl
def site : GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3900 actual_slice
    (by change 3900 + template.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := template) (by decide)) (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4832 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3900) = UInt256.ofNat 4832
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem site_pc : site.startPC = UInt256.ofNat 4853 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3912) = UInt256.ofNat 4853
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem site_pc : site.startPC = UInt256.ofNat 4855 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3914) = UInt256.ofNat 4855
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_4796 (s : State) (e : Env s) : Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 4832).toNat = true := by
  ...
  have hp : Artifact.submissionArtifact.instructionPC 3900 = 4832 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]
    decide
  simpa only [hp, Word.word_toNat_ofNat, Nat.reducePow, Nat.reduceMod] using h
