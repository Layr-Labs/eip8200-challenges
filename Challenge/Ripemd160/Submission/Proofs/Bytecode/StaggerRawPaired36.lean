...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v2 (UInt256.shiftRight (UInt256.mul x.v9 x.v8) (UInt256.ofNat 28))),
    ...
    x.v0,
    x.v7,
    (UInt256.land x.v2 (UInt256.add x.v7 (UInt256.shiftRight (UInt256.mul x.v9 (UInt256.land x.v2 (UInt256.add x.v4 (UInt256.add (MachineState.readWord memory 810) (UInt256.add (UInt256.xor x.v0 (UInt256.lor (UInt256.lnot x.v8) x.v5)) x.v6))))) (UInt256.ofNat 24)))),
    x.v9,
    x.v10,
    x.v11,
    x.v12,
...
theorem site_pc : site.startPC = UInt256.ofNat 2804 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2112) = UInt256.ofNat 2804
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2804, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 2838, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 2804) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 2804) template = UInt256.ofNat 2838 := by decide
  rw [hend] at hraw
