...
theorem site_pc : site.startPC = UInt256.ofNat 4743 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3802) = UInt256.ofNat 4743
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
  ...
  apply DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 4743, stack := StaggerPersistentFrame.frame h off limit rho}
    {s with pc := UInt256.ofNat 4759, stack := packedHash h :: off :: limit :: rho}
    hcode hfork hrun hnp site_pc.symm advances
  have hraw := run_template s (UInt256.ofNat 4743) off limit h rho hstack hrun
  have hend : pcAfter (UInt256.ofNat 4743) template = UInt256.ofNat 4759 := by decide
  rw [hend] at hraw
  exact hraw
