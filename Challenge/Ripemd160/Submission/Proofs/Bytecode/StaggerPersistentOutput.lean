...
theorem site_pc : site.startPC = UInt256.ofNat 4651 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3717) = UInt256.ofNat 4651
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4651, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4674, stack := packedHash h :: off :: limit :: rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 4651, stack := StaggerPersistentFrame.frame h off limit rho}
    {s with pc := UInt256.ofNat 4674, stack := packedHash h :: off :: limit :: rho}
    hcode hfork hrun hnp site_pc.symm advances
  have hraw := run_template s (UInt256.ofNat 4651) off limit h rho hstack hrun
  have hend : pcAfter (UInt256.ofNat 4651) template = UInt256.ofNat 4674 := by decide
  rw [hend] at hraw
  exact hraw
