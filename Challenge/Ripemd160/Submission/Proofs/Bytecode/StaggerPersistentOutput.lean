...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3812).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3812 actual_slice
    (by change 3812 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 4755 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3812) = UInt256.ofNat 4755
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4755, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4778, stack := packedHash h :: off :: limit :: rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 4755, stack := StaggerPersistentFrame.frame h off limit rho}
    {s with pc := UInt256.ofNat 4778, stack := packedHash h :: off :: limit :: rho}
    hcode hfork hrun hnp site_pc.symm advances
  have hraw := run_template s (UInt256.ofNat 4755) off limit h rho hstack hrun
  have hend : pcAfter (UInt256.ofNat 4755) template = UInt256.ofNat 4778 := by decide
  rw [hend] at hraw
  exact hraw
