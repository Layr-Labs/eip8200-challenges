...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3553).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3553 actual_slice
    (by change 3553 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 4645 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3553) = UInt256.ofNat 4645
