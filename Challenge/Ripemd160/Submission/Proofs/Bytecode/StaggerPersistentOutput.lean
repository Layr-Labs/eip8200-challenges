...
theorem site_pc : site.startPC = UInt256.ofNat 4645 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3711) = UInt256.ofNat 4645
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4645, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4668, stack := packedHash h :: off :: limit :: rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    ...
    {s with pc := UInt256.ofNat 4668, stack := packedHash h :: off :: limit :: rho}
