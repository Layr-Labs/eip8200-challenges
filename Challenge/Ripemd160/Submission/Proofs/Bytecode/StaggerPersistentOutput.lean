...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3618).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3618 actual_slice
    (by change 3618 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 4644 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3618) = UInt256.ofNat 4644
