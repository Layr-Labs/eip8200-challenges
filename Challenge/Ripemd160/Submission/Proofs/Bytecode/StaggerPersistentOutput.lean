...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3542).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3542 actual_slice
    (by change 3542 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 4645 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3542) = UInt256.ofNat 4645
