...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3714).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3714 actual_slice
    (by change 3714 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 4641 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3714) = UInt256.ofNat 4641
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4641, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4664, stack := packedHash h :: off :: limit :: rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    ...
    {s with pc := UInt256.ofNat 4664, stack := packedHash h :: off :: limit :: rho}
