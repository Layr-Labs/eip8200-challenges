...
theorem site_pc : site.startPC = UInt256.ofNat 4663 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3613) = UInt256.ofNat 4663
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4663, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4686, stack := packedHash h :: off :: limit :: rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 4663, stack := StaggerPersistentFrame.frame h off limit rho}
    {s with pc := UInt256.ofNat 4686, stack := packedHash h :: off :: limit :: rho}
    hcode hfork hrun hnp site_pc.symm advances
  have hraw := run_template s (UInt256.ofNat 4663) off limit h rho hstack hrun
  have hend : pcAfter (UInt256.ofNat 4663) template = UInt256.ofNat 4686 := by decide
  rw [hend] at hraw
  exact hraw
