...
theorem site_pc : site.startPC = UInt256.ofNat 4638 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3520) = UInt256.ofNat 4638
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4638, stack := StaggerPersistentFrame.frame h off limit rho}
      {prepared s h with pc := UInt256.ofNat 4661, stack := packedHash h :: off :: limit :: rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    ...
    {prepared s h with pc := UInt256.ofNat 4661, stack := packedHash h :: off :: limit :: rho}
