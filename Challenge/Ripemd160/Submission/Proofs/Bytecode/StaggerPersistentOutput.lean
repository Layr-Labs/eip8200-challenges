...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3614).take template.length = template := by rfl
...
theorem site_pc : site.startPC = UInt256.ofNat 4899 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3614) = UInt256.ofNat 4899
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4899, stack := StaggerPersistentFrame.frame h off limit rho}
      {prepared s h with pc := UInt256.ofNat 4922, stack := packedHash h :: off :: limit :: rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    {s with pc := UInt256.ofNat 4899, stack := StaggerPersistentFrame.frame h off limit rho}
    {prepared s h with pc := UInt256.ofNat 4922, stack := packedHash h :: off :: limit :: rho}
    hcode hfork hrun hnp site_pc.symm advances
  have hraw := run_template s (UInt256.ofNat 4899) off limit h rho hstack hrun
  have hend : pcAfter (UInt256.ofNat 4899) template = UInt256.ofNat 4922 := by decide
  rw [hend] at hraw
  exact hraw
