...
theorem site_pc : site.startPC = UInt256.ofNat 4644 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3616) = UInt256.ofNat 4644
...
def gasSteps (s : State) (off limit : UInt256) (h : Compression.HashState)
    ...
    GasSteps {s with pc := UInt256.ofNat 4644, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4667, stack := packedHash h :: off :: limit :: rho} := by
  apply DenseScheduleLift.gasSteps_of_raw site
    ...
    {s with pc := UInt256.ofNat 4667, stack := packedHash h :: off :: limit :: rho}
