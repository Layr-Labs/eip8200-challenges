...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 3687).take template.length = template := by rfl
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3687 actual_slice
    (by change 3687 + template.length ≤ Artifact.submissionInstructions.length
...
theorem site_pc : site.startPC = UInt256.ofNat 4591 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3687) = UInt256.ofNat 4591
