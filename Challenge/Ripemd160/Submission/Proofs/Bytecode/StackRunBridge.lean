...
def compressionSeam (kernel : BlockKernel) :
    ∀ input : ByteArray, CalldataFits input → 0 < input.size →
      DirectCorrect.CompressionSeam input :=
  fun input hfit hpositive => CompressionSeamBridge.toCompressionSeam
    (compressionRun kernel input hfit hpositive)
...
theorem correct_of_block_kernel (kernel : BlockKernel)
  ...
  exact FastOutputResultBridge.correct_of_compression_trace_at input hfit
