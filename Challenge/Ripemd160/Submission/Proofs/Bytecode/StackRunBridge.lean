...
structure BlockContext (s : State) (input : ByteArray) (i : Nat)
  ...
  separated : ∀ k, k < 16 →
    0x400 ≤ (Schedule.loadOffsetWord (DriverTrace.messageOffsetWord i) k).toNat
...
theorem states_word_above (kernel : BlockKernel) (input : ByteArray)
    (address : Nat) (haddress : 0x400 ≤ address) :
...
private theorem padReturned_word_below (input : ByteArray)
  ...
  unfold wordAt
  rw [PaddingTrace.padReturned_readWord input hfit address (by
    have hpadded := Padding.input_and_footer_fit input.size
    unfold Padding.messageOffset at haddress ⊢
    omega)]
...
private theorem blockSeparated (input : ByteArray) (hfit : CalldataFits input)
    ...
    ∀ k, k < 16 →
      0x400 ≤ (Schedule.loadOffsetWord (DriverTrace.messageOffsetWord n) k).toNat := by
...
private theorem hashAt32_of_hashWords
  ...
  rw [show wordAt s 832 = OutputTrace.hWord s 0 by rfl,
    show wordAt s 864 = OutputTrace.hWord s 1 by rfl,
    show wordAt s 896 = OutputTrace.hWord s 2 by rfl,
    show wordAt s 928 = OutputTrace.hWord s 3 by rfl,
    show wordAt s 960 = OutputTrace.hWord s 4 by rfl,
...
theorem correct_of_block_kernel (kernel : BlockKernel)
    ...
    (entryPrefix : GasSteps (initialState submissionBytecode input 0) (Execution.atPC input 271)) :
    ∃ g₀ : Nat, ∀ gas : Nat, g₀ ≤ gas →
