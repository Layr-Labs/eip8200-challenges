...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound64
...
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound64.outputStack memory (input q k) rho =
      stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 420) k q) (nextKey k) rho := by
  simp only [Table80RawRound64.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired80Algorithm.physicalKey 64) =
     Paired80Algorithm.physicalKey 65 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired80Algorithm.physicalKey 64) q = Paired80Algorithm.step 64 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 3939, stack := stack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 3980, stack :=
          stack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord s.memory 420) k q) (nextKey k) rho} := by
   have gs := Table80RawRound64.gasSteps s (input q k) rho hstack hrun hactive hcode hfork hnp
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound64
