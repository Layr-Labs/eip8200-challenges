...
theorem post_slice :
    (Artifact.submissionArtifact.instructions.drop 3791).take postTemplate.length = postTemplate := by rfl

def postSite : GenericRoundSite Artifact.submissionArtifact .Osaka postTemplate :=
  StackSiteBuilder.ofSlice postTemplate 3791 post_slice
    (by change 3791 + postTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem post_pc : postSite.startPC = UInt256.ofNat 4729 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3791) = UInt256.ofNat 4729
...
def gasSteps_continue (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  have gp : GasSteps {s with pc := UInt256.ofNat 4729, stack := StaggerPersistentFrame.frame h off limit rho}
    ...
    · exact run_continue s (UInt256.ofNat 4729) h off limit rho 465 hstack hrun hmiss (valid_loop s hcode)
...
def gasSteps_exit (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  apply PersistentLoopLift.gasSteps_of_raw postSite {s with pc := UInt256.ofNat 4729, stack := StaggerPersistentFrame.frame h off limit rho} _ hcode hfork hrun hnp post_pc.symm
  · exact PersistentLoopLift.advancesAll_sound _ (by decide)
  · have hr := run_exit s (UInt256.ofNat 4729) h off limit rho 465 hstack hrun hhit
    have he : pcAfter (UInt256.ofNat 4729) (StaggerPersistentLoopRaw.template 465) = UInt256.ofNat 4742 := by decide
    rw [he] at hr
    exact hr
