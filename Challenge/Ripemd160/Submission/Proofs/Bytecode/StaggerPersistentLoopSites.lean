...
def postTemplate : List Instr := StaggerPersistentLoopRaw.template 481
...
theorem post_pc : postSite.startPC = UInt256.ofNat 4614 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3704) = UInt256.ofNat 4614
...
theorem join_pc : joinSite.startPC = UInt256.ofNat 481 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 269) = UInt256.ofNat 481
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

theorem valid_loop (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 481).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 269 = 481 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 269 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 481 = true
  rw [hcode]
  exact h
...
def gasSteps_join (s : State) (rho : List UInt256) (hstack : rho.length < 1024)
    ...
    GasSteps {s with pc := UInt256.ofNat 481, stack := rho}
      {s with pc := UInt256.ofNat 482, stack := rho} := by
  apply PadLift.gasSteps_of_raw joinSite {s with pc := UInt256.ofNat 481, stack := rho} _
...
def gasSteps_continue (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  have gp : GasSteps {s with pc := UInt256.ofNat 4614, stack := StaggerPersistentFrame.frame h off limit rho}
    ...
    · exact run_continue s (UInt256.ofNat 4614) h off limit rho 481 (by omega) hrun hmiss (valid_loop s hcode)
...
def gasSteps_bound (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  apply RecognitionLift.gasSteps_of_raw postSite {s with pc := UInt256.ofNat 4614, stack := StaggerPersistentFrame.frame h off limit rho} _ hcode hfork hrun hnp post_pc.symm
  · exact RecognitionLift.advancesAll_sound _ (by decide)
  · have hr := run_exit s (UInt256.ofNat 4614) h off limit rho 481 (by omega) hrun hhit
    have he : pcAfter (UInt256.ofNat 4614) (StaggerPersistentLoopRaw.template 481) = UInt256.ofNat 4626 := by decide
    rw [he] at hr
    exact hr
