...
theorem join_pc : joinSite.startPC = UInt256.ofNat 521 := by
...
def gasSteps_join (s : State) (rho : List UInt256) (hstack : rho.length < 1024)
    ...
    GasSteps {s with pc := UInt256.ofNat 521, stack := rho}
      {s with pc := UInt256.ofNat 494, stack := rho} := by
  apply PadLift.gasSteps_of_raw joinSite {s with pc := UInt256.ofNat 521, stack := rho} _
...
def gasSteps_continue (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  have gp : GasSteps {s with pc := UInt256.ofNat 4662, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 521, stack := StaggerPersistentFrame.frame h (nextOffset off) limit rho} := by
    ...
    · exact run_continue s (UInt256.ofNat 4634) h off limit rho 465 (by omega) hrun hmiss (valid_loop s hcode)
...
def gasSteps_bound (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  apply RecognitionLift.gasSteps_of_raw postSite {s with pc := UInt256.ofNat 4662, stack := StaggerPersistentFrame.frame h off limit rho} _ hcode hfork hrun hnp post_pc.symm
  · exact RecognitionLift.advancesAll_sound _ (by decide)
  · have hr := run_exit s (UInt256.ofNat 4634) h off limit rho 465 (by omega) hrun hhit
    have he : pcAfter (UInt256.ofNat 4662) (StaggerPersistentLoopRaw.template 465) = UInt256.ofNat 4646 := by decide
    rw [he] at hr
    exact hr
