...
theorem post_slice :
    (Artifact.submissionArtifact.instructions.drop 3707).take postTemplate.length = postTemplate := by rfl

def postSite : GenericRoundSite Artifact.submissionArtifact .Osaka postTemplate :=
  StackSiteBuilder.ofSlice postTemplate 3707 post_slice
    (by change 3707 + postTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem post_pc : postSite.startPC = UInt256.ofNat 4631 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3707) = UInt256.ofNat 4631
