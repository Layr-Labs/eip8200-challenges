...
def postTemplate : List Instr := StaggerPersistentLoopRaw.template 4614
...
theorem valid_exit (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
    Decode.isValidJumpDest s.executionEnv.code (UInt256.ofNat 4614).toNat = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 3836 = 4614 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
  have h := Artifact.submissionArtifact.isValidJumpDest_index 3836 (by rfl)
  rw [hpc] at h
  change Decode.isValidJumpDest s.executionEnv.code 4614 = true
  rw [hcode]
  exact h
...
def gasSteps_continue (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  have gp : GasSteps {s with pc := UInt256.ofNat 471, stack := StaggerPersistentFrame.frame h off limit rho}
    ...
    · apply PadLift.advancesAll_sound
      decide
    · have hraw := run_continue s (UInt256.ofNat 471) h off limit rho 4614 hstack hrun hmiss
      have hend : pcAfter (UInt256.ofNat 471) (StaggerPersistentLoopRaw.template 4614) = UInt256.ofNat 484 := by decide
      rw [hend] at hraw
      exact hraw
...
def gasSteps_exit (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  · apply PadLift.advancesAll_sound
    decide
  · exact run_exit s (UInt256.ofNat 471) h off limit rho 4614 hstack hrun hhit (valid_exit s hcode)
