...
theorem post_slice :
    (Artifact.submissionArtifact.instructions.drop 3640).take postTemplate.length = postTemplate := by rfl

def postSite : GenericRoundSite Artifact.submissionArtifact .Osaka postTemplate :=
  StackSiteBuilder.ofSlice postTemplate 3640 post_slice
    (by change 3640 + postTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem post_pc : postSite.startPC = UInt256.ofNat 4557 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3640) = UInt256.ofNat 4557
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

def joinTemplate : List Instr := [.op .JUMPDEST]

theorem join_slice :
    (Artifact.submissionArtifact.instructions.drop 285).take joinTemplate.length = joinTemplate := by rfl

def joinSite : GenericRoundSite Artifact.submissionArtifact .Osaka joinTemplate :=
  StackSiteBuilder.ofSlice joinTemplate 285 join_slice
    (by change 285 + joinTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem join_pc : joinSite.startPC = UInt256.ofNat 473 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 285) = UInt256.ofNat 473
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_loop (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 285 = 473 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_continue (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  have gp : GasSteps {s with pc := UInt256.ofNat 4557, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 473, stack := StaggerPersistentFrame.frame h (nextOffset off) limit rho} := by
    apply RecognitionLift.gasSteps_of_raw postSite {s with pc := UInt256.ofNat 4557, stack := StaggerPersistentFrame.frame h off limit rho} _ hcode hfork hrun hnp post_pc.symm
    · exact RecognitionLift.advancesAll_sound _ (by decide)
    · exact run_continue s (UInt256.ofNat 4557) h off limit rho 473 (by omega) hrun hmiss (valid_loop s hcode)
...
def gasSteps_bound (s : State) (h : Compression.HashState) (off limit : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4557, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4569, stack := StaggerPersistentFrame.frame h (nextOffset off) limit rho} := by
  apply RecognitionLift.gasSteps_of_raw postSite {s with pc := UInt256.ofNat 4557, stack := StaggerPersistentFrame.frame h off limit rho} _ hcode hfork hrun hnp post_pc.symm
  · exact RecognitionLift.advancesAll_sound _ (by decide)
  · have hr := run_exit s (UInt256.ofNat 4557) h off limit rho 473 (by omega) hrun hhit
    have he : pcAfter (UInt256.ofNat 4557) (StaggerPersistentLoopRaw.template 473) = UInt256.ofNat 4569 := by decide
    rw [he] at hr
    exact hr
def gasSteps_pad (s : State) (h : Compression.HashState) (off limit : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4557, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4710, stack := StaggerPersistentFrame.frame h (nextOffset off) limit rho} := by
...
def gasSteps_exit (s : State) (h : Compression.HashState) (off limit : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4557, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4576, stack := StaggerPersistentFrame.frame h (nextOffset off) limit rho} := by
  exact (gasSteps_bound s h off limit rho hstack hrun hbound hcode hfork hnp).trans
