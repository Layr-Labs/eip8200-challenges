...
theorem post_slice :
    (Artifact.submissionArtifact.instructions.drop 3523).take postTemplate.length = postTemplate := by rfl

def postSite : GenericRoundSite Artifact.submissionArtifact .Osaka postTemplate :=
  StackSiteBuilder.ofSlice postTemplate 3523 post_slice
    (by change 3523 + postTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem post_pc : postSite.startPC = UInt256.ofNat 4626 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3523) = UInt256.ofNat 4626
