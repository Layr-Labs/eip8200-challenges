...
theorem post_pc : postSite.startPC = UInt256.ofNat 4625 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3602) = UInt256.ofNat 4625
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

def joinTemplate : List Instr := [.op .JUMPDEST]

theorem join_slice :
    (Artifact.submissionArtifact.instructions.drop 284).take joinTemplate.length = joinTemplate := by rfl

def joinSite : GenericRoundSite Artifact.submissionArtifact .Osaka joinTemplate :=
  StackSiteBuilder.ofSlice joinTemplate 284 join_slice
    (by change 284 + joinTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem join_pc : joinSite.startPC = UInt256.ofNat 486 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 284) = UInt256.ofNat 486
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_loop (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 284 = 486 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_continue (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  have gp : GasSteps {s with pc := UInt256.ofNat 4625, stack := StaggerPersistentFrame.frame h off limit rho}
    ...
    · exact run_continue s (UInt256.ofNat 4625) h off limit rho 486 (by omega) hrun hmiss (valid_loop s hcode)
...
def gasSteps_bound (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  apply RecognitionLift.gasSteps_of_raw postSite {s with pc := UInt256.ofNat 4625, stack := StaggerPersistentFrame.frame h off limit rho} _ hcode hfork hrun hnp post_pc.symm
  · exact RecognitionLift.advancesAll_sound _ (by decide)
  · have hr := run_exit s (UInt256.ofNat 4625) h off limit rho 486 (by omega) hrun hhit
    have he : pcAfter (UInt256.ofNat 4625) (StaggerPersistentLoopRaw.template 486) = UInt256.ofNat 4637 := by decide
    rw [he] at hr
    exact hr
def gasSteps_pad (s : State) (h : Compression.HashState) (off limit : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4625, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4760, stack := StaggerPersistentFrame.frame h (nextOffset off) limit rho} := by
