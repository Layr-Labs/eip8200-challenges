...
theorem post_slice :
    (Artifact.submissionArtifact.instructions.drop 3571).take postTemplate.length = postTemplate := by rfl

def postSite : GenericRoundSite Artifact.submissionArtifact .Osaka postTemplate :=
  StackSiteBuilder.ofSlice postTemplate 3571 post_slice
    (by change 3571 + postTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem post_pc : postSite.startPC = UInt256.ofNat 4629 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3571) = UInt256.ofNat 4629
...
theorem join_pc : joinSite.startPC = UInt256.ofNat 494 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 257) = UInt256.ofNat 494
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
