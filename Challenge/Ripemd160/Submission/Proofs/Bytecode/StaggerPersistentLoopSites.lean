...
def postTemplate : List Instr := StaggerPersistentLoopRaw.template 485

theorem post_slice :
    (Artifact.submissionArtifact.instructions.drop 3699).take postTemplate.length = postTemplate := by rfl

def postSite : GenericRoundSite Artifact.submissionArtifact .Osaka postTemplate :=
  StackSiteBuilder.ofSlice postTemplate 3699 post_slice
    (by change 3699 + postTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem post_pc : postSite.startPC = UInt256.ofNat 4630 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3699) = UInt256.ofNat 4630
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide

def joinTemplate : List Instr := [.op .JUMPDEST]

theorem join_slice :
    (Artifact.submissionArtifact.instructions.drop 273).take joinTemplate.length = joinTemplate := by rfl

def joinSite : GenericRoundSite Artifact.submissionArtifact .Osaka joinTemplate :=
  StackSiteBuilder.ofSlice joinTemplate 273 join_slice
    (by change 273 + joinTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
...
theorem join_pc : joinSite.startPC = UInt256.ofNat 485 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 273) = UInt256.ofNat 485
  rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
theorem valid_loop (s : State) (hcode : s.executionEnv.code = Artifact.submissionArtifact.code) :
  ...
  have hpc : Artifact.submissionArtifact.instructionPC 273 = 485 := by
    rw [ArtifactByteLength.instructionPC_eq_byteLength]; decide
...
def gasSteps_join (s : State) (rho : List UInt256) (hstack : rho.length < 1024)
    ...
    GasSteps {s with pc := UInt256.ofNat 485, stack := rho}
      {s with pc := UInt256.ofNat 486, stack := rho} := by
...
def gasSteps_continue (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  have gp : GasSteps {s with pc := UInt256.ofNat 4630, stack := StaggerPersistentFrame.frame h off limit rho}
    ...
    · exact run_continue s (UInt256.ofNat 4630) h off limit rho 485 (by omega) hrun hmiss (valid_loop s hcode)
...
def gasSteps_bound (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  apply RecognitionLift.gasSteps_of_raw postSite {s with pc := UInt256.ofNat 4630, stack := StaggerPersistentFrame.frame h off limit rho} _ hcode hfork hrun hnp post_pc.symm
  · exact RecognitionLift.advancesAll_sound _ (by decide)
  · have hr := run_exit s (UInt256.ofNat 4630) h off limit rho 485 (by omega) hrun hhit
    have he : pcAfter (UInt256.ofNat 4630) (StaggerPersistentLoopRaw.template 485) = UInt256.ofNat 4642 := by decide
    rw [he] at hr
    exact hr
