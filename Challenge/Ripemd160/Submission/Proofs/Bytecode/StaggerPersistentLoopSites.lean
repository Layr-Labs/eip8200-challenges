...
theorem post_slice :
    (Artifact.submissionArtifact.instructions.drop 3505).take postTemplate.length = postTemplate := by rfl

def postSite : GenericRoundSite Artifact.submissionArtifact .Osaka postTemplate :=
  StackSiteBuilder.ofSlice postTemplate 3505 post_slice
    (by change 3505 + postTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := postTemplate) (by decide))
    (by decide)

theorem post_pc : postSite.startPC = UInt256.ofNat 4618 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3505) = UInt256.ofNat 4618
...
def gasSteps_continue (s : State) (h : Compression.HashState) (off limit : UInt256)
  ...
  have gp : GasSteps {s with pc := UInt256.ofNat 4618, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 508, stack := StaggerPersistentFrame.frame h (nextOffset off) limit rho} := by
    apply RecognitionLift.gasSteps_of_raw postSite {s with pc := UInt256.ofNat 4618, stack := StaggerPersistentFrame.frame h off limit rho} _ hcode hfork hrun hnp post_pc.symm
    · exact RecognitionLift.advancesAll_sound _ (by decide)
    · exact run_continue s (UInt256.ofNat 4618) h off limit rho 508 (by omega) hrun hmiss (valid_loop s hcode)
...
def gasSteps_bound (s : State) (h : Compression.HashState) (off limit : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4618, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4630, stack := StaggerPersistentFrame.frame h (nextOffset off) limit rho} := by
  apply RecognitionLift.gasSteps_of_raw postSite {s with pc := UInt256.ofNat 4618, stack := StaggerPersistentFrame.frame h off limit rho} _ hcode hfork hrun hnp post_pc.symm
  · exact RecognitionLift.advancesAll_sound _ (by decide)
  · have hr := run_exit s (UInt256.ofNat 4618) h off limit rho 508 (by omega) hrun hhit
    have he : pcAfter (UInt256.ofNat 4618) (StaggerPersistentLoopRaw.template 508) = UInt256.ofNat 4630 := by decide
    rw [he] at hr
    exact hr
def gasSteps_pad (s : State) (h : Compression.HashState) (off limit : UInt256)
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      s.executionEnv.fork s.executionEnv.codeAddr = false) :
    GasSteps {s with pc := UInt256.ofNat 4618, stack := StaggerPersistentFrame.frame h off limit rho}
      {s with pc := UInt256.ofNat 4693, stack := StaggerPersistentFrame.frame h (nextOffset off) limit rho} := by
