...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2896 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3658 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2896) = UInt256.ofNat 3662
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3658, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3697, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3658) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3658) template = UInt256.ofNat 3697 := by decide
  rw [hend] at hraw
