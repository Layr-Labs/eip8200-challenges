...
theorem site_pc : site.startPC = UInt256.ofNat 3671 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2848) = UInt256.ofNat 3671
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3671, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3711, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3671) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3671) template = UInt256.ofNat 3711 := by decide
  rw [hend] at hraw
