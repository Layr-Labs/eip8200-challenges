...
noncomputable opaque gasSteps_padding (input : ByteArray) (hfit : CalldataFits input) (hn32 : input.size≠32) (i : Nat) :
    GasSteps {lowState input i with pc:=UInt256.ofNat 4761,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (UInt256.ofNat input.size) maskRho}
      {paddedState input i with pc:=UInt256.ofNat 521,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (Padding.paddedWord input) maskRho} := by
  ...
  have gpad' : GasSteps
      {lowState input i with pc:=UInt256.ofNat 4761,stack:=frame h off (UInt256.ofNat input.size) maskRho}
      {paddedState input i with pc:=UInt256.ofNat 520,stack:=frame h off (Padding.paddedWord input) maskRho} := gp.trans gpad
