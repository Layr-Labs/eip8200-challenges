...
noncomputable opaque gasSteps_padding (input : ByteArray) (hfit : CalldataFits input) (i : Nat) :
    GasSteps {lowState input i with pc:=UInt256.ofNat 4704,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (UInt256.ofNat input.size) maskRho}
      {paddedState input i with pc:=UInt256.ofNat 495,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (Padding.paddedWord input) maskRho} := by
  ...
  have gpad:=PaddingTraceGeneral.gasSteps_padBody input (lowState input i)
    ...
    hcal hr hc hf hnp hfit
  have gpad' : GasSteps
      {lowState input i with pc:=UInt256.ofNat 4704,stack:=frame h off (UInt256.ofNat input.size) maskRho}
      {paddedState input i with pc:=UInt256.ofNat 494,stack:=frame h off (Padding.paddedWord input) maskRho} := gp.trans gpad
