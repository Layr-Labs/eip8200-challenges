...
noncomputable opaque gasSteps_padding (input : ByteArray) (hfit : CalldataFits input) (hn32 : input.size≠32) (i : Nat) :
  ...
  have gpad' : GasSteps
      ...
      {paddedState input i with pc:=UInt256.ofNat 486,stack:=frame h off (Padding.paddedWord input) maskRho} := gp.trans gpad
  have gj:=StaggerPersistentLoopSites.gasSteps_loop_join (paddedState input i)
