...
noncomputable opaque gasSteps_padding (input : ByteArray) (hfit : CalldataFits input) (i : Nat) :
    GasSteps {lowState input i with pc:=UInt256.ofNat 4705,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (Padding.paddedWord input) maskRho}
      {paddedState input i with pc:=UInt256.ofNat 490,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (Padding.paddedWord input) maskRho} := by
  ...
  have gpad' : GasSteps
      {lowState input i with pc:=UInt256.ofNat 4705,stack:=frame h off (Padding.paddedWord input) maskRho}
      {paddedState input i with pc:=UInt256.ofNat 489,stack:=frame h off (Padding.paddedWord input) maskRho} := gpad
