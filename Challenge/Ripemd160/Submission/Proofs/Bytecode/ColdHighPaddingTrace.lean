...
noncomputable opaque gasSteps_padding (input : ByteArray) (hfit : CalldataFits input) (hn32 : input.size≠32) (i : Nat) :
    GasSteps {lowState input i with pc:=UInt256.ofNat 4723,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (UInt256.ofNat input.size) maskRho}
  ...
  have gpad' : GasSteps
      {lowState input i with pc:=UInt256.ofNat 4723,stack:=frame h off (UInt256.ofNat input.size) maskRho}
