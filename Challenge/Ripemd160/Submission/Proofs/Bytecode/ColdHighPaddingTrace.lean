...
noncomputable opaque gasSteps_padding (input : ByteArray) (hfit : CalldataFits input) (hn32 : input.size≠32) (i : Nat) :
    GasSteps {lowState input i with pc:=UInt256.ofNat 4720,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (Padding.paddedWord input) maskRho}
      {paddedState input i with pc:=UInt256.ofNat 505,stack:=frame (hashes input i) (DriverTrace.blockOffsetWord i) (Padding.paddedWord input) maskRho} := by
  ...
  have gpad' : GasSteps
      {lowState input i with pc:=UInt256.ofNat 4720,stack:=frame h off (Padding.paddedWord input) maskRho}
      {paddedState input i with pc:=UInt256.ofNat 504,stack:=frame h off (Padding.paddedWord input) maskRho} := gpad
