import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
   StackSiteBuilder.ofSlice template 2610 actual_slice
     ...
     (by decide)
 theorem site_pc : site.startPC = UInt256.ofNat 3234 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2610) = UInt256.ofNat 3234
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
   ...
   have hraw := run_actual s (UInt256.ofNat 3234) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 3234) template = UInt256.ofNat 3280 := by decide
   rw [hend] at hraw
