...
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound24
...
theorem output_eq (memory : ByteArray) (q : WordLane) (k : UInt256) (rho : List UInt256) :
    Table80RawRound24.outputStack memory (input q k) rho =
      stack [.d, .e, .c, .b, .k, .a, .factor, .pair, .upper, .lower] (eval (MachineState.readWord memory 260) k q) (nextKey k) rho := by
  simp only [Table80RawRound24.outputStack, input, stack, List.map_cons, List.map_nil,
...
#print axioms output_eq
 ...
 theorem nextKey_physical : nextKey (Paired80Algorithm.physicalKey 24) =
     Paired80Algorithm.physicalKey 25 := by decide

 theorem eval_physical (message : UInt256) (q : WordLane) :
     eval message (Paired80Algorithm.physicalKey 24) q = Paired80Algorithm.step 24 message q := by rfl
 ...
 def gasSteps (s : State) (q : WordLane) (k : UInt256) (rho : List UInt256)
     ...
     Challenge.EvmProof.GasSteps {s with pc := UInt256.ofNat 2211, stack := stack [.d, .a, .b, .c, .k, .e, .factor, .pair, .upper, .lower] q k rho}
       {s with pc := UInt256.ofNat 2250, stack :=
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80CoreRound24
