...
Raw `prefixPath` (instructions 4082..4090) from the nonempty dispatcher
entry to `PrefixStateCodecopy.preCopyState`, the generic `CODECOPY` step,
then `firstComparePath` (4092..4096) ending at the compression entry on a
...
/-- State after the first comparison succeeds: pc 4999 (instruction 4097)
...
`JUMPI` is not taken and execution continues at pc 4999 (instruction 4097). -/
