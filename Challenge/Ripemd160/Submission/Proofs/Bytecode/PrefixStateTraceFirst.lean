...
Raw `prefixPath` (instructions 4013..4021) from the nonempty dispatcher
entry to `PrefixStateCodecopy.preCopyState`, the generic `CODECOPY` step,
then `firstComparePath` (4023..4027) ending at the compression entry on a
...
/-- State after the first comparison succeeds: pc 5252 (instruction 4028)
...
`JUMPI` is not taken and execution continues at pc 5252 (instruction 4028). -/
