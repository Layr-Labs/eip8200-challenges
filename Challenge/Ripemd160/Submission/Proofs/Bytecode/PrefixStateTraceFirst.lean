...
Raw `prefixPath` (instructions 4271..4279) from the nonempty dispatcher
entry to `PrefixStateCodecopy.preCopyState`, the generic `CODECOPY` step,
then `firstComparePath` (4281..4285) ending at the compression entry on a
...
/-- State after the first comparison succeeds: pc 5234 (instruction 4286)
...
theorem jumpDest_generic : Decode.isValidJumpDest submissionBytecode 464 = true := by
  have hpc : Artifact.submissionArtifact.instructionPC 272 = 463 := by
...
`JUMPI` is not taken and execution continues at pc 5234 (instruction 4286). -/
