...
Raw `prefixPath` (instructions 4270..4278) from the nonempty dispatcher
entry to `PrefixStateCodecopy.preCopyState`, the generic `CODECOPY` step,
then `firstComparePath` (4280..4284) ending at the compression entry on a
...
/-- State after the first comparison succeeds: pc 5219 (instruction 4285)
...
`JUMPI` is not taken and execution continues at pc 5219 (instruction 4285). -/
