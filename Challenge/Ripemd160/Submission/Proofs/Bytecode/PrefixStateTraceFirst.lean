...
Raw `prefixPath` (instructions 4274..4282) from the nonempty dispatcher
entry to `PrefixStateCodecopy.preCopyState`, the generic `CODECOPY` step,
then `firstComparePath` (4284..4288) ending at the compression entry on a
...
/-- State after the first comparison succeeds: pc 5221 (instruction 4289)
...
`JUMPI` is not taken and execution continues at pc 5221 (instruction 4289). -/
