...
Raw `prefixPath` (instructions 4261..4269) from the nonempty dispatcher
entry to `PrefixStateCodecopy.preCopyState`, the generic `CODECOPY` step,
then `firstComparePath` (4271..4275) ending at the compression entry on a
...
/-- State after the first comparison succeeds: pc 5210 (instruction 4276)
...
`JUMPI` is not taken and execution continues at pc 5210 (instruction 4276). -/
