...
Raw `prefixPath` (instructions 4216..4224) from the nonempty dispatcher
entry to `PrefixStateCodecopy.preCopyState`, the generic `CODECOPY` step,
then `firstComparePath` (4226..4230) ending at the compression entry on a
...
/-- State after the first comparison succeeds: pc 5241 (instruction 4231)
...
`JUMPI` is not taken and execution continues at pc 5241 (instruction 4231). -/
