...
Raw `prefixPath` (instructions 4134..4142) from the nonempty dispatcher
entry to `PrefixStateCodecopy.preCopyState`, the generic `CODECOPY` step,
then `firstComparePath` (4144..4148) ending at the compression entry on a
...
/-- State after the first comparison succeeds: pc 5252 (instruction 4149)
...
`JUMPI` is not taken and execution continues at pc 5252 (instruction 4149). -/
