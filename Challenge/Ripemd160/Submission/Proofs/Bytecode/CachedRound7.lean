...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 876).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 876 actual_slice
    (by change 876 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 1232 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 876) = UInt256.ofNat 1232
