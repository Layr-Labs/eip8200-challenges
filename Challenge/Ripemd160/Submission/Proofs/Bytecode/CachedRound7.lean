...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 774).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 774 actual_slice
    (by change 774 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 1066 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 774) = UInt256.ofNat 1066
