...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 3202 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 4083 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3202) = UInt256.ofNat 4083
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 4083, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 4126, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 4083) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 4083) template = UInt256.ofNat 4126 := by decide
  rw [hend] at hraw
