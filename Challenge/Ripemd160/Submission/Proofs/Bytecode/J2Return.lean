...
private theorem size_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 323 .MSIZE
    (by rfl) (by decide) trivial
  apply Artifact.submissionArtifact.state_decodedOp_of
    (copied (beforeCopy s rho) (source s) rho) 323 e.code ?_ .MSIZE none hd (by change Operation.MSIZE.availableInFork s.fork = true; rw [e.fork]; rfl)
  change (UInt256.ofNat 565).toNat = Artifact.submissionArtifact.instructionPC 323
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
    ...
    GasSteps (atState s 545 rho) (output s rho) := by
  have hp : StackRoundTrace.runInstrSeq J2ReturnSites.selector.template
      (atState s 545 rho) = some (beforeCopy s rho) := by
    have h := run_prefix s (UInt256.ofNat 545) rho (by omega) e.run
  ...
  have hf : StackRoundTrace.runInstrSeq J2ReturnSites.returned.template
      (atState (afterCopy s rho) 566 (32 :: rho)) = some (output s rho) :=
    run_finish (afterCopy s rho) (UInt256.ofNat 566) rho (by omega) e.run
  have hpc : (UInt256.ofNat 564).succ.succ = UInt256.ofNat 566 := by decide
...
theorem output_spec (s : State) (e : Env s) (rho : List UInt256)
  ...
  apply returned_spec (beforeCopy s rho) (source s) (UInt256.ofNat 566) rho
  ...
  have hsz : Artifact.submissionArtifact.code.size = 5235 := by
    change submissionBytecode.size = 5235
    rw [referenceBytecode_size]
  have hsrc : source s = selected (UInt256.ofNat 5235) s.executionEnv.calldata.size := by
