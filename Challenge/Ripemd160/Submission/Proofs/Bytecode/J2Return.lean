...
private theorem copy_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  change (UInt256.ofNat 324).toNat = Artifact.submissionArtifact.instructionPC 213
...
private theorem size_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  change (UInt256.ofNat 325).toNat = Artifact.submissionArtifact.instructionPC 214
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
    ...
    GasSteps (atState s 303 rho) (output s rho) := by
  have hp : StackRoundTrace.runInstrSeq J2ReturnSites.selector.template
      (atState s 303 rho) = some (beforeCopy s rho) := by
    have h := run_prefix s (UInt256.ofNat 303) (UInt256.ofNat 4940) rho (by omega) e.run
