...
private theorem size_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 210 .MSIZE
    (by rfl) (by decide) trivial
  apply Artifact.submissionArtifact.state_decodedOp_of
    (copied (beforeCopy s rho) (source s) rho) 210 e.code ?_ .MSIZE none hd (by change Operation.MSIZE.availableInFork s.fork = true; rw [e.fork]; rfl)
  change (UInt256.ofNat 324).toNat = Artifact.submissionArtifact.instructionPC 210
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
    ...
    GasSteps (atState s 304 rho) (output s rho) := by
  have hp : StackRoundTrace.runInstrSeq J2ReturnSites.selector.template
      (atState s 304 rho) = some (beforeCopy s rho) := by
    have h := run_prefix s (UInt256.ofNat 304) rho (by omega) e.run
  ...
  have hf : StackRoundTrace.runInstrSeq J2ReturnSites.returned.template
      (atState (afterCopy s rho) 325 (32 :: rho)) = some (output s rho) :=
    run_finish (afterCopy s rho) (UInt256.ofNat 325) rho (by omega) e.run
  have hpc : (UInt256.ofNat 323).succ.succ = UInt256.ofNat 325 := by decide
...
theorem output_spec (s : State) (e : Env s) (rho : List UInt256)
  ...
  apply returned_spec (beforeCopy s rho) (source s) (UInt256.ofNat 325) rho
  ...
  have hsz : Artifact.submissionArtifact.code.size = 5212 := by
    change submissionBytecode.size = 5212
    rw [referenceBytecode_size]
  have hsrc : source s = selected (UInt256.ofNat 5212) s.executionEnv.calldata.size := by
