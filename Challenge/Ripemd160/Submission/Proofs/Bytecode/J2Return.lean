...
private theorem size_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 3597 .MSIZE
    (by rfl) (by decide) trivial
  apply Artifact.submissionArtifact.state_decodedOp_of
    (copied (beforeCopy s rho) (source s) rho) 3597 e.code ?_ .MSIZE none hd (by change Operation.MSIZE.availableInFork s.fork = true; rw [e.fork]; rfl)
  change (UInt256.ofNat 4848).toNat = Artifact.submissionArtifact.instructionPC 3597
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
    ...
    GasSteps (atState s 4828 rho) (output s rho) := by
  have hp : StackRoundTrace.runInstrSeq J2ReturnSites.selector.template
      (atState s 4828 rho) = some (beforeCopy s rho) := by
    have h := run_prefix s (UInt256.ofNat 4828) rho (by omega) e.run
  ...
  have hf : StackRoundTrace.runInstrSeq J2ReturnSites.returned.template
      (atState (afterCopy s rho) 4849 (32 :: rho)) = some (output s rho) :=
    run_finish (afterCopy s rho) (UInt256.ofNat 4849) rho (by omega) e.run
  have hpc : (UInt256.ofNat 4847).succ.succ = UInt256.ofNat 4849 := by decide
...
theorem output_spec (s : State) (e : Env s) (rho : List UInt256)
  ...
  apply returned_spec (beforeCopy s rho) (source s) (UInt256.ofNat 4849) rho
  ...
  have hsz : Artifact.submissionArtifact.code.size = 5244 := by
    change submissionBytecode.size = 5244
    rw [referenceBytecode_size]
  have hsrc : source s = selected (UInt256.ofNat 5244) s.executionEnv.calldata.size := by
