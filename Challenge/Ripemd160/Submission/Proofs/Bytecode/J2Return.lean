...
private theorem copy_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  change (UInt256.ofNat 321).toNat = Artifact.submissionArtifact.instructionPC 209
...
private theorem size_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  change (UInt256.ofNat 322).toNat = Artifact.submissionArtifact.instructionPC 210
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
    ...
    GasSteps (atState s 302 rho) (output s rho) := by
  have hp : StackRoundTrace.runInstrSeq J2ReturnSites.selector.template
      (atState s 302 rho) = some (beforeCopy s rho) := by
    have h := run_prefix s (UInt256.ofNat 302) rho (by omega) e.run
  ...
  have hf : StackRoundTrace.runInstrSeq J2ReturnSites.returned.template
      (atState (afterCopy s rho) 323 (32 :: rho)) = some (output s rho) :=
    run_finish (afterCopy s rho) (UInt256.ofNat 323) rho (by omega) e.run
  have hpc : (UInt256.ofNat 321).succ.succ = UInt256.ofNat 323 := by decide
...
theorem output_spec (s : State) (e : Env s) (rho : List UInt256)
  ...
  apply returned_spec (beforeCopy s rho) (source s) (UInt256.ofNat 323) rho
  ...
  have hsz : Artifact.submissionArtifact.code.size = 5198 := by
    change submissionBytecode.size = 5198
    rw [referenceBytecode_size]
  have hsrc : source s = selected (UInt256.ofNat 5198) s.executionEnv.calldata.size := by
