...
import Challenge.Ripemd160.Submission.Proofs.Bytecode.J2ReturnSites
import Challenge.Ripemd160.Submission.Proofs.Bytecode.J2Payload

set_option warningAsError true
set_option maxRecDepth 100000
set_option maxHeartbeats 3000000
set_option linter.unusedSimpArgs false
namespace Challenge.Ripemd160.Submission.Proofs.Bytecode.J2Return
...
private theorem size_decoded (s : State) (e : Env s) (rho : List UInt256) :
  ...
  have hd := Artifact.submissionArtifact.decodeAt_op_index 3550 .MSIZE
    (by rfl) (by decide) trivial
  apply Artifact.submissionArtifact.state_decodedOp_of
    (copied (beforeCopy s rho) (source s) rho) 3550 e.code ?_ .MSIZE none hd (by change Operation.MSIZE.availableInFork s.fork = true; rw [e.fork]; rfl)
  change (UInt256.ofNat 4722).toNat = Artifact.submissionArtifact.instructionPC 3550
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def gasSteps (s : State) (e : Env s) (rho : List UInt256)
    ...
    GasSteps (atState s 4704 rho) (output s rho) := by
  have hp : StackRoundTrace.runInstrSeq J2ReturnSites.selector.template
      (atState s 4704 rho) = some (beforeCopy s rho) := by
    have h := run_prefix s (UInt256.ofNat 4704) rho (by omega) e.run
  ...
  have hf : StackRoundTrace.runInstrSeq J2ReturnSites.returned.template
      (atState (afterCopy s rho) 4723 (32 :: rho)) = some (output s rho) :=
    run_finish (afterCopy s rho) (UInt256.ofNat 4723) rho (by omega) e.run
  have hpc : (UInt256.ofNat 4721).succ.succ = UInt256.ofNat 4723 := by decide
...
theorem output_spec (s : State) (e : Env s) (rho : List UInt256)
  ...
  apply returned_spec (beforeCopy s rho) (source s) (UInt256.ofNat 4723) rho
  ...
  have hsz : Artifact.submissionArtifact.code.size = 5221 := by
    change submissionBytecode.size = 5221
    rw [referenceBytecode_size]
  have hsrc : source s = selected (UInt256.ofNat 5221) s.executionEnv.calldata.size := by
...
end Challenge.Ripemd160.Submission.Proofs.Bytecode.J2Return
