...
private theorem template_slice :
    (A.instructions.drop 2733).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2733 template_slice (by
    change 2733 + template.length ≤ Artifact.submissionInstructions.length
