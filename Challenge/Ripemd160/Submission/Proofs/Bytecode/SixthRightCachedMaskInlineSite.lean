...
private theorem template_slice :
    (A.instructions.drop 2548).take template.length = template := by rfl
...
def site : GenericRoundSite A .Osaka template :=
  StackSiteBuilder.ofSlice _ 2548 template_slice (by
    change 2548 + template.length ≤ Artifact.submissionInstructions.length
