import Challenge.Ripemd160.Submission.Proofs.Bytecode.Table80RawCommon
 ...
 theorem actual_slice :
     (Artifact.submissionArtifact.instructions.drop 3478).take template.length = template := by rfl
 def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
   StackSiteBuilder.ofSlice template 3478 actual_slice
     ...
     (by decide)
 theorem site_pc : site.startPC = UInt256.ofNat 4152 := by
   change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3478) = UInt256.ofNat 4152
 ...
 def gasSteps (s : State) (x : Input) (rho : List UInt256)
     ...
     GasSteps {s with pc := UInt256.ofNat 4152, stack := inputStack x rho}
       {s with pc := UInt256.ofNat 4196, stack := outputStack s.memory x rho} := by
   have hraw := run_actual s (UInt256.ofNat 4152) x rho hstack hrun hactive
   have hend : pcAfter (UInt256.ofNat 4152) template = UInt256.ofNat 4196 := by decide
   rw [hend] at hraw
