...
def site : StackRoundTemplate.GenericRoundSite Artifact.submissionArtifact .Osaka template :=
  StackSiteBuilder.ofSlice template 2382 actual_slice
    ...
    (by decide)
theorem site_pc : site.startPC = UInt256.ofNat 3102 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2382) = UInt256.ofNat 3102
...
def gasSteps (s : State) (x : Input) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 3102, stack := inputStack x rho}
      {s with pc := UInt256.ofNat 3160, stack := outputStack s.memory x rho} := by
  have hraw := run_actual s (UInt256.ofNat 3102) x rho hstack hrun hactive
  have hend : pcAfter (UInt256.ofNat 3102) template = UInt256.ofNat 3160 := by decide
  rw [hend] at hraw
