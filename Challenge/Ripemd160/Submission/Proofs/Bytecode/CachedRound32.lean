...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1887).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1887 actual_slice
    (by change 1887 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2371 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1887) = UInt256.ofNat 2371
