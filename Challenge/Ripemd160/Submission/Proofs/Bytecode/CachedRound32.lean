...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1911).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1911 actual_slice
    (by change 1911 + actualTemplate.length ≤ Artifact.submissionInstructions.length
        rw [Artifact.referenceInstructions_count]; decide)
    StackRoundData.artifact_code_bound
    (StackRoundData.templateWellFormed_mem (instructions := actualTemplate) (by decide))
    (by decide)
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2372 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1911) = UInt256.ofNat 2372
...
def outputStack (memory : ByteArray) (x : Input) (rho : List UInt256) : List UInt256 :=
  [ (UInt256.land x.v7 (UInt256.shiftRight (UInt256.mul x.v6 x.v3) (UInt256.ofNat 22))),
    x.v0,
    x.v2,
    (UInt256.land x.v7 (UInt256.add x.v5 (UInt256.shiftRight (UInt256.mul x.v6 (UInt256.add (UInt256.mul (UInt256.ofNat 3) (UInt256.land x.v9 (UInt256.land x.v7 (UInt256.add x.v0 (UInt256.add (UInt256.lor (MachineState.readWord memory 288) (MachineState.readWord memory 496)) (UInt256.add (UInt256.xor x.v4 (UInt256.lor (UInt256.lnot x.v3) x.v2)) x.v1)))))) (UInt256.land x.v7 (UInt256.add x.v0 (UInt256.add (UInt256.lor (MachineState.readWord memory 288) (MachineState.readWord memory 496)) (UInt256.add (UInt256.xor x.v4 (UInt256.lor (UInt256.lnot x.v3) x.v2)) x.v1)))))) (UInt256.ofNat 23)))),
    x.v4,
    x.v5,
    x.v6,
    x.v7,
...
theorem run_original (s : State) (x : Input) (rho : List UInt256)
    ...
    runInstrSeq originalTemplate {s with pc := UInt256.ofNat 2386, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 2432, stack := outputStack s.memory x rho} := by
...
theorem run_actual (s : State) (x : Input) (rho : List UInt256)
    (hstack : rho.length ≤ 996) (hrun : s.halt = .Running)
    (hactive : 23 ≤ s.activeWords.toNat) :
    runInstrSeq actualTemplate {s with pc := UInt256.ofNat 2372, stack := inputStack s.memory x rho} =
      some {s with pc := UInt256.ofNat 2417, stack := outputStack s.memory x rho} := by
  have hcap (n : Nat) (hn : n ≤ 26) : rho.length + n < 1024 := by omega
  have hactiveAt (address : Nat) (haddress : address ≤ 704) :
      UInt256.ofNat (MachineState.activeWordsAfter s.activeWords.toNat address 32) = s.activeWords :=
    active_schedule_preserved s.activeWords address hactive haddress
...
def gasSteps (s : State) (f : CoreFrame) (rho : List UInt256)
    ...
    GasSteps {s with pc := UInt256.ofNat 2372, stack := coreStack [.k, .a, .b, .c, .d, .e, .factor, .pair, .upper, .lower] f (cache s.memory ++ rho)}
      {s with pc := UInt256.ofNat 2417, stack := coreStack [.d, .k, .c, .b, .e, .a, .factor, .pair, .upper, .lower] (PairedAllInlineCoreTrace.inline32Block.eval s.memory f) (cache s.memory ++ rho)} := by
  ...
  have g := gasSteps_terminal_of_raw actualSite
    {s with pc := UInt256.ofNat 2372, stack := inputStack s.memory x rho} _
