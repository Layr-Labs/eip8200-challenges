...
theorem actual_slice :
    (Artifact.submissionArtifact.instructions.drop 1992).take actualTemplate.length = actualTemplate := by rfl
def actualSite : GenericRoundSite Artifact.submissionArtifact .Osaka actualTemplate :=
  StackSiteBuilder.ofSlice actualTemplate 1992 actual_slice
    (by change 1992 + actualTemplate.length ≤ Artifact.submissionInstructions.length
...
theorem actual_pc : actualSite.startPC = UInt256.ofNat 2531 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 1992) = UInt256.ofNat 2531
