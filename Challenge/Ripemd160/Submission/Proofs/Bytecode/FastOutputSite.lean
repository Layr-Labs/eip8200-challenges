...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 215).take
...
@[simp] theorem fastOutputSite_startPC :
    fastOutputSite.startPC = UInt256.ofNat 4721 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 215) =
    UInt256.ofNat 4721
...
@[simp] theorem fastOutputSite_endPC :
  ...
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3921) =
    UInt256.ofNat 4793
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3921)
  pc_eq := pc_toNat_instructionPC 3921
...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 4721) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 4721) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 4721)
...
private theorem runFastOutputReturn
    ...
    DataStepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 4721) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 4721) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 4721) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 4721, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 4721) rest hstack hrun)
  have hfull_raw :
  ...
  have hstate :
      ...
      FastOutputTrace.fastOutputReturned s (UInt256.ofNat 4721) rest :=
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 3921 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 3921 := by rfl
  have hlocated :
...
def gasSteps_fastOutput
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      ...
      GasSteps
      {s with pc := UInt256.ofNat 4721, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 4721) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 4721, stack := rest}
