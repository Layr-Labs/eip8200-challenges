...
@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x12b4 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2897) =
    UInt256.ofNat 0x12b4
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2897)
  pc_eq := pc_toNat_instructionPC 2897
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 0x12b4 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2897) =
    UInt256.ofNat 0x12b4
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 0x12b4 := fastOutputReturn_pc
...
private theorem runFastOutputReturn
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 2897 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 2897 := by rfl
  have hlocated :
