...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 2370).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 2370
    fastOutput_slice
    (by
      change 2370 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
    fastOutputSite.startPC = UInt256.ofNat 0x12aa := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2370) =
    UInt256.ofNat 0x12aa
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x1363 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2419) =
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2419)
  pc_eq := pc_toNat_instructionPC 2419
...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x12aa) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x12aa) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 0x12aa)
...
private theorem runFastOutputReturn
    ...
    Stepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x12aa) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x12aa) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 0x12aa) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 0x12aa, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 0x12aa) rest hstack hrun)
  have hfull_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputTemplate
          {s with pc := UInt256.ofNat 0x12aa, stack := rest} =
        some (FastOutputTrace.afterFastReturn t t.pc rest) := by
  ...
  have hstate :
      FastOutputTrace.afterFastReturn t t.pc rest =
        FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x12aa) rest :=
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 2419 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 2419 := by rfl
  have hlocated :
...
def gasSteps_fastOutput
    ...
    GasSteps
      {s with pc := UInt256.ofNat 0x12aa, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x12aa) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 0x12aa, stack := rest}
