...
@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x14ef := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4204) =
    UInt256.ofNat 0x14ef
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4204)
  pc_eq := pc_toNat_instructionPC 4204
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 0x14ef := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4204) =
    UInt256.ofNat 0x14ef
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 0x14ef := fastOutputReturn_pc
...
private theorem runFastOutputReturn
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 4204 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 4204 := by rfl
  have hlocated :
