...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 3965
    fastOutput_slice
    (by
      change 3965 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4012)
  pc_eq := pc_toNat_instructionPC 4012
...
private theorem runFastOutputReturn
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 4012 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 4012 := by rfl
  have hlocated :
