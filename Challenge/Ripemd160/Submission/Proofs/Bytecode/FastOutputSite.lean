  ...
  · apply advances_append
    · apply advances_append
      · apply advances_append
        ...
        · exact fastPackStep_advances 864
      · exact fastPackStep_advances 896
    · exact fastPackStep_advances 928
  · exact fastPackStep_advances 960
...
private theorem fastOutputBeforeReturn_advances
  ...
  · exact ClosedEndianReuse.advances 8 h8 hrun
  · exact ClosedEndianReuse.advances 16 h16 hrun
  · exact DenseScheduleLift.runInstr_pc_of_advances
      (fastStoreAndSetup_advances instruction hstore) hrun

private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 3981).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 3981
    fastOutput_slice
    (by
      change 3981 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
    fastOutputSite.startPC = UInt256.ofNat 4718 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3981) =
    UInt256.ofNat 4718
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 4793 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4034) =
    UInt256.ofNat 4793
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4034)
  pc_eq := pc_toNat_instructionPC 4034
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 4793 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4034) =
    UInt256.ofNat 4793
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 4793 := fastOutputReturn_pc
...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 4718) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 4718) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 4718)
...
private theorem runFastOutputReturn
    ...
    Stepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 4718) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 4718) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 4718) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 4718, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 4718) rest hstack hrun)
  have hfull_raw :
  ...
  have hstate :
      ...
      FastOutputTrace.fastOutputReturned s (UInt256.ofNat 4718) rest :=
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 4034 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 4034 := by rfl
  have hlocated :
...
def gasSteps_fastOutput
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      ...
      GasSteps
      {s with pc := UInt256.ofNat 4718, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 4718) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 4718, stack := rest}
