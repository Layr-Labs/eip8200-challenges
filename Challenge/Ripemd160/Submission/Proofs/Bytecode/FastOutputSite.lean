...
private theorem fastOutputBeforeReturn_advances
  ...
  · exact CachedEndianMultiply.advances 8 h8 hrun
  · exact CachedEndianMultiply.advances 16 h16 hrun
  · exact DenseScheduleLift.runInstr_pc_of_advances
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 4021
    fastOutput_slice
    (by
      change 4021 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
    fastOutputSite.startPC = UInt256.ofNat 0x13da := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4021) =
    UInt256.ofNat 0x13da
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x141a := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4068) =
    UInt256.ofNat 0x141a
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4068)
  pc_eq := pc_toNat_instructionPC 4068
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 0x141a := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4068) =
    UInt256.ofNat 0x141a
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 0x141a := fastOutputReturn_pc
...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x13da) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x13da) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 0x13da)
...
private theorem runFastOutputReturn
    ...
    Stepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x13da) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x13da) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 0x13da) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 0x13da, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 0x13da) rest hstack hm8 hm16 hrun)
  have hfull_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputTemplate
          {s with pc := UInt256.ofNat 0x13da, stack := rest} =
        some (FastOutputTrace.afterFastReturn t t.pc rest) := by
  ...
  have hstate :
      ...
      FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x13da) rest :=
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 4068 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 4068 := by rfl
  have hlocated :
...
def gasSteps_fastOutput
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      ...
      GasSteps
      {s with pc := UInt256.ofNat 0x13da, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x13da) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 0x13da, stack := rest}
  ...
  have hreturn : GasSteps middle
    ...
    · exact hmiddle_code
    · exact hmiddle_fork
    · simpa [middle] using runFastOutputReturn s rest hstack hm8 hm16 hrun
    · exact hmiddle_run
    · exact hmiddle_np
