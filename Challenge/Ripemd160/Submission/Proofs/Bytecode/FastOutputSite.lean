...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 4028).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 4028
    fastOutput_slice
    (by
      change 4028 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
    fastOutputSite.startPC = UInt256.ofNat 4784 := by
...
@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 4859 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4081) =
    UInt256.ofNat 4859
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4081)
  pc_eq := pc_toNat_instructionPC 4081
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 4859 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4081) =
    UInt256.ofNat 4859
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 4859 := fastOutputReturn_pc
...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 4784) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 4784) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 4784)
...
private theorem runFastOutputReturn
    ...
    Stepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 4784) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 4784) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 4784) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 4784, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 4784) rest hstack hrun)
  have hfull_raw :
  ...
  have hstate :
      ...
      FastOutputTrace.fastOutputReturned s (UInt256.ofNat 4784) rest :=
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 4081 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 4081 := by rfl
  have hlocated :
...
def gasSteps_fastOutput
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      ...
      GasSteps
      {s with pc := UInt256.ofNat 4784, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 4784) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 4784, stack := rest}
