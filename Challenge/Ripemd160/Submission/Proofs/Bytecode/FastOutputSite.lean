...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 4233).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 4233
    fastOutput_slice
    (by
      change 4233 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
  ...
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4233) =
    UInt256.ofNat 0x1406
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x1451 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4286) =
    UInt256.ofNat 0x1451
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4286)
  pc_eq := pc_toNat_instructionPC 4286
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 0x1451 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4286) =
    UInt256.ofNat 0x1451
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 0x1451 := fastOutputReturn_pc
...
private theorem runFastOutputReturn
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 4286 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 4286 := by rfl
  have hlocated :
