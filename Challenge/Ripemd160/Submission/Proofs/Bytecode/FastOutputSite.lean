...
private theorem fastEndianStage8_advances :
  ...
  simp only [FastOutputTemplate.fastEndianStage8,
    ...
    DenseScheduleTemplate.endianFactorPush,
...
private theorem fastEndianStage16_advances :
  ...
  simp only [FastOutputTemplate.fastEndianStage16,
    ...
    DenseScheduleTemplate.endianFactorPush, if_neg (by decide : (16 : Nat) ≠ 8),
    List.mem_cons, List.not_mem_nil, or_false] at hmem
  rcases hmem with rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl
  all_goals first
    | exact advances_mul
...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 3242).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 3242
    fastOutput_slice
    (by
      change 3242 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
    fastOutputSite.startPC = UInt256.ofNat 0x1256 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3242) =
    UInt256.ofNat 0x1256
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x12d6 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3289) =
    UInt256.ofNat 0x12d6
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3289)
  pc_eq := pc_toNat_instructionPC 3289
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 0x12d6 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3289) =
    UInt256.ofNat 0x12d6
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 0x12d6 := fastOutputReturn_pc
...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x1256) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x1256) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 0x1256)
...
private theorem runFastOutputReturn
    ...
    Stepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x1256) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1256) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 0x1256) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 0x1256, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 0x1256) rest hstack hrun)
  have hfull_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputTemplate
          {s with pc := UInt256.ofNat 0x1256, stack := rest} =
        some (FastOutputTrace.afterFastReturn t t.pc rest) := by
  ...
  have hstate :
      ...
      FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1256) rest :=
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 3289 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 3289 := by rfl
  have hlocated :
...
def gasSteps_fastOutput
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      ...
      GasSteps
      {s with pc := UInt256.ofNat 0x1256, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1256) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 0x1256, stack := rest}
