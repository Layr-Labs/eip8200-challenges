...
private theorem fastEndianStage8_advances :
  ...
  simp only [FastOutputTemplate.fastEndianStage8, DenseScheduleTemplate.endianStage8,
    DenseScheduleTemplate.endianStage, DenseScheduleTemplate.endianFactorPush, List.mem_cons,
    List.not_mem_nil, or_false] at hmem
...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 2848).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 2848
    fastOutput_slice
    (by
      change 2848 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
    fastOutputSite.startPC = UInt256.ofNat 0x11fb := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2848) =
    UInt256.ofNat 0x11fb
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x127b := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2895) =
    UInt256.ofNat 0x127b
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2895)
  pc_eq := pc_toNat_instructionPC 2895
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 0x127b := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2895) =
    UInt256.ofNat 0x127b
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 0x127b := fastOutputReturn_pc
...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x11fb) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x11fb) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 0x11fb)
...
private theorem runFastOutputReturn
    ...
    Stepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x11fb) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x11fb) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 0x11fb) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 0x11fb, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 0x11fb) rest hstack hrun)
  have hfull_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputTemplate
          {s with pc := UInt256.ofNat 0x11fb, stack := rest} =
        some (FastOutputTrace.afterFastReturn t t.pc rest) := by
  ...
  have hstate :
      ...
      FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x11fb) rest :=
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 2895 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 2895 := by rfl
  have hlocated :
...
def gasSteps_fastOutput
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      ...
      GasSteps
      {s with pc := UInt256.ofNat 0x11fb, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x11fb) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 0x11fb, stack := rest}
