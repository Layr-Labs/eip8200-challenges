...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 3333).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 3333
    fastOutput_slice
    (by
      change 3333 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
    fastOutputSite.startPC = UInt256.ofNat 0x12fa := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3333) =
    UInt256.ofNat 0x12fa
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x1345 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3386) =
    UInt256.ofNat 0x1345
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 0x1345 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3386) =
    UInt256.ofNat 0x1345
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 0x1345 := fastOutputReturn_pc
...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x12fa) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x12fa) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 0x12fa)
...
private theorem runFastOutputReturn
    ...
    Stepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x12fa) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x12fa) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 0x12fa) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 0x12fa, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 0x12fa) rest hstack hrun)
  have hfull_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputTemplate
          {s with pc := UInt256.ofNat 0x12fa, stack := rest} =
        some (FastOutputTrace.afterFastReturn t t.pc rest) := by
  ...
  have hstate :
      ...
      FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x12fa) rest :=
...
def gasSteps_fastOutput
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      ...
      GasSteps
      {s with pc := UInt256.ofNat 0x12fa, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x12fa) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 0x12fa, stack := rest}
