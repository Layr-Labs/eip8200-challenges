...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 3873).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 3873
    fastOutput_slice
    (by
      change 3873 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
  ...
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3873) =
    UInt256.ofNat 4690
...
@[simp] theorem fastOutputSite_endPC :
  ...
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3926) =
    UInt256.ofNat 4765
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3926)
  pc_eq := pc_toNat_instructionPC 3926
...
private theorem runFastOutputReturn
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 3926 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 3926 := by rfl
  have hlocated :
