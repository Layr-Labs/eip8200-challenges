...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 4026).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 4026
    fastOutput_slice
    (by
      change 4026 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
    fastOutputSite.startPC = UInt256.ofNat 0x1325 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4026) =
    UInt256.ofNat 0x1325
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x1370 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4079) =
    UInt256.ofNat 0x1370
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4079)
  pc_eq := pc_toNat_instructionPC 4079
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 0x1370 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4079) =
    UInt256.ofNat 0x1370
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 0x1370 := fastOutputReturn_pc
...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x1325) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x1325) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 0x1325)
...
private theorem runFastOutputReturn
    ...
    Stepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x1325) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1325) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 0x1325) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 0x1325, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 0x1325) rest hstack hrun)
  have hfull_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputTemplate
          {s with pc := UInt256.ofNat 0x1325, stack := rest} =
        some (FastOutputTrace.afterFastReturn t t.pc rest) := by
  ...
  have hstate :
      ...
      FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1325) rest :=
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 4079 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 4079 := by rfl
  have hlocated :
...
def gasSteps_fastOutput
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      ...
      GasSteps
      {s with pc := UInt256.ofNat 0x1325, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1325) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 0x1325, stack := rest}
