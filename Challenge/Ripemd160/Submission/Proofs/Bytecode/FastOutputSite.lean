...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 3992).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 3992
    fastOutput_slice
    (by
      change 3992 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
  ...
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 3992) =
    UInt256.ofNat 4715
...
@[simp] theorem fastOutputSite_endPC :
  ...
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4045) =
    UInt256.ofNat 4790
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 4045)
  pc_eq := pc_toNat_instructionPC 4045
...
private theorem runFastOutputReturn
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 4045 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 4045 := by rfl
  have hlocated :
