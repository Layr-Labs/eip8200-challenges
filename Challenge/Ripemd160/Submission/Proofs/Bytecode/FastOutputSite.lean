...
private theorem fastOutput_slice :
    (Artifact.submissionArtifact.instructions.drop 2814).take
...
def fastOutputSite :
  ...
  StackSiteBuilder.ofSlice
    ...
    FastOutputTemplate.fastOutputBeforeReturnTemplate 2814
    fastOutput_slice
    (by
      change 2814 + FastOutputTemplate.fastOutputBeforeReturnTemplate.length ≤
...
@[simp] theorem fastOutputSite_startPC :
  ...
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2814) =
    UInt256.ofNat 0x13f8
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide

@[simp] theorem fastOutputSite_endPC :
    fastOutputSite.endPC = UInt256.ofNat 0x14b1 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2863) =
    UInt256.ofNat 0x14b1
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
def fastOutputReturn : LocatedSite Artifact.submissionArtifact .Osaka where
  ...
  pc := UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2863)
  pc_eq := pc_toNat_instructionPC 2863
...
@[simp] theorem fastOutputReturn_pc :
    fastOutputReturn.pc = UInt256.ofNat 0x14b1 := by
  change UInt256.ofNat (Artifact.submissionArtifact.instructionPC 2863) =
    UInt256.ofNat 0x14b1
  rw [ArtifactByteLength.instructionPC_eq_byteLength]
  decide
...
theorem fastOutputReturn_site_end :
  ...
  calc
    fastOutputReturn.pc = UInt256.ofNat 0x14b1 := fastOutputReturn_pc
...
private theorem runFastOutputReturn
  ...
  have hrun_instr :
    ...
    | some next =>
        ...
        subst next
        rfl
  have hpc_nat : t.pc.toNat = Artifact.submissionArtifact.instructionPC 2863 := by
    calc
      ...
      _ = Artifact.submissionArtifact.instructionPC 2863 := by rfl
  have hlocated :
