...
private theorem fastOutputReturn_pc_eq_state
    ...
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x1332) rest).pc =
      fastOutputReturn.pc := by
  calc
    (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x1332) rest).pc =
        StackRoundTrace.pcAfter (UInt256.ofNat 0x1332)
...
private theorem runFastOutputReturn
    ...
    Stepper.runLocatedBlock fastOutputReturnPath
      (FastOutputTrace.fastOutputBeforeReturnState s (UInt256.ofNat 0x1332) rest) =
      some (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1332) rest) := by
  let t := FastOutputTrace.fastOutputBeforeReturnState s
    (UInt256.ofNat 0x1332) rest
  ...
  have hpre_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputBeforeReturnTemplate
          {s with pc := UInt256.ofNat 0x1332, stack := rest} = some t := by
    simpa [t] using
      (FastOutputTrace.runInstrSeq_fastOutput_beforeReturn s
        (UInt256.ofNat 0x1332) rest hstack hrun)
  have hfull_raw :
      StackRoundTrace.runInstrSeq FastOutputTemplate.fastOutputTemplate
          {s with pc := UInt256.ofNat 0x1332, stack := rest} =
        some (FastOutputTrace.afterFastReturn t t.pc rest) := by
  ...
  have hstate :
      ...
      FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1332) rest :=
...
def gasSteps_fastOutput
    ...
    (hnp : Precompile.isPrecompileWithConfig s.executionEnv.precompileConfig
      ...
      GasSteps
      {s with pc := UInt256.ofNat 0x1332, stack := rest}
      (FastOutputTrace.fastOutputReturned s (UInt256.ofNat 0x1332) rest) := by
  let entry : State := {s with pc := UInt256.ofNat 0x1332, stack := rest}
