...
sixteen iterations therefore end at word `max current (40 + 2 * block)`.
-/
...
theorem scheduledState_activeWords_ge_40 (s : State) (input : ByteArray)
    (hfit : CalldataFits input) (block : Nat)
    (hblock : block < DriverTrace.blockCount input)
    (returnDest : UInt256) (rest : List UInt256) :
    40 ≤ (Schedule.loopState s (DriverTrace.messageOffsetWord block)
