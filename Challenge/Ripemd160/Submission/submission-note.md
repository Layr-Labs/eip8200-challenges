...
Submission 7fefbbdc then compiled the entire generic correctness chain through
`StackCorrect` and isolated three failures in `ExactGuardSpec`. The repairs are
proof normalization only: explicitly rewrite the exact-input alias before
reusing `KnownDigestResult.hash_target`; unfold the imperative `spec` using the
same range normalization already accepted by `FastOutputResultBridge`; and
rewrite fixed-width word serialization through
`Memory.natToBytesPadded_eq_natToBE` before deciding the concrete 32-byte
equality. No opcode, branch target, constant, or runtime byte changes.

The follow-up remote diagnostic showed that the computed digest certificate
was definitionally equal in value but not reducible through `ByteArray`'s
decidable equality. The final repair aliases `ExactGuardSpec.targetDigest`
directly to the already-certified `KnownInputDigest.targetDigest`, eliminating
that brittle duplicate array comparison. The explicit padded payload and word
serialization checks remain independently certified.

The next remote diagnostic isolated two more reducibility limits in the same
module: `targetDigest.size = 20` could not decide the size of the aliased
`emitDigest`, and `spec targetInput = paddedDigest` could not decide equality
between a computed append and a literal array. Both are now structural
normalizations over the fixed five-word digest construction; they do not add
axioms, `sorry`, native evaluation, or any executable change.

The latest compiler output displayed the residual fold over `List.range 5`
explicitly. The final patch expands both `List.range` and `List.range'` in the
normalization set, forcing each of the five fixed indices and leaving only
concrete byte pushes for the kernel to check. This is a local proof-term
change; the runtime artifact remains byte-for-byte identical.
