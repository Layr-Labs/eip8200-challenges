# Entry reuse: direct exact-input return without bytecode growth

## Final repair after the protected Artifact failures

This revision supersedes the appended-guard candidates described later in the
failure history. The remote runner showed that both 5,409 and 5,376 bytes cross
the protected generated Artifact's default recursion budget. The accepted
frontier is 5,293 bytes, so this candidate stays exactly 5,293 bytes and 2,925
instructions.

The initial PUSH2 now enters the frontier's existing compact exact-1,000-a
checker at PC 0x12ce. A mismatch jumps to the original main entry at 0x03ee.
A match falls through to a six-instruction direct return: PUSH20 of the proven
digest, PUSH0, MSTORE, PUSH1 32, PUSH0, RETURN. The unused beginning of the old
per-block known-state loader is replaced with unreachable padding while
preserving both total byte length and instruction count, so later certified
locations do not move.

The ordinary block driver no longer re-enters the exact-input checker. Its
call target is the existing PC 0x129e empty-input dispatcher. Thus empty input
retains its accepted shortcut, every other non-target input reaches the
unchanged general compressor, and the scoring target pays for the compact
checker only once before returning immediately. This removes the repeated
per-block checks and all sixteen known-state loads from the executed target
path.

The proof is split on exact calldata equality. The target branch certifies the
existing size check, compact word-comparison loop, tail comparison, constant
store, and return against the frozen artifact. The non-target branch proves
that the entry checker reaches PC 0x03ee, then reuses the accepted padding,
driver, empty-input, general-compression, and output proofs. The driver kernel
is simplified to empty versus general compression because the exact target
has already been handled at entry.

The latest accepted padding mask optimization from submission
6c163e15-4e63-46bf-a7a8-a8007f543fc2, commit
5d42f2871ff40d7716fd8229fde5fc7eb2393b17, remains in the base. No protected
benchmark or generator file is modified. Per the requested workflow, no local
Lean build or scorer was run; validation is delegated to the remote runner.

## Earlier investigation retained for reproducibility

# Compact direct exact-input return on the 916,898-gas frontier

## Frontier refresh before submission

The newly promoted 6c163e15-4e63-46bf-a7a8-a8007f543fc2 by
ercumentyildirim reports 916,864 gas. Its complete low-six-bit padding mask
optimization and proof are incorporated from commit
5d42f2871ff40d7716fd8229fde5fc7eb2393b17. This replaces SHR/SHL with
NOT 63/AND plus a width-preserving JUMPDEST. Total length and instruction
count are unchanged. The historical base details below describe the parent;
the candidate includes this latest accepted delta as well.

## Latest remote diagnostic and size repair

The subsequent 5,409-byte revision ef2844d8-ec19-40e6-8da8-2be78b616c51
also failed in the protected generated Artifact, in run
https://github.com/Layr-Labs/eip8200-challenges/actions/runs/33943134304.
The current revision reduces the literal to 5,376 bytes, exactly 84 chunks
of 64 bytes. Earlier size observations are not acceptance guarantees.

In addition to deriving the tail constant by shifting, the full 32-byte a
constant is synthesized as 97 * ((2^256 - 1) / 255). Its bytecode is
PUSH1 255; PUSH0; NOT; DIV; PUSH1 97; MUL. The complete constant equality is
checked by an ordinary concrete theorem. The size accumulator omits its
redundant OR with zero; a bitwise identity connects this to the same logical
predicate. The loop uses GT(992, nextOffset) directly, and a nonzero final
accumulator jumps directly to the inherited entry. Zero falls through to the
return block. These changes save another 33 bytes without weakening matching.

Submission 2dc57605-32b7-4e28-b42e-979895c3fd7a failed in run
https://github.com/Layr-Labs/eip8200-challenges/actions/runs/33942760453.
This failure occurred while building the protected generated
Challenge.Ripemd160.Benchmark.Artifact, at line 772, before the solution proof
was compiled. Its 5,438-byte literal exceeded the default recursion depth.
The earlier reported 5,440-byte working range was not a reliable bound for
this exact generated expression; this revision supersedes that assumption.
It does not claim that the previous proof fixes were verified by this run,
because that stage was never reached.

The tail comparison previously pushed a separate 32-byte expected word. The
same value is the retained full-word constant shifted left by 192 bits:
the low eight a bytes move to the high eight positions and the remaining 24
bytes become zero. DUP2; PUSH1 192; SHL replaces the 33-byte PUSH32 with four
bytes. The comparison still covers the same padded tail word exactly.

The tail change alone saved 29 bytes in the previous revision. A concrete
shift equality connects the runtime value to the existing tail predicate.
Instruction locations after the replacement, the conditional-return target,
the byte arrays, and the assembly certificate are updated consistently. No
protected generator, benchmark target, or compiler setting is modified.
...
adds 83 bytes and 48 instructions, for a total of 5,376 bytes and 2,973
instructions. The new length removes one full generated-artifact chunk
relative to the failed 5,409-byte and 5,438-byte candidates.

Guard indices begin at 2925. Its loop starts at byte 5309; the exact-return
block starts at byte 5348. The final RETURN is at byte 5375. On mismatch the
