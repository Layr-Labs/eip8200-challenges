...
namespace Challenge.Ripemd160.Submission.H39Reference.Proofs.Bytecode.Trace
...
theorem pcBound (index : Nat) : Artifact.instructionPC index < 2 ^ 256 := by
  ...
  have hsize := referenceBytecode_size_bound
  omega
...
theorem decodedOpAt (s : State) (index : Nat) (op : Operation)
  ...
  apply Challenge.EvmProof.ProgramArtifact.state_decodedOp_of
    ...
    (by
      ...
      exact hcode) (pcToNat hpc) op none
  · exact Artifact.referenceArtifact.decodeAt_op_index index op hget hopcode hplain
  · exact havailable
...
theorem decodedPushAt (s : State) (index : Nat) (width : Fin 33)
  ...
  apply Challenge.EvmProof.ProgramArtifact.state_decoded_of
    ...
    (by
      change s.executionEnv.code =
        Challenge.Ripemd160.Submission.H39Memo.h39Bytecode
      exact hcode) (pcToNat hpc)
    (.Push ⟨width⟩) (some (value, width.val))
  · exact Artifact.referenceArtifact.decodeAt_push_index index width value hget hfit
  · exact havailable
...
theorem validJumpDestAt (index : Nat)
  ...
  rw [Challenge.EvmProof.Word.word_toNat_ofNat,
    Nat.mod_eq_of_lt (pcBound index)]
  exact Artifact.referenceArtifact.isValidJumpDest_index index hget
...
end Challenge.Ripemd160.Submission.H39Reference.Proofs.Bytecode.Trace
