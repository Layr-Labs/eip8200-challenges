import Challenge.Ripemd160.Submission.H39Reference.Proofs.Bytecode.DirectCorrect
...
namespace Challenge.Ripemd160.Submission.H39Reference.Proofs.Bytecode.OutputResultBridge
...
theorem final_isDone (input : ByteArray) (seam : CompressionSeam input) :
    (outputResult (seam.states (DriverTrace.blockCount input)) input).isDone = true := by
  simp [outputResult, State.isDone, State.isHalted, State.isRunning,
    seam.callStack (DriverTrace.blockCount input) (by omega)]

theorem final_result (input : ByteArray) (seam : CompressionSeam input) :
    (outputResult (seam.states (DriverTrace.blockCount input)) input).toResult =
      .returned (spec input) := by
  rw [State.toResult_returned _ (by rfl)]
  congr 1
  change MachineState.readPadded
    (outputLoopState (seam.states (DriverTrace.blockCount input)) input 5).memory
    0 32 = spec input
  rw [readOutput_eq, outputBytes_eq_spec input seam]

end Challenge.Ripemd160.Submission.H39Reference.Proofs.Bytecode.OutputResultBridge
