...
namespace Challenge.Ripemd160.Submission.H39Reference.Proofs.Bytecode.Main
...
private theorem valueFits (w : Artifact.InitStore)
  ...
  simp only [Challenge.Ripemd160.Submission.H39Memo.Artifact.initStores,
  ...
  all_goals
    simp (config := { maxSteps := 200000 })
      [locatedInitStore, applyInitStore, hpc, hstack, hrun,
        ...
        Challenge.Ripemd160.Submission.H39Memo.Artifact.instructionPC,
...
def gasSteps_bodyInitialization (input : ByteArray) :
  ...
  have body := gasSteps_initStores (Execution.mainStart input)
    ...
    (by norm_num [InitChain,
      Challenge.Ripemd160.Submission.H39Memo.Artifact.initStores])
    (by
      intro w hw
      simp only [Challenge.Ripemd160.Submission.H39Memo.Artifact.initStores,
        List.head?_cons, Option.some.injEq] at hw
      subst w
      rfl)
...
def gasSteps_initialization (input : ByteArray) :
    Challenge.EvmProof.GasSteps (Execution.mainStart input)
      (initializedState input) :=
  gasSteps_bodyInitialization input

end Challenge.Ripemd160.Submission.H39Reference.Proofs.Bytecode.Main
