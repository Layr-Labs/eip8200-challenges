...
- Size: 5,215 bytes; 3,680 instructions; literal-encoding cost 8,181 (ceiling 8,194).
- Base: promoted `c19d26e2` (661,192 gas). Local protected scorer: 661,150 gas (−42), 49/49.

## The change

The terminal paired round (i = 76) no longer masks its message word (`DUP5 AND` deleted, 63
runs over the corpus). Instead the schedule builder ends with `PUSH0 PUSH2 0x262 MSTORE8`
(42 runs), clearing byte 610 — byte 16 of table slot 594 — so the unmasked word has bytes
14..16 zero and a lower half below 2^120. The round sum `a + f + X + K` still cannot carry into
the right lane: `f`'s only junk is in bits 122..143 with bit 121 clear, and `a + X + K < 2^121`.

## What it costs the proof

- Mechanical relocation (pcs ≥ 858: +5, ≥ 4463: +3; indices ≥ 473: +3, ≥ 3364: +1) by a tool
  that learns each literal's class (pc / index / constant) from the previous promotion's
  relocation; byte certificates regenerated with the old chunk partition.
- `PoolShapeV2.clearTerminal` in `resultMemoryV2`; `PoolCertificatesV2.terminal_gap_sources`;
  `PoolFacts.result_terminal_bound`; `Pair13NormalTrace.clearTemplate`/`run_clear` appended to
  both builder templates.
- `StaggerAlgorithm.MessageWord`: `(i ≠ 76 ∨ message mod 2^144 < 2^120) ∧ Safe`;
  `StaggerTerminal75(Math)`: the no-carry bound relaxed to 2^121 and `final_shape` generalised
  to any admissible terminal word; terminal round templates without the mask.
