...
checkpoint. It preserves the H30b prefix except for the driver's PUSH2 immediate
and uses a fixed-width literal in H31b's 16-bit output stage; three JUMPDESTs
retain the original 13-instruction slot. The existing B01 transform is not reapplied.
...
set_option maxRecDepth 50000 in
@[simp] theorem referenceBytecode_size : submissionBytecode.size = 5395 := by
