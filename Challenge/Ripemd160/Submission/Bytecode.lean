...
`submissionBytecode` is the exact `rotladder` artifact (5,254 bytes).  It is the
promoted `frontier` artifact with (A) a MUL-scale dual rotation at the 80 round
sites, (B) an O(1) depth-2 patterned-prefix ladder in the trailing region, and
(C) a literal first-prefix word store replacing the CODECOPY setup.
