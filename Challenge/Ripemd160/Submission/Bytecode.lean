...
`submissionBytecode` is the exact `rotladder` artifact (5,389 bytes).  It is the
promoted `frontier` artifact with (A) a MUL-scale dual rotation at the 80 round
sites and (B) an O(1) depth-4 patterned-prefix ladder in the trailing region.
...
set_option maxRecDepth 50000 in
@[simp] theorem referenceBytecode_size : submissionBytecode.size = 5389 := by
